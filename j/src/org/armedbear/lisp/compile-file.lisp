...
;;; $Id: compile-file.lisp,v 1.22 2004-06-13 17:48:39 asimon Exp $
;;;
...
(defun process-toplevel-form (form stream compile-time-too &optional input-file position record-source-location)
  (let ((source-loc))
    (cond ((atom form)
	   ...
	   (case (car form)
		...
		(return-from process-toplevel-form)))
	     ...
	     ((DEFVAR DEFPARAMETER)
		  ...
		  (when record-source-location
		    (setf source-loc `(system::new-source ',name :kind ',(car form) :path ,input-file :position ,position)))
                  (%defvar name))))
	     (DEFUN
	       (let ((name (second form)))
		 ...
		 (let* ((lambda-list (third form))
		   ...
		   (if compiled-function
			 ...
			 (eval form))))
		   (when record-source-location
		     (setf source-loc `(system::new-source ',name :kind 'defun :path ,input-file :position ,position)))
		 ...
		 (let* ((expr (function-lambda-expression (macro-function name)))
		   ...
		   (if (and classfile
			 ...
			 (when record-source-location
			   (setf source-loc 
			     `(system::new-source ',name :kind 'defmacro :path ,input-file :position ,position)))
			(setf form
	...
	(if (eq (car form) 'IMPORT)
      ...
      (when source-loc
	(let ((*package* (find-package "COMMON-LISP")))
	  (write source-loc :stream stream)
	  (terpri stream))))))
...
(defun compile-file (input-file &key output-file verbose print external-format (record-source-location t #+nil *record-source-location*))
      ...
      (let ((*compile-file-pathname* (pathname in))
        ...
        (with-open-file (out temp-file :direction :output :if-exists :supersede)
          (let ((*readtable* *readtable*)
            ...
            (terpri out)
	    (when record-source-location
	      (write '(setq system::*symbols-defined-in-this-file* nil) :stream out)
	      (terpri out))
            (loop
	     (let ((position (file-position in))
	       ...
	       (when (eq form in)
                  (return))
                (process-toplevel-form form out nil namestring #+nil input-file position record-source-location)))
