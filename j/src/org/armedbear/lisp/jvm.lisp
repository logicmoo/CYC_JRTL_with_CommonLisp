...
(let ((cl-symbol-names (make-hash-table :test 'eq :size 1024)))
  (defun initialize-symbol-names ()
    (dolist (entry '((&ALLOW-OTHER-KEYS "AND_ALLOW_OTHER_KEYS")
                     (&AUX "AND_AUX")
                     (&BODY "AND_BODY")
                     (&ENVIRONMENT "AND_ENVIRONMENT")
                     (&KEY "AND_KEY")
                     (&OPTIONAL "AND_OPTIONAL")
                     (&REST "AND_REST")
                     (&WHOLE "AND_WHOLE")
                     (* "STAR")
                     (** "STAR_STAR")
                     (*** "STAR_STAR_STAR")
                     (*BREAK-ON-SIGNALS* "BREAK_ON_SIGNALS")
                     (*COMPILE-FILE-PATHNAME* "_COMPILE_FILE_PATHNAME_")
                     (*COMPILE-FILE-TRUENAME* "COMPILE_FILE_TRUENAME")
                     (*COMPILE-PRINT* "COMPILE_PRINT")
                     (*COMPILE-VERBOSE* "COMPILE_VERBOSE")
                     (*DEBUG-IO* "DEBUG_IO")
                     (*DEBUGGER-HOOK* "DEBUGGER_HOOK")
                     (*DEFAULT-PATHNAME-DEFAULTS* "DEFAULT_PATHNAME_DEFAULTS")
                     (*ERROR-OUTPUT* "ERROR_OUTPUT")
                     (*FEATURES* "FEATURES")
                     (*GENSYM-COUNTER* "GENSYM_COUNTER")
                     (*LOAD-PATHNAME* "LOAD_PATHNAME")
                     (*LOAD-PRINT* "LOAD_PRINT")
                     (*LOAD-TRUENAME* "LOAD_TRUENAME")
                     ...
                     (*PRINT-CASE* "PRINT_CASE")
                     (*PRINT-CIRCLE* "PRINT_CIRCLE")
                     (*PRINT-ESCAPE* "PRINT_ESCAPE")
                     (*PRINT-GENSYM* "PRINT_GENSYM")
                     (*PRINT-LENGTH* "PRINT_LENGTH")
                     (*PRINT-LEVEL* "PRINT_LEVEL")
                     (*PRINT-LINES* "PRINT_LINES")
                     (*PRINT-MISER-WIDTH* "PRINT_MISER_WIDTH")
                     (*PRINT-PPRINT-DISPATCH* "PRINT_PPRINT_DISPATCH")
                     (*PRINT-PRETTY* "PRINT_PRETTY")
                     (*PRINT-RADIX* "PRINT_RADIX")
                     (*PRINT-READABLY* "PRINT_READABLY")
                     (*PRINT-RIGHT-MARGIN* "PRINT_RIGHT_MARGIN")
                     (*QUERY-IO* "QUERY_IO")
                     (*RANDOM-STATE* "_RANDOM_STATE_")
                     (*READ-BASE* "READ_BASE")
                     (*READ-DEFAULT-FLOAT-FORMAT* "READ_DEFAULT_FLOAT_FORMAT")
                     (*READ-EVAL* "READ_EVAL")
                     (*READ-SUPPRESS* "READ_SUPPRESS")
                     (*READTABLE* "_READTABLE_")
                     (*STANDARD-INPUT* "STANDARD_INPUT")
                     (*STANDARD-OUTPUT* "STANDARD_OUTPUT")
                     (*TERMINAL-IO* "TERMINAL_IO")
                     ...
                     (ARRAY-DIMENSION-LIMIT "ARRAY_DIMENSION_LIMIT")
                     ...
                     (ARRAY-RANK-LIMIT "ARRAY_RANK_LIMIT")
                     (ARRAY-ROW-MAJOR-INDEX "ARRAY_ROW_MAJOR_INDEX")
                     (ARRAY-TOTAL-SIZE "ARRAY_TOTAL_SIZE")
                     (ARRAY-TOTAL-SIZE-LIMIT "ARRAY_TOTAL_SIZE_LIMIT")
                     (ARRAYP "ARRAYP")
                     (ASH "ASH")
                     (ASIN "ASIN")
                     (ASINH "ASINH")
                     ...
                     (CODE-CHAR "CODE_CHAR")
                     (COERCE "COERCE")
                     (COMPILATION-SPEED "COMPILATION_SPEED")
                     (COMPILE "COMPILE")
