...
(defun declare-local-function (local-function)
  (let* ((g (symbol-name (gensym)))
         ...
         (*code* *static-code*))
    (declare-field g +lisp-object+)
    (emit 'ldc (pool-string (file-namestring classfile)))
    ...
    (cond
     ...
     (t
      ...
      (unless (and (single-valued-p arg1)
        ...
        (emit-clear-values))
      (emit-invokevirtual +lisp-object-class+ "EQL" (list +lisp-object+) +lisp-object+)
      (emit-move-from-stack target)))))
  ...
  (case (length form)
    ...
    (t
     ...
     (let* ((*register* *register*)
       ...
       (emit 'aload function-register)
       (emit 'aload values-register)
       (emit-invokevirtual +lisp-object-class+ "execute"
...
;;             (aver (null (compiland-parent compiland))) ;; FIXME
    ...
    (write-class-file args execute-method classfile)
