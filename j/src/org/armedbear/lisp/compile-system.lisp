...
;;; $Id: compile-system.lisp,v 1.17 2004-06-14 19:12:57 asimon Exp $
;;;
...
(defun compile-system ()
  ...
  (time
   (let ((*default-pathname-defaults* (pathname *lisp-home*)))
     (load (compile-file "precompiler.lisp" :record-source-file-info nil))
     (load (compile-file "opcodes.lisp" :record-source-file-info nil))
     (load (compile-file "jvm.lisp" :record-source-file-info nil))
     ;; Order matters for these files.
     (mapc #'(lambda (fn) (compile-file fn :record-source-file-info nil))
           '("collect.lisp"
             "macros.lisp"
             "loop.lisp"))
     ;; But not for these.
     (mapc #'(lambda (file-spec)
              (destructuring-bind (fn record)
                                  (if (consp file-spec)
                                      file-spec
                                      (list file-spec *record-source-file-info*))
                                  (compile-file fn :record-source-file-info record)))
