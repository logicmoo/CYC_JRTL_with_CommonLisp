...
public final class collaborative_sense_making_utilities extends SubLTranslatedFile implements V02 {
    ...
    public static final SubLObject get_date_string_for_run() {
	{
		...
		thread.resetMultipleValues();
		return cconcatenate(format_nil.format_nil_a_no_copy(year), new SubLObject[] { $str_alt13$_, format_nil.format_nil_a_no_copy(month), $str_alt13$_, format_nil.format_nil_a_no_copy(day), $str_alt13$_, format_nil.format_nil_a_no_copy(hour), $str_alt13$_,
			format_nil.format_nil_a_no_copy(minute), $str_alt13$_, format_nil.format_nil_a_no_copy(second) });
	    }
	}
    }
			    ...
			    try {
				{
				    ...
				    for (i = ZERO_INTEGER; !i.numGE(sme_count); i = number_utilities.f_1X(i)) {
					{
					    ...
					    for (j = number_utilities.f_1X(i); !j.numGE(sme_count); j = number_utilities.f_1X(j)) {
						{
						    ...
						    SubLObject outfile = cconcatenate(format_nil.format_nil_a_no_copy(directory_name),
							    new SubLObject[] { format_nil.format_nil_a_no_copy(sme1_name), $str_alt33$_and_, format_nil.format_nil_a_no_copy(sme2_name), $str_alt34$_missed_opportunities_, format_nil.format_nil_a_no_copy(try_no), $str_alt35$_lisp });
			...
			if (NIL == current) {
				...
				return list(CLET, list(stream),
					list(PCOND, list(listS(EQ, destination, $list_alt53), list(CSETQ, stream, destination)), list(list(STREAMP, destination), listS(CSETQ, stream, $list_alt56)),
						list(list(STRINGP, destination),
							list(CLET, list(error), list(CATCH_ERROR_MESSAGE, list(error), list(CSETQ, stream, list(OPEN_TEXT, destination, direction))), list(PWHEN, error, list(WARN, $str_alt61$__Encountered_the_following_error, destination, error)))),
			...
			try {
				...
				SubLObject message = (NIL != list_of_col_opps)
					? ((SubLObject) (cconcatenate(format_nil.$format_nil_percent$.getGlobalValue(),
						new SubLObject[] { $str_alt71$The_following_collaboration_oppor, format_nil.format_nil_a_no_copy(numeric_date_utilities.get_universal_date(reference_time, UNPROVIDED)), $str_alt72$_at_,
							format_nil.format_nil_a_no_copy(numeric_date_utilities.get_universal_second(reference_time)), $str_alt73$_, format_nil.$format_nil_percent$.getGlobalValue(), format_nil.format_nil_a_no_copy(list_of_col_opps),
							format_nil.$format_nil_percent$.getGlobalValue() })))
					: cconcatenate(format_nil.$format_nil_percent$.getGlobalValue(), new SubLObject[] { $str_alt74$No_collaboration_opportunities_ar, format_nil.format_nil_a_no_copy(numeric_date_utilities.get_universal_date(reference_time, UNPROVIDED)), $str_alt72$_at_,
						format_nil.format_nil_a_no_copy(numeric_date_utilities.get_universal_second(reference_time)), $str_alt75$_, format_nil.$format_nil_percent$.getGlobalValue() });
				csm_note(message);
			    }
			} catch (Throwable catch_var) {
			    Errors.handleThrowable(catch_var, NIL);
	...
	return makeBoolean((((((NIL == forts.fort_p(v_term)) || (NIL != cr_search_tool.cr_irrelevant_term(v_term))) || (NIL != forts.invalid_fortP(v_term))) || (NIL != term_too_general_for_conceptual_relatednessP(v_term))) || (NIL != term_name_starts_with_an_irrelevant_prefixP(v_term)))
		|| (NIL != csm_irrelevant_natP(v_term)));
    }
	...
	private static final SubLStructDeclNative structDecl = makeStructDeclNative(collaborative_sense_making_utilities.$csm_assertion_info_entry_native.class, CSM_ASSERTION_INFO_ENTRY, CSM_ASSERTION_INFO_ENTRY_P, $list_alt137, $list_alt138,
		new String[] { "$owner", "$date", "$time", "$universal_time", "$sentence", "$mt", "$forts" }, $list_alt139, $list_alt140, PRINT_CSM_ASSERTION_INFO_ENTRY);
    }
	...
	return v_object.getClass() == collaborative_sense_making_utilities.$csm_assertion_info_entry_native.class ? ((SubLObject) (T)) : NIL;
    }
	...
	format(stream, $str_alt166$__CSM_ASSERTION_INFO__s__s__s__s_, new SubLObject[] { csm_assertion_info_entry_owner(v_object), csm_assertion_info_entry_date(v_object), csm_assertion_info_entry_time(v_object), csm_assertion_info_entry_universal_time(v_object),
		csm_assertion_info_entry_sentence(v_object), csm_assertion_info_entry_mt(v_object), csm_assertion_info_entry_forts(v_object) });
	return NIL;
    }
	...
	private static final SubLStructDeclNative structDecl = makeStructDeclNative(collaborative_sense_making_utilities.$csm_topic_justification_native.class, CSM_TOPIC_JUSTIFICATION, CSM_TOPIC_JUSTIFICATION_P, $list_alt169, $list_alt170, new String[] { "$sentence", "$mt", "$universal_time" },
		$list_alt171, $list_alt172, PRINT_CSM_TOPIC_JUSTIFICATION);
    }
	...
	return v_object.getClass() == collaborative_sense_making_utilities.$csm_topic_justification_native.class ? ((SubLObject) (T)) : NIL;
    }
	...
	return makeBoolean(((NIL != assertion_handles.valid_assertionP(assertion, UNPROVIDED)) && (NIL != fi.assertion_hl_formula(assertion, UNPROVIDED)))
		&& (NIL == list_utilities.member_eqP(cycl_utilities.formula_operator(fi.assertion_hl_formula(assertion, UNPROVIDED)), $csm_implementational_constants$.getGlobalValue())));
    }
	...
	{
	    final SubLThread thread = SubLProcess.currentSubLThread();
	    csm_note(cconcatenate($str_alt253$Identifying_new_topics_for_users_,
		    new SubLObject[] { format_nil.format_nil_a_no_copy(numeric_date_utilities.get_universal_date(reference_time, UNPROVIDED)), $str_alt72$_at_, format_nil.format_nil_a_no_copy(numeric_date_utilities.get_universal_second(reference_time)), $str_alt119$___ }));
	    {
	...
	}
	{
		...
		no_of_ops = operation_communication.load_transcript_file(filename, NIL, queue, UNPROVIDED, UNPROVIDED);
		...
		thread.resetMultipleValues();
		{
		    ...
		    thread.resetMultipleValues();
		    csm_note(cconcatenate(format_nil.$format_nil_percent$.getGlobalValue(), new SubLObject[] { $str_alt278$End_time__, format_nil.format_nil_a_no_copy(end_date), $str_alt268$_, format_nil.format_nil_a_no_copy(end_second), format_nil.$format_nil_percent$.getGlobalValue(),
			    $str_alt279$Total_time__, format_nil.format_nil_a_no_copy(delta_time), format_nil.$format_nil_percent$.getGlobalValue() }));
		}
		return delta_time;
	    }
	}
	...
	return cconcatenate(phase == ONE_INTEGER ? ((SubLObject) ($csm_scalability_experiments_phase_one_debugging_output_directory$.getGlobalValue())) : $csm_scalability_experiments_phase_two_debugging_output_directory$.getGlobalValue(),
		new SubLObject[] { control_vars.cyc_image_id(), $str_alt285$_txt });
    }
	...
	}
	{
		...
		thread.resetMultipleValues();
		{
		    ...
		    SubLObject start_second = thread.secondMultipleValue();
		    thread.resetMultipleValues();
		    csm_note(cconcatenate(format_nil.$format_nil_percent$.getGlobalValue(),
			    new SubLObject[] { $str_alt288$Experiment_start_time__, format_nil.format_nil_a_no_copy(start_date), $str_alt268$_, format_nil.format_nil_a_no_copy(start_second), format_nil.$format_nil_percent$.getGlobalValue() }));
		}
		{
		...
		thread.resetMultipleValues();
		{
		    ...
		    thread.resetMultipleValues();
		    csm_note(cconcatenate(format_nil.$format_nil_percent$.getGlobalValue(),
			    new SubLObject[] { $str_alt289$Experiment_end_time__, format_nil.format_nil_a_no_copy(end_date), $str_alt268$_, format_nil.format_nil_a_no_copy(end_second), format_nil.$format_nil_percent$.getGlobalValue() }));
		}
		csm_note(cconcatenate(format_nil.$format_nil_percent$.getGlobalValue(),
			new SubLObject[] { $str_alt290$Experiment_total_time__, format_nil.format_nil_a_no_copy(subtract(end_time, start_time)), $str_alt268$_, format_nil.format_nil_a_no_copy(NIL), format_nil.$format_nil_percent$.getGlobalValue() }));
		csm_display_cost_effectiveness_data(UNPROVIDED);
		if (NIL != unnote_tsP) {
		    csm_unnote_machine_transcript_association();
		}
		...
		SubLObject result = NIL;
		{
			...
			system_parameters.$allow_guest_to_editP$.bind(T, thread);
			{
				...
				try {
				    ...
				    cyc_bookkeeping.$cyc_bookkeeping_info$.bind(
					    cyc_bookkeeping.new_bookkeeping_info(api_control_vars.$the_cyclist$.getDynamicValue(thread), fi.$the_date$.getDynamicValue(thread), api_control_vars.$ke_purpose$.getDynamicValue(thread), fi.$the_second$.getDynamicValue(thread)), thread);
    ...
    static private final SubLList $list_alt84 = list(new SubLObject[] { constant_handles.reader_make_constant_shell(makeString("CAEAnalysisTask")), constant_handles.reader_make_constant_shell(makeString("Cyc-BasedProject")),
	    constant_handles.reader_make_constant_shell(makeString("CycKBSubsetCollection")), constant_handles.reader_make_constant_shell(makeString("CycLExpressionType")), constant_handles.reader_make_constant_shell(makeString("Cyclist")),
	    constant_handles.reader_make_constant_shell(makeString("EnglishWord")), constant_handles.reader_make_constant_shell(makeString("DisplayingACycQueryJustification")), constant_handles.reader_make_constant_shell(makeString("DocumentationConstant")),
	    constant_handles.reader_make_constant_shell(makeString("FETIntermediateYesOrNoAnswer")), constant_handles.reader_make_constant_shell(makeString("GenericQuantity")), constant_handles.reader_make_constant_shell(makeString("GregorianMonthType")),
	    constant_handles.reader_make_constant_shell(makeString("KBContentRegressionTest")), constant_handles.reader_make_constant_shell(makeString("KEInteractionResource")), constant_handles.reader_make_constant_shell(makeString("KnowledgeBase")),
	    constant_handles.reader_make_constant_shell(makeString("Language")), constant_handles.reader_make_constant_shell(makeString("LinguisticObjectType")), constant_handles.reader_make_constant_shell(makeString("LinguisticTranslationProcess")),
	    constant_handles.reader_make_constant_shell(makeString("MicrotheorySpindle")), constant_handles.reader_make_constant_shell(makeString("PersonByStudentStatus")), constant_handles.reader_make_constant_shell(makeString("ProgramSpecification")),
	    constant_handles.reader_make_constant_shell(makeString("Relation")), constant_handles.reader_make_constant_shell(makeString("ScalarOrVectorInterval")), constant_handles.reader_make_constant_shell(makeString("Schema-LogicalOrPhysical")),
	    constant_handles.reader_make_constant_shell(makeString("SocialQuantityType")), constant_handles.reader_make_constant_shell(makeString("StructuredKnowledgeSource")), constant_handles.reader_make_constant_shell(makeString("Template")),
	    constant_handles.reader_make_constant_shell(makeString("TestingConstant")), constant_handles.reader_make_constant_shell(makeString("TimePoint")), constant_handles.reader_make_constant_shell(makeString("UsingTheFactEntryTool")) });

    static private final SubLList $list_alt85 = list(constant_handles.reader_make_constant_shell(makeString("InterfaceModifierType")), constant_handles.reader_make_constant_shell(makeString("TestQueryConstant")), constant_handles.reader_make_constant_shell(makeString("TheTerm")),
	    constant_handles.reader_make_constant_shell(makeString("TimeDimensionRelatedConstant")));

    static private final SubLList $list_alt86 = list(
	    new SubLObject[] { constant_handles.reader_make_constant_shell(makeString("CycAnalyticEnvironment-TheProgram")), constant_handles.reader_make_constant_shell(makeString("CycNounLearnerTool")), constant_handles.reader_make_constant_shell(makeString("DeemingAConceptRelevant")),
		    constant_handles.reader_make_constant_shell(makeString("DisplayingACycQueryJustification")), constant_handles.reader_make_constant_shell(makeString("DisplayingFactSheetForConcept")), constant_handles.reader_make_constant_shell(makeString("SearchingForACycLTerm")),
		    constant_handles.reader_make_constant_shell(makeString("UsingTheFactEntryTool")), constant_handles.reader_make_constant_shell(makeString("UsingTheQueryLibraryTool")), constant_handles.reader_make_constant_shell(makeString("LebaneseHizballah")) });
    ...
    static private final SubLList $list_alt87 = list(new SubLObject[] { constant_handles.reader_make_constant_shell(makeString("DetectionOfCollaborationOpportunityFn")), constant_handles.reader_make_constant_shell(makeString("RecordMtByUserTaskAndApplicationFn")),
	    constant_handles.reader_make_constant_shell(makeString("UserKEInteractionFolderForTaskFn")), constant_handles.reader_make_constant_shell(makeString("ModelMtByUserAndTaskFn")), constant_handles.reader_make_constant_shell(makeString("EvaluatingFn")),
	    constant_handles.reader_make_constant_shell(makeString("TestQueryFn")), constant_handles.reader_make_constant_shell(makeString("TemplateFromTestQueryFn")), constant_handles.reader_make_constant_shell(makeString("ThingDescribableAsFn")),
	    constant_handles.reader_make_constant_shell(makeString("WorkEventOnSpecificationFn")), constant_handles.reader_make_constant_shell(makeString("MeaningInSystemFn")), constant_handles.reader_make_constant_shell(makeString("STIF")),
	    constant_handles.reader_make_constant_shell(makeString("STIB")) });

    static private final SubLList $list_alt88 = list(new SubLObject[] { makeString("ConceptDisplay-"), makeString("DeemingRelevant-"), makeString("EditingConcept-"), makeString("FETEvaluation-"), makeString("JustificationDisplay-"), makeString("QuerySearch-"), makeString("QueryAsked-"),
	    makeString("RKF-ClonedQuery-"), makeString("TaskDisplay-"), makeString("WebSearch-"), makeString("WebBrowsing-"), makeString("ResearchAbduction-"), makeString("HYP-"), makeString("TMP-"), makeString("KS-") });
    ...
    static private final SubLList $list_alt113 = list(new SubLObject[] { constant_handles.reader_make_constant_shell(makeString("assertionProducedByReformulation")), constant_handles.reader_make_constant_shell(makeString("dateOfPotentialCollaborationOpportunityDetection")),
	    constant_handles.reader_make_constant_shell(makeString("ebmtNLToCycLTrainingExample")), constant_handles.reader_make_constant_shell(makeString("evaluationOutputSentences")), constant_handles.reader_make_constant_shell(makeString("excludeFromChoicesForPredPopTask")),
	    constant_handles.reader_make_constant_shell(makeString("exemptFromEBMT")), constant_handles.reader_make_constant_shell(makeString("formulaTemplateExample")), constant_handles.reader_make_constant_shell(makeString("includeInChoicesForPredPopTask")),
	    ...
	    constant_handles.reader_make_constant_shell(makeString("kEInteractionFolderOfUserForTaskWRTConcept")), constant_handles.reader_make_constant_shell(makeString("notIdentical-AlreadyReviewed")), constant_handles.reader_make_constant_shell(makeString("noVerificationPerformedOnSentence")),
	    constant_handles.reader_make_constant_shell(makeString("possiblyIdentical-NeedToReview")), constant_handles.reader_make_constant_shell(makeString("potentialCollaboratorConcept")), constant_handles.reader_make_constant_shell(makeString("presumedDifferentDespiteSharedLexification")),
	    constant_handles.reader_make_constant_shell(makeString("recordMtByUserTaskAndApplication")), constant_handles.reader_make_constant_shell(makeString("reviewFromRequestedBy")), constant_handles.reader_make_constant_shell(makeString("sentenceParameterValueInSpecification")),
	    constant_handles.reader_make_constant_shell(makeString("suggestedRule")), constant_handles.reader_make_constant_shell(makeString("suggestedSentence")), constant_handles.reader_make_constant_shell(makeString("suggestedSentenceReviewed")),
	    constant_handles.reader_make_constant_shell(makeString("taskConceptForUser")), constant_handles.reader_make_constant_shell(makeString("termOfUnit")), constant_handles.reader_make_constant_shell(makeString("testAnswersCycL-Exact")),
	    constant_handles.reader_make_constant_shell(makeString("testAnswersCycL-NotWanted")), constant_handles.reader_make_constant_shell(makeString("testAnswersCycLUnimportant")), constant_handles.reader_make_constant_shell(makeString("testAnswersCycL-Wanted")),
	    constant_handles.reader_make_constant_shell(makeString("tkbAptimaTieAssertion")), constant_handles.reader_make_constant_shell(makeString("topicCycL")), constant_handles.reader_make_constant_shell(makeString("unverifiedSentence")),
	    constant_handles.reader_make_constant_shell(makeString("verificationFailedOnSentence")), constant_handles.reader_make_constant_shell(makeString("verificationSucceededOnSentence")), constant_handles.reader_make_constant_shell(makeString("verificationPerformedOnSentence")),
	    constant_handles.reader_make_constant_shell(makeString("candidateReplacementForPosition")), constant_handles.reader_make_constant_shell(makeString("excludeReplacementOptionForFormulaTemplate")),
	    constant_handles.reader_make_constant_shell(makeString("excludeReplacementOptionForFormulaTemplatePosition")), constant_handles.reader_make_constant_shell(makeString("exemptFromEBMT")), constant_handles.reader_make_constant_shell(makeString("formulaTemplateFollowUp")),
	    constant_handles.reader_make_constant_shell(makeString("formulaTemplateGloss")), constant_handles.reader_make_constant_shell(makeString("positionInFormulaTemplateIsReplaceable")), constant_handles.reader_make_constant_shell(makeString("querySpecificationForFormulaTemplate")),
	    constant_handles.reader_make_constant_shell(makeString("clumpSyntaxList")), constant_handles.reader_make_constant_shell(makeString("evaluationOfByOn")), constant_handles.reader_make_constant_shell(makeString("evaluee-Direct")),
	    constant_handles.reader_make_constant_shell(makeString("objectOfEvaluation")), constant_handles.reader_make_constant_shell(makeString("temporallyIntersects")), constant_handles.reader_make_constant_shell(makeString("firstPublicationDateForStyleAndRendering")),
    ...
    static private final SubLList $list_alt127 = list(constant_handles.reader_make_constant_shell(makeString("objectOfEvaluation")), constant_handles.reader_make_constant_shell(makeString("evaluee-Direct")), constant_handles.reader_make_constant_shell(makeString("topicCycL")),
	    constant_handles.reader_make_constant_shell(makeString("taskConceptForUser")), constant_handles.reader_make_constant_shell(makeString("mostRecentImportantConceptEvent")));
    ...
    static private final SubLList $list_alt139 = list(makeSymbol("CSM-ASSERTION-OWNER"), makeSymbol("CSM-ASSERTION-DATE"), makeSymbol("CSM-ASSERTION-TIME"), makeSymbol("CSM-ASSERTION-UNIVERSAL-TIME"), makeSymbol("CSM-ASSERTION-SENTENCE"), makeSymbol("CSM-ASSERTION-MT"),
	    makeSymbol("CSM-ASSERTION-FORTS"));

    static private final SubLList $list_alt140 = list(makeSymbol("_CSETF-CSM-ASSERTION-OWNER"), makeSymbol("_CSETF-CSM-ASSERTION-DATE"), makeSymbol("_CSETF-CSM-ASSERTION-TIME"), makeSymbol("_CSETF-CSM-ASSERTION-UNIVERSAL-TIME"), makeSymbol("_CSETF-CSM-ASSERTION-SENTENCE"),
	    makeSymbol("_CSETF-CSM-ASSERTION-MT"), makeSymbol("_CSETF-CSM-ASSERTION-FORTS"));
    ...
    static private final SubLList $list_alt234 = list(constant_handles.reader_make_constant_shell(makeString("dateOfPotentialCollaborationOpportunityDetection")), constant_handles.reader_make_constant_shell(makeString("irrelevantPotentialCollaborationOpportunityDetection")),
	    constant_handles.reader_make_constant_shell(makeString("kEInteractionFolderOfUserForTask")), constant_handles.reader_make_constant_shell(makeString("kEInteractionFolderOfUserForTaskWRTConcept")), constant_handles.reader_make_constant_shell(makeString("potentialCollaboratorConcept")),
	    constant_handles.reader_make_constant_shell(makeString("recordMtByUserTaskAndApplication")), constant_handles.reader_make_constant_shell(makeString("termOfUnit")));
