/**
 * The Random Indexing Plugin gets a Sparql query as input, and does following:
 *  1) Extracts the keywords from that query, append them to a single string;
 *  2) Sends the that string to the Webservice at http://urlwolfpack.gotdns.org:8080,
 *  3) The webservice itself processes the query, creating RDF statements with the 
 *     most similar documents found in the dbpedia;
...
import eu.larkc.plugin.reason.randomindexing.util.QueryProcessor;
import eu.larkc.plugins.reason.esa_ri.webservice.CorruptIndexException_Exception;
import eu.larkc.plugins.reason.esa_ri.webservice.IOException_Exception;
import eu.larkc.plugins.reason.esa_ri.webservice.ParseException_Exception;

/**
 * @author R.B.Vidal / Max-Planck Institute Berlin
 * 
 */
public class RandomIndexing implements Reasoner {

	private static final long serialVersionUID = 1221089204764676732L;
	private static Logger log = Logger.getLogger(RandomIndexing.class
			.getCanonicalName());
	...
	public SetOfStatements sparqlConstruct(SPARQLQuery theQuery,
		...
		if (theQuery instanceof SPARQLQueryImpl) {
			...
			eu.larkc.plugins.reason.esa_ri.webservice.EsaRiWebServiceService service = new eu.larkc.plugins.reason.esa_ri.webservice.EsaRiWebServiceService();
			eu.larkc.plugins.reason.esa_ri.webservice.EsaRiWebService port = service
					.getEsaRiWebServicePort();

			try {
				result = port.getResultsUsingRandomIndexing(QueryProcessor
