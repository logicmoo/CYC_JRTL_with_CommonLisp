...
import eu.larkc.plugin.reason.pion.SimplePIONReasoner.WrappedIterator;

 

	public class PIONwithStopRules implements Reasoner{
		
		
		public static String seletedStopRule = "CardinalityCheckingRule";//by default
		...
		private String ClassDes="PIONwithStopRules";
		

		/* An id to identify this reasoner */
		private String reasonerID = "eu.larkc.plugin.reason.pion.PIONwithStopRules";
	...
	public BooleanInformationSet sparqlAsk(SPARQLQuery theQuery,
		...
		Logger log = LoggerFactory.getLogger(PIONwithStopRules.class);
		  ...
		  // status = true: need no processing rest part of the query, status = false: need to continue processing,
			...
			StopRules stopRules = new StopRules();
            
			if (seletedStopRule.equals("CardinalityCheckingRule"))
				{stopRules.CardinalityCheckingRuleInit();}
			if (seletedStopRule.equals("TimeCostCheckingRule"))
			{stopRules.TimeCostCheckingRuleInit();}
		  ...
		  // default value of the query answer;
			...
			if (consistency){
				 log.info("[PIONwithStopRules]the ontology is consistent\nStarting the standard reasoning processing\n");	
		    ...
		    log.info("[PIONwithStopRules]the ontology is not consistent\nStarting the PION reasoning processing\n");	
...
//		System.out.println("["+ ClassDes + "]triples = " + triples);
		...
		//	 System.out.println("["+ ClassDes+ "]the axioms of the new selectedOntology is\n"+selectedOntology.getAxioms());
		...
		OWLOntology newSelectedOntology = PIONSelectionFunction.Selection(selectedOntology, ontology);
	 	selectedOntology = newSelectedOntology;
	 	
		long s = selectedOntology.getAxiomCount();
       
       if (seletedStopRule.equals("CardinalityCheckingRule"))
		{stopRules.CardinalityCheckingRule(s);}
	   if (seletedStopRule.equals("TimeCostCheckingRule"))
	    {stopRules.TimeCostCheckingRule(s);}
		}
	 	
	    } 
	    while(!result&& consistency &&!stopStatus);
...
//			System.out.println("["+ ClassDes+ "]unprocessedPattern = " + unprocessedPattern + " unprocessedPatternStatus=" + unprocessedPatternStatus);
		    ...
		    Set<OWLClass> incClasses = PIONProcess.getInconsistentClasses(reasoner, factory);
		     ...
		     boolean consistency = incClasses.isEmpty();
		    ...
		    log.info("["+ ClassDes+ "]the ontology is inconsistent. The unsatisfiable concepts are " + incClasses);
		    ...
		    boolean Status = true;//false = to stop; true = continue the processing
			 ...
			 do {
			        ...
			        if (consistency){
			    		...
			    		long s = selectedOntology.getAxiomCount();
			    	//    stopStatus = stopRules.CardinalityCheckingRule(s);
...
//				            System.out.println("["+ ClassDes+ "]the  new result set is " + resultSet);
	 ...
	 //  */	
				 ...
				 } while(consistency&&Status &&!stopStatus);
