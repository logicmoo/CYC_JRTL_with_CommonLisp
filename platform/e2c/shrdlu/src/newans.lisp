      ...
      (PROG (EXP ANS SUCCESS PLAN PLAN2)
	    ...
	    (RETURN ;OUT ONE OF
		  (ANSBUILD (COND (SUCCESS (PLAUSIBILITY? RSS)) ;			       ;THEM. BEFORE
				  (T
				   (- (PLAUSIBILITY? RSS) ;FAILING IT MARKS DOWN WHETHER IT SUCCEEDED AND
...
(DEFUN ANSELIMINATE  (ANSLIST)
      ...
      (PROG (AMB POSSIBILITIES XX)
	    ...
	    UP
	    (SETQ POSSIBILITIES (LIST (CAR AMB))) ;POSSIBILITIES IS THE LIST OF POSSIBLE
...
(DEFUN ANSNAME	(PHRASE)
      ...
      ;;CALLED BEFORE ONEIFYING AND ITIFYING AND THE R3ST OF THAT
      ;;CRAP.
       ;;;
      ...
      (PROG (ANSNODE C N CUT SM-TIME MP-TIME)
	    ...
	    (THSETF (G3T (CAR (SM ANSNODE)) 'REFER=)
				     ...
				     ((NUMBERP NUM)
				      (APPEND (COND ((EQ NUM LENGTH)
					      ...
					      (COND ((EQ NUM LENGTH) NIL) ;THE NUMBER ISN'T REPEATED IF IT IS THE SAME AS
						    (T
						     (PREPPUT (APPEND (NAMESUGAR LENGTH
...
(DEFUN ANSTHM  (EXP)
      ...
      (PROG (NEG VARLIST BODY)
	    ...
	    (COND
		  ...
		  ((NOT (THGET (CAR EXP) 'TELLABLE)) (NOTELL)) ;ASSERTION IT CAN HANDLE.  IT USES GLOBAL-ERR
		  (T
...
;;;############################################################
      ...
      (DEFUN LISTNAMES	(PHRASE SPEC NAMES)
	    ...
	    (PROG (COUNT EXAM X RES ANS COMMA?)
		  ...
		  UP
		  (SETQ COUNT 1.) ;THE BACKREF HACKER - DDM 5-12-73 INSTRUCTIONS
		  (SETQ EXAM (CAR NAMES)) ;FOR NAMING IT.
		  (SETQ NAMES (CDR NAMES))
		  BACK
		  (COND	((SETQ X (ASSOC (CAR EXAM) NAMES))
			...
			RES ;AND THE DESCRIPTION IS CHECKED TO SEE IF "ONE"
			     (MAPCAR #'(LAMBDA (PHRASE2) ;CAN BE USED. ADD THE ONE JUST PRODUCED TO THE
					    (COND ((PROPNAME (CAADR PHRASE2)) ;RESULT LIST. TRY ANOTHER.
						   (CAR PHRASE2))
						  (T
						   (ANSNAME PHRASE2) ;ANSNAME PARSES THE PHRASE AND PUTS THE
       ...
       (PROG (TYPE\: TYPELIST TYPE NAME\: COLOR\: COLORLIST SIZE\:
	     ...
	     (DECLARE (SPECIAL TYPE\: TYPELIST TYPE NAME\: COLOR\:
		  ...
		  (COND	((EQ (SETQ TYPE (CADDAR TYPE\:)) '\#NAME) ;KEPT, AND IF SO PUTS THE RELEVANT SENTENCE
			...
			((MEMQ '\#PROPERTY (THGET TYPE 'SYS)) ;IS ITS OWN NAME
      ...
      (DEFUN ONECHECK  (ITEM)
	    ...
	    (PROG (ANS OLD NEW)
		  ...
		  (OR (EQ (CAR OLD) (CAR NEW))
		      (EQ (CAR OLD) (THGET (CAR NEW) 'ROOT))
		      (EQ (CAR NEW) (THGET (CAR OLD) 'ROOT))
		      (RETURN ITEM)) ;IF THE NOUNS DON'T MATCH, JUST RETURN WHAT YOU
		  LOOP
		  (SETQ NEW (CDR NEW)) ;GOT. MATCHING INCLUDES PLURALS TO THEIR
	    ...
	    (PROG (HAVE NEED XX ANS PLNRCODE LOOP)
		  ...
		  (COND
			...
			((OR? OSS)
			 ...
			 (RETURN (PROG (Y)
				       GO
				       (COND ((SETQ Y (FINDCHOOSE (CAR LOOP)
								  X
								  ANS2))
					      (RETURN Y))
					     ((SETQ LOOP (CDR LOOP))
		  ...
		  (SETQ	PLNRCODE (PLNR-DESCRIBE	(RELATIONS? OSS)
						(VARIABLE? OSS)
						(LIST (VARIABLE? OSS))))
		  (THSETF (G3T OSS 'PLNRCODE=) PLNRCODE)
		  (COND
		  ...
		  (COND
			...
			((SETQ
			 ...
			 (GO GO)))
		  CONJ
		  (SETQ LOOP (OR (AND? RSS) (OR? RSS)))
		  UP
		  (COND	((THGET (CAR LOOP) 'REFER)
			 (SETQ ANS (APPEND (THGET (CAR LOOP) 'REFER)
					   ANS)))
			((SETQ XX
		  ...
		  (COND	((AND ANS (OR? OSS)))
			...
			((RETURN NIL)))
		  DONE
		  (AND (ATOM (VARIABLE? OSS))
		       (THSETF (G3T (VARIABLE? OSS) 'BIND)
			       (REVERSE ANS)))
