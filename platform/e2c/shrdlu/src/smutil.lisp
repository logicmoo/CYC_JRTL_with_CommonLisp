...
#-USE-DEFUN-FEXPR
(PROGN (DEFMACRO BUILD-SHRDLU (&REST %L) (LIST 'APPLY-BUILD-SHRDLU (LIST 'QUOTE %L)))
 (DEFUN APPLY-BUILD-SHRDLU (%L)
  (PROG
   ...
   (MAPC #'(LAMBDA (%PROP) (AND (SETQ %X (EVAL %PROP)) (SETF (GET NODE= %PROP) %X))) %PROPS) (AND BUILD-SEE ((LAMBDA (DPSTOP) (DP NODE=)) SMN-STOP)) (RETURN NODE=)))
 'BUILD-SHRDLU)
...
(DEFUN NEWCOPY (OSS)
	...
	UP   (COND ((NULL OLD)
		    ...
		    (SETF (GET NEW 'PARSENODE=) C)
		    (RETURN NEW))
		   ((EQ (CAR OLD) 'PNAME))
		   ((SETF (GET NEW (CAR OLD)) (CADR OLD))))
	     (SETQ OLD (CDDR OLD))
	     (GO UP)))
...
(DEFUN DOBACKREF (ANSWER)
       ...
       (PROG (ANSRSS)
	     ...
	     (MAPC #'(LAMBDA (PRONOUN) (SETF (GET PRONOUN 'LASTBIND)
					       (GET PRONOUN
						    'BIND))
...
(DEFUN VALUEPUT NIL (SETF (GETF (CDR (LIST THVALUE)) 'WHO) SENTNO))
...
(DEFUN COMPARE-BUILD (NODE DEGREE)
       ...
       (PROG (RESTRICTIONS\: DIMENSION\: DIRECTION\:)
	     ...
	     (SETQQCHECK NIL
			 ...
			 'MEASURE)
								       ;DEFINITION
	     (SETF (GET 'COMPARE-PSEUDO-VERB 'SEMANTICS)
		      (LIST 'RELATION
			    (LIST 'RESTRICTIONS\:
