...
(DEFUN ISTENSE	(NODE ARG)
      ...
      (PROG (X)
	    ...
	    (OR (SETQ X (TENSE? X)) (SETQ X '(PRESENT)))
...
(DEFUN IMPERF?	(TSS_LOCAL)
      (AND (CDR (SETQ TSS_LOCAL (TENSE? TSS_LOCAL))) (EQ (CAR TSS_LOCAL) 'PRESENT)))

(DEFUN IMPERF NIL (ISTENSE C 'IMPERF))

#-USE-DEFUN-FEXPR
(PROGN
      (DEFMACRO BUILD-SHRDLU (&REST %L) (LIST 'APPLY-BUILD-SHRDLU (LIST 'QUOTE %L)))
      (DEFUN APPLY-BUILD-SHRDLU	 (%L)
	    (PROG
		  (%X NEGATIVE=	REFER= PLNRCODE= MARKERS= SYSTEMS= TENSE= TSSNODE= RELMARKERS= ANSNODE=	ACTION=	ANSRSS=	PLAUSIBILITY= DETERMINER= AND= OR= AMBIGUITIES=	RELATIONS= VARIABLE= VARLIST=
		      REL= RSSNODE= PARSENODE= OSSNODE=	NODE= %PROPS)
		  (DECLARE
			(SPECIAL NEGATIVE= REFER= PLNRCODE= MARKERS= SYSTEMS= RELMARKERS= PLAUSIBILITY=	DETERMINER= AND= OR= AMBIGUITIES= RELATIONS= VARIABLE= VARLIST=	REL= RSSNODE= PARSENODE=
				 OSSNODE= TSSNODE= TENSE= ANSNODE= ANSRSS= ACTION=))
		  (SETQQCHECK T
			      %L
			      (SETQ %PROPS
					 '(NEGATIVE= REFER= PLNRCODE= MARKERS= SYSTEMS=	RELMARKERS= PLAUSIBILITY= DETERMINER= AND= OR= AMBIGUITIES= RELATIONS= VARIABLE= VARLIST= REL= RSSNODE=
					   PARSENODE= OSSNODE= TSSNODE=	TENSE= ANSNODE=	ANSRSS=	ACTION=))
			      'BUILD-SHRDLU)
		  ...
		  (AND MARKERS=
		       ...
		       (SETQ MARKERS= (CAR %X))
		       (SETQ SYSTEMS= (CADR %X)))
		  (SETQ NODE= (OR OSSNODE= RSSNODE= TSSNODE= ANSNODE= (ERT |...BUILD-SHRDLU:| NO NODE=)))
		  (MAPC #'(LAMBDA (%PROP) (AND (SETQ %X (EVAL %PROP)) (THSETF (G3T NODE= %PROP) %X))) %PROPS)
		  (AND BUILD-SEE ((LAMBDA (DPSTOP) (DP NODE=)) SMN-STOP))
		  (RETURN NODE=)))
      'BUILD-SHRDLU)

#+USE-DEFUN-FEXPR
(DEFUN-FEXPR BUILD-SHRDLU
	     (%L)
	     ;;BUILD-SHRDLU CONSTRUCTS AN OSS OR AN RSS FROM ITS ARGUMENTS.
;;;
;;;INPUTS:
       ...
       ;;;
	     ...
	     (PROG (%X NEGATIVE= REFER=	PLNRCODE= MARKERS= SYSTEMS= TENSE= TSSNODE= RELMARKERS=	ANSNODE= ACTION= ANSRSS= PLAUSIBILITY= DETERMINER= AND=	OR= AMBIGUITIES= RELATIONS= VARIABLE= VARLIST=
		       REL= RSSNODE= PARSENODE=	OSSNODE= NODE= %PROPS)
		   (DECLARE (SPECIAL NEGATIVE= REFER= PLNRCODE=	MARKERS= SYSTEMS= RELMARKERS= PLAUSIBILITY= DETERMINER=	AND= OR= AMBIGUITIES= RELATIONS= VARIABLE= VARLIST= REL= RSSNODE= PARSENODE=
				     OSSNODE= TSSNODE= TENSE= ANSNODE= ANSRSS= ACTION=))

		   (SETQQCHECK T
			       %L
			       (SETQ %PROPS '(NEGATIVE=	REFER= PLNRCODE= MARKERS= SYSTEMS= RELMARKERS= PLAUSIBILITY= DETERMINER= AND= OR= AMBIGUITIES= RELATIONS= VARIABLE= VARLIST= REL= RSSNODE=
					      PARSENODE= OSSNODE= TSSNODE= TENSE= ANSNODE= ANSRSS= ACTION=))
			       'BUILD-SHRDLU)
		   (AND	RSSNODE=
			(NOT MARKERS=)
		   ...
		   (SETQ NODE= (OR OSSNODE=
				   ...
				   TSSNODE=
				   ANSNODE=
				   (ERT \.\.\.BUILD-SHRDLU\: NO NODE=)))
		   (MAPC #'(LAMBDA (%PROP)
				(AND (SETQ %X (EVAL %PROP))
				     (THSETF (G3T NODE= %PROP) %X)))
			 %PROPS)
		   (AND BUILD-SEE ((LAMBDA (DPSTOP) (DP NODE=)) SMN-STOP))
		   (RETURN NODE=)))
...
(DEFUN NEWCOPY	(OSS)
      (PROG (OLD NEW)
	    ...
	    (SETQ OLD (SYMBOL-PLIST OSS))
 ;WATCH OUT -- THIS IS IMPLEMENTATION DEPENDENT,
	    UP
	    (COND ((NULL OLD)
 ;AND GETS THE ENTIRE PROPERTY LIST IN OUR LISP.
		   (THSETF (G3T NEW 'PARSENODE=) C)
		   (RETURN NEW))
		  ((EQ (CAR OLD) 'PNAME))
		  ((THSETF (G3T NEW (CAR OLD)) (CADR OLD))))
	    (SETQ OLD (CDDR OLD))
	    (GO UP)))

;;;=============================================================================

(DEFUN-FEXPR RELATION
	     (%DEFL)
	     (DECLARE (SPECIAL %DEFL))
...
;;;
;;;SIDE-EFFECTS:
;;;     CREATES AN RSS BY CALLING BUILD-SHRDLU
	     (ITERATE
		   '(LAMBDA (ARG1 ARG2 ARG3 ARG4 ARG5 ARG6)
			  (PROG	(SMCOMP	SMSUB SMOB1 SMOB2 SMOBL	MARKERS\: RESTRICTIONS\: PLAUSIBILITY\:	REL\: PARAPHRASE\: RELMARKERS\:	RSSNAME	PROCEDURE\: \#1	\#2 \#3	%NEWRSS	%OSSNODE)
				(DECLARE (SPECIAL SMCOMP SMSUB SMOB1 SMOB2 SMOBL RESTRICTIONS\:	PROCEDURE\: PLAUSIBILITY\: PARAPHRASE\:	MARKERS\: REL\:	RELMARKERS\: \#1 \#2 \#3))
				(SETQ %DEF   ARG1
				      SMSUB  ARG2
				      SMOB1  ARG3
				      SMOB2  ARG4
 ...
 ;IN WRITING DEFINITIONS, A SIMPLY TERRIBLE IDEA.
					    '(RESTRICTIONS\: PROCEDURE\: PLAUSIBILITY\: PARAPHRASE\: MARKERS\:)
					    'RELATION)
 ...
 ;NEWRSS IS THE NEW RSS NODE NAME CREATED BY
						       RSSNODE=
 ;BUILD-SHRDLU RSSNODE= IS KEYWORD FOR INPUT INTO BUILD
 ...
 ;THE
						       VARIABLE=
						       ((LAMBDA (X) (THSETF (G3T X 'RSSVAR) X))
						       ...
						       RELATIONS=
						       (REVERSE
 ;SEND TO BUILD-SHRDLU FOR MARKERS= THE VALUE SENT TO
							     (MAPCAR
								   #'(LAMBDA (%PLNRPHRASE)
...
(DEFUN DOBACKREF  (ANSWER)
      ...
      (PROG (ANSRSS)
	    (SETQ ANSRSS (THGET ANSWER 'ANSRSS=))
	    (SETQ BACKREF2 BACKREF)
	    (SETQ BACKREF NIL)
	    (SETQ LASTREL (REL? ANSRSS))
	    (MAPC #'(LAMBDA (PRONOUN)
			 (THSETF (G3T PRONOUN 'LASTBIND)
				 (THGET	PRONOUN
					'BIND))
 ...
 ;TRUE IF NODE HAD MULTIPLE INTERPRETATIONS
				      (PRINT (SM BACKNODE))
				      (SETR 'SEMANTICS
					    (ERT DOBACKREF\: RETURN AN OSS FOR BACKNODE)
					    BACKNODE)))
			       (COND
				     ((REFER? (CAR (SM BACKNODE))))
 ;IF NODE HAS REFERENT, FINE
				     (ELSE
				      (THSETF (G3T (CAR (SM BACKNODE)) 'REFER=)
					      (OR (THGET (VARIABLE? (CAR (SM BACKNODE)))
							 'BIND)
						  (ERT DOBACKREF\: RETURN REFERENT FOR BACKNODE))))))
			BACKREF2))
...
(DEFUN-FEXPR ITERATE (%L)
       ...
       ;;;VALUE:
	     ...
	     (PROG (%X %XL)
		   (DECLARE (SPECIAL %XL))
		   (RETURN (EVAL (ITERATEX (CAR %L) (CDR %L))))))
...
(DEFUN MAPBLAND	 (FN L)
      ...
      (PROG (ANS F)
	    ...
	    A
	    (COND ((NULL (SETQ F (APPLY FN (LIST (CAR L))))))
...
(DEFUN MUMBLE  (X)
      ...
      ;;MENTIONED.
      (COND ((NULL WHO))
	    ((EQ WHO 'HE) (GETF (CDR X) 'WHO))
	    ((SETQ X (GETF (CDR X) 'WHO))
	     (NOT (OR (< X (CAR WHO))
		      (> X (CADR WHO)))))))
...
;;;  A SET OF OSS'S ARE CREATED AT THE GLOBAL LEVEL
 ...
 ;%OSS IS A SINGLE OSS NODE NAME PICKED OFF OF
					    (PROG (%OSSNODE %CHKRESULT MARKERS\: SYSTEMS\: PLAUSIBILITY\: DET\: RELATIONS\: PARAPHRASE\: PROCEDURE\:)
						  (DECLARE (SPECIAL %CHKRESULT PARAPHRASE\: PLAUSIBILITY\: PROCEDURE\: MARKERS\:))
...
(DEFUN PLNR-GOALIFY  (PLNRPHRASE)
 ...
 ;PRESENT TENSE TIME MARKERS ARE REMOVED TO
      (COND ((THGET (CAR PLNRPHRASE) 'NOGOAL) PLNRPHRASE)
...
(DEFUN PLNR-NUMREL  (OSS)
      ...
      ;;EXPRESSION.
      (COND ((MEMQ OSS RELLIST) (SETQ REL\: OSS))
	    (OSS)))
...
(DEFUN PLNR-RECOMMENDIFY  (%PLNRPHRASE)
      ...
      (PROG (%ELEMENT)
	    (RETURN
		  (AND (SETQ %ELEMENT (THGET (CAR %PLNRPHRASE)
					     'THMLIST))
...
(DEFUN COMPARE-BUILD  (NODE DEGREE)
 ...
 ;THESE ARE THE POSSIBLE PARTS OF A MEASURE
	    (SETQQCHECK	NIL
			...
			'MEASURE)
 ;DEFINITION
	    (THSETF (G3T 'COMPARE-PSEUDO-VERB 'SEMANTICS)
		    (LIST 'RELATION
			  (LIST	'RESTRICTIONS\:
...
(DEFUN FINDMEASURE  (NODE)
      ...
      (COND ((SETQ X (ASSOC 'MEASURE
			    (THGET (ROOT (NB NODE)) 'SEMANTICS)))
	     (CADR X))
	    ((GLOBAL-ERR (APPEND '(I DON\"T KNOW HOW TO COMPARE THINGS WITH RESPECT TO)
				 (LIST (ROOT (NB NODE))))))))
...
(DEFUN COMPARE-PROC  (VAR NEWVAR ORDINAL)
      (PROG (RESTRICTIONS\: DIRECTION\: DIMENSION\:)
	    (DECLARE (SPECIAL RESTRICTIONS\: DIRECTION\: DIMENSION\:))
	    (SETQQCHECK	NIL
			ORDINAL
	    ...
	    (RETURN (LIST '\#MORE
			  DIMENSION\:
			  (PLNR-VAR (COND (DIRECTION\: NEWVAR)
					  (VAR)))
...
(DEFUN EXPAND  (EXP EVENT)
      ...
      (COND
	    ...
	    ((EQ (CAR EXP) '*ORDINAL*)
	     (COND (ORDINAL
		    (GLOBAL-ERR '(I CAN\ 'T HANDLE TWO ORDINALS OR SUPERLATIVES AT ONCE)))
		   ((SETQ ORDINAL (CADR EXP)) '*ORDINAL*)))
	    ((EQ (CAR EXP) '\#SUBST)
	     (ERT EXPAND - IS \#SUBST BEING HANDLED BY SOMEONE ELSE?)
	     EXP)
	    ((PROG (BODY QUANTIFIER CHOICE VAR MULTIPLE)
		   (DECLARE (SPECIAL QUANTIFIER))
		   (SETQ MULTIPLE (EVAL (THGET (CAR EXP) 'MULTIPLE)))
		   (SETQ
			 EXP
			      (MAPCAR
				    #'(LAMBDA (X)
...
(DEFUN ERQSET  (X)
      ...
      (COND (QUANTIFIER
	     (GLOBAL-ERR '(I CAN\ 'T HANDLE COMBINATIONS OF QUANTIFIERS AND CONNECTIVES WHICH ARE SO COMPLICATED)))
	    ((SETQ QUANTIFIER X))))
	    ...
	    (COND ((NULL %LIST) (RETURN T))
		  ...
		  (T
		   (SETQ %X
			      (APPLY 'ERT
				     (CONS (CAR %LIST)
					   (APPEND '(IS NOT A LEGAL SPECIFICATION FOR)
						   (LIST %NAME)))))))
	    UP
	    (COND ((EQ %X '?)
		   (PRINT %CHECKLIST)
...
(DEFUN THVAL2  (WHO AA)
      ...
      (PROG (RESULT X mplnr-ttime m-gc)
	    ...
	    (AND PLANNERSEE
		 ...
		 PLNRSEE-PAUSE
		 (ERT FOR PLANNER))
	    (SETQ MPLNR-TTIME (GET-INTERNAL-RUN-TIME)
		  M-GC	      0.)
	    (SETQ RESULT (COND (BYPASS-PLANNER (ERT ENTER PLANNER RESULT))
			       (T (THVAL AA '((EV COMMAND))))))
...
(DEFUN WHO  (X)
      (COND ((NULL WHO))
	    ((ATOM X))
	    ((NOT (SETQ X (GETF (CDR X) 'WHO))) NIL)
	    ((EQ WHO 'HE))
	    ((< (CAR WHO) X LASTSENTNO))))

(DEFUN CHECK  (NEW-MARKERS MARKERS SYSTEMS)
      (DECLARE (SPECIAL MARKERS SYSTEMS))
...
;;;
 ...
 ; ADD NEW MARKER TO LIST
	    (SETQ NEW-SYSTEMS (THGET MARKER 'SYS))
...
(DEFUN FINDEVENTS  (RSS)
      ...
      (THSETF (G3T (VARIABLE? RSS) 'USED) T)
      (THVAL2 NIL
	      (PLNR-FINDIFY 'ALL
			    (VARIABLE? RSS)
			    (LIST (VARIABLE? RSS))
