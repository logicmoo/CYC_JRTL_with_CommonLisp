...
(PROGN 
 ...
 (DEFUN APPLY-BUILD-SHRDLU (%L)
  (PROG
   (%X NEGATIVE= REFER= PLNRCODE= MARKERS= SYSTEMS= TENSE= TSSNODE= RELMARKERS= ANSNODE= ACTION= ANSRSS= PLAUSIBILITY= DETERMINER= AND= OR= AMBIGUITIES= RELATIONS=
    VARIABLE= VARLIST= REL= RSSNODE= PARSENODE= OSSNODE= NODE= %PROPS)
   (DECLARE
   ...
   (SETQQCHECK T %L
    (SETQ %PROPS
     '(NEGATIVE= REFER= PLNRCODE= MARKERS= SYSTEMS= RELMARKERS= PLAUSIBILITY= DETERMINER= AND= OR= AMBIGUITIES= RELATIONS= VARIABLE= VARLIST= REL= RSSNODE= PARSENODE=
       OSSNODE= TSSNODE= TENSE= ANSNODE= ANSRSS= ACTION=))
    'BUILD-SHRDLU)
   ...
   (MAPC #'(LAMBDA (%PROP) (AND (SETQ %X (EVAL %PROP)) (THSETF (G3T NODE= %PROP) %X))) %PROPS) (AND BUILD-SEE ((LAMBDA (DPSTOP) (DP NODE=)) SMN-STOP)) (RETURN NODE=)))
 'BUILD-SHRDLU)

#+USE-DEFUN-FEXPR
(DEFUN-FEXPR BUILD-SHRDLU (%L)
       ...
       (PROG (%X NEGATIVE= REFER= PLNRCODE= MARKERS= SYSTEMS=
	     ...
	     (DECLARE (SPECIAL NEGATIVE= REFER= PLNRCODE= MARKERS=
	                       SYSTEMS= RELMARKERS= PLAUSIBILITY=
			       DETERMINER= AND= OR= AMBIGUITIES=
			       RELATIONS= VARIABLE= VARLIST= REL=
			       RSSNODE= PARSENODE= OSSNODE= TSSNODE=
			       TENSE= ANSNODE= ANSRSS= ACTION=))
...
(DEFUN-FEXPR RELATION (%DEFL)
	...
	'(LAMBDA (ARG1 ARG2 ARG3 ARG4 ARG5 ARG6)
	  (PROG (SMCOMP SMSUB SMOB1 SMOB2 SMOBL MARKERS\:
		 ...
		 %OSSNODE)
		(DECLARE (SPECIAL SMCOMP SMSUB SMOB1 SMOB2 SMOBL
		                  RESTRICTIONS\: PROCEDURE\:
		                  PLAUSIBILITY\: PARAPHRASE\:
				  MARKERS\: REL\: RELMARKERS\:
				  \#1 \#2 \#3))
		(SETQ %DEF ARG1
		      SMSUB ARG2
		      SMOB1 ARG3
...
(DEFUN DOBACKREF (ANSWER)
       ...
       (PROG (ANSRSS)
	     ...
	     (MAPC #'(LAMBDA (PRONOUN) (THSETF (G3T PRONOUN 'LASTBIND)
					       (THGET PRONOUN
						    'BIND))
...
(DEFUN MAPBLAND (FN L)
	...
	A    (COND ((NULL (SETQ F (APPLY FN (LIST (CAR L))))))
       ...
       (PROG (%VARNAM)
		...
		'(LAMBDA (%OSS %DEF)
		  ...
		  (PROG (%OSSNODE %CHKRESULT MARKERS\: SYSTEMS\:
			 ...
			 PROCEDURE\:)
			(DECLARE (SPECIAL %CHKRESULT PARAPHRASE\:
			                  PLAUSIBILITY\: PROCEDURE\:
					  MARKERS\:))
...
(DEFUN COMPARE-BUILD (NODE DEGREE)
       ...
       (PROG (RESTRICTIONS\: DIMENSION\: DIRECTION\:)
             (DECLARE (SPECIAL RESTRICTIONS\: DIMENSION\:
	                       DIRECTION\:))
...
(DEFUN RELFIND (NODE)
       ...
       (PROG (REL)
	  (CATCH 'DONE
	     (MAPL #'(LAMBDA (X) (DECLARE (SPECIAL X))
	                         (COND ((ISQ X NG)
					      (AND (NOT (ISQ X COMP))
						   (NOT (ISQ X DEF))
						   (SETQ REL X)
