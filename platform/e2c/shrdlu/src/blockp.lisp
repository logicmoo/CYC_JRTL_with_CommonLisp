...
(DEFS TC-ON
      ...
      (THCONSE (X Y Z)
	       (\#ON (THV X) (THV Y))
	       (THOR (THGOAL (\#SUPPORT (THV Y) (THV X)))
		     (THAND (THASVAL (THV X))
      ...
      (THCONSE (OBJ SURF BLOCKS PYR X Y)
	       ...
	       (THCOND ((NULL (THV BLOCKS))
		       ...
		       ((THSETQ (THNV X) (CAR (THV BLOCKS)))
			     ...
			     (THCOND ((THVSETQ (THNV Y)
					       (OR (PACKON (THV X) (THV PYR))
...
(DEFS TC-PART
	...
	(THGOAL (\#IS (THV Y) \#STACK))
	(THGOAL
	 (\#CHOOSE (THV X) (THNV Z) '(((THGOAL (\#PART (THV *) (THV Y))))))
	 (THUSE TC-CHOOSE))
	(OR (NOT (ATOM (THV Z))) (THSETQ (THNV Z) (LIST (THV Z))))
	GO
	(THCOND
	 ((NULL (THV Z)) (THSUCCEED))
...
(DEFS TC-PUTON
      ...
      (THCONSE (X Y Z (WHY (EV)) EV)
	       ...
	       (COND ((LISTP (THV X)) (OR (CDR (THV X)) (THSETQ (THNV X) (CAR (THV X)))))
		     (T))
	       (NOT (COND ((ATOM (THV X)) (EQ (THV X) (THV Y)))
			  ((MEMQ (THV Y) (THV X)))))
	       (MEMORY)
	       (THCOND ((ATOM (THV X))
			     (THGOAL (\#CLEARTOP (THV X)) (THUSE TC-CLEARTOP))
			 ...
			 (APPEND (THV X)
				 (THVAL	(LIST 'THFIND
						...
						(THGOAL (\#IS (THV Y) \#BLOCK)))
					      '(NOT (EQ (THV X) (THV Y))))
					THALIST)))))
	    (COND ((THVSETQ (THNV PYR) (PACKO (THV X) '\#PYRAMID)) (NULL (CDR (THV PYR))))
		  (T))
...
(DEFS TC-STARTEND3
      ...
      (THCONSE (X Y EV TIME)
	       ...
	       (THGOAL ((THV X) (THV Y) (THV EV) (THV TIME))
		       (THUSE TC-STARTEND4))))
...
(DEFS TCT-AT
      ...
      (THCONSE (X Y Z TIME W)
	       ...
	       (THOR (THGOAL (\#MANIP (THV Y)))
		     (THAND (THGOAL (\#IS (THV Y) \#BOX))
...
(DEFS TCT-2
      ...
      (THCONSE (X EV TIME)
	       ((THV X) (THV TIME))
	       (THGOAL ((THV X) (THV EV) (THV TIME))
		       (THUSE TCTE-3))))

(DEFS TCT-3
      THEOREM
      (THCONSE (X Y EV TIME)
	       ((THV X) (THV Y) (THV TIME))
	       (THGOAL ((THV X) (THV EV) (THV Y) (THV TIME))
		       (THUSE TCTE-4))))
