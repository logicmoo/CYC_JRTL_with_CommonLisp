...
(DEFUN SMCONJ2	(INTERPLIST RESTLIST)
      ...
      ;;SOPHISTICATION.  RESTLIST IS THE R3ST OF NODES YET TO BE
      ;;HANDLED.
...
(DEFUN SMVG  NIL
      ...
      (PROG (TSS_LOCAL1 TENSE)
	    (SETQ TSS_LOCAL1 (GETR 'TIME (MOVE-PT C U (CLAUSE))))
	    (AND (CQ NEG) (ADD-F-PT 'NEG PT))
 ;NEG IS TRANSFERRED FROM THE
	    (SETQ TENSE (GETR 'TENSE C))
 ;VG TO THE CLAUSE IN WHICH IT
	    (COND ((MEMBER TENSE '((PRESENT) (IMPER) (INFINITIVE)) :TEST #'EQUAL)
		   T)
 ;IS EMBEDDED.
		  ((EQUAL TENSE '(MODAL))
...
(DEFUN SMONE2 (X)
       ...
       ;;IF ANY OF THOSE NOUN GROUPS COULD BE THE REFERENT FOR "
					    ONE
					    ".
	...
	UP   (COND ((NULL X) (RETURN NIL))
	     ...
	     ;;"			    ONE
					    " WE NOW PROCEED TO BUILD UP AN OSS FOR THE "
					    ONE
					    "
...
(DEFUN SMONE3 (ONENG)
    (DECLARE (SPECIAL ONENG))
       ;; SMONE3 TAKES AN NG WHICH IS A POSSIBLE REFERENT FOR "
					    ONE
					    ".
...
(DEFUN SMRELATE (NODE)
       ...
       ;;AND REPLACES SM WITH THE NEW LIST OF MODIFIED SS"
					    S.
					    THE
 ...
 ;SENSE OF THE VERB
									...
									(SM (SETQ WORD-BEING-INTERPRETED
								       ...
								       ((CADR (ASSQ (CAR (MEET FE
											       '(ITRNS TRANS ;WHICH WILL PROPABLY  VARY WITH ITS
 ;TRANSITIVITY. THE VALUE THAT IS FINALLY
												 INT ;DETERMINED REPRESENTS ALL POSSIBLE SENSES OF
 ;THE MEANING OF THE WORD THAT ARE APPROPRIATE TO
												 TRANSL ;THE TRANSITIVITY FIGURED OUT BY THE SYNTACTIC
												 TRANS2 ;PROGRAMS
												 THERE ITRNSL)))
										    (SM	(SETQ WORD-BEING-INTERPRETED
												   (GETR 'MVB
