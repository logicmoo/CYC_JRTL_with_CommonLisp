...
(DEFUN PARSE2 (RST P)
       ...
       ;;FIRST MEMBER OF RST - A FEATURE LIST THE PARAMETER P
       ...
       (DECLARE (SPECIAL RST))
       (PROG (UNIT CREATED-NODE END PARENT RE SPESIAL NBB)
             (DECLARE (SPECIAL PARENT RE SPESIAL))
	     (SETQ UNIT (CAR RST) LEVEL (+ 1 LEVEL))
	     ...
	     (COND ((NQ B-SPESIAL)
		    (AND PARSETRACE
			 (PROGN (PRINTC '\ \ SPESIAL\ WORD)
				(PRINC (CAR N))
				(TERPRI)))
		    (EVAL (GETR 'B-SPESIAL N))))
	     (COND ((EQ SPESIAL 'SKIP) (GO SKIP))
		   ((EQ SPESIAL 'DONE) (GO DONE))
		   ((EQ SPESIAL 'LOSE) (SETQ N NBB) (GO LOSE)))
	     (AND PARSETRACE
		  (PROGN (TERPRI)
			 (PRINC '\>)
			 (PRINC LEVEL)
	...
	SKIP (COND ((EQ N CUT))
		   ((NQ SPESIAL) (EVAL (GETR 'SPESIAL N))))
	DONE (OR P
...
(DEFUN PARSE3 (RST P)
       ...
       ;;SENTENCE
       (PROG (XP LABL RE SPESIAL NBB NODE)
	     (COND ((EQ (SETQ NBB N) CUT) (MQ CUT) (RETURN NIL))
		   ((NQ B-SPESIAL)				       ;IS THE NEXT WORD MARKED SPECL?
		    (EVAL (GETR 'B-SPESIAL N))			       ;YES, DO SOMETHING SPECIALL
		    (COND ((EQ SPESIAL 'SKIP) (GO SKIP))
			  ((EQ SPESIAL 'LOSE)
			   (SETQ N NBB)
			   (RETURN NIL))
			  ((EQ SPESIAL 'DONE) (GO DONE)))))
	     (COND ((CAR (SETQ XP RST)))			       ;IF CALL IS (PARSE NIL FOO)
		   ((NEXTWORD? (CADR RST)) (GO OK))		       ;THEN LOOK FOR EXACT WORD "FOO"
	...
	OK   (SETQ 
	      ...
	      (BUILDNODE (MEET (APPEND (FE N) LABL)
			 ...
			 (OR SMN
			     ...
			     (CADR (OR (ASSOC (CAR RST)
					   (SM N) :TEST #'EQUAL)
				       (UNDEFINED))))))
	     (SETQ N (CDR N))
	SKIP (SETQ NN (NOT (EQ N CUT)))
	     (COND ((AND NN (NQ SPESIAL))
		    (EVAL (GETR 'SPESIAL N))))
