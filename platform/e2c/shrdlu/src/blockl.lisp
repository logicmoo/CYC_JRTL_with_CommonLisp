...
#-USE-DEFUN-FEXPR
(PROGN (DEFMACRO GOAL (&REST X) (LIST 'APPLY-GOAL (LIST 'QUOTE X)))
 (DEFUN APPLY-GOAL (X)
      (SETQ PLAN NIL)
      (THVAL (LIST 'THGOAL (CAR X)
               '(THTBF THTRUE))
        '((EV COMMAND)))
   (EVLIS (REVERSE PLAN))) 'GOAL)
...
(DEFUN-FEXPR MEMOREND (A) (OR NOMEM (AND (THSETF (G3T (THV EV) 'END) THTIME)
					 (APPLY-THASSERT
...
(DEFUN STARTHISTORY
 ...
 (SETQ THTIME 0)
 (SETQ GRASPLIST NIL)
 (THSETF (G3T 'EE 'WHY) 'COMMAND)
 (THSETF (G3T 'EE 'START) 0)
 (THSETF (G3T 'EE 'END) 0)
 (THSETF (G3T 'EE 'TYPE) '\#START)
 (SETQ EVENTLIST '(EE))
