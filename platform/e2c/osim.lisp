...
(define sim-eval (&rest all) 
  (format *terminal-io* "~&~s~&" (cons 'sim-eval all)) 
...
(define to-req-string (list) 
 ...
 ;;;;  (sim-assert `(#$afterAdding #$simRelation (#$SubLQuoteFn SIM-AFTER-ADDING-PROPERTY-TOSTRING)) ,*CurrentStateMt*)
  ;;;; (sim-assert `(#$afterRemoving #$simRelation (#$SubLQuoteFn SIM-AFTER-REMOVING-PROPERTY-TOSTRING)) ,*CurrentStateMt*)
  ;;;; (DEFINE-AFTER-ADDING  SIM-AFTER-ADDING-PROPERTY-TOSTRING (str asrt) (ret (sim-eval (list :add str (assertion-el-formula asrt) (assertion-el-Mt asrt)))))
  ;;;; (DEFINE-AFTER-REMOVING  SIM-AFTER-REMOVING-PROPERTY-TOSTRING (str asrt) (ret (sim-eval (list :rem str (assertion-el-formula asrt) (assertion-el-Mt asrt)))))
...
(sim-assert '(#$arity #$SimEvalFn 1) *UVMt*)
...
(defmacro sim-warn (&rest code)  (ret 
    `(with-error-handler 
      #'(lambda () 
         ...
         (terpri)
         (format t "ERROR: ~s ~s" *ERROR-MESSAGE* ',code)
         (terpri)
         (force-output)
           #|(throw :UNEVALUATABLE T)|# )
       (sl::progn ,@code))))
