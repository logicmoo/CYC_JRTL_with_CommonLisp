#!/bin/bash
#
# $Id: cyc-runner.sh 152310 2014-07-02 18:46:28Z builder $
...
#you can also set CYC_JAVA_OPTIONS which does not get cleared in this file
...
PGC_FLAG= 
java -XX:+UseConcMarkSweepGC -version  > /dev/null 2>&1;
if [ $? -eq 0 ] && test -z "${CYC_SUPPRESS_CONCURRENT_GC}"; then
  PGC_FLAG=-XX:+UseConcMarkSweepGC
  echo "Running with concurrent garbage collector ..."
else
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "Running without parallel or concurrent garbage collector ..."
  fi
fi

OLD_SORT_FLAG=
java -Djava.util.Arrays.useLegacyMergeSort=true -version  > /dev/null 2>&1;
if [ $? -eq 0 ]; then
  OLD_SORT_FLAG=-Djava.util.Arrays.useLegacyMergeSort=true
  echo "Running old sort behavior ..."
else
  echo "Running with new sort behavior ..."
fi
...
# Launch the SCG database
if test -d data/scg-repo-v2; then
   DERBY_LAUNCHER=${JAVA_HOME}/db/bin/startNetworkServer
   if test -x ${DERBY_LAUNCHER}; then
      mkdir -vp log
      DERBY_LOGFILE=log/scg-repo-server-$(date +%Y%m%d_%H%M%S).log
      touch ${DERBY_LOGFILE}
      DERBY_LOGFILE_FQ=$(readlink -f ${DERBY_LOGFILE})
      echo 'Launching Derby to server up the SCG repository ....'
      echo '... logging to ' ${DERBY_LOGFILE_FQ} '....'
      echo "(cd data && ${DERBY_LAUNCHER} 2>&1 > ${DERBY_LOGFILE_FQ}) &"
      export DERBY_OPTS="-Dderby.drda.host=0.0.0.0" 
      (cd data && ${DERBY_LAUNCHER} 2>&1 > ${DERBY_LOGFILE_FQ}) &
      # @todo test whether that actually worked
   else
      echo 'Java install contains no Derby Databse support ... cannot not support SCG Repository.'
   fi 
else 
   echo 'Release contains no SCG repository ....'
fi

# Ready to start CYC
echo "Starting Cyc with command"
echo "java ${BIT_FLAG} ${SERVER_FLAG} -Xms${MIN_HEAP} -Xmx${MAX_HEAP} ${CODE_CACHE_FLAG} ${PERM_SIZE_FLAG} \
${EA_FLAG} ${CM_FLAG} ${PGC_FLAG} ${FAST_OPTS_FLAG} ${AGENT_LIB_FLAG} ${EXTRA_OPTIONS} ${CYC_JAVA_OPTIONS} \
${LOG_FLAG} ${ASSERTS_FLAG} -cp ${CLASSPATH} ${MAIN_CLASS} -f ${INIT_FORM} $@"

java ${BIT_FLAG} ${SERVER_FLAG} -Xms${MIN_HEAP} -Xmx${MAX_HEAP} ${OLD_SORT_FLAG} ${CODE_CACHE_FLAG} ${PERM_SIZE_FLAG} \
${EA_FLAG} ${CM_FLAG} ${PGC_FLAG} ${FAST_OPTS_FLAG} ${AGENT_LIB_FLAG} ${EXTRA_OPTIONS} ${CYC_JAVA_OPTIONS} \
${LOG_FLAG} ${ASSERTS_FLAG} -cp "${CLASSPATH}" ${MAIN_CLASS} -f "${INIT_FORM}" "$@"
CYC_RC=$?

if test -n "${DERBY_LAUNCHER}"; then
   DERBY_SHUTDOW="$(dirname "${DERBY_LAUNCHER}")/stopNetworkServer"
   echo 'Shutting down Derby which provides the SCG repository ....'
   ${DERBY_SHUTDOW}
   echo '... see' ${DERBY_LOGFILE_FQ} ' for log output.'
fi

# pass back the CYC return code
exit $CYC_RC