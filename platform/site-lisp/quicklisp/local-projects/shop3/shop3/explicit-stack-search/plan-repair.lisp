...
Modified search state object."
  (let* ((world-state (world-state search-state))
         ...
         (new-state-obj (shop3.common::copy-state world-state)))
    ...
    (shop3.common:retract-state-changes new-state-obj (1+ world-state-tag))
    ...
    (let ((new-tag 
            (shop3.common:tag-state new-state-obj 1)))
      (iter (for (op fact) in divergence)
        (ecase op
          (:add (shop3.common:add-atom-to-state fact new-state-obj 0 :execution-divergence))
          (:delete (shop3.common:delete-atom-from-state fact new-state-obj 0 :execution-divergence))))
      ;; now make it impossible to backtrack before this point...
      (setf (shop3.common::tagged-state-block-at new-state-obj) new-tag))
    ...
    (let ((suffix
            (reverse
             (subseq (shop3.common::tagged-state-tags-info world-state)
                     0
                     (or (position world-state-tag (shop3.common::tagged-state-tags-info world-state)
                                   :key 'first)
