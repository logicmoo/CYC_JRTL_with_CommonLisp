 ...
 *  
 */
public class BracketPairMatcher implements ICharacterPairMatcher {
    ...
    protected LispCodeReader fReader = new LispCodeReader();

    public BracketPairMatcher() {
        fPairs = new char[] { '(', ')'};
    }
...
*/
    ...
    protected int searchForClosingPeer(int offset, int openingPeer,
        ...
        int stack = 1;
        int c = fReader.read();
        while (c != LispCodeReader.EOF) {
            if (c == openingPeer && c != closingPeer)
                stack++;
            else if (c == closingPeer) stack--;

            if (stack == 0) return fReader.getOffset();
