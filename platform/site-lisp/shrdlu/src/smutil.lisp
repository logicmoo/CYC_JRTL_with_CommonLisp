...
(PROGN
      ...
      (DEFUN APPLY-BUILD-SHRDLU	 (%L)
	    (PROG
		  ...
		  (MAPC #'(LAMBDA (%PROP) (AND (SETQ %X (EVAL %PROP)) (S3TF NODE= %PROP %X))) %PROPS)
...
(DEFUN NEWCOPY	(OSS)
 ...
 ;AND GETS THE ENTIRE PROPERTY LIST IN OUR LISP.
		   ...
		   (RETURN NEW))
		  ((EQ (CAR OLD) 'PNAME))
		  ((S3TF NEW (CAR OLD) (CADR OLD))))
	    (SETQ OLD (CDDR OLD))
	    (GO UP)))

;;;=============================================================================
(setq RESTRICTIONS\: nil)
...
(DEFUN DOBACKREF  (ANSWER)
      ...
      (PROG (ANSRSS)
	    (SETQ ANSRSS (G3T ANSWER 'ANSRSS=))
	    (SETQ BACKREF2 BACKREF)
	    (SETQ BACKREF NIL)
	    (SETQ LASTREL (REL? ANSRSS))
	    (MAPC #'(LAMBDA (PRONOUN)
			 (S3TF PRONOUN 'LASTBIND
				 (G3T	PRONOUN
					'BIND))
 ...
 ;IF NODE HAS REFERENT, FINE
				     (ELSE
				      (S3TF (CAR (SM BACKNODE)) 'REFER=
					      (OR (G3T (VARIABLE? (CAR (SM BACKNODE)))
							 'BIND)
...
(DEFUN VALUEPUT NIL (S3TF (CDR (LIST THVALUE)) 'WHO SENTNO))
...
(DEFUN PLNR-GOALIFY  (PLNRPHRASE)
 ...
 ;PRESENT TENSE TIME MARKERS ARE REMOVED TO
      (COND ((G3T (CAR PLNRPHRASE) 'NOGOAL) PLNRPHRASE)
...
(DEFUN PLNR-RECOMMENDIFY  (%PLNRPHRASE)
      ...
      (PROG (%ELEMENT)
	    (RETURN
		  (AND (SETQ %ELEMENT (G3T (CAR %PLNRPHRASE)
					     'THMLIST))
...
(DEFUN COMPARE-BUILD  (NODE DEGREE)
 ...
 ;THESE ARE THE POSSIBLE PARTS OF A MEASURE
	    (SETQQCHECK	NIL
			...
			'MEASURE)
 ;DEFINITION
	    (S3TF 'COMPARE-PSEUDO-VERB 'SEMANTICS
		    (LIST 'RELATION
			  (LIST	'RESTRICTIONS\:
...
(DEFUN EXPAND  (EXP EVENT)
      ...
      (COND
	    ...
	    ((PROG (BODY QUANTIFIER CHOICE VAR MULTIPLE)
		   ...
		   (SETQ MULTIPLE (EVAL (G3T (CAR EXP) 'MULTIPLE)))
		   (SETQ
			 EXP
			      (MAPCAR
				    #'(LAMBDA (X)
...
;;;
 ...
 ; ADD NEW MARKER TO LIST
	    (SETQ NEW-SYSTEMS (G3T MARKER 'SYS))
