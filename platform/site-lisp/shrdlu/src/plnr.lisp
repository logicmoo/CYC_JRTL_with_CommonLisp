...
(DEFUN-FEXPR
 ...
 ;THVALUE OF $?X TO BE THE FIRST INPUT TO THAMONG. THXX SET ;TO
	    ((EQ (CADR (SETQ THXX (THGAL (COND ((EQ (CAAR THA)
 ;OLD BINDING CELL OF $?X (OR $E$?X) IF $?X

						    ;;VALUES PUSHED ONTO THTREE AND THAMONG FAILS TO
						    '
						     THEV)
 ;THUNASSIGNED, OLD VALUE AND LIST OF NEW
...
(DEFUN THAMONGF ;(CAR THTREE) = (THAMONG OLDBINDINGCELL (NEW
 ...
 ;NO NEW VALUES LEFT. RETURN X TO THUNASSIGNED,
...
(DEFUN THASS1
      ...
      (PROG (THX THY1 THY TYPE PSEUDO)
	    ...
	    (AND THTRACE
		 (NOT PSEUDO)
		 (THTRACES (COND (P 'THASSERT)
				 ('THERASE))
			   THX))
	    (SETQ THA (COND (PSEUDO (CDDR THA))
			    ((CDR THA))))
...
(DEFUN-FEXPR THASSERT (THA) (THASS1 THA T))
 ;THASS1 IS USED FOR BOTH ASSERTING AND ERASING,
...
(DEFUN-FEXPR
      THBKPT
      (L)
      (OR (AND THTRACE (THTRACES 'THBKPT L))
	  THVALUE))
...
(DEFUN-FEXPR
      ...
      (AND THA
	   (PROG (THTREE1 THA1 THX)
		 F
		 (SETQ THA1 (COND ((EQ (CAR THA) 'THEOREM)
				   'THPROG)
				  ((EQ (CAR THA) 'THTAG)
				   'THPROG)
				  ((EQ (CAR THA) 'THINF)
		 ...
		 (RETURN NIL)
		 TAGS
		 (SETQ THX (CADDAR THTREE1))
		 LP2
		 (COND ((NULL THX) (GO ALP1))
	    ...
	    (LIST 'THFIND
		  (COND	((EQ (CAR THA) 'ALL) '(1. NIL NIL)) ;STANDARD ALL
			...
			((EQ (CAAR THA) 'AS-MANY-AS)
			 (LIST 1. (CADAR THA) T))
			(T
			 (CONS (CADAR THA) ;ONLY THING LEFT IS AT-LEAST
...
(DEFUN-FEXPR
      THFLUSH ;(THFLUSH) FLUSHES ALL ASSERTIONS AND THEOREMS
...
(DEFUN-FEXPR
 ...
 ;PLANNER VARIABLE OR THVAL OF $E... THA2 =
...
(DEFUN THIP
      ...
      (PROG
	    ...
	    (COND ((NOT (SETQ THT1 (G3T THI1 THWH)))
...
(DEFUN THMATCHLIST
      ...
      (PROG (THB1 THB2 THL THNF THAL THA1 THA2 THRN THL1 THL2 THRVC)
	    ...
	    THP1
	    (OR	THB1
		(RETURN	(COND (THL2 (APPEND THL1 THL2))
			      (THL1))))
	    ...
	    (COND ((NOT (SETQ THA2 (G3T 'THVRB THWH)))
		   (SETQ THA2 '(0. 0.)))
...
(DEFUN THNOFAIL	 (THX)
      (COND (THX (S3TF 'THPROG 'THFAIL 'THPROGT))
	    (T (SET (G3T 'THPROG 'THFAIL) 'THPROGF))))
...
(DEFUN THREMBINDF  NIL
      (SETQ THALIST (CADAR THTREE))
      (THPOPT)
      NIL)

(DEFUN THREMBINDT  NIL
      (SETQ THALIST (CADAR THTREE))
      (THPOPT)
      THVALUE)
...
(DEFUN-FEXPR
 ...
 ;IN REREADABLE FORM. NOTE THAT		 ;IT IS  ;BLIND TO ASSERTIONS THAT BEGIN WITH
      (PROG
	    (THP)
	    ;;EITHER NUMBERS, LIST STRUCTURE, NOHASHED ATOMS
...
(DEFUN THTAE
      ...
      (COND
	    ...
	    (T
	     (PRINT XX)
	     (THTAE (THERT UNCLEAR RECCOMMENDATION \-THTAE)))))
...
(DEFUN THVAL
      ...
      (PROG (THTREE THBRANCH THOLIST THABRANCH THE THMESSAGE THE2)
	    (DECLARE
		  (SPECIAL THTREE THOLIST THMESSAGE THBRANCH THABRANCH))
	    ...
	    (COND ((NULL THTREE)
		  ...
		  ((SETQ THEXP (G3T (CAAR THTREE) 'THSUCCEED))
		   (GO GO2))
	    ...
	    (COND ((NULL THTREE)
		  ...
		  ((SETQ THEXP (G3T (CAAR THTREE) 'THFAIL))
		   (GO GO2))
