...
(DEFUN ANSTHM  (EXP)
      ...
      (PROG (NEG VARLIST BODY)
	    ...
	    (COND
		  ...
		  ((NOT (G3T (CAR EXP) 'TELLABLE)) (NOTELL)) ;ASSERTION IT CAN HANDLE.  IT USES GLOBAL-ERR
		  (T
...
;;;############################################################
      ...
      (DEFUN DISPUT  (ASSERTION)
	    ...
	    (OR (NOT DISCOURSE) (S3TF (CDR ASSERTION) 'WHO SENTNO)))
   ...
   (PROG (TYPE\: TYPELIST TYPE NAME\: COLOR\: COLORLIST SIZE\:
     ...
     (DECLARE (SPECIAL TYPE\: TYPELIST TYPE NAME\: COLOR\:
	  ...
	  (COND	((EQ (SETQ TYPE (CADDAR TYPE\:)) '\#NAME) ;KEPT, AND IF SO PUTS THE RELEVANT SENTENCE
		...
		((MEMQ '\#PROPERTY (G3T TYPE 'SYS)) ;IS ITS OWN NAME
      ...
      (DEFUN ONECHECK  (ITEM)
	    ...
	    (PROG (ANS OLD NEW)
		  ...
		  (OR (EQ (CAR OLD) (CAR NEW))
		      (EQ (CAR OLD) (G3T (CAR NEW) 'ROOT))
		      (EQ (CAR NEW) (G3T (CAR OLD) 'ROOT))
...
;;;############################################################
      ...
      (DEFUN FINDCHOOSE	 (OSS X ANS2)
	    ...
	    (PROG (HAVE NEED XX ANS PLNRCODE LOOP)
		  ...
		  UP
		  (COND	((G3T (CAR LOOP) 'REFER)
			 (SETQ ANS (APPEND (G3T (CAR LOOP) 'REFER)
					   ANS)))
			((SETQ XX
...
;;;  (PROG (Y)
;;;	(SETQ Y
;;;	      (COND ((OR (EQ X (G3T (Q IT) (Q LASTT)))
;;;			 (EQ X (G3T (Q THEY) (Q LASTT))))
;;;		     (Q (THE SAME THING)))
