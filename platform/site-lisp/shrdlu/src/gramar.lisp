...
;;;
	 ...
	 SUB2
	 (: (SHRDLU-POP) NIL FAIL) ;POINT TO IT AND ATTEMPT A NEW PARSING IF
...
;;;
	 (: (SETMVB (COMBINATION? (ROOT (NB MVB)) (WORD (NB H)))) ;IS THIS A LEGITIMATE COMBINATION OF VERB AND
	    ...
	    POPRT)
	 DPRT
	 (: (ISQ H PASV) CHECKPASV NIL) ;SEARCH FOR DISPLACED PARTICLE  NO DISPLACED
...
;;;
 ...
 ;BE REMOVED IF THE PREPG DISCOVERS IT CAN'T FIND
	 PONT
	 (AND (NEXTWORD? 'BY) (PARSE PREPG AGENT) (FQ AGENT)) ;AN OBJECT (THE REMOVING WILL BE DONE WHILE IN
...
;;;************************************************************************************
	 TONT
	 (: (SETQ POSITION-OF-PTW N) NIL RETSM RETSM) ;WE ARE USING THE SAME TECHNIQUE HERE AS WITH
...
;;;************************************* PREPG
	 NPASV
	 (: (AND (NQ PREP) (PARSE PREPG) (CALLSM (SMRELATE H)))
	    NIL
	    NIL
	    RETSM)
...
;;;***************************** PREP QUESTION
 ...
 ;MAKE USE OF THE COMMA CLUE. PREPQ IS HANDLED
	 (GO POLAR) ;MUCH LIKE QADJS LIKE WHEN AND WHERE THE R3ST OF
...
;;;--------------- ORDINALS AND NUMBERS ---------------
	 ...
	 DET1
	 (COND ((ISQ H NS) (FQ NS))
	       (T (FQ NPL))) ;EXPLICIT CHECK FOR THE VALUE 1
...
;;;--------------- PREPG WITH "OF" ---------------
	 OF
	 (: (AND (NQ OF) (PARSE PREPG OF)) SMOF NONE) ;"FIVE OF THE BLOCKS"
	 SMOF
	 (FQ OF)
...
;;;--------------- YOU PROBABLY GOOFED, CUT AND TRY AGAIN. --------------
	 ...
	 (CUT N)
	 UP
	 (: (SHRDLU-POP) UP NIL) ;POP EVERYTHING OFF
...
;;;							       ;CERTAIN RESTRICTIONS PLACED ON THE POSSIBLE
	 QUEST
	 (: (CQ QUEST) NIL NG) ;NOUN GROUPS THAT IT CAN TAKE - HENSE THE
 ;SPECIAL CALL TO PARSE AS ABOVE, IF THE PREPG IS
	 ...
	 REL
	 (: (NEXTWORD? 'WHICH) NIL R3ST) ;NOUN GROUP - NO RESTRICTIONS
...
;;;
	 POPAD
	 (SHRDLU-POP) ;IF THE CUT POINT WAS HIT HAVING ONLY PARSED
 ...
 ;AND CONTINUE
	    ...
	    (PDEFINE
	     ...
	     (:	(COND ((MEMQ (CAR R3ST)
		       ...
		       (PARSE3 (APPEND R3ST '(COMPONENT)) NIL))
		      ((MEMQ (CAR R3ST) '(NG PREPG ADJG))
		       (AND (NOT (CQ OFOBJ))
			    (PARSE2 (APPEND R3ST '(COMPONENT))
				    NIL)))
		      ((EQ (CAR R3ST) 'CLAUSE)
		       ((LAMBDA	(LASTSENT AUXFE)
			      (AND (PARSE2 (APPEND R3ST
						   AUXFE
						   '(COMPONENT))
					   NIL)
		...
		LOSE2)
	     ...
	     (: (CQ LISTA) LISTA NIL)
	     LIST
	     (:	(AND (EQ PREV '|,|) ;COME HERE FOR ABORTED LIST AND CHECK FOR
	     ...
	     (PROG (END)
		   ...
		   (RETURN (PROG (CUT NBB BOTH)
				 ...
				 (AND (FLUSHME)
				      ...
				      (SETQ RE (COND ((MEMQ (CAR R3ST) ;WORD
							    '(PREP ADV))
						      (PARSE3 R3ST T))
						     ((MEMQ (CAR R3ST)
							    '(NG
