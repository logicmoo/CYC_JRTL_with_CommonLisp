...
;;;
;;; cusp-lispworks.lisp --- LispWorks specific code for SLIME. 
;;;
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (require "comm")
  (import-from :stream *gray-stream-symbols* :cusp-backend))

(import-cusp-mop-symbols :clos '(:slot-definition-documentation
...
(defun cusp-mop:slot-definition-documentation (slot)
  (documentation slot t))

(defun cusp-mop:slot-boundp-using-class (class object slotd)
  (clos:slot-boundp-using-class class object
                                (clos:slot-definition-name slotd)))

(defun cusp-mop:slot-value-using-class (class object slotd)
  (clos:slot-value-using-class class object
                               (clos:slot-definition-name slotd)))

(defun (setf cusp-mop:slot-value-using-class) (value class object slotd)
  (setf (clos:slot-value-using-class class object
                                     (clos:slot-definition-name slotd))
        value))

(defun cusp-mop:slot-makunbound-using-class (class object slotd)
  (clos:slot-makunbound-using-class class object
                                    (clos:slot-definition-name slotd)))

(defun cusp-mop:compute-applicable-methods-using-classes (gf classes)
...
(deftype cusp-mop:eql-specializer () 'cons)

(defun cusp-mop:eql-specializer-object (eql-spec)
  (second eql-spec))
...
(defun set-sigint-handler ()
  ;; Set SIGINT handler on CUSP request handler thread.
  #-win32
...
(defmethod env-internals:environment-display-notifier 
  ...
  (funcall (cusp-sym :cusp-debugger-hook) condition *debugger-hook*)
  ;;  nil
  )
...
(defmethod env-internals:confirm-p ((e slime-env) &optional msg &rest args)
  (apply (cusp-sym :y-or-n-p-in-emacs) msg args))
...
(defmacro with-cusp-compilation-unit ((location &rest options) &body body)
...
(defimplementation cusp-compile-file (input-file output-file
  ...
  (with-cusp-compilation-unit (input-file)
...
(defimplementation cusp-compile-string (string &key buffer position filename
  ...
  (let* ((location (list :emacs-buffer buffer position string))
    ...
    (with-cusp-compilation-unit (location)
...
(defun cusp-sym (name) (find-symbol (string name) :cusp))
