...
;;; are adapted from other cusp-*.lisp, in particular from
;;; cusp-allegro (I don't use allegro at all, but it's the shortest
...
;;;; build up a "fake" cusp-mop and then override the methods in the
;;;; inspector.
...
#+#.(cl:if cusp-backend::*have-mop* '(cl:and) '(cl:or))
(progn
...
#-#.(cl:if cusp-backend::*have-mop* '(and) '(or))
(defclass cusp-mop:standard-slot-definition ()
  ()
  (:documentation
   "Dummy class created so that cusp.lisp will compile and load."))
...
(when (find-package :cusp-loader)
  (setf (symbol-function (intern "USER-INIT-FILE" :cusp-loader))
        (lambda ()
          (let ((home (user-homedir-pathname)))
            (and (ext:probe-directory home)
                 (probe-file (format nil "~A/.cusp.lisp"
