;;; cusp-arglists.lisp --- arglist related code ??
;;
...
proper form spec for further processing within CUSP. Returns NIL
...
As a secondary value, return whether &allow-other-keys appears somewhere."
  (let ((keywords '())
    ...
    (dolist (method methods)
	  ...
	  (arglist-keywords
	   (cusp-mop:method-lambda-list method))
...
As a secondary value, return whether &allow-other-keys appears somewhere."
  (methods-keywords 
   (cusp-mop:generic-function-methods generic-function)))
...
whether &allow-other-keys appears somewhere."
  (methods-keywords
   (multiple-value-bind (amuc okp)
       (cusp-mop:compute-applicable-methods-using-classes
...
(defun class-from-class-name-form (class-name-form)
  (when (and (listp class-name-form)
    ...
    (let* ((class-name (cadr class-name-form))
      ...
      (when (and class
                 (not (cusp-mop:class-finalized-p class)))
        ;; Try to finalize the class, which can fail if
        ;; superclasses are not defined yet
        (handler-case (cusp-mop:finalize-inheritance class)
          (program-error (c)
            (declare (ignore c)))))
      class)))
...
(defun extra-keywords/slots (class)
  (multiple-value-bind (slots allow-other-keys-p)
      (if (cusp-mop:class-finalized-p class)
          (values (cusp-mop:class-slots class) nil)
          (values (cusp-mop:class-direct-slots class) t))
    (let ((slot-init-keywords
           (loop for slot in slots append 
                 (mapcar (lambda (initarg)
                           (make-keyword-arg 
                            initarg
                            (cusp-mop:slot-definition-name slot)
                            (cusp-mop:slot-definition-initform slot)))
                         (cusp-mop:slot-definition-initargs slot)))))
        ...
        (multiple-value-bind (slot-init-keywords class-aokp)
          ...
          (multiple-value-bind (allocate-instance-keywords ai-aokp)
            ...
            (multiple-value-bind (initialize-instance-keywords ii-aokp)
                (applicable-methods-keywords 
                 #'initialize-instance (list (cusp-mop:class-prototype class)))
              (multiple-value-bind (shared-initialize-keywords si-aokp)
                  (applicable-methods-keywords 
                   #'shared-initialize (list (cusp-mop:class-prototype class) t))
...
(defmethod arglist-dispatch ((operator-type (eql :function)) (operator (eql 'declare))
                             arguments &key (remove-args t))
  ;; Catching 'DECLARE before CUSP-BACKEND:ARGLIST can barf.
