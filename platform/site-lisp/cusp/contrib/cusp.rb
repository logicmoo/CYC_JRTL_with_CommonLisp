...
# 1. Start the server with something like: ruby -r cusp -e cusp
...
$rpc_entries[:"cusp:connection-info"] = lambda do ||
    [:":pid", $$,
...
def cusp_interactive_eval(string)
  eval(string,TOPLEVEL_BINDING).inspect
end

$rpc_entries[:"cusp:interactive-eval"] = \
$rpc_entries[:"cusp:interactive-eval-region"] = \
$rpc_entries[:"cusp:pprint-eval"] = lambda { |string|
  cusp_interactive_eval string
}

$rpc_entries[:"cusp:throw-to-toplevel"] = lambda { 
  throw :cusp_top_level
}

$rpc_entries[:"cusp:backtrace"] = lambda do |from, to|
  conn, exc = $sldb_context
  conn.sldb_backtrace(exc, from, to)
end

$rpc_entries[:"cusp:frame-source-location"] = lambda do |frame|
  conn, exc = $sldb_context
  conn.frame_src_loc(exc, frame)
end

#ignored
$rpc_entries[:"cusp:buffer-first-change"] = \
$rpc_entries[:"cusp:operator-arglist"] = lambda do 
  :nil
end

$rpc_entries[:"cusp:simple-completions"] = lambda do |prefix, pkg|
  cusp_simple_completions prefix, pkg
end

# def cusp_simple_completions(prefix, pkg)
