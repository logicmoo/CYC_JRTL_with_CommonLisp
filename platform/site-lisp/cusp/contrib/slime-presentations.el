;;; cusp-presentations.el --- imitat LispM' presentations
;;;
...
(defun slime-inspect-presentation (presentation start end buffer)
  (let ((reset-p 
    ...
    (slime-eval-async `(cusp:inspect-presentation ',(slime-presentation-id presentation) ,reset-p)
...
(defun slime-M-.-presentation (presentation start end buffer &optional where)
  (let* ((id (slime-presentation-id presentation))
	 ...
	 (location (slime-eval `(cusp:find-definition-for-thing
				 (cusp::lookup-presented-object
...
(defun slime-describe-presentation (presentation)
  (slime-eval-describe 
     `(cusp::describe-to-string
       (cusp::lookup-presented-object ',(slime-presentation-id presentation)))))
...
(defun slime-menu-choices-for-presentation (presentation buffer from to choice-to-lambda)
  ...
  (let* ((what (slime-presentation-id presentation))
         (choices (with-current-buffer buffer
                    (slime-eval 
                     `(cusp::menu-choices-for-presentation-id ',what)))))
               ...
               (lambda (choice)
                 ...
                 (cons choice 
                       (savel `(lambda ()
                          (interactive)
                          (slime-eval 
                           '(cusp::execute-menu-choice-for-presentation-id
  ...
  (let ((id (slime-presentation-id presentation)))
    (etypecase id
      (number         
       ...
       (format "(cusp:get-repl-result #10r%d)" id))
      (list
       ;; for frame variables and inspector parts
       (format "(cusp:get-repl-result '%s)" id)))))
...
even on Common Lisp implementations without weak hash tables."
  (interactive)
  (slime-eval-async `(cusp:clear-repl-results))
  (unless (eql major-mode 'slime-repl-mode)
    (slime-switch-to-output-buffer))
...
(defun slime-install-presentations ()
  (slime-eval-async '(cusp:cusp-require :cusp-presentations)))
