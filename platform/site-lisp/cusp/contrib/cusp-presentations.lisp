;;; cusp-presentations.lisp --- imitate LispM's presentations
;;
...
(defun present-repl-results (values)
  ;; Override a function in cusp.lisp, so that 
...
(defun cusp-ioify (thing)
  (cond ((keywordp thing) thing)
	((and (symbolp thing)(not (find #\: (symbol-name thing))))
	 (intern (symbol-name thing) 'cusp-io-package))
	((consp thing) (cons (cusp-ioify (car thing)) (cusp-ioify (cdr thing))))
	(t thing)))
...
(defun execute-menu-choice-for-presentation-id (id count item)
  (let ((ob (lookup-presented-object id)))
    ...
    (let ((action (second (nth (1- count) (cdr *presentation-active-menu*)))))
      (cusp-ioify (funcall action item ob id)))))
