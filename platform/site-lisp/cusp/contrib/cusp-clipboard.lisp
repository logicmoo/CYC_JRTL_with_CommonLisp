;;; cusp-clipboard.lisp --- Object clipboard
;;
;; Written by Helmut Eller in 2008.
;; License: Public Domain

(defpackage :cusp-clipboard
  (:use :cl)
  (:import-from :cusp :defslimefun :with-buffer-syntax :destructure-case)
  (:export :add :delete-entry :entries :entry-to-ref :ref))

(in-package :cusp-clipboard)
...
(defslimefun add (datum)
  (let ((value (destructure-case datum
		 ...
		 ((:inspector part) 
		  (cusp:inspector-nth-part part))
		 ((:sldb frame var)
		  (cusp-backend:frame-var-value frame var)))))
    (clipboard-add value)
    (format nil "Added: ~a"
