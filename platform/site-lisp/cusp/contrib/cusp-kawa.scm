;;;; cusp-kawa.scm --- CUSP server for Kawa
;;;
...
;;      kawa -e '(compile-file "cusp-kawa.scm" "cusp-kawa")'
...
(defun kawa-slime-init (file _)
  ...
  (let ((zip ".../slime/contrib/cusp-kawa.zip")) ; <-- insert the right path
    (format "%S\n"
            `(begin (load ,(expand-file-name zip)) (start-cusp ,file)))))
|#
...
(module-export start-cusp create-cusp-server cusp-java-source-path)
...
(df create-cusp-server (port-number) 
  ...
  (mlet* ((charset "iso-8859-1")
    ...
    (while #t
      (mlet ((c . event) (recv* (append (list in out dbg listener)
        ...
        ;;(log "event: ~s\n" event)
        (mcase (list c event)
          ((_ (':emacs-rex ('|cusp:debugger-info-for-emacs| from to)
                           pkg thread id))
           (send dbg `(debug-info ,thread ,from ,to ,id)))
          ((_ (':emacs-rex ('|cusp:throw-to-toplevel|) pkg thread id))
           (send dbg `(throw-to-toplevel ,thread ,id)))
          ((_ (':emacs-rex ('|cusp:sldb-continue|) pkg thread id))
           (send dbg `(thread-continue ,thread ,id)))
          ((_ (':emacs-rex ('|cusp:frame-source-location| frame)
			   pkg thread id))
           (send dbg `(frame-src-loc ,thread ,frame ,id)))
          ((_ (':emacs-rex ('|cusp:frame-locals-and-catch-tags| frame)
                           pkg thread id))
           (send dbg `(frame-details ,thread ,frame ,id)))
          ((_ (':emacs-rex ('|cusp:sldb-disassemble| frame)
                           pkg thread id))
           (send dbg `(disassemble-frame ,thread ,frame ,id)))
          ((_ (':emacs-rex ('|cusp:backtrace| from to) pkg thread id))
           (send dbg `(thread-frames ,thread ,from ,to ,id)))
          ((_ (':emacs-rex ('|cusp:list-threads|) pkg thread id))
           (send dbg `(list-threads ,id)))
          ((_ (':emacs-rex ('|cusp:debug-nth-thread| n) _  _ _))
           (send dbg `(debug-nth-thread ,n)))
          ((_ (':emacs-rex ('|cusp:quit-thread-browser|) _  _ id))
           (send dbg `(quit-thread-browser ,id)))
          ((_ (':emacs-rex ('|cusp:init-inspector| str . _) pkg _ id))
           (set inspector (make-inspector user-env (vm)))
           (send inspector `(init ,str ,id)))
          ((_ (':emacs-rex ('|cusp:inspect-frame-var| frame var) 
                           pkg thread id))
           (mlet ((im . ex) (chan))
             (set inspector (make-inspector user-env (vm)))
             (send dbg `(get-local ,ex ,thread ,frame ,var))
             (send inspector `(init-mirror ,im ,id))))
          ((_ (':emacs-rex ('|cusp:inspect-current-condition|) pkg thread id))
           (mlet ((im . ex) (chan))
             (set inspector (make-inspector user-env (vm)))
             (send dbg `(get-exception ,ex ,thread))
             (send inspector `(init-mirror ,im ,id))))
          ((_ (':emacs-rex ('|cusp:inspect-nth-part| n) pkg _ id))
           (send inspector `(inspect-part ,n ,id)))
          ((_ (':emacs-rex ('|cusp:inspector-pop|) pkg _ id))
           (send inspector `(pop ,id)))
          ((_ (':emacs-rex ('|cusp:quit-inspector|) pkg _ id))
...
(df reader ((in <in>) (c <chan>))
  (! set-name (current-thread) "cusp-net-reader")
...
(df writer ((out <out>) (c <chan>))
  (! set-name (current-thread) "cusp-net-writer")
  (while #t
...
(df lookup-slimefun ((name <symbol>) tab)
  ;; name looks like '|cusp:connection-info|
...
(df listener ((c <chan>) (env <env>))
  (! set-name (current-thread) "cusp-listener")
  (log "listener: ~s ~s ~s ~s\n" 
       (current-thread) ((current-thread):hashCode) c env)
  (let ((out (make-cusp-outport (rpc c `(get-channel)))))
...
(defslimefun cusp-require (#!rest y) '())
...
(define-simple-class <cusp-location> (<location>)
  (file init: #f)
  (line init: #f)
  ((*init* file name) 
   (set (@ file (this)) file)
...
(df src-loc (o => <location>)
  (typecase o
    ...
    (<gnu.expr.GenericProc> (<cusp-location> #f #f))
    (<java.lang.Class> (class>src-loc o))
    (<kawa.lang.Macro> (<cusp-location> #f #f))))
...
(df class>src-loc ((c <java.lang.Class>) => <location>)
  (let* ((type (! reflectedType (as <com.sun.jdi.ClassObjectReference>
    ...
    (cond ((not (! isEmpty locs)) (1st locs))
          (#t (<cusp-location> (1st (! source-paths type #!null))
                                #f)))))
...
(defslimefun cusp-macroexpand-1 (env s) (%cusp-macroexpand s))
(defslimefun cusp-macroexpand (env s) (%cusp-macroexpand s))
(defslimefun cusp-macroexpand-all (env s) (%cusp-macroexpand s))

(df %cusp-macroexpand (string)
...
(define-simple-class <cusp-writer> (<java.io.Writer>)
...
(df cusp-writer ((in <chan>) (q <queue>))
  (! set-name (current-thread) "cusp-redirect-thread")
...
(df make-cusp-outport ((out <chan>))
  (let ((w (<cusp-writer>)))
    (mlet ((in . _) (spawn/chan (fun (c) (cusp-writer c (@ q w)))))
      (send in out))
...
(df vm-monitor ((c <chan>))
  (! set-name (current-thread) "cusp-vm-monitor")
  (let ((vm (vm-attach)))
    (log-vm-props vm)
    ;;(enable-uncaught-exception-events vm)
...
(define-simple-class <cusp-global-variable> ()
...
(df init-global-field ((vm <vm>))
  (when (nul? *global-get-mirror*)
    (set <cusp-global-variable>:var #!null) ; prepare class
    (let* ((c (as <com.sun.jdi.ClassType>
                  (1st (! classes-by-name vm "cusp$Mnglobal$Mnvariable"))))
           (f (! fieldByName c "var")))
      (set *global-get-mirror* (fun () (! getValue c f)))
      (set *global-set-mirror* (fun ((v <obj-ref>)) (! setValue c f v))))
    (set *global-get-raw* (fun () <cusp-global-variable>:var))
    (set *global-set-raw* (fun (x) (set <cusp-global-variable>:var x)))))
...
(define cusp-log-port (current-error-port))
(df log (fstr #!rest args)
  (synchronized cusp-log-port
    (apply format cusp-log-port fstr args)
    (force-output cusp-log-port))
  #!void)
...
;; Local Variables:
;; mode: goo 
;; compile-command:"kawa -e '(compile-file \"cusp-kawa.scm\"\"cusp-kawa\")'" 
;; End: