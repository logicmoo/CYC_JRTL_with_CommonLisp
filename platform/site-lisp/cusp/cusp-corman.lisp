;;;
;;; cusp-corman.lisp --- Corman Lisp specific code for SLIME.
;;;
...
(defclass cusp-mop:standard-slot-definition ()
  ()
  (:documentation "Dummy class created so that cusp.lisp will compile and load."))

(defun named-by-gensym-p (c)
  (null (symbol-package (class-name c))))

(deftype cusp-mop:eql-specializer ()
  '(satisfies named-by-gensym-p))

(defun cusp-mop:eql-specializer-object (specializer)
...
(defun cusp-mop:class-finalized-p (class)
  (declare (ignore class))
  t)

(defun cusp-mop:class-prototype (class)
  (make-instance class))

(defun cusp-mop:specializer-direct-methods (obj)
  (declare (ignore obj))
  nil)

(defun cusp-mop:generic-function-argument-precedence-order (gf)
  (generic-function-lambda-list gf))

(defun cusp-mop:generic-function-method-combination (gf)
  (declare (ignore gf))
  :standard)

(defun cusp-mop:generic-function-declarations (gf)
  (declare (ignore gf))
  nil)
...
(defun cusp-mop:slot-definition-type (slot)
  (declare (ignore slot))
  t)

(import-cusp-mop-symbols :cl '(;; classes
...
;; Hack to make cusp.lisp load, at least
...
(defmethod emacs-inspect ((class standard-class))
          `("Name: " (:value ,(class-name class))
            (:newline)
            "Super classes: "
            ,@(comma-separated (cusp-mop:class-direct-superclasses class))
            (:newline)
            "Direct Slots: "
            ,@(comma-separated
               (cusp-mop:class-direct-slots class)
               (lambda (slot)
                 `(:value ,slot ,(princ-to-string (cusp-mop:slot-definition-name slot)))))
            (:newline)
            "Effective Slots: "
            ...
            "Sub classes: "
            ,@(comma-separated (cusp-mop:class-direct-subclasses class)
                               (lambda (sub)
                                 `(:value ,sub ,(princ-to-string (class-name sub)))))
            (:newline)
            "Precedence List: "
            ,@(if (cusp-mop:class-finalized-p class)
                  (comma-separated (cusp-mop:class-precedence-list class)
                                         (lambda (class)
