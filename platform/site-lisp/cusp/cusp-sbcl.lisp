...
;;;
;;; cusp-sbcl.lisp --- SLIME backend for SBCL.
;;;
...
(import-from :sb-gray *gray-stream-symbols* :cusp-backend)
...
(import-cusp-mop-symbols :sb-mop '(:slot-definition-documentation))
...
#+#.(cusp-backend::with-symbol 'function-lambda-list 'sb-introspect)
...
#+#.(cusp-backend::with-symbol 'deftype-lambda-list 'sb-introspect)
...
(defun signal-compiler-condition (condition context)
  (signal (make-condition
           ...
           :severity (etypecase condition
                       ...
                       #+#.(cusp-backend::with-symbol redefinition-warning sb-kernel)
...
(defun get-compiler-policy (default-policy)
  ...
  #+#.(cusp-backend::with-symbol 'restrict-compiler-policy 'sb-ext)
...
(progn
  ...
  #+#.(cusp-backend::with-symbol 'who-specializes 'sb-introspect)
...
(defun ignored-xref-function-names ()
  #-#.(cusp-backend::sbcl-with-new-stepper-p)
...
(defimplementation condition-extras (condition)
  (cond #+#.(cusp-backend::sbcl-with-new-stepper-p)
...
(defimplementation call-with-debugger-hook (hook fun)
  (let ((*debugger-hook* hook)
    ...
    (handler-bind (#+#.(cusp-backend::sbcl-with-new-stepper-p)
...
(defimplementation frame-restartable-p (frame)
  #+#.(cusp-backend::sbcl-with-restart-frame)
...
(defun code-location-source-location (code-location)
  (let* ((dsource (sb-di:code-location-debug-source code-location))
    ...
    (if (getf plist :emacs-buffer)
        ...
        #+#.(cusp-backend::with-symbol 'debug-source-from 'sb-di)
...
(defun code-location-debug-source-name (code-location)
  (namestring (truename (#+#.(cusp-backend::with-symbol
                              'debug-source-name 'sb-di)
                             sb-c::debug-source-name
                             #-#.(cusp-backend::with-symbol
                                  'debug-source-name 'sb-di)
...
;;; source-path-file-position and friends are in cusp-source-path-parser
...
(progn
  ...
  (defimplementation thread-status (thread)
    (if (sb-thread:thread-alive-p thread)
        "RUNNING"
        "STOPPED"))
  #+#.(cusp-backend::sbcl-with-weak-hash-tables)
  (progn
