...
;;;
;;; cusp-abcl.lisp --- Armedbear CL specific code for SLIME. 
;;;
;;; Adapted from cusp-acl.lisp, Andras Simon, 2004
;;;
...
#-#.(cusp-backend::with-symbol '*invoke-debugger-hook* 'sys)
(defun sys::break (&optional (format-control "BREAK called") 
                   &rest format-arguments)
  (let ((sys::*saved-backtrace* 
         #+#.(cusp-backend::with-symbol 'backtrace 'sys) 
         (sys:backtrace)
         #-#.(cusp-backend::with-symbol 'backtrace 'sys)
...
(defimplementation call-with-debugging-environment (debugger-loop-fn)
  (let* ((magic-token (intern "CUSP-DEBUGGER-HOOK" 'cusp))
         (*sldb-topframe* 
          #+#.(cusp-backend::with-symbol 'backtrace 'sys)
...
(defun backtrace (start end)
  "A backtrace without initial CUSP frames."
  (let ((backtrace 
         #+#.(cusp-backend::with-symbol 'backtrace 'sys)
         (sys:backtrace)
         #-#.(cusp-backend::with-symbol 'backtrace 'sys)
...
part of *sysdep-pathnames* in cusp.loader.lisp. 
