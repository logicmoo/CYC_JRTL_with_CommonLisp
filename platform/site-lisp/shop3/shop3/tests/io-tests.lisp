...
(def-fixture op-def-domain ()
  (let ((domain (make-instance 'domain)))
    (setf (slot-value domain 'shop3::operators)
...
(test method-tests
  (with-fixture empty-domain ()
    (with-fixture method-def ()
      (is (equal (let ((meth-def (shop3::process-method *domain* meth)))
    ...
    (with-fixture complex-method-def ()
      (is
       (equal (let ((meth-def (shop3::process-method *domain* meth)))
              ...
              '(:method (find-movable)
                ...
                placeholder nil '(:ordered (:task shop3::!!inop))))))))
...
(test check-operator-definitions
  ...
  (let ((op (with-fixture op-def-domain ()
              (shop3::parse-domain-item domain :operator '(:operator (!!delete-truck ?truck)
                                                           ()
                                                           ((typevp ?truck truck))
                                                           ())))))
    (is (equal (shop3::operator-head op) '(!!delete-truck ?truck)))
    (is (null (shop3::operator-preconditions op)))
    (is (null (shop3::operator-additions op)))
    (is (equal (shop3::operator-deletions op) '((typevp ?truck truck))))
    (is (= (shop3::operator-cost-fun op) 1.0)))
  (let ((op (with-fixture op-def-domain ()
              (shop3::parse-domain-item domain :op '(:op (!!delete-truck ?truck)
                                                           :delete
                                                           ((typevp ?truck truck)))))))
    (is (equal (shop3::operator-head op) '(!!delete-truck ?truck)))
    (is (null (shop3::operator-preconditions op)))
  ...
  ;; here's a big one
  (let ((op (with-fixture op-def-domain ()
              (shop3::parse-domain-item domain :operator
                                        '(:operator (!takeoff ?p ?flight-alt ?earliest-start ?start ?end)
                                          ...
                                          ;; cost
                                          0)))))
    (is (equal (shop3::operator-head op) '(!takeoff ?p ?flight-alt ?earliest-start ?start ?end)))
    (is (equal (shop3::operator-preconditions op)
        '(
                                           (at ?p (pos ?north ?east ?alt)) ; a/c starts at alt == 0
                                           (= 0 ?alt)
                                           (fuel ?p ?fuel)
    ...
    (is (equal (shop3::operator-additions op)
               '(
                                           ;; update fuel and position
                                           (at ?p (pos ?north ?east ?flight-alt))
                                           (fuel ?p ?fuel-remaining)
    ...
    (is (equal (shop3::operator-deletions op) '(
                                           ;; update fuel and position
                                           (at ?p (pos ?north ?east ?alt))
                                           (fuel ?p ?fuel)
                                           ;; timelines for at update
    ...
    (is (= (shop3::operator-cost-fun op) 0)))
  (let ((op (with-fixture op-def-domain ()
              (shop3::parse-domain-item domain :op
