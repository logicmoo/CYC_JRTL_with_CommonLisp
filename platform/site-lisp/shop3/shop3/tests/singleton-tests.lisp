...
(defun check-operator (sexp name)
  (shop3::set-variable-property (make-instance 'domain) sexp)
  (let ((table (shop3::harvest-variables sexp)))
    (with-singleton-warnings 
      (shop3::check-for-singletons table :construct-type :operator :construct-name name))))

(defun check-method (sexp name)
  (shop3::set-variable-property (make-instance 'domain) sexp)
  (let ((table (shop3::harvest-variables sexp)))
    (with-singleton-warnings 
      (shop3::check-for-singletons table :construct-type :method :construct-name name))))

;;; better alternative than check-method...
(defun test-process-method (sexp)
  (shop3::set-variable-property (make-instance 'domain) sexp)
  (with-singleton-warnings 
    (shop3::process-method (make-instance 'domain) sexp)))

(defun test-process-operator (sexp)
  (shop3::set-variable-property (make-instance 'domain) sexp)
  (with-singleton-warnings 
    (shop3::process-operator (make-instance 'domain) sexp)))

(defun test-process-axiom (sexp)
  (let ((d (make-instance 'domain)))
    (shop3::set-variable-property d sexp)
    (with-singleton-warnings 
      (shop3::process-axiom d sexp))))
...
(fiveam:test blort-singleton
  (let ((warning 
    ...
    (fiveam:is (eq (shop3::construct-type warning) :operator))
    (fiveam:is (eq (shop3::construct-name warning) '!assign-tanker-to-fire))
    (fiveam:is (equal (shop3::variable-names warning)
                      (list '?blort)))))
...
(fiveam:test method-with-singleton
   (let ((warning 
     ...
     (fiveam:is (eq (shop3::construct-type warning)
                    :method))
     (fiveam:is (eq (shop3::construct-name warning)
                    'assess-fire))
     (fiveam:is (equal (shop3::variable-names warning)
                       '(?fa)))))
...
(fiveam:test anonymous-variables-axiom
  (let ((warning
    ...
    (fiveam:is-true warning)
    (fiveam:is (eq (shop3::construct-type warning) ':-))
    (fiveam:is (eq (shop3::construct-name warning) 'foo))
    (fiveam:is (equal (shop3::variable-names warning) (list '?x))))
  (let ((warning
          (nth-value 1
                     (test-process-axiom
                      '(:- (foo)
