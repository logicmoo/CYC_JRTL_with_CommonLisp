...
(defmethod regularize-axiom ((domain domain) axiom)
  (if (shop3-axiom-p axiom) axiom       ; SHOP2 syntax
...
(defun shop3-axiom-p (ax)
  (if (not (listp ax)) nil
      (let ((lax (length ax)))
        (if (<= lax 3) nil (rest-shop3-axiom-p (cddr ax))))))

(defun rest-shop3-axiom-p (ax)
  (let ((lax (length ax)))
    (cond ((< lax 2) nil)
          ((= lax 2) (if (and (atom (car ax)) (listp (cadr ax))) t nil))
          ((= lax 3) nil)
          (t (and (atom (car ax)) (listp (cadr ax)) (rest-shop3-axiom-p (cddr ax)))))))
