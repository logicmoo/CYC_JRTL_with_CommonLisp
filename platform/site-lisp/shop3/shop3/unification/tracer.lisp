...
;;; markings.
(in-package :shop3.unifier)

(defvar *shop-trace* nil)
(defvar *shop-trace-stream* t)
(defvar *trace-query* nil)
...
(defmacro trace-print (type item state &rest formats)
  `(when *shop-trace*
     (when (or (member ,type *shop-trace*) (member ,item *shop-trace*)
       ...
       ,(let ((cpack (find-package :shop3.common)))
          (when cpack
            (let ((s-a (intern (symbol-name '#:state-atoms) cpack)))
              (when (fboundp s-a)
                `(when *trace-query*
                   (funcall (fdefinition 'trace-query-hook) ,type ,item
                            (list ,@formats) (,s-a ,state)))))))
