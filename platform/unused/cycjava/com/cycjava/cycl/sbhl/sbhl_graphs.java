...
public  final class sbhl_graphs extends SubLTranslatedFile {
  ...
  public static final SubLObject declare_sbhl_graphs_file() {
    //declareFunction(myName, "optimize_sbhl_store", "OPTIMIZE-SBHL-STORE", 0, 0, false);
    //declareFunction(myName, "sbhl_graph_object_p", "SBHL-GRAPH-OBJECT-P", 1, 0, false);
    declareFunction(myName, "make_new_sbhl_graph", "MAKE-NEW-SBHL-GRAPH", 0, 0, false);
    //declareFunction(myName, "clear_sbhl_graph", "CLEAR-SBHL-GRAPH", 1, 0, false);
    declareFunction(myName, "initialize_sbhl_graph_caches", "INITIALIZE-SBHL-GRAPH-CACHES", 0, 0, false);
    //declareFunction(myName, "initialize_sbhl_graph_caches_file_vector", "INITIALIZE-SBHL-GRAPH-CACHES-FILE-VECTOR", 2, 0, false);
    declareFunction(myName, "new_cache_strategy_for_sbhl_module", "NEW-CACHE-STRATEGY-FOR-SBHL-MODULE", 1, 1, false);
    declareFunction(myName, "get_cache_strategy_for_sbhl_module", "GET-CACHE-STRATEGY-FOR-SBHL-MODULE", 1, 0, false);
    declareFunction(myName, "set_cache_strategy_for_sbhl_module", "SET-CACHE-STRATEGY-FOR-SBHL-MODULE", 2, 0, false);
    declareFunction(myName, "cache_capacity_for_cache_strategy_for_sbhl_module", "CACHE-CAPACITY-FOR-CACHE-STRATEGY-FOR-SBHL-MODULE", 1, 0, false);
    //declareFunction(myName, "initialize_sbhl_graph_caches_during_load_kb", "INITIALIZE-SBHL-GRAPH-CACHES-DURING-LOAD-KB", 2, 0, false);
    //declareFunction(myName, "sbhl_graph_completely_cachedP", "SBHL-GRAPH-COMPLETELY-CACHED?", 0, 0, false);
    //declareFunction(myName, "get_sbhl_module_caching_ratio", "GET-SBHL-MODULE-CACHING-RATIO", 1, 0, false);
    //declareFunction(myName, "get_sbhl_modules_caching_ratios", "GET-SBHL-MODULES-CACHING-RATIOS", 0, 0, false);
    declareFunction(myName, "get_sbhl_graph_link_from_graph", "GET-SBHL-GRAPH-LINK-FROM-GRAPH", 3, 0, false);
    declareFunction(myName, "put_sbhl_graph_link_into_graph", "PUT-SBHL-GRAPH-LINK-INTO-GRAPH", 4, 0, false);
    declareFunction(myName, "remove_sbhl_graph_link_from_graph", "REMOVE-SBHL-GRAPH-LINK-FROM-GRAPH", 3, 0, false);
    declareFunction(myName, "touch_sbhl_link_graph", "TOUCH-SBHL-LINK-GRAPH", 3, 0, false);
    declareFunction(myName, "get_sbhl_graph_link", "GET-SBHL-GRAPH-LINK", 2, 0, false);
    declareFunction(myName, "set_sbhl_graph_link", "SET-SBHL-GRAPH-LINK", 3, 0, false);
    declareFunction(myName, "touch_sbhl_graph_link", "TOUCH-SBHL-GRAPH-LINK", 3, 0, false);
    declareFunction(myName, "remove_sbhl_graph_link", "REMOVE-SBHL-GRAPH-LINK", 2, 0, false);
    //declareMacro(myName, "do_sbhl_graph_links", "DO-SBHL-GRAPH-LINKS");
    //declareFunction(myName, "swap_in_all_graph_links", "SWAP-IN-ALL-GRAPH-LINKS", 1, 0, false);
    declareFunction(myName, "swap_out_all_pristine_graph_links", "SWAP-OUT-ALL-PRISTINE-GRAPH-LINKS", 1, 0, false);
    declareFunction(myName, "swap_out_all_pristine_sbhl_module_graph_links", "SWAP-OUT-ALL-PRISTINE-SBHL-MODULE-GRAPH-LINKS", 0, 0, false);
    //declareFunction(myName, "get_sbhl_module_cache_strategy_metrics", "GET-SBHL-MODULE-CACHE-STRATEGY-METRICS", 1, 0, false);
    //declareFunction(myName, "get_all_sbhl_module_cache_strategy_metrics", "GET-ALL-SBHL-MODULE-CACHE-STRATEGY-METRICS", 0, 0, false);
    //declareFunction(myName, "get_sbhl_module_cache_strategy_information", "GET-SBHL-MODULE-CACHE-STRATEGY-INFORMATION", 1, 0, false);
    //declareFunction(myName, "show_all_sbhl_module_cache_strategies", "SHOW-ALL-SBHL-MODULE-CACHE-STRATEGIES", 0, 0, false);
    //declareFunction(myName, "stress_test_sbhl_graph_concurrent_swapping", "STRESS-TEST-SBHL-GRAPH-CONCURRENT-SWAPPING", 0, 2, false);
    //declareFunction(myName, "stress_test_read_randomly_from_sbhl", "STRESS-TEST-READ-RANDOMLY-FROM-SBHL", 2, 0, false);
    //declareFunction(myName, "stress_test_sbhl_graph_concurrent_cache_strategy_update", "STRESS-TEST-SBHL-GRAPH-CONCURRENT-CACHE-STRATEGY-UPDATE", 0, 1, false);
    //declareFunction(myName, "stress_test_check_same_specs", "STRESS-TEST-CHECK-SAME-SPECS", 3, 0, false);
    return NIL;
  }
