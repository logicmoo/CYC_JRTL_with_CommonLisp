      ...
      SubLTranslatedFile
{
  ...
  {
    return makeBoolean( conjuncts.isList() && NIL != list_utilities.any_in_list( $sym7$EL_CONJUNCTION_P, conjuncts, UNPROVIDED ) && !conjuncts.equal( lift_conjuncts( conjuncts ) ) );
  }
  ...
  public static SubLObject try_to_simplify_non_wff_into_wff(final SubLObject non_wff, SubLObject wff_function, SubLObject arg2_to_wff_function)
  {
    ...
    if( NIL != czer_vars.$try_to_simplify_non_wff_into_wffP$.getDynamicValue( thread ) )
    {
      ...
      try
      {
        ...
        is_it_wff_nowP = makeBoolean( !non_wff.equal( simpler_sentence ) && NIL != ( ( NIL != arg2_to_wff_function ) ? Functions.funcall( wff_function, simpler_sentence, arg2_to_wff_function )
            ...
            if( NIL == cycl_grammar.fast_cycl_quoted_term_p( sentence ) )
            {
              if( NIL == Functions.funcall( varP, sentence ) )
              {
                if( NIL == assertion_handles.assertion_p( sentence ) )
                {
                  ...
                  {
                    result = simplify_cycl_universal( el_utilities.make_regularly_quantified_sentence( el_utilities.sentence_quantifier( sentence ), el_utilities.quantified_var( sentence ), simplify_cycl_sentence_int(
                        el_utilities.quantified_sub_sentence( sentence ), varP ) ) );
                  }
                  else if( NIL != el_utilities.el_existential_p( sentence ) )
                  {
                    result = simplify_cycl_existential( el_utilities.make_regularly_quantified_sentence( el_utilities.sentence_quantifier( sentence ), el_utilities.quantified_var( sentence ), simplify_cycl_sentence_int(
                        el_utilities.quantified_sub_sentence( sentence ), UNPROVIDED ) ) );
                  }
                  else if( NIL != el_utilities.el_multi_existential_p( sentence ) || NIL != el_utilities.el_multi_universal_p( sentence ) )
                  {
                    result = simplify_cycl_multiply_quantified_sentence( el_utilities.make_multiply_quantified_sentence( el_utilities.sentence_quantifier( sentence ), el_utilities.quantified_var_list( sentence ),
                        simplify_cycl_sentence_int( el_utilities.quantified_sub_sentence( sentence ), UNPROVIDED ) ) );
                  }
                  else if( NIL != el_utilities.el_bounded_existential_p( sentence ) )
                  {
                    result = el_utilities.make_bounded_existential( el_utilities.sentence_quantifier( sentence ), el_utilities.existential_bound( sentence ), el_utilities.quantified_var( sentence ),
                        simplify_cycl_sentence_int( el_utilities.quantified_sub_sentence( sentence ), varP ) );
                  }
  ...
  public static SubLObject simplify_cycl_literal(SubLObject literal, SubLObject var_func)
  {
    ...
    {
      final SubLObject simplified_literal = ( NIL != $simplify_cycl_sentence_and_subsentencesP$.getDynamicValue( thread ) ) ? el_utilities.make_ist_sentence( el_utilities.designated_mt( literal ), fold_equals(
          el_utilities.designated_sentence( literal ) ) ) : literal;
  ...
  public static SubLObject simplify_cycl_literal_int(final SubLObject literal, SubLObject var_func)
  {
    ...
    if( NIL != czer_vars.$simplify_using_semanticsP$.getDynamicValue( thread ) )
    {
      ...
      try
      {
        ...
        if( NIL != control_vars.within_assertP() || NIL != czer_vars.$trying_to_simplify_non_wff_into_wffP$.getDynamicValue( thread ) || NIL == czer_vars.$simplify_non_wff_literalP$.getDynamicValue( thread )
            || NIL != wff.semantically_wf_literalP( literal, mt_relevance_macros.$mt$.getDynamicValue( thread ) ) || NIL == el_utilities.closedP( literal, UNPROVIDED ) )
        {
  ...
  public static SubLObject simplify_cycl_literal_terms_int(final SubLObject literal, SubLObject varP)
  {
    ...
    for( argnum = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), v_term = list_var.first(), argnum = number_utilities.f_1X( argnum ) )
    {
      ...
      try
      {
        wff_vars.$permit_keyword_variablesP$.bind( makeBoolean( NIL != wff_vars.$permit_keyword_variablesP$.getDynamicValue( thread ) || NIL != czer_utilities.arg_permits_keyword_variablesP( pred, argnum, mt ) ),
            thread );
        wff_vars.$permit_generic_arg_variablesP$.bind( makeBoolean( NIL != wff_vars.$permit_generic_arg_variablesP$.getDynamicValue( thread ) || NIL != czer_utilities.arg_permits_generic_arg_variablesP( pred, argnum,
            mt ) ), thread );
    ...
    if( NIL != term.nautP( v_term, varP ) )
    {
      ...
      else if( NIL != el_utilities.el_unary_formula_p( new_term ) )
      {
        ...
        if( NIL != term.nautP( nested_term, varP ) && NIL != el_utilities.el_unary_formula_p( nested_term ) )
        {
          ...
          if( NIL != kb_indexing_datastructures.indexed_term_p( nested_functor ) && ( NIL != somewhere_cache.some_pred_assertion_somewhereP( $const22$inverseFunctions, functor, ONE_INTEGER, UNPROVIDED )
              || NIL != somewhere_cache.some_pred_assertion_somewhereP( $const22$inverseFunctions, nested_functor, ONE_INTEGER, UNPROVIDED ) ) )
          {
  ...
  public static SubLObject simplify_cycl_conjunction(SubLObject conjunctionIn, SubLObject varP)
  {
    ...
    do
    {
      ...
      final SubLObject disjunctions = ( ( NIL != simplify_redundanciesP() ) ? list_utilities.remove_if_not( Symbols.symbol_function( $sym24$EL_DISJUNCTION_P ), positives, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED )
          : NIL );
      ...
      if( NIL == falseP )
      {
        ...
        SubLObject negation;
        for( csome_list_var = negations, negation = NIL, negation = csome_list_var.first(); NIL == falseP && NIL != csome_list_var; falseP = subl_promotions.memberP( cycl_utilities.sentence_arg1( negation,
            $kw16$IGNORE ), positives, Symbols.symbol_function( EQUAL ), UNPROVIDED ), csome_list_var = csome_list_var.rest(), negation = csome_list_var.first() )
        {
        }
      }
      if( NIL != falseP )
  ...
  {
    return makeBoolean( NIL != cycl_utilities.atomic_sentence_with_any_of_preds_p( asent, $list27 ) || ( NIL != fold_equalsP && NIL != cycl_utilities.atomic_sentence_with_pred_p( asent, $const28$equals ) ) );
  }
  ...
  {
    return transform_list_utilities.transform( formula, Symbols.symbol_function( $sym30$NESTED_COLLECTIONSUBSETFN_EXPRESSION_ ), Symbols.symbol_function( $sym31$TRANSFORM_NESTED_COLLECTIONSUBSETFN_EXPRESSION ),
        UNPROVIDED );
  }
  ...
  public static SubLObject transform_nested_collectionsubsetfn_expression(final SubLObject expression)
  {
    ...
    return el_utilities.make_binary_formula( $const32$CollectionSubsetFn, nested_col, el_utilities.make_binary_formula( $const33$TheSetOf, nested_set_var, conjoin( ConsesLow.list( nested_set_sentence, conses_high.nsubst(
        nested_set_var, set_var, set_sentence, UNPROVIDED, UNPROVIDED ) ), T ) ) );
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_formula_p( expression ) && $const32$CollectionSubsetFn.eql( cycl_utilities.formula_arg0( expression ) ) && NIL != el_utilities.el_formula_p( cycl_utilities.formula_arg1(
        expression, UNPROVIDED ) ) && $const32$CollectionSubsetFn.eql( cycl_utilities.formula_arg0( cycl_utilities.formula_arg1( expression, UNPROVIDED ) ) ) && NIL != el_utilities.el_formula_p( cycl_utilities
            .formula_arg2( expression, UNPROVIDED ) ) && $const33$TheSetOf.eql( cycl_utilities.formula_arg0( cycl_utilities.formula_arg2( expression, UNPROVIDED ) ) ) && NIL != el_utilities.el_formula_p( cycl_utilities
                .formula_arg2( cycl_utilities.formula_arg1( expression, UNPROVIDED ), UNPROVIDED ) ) && $const33$TheSetOf.eql( cycl_utilities.formula_arg0( cycl_utilities.formula_arg2( cycl_utilities.formula_arg1(
                    expression, UNPROVIDED ), UNPROVIDED ) ) ) );
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.possibly_naut_p( expression ) && NIL != kb_utilities.kbeq( $const37$IndividualAsFn, cycl_utilities.formula_arg0( expression ) ) );
  }
  ...
  public static SubLObject simplify_sequence_variables(final SubLObject formula)
  {
    ...
    try
    {
      czer_vars.$el_symbol_suffix_table$.bind( ( NIL != czer_vars.$el_symbol_suffix_table$.getDynamicValue( thread ) ) ? czer_vars.$el_symbol_suffix_table$.getDynamicValue( thread )
          : Hashtables.make_hash_table( $int43$32, Symbols.symbol_function( EQL ), UNPROVIDED ), thread );
  ...
  public static SubLObject simplify_sequence_variables_int(SubLObject formula, SubLObject dont_mess_with_these_variables, SubLObject scoped_variables)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject new_dont_touch_vars = list_utilities.fast_delete_duplicates( ConsesLow.append( dont_mess_with_these_variables, conses_high.set_difference( conses_high.intersection( subsentence_free_variables,
          sentence_free_variables, UNPROVIDED, UNPROVIDED ), new_scoped_vars, UNPROVIDED, UNPROVIDED ) ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
      thread.resetMultipleValues();
  ...
  public static SubLObject possible_sequence_var_simplification(final SubLObject formula, final SubLObject seqvar)
  {
    ...
    final SubLObject reln_arity_max = ( NIL != el_utilities.cyc_const_logical_operator_p( cycl_utilities.formula_arg0( formula ) ) ) ? arity.logical_operator_arity( cycl_utilities.formula_arg0( formula ) )
        : arity.max_arity( cycl_utilities.formula_arg0( formula ) );
    final SubLObject reln_arity_min = ( NIL != el_utilities.cyc_const_logical_operator_p( cycl_utilities.formula_arg0( formula ) ) ) ? arity.logical_operator_arity( cycl_utilities.formula_arg0( formula ) )
  ...
  public static SubLObject simplify_contradictions(final SubLObject sentence)
  {
    ...
    if( NIL != el_utilities.el_conjunction_p( sentence ) )
    {
      ...
      if( NIL != asymmetric_pred_conjuncts )
      {
        ...
        SubLObject iteration_state;
        for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( pred_to_conjuncts_dictionary ) ); NIL == asymmetry_foundP && NIL == dictionary_contents
            .do_dictionary_contents_doneP( iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
        {
          ...
          if( NIL != list_utilities.lengthGE( conjuncts, TWO_INTEGER, UNPROVIDED ) )
          {
            ...
            if( NIL == asymmetry_foundP )
            {
              ...
              {
                if( !genl_pred_conjunct.equal( rest_conjunct ) && NIL != kb_utilities.kbeq( cycl_utilities.sentence_arg1( genl_pred_conjunct, UNPROVIDED ), cycl_utilities.sentence_arg2( rest_conjunct, UNPROVIDED ) )
                    && NIL != kb_utilities.kbeq( cycl_utilities.sentence_arg2( genl_pred_conjunct, UNPROVIDED ), cycl_utilities.sentence_arg1( rest_conjunct, UNPROVIDED ) ) )
                {
                  asymmetry_foundP = T;
                }
                csome_list_var = csome_list_var.rest();
  ...
  public static SubLObject subsumed_argsP(final SubLObject pred, final SubLObject formula_vars, final SubLObject existing_const)
  {
    ...
    {
      if( NIL == ke_tools.subsumed_by_stronger_argsP( pred, Numbers.add( index, ONE_INTEGER ), ConsesLow.list( ConsesLow.nth( index, existing_const ) ), ConsesLow.list( ConsesLow.nth( index, formula_vars ) ),
          UNPROVIDED ) )
      {
        flag = NIL;
      }
  ...
  public static SubLObject simplify_transitive_redundancies_in_disjunction(final SubLObject disjunction)
  {
    ...
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( disjunct_constraint_dict ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
  ...
  public static SubLObject necessary_constraint_dict(final SubLObject weak_dict, final SubLObject strong_dict)
  {
    ...
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( weak_dict ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
      ...
      final SubLObject necessary_constraints = list_utilities.fast_set_difference( keyhash_utilities.fast_intersection( weak_constraints, merged_constraints, UNPROVIDED, UNPROVIDED, UNPROVIDED ), strong_constraints,
          UNPROVIDED );
  ...
  public static SubLObject simplify_transitive_redundancies_old(final SubLObject sentence, final SubLObject mt)
  {
    ...
    try
    {
      ...
      SubLObject iteration_state;
      for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( conjunct_constraint_dict ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
        thread.resetMultipleValues();
        final SubLObject key = dictionary_contents.do_dictionary_contents_key_value( iteration_state );
  ...
  {
    SubLFiles.declareFunction( me, "fast_simplify_cycl_sentence", "FAST-SIMPLIFY-CYCL-SENTENCE", 1, 2, false );
    SubLFiles.declareFunction( me, "scg_simplify", "SCG-SIMPLIFY", 1, 1, false );
    SubLFiles.declareFunction( me, "lift_disjuncts", "LIFT-DISJUNCTS", 1, 0, false );
    SubLFiles.declareFunction( me, "nlift_disjuncts", "NLIFT-DISJUNCTS", 1, 0, false );
    SubLFiles.declareFunction( me, "disjoin", "DISJOIN", 1, 1, false );
    SubLFiles.declareFunction( me, "ndisjoin", "NDISJOIN", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_unary_junct", "SIMPLIFY-UNARY-JUNCT", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_unary_juncts", "SIMPLIFY-UNARY-JUNCTS", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_duplicate_juncts", "SIMPLIFY-DUPLICATE-JUNCTS", 1, 0, false );
    SubLFiles.declareFunction( me, "lift_conjuncts", "LIFT-CONJUNCTS", 1, 0, false );
    SubLFiles.declareFunction( me, "nlift_conjuncts", "NLIFT-CONJUNCTS", 1, 0, false );
    SubLFiles.declareFunction( me, "conjoin", "CONJOIN", 1, 1, false );
    SubLFiles.declareFunction( me, "nconjoin", "NCONJOIN", 1, 1, false );
    SubLFiles.declareFunction( me, "possibly_conjoin", "POSSIBLY-CONJOIN", 1, 1, false );
    SubLFiles.declareFunction( me, "lift_conjuncts_recursive", "LIFT-CONJUNCTS-RECURSIVE", 1, 0, false );
    SubLFiles.declareFunction( me, "liftable_conjunctsP", "LIFTABLE-CONJUNCTS?", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_el_syntax", "SIMPLIFY-EL-SYNTAX", 1, 1, false );
    SubLFiles.declareFunction( me, "try_to_simplify_non_wff_into_wff", "TRY-TO-SIMPLIFY-NON-WFF-INTO-WFF", 1, 2, false );
    SubLFiles.declareFunction( me, "simplify_cycl_sentence_deep", "SIMPLIFY-CYCL-SENTENCE-DEEP", 1, 1, false );
    SubLFiles.declareFunction( me, "get_simplified_cycl_sentence", "GET-SIMPLIFIED-CYCL-SENTENCE", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_sentence", "SIMPLIFY-CYCL-SENTENCE", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_sentence_syntax", "SIMPLIFY-CYCL-SENTENCE-SYNTAX", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_sentence_int", "SIMPLIFY-CYCL-SENTENCE-INT", 1, 1, false );
    new $simplify_cycl_sentence_int$UnaryFunction();
    new $simplify_cycl_sentence_int$BinaryFunction();
    SubLFiles.declareFunction( me, "simplify_true_sentence", "SIMPLIFY-TRUE-SENTENCE", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_exception", "SIMPLIFY-EXCEPTION", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_literal_syntax", "SIMPLIFY-CYCL-LITERAL-SYNTAX", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_literal", "SIMPLIFY-CYCL-LITERAL", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_literal_int", "SIMPLIFY-CYCL-LITERAL-INT", 1, 1, false );
    SubLFiles.declareFunction( me, "distributes_out_of_argP", "DISTRIBUTES-OUT-OF-ARG?", 3, 1, false );
    SubLFiles.declareFunction( me, "simplify_distributing_out_args", "SIMPLIFY-DISTRIBUTING-OUT-ARGS", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_literal_terms", "SIMPLIFY-CYCL-LITERAL-TERMS", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_mt_literal_terms", "SIMPLIFY-MT-LITERAL-TERMS", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_literal_terms_int", "SIMPLIFY-CYCL-LITERAL-TERMS-INT", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_term", "SIMPLIFY-CYCL-TERM", 1, 1, false );
    SubLFiles.declareFunction( me, "el_negate", "EL-NEGATE", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_negation", "SIMPLIFY-CYCL-NEGATION", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_conjunction", "SIMPLIFY-CYCL-CONJUNCTION", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_equality_literals", "SIMPLIFY-EQUALITY-LITERALS", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_one_open_open_equality_literal", "SIMPLIFY-ONE-OPEN-OPEN-EQUALITY-LITERAL", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_one_var_var_equality_literal", "SIMPLIFY-ONE-VAR-VAR-EQUALITY-LITERAL", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_one_open_open_equality_literal_int", "SIMPLIFY-ONE-OPEN-OPEN-EQUALITY-LITERAL-INT", 3, 0, false );
    SubLFiles.declareFunction( me, "simplify_one_symmetric_equality_literal", "SIMPLIFY-ONE-SYMMETRIC-EQUALITY-LITERAL", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_open_closed_equality_literals", "SIMPLIFY-OPEN-CLOSED-EQUALITY-LITERALS", 2, 0, false );
    SubLFiles.declareFunction( me, "equality_literalP", "EQUALITY-LITERAL?", 2, 0, false );
    SubLFiles.declareFunction( me, "open_open_equality_literalP", "OPEN-OPEN-EQUALITY-LITERAL?", 4, 0, false );
    SubLFiles.declareFunction( me, "symmetric_equality_literalP", "SYMMETRIC-EQUALITY-LITERAL?", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_disjunction", "SIMPLIFY-CYCL-DISJUNCTION", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_disjunction_int", "SIMPLIFY-CYCL-DISJUNCTION-INT", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_implication", "SIMPLIFY-CYCL-IMPLICATION", 1, 1, false );
    SubLFiles.declareFunction( me, "equal_implication_argsP", "EQUAL-IMPLICATION-ARGS?", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_cycl_universal", "SIMPLIFY-CYCL-UNIVERSAL", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_existential", "SIMPLIFY-CYCL-EXISTENTIAL", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_multiply_quantified_sentence", "SIMPLIFY-CYCL-MULTIPLY-QUANTIFIED-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_cycl_relation", "SIMPLIFY-CYCL-RELATION", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_special_cases", "SIMPLIFY-SPECIAL-CASES", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_nested_collectionsubsetfn_expression", "SIMPLIFY-NESTED-COLLECTIONSUBSETFN-EXPRESSION", 1, 0, false );
    SubLFiles.declareFunction( me, "transform_nested_collectionsubsetfn_expression", "TRANSFORM-NESTED-COLLECTIONSUBSETFN-EXPRESSION", 1, 0, false );
    SubLFiles.declareFunction( me, "nested_collectionsubsetfn_expressionP", "NESTED-COLLECTIONSUBSETFN-EXPRESSION?", 1, 0, false );
    new $nested_collectionsubsetfn_expressionP$UnaryFunction();
    SubLFiles.declareFunction( me, "simplify_individualasfn_expressions", "SIMPLIFY-INDIVIDUALASFN-EXPRESSIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "transform_individualasfn_expression", "TRANSFORM-INDIVIDUALASFN-EXPRESSION", 1, 0, false );
    SubLFiles.declareFunction( me, "individualasfn_expressionP", "INDIVIDUALASFN-EXPRESSION?", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_redundanciesP", "SIMPLIFY-REDUNDANCIES?", 0, 0, false );
    SubLFiles.declareFunction( me, "simplify_kappa_asent", "SIMPLIFY-KAPPA-ASENT", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_ist_sentence", "SIMPLIFY-IST-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "fold_equals", "FOLD-EQUALS", 1, 0, false );
    SubLFiles.declareFunction( me, "fold_equals_in_asent", "FOLD-EQUALS-IN-ASENT", 2, 3, false );
    SubLFiles.declareFunction( me, "do_fold_equals_in_asent", "DO-FOLD-EQUALS-IN-ASENT", 4, 0, false );
    SubLFiles.declareFunction( me, "note_fold_equals_binding", "NOTE-FOLD-EQUALS-BINDING", 3, 0, false );
    SubLFiles.declareFunction( me, "simplify_sequence_variables", "SIMPLIFY-SEQUENCE-VARIABLES", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_sequence_variables_1", "SIMPLIFY-SEQUENCE-VARIABLES-1", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_sequence_variables_int", "SIMPLIFY-SEQUENCE-VARIABLES-INT", 3, 0, false );
    SubLFiles.declareFunction( me, "possible_sequence_var_simplification", "POSSIBLE-SEQUENCE-VAR-SIMPLIFICATION", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_sequence_vars_using_kb_arityP", "SIMPLIFY-SEQUENCE-VARS-USING-KB-ARITY?", 0, 0, false );
    SubLFiles.declareFunction( me, "ignore_sequence_var_if_wff", "IGNORE-SEQUENCE-VAR-IF-WFF", 2, 1, false );
    SubLFiles.declareFunction( me, "regularize_sequence_var_if_wff", "REGULARIZE-SEQUENCE-VAR-IF-WFF", 2, 1, false );
    SubLFiles.declareFunction( me, "split_sequence_var_if_wff", "SPLIT-SEQUENCE-VAR-IF-WFF", 1, 5, false );
    SubLFiles.declareFunction( me, "sequence_var_simplifiableP", "SEQUENCE-VAR-SIMPLIFIABLE?", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_contradictions", "SIMPLIFY-CONTRADICTIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_transitive_redundancies", "SIMPLIFY-TRANSITIVE-REDUNDANCIES", 1, 1, false );
    SubLFiles.declareFunction( me, "simplify_transitive_redundancies_in_cycl_disjunction", "SIMPLIFY-TRANSITIVE-REDUNDANCIES-IN-CYCL-DISJUNCTION", 1, 1, false );
    SubLFiles.declareFunction( me, "subsumed_by_another_conjunctP", "SUBSUMED-BY-ANOTHER-CONJUNCT?", 2, 0, false );
    SubLFiles.declareFunction( me, "conjunct_subsumed_by_conjunctP", "CONJUNCT-SUBSUMED-BY-CONJUNCT?", 2, 0, false );
    SubLFiles.declareFunction( me, "subsumed_args_by_constsP", "SUBSUMED-ARGS-BY-CONSTS?", 3, 0, false );
    SubLFiles.declareFunction( me, "subsumed_argsP", "SUBSUMED-ARGS?", 3, 0, false );
    SubLFiles.declareFunction( me, "simplify_transitive_redundancies_in_disjunction", "SIMPLIFY-TRANSITIVE-REDUNDANCIES-IN-DISJUNCTION", 1, 0, false );
    SubLFiles.declareFunction( me, "necessary_constraint_dict", "NECESSARY-CONSTRAINT-DICT", 2, 0, false );
    SubLFiles.declareFunction( me, "transitive_constraint_dict", "TRANSITIVE-CONSTRAINT-DICT", 2, 0, false );
    SubLFiles.declareFunction( me, "transitive_constraint_raw_info", "TRANSITIVE-CONSTRAINT-RAW-INFO", 1, 0, false );
    SubLFiles.declareFunction( me, "simplify_transitive_redundanciesP", "SIMPLIFY-TRANSITIVE-REDUNDANCIES?", 0, 0, false );
    SubLFiles.declareFunction( me, "simplify_contradictionsP", "SIMPLIFY-CONTRADICTIONS?", 0, 0, false );
    SubLFiles.declareFunction( me, "simplify_transitive_redundancies_old", "SIMPLIFY-TRANSITIVE-REDUNDANCIES-OLD", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_transitive_redundancies_in_disjunction_old", "SIMPLIFY-TRANSITIVE-REDUNDANCIES-IN-DISJUNCTION-OLD", 2, 0, false );
    SubLFiles.declareFunction( me, "simplify_number_expression", "SIMPLIFY-NUMBER-EXPRESSION", 1, 0, false );
    SubLFiles.declareFunction( me, "cycl_coerce_types", "CYCL-COERCE-TYPES", 1, 2, false );
    return NIL;
  }
  ...
  static
  {
    ...
    $sym3$SIMPLIFY_UNARY_JUNCTS = makeSymbol( "SIMPLIFY-UNARY-JUNCTS" );
    $sym4$SIMPLIFY_DUPLICATE_JUNCTS = makeSymbol( "SIMPLIFY-DUPLICATE-JUNCTS" );
    $sym5$LIFTABLE_CONJUNCTS_ = makeSymbol( "LIFTABLE-CONJUNCTS?" );
    $sym6$NLIFT_CONJUNCTS = makeSymbol( "NLIFT-CONJUNCTS" );
    $sym7$EL_CONJUNCTION_P = makeSymbol( "EL-CONJUNCTION-P" );
    $sym8$CYC_VAR_ = makeSymbol( "CYC-VAR?" );
    $sym9$EL_WFF_ = makeSymbol( "EL-WFF?" );
    $sym10$GET_SIMPLIFIED_CYCL_SENTENCE = makeSymbol( "GET-SIMPLIFIED-CYCL-SENTENCE" );
    ...
    $const13$False = constant_handles.reader_make_constant_shell( makeString( "False" ) );
    $str14$_S_is_not_well_formed_ = makeString( "~S is not well formed." );
    $kw15$INVALID_SENTENCE = makeKeyword( "INVALID-SENTENCE" );
    $kw16$IGNORE = makeKeyword( "IGNORE" );
    $kw17$REGULARIZE = makeKeyword( "REGULARIZE" );
    $sym18$SIMPLIFY_CYCL_SENTENCE_INT = makeSymbol( "SIMPLIFY-CYCL-SENTENCE-INT" );
    $const19$distributesOutOfArg = constant_handles.reader_make_constant_shell( makeString( "distributesOutOfArg" ) );
    $sym20$RELEVANT_MT_IS_GENL_MT = makeSymbol( "RELEVANT-MT-IS-GENL-MT" );
    $const21$Unity = constant_handles.reader_make_constant_shell( makeString( "Unity" ) );
    $const22$inverseFunctions = constant_handles.reader_make_constant_shell( makeString( "inverseFunctions" ) );
    $str23$You_tried_to_negate__S___That_was = makeString( "You tried to negate ~S.  That was a bad idea, because it's neither a sentence nor a sentence." );
    $sym24$EL_DISJUNCTION_P = makeSymbol( "EL-DISJUNCTION-P" );
    $sym25$EL_VAR_ = makeSymbol( "EL-VAR?" );
    $sym26$_ = makeSymbol( ">" );
    $list27 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "equalSymbols" ) ), constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) ), constant_handles
        .reader_make_constant_shell( makeString( "termOfUnit" ) ) );
    $const28$equals = constant_handles.reader_make_constant_shell( makeString( "equals" ) );
    $kw29$SIMPLIFICATION_TO_TAUTOLOGY_DUE_TO_POS_AND_NEG_LITERAL = makeKeyword( "SIMPLIFICATION-TO-TAUTOLOGY-DUE-TO-POS-AND-NEG-LITERAL" );
    $sym30$NESTED_COLLECTIONSUBSETFN_EXPRESSION_ = makeSymbol( "NESTED-COLLECTIONSUBSETFN-EXPRESSION?" );
    $sym31$TRANSFORM_NESTED_COLLECTIONSUBSETFN_EXPRESSION = makeSymbol( "TRANSFORM-NESTED-COLLECTIONSUBSETFN-EXPRESSION" );
    $const32$CollectionSubsetFn = constant_handles.reader_make_constant_shell( makeString( "CollectionSubsetFn" ) );
    $const33$TheSetOf = constant_handles.reader_make_constant_shell( makeString( "TheSetOf" ) );
    $sym34$INDIVIDUALASFN_EXPRESSION_ = makeSymbol( "INDIVIDUALASFN-EXPRESSION?" );
    $sym35$TRANSFORM_INDIVIDUALASFN_EXPRESSION = makeSymbol( "TRANSFORM-INDIVIDUALASFN-EXPRESSION" );
    $const36$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $const37$IndividualAsFn = constant_handles.reader_make_constant_shell( makeString( "IndividualAsFn" ) );
    $const38$trueSentence = constant_handles.reader_make_constant_shell( makeString( "trueSentence" ) );
    $const39$ist = constant_handles.reader_make_constant_shell( makeString( "ist" ) );
    $sym40$HL_VARIABLE_P = makeSymbol( "HL-VARIABLE-P" );
    $const41$indexicalReferent = constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) );
    $list42 = ConsesLow.cons( makeSymbol( "VAR" ), makeSymbol( "BINDING" ) );
    $int43$32 = makeInteger( 32 );
    $sym44$FORMULA_WITH_SEQUENCE_TERM_ = makeSymbol( "FORMULA-WITH-SEQUENCE-TERM?" );
    $kw45$SPLIT = makeKeyword( "SPLIT" );
    $str46$Splitting_sequence_variables_into = makeString( "Splitting sequence variables into other sequence variables is currently not supported. -ECA" );
    $list47 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "isa" ) ), constant_handles.reader_make_constant_shell( makeString( "genls" ) ) );
    $sym48$WEAKEST_ARGS = makeSymbol( "WEAKEST-ARGS" );
    $sym49$STRONGEST_ARGS = makeSymbol( "STRONGEST-ARGS" );
    $sym50$EXPLANATION_OF_WHY_NOT_WFF = makeSymbol( "EXPLANATION-OF-WHY-NOT-WFF" );
    $list51 = ConsesLow.list( makeKeyword( "MAL-ARG-WRT-IFF-DEFN" ), makeKeyword( "MAL-ARG-WRT-QUOTED-IFF-DEFN" ), makeKeyword( "MAL-ARG-WRT-SUF-DEFNS" ), makeKeyword( "MAL-ARG-WRT-QUOTED-SUF-DEFNS" ), makeKeyword(
        "MAL-ARG-WRT-COL-DEFN" ) );
    $list52 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "RealNumber" ) ), constant_handles.reader_make_constant_shell( makeString( "SubLRealNumber" ) ) );
    $list53 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "CharacterString" ) ), constant_handles.reader_make_constant_shell( makeString( "SubLString" ) ) );
  }

  public static final class $simplify_cycl_sentence_int$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $simplify_cycl_sentence_int$BinaryFunction
      extends
        BinaryFunction
  {
  ...
  public static final class $nested_collectionsubsetfn_expressionP$UnaryFunction
      extends
        UnaryFunction
  {
