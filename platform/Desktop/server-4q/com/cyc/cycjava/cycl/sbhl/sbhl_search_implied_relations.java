      ...
      SubLTranslatedFile
{
        ...
        try
        {
          ...
          try
          {
            ...
            try
            {
              ...
              try
              {
                result = sbhl_search_methods.sbhl_gather_all_backward_true_nodes( module, node, $sym8$SBHL_GATHER_ALL_DISJOINS, mt, tv, UNPROVIDED );
              }
              finally
              {
                    ...
                    try
                    {
                      ...
                      while ( NIL != cdolist_list_var)
                      {
                        ...
                        try
                        {
                          ...
                          else if( NIL != obsolete.cnat_p( node_$34, UNPROVIDED ) )
                          {
  ...
  public static SubLObject sbhl_disjoins_relation_with_backward_nodes_p(final SubLObject module, final SubLObject node, final SubLObject not_node, SubLObject mt, SubLObject tv)
  {
    ...
    if( NIL != disjoins_module )
    {
      ...
      try
      {
        ...
        sbhl_search_vars.$genl_inverse_mode_p$.bind( NIL, thread );
        sbhl_module_vars.$sbhl_module$.bind( disjoins_module, thread );
        sbhl_search_vars.$sbhl_search_type$.bind( $kw23$BOOLEAN, thread );
        final SubLObject mt_var = mt;
                ...
                try
                {
                  ...
                  try
                  {
                    ...
                    try
                    {
                      ...
                      SubLObject goal_fn = $sym24$SBHL_NODE_IS_GOAL_NODE;
                      ...
                      if( NIL != premarkingP )
                      {
                        ...
                        try
                        {
                          ...
                          sbhl_search_vars.$genl_inverse_mode_p$.bind( sbhl_module_utilities.get_sbhl_link_pred( sbhl_search_vars.get_sbhl_search_module() ).eql( $const25$negationInverse ) ? T
                        ...
                        goal_fn = $sym26$SBHL_NODE_MARKED_AS_GOAL_NODE;
  ...
  public static SubLObject sbhl_disjoins_with_tt_nodes_relation_p(final SubLObject module, final SubLObject node, final SubLObject not_node, SubLObject mt, SubLObject tv)
  {
    ...
    if( NIL != sbhl_search_vars.sbhl_justification_search_p() && NIL != sbhl_search_vars.$sbhl_justification_result$.getDynamicValue( thread ) )
    {
      ...
      sbhl_search_vars.$sbhl_justification_result$.setDynamicValue( ConsesLow.cons( ConsesLow.list( ConsesLow.list( sbhl_module_vars.get_sbhl_module_link_pred( module ), node, link_node ), ( NIL != mt ) ? mt
          : mt_relevance_macros.$mt$.getDynamicValue( thread ), ( NIL != tv ) ? sbhl_search_vars.sbhl_true_tv( tv ) : sbhl_search_vars.sbhl_search_true_tv() ), sbhl_search_vars.$sbhl_justification_result$
  ...
  public static SubLObject sbhl_sibling_disjoint_with_backward_nodes_justification(final SubLObject module, final SubLObject node, final SubLObject not_node, SubLObject mt, SubLObject tv)
  {
    ...
    if( NIL != sd_just_fn )
    {
      ...
      if( NIL == disjointP )
      {
        SubLObject csome_list_var = ConsesLow.cons( node, backward_nodes );
        SubLObject back_node = NIL;
        ...
        while ( NIL == disjointP && NIL != csome_list_var)
        {
          ...
          if( NIL != result )
          {
            ...
            {
              result = ConsesLow.nconc( sbhl_search_utilities.sbhl_handle_justification( $sym27$SBHL_PREDICATE_RELATION_P, module, back_node, node, mt, tv, NIL, T ), result );
            }
  ...
  public static SubLObject sbhl_sibling_disjoint_relation_with_backward_nodes_p(final SubLObject module, final SubLObject node, final SubLObject not_node, SubLObject mt, SubLObject tv)
  {
    ...
    if( NIL != sd_boolean_fn )
    {
      ...
      {
        if( NIL != Functions.funcall( sd_any_boolean_fn, backward_nodes, not_node, mt ) )
        {
          result = T;
        }
      }
      else
      {
        ...
        if( NIL != set.set_p( sol ) )
        {
          ...
          for( basis_object = set_contents.do_set_contents_basis_object( set_contents_var ), state = NIL, state = set_contents.do_set_contents_initial_state( basis_object, set_contents_var ); NIL == disjointP
  ...
  public static SubLObject sbhl_sibling_disjoint_relation_tt_p(final SubLObject module, final SubLObject node, final SubLObject not_node, SubLObject mt, SubLObject tv)
  {
    ...
    else if( sol.isList() )
    {
      if( NIL == result )
      {
        ...
        for( csome_list_var = sol, link_node2 = NIL, link_node2 = csome_list_var.first(); NIL == result && NIL != csome_list_var; result = Functions.funcall( sd_boolean_fn, link_node2, not_node,
            mt ), csome_list_var = csome_list_var.rest(), link_node2 = csome_list_var.first() )
        {
        }
      }
    }
  ...
  public static SubLObject sbhl_sibling_disjoint_tt_justification(final SubLObject module, final SubLObject node, final SubLObject not_node, SubLObject mt, SubLObject tv)
  {
    ...
    if( NIL != set.set_p( sol ) )
    {
      ...
      SubLObject state;
      SubLObject link_node;
      for( basis_object = set_contents.do_set_contents_basis_object( set_contents_var ), state = NIL, state = set_contents.do_set_contents_initial_state( basis_object, set_contents_var ); NIL == justification
          && NIL == set_contents.do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
      {
        link_node = set_contents.do_set_contents_next( basis_object, state );
        if( NIL != set_contents.do_set_contents_element_validP( state, link_node ) )
        {
          ...
          {
            result = ConsesLow.nconc( sbhl_search_utilities.sbhl_handle_justification( $sym27$SBHL_PREDICATE_RELATION_P, module, node, link_node, mt, tv, NIL, T ), justification );
            sbhl_search_vars.sbhl_justification_assembled();
          }
        }
      }
    ...
    else if( sol.isList() )
    {
      if( NIL == justification )
      {
        ...
        while ( NIL == justification && NIL != csome_list_var)
        {
          ...
          if( NIL != justification )
          {
            result = ConsesLow.nconc( sbhl_search_utilities.sbhl_handle_justification( $sym27$SBHL_PREDICATE_RELATION_P, module, node, link_node2, mt, tv, NIL, T ), justification );
            sbhl_search_vars.sbhl_justification_assembled();
          }
          csome_list_var = csome_list_var.rest();
          link_node2 = csome_list_var.first();
      ...
      if( NIL == result && NIL == sbhl_search_vars.sbhl_justification_search_p() && NIL == sdc.ignoring_sdcP() )
      {
        ...
        else if( nodes.isList() )
        {
          if( NIL == result )
          {
            SubLObject csome_list_var;
            SubLObject node3;
            for( csome_list_var = nodes, node3 = NIL, node3 = csome_list_var.first(); NIL == result && NIL != csome_list_var; result = sbhl_sibling_disjoint_relation_tt_p( module, node, node3, mt,
                tv ), csome_list_var = csome_list_var.rest(), node3 = csome_list_var.first() )
            {
            }
          }
        }
      ...
      if( NIL == result && NIL == sbhl_search_vars.sbhl_justification_search_p() && NIL == sdc.ignoring_sdcP() )
      {
        ...
        else if( nodes2.isList() )
        {
          if( NIL == result )
          {
            SubLObject csome_list_var;
            SubLObject node3;
            for( csome_list_var = nodes2, node3 = NIL, node3 = csome_list_var.first(); NIL == result && NIL != csome_list_var; result = sbhl_any_with_sibling_disjoint_relation_p( tt_module, nodes1, node3, mt,
                tv ), csome_list_var = csome_list_var.rest(), node3 = csome_list_var.first() )
            {
            }
          }
  ...
  public static SubLObject why_sbhl_implied_false_predicate_relation_p(final SubLObject module, final SubLObject node1, final SubLObject node2, SubLObject mt, SubLObject tv, SubLObject behavior)
  {
    ...
    assert NIL != sbhl_module_vars.sbhl_module_p( module ) : module;
    return sbhl_search_utilities.sbhl_handle_justification( $sym30$SBHL_IMPLIED_FALSE_PREDICATE_RELATION_P, module, node1, node2, mt, tv, behavior, NIL );
  }
  ...
  public static SubLObject why_sbhl_implied_false_inverse_relation_p(final SubLObject module, final SubLObject node1, final SubLObject node2, SubLObject mt, SubLObject tv, SubLObject behavior)
  {
    ...
    assert NIL != sbhl_module_vars.sbhl_module_p( module ) : module;
    return sbhl_search_utilities.sbhl_handle_justification( $sym31$SBHL_IMPLIED_FALSE_INVERSE_RELATION_P, module, node1, node2, mt, tv, behavior, NIL );
  }
  ...
  public static SubLObject why_sbhl_implied_disjoins_relation_p(final SubLObject module, final SubLObject node1, final SubLObject node2, SubLObject mt, SubLObject tv, SubLObject behavior)
  {
    ...
    assert NIL != sbhl_module_vars.sbhl_module_p( module ) : module;
    return sbhl_search_utilities.sbhl_handle_justification( $sym32$SBHL_IMPLIED_DISJOINS_RELATION_P, module, node1, node2, mt, tv, behavior, NIL );
  }

  public static SubLObject declare_sbhl_search_implied_relations_file()
  {
    SubLFiles.declareFunction( me, "get_sbhl_sibling_disjoint_closure_fn", "GET-SBHL-SIBLING-DISJOINT-CLOSURE-FN", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_all_sibling_disjoint_nodes", "SBHL-ALL-SIBLING-DISJOINT-NODES", 2, 2, false );
    SubLFiles.declareFunction( me, "get_sbhl_sibling_disjoint_max_nodes_fn", "GET-SBHL-SIBLING-DISJOINT-MAX-NODES-FN", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_max_sibling_disjoint_nodes", "SBHL-MAX-SIBLING-DISJOINT-NODES", 2, 2, false );
    SubLFiles.declareFunction( me, "get_sbhl_sibling_disjoint_boolean_fn", "GET-SBHL-SIBLING-DISJOINT-BOOLEAN-FN", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_relation_p", "SBHL-SIBLING-DISJOINT-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "get_sbhl_sibling_disjoint_any_boolean_fn", "GET-SBHL-SIBLING-DISJOINT-ANY-BOOLEAN-FN", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_any_with_sibling_disjoint_relation_p", "SBHL-ANY-WITH-SIBLING-DISJOINT-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "get_sbhl_sibling_disjoint_any_boolean_any_fn", "GET-SBHL-SIBLING-DISJOINT-ANY-BOOLEAN-ANY-FN", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_relation_between_any_p", "SBHL-SIBLING-DISJOINT-RELATION-BETWEEN-ANY-P", 3, 2, false );
    SubLFiles.declareFunction( me, "get_sbhl_sibling_disjoint_justification_fn", "GET-SBHL-SIBLING-DISJOINT-JUSTIFICATION-FN", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_justification", "SBHL-SIBLING-DISJOINT-JUSTIFICATION", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_disjoins_of_backward_closure", "SBHL-DISJOINS-OF-BACKWARD-CLOSURE", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_gather_all_disjoins", "SBHL-GATHER-ALL-DISJOINS", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_all_sibling_disjoint_nodes_of_backward_closure", "SBHL-ALL-SIBLING-DISJOINT-NODES-OF-BACKWARD-CLOSURE", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_disjoins_of_tt_closure", "SBHL-DISJOINS-OF-TT-CLOSURE", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_all_sibling_disjoint_nodes_of_tt", "SBHL-ALL-SIBLING-DISJOINT-NODES-OF-TT", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_tt_closure_of_disjoins", "SBHL-TT-CLOSURE-OF-DISJOINS", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_all_tt_nodes_of_sibling_disjoint_nodes", "SBHL-ALL-TT-NODES-OF-SIBLING-DISJOINT-NODES", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_all_implied_forward_false_nodes", "SBHL-ALL-IMPLIED-FORWARD-FALSE-NODES", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_all_implied_backward_false_nodes", "SBHL-ALL-IMPLIED-BACKWARD-FALSE-NODES", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_all_implied_disjoins", "SBHL-ALL-IMPLIED-DISJOINS", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_implied_max_disjoins", "SBHL-IMPLIED-MAX-DISJOINS", 2, 2, false );
    SubLFiles.declareFunction( me, "sbhl_disjoins_relation_with_backward_nodes_p", "SBHL-DISJOINS-RELATION-WITH-BACKWARD-NODES-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_disjoins_with_tt_nodes_relation_p", "SBHL-DISJOINS-WITH-TT-NODES-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_with_backward_nodes_justification", "SBHL-SIBLING-DISJOINT-WITH-BACKWARD-NODES-JUSTIFICATION", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_relation_with_backward_nodes_p", "SBHL-SIBLING-DISJOINT-RELATION-WITH-BACKWARD-NODES-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_relation_tt_p", "SBHL-SIBLING-DISJOINT-RELATION-TT-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_sibling_disjoint_tt_justification", "SBHL-SIBLING-DISJOINT-TT-JUSTIFICATION", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_argumentation_false_predicate_relation_p", "SBHL-ARGUMENTATION-FALSE-PREDICATE-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_implied_false_predicate_relation_p", "SBHL-IMPLIED-FALSE-PREDICATE-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_implied_false_inverse_relation_p", "SBHL-IMPLIED-FALSE-INVERSE-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_implied_disjoins_relation_p", "SBHL-IMPLIED-DISJOINS-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_any_with_implied_false_inverse_relation_p", "SBHL-ANY-WITH-IMPLIED-FALSE-INVERSE-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_implied_false_predicate_relation_with_any_p", "SBHL-IMPLIED-FALSE-PREDICATE-RELATION-WITH-ANY-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_all_with_implied_false_inverse_relation_p", "SBHL-ALL-WITH-IMPLIED-FALSE-INVERSE-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_any_with_implied_disjoins_relation_p", "SBHL-ANY-WITH-IMPLIED-DISJOINS-RELATION-P", 3, 2, false );
    SubLFiles.declareFunction( me, "sbhl_implied_disjoins_relation_between_any_p", "SBHL-IMPLIED-DISJOINS-RELATION-BETWEEN-ANY-P", 3, 2, false );
    SubLFiles.declareFunction( me, "why_sbhl_implied_false_predicate_relation_p", "WHY-SBHL-IMPLIED-FALSE-PREDICATE-RELATION-P", 3, 3, false );
    SubLFiles.declareFunction( me, "why_sbhl_implied_false_inverse_relation_p", "WHY-SBHL-IMPLIED-FALSE-INVERSE-RELATION-P", 3, 3, false );
    SubLFiles.declareFunction( me, "why_sbhl_implied_disjoins_relation_p", "WHY-SBHL-IMPLIED-DISJOINS-RELATION-P", 3, 3, false );
    return NIL;
  }

  public static SubLObject init_sbhl_search_implied_relations_file()
  {
    $use_fast_sbhl_sibling_disjoint_any_boolean_fnP$ = SubLFiles.defparameter( "*USE-FAST-SBHL-SIBLING-DISJOINT-ANY-BOOLEAN-FN?*", T );
    return NIL;
  }
  ...
  static
  {
    me = new sbhl_search_implied_relations();
    $use_fast_sbhl_sibling_disjoint_any_boolean_fnP$ = null;
    $sym0$SBHL_MODULE_P = makeSymbol( "SBHL-MODULE-P" );
    $const1$genls = constant_handles.reader_make_constant_shell( makeString( "genls" ) );
    $sym2$ALL_SDC = makeSymbol( "ALL-SDC" );
    $sym3$MAX_SDC = makeSymbol( "MAX-SDC" );
    $sym4$SDC_ = makeSymbol( "SDC?" );
    $sym5$ANY_SDC_WRT_ = makeSymbol( "ANY-SDC-WRT?" );
    $sym6$ANY_SDC_ANY_ = makeSymbol( "ANY-SDC-ANY?" );
    $sym7$WHY_SDC_ = makeSymbol( "WHY-SDC?" );
    $sym8$SBHL_GATHER_ALL_DISJOINS = makeSymbol( "SBHL-GATHER-ALL-DISJOINS" );
    ...
    $kw11$RESOURCE = makeKeyword( "RESOURCE" );
    ...
    $str22$Method_only_valid_for_simple_and_ = makeString( "Method only valid for simple and transfer-through modules, not ~a" );
    $kw23$BOOLEAN = makeKeyword( "BOOLEAN" );
    $sym24$SBHL_NODE_IS_GOAL_NODE = makeSymbol( "SBHL-NODE-IS-GOAL-NODE" );
    $const25$negationInverse = constant_handles.reader_make_constant_shell( makeString( "negationInverse" ) );
    $sym26$SBHL_NODE_MARKED_AS_GOAL_NODE = makeSymbol( "SBHL-NODE-MARKED-AS-GOAL-NODE" );
    $sym27$SBHL_PREDICATE_RELATION_P = makeSymbol( "SBHL-PREDICATE-RELATION-P" );
    $str28$Method_only_valid_for_disjoins_mo = makeString( "Method only valid for disjoins modules, not ~a" );
    $str29$Boolean_methods_using_ALL_are_cur = makeString( "Boolean methods using ALL are currently unsupported for justifications." );
    $sym30$SBHL_IMPLIED_FALSE_PREDICATE_RELATION_P = makeSymbol( "SBHL-IMPLIED-FALSE-PREDICATE-RELATION-P" );
    $sym31$SBHL_IMPLIED_FALSE_INVERSE_RELATION_P = makeSymbol( "SBHL-IMPLIED-FALSE-INVERSE-RELATION-P" );
    $sym32$SBHL_IMPLIED_DISJOINS_RELATION_P = makeSymbol( "SBHL-IMPLIED-DISJOINS-RELATION-P" );
  }
}
/*
 * 
