      ...
      SubLTranslatedFile
{
  ...
  {
    if( NIL == conses_high.member( collection, $additional_fort_typing_collections$.getGlobalValue(), Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
    {
      $additional_fort_typing_collections$.setGlobalValue( ConsesLow.cons( collection, $additional_fort_typing_collections$.getGlobalValue() ) );
    }
    $cached_isas$.setGlobalValue( ConsesLow.append( $definitional_fort_typing_collections$.getGlobalValue(), $additional_fort_typing_collections$.getGlobalValue(), $application_specific_fort_typing_collections$
        .getGlobalValue(), $implicit_fort_typing_collections$.getGlobalValue() ) );
    $cached_isas_set$.setGlobalValue( set_utilities.construct_set_from_list( Sequences.reverse( $cached_isas$.getGlobalValue() ), Symbols.symbol_function( EQL ), UNPROVIDED ) );
    SubLObject cdolist_list_var;
    final SubLObject all_instances = cdolist_list_var = list_utilities.remove_if_not( $sym13$FORT_P, isa.all_instances_in_all_mts( collection ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    SubLObject instance = NIL;
    instance = cdolist_list_var.first();
    while ( NIL != cdolist_list_var)
    {
      reset_cached_sbhl_relations_for_node( $const14$isa, instance, T );
  ...
  {
    return cached_nodeP( type, $const14$isa );
  }
  ...
  {
    if( NIL == conses_high.member( collection, $cached_quoted_isas$.getGlobalValue(), Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
    {
      $cached_quoted_isas$.setGlobalValue( ConsesLow.cons( collection, $cached_quoted_isas$.getGlobalValue() ) );
    }
    $cached_quoted_isas_set$.setGlobalValue( set_utilities.construct_set_from_list( Sequences.reverse( $cached_quoted_isas$.getGlobalValue() ), Symbols.symbol_function( EQL ), UNPROVIDED ) );
    SubLObject cdolist_list_var;
    final SubLObject all_quoted_instances = cdolist_list_var = list_utilities.remove_if_not( $sym13$FORT_P, isa.all_quoted_instances_in_all_mts( collection ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    SubLObject quoted_instance = NIL;
    quoted_instance = cdolist_list_var.first();
    while ( NIL != cdolist_list_var)
    {
      reset_cached_sbhl_relations_for_node( $const19$quotedIsa, quoted_instance, T );
      cdolist_list_var = cdolist_list_var.rest();
      quoted_instance = cdolist_list_var.first();
    }
    return collection;
  ...
  public static SubLObject do_sbhl_cached_subsumption_nodes(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      current = ( body = temp );
      return ConsesLow.listS( $sym24$DO_SET, ConsesLow.list( node_var, ConsesLow.list( $sym25$GET_CACHED_NODES_SET_FOR_PRED, pred ) ), ConsesLow.append( body, NIL ) );
    }
  ...
  {
    return cached_nodeP( pred, $const27$genlPreds );
  }
  ...
  {
    return cached_nodeP( col, $const14$isa );
  }
  ...
  {
    return cached_nodeP( col, $const26$genls );
  }
  ...
  {
    return subl_promotions.memberP( pred, $cached_preds$.getGlobalValue(), Symbols.symbol_function( EQL ), UNPROVIDED );
  }
  ...
  public static SubLObject print_sbhl_cache_misses_top_n(SubLObject n)
  {
    ...
    {
      SubLObject cdolist_list_var = list_utilities.first_n( n, Sort.sort( Sequences.remove_duplicates( hash_table_utilities.hash_table_values( $sbhl_cache_misses_hash_table$.getDynamicValue( thread ) ), UNPROVIDED,
          UNPROVIDED, UNPROVIDED, UNPROVIDED ), $sym31$_, UNPROVIDED ) );
      SubLObject v = NIL;
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          PrintLow.format( T, $str30$_A___S__, v, k );
  ...
  public static SubLObject sbhl_cache_use_possible_p(final SubLObject pred, final SubLObject node1, final SubLObject node2)
  {
    ...
    {
      hash_table_utilities.cinc_hash( ConsesLow.list( pred, node2 ), $sbhl_cache_misses_hash_table$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED );
    }
    return result;
  }
  ...
  {
    return makeBoolean( NIL != sbhl_paranoia.check_sbhl_cachesP() && NIL != sbhl_pred_has_caching_p( pred ) && NIL != list_utilities.every_in_list( $sym13$FORT_P, nodes, UNPROVIDED ) && NIL != cached_nodeP( node,
        pred ) );
  }
  ...
  {
    assert NIL != sbhl_module_utilities.sbhl_predicate_p( pred ) : pred;
  ...
  public static SubLObject do_sbhl_cached_relations_for_node(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym35$DO_DICTIONARY, ConsesLow.list( cached_node_var, subnodes_var, ConsesLow.list( $sym36$GET_SBHL_CACHE_FOR_PRED, pred ) ), ConsesLow.list( $sym37$PWHEN, ConsesLow.list(
          $sym38$DICTIONARY_P, subnodes_var ), ConsesLow.list( $sym39$CLET, ConsesLow.list( ConsesLow.list( mts_var, ConsesLow.list( $sym40$DICTIONARY_LOOKUP, subnodes_var, subnode ) ) ), ConsesLow.listS( $sym37$PWHEN,
              mts_var, ConsesLow.append( body, NIL ) ) ) ) );
    }
  ...
  public static SubLObject cached_relations_for_node(final SubLObject pred, final SubLObject node)
  {
    ...
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( get_sbhl_cache_for_pred( pred ) ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      ...
      if( NIL != dictionary.dictionary_p( subnodes_var ) )
      {
        ...
        {
          result = ConsesLow.cons( cached_node, result );
        }
      }
    }
  ...
  {
    return subl_promotions.memberP( mt, dictionary.dictionary_lookup( dictionary.dictionary_lookup( get_sbhl_cache_for_pred( pred ), node, UNPROVIDED ), subnode, UNPROVIDED ), $sym41$HLMT_EQUAL, UNPROVIDED );
  }
  ...
  {
    return Equality.eql( pred, $const14$isa );
  }
  ...
  public static SubLObject initialize_sbhl_caches_for_pred(final SubLObject pred)
  {
    ...
    try
    {
      ...
      utilities_macros.$noting_progress_delayed_mode_string$.bind( Sequences.cconcatenate( $str42$Initializing_SBHL_cache_for_, new SubLObject[] { format_nil.format_nil_a_no_copy( pred ), $str43$_
      } ), thread );
      final SubLObject str = Sequences.cconcatenate( $str42$Initializing_SBHL_cache_for_, new SubLObject[] { format_nil.format_nil_a_no_copy( pred ), $str43$_
      } );
  ...
  {
    possibly_initialize_sbhl_caches_for_pred( pred );
    if( NIL == dictionary_utilities.dictionary_has_keyP( get_sbhl_cache_for_pred_int( pred ), node ) && NIL != cached_nodeP( node, pred ) && NIL != kb_mapping_utilities.fpred_value_in_any_mt( node, pred, TWO_INTEGER,
  ...
  public static SubLObject initialize_sbhl_caches_for_pred_and_node(final SubLObject pred, final SubLObject node)
  {
    ...
    if( pred.eql( $const14$isa ) )
    {
      ...
      {
        utilities_macros.$noting_progress_delayed_mode_seconds$.bind( TWO_INTEGER, thread );
        utilities_macros.$noting_progress_delayed_mode_string$.bind( Sequences.cconcatenate( $str44$Adding_, new SubLObject[] { format_nil.format_nil_a_no_copy( node ), $str45$_to_SBHL_cache_for_, format_nil
            .format_nil_a_no_copy( pred ), $str43$_
        } ), thread );
        final SubLObject str = Sequences.cconcatenate( $str44$Adding_, new SubLObject[] { format_nil.format_nil_a_no_copy( node ), $str45$_to_SBHL_cache_for_, format_nil.format_nil_a_no_copy( pred ), $str43$_
        } );
    ...
    else if( pred.eql( $const19$quotedIsa ) )
    {
      final SubLObject _prev_bind_0 = utilities_macros.$noting_progress_delayed_mode_seconds$.currentBinding( thread );
      final SubLObject _prev_bind_2 = utilities_macros.$noting_progress_delayed_mode_string$.currentBinding( thread );
      try
  ...
  public static SubLObject cached_all_mts_relations_for_node(final SubLObject pred, final SubLObject node)
  {
    ...
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( get_sbhl_all_mts_cache_for_pred( pred ) ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
      final SubLObject cached_node = dictionary_contents.do_dictionary_contents_key_value( iteration_state );
      ...
      {
        cached_relations = ConsesLow.cons( cached_node, cached_relations );
      }
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  public static SubLObject do_sbhl_cached_all_mts_relations(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym39$CLET, ConsesLow.list( ConsesLow.list( v_cache, ConsesLow.list( $sym50$GET_SBHL_ALL_MTS_CACHE_FOR_PRED, pred ) ) ), ConsesLow.list( $sym51$PIF, ConsesLow.list( $sym52$FORT_ID_INDEX_P,
          v_cache ), ConsesLow.list( $sym53$DO_FORT_ID_INDEX, ConsesLow.list( subnode_var, nodes, v_cache ), ConsesLow.listS( $sym24$DO_SET, ConsesLow.list( link_node_var, nodes ), ConsesLow.append( body, NIL ) ) ),
          ConsesLow.list( $sym35$DO_DICTIONARY, ConsesLow.list( link_node_var, subnodes, v_cache ), ConsesLow.listS( $sym24$DO_SET, ConsesLow.list( subnode_var, subnodes ), ConsesLow.append( body, NIL ) ) ) ) );
    }
  ...
  public static SubLObject do_sbhl_cached_link_nodes_for_node_in_mt(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym39$CLET, ConsesLow.list( ConsesLow.list( v_cache, ConsesLow.list( $sym36$GET_SBHL_CACHE_FOR_PRED, pred ) ) ), ConsesLow.list( $sym35$DO_DICTIONARY, ConsesLow.list( cached_link_node,
          cached_nodes_store, v_cache ), ConsesLow.listS( $sym37$PWHEN, ConsesLow.list( $sym57$MEMBER_, mt, ConsesLow.list( $sym40$DICTIONARY_LOOKUP, cached_nodes_store, subnode ) ), ConsesLow.append( body, NIL ) ) ) );
    }
  ...
  public static SubLObject do_sbhl_cached_all_mts_relations_for_node(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym39$CLET, ConsesLow.list( ConsesLow.list( v_cache, ConsesLow.list( $sym50$GET_SBHL_ALL_MTS_CACHE_FOR_PRED, pred ) ) ), ConsesLow.list( $sym51$PIF, ConsesLow.list(
          $sym61$SBHL_PRED_ALL_MTS_CACHE_USES_ID_INDEX_P, pred ), ConsesLow.listS( $sym24$DO_SET, ConsesLow.list( cached_link_node, ConsesLow.list( $sym62$SBHL_ID_INDEX_LOOKUP, v_cache, subnode ) ), ConsesLow.append(
              body, NIL ) ), ConsesLow.list( $sym35$DO_DICTIONARY, ConsesLow.list( cached_link_node, subnodes, v_cache ), ConsesLow.listS( $sym37$PWHEN, ConsesLow.list( $sym63$SET_MEMBER_, subnode, subnodes ), ConsesLow
                  .append( body, NIL ) ) ) ) );
    }
  ...
  {
    reset_sbhl_cache_for_pred( $const27$genlPreds );
    reset_sbhl_cache_for_pred( $const28$genlInverse );
    reset_sbhl_cache_for_pred( $const26$genls );
    reset_sbhl_cache_for_pred( $const14$isa );
    reset_sbhl_cache_for_pred( $const19$quotedIsa );
  ...
  public static SubLObject reset_sbhl_cache_for_pred(final SubLObject pred)
  {
    ...
    else if( pred.eql( $const27$genlPreds ) )
    {
      $genl_predicate_cache$.setGlobalValue( new_sbhl_cache() );
    }
    else if( pred.eql( $const28$genlInverse ) )
    {
      $genl_inverse_cache$.setGlobalValue( new_sbhl_cache() );
    }
    else if( pred.eql( $const26$genls ) )
    {
  ...
  {
    reset_sbhl_all_mts_cache_for_pred( $const27$genlPreds );
    reset_sbhl_all_mts_cache_for_pred( $const28$genlInverse );
    reset_sbhl_all_mts_cache_for_pred( $const26$genls );
    reset_sbhl_all_mts_cache_for_pred( $const14$isa );
    reset_sbhl_all_mts_cache_for_pred( $const19$quotedIsa );
  ...
  {
    $implicit_fort_type_mapping$.setGlobalValue( dictionary.new_dictionary( Symbols.symbol_function( EQL ), UNPROVIDED ) );
    dictionary.dictionary_enter( $implicit_fort_type_mapping$.getGlobalValue(), $const65$Relation, ConsesLow.list( $const66$Predicate, $const67$Function_Denotational, $const68$LogicalConnective, $const69$Quantifier ) );
  ...
  public static SubLObject initialize_all_sbhl_caching()
  {
    ...
    try
    {
      ...
      try
      {
        utilities_macros.$progress_note$.bind( $str70$Computing_all_mts_SBHL_caches_, thread );
        utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
      ...
      try
      {
        utilities_macros.$progress_note$.bind( $str72$Computing_SBHL_caches_from_all_mt, thread );
        utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
        utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
        utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
        utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
  ...
  public static SubLObject initialize_all_mts_caching_for_pred(final SubLObject pred)
  {
    ...
    try
    {
      ...
      mt_relevance_macros.$relevant_mt_function$.bind( $sym73$RELEVANT_MT_IS_EVERYTHING, thread );
      mt_relevance_macros.$mt$.bind( $const74$EverythingPSC, thread );
      if( pred.eql( $const27$genlPreds ) || pred.eql( $const28$genlInverse ) )
      {
  ...
  public static SubLObject add_to_sbhl_cache(final SubLObject pred, final SubLObject node, final SubLObject subnode, final SubLObject mt)
  {
    ...
    if( NIL == subnodes )
    {
      subnodes = dictionary.new_dictionary( Symbols.symbol_function( EQL ), UNPROVIDED );
    }
    dictionary_utilities.dictionary_pushnew( subnodes, subnode, mt, Symbols.symbol_function( EQL ), UNPROVIDED );
  ...
  public static SubLObject add_to_sbhl_all_mts_cache(final SubLObject pred, final SubLObject node, final SubLObject subnode)
  {
    ...
    if( NIL != sbhl_pred_all_mts_cache_uses_id_index_p( pred ) )
    {
      ...
      if( NIL == nodes )
      {
        nodes = set.new_set( Symbols.symbol_function( EQL ), UNPROVIDED );
      }
    ...
    else
    {
      ...
      if( NIL == subnodes )
      {
        subnodes = set.new_set( Symbols.symbol_function( EQL ), UNPROVIDED );
      }
  ...
  {
    if( NIL != recache_sbhl_cachesP() && NIL != assertions_high.assertion_has_truth( assertion, $kw75$TRUE ) )
    {
      final SubLObject pcase_var;
      final SubLObject pred = pcase_var = assertions_high.gaf_predicate( assertion );
      if( pcase_var.eql( $const14$isa ) )
      {
        isa_cache_addition_maintainence( assertion );
      }
      else if( pcase_var.eql( $const19$quotedIsa ) )
      {
        quoted_isa_cache_addition_maintainence( assertion );
      }
      else if( pcase_var.eql( $const26$genls ) )
      {
        genls_cache_addition_maintainence( assertion );
      }
      else if( pcase_var.eql( $const27$genlPreds ) )
      {
        sbhl_genl_preds_cache_addition_maintainence( assertion );
      }
      else if( pcase_var.eql( $const28$genlInverse ) )
      {
        sbhl_genl_preds_cache_addition_maintainence( assertion );
      }
    }
  ...
  public static SubLObject possibly_add_to_sbhl_caches(final SubLObject assertion, final SubLObject term2_check_pred, final SubLObject cache_pred)
  {
    ...
    try
    {
      ...
      SubLObject cdolist_list_var;
      final SubLObject cached_relations_gained = cdolist_list_var = conses_high.set_difference( new_cached_relations, old_cached_relations, Symbols.symbol_function( EQL ), UNPROVIDED );
  ...
  public static SubLObject possibly_remove_from_sbhl_caches(final SubLObject pred, final SubLObject assertion)
  {
    ...
    try
    {
      ...
      SubLObject cdolist_list_var;
      final SubLObject cached_relations_lost = cdolist_list_var = conses_high.set_difference( current_cached_relations, new_cached_relations, Symbols.symbol_function( EQL ), UNPROVIDED );
      ...
      final SubLObject v_cache = get_sbhl_cache_for_pred( pred );
      SubLObject iteration_state;
      for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( v_cache ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
        thread.resetMultipleValues();
  ...
  public static SubLObject clear_sbhl_cached_all_mts_relations_for_node(final SubLObject node, final SubLObject pred)
  {
    ...
    else
    {
      ...
      {
        SubLObject iteration_state;
        for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( v_cache_$13 ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
            iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
        {
          thread.resetMultipleValues();
  ...
  public static SubLObject reset_cached_sbhl_relations_for_node(final SubLObject pred, final SubLObject node, SubLObject clearP)
  {
    ...
    try
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          add_to_sbhl_cache( pred, node, node, $const76$UniversalVocabularyMt );
        }
        else
        {
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject set_var = get_cached_nodes_set_for_pred( $const27$genlPreds );
    final SubLObject set_contents_var = set.do_set_internal( set_var );
    SubLObject basis_object;
    SubLObject state;
    SubLObject pred;
        ...
        try
        {
          ...
          try
          {
            ...
            try
            {
              ...
              sbhl_search_vars.$relevant_sbhl_tv_function$.bind( ( NIL != tv_var ) ? $sym80$RELEVANT_SBHL_TV_IS_GENERAL_TV : sbhl_search_vars.$relevant_sbhl_tv_function$.getDynamicValue( thread ), thread );
              if( NIL != tv_var && NIL != sbhl_paranoia.sbhl_object_type_checking_p() && NIL == sbhl_search_vars.sbhl_true_tv_p( tv_var ) )
              {
                pcase_var = sbhl_paranoia.$sbhl_type_error_action$.getDynamicValue( thread );
                if( pcase_var.eql( $kw81$ERROR ) )
                {
                  sbhl_paranoia.sbhl_error( ONE_INTEGER, $str82$_A_is_not_a__A, tv_var, $sym83$SBHL_TRUE_TV_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                }
                else if( pcase_var.eql( $kw84$CERROR ) )
                {
                  sbhl_paranoia.sbhl_cerror( ONE_INTEGER, $str85$continue_anyway, $str82$_A_is_not_a__A, tv_var, $sym83$SBHL_TRUE_TV_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                }
                else if( pcase_var.eql( $kw86$WARN ) )
                {
                  Errors.warn( $str82$_A_is_not_a__A, tv_var, $sym83$SBHL_TRUE_TV_P );
                }
                else
                {
                  Errors.warn( $str87$_A_is_not_a_valid__sbhl_type_erro, sbhl_paranoia.$sbhl_type_error_action$.getDynamicValue( thread ) );
                  Errors.cerror( $str85$continue_anyway, $str82$_A_is_not_a__A, tv_var, $sym83$SBHL_TRUE_TV_P );
                }
              }
              ...
              try
              {
                    ...
                    for( node_and_predicate_mode = ConsesLow.list( pred, sbhl_search_vars.genl_inverse_mode_p() ); NIL != node_and_predicate_mode; node_and_predicate_mode = deck.deck_pop( recur_deck ) )
                    {
                      ...
                      try
                      {
                        ...
                        if( NIL != forts.fort_p( spec ) )
                        {
                          ...
                          {
                            add_to_sbhl_all_mts_cache( $const28$genlInverse, pred, spec );
                          }
                          else
                          {
                            add_to_sbhl_all_mts_cache( $const27$genlPreds, pred, spec );
                          }
                        }
                        accessible_modules = ( cdolist_list_var = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_vars.get_sbhl_module( $const27$genlPreds ) ) );
                        module_var = NIL;
                        ...
                        while ( NIL != cdolist_list_var)
                        {
                          ...
                          try
                          {
                                          ...
                                          if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv ) )
                                          {
                                            ...
                                            try
                                            {
                                              ...
                                              if( NIL != set.set_p( sol ) )
                                              {
                                                ...
                                                for( basis_object_$28 = set_contents.do_set_contents_basis_object( set_contents_var_$27 ), state_$29 = NIL, state_$29 = set_contents.do_set_contents_initial_state(
                                                    basis_object_$28, set_contents_var_$27 ); NIL == set_contents.do_set_contents_doneP( basis_object_$28, state_$29 ); state_$29 = set_contents
                                                        .do_set_contents_update_state( state_$29 ) )
                                                {
                                                  node_vars_link_node = set_contents.do_set_contents_next( basis_object_$28, state_$29 );
                                                  if( NIL != set_contents.do_set_contents_element_validP( state_$29, node_vars_link_node ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p(
                            ...
                            {
                              new_list = ( cdolist_list_var_$30 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities.get_sbhl_module_relevant_naut_link_generators(
                                ...
                                try
                                {
                                  ...
                                  if( NIL != set.set_p( sol2 ) )
                                  {
                                        ...
                                        set_contents_var_$28 ); NIL == set_contents.do_set_contents_doneP( basis_object_$29, state_$30 ); state_$30 = set_contents.do_set_contents_update_state( state_$30 ) )
                                    {
                                      ...
                                      if( NIL != set_contents.do_set_contents_element_validP( state_$30, node_vars_link_node3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( node_vars_link_node3,
                                          UNPROVIDED ) )
                                      {
                ...
                {
                  sbhl_paranoia.sbhl_warn( TWO_INTEGER, $str90$Node__a_does_not_pass_sbhl_type_t, pred, sbhl_module_utilities.get_sbhl_type_test( sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ), UNPROVIDED, UNPROVIDED,
                      UNPROVIDED );
                }
              }
              finally
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject v_cache = get_sbhl_all_mts_cache_for_pred( $const27$genlPreds );
    if( NIL != forts.fort_id_index_p( v_cache ) )
    {
      final SubLObject fii = v_cache;
      SubLObject idx = forts.do_fii_get_constants( fii );
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        final SubLObject idx_$36 = idx;
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$36, $kw91$SKIP ) )
        {
          ...
          for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
          {
            ...
            if( NIL == id_index.id_index_tombstone_p( nodes ) || NIL == id_index.id_index_skip_tombstones_p( $kw91$SKIP ) )
            {
              if( NIL != id_index.id_index_tombstone_p( nodes ) )
              {
                nodes = $kw91$SKIP;
              }
              ...
              if( NIL != spec )
              {
                    ...
                    .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state, pred ) )
                  {
                    add_to_sbhl_cache( $const27$genlPreds, pred, pred, $const76$UniversalVocabularyMt );
                    add_to_sbhl_cache_for_relevant_mts( $const27$genlPreds, pred, spec );
                  }
                }
              }
            }
            ...
            while ( Hashtables.iteratorHasNext( cdohash_iterator ))
            {
              ...
              if( NIL != spec2 )
              {
                    ...
                    set_contents_var2 ); NIL == set_contents.do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
                {
                  ...
                  {
                    add_to_sbhl_cache( $const27$genlPreds, pred2, pred2, $const76$UniversalVocabularyMt );
                    add_to_sbhl_cache_for_relevant_mts( $const27$genlPreds, pred2, spec2 );
                  }
                }
              }
            }
      ...
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$38, $kw91$SKIP ) )
        {
    ...
    else
    {
          ...
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
        ...
        SubLObject state3;
        SubLObject spec3;
        for( basis_object3 = set_contents.do_set_contents_basis_object( set_contents_var3 ), state3 = NIL, state3 = set_contents.do_set_contents_initial_state( basis_object3, set_contents_var3 ); NIL == set_contents
            .do_set_contents_doneP( basis_object3, state3 ); state3 = set_contents.do_set_contents_update_state( state3 ) )
        {
          ...
          {
            add_to_sbhl_cache( $const27$genlPreds, pred3, pred3, $const76$UniversalVocabularyMt );
            add_to_sbhl_cache_for_relevant_mts( $const27$genlPreds, pred3, spec3 );
          }
        }
      }
      dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject v_cache = get_sbhl_all_mts_cache_for_pred( $const28$genlInverse );
    if( NIL != forts.fort_id_index_p( v_cache ) )
    {
      final SubLObject fii = v_cache;
      SubLObject idx = forts.do_fii_get_constants( fii );
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$40, $kw91$SKIP ) )
        {
            ...
            if( NIL == id_index.id_index_tombstone_p( nodes ) || NIL == id_index.id_index_skip_tombstones_p( $kw91$SKIP ) )
            {
              ...
              if( NIL != spec )
              {
                    ...
                    .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state, pred ) )
                  {
                    add_to_sbhl_cache_for_relevant_mts( $const28$genlInverse, pred, spec );
                  }
                }
              }
            }
            ...
            while ( Hashtables.iteratorHasNext( cdohash_iterator ))
            {
              ...
              if( NIL != spec2 )
              {
                    ...
                    set_contents_var2 ); NIL == set_contents.do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state2, pred2 ) )
                  {
                    add_to_sbhl_cache_for_relevant_mts( $const28$genlInverse, pred2, spec2 );
                  }
                }
              }
            }
      ...
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$42, $kw91$SKIP ) )
        {
    ...
    else
    {
          ...
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
            ...
            .do_set_contents_doneP( basis_object3, state3 ); state3 = set_contents.do_set_contents_update_state( state3 ) )
        {
          ...
          if( NIL != set_contents.do_set_contents_element_validP( state3, spec3 ) )
          {
            add_to_sbhl_cache_for_relevant_mts( $const28$genlInverse, pred3, spec3 );
          }
        }
      }
      dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  public static SubLObject sbhl_genl_preds_cache_addition_maintainence(final SubLObject assertion)
  {
    ...
    if( pcase_var.eql( $const27$genlPreds ) )
    {
      final SubLObject genl_pred_extendedP = possibly_add_to_sbhl_caches( assertion, $const27$genlPreds, $const27$genlPreds );
      final SubLObject genl_inverse_extendedP = possibly_add_to_sbhl_caches( assertion, $const28$genlInverse, $const28$genlInverse );
      extendedP = makeBoolean( NIL != genl_pred_extendedP || NIL != genl_inverse_extendedP );
    }
    else if( pcase_var.eql( $const28$genlInverse ) )
    {
      final SubLObject genl_pred_extendedP = possibly_add_to_sbhl_caches( assertion, $const27$genlPreds, $const28$genlInverse );
      final SubLObject genl_inverse_extendedP = possibly_add_to_sbhl_caches( assertion, $const28$genlInverse, $const27$genlPreds );
      extendedP = makeBoolean( NIL != genl_pred_extendedP || NIL != genl_inverse_extendedP );
    }
    else
    {
  ...
  {
    final SubLObject genl_pred_retractedP = possibly_remove_from_sbhl_caches( $const27$genlPreds, assertion );
    final SubLObject genl_inverse_retractedP = possibly_remove_from_sbhl_caches( $const28$genlInverse, assertion );
  ...
  public static SubLObject reset_cached_spec_preds_and_spec_inverses(final SubLObject pred)
  {
    ...
    try
    {
      ...
      mt_relevance_macros.$mt$.bind( $const74$EverythingPSC, thread );
      sbhl_paranoia.$suspend_sbhl_cache_useP$.bind( T, thread );
      genl_predicates.map_all_spec_preds_and_inverses( $sym93$RESET_CACHED_GENL_PRED_AND_INVERSE_RELATIONS, pred, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
  ...
  {
    reset_cached_sbhl_relations_for_node( $const27$genlPreds, pred, T );
    reset_cached_sbhl_relations_for_node( $const28$genlInverse, pred, T );
    return NIL;
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject set_var = get_cached_nodes_set_for_pred( $const26$genls );
    final SubLObject set_contents_var = set.do_set_internal( set_var );
    SubLObject basis_object;
    SubLObject state;
    SubLObject genl;
        ...
        try
        {
          ...
          try
          {
            ...
            try
            {
              ...
              try
              {
                sbhl_search_vars.$sbhl_search_module$.bind( sbhl_module_vars.get_sbhl_module( $const26$genls ), thread );
                sbhl_search_vars.$sbhl_search_module_type$.bind( sbhl_module_utilities.get_sbhl_module_type( sbhl_module_vars.get_sbhl_module( $const26$genls ) ), thread );
                sbhl_search_vars.$sbhl_add_node_to_result_test$.bind( sbhl_module_utilities.get_sbhl_add_node_to_result_test( sbhl_module_vars.get_sbhl_module( $const26$genls ) ), thread );
                sbhl_search_vars.$genl_inverse_mode_p$.bind( NIL, thread );
                sbhl_module_vars.$sbhl_module$.bind( sbhl_module_vars.get_sbhl_module( $const26$genls ), thread );
                if( NIL != sbhl_paranoia.suspend_sbhl_type_checkingP() || NIL != sbhl_module_utilities.apply_sbhl_module_type_test( genl, sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                {
                  ...
                  try
                  {
                    ...
                    while ( NIL != node_var)
                    {
                      ...
                      if( NIL != forts.fort_p( spec ) )
                      {
                        add_to_sbhl_all_mts_cache( $const26$genls, genl, spec );
                      }
                      accessible_modules = ( cdolist_list_var = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_vars.get_sbhl_module( $const26$genls ) ) );
                      module_var = NIL;
                      module_var = cdolist_list_var.first();
                      while ( NIL != cdolist_list_var)
                      {
                        ...
                        try
                        {
                                        ...
                                        if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv ) )
                                        {
                                          ...
                                          try
                                          {
                                            ...
                                            if( NIL != set.set_p( sol ) )
                                            {
                                              ...
                                              for( basis_object_$56 = set_contents.do_set_contents_basis_object( set_contents_var_$55 ), state_$57 = NIL, state_$57 = set_contents.do_set_contents_initial_state(
                                                  basis_object_$56, set_contents_var_$55 ); NIL == set_contents.do_set_contents_doneP( basis_object_$56, state_$57 ); state_$57 = set_contents.do_set_contents_update_state(
                                                      state_$57 ) )
                                              {
                                                node_vars_link_node = set_contents.do_set_contents_next( basis_object_$56, state_$57 );
                                                if( NIL != set_contents.do_set_contents_element_validP( state_$57, node_vars_link_node ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p(
                                                    node_vars_link_node, UNPROVIDED ) )
                                                {
                                                  sbhl_marking_utilities.sbhl_mark_node_marked( node_vars_link_node, UNPROVIDED );
                          ...
                          {
                            new_list = ( cdolist_list_var_$58 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities.get_sbhl_module_relevant_naut_link_generators(
                                sbhl_link_vars.get_sbhl_link_direction(), sbhl_search_vars.$sbhl_tv$.getDynamicValue( thread ), sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                                : sbhl_module_utilities.get_sbhl_module_relevant_naut_link_generators( sbhl_link_vars.get_sbhl_link_direction(), sbhl_search_vars.$sbhl_tv$.getDynamicValue( thread ), sbhl_module_vars
                                    .get_sbhl_module( UNPROVIDED ) ) ) );
                            generating_fn = NIL;
                              ...
                              try
                              {
                                ...
                                if( NIL != set.set_p( sol2 ) )
                                {
                                      ...
                                      set_contents_var_$56 ); NIL == set_contents.do_set_contents_doneP( basis_object_$57, state_$58 ); state_$58 = set_contents.do_set_contents_update_state( state_$58 ) )
                                  {
                                    ...
                                    if( NIL != set_contents.do_set_contents_element_validP( state_$58, node_vars_link_node3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( node_vars_link_node3,
                                        UNPROVIDED ) )
                                    {
                                      sbhl_marking_utilities.sbhl_mark_node_marked( node_vars_link_node3, UNPROVIDED );
                ...
                {
                  sbhl_paranoia.sbhl_warn( TWO_INTEGER, $str90$Node__a_does_not_pass_sbhl_type_t, genl, sbhl_module_utilities.get_sbhl_type_test( sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ), UNPROVIDED, UNPROVIDED,
                      UNPROVIDED );
                }
              }
              finally
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject v_cache = get_sbhl_all_mts_cache_for_pred( $const26$genls );
    if( NIL != forts.fort_id_index_p( v_cache ) )
    {
      final SubLObject fii = v_cache;
      SubLObject idx = forts.do_fii_get_constants( fii );
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$64, $kw91$SKIP ) )
        {
            ...
            if( NIL == id_index.id_index_tombstone_p( nodes ) || NIL == id_index.id_index_skip_tombstones_p( $kw91$SKIP ) )
            {
              ...
              if( NIL != spec )
              {
                    ...
                    .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state, genl ) )
                  {
                    add_to_sbhl_cache( $const26$genls, genl, genl, $const76$UniversalVocabularyMt );
                    add_to_sbhl_cache_for_relevant_mts( $const26$genls, genl, spec );
                  }
                }
              }
            }
        ...
        if( NIL == id_index.id_index_sparse_objects_empty_p( idx_$65 ) )
        {
            ...
            while ( Hashtables.iteratorHasNext( cdohash_iterator ))
            {
              ...
              if( NIL != spec2 )
              {
                    ...
                    set_contents_var2 ); NIL == set_contents.do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
                {
                  ...
                  {
                    add_to_sbhl_cache( $const26$genls, genl2, genl2, $const76$UniversalVocabularyMt );
                    add_to_sbhl_cache_for_relevant_mts( $const26$genls, genl2, spec2 );
                  }
                }
              }
            }
      ...
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$66, $kw91$SKIP ) )
        {
    ...
    else
    {
          ...
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
            ...
            .do_set_contents_doneP( basis_object3, state3 ); state3 = set_contents.do_set_contents_update_state( state3 ) )
        {
          ...
          if( NIL != set_contents.do_set_contents_element_validP( state3, spec3 ) )
          {
            add_to_sbhl_cache( $const26$genls, genl3, genl3, $const76$UniversalVocabularyMt );
            add_to_sbhl_cache_for_relevant_mts( $const26$genls, genl3, spec3 );
          }
        }
      }
      dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  public static SubLObject genls_cache_addition_maintainence(final SubLObject assertion)
  {
    ...
    SubLObject extendedP = NIL;
    extendedP = possibly_add_to_sbhl_caches( assertion, $const26$genls, $const26$genls );
    if( NIL != extendedP )
    {
      reset_cached_genls_of_all_specs( spec );
    }
    final SubLObject new_cached_isas = sbhl_predicate_relation_to_which_cached_nodes( $const26$genls, spec, $const14$isa );
    final SubLObject old_cached_isas = sbhl_predicate_relation_to_which_cached_nodes_excluding_link_node( $const26$genls, spec, $const14$isa, genl );
    SubLObject types_gained = conses_high.set_difference( new_cached_isas, old_cached_isas, Symbols.symbol_function( EQL ), UNPROVIDED );
    if( NIL != types_gained )
    {
      reset_sbhl_types_of_all_instances( spec );
    }
    final SubLObject new_cached_quoted_isas = sbhl_predicate_relation_to_which_cached_nodes( $const26$genls, spec, $const19$quotedIsa );
    final SubLObject old_cached_quoted_isas = sbhl_predicate_relation_to_which_cached_nodes_excluding_link_node( $const26$genls, spec, $const19$quotedIsa, genl );
    types_gained = conses_high.set_difference( new_cached_quoted_isas, old_cached_quoted_isas, Symbols.symbol_function( EQL ), UNPROVIDED );
    if( NIL != types_gained )
    {
  ...
  public static SubLObject genls_cache_removal_maintainence(final SubLObject assertion)
  {
    ...
    final SubLObject genl = assertions_high.gaf_arg2( assertion );
    SubLObject retractedP = NIL;
    retractedP = possibly_remove_from_sbhl_caches( $const26$genls, assertion );
    if( NIL != retractedP )
    {
      reset_cached_genls_of_all_specs( spec );
    }
    final SubLObject new_cached_isas = sbhl_predicate_relation_to_which_cached_nodes( $const26$genls, spec, $const14$isa );
    final SubLObject old_cached_isas = sbhl_predicate_relation_to_which_cached_nodes( $const26$genls, genl, $const14$isa );
    ...
    final SubLObject new_cached_quoted_isas = sbhl_predicate_relation_to_which_cached_nodes( $const26$genls, spec, $const19$quotedIsa );
    final SubLObject old_cached_quoted_isas = sbhl_predicate_relation_to_which_cached_nodes( $const26$genls, genl, $const19$quotedIsa );
    types_lost = conses_high.set_difference( old_cached_quoted_isas, new_cached_quoted_isas, UNPROVIDED, UNPROVIDED );
    if( NIL != types_lost )
    {
      reset_sbhl_types_of_all_quoted_instances( spec );
    }
  ...
  public static SubLObject reset_sbhl_types_of_all_instances(final SubLObject col)
  {
    ...
    try
    {
      ...
      mt_relevance_macros.$mt$.bind( $const74$EverythingPSC, thread );
      sbhl_paranoia.$suspend_sbhl_cache_useP$.bind( T, thread );
      isa.map_all_instances( $sym95$RESET_CACHED_ISA_RELATIONS, col, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      sbhl_paranoia.$suspend_sbhl_cache_useP$.rebind( _prev_bind_3, thread );
  ...
  public static SubLObject reset_sbhl_types_of_all_quoted_instances(final SubLObject col)
  {
    ...
    try
    {
      ...
      mt_relevance_macros.$mt$.bind( $const74$EverythingPSC, thread );
      sbhl_paranoia.$suspend_sbhl_cache_useP$.bind( T, thread );
      isa.map_all_instances( $sym96$RESET_CACHED_QUOTED_ISA_RELATIONS, col, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      sbhl_paranoia.$suspend_sbhl_cache_useP$.rebind( _prev_bind_3, thread );
  ...
  public static SubLObject reset_cached_genls_of_all_specs(final SubLObject col)
  {
    ...
    try
    {
      ...
      mt_relevance_macros.$mt$.bind( $const74$EverythingPSC, thread );
      sbhl_paranoia.$suspend_sbhl_cache_useP$.bind( T, thread );
      genls.map_all_specs( $sym97$RESET_CACHED_GENLS_RELATIONS, col, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      sbhl_paranoia.$suspend_sbhl_cache_useP$.rebind( _prev_bind_3, thread );
  ...
  {
    reset_cached_sbhl_relations_for_node( $const26$genls, col, T );
    return NIL;
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject set_var = get_cached_nodes_set_for_pred( $const14$isa );
    final SubLObject set_contents_var = set.do_set_internal( set_var );
    SubLObject basis_object;
    SubLObject state;
    SubLObject type;
              ...
              try
              {
                ...
                try
                {
                  ...
                  try
                  {
                    ...
                    if( NIL != sbhl_paranoia.suspend_sbhl_type_checkingP() || NIL != sbhl_module_utilities.apply_sbhl_module_type_test( node_var, sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                    {
                      ...
                      try
                      {
                        ...
                        while ( NIL != node_var_$68)
                        {
                          ...
                          accessible_modules = ( cdolist_list_var = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_vars.get_sbhl_module( $const14$isa ) ) );
                          module_var = NIL;
                          module_var = cdolist_list_var.first();
                          while ( NIL != cdolist_list_var)
                          {
                            ...
                            try
                            {
                              ...
                              if( NIL != sbhl_link_vars.sbhl_node_object_p( node ) )
                              {
                                ...
                                if( NIL != d_link )
                                {
                                  mt_links = sbhl_links.get_sbhl_mt_links( d_link, sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module( $const14$isa ) ), sbhl_module_vars
                                      .get_sbhl_module( UNPROVIDED ) );
                                  if( NIL != mt_links )
                                  {
                                    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( mt_links ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
                                        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
                                    {
                                      ...
                                      if( NIL != mt_relevance_macros.relevant_mtP( mt ) )
                                      {
                                        ...
                                        {
                                          sbhl_link_vars.$sbhl_link_mt$.bind( mt, thread );
                                          for( iteration_state_$80 = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( tv_links ) ); NIL == dictionary_contents
                                              .do_dictionary_contents_doneP( iteration_state_$80 ); iteration_state_$80 = dictionary_contents.do_dictionary_contents_next( iteration_state_$80 ) )
                                          {
                                            thread.resetMultipleValues();
                                            ...
                                            if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv ) )
                                            {
                                              ...
                                              try
                                              {
                                                ...
                                                if( NIL != set.set_p( sol ) )
                                                {
                                                  ...
                                                  for( basis_object_$83 = set_contents.do_set_contents_basis_object( set_contents_var_$82 ), state_$84 = NIL, state_$84 = set_contents.do_set_contents_initial_state(
                                                      basis_object_$83, set_contents_var_$82 ); NIL == set_contents.do_set_contents_doneP( basis_object_$83, state_$84 ); state_$84 = set_contents
                                                          .do_set_contents_update_state( state_$84 ) )
                                                  {
                                                    instance = set_contents.do_set_contents_next( basis_object_$83, state_$84 );
                                                    if( NIL != set_contents.do_set_contents_element_validP( state_$84, instance ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance,
                                                        sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                                    {
                                                      ...
                                                      {
                                                        add_to_sbhl_all_mts_cache( $const14$isa, type, instance );
                                                      }
                                                    }
                                                  }
                                                }
                                                else if( sol.isList() )
                                                {
                                                  ...
                                                  while ( NIL != csome_list_var)
                                                  {
                                                    if( NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance2, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                                    {
                                                      ...
                                                      {
                                                        add_to_sbhl_all_mts_cache( $const14$isa, type, instance2 );
                                                      }
                                                    }
                                                    csome_list_var = csome_list_var.rest();
                                                    instance2 = csome_list_var.first();
                                          ...
                                          if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv2 ) )
                                          {
                                            ...
                                            try
                                            {
                                              ...
                                              if( NIL != set.set_p( sol ) )
                                              {
                                                ...
                                                for( basis_object_$84 = set_contents.do_set_contents_basis_object( set_contents_var_$83 ), state_$85 = NIL, state_$85 = set_contents.do_set_contents_initial_state(
                                                    basis_object_$84, set_contents_var_$83 ); NIL == set_contents.do_set_contents_doneP( basis_object_$84, state_$85 ); state_$85 = set_contents
                                                        .do_set_contents_update_state( state_$85 ) )
                                                {
                                                  instance = set_contents.do_set_contents_next( basis_object_$84, state_$85 );
                                                  if( NIL != set_contents.do_set_contents_element_validP( state_$85, instance ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance,
                                                      sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                                  {
                                                    sbhl_marking_utilities.sbhl_mark_node_marked( instance, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) );
                                                    if( NIL != forts.fort_p( instance ) )
                              ...
                              {
                                new_list = ( cdolist_list_var_$91 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities
                                  ...
                                  try
                                  {
                                    ...
                                    if( NIL != set.set_p( sol2 ) )
                                    {
                                          ...
                                          set_contents_var_$84 ); NIL == set_contents.do_set_contents_doneP( basis_object_$85, state_$86 ); state_$86 = set_contents.do_set_contents_update_state( state_$86 ) )
                                      {
                                        ...
                                        if( NIL != set_contents.do_set_contents_element_validP( state_$86, instance3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance3,
                                            sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                        {
                                          ...
                                          {
                                            add_to_sbhl_all_mts_cache( $const14$isa, type, instance3 );
                                          }
                                        }
                                      }
                                    }
                                    else if( sol2.isList() )
                                    {
                                      ...
                                      while ( NIL != csome_list_var3)
                                      {
                                        if( NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance4, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                        {
                                          ...
                                          {
                                            add_to_sbhl_all_mts_cache( $const14$isa, type, instance4 );
                                          }
                                        }
                          ...
                          accessible_modules2 = ( cdolist_list_var2 = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module(
                              $const14$isa ) ) ) );
                          module_var2 = NIL;
                          ...
                          while ( NIL != cdolist_list_var2)
                          {
                            ...
                            try
                            {
                                        ...
                                        iteration_state2 ); iteration_state2 = dictionary_contents.do_dictionary_contents_next( iteration_state2 ) )
                                    {
                                      ...
                                      if( NIL != mt_relevance_macros.relevant_mtP( mt3 ) )
                                      {
                                        ...
                                        try
                                        {
                                          ...
                                          for( iteration_state_$81 = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( tv_links2 ) ); NIL == dictionary_contents
                                              .do_dictionary_contents_doneP( iteration_state_$81 ); iteration_state_$81 = dictionary_contents.do_dictionary_contents_next( iteration_state_$81 ) )
                                          {
                                            thread.resetMultipleValues();
                                            ...
                                            if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv2 ) )
                                            {
                                              ...
                                              try
                                              {
                                                ...
                                                if( NIL != set.set_p( sol3 ) )
                                                {
                                                  ...
                                                  for( basis_object_$86 = set_contents.do_set_contents_basis_object( set_contents_var_$85 ), state_$87 = NIL, state_$87 = set_contents.do_set_contents_initial_state(
                                                      basis_object_$86, set_contents_var_$85 ); NIL == set_contents.do_set_contents_doneP( basis_object_$86, state_$87 ); state_$87 = set_contents
                                                          .do_set_contents_update_state( state_$87 ) )
                                                  {
                                                    node_vars_link_node = set_contents.do_set_contents_next( basis_object_$86, state_$87 );
                                                    if( NIL != set_contents.do_set_contents_element_validP( state_$87, node_vars_link_node ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p(
                                                        node_vars_link_node, UNPROVIDED ) )
                                                    {
                                                      sbhl_marking_utilities.sbhl_mark_node_marked( node_vars_link_node, UNPROVIDED );
                                                      deck.deck_push( node_vars_link_node, recur_deck );
                              ...
                              {
                                new_list2 = ( cdolist_list_var_$92 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities
                                  ...
                                  try
                                  {
                                    ...
                                    if( NIL != set.set_p( sol4 ) )
                                    {
                                          ...
                                          set_contents_var_$86 ); NIL == set_contents.do_set_contents_doneP( basis_object_$87, state_$88 ); state_$88 = set_contents.do_set_contents_update_state( state_$88 ) )
                                      {
                                        ...
                                        if( NIL != set_contents.do_set_contents_element_validP( state_$88, node_vars_link_node3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( node_vars_link_node3,
                                            UNPROVIDED ) )
                                        {
                                          sbhl_marking_utilities.sbhl_mark_node_marked( node_vars_link_node3, UNPROVIDED );
                                          deck.deck_push( node_vars_link_node3, recur_deck );
                    ...
                    {
                      sbhl_paranoia.sbhl_warn( TWO_INTEGER, $str90$Node__a_does_not_pass_sbhl_type_t, node_var, sbhl_module_utilities.get_sbhl_type_test( sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ), UNPROVIDED,
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject v_cache = get_sbhl_all_mts_cache_for_pred( $const14$isa );
    if( NIL != forts.fort_id_index_p( v_cache ) )
    {
      final SubLObject fii = v_cache;
      SubLObject idx = forts.do_fii_get_constants( fii );
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$111, $kw91$SKIP ) )
        {
            ...
            if( NIL == id_index.id_index_tombstone_p( nodes ) || NIL == id_index.id_index_skip_tombstones_p( $kw91$SKIP ) )
            {
              ...
              if( NIL != instance )
              {
                    ...
                    .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state, type ) )
                  {
                    add_to_sbhl_cache_for_relevant_mts( $const14$isa, type, instance );
                  }
                }
              }
            }
            ...
            while ( Hashtables.iteratorHasNext( cdohash_iterator ))
            {
              ...
              if( NIL != instance2 )
              {
                    ...
                    set_contents_var2 ); NIL == set_contents.do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
                {
                  ...
                  {
                    add_to_sbhl_cache_for_relevant_mts( $const14$isa, type2, instance2 );
                  }
                }
              }
            }
      ...
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$113, $kw91$SKIP ) )
        {
    ...
    else
    {
          ...
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
            ...
            .do_set_contents_doneP( basis_object3, state3 ); state3 = set_contents.do_set_contents_update_state( state3 ) )
        {
          ...
          {
            add_to_sbhl_cache_for_relevant_mts( $const14$isa, type3, instance3 );
          }
        }
      }
      dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  {
    possibly_add_to_sbhl_caches( assertion, $const26$genls, $const14$isa );
    return NIL;
  }
  ...
  {
    possibly_remove_from_sbhl_caches( $const14$isa, assertion );
    return NIL;
  }
  ...
  {
    reset_cached_sbhl_relations_for_node( $const14$isa, node, T );
    return NIL;
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject set_var = get_cached_nodes_set_for_pred( $const19$quotedIsa );
    final SubLObject set_contents_var = set.do_set_internal( set_var );
    SubLObject basis_object;
    SubLObject state;
    SubLObject type;
        ...
        .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state, type ) )
      {
        ...
        try
        {
          sbhl_module_vars.$sbhl_module$.bind( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ), thread );
          sbhl_marking_vars.$sbhl_gather_space$.bind( sbhl_marking_vars.get_sbhl_marking_space(), thread );
          try
          {
            ...
            try
            {
              ...
              try
              {
                ...
                try
                {
                  ...
                  try
                  {
                    sbhl_search_vars.$sbhl_search_module$.bind( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ) ), thread );
                    sbhl_search_vars.$sbhl_search_module_type$.bind( sbhl_module_utilities.get_sbhl_module_type( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module(
                        $const19$quotedIsa ) ) ), thread );
                    sbhl_search_vars.$sbhl_add_node_to_result_test$.bind( sbhl_module_utilities.get_sbhl_add_node_to_result_test( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module(
                        $const19$quotedIsa ) ) ), thread );
                    sbhl_search_vars.$genl_inverse_mode_p$.bind( NIL, thread );
                    sbhl_module_vars.$sbhl_module$.bind( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ) ), thread );
                    if( NIL != sbhl_paranoia.suspend_sbhl_type_checkingP() || NIL != sbhl_module_utilities.apply_sbhl_module_type_test( node_var, sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                    {
                      ...
                      try
                      {
                        sbhl_search_vars.$sbhl_search_direction$.bind( sbhl_search_vars.get_sbhl_backward_search_direction(), thread );
                        sbhl_link_vars.$sbhl_link_direction$.bind( sbhl_module_utilities.sbhl_search_direction_to_link_direction( sbhl_search_vars.get_sbhl_backward_search_direction(), sbhl_module_utilities
                            .get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ) ) ), thread );
                        sbhl_search_vars.$genl_inverse_mode_p$.bind( NIL, thread );
                        sbhl_marking_utilities.sbhl_mark_node_marked( node_var_$115, UNPROVIDED );
                        while ( NIL != node_var_$115)
                        {
                          tt_node_var = node_var_$115;
                          accessible_modules = ( cdolist_list_var = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ) ) );
                          module_var = NIL;
                          module_var = cdolist_list_var.first();
                          while ( NIL != cdolist_list_var)
                          {
                            ...
                            try
                            {
                              ...
                              if( NIL != sbhl_link_vars.sbhl_node_object_p( node ) )
                              {
                                ...
                                if( NIL != d_link )
                                {
                                  mt_links = sbhl_links.get_sbhl_mt_links( d_link, sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ) ), sbhl_module_vars
                                      .get_sbhl_module( UNPROVIDED ) );
                                  if( NIL != mt_links )
                                  {
                                    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( mt_links ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
                                        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
                                    {
                                      ...
                                      if( NIL != mt_relevance_macros.relevant_mtP( mt ) )
                                      {
                                        ...
                                        {
                                          sbhl_link_vars.$sbhl_link_mt$.bind( mt, thread );
                                          for( iteration_state_$127 = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( tv_links ) ); NIL == dictionary_contents
                                              .do_dictionary_contents_doneP( iteration_state_$127 ); iteration_state_$127 = dictionary_contents.do_dictionary_contents_next( iteration_state_$127 ) )
                                          {
                                            thread.resetMultipleValues();
                                            ...
                                            if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv ) )
                                            {
                                              ...
                                              try
                                              {
                                                ...
                                                if( NIL != set.set_p( sol ) )
                                                {
                                                  ...
                                                  for( basis_object_$130 = set_contents.do_set_contents_basis_object( set_contents_var_$129 ), state_$131 = NIL, state_$131 = set_contents.do_set_contents_initial_state(
                                                      basis_object_$130, set_contents_var_$129 ); NIL == set_contents.do_set_contents_doneP( basis_object_$130, state_$131 ); state_$131 = set_contents
                                                          .do_set_contents_update_state( state_$131 ) )
                                                  {
                                                    instance = set_contents.do_set_contents_next( basis_object_$130, state_$131 );
                                                    if( NIL != set_contents.do_set_contents_element_validP( state_$131, instance ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance,
                                                        sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                                    {
                                                      sbhl_marking_utilities.sbhl_mark_node_marked( instance, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) );
                                                      if( NIL != forts.fort_p( instance ) )
                                                      {
                                                        add_to_sbhl_all_mts_cache( $const19$quotedIsa, type, instance );
                                                      }
                                                    }
                                                  }
                                                }
                                                else if( sol.isList() )
                                                {
                                                  ...
                                                  while ( NIL != csome_list_var)
                                                  {
                                                    if( NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance2, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                                    {
                                                      ...
                                                      if( NIL != forts.fort_p( instance2 ) )
                                                      {
                                                        add_to_sbhl_all_mts_cache( $const19$quotedIsa, type, instance2 );
                                                      }
                                                    }
                                                    csome_list_var = csome_list_var.rest();
                                                    instance2 = csome_list_var.first();
                                          ...
                                          if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv2 ) )
                                          {
                                            ...
                                            try
                                            {
                                              ...
                                              if( NIL != set.set_p( sol ) )
                                              {
                                                ...
                                                for( basis_object_$131 = set_contents.do_set_contents_basis_object( set_contents_var_$130 ), state_$132 = NIL, state_$132 = set_contents.do_set_contents_initial_state(
                                                    basis_object_$131, set_contents_var_$130 ); NIL == set_contents.do_set_contents_doneP( basis_object_$131, state_$132 ); state_$132 = set_contents
                                                        .do_set_contents_update_state( state_$132 ) )
                                                {
                                                  instance = set_contents.do_set_contents_next( basis_object_$131, state_$132 );
                                                  if( NIL != set_contents.do_set_contents_element_validP( state_$132, instance ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance,
                                                      sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                                  {
                                                    sbhl_marking_utilities.sbhl_mark_node_marked( instance, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) );
                                                    if( NIL != forts.fort_p( instance ) )
                              ...
                              {
                                new_list = ( cdolist_list_var_$138 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities
                                    .get_sbhl_module_relevant_naut_link_generators( sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module( $const19$quotedIsa ) ),
                                        sbhl_search_vars.$sbhl_tv$.getDynamicValue( thread ), sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                                    : sbhl_module_utilities.get_sbhl_module_relevant_naut_link_generators( sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module(
                                        $const19$quotedIsa ) ), sbhl_search_vars.$sbhl_tv$.getDynamicValue( thread ), sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) ) );
                                generating_fn = NIL;
                                ...
                                while ( NIL != cdolist_list_var_$138)
                                {
                                  ...
                                  try
                                  {
                                    ...
                                    if( NIL != set.set_p( sol2 ) )
                                    {
                                      ...
                                      for( basis_object_$132 = set_contents.do_set_contents_basis_object( set_contents_var_$131 ), state_$133 = NIL, state_$133 = set_contents.do_set_contents_initial_state(
                                          basis_object_$132, set_contents_var_$131 ); NIL == set_contents.do_set_contents_doneP( basis_object_$132, state_$133 ); state_$133 = set_contents.do_set_contents_update_state(
                                              state_$133 ) )
                                      {
                                        instance3 = set_contents.do_set_contents_next( basis_object_$132, state_$133 );
                                        if( NIL != set_contents.do_set_contents_element_validP( state_$133, instance3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance3,
                                            sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                        {
                                          sbhl_marking_utilities.sbhl_mark_node_marked( instance3, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) );
                                          if( NIL != forts.fort_p( instance3 ) )
                                          {
                                            add_to_sbhl_all_mts_cache( $const19$quotedIsa, type, instance3 );
                                          }
                                        }
                                      }
                                    }
                                    else if( sol2.isList() )
                                    {
                                      ...
                                      while ( NIL != csome_list_var3)
                                      {
                                        if( NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( instance4, sbhl_marking_vars.$sbhl_gather_space$.getDynamicValue( thread ) ) )
                                        {
                                          ...
                                          if( NIL != forts.fort_p( instance4 ) )
                                          {
                                            add_to_sbhl_all_mts_cache( $const19$quotedIsa, type, instance4 );
                                          }
                                        }
                                        csome_list_var3 = csome_list_var3.rest();
                                        instance4 = csome_list_var3.first();
                          ...
                          while ( NIL != cdolist_list_var2)
                          {
                            ...
                            try
                            {
                                  ...
                                  if( NIL != mt_links2 )
                                  {
                                        ...
                                        iteration_state2 ); iteration_state2 = dictionary_contents.do_dictionary_contents_next( iteration_state2 ) )
                                    {
                                      ...
                                      if( NIL != mt_relevance_macros.relevant_mtP( mt3 ) )
                                      {
                                        ...
                                        {
                                          sbhl_link_vars.$sbhl_link_mt$.bind( mt3, thread );
                                          for( iteration_state_$128 = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( tv_links2 ) ); NIL == dictionary_contents
                                              .do_dictionary_contents_doneP( iteration_state_$128 ); iteration_state_$128 = dictionary_contents.do_dictionary_contents_next( iteration_state_$128 ) )
                                          {
                                            thread.resetMultipleValues();
                                            ...
                                            if( NIL != sbhl_search_vars.relevant_sbhl_tvP( tv2 ) )
                                            {
                                              ...
                                              try
                                              {
                                                ...
                                                if( NIL != set.set_p( sol3 ) )
                                                {
                                                  ...
                                                  for( basis_object_$133 = set_contents.do_set_contents_basis_object( set_contents_var_$132 ), state_$134 = NIL, state_$134 = set_contents.do_set_contents_initial_state(
                                                      basis_object_$133, set_contents_var_$132 ); NIL == set_contents.do_set_contents_doneP( basis_object_$133, state_$134 ); state_$134 = set_contents
                                                          .do_set_contents_update_state( state_$134 ) )
                                                  {
                                                    node_vars_link_node = set_contents.do_set_contents_next( basis_object_$133, state_$134 );
                                                    if( NIL != set_contents.do_set_contents_element_validP( state_$134, node_vars_link_node ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p(
                                                        node_vars_link_node, UNPROVIDED ) )
                                                    {
                                                      sbhl_marking_utilities.sbhl_mark_node_marked( node_vars_link_node, UNPROVIDED );
                                                      deck.deck_push( node_vars_link_node, recur_deck );
                              ...
                              {
                                new_list2 = ( cdolist_list_var_$139 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities
                                    .get_sbhl_module_relevant_naut_link_generators( sbhl_link_vars.get_sbhl_link_direction(), sbhl_search_vars.$sbhl_tv$.getDynamicValue( thread ), sbhl_module_vars.get_sbhl_module(
                                        UNPROVIDED ) ) )
                                    : sbhl_module_utilities.get_sbhl_module_relevant_naut_link_generators( sbhl_link_vars.get_sbhl_link_direction(), sbhl_search_vars.$sbhl_tv$.getDynamicValue( thread ), sbhl_module_vars
                                        .get_sbhl_module( UNPROVIDED ) ) ) );
                                ...
                                while ( NIL != cdolist_list_var_$139)
                                {
                                  ...
                                  try
                                  {
                                    ...
                                    if( NIL != set.set_p( sol4 ) )
                                    {
                                      ...
                                      for( basis_object_$134 = set_contents.do_set_contents_basis_object( set_contents_var_$133 ), state_$135 = NIL, state_$135 = set_contents.do_set_contents_initial_state(
                                          basis_object_$134, set_contents_var_$133 ); NIL == set_contents.do_set_contents_doneP( basis_object_$134, state_$135 ); state_$135 = set_contents.do_set_contents_update_state(
                                              state_$135 ) )
                                      {
                                        node_vars_link_node3 = set_contents.do_set_contents_next( basis_object_$134, state_$135 );
                                        if( NIL != set_contents.do_set_contents_element_validP( state_$135, node_vars_link_node3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( node_vars_link_node3,
                                            UNPROVIDED ) )
                                        {
                                          sbhl_marking_utilities.sbhl_mark_node_marked( node_vars_link_node3, UNPROVIDED );
                                          deck.deck_push( node_vars_link_node3, recur_deck );
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject v_cache = get_sbhl_all_mts_cache_for_pred( $const19$quotedIsa );
    if( NIL != forts.fort_id_index_p( v_cache ) )
    {
      final SubLObject fii = v_cache;
      SubLObject idx = forts.do_fii_get_constants( fii );
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$158, $kw91$SKIP ) )
        {
            ...
            if( NIL == id_index.id_index_tombstone_p( nodes ) || NIL == id_index.id_index_skip_tombstones_p( $kw91$SKIP ) )
            {
              ...
              if( NIL != instance )
              {
                    ...
                    .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state, type ) )
                  {
                    add_to_sbhl_cache_for_relevant_mts( $const19$quotedIsa, type, instance );
                  }
                }
              }
            }
        ...
        if( NIL == id_index.id_index_sparse_objects_empty_p( idx_$159 ) )
        {
            ...
            while ( Hashtables.iteratorHasNext( cdohash_iterator ))
            {
              ...
              if( NIL != instance2 )
              {
                    ...
                    set_contents_var2 ); NIL == set_contents.do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
                {
                  ...
                  if( NIL != set_contents.do_set_contents_element_validP( state2, type2 ) )
                  {
                    add_to_sbhl_cache_for_relevant_mts( $const19$quotedIsa, type2, instance2 );
                  }
                }
              }
            }
      ...
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw91$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$160, $kw91$SKIP ) )
        {
        ...
        if( NIL == id_index.id_index_sparse_objects_empty_p( idx_$161 ) )
        {
    ...
    else
    {
          ...
          iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
      {
            ...
            .do_set_contents_doneP( basis_object3, state3 ); state3 = set_contents.do_set_contents_update_state( state3 ) )
        {
          ...
          if( NIL != set_contents.do_set_contents_element_validP( state3, instance3 ) )
          {
            add_to_sbhl_cache_for_relevant_mts( $const19$quotedIsa, type3, instance3 );
          }
        }
      }
      dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  {
    possibly_add_to_sbhl_caches( assertion, $const26$genls, $const19$quotedIsa );
    return NIL;
  }
  ...
  {
    possibly_remove_from_sbhl_caches( $const19$quotedIsa, assertion );
    return NIL;
  }
  ...
  {
    reset_cached_sbhl_relations_for_node( $const19$quotedIsa, node, T );
    return NIL;
  }

  public static SubLObject declare_sbhl_cache_file()
  {
    SubLFiles.declareFunction( me, "note_sbhl_caches_initialized", "NOTE-SBHL-CACHES-INITIALIZED", 0, 0, false );
    SubLFiles.declareFunction( me, "sbhl_caches_initialized_p", "SBHL-CACHES-INITIALIZED-P", 0, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cache_add_new_additional_fort_typing_collection", "SBHL-CACHE-ADD-NEW-ADDITIONAL-FORT-TYPING-COLLECTION", 1, 0, false );
    SubLFiles.declareFunction( me, "all_fort_types", "ALL-FORT-TYPES", 0, 0, false );
    SubLFiles.declareFunction( me, "valid_fort_typeP", "VALID-FORT-TYPE?", 1, 0, false );
    new $valid_fort_typeP$UnaryFunction();
    SubLFiles.declareFunction( me, "sbhl_cache_add_new_additional_fort_typing_collection_quoted", "SBHL-CACHE-ADD-NEW-ADDITIONAL-FORT-TYPING-COLLECTION-QUOTED", 1, 0, false );
    SubLFiles.declareMacro( me, "do_sbhl_cached_subsumption_nodes", "DO-SBHL-CACHED-SUBSUMPTION-NODES" );
    SubLFiles.declareFunction( me, "cached_nodeP", "CACHED-NODE?", 2, 0, false );
    SubLFiles.declareFunction( me, "sbhl_id_index_lookup", "SBHL-ID-INDEX-LOOKUP", 2, 0, false );
    SubLFiles.declareFunction( me, "get_cached_nodes_set_for_pred", "GET-CACHED-NODES-SET-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "get_sbhl_cached_nodes_for_pred", "GET-SBHL-CACHED-NODES-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "valid_cached_predicate_p", "VALID-CACHED-PREDICATE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "valid_cached_fort_type_p", "VALID-CACHED-FORT-TYPE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "valid_cached_genl_p", "VALID-CACHED-GENL-P", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_pred_has_caching_p", "SBHL-PRED-HAS-CACHING-P", 1, 0, false );
    SubLFiles.declareFunction( me, "print_sbhl_cache_misses_top_n", "PRINT-SBHL-CACHE-MISSES-TOP-N", 0, 1, false );
    SubLFiles.declareFunction( me, "sbhl_cache_use_possible_p", "SBHL-CACHE-USE-POSSIBLE-P", 3, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cache_use_possible_for_nodes_p", "SBHL-CACHE-USE-POSSIBLE-FOR-NODES-P", 3, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cached_predicate_relation_p", "SBHL-CACHED-PREDICATE-RELATION-P", 3, 1, false );
    SubLFiles.declareFunction( me, "sbhl_cached_relations_for_node", "SBHL-CACHED-RELATIONS-FOR-NODE", 2, 1, false );
    SubLFiles.declareFunction( me, "sbhl_all_subnodes_for_cached_node", "SBHL-ALL-SUBNODES-FOR-CACHED-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cached_relation_p", "SBHL-CACHED-RELATION-P", 3, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cached_relations", "SBHL-CACHED-RELATIONS", 2, 0, false );
    SubLFiles.declareFunction( me, "get_sbhl_cache_for_pred", "GET-SBHL-CACHE-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "get_sbhl_cache_for_pred_int", "GET-SBHL-CACHE-FOR-PRED-INT", 1, 0, false );
    SubLFiles.declareFunction( me, "get_mts_for_cached_sbhl_relation", "GET-MTS-FOR-CACHED-SBHL-RELATION", 3, 0, false );
    SubLFiles.declareFunction( me, "cached_relation_p", "CACHED-RELATION-P", 3, 0, false );
    SubLFiles.declareMacro( me, "do_sbhl_cached_relations_for_node", "DO-SBHL-CACHED-RELATIONS-FOR-NODE" );
    SubLFiles.declareFunction( me, "cached_relations_for_node", "CACHED-RELATIONS-FOR-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "cached_relation_in_cache_p", "CACHED-RELATION-IN-CACHE-P", 4, 0, false );
    SubLFiles.declareFunction( me, "sbhl_pred_all_mts_cache_uses_id_index_p", "SBHL-PRED-ALL-MTS-CACHE-USES-ID-INDEX-P", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_finalize_all_mts_cache", "SBHL-FINALIZE-ALL-MTS-CACHE", 1, 0, false );
    SubLFiles.declareFunction( me, "get_sbhl_all_mts_cache_for_pred", "GET-SBHL-ALL-MTS-CACHE-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "get_sbhl_all_mts_cache_for_pred_int", "GET-SBHL-ALL-MTS-CACHE-FOR-PRED-INT", 1, 0, false );
    SubLFiles.declareFunction( me, "possibly_initialize_sbhl_caches_for_pred", "POSSIBLY-INITIALIZE-SBHL-CACHES-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "initialize_sbhl_caches_for_pred", "INITIALIZE-SBHL-CACHES-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "possibly_initialize_sbhl_caches_for_pred_and_node", "POSSIBLY-INITIALIZE-SBHL-CACHES-FOR-PRED-AND-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "initialize_sbhl_caches_for_pred_and_node", "INITIALIZE-SBHL-CACHES-FOR-PRED-AND-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "cached_all_mts_relation_p", "CACHED-ALL-MTS-RELATION-P", 3, 0, false );
    SubLFiles.declareFunction( me, "cached_all_mts_relations_for_node", "CACHED-ALL-MTS-RELATIONS-FOR-NODE", 2, 0, false );
    SubLFiles.declareMacro( me, "do_sbhl_cached_all_mts_relations", "DO-SBHL-CACHED-ALL-MTS-RELATIONS" );
    SubLFiles.declareMacro( me, "do_sbhl_cached_link_nodes_for_node_in_mt", "DO-SBHL-CACHED-LINK-NODES-FOR-NODE-IN-MT" );
    SubLFiles.declareMacro( me, "do_sbhl_cached_all_mts_relations_for_node", "DO-SBHL-CACHED-ALL-MTS-RELATIONS-FOR-NODE" );
    SubLFiles.declareFunction( me, "initialize_sbhl_caches", "INITIALIZE-SBHL-CACHES", 0, 0, false );
    SubLFiles.declareFunction( me, "reset_sbhl_cache_for_pred", "RESET-SBHL-CACHE-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "initialize_all_mts_sbhl_caches", "INITIALIZE-ALL-MTS-SBHL-CACHES", 0, 0, false );
    SubLFiles.declareFunction( me, "reset_sbhl_all_mts_cache_for_pred", "RESET-SBHL-ALL-MTS-CACHE-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "new_sbhl_cache", "NEW-SBHL-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "new_sbhl_sub_cache", "NEW-SBHL-SUB-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "initialize_implicit_fort_type_mapping", "INITIALIZE-IMPLICIT-FORT-TYPE-MAPPING", 0, 0, false );
    SubLFiles.declareFunction( me, "initialize_all_sbhl_caching", "INITIALIZE-ALL-SBHL-CACHING", 0, 0, false );
    SubLFiles.declareFunction( me, "initialize_all_mts_caching_for_pred", "INITIALIZE-ALL-MTS-CACHING-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "compute_sbhl_cache_from_all_mts_cache", "COMPUTE-SBHL-CACHE-FROM-ALL-MTS-CACHE", 1, 0, false );
    SubLFiles.declareFunction( me, "extend_sbhl_caches", "EXTEND-SBHL-CACHES", 4, 0, false );
    SubLFiles.declareFunction( me, "add_to_sbhl_cache", "ADD-TO-SBHL-CACHE", 4, 0, false );
    SubLFiles.declareFunction( me, "add_to_sbhl_cache_for_relevant_mts", "ADD-TO-SBHL-CACHE-FOR-RELEVANT-MTS", 3, 0, false );
    SubLFiles.declareFunction( me, "add_to_sbhl_all_mts_cache", "ADD-TO-SBHL-ALL-MTS-CACHE", 3, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cache_addition_maintainence", "SBHL-CACHE-ADDITION-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "possibly_add_to_sbhl_caches", "POSSIBLY-ADD-TO-SBHL-CACHES", 3, 0, false );
    SubLFiles.declareFunction( me, "retract_cached_relation", "RETRACT-CACHED-RELATION", 4, 0, false );
    SubLFiles.declareFunction( me, "retract_from_sbhl_cache", "RETRACT-FROM-SBHL-CACHE", 4, 0, false );
    SubLFiles.declareFunction( me, "retract_from_sbhl_all_mts_cache", "RETRACT-FROM-SBHL-ALL-MTS-CACHE", 3, 0, false );
    SubLFiles.declareFunction( me, "sbhl_cache_removal_maintainence", "SBHL-CACHE-REMOVAL-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "possibly_remove_from_sbhl_caches", "POSSIBLY-REMOVE-FROM-SBHL-CACHES", 2, 0, false );
    SubLFiles.declareFunction( me, "recache_sbhl_cachesP", "RECACHE-SBHL-CACHES?", 0, 0, false );
    SubLFiles.declareFunction( me, "already_in_sbhl_caches_p", "ALREADY-IN-SBHL-CACHES-P", 4, 0, false );
    SubLFiles.declareFunction( me, "sbhl_predicate_relation_to_which_cached_nodes", "SBHL-PREDICATE-RELATION-TO-WHICH-CACHED-NODES", 3, 0, false );
    SubLFiles.declareFunction( me, "sbhl_predicate_relation_to_which_cached_nodes_excluding_link_node", "SBHL-PREDICATE-RELATION-TO-WHICH-CACHED-NODES-EXCLUDING-LINK-NODE", 4, 0, false );
    SubLFiles.declareFunction( me, "clear_sbhl_cached_all_mts_relations_for_node", "CLEAR-SBHL-CACHED-ALL-MTS-RELATIONS-FOR-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_sbhl_cached_relations_for_node", "CLEAR-SBHL-CACHED-RELATIONS-FOR-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_sbhl_relations_for_node", "RESET-CACHED-SBHL-RELATIONS-FOR-NODE", 2, 1, false );
    SubLFiles.declareFunction( me, "initialize_all_mts_cache_for_genl_preds_and_inverse", "INITIALIZE-ALL-MTS-CACHE-FOR-GENL-PREDS-AND-INVERSE", 0, 0, false );
    SubLFiles.declareFunction( me, "compute_cached_predicates_from_all_mts_cache", "COMPUTE-CACHED-PREDICATES-FROM-ALL-MTS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "compute_cached_inverses_from_all_mts_cache", "COMPUTE-CACHED-INVERSES-FROM-ALL-MTS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "sbhl_genl_preds_cache_addition_maintainence", "SBHL-GENL-PREDS-CACHE-ADDITION-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "sbhl_genl_preds_cache_removal_maintainence", "SBHL-GENL-PREDS-CACHE-REMOVAL-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_spec_preds_and_spec_inverses", "RESET-CACHED-SPEC-PREDS-AND-SPEC-INVERSES", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_genl_pred_and_inverse_relations", "RESET-CACHED-GENL-PRED-AND-INVERSE-RELATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "initialize_all_mts_cache_for_genls", "INITIALIZE-ALL-MTS-CACHE-FOR-GENLS", 0, 0, false );
    SubLFiles.declareFunction( me, "compute_cached_genls_from_all_mts_cache", "COMPUTE-CACHED-GENLS-FROM-ALL-MTS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "genls_cache_addition_maintainence", "GENLS-CACHE-ADDITION-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "genls_cache_removal_maintainence", "GENLS-CACHE-REMOVAL-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_sbhl_types_of_all_instances", "RESET-SBHL-TYPES-OF-ALL-INSTANCES", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_sbhl_types_of_all_quoted_instances", "RESET-SBHL-TYPES-OF-ALL-QUOTED-INSTANCES", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_genls_of_all_specs", "RESET-CACHED-GENLS-OF-ALL-SPECS", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_genls_relations", "RESET-CACHED-GENLS-RELATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "initialize_all_mts_cache_for_isa", "INITIALIZE-ALL-MTS-CACHE-FOR-ISA", 0, 0, false );
    SubLFiles.declareFunction( me, "compute_cached_isas_from_all_mts_cache", "COMPUTE-CACHED-ISAS-FROM-ALL-MTS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "isa_cache_addition_maintainence", "ISA-CACHE-ADDITION-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "isa_cache_removal_maintainence", "ISA-CACHE-REMOVAL-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_isa_relations", "RESET-CACHED-ISA-RELATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "initialize_all_mts_cache_for_quoted_isa", "INITIALIZE-ALL-MTS-CACHE-FOR-QUOTED-ISA", 0, 0, false );
    SubLFiles.declareFunction( me, "compute_cached_quoted_isas_from_all_mts_cache", "COMPUTE-CACHED-QUOTED-ISAS-FROM-ALL-MTS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "quoted_isa_cache_addition_maintainence", "QUOTED-ISA-CACHE-ADDITION-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "quoted_isa_cache_removal_maintainence", "QUOTED-ISA-CACHE-REMOVAL-MAINTAINENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "reset_cached_quoted_isa_relations", "RESET-CACHED-QUOTED-ISA-RELATIONS", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_sbhl_cache_file()
  {
    $sbhl_caches_initializedP$ = SubLFiles.deflexical( "*SBHL-CACHES-INITIALIZED?*", ( maybeDefault( $sym0$_SBHL_CACHES_INITIALIZED__, $sbhl_caches_initializedP$, NIL ) ) );
    $cached_genl_predicates$ = SubLFiles.deflexical( "*CACHED-GENL-PREDICATES*", $list1 );
    $cached_genl_predicates_set$ = SubLFiles.deflexical( "*CACHED-GENL-PREDICATES-SET*", set_utilities.construct_set_from_list( $cached_genl_predicates$.getGlobalValue(), Symbols.symbol_function( EQL ), UNPROVIDED ) );
    $genl_predicate_cache$ = SubLFiles.deflexical( "*GENL-PREDICATE-CACHE*", ( maybeDefault( $sym2$_GENL_PREDICATE_CACHE_, $genl_predicate_cache$, NIL ) ) );
    $genl_inverse_cache$ = SubLFiles.deflexical( "*GENL-INVERSE-CACHE*", ( maybeDefault( $sym3$_GENL_INVERSE_CACHE_, $genl_inverse_cache$, NIL ) ) );
    $all_mts_genl_predicate_cache$ = SubLFiles.deflexical( "*ALL-MTS-GENL-PREDICATE-CACHE*", ( maybeDefault( $sym4$_ALL_MTS_GENL_PREDICATE_CACHE_, $all_mts_genl_predicate_cache$, NIL ) ) );
    $all_mts_genl_inverse_cache$ = SubLFiles.deflexical( "*ALL-MTS-GENL-INVERSE-CACHE*", ( maybeDefault( $sym5$_ALL_MTS_GENL_INVERSE_CACHE_, $all_mts_genl_inverse_cache$, NIL ) ) );
    $cached_genls$ = SubLFiles.deflexical( "*CACHED-GENLS*", $list6 );
    $cached_genls_set$ = SubLFiles.deflexical( "*CACHED-GENLS-SET*", set_utilities.construct_set_from_list( $cached_genls$.getGlobalValue(), Symbols.symbol_function( EQL ), UNPROVIDED ) );
    $genls_cache$ = SubLFiles.deflexical( "*GENLS-CACHE*", ( maybeDefault( $sym7$_GENLS_CACHE_, $genls_cache$, NIL ) ) );
    $all_mts_genls_cache$ = SubLFiles.deflexical( "*ALL-MTS-GENLS-CACHE*", ( maybeDefault( $sym8$_ALL_MTS_GENLS_CACHE_, $all_mts_genls_cache$, NIL ) ) );
    $definitional_fort_typing_collections$ = SubLFiles.defconstant( "*DEFINITIONAL-FORT-TYPING-COLLECTIONS*", $list9 );
    $additional_fort_typing_collections$ = SubLFiles.deflexical( "*ADDITIONAL-FORT-TYPING-COLLECTIONS*", $list10 );
    $application_specific_fort_typing_collections$ = SubLFiles.defconstant( "*APPLICATION-SPECIFIC-FORT-TYPING-COLLECTIONS*", $list11 );
    $implicit_fort_typing_collections$ = SubLFiles.deflexical( "*IMPLICIT-FORT-TYPING-COLLECTIONS*", $list12 );
    $cached_isas$ = SubLFiles.deflexical( "*CACHED-ISAS*", ConsesLow.append( $definitional_fort_typing_collections$.getGlobalValue(), $additional_fort_typing_collections$.getGlobalValue(),
        $application_specific_fort_typing_collections$.getGlobalValue(), $implicit_fort_typing_collections$.getGlobalValue() ) );
    $cached_isas_set$ = SubLFiles.deflexical( "*CACHED-ISAS-SET*", set_utilities.construct_set_from_list( Sequences.reverse( $cached_isas$.getGlobalValue() ), Symbols.symbol_function( EQL ), UNPROVIDED ) );
    $isa_cache$ = SubLFiles.deflexical( "*ISA-CACHE*", ( maybeDefault( $sym15$_ISA_CACHE_, $isa_cache$, NIL ) ) );
    $implicit_fort_type_mapping$ = SubLFiles.deflexical( "*IMPLICIT-FORT-TYPE-MAPPING*", ( maybeDefault( $sym16$_IMPLICIT_FORT_TYPE_MAPPING_, $implicit_fort_type_mapping$, NIL ) ) );
    $all_mts_isa_cache$ = SubLFiles.deflexical( "*ALL-MTS-ISA-CACHE*", ( maybeDefault( $sym17$_ALL_MTS_ISA_CACHE_, $all_mts_isa_cache$, NIL ) ) );
    $cached_quoted_isas$ = SubLFiles.deflexical( "*CACHED-QUOTED-ISAS*", $list18 );
    $cached_quoted_isas_set$ = SubLFiles.deflexical( "*CACHED-QUOTED-ISAS-SET*", set_utilities.construct_set_from_list( Sequences.reverse( $cached_quoted_isas$.getGlobalValue() ), Symbols.symbol_function( EQL ),
        UNPROVIDED ) );
    $quoted_isa_cache$ = SubLFiles.deflexical( "*QUOTED-ISA-CACHE*", ( maybeDefault( $sym20$_QUOTED_ISA_CACHE_, $quoted_isa_cache$, NIL ) ) );
    $all_mts_quoted_isa_cache$ = SubLFiles.deflexical( "*ALL-MTS-QUOTED-ISA-CACHE*", ( maybeDefault( $sym21$_ALL_MTS_QUOTED_ISA_CACHE_, $all_mts_quoted_isa_cache$, NIL ) ) );
    $cached_preds$ = SubLFiles.deflexical( "*CACHED-PREDS*", $list22 );
    $sbhl_cache_misses_hash_table$ = SubLFiles.defvar( "*SBHL-CACHE-MISSES-HASH-TABLE*", NIL );
    return NIL;
  }
  ...
  public static SubLObject setup_sbhl_cache_file()
  {
    ...
    subl_macro_promotions.declare_defglobal( $sym2$_GENL_PREDICATE_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym3$_GENL_INVERSE_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym4$_ALL_MTS_GENL_PREDICATE_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym5$_ALL_MTS_GENL_INVERSE_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym7$_GENLS_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym8$_ALL_MTS_GENLS_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym15$_ISA_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym16$_IMPLICIT_FORT_TYPE_MAPPING_ );
    subl_macro_promotions.declare_defglobal( $sym17$_ALL_MTS_ISA_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym20$_QUOTED_ISA_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym21$_ALL_MTS_QUOTED_ISA_CACHE_ );
    return NIL;
  }
  ...
  {
    me = new sbhl_cache();
    $sbhl_caches_initializedP$ = null;
    $cached_genl_predicates$ = null;
    $cached_genl_predicates_set$ = null;
    $genl_predicate_cache$ = null;
    $genl_inverse_cache$ = null;
    $all_mts_genl_predicate_cache$ = null;
    $all_mts_genl_inverse_cache$ = null;
    $cached_genls$ = null;
    $cached_genls_set$ = null;
    $genls_cache$ = null;
    $all_mts_genls_cache$ = null;
    $definitional_fort_typing_collections$ = null;
    $additional_fort_typing_collections$ = null;
    $application_specific_fort_typing_collections$ = null;
    $implicit_fort_typing_collections$ = null;
    $cached_isas$ = null;
    $cached_isas_set$ = null;
    $isa_cache$ = null;
    $implicit_fort_type_mapping$ = null;
    $all_mts_isa_cache$ = null;
    $cached_quoted_isas$ = null;
    $cached_quoted_isas_set$ = null;
    $quoted_isa_cache$ = null;
    $all_mts_quoted_isa_cache$ = null;
    $cached_preds$ = null;
    $sbhl_cache_misses_hash_table$ = null;
    $sym0$_SBHL_CACHES_INITIALIZED__ = makeSymbol( "*SBHL-CACHES-INITIALIZED?*" );
    $list1 = ConsesLow.list( new SubLObject[] { constant_handles.reader_make_constant_shell( makeString( "different" ) ), constant_handles.reader_make_constant_shell( makeString( "conceptuallyRelated" ) ),
      constant_handles.reader_make_constant_shell( makeString( "equals" ) ), constant_handles.reader_make_constant_shell( makeString( "equalSymbols" ) ), constant_handles.reader_make_constant_shell( makeString(
          "isa" ) ), constant_handles.reader_make_constant_shell( makeString( "quotedIsa" ) ), constant_handles.reader_make_constant_shell( makeString( "sksiIsa" ) ), constant_handles.reader_make_constant_shell(
              makeString( "sksiUnknownSentence" ) ), constant_handles.reader_make_constant_shell( makeString( "subStructures" ) ), constant_handles.reader_make_constant_shell( makeString( "termPhrases" ) ),
      constant_handles.reader_make_constant_shell( makeString( "termStrings" ) ), constant_handles.reader_make_constant_shell( makeString( "objectFoundInLocation" ) ), constant_handles.reader_make_constant_shell(
          makeString( "physicalDecompositions" ) ), constant_handles.reader_make_constant_shell( makeString( "properSubEvents" ) ), constant_handles.reader_make_constant_shell( makeString( "startsAfterEndingOf" ) ),
      constant_handles.reader_make_constant_shell( makeString( "elementOf" ) ), constant_handles.reader_make_constant_shell( makeString( "unknownSentence" ) ), constant_handles.reader_make_constant_shell( makeString(
          "memberOfList" ) ), constant_handles.reader_make_constant_shell( makeString( "quantitySubsumes" ) ), constant_handles.reader_make_constant_shell( makeString( "ist" ) ), constant_handles
              .reader_make_constant_shell( makeString( "agendaHasTaskWithPriority" ) ), constant_handles.reader_make_constant_shell( makeString( "unlikelyValueRangeForGaugeInWell" ) )
    } );
    $sym2$_GENL_PREDICATE_CACHE_ = makeSymbol( "*GENL-PREDICATE-CACHE*" );
    $sym3$_GENL_INVERSE_CACHE_ = makeSymbol( "*GENL-INVERSE-CACHE*" );
    $sym4$_ALL_MTS_GENL_PREDICATE_CACHE_ = makeSymbol( "*ALL-MTS-GENL-PREDICATE-CACHE*" );
    $sym5$_ALL_MTS_GENL_INVERSE_CACHE_ = makeSymbol( "*ALL-MTS-GENL-INVERSE-CACHE*" );
    $list6 = ConsesLow.list( new SubLObject[] { constant_handles.reader_make_constant_shell( makeString( "CycLAssertion" ) ), constant_handles.reader_make_constant_shell( makeString( "CycLExpression" ) ),
      constant_handles.reader_make_constant_shell( makeString( "CycLExpression-Askable" ) ), constant_handles.reader_make_constant_shell( makeString( "CycLExpression-Assertible" ) ), constant_handles
          .reader_make_constant_shell( makeString( "CycLFormula" ) ), constant_handles.reader_make_constant_shell( makeString( "CycLIndexedTerm" ) ), constant_handles.reader_make_constant_shell( makeString(
              "CycLReifiableDenotationalTerm" ) ), constant_handles.reader_make_constant_shell( makeString( "CycLSentence" ) ), constant_handles.reader_make_constant_shell( makeString( "CycLSentence-Askable" ) ),
      constant_handles.reader_make_constant_shell( makeString( "CycLSentence-Assertible" ) ), constant_handles.reader_make_constant_shell( makeString( "Microtheory" ) ), constant_handles.reader_make_constant_shell(
          makeString( "SubLList" ) ), constant_handles.reader_make_constant_shell( makeString( "Function-Denotational" ) ), constant_handles.reader_make_constant_shell( makeString( "CalendarMonth" ) ), constant_handles
              .reader_make_constant_shell( makeString( "Time-Quantity" ) ), constant_handles.reader_make_constant_shell( makeString( "Fraction" ) )
    } );
    $sym7$_GENLS_CACHE_ = makeSymbol( "*GENLS-CACHE*" );
    $sym8$_ALL_MTS_GENLS_CACHE_ = makeSymbol( "*ALL-MTS-GENLS-CACHE*" );
    $list9 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "Collection" ) ), constant_handles.reader_make_constant_shell( makeString( "Predicate" ) ), constant_handles
    ...
    $list10 = ConsesLow.list( new SubLObject[] { constant_handles.reader_make_constant_shell( makeString( "AntiSymmetricBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString(
        "AntiTransitiveBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "ArgTypeBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "ArgTypeTernaryPredicate" ) ),
      constant_handles.reader_make_constant_shell( makeString( "AsymmetricBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "BookkeepingPredicate" ) ), constant_handles
          .reader_make_constant_shell( makeString( "BroadMicrotheory" ) ), constant_handles.reader_make_constant_shell( makeString( "CalendarCoveringType" ) ), constant_handles.reader_make_constant_shell( makeString(
              "CommutativeRelation" ) ), constant_handles.reader_make_constant_shell( makeString( "ConventionallyClassifiedTimeIntervalSeries-DisjointWithGaps" ) ), constant_handles.reader_make_constant_shell(
                  makeString( "DefaultMonotonicPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "DisjointCollectionType" ) ), constant_handles.reader_make_constant_shell( makeString(
                      "DistributingMetaKnowledgePredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "DocumentationPredicate" ) ), constant_handles.reader_make_constant_shell( makeString(
                          "ELRelation" ) ), constant_handles.reader_make_constant_shell( makeString( "EvaluatableFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "EvaluatablePredicate" ) ),
      constant_handles.reader_make_constant_shell( makeString( "EvaluatableRelation-Contextualized" ) ), constant_handles.reader_make_constant_shell( makeString( "ExistentialQuantifier" ) ), constant_handles
          .reader_make_constant_shell( makeString( "ExistentialQuantifier-Bounded" ) ), constant_handles.reader_make_constant_shell( makeString( "IndeterminateTermDenotingFunction" ) ), constant_handles
              .reader_make_constant_shell( makeString( "InferenceRelatedBookkeepingPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "IrreflexiveBinaryPredicate" ) ), constant_handles
                  .reader_make_constant_shell( makeString( "LogicalConnective" ) ), constant_handles.reader_make_constant_shell( makeString( "MicrotheoryDesignatingRelation" ) ), constant_handles
                      .reader_make_constant_shell( makeString( "PartiallyCommutativePredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "PartiallyCommutativeRelation" ) ), constant_handles
                          .reader_make_constant_shell( makeString( "ProblemSolvingCntxt" ) ), constant_handles.reader_make_constant_shell( makeString( "Quantifier" ) ), constant_handles.reader_make_constant_shell(
                              makeString( "ReflexiveBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "ReifiableFunction" ) ), constant_handles.reader_make_constant_shell( makeString(
                                  "RuleMacroPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "ScopingRelation" ) ), constant_handles.reader_make_constant_shell( makeString(
                                      "SiblingDisjointCollectionType" ) ), constant_handles.reader_make_constant_shell( makeString( "SkolemFunction" ) ), constant_handles.reader_make_constant_shell( makeString(
                                          "SymmetricBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "TransitiveBinaryPredicate" ) ), constant_handles.reader_make_constant_shell( makeString(
                                              "TruthFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "UnitOfMeasure" ) ), constant_handles.reader_make_constant_shell( makeString(
                                                  "UnknownScalarInterval" ) ), constant_handles.reader_make_constant_shell( makeString( "VariableArityRelation" ) ), constant_handles.reader_make_constant_shell(
                                                      makeString( "FirstOrderCollection" ) ), constant_handles.reader_make_constant_shell( makeString( "SecondOrderCollection" ) ), constant_handles
                                                          .reader_make_constant_shell( makeString( "ThirdOrderCollection" ) ), constant_handles.reader_make_constant_shell( makeString( "FourthOrderCollection" ) ),
      constant_handles.reader_make_constant_shell( makeString( "CollectionDenotingFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "WFFConstraintPredicate" ) ), constant_handles
          .reader_make_constant_shell( makeString( "MonthOfYearType" ) )
    } );
    $list11 = ConsesLow.list( new SubLObject[] { constant_handles.reader_make_constant_shell( makeString( "NLDefinitenessAttribute" ) ), constant_handles.reader_make_constant_shell( makeString( "NLQuantAttribute" ) ),
      constant_handles.reader_make_constant_shell( makeString( "NLSemanticPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "ProperNamePredicate-Strict" ) ), constant_handles
          .reader_make_constant_shell( makeString( "SpeechPart" ) ), constant_handles.reader_make_constant_shell( makeString( "CSQLComparisonPredicate" ) ), constant_handles.reader_make_constant_shell( makeString(
              "CSQLConstantFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "CSQLFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "CSQLLogicalConnective" ) ),
      constant_handles.reader_make_constant_shell( makeString( "CSQLQuantifier" ) ), constant_handles.reader_make_constant_shell( makeString( "LogicalSchema" ) ), constant_handles.reader_make_constant_shell( makeString(
          "SKSIContentMicrotheory" ) ), constant_handles.reader_make_constant_shell( makeString( "SKSIExternalTermDenotingFunction" ) ), constant_handles.reader_make_constant_shell( makeString(
              "SKSISupportedComparisonPredicate" ) ), constant_handles.reader_make_constant_shell( makeString( "SKSISupportedConstant" ) ), constant_handles.reader_make_constant_shell( makeString(
                  "SKSISupportedFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "StructuredKnowledgeSource" ) ), constant_handles.reader_make_constant_shell( makeString(
                      "CytologicalProcessProjectKBContentTestQuery" ) ), constant_handles.reader_make_constant_shell( makeString( "SingleInteractionContextEventType" ) ), constant_handles.reader_make_constant_shell(
                          makeString( "PlannedEvent" ) ), constant_handles.reader_make_constant_shell( makeString( "InteractionHistoryMicrotheory" ) ), constant_handles.reader_make_constant_shell( makeString(
                              "BELLAInteractionHistoryMicrotheory" ) ), constant_handles.reader_make_constant_shell( makeString( "InstantaneousSimulatedActionType" ) ), constant_handles.reader_make_constant_shell(
                                  makeString( "MathematicalExpressionDenotingFunction" ) ), constant_handles.reader_make_constant_shell( makeString( "SpreadsheetSKS" ) )
    } );
    $list12 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "Relation" ) ) );
    $sym13$FORT_P = makeSymbol( "FORT-P" );
    $const14$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $sym15$_ISA_CACHE_ = makeSymbol( "*ISA-CACHE*" );
    $sym16$_IMPLICIT_FORT_TYPE_MAPPING_ = makeSymbol( "*IMPLICIT-FORT-TYPE-MAPPING*" );
    $sym17$_ALL_MTS_ISA_CACHE_ = makeSymbol( "*ALL-MTS-ISA-CACHE*" );
    $list18 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "CoreUnionConstant" ) ), constant_handles.reader_make_constant_shell( makeString( "IndeterminateTerm" ) ), constant_handles
        .reader_make_constant_shell( makeString( "LogicalFieldIndexical" ) ), constant_handles.reader_make_constant_shell( makeString( "NonSkolemIndeterminateTerm" ) ), constant_handles.reader_make_constant_shell(
            makeString( "PhysicalFieldIndexical" ) ), constant_handles.reader_make_constant_shell( makeString( "TermAcceptableForQuerySearch-TriggerOnly" ) ), constant_handles.reader_make_constant_shell( makeString(
                "TermExemptFromUniqueNamesAssumption" ) ), constant_handles.reader_make_constant_shell( makeString( "ObsoleteConstant" ) ) );
    $const19$quotedIsa = constant_handles.reader_make_constant_shell( makeString( "quotedIsa" ) );
    $sym20$_QUOTED_ISA_CACHE_ = makeSymbol( "*QUOTED-ISA-CACHE*" );
    $sym21$_ALL_MTS_QUOTED_ISA_CACHE_ = makeSymbol( "*ALL-MTS-QUOTED-ISA-CACHE*" );
    $list22 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "isa" ) ), constant_handles.reader_make_constant_shell( makeString( "quotedIsa" ) ), constant_handles.reader_make_constant_shell(
        makeString( "genls" ) ), constant_handles.reader_make_constant_shell( makeString( "genlPreds" ) ), constant_handles.reader_make_constant_shell( makeString( "genlInverse" ) ) );
    $list23 = ConsesLow.list( ConsesLow.list( makeSymbol( "NODE-VAR" ), makeSymbol( "PRED" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym24$DO_SET = makeSymbol( "DO-SET" );
    $sym25$GET_CACHED_NODES_SET_FOR_PRED = makeSymbol( "GET-CACHED-NODES-SET-FOR-PRED" );
    ...
    $str29$_sbhl_cache_misses_hash_table__is = makeString( "*sbhl-cache-misses-hash-table* is not a hash table" );
    $str30$_A___S__ = makeString( "~A: ~S~%" );
    $sym31$_ = makeSymbol( ">" );
    $sym32$SBHL_PREDICATE_P = makeSymbol( "SBHL-PREDICATE-P" );
    $list33 = ConsesLow.list( ConsesLow.list( makeSymbol( "CACHED-NODE-VAR" ), makeSymbol( "MTS-VAR" ), makeSymbol( "SUBNODE" ), makeSymbol( "PRED" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym34$SUBNODES_VAR = makeUninternedSymbol( "SUBNODES-VAR" );
    $sym35$DO_DICTIONARY = makeSymbol( "DO-DICTIONARY" );
    $sym36$GET_SBHL_CACHE_FOR_PRED = makeSymbol( "GET-SBHL-CACHE-FOR-PRED" );
    $sym37$PWHEN = makeSymbol( "PWHEN" );
    $sym38$DICTIONARY_P = makeSymbol( "DICTIONARY-P" );
    $sym39$CLET = makeSymbol( "CLET" );
    $sym40$DICTIONARY_LOOKUP = makeSymbol( "DICTIONARY-LOOKUP" );
    $sym41$HLMT_EQUAL = makeSymbol( "HLMT-EQUAL" );
    $str42$Initializing_SBHL_cache_for_ = makeString( "Initializing SBHL cache for " );
    $str43$_ = makeString( "." );
    $str44$Adding_ = makeString( "Adding " );
    $str45$_to_SBHL_cache_for_ = makeString( " to SBHL cache for " );
    $list46 = ConsesLow.list( ConsesLow.list( makeSymbol( "LINK-NODE-VAR" ), makeSymbol( "SUBNODE-VAR" ), makeSymbol( "PRED" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym47$SUBNODES = makeUninternedSymbol( "SUBNODES" );
    $sym48$NODES = makeUninternedSymbol( "NODES" );
    $sym49$CACHE = makeUninternedSymbol( "CACHE" );
    $sym50$GET_SBHL_ALL_MTS_CACHE_FOR_PRED = makeSymbol( "GET-SBHL-ALL-MTS-CACHE-FOR-PRED" );
    $sym51$PIF = makeSymbol( "PIF" );
    $sym52$FORT_ID_INDEX_P = makeSymbol( "FORT-ID-INDEX-P" );
    $sym53$DO_FORT_ID_INDEX = makeSymbol( "DO-FORT-ID-INDEX" );
    $list54 = ConsesLow.list( ConsesLow.list( makeSymbol( "CACHED-LINK-NODE" ), makeSymbol( "PRED" ), makeSymbol( "SUBNODE" ), makeSymbol( "MT" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym55$CACHE = makeUninternedSymbol( "CACHE" );
    $sym56$CACHED_NODES_STORE = makeUninternedSymbol( "CACHED-NODES-STORE" );
    $sym57$MEMBER_ = makeSymbol( "MEMBER?" );
    $list58 = ConsesLow.list( ConsesLow.list( makeSymbol( "CACHED-LINK-NODE" ), makeSymbol( "PRED" ), makeSymbol( "SUBNODE" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym59$CACHE = makeUninternedSymbol( "CACHE" );
    $sym60$SUBNODES = makeUninternedSymbol( "SUBNODES" );
    $sym61$SBHL_PRED_ALL_MTS_CACHE_USES_ID_INDEX_P = makeSymbol( "SBHL-PRED-ALL-MTS-CACHE-USES-ID-INDEX-P" );
    $sym62$SBHL_ID_INDEX_LOOKUP = makeSymbol( "SBHL-ID-INDEX-LOOKUP" );
    $sym63$SET_MEMBER_ = makeSymbol( "SET-MEMBER?" );
    $list64 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "genlPreds" ) ), constant_handles.reader_make_constant_shell( makeString( "genlInverse" ) ), constant_handles
        .reader_make_constant_shell( makeString( "genls" ) ), constant_handles.reader_make_constant_shell( makeString( "isa" ) ), constant_handles.reader_make_constant_shell( makeString( "quotedIsa" ) ) );
    $const65$Relation = constant_handles.reader_make_constant_shell( makeString( "Relation" ) );
    $const66$Predicate = constant_handles.reader_make_constant_shell( makeString( "Predicate" ) );
    $const67$Function_Denotational = constant_handles.reader_make_constant_shell( makeString( "Function-Denotational" ) );
    $const68$LogicalConnective = constant_handles.reader_make_constant_shell( makeString( "LogicalConnective" ) );
    $const69$Quantifier = constant_handles.reader_make_constant_shell( makeString( "Quantifier" ) );
    $str70$Computing_all_mts_SBHL_caches_ = makeString( "Computing all-mts SBHL caches." );
    $str71$cdolist = makeString( "cdolist" );
    $str72$Computing_SBHL_caches_from_all_mt = makeString( "Computing SBHL caches from all-mts caches." );
    $sym73$RELEVANT_MT_IS_EVERYTHING = makeSymbol( "RELEVANT-MT-IS-EVERYTHING" );
    $const74$EverythingPSC = constant_handles.reader_make_constant_shell( makeString( "EverythingPSC" ) );
    $kw75$TRUE = makeKeyword( "TRUE" );
    $const76$UniversalVocabularyMt = constant_handles.reader_make_constant_shell( makeString( "UniversalVocabularyMt" ) );
    $kw77$DEPTH = makeKeyword( "DEPTH" );
    $kw78$STACK = makeKeyword( "STACK" );
    $kw79$QUEUE = makeKeyword( "QUEUE" );
    $sym80$RELEVANT_SBHL_TV_IS_GENERAL_TV = makeSymbol( "RELEVANT-SBHL-TV-IS-GENERAL-TV" );
    $kw81$ERROR = makeKeyword( "ERROR" );
    $str82$_A_is_not_a__A = makeString( "~A is not a ~A" );
    $sym83$SBHL_TRUE_TV_P = makeSymbol( "SBHL-TRUE-TV-P" );
    $kw84$CERROR = makeKeyword( "CERROR" );
    $str85$continue_anyway = makeString( "continue anyway" );
    $kw86$WARN = makeKeyword( "WARN" );
    $str87$_A_is_not_a_valid__sbhl_type_erro = makeString( "~A is not a valid *sbhl-type-error-action* value" );
    $str88$_A_is_neither_SET_P_nor_LISTP_ = makeString( "~A is neither SET-P nor LISTP." );
    $str89$attempting_to_bind_direction_link = makeString( "attempting to bind direction link variable, to NIL. macro body not executed." );
    $str90$Node__a_does_not_pass_sbhl_type_t = makeString( "Node ~a does not pass sbhl-type-test ~a~%" );
    $kw91$SKIP = makeKeyword( "SKIP" );
    $str92$genlPreds___genlInverse_after_add = makeString( "genlPreds / genlInverse after adding used for assertion with predicate ~a." );
    $sym93$RESET_CACHED_GENL_PRED_AND_INVERSE_RELATIONS = makeSymbol( "RESET-CACHED-GENL-PRED-AND-INVERSE-RELATIONS" );
    $kw94$BREADTH = makeKeyword( "BREADTH" );
    $sym95$RESET_CACHED_ISA_RELATIONS = makeSymbol( "RESET-CACHED-ISA-RELATIONS" );
    $sym96$RESET_CACHED_QUOTED_ISA_RELATIONS = makeSymbol( "RESET-CACHED-QUOTED-ISA-RELATIONS" );
    $sym97$RESET_CACHED_GENLS_RELATIONS = makeSymbol( "RESET-CACHED-GENLS-RELATIONS" );
    $list98 = ConsesLow.list( makeUninternedSymbol( "LINK-NODE" ), makeUninternedSymbol( "MT" ), makeUninternedSymbol( "TV" ) );
  }

  public static final class $valid_fort_typeP$UnaryFunction
      extends
        UnaryFunction
  {
