      ...
      SubLTranslatedFile
{
  ...
  public static SubLObject psp_chart_for_string(final SubLObject input_string, SubLObject lexicon, SubLObject gap_type, SubLObject do_syntactic_parsingP)
  {
    ...
    assert NIL != psp_chart.psp_gap_constraint_p( gap_type ) : gap_type;
        ...
        try
        {
          if( NIL == stop_working_on_chartP )
          {
            ...
            while ( NIL == stop_working_on_chartP && NIL != csome_list_var)
            {
              ...
              if( NIL != report_spanP )
              {
                PrintLow.format( T, $str5$__Doing_lexical_span__S___, span );
              }
              ...
              if( span == $kw7$ANY )
              {
                final SubLObject pcase_var = $kw8$LEXICAL;
                if( pcase_var.eql( $kw7$ANY ) )
                {
                  final SubLObject chart_to_use_$5 = ( NIL != chart_to_use ) ? chart_to_use : parsing_vars.$psp_chart$.getDynamicValue( thread );
                  assert NIL != psp_chart.phrase_structure_parser_chart_p( chart_to_use_$5 ) : chart_to_use_$5;
                  SubLObject iteration_state;
                  for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( psp_chart.psp_chart_spans_to_edges( chart_to_use_$5 ) ) ); NIL == stop_working_on_spanP
                      && NIL == dictionary_contents.do_dictionary_contents_doneP( iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
                  {
                    ...
                    if( NIL == stop_working_on_spanP )
                    {
                      ...
                      {
                        if( NIL != psp_chart.psp_do_edge_okP( lexical_edge, $kw7$ANY, T ) )
                        {
                          ...
                          else if( NIL != psp_chart.psp_lexical_span_parsedP( span, UNPROVIDED ) )
                          {
                            stop_working_on_spanP = T;
                          }
                          else if( NIL != psp_chart.psp_span_too_many_edgesP( span, UNPROVIDED ) )
                          {
                            stop_working_on_spanP = T;
                          }
                          else if( NIL != psp_chart.psp_chart_too_many_edgesP( UNPROVIDED ) )
                          {
                            stop_working_on_spanP = T;
                            stop_working_on_chartP = T;
                          }
                ...
                else if( pcase_var.eql( $kw8$LEXICAL ) )
                {
                  final SubLObject chart_to_use_$6 = ( NIL != chart_to_use ) ? chart_to_use : parsing_vars.$psp_chart$.getDynamicValue( thread );
                  assert NIL != psp_chart.phrase_structure_parser_chart_p( chart_to_use_$6 ) : chart_to_use_$6;
                  SubLObject iteration_state;
                  for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( psp_chart.psp_chart_spans_to_edges( chart_to_use_$6 ) ) ); NIL == stop_working_on_spanP
                      && NIL == dictionary_contents.do_dictionary_contents_doneP( iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
                  {
                    thread.resetMultipleValues();
                    final SubLObject span_to_do = dictionary_contents.do_dictionary_contents_key_value( iteration_state );
              ...
              else if( NIL != interval_span.interval_span_p( span ) )
              {
                final SubLObject pcase_var = $kw8$LEXICAL;
                if( pcase_var.eql( $kw7$ANY ) )
                {
                  if( NIL == stop_working_on_spanP )
                  {
                    ...
                    {
                      if( NIL != psp_chart.psp_do_edge_okP( lexical_edge2, $kw7$ANY, T ) )
                      {
                        if( NIL != parsing_macros.parsing_timeout_time_reachedP() )
                        {
                          stop_working_on_spanP = T;
                ...
                else if( pcase_var.eql( $kw8$LEXICAL ) && NIL == stop_working_on_spanP )
                {
              ...
              if( NIL != report_spanP )
              {
                PrintLow.format( T, $str9$___Edges_so_far___S__Completed___, new SubLObject[] { psp_chart.psp_edge_count( UNPROVIDED ), Sequences.length( psp_chart.psp_all_complete_edges( NIL ) ), Sequences.length(
                    psp_chart.psp_all_complete_edges( T ) )
                } );
              }
  ...
  public static SubLObject psp_process_edges(final SubLObject edges)
  {
    ...
    if( NIL == stopP )
    {
      ...
      SubLObject edge;
      for( csome_list_var = edges, edge = NIL, edge = csome_list_var.first(); NIL == stopP && NIL != csome_list_var; stopP = makeBoolean( $kw10$ABORTED == psp_process_new_edges_from_edge( edge ) || NIL != parsing_macros
          .parsing_timeout_time_reachedP() || NIL != psp_chart.psp_chart_too_many_edgesP( UNPROVIDED ) || NIL != psp_chart.psp_span_too_many_edgesP( psp_chart.get_psp_edge_span( edge ),
              UNPROVIDED ) ), csome_list_var = csome_list_var.rest(), edge = csome_list_var.first() )
      {
        psp_chart.psp_chart_add_edge( edge, UNPROVIDED );
      }
    }
    return ( NIL != stopP ) ? $kw10$ABORTED : $kw11$PROCESSED;
  }
  ...
  public static SubLObject psp_new_edges_from_gaps(final SubLObject edge)
  {
    ...
    return ( NIL != new_edge ) ? ConsesLow.list( new_edge ) : NIL;
  }
  ...
  public static SubLObject psp_new_edges_from_rules(final SubLObject edge)
  {
    ...
    if( NIL == psp_chart.psp_span_too_many_edgesP( span, UNPROVIDED ) )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        final SubLObject new_dtr_is_headP = Equality.eql( psp_rules.psp_rule_head( rule ), ONE_INTEGER );
        final SubLObject pos_pred = ( NIL != new_dtr_is_headP ) ? psp_chart.get_psp_edge_pos_pred( edge ) : NIL;
        final SubLObject cycls = $kw12$NEW;
        ...
        final SubLObject non_head_dtrs = ( NIL != new_dtr_is_headP ) ? NIL : ConsesLow.list( edge );
        final SubLObject weight = NIL;
        final SubLObject new_edge = psp_chart.new_psp_phrasal_edge( parsing_vars.$psp_chart$.getDynamicValue( thread ), span, string, category, pos_pred, cycls, weight, rule, new_dtr_constraints, head_dtr,
            non_head_dtrs );
        new_edges = ConsesLow.cons( new_edge, new_edges );
    ...
    {
      PrintLow.format( T, $str13$__Found_new_edges_starting_with__, edge, new_edges );
    }
    return new_edges;
  }
  ...
  public static SubLObject psp_new_edges_from_edges(final SubLObject edge)
  {
    ...
    assert NIL != psp_chart.phrase_structure_parser_chart_p( chart_to_use ) : chart_to_use;
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( psp_chart.psp_chart_spans_to_edges( chart_to_use ) ) ); NIL == stop_working_on_edgeP
        && NIL == dictionary_contents.do_dictionary_contents_doneP( iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
      ...
      if( NIL != psp_adjacent_spansP( span, psp_chart.get_psp_edge_span( edge ) ) )
      {
        ...
        assert NIL != psp_chart.phrase_structure_parser_chart_p( chart_to_use_$12 ) : chart_to_use_$12;
        if( span == $kw7$ANY )
        {
          final SubLObject pcase_var = $kw7$ANY;
          if( pcase_var.eql( $kw7$ANY ) )
          {
            final SubLObject chart_to_use_$13 = ( NIL != chart_to_use_$12 ) ? chart_to_use_$12 : parsing_vars.$psp_chart$.getDynamicValue( thread );
            assert NIL != psp_chart.phrase_structure_parser_chart_p( chart_to_use_$13 ) : chart_to_use_$13;
            SubLObject iteration_state_$14;
            for( iteration_state_$14 = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( psp_chart.psp_chart_spans_to_edges( chart_to_use_$13 ) ) ); NIL == stop_working_on_spanP
                && NIL == dictionary_contents.do_dictionary_contents_doneP( iteration_state_$14 ); iteration_state_$14 = dictionary_contents.do_dictionary_contents_next( iteration_state_$14 ) )
            {
              thread.resetMultipleValues();
              ...
              if( NIL == stop_working_on_spanP )
              {
                ...
                {
                  if( NIL != psp_chart.psp_do_edge_okP( old_edge, $kw7$ANY, NIL ) )
                  {
                    final SubLObject var;
                    final SubLObject new_edge = var = psp_extend_old_edge( old_edge, edge );
                    if( NIL != var )
                    {
                      new_edges = ConsesLow.cons( var, new_edges );
                    }
                  }
                  csome_list_var = csome_list_var.rest();
                  old_edge = csome_list_var.first();
          ...
          else if( pcase_var.eql( $kw8$LEXICAL ) )
          {
            ...
            assert NIL != psp_chart.phrase_structure_parser_chart_p( chart_to_use_$14 ) : chart_to_use_$14;
            SubLObject iteration_state_$15;
            for( iteration_state_$15 = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( psp_chart.psp_chart_spans_to_edges( chart_to_use_$14 ) ) ); NIL == stop_working_on_spanP
        ...
        else if( NIL != interval_span.interval_span_p( span ) )
        {
          ...
          if( pcase_var.eql( $kw7$ANY ) )
          {
            if( NIL == stop_working_on_spanP )
            {
              ...
              {
                if( NIL != psp_chart.psp_do_edge_okP( old_edge2, $kw7$ANY, NIL ) )
                {
                  final SubLObject var2;
                  final SubLObject new_edge2 = var2 = psp_extend_old_edge( old_edge2, edge );
                  if( NIL != var2 )
                  {
                    new_edges = ConsesLow.cons( var2, new_edges );
                  }
                }
    ...
    if( NIL != parsing_vars.$psp_verboseP$.getDynamicValue( thread ) && NIL != new_edges )
    {
      PrintLow.format( T, $str14$__Found_new_edges_continued_by__S, edge, new_edges );
    }
    return new_edges;
  }
  ...
  public static SubLObject psp_extend_old_edge(final SubLObject old_edge, SubLObject edge)
  {
    ...
    if( NIL == psp_chart.psp_completed_edgeP( old_edge ) )
    {
      ...
      if( NIL != psp_rules.psp_edge_meets_syntax_constraintsP( edge, constraints, old_edge, traced_rule_fort ) )
      {
        ...
        {
          PrintLow.format( T, $str16$___S___meets_next_dtr_constraints, edge );
        }
        if( edge == $kw15$GAP )
        {
        ...
        if( NIL != psp_chart.psp_preposition_gap_edge_p( edge ) )
        {
          ...
          {
            final SubLObject pp_gap_edge = psp_chart.new_psp_gap_edge( psp_chart.psp_edge_end_index( old_edge ), $const17$PrepositionalPhrase );
            final SubLObject np_gap_edge = psp_chart.new_psp_gap_edge( psp_chart.psp_edge_end_index( old_edge ), $const18$NounPhrase );
            psp_chart.set_psp_edge_rule( pp_gap_edge, psp_rules.psp_rule_struct_for_rule_fort( $const19$PSRule_PP ) );
            psp_chart.set_psp_edge_head_dtr( pp_gap_edge, edge );
            psp_chart.set_psp_edge_non_head_dtrs( pp_gap_edge, ConsesLow.list( np_gap_edge ) );
            new_edge = do_psp_extend_old_edge( old_edge, pp_gap_edge );
          }
          else
          {
            final SubLObject np_gap_edge2 = psp_chart.new_psp_gap_edge( psp_chart.psp_edge_end_index( old_edge ), $const18$NounPhrase );
            new_edge = do_psp_extend_old_edge( new_edge, np_gap_edge2 );
          }
        }
        if( NIL != traced_rule_fort )
        {
          PrintLow.format( T, $str20$__Continued__S_with____S_to_form_, new SubLObject[] { old_edge, edge, new_edge, psp_chart.get_psp_edge_head_dtr( new_edge )
          } );
        }
      }
    }
  ...
  public static SubLObject do_psp_extend_old_edge(final SubLObject old_edge, final SubLObject edge)
  {
    ...
    final SubLObject weight = NIL;
    final SubLObject new_edge = psp_chart.new_psp_phrasal_edge( parsing_vars.$psp_chart$.getDynamicValue( thread ), new_span, new_string, category, pos_pred, cycls, weight, rule, new_dtr_constraints, head_dtr,
        non_head_dtrs );
    return new_edge;
  }
  ...
  {
    SubLFiles.declareFunction( me, "psp_chart_for_string", "PSP-CHART-FOR-STRING", 1, 3, false );
    SubLFiles.declareFunction( me, "psp_chart_do_syntactic_parsing", "PSP-CHART-DO-SYNTACTIC-PARSING", 0, 3, false );
    SubLFiles.declareFunction( me, "psp_process_edges", "PSP-PROCESS-EDGES", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_process_new_edges_from_edge", "PSP-PROCESS-NEW-EDGES-FROM-EDGE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_new_edges_from_edge", "PSP-NEW-EDGES-FROM-EDGE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_new_edges_from_gaps", "PSP-NEW-EDGES-FROM-GAPS", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_new_edges_from_rules", "PSP-NEW-EDGES-FROM-RULES", 1, 0, false );
    SubLFiles.declareFunction( me, "find_psp_edge_with_properties", "FIND-PSP-EDGE-WITH-PROPERTIES", 3, 1, false );
    SubLFiles.declareFunction( me, "psp_adjacent_spansP", "PSP-ADJACENT-SPANS?", 2, 0, false );
    SubLFiles.declareFunction( me, "psp_new_edges_from_edges", "PSP-NEW-EDGES-FROM-EDGES", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_extend_old_edge", "PSP-EXTEND-OLD-EDGE", 1, 1, false );
    SubLFiles.declareFunction( me, "do_psp_extend_old_edge", "DO-PSP-EXTEND-OLD-EDGE", 2, 0, false );
    return NIL;
  }
  ...
  static
  {
    ...
    $sym2$PSP_LEXICON_P = makeSymbol( "PSP-LEXICON-P" );
    $sym3$PSP_GAP_CONSTRAINT_P = makeSymbol( "PSP-GAP-CONSTRAINT-P" );
    $sym4$INTERVAL_SPAN_PRECEDES_ = makeSymbol( "INTERVAL-SPAN-PRECEDES?" );
    $str5$__Doing_lexical_span__S___ = makeString( "~&Doing lexical span ~S.~%" );
    $sym6$PHRASE_STRUCTURE_PARSER_CHART_P = makeSymbol( "PHRASE-STRUCTURE-PARSER-CHART-P" );
    $kw7$ANY = makeKeyword( "ANY" );
    $kw8$LEXICAL = makeKeyword( "LEXICAL" );
    $str9$___Edges_so_far___S__Completed___ = makeString( "~& Edges so far: ~S. Completed: ~S Non-trivial: ~S~%" );
    $kw10$ABORTED = makeKeyword( "ABORTED" );
    $kw11$PROCESSED = makeKeyword( "PROCESSED" );
    $kw12$NEW = makeKeyword( "NEW" );
    $str13$__Found_new_edges_starting_with__ = makeString( "~&Found new edges starting with ~S:~% ~S.~%" );
    $str14$__Found_new_edges_continued_by__S = makeString( "~&Found new edges continued by ~S:~% ~S.~%" );
    $kw15$GAP = makeKeyword( "GAP" );
    $str16$___S___meets_next_dtr_constraints = makeString( "~&~S~% meets next-dtr constraints.~%" );
    $const17$PrepositionalPhrase = constant_handles.reader_make_constant_shell( makeString( "PrepositionalPhrase" ) );
    $const18$NounPhrase = constant_handles.reader_make_constant_shell( makeString( "NounPhrase" ) );
    $const19$PSRule_PP = constant_handles.reader_make_constant_shell( makeString( "PSRule-PP" ) );
    $str20$__Continued__S_with____S_to_form_ = makeString( "~&Continued ~S with~% ~S to form~% ~S, whose head is~% ~S.~%" );
  }
}
/*
 * 
