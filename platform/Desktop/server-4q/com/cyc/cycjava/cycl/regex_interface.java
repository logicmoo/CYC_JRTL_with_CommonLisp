package com.cyc.cycjava.cycl;

import com.cyc.tool.subl.jrtl.nativeCode.type.core.SubLObject;
import com.cyc.tool.subl.util.SubLFile;
import com.cyc.tool.subl.util.SubLTranslatedFile;

public final class regex_interface extends SubLTranslatedFile
{
    public static final SubLFile me;
    public static final String myName = "com.cyc.cycjava.cycl.regex_interface";
    public static final String myFingerPrint = "c19299fb9e18c9dff77296b35b8b153a4629ccca5e772b3b409df956a6d0acb2";
    
    public static SubLObject declare_regex_interface_file() {
        return (SubLObject)regex_interface.NIL;
    }
    
    public static SubLObject init_regex_interface_file() {
        return (SubLObject)regex_interface.NIL;
    }
    
    public static SubLObject setup_regex_interface_file() {
        return (SubLObject)regex_interface.NIL;
    }
    
    @Override
	public void declareFunctions() {
        declare_regex_interface_file();
    }
    
    @Override
	public void initializeVariables() {
        init_regex_interface_file();
    }
    
    @Override
	public void runTopLevelForms() {
        setup_regex_interface_file();
    }
    
    static {
        me = (SubLFile)new regex_interface();
    }
}

/*

	Total time: 60 ms
	
*/