...
public final class cb_refresher_applet
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject cb_refresher_applet(final SubLObject url, final SubLObject update_func, final SubLObject refresh_msecs, SubLObject extra_params, SubLObject target_name, SubLObject host_name,
      SubLObject port_num)
  {
    ...
    PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $cb_refresher_javascript_template$.getGlobalValue(), new SubLObject[] { $cb_refresher_class_file$.getGlobalValue(), $cb_refresher_codebase$
        .getGlobalValue(), $cb_refresher_archive$.getGlobalValue(), PrintLow.write_to_string( update_func, EMPTY_SUBL_OBJECT_ARRAY ), refresh_msecs, extra_params, host_name, port_num, target_name
    } );
    html_utilities.html_markup( $cb_refresher_caller$.getGlobalValue() );
    return NIL;
  }

  public static SubLObject declare_cb_refresher_applet_file()
  {
    SubLFiles.declareFunction( me, "cb_refresher_applet", "CB-REFRESHER-APPLET", 3, 4, false );
    return NIL;
  }

  public static SubLObject init_cb_refresher_applet_file()
  {
    $cb_refresher_archive$ = SubLFiles.deflexical( "*CB-REFRESHER-ARCHIVE*", $str0$Refresher_jar___lib_OpenCyc_jar__ );
    $cb_refresher_codebase$ = SubLFiles.deflexical( "*CB-REFRESHER-CODEBASE*", $str1$_java_refresher );
    $cb_refresher_class_file$ = SubLFiles.deflexical( "*CB-REFRESHER-CLASS-FILE*", $str2$com_cyc_common_applet_refresher_R );
    $cb_refresher_javascript_template$ = SubLFiles.deflexical( "*CB-REFRESHER-JAVASCRIPT-TEMPLATE*", $str3$_script_LANGUAGE__JavaScript_____ );
    $cb_refresher_caller$ = SubLFiles.deflexical( "*CB-REFRESHER-CALLER*", $str4$_script_LANGUAGE__JavaScript_____ );
    return NIL;
  }
  ...
  static
  {
    me = new cb_refresher_applet();
    $cb_refresher_archive$ = null;
    $cb_refresher_codebase$ = null;
    $cb_refresher_class_file$ = null;
    $cb_refresher_javascript_template$ = null;
    $cb_refresher_caller$ = null;
    $str0$Refresher_jar___lib_OpenCyc_jar__ = makeString( "Refresher.jar , lib/OpenCyc.jar , lib/jakarta-oro-2.0.3.jar" );
    $str1$_java_refresher = makeString( "/java/refresher" );
    $str2$com_cyc_common_applet_refresher_R = makeString( "com.cyc.common.applet.refresher.RefresherApplet" );
    $str3$_script_LANGUAGE__JavaScript_____ = makeString(
    ...
    $str4$_script_LANGUAGE__JavaScript_____ = makeString( "<script LANGUAGE=\"JavaScript\"> \n<!-- \ncallRefresherApplet() \n// --> \n</script>" );
    $str5$_self = makeString( "_self" );
  }
}
/*
 * 
