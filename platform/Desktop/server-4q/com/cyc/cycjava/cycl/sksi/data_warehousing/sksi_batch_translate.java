...
public final class sksi_batch_translate
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject with_sksi_batch_sql_select_statement(final SubLObject macroform, final SubLObject environment)
  {
    ...
    return ConsesLow.listS( $sym2$CLET, ConsesLow.list( ConsesLow.list( $sym8$_SKSI_BATCH_SQL_SELECT_STATEMENT_, string ) ), ConsesLow.append( body, NIL ) );
  }
  ...
  public static SubLObject sksi_batch_assert_translations_of_table(final SubLObject table, SubLObject check_wffnessP, SubLObject start_row, SubLObject end_row)
  {
    ...
    try
    {
      $sksi_batch_assert_justification_table$.bind( Hashtables.make_hash_table( $int9$1000, Symbols.symbol_function( EQUAL ), UNPROVIDED ), thread );
      ...
      count = Sequences.length( sksi_translate_all_sk_sources_in_mt( content_mt, meta_mt, check_wffnessP, start_row, end_row, T ) );
    }
    finally
    {
  ...
  public static SubLObject sksi_batch_assert_all_sk_sources_in_mt_to_ke_stream(final SubLObject content_mt, final SubLObject meta_mt, final SubLObject target_mt, SubLObject stream, SubLObject check_wffnessP,
      SubLObject start_row, SubLObject end_row)
  {
    ...
    {
      final SubLObject sentence_to_be_asserted = cycl_utilities.sentence_arg2( current_sentence, UNPROVIDED );
      PrintLow.format( stream, $str10$in_mt___S___, cycl_utilities.hl_to_el( target_mt ) );
      PrintLow.format( stream, $str11$f___S___, sentence_to_be_asserted );
      cdolist_list_var = cdolist_list_var.rest();
      current_sentence = cdolist_list_var.first();
    }
    return NIL;
  ...
  public static SubLObject sksi_batch_assert_all_sk_sources_in_mt_to_ke_string(final SubLObject content_mt, final SubLObject meta_mt, final SubLObject target_mt, SubLObject check_wffnessP, SubLObject start_row,
      SubLObject end_row)
  {
    if( check_wffnessP == UNPROVIDED )
    {
  ...
  public static SubLObject sksi_batch_assert_all_sk_sources_in_mt_to_ke_file(final SubLObject filename, final SubLObject target_mt, final SubLObject content_mt, final SubLObject meta_mt, SubLObject check_wff_nessP,
      SubLObject start_row, SubLObject end_row)
  {
    ...
    try
    {
      stream = compatibility.open_text( filename, $kw12$OUTPUT );
      if( !stream.isStream() )
      {
  ...
  public static SubLObject sksi_translate_all_sk_sources_in_mt(final SubLObject content_mt, final SubLObject meta_mt, SubLObject check_wff_nessP, SubLObject start_row, SubLObject end_row, SubLObject assert_as_you_goP)
  {
    ...
    final SubLObject deck_type = $kw16$STACK;
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          sbhl_search_vars.$relevant_sbhl_tv_function$.bind( ( NIL != tv_var ) ? $sym17$RELEVANT_SBHL_TV_IS_GENERAL_TV : sbhl_search_vars.$relevant_sbhl_tv_function$.getDynamicValue( thread ), thread );
          if( NIL != tv_var && NIL != sbhl_paranoia.sbhl_object_type_checking_p() && NIL == sbhl_search_vars.sbhl_true_tv_p( tv_var ) )
          {
            final SubLObject pcase_var = sbhl_paranoia.$sbhl_type_error_action$.getDynamicValue( thread );
            if( pcase_var.eql( $kw18$ERROR ) )
            {
              sbhl_paranoia.sbhl_error( ONE_INTEGER, $str19$_A_is_not_a__A, tv_var, $sym20$SBHL_TRUE_TV_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
            }
            else if( pcase_var.eql( $kw21$CERROR ) )
            {
              sbhl_paranoia.sbhl_cerror( ONE_INTEGER, $str22$continue_anyway, $str19$_A_is_not_a__A, tv_var, $sym20$SBHL_TRUE_TV_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
            }
            else if( pcase_var.eql( $kw23$WARN ) )
            {
              Errors.warn( $str19$_A_is_not_a__A, tv_var, $sym20$SBHL_TRUE_TV_P );
            }
            else
            {
              Errors.warn( $str24$_A_is_not_a_valid__sbhl_type_erro, sbhl_paranoia.$sbhl_type_error_action$.getDynamicValue( thread ) );
              Errors.cerror( $str22$continue_anyway, $str19$_A_is_not_a__A, tv_var, $sym20$SBHL_TRUE_TV_P );
            }
          }
          ...
          try
          {
            sbhl_search_vars.$sbhl_search_module$.bind( sbhl_module_vars.get_sbhl_module( $const25$genlMt ), thread );
            sbhl_search_vars.$sbhl_search_module_type$.bind( sbhl_module_utilities.get_sbhl_module_type( sbhl_module_vars.get_sbhl_module( $const25$genlMt ) ), thread );
            sbhl_search_vars.$sbhl_add_node_to_result_test$.bind( sbhl_module_utilities.get_sbhl_add_node_to_result_test( sbhl_module_vars.get_sbhl_module( $const25$genlMt ) ), thread );
            sbhl_search_vars.$genl_inverse_mode_p$.bind( NIL, thread );
            sbhl_module_vars.$sbhl_module$.bind( sbhl_module_vars.get_sbhl_module( $const25$genlMt ), thread );
            if( NIL != sbhl_paranoia.suspend_sbhl_type_checkingP() || NIL != sbhl_module_utilities.apply_sbhl_module_type_test( content_mt, sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
            {
              ...
              try
              {
                ...
                while ( NIL != node_var)
                {
                  ...
                  SubLObject cdolist_list_var;
                  final SubLObject accessible_modules = cdolist_list_var = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_vars.get_sbhl_module( $const25$genlMt ) );
            ...
            {
              sbhl_paranoia.sbhl_warn( TWO_INTEGER, $str28$Node__a_does_not_pass_sbhl_type_t, content_mt, sbhl_module_utilities.get_sbhl_type_test( sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ), UNPROVIDED,
  ...
  public static SubLObject sksi_translate_all_sk_source_in_just_mt(final SubLObject content_mt, final SubLObject meta_mt, SubLObject check_wff_nessP, SubLObject start_row, SubLObject end_row,
      SubLObject assert_as_you_goP)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
      check_wff_nessP = NIL;
  ...
  public static SubLObject sksi_translate_all_sk_sources_in_mt_to_file(final SubLObject filename, final SubLObject content_mt, final SubLObject meta_mt, SubLObject check_wff_nessP, SubLObject start_row,
      SubLObject end_row)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
  ...
  public static SubLObject sksi_translate_all_simple_sk_source(final SubLObject simple_sk_source, final SubLObject meta_mt, SubLObject check_wff_nessP, SubLObject start_row, SubLObject end_row,
      SubLObject assert_as_you_goP)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
  ...
  public static SubLObject sksi_translate_all_simple_sk_source_to_stream(final SubLObject simple_sk_source, final SubLObject meta_mt, SubLObject stream, SubLObject check_wff_nessP, SubLObject start_row,
      SubLObject end_row)
  {
    if( stream == UNPROVIDED )
    {
  ...
  public static SubLObject sksi_translate_simple_sk_source(final SubLObject simple_sk_source, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject meta_mt, SubLObject check_wff_nessP,
      SubLObject start_row, SubLObject end_row, SubLObject assert_as_you_goP)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
      check_wff_nessP = NIL;
            ...
            try
            {
              ...
              for( SubLObject done_var = NIL; NIL == done_var; done_var = makeBoolean( NIL == valid ) )
              {
                ...
                if( NIL != valid )
                {
                  ...
                  if( NIL != ist_sentence )
                  {
                    sentences = ConsesLow.cons( ist_sentence, sentences );
                  }
                }
              }
            }
  ...
  public static SubLObject sksi_translate_simple_sk_source_to_stream(final SubLObject simple_sk_source, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject meta_mt, SubLObject stream,
      SubLObject check_wff_nessP, SubLObject start_row, SubLObject end_row)
  {
    if( stream == UNPROVIDED )
    {
      stream = StreamsLow.$standard_output$.getDynamicValue();
      ...
      if( NIL != content_mt )
      {
        ...
        if( NIL != iteration.iterator_p( raw_iterator ) )
        {
          ...
          try
          {
            ...
            try
            {
              PrintLow.format( stream, $str29$____Translation_Time____A__, numeric_date_utilities.timestring( UNPROVIDED ) );
              PrintLow.format( stream, $str30$_____ );
              PrintLow.format( stream, $str31$___Meta_Mt_____________S__, meta_mt );
              PrintLow.format( stream, $str32$___Source______________S__, simple_sk_source );
              PrintLow.format( stream, $str33$___Physical_Schema_____S__, physical_schema );
              PrintLow.format( stream, $str34$___Logical_Schema______S__, logical_schema );
              PrintLow.format( stream, $str30$_____ );
              PrintLow.format( stream, $str35$___Content_Mt__________S, content_mt );
              SubLObject valid;
              for( SubLObject done_var = NIL; NIL == done_var; done_var = makeBoolean( NIL == valid ) )
              {
                thread.resetMultipleValues();
                ...
                if( NIL != valid )
                {
                  ...
                  try
                  {
                    ...
                    PrintLow.format( stream, $str36$________S_, raw_tuple );
                  }
                  finally
                  {
  ...
  public static SubLObject sksi_translate_simple_sk_source_to_file(final SubLObject filename, final SubLObject sk_source, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject meta_mt,
      SubLObject check_wff_nessP, SubLObject start_row, SubLObject end_row)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
  ...
  public static SubLObject new_sksi_translate_iterator(final SubLObject sk_source, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject meta_mt, SubLObject check_wff_nessP,
      SubLObject start_row, SubLObject end_row)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
    ...
    return iteration.new_iterator( make_sksi_translate_iterator_state( sk_source, physical_schema, logical_schema, content_mt, meta_mt, check_wff_nessP, start_row, end_row ), $sym37$SKSI_TRANSLATE_ITERATE_DONE,
        $sym38$SKSI_TRANSLATE_ITERATE_NEXT, $sym39$BOOLEAN );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-batch-translate.lisp", position = 31958L)
  public static SubLObject make_sksi_translate_iterator_state(final SubLObject sk_source, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject content_mt, final SubLObject meta_mt,
      final SubLObject check_wff_nessP, final SubLObject start_row, final SubLObject end_row)
  {
    ...
    return ConsesLow.list( tuple_iterator, v_memoization_state, physical_schema, logical_schema, content_mt, meta_mt, check_wff_nessP );
  }
  ...
  public static SubLObject sksi_translate_iterate_done(final SubLObject state)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( state, state, $list40 );
    tuple_iterator = state.first();
    final SubLObject rest;
    final SubLObject current = rest = state.rest();
    return Equality.eq( $kw41$DONE, tuple_iterator );
  }
  ...
  public static SubLObject sksi_translate_iterate_next(final SubLObject state)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( state, state, $list42 );
    tuple_iterator = state.first();
    SubLObject current = state.rest();
    cdestructuring_bind.destructuring_bind_must_consp( current, state, $list42 );
    ...
    if( NIL != current )
    {
      cdestructuring_bind.cdestructuring_bind_error( state, $list42 );
      return NIL;
    }
    thread.resetMultipleValues();
    ...
    if( NIL == valid )
    {
      final SubLObject update = $kw41$DONE;
    ...
    return Values.values( logical_sentence, state, NIL );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-batch-translate.lisp", position = 33329L)
  public static SubLObject sksi_translate_one_raw_tuple_to_ist_logical_sentence(final SubLObject raw_tuple, final SubLObject content_mt, final SubLObject physical_schema, final SubLObject logical_schema,
      final SubLObject meta_mt, SubLObject check_wff_nessP, SubLObject assert_as_you_goP)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
      check_wff_nessP = NIL;
    ...
    if( NIL != logical_sentence )
    {
      ...
      final SubLObject ist_sentence = el_utilities.make_binary_formula( $const43$ist, content_mt, el_sentence );
  ...
  public static SubLObject sksi_translate_one_raw_tuple_to_logical_sentence(final SubLObject raw_tuple, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject content_mt,
      final SubLObject meta_mt, SubLObject check_wff_nessP)
  {
    if( check_wff_nessP == UNPROVIDED )
    {
      check_wff_nessP = NIL;
  ...
  public static SubLObject sksi_translate_one_raw_tuple_to_logical_sentence_int(final SubLObject raw_tuple, final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject content_mt,
      final SubLObject meta_mt)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( THREE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str45$__a__Translating__s_wrt__a__a__a_ ), new SubLObject[] { THREE_INTEGER,
        numeric_date_utilities.timestring( UNPROVIDED ), raw_tuple, physical_schema, logical_schema, meta_mt, content_mt
      } );
    ...
    try
    {
      ...
      if( NIL != raw_sentence )
      {
        ...
        {
          PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str46$Raw_sentence___s ), FOUR_INTEGER, raw_sentence );
          streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
        }
        thread.resetMultipleValues();
        ...
        if( NIL != decoded_sentence )
        {
          if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( FOUR_INTEGER ) )
          {
            PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str47$Decoded_sentence___s ), FOUR_INTEGER, decoded_sentence );
            streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
          }
  ...
  public static SubLObject sksi_postprocess_translated_sentence(final SubLObject logical_sentence, final SubLObject raw_tuple, final SubLObject physical_schema, final SubLObject logical_schema,
      final SubLObject content_mt, final SubLObject check_wff_nessP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( FOUR_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str48$Logical_sentence___s ), FOUR_INTEGER, logical_sentence );
      streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str49$Logical_conjunct___s ), FOUR_INTEGER, conjunct );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
      ...
      if( NIL != var )
      {
        processed_sentences = ConsesLow.cons( var, processed_sentences );
      }
      cdolist_list_var = cdolist_list_var.rest();
      conjunct = cdolist_list_var.first();
    }
    final SubLObject result = simplifier.possibly_conjoin( Sequences.nreverse( processed_sentences ), UNPROVIDED );
    return result;
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-batch-translate.lisp", position = 37599L)
  public static SubLObject sksi_postprocess_translated_asent(final SubLObject raw_tuple, final SubLObject physical_schema, final SubLObject logical_schema, SubLObject logical_sentence, final SubLObject content_mt,
      final SubLObject check_wff_nessP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    if( NIL == logical_sentence )
    {
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( THREE_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str50$Skipped_due_to_wholly_untranslata ), THREE_INTEGER );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
      return NIL;
    }
    logical_sentence = sksi_remove_unreformulatable( logical_sentence );
    if( NIL != kb_utilities.kbeq( $const51$True, logical_sentence ) )
    {
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( THREE_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str52$Skipped_due_to_tautology_ ), THREE_INTEGER );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
      return NIL;
    }
    if( NIL == logical_sentence )
    {
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( THREE_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str53$Skipped_due_to_unreformulatable_ ), THREE_INTEGER );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
      return NIL;
    }
    ...
    if( NIL != result )
    {
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( THREE_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str54$Translation___s ), THREE_INTEGER, result );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
    }
    else
    {
      ...
      why_not_wff_format_string = Sequences.cconcatenate( $str56$The_raw_tuple__S_using_the_physic, why_not_wff_format_string );
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( TWO_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, why_not_wff_format_string ), new SubLObject[] { TWO_INTEGER, raw_tuple, physical_schema,
          logical_schema, logical_sentence, content_mt, why_not_wff_explanation
        } );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
    }
    ...
    if( NIL != forts.fort_p( cycl_utilities.formula_operator( logical_sentence ) ) )
    {
      ...
      try
      {
        ...
        if( NIL != sksi_batch_translate_coerce_typesP() )
        {
          ...
          if( NIL != explanation )
          {
            final SubLObject explanation_of_why_not_wff_func = ( NIL != check_wff_nessP ) ? $sym57$EXPLANATION_OF_WHY_NOT_WFF_ASSERT : $sym58$EXPLANATION_OF_WHY_NOT_ADMITTED_SENTENCE_WRT_ASENT_ARG_CONSTRAINT;
            thread.resetMultipleValues();
            ...
            else if( NIL != simplified_explanation )
            {
  ...
  public static SubLObject sksi_remove_untranslatable(final SubLObject sentence)
  {
    if( NIL == el_utilities.atomic_sentenceP( sentence ) )
    {
      ...
      SubLObject cdolist_list_var;
      final SubLObject args = cdolist_list_var = cycl_utilities.formula_args( sentence, $kw64$IGNORE );
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          results = ConsesLow.cons( sub_sentence, results );
        }
        cdolist_list_var = cdolist_list_var.rest();
        sub_sentence = cdolist_list_var.first();
      }
  ...
  {
    return cycl_utilities.expression_find( $kw65$UNTRANSLATABLE, sentence, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  {
    return makeBoolean( NIL != cycl_utilities.expression_find( $kw66$UNREFORMULATABLE, sentence, UNPROVIDED, UNPROVIDED, UNPROVIDED ) || NIL != cycl_utilities.expression_find( $kw67$NULL, sentence, UNPROVIDED,
        UNPROVIDED, UNPROVIDED ) );
  }
  ...
  public static SubLObject sksi_translate_raw_sentence(final SubLObject raw_sentence, final SubLObject physical_schema, final SubLObject relevant_pf_indexicals, final SubLObject logical_schema,
  ...
  public static SubLObject sksi_translate_decoded_sentence(final SubLObject decoded_sentence, final SubLObject logical_schema, final SubLObject relevant_logical_field_indexicals, final SubLObject content_mt,
      final SubLObject meta_mt)
  {
  ...
  public static SubLObject new_sk_source_row_iterator(final SubLObject sk_source, final SubLObject physical_schema, final SubLObject meta_mt, SubLObject start_row, SubLObject end_row,
      SubLObject specified_physical_fields)
  {
    if( start_row == UNPROVIDED )
    {
      start_row = ZERO_INTEGER;
    ...
    if( NIL != list_utilities.alist_has_keyP( $sksi_row_iterator_overrides$.getDynamicValue( thread ), el_sk_source, Symbols.symbol_function( EQUAL ) ) )
    {
      final SubLObject overrides = list_utilities.alist_lookup( $sksi_row_iterator_overrides$.getDynamicValue( thread ), el_sk_source, Symbols.symbol_function( EQUAL ), UNPROVIDED );
    ...
    try
    {
      ...
      input_iterator = sksi_sks_interaction.generate_iterator_from_csql( csql, sk_source, NIL, start_row, end_row, $kw69$BATCH );
    }
    finally
    {
      mt_relevance_macros.$relevant_mts$.rebind( _prev_bind_3, thread );
  ...
  public static SubLObject sk_source_row_iterator_next(final SubLObject iterator)
  {
    ...
    return Values.values( outer_answer, iterator, makeBoolean( NIL == outer_valid ) );
  }
  ...
  public static SubLObject sksi_raw_tuple_translation_pattern_internal(final SubLObject physical_schema)
  {
    ...
    for( field_number = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), pf_indexical = list_var.first(), field_number = Numbers.add( ONE_INTEGER, field_number ) )
    {
      ...
      datum_vars = ConsesLow.cons( datum_var, datum_vars );
      if( NIL != pf_indexical )
      {
        final SubLObject field_value_literal = ConsesLow.list( $const71$fieldValue, pf_indexical, ConsesLow.list( $kw72$VALUE, datum_var ) );
    ...
    final SubLObject translation_pattern = ConsesLow.list( $kw73$TUPLE, datum_vars, el_utilities.make_conjunction( field_value_literals ) );
  ...
  public static SubLObject sksi_raw_tuple_translation_pattern(final SubLObject physical_schema)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym70$SKSI_RAW_TUPLE_TRANSLATION_PATTERN, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym70$SKSI_RAW_TUPLE_TRANSLATION_PATTERN, ONE_INTEGER, NIL, EQL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym70$SKSI_RAW_TUPLE_TRANSLATION_PATTERN, caching_state );
    }
    ...
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_raw_tuple_translation_pattern_internal( physical_schema ) ) );
  ...
  {
    if( physical_schema.eql( $const75$IMDB_MovieActors_PS ) && logical_schema.eql( $const76$IMDB_MovieActors_LS ) )
    {
      return Values.values( $list77, $list78 );
    }
    ...
    for( field_number = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), pf_indexical = list_var.first(), field_number = Numbers.add( ONE_INTEGER, field_number ) )
    {
      ...
      final SubLObject field_value_literal = ConsesLow.list( $const71$fieldValue, $kw79$ANYTHING, ConsesLow.list( $kw80$BIND, raw_var ) );
      if( NIL != physical_field_has_physical_override( pf, mapping_mt ) )
      {
        raw_var_map = ConsesLow.cons( ConsesLow.cons( pf_indexical, ConsesLow.list( $kw81$CALL, $sym82$POSSIBLY_OVERRIDE_PHYSICAL_FIELD_VALUE, ConsesLow.list( $kw72$VALUE, raw_var ), pf, mapping_mt ) ), raw_var_map );
      }
      else
      {
        raw_var_map = ConsesLow.cons( ConsesLow.cons( pf_indexical, ConsesLow.list( $kw72$VALUE, raw_var ) ), raw_var_map );
      }
      field_value_literals = ConsesLow.cons( field_value_literal, field_value_literals );
    }
    raw_var_map = Sequences.nreverse( raw_var_map );
    field_value_literals = Sequences.nreverse( field_value_literals );
    final SubLObject relevant_logical_fields = sksi_kb_accessors.sksi_determine_relevant_logical_fields( Mapping.mapcar( $sym83$PHYSICAL_FIELD_FOR_INDEXICAL, pf_indexicals ), physical_schema, logical_schema );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        final SubLObject decoding = conses_high.sublis( raw_var_map, recipe, UNPROVIDED, UNPROVIDED );
        final SubLObject indexical_referent_literal = ConsesLow.list( $const84$indexicalReferent, logical_field_indexical, ConsesLow.list( $kw81$CALL, $sym85$SKSI_BATCH_REFORMULATE, decoding, content_mt ) );
    ...
    final SubLObject translation_pattern = ConsesLow.list( $kw86$TEMPLATE, el_utilities.make_conjunction( field_value_literals ), ConsesLow.list( $kw81$CALL, $sym87$SKSI_SIMPLIFY_DECODED_SENTENCE, el_utilities
        .make_conjunction( indexical_referent_literals ) ) );
  ...
  public static SubLObject sksi_raw_sentence_translation_pattern(final SubLObject physical_schema, final SubLObject pf_indexicals, final SubLObject logical_schema, final SubLObject content_mt)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym74$SKSI_RAW_SENTENCE_TRANSLATION_PATTERN, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym74$SKSI_RAW_SENTENCE_TRANSLATION_PATTERN, FOUR_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym74$SKSI_RAW_SENTENCE_TRANSLATION_PATTERN, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_raw_sentence_translation_pattern_internal( physical_schema, pf_indexicals, logical_schema, content_mt ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( physical_schema, pf_indexicals, logical_schema, content_mt ) );
  ...
  public static SubLObject sksi_batch_reformulate(final SubLObject expression, final SubLObject content_mt)
  {
    ...
    try
    {
      ...
      {
        v_answer = $kw89$UNDECODED;
      }
      else
      {
        v_answer = sksi_reformulate.sksi_reformulate( expression, $kw90$DECODE, sk_source );
      }
    }
    finally
    {
  ...
  public static SubLObject sksi_simplify_decoded_sentence(final SubLObject decoded_sentence)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      if( $kw89$UNDECODED != cycl_utilities.formula_arg2( indexical_referent, UNPROVIDED ) )
      {
        relevant_indexical_referents = ConsesLow.cons( indexical_referent, relevant_indexical_referents );
      }
      cdolist_list_var = cdolist_list_var.rest();
      indexical_referent = cdolist_list_var.first();
    }
  ...
  public static SubLObject sksi_logical_field_indexicals_of_decoded_sentence(final SubLObject decoded_sentence)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      relevant_logical_field_indexicals = ConsesLow.cons( logical_field_indexical, relevant_logical_field_indexicals );
      cdolist_list_var = cdolist_list_var.rest();
      indexical_referent = cdolist_list_var.first();
    }
  ...
  public static SubLObject prune_consequent_conditional_meaning_sentences_wrt_antecedents(final SubLObject conditional_meaning_sentences, final SubLObject decoded_sentence, final SubLObject content_mt,
      final SubLObject meta_mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        result = ConsesLow.cons( sksi_meaning_sentence_utilities.get_consequent_meaning_sentence_from_conditional_meaning_sentence( conditional_meaning_sentence ), result );
      }
      cdolist_list_var = cdolist_list_var.rest();
      conditional_meaning_sentence = cdolist_list_var.first();
    }
  ...
  public static SubLObject sksi_decoded_sentence_translation_pattern(final SubLObject logical_schema, final SubLObject relevant_logical_field_indexicals, final SubLObject decoded_sentence, final SubLObject content_mt,
      final SubLObject meta_mt)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    for( field_number = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), logical_field_indexical = list_var.first(), field_number = Numbers.add( ONE_INTEGER, field_number ) )
    {
      ...
      final SubLObject indexical_referent_literal = ConsesLow.list( $const84$indexicalReferent, $kw79$ANYTHING, ConsesLow.list( $kw80$BIND, arg_var ) );
      arg_var_map = ConsesLow.cons( ConsesLow.cons( logical_field_indexical, ConsesLow.list( $kw72$VALUE, arg_var ) ), arg_var_map );
      indexical_referent_literals = ConsesLow.cons( indexical_referent_literal, indexical_referent_literals );
    }
    arg_var_map = Sequences.nreverse( arg_var_map );
    indexical_referent_literals = Sequences.nreverse( indexical_referent_literals );
    final SubLObject meaning_sentences = sksi_meaning_sentence_utilities.relevant_logical_schema_meaning_sentences( logical_schema, relevant_logical_field_indexicals,
        $sksi_batch_translate_only_asserted_meaning_sentencesP$.getDynamicValue( thread ), UNPROVIDED );
    ...
    final SubLObject logical_sentence_recipe = simplifier.simplify_cycl_sentence_syntax( el_utilities.make_conjunction( ConsesLow.append( meaning_sentences, applicable_conditional_meaning_sentences ) ), UNPROVIDED );
    final SubLObject logical_sentence_template = conses_high.sublis( arg_var_map, logical_sentence_recipe, Symbols.symbol_function( EQUAL ), UNPROVIDED );
    return ConsesLow.list( $kw86$TEMPLATE, el_utilities.make_conjunction( indexical_referent_literals ), logical_sentence_template );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject primary_key_values_list = Hashtables.gethash( sentence_to_be_asserted, $sksi_batch_assert_justification_table$.getDynamicValue( thread ), UNPROVIDED );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject pk_values_sentence = ConsesLow.listS( $const92$tablePrimaryKeyValues, table, ConsesLow.append( pk_value_strings, NIL ) );
      ...
      final SubLObject sksi_support = fi.make_el_support( $kw93$OPAQUE, ConsesLow.list( $const94$schemaTranslation, table, target_mt ), meta_mt, UNPROVIDED );
      if( NIL == sksi_batch_add_argument( sentence_to_be_asserted, target_mt, ConsesLow.list( rule, pk_assertion, sksi_support ), NIL ) )
      {
        successP = NIL;
      }
      cdolist_list_var = cdolist_list_var.rest();
  ...
  public static SubLObject possibly_remember_primary_key_that_resulted_in_raw_tuple(final SubLObject raw_tuple, final SubLObject el_sentence)
  {
    ...
    if( NIL != misc_utilities.initialized_p( $sksi_batch_assert_justification_table$.getDynamicValue( thread ) ) )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        primary_key_values = ConsesLow.cons( value, primary_key_values );
        cdolist_list_var = cdolist_list_var.rest();
        i = cdolist_list_var.first();
      }
  ...
  public static SubLObject with_sksi_batch_templates(final SubLObject macroform, final SubLObject environment)
  {
    ...
    final SubLObject body;
    final SubLObject current = body = datum;
    return ConsesLow.listS( $sym2$CLET, $list95, reader.bq_cons( $sym96$PROGN, ConsesLow.append( body, NIL ) ), $list97 );
  }
  ...
  public static SubLObject possibly_with_sksi_batch_templates(final SubLObject macroform, final SubLObject environment)
  {
    ...
    final SubLObject body;
    current = ( body = current.rest() );
    return ConsesLow.list( $sym99$PIF, use_themP, reader.bq_cons( $sym100$WITH_SKSI_BATCH_TEMPLATES, ConsesLow.append( body, NIL ) ), reader.bq_cons( $sym96$PROGN, ConsesLow.append( body, NIL ) ) );
  }
  ...
  {
    $sksi_batch_templates$.setDynamicValue( $kw0$UNINITIALIZED );
    return NIL;
  }
  ...
  {
    return makeBoolean( v_object.isCons() && NIL != list_utilities.lengthE( v_object, TWO_INTEGER, UNPROVIDED ) && NIL != inference_utilities.allowed_rules_spec_p( v_object.first() ) && NIL != Types.booleanp( conses_high
        .second( v_object ) ) );
  }
  ...
  {
    assert NIL != inference_utilities.allowed_rules_spec_p( rules ) : rules;
    assert NIL != Types.booleanp( use_type_coercionP ) : use_type_coercionP;
    return ConsesLow.list( rules, use_type_coercionP );
  }
  ...
  public static SubLObject remember_batch_template_for_future_use(final SubLObject el_sentence, final SubLObject content_mt, final SubLObject physical_schema, final SubLObject logical_schema)
  {
    ...
    {
      return $kw0$UNINITIALIZED;
    }
    final SubLObject narts = Sequences.remove_if( $sym105$LOGICAL_FIELD_INDEXICAL_P, Sequences.remove( content_mt, cycl_utilities.expression_gather( el_sentence, $sym106$NART_P, UNPROVIDED, UNPROVIDED, UNPROVIDED,
        UNPROVIDED ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject sksi_batch_template_lookup(final SubLObject physical_schema, final SubLObject logical_schema, final SubLObject content_mt)
  {
    ...
    final SubLObject key = ConsesLow.list( physical_schema, logical_schema, content_mt );
    return dictionary.dictionary_lookup_without_values( $sksi_batch_templates$.getDynamicValue( thread ), key, $kw0$UNINITIALIZED );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    assert NIL != sksi_batch_template_p( value ) : value;
    final SubLObject key = ConsesLow.list( physical_schema, logical_schema, content_mt );
  ...
  {
    return Mapping.mapcar( $sym108$CONVERT_SQL_DATUM_TO_STRING, data );
  }
  ...
  public static SubLObject convert_sql_datum_to_string(final SubLObject datum)
  {
    ...
    try
    {
      reader.$read_default_float_format$.bind( $sym109$DOUBLE_FLOAT, thread );
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $sksi_schema_translation_lifting_rule_caching_state$.getGlobalValue(), ConsesLow.list( EMPTY_SUBL_OBJECT_ARRAY ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject sksi_schema_translation_lifting_rule()
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym112$SKSI_SCHEMA_TRANSLATION_LIFTING_RULE, $sym114$_SKSI_SCHEMA_TRANSLATION_LIFTING_RULE_CACHING_STATE_, ONE_INTEGER, EQL, ZERO_INTEGER,
          ZERO_INTEGER );
    }
    SubLObject results = memoization_state.caching_state_get_zero_arg_results( caching_state, UNPROVIDED );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( sksi_schema_translation_lifting_rule_internal() ) );
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $sksi_get_datum_var_caching_state$.getGlobalValue(), ConsesLow.list( number ), UNPROVIDED, UNPROVIDED );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-batch-translate.lisp", position = 60984L)
  public static SubLObject sksi_get_datum_var_internal(final SubLObject number)
  {
    return Packages.intern( Sequences.cconcatenate( $str116$DATUM_, format_nil.format_nil_s_no_copy( number ) ), UNPROVIDED );
  }
  ...
  public static SubLObject sksi_get_datum_var(final SubLObject number)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym115$SKSI_GET_DATUM_VAR, $sym117$_SKSI_GET_DATUM_VAR_CACHING_STATE_, NIL, EQL, ONE_INTEGER, TEN_INTEGER );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( sksi_get_datum_var_internal( number ) ) );
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $sksi_get_raw_var_caching_state$.getGlobalValue(), ConsesLow.list( number ), UNPROVIDED, UNPROVIDED );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-batch-translate.lisp", position = 61314L)
  public static SubLObject sksi_get_raw_var_internal(final SubLObject number)
  {
    return Packages.intern( Sequences.cconcatenate( $str119$RAW_, format_nil.format_nil_s_no_copy( number ) ), UNPROVIDED );
  }
  ...
  public static SubLObject sksi_get_raw_var(final SubLObject number)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym118$SKSI_GET_RAW_VAR, $sym120$_SKSI_GET_RAW_VAR_CACHING_STATE_, NIL, EQL, ONE_INTEGER, TEN_INTEGER );
    }
    SubLObject results = memoization_state.caching_state_lookup( caching_state, number, memoization_state.$memoized_item_not_found$.getGlobalValue() );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( sksi_get_raw_var_internal( number ) ) );
      memoization_state.caching_state_put( caching_state, number, results, UNPROVIDED );
    }
    return memoization_state.caching_results( results );
  }
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $sksi_get_arg_var_caching_state$.getGlobalValue(), ConsesLow.list( number ), UNPROVIDED, UNPROVIDED );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-batch-translate.lisp", position = 61432L)
  public static SubLObject sksi_get_arg_var_internal(final SubLObject number)
  {
    return Packages.intern( Sequences.cconcatenate( $str122$ARG_, format_nil.format_nil_s_no_copy( number ) ), UNPROVIDED );
  }
  ...
  public static SubLObject sksi_get_arg_var(final SubLObject number)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym121$SKSI_GET_ARG_VAR, $sym123$_SKSI_GET_ARG_VAR_CACHING_STATE_, NIL, EQL, ONE_INTEGER, TEN_INTEGER );
    }
    SubLObject results = memoization_state.caching_state_lookup( caching_state, number, memoization_state.$memoized_item_not_found$.getGlobalValue() );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( sksi_get_arg_var_internal( number ) ) );
      memoization_state.caching_state_put( caching_state, number, results, UNPROVIDED );
    }
    return memoization_state.caching_results( results );
  }
  ...
  public static SubLObject sksi_batch_assert(final SubLObject sentence, final SubLObject mt, final SubLObject check_wffP)
  {
    ...
    if( NIL != $sksi_batch_add_operations_to_transcriptP$.getDynamicValue( thread ) )
    {
      ...
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str124$Assertion_of__s_in__s_failed___a ), new SubLObject[] { ONE_INTEGER, sentence, mt,
          error
        } );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
    ...
    final SubLObject successP = cyc_kernel.cyc_assert( sentence, mt, ConsesLow.list( $kw125$CHECK_WFF_, check_wffP ) );
    if( NIL == successP && NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( ONE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str124$Assertion_of__s_in__s_failed___a ), new SubLObject[] { ONE_INTEGER, sentence, mt, fi
          .fi_get_error_int()
      } );
      streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
    }
  ...
  public static SubLObject sksi_batch_add_argument(final SubLObject sentence, final SubLObject mt, final SubLObject supports, final SubLObject check_wffP)
  {
    ...
    final SubLObject successP = cyc_kernel.cyc_add_argument( sentence, supports, mt, ConsesLow.list( $kw125$CHECK_WFF_, check_wffP ), UNPROVIDED );
  ...
  public static SubLObject sksi_batch_add_argument_return_value_handler(final SubLObject sentence, final SubLObject mt, final SubLObject supports, final SubLObject successP, SubLObject error)
  {
    ...
    if( NIL == successP )
    {
      ...
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str126$Deduction_of__s_in__s_with_suppor ), new SubLObject[] { ONE_INTEGER, sentence, mt,
          supports, error
        } );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
    ...
    {
      Functions.funcall( $sksi_batch_translate_add_argument_callback$.getDynamicValue( thread ), sentence, mt, supports, successP );
    }
    return successP;
  }
  ...
  public static SubLObject sksi_batch_unassert_return_value_handler(final SubLObject sentence, final SubLObject mt, final SubLObject successP, SubLObject error)
  {
    ...
    if( NIL == successP )
    {
      ...
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( TWO_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str44$__SKSI__A_, $str127$Unassert_of__s_from__s_failed___a ), new SubLObject[] { TWO_INTEGER, sentence, mt,
          error
        } );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
    ...
    {
      Functions.funcall( $sksi_batch_translate_unassert_callback$.getDynamicValue( thread ), sentence, mt, successP );
    }
    return successP;
  }
  ...
  public static SubLObject test_sksi_wff_check_batch_translated_sentence(final SubLObject logical_sentence, final SubLObject content_mt, final SubLObject check_wff_nessP, final SubLObject coerce_typesP)
  {
    ...
    try
    {
      ...
      thread.resetMultipleValues();
      return ConsesLow.list( result, format_string, explanation );
    }
    finally
    {
  ...
  {
    SubLFiles.declareFunction( me, "set_sksi_batch_translate_only_asserted_meaning_sentences", "SET-SKSI-BATCH-TRANSLATE-ONLY-ASSERTED-MEANING-SENTENCES", 1, 0, false );
    SubLFiles.declareFunction( me, "set_sksi_batch_translate_coerce_types", "SET-SKSI-BATCH-TRANSLATE-COERCE-TYPES", 1, 0, false );
    SubLFiles.declareMacro( me, "with_sksi_batch_translate_only_asserted_meaning_sentences", "WITH-SKSI-BATCH-TRANSLATE-ONLY-ASSERTED-MEANING-SENTENCES" );
    SubLFiles.declareMacro( me, "without_sksi_batch_translate_only_asserted_meaning_sentences", "WITHOUT-SKSI-BATCH-TRANSLATE-ONLY-ASSERTED-MEANING-SENTENCES" );
    SubLFiles.declareMacro( me, "with_sksi_batch_translate_coerce_types", "WITH-SKSI-BATCH-TRANSLATE-COERCE-TYPES" );
    SubLFiles.declareMacro( me, "without_sksi_batch_translate_coerce_types", "WITHOUT-SKSI-BATCH-TRANSLATE-COERCE-TYPES" );
    SubLFiles.declareMacro( me, "with_sksi_batch_sql_select_statement", "WITH-SKSI-BATCH-SQL-SELECT-STATEMENT" );
    SubLFiles.declareFunction( me, "sksi_batch_assert_translations_of_table", "SKSI-BATCH-ASSERT-TRANSLATIONS-OF-TABLE", 1, 3, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert_all_sk_sources_in_mt", "SKSI-BATCH-ASSERT-ALL-SK-SOURCES-IN-MT", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert_all_sk_sources_in_mt_to_ke_stream", "SKSI-BATCH-ASSERT-ALL-SK-SOURCES-IN-MT-TO-KE-STREAM", 3, 4, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert_all_sk_sources_in_mt_to_ke_string", "SKSI-BATCH-ASSERT-ALL-SK-SOURCES-IN-MT-TO-KE-STRING", 3, 3, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert_all_sk_sources_in_mt_to_ke_file", "SKSI-BATCH-ASSERT-ALL-SK-SOURCES-IN-MT-TO-KE-FILE", 4, 3, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_sk_sources_in_mt", "SKSI-TRANSLATE-ALL-SK-SOURCES-IN-MT", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_sk_source_in_just_mt", "SKSI-TRANSLATE-ALL-SK-SOURCE-IN-JUST-MT", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_sk_sources_in_mt_to_stream", "SKSI-TRANSLATE-ALL-SK-SOURCES-IN-MT-TO-STREAM", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_sk_sources_in_mt_to_file", "SKSI-TRANSLATE-ALL-SK-SOURCES-IN-MT-TO-FILE", 3, 3, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_spec_sk_sources", "SKSI-TRANSLATE-ALL-SPEC-SK-SOURCES", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_spec_sk_sources_to_stream", "SKSI-TRANSLATE-ALL-SPEC-SK-SOURCES-TO-STREAM", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_spec_sk_sources_to_file", "SKSI-TRANSLATE-ALL-SPEC-SK-SOURCES-TO-FILE", 3, 3, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_simple_sk_source", "SKSI-TRANSLATE-ALL-SIMPLE-SK-SOURCE", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_all_simple_sk_source_to_stream", "SKSI-TRANSLATE-ALL-SIMPLE-SK-SOURCE-TO-STREAM", 2, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_simple_sk_source", "SKSI-TRANSLATE-SIMPLE-SK-SOURCE", 4, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_simple_sk_source_to_stream", "SKSI-TRANSLATE-SIMPLE-SK-SOURCE-TO-STREAM", 4, 4, false );
    SubLFiles.declareFunction( me, "sksi_translate_simple_sk_source_to_file", "SKSI-TRANSLATE-SIMPLE-SK-SOURCE-TO-FILE", 5, 3, false );
    SubLFiles.declareFunction( me, "new_sksi_translate_iterator", "NEW-SKSI-TRANSLATE-ITERATOR", 4, 3, false );
    SubLFiles.declareFunction( me, "make_sksi_translate_iterator_state", "MAKE-SKSI-TRANSLATE-ITERATOR-STATE", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_iterate_done", "SKSI-TRANSLATE-ITERATE-DONE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_iterate_next", "SKSI-TRANSLATE-ITERATE-NEXT", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_one_raw_tuple_to_ist_logical_sentence", "SKSI-TRANSLATE-ONE-RAW-TUPLE-TO-IST-LOGICAL-SENTENCE", 5, 2, false );
    SubLFiles.declareFunction( me, "sksi_translate_one_raw_tuple_to_logical_sentence", "SKSI-TRANSLATE-ONE-RAW-TUPLE-TO-LOGICAL-SENTENCE", 5, 1, false );
    SubLFiles.declareFunction( me, "sksi_translate_one_raw_tuple_to_logical_sentence_int", "SKSI-TRANSLATE-ONE-RAW-TUPLE-TO-LOGICAL-SENTENCE-INT", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_postprocess_translated_sentence", "SKSI-POSTPROCESS-TRANSLATED-SENTENCE", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_postprocess_translated_asent", "SKSI-POSTPROCESS-TRANSLATED-ASENT", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_wff_check_batch_translated_sentence", "SKSI-WFF-CHECK-BATCH-TRANSLATED-SENTENCE", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_remove_untranslatable", "SKSI-REMOVE-UNTRANSLATABLE", 1, 0, false );
    SubLFiles.declareFunction( me, "sentence_contains_untranslatableP", "SENTENCE-CONTAINS-UNTRANSLATABLE?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_remove_unreformulatable", "SKSI-REMOVE-UNREFORMULATABLE", 1, 0, false );
    SubLFiles.declareFunction( me, "sentence_contains_unreformulatableP", "SENTENCE-CONTAINS-UNREFORMULATABLE?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_raw_tuple", "SKSI-TRANSLATE-RAW-TUPLE", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_raw_sentence", "SKSI-TRANSLATE-RAW-SENTENCE", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_decoded_sentence", "SKSI-TRANSLATE-DECODED-SENTENCE", 5, 0, false );
    SubLFiles.declareFunction( me, "new_sk_source_row_iterator", "NEW-SK-SOURCE-ROW-ITERATOR", 3, 3, false );
    SubLFiles.declareFunction( me, "sk_source_row_iterator_next", "SK-SOURCE-ROW-ITERATOR-NEXT", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_raw_tuple_translation_pattern_internal", "SKSI-RAW-TUPLE-TRANSLATION-PATTERN-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_raw_tuple_translation_pattern", "SKSI-RAW-TUPLE-TRANSLATION-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_raw_sentence_translation_pattern_internal", "SKSI-RAW-SENTENCE-TRANSLATION-PATTERN-INTERNAL", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_raw_sentence_translation_pattern", "SKSI-RAW-SENTENCE-TRANSLATION-PATTERN", 4, 0, false );
    SubLFiles.declareFunction( me, "physical_field_has_physical_override", "PHYSICAL-FIELD-HAS-PHYSICAL-OVERRIDE", 2, 0, false );
    SubLFiles.declareFunction( me, "possibly_override_physical_field_value", "POSSIBLY-OVERRIDE-PHYSICAL-FIELD-VALUE", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_reformulate", "SKSI-BATCH-REFORMULATE", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_simplify_decoded_sentence", "SKSI-SIMPLIFY-DECODED-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_logical_field_indexicals_of_decoded_sentence", "SKSI-LOGICAL-FIELD-INDEXICALS-OF-DECODED-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "prune_consequent_conditional_meaning_sentences_wrt_antecedents", "PRUNE-CONSEQUENT-CONDITIONAL-MEANING-SENTENCES-WRT-ANTECEDENTS", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_decoded_sentence_translation_pattern", "SKSI-DECODED-SENTENCE-TRANSLATION-PATTERN", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert_sentence_with_deduced_argument_assuming_table", "SKSI-BATCH-ASSERT-SENTENCE-WITH-DEDUCED-ARGUMENT-ASSUMING-TABLE", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert_sentence_with_deduced_argument", "SKSI-BATCH-ASSERT-SENTENCE-WITH-DEDUCED-ARGUMENT", 4, 0, false );
    SubLFiles.declareFunction( me, "possibly_remember_primary_key_that_resulted_in_raw_tuple", "POSSIBLY-REMEMBER-PRIMARY-KEY-THAT-RESULTED-IN-RAW-TUPLE", 2, 0, false );
    SubLFiles.declareMacro( me, "with_sksi_batch_templates", "WITH-SKSI-BATCH-TEMPLATES" );
    SubLFiles.declareMacro( me, "possibly_with_sksi_batch_templates", "POSSIBLY-WITH-SKSI-BATCH-TEMPLATES" );
    SubLFiles.declareFunction( me, "initialize_sksi_batch_templates", "INITIALIZE-SKSI-BATCH-TEMPLATES", 0, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_batch_templates", "CLEAR-SKSI-BATCH-TEMPLATES", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_template_p", "SKSI-BATCH-TEMPLATE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_template_allowed_rules", "SKSI-BATCH-TEMPLATE-ALLOWED-RULES", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_template_use_type_coercionP", "SKSI-BATCH-TEMPLATE-USE-TYPE-COERCION?", 1, 0, false );
    SubLFiles.declareFunction( me, "new_sksi_batch_template", "NEW-SKSI-BATCH-TEMPLATE", 2, 0, false );
    SubLFiles.declareFunction( me, "remember_batch_template_for_future_use", "REMEMBER-BATCH-TEMPLATE-FOR-FUTURE-USE", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_template_lookup", "SKSI-BATCH-TEMPLATE-LOOKUP", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_template_enter", "SKSI-BATCH-TEMPLATE-ENTER", 4, 0, false );
    SubLFiles.declareFunction( me, "current_sksi_batch_template", "CURRENT-SKSI-BATCH-TEMPLATE", 0, 0, false );
    SubLFiles.declareFunction( me, "current_sksi_batch_template_allowed_rules", "CURRENT-SKSI-BATCH-TEMPLATE-ALLOWED-RULES", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_translate_coerce_typesP", "SKSI-BATCH-TRANSLATE-COERCE-TYPES?", 0, 0, false );
    SubLFiles.declareFunction( me, "convert_sql_data_to_strings", "CONVERT-SQL-DATA-TO-STRINGS", 1, 0, false );
    SubLFiles.declareFunction( me, "convert_sql_datum_to_string", "CONVERT-SQL-DATUM-TO-STRING", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_schema_translation_lifting_rule", "CLEAR-SKSI-SCHEMA-TRANSLATION-LIFTING-RULE", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_sksi_schema_translation_lifting_rule", "REMOVE-SKSI-SCHEMA-TRANSLATION-LIFTING-RULE", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_schema_translation_lifting_rule_internal", "SKSI-SCHEMA-TRANSLATION-LIFTING-RULE-INTERNAL", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_schema_translation_lifting_rule", "SKSI-SCHEMA-TRANSLATION-LIFTING-RULE", 0, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_get_datum_var", "CLEAR-SKSI-GET-DATUM-VAR", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_sksi_get_datum_var", "REMOVE-SKSI-GET-DATUM-VAR", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_datum_var_internal", "SKSI-GET-DATUM-VAR-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_datum_var", "SKSI-GET-DATUM-VAR", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_get_raw_var", "CLEAR-SKSI-GET-RAW-VAR", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_sksi_get_raw_var", "REMOVE-SKSI-GET-RAW-VAR", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_raw_var_internal", "SKSI-GET-RAW-VAR-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_raw_var", "SKSI-GET-RAW-VAR", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_get_arg_var", "CLEAR-SKSI-GET-ARG-VAR", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_sksi_get_arg_var", "REMOVE-SKSI-GET-ARG-VAR", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_arg_var_internal", "SKSI-GET-ARG-VAR-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_arg_var", "SKSI-GET-ARG-VAR", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_assert", "SKSI-BATCH-ASSERT", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_add_argument", "SKSI-BATCH-ADD-ARGUMENT", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_add_argument_return_value_handler", "SKSI-BATCH-ADD-ARGUMENT-RETURN-VALUE-HANDLER", 4, 1, false );
    SubLFiles.declareFunction( me, "sksi_batch_unassert", "SKSI-BATCH-UNASSERT", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_batch_unassert_return_value_handler", "SKSI-BATCH-UNASSERT-RETURN-VALUE-HANDLER", 3, 1, false );
    SubLFiles.declareFunction( me, "test_sksi_wff_check_batch_translated_sentence", "TEST-SKSI-WFF-CHECK-BATCH-TRANSLATED-SENTENCE", 4, 0, false );
    return NIL;
  }

  public static SubLObject init_sksi_batch_translate_file()
  {
    $sksi_batch_translate_only_asserted_meaning_sentencesP$ = SubLFiles.defvar( "*SKSI-BATCH-TRANSLATE-ONLY-ASSERTED-MEANING-SENTENCES?*", T );
    $sksi_batch_translate_coerce_typesP$ = SubLFiles.defvar( "*SKSI-BATCH-TRANSLATE-COERCE-TYPES?*", NIL );
    $sksi_batch_translate_type_coercion_successfulP$ = SubLFiles.defparameter( "*SKSI-BATCH-TRANSLATE-TYPE-COERCION-SUCCESSFUL?*", $kw0$UNINITIALIZED );
    $sksi_batch_assert_justification_table$ = SubLFiles.defparameter( "*SKSI-BATCH-ASSERT-JUSTIFICATION-TABLE*", $kw0$UNINITIALIZED );
    $sksi_batch_primary_key_columns$ = SubLFiles.defparameter( "*SKSI-BATCH-PRIMARY-KEY-COLUMNS*", $kw0$UNINITIALIZED );
    $sksi_batch_sql_select_statement$ = SubLFiles.defparameter( "*SKSI-BATCH-SQL-SELECT-STATEMENT*", NIL );
    $sksi_batch_sql_order_by_primary_keyP$ = SubLFiles.defparameter( "*SKSI-BATCH-SQL-ORDER-BY-PRIMARY-KEY?*", NIL );
    $sksi_batch_add_operations_to_transcriptP$ = SubLFiles.defparameter( "*SKSI-BATCH-ADD-OPERATIONS-TO-TRANSCRIPT?*", NIL );
    $current_sksi_batch_template$ = SubLFiles.defparameter( "*CURRENT-SKSI-BATCH-TEMPLATE*", $kw0$UNINITIALIZED );
    $sksi_batch_templates$ = SubLFiles.defvar( "*SKSI-BATCH-TEMPLATES*", $kw0$UNINITIALIZED );
    $sksi_batch_translate_add_argument_callback$ = SubLFiles.defparameter( "*SKSI-BATCH-TRANSLATE-ADD-ARGUMENT-CALLBACK*", NIL );
    $sksi_batch_translate_unassert_callback$ = SubLFiles.defparameter( "*SKSI-BATCH-TRANSLATE-UNASSERT-CALLBACK*", NIL );
    $sksi_batch_asserting_translations_of_this_table$ = SubLFiles.defparameter( "*SKSI-BATCH-ASSERTING-TRANSLATIONS-OF-THIS-TABLE*", $kw0$UNINITIALIZED );
    $sksi_row_iterator_overrides$ = SubLFiles.defparameter( "*SKSI-ROW-ITERATOR-OVERRIDES*", $list68 );
    $sksi_schema_translation_lifting_rule_sentence$ = SubLFiles.deflexical( "*SKSI-SCHEMA-TRANSLATION-LIFTING-RULE-SENTENCE*", $list110 );
    $sksi_schema_translation_lifting_rule_mt$ = SubLFiles.deflexical( "*SKSI-SCHEMA-TRANSLATION-LIFTING-RULE-MT*", $const111$SKSIMt );
    $sksi_schema_translation_lifting_rule_caching_state$ = SubLFiles.deflexical( "*SKSI-SCHEMA-TRANSLATION-LIFTING-RULE-CACHING-STATE*", NIL );
    $sksi_get_datum_var_caching_state$ = SubLFiles.deflexical( "*SKSI-GET-DATUM-VAR-CACHING-STATE*", NIL );
    $sksi_get_raw_var_caching_state$ = SubLFiles.deflexical( "*SKSI-GET-RAW-VAR-CACHING-STATE*", NIL );
    $sksi_get_arg_var_caching_state$ = SubLFiles.deflexical( "*SKSI-GET-ARG-VAR-CACHING-STATE*", NIL );
    return NIL;
  }

  public static SubLObject setup_sksi_batch_translate_file()
  {
    memoization_state.note_memoized_function( $sym70$SKSI_RAW_TUPLE_TRANSLATION_PATTERN );
    memoization_state.note_memoized_function( $sym74$SKSI_RAW_SENTENCE_TRANSLATION_PATTERN );
    access_macros.register_macro_helper( $sym101$INITIALIZE_SKSI_BATCH_TEMPLATES, $sym100$WITH_SKSI_BATCH_TEMPLATES );
    access_macros.register_macro_helper( $sym103$CLEAR_SKSI_BATCH_TEMPLATES, $sym100$WITH_SKSI_BATCH_TEMPLATES );
    memoization_state.note_globally_cached_function( $sym112$SKSI_SCHEMA_TRANSLATION_LIFTING_RULE );
    memoization_state.note_globally_cached_function( $sym115$SKSI_GET_DATUM_VAR );
    memoization_state.note_globally_cached_function( $sym118$SKSI_GET_RAW_VAR );
    memoization_state.note_globally_cached_function( $sym121$SKSI_GET_ARG_VAR );
    utilities_macros.note_funcall_helper_function( $sym129$TEST_SKSI_WFF_CHECK_BATCH_TRANSLATED_SENTENCE );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_batch_translate();
    $sksi_batch_translate_only_asserted_meaning_sentencesP$ = null;
    $sksi_batch_translate_coerce_typesP$ = null;
    $sksi_batch_translate_type_coercion_successfulP$ = null;
    $sksi_batch_assert_justification_table$ = null;
    $sksi_batch_primary_key_columns$ = null;
    $sksi_batch_sql_select_statement$ = null;
    $sksi_batch_sql_order_by_primary_keyP$ = null;
    $sksi_batch_add_operations_to_transcriptP$ = null;
    $current_sksi_batch_template$ = null;
    $sksi_batch_templates$ = null;
    $sksi_batch_translate_add_argument_callback$ = null;
    $sksi_batch_translate_unassert_callback$ = null;
    $sksi_batch_asserting_translations_of_this_table$ = null;
    $sksi_row_iterator_overrides$ = null;
    $sksi_schema_translation_lifting_rule_sentence$ = null;
    $sksi_schema_translation_lifting_rule_mt$ = null;
    $sksi_schema_translation_lifting_rule_caching_state$ = null;
    $sksi_get_datum_var_caching_state$ = null;
    $sksi_get_raw_var_caching_state$ = null;
    $sksi_get_arg_var_caching_state$ = null;
    $kw0$UNINITIALIZED = makeKeyword( "UNINITIALIZED" );
    $sym1$BOOLEANP = makeSymbol( "BOOLEANP" );
    $sym2$CLET = makeSymbol( "CLET" );
    $list3 = ConsesLow.list( ConsesLow.list( makeSymbol( "*SKSI-BATCH-TRANSLATE-ONLY-ASSERTED-MEANING-SENTENCES?*" ), T ) );
    $list4 = ConsesLow.list( ConsesLow.list( makeSymbol( "*SKSI-BATCH-TRANSLATE-ONLY-ASSERTED-MEANING-SENTENCES?*" ), NIL ) );
    $list5 = ConsesLow.list( ConsesLow.list( makeSymbol( "*SKSI-BATCH-TRANSLATE-COERCE-TYPES?*" ), T ) );
    $list6 = ConsesLow.list( ConsesLow.list( makeSymbol( "*SKSI-BATCH-TRANSLATE-COERCE-TYPES?*" ), NIL ) );
    $list7 = ConsesLow.list( makeSymbol( "STRING" ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym8$_SKSI_BATCH_SQL_SELECT_STATEMENT_ = makeSymbol( "*SKSI-BATCH-SQL-SELECT-STATEMENT*" );
    $int9$1000 = makeInteger( 1000 );
    $str10$in_mt___S___ = makeString( "in mt: ~S.~%" );
    $str11$f___S___ = makeString( "f: ~S.~%" );
    $kw12$OUTPUT = makeKeyword( "OUTPUT" );
    $str13$Unable_to_open__S = makeString( "Unable to open ~S" );
    $kw14$BREADTH = makeKeyword( "BREADTH" );
    $kw15$QUEUE = makeKeyword( "QUEUE" );
    $kw16$STACK = makeKeyword( "STACK" );
    $sym17$RELEVANT_SBHL_TV_IS_GENERAL_TV = makeSymbol( "RELEVANT-SBHL-TV-IS-GENERAL-TV" );
    $kw18$ERROR = makeKeyword( "ERROR" );
    $str19$_A_is_not_a__A = makeString( "~A is not a ~A" );
    $sym20$SBHL_TRUE_TV_P = makeSymbol( "SBHL-TRUE-TV-P" );
    $kw21$CERROR = makeKeyword( "CERROR" );
    $str22$continue_anyway = makeString( "continue anyway" );
    $kw23$WARN = makeKeyword( "WARN" );
    $str24$_A_is_not_a_valid__sbhl_type_erro = makeString( "~A is not a valid *sbhl-type-error-action* value" );
    $const25$genlMt = constant_handles.reader_make_constant_shell( makeString( "genlMt" ) );
    ...
    $str29$____Translation_Time____A__ = makeString( "~%;;Translation Time : ~A~%" );
    $str30$_____ = makeString( ";; ~%" );
    $str31$___Meta_Mt_____________S__ = makeString( ";; Meta Mt          : ~S~%" );
    $str32$___Source______________S__ = makeString( ";; Source           : ~S~%" );
    $str33$___Physical_Schema_____S__ = makeString( ";; Physical Schema  : ~S~%" );
    $str34$___Logical_Schema______S__ = makeString( ";; Logical Schema   : ~S~%" );
    $str35$___Content_Mt__________S = makeString( ";; Content Mt       : ~S" );
    $str36$________S_ = makeString( "~%~%;; ~S " );
    $sym37$SKSI_TRANSLATE_ITERATE_DONE = makeSymbol( "SKSI-TRANSLATE-ITERATE-DONE" );
    $sym38$SKSI_TRANSLATE_ITERATE_NEXT = makeSymbol( "SKSI-TRANSLATE-ITERATE-NEXT" );
    $sym39$BOOLEAN = makeSymbol( "BOOLEAN" );
    $list40 = ConsesLow.list( makeSymbol( "TUPLE-ITERATOR" ), makeSymbol( "&REST" ), makeSymbol( "REST" ) );
    $kw41$DONE = makeKeyword( "DONE" );
    $list42 = ConsesLow.list( makeSymbol( "TUPLE-ITERATOR" ), makeSymbol( "MEMOIZATION-STATE" ), makeSymbol( "PHYSICAL-SCHEMA" ), makeSymbol( "LOGICAL-SCHEMA" ), makeSymbol( "CONTENT-MT" ), makeSymbol( "META-MT" ),
        makeSymbol( "CHECK-WFF-NESS?" ) );
    $const43$ist = constant_handles.reader_make_constant_shell( makeString( "ist" ) );
    $str44$__SKSI__A_ = makeString( "~&SKSI-~A " );
    $str45$__a__Translating__s_wrt__a__a__a_ = makeString( "[~a] Translating ~s wrt ~a ~a ~a into ~a" );
    $str46$Raw_sentence___s = makeString( "Raw sentence: ~s" );
    $str47$Decoded_sentence___s = makeString( "Decoded sentence: ~s" );
    $str48$Logical_sentence___s = makeString( "Logical sentence: ~s" );
    $str49$Logical_conjunct___s = makeString( "Logical conjunct: ~s" );
    $str50$Skipped_due_to_wholly_untranslata = makeString( "Skipped due to wholly untranslatable." );
    $const51$True = constant_handles.reader_make_constant_shell( makeString( "True" ) );
    $str52$Skipped_due_to_tautology_ = makeString( "Skipped due to tautology." );
    $str53$Skipped_due_to_unreformulatable_ = makeString( "Skipped due to unreformulatable." );
    $str54$Translation___s = makeString( "Translation: ~s" );
    $str55$skipped_for_an_unknown_reason___a = makeString( "skipped for an unknown reason: ~a" );
    $str56$The_raw_tuple__S_using_the_physic = makeString( "The raw tuple ~S using the physical schema ~S and the logical schema ~S resulted in the logical sentence ~S in the content mt ~S which was " );
    $sym57$EXPLANATION_OF_WHY_NOT_WFF_ASSERT = makeSymbol( "EXPLANATION-OF-WHY-NOT-WFF-ASSERT" );
    $sym58$EXPLANATION_OF_WHY_NOT_ADMITTED_SENTENCE_WRT_ASENT_ARG_CONSTRAINT = makeSymbol( "EXPLANATION-OF-WHY-NOT-ADMITTED-SENTENCE-WRT-ASENT-ARG-CONSTRAINTS" );
    $str59$skipped_due_to_ill_formedness__ty = makeString( "skipped due to ill-formedness, type coercion successful but ill-formed: ~a" );
    $str60$skipped_due_to_ill_formedness__ty = makeString( "skipped due to ill-formedness, type coercion unsuccessful: ~a" );
    $str61$skipped_due_to_ill_formedness__ty = makeString( "skipped due to ill-formedness (type coercion unnecessary): ~a" );
    $str62$skipped_due_to_ill_formedness___a = makeString( "skipped due to ill-formedness: ~a" );
    $str63$skipped_due_to_an_invalid_predica = makeString( "skipped due to an invalid predicate: ~a" );
    $kw64$IGNORE = makeKeyword( "IGNORE" );
    $kw65$UNTRANSLATABLE = makeKeyword( "UNTRANSLATABLE" );
    $kw66$UNREFORMULATABLE = makeKeyword( "UNREFORMULATABLE" );
    $kw67$NULL = makeKeyword( "NULL" );
    $list68 = ConsesLow.list( ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "IMDB-MovieActors-KS" ) ), ConsesLow.list( makeString( "Unforgiven (1992)" ), makeString( "Eastwood, Clint" ) ),
        ConsesLow.list( makeString( "Unforgiven (1992)" ), makeString( "Hackman, Gene" ) ) ) );
    $kw69$BATCH = makeKeyword( "BATCH" );
    $sym70$SKSI_RAW_TUPLE_TRANSLATION_PATTERN = makeSymbol( "SKSI-RAW-TUPLE-TRANSLATION-PATTERN" );
    $const71$fieldValue = constant_handles.reader_make_constant_shell( makeString( "fieldValue" ) );
    $kw72$VALUE = makeKeyword( "VALUE" );
    $kw73$TUPLE = makeKeyword( "TUPLE" );
    $sym74$SKSI_RAW_SENTENCE_TRANSLATION_PATTERN = makeSymbol( "SKSI-RAW-SENTENCE-TRANSLATION-PATTERN" );
    $const75$IMDB_MovieActors_PS = constant_handles.reader_make_constant_shell( makeString( "IMDB-MovieActors-PS" ) );
    $const76$IMDB_MovieActors_LS = constant_handles.reader_make_constant_shell( makeString( "IMDB-MovieActors-LS" ) );
    $list77 = ConsesLow.list( makeKeyword( "TEMPLATE" ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "and" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString(
        "fieldValue" ) ), makeKeyword( "ANYTHING" ), ConsesLow.list( makeKeyword( "BIND" ), makeSymbol( "RAW-1" ) ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "fieldValue" ) ),
            makeKeyword( "ANYTHING" ), ConsesLow.list( makeKeyword( "BIND" ), makeSymbol( "RAW-2" ) ) ) ), ConsesLow.list( makeKeyword( "CALL" ), makeSymbol( "SKSI-SIMPLIFY-DECODED-SENTENCE" ), ConsesLow.list(
                constant_handles.reader_make_constant_shell( makeString( "and" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) ), ConsesLow.list( constant_handles
                    .reader_make_constant_shell( makeString( "TheFn" ) ), constant_handles.reader_make_constant_shell( makeString( "Movie-CW" ) ) ), ConsesLow.list( makeKeyword( "CALL" ), makeSymbol(
                        "SKSI-BATCH-REFORMULATE" ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "MovieNamedFn" ) ), ConsesLow.list( makeKeyword( "VALUE" ), makeSymbol( "RAW-1" ) ) ) ),
                    makeSymbol( "CONTENT-MT" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString(
                        "TheFn" ) ), constant_handles.reader_make_constant_shell( makeString( "Person" ) ) ), ConsesLow.list( makeKeyword( "CALL" ), makeSymbol( "SKSI-BATCH-REFORMULATE" ), ConsesLow.list(
                            constant_handles.reader_make_constant_shell( makeString( "PersonNamedFn" ) ), ConsesLow.list( makeKeyword( "VALUE" ), makeSymbol( "RAW-2" ) ) ) ), makeSymbol( "CONTENT-MT" ) ) ) ) );
    $list78 = ConsesLow.list( ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "TheFn" ) ), constant_handles.reader_make_constant_shell( makeString( "Movie-CW" ) ) ), ConsesLow.list(
        constant_handles.reader_make_constant_shell( makeString( "TheFn" ) ), constant_handles.reader_make_constant_shell( makeString( "Person" ) ) ) );
    $kw79$ANYTHING = makeKeyword( "ANYTHING" );
    $kw80$BIND = makeKeyword( "BIND" );
    $kw81$CALL = makeKeyword( "CALL" );
    $sym82$POSSIBLY_OVERRIDE_PHYSICAL_FIELD_VALUE = makeSymbol( "POSSIBLY-OVERRIDE-PHYSICAL-FIELD-VALUE" );
    $sym83$PHYSICAL_FIELD_FOR_INDEXICAL = makeSymbol( "PHYSICAL-FIELD-FOR-INDEXICAL" );
    $const84$indexicalReferent = constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) );
    $sym85$SKSI_BATCH_REFORMULATE = makeSymbol( "SKSI-BATCH-REFORMULATE" );
    $kw86$TEMPLATE = makeKeyword( "TEMPLATE" );
    $sym87$SKSI_SIMPLIFY_DECODED_SENTENCE = makeSymbol( "SKSI-SIMPLIFY-DECODED-SENTENCE" );
    $const88$fieldWithPhysicalValueMapsToPhysi = constant_handles.reader_make_constant_shell( makeString( "fieldWithPhysicalValueMapsToPhysicalValue" ) );
    $kw89$UNDECODED = makeKeyword( "UNDECODED" );
    $kw90$DECODE = makeKeyword( "DECODE" );
    $str91$Failed_to_compute_a_justification = makeString( "Failed to compute a justification for ~a in ~a: this assertion will not be retractable" );
    $const92$tablePrimaryKeyValues = constant_handles.reader_make_constant_shell( makeString( "tablePrimaryKeyValues" ) );
    $kw93$OPAQUE = makeKeyword( "OPAQUE" );
    $const94$schemaTranslation = constant_handles.reader_make_constant_shell( makeString( "schemaTranslation" ) );
    $list95 = ConsesLow.list( ConsesLow.list( makeSymbol( "*SKSI-BATCH-TEMPLATES*" ), ConsesLow.list( makeSymbol( "INITIALIZE-SKSI-BATCH-TEMPLATES" ) ) ) );
    $sym96$PROGN = makeSymbol( "PROGN" );
    $list97 = ConsesLow.list( ConsesLow.list( makeSymbol( "CLEAR-SKSI-BATCH-TEMPLATES" ) ) );
    $list98 = ConsesLow.list( makeSymbol( "USE-THEM?" ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym99$PIF = makeSymbol( "PIF" );
    $sym100$WITH_SKSI_BATCH_TEMPLATES = makeSymbol( "WITH-SKSI-BATCH-TEMPLATES" );
    $sym101$INITIALIZE_SKSI_BATCH_TEMPLATES = makeSymbol( "INITIALIZE-SKSI-BATCH-TEMPLATES" );
    $str102$Cannot_handle_nested_SKSI_batch_i = makeString( "Cannot handle nested SKSI batch instantiation templates" );
    $sym103$CLEAR_SKSI_BATCH_TEMPLATES = makeSymbol( "CLEAR-SKSI-BATCH-TEMPLATES" );
    $sym104$ALLOWED_RULES_SPEC_P = makeSymbol( "ALLOWED-RULES-SPEC-P" );
    $sym105$LOGICAL_FIELD_INDEXICAL_P = makeSymbol( "LOGICAL-FIELD-INDEXICAL-P" );
    $sym106$NART_P = makeSymbol( "NART-P" );
    $sym107$SKSI_BATCH_TEMPLATE_P = makeSymbol( "SKSI-BATCH-TEMPLATE-P" );
    $sym108$CONVERT_SQL_DATUM_TO_STRING = makeSymbol( "CONVERT-SQL-DATUM-TO-STRING" );
    $sym109$DOUBLE_FLOAT = makeSymbol( "DOUBLE-FLOAT" );
    $list110 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "implies" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "and" ) ), ConsesLow.list( constant_handles
        .reader_make_constant_shell( makeString( "schemaTranslation" ) ), makeSymbol( "?SOURCE" ), makeSymbol( "?CONTENT-MT" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString(
            "rowInSourceClaims" ) ), makeSymbol( "?SOURCE" ), makeSymbol( "?SENTENCE" ) ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "ist" ) ), makeSymbol( "?CONTENT-MT" ), makeSymbol(
                "?SENTENCE" ) ) );
    $const111$SKSIMt = constant_handles.reader_make_constant_shell( makeString( "SKSIMt" ) );
    $sym112$SKSI_SCHEMA_TRANSLATION_LIFTING_RULE = makeSymbol( "SKSI-SCHEMA-TRANSLATION-LIFTING-RULE" );
    $str113$Could_not_find_the_SKSI_schemaTra = makeString( "Could not find the SKSI schemaTranslation lifting rule ~s in ~s" );
    $sym114$_SKSI_SCHEMA_TRANSLATION_LIFTING_RULE_CACHING_STATE_ = makeSymbol( "*SKSI-SCHEMA-TRANSLATION-LIFTING-RULE-CACHING-STATE*" );
    $sym115$SKSI_GET_DATUM_VAR = makeSymbol( "SKSI-GET-DATUM-VAR" );
    $str116$DATUM_ = makeString( "DATUM-" );
    $sym117$_SKSI_GET_DATUM_VAR_CACHING_STATE_ = makeSymbol( "*SKSI-GET-DATUM-VAR-CACHING-STATE*" );
    $sym118$SKSI_GET_RAW_VAR = makeSymbol( "SKSI-GET-RAW-VAR" );
    $str119$RAW_ = makeString( "RAW-" );
    $sym120$_SKSI_GET_RAW_VAR_CACHING_STATE_ = makeSymbol( "*SKSI-GET-RAW-VAR-CACHING-STATE*" );
    $sym121$SKSI_GET_ARG_VAR = makeSymbol( "SKSI-GET-ARG-VAR" );
    $str122$ARG_ = makeString( "ARG-" );
    $sym123$_SKSI_GET_ARG_VAR_CACHING_STATE_ = makeSymbol( "*SKSI-GET-ARG-VAR-CACHING-STATE*" );
    $str124$Assertion_of__s_in__s_failed___a = makeString( "Assertion of ~s in ~s failed: ~a" );
    $kw125$CHECK_WFF_ = makeKeyword( "CHECK-WFF?" );
    $str126$Deduction_of__s_in__s_with_suppor = makeString( "Deduction of ~s in ~s with supports ~s failed: ~a" );
    $str127$Unassert_of__s_from__s_failed___a = makeString( "Unassert of ~s from ~s failed: ~a" );
    $sym128$_EXIT = makeSymbol( "%EXIT" );
    $sym129$TEST_SKSI_WFF_CHECK_BATCH_TRANSLATED_SENTENCE = makeSymbol( "TEST-SKSI-WFF-CHECK-BATCH-TRANSLATED-SENTENCE" );
  }
}
/*
 * 
