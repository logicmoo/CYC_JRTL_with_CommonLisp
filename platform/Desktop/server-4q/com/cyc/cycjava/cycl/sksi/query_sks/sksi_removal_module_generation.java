...
public final class sksi_removal_module_generation
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject note_multiply_indexed_conditional_meaning_sentence_for_sks(final SubLObject sks, final SubLObject conditional_meaning_sentence_gaf)
  {
    ...
    final SubLObject new_val = set.new_set( Symbols.symbol_function( EQ ), UNPROVIDED );
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        if( items_var.isVector() )
        {
          ...
          for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
          {
            ...
            v_modules = ConsesLow.cons( item, v_modules );
          }
        }
        else
        {
          ...
          {
            v_modules = ConsesLow.cons( item2, v_modules );
  ...
  public static SubLObject do_code_mapping_sks_cache(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym17$DO_DICTIONARY, ConsesLow.listS( sks, tuples, $list18 ), ConsesLow.list( $sym4$CDOLIST, ConsesLow.list( tuple, tuples ), ConsesLow.listS( $sym19$CDESTRUCTURING_BIND, ConsesLow.list( ps,
          ls, mapping_mt ), tuple, ConsesLow.append( body, NIL ) ) ) );
    }
  ...
  {
    return dictionary_utilities.dictionary_push( $sksi_code_mapping_sks_cache$.getGlobalValue(), sks, ConsesLow.list( ps, ls, mapping_mt ) );
  }
  ...
  {
    return dictionary_utilities.dictionary_remove_from_value( $sksi_code_mapping_sks_cache$.getGlobalValue(), sks, ConsesLow.list( ps, ls, mapping_mt ), Symbols.symbol_function( EQUAL ), UNPROVIDED );
  }
  ...
  {
    return kb_mapping.gather_predicate_rule_index( $const21$codeMapping, $kw22$NEG, $const23$SKSIMt, $kw24$FORWARD );
  }
  ...
  public static SubLObject code_mapping_forward_rules_to_repropagate()
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym20$CODE_MAPPING_FORWARD_RULES_TO_REPROPAGATE, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym20$CODE_MAPPING_FORWARD_RULES_TO_REPROPAGATE, ZERO_INTEGER, NIL, EQ, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym20$CODE_MAPPING_FORWARD_RULES_TO_REPROPAGATE, caching_state );
    }
    SubLObject results = memoization_state.caching_state_get_zero_arg_results( caching_state, UNPROVIDED );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( code_mapping_forward_rules_to_repropagate_internal() ) );
          ...
          try
          {
            ...
            try
            {
              ...
              for( SubLObject done_var = NIL; NIL == done_var; done_var = makeBoolean( NIL == valid ) )
              {
                ...
                thread.resetMultipleValues();
                if( NIL != valid && NIL != cycl_utilities.atomic_sentence_with_pred_p( sentence, $const21$codeMapping ) )
                {
                  final SubLObject arg3 = cycl_utilities.atomic_sentence_arg3( sentence, UNPROVIDED );
                  if( NIL == sksi_kb_accessors.schema_object_fn_expression_p( arg3 ) && NIL == sksi_kb_accessors.source_schema_object_fn_expression_p( arg3 ) && NIL != cyc_kernel.cyc_assert_wff( sentence, mt,
                      UNPROVIDED ) )
                  {
  ...
  public static SubLObject isa_removal_module_p(final SubLObject hl_module)
  {
    ...
    {
      return genl_predicates.genl_predP( inference_modules.hl_module_predicate( hl_module ), $const27$isa, UNPROVIDED, UNPROVIDED );
    }
    return NIL;
  }
  ...
  {
    if( NIL == conses_high.member( mt, $isa_sks_term_to_cycl_term_mts$.getGlobalValue(), $sym29$HLMT_EQUAL, Symbols.symbol_function( IDENTITY ) ) )
    {
      $isa_sks_term_to_cycl_term_mts$.setGlobalValue( ConsesLow.cons( mt, $isa_sks_term_to_cycl_term_mts$.getGlobalValue() ) );
    }
    return NIL;
  }
  ...
  public static SubLObject add_isa_sks_term_to_cycl_term_store(final SubLObject v_term, final SubLObject mt)
  {
    ...
    dictionary_utilities.dictionary_pushnew( $isa_sks_term_to_cycl_term_store$.getGlobalValue(), v_term, mt, $sym29$HLMT_EQUAL, UNPROVIDED );
    return NIL;
  }
  ...
  public static SubLObject any_sks_term_spec_to_cycl_term_in_relevant_mtP(final SubLObject cycl_term)
  {
    ...
    for( foundP = NIL, iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( $isa_sks_term_to_cycl_term_store$.getGlobalValue() ) ); NIL == foundP
  ...
  public static SubLObject all_sksi_relevant_rewrite_specs_for_term_int_internal(final SubLObject col, final SubLObject mt)
  {
    ...
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( $isa_sks_term_to_cycl_term_store$.getGlobalValue() ) ); NIL == dictionary_contents
        .do_dictionary_contents_doneP( iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      ...
      {
        result = ConsesLow.cons( sks_term, result );
      }
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
    return result;
  ...
  public static SubLObject all_sksi_relevant_rewrite_specs_for_term_int(final SubLObject col, final SubLObject mt)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym31$ALL_SKSI_RELEVANT_REWRITE_SPECS_FOR_TERM_INT, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym31$ALL_SKSI_RELEVANT_REWRITE_SPECS_FOR_TERM_INT, TWO_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym31$ALL_SKSI_RELEVANT_REWRITE_SPECS_FOR_TERM_INT, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( all_sksi_relevant_rewrite_specs_for_term_int_internal( col, mt ) ) );
  ...
  public static SubLObject all_sksi_relevant_rewrite_isas_int(final SubLObject mt)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym32$ALL_SKSI_RELEVANT_REWRITE_ISAS_INT, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym32$ALL_SKSI_RELEVANT_REWRITE_ISAS_INT, ONE_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym32$ALL_SKSI_RELEVANT_REWRITE_ISAS_INT, caching_state );
    }
    SubLObject results = memoization_state.caching_state_lookup( caching_state, mt, memoization_state.$memoized_item_not_found$.getGlobalValue() );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( all_sksi_relevant_rewrite_isas_int_internal( mt ) ) );
  ...
  {
    if( NIL == conses_high.member( mt, $meaning_sentence_mts$.getGlobalValue(), $sym29$HLMT_EQUAL, Symbols.symbol_function( IDENTITY ) ) )
    {
      $meaning_sentence_mts$.setGlobalValue( ConsesLow.cons( mt, $meaning_sentence_mts$.getGlobalValue() ) );
    }
    return NIL;
  }
  ...
  public static SubLObject add_meaning_sentence_pred_to_store(final SubLObject pred, final SubLObject mt)
  {
    ...
    dictionary_utilities.dictionary_pushnew( $meaning_sentence_pred_store$.getGlobalValue(), pred, mt, $sym29$HLMT_EQUAL, UNPROVIDED );
    return NIL;
  }
  ...
  public static SubLObject any_sksi_meaning_sentence_spec_predP(final SubLObject pred)
  {
    ...
    SubLObject foundP;
    SubLObject iteration_state;
    for( foundP = NIL, iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( $meaning_sentence_pred_store$.getGlobalValue() ) ); NIL == foundP && NIL == dictionary_contents
        .do_dictionary_contents_doneP( iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
  ...
  public static SubLObject all_sksi_relevant_rewrite_spec_preds_for_pred_int_internal(final SubLObject pred, final SubLObject mt)
  {
    ...
    SubLObject result = NIL;
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( $meaning_sentence_pred_store$.getGlobalValue() ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      ...
      final SubLObject mts = thread.secondMultipleValue();
      thread.resetMultipleValues();
      if( NIL != list_utilities.not_eq( spec_pred, pred ) && NIL != mt_relevance_macros.any_relevant_mtP( mts ) && ( NIL != cycl_grammar.hl_variable_p( pred ) || NIL != genl_predicates.genl_predP( spec_pred, pred,
  ...
  public static SubLObject all_sksi_relevant_rewrite_spec_preds_for_pred_int(final SubLObject pred, final SubLObject mt)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym37$ALL_SKSI_RELEVANT_REWRITE_SPEC_PREDS_FOR_PRED_INT, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym37$ALL_SKSI_RELEVANT_REWRITE_SPEC_PREDS_FOR_PRED_INT, TWO_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym37$ALL_SKSI_RELEVANT_REWRITE_SPEC_PREDS_FOR_PRED_INT, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( all_sksi_relevant_rewrite_spec_preds_for_pred_int_internal( pred, mt ) ) );
  ...
  public static SubLObject all_sksi_relevant_rewrite_spec_inverses_for_pred_int_internal(final SubLObject pred, final SubLObject mt)
  {
        ...
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      ...
      {
        result = ConsesLow.cons( spec_inv, result );
      }
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
    return result;
  ...
  public static SubLObject all_sksi_relevant_rewrite_spec_inverses_for_pred_int(final SubLObject pred, final SubLObject mt)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym38$ALL_SKSI_RELEVANT_REWRITE_SPEC_INVERSES_FOR_PRED_INT, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym38$ALL_SKSI_RELEVANT_REWRITE_SPEC_INVERSES_FOR_PRED_INT, TWO_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym38$ALL_SKSI_RELEVANT_REWRITE_SPEC_INVERSES_FOR_PRED_INT, caching_state );
    }
    final SubLObject sxhash = memoization_state.sxhash_calc_2( pred, mt );
    final SubLObject collisions = memoization_state.caching_state_lookup( caching_state, sxhash, UNPROVIDED );
    if( !collisions.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( all_sksi_relevant_rewrite_spec_inverses_for_pred_int_internal( pred, mt ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( pred, mt ) );
    return memoization_state.caching_results( results3 );
  }
  ...
  public static SubLObject register_sksi_removal_modules_for_just_sks(final SubLObject sks, SubLObject mapping_mt, SubLObject reclassifyP, SubLObject top_ks, SubLObject add_to_allowed_ksesP)
  {
    ...
    try
    {
        ...
        try
        {
          ...
          try
          {
            ...
            if( NIL != gaf )
            {
              ...
              thread.resetMultipleValues();
              final SubLObject hl_module_infos_$9 = sk_source_module_infos( sks, content_mt, ConsesLow.list( gaf ), top_ks, mapping_mt );
              ...
              while ( NIL != cdolist_list_var)
              {
                ...
                if( NIL == current )
                {
                  ...
                  final SubLObject supports = module_info_get( hl_module_info_plist, $kw41$MODULE_SUPPORTS, UNPROVIDED );
                  final SubLObject simple_required_pattern = replace_pattern_special_subpatterns_with_forts( module_info_get( hl_module_info_plist, $kw42$SIMPLE_REQUIRED_PATTERN, UNPROVIDED ) );
                  final SubLObject meaning_sentence = module_info_get( hl_module_info_plist, $kw43$MEANING_SENTENCE, UNPROVIDED );
                  final SubLObject multiply_indexed_cms = module_info_get( hl_module_info_plist, $kw44$MULTIPLY_INDEXED_CMS, UNPROVIDED );
      ...
      count = Numbers.add( count, $sks_prune_modules_count$.getDynamicValue( thread ) );
    }
    finally
    {
  ...
  public static SubLObject deregister_sksi_removal_modules_for_just_sks(final SubLObject sks, SubLObject reclassifyP, SubLObject remove_from_allowed_ksesP)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      rm_count = Numbers.add( rm_count, deregister_sksi_removal_module( hl_module, NIL ) );
      cdolist_list_var = cdolist_list_var.rest();
      hl_module = cdolist_list_var.first();
    }
  ...
  public static SubLObject deregister_all_sksi_removal_modules()
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject name = inference_modules.hl_module_name( hl_module );
      inference_modules.undeclare_inference_removal_module( name, NIL );
      rm_count = Numbers.add( rm_count, ONE_INTEGER );
      cdolist_list_var = cdolist_list_var.rest();
      hl_module = cdolist_list_var.first();
    }
    inference_modules.reclassify_removal_modules();
    ...
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( $sksi_code_mapping_sks_cache$.getGlobalValue() ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL == Errors.$ignore_mustsP$.getDynamicValue( thread ) && NIL == subl_promotions.memberP( property, inference_modules.removal_module_plist_indicators(), UNPROVIDED, UNPROVIDED ) && NIL == conses_high.member(
        property, $sksi_specific_module_properties$.getGlobalValue(), UNPROVIDED, UNPROVIDED ) )
    {
  ...
  {
    final SubLObject module_name = module_info_get( module_info, $kw48$MODULE_NAME, UNPROVIDED );
  ...
  {
    SubLObject documentation = module_info_get( module_info, $kw49$DOCUMENTATION, UNPROVIDED );
    final SubLObject sense = module_info_get( module_info, $kw50$SENSE, UNPROVIDED );
    documentation = Strings.string_downcase( documentation, UNPROVIDED, UNPROVIDED );
    documentation = Sequences.cconcatenate( $str51$sksi_, documentation );
  ...
  public static SubLObject sk_source_module_infos(final SubLObject sk_source, final SubLObject content_mt, final SubLObject module_supports, final SubLObject top_ks, final SubLObject mapping_mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      thread.resetMultipleValues();
      final SubLObject module_infos = physical_schema_module_infos( ps, sk_source, content_mt, ConsesLow.cons( gaf, module_supports ), top_ks, mapping_mt );
  ...
  public static SubLObject physical_schema_module_infos(final SubLObject physical_schema, final SubLObject sk_source, final SubLObject content_mt, final SubLObject module_supports, final SubLObject top_ks,
      final SubLObject mapping_mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject module_infos = logical_schema_module_infos( ls, physical_schema, sk_source, content_mt, ConsesLow.cons( gaf, module_supports ), top_ks );
      result = ConsesLow.nconc( result, module_infos );
  ...
  public static SubLObject logical_schema_module_infos(final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject sk_source, final SubLObject content_mt, final SubLObject module_supports,
      final SubLObject top_ks)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject module_infos = meaning_sentence_gaf_module_infos( gaf, logical_schema, physical_schema, sk_source, content_mt, ConsesLow.cons( gaf, module_supports ) );
      result = ConsesLow.nconc( result, module_infos );
      cdolist_list_var = cdolist_list_var.rest();
      gaf = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject module_infos = conditional_meaning_sentence_gaf_module_infos( gaf, logical_schema, physical_schema, sk_source, content_mt, ConsesLow.cons( gaf, module_supports ), top_ks );
      result = ConsesLow.nconc( result, module_infos );
      cdolist_list_var = cdolist_list_var.rest();
      gaf = cdolist_list_var.first();
    }
  ...
  public static SubLObject meaning_sentence_gaf_module_infos(final SubLObject meaning_sentence_gaf, final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject sk_source,
      final SubLObject content_mt, final SubLObject module_supports)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    try
    {
      ...
      try
      {
        Errors.$error_handler$.bind( $sym52$CATCH_ERROR_MESSAGE_HANDLER, thread );
        try
        {
          ...
          {
            module_infos = pos_atomic_meaning_sentence_module_infos( rev_sentence, ConsesLow.list( logical_schema ), ConsesLow.list( physical_schema ), sk_source, content_mt, module_supports, UNPROVIDED, UNPROVIDED,
                UNPROVIDED, UNPROVIDED );
          }
          else if( NIL != el_utilities.el_negation_p( rev_sentence ) && NIL != el_utilities.atomic_sentenceP( cycl_utilities.sentence_arg1( rev_sentence, UNPROVIDED ) ) )
          {
            final SubLObject subsentence = cycl_utilities.sentence_arg1( rev_sentence, UNPROVIDED );
            module_infos = negated_atomic_meaning_sentence_module_infos( subsentence, ConsesLow.list( logical_schema ), ConsesLow.list( physical_schema ), sk_source, content_mt, module_supports );
          }
          else
          {
  ...
  public static SubLObject negated_atomic_meaning_sentence_module_infos(final SubLObject pos_sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject sk_source,
      final SubLObject content_mt, final SubLObject module_supports)
  {
    return NIL;
  }
  ...
  public static SubLObject pos_atomic_meaning_sentence_module_infos(final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject sk_source,
      final SubLObject content_mt, final SubLObject module_supports, SubLObject antecedent_sentence, SubLObject where_expression, SubLObject from_expression, SubLObject eval_conditions)
  {
    ...
    SubLObject module_infos = NIL;
    final SubLObject any_logical_field_indexicals_appearingP = cycl_utilities.expression_find_if( $sym55$LOGICAL_FIELD_INDEXICAL_P, sentence, T, UNPROVIDED );
    ...
    SubLObject cdolist_list_var;
    final SubLObject required_module_infos = cdolist_list_var = pos_atomic_meaning_sentence_required_pattern_infos( sentence, logical_schemata, physical_schemata, sk_source, content_mt, module_supports,
        antecedent_sentence );
  ...
  public static SubLObject conditional_meaning_sentence_gaf_module_infos(final SubLObject conditional_meaning_sentence_gaf, final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject sk_source,
      final SubLObject content_mt, final SubLObject module_supports, final SubLObject top_ks)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject module_infos = NIL;
    SubLObject error_message = NIL;
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          if( NIL != sksi_kb_accessors.sk_source_super_ksP( new_sk_source, top_ks ) && ( NIL != list_utilities.singletonP( consequent_logical_schema_set )
              || NIL == multiply_indexed_conditional_meaning_sentence_noted_for_sksP( new_sk_source, conditional_meaning_sentence_gaf ) ) )
          {
            ...
            final SubLObject all_physical_schemata = conses_high.union( consequent_physical_schemata, antecedent_physical_schemata, UNPROVIDED, UNPROVIDED );
            final SubLObject new_content_mt = sksi_kb_accessors.sk_source_content_mt( new_sk_source );
            if( NIL != el_utilities.atomic_sentenceP( reconstructed_cms ) )
            {
              thread.resetMultipleValues();
              final SubLObject where_expression = sksi_query_utilities.get_where_expression_from_antecedent_meaning_sentence( conditional_meaning_sentence_gaf, new_content_mt, new_sk_source, all_physical_schemata,
                  UNPROVIDED, UNPROVIDED );
              final SubLObject eval_conditions = thread.secondMultipleValue();
              thread.resetMultipleValues();
              final SubLObject from_expression = sksi_query_utilities.get_from_expression_from_antecedent_meaning_sentence( conditional_meaning_sentence_gaf, new_content_mt, new_sk_source, all_physical_schemata,
                  UNPROVIDED );
              final SubLObject cms_module_infos = pos_atomic_meaning_sentence_module_infos( reconstructed_cms, consequent_logical_schema_set, list_utilities.delete_if_not( $sym59$PHYSICAL_SCHEMA_SK_SOURCES_MEMOIZED,
                  Mapping.mapcan( $sym60$LOGICAL_SCHEMA_PHYSICAL_SCHEMATA, consequent_logical_schema_set, EMPTY_SUBL_OBJECT_ARRAY ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ), new_sk_source, content_mt,
                  module_supports, reconstructed_ams, where_expression, from_expression, eval_conditions );
              ...
              while ( NIL != cdolist_list_var)
              {
                ...
                final SubLObject output_generate_pattern = module_info_get( module_plist, $kw61$OUTPUT_GENERATE_PATTERN, NIL );
                final SubLObject output_check_pattern = ( NIL != output_generate_pattern ) ? NIL : module_info_get( module_plist, $kw62$OUTPUT_CHECK_PATTERN, NIL );
                module_plist = module_info_add( module_plist, $kw63$REQUIRED_MT, new_content_mt );
                module_plist = module_info_add( module_plist, $kw64$SUPPORT_MT, new_content_mt );
                if( NIL == list_utilities.singletonP( consequent_logical_schema_set ) )
                {
                  module_plist = module_info_add( module_plist, $kw44$MULTIPLY_INDEXED_CMS, conditional_meaning_sentence_gaf );
                }
                if( NIL != output_generate_pattern )
                {
                  final SubLObject new_output_generate_pattern = condition_output_pattern( output_generate_pattern, from_expression, where_expression, eval_conditions, sk_source, content_mt, new_sk_source,
                      new_content_mt );
                  module_plist = module_info_add( module_plist, $kw61$OUTPUT_GENERATE_PATTERN, new_output_generate_pattern );
                }
                else if( NIL != output_check_pattern )
                {
                  ...
                  module_plist = module_info_add( module_plist, $kw62$OUTPUT_CHECK_PATTERN, new_output_check_pattern );
                }
                else
                {
                  Errors.error( $str65$No__output_generate_pattern_or__o );
                }
                new_module_infos = ConsesLow.cons( ConsesLow.cons( module_plist, module_rest ), new_module_infos );
                cdolist_list_var = cdolist_list_var.rest();
                module_info = cdolist_list_var.first();
              }
              module_infos = new_module_infos;
            }
            else if( NIL != el_utilities.el_negation_p( reconstructed_cms ) && NIL != el_utilities.atomic_sentenceP( cycl_utilities.sentence_arg1( reconstructed_cms, UNPROVIDED ) ) )
            {
              module_infos = NIL;
            }
            else
  ...
  public static SubLObject condition_output_pattern(final SubLObject output_pattern, final SubLObject from_expression, final SubLObject where_expression, final SubLObject eval_conditions, final SubLObject old_sk_source,
      final SubLObject old_content_mt, final SubLObject new_sk_source, final SubLObject new_content_mt)
  {
    if( NIL != sksi_csql_utilities.csql_true_p( where_expression ) )
    {
      return output_pattern;
    }
    final SubLObject old_from = list_utilities.tree_find_if( $sym66$CSQL_FROM_CLAUSE_P, output_pattern, UNPROVIDED );
    final SubLObject old_tables = conses_high.second( old_from );
    final SubLObject new_tables = conses_high.union( old_tables, from_expression, Symbols.symbol_function( EQUAL ), UNPROVIDED );
    final SubLObject old_where = list_utilities.tree_find_if( $sym67$CSQL_WHERE_CLAUSE_P, output_pattern, UNPROVIDED );
    ...
    if( NIL != old_where )
    {
      ...
      final SubLObject new_where_clauses = ConsesLow.cons( where_expression, old_where_clauses );
      new_output_pattern = list_utilities.tree_substitute( new_output_pattern, old_where_clauses, new_where_clauses );
    }
    else
    {
      final SubLObject new_where = ConsesLow.cons( $kw68$WHERE, ConsesLow.list( ConsesLow.list( where_expression ) ) );
      final SubLObject old_select = list_utilities.tree_find_if( $sym69$CSQL_SELECT_QUERY_P, new_output_pattern, UNPROVIDED );
      ...
      if( NIL != sksi_kb_accessors.schema_object_fn_expression_p( field_decoding ) )
      {
        ...
        if( NIL != forts.fort_p( code_mapping_schema ) )
        {
          ...
          while ( NIL != cdolist_list_var_$12)
          {
            ...
            {
              pred_list = ConsesLow.cons( v_term, pred_list );
            }
            cdolist_list_var_$12 = cdolist_list_var_$12.rest();
            v_term = cdolist_list_var_$12.first();
          }
          schema_object_fn_expression_list = ConsesLow.cons( ConsesLow.listS( $const71$SchemaObjectFn, code_mapping_schema, $list72 ), schema_object_fn_expression_list );
          schema_object_fn_expression_list = ConsesLow.cons( ConsesLow.listS( $const73$SourceSchemaObjectFn, code_mapping_schema, $list72 ), schema_object_fn_expression_list );
        }
      }
      cdolist_list_var = cdolist_list_var.rest();
      field_decoding = cdolist_list_var.first();
    }
    return ConsesLow.nconc( kb_utilities.sort_terms( list_utilities.fast_delete_duplicates( pred_list, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ), NIL, T, T, UNPROVIDED, UNPROVIDED, UNPROVIDED ),
        list_utilities.fast_delete_duplicates( schema_object_fn_expression_list, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
  }
  ...
  public static SubLObject sksi_get_any_predicates_pattern(final SubLObject pred, final SubLObject logical_schemata, final SubLObject physical_schemata)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym70$SKSI_GET_ANY_PREDICATES_PATTERN, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym70$SKSI_GET_ANY_PREDICATES_PATTERN, THREE_INTEGER, NIL, EQ, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym70$SKSI_GET_ANY_PREDICATES_PATTERN, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_get_any_predicates_pattern_internal( pred, logical_schemata, physical_schemata ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( pred, logical_schemata, physical_schemata ) );
    return memoization_state.caching_results( results3 );
  }
  ...
  {
    final SubLObject lfi_alias_fn_pos_list = cycl_utilities.arg_positions_if_bfs( $sym74$LFI_ALIAS_FN_NAUT_P, meaning_sentence, UNPROVIDED );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        unbound_pos_count = Numbers.add( unbound_pos_count, ONE_INTEGER );
      }
    ...
    final SubLObject expression_lfi_forts = cycl_utilities.expression_gather( meaning_sentence, $sym75$LOGICAL_FIELD_INDEXICAL_FORT_P, T, Symbols.symbol_function( EQ ), Symbols.symbol_function( IDENTITY ), NIL );
    ...
    if( NIL == bound_non_lfi_aliasP )
    {
      ...
      while ( NIL == bound_non_lfi_aliasP && NIL != csome_list_var)
      {
        ...
        if( lfi_boundedness == $kw76$FULLY_BOUND )
        {
  ...
  {
    if( NIL == el_utilities.atomic_sentenceP( antecedent_meaning_sentence ) )
    {
      SubLObject doneP = NIL;
      final SubLObject args = cycl_utilities.formula_args( antecedent_meaning_sentence, $kw79$IGNORE );
      SubLObject rest;
      SubLObject arg;
    ...
    if( NIL != kb_utilities.kbeq( cycl_utilities.formula_operator( antecedent_meaning_sentence ), $const77$evaluate ) )
    {
      return NIL;
    }
    final SubLObject args2 = cycl_utilities.formula_args( antecedent_meaning_sentence, UNPROVIDED );
    if( NIL != list_utilities.any_in_list( $sym78$NON_LOGICAL_FIELD_INDEXICAL_P, args2, UNPROVIDED ) )
    {
      return T;
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        alias_index_list = ConsesLow.cons( sksi_kb_accessors.lfi_alias_fn_naut_index( arg ), alias_index_list );
      }
      cdolist_list_var = cdolist_list_var.rest();
      arg = cdolist_list_var.first();
    }
  ...
  {
    SubLObject cdolist_list_var = cycl_utilities.arg_positions_bfs( $kw81$NOT_FULLY_BOUND, pattern, UNPROVIDED );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject doomy_assertions = cdolist_list_var_$13 = ( NIL != sksi_kb_accessors.logical_field_indexical_p( lfi ) ) ? kb_mapping_utilities.pred_values( lfi, $const80$sksiModulesDoomedUnlessArgBound,
          ONE_INTEGER, TWO_INTEGER, UNPROVIDED ) : NIL;
  ...
  public static SubLObject pos_atomic_meaning_sentence_required_pattern_infos(SubLObject meaning_sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject sk_source,
      final SubLObject content_mt, final SubLObject module_supports, SubLObject antecedent_sentence)
  {
    if( antecedent_sentence == UNPROVIDED )
    {
      antecedent_sentence = NIL;
    ...
    if( NIL != lfi_predP )
    {
      ...
      {
        logical_field_indexical_termnums = Sequences.delete( $list83, logical_field_indexical_termnums, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
      }
      else
      {
    ...
    SubLObject cdolist_list_var;
    final SubLObject permutations = cdolist_list_var = compute_meaning_sentence_required_patterns( meaning_sentence, logical_field_indexical_termnums, $kw84$POS );
      ...
      if( NIL == current )
      {
        ...
        if( NIL == valid_required_patternP )
        {
          ...
          else
          {
            ...
            sksi_preference_module_generation.register_sksi_preference_module_for_sks( sk_source, tuple, $kw84$POS, ( NIL != lfi_predP ) ? constrained_pred : pred, pred_list, content_mt, $kw87$DISALLOWED );
            $sks_preference_modules_count$.setDynamicValue( Numbers.add( $sks_preference_modules_count$.getDynamicValue( thread ), ONE_INTEGER ), thread );
          }
        }
        if( NIL != valid_required_patternP )
        {
          SubLObject module_info = new_module_info();
          module_info = module_info_add( module_info, $kw88$INPUT_TERMNUMS, input_termnums );
          module_info = module_info_add( module_info, $kw89$OUTPUT_TERMNUMS, output_termnums );
          module_info = module_info_add( module_info, $kw41$MODULE_SUPPORTS, module_supports );
          module_info = module_info_add( module_info, $kw50$SENSE, $kw84$POS );
          if( NIL != lfi_predP )
          {
            if( NIL != constrained_pred )
            {
              module_info = module_info_add( module_info, $kw90$PREDICATE, constrained_pred );
            }
            else if( NIL != pred_list && NIL == sksi_not_fully_bound_required_pattern_predicateP( tuple ) )
            {
              module_info = module_info_add( module_info, $kw91$ANY_PREDICATES, pred_list );
            }
          }
          else
          {
            module_info = module_info_add( module_info, $kw90$PREDICATE, pred );
          }
          final SubLObject documentation = sksi_make_documentation_string_from_required_pattern_and_sense( tuple, $kw84$POS );
          ...
          module_info = module_info_add( module_info, $kw42$SIMPLE_REQUIRED_PATTERN, tuple );
          module_info = module_info_add( module_info, $kw43$MEANING_SENTENCE, meaning_sentence );
          module_info = module_info_add( module_info, $kw92$REQUIRED_PATTERN, required_pattern );
          module_info = module_info_add( module_info, $kw49$DOCUMENTATION, documentation );
          module_info = module_info_add( module_info, $kw63$REQUIRED_MT, content_mt );
          if( NIL != enumerable_schemaP )
          {
            ...
            {
              module_info = module_info_push( module_info, $kw41$MODULE_SUPPORTS, sksi_kb_accessors.enumerable_schema_gaf( physical_schema ) );
              cdolist_list_var_$16 = cdolist_list_var_$16.rest();
              physical_schema = cdolist_list_var_$16.first();
            }
          }
          module_info = module_info_add( module_info, $kw48$MODULE_NAME, determine_module_info_name( module_info ) );
          module_infos = ConsesLow.cons( module_info, module_infos );
        }
      }
      else
  ...
  {
    return makeBoolean( NIL == list_utilities.tree_find( $kw76$FULLY_BOUND, required_pattern_terms, UNPROVIDED, UNPROVIDED ) );
  }
  ...
  {
    return Equality.eq( required_pattern_terms.first(), $kw81$NOT_FULLY_BOUND );
  }
  ...
  {
    return list_utilities.equal_duplicatesP( list, $sym93$FIRST );
  }
  ...
  public static SubLObject compute_boundedness_permutations_for_logical_field_indexical_termnums(final SubLObject logical_field_indexical_termnums, final SubLObject sense)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      result = ConsesLow.cons( list_utilities.cartesian_product( ConsesLow.list( ConsesLow.list( termnums_list ), ( sense == $kw84$POS ) ? $list94 : $list95 ), UNPROVIDED, UNPROVIDED, UNPROVIDED ), result );
      cdolist_list_var = cdolist_list_var.rest();
      termnums_list = cdolist_list_var.first();
    }
    return list_utilities.cartesian_product( Sequences.nreverse( result ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  {
    return makeBoolean( NIL == subl_promotions.memberP( binding_1.first(), binding_2, Symbols.symbol_function( EQUAL ), $sym96$CAR ) );
  }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$17)
      {
        ...
        if( NIL == current )
        {
          ...
          if( pcase_var.eql( $kw76$FULLY_BOUND ) )
          {
            input_termnums = ConsesLow.append( input_termnums, arg_pos_list );
          }
          else if( pcase_var.eql( $kw81$NOT_FULLY_BOUND ) )
          {
      ...
      result = ConsesLow.cons( ConsesLow.list( pattern, input_termnums, output_termnums ), result );
      cdolist_list_var = cdolist_list_var.rest();
      permutation = cdolist_list_var.first();
    }
    return result;
  ...
  {
    final SubLObject new_subpattern = list_utilities.tree_transform_if( cycl_utilities.formula_args( pattern, UNPROVIDED ), $sym98$NART_P, $sym99$MAKE_NAT_SUBPATTERN, UNPROVIDED );
    return el_utilities.make_el_formula( cycl_utilities.formula_operator( pattern ), new_subpattern, UNPROVIDED );
  }
  ...
  {
    return ConsesLow.list( $kw100$NAT, narts_high.nart_el_formula( nart ) );
  }
  ...
  {
    return makeBoolean( pattern.isCons() && pattern.first() == $kw100$NAT );
  }
  ...
  {
    return makeBoolean( pattern.isCons() && pattern.first() == $kw101$OR );
  }
  ...
  {
    return ConsesLow.listS( $kw101$OR, nat_pattern, $list102 );
  }
  ...
  {
    return ConsesLow.list( $kw103$UNIFY, fort );
  }
  ...
  {
    return makeBoolean( pattern.isCons() && pattern.first() == $kw103$UNIFY );
  }
  ...
  {
    final SubLObject new_subpattern = list_utilities.tree_transform_if( cycl_utilities.formula_args( pattern, UNPROVIDED ), $sym104$NAUT_, $sym105$MAKE_UNIFY_SUBPATTERN, UNPROVIDED );
    return el_utilities.make_el_formula( cycl_utilities.formula_operator( pattern ), new_subpattern, UNPROVIDED );
  }
  ...
  {
    SubLObject new_subpattern = list_utilities.tree_transform_if( cycl_utilities.formula_args( pattern, UNPROVIDED ), $sym106$NON_SENTENTIAL_RELATION_FORT_P, $sym105$MAKE_UNIFY_SUBPATTERN, UNPROVIDED );
    new_subpattern = list_utilities.tree_transform_if( new_subpattern, $sym107$NAT_SUBPATTERN_P, $sym108$MAKE_DISJUNCTIVE_SUBPATTERN, UNPROVIDED );
    return el_utilities.make_el_formula( cycl_utilities.formula_operator( pattern ), new_subpattern, UNPROVIDED );
  }
  ...
  {
    return makeBoolean( NIL != forts.fort_p( v_object ) && NIL == isa.isaP( v_object, $const109$SententialRelation, UNPROVIDED, UNPROVIDED ) );
  }
  ...
  public static SubLObject non_sentential_relation_fort_p(final SubLObject v_object)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym106$NON_SENTENTIAL_RELATION_FORT_P, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym106$NON_SENTENTIAL_RELATION_FORT_P, ONE_INTEGER, NIL, EQL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym106$NON_SENTENTIAL_RELATION_FORT_P, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( non_sentential_relation_fort_p_internal( v_object ) ) );
  ...
  {
    SubLObject new_subpattern = list_utilities.tree_transform_if( cycl_utilities.formula_args( pattern, UNPROVIDED ), $sym110$UNIFY_SUBPATTERN_P, $sym111$SECOND, UNPROVIDED );
    new_subpattern = list_utilities.tree_transform_if( new_subpattern, $sym112$DISJUNCTIVE_SUBPATTERN_P, $sym111$SECOND, UNPROVIDED );
    new_subpattern = list_utilities.tree_transform_if( new_subpattern, $sym107$NAT_SUBPATTERN_P, $sym113$NARTIFY_NAT_PATTERN, UNPROVIDED );
    return el_utilities.make_el_formula( cycl_utilities.formula_operator( pattern ), new_subpattern, UNPROVIDED );
  }
  ...
  {
    final SubLObject new_subpattern = conses_high.subst_if( $kw114$ANYTHING, $sym110$UNIFY_SUBPATTERN_P, conses_high.subst_if( $kw114$ANYTHING, $sym107$NAT_SUBPATTERN_P, conses_high.subst_if( $kw114$ANYTHING,
        $sym112$DISJUNCTIVE_SUBPATTERN_P, cycl_utilities.formula_args( pattern, UNPROVIDED ), UNPROVIDED ), UNPROVIDED ), UNPROVIDED );
    return el_utilities.make_el_formula( cycl_utilities.formula_operator( pattern ), new_subpattern, UNPROVIDED );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/query-sks/sksi-removal-module-generation.lisp", position = 76254L)
  public static SubLObject add_cmls_pruning_to_required_pattern(final SubLObject required_pattern, final SubLObject sentence, final SubLObject logical_field_indexical_termnums, final SubLObject logical_schemata,
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $generate_sksi_slrm_required_function_caching_state$.getGlobalValue(), ConsesLow.list( sk_source ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject generate_sksi_slrm_required_function_internal(final SubLObject sk_source)
  {
    ...
    return Eval.eval( ConsesLow.list( $sym116$DEFINE_PRIVATE, required_func_name, $list117, $list118, ConsesLow.list( $sym119$RET, ConsesLow.list( $sym120$EXTERNAL_SOURCE_ACTIVATED_IN_IMAGE_, sk_source ) ) ) );
  }
  ...
  public static SubLObject generate_sksi_slrm_required_function(final SubLObject sk_source)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym115$GENERATE_SKSI_SLRM_REQUIRED_FUNCTION, $sym121$_GENERATE_SKSI_SLRM_REQUIRED_FUNCTION_CACHING_STATE_, NIL, EQ, ONE_INTEGER,
          ZERO_INTEGER );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( generate_sksi_slrm_required_function_internal( sk_source ) ) );
  ...
  {
    return Symbols.make_symbol( Sequences.cconcatenate( Strings.string_upcase( kb_utilities.fort_alphanumeric_name( sk_source ), UNPROVIDED, UNPROVIDED ), $str122$_REQUIRED_FUNC ) );
  }
  ...
  {
    return isa.isa_in_any_mtP( pred, $const124$SKSIOnlyPredicate );
  }
  ...
  public static SubLObject sksi_only_predicateP(final SubLObject pred)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym123$SKSI_ONLY_PREDICATE_, ONE_INTEGER, $int125$1024, EQUAL, UNPROVIDED );
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_only_predicateP_internal( pred ) ) );
  ...
  public static SubLObject required_info_module_info(SubLObject module_info, final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject sk_source,
      final SubLObject content_mt, final SubLObject where_expression, final SubLObject from_expression, final SubLObject eval_conditions)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject required_pattern = module_info_get( module_info, $kw42$SIMPLE_REQUIRED_PATTERN, UNPROVIDED );
    final SubLObject pred = module_info_get( module_info, $kw90$PREDICATE, UNPROVIDED );
    final SubLObject pred_list = module_info_get( module_info, $kw91$ANY_PREDICATES, UNPROVIDED );
    final SubLObject sense = module_info_get( module_info, $kw50$SENSE, UNPROVIDED );
    ...
    final SubLObject module_supports = module_info_get( module_info, $kw41$MODULE_SUPPORTS, UNPROVIDED );
    final SubLObject checkP = required_pattern_is_checkP( required_pattern );
    final SubLObject input_termnums = module_info_get( module_info, $kw88$INPUT_TERMNUMS, UNPROVIDED );
    final SubLObject output_termnums = module_info_get( module_info, $kw89$OUTPUT_TERMNUMS, UNPROVIDED );
    final SubLObject cost = ( NIL != checkP ) ? $sksi_default_check_cost$.getGlobalValue()
        : sksi_removal_module_cost( required_pattern, sentence, logical_schemata, physical_schemata, input_termnums, output_termnums, sk_source, ( where_expression == T ) ? NIL : where_expression, from_expression );
    ...
    try
    {
      ...
      final SubLObject support_module = $kw126$SKSI;
      final SubLObject support_strength = $kw127$DEFAULT;
      thread.resetMultipleValues();
      ...
      if( NIL != checkP )
      {
        output_check_pattern = sksi_determine_output_check_pattern( relevant_physical_field_indexicals, sentence, logical_schemata, physical_schemata, sk_source, input_termnums, output_termnums, sense, content_mt,
            eval_conditions );
      }
      else
      {
        thread.resetMultipleValues();
        final SubLObject output_generate_pattern_$19 = sksi_determine_output_generate_pattern( required_pattern, relevant_physical_field_indexicals, sentence, logical_schemata, physical_schemata, sk_source,
            input_termnums, output_termnums, content_mt, eval_conditions );
        ...
        {
          sksi_preference_module_generation.register_sksi_preference_module_for_sks( sk_source, required_pattern, $kw84$POS, pred, pred_list, content_mt, $kw128$DISPREFERRED );
          $sks_preference_modules_count$.setDynamicValue( Numbers.add( $sks_preference_modules_count$.getDynamicValue( thread ), ONE_INTEGER ), thread );
        }
      }
      if( NIL != prune_patterns && NIL != sksi_only_predicateP( pred ) )
      {
        final SubLObject supplanted_module_name = module_info_get( module_info, $kw48$MODULE_NAME, UNPROVIDED );
        final SubLObject prune_module_name = string_utilities.keyword_from_string( Strings.string_upcase( Sequences.cconcatenate( string_utilities.string_from_keyword( supplanted_module_name ),
            $str129$_prune_unreformulatable ), UNPROVIDED, UNPROVIDED ) );
        ...
        while ( NIL != cdolist_list_var)
        {
          prune_required_pattern = ConsesLow.cons( prune_pattern, prune_required_pattern );
          cdolist_list_var = cdolist_list_var.rest();
          prune_pattern = cdolist_list_var.first();
        }
        prune_required_pattern = ConsesLow.cons( $kw101$OR, prune_required_pattern );
        prune_required_pattern = ConsesLow.list( prune_required_pattern );
        prune_required_pattern = ConsesLow.cons( required_pattern, prune_required_pattern );
        prune_required_pattern = ConsesLow.cons( $kw130$AND, prune_required_pattern );
        SubLObject hl_module_plist = ConsesLow.listS( $kw131$MODULE_TYPE, new SubLObject[] { $kw132$REMOVAL, $kw133$MODULE_SUBTYPE, $list134, $kw135$MODULE_SOURCE, sk_source, $kw136$CHECK, NIL, $kw50$SENSE, sense,
          $kw137$DIRECTION, direction, $kw92$REQUIRED_PATTERN, prune_required_pattern, $kw63$REQUIRED_MT, content_mt, $kw138$EXCLUSIVE, $sym139$TRUE, $kw140$SUPPLANTS, ConsesLow.list( $list141, supplanted_module_name ),
          $kw142$COMPLETENESS, completeness, $list143
        } );
        SubLObject hl_module = NIL;
        if( NIL != pred )
        {
          hl_module_plist = conses_high.putf( hl_module_plist, $kw90$PREDICATE, pred );
        }
        if( NIL != pred_list )
        {
          hl_module_plist = conses_high.putf( hl_module_plist, $kw91$ANY_PREDICATES, pred_list );
        }
        hl_module = inference_modules.inference_removal_module( prune_module_name, hl_module_plist );
        $sks_prune_modules_count$.setDynamicValue( Numbers.add( $sks_prune_modules_count$.getDynamicValue( thread ), ONE_INTEGER ), thread );
        note_sksi_module( hl_module );
      ...
      module_info = module_info_add( module_info, $kw133$MODULE_SUBTYPE, $kw126$SKSI );
      module_info = module_info_add( module_info, $kw135$MODULE_SOURCE, sk_source );
      module_info = module_info_add( module_info, $kw136$CHECK, checkP );
      module_info = module_info_add( module_info, $kw144$REQUIRED, required_func_name );
      module_info = module_info_add( module_info, $kw137$DIRECTION, direction );
      module_info = module_info_add( module_info, $kw142$COMPLETENESS, completeness );
      if( NIL != supplants_allP )
      {
        module_info = module_info_add( module_info, $kw138$EXCLUSIVE, $sym139$TRUE );
        module_info = module_info_add( module_info, $kw140$SUPPLANTS, $list145 );
      }
      if( NIL != input_extract_pattern )
      {
        module_info = module_info_add( module_info, $kw146$INPUT_EXTRACT_PATTERN, input_extract_pattern );
      }
      if( NIL != input_encode_pattern )
      {
        module_info = module_info_add( module_info, $kw147$INPUT_ENCODE_PATTERN, input_encode_pattern );
      }
      if( NIL != input_verify_pattern )
      {
        module_info = module_info_add( module_info, $kw148$INPUT_VERIFY_PATTERN, input_verify_pattern );
      }
      if( NIL != output_generate_pattern )
      {
        module_info = module_info_add( module_info, $kw61$OUTPUT_GENERATE_PATTERN, output_generate_pattern );
      }
      if( NIL != output_check_pattern )
      {
        module_info = module_info_add( module_info, $kw62$OUTPUT_CHECK_PATTERN, output_check_pattern );
      }
      if( NIL != output_decode_pattern )
      {
        module_info = module_info_add( module_info, $kw149$OUTPUT_DECODE_PATTERN, output_decode_pattern );
      }
      if( NIL != output_verify_pattern )
      {
        module_info = module_info_add( module_info, $kw150$OUTPUT_VERIFY_PATTERN, output_verify_pattern );
      }
      if( NIL != output_construct_pattern )
      {
        module_info = module_info_add( module_info, $kw151$OUTPUT_CONSTRUCT_PATTERN, output_construct_pattern );
      }
      module_info = module_info_add( module_info, $kw152$COST_EXPRESSION, cost );
      module_info = module_info_add( module_info, $kw153$COST_PATTERN, cost_pattern );
      if( NIL != support_module )
      {
        module_info = module_info_add( module_info, $kw154$SUPPORT_MODULE, support_module );
      }
      if( NIL != content_mt )
      {
        module_info = module_info_add( module_info, $kw64$SUPPORT_MT, content_mt );
      }
      if( NIL != support_strength )
      {
        module_info = module_info_add( module_info, $kw155$SUPPORT_STRENGTH, support_strength );
      }
      ...
      if( NIL != supports )
      {
        module_info = module_info_add( module_info, $kw41$MODULE_SUPPORTS, supports );
      }
    }
    finally
    {
    ...
    return ( NIL != errorP ) ? NIL : ConsesLow.list( module_info, referenced_code_mapping_schemas, referenced_instance_named_fn_colls );
  }
  ...
  public static SubLObject sksi_removal_module_completeness(final SubLObject sentence, final SubLObject logical_schemata)
  {
    ...
    return ( NIL != completeP ) ? $kw156$COMPLETE : $kw157$INCOMPLETE;
  }
  ...
  public static SubLObject sksi_make_documentation_string_from_required_pattern_and_sense(final SubLObject required_pattern, final SubLObject sense)
  {
    ...
    if( sense == $kw22$NEG )
    {
      string = Sequences.cconcatenate( $str158$___not_, new SubLObject[] { string, $str159$_
      } );
    }
  ...
  public static SubLObject sksi_determine_input_extract_pattern(final SubLObject required_pattern, final SubLObject sentence, final SubLObject input_termnums, final SubLObject eval_conditions)
  {
    ...
    template_sentence = cycl_utilities.expression_subst( $kw114$ANYTHING, $kw81$NOT_FULLY_BOUND, template_sentence, UNPROVIDED, UNPROVIDED );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject new_term = ConsesLow.list( $kw166$BIND, symbol );
      final SubLObject new_pair = ConsesLow.list( $kw167$VALUE, symbol );
      thread.resetMultipleValues();
      ...
      if( NIL != foundP )
      {
        ...
        transform_tuple = ConsesLow.cons( new_pair, transform_tuple );
      }
      cdolist_list_var = cdolist_list_var.rest();
      input_termnum = cdolist_list_var.first();
    }
    transform_tuple = Sequences.nreverse( transform_tuple );
    final SubLObject input_extract_pattern = ConsesLow.list( $kw168$TEMPLATE, template_sentence, transform_tuple );
  ...
  public static SubLObject sksi_determine_input_encode_pattern(final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject sk_source,
      final SubLObject input_termnums, final SubLObject eval_conditions)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    final SubLObject tuple_variables = Mapping.mapcar( $sym171$TOKEN_CYCL_SYMBOL_FOR_TERMNUM, input_termnums );
    SubLObject subpatterns = NIL;
    SubLObject resulting_physical_field_indexicals = NIL;
    final SubLObject prune_pattern_template = ConsesLow.make_list( Sequences.length( sentence ), $kw114$ANYTHING );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != subpattern )
      {
        ...
        resulting_physical_field_indexicals = ConsesLow.nconc( resulting_physical_field_indexicals, ConsesLow.make_list( Sequences.length( subpattern ), pfi ) );
      }
      ...
      while ( NIL != cdolist_list_var_$25)
      {
        ...
        prune_patterns = ConsesLow.cons( prune_pattern, prune_patterns );
    ...
    final SubLObject input_encode_pattern = ConsesLow.list( $kw172$TUPLE, tuple_variables, subpatterns );
  ...
  {
    return template.isCons() ? makeBoolean( NIL == sksi_query_utilities.sksi_unreformulatable_p( sksi_reformulate.sksi_reformulate( list_utilities.tree_substitute( template, $kw167$VALUE, expression ), direction,
        sks ) ) ) : makeBoolean( NIL == sksi_query_utilities.sksi_unreformulatable_p( sksi_reformulate.sksi_reformulate( expression, direction, sks ) ) );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/query-sks/sksi-removal-module-generation.lisp", position = 93698L)
  public static SubLObject sksi_determine_field_input_encode_pattern(final SubLObject sentence, final SubLObject physical_field_indexical, final SubLObject physical_schemata, final SubLObject logical_field_indexicals,
      final SubLObject logical_schemata, final SubLObject sk_source)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        if( ( NIL != forts.fort_p( physical_field_indexical ) && NIL != forts.fort_p( lfi ) ) || ( NIL != sksi_kb_accessors.pfi_alias_fn_naut_p( physical_field_indexical ) && NIL != sksi_kb_accessors.lfi_alias_fn_naut_p(
            lfi ) && NIL != kb_utilities.kbeq( sksi_kb_accessors.pfi_alias_fn_naut_index( physical_field_indexical ), sksi_kb_accessors.lfi_alias_fn_naut_index( lfi ) ) ) )
        {
          ...
          {
            final SubLObject replacement = ConsesLow.list( $kw167$VALUE, cycl_token_for_logical_field_indexical( lfi, sentence ) );
            ...
            prune_result = ConsesLow.list( lfi_positions, ConsesLow.list( $kw175$NOT, ConsesLow.list( $kw176$TEST, $sym173$SKSI_REFORMULATABLE_VIA_TEMPLATE_, cycl_utilities.expression_nsubst( $kw167$VALUE, lfi_fort,
                prune_result, UNPROVIDED, UNPROVIDED ), $kw177$ENCODE, sk_source ) ) );
            foundP = T;
          }
        }
      ...
      if( NIL != foundP )
      {
        patterns = ConsesLow.cons( ConsesLow.list( $kw178$CALL, $sym179$SKSI_REFORMULATE, result, $kw177$ENCODE, pfi_source ), patterns );
        prune_patterns = ConsesLow.cons( prune_result, prune_patterns );
      }
      cdolist_list_var = cdolist_list_var.rest();
      field_encoding = cdolist_list_var.first();
    }
  ...
  public static SubLObject sksi_determine_output_decode_pattern(final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject output_termnums,
      final SubLObject sk_source, final SubLObject input_termnums)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != subpattern )
      {
        subpatterns = ConsesLow.cons( subpattern, subpatterns );
      }
      cdolist_list_var = cdolist_list_var.rest();
      lfi = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject logical_field_indexical_arg_pos = el_utilities.formula_position( lfi, sentence, Symbols.symbol_function( EQUAL ) );
      if( logical_field_indexical_arg_pos.isInteger() )
      {
        ...
        {
          referenced_code_mapping_schemas = ConsesLow.nconc( referenced_code_mapping_schemas, ConsesLow.list( ConsesLow.list( logical_field_indexical_arg_pos, code_mapping_schemas2 ) ) );
        }
        if( NIL != instance_named_fn_colls2 )
        {
          referenced_instance_named_fn_colls = ConsesLow.nconc( referenced_instance_named_fn_colls, ConsesLow.list( ConsesLow.list( logical_field_indexical_arg_pos, instance_named_fn_colls2 ) ) );
        }
      }
      cdolist_list_var = cdolist_list_var.rest();
      lfi = cdolist_list_var.first();
    ...
    final SubLObject output_decode_pattern = ConsesLow.list( $kw172$TUPLE, tuple_variables, subpatterns );
  ...
  public static SubLObject sksi_determine_field_output_decode_pattern(final SubLObject logical_field_indexical, final SubLObject physical_schemata, final SubLObject physical_field_indexicals,
      final SubLObject logical_schemata, final SubLObject sk_source, final SubLObject gather_code_mapping_schemas_onlyP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
  ...
  public static SubLObject sksi_determine_field_output_decode_pattern_int(final SubLObject field_decoding, final SubLObject physical_field_indexicals, final SubLObject referenced_physical_field_indexicals,
      final SubLObject lfi_source)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject pfi_index = number_utilities.f_1X( Sequences.position( pfi, physical_field_indexicals, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
      final SubLObject raw_symbol = token_raw_symbol_for_termnum( pfi_index );
      final SubLObject replacement = ConsesLow.list( $kw167$VALUE, raw_symbol );
      result = cycl_utilities.expression_nsubst( replacement, sksi_kb_accessors.pfi_fort_for_pfi( pfi ), result, UNPROVIDED, UNPROVIDED );
      cdolist_list_var = cdolist_list_var.rest();
      pfi = cdolist_list_var.first();
    }
    return ConsesLow.list( $kw178$CALL, $sym179$SKSI_REFORMULATE, result, $kw181$DECODE, lfi_source );
  }
  ...
  public static SubLObject gather_external_term_denoting_expressions_of_type_in_field_decoding(final SubLObject field_decoding, final SubLObject pred)
  {
    ...
    try
    {
      cycl_utilities.$opaque_arg_function$.bind( $sym182$DUMMY_EXTERNAL_TERM_DENOTING_FUNCTION_EXPRESSION_P, thread );
      v_answer = cycl_utilities.expression_gather( field_decoding, pred, T, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED );
    }
    finally
    {
  ...
  public static SubLObject field_decoding_contains_city_named_fn_expressionP(final SubLObject field_decoding)
  {
    ...
    {
      cycl_utilities.$opaque_arg_function$.bind( $sym182$DUMMY_EXTERNAL_TERM_DENOTING_FUNCTION_EXPRESSION_P, thread );
      v_answer = cycl_utilities.expression_find_if( $sym183$CITY_NAMED_FN_EXPRESSION_P, field_decoding, T, UNPROVIDED );
    }
    finally
    {
      cycl_utilities.$opaque_arg_function$.rebind( _prev_bind_0, thread );
  ...
  {
    final SubLObject schema_object_fn_terms = gather_external_term_denoting_expressions_of_type_in_field_decoding( field_decoding, $sym184$SCHEMA_OBJECT_FN_EXPRESSION_P );
    final SubLObject source_schema_object_fn_terms = gather_external_term_denoting_expressions_of_type_in_field_decoding( field_decoding, $sym185$SOURCE_SCHEMA_OBJECT_FN_EXPRESSION_P );
    final SubLObject instance_named_fn_terms = gather_external_term_denoting_expressions_of_type_in_field_decoding( field_decoding, $sym186$INSTANCE_NAMED_FN_EXPRESSION_P );
    ...
    while ( NIL != cdolist_list_var)
    {
      code_mapping_schemas = ConsesLow.cons( cycl_utilities.nat_arg1( v_term, UNPROVIDED ), code_mapping_schemas );
      cdolist_list_var = cdolist_list_var.rest();
      v_term = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      code_mapping_schemas = ConsesLow.cons( cycl_utilities.nat_arg2( v_term, UNPROVIDED ), code_mapping_schemas );
      cdolist_list_var = cdolist_list_var.rest();
      v_term = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      instance_named_fn_colls = ConsesLow.cons( cycl_utilities.nat_arg2( v_term, UNPROVIDED ), instance_named_fn_colls );
      cdolist_list_var = cdolist_list_var.rest();
      v_term = cdolist_list_var.first();
    }
    if( NIL != field_decoding_contains_city_named_fn_expressionP( field_decoding ) )
    {
      ...
      field_isas = ConsesLow.cons( $const187$City, field_isas );
      instance_named_fn_colls = ConsesLow.nconc( instance_named_fn_colls, genls.min_cols( field_isas, UNPROVIDED, UNPROVIDED ) );
    }
  ...
  public static SubLObject sksi_determine_output_construct_pattern(final SubLObject sentence, final SubLObject required_pattern, final SubLObject input_termnums, final SubLObject output_termnums)
  {
    ...
    return ConsesLow.list( $kw172$TUPLE, tuple_output, template );
  }
  ...
  public static SubLObject determine_output_construct_pattern_template_and_tuple_output(final SubLObject pattern, final SubLObject input_termnums, final SubLObject output_termnums)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      template = list_utilities.replace_nested_nth( template, input_termnum, ConsesLow.list( $kw167$VALUE, token_term ) );
      cdolist_list_var = cdolist_list_var.rest();
      input_termnum = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      template = list_utilities.replace_nested_nth( template, output_termnum, ConsesLow.list( $kw167$VALUE, token_term ) );
      tuple_output = ConsesLow.cons( token_term, tuple_output );
      cdolist_list_var = cdolist_list_var.rest();
      output_termnum = cdolist_list_var.first();
    }
  ...
  public static SubLObject sksi_determine_cost_pattern(final SubLObject sense, final SubLObject sk_source, SubLObject cost)
  {
    ...
    return ( NIL != cost ) ? ConsesLow.list( $kw178$CALL, $sym188$GET_SKSI_REMOVAL_MODULE_COST, $kw189$INPUT, sense, sk_source, cost )
        : ConsesLow.list( $kw178$CALL, $sym188$GET_SKSI_REMOVAL_MODULE_COST, $kw189$INPUT, sense, sk_source );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/query-sks/sksi-removal-module-generation.lisp", position = 106396L)
  public static SubLObject sksi_removal_module_cost(final SubLObject required_pattern, final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject input_termnums,
      final SubLObject output_termnums, SubLObject sk_source, SubLObject where_expression, SubLObject from_expression)
  {
    ...
    if( NIL != cost )
    {
      return Numbers.max( $sksi_default_check_cost$.getGlobalValue(), cost );
    }
  ...
  public static SubLObject sksi_removal_module_logical_cost(final SubLObject sentence, final SubLObject logical_schemata, final SubLObject input_termnums, final SubLObject output_termnums, SubLObject sk_source)
  {
    ...
    {
      final SubLObject not_fully_bound_logical_field_indexicals = list_utilities.fast_delete_duplicates( sksi_referenced_logical_field_indexicals( sentence, output_termnums ), UNPROVIDED, UNPROVIDED, UNPROVIDED,
          UNPROVIDED, UNPROVIDED );
      final SubLObject fully_bound_logical_field_indexicals = list_utilities.fast_delete_duplicates( sksi_referenced_logical_field_indexicals( sentence, input_termnums ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
          UNPROVIDED );
      final SubLObject not_fully_bound_logical_fields_list = Mapping.mapcar( Symbols.symbol_function( $sym190$LOGICAL_FIELD_FOR_INDEXICAL ), not_fully_bound_logical_field_indexicals );
      final SubLObject fully_bound_logical_fields_list = Mapping.mapcar( Symbols.symbol_function( $sym190$LOGICAL_FIELD_FOR_INDEXICAL ), fully_bound_logical_field_indexicals );
  ...
  public static SubLObject sksi_removal_module_physical_cost(final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata, final SubLObject input_termnums,
      final SubLObject output_termnums, SubLObject sk_source, SubLObject where_expression, SubLObject from_expression)
  {
    if( sk_source == UNPROVIDED )
    {
    ...
    final SubLObject rs_cardinality_gafs = ( NIL != where_expression ) ? sksi_kb_accessors.physical_schemata_result_set_cardinality_wXconstraints_gafs( physical_schemata )
        : sksi_kb_accessors.physical_schemata_result_set_cardinality_gafs( physical_schemata );
    SubLObject cost = NIL;
    if( NIL != rs_cardinality_gafs )
    {
      ...
      final SubLObject subset_testP = Numbers.numG( Numbers.add( Sequences.length( fully_bound_physical_fields_list ), Sequences.length( not_fully_bound_physical_fields_list ) ), ONE_INTEGER );
  ...
  public static SubLObject substitute_logical_field_indexicals_with_physical_fields_list(final SubLObject logical_field_indexicals, final SubLObject logical_physical_field_mappings)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        new_pf_list = ConsesLow.cons( sksi_kb_accessors.pf_for_lfi( pf, lfi ), new_pf_list );
  ...
  public static SubLObject sksi_determine_output_generate_pattern(final SubLObject required_pattern, final SubLObject relevant_physical_field_indexicals, final SubLObject sentence, final SubLObject logical_schemata,
      final SubLObject physical_schemata, final SubLObject sk_source, final SubLObject input_termnums, final SubLObject output_termnums, final SubLObject content_mt, final SubLObject eval_conditions)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    thread.resetMultipleValues();
    final SubLObject tuple_expression = sksi_determine_output_csql_tuple_expression( relevant_physical_field_indexicals, sentence, logical_schemata, physical_schemata, sk_source, input_termnums, output_termnums,
        eval_conditions );
    ...
    {
      return Values.values( ConsesLow.list( $kw178$CALL, $sym194$GENERATE_SKSI_SINGLE_LITERAL_COST_RECORDING_ITERATOR, tuple_expression, sk_source, content_mt ), boolean_csqlP, missing_required_fieldsP );
    }
    return Values.values( NIL, boolean_csqlP, missing_required_fieldsP );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/query-sks/sksi-removal-module-generation.lisp", position = 112897L)
  public static SubLObject sksi_determine_output_check_pattern(final SubLObject relevant_physical_field_indexicals, final SubLObject sentence, final SubLObject logical_schemata, final SubLObject physical_schemata,
      final SubLObject sk_source, final SubLObject input_termnums, final SubLObject output_termnums, final SubLObject sense, final SubLObject content_mt, final SubLObject eval_conditions)
  {
    final SubLObject tuple_expression = sksi_determine_output_csql_tuple_expression( relevant_physical_field_indexicals, sentence, logical_schemata, physical_schemata, sk_source, input_termnums, output_termnums,
        eval_conditions );
    final SubLObject subpattern = ConsesLow.list( $kw178$CALL, $sym195$GENERATE_BOOLEAN_FROM_CSQL, tuple_expression, sk_source, content_mt );
    if( $kw84$POS == sense )
    {
      return subpattern;
    }
    return ConsesLow.list( $kw178$CALL, $sym196$NOT_BOOLEAN, subpattern );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/query-sks/sksi-removal-module-generation.lisp", position = 113520L)
  public static SubLObject sksi_determine_output_csql_tuple_expression(final SubLObject relevant_physical_field_indexicals, final SubLObject sentence, final SubLObject logical_schemata,
      final SubLObject physical_schemata, final SubLObject sk_source, final SubLObject input_termnums, final SubLObject output_termnums, final SubLObject eval_conditions)
  {
    final SubLObject required_fields = Mapping.mapcan( $sym197$PHYSICAL_SCHEMA_REQUIRED_FIELDS, physical_schemata, EMPTY_SUBL_OBJECT_ARRAY );
    final SubLObject raw_terms = sksi_determine_csql_tuple_terms( relevant_physical_field_indexicals );
    final SubLObject csql_output = sksi_csql_generation.sksi_determine_output_csql( sentence, logical_schemata, physical_schemata, sk_source, input_termnums, output_termnums, relevant_physical_field_indexicals,
        eval_conditions, UNPROVIDED, UNPROVIDED );
    ...
    {
      return Values.values( ConsesLow.list( $kw172$TUPLE, raw_terms, csql_output ), sksi_csql_utilities.boolean_csqlP( csql_output ), makeBoolean( NIL == required_fields_in_placeP ) );
    }
    return Values.values( NIL, NIL, makeBoolean( NIL == required_fields_in_placeP ) );
  }
  ...
  public static SubLObject required_fields_have_values_in_csqlP(final SubLObject csql, final SubLObject required_fields, final SubLObject sk_source)
  {
    ...
    if( NIL != required_fields )
    {
      ...
      SubLObject from = NIL;
      cdestructuring_bind.destructuring_bind_must_consp( csql, csql, $list198 );
      select_keyword = csql.first();
      SubLObject current = csql.rest();
      cdestructuring_bind.destructuring_bind_must_consp( current, csql, $list198 );
      ...
      cdestructuring_bind.destructuring_bind_must_listp( current, csql, $list198 );
      current = current.rest();
      if( NIL == current )
      {
        if( NIL != where )
        {
          ...
          cdestructuring_bind.destructuring_bind_must_consp( current_$32, datum_$31, $list199 );
          ...
          if( NIL == current_$32 )
          {
            if( NIL == failureP )
            {
              ...
              {
                final SubLObject naked_field_name = ConsesLow.list( sksi_kb_accessors.physical_field_name( required_field ) );
          ...
          {
            cdestructuring_bind.cdestructuring_bind_error( datum_$31, $list199 );
          }
        }
        else
        {
      ...
      {
        cdestructuring_bind.cdestructuring_bind_error( csql, $list198 );
      }
    }
  ...
  public static SubLObject sksi_generate_raw_symbols_for_relevant_physical_field_indexicals(final SubLObject relevant_physical_field_indexicals)
  {
    ...
    SubLObject i;
    for( cdotimes_end_var = Sequences.length( relevant_physical_field_indexicals ), i = NIL, i = ZERO_INTEGER; i.numL( cdotimes_end_var ); i = Numbers.add( i, ONE_INTEGER ) )
    {
      tuple_output = ConsesLow.cons( token_raw_symbol_for_termnum( number_utilities.f_1X( i ) ), tuple_output );
    }
  ...
  {
    final SubLObject termnum = cycl_utilities.arg_positions_bfs( lfi, sentence, Symbols.symbol_function( EQUAL ) ).first();
  ...
  public static SubLObject sksi_referenced_logical_field_indexicals(final SubLObject sentence, final SubLObject termnums)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        logical_field_indexicals = ConsesLow.cons( lfi, logical_field_indexicals );
      }
      cdolist_list_var = cdolist_list_var.rest();
      termnum = cdolist_list_var.first();
    }
  ...
  {
    return Packages.intern( Sequences.cconcatenate( $str200$CYCL_TERM, format_nil.format_nil_a_no_copy( termnum ) ), UNPROVIDED );
  }
  ...
  {
    return Packages.intern( Sequences.cconcatenate( $str201$RAW_TERM, format_nil.format_nil_a_no_copy( termnum ) ), UNPROVIDED );
  }
  ...
  public static SubLObject token_raw_symbol_for_cycl_symbol(final SubLObject cycl_symbol)
  {
    ...
    final SubLObject raw_name = Sequences.cconcatenate( $str202$RAW, postfix );
    return Packages.intern( raw_name, UNPROVIDED );
  }
  ...
  public static SubLObject token_cycl_symbol_for_raw_symbol(final SubLObject raw_symbol)
  {
    ...
    final SubLObject cycl_name = Sequences.cconcatenate( $str203$CYCL, postfix );
    return Packages.intern( cycl_name, UNPROVIDED );
  }
  ...
  {
    final SubLObject logical_field_indexicals_appearing = list_utilities.fast_delete_duplicates( list_utilities.tree_gather_ordered( sentence, $sym55$LOGICAL_FIELD_INDEXICAL_P, Symbols.symbol_function( IDENTITY ), NIL ),
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject positions = cycl_utilities.arg_positions_bfs( lfi, sentence, Symbols.symbol_function( EQUAL ) );
      termnums = ConsesLow.cons( positions, termnums );
      cdolist_list_var = cdolist_list_var.rest();
      lfi = cdolist_list_var.first();
    }
  ...
  {
    return makeBoolean( NIL == cycl_utilities.expression_find( $kw81$NOT_FULLY_BOUND, required_pattern, T, UNPROVIDED, UNPROVIDED ) && NIL == cycl_utilities.expression_find( $kw103$UNIFY, required_pattern, T, UNPROVIDED,
        UNPROVIDED ) );
  }
  ...
  {
    $virtual_assertion_count$.setGlobalValue( Numbers.add( $virtual_assertion_count$.getGlobalValue(), Numbers.round( delta_count, UNPROVIDED ) ) );
    return $virtual_assertion_count$.getGlobalValue();
  }
  ...
  {
    $virtual_assertion_count$.setGlobalValue( Numbers.subtract( $virtual_assertion_count$.getGlobalValue(), Numbers.round( delta_count, UNPROVIDED ) ) );
    return $virtual_assertion_count$.getGlobalValue();
  }
  ...
  {
    SubLObject current_count = Hashtables.gethash( mt, $mt_virtual_assertion_count_table$.getGlobalValue(), ZERO_INTEGER );
    current_count = Numbers.add( current_count, Numbers.round( assertion_count, UNPROVIDED ) );
    Hashtables.sethash( mt, $mt_virtual_assertion_count_table$.getGlobalValue(), current_count );
  ...
  {
    SubLObject current_count = Hashtables.gethash( mt, $mt_virtual_assertion_count_table$.getGlobalValue(), ZERO_INTEGER );
    current_count = Numbers.subtract( current_count, Numbers.round( assertion_count, UNPROVIDED ) );
    ...
    {
      Hashtables.remhash( mt, $mt_virtual_assertion_count_table$.getGlobalValue() );
    }
  ...
  {
    return Hashtables.clrhash( $mt_virtual_assertion_count_table$.getGlobalValue() );
  }
  ...
  public static SubLObject sksi_module_adequate_for_browsing_supportP(final SubLObject required_pattern, final SubLObject predicate_list, final SubLObject relevant_code_mapping_schemas_or_terms)
  {
    ...
    final SubLObject relevant_code_mapping_arg_positions = ( NIL != all_variablesP ) ? NIL : Mapping.mapcar( $sym93$FIRST, relevant_code_mapping_schemas_or_terms );
    ...
    return Values.values( T, all_variablesP );
  }
  ...
  {
    return list_utilities.not_eq( required_pattern_term, $kw81$NOT_FULLY_BOUND );
  }
  ...
  public static SubLObject sksi_removal_module_assertion_count(final SubLObject hl_module)
  {
    ...
    if( count.isNumber() )
    {
      count = Numbers.round( count, UNPROVIDED );
    }
    else
    {
      count = NIL;
  ...
  {
    return list_utilities.remove_if_not( $sym209$SKSI_SUPPORTED_PREDICATE_TYPE_P, inference_modules.hl_module_any_predicates( module ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject sksi_module_any_predicates_list(final SubLObject module)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym208$SKSI_MODULE_ANY_PREDICATES_LIST, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym208$SKSI_MODULE_ANY_PREDICATES_LIST, ONE_INTEGER, NIL, EQ, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym208$SKSI_MODULE_ANY_PREDICATES_LIST, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_module_any_predicates_list_internal( module ) ) );
  ...
  public static SubLObject register_sksi_supported_browsing_for_module(final SubLObject hl_module, final SubLObject meaning_sentence, final SubLObject required_pattern, final SubLObject relevant_code_mapping_schemas,
      final SubLObject relevant_instance_named_fn_colls, final SubLObject content_mt)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    {
      final SubLObject required_mt = inference_modules.hl_module_required_mt( hl_module );
      final SubLObject fort_arguments = keyhash_utilities.fast_intersection( cycl_utilities.formula_forts( required_pattern, UNPROVIDED, UNPROVIDED, UNPROVIDED ), cycl_utilities.formula_args( required_pattern,
      ...
      if( NIL == predicate_list )
      {
        ...
        if( NIL != pred )
        {
          predicate_list = ConsesLow.list( pred );
        }
      }
      if( NIL != normal_term_browsingP )
      {
        final SubLObject term_arguments = keyhash_utilities.fast_intersection( cycl_utilities.formula_forts( required_pattern, UNPROVIDED, UNPROVIDED, UNPROVIDED ), cycl_utilities.formula_args( required_pattern,
            UNPROVIDED ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
        if( NIL != count )
        {
        ...
        final SubLObject fort_count = list_utilities.tree_count_if( $sym211$FORT_P, fort_arguments, UNPROVIDED );
      ...
      else if( NIL != module_required_pattern_adequate_for_gaf_argument_browsing_supportP( required_pattern ) )
      {
        ...
        final SubLObject term_count = list_utilities.tree_count_if( $sym211$FORT_P, all_terms, UNPROVIDED );
  ...
  {
    return list_utilities.occurs_at_most_n_timesP( $kw76$FULLY_BOUND, required_pattern.rest(), ONE_INTEGER, UNPROVIDED );
  }
      ...
      if( NIL == current )
      {
        ...
        while ( NIL != cdolist_list_var_$36)
        {
          if( arg_pos.numG( ZERO_INTEGER ) )
          {
            ...
            if( NIL != cycl_terms )
            {
              code_mapping_terms = ConsesLow.nconc( code_mapping_terms, ConsesLow.list( ConsesLow.list( arg_pos, cycl_terms ) ) );
            }
          }
  ...
  public static SubLObject gather_cycl_terms_mapped_to_by_code_mapping_schemas(final SubLObject code_mapping_schemas)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym214$GATHER_CYCL_TERMS_MAPPED_TO_BY_CODE_MAPPING_SCHEMAS, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym214$GATHER_CYCL_TERMS_MAPPED_TO_BY_CODE_MAPPING_SCHEMAS, ONE_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym214$GATHER_CYCL_TERMS_MAPPED_TO_BY_CODE_MAPPING_SCHEMAS, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( gather_cycl_terms_mapped_to_by_code_mapping_schemas_internal( code_mapping_schemas ) ) );
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        if( NIL == current )
        {
          ...
          while ( NIL != cdolist_list_var_$37)
          {
            ...
            if( NIL != cycl_terms )
            {
              result = ConsesLow.nconc( result, ConsesLow.list( ConsesLow.list( arg_pos, cycl_terms ) ) );
            }
  ...
  public static SubLObject deregister_sksi_supported_browsing_for_module(final SubLObject hl_module)
  {
    ...
    if( NIL != browsing_supportP )
    {
      ...
      if( NIL == predicate_list )
      {
        predicate_list = ConsesLow.list( inference_modules.hl_module_predicate( hl_module ) );
      }
      if( NIL != normal_term_browsingP )
      {
        if( NIL != count )
        ...
        final SubLObject term_arguments = keyhash_utilities.fast_intersection( ( NIL != required_pattern ) ? cycl_utilities.formula_forts( required_pattern, UNPROVIDED, UNPROVIDED, UNPROVIDED ) : NIL, cycl_utilities
            .formula_args( required_pattern, UNPROVIDED ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject sksi_removal_module_generation_error(final SubLObject cycl, final SubLObject error_message)
  {
    ...
    {
      final SubLObject cont_string = Sequences.cconcatenate( $str217$Continue_without_generating_remov, format_nil.format_nil_a_no_copy( cycl ) );
  ...
  {
    SubLFiles.declareFunction( me, "sksi_module_p", "SKSI-MODULE-P", 1, 0, false );
    SubLFiles.declareMacro( me, "do_sksi_modules", "DO-SKSI-MODULES" );
    SubLFiles.declareMacro( me, "do_sksi_modules_list", "DO-SKSI-MODULES-LIST" );
    SubLFiles.declareFunction( me, "sksi_modules_count", "SKSI-MODULES-COUNT", 0, 0, false );
    SubLFiles.declareFunction( me, "some_sksi_single_literal_removal_module_registeredP", "SOME-SKSI-SINGLE-LITERAL-REMOVAL-MODULE-REGISTERED?", 0, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module", "NOTE-SKSI-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "un_note_sksi_module", "UN-NOTE-SKSI-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "un_note_all_sksi_modules", "UN-NOTE-ALL-SKSI-MODULES", 0, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module_supports", "NOTE-SKSI-MODULE-SUPPORTS", 2, 0, false );
    SubLFiles.declareFunction( me, "un_note_sksi_module_supports", "UN-NOTE-SKSI-MODULE-SUPPORTS", 1, 0, false );
    SubLFiles.declareFunction( me, "un_note_all_sksi_module_supports", "UN-NOTE-ALL-SKSI-MODULE-SUPPORTS", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_supports", "SKSI-MODULE-SUPPORTS", 1, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module_simple_required_pattern", "NOTE-SKSI-MODULE-SIMPLE-REQUIRED-PATTERN", 2, 0, false );
    SubLFiles.declareFunction( me, "un_note_sksi_module_simple_required_pattern", "UN-NOTE-SKSI-MODULE-SIMPLE-REQUIRED-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "un_note_all_sksi_module_simple_required_patterns", "UN-NOTE-ALL-SKSI-MODULE-SIMPLE-REQUIRED-PATTERNS", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_simple_required_pattern", "SKSI-MODULE-SIMPLE-REQUIRED-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "note_multiply_indexed_conditional_meaning_sentence_for_sks", "NOTE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-FOR-SKS", 2, 0, false );
    SubLFiles.declareFunction( me, "un_note_multiply_indexed_conditional_meaning_sentence_for_sks", "UN-NOTE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-FOR-SKS", 2, 0, false );
    SubLFiles.declareFunction( me, "multiply_indexed_conditional_meaning_sentence_noted_for_sksP", "MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-NOTED-FOR-SKS?", 2, 0, false );
    SubLFiles.declareFunction( me, "un_note_all_multiply_indexed_conditional_meaning_sentences", "UN-NOTE-ALL-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCES", 0, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module_multiply_indexed_conditional_meaning_sentence", "NOTE-SKSI-MODULE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE", 2, 0, false );
    SubLFiles.declareFunction( me, "un_note_sksi_module_multiply_indexed_conditional_meaning_sentence", "UN-NOTE-SKSI-MODULE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "un_note_all_sksi_module_multiply_indexed_conditional_meaning_sentences", "UN-NOTE-ALL-SKSI-MODULE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCES", 0, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module_by_sks", "NOTE-SKSI-MODULE-BY-SKS", 2, 0, false );
    SubLFiles.declareFunction( me, "un_note_sksi_modules_by_sks", "UN-NOTE-SKSI-MODULES-BY-SKS", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_modules_by_sks_cache", "CLEAR-SKSI-MODULES-BY-SKS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "un_note_sksi_modules_by_sks_for_module", "UN-NOTE-SKSI-MODULES-BY-SKS-FOR-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_modules_by_sks_get_skses", "SKSI-MODULES-BY-SKS-GET-SKSES", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_modules_by_sks_find_modules", "SKSI-MODULES-BY-SKS-FIND-MODULES", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_modules_by_sks_find_modules_with_children", "SKSI-MODULES-BY-SKS-FIND-MODULES-WITH-CHILDREN", 1, 0, false );
    SubLFiles.declareMacro( me, "do_code_mapping_sks_cache", "DO-CODE-MAPPING-SKS-CACHE" );
    SubLFiles.declareFunction( me, "note_code_mapping_sks", "NOTE-CODE-MAPPING-SKS", 4, 0, false );
    SubLFiles.declareFunction( me, "unnote_code_mapping_sks", "UNNOTE-CODE-MAPPING-SKS", 4, 0, false );
    SubLFiles.declareFunction( me, "clear_code_mapping_sks_cache", "CLEAR-CODE-MAPPING-SKS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "code_mapping_sks_cache_lookup", "CODE-MAPPING-SKS-CACHE-LOOKUP", 1, 0, false );
    SubLFiles.declareFunction( me, "code_mapping_forward_rules_to_repropagate_internal", "CODE-MAPPING-FORWARD-RULES-TO-REPROPAGATE-INTERNAL", 0, 0, false );
    SubLFiles.declareFunction( me, "code_mapping_forward_rules_to_repropagate", "CODE-MAPPING-FORWARD-RULES-TO-REPROPAGATE", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_assert_code_mapping_sks_content", "SKSI-ASSERT-CODE-MAPPING-SKS-CONTENT", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_unassert_code_mapping_sks_content", "SKSI-UNASSERT-CODE-MAPPING-SKS-CONTENT", 4, 0, false );
    SubLFiles.declareFunction( me, "isa_removal_module_p", "ISA-REMOVAL-MODULE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "add_isa_sks_term_to_cycl_term_mts", "ADD-ISA-SKS-TERM-TO-CYCL-TERM-MTS", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_isa_sks_term_to_cycl_term_mts", "CLEAR-ISA-SKS-TERM-TO-CYCL-TERM-MTS", 0, 0, false );
    SubLFiles.declareFunction( me, "isa_sks_term_to_cycl_term_mt_relevantP", "ISA-SKS-TERM-TO-CYCL-TERM-MT-RELEVANT?", 0, 0, false );
    SubLFiles.declareFunction( me, "add_isa_sks_term_to_cycl_term_store", "ADD-ISA-SKS-TERM-TO-CYCL-TERM-STORE", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_isa_sks_term_to_cycl_term_store", "CLEAR-ISA-SKS-TERM-TO-CYCL-TERM-STORE", 0, 0, false );
    SubLFiles.declareFunction( me, "any_sks_term_spec_to_cycl_term_in_relevant_mtP", "ANY-SKS-TERM-SPEC-TO-CYCL-TERM-IN-RELEVANT-MT?", 1, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_specs_for_term", "ALL-SKSI-RELEVANT-REWRITE-SPECS-FOR-TERM", 1, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_specs_for_term_int_internal", "ALL-SKSI-RELEVANT-REWRITE-SPECS-FOR-TERM-INT-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_specs_for_term_int", "ALL-SKSI-RELEVANT-REWRITE-SPECS-FOR-TERM-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_isas", "ALL-SKSI-RELEVANT-REWRITE-ISAS", 0, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_isas_int_internal", "ALL-SKSI-RELEVANT-REWRITE-ISAS-INT-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_isas_int", "ALL-SKSI-RELEVANT-REWRITE-ISAS-INT", 1, 0, false );
    SubLFiles.declareFunction( me, "add_isa_sks_term_to_cycl_term", "ADD-ISA-SKS-TERM-TO-CYCL-TERM", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_isa_sks_term_to_cycl_term", "CLEAR-ISA-SKS-TERM-TO-CYCL-TERM", 0, 0, false );
    SubLFiles.declareFunction( me, "store_sksi_isa_module_rewrite_info", "STORE-SKSI-ISA-MODULE-REWRITE-INFO", 2, 1, false );
    SubLFiles.declareFunction( me, "genls_rewrite_supported_term_p", "GENLS-REWRITE-SUPPORTED-TERM-P", 2, 0, false );
    SubLFiles.declareFunction( me, "gather_code_mapping_arg2s", "GATHER-CODE-MAPPING-ARG2S", 2, 0, false );
    SubLFiles.declareFunction( me, "recalculate_sksi_isa_module_rewrite_info", "RECALCULATE-SKSI-ISA-MODULE-REWRITE-INFO", 0, 0, false );
    SubLFiles.declareFunction( me, "filtered_sksi_predP", "FILTERED-SKSI-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "add_to_meaning_sentence_mts", "ADD-TO-MEANING-SENTENCE-MTS", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_meaning_sentence_mts", "CLEAR-MEANING-SENTENCE-MTS", 0, 0, false );
    SubLFiles.declareFunction( me, "any_sksi_meaning_sentence_mt_relevantP", "ANY-SKSI-MEANING-SENTENCE-MT-RELEVANT?", 0, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_pred_store_count", "MEANING-SENTENCE-PRED-STORE-COUNT", 0, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_predP", "MEANING-SENTENCE-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "add_meaning_sentence_pred_to_store", "ADD-MEANING-SENTENCE-PRED-TO-STORE", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_meaning_sentence_pred_store", "CLEAR-MEANING-SENTENCE-PRED-STORE", 0, 0, false );
    SubLFiles.declareFunction( me, "any_sksi_meaning_sentence_spec_predP", "ANY-SKSI-MEANING-SENTENCE-SPEC-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "any_sksi_meaning_sentence_spec_inverseP", "ANY-SKSI-MEANING-SENTENCE-SPEC-INVERSE?", 1, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_spec_preds_for_pred", "ALL-SKSI-RELEVANT-REWRITE-SPEC-PREDS-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_spec_preds_for_pred_int_internal", "ALL-SKSI-RELEVANT-REWRITE-SPEC-PREDS-FOR-PRED-INT-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_spec_preds_for_pred_int", "ALL-SKSI-RELEVANT-REWRITE-SPEC-PREDS-FOR-PRED-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_spec_inverses_for_pred", "ALL-SKSI-RELEVANT-REWRITE-SPEC-INVERSES-FOR-PRED", 1, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_spec_inverses_for_pred_int_internal", "ALL-SKSI-RELEVANT-REWRITE-SPEC-INVERSES-FOR-PRED-INT-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "all_sksi_relevant_rewrite_spec_inverses_for_pred_int", "ALL-SKSI-RELEVANT-REWRITE-SPEC-INVERSES-FOR-PRED-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "add_meaning_sentence_pred", "ADD-MEANING-SENTENCE-PRED", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_meaning_sentence_preds", "CLEAR-MEANING-SENTENCE-PREDS", 0, 0, false );
    SubLFiles.declareFunction( me, "store_sksi_module_rewrite_pred_info", "STORE-SKSI-MODULE-REWRITE-PRED-INFO", 1, 0, false );
    SubLFiles.declareFunction( me, "recalculate_sksi_rewrite_pred_info", "RECALCULATE-SKSI-REWRITE-PRED-INFO", 0, 0, false );
    SubLFiles.declareFunction( me, "store_sksi_module_rewrite_info", "STORE-SKSI-MODULE-REWRITE-INFO", 3, 0, false );
    SubLFiles.declareFunction( me, "recalculate_sksi_rewrite_info", "RECALCULATE-SKSI-REWRITE-INFO", 0, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_rewrite_info", "CLEAR-SKSI-REWRITE-INFO", 0, 0, false );
    SubLFiles.declareFunction( me, "register_sksi_removal_modules_for_sks", "REGISTER-SKSI-REMOVAL-MODULES-FOR-SKS", 1, 3, false );
    SubLFiles.declareFunction( me, "register_sksi_removal_modules_for_just_sks", "REGISTER-SKSI-REMOVAL-MODULES-FOR-JUST-SKS", 1, 4, false );
    SubLFiles.declareFunction( me, "repropagate_code_mapping_forward_rules", "REPROPAGATE-CODE-MAPPING-FORWARD-RULES", 0, 0, false );
    SubLFiles.declareFunction( me, "deregister_sksi_removal_modules_for_sks", "DEREGISTER-SKSI-REMOVAL-MODULES-FOR-SKS", 1, 3, false );
    SubLFiles.declareFunction( me, "deregister_sksi_removal_modules_for_just_sks", "DEREGISTER-SKSI-REMOVAL-MODULES-FOR-JUST-SKS", 1, 2, false );
    SubLFiles.declareFunction( me, "deregister_sksi_removal_module", "DEREGISTER-SKSI-REMOVAL-MODULE", 1, 1, false );
    SubLFiles.declareFunction( me, "deregister_all_sksi_removal_modules", "DEREGISTER-ALL-SKSI-REMOVAL-MODULES", 0, 0, false );
    SubLFiles.declareFunction( me, "module_info_p", "MODULE-INFO-P", 1, 0, false );
    SubLFiles.declareFunction( me, "new_module_info", "NEW-MODULE-INFO", 0, 0, false );
    SubLFiles.declareFunction( me, "module_info_add", "MODULE-INFO-ADD", 3, 0, false );
    SubLFiles.declareFunction( me, "module_info_push", "MODULE-INFO-PUSH", 3, 0, false );
    SubLFiles.declareFunction( me, "module_info_get", "MODULE-INFO-GET", 2, 1, false );
    SubLFiles.declareFunction( me, "module_info_remove", "MODULE-INFO-REMOVE", 2, 0, false );
    SubLFiles.declareFunction( me, "module_info_to_inference_module_info", "MODULE-INFO-TO-INFERENCE-MODULE-INFO", 1, 0, false );
    SubLFiles.declareFunction( me, "inference_removal_module_info", "INFERENCE-REMOVAL-MODULE-INFO", 1, 0, false );
    SubLFiles.declareFunction( me, "determine_module_info_name", "DETERMINE-MODULE-INFO-NAME", 1, 0, false );
    SubLFiles.declareFunction( me, "sk_source_module_infos", "SK-SOURCE-MODULE-INFOS", 5, 0, false );
    SubLFiles.declareFunction( me, "physical_schema_module_infos", "PHYSICAL-SCHEMA-MODULE-INFOS", 6, 0, false );
    SubLFiles.declareFunction( me, "logical_schema_module_infos", "LOGICAL-SCHEMA-MODULE-INFOS", 6, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_gaf_module_infos", "MEANING-SENTENCE-GAF-MODULE-INFOS", 6, 0, false );
    SubLFiles.declareFunction( me, "negated_atomic_meaning_sentence_module_infos", "NEGATED-ATOMIC-MEANING-SENTENCE-MODULE-INFOS", 6, 0, false );
    SubLFiles.declareFunction( me, "pos_atomic_meaning_sentence_module_infos", "POS-ATOMIC-MEANING-SENTENCE-MODULE-INFOS", 6, 4, false );
    SubLFiles.declareFunction( me, "conditional_meaning_sentence_gaf_module_infos", "CONDITIONAL-MEANING-SENTENCE-GAF-MODULE-INFOS", 7, 0, false );
    SubLFiles.declareFunction( me, "condition_output_pattern", "CONDITION-OUTPUT-PATTERN", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_any_predicates_pattern_internal", "SKSI-GET-ANY-PREDICATES-PATTERN-INTERNAL", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_get_any_predicates_pattern", "SKSI-GET-ANY-PREDICATES-PATTERN", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_too_many_unbound_required_pattern_termsP", "SKSI-TOO-MANY-UNBOUND-REQUIRED-PATTERN-TERMS?", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_too_many_unbound_required_pattern_terms_intP", "SKSI-TOO-MANY-UNBOUND-REQUIRED-PATTERN-TERMS-INT?", 3, 0, false );
    SubLFiles.declareFunction( me, "antecedent_sentence_contains_restrictive_constraintP", "ANTECEDENT-SENTENCE-CONTAINS-RESTRICTIVE-CONSTRAINT?", 1, 0, false );
    SubLFiles.declareFunction( me, "non_logical_field_indexical_p", "NON-LOGICAL-FIELD-INDEXICAL-P", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_doomed_unless_arg_boundP", "SKSI-DOOMED-UNLESS-ARG-BOUND?", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_valid_removal_module_required_patternP", "SKSI-VALID-REMOVAL-MODULE-REQUIRED-PATTERN?", 4, 0, false );
    SubLFiles.declareFunction( me, "pos_atomic_meaning_sentence_required_pattern_infos", "POS-ATOMIC-MEANING-SENTENCE-REQUIRED-PATTERN-INFOS", 6, 1, false );
    SubLFiles.declareFunction( me, "sksi_fully_unbound_required_pattern_termsP", "SKSI-FULLY-UNBOUND-REQUIRED-PATTERN-TERMS?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_not_fully_bound_required_pattern_predicateP", "SKSI-NOT-FULLY-BOUND-REQUIRED-PATTERN-PREDICATE?", 1, 0, false );
    SubLFiles.declareFunction( me, "has_duplicate_keysP", "HAS-DUPLICATE-KEYS?", 1, 0, false );
    SubLFiles.declareFunction( me, "compute_boundedness_permutations_for_logical_field_indexical_termnums", "COMPUTE-BOUNDEDNESS-PERMUTATIONS-FOR-LOGICAL-FIELD-INDEXICAL-TERMNUMS", 2, 0, false );
    SubLFiles.declareFunction( me, "distinct_logical_field_termnum_bindingsP", "DISTINCT-LOGICAL-FIELD-TERMNUM-BINDINGS?", 2, 0, false );
    SubLFiles.declareFunction( me, "compute_meaning_sentence_required_patterns", "COMPUTE-MEANING-SENTENCE-REQUIRED-PATTERNS", 3, 0, false );
    SubLFiles.declareFunction( me, "replace_pattern_parts", "REPLACE-PATTERN-PARTS", 2, 0, false );
    SubLFiles.declareFunction( me, "replace_pattern_narts_with_nat_subpatterns", "REPLACE-PATTERN-NARTS-WITH-NAT-SUBPATTERNS", 1, 0, false );
    SubLFiles.declareFunction( me, "make_nat_subpattern", "MAKE-NAT-SUBPATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "nat_subpattern_p", "NAT-SUBPATTERN-P", 1, 0, false );
    SubLFiles.declareFunction( me, "nartify_nat_pattern", "NARTIFY-NAT-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "disjunctive_subpattern_p", "DISJUNCTIVE-SUBPATTERN-P", 1, 0, false );
    SubLFiles.declareFunction( me, "make_disjunctive_subpattern", "MAKE-DISJUNCTIVE-SUBPATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "make_unify_subpattern", "MAKE-UNIFY-SUBPATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "unify_subpattern_p", "UNIFY-SUBPATTERN-P", 1, 0, false );
    SubLFiles.declareFunction( me, "replace_pattern_nauts_with_unify_subpatterns", "REPLACE-PATTERN-NAUTS-WITH-UNIFY-SUBPATTERNS", 1, 0, false );
    SubLFiles.declareFunction( me, "replace_pattern_forts_with_unify_subpatterns", "REPLACE-PATTERN-FORTS-WITH-UNIFY-SUBPATTERNS", 1, 0, false );
    SubLFiles.declareFunction( me, "non_sentential_relation_fort_p_internal", "NON-SENTENTIAL-RELATION-FORT-P-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "non_sentential_relation_fort_p", "NON-SENTENTIAL-RELATION-FORT-P", 1, 0, false );
    SubLFiles.declareFunction( me, "replace_pattern_special_subpatterns_with_forts", "REPLACE-PATTERN-SPECIAL-SUBPATTERNS-WITH-FORTS", 1, 0, false );
    SubLFiles.declareFunction( me, "replace_pattern_special_subpatterns_with_anything", "REPLACE-PATTERN-SPECIAL-SUBPATTERNS-WITH-ANYTHING", 1, 0, false );
    SubLFiles.declareFunction( me, "add_cmls_pruning_to_required_pattern", "ADD-CMLS-PRUNING-TO-REQUIRED-PATTERN", 5, 0, false );
    SubLFiles.declareFunction( me, "clear_generate_sksi_slrm_required_function", "CLEAR-GENERATE-SKSI-SLRM-REQUIRED-FUNCTION", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_generate_sksi_slrm_required_function", "REMOVE-GENERATE-SKSI-SLRM-REQUIRED-FUNCTION", 1, 0, false );
    SubLFiles.declareFunction( me, "generate_sksi_slrm_required_function_internal", "GENERATE-SKSI-SLRM-REQUIRED-FUNCTION-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "generate_sksi_slrm_required_function", "GENERATE-SKSI-SLRM-REQUIRED-FUNCTION", 1, 0, false );
    SubLFiles.declareFunction( me, "determine_sk_source_required_func_name", "DETERMINE-SK-SOURCE-REQUIRED-FUNC-NAME", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_only_predicateP_internal", "SKSI-ONLY-PREDICATE?-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_only_predicateP", "SKSI-ONLY-PREDICATE?", 1, 0, false );
    SubLFiles.declareFunction( me, "required_info_module_info", "REQUIRED-INFO-MODULE-INFO", 9, 0, false );
    SubLFiles.declareFunction( me, "sksi_removal_module_completeness", "SKSI-REMOVAL-MODULE-COMPLETENESS", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_make_documentation_string_from_required_pattern_and_sense", "SKSI-MAKE-DOCUMENTATION-STRING-FROM-REQUIRED-PATTERN-AND-SENSE", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_input_extract_pattern", "SKSI-DETERMINE-INPUT-EXTRACT-PATTERN", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_output_verify_pattern", "SKSI-DETERMINE-OUTPUT-VERIFY-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_input_verify_pattern", "SKSI-DETERMINE-INPUT-VERIFY-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_input_encode_pattern", "SKSI-DETERMINE-INPUT-ENCODE-PATTERN", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_reformulatable_via_templateP", "SKSI-REFORMULATABLE-VIA-TEMPLATE?", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_field_input_encode_pattern", "SKSI-DETERMINE-FIELD-INPUT-ENCODE-PATTERN", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_output_decode_pattern", "SKSI-DETERMINE-OUTPUT-DECODE-PATTERN", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_field_output_decode_pattern", "SKSI-DETERMINE-FIELD-OUTPUT-DECODE-PATTERN", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_field_output_decode_pattern_int", "SKSI-DETERMINE-FIELD-OUTPUT-DECODE-PATTERN-INT", 4, 0, false );
    SubLFiles.declareFunction( me, "dummy_external_term_denoting_function_expression_p", "DUMMY-EXTERNAL-TERM-DENOTING-FUNCTION-EXPRESSION-P", 2, 0, false );
    SubLFiles.declareFunction( me, "gather_external_term_denoting_expressions_of_type_in_field_decoding", "GATHER-EXTERNAL-TERM-DENOTING-EXPRESSIONS-OF-TYPE-IN-FIELD-DECODING", 2, 0, false );
    SubLFiles.declareFunction( me, "field_decoding_contains_city_named_fn_expressionP", "FIELD-DECODING-CONTAINS-CITY-NAMED-FN-EXPRESSION?", 1, 0, false );
    SubLFiles.declareFunction( me, "gather_code_mapping_schemas_mentioned_in_field_decoding", "GATHER-CODE-MAPPING-SCHEMAS-MENTIONED-IN-FIELD-DECODING", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_output_construct_pattern", "SKSI-DETERMINE-OUTPUT-CONSTRUCT-PATTERN", 4, 0, false );
    SubLFiles.declareFunction( me, "determine_output_construct_pattern_template_and_tuple_output", "DETERMINE-OUTPUT-CONSTRUCT-PATTERN-TEMPLATE-AND-TUPLE-OUTPUT", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_cost_pattern", "SKSI-DETERMINE-COST-PATTERN", 2, 1, false );
    SubLFiles.declareFunction( me, "sksi_removal_module_cost", "SKSI-REMOVAL-MODULE-COST", 6, 3, false );
    SubLFiles.declareFunction( me, "sksi_removal_module_logical_cost", "SKSI-REMOVAL-MODULE-LOGICAL-COST", 4, 1, false );
    SubLFiles.declareFunction( me, "sksi_removal_module_physical_cost", "SKSI-REMOVAL-MODULE-PHYSICAL-COST", 5, 3, false );
    SubLFiles.declareFunction( me, "substitute_logical_field_indexicals_with_physical_fields_list", "SUBSTITUTE-LOGICAL-FIELD-INDEXICALS-WITH-PHYSICAL-FIELDS-LIST", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_output_generate_pattern", "SKSI-DETERMINE-OUTPUT-GENERATE-PATTERN", 10, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_output_check_pattern", "SKSI-DETERMINE-OUTPUT-CHECK-PATTERN", 10, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_output_csql_tuple_expression", "SKSI-DETERMINE-OUTPUT-CSQL-TUPLE-EXPRESSION", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_determine_csql_tuple_terms", "SKSI-DETERMINE-CSQL-TUPLE-TERMS", 1, 0, false );
    SubLFiles.declareFunction( me, "required_fields_have_values_in_csqlP", "REQUIRED-FIELDS-HAVE-VALUES-IN-CSQL?", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_generate_raw_symbols_for_relevant_physical_field_indexicals", "SKSI-GENERATE-RAW-SYMBOLS-FOR-RELEVANT-PHYSICAL-FIELD-INDEXICALS", 1, 0, false );
    SubLFiles.declareFunction( me, "cycl_token_for_logical_field_indexical", "CYCL-TOKEN-FOR-LOGICAL-FIELD-INDEXICAL", 2, 0, false );
    SubLFiles.declareFunction( me, "logical_field_indexical_positions_in_sentence", "LOGICAL-FIELD-INDEXICAL-POSITIONS-IN-SENTENCE", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_referenced_logical_field_indexicals", "SKSI-REFERENCED-LOGICAL-FIELD-INDEXICALS", 2, 0, false );
    SubLFiles.declareFunction( me, "token_cycl_symbol_for_termnum", "TOKEN-CYCL-SYMBOL-FOR-TERMNUM", 1, 0, false );
    SubLFiles.declareFunction( me, "token_raw_symbol_for_termnum", "TOKEN-RAW-SYMBOL-FOR-TERMNUM", 1, 0, false );
    SubLFiles.declareFunction( me, "token_raw_symbol_for_cycl_symbol", "TOKEN-RAW-SYMBOL-FOR-CYCL-SYMBOL", 1, 0, false );
    SubLFiles.declareFunction( me, "token_cycl_symbol_for_raw_symbol", "TOKEN-CYCL-SYMBOL-FOR-RAW-SYMBOL", 1, 0, false );
    SubLFiles.declareFunction( me, "sentence_termnums_containing_logical_field_indexicals", "SENTENCE-TERMNUMS-CONTAINING-LOGICAL-FIELD-INDEXICALS", 1, 0, false );
    SubLFiles.declareFunction( me, "required_pattern_is_checkP", "REQUIRED-PATTERN-IS-CHECK?", 1, 0, false );
    SubLFiles.declareFunction( me, "virtual_assertion_count", "VIRTUAL-ASSERTION-COUNT", 0, 0, false );
    SubLFiles.declareFunction( me, "increment_virtual_assertion_count", "INCREMENT-VIRTUAL-ASSERTION-COUNT", 1, 0, false );
    SubLFiles.declareFunction( me, "decrement_virtual_assertion_count", "DECREMENT-VIRTUAL-ASSERTION-COUNT", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_virtual_assertion_count", "CLEAR-VIRTUAL-ASSERTION-COUNT", 0, 0, false );
    SubLFiles.declareFunction( me, "mt_virtual_assertion_count_table", "MT-VIRTUAL-ASSERTION-COUNT-TABLE", 0, 0, false );
    SubLFiles.declareFunction( me, "increment_assertion_count_for_mt", "INCREMENT-ASSERTION-COUNT-FOR-MT", 2, 0, false );
    SubLFiles.declareFunction( me, "decrement_assertion_count_for_mt", "DECREMENT-ASSERTION-COUNT-FOR-MT", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_assertion_count_for_mt_table", "CLEAR-ASSERTION-COUNT-FOR-MT-TABLE", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_code_mapping_terms", "SKSI-MODULE-CODE-MAPPING-TERMS", 1, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module_code_mapping_terms", "NOTE-SKSI-MODULE-CODE-MAPPING-TERMS", 2, 0, false );
    SubLFiles.declareFunction( me, "unnote_sksi_module_code_mapping_terms", "UNNOTE-SKSI-MODULE-CODE-MAPPING-TERMS", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_module_code_mapping_terms_cache", "CLEAR-SKSI-MODULE-CODE-MAPPING-TERMS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_logical_schemas", "SKSI-MODULE-LOGICAL-SCHEMAS", 1, 0, false );
    SubLFiles.declareFunction( me, "note_sksi_module_logical_schemas", "NOTE-SKSI-MODULE-LOGICAL-SCHEMAS", 2, 0, false );
    SubLFiles.declareFunction( me, "unnote_sksi_module_logical_schemas", "UNNOTE-SKSI-MODULE-LOGICAL-SCHEMAS", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_sksi_module_logical_schemas_cache", "CLEAR-SKSI-MODULE-LOGICAL-SCHEMAS-CACHE", 0, 0, false );
    SubLFiles.declareFunction( me, "deregister_sksi_supported_browsing", "DEREGISTER-SKSI-SUPPORTED-BROWSING", 0, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_adequate_for_browsing_supportP", "SKSI-MODULE-ADEQUATE-FOR-BROWSING-SUPPORT?", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_fully_bound_required_pattern_termP", "SKSI-FULLY-BOUND-REQUIRED-PATTERN-TERM?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_removal_module_assertion_count", "SKSI-REMOVAL-MODULE-ASSERTION-COUNT", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_any_predicates_list_internal", "SKSI-MODULE-ANY-PREDICATES-LIST-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_module_any_predicates_list", "SKSI-MODULE-ANY-PREDICATES-LIST", 1, 0, false );
    SubLFiles.declareFunction( me, "register_sksi_supported_browsing_for_module", "REGISTER-SKSI-SUPPORTED-BROWSING-FOR-MODULE", 6, 0, false );
    SubLFiles.declareFunction( me, "module_required_pattern_adequate_for_gaf_argument_browsing_supportP", "MODULE-REQUIRED-PATTERN-ADEQUATE-FOR-GAF-ARGUMENT-BROWSING-SUPPORT?", 1, 0, false );
    SubLFiles.declareFunction( me, "gather_cycl_terms_mapped_to_by_code_mapping_schemas_internal", "GATHER-CYCL-TERMS-MAPPED-TO-BY-CODE-MAPPING-SCHEMAS-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "gather_cycl_terms_mapped_to_by_code_mapping_schemas", "GATHER-CYCL-TERMS-MAPPED-TO-BY-CODE-MAPPING-SCHEMAS", 1, 0, false );
    SubLFiles.declareFunction( me, "gather_cycl_terms_mapped_to_by_instance_named_fn_collections", "GATHER-CYCL-TERMS-MAPPED-TO-BY-INSTANCE-NAMED-FN-COLLECTIONS", 2, 0, false );
    SubLFiles.declareFunction( me, "deregister_sksi_supported_browsing_for_module", "DEREGISTER-SKSI-SUPPORTED-BROWSING-FOR-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_removal_module_generation_error", "SKSI-REMOVAL-MODULE-GENERATION-ERROR", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_meaning_sentence_gaf_infos_error", "SKSI-MEANING-SENTENCE-GAF-INFOS-ERROR", 2, 0, false );
    return NIL;
  }

  public static SubLObject init_sksi_removal_module_generation_file()
  {
    $sksi_modules$ = SubLFiles.deflexical( "*SKSI-MODULES*", maybeDefault( $sym0$_SKSI_MODULES_, $sksi_modules$, () -> ( set.new_set( UNPROVIDED, UNPROVIDED ) ) ) );
    $sksi_browsing_enabled$ = SubLFiles.defparameter( "*SKSI-BROWSING-ENABLED*", T );
    $sksi_module_supports$ = SubLFiles.deflexical( "*SKSI-MODULE-SUPPORTS*", maybeDefault( $sym6$_SKSI_MODULE_SUPPORTS_, $sksi_module_supports$, () -> ( dictionary.new_dictionary( UNPROVIDED, UNPROVIDED ) ) ) );
    $sksi_module_simple_required_pattern$ = SubLFiles.deflexical( "*SKSI-MODULE-SIMPLE-REQUIRED-PATTERN*", maybeDefault( $sym7$_SKSI_MODULE_SIMPLE_REQUIRED_PATTERN_, $sksi_module_simple_required_pattern$,
        () -> ( dictionary.new_dictionary( UNPROVIDED, UNPROVIDED ) ) ) );
    $sks_multiply_indexed_conditional_meaning_sentence_cache$ = SubLFiles.deflexical( "*SKS-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-CACHE*", maybeDefault(
        $sym8$_SKS_MULTIPLY_INDEXED_CONDITIONAL_MEANING_SENTENCE_CACHE_, $sks_multiply_indexed_conditional_meaning_sentence_cache$, () -> ( dictionary.new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED ) ) ) );
    $sksi_module_multiply_indexed_conditional_meaning_sentence_cache$ = SubLFiles.deflexical( "*SKSI-MODULE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-CACHE*", maybeDefault(
        $sym9$_SKSI_MODULE_MULTIPLY_INDEXED_CONDITIONAL_MEANING_SENTENCE_CACHE_, $sksi_module_multiply_indexed_conditional_meaning_sentence_cache$, () -> ( dictionary.new_dictionary( Symbols.symbol_function( EQ ),
            UNPROVIDED ) ) ) );
    $sksi_modules_by_sks$ = SubLFiles.deflexical( "*SKSI-MODULES-BY-SKS*", maybeDefault( $sym10$_SKSI_MODULES_BY_SKS_, $sksi_modules_by_sks$, () -> ( dictionary.new_dictionary( Symbols.symbol_function( EQ ),
        UNPROVIDED ) ) ) );
    $sksi_code_mapping_sks_cache$ = SubLFiles.deflexical( "*SKSI-CODE-MAPPING-SKS-CACHE*", maybeDefault( $sym13$_SKSI_CODE_MAPPING_SKS_CACHE_, $sksi_code_mapping_sks_cache$, () -> ( dictionary.new_dictionary( Symbols
        .symbol_function( EQ ), UNPROVIDED ) ) ) );
    $isa_sks_term_to_cycl_term_mts$ = SubLFiles.deflexical( "*ISA-SKS-TERM-TO-CYCL-TERM-MTS*", ( maybeDefault( $sym28$_ISA_SKS_TERM_TO_CYCL_TERM_MTS_, $isa_sks_term_to_cycl_term_mts$, NIL ) ) );
    $isa_sks_term_to_cycl_term_store$ = SubLFiles.deflexical( "*ISA-SKS-TERM-TO-CYCL-TERM-STORE*", ( maybeDefault( $sym30$_ISA_SKS_TERM_TO_CYCL_TERM_STORE_, $isa_sks_term_to_cycl_term_store$, NIL ) ) );
    $filtered_sksi_preds$ = SubLFiles.deflexical( "*FILTERED-SKSI-PREDS*", $list34 );
    $meaning_sentence_mts$ = SubLFiles.deflexical( "*MEANING-SENTENCE-MTS*", ( maybeDefault( $sym35$_MEANING_SENTENCE_MTS_, $meaning_sentence_mts$, NIL ) ) );
    $meaning_sentence_pred_store$ = SubLFiles.deflexical( "*MEANING-SENTENCE-PRED-STORE*", ( maybeDefault( $sym36$_MEANING_SENTENCE_PRED_STORE_, $meaning_sentence_pred_store$, NIL ) ) );
    $sks_preference_modules_count$ = SubLFiles.defparameter( "*SKS-PREFERENCE-MODULES-COUNT*", NIL );
    $sks_prune_modules_count$ = SubLFiles.defparameter( "*SKS-PRUNE-MODULES-COUNT*", NIL );
    $sksi_specific_module_properties$ = SubLFiles.deflexical( "*SKSI-SPECIFIC-MODULE-PROPERTIES*", $list46 );
    $generate_sksi_slrm_required_function_caching_state$ = SubLFiles.deflexical( "*GENERATE-SKSI-SLRM-REQUIRED-FUNCTION-CACHING-STATE*", NIL );
    $sksi_default_check_cost$ = SubLFiles.deflexical( "*SKSI-DEFAULT-CHECK-COST*", control_vars.$expensive_hl_module_check_cost$.getGlobalValue() );
    $virtual_assertion_count$ = SubLFiles.deflexical( "*VIRTUAL-ASSERTION-COUNT*", ( maybeDefault( $sym204$_VIRTUAL_ASSERTION_COUNT_, $virtual_assertion_count$, ZERO_INTEGER ) ) );
    $mt_virtual_assertion_count_table$ = SubLFiles.deflexical( "*MT-VIRTUAL-ASSERTION-COUNT-TABLE*", maybeDefault( $sym205$_MT_VIRTUAL_ASSERTION_COUNT_TABLE_, $mt_virtual_assertion_count_table$, () -> ( Hashtables
        .make_hash_table( SIXTEEN_INTEGER, UNPROVIDED, UNPROVIDED ) ) ) );
    $sksi_module_code_mapping_terms_cache$ = SubLFiles.deflexical( "*SKSI-MODULE-CODE-MAPPING-TERMS-CACHE*", maybeDefault( $sym206$_SKSI_MODULE_CODE_MAPPING_TERMS_CACHE_, $sksi_module_code_mapping_terms_cache$,
        () -> ( dictionary.new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED ) ) ) );
    $sksi_module_logical_schemas_cache$ = SubLFiles.deflexical( "*SKSI-MODULE-LOGICAL-SCHEMAS-CACHE*", maybeDefault( $sym207$_SKSI_MODULE_LOGICAL_SCHEMAS_CACHE_, $sksi_module_logical_schemas_cache$, () -> ( dictionary
        .new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED ) ) ) );
    $sksi_supported_gaf_argument_browsing_max_term_cutoff$ = SubLFiles.defparameter( "*SKSI-SUPPORTED-GAF-ARGUMENT-BROWSING-MAX-TERM-CUTOFF*", $int210$5000 );
    $show_sksi_supported_gaf_argument_browsing_skipsP$ = SubLFiles.defparameter( "*SHOW-SKSI-SUPPORTED-GAF-ARGUMENT-BROWSING-SKIPS?*", NIL );
    return NIL;
  }

  public static SubLObject setup_sksi_removal_module_generation_file()
  {
    subl_macro_promotions.declare_defglobal( $sym0$_SKSI_MODULES_ );
    subl_macro_promotions.declare_defglobal( $sym6$_SKSI_MODULE_SUPPORTS_ );
    subl_macro_promotions.declare_defglobal( $sym7$_SKSI_MODULE_SIMPLE_REQUIRED_PATTERN_ );
    subl_macro_promotions.declare_defglobal( $sym8$_SKS_MULTIPLY_INDEXED_CONDITIONAL_MEANING_SENTENCE_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym9$_SKSI_MODULE_MULTIPLY_INDEXED_CONDITIONAL_MEANING_SENTENCE_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym10$_SKSI_MODULES_BY_SKS_ );
    subl_macro_promotions.declare_defglobal( $sym13$_SKSI_CODE_MAPPING_SKS_CACHE_ );
    memoization_state.note_memoized_function( $sym20$CODE_MAPPING_FORWARD_RULES_TO_REPROPAGATE );
    subl_macro_promotions.declare_defglobal( $sym28$_ISA_SKS_TERM_TO_CYCL_TERM_MTS_ );
    subl_macro_promotions.declare_defglobal( $sym30$_ISA_SKS_TERM_TO_CYCL_TERM_STORE_ );
    memoization_state.note_memoized_function( $sym31$ALL_SKSI_RELEVANT_REWRITE_SPECS_FOR_TERM_INT );
    memoization_state.note_memoized_function( $sym32$ALL_SKSI_RELEVANT_REWRITE_ISAS_INT );
    subl_macro_promotions.declare_defglobal( $sym35$_MEANING_SENTENCE_MTS_ );
    subl_macro_promotions.declare_defglobal( $sym36$_MEANING_SENTENCE_PRED_STORE_ );
    memoization_state.note_memoized_function( $sym37$ALL_SKSI_RELEVANT_REWRITE_SPEC_PREDS_FOR_PRED_INT );
    memoization_state.note_memoized_function( $sym38$ALL_SKSI_RELEVANT_REWRITE_SPEC_INVERSES_FOR_PRED_INT );
    memoization_state.note_memoized_function( $sym70$SKSI_GET_ANY_PREDICATES_PATTERN );
    memoization_state.note_memoized_function( $sym106$NON_SENTENTIAL_RELATION_FORT_P );
    memoization_state.note_globally_cached_function( $sym115$GENERATE_SKSI_SLRM_REQUIRED_FUNCTION );
    memoization_state.note_memoized_function( $sym123$SKSI_ONLY_PREDICATE_ );
    utilities_macros.note_funcall_helper_function( $sym173$SKSI_REFORMULATABLE_VIA_TEMPLATE_ );
    subl_macro_promotions.declare_defglobal( $sym204$_VIRTUAL_ASSERTION_COUNT_ );
    subl_macro_promotions.declare_defglobal( $sym205$_MT_VIRTUAL_ASSERTION_COUNT_TABLE_ );
    subl_macro_promotions.declare_defglobal( $sym206$_SKSI_MODULE_CODE_MAPPING_TERMS_CACHE_ );
    subl_macro_promotions.declare_defglobal( $sym207$_SKSI_MODULE_LOGICAL_SCHEMAS_CACHE_ );
    memoization_state.note_memoized_function( $sym208$SKSI_MODULE_ANY_PREDICATES_LIST );
    memoization_state.note_memoized_function( $sym214$GATHER_CYCL_TERMS_MAPPED_TO_BY_CODE_MAPPING_SCHEMAS );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_removal_module_generation();
    $sksi_modules$ = null;
    $sksi_browsing_enabled$ = null;
    $sksi_module_supports$ = null;
    $sksi_module_simple_required_pattern$ = null;
    $sks_multiply_indexed_conditional_meaning_sentence_cache$ = null;
    $sksi_module_multiply_indexed_conditional_meaning_sentence_cache$ = null;
    $sksi_modules_by_sks$ = null;
    $sksi_code_mapping_sks_cache$ = null;
    $isa_sks_term_to_cycl_term_mts$ = null;
    $isa_sks_term_to_cycl_term_store$ = null;
    $filtered_sksi_preds$ = null;
    $meaning_sentence_mts$ = null;
    $meaning_sentence_pred_store$ = null;
    $sks_preference_modules_count$ = null;
    $sks_prune_modules_count$ = null;
    $sksi_specific_module_properties$ = null;
    $generate_sksi_slrm_required_function_caching_state$ = null;
    $sksi_default_check_cost$ = null;
    $virtual_assertion_count$ = null;
    $mt_virtual_assertion_count_table$ = null;
    $sksi_module_code_mapping_terms_cache$ = null;
    $sksi_module_logical_schemas_cache$ = null;
    $sksi_supported_gaf_argument_browsing_max_term_cutoff$ = null;
    $show_sksi_supported_gaf_argument_browsing_skipsP$ = null;
    $sym0$_SKSI_MODULES_ = makeSymbol( "*SKSI-MODULES*" );
    $list1 = ConsesLow.list( ConsesLow.list( makeSymbol( "HL-MODULE" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym2$DO_SET = makeSymbol( "DO-SET" );
    $list3 = ConsesLow.list( makeSymbol( "*SKSI-MODULES*" ) );
    $sym4$CDOLIST = makeSymbol( "CDOLIST" );
    $list5 = ConsesLow.list( ConsesLow.list( makeSymbol( "SET-ELEMENT-LIST" ), makeSymbol( "*SKSI-MODULES*" ) ) );
    $sym6$_SKSI_MODULE_SUPPORTS_ = makeSymbol( "*SKSI-MODULE-SUPPORTS*" );
    $sym7$_SKSI_MODULE_SIMPLE_REQUIRED_PATTERN_ = makeSymbol( "*SKSI-MODULE-SIMPLE-REQUIRED-PATTERN*" );
    $sym8$_SKS_MULTIPLY_INDEXED_CONDITIONAL_MEANING_SENTENCE_CACHE_ = makeSymbol( "*SKS-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-CACHE*" );
    $sym9$_SKSI_MODULE_MULTIPLY_INDEXED_CONDITIONAL_MEANING_SENTENCE_CACHE_ = makeSymbol( "*SKSI-MODULE-MULTIPLY-INDEXED-CONDITIONAL-MEANING-SENTENCE-CACHE*" );
    $sym10$_SKSI_MODULES_BY_SKS_ = makeSymbol( "*SKSI-MODULES-BY-SKS*" );
    ...
    $sym13$_SKSI_CODE_MAPPING_SKS_CACHE_ = makeSymbol( "*SKSI-CODE-MAPPING-SKS-CACHE*" );
    $list14 = ConsesLow.list( ConsesLow.list( makeSymbol( "SKS" ), makeSymbol( "PS" ), makeSymbol( "LS" ), makeSymbol( "MAPPING-MT" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym15$TUPLE = makeUninternedSymbol( "TUPLE" );
    $sym16$TUPLES = makeUninternedSymbol( "TUPLES" );
    $sym17$DO_DICTIONARY = makeSymbol( "DO-DICTIONARY" );
    $list18 = ConsesLow.list( makeSymbol( "*SKSI-CODE-MAPPING-SKS-CACHE*" ) );
    $sym19$CDESTRUCTURING_BIND = makeSymbol( "CDESTRUCTURING-BIND" );
    $sym20$CODE_MAPPING_FORWARD_RULES_TO_REPROPAGATE = makeSymbol( "CODE-MAPPING-FORWARD-RULES-TO-REPROPAGATE" );
    $const21$codeMapping = constant_handles.reader_make_constant_shell( makeString( "codeMapping" ) );
    $kw22$NEG = makeKeyword( "NEG" );
    $const23$SKSIMt = constant_handles.reader_make_constant_shell( makeString( "SKSIMt" ) );
    $kw24$FORWARD = makeKeyword( "FORWARD" );
    ...
    $sym28$_ISA_SKS_TERM_TO_CYCL_TERM_MTS_ = makeSymbol( "*ISA-SKS-TERM-TO-CYCL-TERM-MTS*" );
    $sym29$HLMT_EQUAL = makeSymbol( "HLMT-EQUAL" );
    $sym30$_ISA_SKS_TERM_TO_CYCL_TERM_STORE_ = makeSymbol( "*ISA-SKS-TERM-TO-CYCL-TERM-STORE*" );
    $sym31$ALL_SKSI_RELEVANT_REWRITE_SPECS_FOR_TERM_INT = makeSymbol( "ALL-SKSI-RELEVANT-REWRITE-SPECS-FOR-TERM-INT" );
    $sym32$ALL_SKSI_RELEVANT_REWRITE_ISAS_INT = makeSymbol( "ALL-SKSI-RELEVANT-REWRITE-ISAS-INT" );
    $list33 = ConsesLow.list( makeSymbol( "ARG-POS" ), makeSymbol( "TERMS" ) );
    $list34 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "holdsIn" ) ) );
    $sym35$_MEANING_SENTENCE_MTS_ = makeSymbol( "*MEANING-SENTENCE-MTS*" );
    $sym36$_MEANING_SENTENCE_PRED_STORE_ = makeSymbol( "*MEANING-SENTENCE-PRED-STORE*" );
    $sym37$ALL_SKSI_RELEVANT_REWRITE_SPEC_PREDS_FOR_PRED_INT = makeSymbol( "ALL-SKSI-RELEVANT-REWRITE-SPEC-PREDS-FOR-PRED-INT" );
    $sym38$ALL_SKSI_RELEVANT_REWRITE_SPEC_INVERSES_FOR_PRED_INT = makeSymbol( "ALL-SKSI-RELEVANT-REWRITE-SPEC-INVERSES-FOR-PRED-INT" );
    $sym39$CODE_MAPPING_KNOWLEDGE_SOURCE_ = makeSymbol( "CODE-MAPPING-KNOWLEDGE-SOURCE?" );
    $list40 = ConsesLow.list( makeSymbol( "HL-MODULE-INFO-PLIST" ), makeSymbol( "HL-MODULE-RELEVANT-CODE-MAPPING-SCHEMAS" ), makeSymbol( "HL-MODULE-RELEVANT-INSTANCE-NAMED-FN-COLLS" ) );
    $kw41$MODULE_SUPPORTS = makeKeyword( "MODULE-SUPPORTS" );
    $kw42$SIMPLE_REQUIRED_PATTERN = makeKeyword( "SIMPLE-REQUIRED-PATTERN" );
    $kw43$MEANING_SENTENCE = makeKeyword( "MEANING-SENTENCE" );
    $kw44$MULTIPLY_INDEXED_CMS = makeKeyword( "MULTIPLY-INDEXED-CMS" );
    $list45 = ConsesLow.list( makeSymbol( "PS" ), makeSymbol( "LS" ), makeSymbol( "MAPPING-MT" ) );
    $list46 = ConsesLow.list( makeKeyword( "MODULE-NAME" ), makeKeyword( "SIMPLE-REQUIRED-PATTERN" ), makeKeyword( "MODULE-SUPPORTS" ), makeKeyword( "INPUT-TERMNUMS" ), makeKeyword( "OUTPUT-TERMNUMS" ), makeKeyword(
        "MULTIPLY-INDEXED-CMS" ), makeKeyword( "MEANING-SENTENCE" ) );
    $str47$_S_was_not_a_valid_module_paramet = makeString( "~S was not a valid module parameter" );
    $kw48$MODULE_NAME = makeKeyword( "MODULE-NAME" );
    $kw49$DOCUMENTATION = makeKeyword( "DOCUMENTATION" );
    $kw50$SENSE = makeKeyword( "SENSE" );
    $str51$sksi_ = makeString( "sksi-" );
    $sym52$CATCH_ERROR_MESSAGE_HANDLER = makeSymbol( "CATCH-ERROR-MESSAGE-HANDLER" );
    $str53$Invalid_predicate_for_meaning_sen = makeString( "Invalid predicate for meaning sentence GAF: ~a." );
    $str54$complex_non_atomic_meaning_senten = makeString( "complex non-atomic meaning sentences not handled yet" );
    $sym55$LOGICAL_FIELD_INDEXICAL_P = makeSymbol( "LOGICAL-FIELD-INDEXICAL-P" );
    $str56$No_logical_field_indexicals_for_m = makeString( "No logical field indexicals for meaning sentence ~a in logical schemata ~a." );
    $str57$The_following_conditional_meaning = makeString( "The following conditional meaning sentence GAF is not supported: ~%~a" );
    $str58$Invalid_consequent_predicate_for_ = makeString( "Invalid consequent predicate for conditional meaning sentence GAF: ~a." );
    $sym59$PHYSICAL_SCHEMA_SK_SOURCES_MEMOIZED = makeSymbol( "PHYSICAL-SCHEMA-SK-SOURCES-MEMOIZED" );
    $sym60$LOGICAL_SCHEMA_PHYSICAL_SCHEMATA = makeSymbol( "LOGICAL-SCHEMA-PHYSICAL-SCHEMATA" );
    $kw61$OUTPUT_GENERATE_PATTERN = makeKeyword( "OUTPUT-GENERATE-PATTERN" );
    $kw62$OUTPUT_CHECK_PATTERN = makeKeyword( "OUTPUT-CHECK-PATTERN" );
    $kw63$REQUIRED_MT = makeKeyword( "REQUIRED-MT" );
    $kw64$SUPPORT_MT = makeKeyword( "SUPPORT-MT" );
    $str65$No__output_generate_pattern_or__o = makeString( "No :output-generate-pattern or :output-check-pattern." );
    $sym66$CSQL_FROM_CLAUSE_P = makeSymbol( "CSQL-FROM-CLAUSE-P" );
    $sym67$CSQL_WHERE_CLAUSE_P = makeSymbol( "CSQL-WHERE-CLAUSE-P" );
    $kw68$WHERE = makeKeyword( "WHERE" );
    $sym69$CSQL_SELECT_QUERY_P = makeSymbol( "CSQL-SELECT-QUERY-P" );
    $sym70$SKSI_GET_ANY_PREDICATES_PATTERN = makeSymbol( "SKSI-GET-ANY-PREDICATES-PATTERN" );
    $const71$SchemaObjectFn = constant_handles.reader_make_constant_shell( makeString( "SchemaObjectFn" ) );
    $list72 = ConsesLow.list( makeKeyword( "ANYTHING" ) );
    $const73$SourceSchemaObjectFn = constant_handles.reader_make_constant_shell( makeString( "SourceSchemaObjectFn" ) );
    $sym74$LFI_ALIAS_FN_NAUT_P = makeSymbol( "LFI-ALIAS-FN-NAUT-P" );
    $sym75$LOGICAL_FIELD_INDEXICAL_FORT_P = makeSymbol( "LOGICAL-FIELD-INDEXICAL-FORT-P" );
    $kw76$FULLY_BOUND = makeKeyword( "FULLY-BOUND" );
    $const77$evaluate = constant_handles.reader_make_constant_shell( makeString( "evaluate" ) );
    $sym78$NON_LOGICAL_FIELD_INDEXICAL_P = makeSymbol( "NON-LOGICAL-FIELD-INDEXICAL-P" );
    $kw79$IGNORE = makeKeyword( "IGNORE" );
    $const80$sksiModulesDoomedUnlessArgBound = constant_handles.reader_make_constant_shell( makeString( "sksiModulesDoomedUnlessArgBound" ) );
    $kw81$NOT_FULLY_BOUND = makeKeyword( "NOT-FULLY-BOUND" );
    $sym82$PHYSICAL_SCHEMA_ENUMERABLE_ = makeSymbol( "PHYSICAL-SCHEMA-ENUMERABLE?" );
    $list83 = ConsesLow.list( ConsesLow.list( ZERO_INTEGER ) );
    $kw84$POS = makeKeyword( "POS" );
    $list85 = ConsesLow.list( makeSymbol( "TUPLE" ), makeSymbol( "INPUT-TERMNUMS" ), makeSymbol( "OUTPUT-TERMNUMS" ) );
    $str86$Could_not_constrain_the__not_full = makeString( "Could not constrain the :not-fully-bound predicate for ~a" );
    $kw87$DISALLOWED = makeKeyword( "DISALLOWED" );
    $kw88$INPUT_TERMNUMS = makeKeyword( "INPUT-TERMNUMS" );
    $kw89$OUTPUT_TERMNUMS = makeKeyword( "OUTPUT-TERMNUMS" );
    $kw90$PREDICATE = makeKeyword( "PREDICATE" );
    $kw91$ANY_PREDICATES = makeKeyword( "ANY-PREDICATES" );
    $kw92$REQUIRED_PATTERN = makeKeyword( "REQUIRED-PATTERN" );
    $sym93$FIRST = makeSymbol( "FIRST" );
    $list94 = ConsesLow.list( makeKeyword( "FULLY-BOUND" ), makeKeyword( "NOT-FULLY-BOUND" ) );
    $list95 = ConsesLow.list( makeKeyword( "FULLY-BOUND" ) );
    $sym96$CAR = makeSymbol( "CAR" );
    $list97 = ConsesLow.list( makeSymbol( "ARG-POS-LIST" ), makeSymbol( "VALUE" ) );
    $sym98$NART_P = makeSymbol( "NART-P" );
    $sym99$MAKE_NAT_SUBPATTERN = makeSymbol( "MAKE-NAT-SUBPATTERN" );
    $kw100$NAT = makeKeyword( "NAT" );
    $kw101$OR = makeKeyword( "OR" );
    $list102 = ConsesLow.list( makeKeyword( "NOT-FULLY-BOUND" ) );
    $kw103$UNIFY = makeKeyword( "UNIFY" );
    $sym104$NAUT_ = makeSymbol( "NAUT?" );
    $sym105$MAKE_UNIFY_SUBPATTERN = makeSymbol( "MAKE-UNIFY-SUBPATTERN" );
    $sym106$NON_SENTENTIAL_RELATION_FORT_P = makeSymbol( "NON-SENTENTIAL-RELATION-FORT-P" );
    $sym107$NAT_SUBPATTERN_P = makeSymbol( "NAT-SUBPATTERN-P" );
    $sym108$MAKE_DISJUNCTIVE_SUBPATTERN = makeSymbol( "MAKE-DISJUNCTIVE-SUBPATTERN" );
    $const109$SententialRelation = constant_handles.reader_make_constant_shell( makeString( "SententialRelation" ) );
    $sym110$UNIFY_SUBPATTERN_P = makeSymbol( "UNIFY-SUBPATTERN-P" );
    $sym111$SECOND = makeSymbol( "SECOND" );
    $sym112$DISJUNCTIVE_SUBPATTERN_P = makeSymbol( "DISJUNCTIVE-SUBPATTERN-P" );
    $sym113$NARTIFY_NAT_PATTERN = makeSymbol( "NARTIFY-NAT-PATTERN" );
    $kw114$ANYTHING = makeKeyword( "ANYTHING" );
    $sym115$GENERATE_SKSI_SLRM_REQUIRED_FUNCTION = makeSymbol( "GENERATE-SKSI-SLRM-REQUIRED-FUNCTION" );
    $sym116$DEFINE_PRIVATE = makeSymbol( "DEFINE-PRIVATE" );
    $list117 = ConsesLow.list( makeSymbol( "ASENT" ), makeSymbol( "SENSE" ) );
    $list118 = ConsesLow.list( makeSymbol( "IGNORE" ), makeSymbol( "ASENT" ), makeSymbol( "SENSE" ) );
    $sym119$RET = makeSymbol( "RET" );
    $sym120$EXTERNAL_SOURCE_ACTIVATED_IN_IMAGE_ = makeSymbol( "EXTERNAL-SOURCE-ACTIVATED-IN-IMAGE?" );
    $sym121$_GENERATE_SKSI_SLRM_REQUIRED_FUNCTION_CACHING_STATE_ = makeSymbol( "*GENERATE-SKSI-SLRM-REQUIRED-FUNCTION-CACHING-STATE*" );
    $str122$_REQUIRED_FUNC = makeString( "-REQUIRED-FUNC" );
    $sym123$SKSI_ONLY_PREDICATE_ = makeSymbol( "SKSI-ONLY-PREDICATE?" );
    $const124$SKSIOnlyPredicate = constant_handles.reader_make_constant_shell( makeString( "SKSIOnlyPredicate" ) );
    $int125$1024 = makeInteger( 1024 );
    $kw126$SKSI = makeKeyword( "SKSI" );
    $kw127$DEFAULT = makeKeyword( "DEFAULT" );
    $kw128$DISPREFERRED = makeKeyword( "DISPREFERRED" );
    $str129$_prune_unreformulatable = makeString( "-prune-unreformulatable" );
    $kw130$AND = makeKeyword( "AND" );
    $kw131$MODULE_TYPE = makeKeyword( "MODULE-TYPE" );
    $kw132$REMOVAL = makeKeyword( "REMOVAL" );
    $kw133$MODULE_SUBTYPE = makeKeyword( "MODULE-SUBTYPE" );
    $list134 = ConsesLow.list( makeKeyword( "SKSI" ) );
    $kw135$MODULE_SOURCE = makeKeyword( "MODULE-SOURCE" );
    $kw136$CHECK = makeKeyword( "CHECK" );
    $kw137$DIRECTION = makeKeyword( "DIRECTION" );
    $kw138$EXCLUSIVE = makeKeyword( "EXCLUSIVE" );
    $sym139$TRUE = makeSymbol( "TRUE" );
    $kw140$SUPPLANTS = makeKeyword( "SUPPLANTS" );
    $list141 = ConsesLow.list( makeKeyword( "NOT" ), ConsesLow.list( makeKeyword( "MODULE-SUBTYPE" ), makeKeyword( "SKSI" ) ) );
    $kw142$COMPLETENESS = makeKeyword( "COMPLETENESS" );
    $list143 = ConsesLow.list( makeKeyword( "COST-EXPRESSION" ), ZERO_INTEGER );
    $kw144$REQUIRED = makeKeyword( "REQUIRED" );
    $list145 = ConsesLow.list( ConsesLow.list( makeKeyword( "NOT" ), ConsesLow.list( makeKeyword( "MODULE-SUBTYPE" ), makeKeyword( "SKSI" ) ) ) );
    $kw146$INPUT_EXTRACT_PATTERN = makeKeyword( "INPUT-EXTRACT-PATTERN" );
    $kw147$INPUT_ENCODE_PATTERN = makeKeyword( "INPUT-ENCODE-PATTERN" );
    $kw148$INPUT_VERIFY_PATTERN = makeKeyword( "INPUT-VERIFY-PATTERN" );
    $kw149$OUTPUT_DECODE_PATTERN = makeKeyword( "OUTPUT-DECODE-PATTERN" );
    $kw150$OUTPUT_VERIFY_PATTERN = makeKeyword( "OUTPUT-VERIFY-PATTERN" );
    $kw151$OUTPUT_CONSTRUCT_PATTERN = makeKeyword( "OUTPUT-CONSTRUCT-PATTERN" );
    $kw152$COST_EXPRESSION = makeKeyword( "COST-EXPRESSION" );
    $kw153$COST_PATTERN = makeKeyword( "COST-PATTERN" );
    $kw154$SUPPORT_MODULE = makeKeyword( "SUPPORT-MODULE" );
    $kw155$SUPPORT_STRENGTH = makeKeyword( "SUPPORT-STRENGTH" );
    $kw156$COMPLETE = makeKeyword( "COMPLETE" );
    $kw157$INCOMPLETE = makeKeyword( "INCOMPLETE" );
    $str158$___not_ = makeString( "(#$not " );
    $str159$_ = makeString( ")" );
    $str160$_FULLY_BOUND = makeString( " FULLY-BOUND" );
    $str161$__FULLY_BOUND_ = makeString( " <FULLY-BOUND>" );
    $str162$_FULLY_BOUND = makeString( "(FULLY-BOUND" );
    $str163$__FULLY_BOUND_ = makeString( "(<FULLY-BOUND>" );
    $str164$NOT_FULLY_BOUND = makeString( "NOT-FULLY-BOUND" );
    $str165$_NOT_FULLY_BOUND_ = makeString( "<NOT-FULLY-BOUND>" );
    $kw166$BIND = makeKeyword( "BIND" );
    $kw167$VALUE = makeKeyword( "VALUE" );
    $kw168$TEMPLATE = makeKeyword( "TEMPLATE" );
    $list169 = ConsesLow.list( makeKeyword( "TEST" ), makeSymbol( "SKSI-REFORMULATION-CHECK" ) );
    $list170 = ConsesLow.list( makeKeyword( "TEST" ), makeSymbol( "SKSI-NOT-CONTAINING-NIL-P" ) );
    $sym171$TOKEN_CYCL_SYMBOL_FOR_TERMNUM = makeSymbol( "TOKEN-CYCL-SYMBOL-FOR-TERMNUM" );
    $kw172$TUPLE = makeKeyword( "TUPLE" );
    $sym173$SKSI_REFORMULATABLE_VIA_TEMPLATE_ = makeSymbol( "SKSI-REFORMULATABLE-VIA-TEMPLATE?" );
    $str174$No_field_encodings_for_physical_f = makeString( "No field encodings for physical field indexical ~a, physical schemata ~a, and logical schemata ~a." );
    $kw175$NOT = makeKeyword( "NOT" );
    $kw176$TEST = makeKeyword( "TEST" );
    $kw177$ENCODE = makeKeyword( "ENCODE" );
    $kw178$CALL = makeKeyword( "CALL" );
    $sym179$SKSI_REFORMULATE = makeSymbol( "SKSI-REFORMULATE" );
    $str180$No_field_decodings_for_logical_fi = makeString( "No field decodings for logical field indexical ~a, logical schemata ~a, and physical schemata ~a." );
    $kw181$DECODE = makeKeyword( "DECODE" );
    $sym182$DUMMY_EXTERNAL_TERM_DENOTING_FUNCTION_EXPRESSION_P = makeSymbol( "DUMMY-EXTERNAL-TERM-DENOTING-FUNCTION-EXPRESSION-P" );
    $sym183$CITY_NAMED_FN_EXPRESSION_P = makeSymbol( "CITY-NAMED-FN-EXPRESSION-P" );
    $sym184$SCHEMA_OBJECT_FN_EXPRESSION_P = makeSymbol( "SCHEMA-OBJECT-FN-EXPRESSION-P" );
    $sym185$SOURCE_SCHEMA_OBJECT_FN_EXPRESSION_P = makeSymbol( "SOURCE-SCHEMA-OBJECT-FN-EXPRESSION-P" );
    $sym186$INSTANCE_NAMED_FN_EXPRESSION_P = makeSymbol( "INSTANCE-NAMED-FN-EXPRESSION-P" );
    $const187$City = constant_handles.reader_make_constant_shell( makeString( "City" ) );
    $sym188$GET_SKSI_REMOVAL_MODULE_COST = makeSymbol( "GET-SKSI-REMOVAL-MODULE-COST" );
    $kw189$INPUT = makeKeyword( "INPUT" );
    $sym190$LOGICAL_FIELD_FOR_INDEXICAL = makeSymbol( "LOGICAL-FIELD-FOR-INDEXICAL" );
    $sym191$FAST_SUBSET_ = makeSymbol( "FAST-SUBSET?" );
    $sym192$SETS_EQUAL_ = makeSymbol( "SETS-EQUAL?" );
    $sym193$FAST_SUPERSET_ = makeSymbol( "FAST-SUPERSET?" );
    $sym194$GENERATE_SKSI_SINGLE_LITERAL_COST_RECORDING_ITERATOR = makeSymbol( "GENERATE-SKSI-SINGLE-LITERAL-COST-RECORDING-ITERATOR" );
    $sym195$GENERATE_BOOLEAN_FROM_CSQL = makeSymbol( "GENERATE-BOOLEAN-FROM-CSQL" );
    $sym196$NOT_BOOLEAN = makeSymbol( "NOT-BOOLEAN" );
    $sym197$PHYSICAL_SCHEMA_REQUIRED_FIELDS = makeSymbol( "PHYSICAL-SCHEMA-REQUIRED-FIELDS" );
    $list198 = ConsesLow.list( makeSymbol( "SELECT-KEYWORD" ), makeSymbol( "FIELDS" ), makeSymbol( "FROM" ), makeSymbol( "&OPTIONAL" ), makeSymbol( "WHERE" ) );
    $list199 = ConsesLow.list( makeSymbol( "WHERE-KEYWORD" ), makeSymbol( "WHERE-CLAUSE" ) );
    $str200$CYCL_TERM = makeString( "CYCL-TERM" );
    $str201$RAW_TERM = makeString( "RAW-TERM" );
    $str202$RAW = makeString( "RAW" );
    $str203$CYCL = makeString( "CYCL" );
    $sym204$_VIRTUAL_ASSERTION_COUNT_ = makeSymbol( "*VIRTUAL-ASSERTION-COUNT*" );
    $sym205$_MT_VIRTUAL_ASSERTION_COUNT_TABLE_ = makeSymbol( "*MT-VIRTUAL-ASSERTION-COUNT-TABLE*" );
    $sym206$_SKSI_MODULE_CODE_MAPPING_TERMS_CACHE_ = makeSymbol( "*SKSI-MODULE-CODE-MAPPING-TERMS-CACHE*" );
    $sym207$_SKSI_MODULE_LOGICAL_SCHEMAS_CACHE_ = makeSymbol( "*SKSI-MODULE-LOGICAL-SCHEMAS-CACHE*" );
    $sym208$SKSI_MODULE_ANY_PREDICATES_LIST = makeSymbol( "SKSI-MODULE-ANY-PREDICATES-LIST" );
    $sym209$SKSI_SUPPORTED_PREDICATE_TYPE_P = makeSymbol( "SKSI-SUPPORTED-PREDICATE-TYPE-P" );
    $int210$5000 = makeInteger( 5000 );
    $sym211$FORT_P = makeSymbol( "FORT-P" );
    $str212$Too_many_term_arguments___S__for_ = makeString( "Too many term arguments (~S) for meaning-sentence ~S to register browsing on all of them.  Skipping." );
    $str213$Too_many_code_mapping_terms___S__ = makeString( "Too many code mapping terms (~S) for meaning-sentence ~S to register browsing on all of them.  Skipping." );
    $sym214$GATHER_CYCL_TERMS_MAPPED_TO_BY_CODE_MAPPING_SCHEMAS = makeSymbol( "GATHER-CYCL-TERMS-MAPPED-TO-BY-CODE-MAPPING-SCHEMAS" );
    $list215 = ConsesLow.list( makeSymbol( "ARG-POS" ), makeSymbol( "CODE-MAPPING-SCHEMAS" ) );
    $list216 = ConsesLow.list( makeSymbol( "ARG-POS" ), makeSymbol( "COLLS" ) );
    $str217$Continue_without_generating_remov = makeString( "Continue without generating removal modules for " );
  }
}
/*
 * 
