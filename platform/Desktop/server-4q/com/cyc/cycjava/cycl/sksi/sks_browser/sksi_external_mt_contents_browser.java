...
public final class sksi_external_mt_contents_browser
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject cb_link_sksi_external_mt_contents(final SubLObject mt)
  {
    ...
    PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $str1$sksi_external_mt_contents__A, cb_utilities.cb_index_identifier( mt ) );
    html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
    ...
    try
    {
      ...
      html_utilities.html_princ( $str2$External_Microtheory_Contents );
    }
    finally
    {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == current )
      {
        if( NIL != sksi_external_browser_utilities.count_not_too_expensive_for_browser_supportP( count ) )
        {
          ...
          final SubLObject frame_name_var2 = cb_utilities.cb_frame_name( $kw0$CONTENT );
          html_utilities.html_markup( html_macros.$html_anchor_head$.getGlobalValue() );
          html_utilities.html_markup( html_macros.$html_anchor_href$.getGlobalValue() );
          html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
          cb_parameters.cyc_cgi_url_int();
          ...
          if( NIL != predicate )
          {
            PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $str4$__a, cb_utilities.cb_fort_identifier( predicate ) );
          }
          html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
          ...
          {
            PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $str6$__N_A_ );
          }
        }
      }
      else
  ...
  public static SubLObject sksi_external_mt_contents(final SubLObject args)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( args, args, $list9 );
    index_spec = args.first();
    SubLObject current = args.rest();
    final SubLObject pred_spec = current.isCons() ? current.first() : NIL;
    cdestructuring_bind.destructuring_bind_must_listp( current, args, $list9 );
    current = current.rest();
    if( NIL == current )
    {
      final SubLObject mt = cb_utilities.cb_guess_index( index_spec );
      SubLObject pred = NIL;
      if( NIL == cb_utilities.cb_index_hook_p( mt ) )
      {
        return cb_utilities.cb_error( $str10$Could_not_determine_an_index_hook, index_spec, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
      }
      if( NIL != pred_spec )
      {
        pred = cb_utilities.cb_guess_fort( pred_spec, UNPROVIDED );
        if( NIL == forts.fort_p( pred ) )
        {
          return cb_utilities.cb_error( $str11$Could_not_determine_a_term_from__, pred_spec, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
        }
      }
      final SubLObject title_var;
      final SubLObject title = title_var = Sequences.cconcatenate( $str12$Microthory_contents_for_, format_nil.format_nil_s_no_copy( mt ) );
      ...
      try
      {
        ...
        try
        {
          cyc_file_dependencies.$html_emitted_file_dependencies$.bind( ( cyc_file_dependencies.$html_emitted_file_dependencies$.getDynamicValue( thread ) == $kw15$UNINITIALIZED ) ? ConsesLow.list(
          ...
          try
          {
            ...
            try
            {
              ...
              try
              {
                ...
                html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
                html_utilities.html_princ( $str21$Refresh );
                html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
                html_utilities.html_markup( html_macros.$html_event_attribute_onclick$.getGlobalValue() );
                html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
                html_utilities.html_markup( $str22$reloadCurrentFrame__reloadFrameBu );
                html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
    ...
    {
      cdestructuring_bind.cdestructuring_bind_error( args, $list9 );
    }
    return NIL;
  }
  ...
  public static SubLObject sksi_external_mt_contents_internal(final SubLObject mt, final SubLObject pred)
  {
    ...
    try
    {
      ...
      html_utilities.html_princ( $str25$_Contents__External___ );
        ...
        try
        {
          ...
          try
          {
            ...
            try
            {
              ...
              if( new_or_reused == $kw26$NEW && NIL != memoization_state.memoization_state_p( pph_macros.$pph_memoization_state$.getDynamicValue( thread ) ) )
              {
  ...
  {
    SubLFiles.declareFunction( me, "cb_link_sksi_external_mt_contents", "CB-LINK-SKSI-EXTERNAL-MT-CONTENTS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_external_mt_contents", "SKSI-EXTERNAL-MT-CONTENTS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_external_mt_contents_internal", "SKSI-EXTERNAL-MT-CONTENTS-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_external_mt_contents_rec", "SKSI-EXTERNAL-MT-CONTENTS-REC", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_external_mt_contents_rec_pred", "SKSI-EXTERNAL-MT-CONTENTS-REC-PRED", 2, 0, false );
    return NIL;
  }
  ...
  {
    cb_utilities.setup_cb_link_method( $kw7$SKSI_EXTERNAL_MT_CONTENTS, $sym8$CB_LINK_SKSI_EXTERNAL_MT_CONTENTS, ONE_INTEGER );
    html_macros.note_cgi_handler_function( $sym23$SKSI_EXTERNAL_MT_CONTENTS, $kw24$HTML_HANDLER );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_external_mt_contents_browser();
    $kw0$CONTENT = makeKeyword( "CONTENT" );
    $str1$sksi_external_mt_contents__A = makeString( "sksi-external-mt-contents&~A" );
    $str2$External_Microtheory_Contents = makeString( "External Microtheory Contents" );
    $list3 = ConsesLow.list( makeSymbol( "PREDICATE" ), makeSymbol( "COUNT" ) );
    $str4$__a = makeString( "&~a" );
    $str5$___a_ = makeString( " (~a)" );
    $str6$__N_A_ = makeString( " (N/A)" );
    $kw7$SKSI_EXTERNAL_MT_CONTENTS = makeKeyword( "SKSI-EXTERNAL-MT-CONTENTS" );
    $sym8$CB_LINK_SKSI_EXTERNAL_MT_CONTENTS = makeSymbol( "CB-LINK-SKSI-EXTERNAL-MT-CONTENTS" );
    $list9 = ConsesLow.list( makeSymbol( "INDEX-SPEC" ), makeSymbol( "&OPTIONAL" ), makeSymbol( "PRED-SPEC" ) );
    $str10$Could_not_determine_an_index_hook = makeString( "Could not determine an index-hook from ~a" );
    $str11$Could_not_determine_a_term_from__ = makeString( "Could not determine a term from ~a" );
    $str12$Microthory_contents_for_ = makeString( "Microthory contents for " );
    ...
    $str21$Refresh = makeString( "Refresh" );
    $str22$reloadCurrentFrame__reloadFrameBu = makeString( "reloadCurrentFrame('reloadFrameButton');" );
    $sym23$SKSI_EXTERNAL_MT_CONTENTS = makeSymbol( "SKSI-EXTERNAL-MT-CONTENTS" );
    $kw24$HTML_HANDLER = makeKeyword( "HTML-HANDLER" );
    $str25$_Contents__External___ = makeString( " Contents (External) :" );
    $kw26$NEW = makeKeyword( "NEW" );
  }
}
/*
 * 
