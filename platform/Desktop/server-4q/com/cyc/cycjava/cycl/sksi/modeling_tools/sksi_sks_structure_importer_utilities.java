...
public final class sksi_sks_structure_importer_utilities
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject sks_find_or_create_ps(final SubLObject sks, final SubLObject physical_schemabase_mt)
  {
    ...
    if( NIL != ps )
    {
      ...
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const0$isa, ps, $const1$PhysicalSchema ), $const2$UniversalVocabularyMt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const3$physicalSchemaSourceMap, ps, sks ), physical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const4$comment, ps, Sequences.cconcatenate( $str5$A_physical_schema_for_, new SubLObject[] { format_nil.format_nil_a_no_copy( sks ), $str6$_
      } ) ), $const2$UniversalVocabularyMt );
    }
    return ps;
  }
  ...
  {
    return narts_high.find_nart( el_utilities.make_binary_nat_formula( $const9$PhysicalFieldFn, ps, field_name ) );
  }
  ...
  public static SubLObject sksi_sks_get_physical_schemas(final SubLObject sks)
  {
    ...
    try
    {
      ...
      result = kb_mapping_utilities.pred_values( sks, $const3$physicalSchemaSourceMap, TWO_INTEGER, ONE_INTEGER, UNPROVIDED );
    }
    finally
    {
  ...
  {
    SubLObject result = NIL;
    SubLObject cdolist_list_var = sksi_sks_accessors.sks_get( sks, $const12$subKS_Direct, UNPROVIDED );
    SubLObject sub_sks = NIL;
    sub_sks = cdolist_list_var.first();
    while ( NIL != cdolist_list_var)
    {
      final SubLObject physical_schema = sksi_sks_accessors.sks_fget( sub_sks, $const3$physicalSchemaSourceMap, UNPROVIDED );
      result = ConsesLow.cons( physical_schema, result );
  ...
  public static SubLObject sksi_physical_schema_get_physical_fields(final SubLObject physical_schema)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym10$RELEVANT_MT_IS_EVERYTHING, thread );
      mt_relevance_macros.$mt$.bind( $const11$EverythingPSC, thread );
      result = kb_mapping_utilities.pred_values( physical_schema, $const13$physicalFields, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      mt_relevance_macros.$mt$.rebind( _prev_bind_2, thread );
  ...
  public static SubLObject sksi_physical_schema_get_physical_field_indexicals(final SubLObject physical_schema)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym10$RELEVANT_MT_IS_EVERYTHING, thread );
      mt_relevance_macros.$mt$.bind( $const11$EverythingPSC, thread );
      result = kb_mapping_utilities.pred_values( physical_schema, $const14$physicalFieldIndexicals, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      mt_relevance_macros.$mt$.rebind( _prev_bind_2, thread );
  ...
  public static SubLObject sksi_sks_get_all_physical_schema_table_names(final SubLObject sks, SubLObject include_schemasP)
  {
    ...
    try
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          result = ConsesLow.cons( kb_mapping_utilities.pred_values( physical_schema, $const16$schemaName_SKS, ONE_INTEGER, TWO_INTEGER, UNPROVIDED ).first(), result );
        }
        else
        {
          result = ConsesLow.cons( ConsesLow.list( kb_mapping_utilities.pred_values( physical_schema, $const16$schemaName_SKS, ONE_INTEGER, TWO_INTEGER, UNPROVIDED ).first(), kb_mapping_utilities.pred_values(
              physical_schema, $const17$schemaNameSchema_SKS, ONE_INTEGER, TWO_INTEGER, UNPROVIDED ).first() ), result );
        }
        cdolist_list_var = cdolist_list_var.rest();
        sub_sks = cdolist_list_var.first();
      }
  ...
  public static SubLObject sks_initial_ke_validate_database(final SubLObject sks)
  {
    ...
    ke_plist = conses_high.putf( ke_plist, $const24$definingMt, ConsesLow.list( defining_mt ) );
    ...
    {
      return Values.values( ke_plist, NIL );
    }
    return Values.values( ke_plist, T );
  }
  ...
  public static SubLObject sks_gather_physical_database_table_names(final SubLObject sks, SubLObject include_schemasP, SubLObject schema_pattern)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          final SubLObject table_name = conses_high.getf( table_meta_data, $kw30$TABLE_NAME, UNPROVIDED );
          final SubLObject table_schema = conses_high.getf( table_meta_data, $kw31$TABLE_SCHEM, UNPROVIDED );
          if( NIL != include_schemasP )
          {
            table_names = ConsesLow.cons( ConsesLow.list( table_schema, table_name ), table_names );
          }
          else
          {
            table_names = ConsesLow.cons( table_name, table_names );
          }
          cdolist_list_var = cdolist_list_var.rest();
          table_meta_data = cdolist_list_var.first();
        }
  ...
  public static SubLObject sks_gather_asserted_table_meta_data_for_table(final SubLObject sks, final SubLObject table_name)
  {
    ...
    try
    {
      ...
      final SubLObject ps = ask_utilities.query_variable( $sym34$_PS, ConsesLow.listS( $const35$and, ConsesLow.listS( $const12$subKS_Direct, sks, $list36 ), ConsesLow.list( $const16$schemaName_SKS, $sym34$_PS,
          table_name ), $list37 ), mt_relevance_macros.$mt$.getDynamicValue( thread ), UNPROVIDED ).first();
      SubLObject result = NIL;
      SubLObject pf = NIL;
      final SubLObject pred_var = $const38$physicalSchemaFieldWithName;
      if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( ps, ONE_INTEGER, pred_var ) )
      {
        ...
        while ( NIL == done_var)
        {
          ...
          if( NIL != valid )
          {
            ...
            try
            {
              final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw39$GAF, NIL, NIL );
              ...
              while ( NIL == done_var_$3)
              {
                ...
                if( NIL != valid_$5 )
                {
                  ...
                  final SubLObject is_primary = kb_mapping_utilities.pred_u_v_holds( $const40$primaryKey, ps, pf, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                  final SubLObject is_nullable = kb_mapping_utilities.pred_u_v_holds( $const41$nonNullFields, ps, pf, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                  ...
                  result = ConsesLow.cons( ConsesLow.list( column_name, datatype, is_primary, is_nullable ), result );
                }
  ...
  {
    final SubLObject field_data_type = kb_mapping_utilities.pred_values( physical_field, $const43$fieldDataType, ONE_INTEGER, TWO_INTEGER, UNPROVIDED ).first();
    SubLObject ret_value = $str44$unknown;
    if( field_data_type.equal( narts_high.nart_substitute( $list45 ) ) )
    {
      ret_value = $str46$date_13_;
    }
    else if( NIL != obsolete.nat_p( field_data_type ) && cycl_utilities.formula_operator( field_data_type ).eql( $const47$DecimalRealNumberOfMaximumPrecisi ) )
    {
      ret_value = Sequences.cconcatenate( $str48$numeric_, new SubLObject[] { format_nil.format_nil_a_no_copy( cycl_utilities.formula_arg1( field_data_type, UNPROVIDED ) ), $str49$_
      } );
    }
    else if( NIL != obsolete.nat_p( field_data_type ) && cycl_utilities.formula_operator( field_data_type ).eql( $const50$StringOfMaximumLengthFn ) )
    {
      ret_value = Sequences.cconcatenate( $str51$varchar_, new SubLObject[] { format_nil.format_nil_a_no_copy( cycl_utilities.formula_arg1( field_data_type, UNPROVIDED ) ), $str49$_
      } );
    }
    else if( field_data_type.equal( narts_high.nart_substitute( $list52 ) ) )
    {
      ret_value = $str53$int4_10_;
    }
    final SubLObject default_value = kb_mapping_utilities.pred_values( physical_field, $const54$defaultFieldValue, TWO_INTEGER, THREE_INTEGER, UNPROVIDED ).first();
    if( NIL != default_value )
    {
      ret_value = Sequences.cconcatenate( format_nil.format_nil_a_no_copy( ret_value ), new SubLObject[] { $str55$_default_, format_nil.format_nil_a_no_copy( default_value )
      } );
    }
    return ret_value;
  }
  ...
  {
    final SubLObject map = dictionary.new_dictionary( Symbols.symbol_function( EQUAL ), UNPROVIDED );
    ...
    {
      final SubLObject entries = conses_high.copy_list( map_utilities.map_get( map, schema, UNPROVIDED ) );
      result = ConsesLow.cons( ConsesLow.list( schema, Sort.sort( entries, $sym58$STRING_, $sym59$FIRST ) ), result );
  ...
  public static SubLObject sks_physical_structure_database_row_count(final SubLObject connection, final SubLObject table_name)
  {
    ...
    final SubLObject row_count_sql = Sequences.cconcatenate( $str61$select_count____from_, format_nil.format_nil_a_no_copy( table_name ) );
  ...
  public static SubLObject sks_create_and_add_sub_ks_int(final SubLObject sks_name, final SubLObject super_sks, final SubLObject defining_mt, final SubLObject table_schema, SubLObject row_count)
  {
    ...
    assert NIL != sksi_sks_accessors.sks_p( super_sks ) : super_sks;
    assert NIL != hlmt.hlmt_p( defining_mt ) : defining_mt;
    assert NIL != Types.stringp( table_schema ) : table_schema;
    if( NIL != row_count && !assertionsDisabledInClass && NIL == Types.numberp( row_count ) )
    {
    ...
    sksi_sks_accessors.sks_set_type( sks, $const68$Database_Physical );
    if( NIL != row_count )
    {
  ...
  {
    assert NIL != hlmt.hlmt_p( logical_schemabase_mt ) : logical_schemabase_mt;
    ...
    if( NIL != logical_schema )
    {
      ...
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const0$isa, logical_schema, $const70$Individual ), $const2$UniversalVocabularyMt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const0$isa, logical_schema, $const71$ObjectDefiningSchema ), logical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const0$isa, logical_schema, $const72$LogicalSchema ), logical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const73$logicalPhysicalSchemaMap, logical_schema, physical_schema ), logical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const4$comment, logical_schema, Sequences.cconcatenate( $str74$A_logical_schema_corresponding_to, new SubLObject[] { format_nil
          .format_nil_a_no_copy( physical_schema_name ), $str75$_physical_schema_
      } ) ), $const2$UniversalVocabularyMt );
    }
  ...
  public static SubLObject sks_create_and_add_ps_database(final SubLObject sks, final SubLObject physical_schemabase_mt, SubLObject db_schema_name)
  {
    ...
    if( NIL != ps )
    {
      ...
      final SubLObject sks_name = table_name = sksi_sks_accessors.sks_fget( sks, $const76$structuredKnowledgeSourceName, defining_mt );
      final SubLObject ps_name = sks_make_physical_schema_name( sks );
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          db_schema_name = conses_high.getf( column_meta_data, $kw31$TABLE_SCHEM, UNPROVIDED );
        }
        field_names = ConsesLow.cons( conses_high.getf( column_meta_data, $kw78$COLUMN_NAME, UNPROVIDED ), field_names );
        cdolist_list_var = cdolist_list_var.rest();
        column_meta_data = cdolist_list_var.first();
      }
      field_names = Sequences.nreverse( field_names );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const16$schemaName_SKS, ps, sks_name ), physical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const17$schemaNameSchema_SKS, ps, db_schema_name ), physical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_unary_formula( $const79$enumerableSchema, ps ), physical_schemabase_mt );
      sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const80$schemaFieldNameList, ps, el_utilities.make_el_list( field_names, UNPROVIDED ) ), physical_schemabase_mt );
      return ps;
    }
    return NIL;
  }
  ...
  public static SubLObject sksi_sdbc_column_type_to_cycl(final SubLObject data_type, SubLObject column_size, SubLObject decimal_digits, SubLObject type_name)
  {
    ...
    assert NIL != Types.integerp( data_type ) : data_type;
    if( !column_size.isInteger() )
    {
      column_size = $kw82$NULL;
    }
    if( !decimal_digits.isInteger() )
    {
      decimal_digits = $kw82$NULL;
    }
    if( !type_name.isString() )
    {
      type_name = $kw82$NULL;
    }
    final SubLObject jdbc_type = ConsesLow.list( $const83$JDBCDatatypeDescribedByFn, data_type, column_size, decimal_digits, type_name );
    SubLObject cycl_data_type = bindings.variable_lookup( $sym84$_WHAT, inference_kernel.new_cyc_query( ConsesLow.list( $const85$rewriteOf, $sym84$_WHAT, jdbc_type ), $const86$JDBCDataMt, ConsesLow.list(
        $kw87$INFERENCE_MODE, $kw88$SHALLOW, $kw89$NEW_TERMS_ALLOWED_, T ) ).first() );
    ...
    if( NIL != ps )
    {
      ...
      try
      {
        ...
        try
        {
          ...
          while ( NIL != cdolist_list_var)
          {
            final SubLObject column_name = conses_high.getf( column_meta_data, $kw78$COLUMN_NAME, UNPROVIDED );
            final SubLObject nullable = conses_high.getf( column_meta_data, $kw92$IS_NULLABLE, UNPROVIDED );
            if( nullable.equal( $str93$NO ) )
            {
              final SubLObject pf = sksi_find_ps_field( ps, column_name );
              non_null_pfs = ConsesLow.cons( pf, non_null_pfs );
            }
            cdolist_list_var = cdolist_list_var.rest();
            column_meta_data = cdolist_list_var.first();
          }
          ...
          if( NIL != non_null_pfs )
          {
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const94$nonNullFieldList, ps, el_utilities.make_el_list( non_null_pfs, UNPROVIDED ) ), physical_schemabase_mt );
          }
          ...
          while ( NIL != cdolist_list_var2)
          {
            final SubLObject column_name2 = conses_high.getf( column_meta_data2, $kw78$COLUMN_NAME, UNPROVIDED );
            final SubLObject data_type = conses_high.getf( column_meta_data2, $kw95$DATA_TYPE, UNPROVIDED );
            final SubLObject type_name = conses_high.getf( column_meta_data2, $kw96$TYPE_NAME, UNPROVIDED );
            final SubLObject column_size = conses_high.getf( column_meta_data2, $kw97$COLUMN_SIZE, UNPROVIDED );
            final SubLObject decimal_digits = conses_high.getf( column_meta_data2, $kw98$DECIMAL_DIGITS, UNPROVIDED );
            SubLObject column_def = conses_high.getf( column_meta_data2, $kw99$COLUMN_DEF, UNPROVIDED );
            ...
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const43$fieldDataType, pf2, cycl_datatype ), physical_schemabase_mt );
            if( NIL != string_utilities.non_empty_stringP( column_def ) )
            {
              SubLObject poss_column_def = NIL;
              if( NIL != genls.genl_in_any_mtP( cycl_datatype, $const100$IntegerComputerNumber ) )
              {
                poss_column_def = string_utilities.string_to_integer( column_def );
              }
              else if( NIL != genls.genl_in_any_mtP( cycl_datatype, $const101$DoubleFloatObject ) )
              {
                poss_column_def = reader.read_from_string_ignoring_errors( column_def, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
              }
              if( NIL != poss_column_def )
              {
                column_def = poss_column_def;
              }
              sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_ternary_formula( $const54$defaultFieldValue, ps, pf2, column_def ), physical_schemabase_mt );
            }
            cdolist_list_var2 = cdolist_list_var2.rest();
            column_meta_data2 = cdolist_list_var2.first();
          }
  ...
  public static SubLObject sks_ps_get_schema_name(final SubLObject ps, final SubLObject physical_schemabase_mt)
  {
    ...
    try
    {
      ...
      result = kb_mapping_utilities.fpred_value( ps, $const17$schemaNameSchema_SKS, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      mt_relevance_macros.$mt$.rebind( _prev_bind_2, thread );
  ...
  public static SubLObject sks_ps_add_primary_keys(final SubLObject sks, final SubLObject ps, final SubLObject physical_schemabase_mt)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject defining_mt = sksi_sks_accessors.sks_get_defining_mt( sks, UNPROVIDED );
    final SubLObject table_name = sksi_sks_accessors.sks_fget( sks, $const76$structuredKnowledgeSourceName, defining_mt );
    ...
    try
    {
      ...
      try
      {
        ...
        SubLObject cdolist_list_var;
        primary_keys_meta_data = ( cdolist_list_var = list_utilities.plistlist_sort( primary_keys_meta_data, $kw102$KEY_SEQ, $sym103$_ ) );
        SubLObject primary_key_meta_data = NIL;
        primary_key_meta_data = cdolist_list_var.first();
        while ( NIL != cdolist_list_var)
        {
          final SubLObject primary_key_column_name = conses_high.getf( primary_key_meta_data, $kw78$COLUMN_NAME, UNPROVIDED );
          ...
          {
            primary_key_pfs = ConsesLow.cons( pf, primary_key_pfs );
          }
          cdolist_list_var = cdolist_list_var.rest();
          primary_key_meta_data = cdolist_list_var.first();
        }
        ...
        {
          sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const40$primaryKey, ps, el_utilities.make_el_list( primary_key_pfs, UNPROVIDED ) ), physical_schemabase_mt );
        }
      }
      finally
      {
  ...
  public static SubLObject sks_ps_add_exported_keys(final SubLObject sks, final SubLObject ps, final SubLObject physical_schemabase_mt)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        SubLObject cdolist_list_var;
        final SubLObject foreign_key_names = cdolist_list_var = plistlist_indicator_range( exported_keys_meta_data, $kw105$FK_NAME );
        SubLObject foreign_key_name = NIL;
        foreign_key_name = cdolist_list_var.first();
        while ( NIL != cdolist_list_var)
        {
          final SubLObject exported_key_columns_meta_data = plistlist_filter_and_sort( exported_keys_meta_data, $kw105$FK_NAME, foreign_key_name, Symbols.symbol_function( EQUAL ), $kw102$KEY_SEQ, $sym103$_ );
          SubLObject foreign_key_pred = $const106$foreignKeyMap;
          ...
          {
            final SubLObject update_rule = conses_high.getf( exported_key_column_meta_data, $kw107$UPDATE_RULE, UNPROVIDED );
            final SubLObject delete_rule = conses_high.getf( exported_key_column_meta_data, $kw108$DELETE_RULE, UNPROVIDED );
            final SubLObject primary_key_column_name = conses_high.getf( exported_key_column_meta_data, $kw109$PKCOLUMN_NAME, UNPROVIDED );
            final SubLObject foreign_key_table_name = conses_high.getf( exported_key_column_meta_data, $kw110$FKTABLE_NAME, UNPROVIDED );
            final SubLObject foreign_key_column_name = conses_high.getf( exported_key_column_meta_data, $kw111$FKCOLUMN_NAME, UNPROVIDED );
            ...
            {
              foreign_key_pred = $const112$foreignKeyMap_Cascade;
            }
            if( update_rule.eql( ONE_INTEGER ) || delete_rule.eql( ONE_INTEGER ) )
            {
              foreign_key_pred = $const113$foreignKeyMap_Restrict;
            }
            ...
            {
              primary_key_pfs = ConsesLow.cons( pk_pf, primary_key_pfs );
            }
            if( NIL == fk_pf )
            {
            ...
            {
              foreign_key_pfs = ConsesLow.cons( fk_pf, foreign_key_pfs );
            }
          ...
          {
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_quaternary_formula( foreign_key_pred, ps, el_utilities.make_el_list( primary_key_pfs, UNPROVIDED ), foreign_ps, el_utilities.make_el_list(
                foreign_key_pfs, UNPROVIDED ) ), physical_schemabase_mt );
          }
          cdolist_list_var = cdolist_list_var.rest();
          foreign_key_name = cdolist_list_var.first();
        }
  ...
  public static SubLObject sks_ps_add_imported_keys(final SubLObject sks, final SubLObject ps, final SubLObject physical_schemabase_mt)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        SubLObject cdolist_list_var;
        final SubLObject foreign_key_names = cdolist_list_var = plistlist_indicator_range( imported_keys_meta_data, $kw105$FK_NAME );
        SubLObject foreign_key_name = NIL;
        foreign_key_name = cdolist_list_var.first();
        while ( NIL != cdolist_list_var)
        {
          final SubLObject imported_key_columns_meta_data = plistlist_filter_and_sort( imported_keys_meta_data, $kw105$FK_NAME, foreign_key_name, Symbols.symbol_function( EQUAL ), $kw102$KEY_SEQ, $sym103$_ );
          SubLObject foreign_key_pred = $const106$foreignKeyMap;
          ...
          {
            final SubLObject update_rule = conses_high.getf( imported_key_column_meta_data, $kw107$UPDATE_RULE, UNPROVIDED );
            final SubLObject delete_rule = conses_high.getf( imported_key_column_meta_data, $kw108$DELETE_RULE, UNPROVIDED );
            final SubLObject primary_key_table_name = conses_high.getf( imported_key_column_meta_data, $kw115$PKTABLE_NAME, UNPROVIDED );
            final SubLObject primary_key_column_name = conses_high.getf( imported_key_column_meta_data, $kw109$PKCOLUMN_NAME, UNPROVIDED );
            final SubLObject foreign_key_column_name = conses_high.getf( imported_key_column_meta_data, $kw111$FKCOLUMN_NAME, UNPROVIDED );
          ...
          if( NIL != primary_key_pfs && NIL != foreign_key_pfs )
          {
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_quaternary_formula( foreign_key_pred, primary_ps, el_utilities.make_el_list( primary_key_pfs, UNPROVIDED ), ps, el_utilities.make_el_list(
                foreign_key_pfs, UNPROVIDED ) ), physical_schemabase_mt );
          }
          cdolist_list_var = cdolist_list_var.rest();
          foreign_key_name = cdolist_list_var.first();
  ...
  public static SubLObject sksi_index_type_from_index_type_id(final SubLObject type_id)
  {
    ...
    {
      return $const116$BTree_DataStructure;
    }
    if( type_id.eql( TWO_INTEGER ) )
    {
      return $const117$HashTable_DataStructure;
    }
  ...
  public static SubLObject sks_ps_add_indices(final SubLObject sks, SubLObject ps, final SubLObject physical_schemabase_mt)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        SubLObject cdolist_list_var;
        final SubLObject index_names = cdolist_list_var = plistlist_indicator_range( indices_meta_data, $kw118$INDEX_NAME );
        SubLObject index_name = NIL;
        index_name = cdolist_list_var.first();
        while ( NIL != cdolist_list_var)
        {
          final SubLObject index_columns_meta_data = plistlist_filter_and_sort( indices_meta_data, $kw118$INDEX_NAME, index_name, Symbols.symbol_function( EQUAL ), $kw119$ORDINAL_POSITION, $sym120$OBJECT_STRING__ );
          ...
          {
            final SubLObject non_unique = conses_high.getf( index_column_meta_data, $kw121$NON_UNIQUE, UNPROVIDED );
            final SubLObject index_table_name = conses_high.getf( index_column_meta_data, $kw30$TABLE_NAME, UNPROVIDED );
            final SubLObject index_column_name = conses_high.getf( index_column_meta_data, $kw78$COLUMN_NAME, UNPROVIDED );
            final SubLObject index_pf = sksi_find_ps_field( ps, index_column_name );
            index_type = conses_high.getf( index_column_meta_data, $kw122$TYPE, UNPROVIDED );
            if( non_unique.equal( $str123$true ) )
            {
              index_non_uniqueP = T;
            }
            ...
            {
              index_pfs = ConsesLow.cons( index_pf, index_pfs );
            }
          ...
          if( NIL != index_pfs )
          {
            final SubLObject index_constant_name = Sequences.cconcatenate( string_utilities.post_remove( constants_high.constant_name( sks ), $str124$KS, UNPROVIDED ), new SubLObject[] { index_name, $str125$_TheIndex
            } );
            ...
            {
              sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const126$uniqueFieldList, ps, el_utilities.make_el_list( index_pfs, UNPROVIDED ) ), physical_schemabase_mt );
            }
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const0$isa, index_constant, $const127$IndexDataStructure ), $const2$UniversalVocabularyMt );
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_quaternary_formula( $const128$schemaFieldsIndex, ps, el_utilities.make_el_list( index_pfs, UNPROVIDED ), index_name, index_constant ),
                physical_schemabase_mt );
            sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const129$indexName, index_constant, index_name ), physical_schemabase_mt );
            if( NIL != index_type_constant )
            {
              sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const130$indexType, index_constant, index_type_constant ), physical_schemabase_mt );
            }
          }
          cdolist_list_var = cdolist_list_var.rest();
          index_name = cdolist_list_var.first();
  ...
  public static SubLObject sks_create_and_add_ps_for_database_int(final SubLObject sks, final SubLObject field_names, final SubLObject physical_schemabase_mt, SubLObject primary_key)
  {
    ...
    assert NIL != sksi_sks_accessors.sks_p( sks ) : sks;
    assert NIL != hlmt.hlmt_p( physical_schemabase_mt ) : physical_schemabase_mt;
    final SubLObject ps_name = sks_make_physical_schema_name( sks );
    final SubLObject ps = sksi_kb_accessors.sksi_find_or_create( ps_name );
    sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_unary_formula( $const79$enumerableSchema, ps ), physical_schemabase_mt );
    sksi_kb_accessors.sksi_assert_if_new( el_utilities.make_binary_formula( $const80$schemaFieldNameList, ps, el_utilities.make_el_list( field_names, UNPROVIDED ) ), physical_schemabase_mt );
  ...
  public static SubLObject sksi_find_ps_by_table(final SubLObject table_name, final SubLObject physical_schemabase_mt)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym33$RELEVANT_MT_IS_GENL_MT, thread );
      mt_relevance_macros.$mt$.bind( physical_schemabase_mt, thread );
      final SubLObject pred_var = $const16$schemaName_SKS;
  ...
  public static SubLObject plistlist_indicator_range(final SubLObject plistlist, final SubLObject indicator)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      result = ConsesLow.cons( conses_high.getf( plist, indicator, UNPROVIDED ), result );
      cdolist_list_var = cdolist_list_var.rest();
      plist = cdolist_list_var.first();
    }
  ...
  public static SubLObject plistlist_filter_and_sort(final SubLObject plistlist, final SubLObject filter_indicator, final SubLObject filter_value, final SubLObject filter_test, final SubLObject sort_indicator,
      final SubLObject sort_test)
  {
  ...
  {
    return Strings.stringL( print_high.princ_to_string( object1 ), print_high.princ_to_string( object2 ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject repropagate_isas_for_ks_and_each_sub_ks(final SubLObject sks)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        mt_relevance_macros.$relevant_mt_function$.bind( $sym33$RELEVANT_MT_IS_GENL_MT, thread );
        mt_relevance_macros.$mt$.bind( $const2$UniversalVocabularyMt, thread );
        ...
        if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( sub_ks, ONE_INTEGER, pred_var ) )
        {
          ...
          while ( NIL == done_var)
          {
            ...
            if( NIL != valid )
            {
              ...
              try
              {
                final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw39$GAF, $kw131$TRUE, NIL );
                ...
                while ( NIL == done_var_$58)
                {
                  ...
                  if( NIL != valid_$60 && NIL != assertions_high.gaf_assertionP( assertion ) && assertions_high.gaf_arg2( assertion ).equal( $const132$StructuredKnowledgeSource ) )
                  {
        ...
        if( NIL == isa.isaP( sub_ks, $const133$StructuredKnowledgeSource_Atomic, UNPROVIDED, UNPROVIDED ) )
        {
  ...
  {
    SubLFiles.declareFunction( me, "sks_physical_structure_decode", "SKS-PHYSICAL-STRUCTURE-DECODE", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_initial_ke_validate", "SKS-INITIAL-KE-VALIDATE", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_create_and_add_raw_ls", "SKS-CREATE-AND-ADD-RAW-LS", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_keys", "SKS-PS-ADD-KEYS", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_pf_properties", "SKS-PS-ADD-PF-PROPERTIES", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_find_or_create_ps", "SKS-FIND-OR-CREATE-PS", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_make_physical_schema_name", "SKS-MAKE-PHYSICAL-SCHEMA-NAME", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_find_ps_field", "SKSI-FIND-PS-FIELD", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_sks_get_physical_schemas", "SKSI-SKS-GET-PHYSICAL-SCHEMAS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_sks_get_all_physical_schemas", "SKSI-SKS-GET-ALL-PHYSICAL-SCHEMAS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_physical_schema_get_physical_fields", "SKSI-PHYSICAL-SCHEMA-GET-PHYSICAL-FIELDS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_physical_schema_get_physical_field_indexicals", "SKSI-PHYSICAL-SCHEMA-GET-PHYSICAL-FIELD-INDEXICALS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_sks_get_all_physical_schema_table_names", "SKSI-SKS-GET-ALL-PHYSICAL-SCHEMA-TABLE-NAMES", 1, 1, false );
    SubLFiles.declareFunction( me, "ignore_table_schema_specification_p", "IGNORE-TABLE-SCHEMA-SPECIFICATION-P", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_initial_ke_validate_database", "SKS-INITIAL-KE-VALIDATE-DATABASE", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_gather_physical_database_table_names", "SKS-GATHER-PHYSICAL-DATABASE-TABLE-NAMES", 1, 2, false );
    SubLFiles.declareFunction( me, "sks_gather_asserted_table_meta_data_for_table", "SKS-GATHER-ASSERTED-TABLE-META-DATA-FOR-TABLE", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_get_datatype_string_for_physical_field", "SKS-GET-DATATYPE-STRING-FOR-PHYSICAL-FIELD", 1, 0, false );
    SubLFiles.declareFunction( me, "cluster_physical_database_table_information_by_schemata", "CLUSTER-PHYSICAL-DATABASE-TABLE-INFORMATION-BY-SCHEMATA", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_physical_structure_decode_database_table", "SKS-PHYSICAL-STRUCTURE-DECODE-DATABASE-TABLE", 2, 2, false );
    SubLFiles.declareFunction( me, "sks_physical_structure_decode_database", "SKS-PHYSICAL-STRUCTURE-DECODE-DATABASE", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_physical_structure_database_row_count", "SKS-PHYSICAL-STRUCTURE-DATABASE-ROW-COUNT", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_create_and_add_sub_ks_int", "SKS-CREATE-AND-ADD-SUB-KS-INT", 4, 1, false );
    SubLFiles.declareFunction( me, "sks_create_and_add_raw_ls_database", "SKS-CREATE-AND-ADD-RAW-LS-DATABASE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_create_initial_logical_schemas", "SKSI-CREATE-INITIAL-LOGICAL-SCHEMAS", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_create_logical_schema_for_physical", "SKSI-CREATE-LOGICAL-SCHEMA-FOR-PHYSICAL", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_create_and_add_ps_database", "SKS-CREATE-AND-ADD-PS-DATABASE", 2, 1, false );
    SubLFiles.declareFunction( me, "sksi_sdbc_column_type_to_cycl", "SKSI-SDBC-COLUMN-TYPE-TO-CYCL", 1, 3, false );
    SubLFiles.declareFunction( me, "sks_ps_add_pf_properties_database", "SKS-PS-ADD-PF-PROPERTIES-DATABASE", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_get_schema_name", "SKS-PS-GET-SCHEMA-NAME", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_primary_keys", "SKS-PS-ADD-PRIMARY-KEYS", 3, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_exported_keys", "SKS-PS-ADD-EXPORTED-KEYS", 3, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_imported_keys", "SKS-PS-ADD-IMPORTED-KEYS", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_index_type_from_index_type_id", "SKSI-INDEX-TYPE-FROM-INDEX-TYPE-ID", 1, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_indices", "SKS-PS-ADD-INDICES", 3, 0, false );
    SubLFiles.declareFunction( me, "sks_ps_add_keys_database", "SKS-PS-ADD-KEYS-DATABASE", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_create_and_add_ps_for_database_int", "SKS-CREATE-AND-ADD-PS-FOR-DATABASE-INT", 3, 1, false );
    SubLFiles.declareFunction( me, "sksi_find_ps_by_table", "SKSI-FIND-PS-BY-TABLE", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_find_ps_field_by_table_column", "SKSI-FIND-PS-FIELD-BY-TABLE-COLUMN", 3, 0, false );
    SubLFiles.declareFunction( me, "plistlist_indicator_range", "PLISTLIST-INDICATOR-RANGE", 2, 0, false );
    SubLFiles.declareFunction( me, "plistlist_filter_and_sort", "PLISTLIST-FILTER-AND-SORT", 6, 0, false );
    SubLFiles.declareFunction( me, "object_string_L", "OBJECT-STRING-<", 2, 0, false );
    SubLFiles.declareFunction( me, "repropagate_isas_for_ks_and_each_sub_ks", "REPROPAGATE-ISAS-FOR-KS-AND-EACH-SUB-KS", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_sksi_sks_structure_importer_utilities_file()
  {
    $sks_required_fields_database$ = SubLFiles.deflexical( "*SKS-REQUIRED-FIELDS-DATABASE*", $list18 );
    $sks_inclusive_catalog_pattern$ = SubLFiles.deflexical( "*SKS-INCLUSIVE-CATALOG-PATTERN*", NIL );
    $sks_inclusive_schema_pattern$ = SubLFiles.deflexical( "*SKS-INCLUSIVE-SCHEMA-PATTERN*", $str21$_ );
    return NIL;
  }

  public static SubLObject setup_sksi_sks_structure_importer_utilities_file()
  {
    access_macros.register_external_symbol( $sym15$SKSI_SKS_GET_ALL_PHYSICAL_SCHEMA_TABLE_NAMES );
    access_macros.register_external_symbol( $sym19$_SKS_INCLUSIVE_CATALOG_PATTERN_ );
    access_macros.register_external_symbol( $sym20$_SKS_INCLUSIVE_SCHEMA_PATTERN_ );
    access_macros.register_external_symbol( $sym22$IGNORE_TABLE_SCHEMA_SPECIFICATION_P );
    access_macros.register_external_symbol( $sym26$SKS_GATHER_PHYSICAL_DATABASE_TABLE_NAMES );
    access_macros.register_external_symbol( $sym32$SKS_GATHER_ASSERTED_TABLE_META_DATA_FOR_TABLE );
    access_macros.register_external_symbol( $sym56$CLUSTER_PHYSICAL_DATABASE_TABLE_INFORMATION_BY_SCHEMATA );
    access_macros.register_external_symbol( $sym60$SKS_PHYSICAL_STRUCTURE_DECODE_DATABASE_TABLE );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_sks_structure_importer_utilities();
    $sks_required_fields_database$ = null;
    $sks_inclusive_catalog_pattern$ = null;
    $sks_inclusive_schema_pattern$ = null;
    $const0$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $const1$PhysicalSchema = constant_handles.reader_make_constant_shell( makeString( "PhysicalSchema" ) );
    $const2$UniversalVocabularyMt = constant_handles.reader_make_constant_shell( makeString( "UniversalVocabularyMt" ) );
    $const3$physicalSchemaSourceMap = constant_handles.reader_make_constant_shell( makeString( "physicalSchemaSourceMap" ) );
    $const4$comment = constant_handles.reader_make_constant_shell( makeString( "comment" ) );
    $str5$A_physical_schema_for_ = makeString( "A physical schema for " );
    $str6$_ = makeString( "." );
    $str7$_KS = makeString( "-KS" );
    $str8$_PS = makeString( "-PS" );
    $const9$PhysicalFieldFn = constant_handles.reader_make_constant_shell( makeString( "PhysicalFieldFn" ) );
    ...
    $const12$subKS_Direct = constant_handles.reader_make_constant_shell( makeString( "subKS-Direct" ) );
    $const13$physicalFields = constant_handles.reader_make_constant_shell( makeString( "physicalFields" ) );
    $const14$physicalFieldIndexicals = constant_handles.reader_make_constant_shell( makeString( "physicalFieldIndexicals" ) );
    $sym15$SKSI_SKS_GET_ALL_PHYSICAL_SCHEMA_TABLE_NAMES = makeSymbol( "SKSI-SKS-GET-ALL-PHYSICAL-SCHEMA-TABLE-NAMES" );
    $const16$schemaName_SKS = constant_handles.reader_make_constant_shell( makeString( "schemaName-SKS" ) );
    $const17$schemaNameSchema_SKS = constant_handles.reader_make_constant_shell( makeString( "schemaNameSchema-SKS" ) );
    $list18 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "passwordForSKS" ) ), constant_handles.reader_make_constant_shell( makeString( "serverOfSKS" ) ), constant_handles
        .reader_make_constant_shell( makeString( "sksIsa" ) ), constant_handles.reader_make_constant_shell( makeString( "sqlProgramForSKS" ) ), constant_handles.reader_make_constant_shell( makeString(
            "structuredKnowledgeSourceName" ) ), constant_handles.reader_make_constant_shell( makeString( "subProtocolForSKS" ) ), constant_handles.reader_make_constant_shell( makeString( "userNameForSKS" ) ) );
    $sym19$_SKS_INCLUSIVE_CATALOG_PATTERN_ = makeSymbol( "*SKS-INCLUSIVE-CATALOG-PATTERN*" );
    $sym20$_SKS_INCLUSIVE_SCHEMA_PATTERN_ = makeSymbol( "*SKS-INCLUSIVE-SCHEMA-PATTERN*" );
    $str21$_ = makeString( "%" );
    $sym22$IGNORE_TABLE_SCHEMA_SPECIFICATION_P = makeSymbol( "IGNORE-TABLE-SCHEMA-SPECIFICATION-P" );
    $str23$_A_does_not_have_a_corresponding_ = makeString( "~A does not have a corresponding #$definingMt (or legacy #$mappingMt)" );
    $const24$definingMt = constant_handles.reader_make_constant_shell( makeString( "definingMt" ) );
    $str25$_A_is_missing__A_data = makeString( "~A is missing ~A data" );
    $sym26$SKS_GATHER_PHYSICAL_DATABASE_TABLE_NAMES = makeSymbol( "SKS-GATHER-PHYSICAL-DATABASE-TABLE-NAMES" );
    $str27$Problem_connecting_to_sks__A__cou = makeString( "Problem connecting to sks ~A--couldn't determine access path." );
    $str28$Problem_opening_connection_to_sks = makeString( "Problem opening connection to sks ~A." );
    $list29 = ConsesLow.list( makeString( "TABLE" ) );
    $kw30$TABLE_NAME = makeKeyword( "TABLE-NAME" );
    $kw31$TABLE_SCHEM = makeKeyword( "TABLE-SCHEM" );
    $sym32$SKS_GATHER_ASSERTED_TABLE_META_DATA_FOR_TABLE = makeSymbol( "SKS-GATHER-ASSERTED-TABLE-META-DATA-FOR-TABLE" );
    $sym33$RELEVANT_MT_IS_GENL_MT = makeSymbol( "RELEVANT-MT-IS-GENL-MT" );
    $sym34$_PS = makeSymbol( "?PS" );
    $const35$and = constant_handles.reader_make_constant_shell( makeString( "and" ) );
    $list36 = ConsesLow.list( makeSymbol( "?SUB-KS" ) );
    $list37 = ConsesLow.list( ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "physicalSchemaSourceMap" ) ), makeSymbol( "?PS" ), makeSymbol( "?SUB-KS" ) ) );
    $const38$physicalSchemaFieldWithName = constant_handles.reader_make_constant_shell( makeString( "physicalSchemaFieldWithName" ) );
    $kw39$GAF = makeKeyword( "GAF" );
    $const40$primaryKey = constant_handles.reader_make_constant_shell( makeString( "primaryKey" ) );
    $const41$nonNullFields = constant_handles.reader_make_constant_shell( makeString( "nonNullFields" ) );
    $sym42$_EXIT = makeSymbol( "%EXIT" );
    $const43$fieldDataType = constant_handles.reader_make_constant_shell( makeString( "fieldDataType" ) );
    $str44$unknown = makeString( "unknown" );
    $list45 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "ProgramRepresentationFn" ) ), constant_handles.reader_make_constant_shell( makeString( "CalendarDay" ) ) );
    $str46$date_13_ = makeString( "date(13)" );
    $const47$DecimalRealNumberOfMaximumPrecisi = constant_handles.reader_make_constant_shell( makeString( "DecimalRealNumberOfMaximumPrecisionFn" ) );
    $str48$numeric_ = makeString( "numeric(" );
    $str49$_ = makeString( ")" );
    $const50$StringOfMaximumLengthFn = constant_handles.reader_make_constant_shell( makeString( "StringOfMaximumLengthFn" ) );
    $str51$varchar_ = makeString( "varchar(" );
    $list52 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "SignedFn" ) ), constant_handles.reader_make_constant_shell( makeString( "4ByteIntegerDatatype" ) ) );
    $str53$int4_10_ = makeString( "int4(10)" );
    $const54$defaultFieldValue = constant_handles.reader_make_constant_shell( makeString( "defaultFieldValue" ) );
    $str55$_default_ = makeString( ":default=" );
    $sym56$CLUSTER_PHYSICAL_DATABASE_TABLE_INFORMATION_BY_SCHEMATA = makeSymbol( "CLUSTER-PHYSICAL-DATABASE-TABLE-INFORMATION-BY-SCHEMATA" );
    $list57 = ConsesLow.list( makeSymbol( "TABLE-SCHEMA" ), makeSymbol( "&REST" ), makeSymbol( "TABLE-INFO" ) );
    $sym58$STRING_ = makeSymbol( "STRING<" );
    $sym59$FIRST = makeSymbol( "FIRST" );
    $sym60$SKS_PHYSICAL_STRUCTURE_DECODE_DATABASE_TABLE = makeSymbol( "SKS-PHYSICAL-STRUCTURE-DECODE-DATABASE-TABLE" );
    $str61$select_count____from_ = makeString( "select count(*) from " );
    $sym62$NON_EMPTY_STRING_ = makeSymbol( "NON-EMPTY-STRING?" );
    $sym63$SKS_P = makeSymbol( "SKS-P" );
    $sym64$HLMT_P = makeSymbol( "HLMT-P" );
    $sym65$STRINGP = makeSymbol( "STRINGP" );
    $sym66$NUMBERP = makeSymbol( "NUMBERP" );
    $str67$Couldn_t_find___sksPhysicalSchema = makeString( "Couldn't find #$sksPhysicalSchemaDescriptionMt assertion for ~A in #$UniversalVocabularyMt" );
    $const68$Database_Physical = constant_handles.reader_make_constant_shell( makeString( "Database-Physical" ) );
    $str69$_LS = makeString( "-LS" );
    $const70$Individual = constant_handles.reader_make_constant_shell( makeString( "Individual" ) );
    $const71$ObjectDefiningSchema = constant_handles.reader_make_constant_shell( makeString( "ObjectDefiningSchema" ) );
    $const72$LogicalSchema = constant_handles.reader_make_constant_shell( makeString( "LogicalSchema" ) );
    $const73$logicalPhysicalSchemaMap = constant_handles.reader_make_constant_shell( makeString( "logicalPhysicalSchemaMap" ) );
    $str74$A_logical_schema_corresponding_to = makeString( "A logical schema corresponding to a portion of the #$" );
    $str75$_physical_schema_ = makeString( " physical schema." );
    $const76$structuredKnowledgeSourceName = constant_handles.reader_make_constant_shell( makeString( "structuredKnowledgeSourceName" ) );
    $str77$ = makeString( "" );
    $kw78$COLUMN_NAME = makeKeyword( "COLUMN-NAME" );
    $const79$enumerableSchema = constant_handles.reader_make_constant_shell( makeString( "enumerableSchema" ) );
    $const80$schemaFieldNameList = constant_handles.reader_make_constant_shell( makeString( "schemaFieldNameList" ) );
    $sym81$INTEGERP = makeSymbol( "INTEGERP" );
    $kw82$NULL = makeKeyword( "NULL" );
    $const83$JDBCDatatypeDescribedByFn = constant_handles.reader_make_constant_shell( makeString( "JDBCDatatypeDescribedByFn" ) );
    $sym84$_WHAT = makeSymbol( "?WHAT" );
    $const85$rewriteOf = constant_handles.reader_make_constant_shell( makeString( "rewriteOf" ) );
    $const86$JDBCDataMt = constant_handles.reader_make_constant_shell( makeString( "JDBCDataMt" ) );
    $kw87$INFERENCE_MODE = makeKeyword( "INFERENCE-MODE" );
    $kw88$SHALLOW = makeKeyword( "SHALLOW" );
    $kw89$NEW_TERMS_ALLOWED_ = makeKeyword( "NEW-TERMS-ALLOWED?" );
    $str90$No_better_type_found_for__A__usin = makeString( "No better type found for ~A; using it." );
    $str91$Also__failed_to_reify__A_ = makeString( "Also, failed to reify ~A." );
    $kw92$IS_NULLABLE = makeKeyword( "IS-NULLABLE" );
    $str93$NO = makeString( "NO" );
    $const94$nonNullFieldList = constant_handles.reader_make_constant_shell( makeString( "nonNullFieldList" ) );
    $kw95$DATA_TYPE = makeKeyword( "DATA-TYPE" );
    $kw96$TYPE_NAME = makeKeyword( "TYPE-NAME" );
    $kw97$COLUMN_SIZE = makeKeyword( "COLUMN-SIZE" );
    $kw98$DECIMAL_DIGITS = makeKeyword( "DECIMAL-DIGITS" );
    $kw99$COLUMN_DEF = makeKeyword( "COLUMN-DEF" );
    $const100$IntegerComputerNumber = constant_handles.reader_make_constant_shell( makeString( "IntegerComputerNumber" ) );
    $const101$DoubleFloatObject = constant_handles.reader_make_constant_shell( makeString( "DoubleFloatObject" ) );
    $kw102$KEY_SEQ = makeKeyword( "KEY-SEQ" );
    $sym103$_ = makeSymbol( "<" );
    $str104$physical_field_in__A_named__A_not = makeString( "physical field in ~A named ~A not found." );
    $kw105$FK_NAME = makeKeyword( "FK-NAME" );
    $const106$foreignKeyMap = constant_handles.reader_make_constant_shell( makeString( "foreignKeyMap" ) );
    $kw107$UPDATE_RULE = makeKeyword( "UPDATE-RULE" );
    $kw108$DELETE_RULE = makeKeyword( "DELETE-RULE" );
    $kw109$PKCOLUMN_NAME = makeKeyword( "PKCOLUMN-NAME" );
    $kw110$FKTABLE_NAME = makeKeyword( "FKTABLE-NAME" );
    $kw111$FKCOLUMN_NAME = makeKeyword( "FKCOLUMN-NAME" );
    $const112$foreignKeyMap_Cascade = constant_handles.reader_make_constant_shell( makeString( "foreignKeyMap-Cascade" ) );
    $const113$foreignKeyMap_Restrict = constant_handles.reader_make_constant_shell( makeString( "foreignKeyMap-Restrict" ) );
    $str114$physical_field_in__A_table__A_nam = makeString( "physical field in ~A table ~A named ~A not found." );
    $kw115$PKTABLE_NAME = makeKeyword( "PKTABLE-NAME" );
    $const116$BTree_DataStructure = constant_handles.reader_make_constant_shell( makeString( "BTree-DataStructure" ) );
    $const117$HashTable_DataStructure = constant_handles.reader_make_constant_shell( makeString( "HashTable-DataStructure" ) );
    $kw118$INDEX_NAME = makeKeyword( "INDEX-NAME" );
    $kw119$ORDINAL_POSITION = makeKeyword( "ORDINAL-POSITION" );
    $sym120$OBJECT_STRING__ = makeSymbol( "OBJECT-STRING-<" );
    $kw121$NON_UNIQUE = makeKeyword( "NON-UNIQUE" );
    $kw122$TYPE = makeKeyword( "TYPE" );
    $str123$true = makeString( "true" );
    $str124$KS = makeString( "KS" );
    $str125$_TheIndex = makeString( "-TheIndex" );
    $const126$uniqueFieldList = constant_handles.reader_make_constant_shell( makeString( "uniqueFieldList" ) );
    $const127$IndexDataStructure = constant_handles.reader_make_constant_shell( makeString( "IndexDataStructure" ) );
    $const128$schemaFieldsIndex = constant_handles.reader_make_constant_shell( makeString( "schemaFieldsIndex" ) );
    $const129$indexName = constant_handles.reader_make_constant_shell( makeString( "indexName" ) );
    $const130$indexType = constant_handles.reader_make_constant_shell( makeString( "indexType" ) );
    $kw131$TRUE = makeKeyword( "TRUE" );
    $const132$StructuredKnowledgeSource = constant_handles.reader_make_constant_shell( makeString( "StructuredKnowledgeSource" ) );
    $const133$StructuredKnowledgeSource_Atomic = constant_handles.reader_make_constant_shell( makeString( "StructuredKnowledgeSource-Atomic" ) );
  }
}
/*
 * 
