...
public final class sksi_data_warehousing_utilities
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject sksi_encode_memoized(final SubLObject cycl, final SubLObject ls, final SubLObject sks, SubLObject mt_info)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym0$SKSI_ENCODE_MEMOIZED, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym0$SKSI_ENCODE_MEMOIZED, FOUR_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym0$SKSI_ENCODE_MEMOIZED, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_encode_memoized_internal( cycl, ls, sks, mt_info ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( cycl, ls, sks, mt_info ) );
  ...
  {
    final SubLObject v_cache = dictionary.new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED );
    SubLObject cdolist_list_var;
    final SubLObject args = cdolist_list_var = cycl_utilities.formula_args( decoded_sentence, $kw1$IGNORE );
  ...
  public static SubLObject decoded_sentence_satisfies_conditionP(final SubLObject cms, final SubLObject lfi_value_cache, final SubLObject content_mt, final SubLObject meta_mt)
  {
    ...
    if( NIL == missingP && ( NIL != sksi_query_utilities.comparison_literal_p( result_exp ) || ( NIL != cycl_utilities.negatedP( result_exp ) && NIL != sksi_query_utilities.comparison_literal_p( cycl_utilities
        .formula_arg1( result_exp, UNPROVIDED ) ) ) || NIL != cycl_grammar.cycl_boolean_combination_of_type( $sym2$COMPARISON_LITERAL_P, result_exp ) ) )
    {
  ...
  public static SubLObject closed_decoded_sentence_satisfies_conditionP(final SubLObject sentence, final SubLObject mt, SubLObject v_properties)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym3$CLOSED_DECODED_SENTENCE_SATISFIES_CONDITION_, THREE_INTEGER, TWENTY_INTEGER, EQUAL,
          UNPROVIDED );
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( closed_decoded_sentence_satisfies_conditionP_internal( sentence, mt, v_properties ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( sentence, mt, v_properties ) );
    return memoization_state.caching_results( results3 );
  }
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          final SubLObject conditions = ( NIL != el_utilities.atomic_sentenceP( sentence ) ) ? ConsesLow.list( sentence ) : cycl_utilities.formula_args( sentence, UNPROVIDED );
          ...
          while ( NIL != cdolist_list_var)
          {
            ...
            if( NIL != doneP )
            {
              other_conditions = ConsesLow.cons( cond, other_conditions );
            }
            else
            {
              cons_lf_conditions = ConsesLow.cons( cond, cons_lf_conditions );
            }
            cdolist_list_var = cdolist_list_var.rest();
            cond = cdolist_list_var.first();
          }
          if( NIL != el_utilities.atomic_sentenceP( sentence ) )
          {
            ...
            {
              thread.resetMultipleValues();
              final SubLObject from_expression = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                  antecedent_logical_schema_set, other_conditions.first(), antecedent_sentence, consequent_sentence, meta_mt );
            ...
            else if( NIL != cons_lf_conditions )
            {
              thread.resetMultipleValues();
              final SubLObject from_expression = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                  antecedent_logical_schema_set, cons_lf_conditions.first(), antecedent_sentence, consequent_sentence, meta_mt );
              final SubLObject where_expression = thread.secondMultipleValue();
              final SubLObject sk_source = thread.thirdMultipleValue();
              thread.resetMultipleValues();
          ...
          else if( NIL != cycl_utilities.negatedP( sentence ) )
          {
            if( NIL != other_conditions )
            {
              thread.resetMultipleValues();
              final SubLObject from_expression = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                  antecedent_logical_schema_set, el_utilities.make_negation( other_conditions.first() ), antecedent_sentence, consequent_sentence, meta_mt );
              final SubLObject where_expression = thread.secondMultipleValue();
              final SubLObject sk_source = thread.thirdMultipleValue();
              thread.resetMultipleValues();
              result1 = check_csql_conditions_with_sk_source( from_expression, where_expression, sk_source );
            ...
            else if( NIL != cons_lf_conditions )
            {
              thread.resetMultipleValues();
              final SubLObject from_expression = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                  antecedent_logical_schema_set, el_utilities.make_negation( cons_lf_conditions.first() ), antecedent_sentence, consequent_sentence, meta_mt );
              final SubLObject where_expression = thread.secondMultipleValue();
              final SubLObject sk_source = thread.thirdMultipleValue();
              thread.resetMultipleValues();
              result1 = from_expression;
          ...
          else if( NIL != el_utilities.el_conjunction_p( sentence ) )
          {
            ...
            {
              thread.resetMultipleValues();
              final SubLObject from_expression2 = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                  antecedent_logical_schema_set, el_utilities.make_conjunction( other_conditions ), antecedent_sentence, consequent_sentence, meta_mt );
            ...
            if( NIL == failP )
            {
              if( NIL != cons_lf_conditions )
              {
                thread.resetMultipleValues();
                final SubLObject from_expression2 = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                    antecedent_logical_schema_set, el_utilities.make_conjunction( cons_lf_conditions ), antecedent_sentence, consequent_sentence, meta_mt );
                final SubLObject where_expression2 = thread.secondMultipleValue();
                final SubLObject sk_source2 = thread.thirdMultipleValue();
                thread.resetMultipleValues();
          ...
          else if( NIL != el_utilities.el_disjunction_p( sentence ) )
          {
            ...
            if( NIL != other_conditions )
            {
              thread.resetMultipleValues();
              final SubLObject from_expression2 = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                  antecedent_logical_schema_set, el_utilities.make_disjunction( other_conditions ), antecedent_sentence, consequent_sentence, meta_mt );
              final SubLObject where_expression2 = thread.secondMultipleValue();
              final SubLObject sk_source2 = thread.thirdMultipleValue();
              thread.resetMultipleValues();
              if( NIL != check_csql_conditions_with_sk_source( from_expression2, where_expression2, sk_source2 ) )
            ...
            if( NIL == successP )
            {
              if( NIL != cons_lf_conditions )
              {
                thread.resetMultipleValues();
                final SubLObject from_expression2 = get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence( cms, consequent_physical_schema_set, consequent_logical_schema_set,
                    antecedent_logical_schema_set, el_utilities.make_disjunction( cons_lf_conditions ), antecedent_sentence, consequent_sentence, meta_mt );
                final SubLObject where_expression2 = thread.secondMultipleValue();
                final SubLObject sk_source2 = thread.thirdMultipleValue();
                thread.resetMultipleValues();
                result1 = from_expression2;
  ...
  public static SubLObject get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence(final SubLObject cms, final SubLObject consequent_ps_list, final SubLObject consequent_logical_schema_set,
      final SubLObject antecedent_logical_schema_set, final SubLObject antecedent_sentence, final SubLObject antecedent_meaning_sentence, final SubLObject consequent_sentence, final SubLObject meta_mt)
  {
    ...
    from_expression = sksi_query_utilities.get_from_expression_from_antecedent_meaning_sentence_int( antecedent_logical_schema_set, antecedent_sentence, consequent_logical_schema_set, consequent_sentence, new_content_mt,
        new_sk_source, all_physical_schemata, NIL );
    where_expression = sksi_query_utilities.get_where_expression_from_antecedent_meaning_sentence_int( sksi_infrastructure_utilities.find_assertion_cycl_memoized( cms, meta_mt, UNPROVIDED ), antecedent_meaning_sentence,
        antecedent_logical_schema_set, consequent_sentence, consequent_logical_schema_set, new_content_mt, new_sk_source, all_physical_schemata, NIL, NIL );
    if( NIL != where_expression )
    {
      where_expression = ConsesLow.list( where_expression );
    }
    else
    {
      where_expression = sksi_query_utilities.get_where_expression_from_antecedent_meaning_sentence_int( sksi_infrastructure_utilities.find_assertion_cycl_memoized( cms, meta_mt, UNPROVIDED ),
          antecedent_meaning_sentence, antecedent_logical_schema_set, consequent_sentence, consequent_logical_schema_set, new_content_mt, new_sk_source, all_physical_schemata, NIL, NIL );
      where_expression = ConsesLow.list( where_expression );
      SubLObject cdolist_list_var;
        ...
        while ( NIL != cdolist_list_var_$1)
        {
          ...
          while ( NIL != cdolist_list_var_$2)
          {
            if( NIL != cycl_utilities.expression_find( lfi, encoding, UNPROVIDED, UNPROVIDED, UNPROVIDED ) )
            {
              ...
              final SubLObject reformulated_encoding = sksi_reformulate.sksi_reformulate( new_encoding, $kw6$ENCODE, new_sk_source );
              where_expression = ConsesLow.cons( sksi_csql_utilities.make_csql_expression( $const7$CSQLEquals, ConsesLow.list( csql_field, reformulated_encoding ) ), where_expression );
            }
  ...
  public static SubLObject decoded_sentence_satisfies_condition_recursiveP(final SubLObject sentence, final SubLObject antecedent_sentence, final SubLObject content_mt)
  {
    ...
    if( NIL != el_utilities.el_conjunction_p( sentence ) )
    {
      ...
      final SubLObject args = cycl_utilities.formula_args( sentence, $kw1$IGNORE );
      SubLObject rest;
      SubLObject conjunct;
      for( rest = NIL, rest = args; NIL == failP && NIL != rest; rest = rest.rest() )
      {
        conjunct = rest.first();
        conjunct_index = Numbers.add( conjunct_index, ONE_INTEGER );
    ...
    if( NIL != el_utilities.el_disjunction_p( sentence ) )
    {
      ...
      {
        disjunct = rest.first();
        disjunct_index = Numbers.add( disjunct_index, ONE_INTEGER );
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( !$const8$equals.eql( cycl_utilities.formula_operator( sentence ) ) )
    {
      return NIL;
    }
    ...
    final SubLObject bound_arg = conses_high.set_difference( cycl_utilities.formula_args( sentence, UNPROVIDED ), lfi_list, UNPROVIDED, UNPROVIDED ).first();
    final SubLObject original_bound_arg = cycl_utilities.formula_arg_position( antecedent_sentence, cycl_utilities.arg_positions_dfs( bound_arg, sentence, Symbols.symbol_function( EQUAL ) ).first(), UNPROVIDED );
    ...
    try
    {
      ...
      query_sentence = el_utilities.make_el_formula( $const9$isa, ConsesLow.list( bound_arg, field_isa ), UNPROVIDED );
    ...
    return closed_decoded_sentence_satisfies_conditionP( query_sentence, query_mt, ConsesLow.list( $kw10$ALLOWED_MODULES, allowed_modules_list ) );
  }
  ...
  {
    return conses_high.subst_if( $kw11$X, $sym12$CYC_VAR_, sentence, UNPROVIDED );
  }
  ...
  public static SubLObject sksi_template_appropriate_for_schemasP(final SubLObject template, final SubLObject logical_schema, final SubLObject physical_schema, SubLObject replace_atomic_sentence_argsP)
  {
    ...
    if( NIL != fort_types_interface.predicate_p( template_pred ) )
    {
      if( NIL == appropriateP )
      {
        ...
        for( csome_list_var = meaning_sentences, meaning_sentence = NIL, meaning_sentence = csome_list_var.first(); NIL == appropriateP && NIL != csome_list_var; appropriateP = sksi_template_appropriate_for_schemas_intP(
            template_pred, template, meaning_sentence, logical_schema, physical_schema, replace_atomic_sentence_argsP ), csome_list_var = csome_list_var.rest(), meaning_sentence = csome_list_var.first() )
        {
        }
      }
    }
    else if( NIL != el_utilities.cyc_const_logical_operator_p( template_pred ) )
    {
      ...
      if( NIL == some_meaning_sentence_inappropriateP )
      {
        ...
        while ( NIL == some_meaning_sentence_inappropriateP && NIL != csome_list_var2)
        {
          ...
          final SubLObject args = cycl_utilities.formula_args( template, $kw1$IGNORE );
          SubLObject rest;
          SubLObject conjunct;
    ...
    if( NIL == appropriateP )
    {
      ...
      if( NIL == appropriateP )
      {
        ...
        while ( NIL == appropriateP && NIL != csome_list_var2)
        {
          ...
          if( NIL != sksi_template_appropriate_for_schemas_intP( template_pred, template, new_consequent_sentence, logical_schema, physical_schema, replace_atomic_sentence_argsP )
              && NIL != sksi_meaning_sentence_utilities.supported_conditional_meaning_sentence_gaf_p( cms_gaf, UNPROVIDED ) )
          {
  ...
  public static SubLObject sksi_template_appropriate_for_schemas_intP(final SubLObject template_pred, final SubLObject template, final SubLObject meaning_sentence, final SubLObject logical_schema,
      final SubLObject physical_schema, final SubLObject replace_atomic_sentence_argsP)
  {
    SubLObject appropriateP = NIL;
    ...
    if( NIL == inappropriateP )
    {
      ...
      {
        pattern = conses_high.subst_if( $kw14$FULLY_BOUND, $sym15$ATOMIC_SENTENCE_MEMOIZED_, pattern, UNPROVIDED );
      }
      pattern = conses_high.subst_if( $kw14$FULLY_BOUND, $sym16$LOGICAL_FIELD_INDEXICAL_P, pattern, UNPROVIDED );
  ...
  {
    SubLFiles.declareFunction( me, "sksi_encode_memoized_internal", "SKSI-ENCODE-MEMOIZED-INTERNAL", 3, 1, false );
    SubLFiles.declareFunction( me, "sksi_encode_memoized", "SKSI-ENCODE-MEMOIZED", 3, 1, false );
    SubLFiles.declareFunction( me, "construct_lfi_value_cache", "CONSTRUCT-LFI-VALUE-CACHE", 1, 0, false );
    SubLFiles.declareFunction( me, "decoded_sentence_satisfies_conditionP", "DECODED-SENTENCE-SATISFIES-CONDITION?", 4, 0, false );
    SubLFiles.declareFunction( me, "closed_decoded_sentence_satisfies_conditionP_internal", "CLOSED-DECODED-SENTENCE-SATISFIES-CONDITION?-INTERNAL", 2, 1, false );
    SubLFiles.declareFunction( me, "closed_decoded_sentence_satisfies_conditionP", "CLOSED-DECODED-SENTENCE-SATISFIES-CONDITION?", 2, 1, false );
    SubLFiles.declareFunction( me, "decoded_sentence_satisfies_condition_intP", "DECODED-SENTENCE-SATISFIES-CONDITION-INT?", 4, 0, false );
    SubLFiles.declareFunction( me, "check_csql_conditions_with_sk_source", "CHECK-CSQL-CONDITIONS-WITH-SK-SOURCE", 3, 0, false );
    SubLFiles.declareFunction( me, "get_from_and_where_expression_for_antecedent_of_conditional_meaning_sentence", "GET-FROM-AND-WHERE-EXPRESSION-FOR-ANTECEDENT-OF-CONDITIONAL-MEANING-SENTENCE", 8, 0, false );
    SubLFiles.declareFunction( me, "decoded_sentence_satisfies_condition_recursiveP", "DECODED-SENTENCE-SATISFIES-CONDITION-RECURSIVE?", 3, 0, false );
    SubLFiles.declareFunction( me, "atomic_sentence_satisfies_conditionP", "ATOMIC-SENTENCE-SATISFIES-CONDITION?", 3, 0, false );
    SubLFiles.declareFunction( me, "open_decoded_sentence_satisfies_conditionP", "OPEN-DECODED-SENTENCE-SATISFIES-CONDITION?", 4, 0, false );
    SubLFiles.declareFunction( me, "make_sentence_template", "MAKE-SENTENCE-TEMPLATE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_template_appropriate_for_schemasP", "SKSI-TEMPLATE-APPROPRIATE-FOR-SCHEMAS?", 3, 1, false );
    SubLFiles.declareFunction( me, "sksi_template_appropriate_for_schemas_intP", "SKSI-TEMPLATE-APPROPRIATE-FOR-SCHEMAS-INT?", 6, 0, false );
    return NIL;
  }
  ...
  {
    memoization_state.note_memoized_function( $sym0$SKSI_ENCODE_MEMOIZED );
  ...
  static
  {
    me = new sksi_data_warehousing_utilities();
    $sym0$SKSI_ENCODE_MEMOIZED = makeSymbol( "SKSI-ENCODE-MEMOIZED" );
    $kw1$IGNORE = makeKeyword( "IGNORE" );
    $sym2$COMPARISON_LITERAL_P = makeSymbol( "COMPARISON-LITERAL-P" );
    $sym3$CLOSED_DECODED_SENTENCE_SATISFIES_CONDITION_ = makeSymbol( "CLOSED-DECODED-SENTENCE-SATISFIES-CONDITION?" );
    ...
    $kw6$ENCODE = makeKeyword( "ENCODE" );
    $const7$CSQLEquals = constant_handles.reader_make_constant_shell( makeString( "CSQLEquals" ) );
    $const8$equals = constant_handles.reader_make_constant_shell( makeString( "equals" ) );
    $const9$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $kw10$ALLOWED_MODULES = makeKeyword( "ALLOWED-MODULES" );
    $kw11$X = makeKeyword( "X" );
    $sym12$CYC_VAR_ = makeSymbol( "CYC-VAR?" );
    $str13$Unexpected_SKSI_template_type__s = makeString( "Unexpected SKSI template type ~s" );
    $kw14$FULLY_BOUND = makeKeyword( "FULLY-BOUND" );
    $sym15$ATOMIC_SENTENCE_MEMOIZED_ = makeSymbol( "ATOMIC-SENTENCE-MEMOIZED?" );
    $sym16$LOGICAL_FIELD_INDEXICAL_P = makeSymbol( "LOGICAL-FIELD-INDEXICAL-P" );
  }
}
/*
 * 
