...
public final class sksi_conjunctive_removal_modules_cost
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject sksi_query_cost_contextualized_dnf(final SubLObject contextualized_dnf_clause, final SubLObject sks)
  {
    ...
    if( NIL != kb_utilities.kbeq( sks_type, $const2$RDFTripleStore_Physical ) )
    {
  ...
  public static SubLObject guess_cost_from_literals(final SubLObject contextualized_dnf_clause, final SubLObject sks)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject asent_cost = sksi_query_utilities.look_up_sksi_query_cost_asent_sense( asent, $kw3$POS, sks, UNPROVIDED );
      if( NIL != asent_cost )
      {
        costs = ConsesLow.cons( asent_cost, costs );
      }
  ...
  public static SubLObject guess_cost_from_literal_costs(final SubLObject costs)
  {
    ...
    if( NIL != costs )
    {
      final SubLObject min_cost = Functions.apply( $sym4$MIN, costs );
      cost = ( min_cost.isZero() ? min_cost : Numbers.max( adjust_cost_guess( min_cost ), $float5$0_01 ) );
    }
    else
    {
    ...
    if( NIL != list_utilities.every_in_list( $sym6$ZEROP, costs, UNPROVIDED ) )
    {
      cost = ZERO_INTEGER;
    }
    else
  ...
  public static SubLObject guess_cost_from_literals_rdf_store(final SubLObject contextualized_dnf_clause, final SubLObject sks)
  {
    ...
    {
      final SubLObject asent = inference_datastructures_problem_query.contextualized_asent_asent( lit );
      final SubLObject focused_cost_estimateP = Numbers.numGE( Sequences.length( cycl_utilities.formula_gather( asent, $sym7$INDEXED_TERM_P, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ), TWO_INTEGER );
      final SubLObject asent_cost = sksi_query_utilities.look_up_sksi_query_cost_asent_sense( asent, $kw3$POS, sks, UNPROVIDED );
      if( NIL != number_utilities.positive_number_p( asent_cost ) )
      {
        if( NIL != focused_cost_estimateP )
        {
          focused_costs = ConsesLow.cons( asent_cost, focused_costs );
        }
        costs = ConsesLow.cons( asent_cost, costs );
      }
      cdolist_list_var = cdolist_list_var.rest();
  ...
  public static SubLObject guess_cost_from_literal_costs_rdf_triple_store(final SubLObject costs)
  {
    ...
    if( NIL != costs )
    {
      cost = Functions.apply( $sym4$MIN, costs );
      SubLObject cdotimes_end_var;
      SubLObject n;
      for( cdotimes_end_var = Numbers.subtract( Sequences.length( costs ), ONE_INTEGER ), n = NIL, n = ZERO_INTEGER; n.numL( cdotimes_end_var ); n = Numbers.add( n, ONE_INTEGER ) )
      {
        cost = Numbers.truncate( Numbers.divide( cost, TWO_INTEGER ), UNPROVIDED );
      }
    }
    return cost;
  }
  ...
  public static SubLObject removal_sksi_conjunction_pos_lits_completeness(final SubLObject contextualized_dnf_clause, final SubLObject sks_profile)
  {
    ...
    if( NIL != $phoenix_hospitals_hackP$.getDynamicValue( thread ) && NIL != list_utilities.member_eqP( sksi_query_datastructures.sks_profile_sks( sks_profile ), $list8 ) && NIL != hospitals_found_in_us_city(
        contextualized_dnf_clause, sks_profile ) )
    {
      return $kw9$COMPLETE;
    }
    if( NIL != $sksi_strongly_prefer_conjunctive_removal_modulesP$.getDynamicValue( thread ) )
    {
      completeness = $kw9$COMPLETE;
    }
    else
    {
      ...
      try
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          if( NIL == current )
          {
            ms_gaf_iterators = ConsesLow.cons( sksi_conjunctive_removal_module_utilities.get_meaning_sentence_gaf_iterator_for_asent( asent, logical_schemata, T, mt, UNPROVIDED, UNPROVIDED, UNPROVIDED ),
                ms_gaf_iterators );
          }
          else
          {
  ...
  {
    return ( NIL != removal_sksi_conjunction_pos_lits_completeness_int_recursive( NIL, ms_gaf_iterators ) ) ? $kw9$COMPLETE : $kw11$INCOMPLETE;
  }
  ...
  public static SubLObject removal_sksi_conjunction_pos_lits_completeness_int_recursive(final SubLObject gafs_so_far, final SubLObject remaining_iterators)
  {
    ...
    if( NIL == remaining_iterators )
    {
      ...
      if( NIL == failureP )
      {
        ...
        SubLObject gaf;
        for( csome_list_var = gafs_so_far, gaf = NIL, gaf = csome_list_var.first(); NIL == failureP && NIL != csome_list_var; failureP = makeBoolean( NIL == meaning_sentence_gaf_completeP(
            gaf ) ), csome_list_var = csome_list_var.rest(), gaf = csome_list_var.first() )
        {
        }
      }
    ...
    else
    {
      ...
      if( NIL != iterator )
      {
        ...
        for( SubLObject done_var = successP; NIL == done_var; done_var = makeBoolean( NIL == valid || NIL != successP ) )
        {
          ...
          if( NIL != valid )
          {
            successP = removal_sksi_conjunction_pos_lits_completeness_int_recursive( ConsesLow.cons( gaf, gafs_so_far ), remaining_iterators.rest() );
          }
        }
        if( NIL == successP )
        {
  ...
  public static SubLObject meaning_sentence_gaf_completeP(final SubLObject meaning_sentence_gaf)
  {
    ...
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( meaning_sentence_gaf_completeP_internal( meaning_sentence_gaf ) ) );
  ...
  public static SubLObject hospitals_found_in_us_city_int(final SubLObject pos_lits)
  {
    ...
    if( pcase_var.eql( TWO_INTEGER ) )
    {
      ...
      cdestructuring_bind.destructuring_bind_must_consp( pos_lits, pos_lits, $list14 );
      first = pos_lits.first();
      SubLObject current = pos_lits.rest();
      cdestructuring_bind.destructuring_bind_must_consp( current, pos_lits, $list14 );
      ...
      {
        cdestructuring_bind.cdestructuring_bind_error( pos_lits, $list14 );
      }
    }
    else
    {
      ...
      cdestructuring_bind.destructuring_bind_must_consp( pos_lits, pos_lits, $list19 );
      first = pos_lits.first();
      SubLObject current = pos_lits.rest();
      cdestructuring_bind.destructuring_bind_must_consp( current, pos_lits, $list19 );
      second = current.first();
      current = current.rest();
      cdestructuring_bind.destructuring_bind_must_consp( current, pos_lits, $list19 );
      third = current.first();
      ...
      if( NIL == current )
      {
        ...
        else if( NIL != formula_pattern_match.pattern_matches_formula( $list16, first ) )
        {
          ...
          {
            return makeBoolean( NIL != formula_pattern_match.pattern_matches_formula( $list20, third ) && cycl_utilities.formula_arg1( first, UNPROVIDED ).equal( cycl_utilities.formula_arg1( third, UNPROVIDED ) )
                && cycl_utilities.formula_arg1( second, UNPROVIDED ).equal( cycl_utilities.formula_arg2( third, UNPROVIDED ) ) );
          }
          return NIL;
        }
        else
        {
          ...
          {
            return makeBoolean( NIL != formula_pattern_match.pattern_matches_formula( $list20, third ) && cycl_utilities.formula_arg1( first, UNPROVIDED ).equal( cycl_utilities.formula_arg2( third, UNPROVIDED ) )
                && cycl_utilities.formula_arg1( second, UNPROVIDED ).equal( cycl_utilities.formula_arg1( third, UNPROVIDED ) ) );
          }
          return NIL;
        }
      }
      ...
      {
        cdestructuring_bind.cdestructuring_bind_error( pos_lits, $list19 );
      }
    }
    return NIL;
  }
  ...
  {
    SubLFiles.declareFunction( me, "removal_sksi_conjunction_pos_lits_cost", "REMOVAL-SKSI-CONJUNCTION-POS-LITS-COST", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_query_cost_contextualized_dnf", "SKSI-QUERY-COST-CONTEXTUALIZED-DNF", 2, 0, false );
    SubLFiles.declareFunction( me, "guess_cost_from_literals", "GUESS-COST-FROM-LITERALS", 2, 0, false );
    SubLFiles.declareFunction( me, "guess_cost_from_literal_costs", "GUESS-COST-FROM-LITERAL-COSTS", 1, 0, false );
    SubLFiles.declareFunction( me, "adjust_cost_guess", "ADJUST-COST-GUESS", 1, 0, false );
    SubLFiles.declareFunction( me, "guess_cost_from_literals_rdf_store", "GUESS-COST-FROM-LITERALS-RDF-STORE", 2, 0, false );
    SubLFiles.declareFunction( me, "guess_cost_from_literal_costs_rdf_triple_store", "GUESS-COST-FROM-LITERAL-COSTS-RDF-TRIPLE-STORE", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_sksi_conjunction_pos_lits_completeness", "REMOVAL-SKSI-CONJUNCTION-POS-LITS-COMPLETENESS", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_sksi_conjunction_pos_lits_completeness_int", "REMOVAL-SKSI-CONJUNCTION-POS-LITS-COMPLETENESS-INT", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_sksi_conjunction_pos_lits_completeness_int_recursive", "REMOVAL-SKSI-CONJUNCTION-POS-LITS-COMPLETENESS-INT-RECURSIVE", 2, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_gaf_completeP_internal", "MEANING-SENTENCE-GAF-COMPLETE?-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_gaf_completeP", "MEANING-SENTENCE-GAF-COMPLETE?", 1, 0, false );
    SubLFiles.declareFunction( me, "hospitals_found_in_us_city", "HOSPITALS-FOUND-IN-US-CITY", 2, 0, false );
    SubLFiles.declareFunction( me, "hospitals_found_in_us_city_int", "HOSPITALS-FOUND-IN-US-CITY-INT", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_sksi_conjunctive_removal_modules_cost_file()
  {
    $sksi_strongly_prefer_conjunctive_removal_modulesP$ = SubLFiles.defvar( "*SKSI-STRONGLY-PREFER-CONJUNCTIVE-REMOVAL-MODULES?*", T );
    $phoenix_hospitals_hackP$ = SubLFiles.defparameter( "*PHOENIX-HOSPITALS-HACK?*", NIL );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_conjunctive_removal_modules_cost();
    $sksi_strongly_prefer_conjunctive_removal_modulesP$ = null;
    $phoenix_hospitals_hackP$ = null;
    ...
    $const2$RDFTripleStore_Physical = constant_handles.reader_make_constant_shell( makeString( "RDFTripleStore-Physical" ) );
    $kw3$POS = makeKeyword( "POS" );
    $sym4$MIN = makeSymbol( "MIN" );
    $float5$0_01 = makeDouble( 0.01 );
    $sym6$ZEROP = makeSymbol( "ZEROP" );
    $sym7$INDEXED_TERM_P = makeSymbol( "INDEXED-TERM-P" );
    $list8 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "CATS-KS" ) ), constant_handles.reader_make_constant_shell( makeString( "CATS-hospital-KS" ) ) );
    $kw9$COMPLETE = makeKeyword( "COMPLETE" );
    $list10 = ConsesLow.list( makeSymbol( "MT" ), makeSymbol( "ASENT" ) );
    $kw11$INCOMPLETE = makeKeyword( "INCOMPLETE" );
    $sym12$MEANING_SENTENCE_GAF_COMPLETE_ = makeSymbol( "MEANING-SENTENCE-GAF-COMPLETE?" );
    $list13 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "objectFoundInLocation" ) ), ConsesLow.list( makeKeyword( "ISA" ), constant_handles.reader_make_constant_shell( makeString(
        "Hospital" ) ) ), ConsesLow.list( makeKeyword( "ISA" ), constant_handles.reader_make_constant_shell( makeString( "USCity" ) ) ) );
    $list14 = ConsesLow.list( makeSymbol( "FIRST" ), makeSymbol( "SECOND" ) );
    $list15 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "objectFoundInLocation" ) ), makeKeyword( "NOT-FULLY-BOUND" ), ConsesLow.list( makeKeyword( "ISA" ), constant_handles
        .reader_make_constant_shell( makeString( "USCity" ) ) ) );
    $list16 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "isa" ) ), makeKeyword( "NOT-FULLY-BOUND" ), constant_handles.reader_make_constant_shell( makeString( "Hospital" ) ) );
    $list17 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "objectFoundInLocation" ) ), ConsesLow.list( makeKeyword( "ISA" ), constant_handles.reader_make_constant_shell( makeString(
        "Hospital" ) ) ), makeKeyword( "NOT-FULLY-BOUND" ) );
    $list18 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "isa" ) ), makeKeyword( "NOT-FULLY-BOUND" ), constant_handles.reader_make_constant_shell( makeString( "USCity" ) ) );
    $list19 = ConsesLow.list( makeSymbol( "FIRST" ), makeSymbol( "SECOND" ), makeSymbol( "THIRD" ) );
    $list20 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "objectFoundInLocation" ) ), makeKeyword( "NOT-FULLY-BOUND" ), makeKeyword( "NOT-FULLY-BOUND" ) );
  }
}
/*
 * 
