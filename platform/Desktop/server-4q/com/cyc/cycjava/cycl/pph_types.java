      ...
      SubLTranslatedFile
{
  ...
  public static SubLObject get_paraphrase_methods(final SubLObject v_object)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != type_methods )
      {
        ...
        {
          format_nil.force_format( T, $str5$__Found__D_methods_for__S___S___, Sequences.length( type_methods ), type, type_methods, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
        }
      }
      else if( NIL != pph_error.pph_trace_level_exceeds_minimumP( TWO_INTEGER ) && NIL == pph_error.suppress_pph_warningsP() )
      {
      ...
      while ( NIL != cdolist_list_var_$1)
      {
        ...
        if( NIL == current )
        {
          ...
          final SubLObject item_var = ConsesLow.list( method_symbol, method_demerits );
          if( NIL == conses_high.member( item_var, v_methods, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
          {
            v_methods = ConsesLow.cons( item_var, v_methods );
          }
        }
        else
        {
  ...
  public static SubLObject pph_register_method(final SubLObject symbol, final SubLObject type, final SubLObject quality)
  {
    ...
    {
      final SubLObject new_format_string = Sequences.cconcatenate( $str10$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( TWO_INTEGER ), $str11$__, format_nil.format_nil_a_no_copy(
          $str12$___S_is_not_a_valid_paraphrase_ty )
      } );
      pph_error.pph_handle_error( new_format_string, ConsesLow.list( type ) );
      return NIL;
    }
    if( !symbol.isSymbol() )
    {
      final SubLObject new_format_string = Sequences.cconcatenate( $str10$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( TWO_INTEGER ), $str11$__, format_nil.format_nil_a_no_copy(
          $str13$___S_is_not_a_symbol___ )
      } );
      pph_error.pph_handle_error( new_format_string, ConsesLow.list( symbol ) );
      return NIL;
    }
    if( NIL == pph_valid_method_qualityP( quality ) )
    {
      final SubLObject new_format_string = Sequences.cconcatenate( $str10$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( TWO_INTEGER ), $str11$__, format_nil.format_nil_a_no_copy(
          $str14$___S_is_not_a_valid_paraphrase_me )
      } );
      pph_error.pph_handle_error( new_format_string, ConsesLow.list( type, $pph_valid_method_qualities$.getGlobalValue() ) );
      return NIL;
    }
    final SubLObject existing = pph_type_entries( type );
    final SubLObject new_pair = ConsesLow.list( symbol, quality );
    final SubLObject new_value = pph_sort_methods( ConsesLow.cons( new_pair, Sequences.remove( symbol, existing, Symbols.symbol_function( EQ ), Symbols.symbol_function( $sym15$FIRST ), UNPROVIDED, UNPROVIDED,
        UNPROVIDED ) ) );
    Hashtables.sethash( type, $pph_type_method_hash$.getGlobalValue(), new_value );
  ...
  public static SubLObject pph_deregister_method(final SubLObject symbol, final SubLObject type)
  {
    ...
    final SubLObject entry = conses_high.assoc( symbol, entries, UNPROVIDED, UNPROVIDED );
    if( NIL == entry )
    {
    ...
    {
      Hashtables.sethash( type, $pph_type_method_hash$.getGlobalValue(), Sequences.remove( entry, entries, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
    }
    return pph_type_entries( type );
  }
  ...
  public static SubLObject pph_method_info(final SubLObject method_symbol, final SubLObject object_type)
  {
    ...
    if( NIL != object_type )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        if( NIL == current )
        {
          ...
          {
            ans = ConsesLow.cons( ConsesLow.list( object_type, quality ), ans );
          }
        }
        else
        {
    ...
    else
    {
      ...
      final Iterator cdohash_iterator = Hashtables.getEntrySetIterator( $pph_type_method_hash$.getGlobalValue() );
      try
      {
        while ( Hashtables.iteratorHasNext( cdohash_iterator ))
        {
          ...
          while ( NIL != cdolist_list_var2)
          {
            ...
            if( NIL == current2 )
            {
              ...
              {
                ans = ConsesLow.cons( ConsesLow.list( type, quality2 ), ans );
              }
            }
            else
            {
  ...
  {
    return Hashtables.gethash( type, $pph_type_method_hash$.getGlobalValue(), UNPROVIDED );
  }
  ...
  {
    return conses_high.second( list_utilities.alist_lookup( $pph_type_tree$.getGlobalValue(), super_type, UNPROVIDED, UNPROVIDED ) );
  }
  ...
  {
    final SubLObject subtypes = list_utilities.remove_if_not( Symbols.symbol_function( $sym23$LISTP ), pph_subtypes( super_type ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    return list_utilities.alist_lookup( subtypes, sub_type, Symbols.symbol_function( EQ ), ZERO_INTEGER );
  }
  ...
  {
    return Sort.stable_sort( conses_high.copy_list( v_methods ), Symbols.symbol_function( $sym24$PPH_HIGHER_METHOD_QUALITY_ ), Symbols.symbol_function( $sym25$SECOND ) );
  }
  ...
  {
    return Sort.stable_sort( conses_high.copy_list( v_methods ), Symbols.symbol_function( $sym26$_ ), Symbols.symbol_function( $sym25$SECOND ) );
  }
  ...
  {
    return Sequences.position( quality, $pph_valid_method_qualities$.getGlobalValue(), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject pph_method_quality_demerits(final SubLObject quality)
  {
    ...
    return score.isInteger() ? Numbers.subtract( Sequences.length( $pph_valid_method_qualities$.getGlobalValue() ), number_utilities.f_1X( score ) ) : NIL;
  }
  ...
  public static SubLObject pph_identify_type_int(final SubLObject v_object, final SubLObject untried_types, SubLObject identified_types)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( untried_types, untried_types, $list28 );
    ...
    {
      identified_types = ConsesLow.cons( type, identified_types );
    }
    if( NIL != object_passes_testP )
    {
      if( NIL == sub_types )
    ...
    final SubLObject new_format_string = Sequences.cconcatenate( $str10$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str11$__, format_nil.format_nil_a_no_copy(
        $str34$___S___does_not_pass_top_level_te )
    } );
    pph_error.pph_handle_error( new_format_string, ConsesLow.list( v_object, test ) );
    return NIL;
  }
  ...
  {
    final SubLObject test = paraphrase_test( type );
    return ( NIL != Symbols.fboundp( test ) ) ? Functions.funcall( test, v_object ) : NIL;
  }
  ...
  public static SubLObject pph_collection_denoting_natP(final SubLObject naut)
  {
    ...
    return pph_utilities.pph_isaP( fn, $const35$CollectionDenotingFunction, UNPROVIDED );
  }
  ...
  {
    return makeBoolean( cycl_utilities.nat_functor( naut ).eql( $const36$TheSetOf ) && NIL != el_utilities.el_formula_p( cycl_utilities.nat_arg2( naut, UNPROVIDED ) ) );
  }
  ...
  public static SubLObject pph_nautP(final SubLObject obj)
  {
    ...
    final SubLObject ans = makeBoolean( NIL != el_utilities.el_formula_p( obj ) && NIL != ( ( NIL != forts.fort_p( arg0 ) ) ? fort_types_interface.non_predicate_functionP( arg0 )
        : pph_utilities.pph_isaP( arg0, $const42$Function_Denotational, pph_vars.$pph_domain_mt$.getDynamicValue( thread ) ) ) );
    return ans;
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_formula_p( v_object ) && NIL != formula_pattern_match.formula_matches_pattern( v_object, $list43 ) );
  }
  ...
  public static SubLObject pph_cycl_sentenceP(final SubLObject obj)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject arg0 = obj.first();
    final SubLObject ans = makeBoolean( NIL != el_utilities.el_formula_p( obj ) && ( NIL != fort_types_interface.predicateP( arg0 ) || NIL != cycl_variables.el_varP( arg0 ) || NIL != ( ( NIL != forts.fort_p( arg0 ) )
        ? fort_types_interface.sentential_relation_p( arg0 )
        : pph_utilities.pph_isaP( arg0, $const44$TruthFunction, pph_vars.$pph_domain_mt$.getDynamicValue( thread ) ) ) ) );
    return ans;
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_conjunction_p( obj ) || NIL != el_utilities.el_disjunction_p( obj ) );
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_universal_p( obj ) && NIL != cycl_variables.el_varP( cycl_utilities.formula_arg1( obj, UNPROVIDED ) ) );
  }
  ...
  {
    return makeBoolean( obj.isSymbol() && NIL != Symbols.fboundp( obj ) );
  }
  ...
  public static SubLObject pph_lexified_nartP(final SubLObject obj)
  {
    ...
    SubLObject cdolist_list_var = kb_mapping_utilities.pred_values( obj, $const45$rewriteOf, TWO_INTEGER, ONE_INTEGER, $kw46$TRUE );
  ...
  {
    return Equality.eq( v_object, $const47$True );
  }
  ...
  {
    return Equality.eq( v_object, $const48$False );
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_negation_p( v_object ) && NIL != el_utilities.groundP( cycl_utilities.formula_arg1( v_object, UNPROVIDED ), UNPROVIDED ) );
  }
  ...
  public static SubLObject pph_reln_has_expansionP(final SubLObject reln, SubLObject mt)
  {
    ...
    if( NIL != kb_indexing_datastructures.indexed_term_p( reln ) )
    {
      ...
      try
      {
        ...
        mt_relevance_macros.$relevant_mt_function$.bind( mt_relevance_macros.update_inference_mt_relevance_function( mt_var ), thread );
        mt_relevance_macros.$relevant_mts$.bind( mt_relevance_macros.update_inference_mt_relevance_mt_list( mt_var ), thread );
        ans = kb_mapping_utilities.some_pred_value( reln, $const49$expansion, UNPROVIDED, UNPROVIDED );
      }
      finally
      {
        mt_relevance_macros.$relevant_mts$.rebind( _prev_bind_3, thread );
  ...
  {
    return makeBoolean( NIL != el_utilities.el_formula_p( formula ) && NIL != el_utilities.formula_arityE( formula, THREE_INTEGER, UNPROVIDED ) && NIL != kb_indexing_datastructures.indexed_term_p( cycl_utilities
        .formula_arg1( formula, UNPROVIDED ) ) && NIL != pph_methods_formulas.specified_wide_scope_argnum( cycl_utilities.formula_operator( formula ) ) );
  }
  ...
  {
    return makeBoolean( NIL != constant_handles.valid_constantP( $const50$cfImplies, UNPROVIDED ) && NIL != el_utilities.el_formula_with_operator_p( v_object, $const50$cfImplies ) );
  }
  ...
  {
    return el_utilities.el_formula_with_operator_p( v_object, $const51$compositeFieldFormat );
  }
  ...
  {
    return makeBoolean( NIL != constant_handles.valid_constantP( $const52$ScriptPredicate, UNPROVIDED ) && NIL != el_utilities.el_formula_with_operator_p( v_object, $const49$expansion ) && NIL != pph_utilities.pph_isaP(
        cycl_utilities.formula_arg1( v_object, UNPROVIDED ), $const52$ScriptPredicate, UNPROVIDED ) );
  }
  ...
  {
    return makeBoolean( NIL != pph_formulaP( v_object ) && NIL != conses_high.member( v_object.first(), $list53, UNPROVIDED, UNPROVIDED ) );
  }
  ...
  {
    return makeBoolean( NIL == pph_utilities.pph_isaP( fort, $const54$Collection, UNPROVIDED ) && NIL != pph_utilities.pph_isaP( fort, $const55$GeopoliticalEntity, $const56$InferencePSC ) );
  }
  ...
  {
    return pph_utilities.pph_isaP( fort, $const57$City, $const56$InferencePSC );
  }
  ...
  {
    return pph_utilities.pph_isaP( fort, $const58$USCity, $const56$InferencePSC );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return makeBoolean( NIL != pph_vars.$pph_handle_hypotheticalsP$.getDynamicValue( thread ) && NIL != pph_utilities.pph_removal_ask_boolean( ConsesLow.list( $const59$hypotheticalTerm, constant ),
        $const56$InferencePSC ) );
  }
  ...
  {
    return makeBoolean( NIL != pph_utilities.pph_environment_search( $kw60$UNIVERSAL_DATE ) && NIL != numeric_date_utilities.universal_date_p( obj ) );
  }
  ...
  {
    return makeBoolean( NIL != pph_utilities.pph_environment_search( $kw61$UNIVERSAL_SECOND ) && NIL != numeric_date_utilities.universal_second_p( obj ) );
  }
  ...
  public static SubLObject pph_formulaP(final SubLObject v_object)
  {
    ...
    try
    {
      ...
      ans = makeBoolean( NIL != el_utilities.el_formulaP( v_object ) || NIL != term.nautP( v_object, UNPROVIDED ) || NIL != pph_utilities.pph_isaP( v_object, $const64$TruthValue, UNPROVIDED ) );
    }
    finally
    {
  ...
  {
    return makeBoolean( NIL != collection_defns.cycl_formulaP( cycl ) && cycl_utilities.formula_operator( cycl ).eql( $const65$TheResultOfParsing ) );
  }
  ...
  {
    return Equality.eql( cycl_utilities.nat_functor( nat ), $const66$TheCollectionOf );
  }
  ...
  public static SubLObject initialize_pph_subcol_fns()
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym62$RELEVANT_MT_IS_EVERYTHING, thread );
      mt_relevance_macros.$mt$.bind( $const63$EverythingPSC, thread );
      final SubLObject node_var = $const67$SubcollectionRelationFunction;
      ...
      try
      {
        sbhl_module_vars.$sbhl_module$.bind( sbhl_module_vars.get_sbhl_module( $const68$isa ), thread );
        sbhl_marking_vars.$sbhl_gather_space$.bind( sbhl_marking_vars.get_sbhl_marking_space(), thread );
        try
        {
          SubLObject node_var_$4 = node_var;
          final SubLObject deck_type = $kw71$STACK;
          ...
          try
          {
            ...
            try
            {
              ...
              try
              {
                ...
                sbhl_search_vars.$relevant_sbhl_tv_function$.bind( ( NIL != tv_var ) ? $sym72$RELEVANT_SBHL_TV_IS_GENERAL_TV : sbhl_search_vars.$relevant_sbhl_tv_function$.getDynamicValue( thread ), thread );
                if( NIL != tv_var && NIL != sbhl_paranoia.sbhl_object_type_checking_p() && NIL == sbhl_search_vars.sbhl_true_tv_p( tv_var ) )
                {
                  final SubLObject pcase_var = sbhl_paranoia.$sbhl_type_error_action$.getDynamicValue( thread );
                  if( pcase_var.eql( $kw73$ERROR ) )
                  {
                    sbhl_paranoia.sbhl_error( ONE_INTEGER, $str74$_A_is_not_a__A, tv_var, $sym75$SBHL_TRUE_TV_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                  }
                  else if( pcase_var.eql( $kw76$CERROR ) )
                  {
                    sbhl_paranoia.sbhl_cerror( ONE_INTEGER, $str77$continue_anyway, $str74$_A_is_not_a__A, tv_var, $sym75$SBHL_TRUE_TV_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                  }
                  else if( pcase_var.eql( $kw78$WARN ) )
                  {
                    Errors.warn( $str74$_A_is_not_a__A, tv_var, $sym75$SBHL_TRUE_TV_P );
                  }
                  else
                  {
                    Errors.warn( $str79$_A_is_not_a_valid__sbhl_type_erro, sbhl_paranoia.$sbhl_type_error_action$.getDynamicValue( thread ) );
                    Errors.cerror( $str77$continue_anyway, $str74$_A_is_not_a__A, tv_var, $sym75$SBHL_TRUE_TV_P );
                  }
                }
                ...
                try
                {
                  sbhl_search_vars.$sbhl_search_module$.bind( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module( $const68$isa ) ), thread );
                  ...
                  sbhl_module_vars.$sbhl_module$.bind( sbhl_module_utilities.get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module( $const68$isa ) ), thread );
                  if( NIL != sbhl_paranoia.suspend_sbhl_type_checkingP() || NIL != sbhl_module_utilities.apply_sbhl_module_type_test( node_var, sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                  {
                    ...
                    try
                    {
                      ...
                      sbhl_link_vars.$sbhl_link_direction$.bind( sbhl_module_utilities.sbhl_search_direction_to_link_direction( sbhl_search_vars.get_sbhl_backward_search_direction(), sbhl_module_utilities
                          .get_sbhl_transfers_through_module( sbhl_module_vars.get_sbhl_module( $const68$isa ) ) ), thread );
                      sbhl_search_vars.$genl_inverse_mode_p$.bind( NIL, thread );
                      ...
                      while ( NIL != node_var_$4)
                      {
                        ...
                        final SubLObject accessible_modules = cdolist_list_var = sbhl_macros.get_sbhl_accessible_modules( sbhl_module_vars.get_sbhl_module( $const68$isa ) );
                        ...
                        while ( NIL != cdolist_list_var)
                        {
                          ...
                          try
                          {
                            ...
                            if( NIL != sbhl_link_vars.sbhl_node_object_p( node ) )
                            {
                              ...
                              if( NIL != d_link )
                              {
                                final SubLObject mt_links = sbhl_links.get_sbhl_mt_links( d_link, sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module( $const68$isa ) ),
                                            ...
                                            try
                                            {
                                              ...
                                              if( NIL != set.set_p( sol ) )
                                              {
                                                    ...
                                                    set_contents_var ); NIL == set_contents.do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
                                                {
                                                  ...
                                                  if( NIL != set_contents.do_set_contents_element_validP( state, fn ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( fn,
                            ...
                            else if( NIL != obsolete.cnat_p( node, UNPROVIDED ) )
                            {
                              ...
                              final SubLObject new_list = cdolist_list_var_$21 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities
                                  .get_sbhl_module_relevant_naut_link_generators( sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module( $const68$isa ) ), sbhl_search_vars.$sbhl_tv$
                                      .getDynamicValue( thread ), sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ) )
                                  : sbhl_module_utilities.get_sbhl_module_relevant_naut_link_generators( sbhl_module_utilities.get_sbhl_module_backward_direction( sbhl_module_vars.get_sbhl_module( $const68$isa ) ),
                                ...
                                try
                                {
                                  ...
                                  if( NIL != set.set_p( sol2 ) )
                                  {
                                        ...
                                        set_contents_var2 ); NIL == set_contents.do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
                                    {
                                      ...
                                      if( NIL != set_contents.do_set_contents_element_validP( state2, fn3 ) && NIL == sbhl_marking_utilities.sbhl_search_path_termination_p( fn3, sbhl_marking_vars.$sbhl_gather_space$
                        ...
                        while ( NIL != cdolist_list_var2)
                        {
                          ...
                          try
                          {
                            ...
                            else if( NIL != obsolete.cnat_p( node2, UNPROVIDED ) )
                            {
                              ...
                              final SubLObject new_list2 = cdolist_list_var_$22 = ( ( NIL != sbhl_link_vars.sbhl_randomize_lists_p() ) ? list_utilities.randomize_list( sbhl_module_utilities
                  ...
                  {
                    sbhl_paranoia.sbhl_warn( TWO_INTEGER, $str83$Node__a_does_not_pass_sbhl_type_t, node_var, sbhl_module_utilities.get_sbhl_type_test( sbhl_module_vars.get_sbhl_module( UNPROVIDED ) ), UNPROVIDED,
    ...
    set.set_add( $const84$CollectionSubsetFn, $pph_subcol_fns$.getDynamicValue( thread ) );
    return $kw85$INITIALIZED;
  }
  ...
  public static SubLObject pph_subcollection_relation_naut_has_relational_nounP(final SubLObject subcol_naut)
  {
    ...
    if( NIL != success )
    {
      final SubLObject value = list_utilities.alist_lookup_without_values( v_bindings, $sym87$VALUE, UNPROVIDED, UNPROVIDED );
      final SubLObject bin_pred = list_utilities.alist_lookup_without_values( v_bindings, $sym88$BIN_PRED, UNPROVIDED, UNPROVIDED );
      final SubLObject genl = list_utilities.alist_lookup_without_values( v_bindings, $sym89$GENL, UNPROVIDED, UNPROVIDED );
      final SubLObject fn = list_utilities.alist_lookup_without_values( v_bindings, $sym90$FN, UNPROVIDED, UNPROVIDED );
      if( NIL != kb_indexing_datastructures.indexed_term_p( bin_pred ) )
      {
        ...
        try
        {
          ...
          if( NIL == ans )
          {
            ...
            while ( NIL == ans && NIL != csome_list_var)
            {
              ...
              if( denot_arg.isInteger() && NIL != kb_mapping_utilities.some_pred_value( bin_pred, arg_in_reln_pred, denot_arg, $kw46$TRUE ) )
              {
                ans = T;
              }
              csome_list_var = csome_list_var.rest();
  ...
  public static SubLObject pph_term_has_expansionP_internal(final SubLObject v_term, SubLObject mt)
  {
    ...
    if( mt == $kw92$ANY )
    {
      return kb_mapping_utilities.some_pred_value_in_any_mt( v_term, $const49$expansion, UNPROVIDED, UNPROVIDED );
    }
    SubLObject ans = NIL;
    final SubLObject mt_var = mt_relevance_macros.with_inference_mt_relevance_validate( mt );
    final SubLObject _prev_bind_0 = mt_relevance_macros.$mt$.currentBinding( thread );
    ...
    try
    {
      ...
      mt_relevance_macros.$relevant_mt_function$.bind( mt_relevance_macros.update_inference_mt_relevance_function( mt_var ), thread );
      mt_relevance_macros.$relevant_mts$.bind( mt_relevance_macros.update_inference_mt_relevance_mt_list( mt_var ), thread );
      ans = kb_mapping_utilities.some_pred_value_in_relevant_mts( v_term, $const49$expansion, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
      mt_relevance_macros.$relevant_mts$.rebind( _prev_bind_3, thread );
  ...
  public static SubLObject pph_term_has_expansionP(final SubLObject v_term, SubLObject mt)
  {
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( pph_term_has_expansionP_internal( v_term, mt ) ) );
  ...
  public static SubLObject pph_matching_verb_semtrans_assertions_internal(final SubLObject cycl, final SubLObject mt)
  {
    ...
    for( core_cycl = cycl; NIL != el_utilities.el_existential_p( core_cycl ); core_cycl = cycl_utilities.formula_arg2( core_cycl, UNPROVIDED ) )
    {
    ...
    {
      SubLObject cdolist_list_var;
      final SubLObject args = cdolist_list_var = cycl_utilities.formula_args( core_cycl, $kw94$IGNORE );
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        if( NIL != success )
        {
          final SubLObject arg2 = list_utilities.alist_lookup_without_values( v_bindings, $sym96$ARG2, UNPROVIDED, UNPROVIDED );
          final SubLObject arg3 = list_utilities.alist_lookup_without_values( v_bindings, $sym97$ARG1, UNPROVIDED, UNPROVIDED );
          if( NIL != set.set_memberP( arg3, existential_vars ) )
          {
            ...
            try
            {
              ...
              mt_relevance_macros.$relevant_mt_function$.bind( mt_relevance_macros.update_inference_mt_relevance_function( mt_var ), thread );
              mt_relevance_macros.$relevant_mts$.bind( mt_relevance_macros.update_inference_mt_relevance_mt_list( mt_var ), thread );
              final SubLObject pred_var = $const99$verbSemTrans_Canonical;
              if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( arg2, FOUR_INTEGER, pred_var ) )
              {
                ...
                while ( NIL == done_var)
                {
                  ...
                  if( NIL != valid )
                  {
                    ...
                    try
                    {
                      final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw100$GAF, $kw46$TRUE, NIL );
    ...
    return pph_utilities.pph_stable_sort_assertions( Sort.sort( set.set_element_list( ans ), $sym102$PPH_FRAME_ARITY__, $sym103$GAF_ARG3 ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject pph_matching_verb_semtrans_assertions(final SubLObject cycl, final SubLObject mt)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym93$PPH_MATCHING_VERB_SEMTRANS_ASSERTIONS, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym93$PPH_MATCHING_VERB_SEMTRANS_ASSERTIONS, TWO_INTEGER, $int104$24, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym93$PPH_MATCHING_VERB_SEMTRANS_ASSERTIONS, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( pph_matching_verb_semtrans_assertions_internal( cycl, mt ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( cycl, mt ) );
    return memoization_state.caching_results( results3 );
  }
  ...
  {
    return makeBoolean( v_object.isList() && v_object.first().eql( $const105$FunctionToArg ) && conses_high.second( v_object ).isInteger() && NIL != fort_types_interface.predicateP( conses_high.third( v_object ) ) );
  }
  ...
  {
    if( v_object.isList() && v_object.first().eql( $const106$Kappa ) )
    {
      final SubLObject vars = conses_high.second( v_object );
      final SubLObject formula = conses_high.third( v_object );
      return makeBoolean( NIL != vars && NIL == list_utilities.find_if_not( Symbols.symbol_function( $sym107$EL_VAR_ ), vars, UNPROVIDED, UNPROVIDED, UNPROVIDED ) && NIL != pph_formulaP( formula ) );
    }
    return NIL;
  }
  ...
  {
    return makeBoolean( NIL != pph_db_fieldP( v_object ) && NIL != list_utilities.cons_tree_find_if( Symbols.symbol_function( $sym107$EL_VAR_ ), v_object, UNPROVIDED ) );
  }
  ...
  public static SubLObject pph_db_fieldP(final SubLObject v_object)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym62$RELEVANT_MT_IS_EVERYTHING, thread );
      mt_relevance_macros.$mt$.bind( $const63$EverythingPSC, thread );
      ans = makeBoolean( ( NIL != kb_indexing_datastructures.indexed_term_p( v_object ) && NIL != kb_mapping_utilities.some_pred_value( v_object, $const108$fieldCodes, UNPROVIDED, UNPROVIDED ) ) || NIL != conses_high
          .member( cycl_utilities.formula_operator( v_object ), $list109, UNPROVIDED, UNPROVIDED ) );
    }
    finally
    {
      mt_relevance_macros.$mt$.rebind( _prev_bind_2, thread );
  ...
  public static SubLObject pph_db_entry_fnP(final SubLObject v_object)
  {
    ...
    return el_utilities.el_formula_with_operator_p( lispy_form, $const110$DBEntryFn );
  }
  ...
  {
    SubLFiles.declareFunction( me, "get_paraphrase_methods", "GET-PARAPHRASE-METHODS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_register_method", "PPH-REGISTER-METHOD", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_deregister_method", "PPH-DEREGISTER-METHOD", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_method_info", "PPH-METHOD-INFO", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_register_method_info", "PPH-REGISTER-METHOD-INFO", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_deregister_method_info", "PPH-DEREGISTER-METHOD-INFO", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_type_entries", "PPH-TYPE-ENTRIES", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_subtypes", "PPH-SUBTYPES", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_type_transition_demerits", "PPH-TYPE-TRANSITION-DEMERITS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_sort_methods", "PPH-SORT-METHODS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_sort_methods_by_demerits", "PPH-SORT-METHODS-BY-DEMERITS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_higher_method_qualityP", "PPH-HIGHER-METHOD-QUALITY?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_method_quality_score", "PPH-METHOD-QUALITY-SCORE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_method_quality_demerits", "PPH-METHOD-QUALITY-DEMERITS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_valid_typeP", "PPH-VALID-TYPE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_valid_method_qualityP", "PPH-VALID-METHOD-QUALITY?", 1, 0, false );
    SubLFiles.declareFunction( me, "paraphrase_test", "PARAPHRASE-TEST", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_methods_for_type", "PPH-METHODS-FOR-TYPE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_identify_type", "PPH-IDENTIFY-TYPE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_identify_type_int", "PPH-IDENTIFY-TYPE-INT", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_object_has_typeP", "PPH-OBJECT-HAS-TYPE?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_paraphrasable_objectP", "PPH-PARAPHRASABLE-OBJECT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_collection_denoting_natP", "PPH-COLLECTION-DENOTING-NAT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_nl_tagged_termP", "PPH-NL-TAGGED-TERM?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_set_nautP", "PPH-SET-NAUT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_exponent_nautP", "PPH-EXPONENT-NAUT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_unicode_naut_p", "PPH-UNICODE-NAUT-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_date_p", "PPH-DATE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_year_p", "PPH-YEAR-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_month_p", "PPH-MONTH-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_day_p", "PPH-DAY-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_time_p", "PPH-TIME-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_universal_with_atomic_scopeP", "PPH-UNIVERSAL-WITH-ATOMIC-SCOPE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_nautP", "PPH-NAUT?", 1, 0, false );
    new $pph_nautP$UnaryFunction();
    SubLFiles.declareFunction( me, "ccf_bnode_p", "CCF-BNODE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_cycl_sentenceP", "PPH-CYCL-SENTENCE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_coordination_p", "PPH-COORDINATION-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_syntactic_universal_p", "PPH-SYNTACTIC-UNIVERSAL-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_atomic_sentenceP", "PPH-ATOMIC-SENTENCE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_cycl_symbolP", "PPH-CYCL-SYMBOL?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_lexified_nartP", "PPH-LEXIFIED-NART?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_non_cycl_symbolP", "PPH-NON-CYCL-SYMBOL?", 1, 0, false );
    SubLFiles.declareFunction( me, "cyc_const_trueP", "CYC-CONST-TRUE?", 1, 0, false );
    SubLFiles.declareFunction( me, "cyc_const_falseP", "CYC-CONST-FALSE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_rel_variableP", "PPH-REL-VARIABLE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_simple_negation_p", "PPH-SIMPLE-NEGATION-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_rmp_sentenceP", "PPH-RMP-SENTENCE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_reln_has_expansionP", "PPH-RELN-HAS-EXPANSION?", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_specified_scope_sentenceP", "PPH-SPECIFIED-SCOPE-SENTENCE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_cf_implicationP", "PPH-CF-IMPLICATION?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_cff_exprP", "PPH-CFF-EXPR?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_script_expP", "PPH-SCRIPT-EXP?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_nth_exprP", "PPH-NTH-EXPR?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_skolem_termP", "PPH-SKOLEM-TERM?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_gpeP", "PPH-GPE?", 1, 0, false );
    new $pph_gpeP$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_cityP", "PPH-CITY?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_us_cityP", "PPH-US-CITY?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_non_us_cityP", "PPH-NON-US-CITY?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_hypotheticalP", "PPH-HYPOTHETICAL?", 1, 0, false );
    new $pph_hypotheticalP$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_lexical_wordP", "PPH-LEXICAL-WORD?", 1, 0, false );
    new $pph_lexical_wordP$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_universal_dateP", "PPH-UNIVERSAL-DATE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_universal_secondP", "PPH-UNIVERSAL-SECOND?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_formulaP", "PPH-FORMULA?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_rop_expressionP", "PPH-ROP-EXPRESSION?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_rop_natP", "PPH-ROP-NAT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_subcollection_relation_nautP", "PPH-SUBCOLLECTION-RELATION-NAUT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_the_collection_of_natP", "PPH-THE-COLLECTION-OF-NAT?", 1, 0, false );
    SubLFiles.declareFunction( me, "initialize_pph_subcol_fns", "INITIALIZE-PPH-SUBCOL-FNS", 0, 0, false );
    SubLFiles.declareFunction( me, "pph_subcol_fnP", "PPH-SUBCOL-FN?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_subcollection_relation_naut_has_relational_nounP", "PPH-SUBCOLLECTION-RELATION-NAUT-HAS-RELATIONAL-NOUN?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_nautP", "PPH-PHRASE-NAUT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_expandible_phrase_nautP", "PPH-EXPANDIBLE-PHRASE-NAUT?", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_term_has_expansionP_internal", "PPH-TERM-HAS-EXPANSION?-INTERNAL", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_term_has_expansionP", "PPH-TERM-HAS-EXPANSION?", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_naut_for_string", "PPH-PHRASE-NAUT-FOR-STRING", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_verb_semtrans_matcherP", "PPH-VERB-SEMTRANS-MATCHER?", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_matching_verb_semtrans_assertions_internal", "PPH-MATCHING-VERB-SEMTRANS-ASSERTIONS-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_matching_verb_semtrans_assertions", "PPH-MATCHING-VERB-SEMTRANS-ASSERTIONS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_potential_verb_semtrans_matcherP", "PPH-POTENTIAL-VERB-SEMTRANS-MATCHER?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_frame_arity_G", "PPH-FRAME-ARITY->", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_fn_to_argP", "PPH-FN-TO-ARG?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_fn_to_arg_0P", "PPH-FN-TO-ARG-0?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_kappa_exprP", "PPH-KAPPA-EXPR?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_open_db_fieldP", "PPH-OPEN-DB-FIELD?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_ground_db_fieldP", "PPH-GROUND-DB-FIELD?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_db_fieldP", "PPH-DB-FIELD?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_db_entry_fnP", "PPH-DB-ENTRY-FN?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_image_urlP", "PPH-IMAGE-URL?", 1, 1, false );
    return NIL;
  }

  public static SubLObject init_pph_types_file()
  {
    $pph_type_tree$ = SubLFiles.deflexical( "*PPH-TYPE-TREE*", $list0 );
    $paraphrase_tests$ = SubLFiles.deflexical( "*PARAPHRASE-TESTS*", $list1 );
    $pph_type_method_hash$ = SubLFiles.deflexical( "*PPH-TYPE-METHOD-HASH*", maybeDefault( $sym2$_PPH_TYPE_METHOD_HASH_, $pph_type_method_hash$, () -> ( Hashtables.make_hash_table( $int3$256, Symbols.symbol_function(
        EQ ), UNPROVIDED ) ) ) );
    $pph_valid_method_qualities$ = SubLFiles.deflexical( "*PPH-VALID-METHOD-QUALITIES*", $list4 );
    $pph_date_denoting_functions$ = SubLFiles.deflexical( "*PPH-DATE-DENOTING-FUNCTIONS*", $list38 );
    $pph_subcol_fns$ = SubLFiles.defparameter( "*PPH-SUBCOL-FNS*", set.new_set( Symbols.symbol_function( EQUAL ), UNPROVIDED ) );
    return NIL;
  }

  public static SubLObject setup_pph_types_file()
  {
    subl_macro_promotions.declare_defglobal( $sym2$_PPH_TYPE_METHOD_HASH_ );
    access_macros.register_macro_helper( $sym17$PPH_METHOD_INFO, $sym18$SUSPENDING_PARAPHRASE_METHOD );
    access_macros.register_macro_helper( $sym20$PPH_REGISTER_METHOD_INFO, $sym18$SUSPENDING_PARAPHRASE_METHOD );
    access_macros.register_macro_helper( $sym22$PPH_DEREGISTER_METHOD_INFO, $sym18$SUSPENDING_PARAPHRASE_METHOD );
    memoization_state.note_memoized_function( $sym91$PPH_TERM_HAS_EXPANSION_ );
    memoization_state.note_memoized_function( $sym93$PPH_MATCHING_VERB_SEMTRANS_ASSERTIONS );
    return NIL;
  }
  ...
  static
  {
    me = new pph_types();
    $pph_type_tree$ = null;
    $paraphrase_tests$ = null;
    $pph_type_method_hash$ = null;
    $pph_valid_method_qualities$ = null;
    $pph_date_denoting_functions$ = null;
    $pph_subcol_fns$ = null;
    $list0 = ConsesLow.list( new SubLObject[] { ConsesLow.list( makeKeyword( "OBJECT" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "LIST" ), ConsesLow.cons( makeKeyword( "ASSERTION" ), THREE_INTEGER ), ConsesLow
        .cons( makeKeyword( "CONSTANT" ), THREE_INTEGER ), ConsesLow.cons( makeKeyword( "NART" ), THREE_INTEGER ), makeKeyword( "STRING" ), makeKeyword( "NUMBER" ), makeKeyword( "SYMBOL" ) ) ), ConsesLow.list(
            makeKeyword( "LIST" ), makeSymbol( "->" ), ConsesLow.list( ConsesLow.cons( makeKeyword( "NAUT" ), THREE_INTEGER ), ConsesLow.cons( makeKeyword( "CYCL-SENTENCE" ), THREE_INTEGER ), makeKeyword(
                "ROP-EXPRESSION" ) ) ), ConsesLow.list( makeKeyword( "SYMBOL" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "VARIABLE" ), makeKeyword( "KEYWORD" ), makeKeyword( "CYCL-SYMBOL" ), makeKeyword(
                    "NON-CYCL-SYMBOL" ) ) ), ConsesLow.list( makeKeyword( "VARIABLE" ), makeSymbol( "->" ), ConsesLow.list( ConsesLow.cons( makeKeyword( "LEXIFIED-VARIABLE" ), THREE_INTEGER ) ) ), ConsesLow.list(
                        makeKeyword( "CYCL-SENTENCE" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "ATOMIC-SENTENCE" ), makeKeyword( "NEGATION" ), makeKeyword( "IMPLICATION" ), ConsesLow.cons( makeKeyword(
                            "EXISTENTIAL" ), THREE_INTEGER ), makeKeyword( "UNIVERSAL" ), makeKeyword( "COORDINATION" ), makeKeyword( "CFF-EXPR" ), makeKeyword( "CF-IMPLICATION" ) ) ), ConsesLow.list( makeKeyword(
                                "NEGATION" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "SIMPLE-NEGATION" ) ) ), ConsesLow.list( makeKeyword( "ATOMIC-SENTENCE" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword(
                                    "ISA-EXPR" ), makeKeyword( "SCRIPT-EXPANSION" ), makeKeyword( "RMP-SENTENCE" ), makeKeyword( "ABNORMAL" ), makeKeyword( "EXCEPTION" ), makeKeyword( "NTH-EXPR" ) ) ), ConsesLow.list(
                                        makeKeyword( "COORDINATION" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "CONJUNCTION" ), makeKeyword( "DISJUNCTION" ) ) ), ConsesLow.list( makeKeyword( "RMP-SENTENCE" ),
                                            makeSymbol( "->" ), ConsesLow.list( makeKeyword( "SPECIFIED-SCOPE-SENTENCE" ) ) ), ConsesLow.list( makeKeyword( "EXISTENTIAL" ), makeSymbol( "->" ), ConsesLow.list(
                                                makeKeyword( "VERB-SEMTRANS-MATCHER" ) ) ), ConsesLow.list( makeKeyword( "CONSTANT" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "TRUE" ), makeKeyword( "FALSE" ),
                                                    makeKeyword( "HYPOTHETICAL" ), makeKeyword( "LEXICAL-WORD" ), makeKeyword( "DISCOURSE-PARTICIPANT" ), makeKeyword( "GEOPOLITICAL-ENTITY" ) ) ), ConsesLow.list(
                                                        makeKeyword( "NART" ), makeSymbol( "->" ), ConsesLow.list( ConsesLow.cons( makeKeyword( "LEXIFIED-NART" ), ONE_INTEGER ), makeKeyword( "SKOLEM-TERM" ), makeKeyword(
                                                            "THE-COLLECTION-OF-NAT" ), makeKeyword( "GEOPOLITICAL-ENTITY" ) ) ), ConsesLow.list( makeKeyword( "NAUT" ), makeSymbol( "->" ), ConsesLow.list( new SubLObject[]
                                                            { makeKeyword( "PHRASE-DENOTING-NAUT" ), makeKeyword( "DATE-NAUT" ), makeKeyword( "IMAGE-URL" ), makeKeyword( "COLLECTION-DENOTING-NAT" ), ConsesLow.cons(
                                                                makeKeyword( "SKSI-SUPPORTED-EXTERNAL-TERM" ), THREE_INTEGER ), ConsesLow.cons( makeKeyword( "RESOURCE-WITH-URI" ), THREE_INTEGER ), makeKeyword(
                                                                    "SET-NAUT" ), makeKeyword( "EXPONENT-NAUT" ), makeKeyword( "UNICODE-NAUT" ), makeKeyword( "FN-TO-ARG-0" ), makeKeyword( "KAPPA-EXPR" ), makeKeyword(
                                                                        "SKOLEM-TERM" )
        } ) ), ConsesLow.list( makeKeyword( "DATE-NAUT" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "YEAR-NAUT" ), makeKeyword( "MONTH-NAUT" ), makeKeyword( "DAY-NAUT" ) ) ), ConsesLow.list( makeKeyword(
            "COLLECTION-DENOTING-NAT" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "SUBCOLLECTION-RELATION-NAUT" ), makeKeyword( "THE-COLLECTION-OF-NAT" ), makeKeyword( "NL-TAGGED-TERM" ) ) ), ConsesLow.list(
                makeKeyword( "SUBCOLLECTION-RELATION-NAUT" ), makeSymbol( "->" ), ConsesLow.list( makeKeyword( "RELATIONAL-NOUN-SUBCOLLECTION-NAUT" ) ) ), ConsesLow.list( makeKeyword( "NUMBER" ), makeSymbol( "->" ),
                    ConsesLow.list( makeKeyword( "UNIVERSAL-DATE" ), makeKeyword( "UNIVERSAL-SECOND" ), makeKeyword( "FLOAT" ) ) )
    } );
    $list1 = ConsesLow.list( new SubLObject[] { ConsesLow.cons( makeKeyword( "OBJECT" ), makeSymbol( "TRUE" ) ), ConsesLow.cons( makeKeyword( "ASSERTION" ), makeSymbol( "ASSERTION-P" ) ), ConsesLow.cons( makeKeyword(
        "LIST" ), makeSymbol( "LISTP" ) ), ConsesLow.cons( makeKeyword( "SYMBOL" ), makeSymbol( "SYMBOLP" ) ), ConsesLow.cons( makeKeyword( "NON-CYCL-SYMBOL" ), makeSymbol( "PPH-NON-CYCL-SYMBOL?" ) ), ConsesLow.cons(
            makeKeyword( "SCRIPT-EXPANSION" ), makeSymbol( "PPH-SCRIPT-EXP?" ) ), ConsesLow.cons( makeKeyword( "NTH-EXPR" ), makeSymbol( "PPH-NTH-EXPR?" ) ), ConsesLow.cons( makeKeyword( "VARIABLE" ), makeSymbol(
                "EL-VAR?" ) ), ConsesLow.cons( makeKeyword( "LEXIFIED-VARIABLE" ), makeSymbol( "PPH-VAR-PREVIOUSLY-LEXIFIED?" ) ), ConsesLow.cons( makeKeyword( "NAUT" ), makeSymbol( "PPH-NAUT?" ) ), ConsesLow.cons(
                    makeKeyword( "SKSI-SUPPORTED-EXTERNAL-TERM" ), makeSymbol( "SKSI-SUPPORTED-EXTERNAL-TERM?" ) ), ConsesLow.cons( makeKeyword( "RESOURCE-WITH-URI" ), makeSymbol( "PPH-RESOURCE-WITH-URI?" ) ), ConsesLow
                        .cons( makeKeyword( "DATE-NAUT" ), makeSymbol( "PPH-DATE-P" ) ), ConsesLow.cons( makeKeyword( "YEAR-NAUT" ), makeSymbol( "PPH-YEAR-P" ) ), ConsesLow.cons( makeKeyword( "MONTH-NAUT" ), makeSymbol(
                            "PPH-MONTH-P" ) ), ConsesLow.cons( makeKeyword( "DAY-NAUT" ), makeSymbol( "PPH-DAY-P" ) ), ConsesLow.cons( makeKeyword( "IMAGE-URL" ), makeSymbol( "PPH-IMAGE-URL?" ) ), ConsesLow.cons(
                                makeKeyword( "SET-NAUT" ), makeSymbol( "PPH-SET-NAUT?" ) ), ConsesLow.cons( makeKeyword( "EXPONENT-NAUT" ), makeSymbol( "PPH-EXPONENT-NAUT?" ) ), ConsesLow.cons( makeKeyword(
                                    "UNICODE-NAUT" ), makeSymbol( "PPH-UNICODE-NAUT-P" ) ), ConsesLow.cons( makeKeyword( "PHRASE-DENOTING-NAUT" ), makeSymbol( "PPH-PHRASE-NAUT?" ) ), ConsesLow.cons( makeKeyword(
                                        "SUBCOLLECTION-RELATION-NAUT" ), makeSymbol( "PPH-SUBCOLLECTION-RELATION-NAUT?" ) ), ConsesLow.cons( makeKeyword( "THE-COLLECTION-OF-NAT" ), makeSymbol(
                                            "PPH-THE-COLLECTION-OF-NAT?" ) ), ConsesLow.cons( makeKeyword( "RELATIONAL-NOUN-SUBCOLLECTION-NAUT" ), makeSymbol( "PPH-SUBCOLLECTION-RELATION-NAUT-HAS-RELATIONAL-NOUN?" ) ),
      ConsesLow.cons( makeKeyword( "NUMBER" ), makeSymbol( "NUMBERP" ) ), ConsesLow.cons( makeKeyword( "UNIVERSAL-DATE" ), makeSymbol( "PPH-UNIVERSAL-DATE?" ) ), ConsesLow.cons( makeKeyword( "UNIVERSAL-SECOND" ),
          makeSymbol( "PPH-UNIVERSAL-SECOND?" ) ), ConsesLow.cons( makeKeyword( "FLOAT" ), makeSymbol( "FLOATP" ) ), ConsesLow.cons( makeKeyword( "STRING" ), makeSymbol( "STRINGP" ) ), ConsesLow.cons( makeKeyword(
              "CONSTANT" ), makeSymbol( "CONSTANT-P" ) ), ConsesLow.cons( makeKeyword( "CYCL-SENTENCE" ), makeSymbol( "PPH-CYCL-SENTENCE?" ) ), ConsesLow.cons( makeKeyword( "ROP-EXPRESSION" ), makeSymbol(
                  "PPH-ROP-EXPRESSION?" ) ), ConsesLow.cons( makeKeyword( "REL-VARIABLE" ), makeSymbol( "PPH-REL-VARIABLE?" ) ), ConsesLow.cons( makeKeyword( "SKOLEM-TERM" ), makeSymbol( "PPH-SKOLEM-TERM?" ) ), ConsesLow
                      .cons( makeKeyword( "GEOPOLITICAL-ENTITY" ), makeSymbol( "PPH-GPE?" ) ), ConsesLow.cons( makeKeyword( "CITY" ), makeSymbol( "PPH-CITY?" ) ), ConsesLow.cons( makeKeyword( "US-CITY" ), makeSymbol(
                          "PPH-US-CITY?" ) ), ConsesLow.cons( makeKeyword( "NON-US-CITY" ), makeSymbol( "PPH-NON-US-CITY?" ) ), ConsesLow.cons( makeKeyword( "KEYWORD" ), makeSymbol( "KEYWORDP" ) ), ConsesLow.cons(
                              makeKeyword( "CYCL-SYMBOL" ), makeSymbol( "PPH-CYCL-SYMBOL?" ) ), ConsesLow.cons( makeKeyword( "ATOMIC-SENTENCE" ), makeSymbol( "PPH-ATOMIC-SENTENCE?" ) ), ConsesLow.cons( makeKeyword(
                                  "NEGATION" ), makeSymbol( "EL-NEGATION-P" ) ), ConsesLow.cons( makeKeyword( "SIMPLE-NEGATION" ), makeSymbol( "PPH-SIMPLE-NEGATION-P" ) ), ConsesLow.cons( makeKeyword( "RMP-SENTENCE" ),
                                      makeSymbol( "PPH-RMP-SENTENCE?" ) ), ConsesLow.cons( makeKeyword( "SPECIFIED-SCOPE-SENTENCE" ), makeSymbol( "PPH-SPECIFIED-SCOPE-SENTENCE?" ) ), ConsesLow.cons( makeKeyword(
                                          "IMPLICATION" ), makeSymbol( "EL-IMPLICATION-P" ) ), ConsesLow.cons( makeKeyword( "EXISTENTIAL" ), makeSymbol( "EL-EXISTENTIAL-P" ) ), ConsesLow.cons( makeKeyword( "UNIVERSAL" ),
                                              makeSymbol( "PPH-SYNTACTIC-UNIVERSAL-P" ) ), ConsesLow.cons( makeKeyword( "COORDINATION" ), makeSymbol( "PPH-COORDINATION-P" ) ), ConsesLow.cons( makeKeyword(
                                                  "CONJUNCTION" ), makeSymbol( "EL-CONJUNCTION-P" ) ), ConsesLow.cons( makeKeyword( "DISJUNCTION" ), makeSymbol( "EL-DISJUNCTION-P" ) ), ConsesLow.cons( makeKeyword(
                                                      "VERB-SEMTRANS-MATCHER" ), makeSymbol( "PPH-VERB-SEMTRANS-MATCHER?" ) ), ConsesLow.cons( makeKeyword( "NART" ), makeSymbol( "NART-P" ) ), ConsesLow.cons( makeKeyword(
                                                          "LEXIFIED-NART" ), makeSymbol( "PPH-LEXIFIED-NART?" ) ), ConsesLow.cons( makeKeyword( "TRUE" ), makeSymbol( "CYC-CONST-TRUE?" ) ), ConsesLow.cons( makeKeyword(
                                                              "FALSE" ), makeSymbol( "CYC-CONST-FALSE?" ) ), ConsesLow.cons( makeKeyword( "HYPOTHETICAL" ), makeSymbol( "PPH-HYPOTHETICAL?" ) ), ConsesLow.cons(
                                                                  makeKeyword( "LEXICAL-WORD" ), makeSymbol( "PPH-LEXICAL-WORD?" ) ), ConsesLow.cons( makeKeyword( "DISCOURSE-PARTICIPANT" ), makeSymbol(
                                                                      "PPH-DISCOURSE-PARTICIPANT-P" ) ), ConsesLow.cons( makeKeyword( "OTHER-CONSTANT" ), makeSymbol( "VALID-CONSTANT?" ) ), ConsesLow.cons( makeKeyword(
                                                                          "COLLECTION-DENOTING-NAT" ), makeSymbol( "PPH-COLLECTION-DENOTING-NAT?" ) ), ConsesLow.cons( makeKeyword( "NL-TAGGED-TERM" ), makeSymbol(
                                                                              "PPH-NL-TAGGED-TERM?" ) ), ConsesLow.cons( makeKeyword( "DB-FIELD" ), makeSymbol( "PPH-DB-FIELD?" ) ), ConsesLow.cons( makeKeyword(
                                                                                  "GROUND-DB-FIELD" ), makeSymbol( "PPH-GROUND-DB-FIELD?" ) ), ConsesLow.cons( makeKeyword( "OPEN-DB-FIELD" ), makeSymbol(
                                                                                      "PPH-OPEN-DB-FIELD?" ) ), ConsesLow.cons( makeKeyword( "KAPPA-EXPR" ), makeSymbol( "PPH-KAPPA-EXPR?" ) ), ConsesLow.cons( makeKeyword(
                                                                                          "FN-TO-ARG-0" ), makeSymbol( "PPH-FN-TO-ARG-0?" ) ), ConsesLow.cons( makeKeyword( "ABNORMAL" ), makeSymbol( "EL-ABNORMAL-P" ) ),
      ConsesLow.cons( makeKeyword( "EXCEPTION" ), makeSymbol( "EL-EXCEPTION-P" ) ), ConsesLow.cons( makeKeyword( "CFF-EXPR" ), makeSymbol( "PPH-CFF-EXPR?" ) ), ConsesLow.cons( makeKeyword( "CF-IMPLICATION" ), makeSymbol(
          "PPH-CF-IMPLICATION?" ) ), ConsesLow.cons( makeKeyword( "DB-ENTRY-FN" ), makeSymbol( "PPH-DB-ENTRY-FN?" ) )
    } );
    $sym2$_PPH_TYPE_METHOD_HASH_ = makeSymbol( "*PPH-TYPE-METHOD-HASH*" );
    $int3$256 = makeInteger( 256 );
    $list4 = ConsesLow.list( makeKeyword( "FALLBACK" ), makeKeyword( "DECENT" ), makeKeyword( "BEST" ) );
    $str5$__Found__D_methods_for__S___S___ = makeString( "~&Found ~D methods for ~S: ~S.~%" );
    $str6$__Found_no_methods_for__S___ = makeString( "~&Found no methods for ~S.~%" );
    $list7 = ConsesLow.list( makeSymbol( "METHOD-SYMBOL" ), makeSymbol( "METHOD-QUALITY" ) );
    $str8$__Found_paraphrase_methods_for__S = makeString( "~&Found paraphrase methods for ~S:~% ~S~%" );
    $str9$__No_paraphrase_methods_found_for = makeString( "~&No paraphrase methods found for ~S.~%" );
    $str10$_PPH_error_level_ = makeString( "(PPH error level " );
    $str11$__ = makeString( ") " );
    $str12$___S_is_not_a_valid_paraphrase_ty = makeString( "~&~S is not a valid paraphrase type.~%" );
    $str13$___S_is_not_a_symbol___ = makeString( "~&~S is not a symbol.~%" );
    $str14$___S_is_not_a_valid_paraphrase_me = makeString( "~&~S is not a valid paraphrase method quality.~%Must be in ~S.~%" );
    $sym15$FIRST = makeSymbol( "FIRST" );
    $str16$___S_is_not_a_registered_method_f = makeString( "~&~S is not a registered method for ~S.~%" );
    $sym17$PPH_METHOD_INFO = makeSymbol( "PPH-METHOD-INFO" );
    $sym18$SUSPENDING_PARAPHRASE_METHOD = makeSymbol( "SUSPENDING-PARAPHRASE-METHOD" );
    $list19 = ConsesLow.list( makeSymbol( "SYMBOL" ), makeSymbol( "QUALITY" ) );
    $sym20$PPH_REGISTER_METHOD_INFO = makeSymbol( "PPH-REGISTER-METHOD-INFO" );
    $list21 = ConsesLow.list( makeSymbol( "TYPE" ), makeSymbol( "QUALITY" ) );
    $sym22$PPH_DEREGISTER_METHOD_INFO = makeSymbol( "PPH-DEREGISTER-METHOD-INFO" );
    $sym23$LISTP = makeSymbol( "LISTP" );
    $sym24$PPH_HIGHER_METHOD_QUALITY_ = makeSymbol( "PPH-HIGHER-METHOD-QUALITY?" );
    $sym25$SECOND = makeSymbol( "SECOND" );
    $sym26$_ = makeSymbol( "<" );
    $list27 = ConsesLow.list( makeKeyword( "OBJECT" ) );
    $list28 = ConsesLow.cons( makeSymbol( "TYPE" ), makeSymbol( "MORE-TYPES" ) );
    $str29$___S___passed_test__S_for_termina = makeString( "~&~S~% passed test ~S for terminal type ~S.~%" );
    $str30$___S___passed_test__S_for_non_ter = makeString( "~&~S~% passed test ~S for non-terminal type ~S.~%" );
    $str31$___S___failed_test__S_for_non_fin = makeString( "~&~S~% failed test ~S for non-final type ~S.~%" );
    $str32$___S___failed_test__S_for_final_t = makeString( "~&~S~% failed test ~S for final type ~S.~%" );
    $str33$___S___judged_to_be_non_terminal_ = makeString( "~&~S~% judged to be non-terminal type ~S.~%" );
    $str34$___S___does_not_pass_top_level_te = makeString( "~&~S~% does not pass top-level test ~S.~%" );
    $const35$CollectionDenotingFunction = constant_handles.reader_make_constant_shell( makeString( "CollectionDenotingFunction" ) );
    $const36$TheSetOf = constant_handles.reader_make_constant_shell( makeString( "TheSetOf" ) );
    $list37 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "ExponentFn" ) ), makeKeyword( "ANYTHING" ), makeKeyword( "ANYTHING" ) );
    $list38 = ConsesLow.list( new SubLObject[] { constant_handles.reader_make_constant_shell( makeString( "CenturyFn" ) ), constant_handles.reader_make_constant_shell( makeString( "DecadeFn" ) ), constant_handles
        .reader_make_constant_shell( makeString( "YearFn" ) ), constant_handles.reader_make_constant_shell( makeString( "QuarterFn" ) ), constant_handles.reader_make_constant_shell( makeString( "MonthFn" ) ),
      constant_handles.reader_make_constant_shell( makeString( "DayFn" ) ), constant_handles.reader_make_constant_shell( makeString( "HourFn" ) ), constant_handles.reader_make_constant_shell( makeString( "MinuteFn" ) ),
      constant_handles.reader_make_constant_shell( makeString( "SecondFn" ) ), constant_handles.reader_make_constant_shell( makeString( "MilliSecondFn" ) )
    } );
    $list39 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "YearFn" ) ), ConsesLow.list( makeKeyword( "TEST" ), makeSymbol( "POSITIVE-INTEGER-P" ) ) );
    $list40 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "MonthFn" ) ), makeKeyword( "FORT" ), ConsesLow.list( makeKeyword( "TEST" ), makeSymbol( "PPH-YEAR-P" ) ) );
    $list41 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "DayFn" ) ), ConsesLow.list( makeKeyword( "TEST" ), makeSymbol( "POSITIVE-INTEGER-P" ) ), ConsesLow.list( makeKeyword( "TEST" ),
        makeSymbol( "PPH-MONTH-P" ) ) );
    $const42$Function_Denotational = constant_handles.reader_make_constant_shell( makeString( "Function-Denotational" ) );
    $list43 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "SchemaObjectFn" ) ), constant_handles.reader_make_constant_shell( makeString( "CCF-SDB-Bnode-CMLS" ) ), makeKeyword( "STRING" ) );
    $const44$TruthFunction = constant_handles.reader_make_constant_shell( makeString( "TruthFunction" ) );
    $const45$rewriteOf = constant_handles.reader_make_constant_shell( makeString( "rewriteOf" ) );
    ...
    $const49$expansion = constant_handles.reader_make_constant_shell( makeString( "expansion" ) );
    $const50$cfImplies = constant_handles.reader_make_constant_shell( makeString( "cfImplies" ) );
    $const51$compositeFieldFormat = constant_handles.reader_make_constant_shell( makeString( "compositeFieldFormat" ) );
    $const52$ScriptPredicate = constant_handles.reader_make_constant_shell( makeString( "ScriptPredicate" ) );
    $list53 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "nthLargestElement" ) ), constant_handles.reader_make_constant_shell( makeString( "nthSmallestElement" ) ) );
    $const54$Collection = constant_handles.reader_make_constant_shell( makeString( "Collection" ) );
    $const55$GeopoliticalEntity = constant_handles.reader_make_constant_shell( makeString( "GeopoliticalEntity" ) );
    $const56$InferencePSC = constant_handles.reader_make_constant_shell( makeString( "InferencePSC" ) );
    $const57$City = constant_handles.reader_make_constant_shell( makeString( "City" ) );
    $const58$USCity = constant_handles.reader_make_constant_shell( makeString( "USCity" ) );
    $const59$hypotheticalTerm = constant_handles.reader_make_constant_shell( makeString( "hypotheticalTerm" ) );
    $kw60$UNIVERSAL_DATE = makeKeyword( "UNIVERSAL-DATE" );
    $kw61$UNIVERSAL_SECOND = makeKeyword( "UNIVERSAL-SECOND" );
    ...
    $const64$TruthValue = constant_handles.reader_make_constant_shell( makeString( "TruthValue" ) );
    $const65$TheResultOfParsing = constant_handles.reader_make_constant_shell( makeString( "TheResultOfParsing" ) );
    $const66$TheCollectionOf = constant_handles.reader_make_constant_shell( makeString( "TheCollectionOf" ) );
    $const67$SubcollectionRelationFunction = constant_handles.reader_make_constant_shell( makeString( "SubcollectionRelationFunction" ) );
    $const68$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $kw69$BREADTH = makeKeyword( "BREADTH" );
    $kw70$QUEUE = makeKeyword( "QUEUE" );
    $kw71$STACK = makeKeyword( "STACK" );
    $sym72$RELEVANT_SBHL_TV_IS_GENERAL_TV = makeSymbol( "RELEVANT-SBHL-TV-IS-GENERAL-TV" );
    $kw73$ERROR = makeKeyword( "ERROR" );
    $str74$_A_is_not_a__A = makeString( "~A is not a ~A" );
    $sym75$SBHL_TRUE_TV_P = makeSymbol( "SBHL-TRUE-TV-P" );
    $kw76$CERROR = makeKeyword( "CERROR" );
    $str77$continue_anyway = makeString( "continue anyway" );
    $kw78$WARN = makeKeyword( "WARN" );
    $str79$_A_is_not_a_valid__sbhl_type_erro = makeString( "~A is not a valid *sbhl-type-error-action* value" );
    $str80$_A_is_neither_SET_P_nor_LISTP_ = makeString( "~A is neither SET-P nor LISTP." );
    $str81$attempting_to_bind_direction_link = makeString( "attempting to bind direction link variable, to NIL. macro body not executed." );
    $list82 = ConsesLow.list( makeUninternedSymbol( "LINK-NODE" ), makeUninternedSymbol( "MT" ), makeUninternedSymbol( "TV" ) );
    $str83$Node__a_does_not_pass_sbhl_type_t = makeString( "Node ~a does not pass sbhl-type-test ~a~%" );
    $const84$CollectionSubsetFn = constant_handles.reader_make_constant_shell( makeString( "CollectionSubsetFn" ) );
    $kw85$INITIALIZED = makeKeyword( "INITIALIZED" );
    $list86 = ConsesLow.list( ConsesLow.list( makeKeyword( "OR" ), constant_handles.reader_make_constant_shell( makeString( "SubcollectionOfWithRelationFromTypeFn" ) ), constant_handles.reader_make_constant_shell(
        makeString( "SubcollectionOfWithRelationToTypeFn" ) ), constant_handles.reader_make_constant_shell( makeString( "SubcollectionOfWithRelationFromFn" ) ), constant_handles.reader_make_constant_shell( makeString(
            "SubcollectionOfWithRelationToFn" ) ) ), ConsesLow.list( makeKeyword( "BIND" ), makeSymbol( "GENL" ) ), ConsesLow.list( makeKeyword( "BIND" ), makeSymbol( "BIN-PRED" ) ), ConsesLow.list( makeKeyword(
                "BIND" ), makeSymbol( "VALUE" ) ) );
    $sym87$VALUE = makeSymbol( "VALUE" );
    $sym88$BIN_PRED = makeSymbol( "BIN-PRED" );
    $sym89$GENL = makeSymbol( "GENL" );
    $sym90$FN = makeSymbol( "FN" );
    $sym91$PPH_TERM_HAS_EXPANSION_ = makeSymbol( "PPH-TERM-HAS-EXPANSION?" );
    $kw92$ANY = makeKeyword( "ANY" );
    $sym93$PPH_MATCHING_VERB_SEMTRANS_ASSERTIONS = makeSymbol( "PPH-MATCHING-VERB-SEMTRANS-ASSERTIONS" );
    $kw94$IGNORE = makeKeyword( "IGNORE" );
    $list95 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "isa" ) ), ConsesLow.list( makeKeyword( "BIND" ), makeSymbol( "ARG1" ) ), ConsesLow.list( makeKeyword( "BIND" ), makeSymbol(
        "ARG2" ) ) );
    $sym96$ARG2 = makeSymbol( "ARG2" );
    $sym97$ARG1 = makeSymbol( "ARG1" );
    $str98$Found_head_binding_conjunct_____S = makeString( "Found head-binding conjunct:~% ~S" );
    $const99$verbSemTrans_Canonical = constant_handles.reader_make_constant_shell( makeString( "verbSemTrans-Canonical" ) );
    $kw100$GAF = makeKeyword( "GAF" );
    $str101$Found_matching_vST_Canonical_____ = makeString( "Found matching vST-Canonical:~% ~S" );
    $sym102$PPH_FRAME_ARITY__ = makeSymbol( "PPH-FRAME-ARITY->" );
    $sym103$GAF_ARG3 = makeSymbol( "GAF-ARG3" );
    $int104$24 = makeInteger( 24 );
    $const105$FunctionToArg = constant_handles.reader_make_constant_shell( makeString( "FunctionToArg" ) );
    $const106$Kappa = constant_handles.reader_make_constant_shell( makeString( "Kappa" ) );
    $sym107$EL_VAR_ = makeSymbol( "EL-VAR?" );
    $const108$fieldCodes = constant_handles.reader_make_constant_shell( makeString( "fieldCodes" ) );
    $list109 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "SchemaLF" ) ), constant_handles.reader_make_constant_shell( makeString( "SchemaPF" ) ) );
    $const110$DBEntryFn = constant_handles.reader_make_constant_shell( makeString( "DBEntryFn" ) );
    $list111 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "ImageURLFn" ) ), makeKeyword( "ANYTHING" ) );
  }

  public static final class $pph_nautP$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_gpeP$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_hypotheticalP$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_lexical_wordP$UnaryFunction
      extends
        UnaryFunction
  {
