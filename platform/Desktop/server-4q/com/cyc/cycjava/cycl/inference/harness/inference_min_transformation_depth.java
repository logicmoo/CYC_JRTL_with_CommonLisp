      ...
      SubLTranslatedFile
{
  ...
  public static SubLObject min_transformation_depth_from_signature(final SubLObject signature)
  {
    ...
    if( inference_datastructures_problem.$transformation_depth_computation$.getDynamicValue( thread ) == $kw1$COUNTERINTUITIVE )
    {
  ...
  public static SubLObject propagate_min_transformation_depth_signature_via_link(final SubLObject link)
  {
    ...
    if( NIL == id_index.id_index_objects_empty_p( idx, $kw3$SKIP ) )
    {
      ...
      if( NIL == id_index.id_index_dense_objects_empty_p( idx_$1, $kw3$SKIP ) )
      {
        ...
        for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
        {
          ...
          if( NIL == id_index.id_index_tombstone_p( inference ) || NIL == id_index.id_index_skip_tombstones_p( $kw3$SKIP ) )
          {
  ...
  public static SubLObject propagate_min_transformation_depth_signature(final SubLObject problem, final SubLObject mtds, final SubLObject inference)
  {
    ...
    if( NIL != updatedP )
    {
      ...
      if( NIL == id_index.id_index_objects_empty_p( idx, $kw3$SKIP ) )
      {
        ...
        if( NIL == id_index.id_index_dense_objects_empty_p( idx_$3, $kw3$SKIP ) )
        {
          ...
          for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
          {
            ...
            if( NIL == id_index.id_index_tombstone_p( inference_$4 ) || NIL == id_index.id_index_skip_tombstones_p( $kw3$SKIP ) )
            {
              if( NIL != id_index.id_index_tombstone_p( inference_$4 ) )
              {
                inference_$4 = $kw3$SKIP;
              }
  ...
  public static SubLObject compute_residual_transformation_mtds(final SubLObject parent_mtds, final SubLObject rt_link)
  {
    ...
    if( NIL != focal_spec )
    {
      ...
      if( NIL == Errors.$ignore_mustsP$.getDynamicValue( thread ) && NIL != list_utilities.simple_tree_findP( $kw5$TRANSFORMATION, child_mtds ) )
      {
  ...
  public static SubLObject compute_clause_literal_map(final SubLObject parent_clause, final SubLObject child_clause)
  {
    ...
    if( NIL == current )
    {
      ...
      if( NIL == current_$8 )
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          final SubLObject child_index = Sequences.position( parent_neg_lit, child_neg_lits, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
          neg_lit_map = ConsesLow.cons( child_index, neg_lit_map );
          cdolist_list_var = cdolist_list_var.rest();
          parent_neg_lit = cdolist_list_var.first();
        }
        ...
        while ( NIL != cdolist_list_var)
        {
          final SubLObject child_index = Sequences.position( parent_pos_lit, child_pos_lits, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
          pos_lit_map = ConsesLow.cons( child_index, pos_lit_map );
          cdolist_list_var = cdolist_list_var.rest();
          parent_pos_lit = cdolist_list_var.first();
        }
        pos_lit_map = Sequences.nreverse( pos_lit_map );
        final SubLObject literal_map = ConsesLow.list( neg_lit_map, pos_lit_map );
        return literal_map;
      }
  ...
  {
    return makeBoolean( NIL != single_literal_problem_query_depth_signature_p( v_object ) || NIL != multi_literal_problem_query_depth_signature_p( v_object ) || NIL != multi_clause_problem_query_depth_signature_p(
        v_object ) );
  }
  ...
  {
    return makeBoolean( v_object.isCons() && NIL != list_utilities.lengthE( v_object, TWO_INTEGER, UNPROVIDED ) && v_object.first().isList() && NIL != list_utilities.every_in_list(
        $sym11$SINGLE_LITERAL_PROBLEM_QUERY_DEPTH_SIGNATURE_P, v_object.first(), UNPROVIDED ) && conses_high.second( v_object ).isList() && NIL != list_utilities.every_in_list(
            $sym11$SINGLE_LITERAL_PROBLEM_QUERY_DEPTH_SIGNATURE_P, conses_high.second( v_object ), UNPROVIDED ) );
  }
  ...
  {
    return makeBoolean( v_object.isCons() && NIL != list_utilities.lengthGE( v_object, TWO_INTEGER, UNPROVIDED ) && NIL != list_utilities.every_in_list( $sym12$MULTI_LITERAL_PROBLEM_QUERY_DEPTH_SIGNATURE_P, v_object,
        UNPROVIDED ) );
  }
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $intern_problem_query_depth_signature_caching_state$.getGlobalValue(), ConsesLow.list( pqds ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject intern_problem_query_depth_signature(final SubLObject pqds)
  {
    ...
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym13$INTERN_PROBLEM_QUERY_DEPTH_SIGNATURE, $sym15$_INTERN_PROBLEM_QUERY_DEPTH_SIGNATURE_CACHING_STATE_, NIL, EQUAL, ONE_INTEGER,
          ZERO_INTEGER );
    }
  ...
  public static SubLObject new_initial_pqds(final SubLObject problem_query, SubLObject depth)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      clause_pqds_list = ConsesLow.cons( new_initial_clause_pqds( contextualized_clause, depth ), clause_pqds_list );
      cdolist_list_var = cdolist_list_var.rest();
      contextualized_clause = cdolist_list_var.first();
    }
  ...
  public static SubLObject new_initial_clause_pqds(final SubLObject contextualized_clause, SubLObject depth)
  {
    ...
    SubLObject pqds = ConsesLow.list( ConsesLow.make_list( Sequences.length( clauses.neg_lits( contextualized_clause ) ), depth ), ConsesLow.make_list( Sequences.length( clauses.pos_lits( contextualized_clause ) ),
        depth ) );
    if( depth.isInteger() )
    {
  ...
  public static SubLObject new_subclause_pqds(final SubLObject parent_pqds, final SubLObject subclause_spec)
  {
    ...
    if( NIL == current )
    {
      ...
      if( NIL == current_$10 )
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          result_neg_lit_pqds = ConsesLow.cons( ConsesLow.nth( neg_lit_spec, neg_lit_pqds ), result_neg_lit_pqds );
          cdolist_list_var = cdolist_list_var.rest();
          neg_lit_spec = cdolist_list_var.first();
        }
        ...
        while ( NIL != cdolist_list_var)
        {
          result_pos_lit_pqds = ConsesLow.cons( ConsesLow.nth( pos_lit_spec, pos_lit_pqds ), result_pos_lit_pqds );
          cdolist_list_var = cdolist_list_var.rest();
          pos_lit_spec = cdolist_list_var.first();
        }
        ...
        return intern_problem_query_depth_signature( ConsesLow.list( result_neg_lit_pqds, result_pos_lit_pqds ) );
      }
      else
      {
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $pqds_depth_caching_state$.getGlobalValue(), ConsesLow.list( pqds, mode ), UNPROVIDED, UNPROVIDED );
  }

  @SubLTranslatedFile.SubL(source = "cycl/inference/harness/inference-min-transformation-depth.lisp", position = 18785L)
  public static SubLObject pqds_depth_internal(final SubLObject pqds, final SubLObject mode)
  {
    if( mode.eql( $kw19$UNINTUITIVE ) )
    {
      return tree_min_number( pqds );
    }
    if( mode.eql( $kw20$INTUITIVE ) )
    {
  ...
  public static SubLObject pqds_depth(final SubLObject pqds, final SubLObject mode)
  {
    ...
    final SubLObject results3 = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( pqds_depth_internal( pqds, mode ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( pqds, mode ) );
  ...
  public static SubLObject multi_literal_pqds_merge(final SubLObject pqds1, final SubLObject pqds2)
  {
    ...
    {
      merged_neg_pqds = ConsesLow.cons( single_literal_pqds_merge( lit_pqds1_$11, lit_pqds2_$12 ), merged_neg_pqds );
    ...
    while ( NIL != lit_pqds2 || NIL != lit_pqds1)
    {
      merged_pos_pqds = ConsesLow.cons( single_literal_pqds_merge( lit_pqds1_$12, lit_pqds2_$13 ), merged_pos_pqds );
      lit_pqds1 = lit_pqds1.rest();
      lit_pqds1_$12 = lit_pqds1.first();
      lit_pqds2 = lit_pqds2.rest();
      lit_pqds2_$13 = lit_pqds2.first();
    }
    final SubLObject merged_pqds = ConsesLow.list( Sequences.nreverse( merged_neg_pqds ), Sequences.nreverse( merged_pos_pqds ) );
  ...
  public static SubLObject multi_clause_pqds_merge(final SubLObject pqds1, final SubLObject pqds2)
  {
    ...
    {
      merged_pqds = ConsesLow.cons( pqds_merge( clause_pqds1_$15, clause_pqds2_$16 ), merged_pqds );
  ...
  public static SubLObject new_subclause_literal_map(final SubLObject clause, final SubLObject subclause, SubLObject missing)
  {
    ...
    if( NIL == current )
    {
      ...
      if( NIL == current_$18 )
      {
        final SubLObject map_neg_lits = ConsesLow.make_list( Sequences.length( clause_neg_lits ), missing );
        final SubLObject map_pos_lits = ConsesLow.make_list( Sequences.length( clause_pos_lits ), missing );
        ...
        {
          final SubLObject subclause_index = Sequences.position( clause_lit, subclause_neg_lits, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
        ...
        for( clause_index = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), clause_lit = list_var.first(), clause_index = Numbers.add( ONE_INTEGER, clause_index ) )
        {
          final SubLObject subclause_index = Sequences.position( clause_lit, subclause_pos_lits, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
          if( subclause_index.isInteger() )
          {
            ConsesLow.set_nth( clause_index, map_pos_lits, subclause_index );
          }
        }
        final SubLObject literal_map = ConsesLow.list( map_neg_lits, map_pos_lits );
  ...
  public static SubLObject napply_literal_map(final SubLObject source_mtds, final SubLObject literal_map, SubLObject target_mtds)
  {
    ...
    if( NIL == current )
    {
      ...
      if( NIL == current_$20 )
      {
        ...
        final SubLObject target_pos_mtds_list = target_mtds.isCons() ? conses_high.second( target_mtds ) : NIL;
  ...
  {
    return tree_min_number_recursive( tree, $kw29$POSITIVE_INFINITY );
  }
  ...
  {
    SubLFiles.declareFunction( me, "problem_min_transformation_depth_from_signature", "PROBLEM-MIN-TRANSFORMATION-DEPTH-FROM-SIGNATURE", 2, 0, false );
    SubLFiles.declareFunction( me, "min_transformation_depth_from_signature", "MIN-TRANSFORMATION-DEPTH-FROM-SIGNATURE", 1, 0, false );
    SubLFiles.declareFunction( me, "logical_tactic_lookahead_min_transformation_depth", "LOGICAL-TACTIC-LOOKAHEAD-MIN-TRANSFORMATION-DEPTH", 2, 0, false );
    SubLFiles.declareFunction( me, "propagate_min_transformation_depth_signature_via_link", "PROPAGATE-MIN-TRANSFORMATION-DEPTH-SIGNATURE-VIA-LINK", 1, 0, false );
    SubLFiles.declareFunction( me, "propagate_min_transformation_depth_signature", "PROPAGATE-MIN-TRANSFORMATION-DEPTH-SIGNATURE", 3, 0, false );
    SubLFiles.declareFunction( me, "propagate_min_transformation_depth_signature_via_link_wrt_inference", "PROPAGATE-MIN-TRANSFORMATION-DEPTH-SIGNATURE-VIA-LINK-WRT-INFERENCE", 2, 0, false );
    SubLFiles.declareFunction( me, "propagate_mtds_via_transformation_link", "PROPAGATE-MTDS-VIA-TRANSFORMATION-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "propagate_mtds_via_residual_transformation_link", "PROPAGATE-MTDS-VIA-RESIDUAL-TRANSFORMATION-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "compute_residual_transformation_mtds", "COMPUTE-RESIDUAL-TRANSFORMATION-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "propagate_mtds_via_join_ordered_link", "PROPAGATE-MTDS-VIA-JOIN-ORDERED-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "join_ordered_link_focal_mtds", "JOIN-ORDERED-LINK-FOCAL-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "join_ordered_link_non_focal_mtds", "JOIN-ORDERED-LINK-NON-FOCAL-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "join_ordered_tactic_lookahead_mtds", "JOIN-ORDERED-TACTIC-LOOKAHEAD-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "propagate_mtds_via_split_link", "PROPAGATE-MTDS-VIA-SPLIT-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "split_tactic_lookahead_mtds", "SPLIT-TACTIC-LOOKAHEAD-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "split_problem_conjunct_mtds", "SPLIT-PROBLEM-CONJUNCT-MTDS", 3, 0, false );
    SubLFiles.declareFunction( me, "split_problem_conjunct_literal_map", "SPLIT-PROBLEM-CONJUNCT-LITERAL-MAP", 2, 0, false );
    SubLFiles.declareFunction( me, "propagate_mtds_via_restriction_link", "PROPAGATE-MTDS-VIA-RESTRICTION-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "restriction_link_restricted_mtds", "RESTRICTION-LINK-RESTRICTED-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "restriction_link_literal_map", "RESTRICTION-LINK-LITERAL-MAP", 1, 0, false );
    SubLFiles.declareFunction( me, "compute_restricted_clause_literal_map", "COMPUTE-RESTRICTED-CLAUSE-LITERAL-MAP", 2, 0, false );
    SubLFiles.declareFunction( me, "compute_clause_literal_map", "COMPUTE-CLAUSE-LITERAL-MAP", 2, 0, false );
    SubLFiles.declareFunction( me, "propagate_mtds_via_union_link", "PROPAGATE-MTDS-VIA-UNION-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "union_tactic_lookahead_mtds", "UNION-TACTIC-LOOKAHEAD-MTDS", 2, 0, false );
    SubLFiles.declareFunction( me, "problem_query_depth_signature_p", "PROBLEM-QUERY-DEPTH-SIGNATURE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "single_literal_problem_query_depth_signature_p", "SINGLE-LITERAL-PROBLEM-QUERY-DEPTH-SIGNATURE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "multi_literal_problem_query_depth_signature_p", "MULTI-LITERAL-PROBLEM-QUERY-DEPTH-SIGNATURE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "multi_clause_problem_query_depth_signature_p", "MULTI-CLAUSE-PROBLEM-QUERY-DEPTH-SIGNATURE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_intern_problem_query_depth_signature", "CLEAR-INTERN-PROBLEM-QUERY-DEPTH-SIGNATURE", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_intern_problem_query_depth_signature", "REMOVE-INTERN-PROBLEM-QUERY-DEPTH-SIGNATURE", 1, 0, false );
    SubLFiles.declareFunction( me, "intern_problem_query_depth_signature_internal", "INTERN-PROBLEM-QUERY-DEPTH-SIGNATURE-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "intern_problem_query_depth_signature", "INTERN-PROBLEM-QUERY-DEPTH-SIGNATURE", 1, 0, false );
    SubLFiles.declareFunction( me, "new_initial_pqds", "NEW-INITIAL-PQDS", 1, 1, false );
    SubLFiles.declareFunction( me, "new_initial_clause_pqds", "NEW-INITIAL-CLAUSE-PQDS", 1, 1, false );
    SubLFiles.declareFunction( me, "new_subclause_pqds", "NEW-SUBCLAUSE-PQDS", 2, 0, false );
    SubLFiles.declareFunction( me, "clear_pqds_depth", "CLEAR-PQDS-DEPTH", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_pqds_depth", "REMOVE-PQDS-DEPTH", 2, 0, false );
    SubLFiles.declareFunction( me, "pqds_depth_internal", "PQDS-DEPTH-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "pqds_depth", "PQDS-DEPTH", 2, 0, false );
    SubLFiles.declareFunction( me, "pqds_merge", "PQDS-MERGE", 2, 0, false );
    SubLFiles.declareFunction( me, "single_literal_pqds_merge", "SINGLE-LITERAL-PQDS-MERGE", 2, 0, false );
    SubLFiles.declareFunction( me, "multi_literal_pqds_merge", "MULTI-LITERAL-PQDS-MERGE", 2, 0, false );
    SubLFiles.declareFunction( me, "multi_clause_pqds_merge", "MULTI-CLAUSE-PQDS-MERGE", 2, 0, false );
    SubLFiles.declareFunction( me, "new_subclause_literal_map", "NEW-SUBCLAUSE-LITERAL-MAP", 2, 1, false );
    SubLFiles.declareFunction( me, "napply_literal_map", "NAPPLY-LITERAL-MAP", 3, 0, false );
    SubLFiles.declareFunction( me, "tree_sum", "TREE-SUM", 1, 0, false );
    SubLFiles.declareFunction( me, "tree_sum_recursive", "TREE-SUM-RECURSIVE", 2, 0, false );
    SubLFiles.declareFunction( me, "tree_min_number", "TREE-MIN-NUMBER", 1, 0, false );
    SubLFiles.declareFunction( me, "tree_min_number_recursive", "TREE-MIN-NUMBER-RECURSIVE", 2, 0, false );
    return NIL;
  }
  ...
  public static SubLObject init_inference_min_transformation_depth_file()
  {
    ...
    $pqds_depth_caching_state$ = SubLFiles.deflexical( "*PQDS-DEPTH-CACHING-STATE*", NIL );
    return NIL;
  }
  ...
  static
  {
    ...
    $kw0$UNDETERMINED = makeKeyword( "UNDETERMINED" );
    $kw1$COUNTERINTUITIVE = makeKeyword( "COUNTERINTUITIVE" );
    $str2$Unexpected_logical_tactic____S = makeString( "Unexpected logical-tactic : ~S" );
    $kw3$SKIP = makeKeyword( "SKIP" );
    $str4$Time_to_support__S_propagation = makeString( "Time to support ~S propagation" );
    $kw5$TRANSFORMATION = makeKeyword( "TRANSFORMATION" );
    $str6$Propagation_error____S = makeString( "Propagation error : ~S" );
    $kw7$UNINITIALIZED = makeKeyword( "UNINITIALIZED" );
    $list8 = ConsesLow.list( makeSymbol( "PARENT-NEG-LITS" ), makeSymbol( "PARENT-POS-LITS" ) );
    $list9 = ConsesLow.list( makeSymbol( "CHILD-NEG-LITS" ), makeSymbol( "CHILD-POS-LITS" ) );
    ...
    $list16 = ConsesLow.list( makeSymbol( "NEG-LIT-PQDS" ), makeSymbol( "POS-LIT-PQDS" ) );
    $list17 = ConsesLow.list( makeSymbol( "NEG-LIT-SPECS" ), makeSymbol( "POS-LIT-SPECS" ) );
    $sym18$PQDS_DEPTH = makeSymbol( "PQDS-DEPTH" );
    $kw19$UNINTUITIVE = makeKeyword( "UNINTUITIVE" );
    $kw20$INTUITIVE = makeKeyword( "INTUITIVE" );
    ...
    $list23 = ConsesLow.list( makeSymbol( "CLAUSE-NEG-LITS" ), makeSymbol( "CLAUSE-POS-LITS" ) );
    $list24 = ConsesLow.list( makeSymbol( "SUBCLAUSE-NEG-LITS" ), makeSymbol( "SUBCLAUSE-POS-LITS" ) );
    $sym25$INTEGERP = makeSymbol( "INTEGERP" );
    $str26$Failed_literal_map__S = makeString( "Failed literal map ~S" );
    $list27 = ConsesLow.list( makeSymbol( "SOURCE-NEG-MTDS-LIST" ), makeSymbol( "SOURCE-POS-MTDS-LIST" ) );
    $list28 = ConsesLow.list( makeSymbol( "NEG-LIT-MAP" ), makeSymbol( "POS-LIT-MAP" ) );
    $kw29$POSITIVE_INFINITY = makeKeyword( "POSITIVE-INFINITY" );
  }
}
/*
 * 
