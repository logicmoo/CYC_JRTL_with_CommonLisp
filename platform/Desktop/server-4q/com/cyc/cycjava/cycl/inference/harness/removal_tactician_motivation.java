      ...
      SubLTranslatedFile
{
  ...
  public static SubLObject removal_strategy_with_strategically_active_strategem(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.listS( $sym3$CLET, ConsesLow.list( ConsesLow.list( strategem_var, strategem ), ConsesLow.list( problem, ConsesLow.list( $sym4$STRATEGEM_PROBLEM, strategem_var ) ) ), ConsesLow.list(
          $sym5$REMOVAL_STRATEGY_DEACTIVATE_STRATEGEM, strategy, strategem_var ), ConsesLow.append( body, ConsesLow.list( ConsesLow.list( $sym6$REMOVAL_STRATEGY_POSSIBLY_DEACTIVATE_PROBLEM, strategy, problem ) ) ) );
    }
  ...
  public static SubLObject removal_strategy_possibly_propagate_motivation_to_problem(final SubLObject strategy, final SubLObject problem)
  {
    ...
    if( NIL == already_motivatedP )
    {
          ...
          .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
      {
        ...
        if( NIL != set_contents.do_set_contents_element_validP( state, join_ordered_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( join_ordered_link, $kw9$JOIN_ORDERED ) )
        {
          ...
          {
            if( NIL != inference_macros.do_problem_link_open_matchP( T, link_var, mapped_problem ) && problem.eql( inference_datastructures_problem_link.mapped_problem_problem( mapped_problem ) )
                && NIL != removal_strategy_link_motivates_problemP( strategy, join_ordered_link, problem ) )
            {
              status_var = $kw10$PROVEN;
        ...
        .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state, restriction_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( restriction_link, $kw11$RESTRICTION ) )
      {
            ...
            set_contents_var_$3 ); NIL == set_contents.do_set_contents_doneP( basis_object_$4, state_$5 ); state_$5 = set_contents.do_set_contents_update_state( state_$5 ) )
        {
          ...
          if( NIL != set_contents.do_set_contents_element_validP( state_$5, join_ordered_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( join_ordered_link, $kw9$JOIN_ORDERED )
              && NIL != inference_worker_join_ordered.join_ordered_link_restricted_non_focal_linkP( join_ordered_link, restriction_link ) && non_focal_problem.eql( inference_worker_join_ordered
                  .join_ordered_link_non_focal_problem( join_ordered_link ) ) && NIL != inference_datastructures_problem_link.problem_link_openP( join_ordered_link )
        ...
        && NIL == set_contents.do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state, restriction_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( restriction_link, $kw11$RESTRICTION ) )
      {
        ...
        for( basis_object_$7 = set_contents.do_set_contents_basis_object( set_contents_var_$6 ), state_$8 = NIL, state_$8 = set_contents.do_set_contents_initial_state( basis_object_$7,
            set_contents_var_$6 ); NIL == part_of_join_orderedP && NIL == set_contents.do_set_contents_doneP( basis_object_$7, state_$8 ); state_$8 = set_contents.do_set_contents_update_state( state_$8 ) )
        {
          join_ordered_link = set_contents.do_set_contents_next( basis_object_$7, state_$8 );
          if( NIL != set_contents.do_set_contents_element_validP( state_$8, join_ordered_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( join_ordered_link, $kw9$JOIN_ORDERED )
              && NIL != inference_worker_join_ordered.join_ordered_link_restricted_non_focal_linkP( join_ordered_link, restriction_link ) && non_focal_problem.eql( inference_worker_join_ordered
  ...
  public static SubLObject removal_strategy_motivates_problem_via_rewriteP(final SubLObject strategy, final SubLObject problem)
  {
    if( NIL != inference_datastructures_problem_store.problem_store_rewrite_allowedP( inference_datastructures_strategy.strategy_problem_store( strategy ) ) )
    {
          ...
          .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
      {
        ...
        if( NIL != set_contents.do_set_contents_element_validP( state, link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( link, $kw12$REWRITE ) && NIL != inference_datastructures_strategy
            .problem_motivatedP( inference_datastructures_problem_link.problem_link_supported_problem( link ), strategy ) )
        {
          return T;
        }
      }
  ...
  public static SubLObject removal_strategy_filter_strategems_by_rlt_tactic_types_int(final SubLObject bt_strategems, final SubLObject rlt_strategems)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        strategems = ConsesLow.cons( bt_strategem, strategems );
      }
      cdolist_list_var = cdolist_list_var.rest();
      bt_strategem = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        strategems = ConsesLow.cons( rlt_strategem, strategems );
      }
      cdolist_list_var = cdolist_list_var.rest();
      rlt_strategem = cdolist_list_var.first();
    }
  ...
  public static SubLObject removal_strategy_categorize_strategems(final SubLObject strategy, final SubLObject problem)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      strategems_to_activate = ConsesLow.cons( removal_tactic, strategems_to_activate );
      cdolist_list_var = cdolist_list_var.rest();
      removal_tactic = cdolist_list_var.first();
    }
    if( NIL != best_complete_removal_tactic )
    {
      strategems_to_activate = ConsesLow.cons( best_complete_removal_tactic, strategems_to_activate );
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( meta_structural_tactic, $kw18$META_STRUCTURAL ) )
      {
        strategems_to_activate = ConsesLow.cons( meta_structural_tactic, strategems_to_activate );
      }
      cdolist_list_var = cdolist_list_var.rest();
      meta_structural_tactic = cdolist_list_var.first();
    }
  ...
  public static SubLObject removal_strategy_categorize_removal_tactics(final SubLObject strategy, final SubLObject problem, final SubLObject problem_set_asideP, final SubLObject problem_thrown_awayP)
  {
    ...
    if( NIL == problem_thrown_awayP )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( removal_tactic, $kw19$GENERALIZED_REMOVAL_OR_REWRITE ) && NIL != inference_datastructures_problem.do_problem_tactics_status_match(
            removal_tactic, $kw20$POSSIBLE ) && NIL == removal_tactician_uninterestingness.removal_strategy_chooses_to_throw_away_tacticP( strategy, removal_tactic, T, UNPROVIDED, UNPROVIDED ) )
        {
          if( NIL != problem_set_asideP || NIL != removal_tactician_uninterestingness.removal_strategy_chooses_to_set_aside_tacticP( strategy, removal_tactic, T, UNPROVIDED, UNPROVIDED ) )
          {
            if( NIL == best_complete_removal_tactic )
            {
              set_aside_removal_tactics = ConsesLow.cons( removal_tactic, set_aside_removal_tactics );
            }
          }
          else
          {
            ...
            if( pcase_var.eql( $kw21$COMPLETE ) )
            {
            ...
            else if( ( pcase_var.eql( $kw22$INCOMPLETE ) || pcase_var.eql( $kw23$GROSSLY_INCOMPLETE ) ) && ( NIL == best_complete_removal_tactic || NIL != inference_worker_removal.meta_removal_tactic_p(
                best_complete_removal_tactic ) ) )
            {
              possible_non_complete_removal_tactics = ConsesLow.cons( removal_tactic, possible_non_complete_removal_tactics );
            }
          }
        }
        cdolist_list_var = cdolist_list_var.rest();
    ...
    {
      if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( removal_tactic2, $kw19$GENERALIZED_REMOVAL_OR_REWRITE ) && NIL != inference_datastructures_problem.do_problem_tactics_status_match(
          removal_tactic2, $kw20$POSSIBLE ) && !removal_tactic2.eql( best_complete_removal_tactic ) && NIL == list_utilities.member_eqP( removal_tactic2, possible_non_complete_removal_tactics ) && NIL == list_utilities
              .member_eqP( removal_tactic2, set_aside_removal_tactics ) )
      {
        thrown_away_removal_tactics = ConsesLow.cons( removal_tactic2, thrown_away_removal_tactics );
      }
  ...
  public static SubLObject removal_strategy_categorize_disjunctive_tactics(final SubLObject strategy, final SubLObject problem, final SubLObject problem_set_asideP, final SubLObject problem_thrown_awayP)
  {
    ...
    if( NIL == problem_thrown_awayP )
    {
      ...
      {
        link = set_contents.do_set_contents_next( basis_object, state );
        if( NIL != set_contents.do_set_contents_element_validP( state, link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( link, $kw25$DISJUNCTIVE )
            && NIL == removal_strategy_link_motivates_problemP( strategy, link, UNPROVIDED ) && NIL == removal_tactician_uninterestingness.removal_strategy_chooses_to_throw_away_disjunctive_linkP( strategy, link ) )
        {
          ...
          {
            set_aside_motivation_strategems = ConsesLow.cons( disjunctive_tactic, set_aside_motivation_strategems );
          }
          else
          {
            possible_motivation_strategems = ConsesLow.cons( disjunctive_tactic, possible_motivation_strategems );
          }
        }
      }
        ...
        .do_set_contents_doneP( basis_object2, state2 ); state2 = set_contents.do_set_contents_update_state( state2 ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state2, link2 ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( link2, $kw25$DISJUNCTIVE ) )
      {
        ...
        {
          thrown_away_motivation_strategems = ConsesLow.cons( disjunctive_tactic2, thrown_away_motivation_strategems );
        }
      }
    }
    return subl_promotions.values3( possible_motivation_strategems, set_aside_motivation_strategems, thrown_away_motivation_strategems );
  ...
  public static SubLObject removal_strategy_categorize_split_tactics(final SubLObject strategy, final SubLObject problem, final SubLObject problem_set_asideP, final SubLObject problem_thrown_awayP)
  {
    ...
    if( NIL == problem_thrown_awayP )
    {
      ...
      if( NIL != split_link )
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( split_tactic, $kw26$SPLIT ) && NIL != inference_datastructures_problem.do_problem_tactics_status_match( split_tactic,
              $kw27$NON_DISCARDED ) && NIL == removal_tactician_datastructures.removal_strategy_link_head_motivatedP( strategy, split_tactic ) && NIL == removal_tactician_uninterestingness
                  .removal_strategy_chooses_to_throw_away_tacticP( strategy, split_tactic, T, UNPROVIDED, UNPROVIDED ) )
          {
            ...
            {
              set_aside_motivation_strategems = ConsesLow.cons( split_tactic, set_aside_motivation_strategems );
            }
            else
            {
              possible_motivation_strategems = ConsesLow.cons( split_tactic, possible_motivation_strategems );
            }
          }
          cdolist_list_var = cdolist_list_var.rest();
          split_tactic = cdolist_list_var.first();
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( split_tactic, $kw26$SPLIT ) && NIL != inference_datastructures_problem.do_problem_tactics_status_match( split_tactic, $kw27$NON_DISCARDED )
          && NIL == list_utilities.member_eqP( split_tactic, possible_motivation_strategems ) && NIL == list_utilities.member_eqP( split_tactic, set_aside_motivation_strategems ) )
      {
        thrown_away_motivation_strategems = ConsesLow.cons( split_tactic, thrown_away_motivation_strategems );
      }
      cdolist_list_var = cdolist_list_var.rest();
      split_tactic = cdolist_list_var.first();
    }
  ...
  public static SubLObject removal_strategy_categorize_connected_conjunction_tactics(final SubLObject strategy, final SubLObject problem, final SubLObject problem_set_asideP, final SubLObject problem_thrown_awayP)
  {
    ...
    SubLObject committed_tactic_productivity = $kw28$POSITIVE_INFINITY;
    SubLObject committed_tactic_preference = $kw29$DISALLOWED;
    ...
    if( NIL == problem_thrown_awayP )
    {
      final SubLObject type_var = $kw30$CONNECTED_CONJUNCTION;
      ...
      while ( NIL != cdolist_list_var)
      {
        if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( candidate_tactic, type_var ) && NIL == inference_tactician.some_subsuming_join_ordered_tacticP( candidate_tactic,
            subsuming_join_ordered_tactics, strategy ) )
        {
          final SubLObject link = inference_worker.logical_tactic_link( candidate_tactic );
          final SubLObject candidate_tactic_module_spec = ( NIL != inference_worker_join.join_tactic_p( candidate_tactic ) ) ? $kw31$JOIN : $kw9$JOIN_ORDERED;
          if( NIL == removal_strategy_link_motivates_problemP( strategy, link, UNPROVIDED ) && NIL == removal_tactician_uninterestingness.removal_strategy_chooses_to_throw_away_connected_conjunction_linkP( strategy,
              link ) )
          {
            ...
            {
              set_aside_motivation_strategems = ConsesLow.cons( candidate_tactic, set_aside_motivation_strategems );
            }
            else
            {
              ...
              if( NIL != magic_wandP )
              {
                candidate_tactic_preference = $kw29$DISALLOWED;
              }
              if( NIL == committed_tactic || NIL != removal_strategy_deems_conjunctive_tactic_spec_betterP( strategy, candidate_tactic, candidate_tactic_productivity, candidate_tactic_preference,
                  candidate_tactic_module_spec, candidate_tactic_literals_score, committed_tactic, committed_tactic_productivity, committed_tactic_preference, committed_tactic_module_spec,
                  committed_tactic_literals_score ) )
              {
              ...
              {
                cheap_backtracking_tactics = ConsesLow.cons( candidate_tactic, cheap_backtracking_tactics );
              }
            }
          }
        }
      ...
      if( NIL != committed_tactic )
      {
        ...
        possible_motivation_strategems = ConsesLow.cons( committed_tactic, possible_motivation_strategems );
        ...
        while ( NIL != cdolist_list_var2)
        {
          possible_motivation_strategems = ConsesLow.cons( backtracking_tactic, possible_motivation_strategems );
          cdolist_list_var2 = cdolist_list_var2.rest();
          backtracking_tactic = cdolist_list_var2.first();
        }
        possible_motivation_strategems = inference_tactician.strategy_sort( strategy, possible_motivation_strategems, $sym16$LOGICAL_TACTIC_BETTER_WRT_REMOVAL_ );
    ...
    SubLObject thrown_away_motivation_strategems = NIL;
    final SubLObject type_var2 = $kw30$CONNECTED_CONJUNCTION;
    ...
    {
      if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( conjunctive_tactic, type_var2 ) && NIL == inference_tactician.some_subsuming_join_ordered_tacticP( conjunctive_tactic,
          subsuming_join_ordered_tactics2, strategy ) && NIL == list_utilities.member_eqP( conjunctive_tactic, possible_motivation_strategems ) && NIL == list_utilities.member_eqP( conjunctive_tactic,
              set_aside_motivation_strategems ) )
      {
        thrown_away_motivation_strategems = ConsesLow.cons( conjunctive_tactic, thrown_away_motivation_strategems );
      }
  ...
  public static SubLObject removal_strategy_deems_conjunctive_tactic_spec_betterP(final SubLObject strategy, final SubLObject candidate_tactic, final SubLObject candidate_tactic_productivity,
      final SubLObject candidate_tactic_preference, final SubLObject candidate_tactic_module_spec, final SubLObject candidate_tactic_literals_score, final SubLObject committed_tactic,
      final SubLObject committed_tactic_productivity, final SubLObject committed_tactic_preference, final SubLObject committed_tactic_module_spec, final SubLObject committed_tactic_literals_score)
  {
    ...
    if( NIL != inference_datastructures_problem_store.problem_store_transformation_allowedP( inference_datastructures_tactic.tactic_store( committed_tactic ) ) && NIL != inference_datastructures_enumerated_types
        .productivity_E( candidate_tactic_productivity, committed_tactic_productivity ) && candidate_tactic_preference.eql( committed_tactic_preference ) )
    {
    ...
    return inference_tactician_utilities.strategy_deems_conjunctive_tactic_spec_betterP( candidate_tactic_productivity, candidate_tactic_preference, candidate_tactic_module_spec, candidate_tactic_literals_score,
  ...
  {
    if( NIL != ( ( NIL != inference_datastructures_problem_store.problem_store_transformation_allowedP( inference_datastructures_tactic.tactic_store( committed_tactic ) ) ) ? Equality.eq( $kw21$COMPLETE, inference_worker
        .logical_tactic_generalized_removal_completeness( committed_tactic, strategy ) ) : Equality.eq( $kw32$PREFERRED, committed_tactic_preference_level ) )
        && NIL != removal_strategy_logical_tactic_removal_backtracking_cheapP( committed_tactic, strategy ) )
    {
      return T;
    }
  ...
  public static SubLObject removal_strategy_possibly_reconsider_split_set_asides(final SubLObject strategy, final SubLObject split_tactic)
  {
    ...
    {
      final SubLObject set_aside_split_tactics = list_utilities.delete_if_not( $sym33$SPLIT_TACTIC_P, set.set_element_list( v_set ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
      SubLObject cdolist_list_var;
  ...
  {
    SubLFiles.declareMacro( me, "removal_strategy_with_strategically_active_strategem", "REMOVAL-STRATEGY-WITH-STRATEGICALLY-ACTIVE-STRATEGEM" );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_propagate_motivation_to_link_head", "REMOVAL-STRATEGY-POSSIBLY-PROPAGATE-MOTIVATION-TO-LINK-HEAD", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_propagate_motivation_to_link_head", "REMOVAL-STRATEGY-PROPAGATE-MOTIVATION-TO-LINK-HEAD", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_link_motivates_problemP", "REMOVAL-STRATEGY-LINK-MOTIVATES-PROBLEM?", 2, 1, false );
    SubLFiles.declareFunction( me, "removal_strategy_link_motivates_lookahead_problemP", "REMOVAL-STRATEGY-LINK-MOTIVATES-LOOKAHEAD-PROBLEM?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_propagate_motivation_to_problem", "REMOVAL-STRATEGY-POSSIBLY-PROPAGATE-MOTIVATION-TO-PROBLEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_activate_problem", "REMOVAL-STRATEGY-POSSIBLY-ACTIVATE-PROBLEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_problem_is_the_rest_of_a_removalP", "REMOVAL-STRATEGY-PROBLEM-IS-THE-REST-OF-A-REMOVAL?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_problem_is_the_rest_of_a_join_orderedP", "REMOVAL-STRATEGY-PROBLEM-IS-THE-REST-OF-A-JOIN-ORDERED?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_propagate_proof_spec_to_restricted_non_focals", "REMOVAL-STRATEGY-POSSIBLY-PROPAGATE-PROOF-SPEC-TO-RESTRICTED-NON-FOCALS", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_motivates_problem_via_rewriteP", "REMOVAL-STRATEGY-MOTIVATES-PROBLEM-VIA-REWRITE?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_chooses_not_to_examine_problemP", "REMOVAL-STRATEGY-CHOOSES-NOT-TO-EXAMINE-PROBLEM?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_chooses_not_to_activate_problemP", "REMOVAL-STRATEGY-CHOOSES-NOT-TO-ACTIVATE-PROBLEM?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_activate_problem", "REMOVAL-STRATEGY-ACTIVATE-PROBLEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_activate_strategems", "REMOVAL-STRATEGY-POSSIBLY-ACTIVATE-STRATEGEMS", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_filter_strategems_by_rlt_tactic_types", "REMOVAL-STRATEGY-FILTER-STRATEGEMS-BY-RLT-TACTIC-TYPES", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_filter_strategems_by_rlt_tactic_types_int", "REMOVAL-STRATEGY-FILTER-STRATEGEMS-BY-RLT-TACTIC-TYPES-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_note_new_tactic", "REMOVAL-STRATEGY-NOTE-NEW-TACTIC", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_note_split_tactics_strategically_possible", "REMOVAL-STRATEGY-NOTE-SPLIT-TACTICS-STRATEGICALLY-POSSIBLE", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_note_new_tactic_possible", "REMOVAL-STRATEGY-NOTE-NEW-TACTIC-POSSIBLE", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_categorize_strategems", "REMOVAL-STRATEGY-CATEGORIZE-STRATEGEMS", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_categorize_motivation_strategems", "REMOVAL-STRATEGY-CATEGORIZE-MOTIVATION-STRATEGEMS", 4, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_categorize_removal_tactics", "REMOVAL-STRATEGY-CATEGORIZE-REMOVAL-TACTICS", 4, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_categorize_disjunctive_tactics", "REMOVAL-STRATEGY-CATEGORIZE-DISJUNCTIVE-TACTICS", 4, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_categorize_split_tactics", "REMOVAL-STRATEGY-CATEGORIZE-SPLIT-TACTICS", 4, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_categorize_connected_conjunction_tactics", "REMOVAL-STRATEGY-CATEGORIZE-CONNECTED-CONJUNCTION-TACTICS", 4, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_deems_conjunctive_tactic_spec_betterP", "REMOVAL-STRATEGY-DEEMS-CONJUNCTIVE-TACTIC-SPEC-BETTER?", 11, 0, false );
    SubLFiles.declareFunction( me, "join_ordered_tactic_that_leads_to_a_potentially_simplifying_literalP", "JOIN-ORDERED-TACTIC-THAT-LEADS-TO-A-POTENTIALLY-SIMPLIFYING-LITERAL?", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_commits_to_no_removal_backtrackingP", "REMOVAL-STRATEGY-COMMITS-TO-NO-REMOVAL-BACKTRACKING?", 3, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_logical_tactic_removal_backtracking_cheapP", "REMOVAL-STRATEGY-LOGICAL-TACTIC-REMOVAL-BACKTRACKING-CHEAP?", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_reconsider_split_set_asides", "REMOVAL-STRATEGY-POSSIBLY-RECONSIDER-SPLIT-SET-ASIDES", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_reconsider_one_split_set_aside", "REMOVAL-STRATEGY-RECONSIDER-ONE-SPLIT-SET-ASIDE", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_clear_strategic_status", "REMOVAL-STRATEGY-POSSIBLY-CLEAR-STRATEGIC-STATUS", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_reactivate_executable_strategem", "REMOVAL-STRATEGY-REACTIVATE-EXECUTABLE-STRATEGEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_strategically_deactivate_strategem", "REMOVAL-STRATEGY-STRATEGICALLY-DEACTIVATE-STRATEGEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_deactivate_strategem", "REMOVAL-STRATEGY-DEACTIVATE-STRATEGEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_possibly_deactivate_problem", "REMOVAL-STRATEGY-POSSIBLY-DEACTIVATE-PROBLEM", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_consider_that_problem_could_be_strategically_pending", "REMOVAL-STRATEGY-CONSIDER-THAT-PROBLEM-COULD-BE-STRATEGICALLY-PENDING", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_make_problem_pending", "REMOVAL-STRATEGY-MAKE-PROBLEM-PENDING", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_strategy_problem_nothing_to_doP", "REMOVAL-STRATEGY-PROBLEM-NOTHING-TO-DO?", 2, 0, false );
    return NIL;
  }
  ...
  static
  {
    ...
    $kw9$JOIN_ORDERED = makeKeyword( "JOIN-ORDERED" );
    $kw10$PROVEN = makeKeyword( "PROVEN" );
    $kw11$RESTRICTION = makeKeyword( "RESTRICTION" );
    $kw12$REWRITE = makeKeyword( "REWRITE" );
    $sym13$PROBLEM_P = makeSymbol( "PROBLEM-P" );
    $list14 = ConsesLow.list( makeKeyword( "GENERALIZED-REMOVAL" ), makeKeyword( "CONNECTED-CONJUNCTION" ), makeKeyword( "SPLIT" ), makeKeyword( "UNION" ) );
    ...
    $kw18$META_STRUCTURAL = makeKeyword( "META-STRUCTURAL" );
    $kw19$GENERALIZED_REMOVAL_OR_REWRITE = makeKeyword( "GENERALIZED-REMOVAL-OR-REWRITE" );
    $kw20$POSSIBLE = makeKeyword( "POSSIBLE" );
    $kw21$COMPLETE = makeKeyword( "COMPLETE" );
    $kw22$INCOMPLETE = makeKeyword( "INCOMPLETE" );
    $kw23$GROSSLY_INCOMPLETE = makeKeyword( "GROSSLY-INCOMPLETE" );
    $sym24$TACTIC_STRATEGIC_PRODUCTIVITY__ = makeSymbol( "TACTIC-STRATEGIC-PRODUCTIVITY-<" );
    $kw25$DISJUNCTIVE = makeKeyword( "DISJUNCTIVE" );
    $kw26$SPLIT = makeKeyword( "SPLIT" );
    $kw27$NON_DISCARDED = makeKeyword( "NON-DISCARDED" );
    $kw28$POSITIVE_INFINITY = makeKeyword( "POSITIVE-INFINITY" );
    $kw29$DISALLOWED = makeKeyword( "DISALLOWED" );
    $kw30$CONNECTED_CONJUNCTION = makeKeyword( "CONNECTED-CONJUNCTION" );
    $kw31$JOIN = makeKeyword( "JOIN" );
    $kw32$PREFERRED = makeKeyword( "PREFERRED" );
