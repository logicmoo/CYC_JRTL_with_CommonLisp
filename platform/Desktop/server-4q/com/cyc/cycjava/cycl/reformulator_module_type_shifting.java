...
public final class reformulator_module_type_shifting
    extends
      SubLTranslatedFile
{
  ...
  {
    return makeBoolean( NIL != el_utilities.isa_litP( asent ) && NIL != cycl_variables.el_varP( cycl_utilities.sentence_arg1( asent, UNPROVIDED ) ) );
  }
  ...
  {
    return somewhere_cache.some_pred_assertion_somewhereP( $const2$typeLevelVersionInArg, pred, ONE_INTEGER, UNPROVIDED );
  }
  ...
  {
    return makeBoolean( NIL != some_tlvia_assertions_somewhereP( pred ) && NIL != kb_mapping_utilities.some_pred_value_in_relevant_mts( pred, $const2$typeLevelVersionInArg, mt, ONE_INTEGER, $kw3$TRUE ) );
  }
  ...
  public static SubLObject typeshift_clause_requiredP(final SubLObject clause, final SubLObject mt)
  {
    ...
    if( NIL != poslits && NIL != neglits && NIL != list_utilities.any_in_list( $sym4$ISA_VAR_COL_ASENT_, neglits, UNPROVIDED ) && NIL != list_utilities.any_in_list( $sym5$PRED_HAS_SOME_TLVIA_ASSERTIONS_SOMEWHERE_,
        poslits, UNPROVIDED ) )
    {
      return T;
    }
  ...
  public static SubLObject typeshift_reformulate(final SubLObject expression, final SubLObject original_clause, final SubLObject mt, final SubLObject settings)
  {
    ...
    for( clause_num = ZERO_INTEGER; NIL == new_clauses && NIL != list_var; list_var = list_var.rest(), clause = list_var.first(), clause_num = Numbers.add( ONE_INTEGER, clause_num ) )
    {
      if( NIL != typeshift_clause_requiredP( clause, mt ) )
      {
    ...
    if( NIL != new_clauses )
    {
      return Values.values( new_clauses, T );
    }
  ...
  public static SubLObject typeshift_reformulate_wrt_clause(final SubLObject v_clauses, final SubLObject clause_num, final SubLObject mt, final SubLObject settings)
  {
    ...
    for( target_poslit_num = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), target_poslit = list_var.first(), target_poslit_num = Numbers.add( ONE_INTEGER, target_poslit_num ) )
    {
  ...
  public static SubLObject typeshift_reformulate_wrt_clause_int(final SubLObject v_clauses, final SubLObject clause_num, final SubLObject mt, final SubLObject target_poslit_num, final SubLObject type_pred)
  {
    ...
    {
      SubLObject result_clauses = NIL;
      final SubLObject smitten_vars = set.new_set( Symbols.symbol_function( EQL ), UNPROVIDED );
      ...
      if( NIL != new_target_clause )
      {
        ...
        for( cdotimes_end_var = Sequences.length( v_clauses ), num = NIL, num = ZERO_INTEGER; num.numL( cdotimes_end_var ); num = Numbers.add( num, ONE_INTEGER ) )
        {
          if( num.numE( clause_num ) )
          {
            result_clauses = ConsesLow.cons( new_target_clause, result_clauses );
          }
          else
          {
            thread.resetMultipleValues();
            ...
            if( !new_clause.eql( $kw7$EXCISE ) )
            {
              ...
              result_clauses = ConsesLow.cons( new_clause, result_clauses );
            }
          }
        }
      }
  ...
  public static SubLObject typeshift_reformulate_clause(final SubLObject v_clauses, final SubLObject clause_num, final SubLObject mt, final SubLObject info_list, final SubLObject type_pred,
      final SubLObject target_poslit, final SubLObject target_poslit_num)
  {
    final SubLObject clause = ConsesLow.nth( clause_num, v_clauses );
    final SubLObject neglits = clauses.neg_lits( clause );
    final SubLObject poslits = clauses.pos_lits( clause );
    ...
    for( neglit_num = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), neglit = list_var.first(), neglit_num = Numbers.add( ONE_INTEGER, neglit_num ) )
    {
      if( sense.eql( $kw8$NEG ) && NIL != isa_var_lit_to_be_excisedP( v_clauses, sense, clause_num, neglit_num, info_list ) )
      {
        new_neglits = ConsesLow.cons( el_utilities.make_unary_formula( $const9$trueSentence, $const10$True ), new_neglits );
      }
      else
      {
        final SubLObject new_neglit = excise_dependent_skolem_vars( neglit, doomed_vars );
        new_neglits = ConsesLow.cons( new_neglit, new_neglits );
      }
    }
    ...
    for( poslit_num = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), poslit = list_var.first(), poslit_num = Numbers.add( ONE_INTEGER, poslit_num ) )
    {
      if( poslit_num.eql( target_poslit_num ) )
      {
        ...
        new_poslits = ConsesLow.cons( new_target_poslit, new_poslits );
      }
      else if( sense.eql( $kw11$POS ) && NIL != isa_var_lit_to_be_excisedP( v_clauses, sense, clause_num, poslit_num, info_list ) )
      {
        new_poslits = ConsesLow.cons( el_utilities.make_unary_formula( $const9$trueSentence, $const10$True ), new_poslits );
      }
      else
      {
        final SubLObject new_poslit = excise_dependent_skolem_vars( poslit, doomed_vars );
        new_poslits = ConsesLow.cons( new_poslit, new_poslits );
      }
    }
  ...
  public static SubLObject typeshift_reformulate_target_poslit(final SubLObject v_clauses, final SubLObject sense, final SubLObject target_poslit_clause_num, final SubLObject target_poslit_lit_num,
      final SubLObject type_pred, final SubLObject info_list)
  {
    ...
    result = el_utilities.replace_formula_arg( ZERO_INTEGER, type_pred, result );
  ...
  public static SubLObject typeshift_clause_info_list(final SubLObject v_clauses, final SubLObject mt, final SubLObject type_pred, final SubLObject target_poslit, final SubLObject target_poslit_clause_num,
      final SubLObject target_poslit_lit_num)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject pred = cycl_utilities.atomic_sentence_predicate( target_poslit );
    final SubLObject sense = sense_for_pred_and_type_pred( pred, type_pred, mt );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == current )
      {
        ...
        if( ( $kw8$NEG.eql( sense ) && NIL != cycl_variables.el_varP( var ) ) || ( $kw11$POS.eql( sense ) && NIL != term.unreified_skolem_termP( var ) ) )
        {
          if( $kw11$POS.eql( sense ) && NIL != term.unreified_skolem_termP( var ) )
          {
          ...
          {
            this_arg_okP = T;
            info_list = ConsesLow.cons( make_typeshift_info( pred_argnum, type_pred_argnum, isa_clause_num, isa_lit_num ), info_list );
          }
        }
        if( NIL == this_arg_okP )
        {
  ...
  {
    return ConsesLow.list( pred_argnum, type_pred_argnum, isa_clause_num, isa_lit_num );
  }
  ...
  public static SubLObject excise_dependent_skolem_vars(final SubLObject asent, final SubLObject doomed_vars)
  {
    ...
    try
    {
      ...
      result = cycl_utilities.expression_transform( asent, $sym13$UNREIFIED_SKOLEM_TERM_WITH_DOOMED_VARS_, $sym14$EXCISE_DOOMED_VARS_FROM_UNREIFIED_SKOLEM_TERM, NIL, UNPROVIDED );
    }
    finally
    {
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return makeBoolean( NIL != term.unreified_skolem_termP( v_term ) && NIL != keyhash_utilities.fast_intersection( skolems.skolem_function_dependent_vars( v_term ), $typeshift_doomed_vars$.getDynamicValue( thread ),
  ...
  public static SubLObject excise_doomed_vars_from_unreified_skolem_term(final SubLObject v_term)
  {
    ...
    final SubLObject new_term = el_utilities.replace_formula_arg( ONE_INTEGER, new_dependent_vars, v_term );
    return new_term;
  }
  ...
  public static SubLObject vars_to_be_excised(final SubLObject target_poslit, final SubLObject info_list)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      doomed_vars = ConsesLow.cons( var, doomed_vars );
      cdolist_list_var = cdolist_list_var.rest();
      info = cdolist_list_var.first();
    }
    return doomed_vars;
  ...
  public static SubLObject tlvia_argnum_pairs_for_pred_and_type_pred(final SubLObject pred, final SubLObject type_pred, final SubLObject mt)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym15$RELEVANT_MT_IS_GENL_MT, thread );
      mt_relevance_macros.$mt$.bind( mt, thread );
      SubLObject cdolist_list_var;
      final SubLObject assertions = cdolist_list_var = kb_mapping.gather_gaf_arg_index( pred, ONE_INTEGER, $const2$typeLevelVersionInArg, UNPROVIDED, UNPROVIDED );
      SubLObject ass = NIL;
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          result = ConsesLow.cons( ConsesLow.list( assertions_high.gaf_arg3( ass ), assertions_high.gaf_arg4( ass ) ), result );
        }
        cdolist_list_var = cdolist_list_var.rest();
        ass = cdolist_list_var.first();
      }
  ...
  public static SubLObject possible_type_preds_for_pred(final SubLObject pred, final SubLObject mt)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym15$RELEVANT_MT_IS_GENL_MT, thread );
      mt_relevance_macros.$mt$.bind( mt, thread );
      final SubLObject assertions = kb_mapping.gather_gaf_arg_index( pred, ONE_INTEGER, $const2$typeLevelVersionInArg, UNPROVIDED, UNPROVIDED );
      final SubLObject type_preds = result = Mapping.mapcar( $sym16$GAF_ARG2, assertions );
    }
    finally
    {
      mt_relevance_macros.$mt$.rebind( _prev_bind_2, thread );
    ...
    try
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        if( type_pred.eql( assertions_high.gaf_arg2( ass ) ) )
        {
          ...
          if( pcase_var.eql( $const17$forAll ) )
          {
            result = $kw8$NEG;
          }
          else if( pcase_var.eql( $const18$thereExists ) )
          {
            result = $kw11$POS;
          }
        }
  ...
  public static SubLObject isa_x_col_asent_pos(final SubLObject v_clauses, final SubLObject sense, final SubLObject x)
  {
    ...
    list_var = v_clauses;
    clause = list_var.first();
    for( clause_num = ZERO_INTEGER; NIL == result_clause_num && NIL != list_var; list_var = list_var.rest(), clause = list_var.first(), clause_num = Numbers.add( ONE_INTEGER, clause_num ) )
    {
      ...
      for( asent_num = ZERO_INTEGER; NIL == result_lit_num && NIL != list_var_$1; list_var_$1 = list_var_$1.rest(), asent = list_var_$1.first(), asent_num = Numbers.add( ONE_INTEGER, asent_num ) )
      {
  ...
  {
    if( sense.eql( $kw8$NEG ) )
    {
      return makeBoolean( NIL != el_utilities.isa_litP( asent ) && x.equal( cycl_utilities.sentence_arg1( asent, UNPROVIDED ) ) );
    }
    if( sense.eql( $kw11$POS ) )
    {
      return makeBoolean( NIL != el_utilities.isa_litP( asent ) && NIL != term.unreified_skolem_termP( cycl_utilities.sentence_arg1( asent, UNPROVIDED ) ) && x.equal( skolems.skolem_function_var( cycl_utilities
          .sentence_arg1( asent, UNPROVIDED ) ) ) );
    }
    return NIL;
  }
  ...
  public static SubLObject appears_nowhere_but(final SubLObject v_clauses, final SubLObject x, final SubLObject clause_num, final SubLObject lit_num, final SubLObject sense, final SubLObject poslit_clause_num,
      final SubLObject poslit_lit_num, final SubLObject poslit_argnum)
  {
    ...
    list_var = v_clauses;
    clause = list_var.first();
    for( cnum = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), clause = list_var.first(), cnum = Numbers.add( ONE_INTEGER, cnum ) )
    {
      ...
      for( lnum = ZERO_INTEGER; NIL != list_var_$2; list_var_$2 = list_var_$2.rest(), neglit = list_var_$2.first(), lnum = Numbers.add( ONE_INTEGER, lnum ) )
      {
        if( ( !sense.eql( $kw8$NEG ) || !cnum.eql( clause_num ) || !lnum.eql( lit_num ) ) && ( !sense.eql( $kw8$NEG ) || cnum.eql( clause_num ) || !neglit.equal( ok_lit ) ) && NIL != typeshift_expression_find( x,
            neglit ) )
        {
          return NIL;
        }
      ...
      for( lnum = ZERO_INTEGER; NIL != list_var_$3; list_var_$3 = list_var_$3.rest(), poslit = list_var_$3.first(), lnum = Numbers.add( ONE_INTEGER, lnum ) )
      {
        if( cnum.eql( poslit_clause_num ) && lnum.eql( poslit_lit_num ) )
        {
          SubLObject argnum = ZERO_INTEGER;
          SubLObject cdolist_list_var;
          final SubLObject args = cdolist_list_var = cycl_utilities.formula_args( poslit, $kw19$IGNORE );
          SubLObject arg = NIL;
        ...
        else if( ( !sense.eql( $kw11$POS ) || !cnum.eql( clause_num ) || !lnum.eql( lit_num ) ) && ( !sense.eql( $kw11$POS ) || cnum.eql( clause_num ) || !poslit.equal( ok_lit ) ) && NIL != typeshift_expression_find( x,
            poslit ) )
        {
          return NIL;
        }
  ...
  {
    SubLFiles.declareFunction( me, "isa_var_col_asentP", "ISA-VAR-COL-ASENT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pred_has_some_tlvia_assertions_somewhereP", "PRED-HAS-SOME-TLVIA-ASSERTIONS-SOMEWHERE?", 1, 0, false );
    SubLFiles.declareFunction( me, "some_tlvia_assertions_somewhereP", "SOME-TLVIA-ASSERTIONS-SOMEWHERE?", 1, 0, false );
    SubLFiles.declareFunction( me, "some_tlvia_assertionsP", "SOME-TLVIA-ASSERTIONS?", 2, 0, false );
    SubLFiles.declareFunction( me, "typeshift_required", "TYPESHIFT-REQUIRED", 3, 0, false );
    SubLFiles.declareFunction( me, "typeshift_clause_requiredP", "TYPESHIFT-CLAUSE-REQUIRED?", 2, 0, false );
    SubLFiles.declareFunction( me, "typeshift_cost", "TYPESHIFT-COST", 3, 0, false );
    SubLFiles.declareFunction( me, "typeshift_reformulate", "TYPESHIFT-REFORMULATE", 4, 0, false );
    SubLFiles.declareFunction( me, "typeshift_reformulate_wrt_clause", "TYPESHIFT-REFORMULATE-WRT-CLAUSE", 4, 0, false );
    SubLFiles.declareFunction( me, "typeshift_reformulate_wrt_clause_int", "TYPESHIFT-REFORMULATE-WRT-CLAUSE-INT", 5, 0, false );
    SubLFiles.declareFunction( me, "typeshift_reformulate_clause", "TYPESHIFT-REFORMULATE-CLAUSE", 7, 0, false );
    SubLFiles.declareFunction( me, "doomed_vars_smittenP", "DOOMED-VARS-SMITTEN?", 2, 0, false );
    SubLFiles.declareFunction( me, "typeshift_reformulate_target_poslit", "TYPESHIFT-REFORMULATE-TARGET-POSLIT", 6, 0, false );
    SubLFiles.declareFunction( me, "typeshift_clause_info_list", "TYPESHIFT-CLAUSE-INFO-LIST", 6, 0, false );
    SubLFiles.declareFunction( me, "make_typeshift_info", "MAKE-TYPESHIFT-INFO", 4, 0, false );
    SubLFiles.declareFunction( me, "typeshift_info_pred_argnum", "TYPESHIFT-INFO-PRED-ARGNUM", 1, 0, false );
    SubLFiles.declareFunction( me, "typeshift_info_type_pred_argnum", "TYPESHIFT-INFO-TYPE-PRED-ARGNUM", 1, 0, false );
    SubLFiles.declareFunction( me, "typeshift_info_isa_clause_num", "TYPESHIFT-INFO-ISA-CLAUSE-NUM", 1, 0, false );
    SubLFiles.declareFunction( me, "typeshift_info_isa_lit_num", "TYPESHIFT-INFO-ISA-LIT-NUM", 1, 0, false );
    SubLFiles.declareFunction( me, "excise_dependent_skolem_vars", "EXCISE-DEPENDENT-SKOLEM-VARS", 2, 0, false );
    SubLFiles.declareFunction( me, "unreified_skolem_term_with_doomed_varsP", "UNREIFIED-SKOLEM-TERM-WITH-DOOMED-VARS?", 1, 0, false );
    SubLFiles.declareFunction( me, "excise_doomed_vars_from_unreified_skolem_term", "EXCISE-DOOMED-VARS-FROM-UNREIFIED-SKOLEM-TERM", 1, 0, false );
    SubLFiles.declareFunction( me, "isa_var_lit_to_be_excisedP", "ISA-VAR-LIT-TO-BE-EXCISED?", 5, 0, false );
    SubLFiles.declareFunction( me, "vars_to_be_excised", "VARS-TO-BE-EXCISED", 2, 0, false );
    SubLFiles.declareFunction( me, "tlvia_argnum_pairs_for_pred_and_type_pred", "TLVIA-ARGNUM-PAIRS-FOR-PRED-AND-TYPE-PRED", 3, 0, false );
    SubLFiles.declareFunction( me, "possible_type_preds_for_pred", "POSSIBLE-TYPE-PREDS-FOR-PRED", 2, 0, false );
    SubLFiles.declareFunction( me, "sense_for_pred_and_type_pred", "SENSE-FOR-PRED-AND-TYPE-PRED", 3, 0, false );
    SubLFiles.declareFunction( me, "isa_x_col_asent_pos", "ISA-X-COL-ASENT-POS", 3, 0, false );
    SubLFiles.declareFunction( me, "isa_x_col_asentP", "ISA-X-COL-ASENT?", 3, 0, false );
    SubLFiles.declareFunction( me, "tlvia_ambiguity_violationP", "TLVIA-AMBIGUITY-VIOLATION?", 1, 0, false );
    SubLFiles.declareFunction( me, "appears_nowhere_but", "APPEARS-NOWHERE-BUT", 8, 0, false );
    SubLFiles.declareFunction( me, "typeshift_expression_find", "TYPESHIFT-EXPRESSION-FIND", 2, 0, false );
    SubLFiles.declareFunction( me, "unreified_skolem_or_default_opaque_argP", "UNREIFIED-SKOLEM-OR-DEFAULT-OPAQUE-ARG?", 2, 0, false );
    return NIL;
  }

  public static SubLObject init_reformulator_module_type_shifting_file()
  {
    $typeshift_microseconds$ = SubLFiles.deflexical( "*TYPESHIFT-MICROSECONDS*", $int6$2500 );
    $typeshift_doomed_vars$ = SubLFiles.defparameter( "*TYPESHIFT-DOOMED-VARS*", NIL );
    return NIL;
  }

  public static SubLObject setup_reformulator_module_type_shifting_file()
  {
    reformulator_module_harness.declare_rl_module( $const0$TypeShiftingRLModule, $list1 );
    return NIL;
  }
  ...
  static
  {
    me = new reformulator_module_type_shifting();
    $typeshift_microseconds$ = null;
    $typeshift_doomed_vars$ = null;
    $const0$TypeShiftingRLModule = constant_handles.reader_make_constant_shell( makeString( "TypeShiftingRLModule" ) );
    $list1 = ConsesLow.list( new SubLObject[] { makeKeyword( "REQUIRED" ), makeSymbol( "TYPESHIFT-REQUIRED" ), makeKeyword( "COST" ), makeSymbol( "TYPESHIFT-COST" ), makeKeyword( "REFORMULATE" ), makeSymbol(
        "TYPESHIFT-REFORMULATE" ), makeKeyword( "DOCUMENTATION" ), makeString(
            "Based on #$typeLevelVersionInArg assertions in the KB, will\n reformulate instance-level expressions into type-level expressions\n when possible, even if they're in the middle of a bunch of other\n stuff." ),
      makeKeyword( "EXAMPLE-SOURCE" ), makeString(
    ...
    $const2$typeLevelVersionInArg = constant_handles.reader_make_constant_shell( makeString( "typeLevelVersionInArg" ) );
    $kw3$TRUE = makeKeyword( "TRUE" );
    $sym4$ISA_VAR_COL_ASENT_ = makeSymbol( "ISA-VAR-COL-ASENT?" );
    $sym5$PRED_HAS_SOME_TLVIA_ASSERTIONS_SOMEWHERE_ = makeSymbol( "PRED-HAS-SOME-TLVIA-ASSERTIONS-SOMEWHERE?" );
    $int6$2500 = makeInteger( 2500 );
    $kw7$EXCISE = makeKeyword( "EXCISE" );
    $kw8$NEG = makeKeyword( "NEG" );
    $const9$trueSentence = constant_handles.reader_make_constant_shell( makeString( "trueSentence" ) );
    $const10$True = constant_handles.reader_make_constant_shell( makeString( "True" ) );
    $kw11$POS = makeKeyword( "POS" );
    $list12 = ConsesLow.list( makeSymbol( "PRED-ARGNUM" ), makeSymbol( "TYPE-PRED-ARGNUM" ) );
    $sym13$UNREIFIED_SKOLEM_TERM_WITH_DOOMED_VARS_ = makeSymbol( "UNREIFIED-SKOLEM-TERM-WITH-DOOMED-VARS?" );
    $sym14$EXCISE_DOOMED_VARS_FROM_UNREIFIED_SKOLEM_TERM = makeSymbol( "EXCISE-DOOMED-VARS-FROM-UNREIFIED-SKOLEM-TERM" );
    $sym15$RELEVANT_MT_IS_GENL_MT = makeSymbol( "RELEVANT-MT-IS-GENL-MT" );
    $sym16$GAF_ARG2 = makeSymbol( "GAF-ARG2" );
    $const17$forAll = constant_handles.reader_make_constant_shell( makeString( "forAll" ) );
    $const18$thereExists = constant_handles.reader_make_constant_shell( makeString( "thereExists" ) );
    $kw19$IGNORE = makeKeyword( "IGNORE" );
    $sym20$UNREIFIED_SKOLEM_OR_DEFAULT_OPAQUE_ARG_ = makeSymbol( "UNREIFIED-SKOLEM-OR-DEFAULT-OPAQUE-ARG?" );
  }
}
/*
 * 
