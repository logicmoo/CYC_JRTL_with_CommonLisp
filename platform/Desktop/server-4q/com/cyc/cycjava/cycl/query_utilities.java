...
public final class query_utilities
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject get_possible_sentence_combinations_wrt_var(final SubLObject sentence1, final SubLObject mt1, final SubLObject sentence2, final SubLObject mt2, final SubLObject variable)
  {
    ...
    if( mt1.eql( $const0$EverythingPSC ) || mt1.eql( $const1$InferencePSC ) )
    {
      ...
      try
      {
        ...
        mt_relevance_macros.$mt$.bind( $const0$EverythingPSC, thread );
      ...
      all_variables = Sequences.remove_duplicates( all_variables, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    else if( mt2.eql( $const0$EverythingPSC ) || mt2.eql( $const1$InferencePSC ) )
    {
      final SubLObject _prev_bind_0 = mt_relevance_macros.$relevant_mt_function$.currentBinding( thread );
      final SubLObject _prev_bind_2 = mt_relevance_macros.$mt$.currentBinding( thread );
      try
    ...
    else
    {
      ...
      temp_variables = ( cdolist_list_var = Sequences.remove_duplicates( temp_variables, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
      SubLObject var = NIL;
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          all_variables = ConsesLow.cons( var, all_variables );
        }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      if( NIL != v_term )
      {
        replaceable = ConsesLow.cons( v_term, replaceable );
      }
      else
      {
        replaceable = ConsesLow.cons( combine_variable, replaceable );
      }
  ...
  public static SubLObject qu_get_possible_sentence_combinations_wrt_var_in_mt(final SubLObject sentence1, final SubLObject sentence2, final SubLObject mt, final SubLObject variable, final SubLObject var_to_term_dict)
  {
    ...
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( var_to_term_dict ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
  ...
  public static SubLObject qu_get_variable_combinations_replacing_term(final SubLObject variable, final SubLObject isa_constraints, final SubLObject genl_constraints, final SubLObject sentence, final SubLObject mt,
      final SubLObject v_term, final SubLObject replacing_var)
  {
    final SubLObject replaced_sentence = conses_high.subst( replacing_var, v_term, sentence, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject qu_get_variable_combinations(final SubLObject variable, final SubLObject isa_constraints, final SubLObject genl_constraints, final SubLObject sentence, final SubLObject mt)
  {
    ...
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( sentence_arg_constraints ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
      final SubLObject var = dictionary_contents.do_dictionary_contents_key_value( iteration_state );
      ...
      thread.resetMultipleValues();
      if( NIL != var && NIL != subl_promotions.memberP( var, sentence_vars, UNPROVIDED, UNPROVIDED ) && NIL != qu_constraints_combination_okP( variable, isa_constraints, isa_genl_quoted_isa_consts.first(), $kw4$ISA, mt )
          && NIL != qu_constraints_combination_okP( variable, genl_constraints, conses_high.second( isa_genl_quoted_isa_consts ), $kw5$GENLS, mt ) )
      {
        replaceable_variables = ConsesLow.cons( var, replaceable_variables );
      }
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  public static SubLObject qu_constraints_combination_okP(final SubLObject var, final SubLObject constraints1, final SubLObject constraints2, final SubLObject type, final SubLObject mt)
  {
    ...
    total_constraints = ConsesLow.cons( var, total_constraints );
    return at_var_types.constraint_var_types_okP( ConsesLow.list( total_constraints ), type, mt );
  }
  ...
  public static SubLObject qu_get_replaceable_variables_for_terms(final SubLObject sentence)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      replaced_sent = conses_high.subst( new_var, v_term, replaced_sent, Symbols.symbol_function( EQUAL ), UNPROVIDED );
      cdolist_list_var = cdolist_list_var.rest();
      v_term = cdolist_list_var.first();
    }
  ...
  {
    SubLObject args = NIL;
    if( NIL != el_utilities.el_logical_operator_formula_p( sentence ) || NIL != el_utilities.possibly_el_quantified_sentence_p( sentence ) )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        if( NIL != el_utilities.el_formula_p( sub_sentence ) )
        {
      ...
      args = Sequences.remove_duplicates( args, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    else
    {
      ...
      while ( NIL != cdolist_list_var2)
      {
        ...
        {
          args = ConsesLow.cons( formula_arg, args );
        }
        cdolist_list_var2 = cdolist_list_var2.rest();
        formula_arg = cdolist_list_var2.first();
      }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return makeBoolean( !$indexicalized_query_term$.getDynamicValue( thread ).equal( expression ) && NIL != kb_accessors.indexical_p( expression ) && NIL == backward.removal_ask_variable( $sym9$_REFERENT, ConsesLow
        .listS( $const10$indexicalReferent, expression, $list11 ), $const1$InferencePSC, UNPROVIDED, UNPROVIDED ) && NIL == relation_evaluation.cyc_evaluate( expression ) && NIL == cycl_utilities.expression_find(
            expression, $indexicalized_query_term$.getDynamicValue( thread ), NIL, Symbols.symbol_function( EQUAL ), UNPROVIDED ) );
  }
  ...
  public static SubLObject indexicalized_query_substitute_term(final SubLObject v_term, final SubLObject raw_expression)
  {
    ...
    try
    {
      ...
      ans = cycl_utilities.expression_transform( cycl_utilities.hl_to_el( raw_expression ), $sym12$INDEXICALIZED_QUERY_INDEXICAL_P, $sym13$INDEXICALIZED_QUERY_TERM_FOR_INDEXICAL, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
  ...
  {
    SubLFiles.declareFunction( me, "get_possible_sentence_combinations_wrt_argpos", "GET-POSSIBLE-SENTENCE-COMBINATIONS-WRT-ARGPOS", 5, 0, false );
    SubLFiles.declareFunction( me, "get_possible_sentence_combinations_wrt_var", "GET-POSSIBLE-SENTENCE-COMBINATIONS-WRT-VAR", 5, 0, false );
    SubLFiles.declareFunction( me, "qu_get_possible_sentence_combinations_wrt_var_in_mt", "QU-GET-POSSIBLE-SENTENCE-COMBINATIONS-WRT-VAR-IN-MT", 5, 0, false );
    SubLFiles.declareFunction( me, "qu_get_variable_combinations_replacing_term", "QU-GET-VARIABLE-COMBINATIONS-REPLACING-TERM", 7, 0, false );
    SubLFiles.declareFunction( me, "qu_get_variable_combinations", "QU-GET-VARIABLE-COMBINATIONS", 5, 0, false );
    SubLFiles.declareFunction( me, "qu_constraints_combination_okP", "QU-CONSTRAINTS-COMBINATION-OK?", 5, 0, false );
    SubLFiles.declareFunction( me, "qu_get_replaceable_variables_for_terms", "QU-GET-REPLACEABLE-VARIABLES-FOR-TERMS", 1, 0, false );
    SubLFiles.declareFunction( me, "qu_get_args_recursive", "QU-GET-ARGS-RECURSIVE", 1, 0, false );
    SubLFiles.declareFunction( me, "new_cyc_query_from_indexicalized_kbq", "NEW-CYC-QUERY-FROM-INDEXICALIZED-KBQ", 2, 1, false );
    SubLFiles.declareFunction( me, "indexicalized_query_term_for_indexical", "INDEXICALIZED-QUERY-TERM-FOR-INDEXICAL", 1, 0, false );
    SubLFiles.declareFunction( me, "indexicalized_query_indexical_p", "INDEXICALIZED-QUERY-INDEXICAL-P", 1, 0, false );
    SubLFiles.declareFunction( me, "indexicalized_query_substitute_term", "INDEXICALIZED-QUERY-SUBSTITUTE-TERM", 2, 0, false );
    return NIL;
  }

  public static SubLObject init_query_utilities_file()
  {
    $indexicalized_query_term$ = SubLFiles.defparameter( "*INDEXICALIZED-QUERY-TERM*", $kw8$UNINITIALIZED );
    return NIL;
  }
  ...
  static
  {
    me = new query_utilities();
    $indexicalized_query_term$ = null;
    ...
    $kw4$ISA = makeKeyword( "ISA" );
    $kw5$GENLS = makeKeyword( "GENLS" );
    $sym6$PROPERTY_LIST_P = makeSymbol( "PROPERTY-LIST-P" );
    $sym7$QUERY_PROPERTY_P = makeSymbol( "QUERY-PROPERTY-P" );
    $kw8$UNINITIALIZED = makeKeyword( "UNINITIALIZED" );
    $sym9$_REFERENT = makeSymbol( "?REFERENT" );
    $const10$indexicalReferent = constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) );
    $list11 = ConsesLow.list( makeSymbol( "?REFERENT" ) );
    $sym12$INDEXICALIZED_QUERY_INDEXICAL_P = makeSymbol( "INDEXICALIZED-QUERY-INDEXICAL-P" );
    $sym13$INDEXICALIZED_QUERY_TERM_FOR_INDEXICAL = makeSymbol( "INDEXICALIZED-QUERY-TERM-FOR-INDEXICAL" );
  }
}
/*
 * 
