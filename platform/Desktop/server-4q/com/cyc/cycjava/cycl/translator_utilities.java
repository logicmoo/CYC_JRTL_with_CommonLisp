...
public final class translator_utilities
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject td_translation_modules_memoized()
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym1$TD_TRANSLATION_MODULES_MEMOIZED, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym1$TD_TRANSLATION_MODULES_MEMOIZED, ZERO_INTEGER, NIL, EQ, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym1$TD_TRANSLATION_MODULES_MEMOIZED, caching_state );
    }
    SubLObject results = memoization_state.caching_state_get_zero_arg_results( caching_state, UNPROVIDED );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( td_translation_modules_memoized_internal() ) );
  ...
  {
    return Sequences.position( module, td_translation_modules_memoized(), Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject td_module_translation_position(final SubLObject module)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym2$TD_MODULE_TRANSLATION_POSITION, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym2$TD_MODULE_TRANSLATION_POSITION, ONE_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym2$TD_MODULE_TRANSLATION_POSITION, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( td_module_translation_position_internal( module ) ) );
  ...
  public static SubLObject td_sort_modules_by_load_order(final SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        result = Sort.sort( conses_high.copy_list( v_modules ), Symbols.symbol_function( $sym3$TD_MODULE_EARLIER ), UNPROVIDED );
      }
      finally
      {
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    assert NIL != td_module_p( module ) : module;
    final SubLObject pcase_var = $xref_database_mode$.getDynamicValue( thread );
    if( pcase_var.eql( $kw0$NEW ) )
    {
  ...
  public static SubLObject td_global_binding_type(final SubLObject global)
  {
    ...
    if( pcase_var.eql( $kw6$DYNAMIC ) )
    {
      return $kw7$SPECIAL;
    }
    if( pcase_var.eql( $kw8$UNSPECIFIED ) )
    {
      return NIL;
    }
    return binding_type;
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    assert NIL != Types.symbolp( client_method ) : client_method;
    assert NIL != Types.symbolp( method ) : method;
    final SubLObject pcase_var = $xref_database_mode$.getDynamicValue( thread );
    if( pcase_var.eql( $kw0$NEW ) )
    {
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    assert NIL != td_module_p( server_module ) : server_module;
    final SubLObject pcase_var = $xref_database_mode$.getDynamicValue( thread );
    if( pcase_var.eql( $kw0$NEW ) )
    {
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    assert NIL != td_module_p( client_module ) : client_module;
    final SubLObject pcase_var = $xref_database_mode$.getDynamicValue( thread );
    if( pcase_var.eql( $kw0$NEW ) )
    {
  ...
  public static SubLObject td_modules_implied_by_methods(final SubLObject v_methods, SubLObject v_modules)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == conses_high.member( item_var, v_modules, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
      {
  ...
  public static SubLObject td_modules_cluster_containing_module(final SubLObject seed_module, SubLObject verboseP)
  {
    ...
    final SubLObject mark_space = set.new_set( Symbols.symbol_function( EQUALP ), UNPROVIDED );
    SubLObject unvisited = NIL;
    unvisited = ConsesLow.cons( seed_module, unvisited );
    while ( NIL != unvisited)
    {
      ...
      if( NIL != td_module_p( current ) )
      {
        if( NIL != verboseP )
        {
          PrintLow.format( T, $str9$Checking__A_for_dependencies___, current );
        }
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          {
            unvisited = ConsesLow.cons( neighbor, unvisited );
          }
          cdolist_list_var = cdolist_list_var.rest();
          neighbor = cdolist_list_var.first();
        }
  ...
  public static SubLObject td_modules_properly_dependent_on_modules(final SubLObject leaf_modules, SubLObject dependent_modules)
  {
    ...
    return conses_high.set_difference( td_modules_dependent_on_modules( leaf_modules, dependent_modules ), leaf_modules, EQUAL, UNPROVIDED );
  }
  ...
  public static SubLObject td_modules_dependent_on_module_methods(final SubLObject leaf_module, SubLObject dependent_modules)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$3)
      {
        ...
        if( NIL == conses_high.member( item_var, dependent_modules, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
        {
          dependent_modules = ConsesLow.cons( item_var, dependent_modules );
        }
      ...
      while ( NIL != cdolist_list_var_$4)
      {
        ...
        if( NIL == conses_high.member( item_var2, dependent_modules, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
        {
          dependent_modules = ConsesLow.cons( item_var2, dependent_modules );
        }
  ...
  public static SubLObject td_explain_how_module_depends_on_module(final SubLObject superior, final SubLObject inferior)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$8)
      {
        ...
        {
          final SubLObject item_var = client_method;
          if( NIL == conses_high.member( item_var, globals, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
          {
            globals = ConsesLow.cons( item_var, globals );
          }
        }
  ...
  public static SubLObject td_explain_how_module_depends_on_modules(final SubLObject superiors, final SubLObject inferior)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != v_methods )
      {
        ...
        {
          all_methods = conses_high.union( v_methods, all_methods, UNPROVIDED, UNPROVIDED );
        }
      }
      if( NIL != globals )
      {
        ...
        {
          all_globals = conses_high.union( globals, all_globals, UNPROVIDED, UNPROVIDED );
        }
      }
      cdolist_list_var = cdolist_list_var.rest();
      superior = cdolist_list_var.first();
  ...
  public static SubLObject td_method_calls_transitively(final SubLObject calling_method)
  {
    ...
    try
    {
      $td_methods_called_transitively$.bind( set.new_set( Symbols.symbol_function( EQ ), UNPROVIDED ), thread );
      $td_globals_called_transitively$.bind( set.new_set( Symbols.symbol_function( EQ ), UNPROVIDED ), thread );
    ...
    called_methods = Sort.sort( called_methods, Symbols.symbol_function( $sym10$STRING_LESSP ), Symbols.symbol_function( $sym11$SYMBOL_NAME ) );
    called_globals = Sort.sort( called_globals, Symbols.symbol_function( $sym10$STRING_LESSP ), Symbols.symbol_function( $sym11$SYMBOL_NAME ) );
  ...
  public static SubLObject td_method_references_modules_transitively(final SubLObject calling_method)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != referenced_module )
      {
        ...
        if( NIL == conses_high.member( item_var, referenced_modules, Symbols.symbol_function( EQUAL ), Symbols.symbol_function( IDENTITY ) ) )
        {
          referenced_modules = ConsesLow.cons( item_var, referenced_modules );
        }
      }
      cdolist_list_var = cdolist_list_var.rest();
      called_method = cdolist_list_var.first();
    ...
    referenced_modules = Sort.sort( referenced_modules, Symbols.symbol_function( $sym10$STRING_LESSP ), UNPROVIDED );
  ...
  public static SubLObject td_method_latest_transitively_referenced_module(final SubLObject calling_method)
  {
    ...
    final SubLObject latest_module = list_utilities.extremal( referenced_modules, Symbols.symbol_function( $sym12$TD_MODULE_LATER ), UNPROVIDED );
  ...
  public static SubLObject td_method_latest_transitively_referenced_module_memoized(final SubLObject referenced_method)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym13$TD_METHOD_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym13$TD_METHOD_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED, ONE_INTEGER, NIL, EQ,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym13$TD_METHOD_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( td_method_latest_transitively_referenced_module_memoized_internal( referenced_method ) ) );
  ...
  public static SubLObject td_justify_method_latest_transitively_referenced_module(final SubLObject calling_method, final SubLObject latest_module)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        justifying_methods = ConsesLow.cons( called_method, justifying_methods );
      }
      cdolist_list_var = cdolist_list_var.rest();
      called_method = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        justifying_globals = ConsesLow.cons( called_global, justifying_globals );
      }
      cdolist_list_var = cdolist_list_var.rest();
      called_global = cdolist_list_var.first();
    }
  ...
  {
    final SubLObject seen_table = dictionary.new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED );
    ...
    while ( NIL == queues.queue_empty_p( unvisited_queue ))
    {
      ...
      if( NIL == foundP )
      {
        ...
        {
          if( $kw14$UNSEEN == dictionary.dictionary_lookup_without_values( seen_table, called_method, $kw14$UNSEEN ) )
          {
    ...
    if( NIL != foundP )
    {
      ...
      {
        call_path = ConsesLow.cons( method2, call_path );
      }
    }
  ...
  public static SubLObject td_global_latest_transitively_referenced_module_memoized(final SubLObject referenced_global)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym15$TD_GLOBAL_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym15$TD_GLOBAL_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED, ONE_INTEGER, NIL, EQ,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym15$TD_GLOBAL_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( td_global_latest_transitively_referenced_module_memoized_internal( referenced_global ) ) );
  ...
  public static SubLObject incongruent_features(final SubLObject defined_features, final SubLObject referenced_features)
  {
    ...
    final SubLObject implication = ConsesLow.list( $kw16$OR, ConsesLow.list( $kw17$NOT, defined_prop_sentence ), referenced_prop_sentence );
    ...
    try
    {
      prop_sentence_clausifier.$prop_sentence_and$.bind( $kw18$AND, thread );
      prop_sentence_clausifier.$prop_sentence_or$.bind( $kw16$OR, thread );
      prop_sentence_clausifier.$prop_sentence_not$.bind( $kw17$NOT, thread );
      prop_sentence_clausifier.$prop_sentence_true$.bind( T, thread );
      prop_sentence_clausifier.$prop_sentence_false$.bind( NIL, thread );
      incongruence_explanation = prop_sentence_clausifier.prop_sentence_simplify( prop_sentence_clausifier.prop_sentence_to_dnf( prop_sentence_clausifier.prop_sentence_negate( prop_sentence_clausifier
          .prop_sentence_simplify( prop_sentence_clausifier.prop_sentence_to_cnf( implication ) ) ) ) );
    }
    finally
    {
  ...
  {
    return transform_list_utilities.transform( v_object, Symbols.symbol_function( $sym19$NON_FEATURE_SYMBOL_P ), Symbols.symbol_function( $sym20$TRANSFORM_NON_FEATURE_SYMBOL ), UNPROVIDED );
  }
  ...
  public static SubLObject all_module_level_feature_problems(SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          utilities_macros.$progress_note$.bind( $str21$Identifying_module_level_feature_, thread );
          ...
          try
          {
            ...
            while ( NIL != csome_list_var)
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                all_feature_problems = ConsesLow.cons( feature_problem, all_feature_problems );
                cdolist_list_var = cdolist_list_var.rest();
                feature_problem = cdolist_list_var.first();
              }
  ...
  public static SubLObject module_level_feature_problems(final SubLObject defined_module)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$12)
      {
        ...
        if( NIL != referenced_module )
        {
          ...
          if( NIL != incongruence )
          {
            feature_problems = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module, defined_features ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module,
      ...
      while ( NIL != cdolist_list_var_$13)
      {
        ...
        if( NIL != referenced_module )
        {
          ...
          if( NIL != incongruence )
          {
            feature_problems = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module, defined_features ), ConsesLow.list( $kw24$GLOBAL, referenced_global, referenced_module,
                referenced_features ), incongruence ), feature_problems );
          }
        }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$14)
      {
        ...
        if( NIL != referenced_module )
        {
          ...
          if( NIL != incongruence )
          {
            feature_problems = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module, defined_features ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module,
                referenced_features ), incongruence ), feature_problems );
          }
        }
      ...
      while ( NIL != cdolist_list_var_$15)
      {
        ...
        if( NIL != referenced_module )
        {
          ...
          if( NIL != incongruence )
          {
            feature_problems = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module, defined_features ), ConsesLow.list( $kw24$GLOBAL, referenced_global, referenced_module,
                referenced_features ), incongruence ), feature_problems );
          }
        }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      if( NIL != referenced_module2 )
      {
        ...
        {
          feature_problems = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module, defined_features ), ConsesLow.list( $kw23$METHOD, referenced_method2, referenced_module2, referenced_features2 ),
              incongruence2 ), feature_problems );
        }
      }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      if( NIL != referenced_module2 )
      {
        ...
        if( NIL != incongruence2 )
        {
          feature_problems = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module, defined_features ), ConsesLow.list( $kw24$GLOBAL, referenced_global2, referenced_module2, referenced_features2 ),
              incongruence2 ), feature_problems );
        }
      }
      cdolist_list_var2 = cdolist_list_var2.rest();
  ...
  public static SubLObject potential_module_level_feature_incongruence(final SubLObject defined_features, final SubLObject referenced_features)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym26$POTENTIAL_MODULE_LEVEL_FEATURE_INCONGRUENCE, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym26$POTENTIAL_MODULE_LEVEL_FEATURE_INCONGRUENCE, TWO_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym26$POTENTIAL_MODULE_LEVEL_FEATURE_INCONGRUENCE, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( potential_module_level_feature_incongruence_internal( defined_features, referenced_features ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( defined_features, referenced_features ) );
  ...
  public static SubLObject show_one_feature_problem(final SubLObject feature_problem_spec, SubLObject stream)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( feature_problem_spec, feature_problem_spec, $list27 );
    final SubLObject temp = feature_problem_spec.rest();
    SubLObject current = feature_problem_spec.first();
    SubLObject def_type = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( current, feature_problem_spec, $list27 );
    ...
    if( NIL == current )
    {
      ...
      if( pcase_var.eql( $kw25$MODULE ) )
      {
        SubLObject current_$18;
        final SubLObject datum_$17 = current_$18 = def_args;
        SubLObject module = NIL;
        SubLObject def_features = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$18, datum_$17, $list28 );
        ...
        if( NIL == current_$18 )
        {
          PrintLow.format( stream, $str29$__Top_level_form_in__A__, module );
          show_one_feature_assumption( def_features, stream );
        }
        else
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$17, $list28 );
        }
      }
      else if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        cdestructuring_bind.destructuring_bind_must_consp( current_$19, datum_$18, $list30 );
        ...
        if( NIL == current_$19 )
        {
          PrintLow.format( stream, $str31$__Method__A_in__A__, def_object, module2 );
          show_one_feature_assumption( def_features2, stream );
        }
        else
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$18, $list30 );
        }
      }
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        ...
        SubLObject module2 = NIL;
        SubLObject def_features2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$20, datum_$19, $list30 );
        ...
        if( NIL == current_$20 )
        {
          PrintLow.format( stream, $str32$__Global__A_in__A__, def_object, module2 );
          show_one_feature_assumption( def_features2, stream );
        }
        else
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$19, $list30 );
        }
      }
      PrintLow.format( stream, $str33$_references );
      pcase_var = ref_type;
      if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        if( NIL == current_$21 )
        {
          PrintLow.format( stream, $str35$__method__A_from__A__, ref_object, module2 );
      ...
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        ...
        if( NIL == current_$22 )
        {
          PrintLow.format( stream, $str36$__global__A_from__A__, ref_object, module2 );
          show_one_feature_assumption( ref_features, stream );
        }
        else
        {
      ...
      PrintLow.format( stream, $str37$__which_will_have_problems_when__, feature_explanation( incongruence ) );
    }
    else
    {
      cdestructuring_bind.cdestructuring_bind_error( feature_problem_spec, $list27 );
    }
  ...
  {
    return PrintLow.format( stream, $str38$assuming__A, feature_explanation( feature_expression ) );
  }
  ...
  {
    return ( T == feature_expression ) ? $str39$no_features : feature_expression;
  }
  ...
  public static SubLObject all_privacy_violations(SubLObject v_modules)
  {
    ...
    try
    {
      utilities_macros.$progress_note$.bind( $str40$Identifying_privacy_violations, thread );
      utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
      utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
      utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
      utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
      ...
      try
      {
        ...
        while ( NIL != csome_list_var)
        {
          ...
          while ( NIL != cdolist_list_var)
          {
            all_privacy_violations = ConsesLow.cons( privacy_violation, all_privacy_violations );
            cdolist_list_var = cdolist_list_var.rest();
            privacy_violation = cdolist_list_var.first();
          }
          utilities_macros.$progress_sofar$.setDynamicValue( Numbers.add( utilities_macros.$progress_sofar$.getDynamicValue( thread ), ONE_INTEGER ), thread );
  ...
  public static SubLObject module_privacy_violations(final SubLObject defined_module)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$28)
      {
        ...
        {
          privacy_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module ) ), privacy_violations );
        }
      ...
      while ( NIL != cdolist_list_var_$29)
      {
        ...
        {
          privacy_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global, referenced_module ) ), privacy_violations );
        }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$30)
      {
        ...
        if( NIL != referenced_module && !defined_module.equal( referenced_module ) && NIL != form_translation.private_method_p( referenced_method ) )
        {
          privacy_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module ) ), privacy_violations );
        }
      ...
      while ( NIL != cdolist_list_var_$31)
      {
        ...
        if( NIL != referenced_module && !defined_module.equal( referenced_module ) && NIL != form_translation.private_global_p( referenced_global ) )
        {
          privacy_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global, referenced_module ) ), privacy_violations );
        }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      {
        privacy_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method2, referenced_module2 ) ), privacy_violations );
      }
      cdolist_list_var2 = cdolist_list_var2.rest();
      referenced_method2 = cdolist_list_var2.first();
    }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      {
        privacy_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global2, referenced_module2 ) ), privacy_violations );
      }
      cdolist_list_var2 = cdolist_list_var2.rest();
      referenced_global2 = cdolist_list_var2.first();
    }
  ...
  public static SubLObject show_one_privacy_violation(final SubLObject violation_spec, SubLObject stream)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( violation_spec, violation_spec, $list41 );
    final SubLObject temp = violation_spec.rest();
    SubLObject current = violation_spec.first();
    SubLObject def_type = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( current, violation_spec, $list41 );
    def_type = current.first();
    final SubLObject def_args;
    current = ( def_args = current.rest() );
    current = temp;
    ...
    if( NIL == current )
    {
      ...
      if( pcase_var.eql( $kw25$MODULE ) )
      {
        ...
        SubLObject module = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$34, datum_$33, $list42 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$33, $list42 );
        }
      }
      else if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        SubLObject def_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$35, datum_$34, $list43 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$34, $list43 );
        }
      }
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        ...
        SubLObject def_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$36, datum_$35, $list43 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$35, $list43 );
        }
      }
      PrintLow.format( stream, $str44$_references_private );
      pcase_var = ref_type;
      if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        SubLObject ref_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$37, datum_$36, $list45 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$36, $list45 );
        }
      }
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        ...
        SubLObject ref_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$38, datum_$37, $list45 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$37, $list45 );
        }
      }
    }
    else
    {
      cdestructuring_bind.cdestructuring_bind_error( violation_spec, $list41 );
    }
  ...
  public static SubLObject all_early_rebinding_violations(SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          utilities_macros.$progress_note$.bind( $str46$Identifying_early_rebinding_viola, thread );
          utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
          utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
          utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
          utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
          ...
          try
          {
            ...
            while ( NIL != csome_list_var)
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                all_early_rebinding_violations = ConsesLow.cons( early_rebinding_violation, all_early_rebinding_violations );
                cdolist_list_var = cdolist_list_var.rest();
                early_rebinding_violation = cdolist_list_var.first();
              }
              utilities_macros.$progress_sofar$.setDynamicValue( Numbers.add( utilities_macros.$progress_sofar$.getDynamicValue( thread ), ONE_INTEGER ), thread );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$46)
      {
        ...
        if( NIL != referenced_module )
        {
          ...
          if( NIL != td_early_evaluation_reference_p( defined_module, defined_position, referenced_module, referenced_position, $kw24$GLOBAL, rebound_global ) )
          {
            early_rebinding_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw24$GLOBAL, rebound_global, referenced_module ) ),
  ...
  public static SubLObject show_one_early_rebinding_violation(final SubLObject violation_spec, SubLObject stream)
  {
    ...
    if( NIL == current )
    {
      ...
      if( pcase_var.eql( $kw25$MODULE ) )
      {
        ...
        SubLObject module = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$49, datum_$48, $list42 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$48, $list42 );
        }
      }
      else if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        SubLObject def_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$50, datum_$49, $list43 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$49, $list43 );
        }
      }
      PrintLow.format( stream, $str47$_rebinds );
      pcase_var = ref_type;
      if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        SubLObject current_$51;
        final SubLObject datum_$50 = current_$51 = ref_args;
        SubLObject ref_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$51, datum_$50, $list45 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$50, $list45 );
        }
      }
      PrintLow.format( stream, $str48$_before_it_is_defined__ );
    }
    else
    {
      cdestructuring_bind.cdestructuring_bind_error( violation_spec, $list41 );
  ...
  public static SubLObject all_early_reference_violations(SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          utilities_macros.$progress_note$.bind( $str49$Identifying_early_reference_viola, thread );
          utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
          utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
          utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
          utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
          ...
          try
          {
            ...
            while ( NIL != csome_list_var)
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                all_early_reference_violations = ConsesLow.cons( early_reference_violation, all_early_reference_violations );
                cdolist_list_var = cdolist_list_var.rest();
                early_reference_violation = cdolist_list_var.first();
              }
              utilities_macros.$progress_sofar$.setDynamicValue( Numbers.add( utilities_macros.$progress_sofar$.getDynamicValue( thread ), ONE_INTEGER ), thread );
  ...
  public static SubLObject module_early_reference_violations(final SubLObject defined_module)
  {
      ...
      while ( NIL != cdolist_list_var_$57)
      {
        if( NIL != td_method_macro_p( referenced_method ) )
        {
          ...
          if( NIL != referenced_module )
          {
            ...
            {
              early_reference_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module ),
                  justifying_methods, justifying_globals ), early_reference_violations );
            }
          }
        }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$58)
      {
        ...
        if( NIL != referenced_module )
        {
          final SubLObject referenced_position = td_method_definition_position( referenced_method );
          thread.resetMultipleValues();
          final SubLObject justifying_methods = td_early_evaluation_reference_p( defined_module, defined_position, referenced_module, referenced_position, $kw23$METHOD, referenced_method );
          final SubLObject justifying_globals = thread.secondMultipleValue();
          thread.resetMultipleValues();
          if( NIL != justifying_methods || NIL != justifying_globals )
          {
            early_reference_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module ),
                justifying_methods, justifying_globals ), early_reference_violations );
          }
        }
      ...
      while ( NIL != cdolist_list_var_$59)
      {
        ...
        if( NIL != referenced_module )
        {
          ...
          thread.resetMultipleValues();
          final SubLObject justifying_methods = td_early_evaluation_reference_p( defined_module, defined_position, referenced_module, referenced_position, $kw24$GLOBAL, referenced_global );
          final SubLObject justifying_globals = thread.secondMultipleValue();
          thread.resetMultipleValues();
          if( NIL != justifying_methods || NIL != justifying_globals )
          {
            early_reference_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global, referenced_module ),
                justifying_methods, justifying_globals ), early_reference_violations );
          }
        }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      if( NIL != referenced_module2 )
      {
        ...
        {
          thread.resetMultipleValues();
          final SubLObject justifying_methods = td_early_evaluation_reference_p( defined_module, module_position, referenced_module2, referenced_position2, $kw23$METHOD, referenced_method2 );
          final SubLObject justifying_globals = thread.secondMultipleValue();
          thread.resetMultipleValues();
          if( NIL != justifying_methods || NIL != justifying_globals )
          {
            early_reference_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module, module_position ), ConsesLow.list( $kw23$METHOD, referenced_method2, referenced_module2 ),
                justifying_methods, justifying_globals ), early_reference_violations );
          }
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      if( NIL != referenced_module2 )
      {
        ...
        {
          thread.resetMultipleValues();
          final SubLObject justifying_methods = td_early_evaluation_reference_p( defined_module, module_position, referenced_module2, referenced_position2, $kw24$GLOBAL, referenced_global2 );
          final SubLObject justifying_globals = thread.secondMultipleValue();
          thread.resetMultipleValues();
          if( NIL != justifying_methods || NIL != justifying_globals )
          {
            early_reference_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module, module_position ), ConsesLow.list( $kw24$GLOBAL, referenced_global2, referenced_module2 ),
                justifying_methods, justifying_globals ), early_reference_violations );
          }
  ...
  public static SubLObject td_early_macro_use_p(final SubLObject referencing_module, final SubLObject referencing_position, final SubLObject macro_module, final SubLObject macro_position,
      final SubLObject referenced_macro)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject justifying_methods = NIL;
    SubLObject justifying_globals = NIL;
    ...
    if( ( NIL != td_module_later( macro_module, referencing_module ) || ( referencing_module.equal( macro_module ) && macro_position.numG( referencing_position ) ) ) && NIL == conses_high.member( referenced_macro,
        justifying_methods, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
    {
      justifying_methods = ConsesLow.cons( referenced_macro, justifying_methods );
    }
  ...
  public static SubLObject td_early_evaluation_reference_p(final SubLObject referencing_module, final SubLObject referencing_position, final SubLObject referenced_module, final SubLObject referenced_position,
      final SubLObject reference_type, final SubLObject referenced_object)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject justifying_methods = NIL;
    SubLObject justifying_globals = NIL;
    final SubLObject latest_module_referenced_by_evaluation = ( $kw23$METHOD == reference_type ) ? td_method_latest_transitively_referenced_module_memoized( referenced_object )
        : td_global_latest_transitively_referenced_module_memoized( referenced_object );
    ...
    {
      thread.resetMultipleValues();
      final SubLObject candidate_methods = ( $kw23$METHOD == reference_type ) ? td_justify_method_latest_transitively_referenced_module( referenced_object, latest_module_referenced_by_evaluation )
          : td_justify_global_latest_transitively_referenced_module( referenced_object, latest_module_referenced_by_evaluation );
      ...
      else
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          if( definition_position.numG( referencing_position ) )
          {
            ...
            if( NIL == conses_high.member( item_var, justifying_methods, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
            {
              justifying_methods = ConsesLow.cons( item_var, justifying_methods );
            }
          }
          cdolist_list_var = cdolist_list_var.rest();
          candidate_method = cdolist_list_var.first();
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          if( definition_position.numG( referencing_position ) )
          {
            ...
            if( NIL == conses_high.member( item_var, justifying_globals, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
            {
              justifying_globals = ConsesLow.cons( item_var, justifying_globals );
            }
          }
          cdolist_list_var = cdolist_list_var.rest();
          candidate_global = cdolist_list_var.first();
    ...
    if( ( NIL != td_module_later( referenced_module, referencing_module ) || ( referencing_module.equal( referenced_module ) && referenced_position.numG( referencing_position ) ) ) && NIL == conses_high.member(
        referenced_object, justifying_methods, Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
    {
      justifying_methods = ConsesLow.cons( referenced_object, justifying_methods );
    }
    return Values.values( justifying_methods, justifying_globals );
  }
  ...
  public static SubLObject show_one_early_reference_violation(final SubLObject violation_spec, SubLObject stream)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( violation_spec, violation_spec, $list50 );
    final SubLObject temp = violation_spec.rest();
    SubLObject current = violation_spec.first();
    SubLObject def_type = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( current, violation_spec, $list50 );
    def_type = current.first();
    final SubLObject def_args;
    current = ( def_args = current.rest() );
    current = temp;
    ...
    if( NIL == current )
    {
      ...
      if( NIL == current )
      {
        ...
        if( pcase_var.eql( $kw25$MODULE ) )
        {
          ...
          cdestructuring_bind.destructuring_bind_must_consp( current_$66, datum_$65, $list51 );
          ...
          if( NIL == current_$66 )
          {
            PrintLow.format( stream, $str52$__Top_level_form_in__A_at__S__, module_$67, position );
          }
          else
          {
            cdestructuring_bind.cdestructuring_bind_error( datum_$65, $list51 );
          }
        }
        else if( pcase_var.eql( $kw23$METHOD ) )
        {
          ...
          cdestructuring_bind.destructuring_bind_must_consp( current_$67, datum_$66, $list43 );
          ...
          if( NIL == current_$67 )
          {
            PrintLow.format( stream, $str31$__Method__A_in__A__, def_object, module_$68 );
          }
          else
          {
            cdestructuring_bind.cdestructuring_bind_error( datum_$66, $list43 );
          }
        }
        else if( pcase_var.eql( $kw24$GLOBAL ) )
        {
          ...
          cdestructuring_bind.destructuring_bind_must_consp( current_$68, datum_$67, $list43 );
          ...
          if( NIL == current_$68 )
          {
            PrintLow.format( stream, $str32$__Global__A_in__A__, def_object, module_$69 );
          }
          else
          {
            cdestructuring_bind.cdestructuring_bind_error( datum_$67, $list43 );
          }
        }
        PrintLow.format( stream, $str33$_references );
        pcase_var = ref_type;
        if( pcase_var.eql( $kw23$METHOD ) )
        {
          PrintLow.format( stream, $str35$__method__A_from__A__, ref_object, module );
        }
        else if( pcase_var.eql( $kw24$GLOBAL ) )
        {
          PrintLow.format( stream, $str36$__global__A_from__A__, ref_object, module );
        }
        PrintLow.format( stream, $str53$_before_it_is );
        if( NIL == justifying_globals && NIL != list_utilities.singletonP( justifying_methods ) && ref_object.eql( justifying_methods.first() ) )
        {
          PrintLow.format( stream, $str54$_defined__ );
        }
        else
        {
          PrintLow.format( stream, $str55$_completely_defined_due_to__ );
          SubLObject cdolist_list_var = justifying_methods;
          SubLObject justifying_method = NIL;
          justifying_method = cdolist_list_var.first();
          while ( NIL != cdolist_list_var)
          {
            PrintLow.format( stream, $str56$method__A__, justifying_method );
            cdolist_list_var = cdolist_list_var.rest();
            justifying_method = cdolist_list_var.first();
          }
          ...
          while ( NIL != cdolist_list_var)
          {
            PrintLow.format( stream, $str57$global__A__, justifying_global );
            cdolist_list_var = cdolist_list_var.rest();
            justifying_global = cdolist_list_var.first();
          }
        }
      ...
      {
        cdestructuring_bind.cdestructuring_bind_error( violation_spec, $list50 );
      }
    }
    else
    {
  ...
  public static SubLObject all_early_macro_use_violations(SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          utilities_macros.$progress_note$.bind( $str58$Identifying_early_macro_use_viola, thread );
          utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
          utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
          utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
          utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
          ...
          try
          {
            ...
            while ( NIL != csome_list_var)
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                all_early_macro_use_violations = ConsesLow.cons( early_macro_use_violation, all_early_macro_use_violations );
                cdolist_list_var = cdolist_list_var.rest();
                early_macro_use_violation = cdolist_list_var.first();
              }
              utilities_macros.$progress_sofar$.setDynamicValue( Numbers.add( utilities_macros.$progress_sofar$.getDynamicValue( thread ), ONE_INTEGER ), thread );
      ...
      while ( NIL != cdolist_list_var_$77)
      {
        if( NIL != td_method_macro_p( referenced_method ) )
        {
          ...
          if( NIL != referenced_module )
          {
            ...
            if( NIL != justifying_methods || NIL != justifying_globals )
            {
              early_macro_use_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module ),
                  justifying_methods, justifying_globals ), early_macro_use_violations );
            }
          }
        }
      ...
      while ( NIL != cdolist_list_var_$78)
      {
        if( NIL != td_method_macro_p( referenced_method ) )
        {
          ...
          if( NIL != referenced_module )
          {
            ...
            if( NIL != justifying_methods || NIL != justifying_globals )
            {
              early_macro_use_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method, referenced_module ),
                  justifying_methods, justifying_globals ), early_macro_use_violations );
            }
          }
        }
      ...
      if( NIL != td_method_macro_p( referenced_method2 ) )
      {
        ...
        if( NIL != referenced_module2 )
        {
          ...
          while ( NIL != cdolist_list_var_$79)
          {
            ...
            if( NIL != justifying_methods || NIL != justifying_globals )
            {
              early_macro_use_violations = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module, module_position ), ConsesLow.list( $kw23$METHOD, referenced_method2, referenced_module2 ),
                  justifying_methods, justifying_globals ), early_macro_use_violations );
            }
  ...
  public static SubLObject all_undefined_references(SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          utilities_macros.$progress_note$.bind( $str59$Identifying_undefined_references, thread );
          utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
          utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
          utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
          utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
          ...
          try
          {
            ...
            while ( NIL != csome_list_var)
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                all_undefined_references = ConsesLow.cons( undefined_reference, all_undefined_references );
                cdolist_list_var = cdolist_list_var.rest();
                undefined_reference = cdolist_list_var.first();
              }
              utilities_macros.$progress_sofar$.setDynamicValue( Numbers.add( utilities_macros.$progress_sofar$.getDynamicValue( thread ), ONE_INTEGER ), thread );
  ...
  public static SubLObject module_undefined_references(final SubLObject defined_module)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$83)
      {
        if( NIL == td_predefined_method_p( referenced_method ) )
        {
          ...
          if( NIL == referenced_module )
          {
            undefined_references = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method ) ), undefined_references );
          }
        }
      ...
      while ( NIL != cdolist_list_var_$84)
      {
        if( NIL == td_predefined_global_p( referenced_global ) )
        {
          ...
          if( NIL == referenced_module )
          {
            undefined_references = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global ) ), undefined_references );
          }
        }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$85)
      {
        if( NIL == td_predefined_method_p( referenced_method ) )
        {
          ...
          if( NIL == referenced_module )
          {
            undefined_references = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method ) ), undefined_references );
          }
        }
      ...
      while ( NIL != cdolist_list_var_$86)
      {
        if( NIL == td_predefined_global_p( referenced_global ) )
        {
          ...
          if( NIL == referenced_module )
          {
            undefined_references = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global ) ), undefined_references );
          }
        }
    ...
    while ( NIL != cdolist_list_var2)
    {
      if( NIL == td_predefined_method_p( referenced_method2 ) )
      {
        ...
        if( NIL == referenced_module2 )
        {
          undefined_references = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module ), ConsesLow.list( $kw23$METHOD, referenced_method2 ) ), undefined_references );
        }
      }
      cdolist_list_var2 = cdolist_list_var2.rest();
      referenced_method2 = cdolist_list_var2.first();
    ...
    while ( NIL != cdolist_list_var2)
    {
      if( NIL == td_predefined_global_p( referenced_global2 ) )
      {
        ...
        if( NIL == referenced_module2 )
        {
          undefined_references = ConsesLow.cons( ConsesLow.list( ConsesLow.list( $kw25$MODULE, defined_module ), ConsesLow.list( $kw24$GLOBAL, referenced_global2 ) ), undefined_references );
        }
      }
      cdolist_list_var2 = cdolist_list_var2.rest();
      referenced_global2 = cdolist_list_var2.first();
  ...
  public static SubLObject show_one_undefined_reference(final SubLObject reference_spec, SubLObject stream)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( reference_spec, reference_spec, $list41 );
    final SubLObject temp = reference_spec.rest();
    SubLObject current = reference_spec.first();
    SubLObject def_type = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( current, reference_spec, $list41 );
    def_type = current.first();
    final SubLObject def_args;
    current = ( def_args = current.rest() );
    current = temp;
    ...
    if( NIL == current )
    {
      ...
      if( pcase_var.eql( $kw25$MODULE ) )
      {
        ...
        SubLObject module = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$89, datum_$88, $list42 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$88, $list42 );
        }
      }
      else if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        SubLObject def_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$90, datum_$89, $list43 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$89, $list43 );
        }
      }
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        ...
        SubLObject def_object = NIL;
        SubLObject module2 = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$91, datum_$90, $list43 );
        ...
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$90, $list43 );
        }
      }
      PrintLow.format( stream, $str60$_references_undefined );
      pcase_var = ref_type;
      if( pcase_var.eql( $kw23$METHOD ) )
      {
        ...
        SubLObject ref_object = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$92, datum_$91, $list61 );
        ref_object = current_$92.first();
        current_$92 = current_$92.rest();
        if( NIL == current_$92 )
        {
          PrintLow.format( stream, $str62$__method__A__, ref_object );
        }
        else
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$91, $list61 );
        }
      }
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        ...
        SubLObject ref_object = NIL;
        cdestructuring_bind.destructuring_bind_must_consp( current_$93, datum_$92, $list61 );
        ref_object = current_$93.first();
        current_$93 = current_$93.rest();
        if( NIL == current_$93 )
        {
          PrintLow.format( stream, $str63$__global__A__, ref_object );
        }
        else
        {
          cdestructuring_bind.cdestructuring_bind_error( datum_$92, $list61 );
        }
      }
    }
    else
    {
      cdestructuring_bind.cdestructuring_bind_error( reference_spec, $list41 );
    }
      ...
      try
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          while ( NIL != cdolist_list_var_$98)
          {
            ...
            if( NIL == conses_high.member( item_var, all_multiple_definitions, Symbols.symbol_function( EQUAL ), Symbols.symbol_function( IDENTITY ) ) )
            {
              all_multiple_definitions = ConsesLow.cons( item_var, all_multiple_definitions );
            }
  ...
  public static SubLObject module_multiple_definitions(final SubLObject defined_module)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL != td_method_has_multiple_definitionsP( defined_method ) )
      {
        ...
        multiple_definitions = ConsesLow.cons( ConsesLow.list( $kw23$METHOD, defined_method, position_specs ), multiple_definitions );
      }
      cdolist_list_var = cdolist_list_var.rest();
      defined_method = cdolist_list_var.first();
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL != td_global_has_multiple_definitionsP( defined_global ) )
      {
        ...
        multiple_definitions = ConsesLow.cons( ConsesLow.list( $kw24$GLOBAL, defined_global, position_specs ), multiple_definitions );
      }
      cdolist_list_var = cdolist_list_var.rest();
      defined_global = cdolist_list_var.first();
    }
  ...
  public static SubLObject show_one_multiple_definition(final SubLObject definition_spec, SubLObject stream)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( definition_spec, definition_spec, $list64 );
    def_type = definition_spec.first();
    SubLObject current = definition_spec.rest();
    cdestructuring_bind.destructuring_bind_must_consp( current, definition_spec, $list64 );
    ...
    if( NIL == current )
    {
      ...
      if( pcase_var.eql( $kw23$METHOD ) )
      {
        PrintLow.format( stream, $str65$__Method__A_has_multiple_definiti, def_object );
      }
      else if( pcase_var.eql( $kw24$GLOBAL ) )
      {
        PrintLow.format( stream, $str66$__Global__A_has_multiple_definiti, def_object );
      }
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        cdestructuring_bind.destructuring_bind_must_consp( current_$101, datum_$100, $list67 );
        module = current_$101.first();
        current_$101 = ( file_position = current_$101.rest() );
        PrintLow.format( stream, $str68$_____A_at_position__S, module, file_position );
        cdolist_list_var = cdolist_list_var.rest();
        definition_position = cdolist_list_var.first();
      }
      streams_high.terpri( stream );
    ...
    {
      cdestructuring_bind.cdestructuring_bind_error( definition_spec, $list64 );
    }
      ...
      try
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          while ( NIL != cdolist_list_var_$102)
          {
            ...
            if( NIL == conses_high.member( item_var, all_early_constant_references, Symbols.symbol_function( EQUAL ), Symbols.symbol_function( IDENTITY ) ) )
            {
              all_early_constant_references = ConsesLow.cons( item_var, all_early_constant_references );
            }
  ...
  {
    final SubLObject referenced_method = $sym69$READER_MAKE_CONSTANT_SHELL;
    final SubLObject referenced_module = td_method_defining_module( referenced_method );
    if( NIL != td_module_earlier( defined_module, referenced_module ) )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          early_constant_references = ConsesLow.cons( ConsesLow.list( $kw23$METHOD, defined_method, defined_module ), early_constant_references );
        }
        cdolist_list_var = cdolist_list_var.rest();
        defined_method = cdolist_list_var.first();
      }
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          early_constant_references = ConsesLow.cons( ConsesLow.list( $kw24$GLOBAL, defined_global, defined_module ), early_constant_references );
        }
        cdolist_list_var = cdolist_list_var.rest();
        defined_global = cdolist_list_var.first();
      }
      ...
      {
        early_constant_references = ConsesLow.cons( ConsesLow.list( $kw25$MODULE, defined_module ), early_constant_references );
      }
  ...
  public static SubLObject show_one_early_constant_reference(final SubLObject reference_spec, SubLObject stream)
  {
    ...
    SubLObject ref_type = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( reference_spec, reference_spec, $list70 );
    ...
    if( pcase_var.eql( $kw25$MODULE ) )
    {
      ...
      SubLObject module = NIL;
      cdestructuring_bind.destructuring_bind_must_consp( current_$105, datum_$104, $list42 );
      ...
      {
        cdestructuring_bind.cdestructuring_bind_error( datum_$104, $list42 );
      }
    }
    else if( pcase_var.eql( $kw23$METHOD ) )
    {
      ...
      SubLObject ref_object = NIL;
      SubLObject module2 = NIL;
      cdestructuring_bind.destructuring_bind_must_consp( current_$106, datum_$105, $list45 );
      ...
      if( NIL == current_$106 )
      {
        PrintLow.format( stream, $str31$__Method__A_in__A__, ref_object, module2 );
      }
      else
      {
        cdestructuring_bind.cdestructuring_bind_error( datum_$105, $list45 );
      }
    }
    else if( pcase_var.eql( $kw24$GLOBAL ) )
    {
      ...
      SubLObject ref_object = NIL;
      SubLObject module2 = NIL;
      cdestructuring_bind.destructuring_bind_must_consp( current_$107, datum_$106, $list45 );
      ...
      if( NIL == current_$107 )
      {
        PrintLow.format( stream, $str32$__Global__A_in__A__, ref_object, module2 );
      }
      else
      {
        cdestructuring_bind.cdestructuring_bind_error( datum_$106, $list45 );
      }
    }
    PrintLow.format( stream, $str71$references____before_it_is_comple );
    return reference_spec;
  }
  ...
  public static SubLObject show_api_macros_with_non_api_expansions(SubLObject stream)
  {
    ...
    try
    {
      while ( Hashtables.iteratorHasNext( cdohash_iterator ))
      {
        ...
        {
          problems = ConsesLow.cons( ConsesLow.cons( macro, non_api_expansions ), problems );
        }
      }
    }
    finally
    ...
    if( NIL != problems )
    {
      final SubLObject build_problem_spec = ConsesLow.list( $kw72$API_MACRO_VISIBILITY, problems );
  ...
  public static SubLObject show_one_api_macro_visibility_problem(final SubLObject violation_spec, SubLObject stream)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( violation_spec, violation_spec, $list73 );
    macro = violation_spec.first();
    final SubLObject current = non_api_expansions = violation_spec.rest();
    PrintLow.format( stream, $str74$__API_macro__A_expands_to_non_API, macro, non_api_expansions );
    return NIL;
  }
  ...
  {
    assert NIL != utilities_macros.api_predefined_macro_p( macro ) : macro;
    SubLObject non_api_expansions = NIL;
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL == utilities_macros.api_predefined_macro_p( method ) && NIL == utilities_macros.api_predefined_function_p( method ) )
      {
        non_api_expansions = ConsesLow.cons( method, non_api_expansions );
      }
      cdolist_list_var = cdolist_list_var.rest();
      method = cdolist_list_var.first();
    }
  ...
  public static SubLObject all_build_problems(SubLObject v_modules)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          utilities_macros.$progress_note$.bind( $str76$Determining_build_problems, thread );
          utilities_macros.$progress_start_time$.bind( Time.get_universal_time(), thread );
          utilities_macros.$progress_total$.bind( Sequences.length( list_var ), thread );
          utilities_macros.$progress_sofar$.bind( ZERO_INTEGER, thread );
          utilities_macros.$last_percent_progress_index$.bind( ZERO_INTEGER, thread );
          ...
          try
          {
            ...
            while ( NIL != csome_list_var)
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                all_build_problems = ConsesLow.cons( build_problem, all_build_problems );
                cdolist_list_var = cdolist_list_var.rest();
                build_problem = cdolist_list_var.first();
              }
              utilities_macros.$progress_sofar$.setDynamicValue( Numbers.add( utilities_macros.$progress_sofar$.getDynamicValue( thread ), ONE_INTEGER ), thread );
  ...
  public static SubLObject module_build_problems(final SubLObject defined_module)
  {
    ...
    {
      build_problems = ConsesLow.cons( ConsesLow.list( $kw77$MULTIPLE_DEFINITIONS, multiple_definitions ), build_problems );
    }
    final SubLObject undefined_references = module_undefined_references( defined_module );
    if( NIL != undefined_references )
    {
      build_problems = ConsesLow.cons( ConsesLow.list( $kw78$UNDEFINED_REFERENCES, undefined_references ), build_problems );
    }
    final SubLObject privacy_violations = module_privacy_violations( defined_module );
    if( NIL != privacy_violations )
    {
      build_problems = ConsesLow.cons( ConsesLow.list( $kw79$PRIVACY_VIOLATIONS, privacy_violations ), build_problems );
    }
    ...
    {
      build_problems = ConsesLow.cons( ConsesLow.list( $kw80$EARLY_CONSTANT_REFERENCES, early_constant_references ), build_problems );
    }
    ...
    {
      build_problems = ConsesLow.cons( ConsesLow.list( $kw81$EARLY_REFERENCE_VIOLATIONS, early_reference_violations ), build_problems );
    }
    ...
    {
      build_problems = ConsesLow.cons( ConsesLow.list( $kw82$EARLY_REBINDING_VIOLATIONS, early_rebinding_violations ), build_problems );
    }
  ...
  public static SubLObject show_build_problems(SubLObject build_problem_specs, SubLObject stream)
  {
    ...
    {
      PrintLow.format( stream, $str83$__No_build_problems_ );
    }
    else
    {
  ...
  public static SubLObject show_one_build_problem(final SubLObject build_problem_spec, SubLObject stream)
  {
    ...
    SubLObject problems = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( build_problem_spec, build_problem_spec, $list84 );
    problem_class = build_problem_spec.first();
    SubLObject current = build_problem_spec.rest();
    cdestructuring_bind.destructuring_bind_must_consp( current, build_problem_spec, $list84 );
    problems = current.first();
    current = current.rest();
    if( NIL == current )
    {
      final SubLObject pcase_var = problem_class;
      if( pcase_var.eql( $kw77$MULTIPLE_DEFINITIONS ) )
      {
      ...
      else if( pcase_var.eql( $kw78$UNDEFINED_REFERENCES ) )
      {
        SubLObject cdolist_list_var = problems;
        SubLObject problem = NIL;
        problem = cdolist_list_var.first();
      ...
      else if( pcase_var.eql( $kw79$PRIVACY_VIOLATIONS ) )
      {
        SubLObject cdolist_list_var = problems;
        SubLObject problem = NIL;
        problem = cdolist_list_var.first();
      ...
      else if( pcase_var.eql( $kw80$EARLY_CONSTANT_REFERENCES ) )
      {
        SubLObject cdolist_list_var = problems;
        SubLObject problem = NIL;
        problem = cdolist_list_var.first();
      ...
      else if( pcase_var.eql( $kw81$EARLY_REFERENCE_VIOLATIONS ) )
      {
        SubLObject cdolist_list_var = problems;
        SubLObject problem = NIL;
        problem = cdolist_list_var.first();
      ...
      else if( pcase_var.eql( $kw82$EARLY_REBINDING_VIOLATIONS ) )
      {
        SubLObject cdolist_list_var = problems;
        SubLObject problem = NIL;
        problem = cdolist_list_var.first();
      ...
      else if( pcase_var.eql( $kw72$API_MACRO_VISIBILITY ) )
      {
        SubLObject cdolist_list_var = problems;
        SubLObject problem = NIL;
        problem = cdolist_list_var.first();
    ...
    {
      cdestructuring_bind.cdestructuring_bind_error( build_problem_spec, $list84 );
    }
    return NIL;
  }
  ...
  public static SubLObject build_problem_counts_by_module(SubLObject build_problem_specs)
  {
    ...
    final SubLObject table = dictionary.new_dictionary( Symbols.symbol_function( EQUAL ), ZERO_INTEGER );
    SubLObject cdolist_list_var = build_problem_specs;
    SubLObject build_problem_spec = NIL;
    build_problem_spec = cdolist_list_var.first();
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == current )
      {
        ...
        final SubLObject module = list_utilities.tree_find_if( Symbols.symbol_function( $sym86$STRINGP ), problem, UNPROVIDED );
        if( module.isString() )
        {
    ...
    problem_counts = Sort.sort( problem_counts, Symbols.symbol_function( $sym87$_ ), Symbols.symbol_function( $sym88$CDR ) );
  ...
  {
    SubLObject all_co_called_methods = $kw89$UNINITIALIZED;
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( $kw89$UNINITIALIZED == all_co_called_methods )
      {
        all_co_called_methods = conses_high.copy_list( co_called_methods );
      }
      else
      {
        all_co_called_methods = conses_high.nintersection( all_co_called_methods, conses_high.copy_list( co_called_methods ), Symbols.symbol_function( EQ ), UNPROVIDED );
      }
      cdolist_list_var = cdolist_list_var.rest();
      calling_method = cdolist_list_var.first();
    }
    if( $kw89$UNINITIALIZED != all_co_called_methods )
    {
      return Sequences.delete( called_method, all_co_called_methods, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    return NIL;
  }
  ...
  {
    SubLObject all_co_called_globals = $kw89$UNINITIALIZED;
    SubLObject cdolist_list_var = td_methods_calling_method( called_method );
    SubLObject calling_method = NIL;
    calling_method = cdolist_list_var.first();
    while ( NIL != cdolist_list_var)
    {
      ...
      if( $kw89$UNINITIALIZED == all_co_called_globals )
      {
        all_co_called_globals = conses_high.copy_list( co_called_globals );
      }
      else
      {
        all_co_called_globals = conses_high.nintersection( all_co_called_globals, conses_high.copy_list( co_called_globals ), Symbols.symbol_function( EQ ), UNPROVIDED );
      }
      cdolist_list_var = cdolist_list_var.rest();
      calling_method = cdolist_list_var.first();
    }
  ...
  public static SubLObject td_possible_orphan_methods_from_module(final SubLObject defined_module)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL == html_macros.html_handler_functionP( defined_method ) && NIL == utilities_macros.funcall_helper_functionP( defined_method ) && NIL == access_macros.external_function_p( defined_method )
          && NIL == td_methods_calling_method( defined_method ) && NIL == string_utilities.starts_with( Symbols.symbol_name( defined_method ), $str90$CLEAR ) && NIL == string_utilities.starts_with( Symbols.symbol_name(
              defined_method ), $str91$REMOVE ) )
      {
        unused_methods = ConsesLow.cons( defined_method, unused_methods );
      }
      cdolist_list_var = cdolist_list_var.rest();
      defined_method = cdolist_list_var.first();
    }
  ...
  {
    SubLFiles.declareFunction( me, "td_module_p", "TD-MODULE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_current_translation", "TD-CURRENT-TRANSLATION", 0, 0, false );
    SubLFiles.declareFunction( me, "td_translation_modules", "TD-TRANSLATION-MODULES", 0, 0, false );
    SubLFiles.declareFunction( me, "td_translation_modules_memoized_internal", "TD-TRANSLATION-MODULES-MEMOIZED-INTERNAL", 0, 0, false );
    SubLFiles.declareFunction( me, "td_translation_modules_memoized", "TD-TRANSLATION-MODULES-MEMOIZED", 0, 0, false );
    SubLFiles.declareFunction( me, "td_module_earlier", "TD-MODULE-EARLIER", 2, 0, false );
    SubLFiles.declareFunction( me, "td_module_translation_position_internal", "TD-MODULE-TRANSLATION-POSITION-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_module_translation_position", "TD-MODULE-TRANSLATION-POSITION", 1, 0, false );
    SubLFiles.declareFunction( me, "td_module_later", "TD-MODULE-LATER", 2, 0, false );
    SubLFiles.declareFunction( me, "td_sort_modules_by_load_order", "TD-SORT-MODULES-BY-LOAD-ORDER", 1, 0, false );
    SubLFiles.declareFunction( me, "td_module_earlier_memoized", "TD-MODULE-EARLIER-MEMOIZED", 2, 0, false );
    SubLFiles.declareFunction( me, "td_module_later_memoized", "TD-MODULE-LATER-MEMOIZED", 2, 0, false );
    SubLFiles.declareFunction( me, "td_module_features", "TD-MODULE-FEATURES", 1, 0, false );
    SubLFiles.declareFunction( me, "td_translation_features", "TD-TRANSLATION-FEATURES", 0, 0, false );
    SubLFiles.declareFunction( me, "td_module_pathname", "TD-MODULE-PATHNAME", 1, 0, false );
    SubLFiles.declareFunction( me, "td_predefined_method_p", "TD-PREDEFINED-METHOD-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_predefined_global_p", "TD-PREDEFINED-GLOBAL-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_macro_p", "TD-METHOD-MACRO-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_defining_module", "TD-METHOD-DEFINING-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_definition_position", "TD-METHOD-DEFINITION-POSITION", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_has_multiple_definitionsP", "TD-METHOD-HAS-MULTIPLE-DEFINITIONS?", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_definition_positions", "TD-METHOD-DEFINITION-POSITIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_formal_arglist", "TD-METHOD-FORMAL-ARGLIST", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_binding_type", "TD-GLOBAL-BINDING-TYPE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_defining_module", "TD-GLOBAL-DEFINING-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_definition_position", "TD-GLOBAL-DEFINITION-POSITION", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_has_multiple_definitionsP", "TD-GLOBAL-HAS-MULTIPLE-DEFINITIONS?", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_definition_positions", "TD-GLOBAL-DEFINITION-POSITIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "td_methods_defined_by_module", "TD-METHODS-DEFINED-BY-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_defined_by_module", "TD-GLOBALS-DEFINED-BY-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_called_by_method", "TD-GLOBALS-CALLED-BY-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_rebound_by_method", "TD-GLOBALS-REBOUND-BY-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_methods_called_by_method", "TD-METHODS-CALLED-BY-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_called_by_methodP", "TD-METHOD-CALLED-BY-METHOD?", 2, 0, false );
    SubLFiles.declareFunction( me, "td_modules_referenced_by_method", "TD-MODULES-REFERENCED-BY-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_called_by_global", "TD-GLOBALS-CALLED-BY-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_methods_called_by_global", "TD-METHODS-CALLED-BY-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_called_by_globalP", "TD-METHOD-CALLED-BY-GLOBAL?", 2, 0, false );
    SubLFiles.declareFunction( me, "td_modules_referenced_by_global", "TD-MODULES-REFERENCED-BY-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_called_by_module", "TD-GLOBALS-CALLED-BY-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_methods_called_by_module", "TD-METHODS-CALLED-BY-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_called_by_moduleP", "TD-METHOD-CALLED-BY-MODULE?", 2, 0, false );
    SubLFiles.declareFunction( me, "td_module_positions_calling_global", "TD-MODULE-POSITIONS-CALLING-GLOBAL", 2, 0, false );
    SubLFiles.declareFunction( me, "td_module_positions_calling_method", "TD-MODULE-POSITIONS-CALLING-METHOD", 2, 0, false );
    SubLFiles.declareFunction( me, "td_method_unused_p", "TD-METHOD-UNUSED-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_calling_method", "TD-GLOBALS-CALLING-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_methods_calling_method", "TD-METHODS-CALLING-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_calling_method", "TD-MODULES-CALLING-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_referencing_method", "TD-MODULES-REFERENCING-METHOD", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_unused_p", "TD-GLOBAL-UNUSED-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_globals_calling_global", "TD-GLOBALS-CALLING-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_methods_calling_global", "TD-METHODS-CALLING-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_calling_global", "TD-MODULES-CALLING-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_referencing_global", "TD-MODULES-REFERENCING-GLOBAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_potentially_private_p", "TD-METHOD-POTENTIALLY-PRIVATE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_potentially_private_p", "TD-GLOBAL-POTENTIALLY-PRIVATE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_referencing_module", "TD-MODULES-REFERENCING-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_referenced_by_module", "TD-MODULES-REFERENCED-BY-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_justify_module_referencing_module", "TD-JUSTIFY-MODULE-REFERENCING-MODULE", 2, 0, false );
    SubLFiles.declareFunction( me, "td_modules_implied_by_methods", "TD-MODULES-IMPLIED-BY-METHODS", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_implied_by_globals", "TD-MODULES-IMPLIED-BY-GLOBALS", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_affected_by_arglist_change", "TD-MODULES-AFFECTED-BY-ARGLIST-CHANGE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_modules_cluster_containing_module", "TD-MODULES-CLUSTER-CONTAINING-MODULE", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_properly_dependent_on_modules", "TD-MODULES-PROPERLY-DEPENDENT-ON-MODULES", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_dependent_on_modules", "TD-MODULES-DEPENDENT-ON-MODULES", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_dependent_on_module", "TD-MODULES-DEPENDENT-ON-MODULE", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_dependent_on_module_methods", "TD-MODULES-DEPENDENT-ON-MODULE-METHODS", 1, 1, false );
    SubLFiles.declareFunction( me, "td_modules_dependent_on_module_globals", "TD-MODULES-DEPENDENT-ON-MODULE-GLOBALS", 1, 1, false );
    SubLFiles.declareFunction( me, "td_explain_how_module_depends_on_module", "TD-EXPLAIN-HOW-MODULE-DEPENDS-ON-MODULE", 2, 0, false );
    SubLFiles.declareFunction( me, "td_explain_how_module_depends_on_modules", "TD-EXPLAIN-HOW-MODULE-DEPENDS-ON-MODULES", 2, 0, false );
    SubLFiles.declareFunction( me, "td_method_calls_transitively", "TD-METHOD-CALLS-TRANSITIVELY", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_calls_transitively", "TD-GLOBAL-CALLS-TRANSITIVELY", 1, 0, false );
    SubLFiles.declareFunction( me, "td_note_method_called_transitively", "TD-NOTE-METHOD-CALLED-TRANSITIVELY", 1, 0, false );
    SubLFiles.declareFunction( me, "td_note_global_called_transitively", "TD-NOTE-GLOBAL-CALLED-TRANSITIVELY", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_references_modules_transitively", "TD-METHOD-REFERENCES-MODULES-TRANSITIVELY", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_latest_transitively_referenced_module", "TD-METHOD-LATEST-TRANSITIVELY-REFERENCED-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_latest_transitively_referenced_module_memoized_internal", "TD-METHOD-LATEST-TRANSITIVELY-REFERENCED-MODULE-MEMOIZED-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_latest_transitively_referenced_module_memoized", "TD-METHOD-LATEST-TRANSITIVELY-REFERENCED-MODULE-MEMOIZED", 1, 0, false );
    SubLFiles.declareFunction( me, "td_justify_method_latest_transitively_referenced_module", "TD-JUSTIFY-METHOD-LATEST-TRANSITIVELY-REFERENCED-MODULE", 2, 0, false );
    SubLFiles.declareFunction( me, "td_justify_method_calls_transitively", "TD-JUSTIFY-METHOD-CALLS-TRANSITIVELY", 2, 0, false );
    SubLFiles.declareFunction( me, "td_global_references_modules_transitively", "TD-GLOBAL-REFERENCES-MODULES-TRANSITIVELY", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_latest_transitively_referenced_module", "TD-GLOBAL-LATEST-TRANSITIVELY-REFERENCED-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_latest_transitively_referenced_module_memoized_internal", "TD-GLOBAL-LATEST-TRANSITIVELY-REFERENCED-MODULE-MEMOIZED-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "td_global_latest_transitively_referenced_module_memoized", "TD-GLOBAL-LATEST-TRANSITIVELY-REFERENCED-MODULE-MEMOIZED", 1, 0, false );
    SubLFiles.declareFunction( me, "td_justify_global_latest_transitively_referenced_module", "TD-JUSTIFY-GLOBAL-LATEST-TRANSITIVELY-REFERENCED-MODULE", 2, 0, false );
    SubLFiles.declareFunction( me, "incongruent_features", "INCONGRUENT-FEATURES", 2, 0, false );
    SubLFiles.declareFunction( me, "feature_expression_transform", "FEATURE-EXPRESSION-TRANSFORM", 1, 0, false );
    SubLFiles.declareFunction( me, "non_feature_symbol_p", "NON-FEATURE-SYMBOL-P", 1, 0, false );
    SubLFiles.declareFunction( me, "transform_non_feature_symbol", "TRANSFORM-NON-FEATURE-SYMBOL", 1, 0, false );
    SubLFiles.declareFunction( me, "all_module_level_feature_problems", "ALL-MODULE-LEVEL-FEATURE-PROBLEMS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_level_feature_problems", "MODULE-LEVEL-FEATURE-PROBLEMS", 1, 0, false );
    SubLFiles.declareFunction( me, "potential_module_level_feature_incongruence_internal", "POTENTIAL-MODULE-LEVEL-FEATURE-INCONGRUENCE-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "potential_module_level_feature_incongruence", "POTENTIAL-MODULE-LEVEL-FEATURE-INCONGRUENCE", 2, 0, false );
    SubLFiles.declareFunction( me, "show_feature_problems", "SHOW-FEATURE-PROBLEMS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_feature_problem", "SHOW-ONE-FEATURE-PROBLEM", 1, 1, false );
    SubLFiles.declareFunction( me, "show_one_feature_assumption", "SHOW-ONE-FEATURE-ASSUMPTION", 2, 0, false );
    SubLFiles.declareFunction( me, "feature_explanation", "FEATURE-EXPLANATION", 1, 0, false );
    SubLFiles.declareFunction( me, "all_privacy_violations", "ALL-PRIVACY-VIOLATIONS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_privacy_violations", "MODULE-PRIVACY-VIOLATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "show_privacy_violations", "SHOW-PRIVACY-VIOLATIONS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_privacy_violation", "SHOW-ONE-PRIVACY-VIOLATION", 1, 1, false );
    SubLFiles.declareFunction( me, "all_early_rebinding_violations", "ALL-EARLY-REBINDING-VIOLATIONS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_early_rebinding_violations", "MODULE-EARLY-REBINDING-VIOLATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "show_early_rebinding_violations", "SHOW-EARLY-REBINDING-VIOLATIONS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_early_rebinding_violation", "SHOW-ONE-EARLY-REBINDING-VIOLATION", 1, 1, false );
    SubLFiles.declareFunction( me, "all_early_reference_violations", "ALL-EARLY-REFERENCE-VIOLATIONS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_early_reference_violations", "MODULE-EARLY-REFERENCE-VIOLATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "td_early_macro_use_p", "TD-EARLY-MACRO-USE-P", 5, 0, false );
    SubLFiles.declareFunction( me, "td_early_evaluation_reference_p", "TD-EARLY-EVALUATION-REFERENCE-P", 6, 0, false );
    SubLFiles.declareFunction( me, "td_early_global_binding_p", "TD-EARLY-GLOBAL-BINDING-P", 3, 0, false );
    SubLFiles.declareFunction( me, "show_early_reference_violations", "SHOW-EARLY-REFERENCE-VIOLATIONS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_early_reference_violation", "SHOW-ONE-EARLY-REFERENCE-VIOLATION", 1, 1, false );
    SubLFiles.declareFunction( me, "all_early_macro_use_violations", "ALL-EARLY-MACRO-USE-VIOLATIONS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_early_macro_use_violations", "MODULE-EARLY-MACRO-USE-VIOLATIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "show_early_macro_use_violations", "SHOW-EARLY-MACRO-USE-VIOLATIONS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_early_macro_use_violation", "SHOW-ONE-EARLY-MACRO-USE-VIOLATION", 1, 1, false );
    SubLFiles.declareFunction( me, "all_undefined_references", "ALL-UNDEFINED-REFERENCES", 0, 1, false );
    SubLFiles.declareFunction( me, "module_undefined_references", "MODULE-UNDEFINED-REFERENCES", 1, 0, false );
    SubLFiles.declareFunction( me, "show_undefined_references", "SHOW-UNDEFINED-REFERENCES", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_undefined_reference", "SHOW-ONE-UNDEFINED-REFERENCE", 1, 1, false );
    SubLFiles.declareFunction( me, "all_multiple_definitions", "ALL-MULTIPLE-DEFINITIONS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_multiple_definitions", "MODULE-MULTIPLE-DEFINITIONS", 1, 0, false );
    SubLFiles.declareFunction( me, "show_multiple_definitions", "SHOW-MULTIPLE-DEFINITIONS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_multiple_definition", "SHOW-ONE-MULTIPLE-DEFINITION", 1, 1, false );
    SubLFiles.declareFunction( me, "all_early_constant_references", "ALL-EARLY-CONSTANT-REFERENCES", 0, 1, false );
    SubLFiles.declareFunction( me, "module_early_constant_references", "MODULE-EARLY-CONSTANT-REFERENCES", 1, 0, false );
    SubLFiles.declareFunction( me, "show_early_constant_references", "SHOW-EARLY-CONSTANT-REFERENCES", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_early_constant_reference", "SHOW-ONE-EARLY-CONSTANT-REFERENCE", 1, 1, false );
    SubLFiles.declareFunction( me, "show_api_macros_with_non_api_expansions", "SHOW-API-MACROS-WITH-NON-API-EXPANSIONS", 0, 1, false );
    SubLFiles.declareFunction( me, "show_one_api_macro_visibility_problem", "SHOW-ONE-API-MACRO-VISIBILITY-PROBLEM", 1, 1, false );
    SubLFiles.declareFunction( me, "non_api_expansions_for_api_macro", "NON-API-EXPANSIONS-FOR-API-MACRO", 1, 0, false );
    SubLFiles.declareFunction( me, "all_build_problems", "ALL-BUILD-PROBLEMS", 0, 1, false );
    SubLFiles.declareFunction( me, "module_build_problems", "MODULE-BUILD-PROBLEMS", 1, 0, false );
    SubLFiles.declareFunction( me, "show_build_problems", "SHOW-BUILD-PROBLEMS", 0, 2, false );
    SubLFiles.declareFunction( me, "show_one_build_problem", "SHOW-ONE-BUILD-PROBLEM", 1, 1, false );
    SubLFiles.declareFunction( me, "count_build_problems", "COUNT-BUILD-PROBLEMS", 0, 1, false );
    SubLFiles.declareFunction( me, "build_problem_counts_by_module", "BUILD-PROBLEM-COUNTS-BY-MODULE", 0, 1, false );
    SubLFiles.declareFunction( me, "td_method_co_called_methods", "TD-METHOD-CO-CALLED-METHODS", 1, 0, false );
    SubLFiles.declareFunction( me, "td_method_co_called_globals", "TD-METHOD-CO-CALLED-GLOBALS", 1, 0, false );
    SubLFiles.declareFunction( me, "td_macro_expansion_calls", "TD-MACRO-EXPANSION-CALLS", 1, 0, false );
    SubLFiles.declareFunction( me, "td_possible_orphan_methods_from_module", "TD-POSSIBLE-ORPHAN-METHODS-FROM-MODULE", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_translator_utilities_file()
  {
    $xref_database_mode$ = SubLFiles.defvar( "*XREF-DATABASE-MODE*", $kw0$NEW );
    $td_methods_called_transitively$ = SubLFiles.defparameter( "*TD-METHODS-CALLED-TRANSITIVELY*", NIL );
    $td_globals_called_transitively$ = SubLFiles.defparameter( "*TD-GLOBALS-CALLED-TRANSITIVELY*", NIL );
    return NIL;
  }

  public static SubLObject setup_translator_utilities_file()
  {
    memoization_state.note_memoized_function( $sym1$TD_TRANSLATION_MODULES_MEMOIZED );
    memoization_state.note_memoized_function( $sym2$TD_MODULE_TRANSLATION_POSITION );
    memoization_state.note_memoized_function( $sym13$TD_METHOD_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED );
    memoization_state.note_memoized_function( $sym15$TD_GLOBAL_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED );
    memoization_state.note_memoized_function( $sym26$POTENTIAL_MODULE_LEVEL_FEATURE_INCONGRUENCE );
    return NIL;
  }
  ...
  static
  {
    me = new translator_utilities();
    $xref_database_mode$ = null;
    $td_methods_called_transitively$ = null;
    $td_globals_called_transitively$ = null;
    $kw0$NEW = makeKeyword( "NEW" );
    $sym1$TD_TRANSLATION_MODULES_MEMOIZED = makeSymbol( "TD-TRANSLATION-MODULES-MEMOIZED" );
    $sym2$TD_MODULE_TRANSLATION_POSITION = makeSymbol( "TD-MODULE-TRANSLATION-POSITION" );
    $sym3$TD_MODULE_EARLIER = makeSymbol( "TD-MODULE-EARLIER" );
    $sym4$TD_MODULE_P = makeSymbol( "TD-MODULE-P" );
    $sym5$SYMBOLP = makeSymbol( "SYMBOLP" );
    $kw6$DYNAMIC = makeKeyword( "DYNAMIC" );
    $kw7$SPECIAL = makeKeyword( "SPECIAL" );
    $kw8$UNSPECIFIED = makeKeyword( "UNSPECIFIED" );
    $str9$Checking__A_for_dependencies___ = makeString( "Checking ~A for dependencies.~%" );
    $sym10$STRING_LESSP = makeSymbol( "STRING-LESSP" );
    $sym11$SYMBOL_NAME = makeSymbol( "SYMBOL-NAME" );
    $sym12$TD_MODULE_LATER = makeSymbol( "TD-MODULE-LATER" );
    $sym13$TD_METHOD_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED = makeSymbol( "TD-METHOD-LATEST-TRANSITIVELY-REFERENCED-MODULE-MEMOIZED" );
    $kw14$UNSEEN = makeKeyword( "UNSEEN" );
    $sym15$TD_GLOBAL_LATEST_TRANSITIVELY_REFERENCED_MODULE_MEMOIZED = makeSymbol( "TD-GLOBAL-LATEST-TRANSITIVELY-REFERENCED-MODULE-MEMOIZED" );
    $kw16$OR = makeKeyword( "OR" );
    $kw17$NOT = makeKeyword( "NOT" );
    $kw18$AND = makeKeyword( "AND" );
    $sym19$NON_FEATURE_SYMBOL_P = makeSymbol( "NON-FEATURE-SYMBOL-P" );
    $sym20$TRANSFORM_NON_FEATURE_SYMBOL = makeSymbol( "TRANSFORM-NON-FEATURE-SYMBOL" );
    $str21$Identifying_module_level_feature_ = makeString( "Identifying module-level feature problems" );
    $str22$cdolist = makeString( "cdolist" );
    $kw23$METHOD = makeKeyword( "METHOD" );
    $kw24$GLOBAL = makeKeyword( "GLOBAL" );
    $kw25$MODULE = makeKeyword( "MODULE" );
    $sym26$POTENTIAL_MODULE_LEVEL_FEATURE_INCONGRUENCE = makeSymbol( "POTENTIAL-MODULE-LEVEL-FEATURE-INCONGRUENCE" );
    $list27 = ConsesLow.list( ConsesLow.list( makeSymbol( "DEF-TYPE" ), makeSymbol( "&REST" ), makeSymbol( "DEF-ARGS" ) ), ConsesLow.list( makeSymbol( "REF-TYPE" ), makeSymbol( "&REST" ), makeSymbol( "REF-ARGS" ) ),
        makeSymbol( "INCONGRUENCE" ) );
    $list28 = ConsesLow.list( makeSymbol( "MODULE" ), makeSymbol( "DEF-FEATURES" ) );
    $str29$__Top_level_form_in__A__ = makeString( "~%Top-level form in ~A~%" );
    $list30 = ConsesLow.list( makeSymbol( "DEF-OBJECT" ), makeSymbol( "MODULE" ), makeSymbol( "DEF-FEATURES" ) );
    $str31$__Method__A_in__A__ = makeString( "~%Method ~A in ~A~%" );
    $str32$__Global__A_in__A__ = makeString( "~%Global ~A in ~A~%" );
    $str33$_references = makeString( " references" );
    $list34 = ConsesLow.list( makeSymbol( "REF-OBJECT" ), makeSymbol( "MODULE" ), makeSymbol( "REF-FEATURES" ) );
    $str35$__method__A_from__A__ = makeString( "~%method ~A from ~A~%" );
    $str36$__global__A_from__A__ = makeString( "~%global ~A from ~A~%" );
    $str37$__which_will_have_problems_when__ = makeString( "~%which will have problems when :~%  ~A~%" );
    $str38$assuming__A = makeString( "assuming ~A" );
    $str39$no_features = makeString( "no features" );
    $str40$Identifying_privacy_violations = makeString( "Identifying privacy violations" );
    $list41 = ConsesLow.list( ConsesLow.list( makeSymbol( "DEF-TYPE" ), makeSymbol( "&REST" ), makeSymbol( "DEF-ARGS" ) ), ConsesLow.list( makeSymbol( "REF-TYPE" ), makeSymbol( "&REST" ), makeSymbol( "REF-ARGS" ) ) );
    $list42 = ConsesLow.list( makeSymbol( "MODULE" ) );
    $list43 = ConsesLow.list( makeSymbol( "DEF-OBJECT" ), makeSymbol( "MODULE" ) );
    $str44$_references_private = makeString( " references private" );
    $list45 = ConsesLow.list( makeSymbol( "REF-OBJECT" ), makeSymbol( "MODULE" ) );
    $str46$Identifying_early_rebinding_viola = makeString( "Identifying early rebinding violations" );
    $str47$_rebinds = makeString( " rebinds" );
    $str48$_before_it_is_defined__ = makeString( " before it is defined~%" );
    $str49$Identifying_early_reference_viola = makeString( "Identifying early reference violations" );
    $list50 = ConsesLow.list( ConsesLow.list( makeSymbol( "DEF-TYPE" ), makeSymbol( "&REST" ), makeSymbol( "DEF-ARGS" ) ), ConsesLow.list( makeSymbol( "REF-TYPE" ), makeSymbol( "REF-OBJECT" ), makeSymbol( "MODULE" ) ),
        makeSymbol( "JUSTIFYING-METHODS" ), makeSymbol( "JUSTIFYING-GLOBALS" ) );
    $list51 = ConsesLow.list( makeSymbol( "MODULE" ), makeSymbol( "POSITION" ) );
    $str52$__Top_level_form_in__A_at__S__ = makeString( "~%Top-level form in ~A at ~S~%" );
    $str53$_before_it_is = makeString( " before it is" );
    $str54$_defined__ = makeString( " defined~%" );
    $str55$_completely_defined_due_to__ = makeString( " completely defined due to~%" );
    $str56$method__A__ = makeString( "method ~A~%" );
    $str57$global__A__ = makeString( "global ~A~%" );
    $str58$Identifying_early_macro_use_viola = makeString( "Identifying early macro use violations" );
    $str59$Identifying_undefined_references = makeString( "Identifying undefined references" );
    $str60$_references_undefined = makeString( " references undefined" );
    $list61 = ConsesLow.list( makeSymbol( "REF-OBJECT" ) );
    $str62$__method__A__ = makeString( "~%method ~A~%" );
    $str63$__global__A__ = makeString( "~%global ~A~%" );
    $list64 = ConsesLow.list( makeSymbol( "DEF-TYPE" ), makeSymbol( "DEF-OBJECT" ), makeSymbol( "DEFINITION-POSITIONS" ) );
    $str65$__Method__A_has_multiple_definiti = makeString( "~%Method ~A has multiple definitions in" );
    $str66$__Global__A_has_multiple_definiti = makeString( "~%Global ~A has multiple definitions in" );
    $list67 = ConsesLow.cons( makeSymbol( "MODULE" ), makeSymbol( "FILE-POSITION" ) );
    $str68$_____A_at_position__S = makeString( "~%  ~A at position ~S" );
    $sym69$READER_MAKE_CONSTANT_SHELL = makeSymbol( "READER-MAKE-CONSTANT-SHELL" );
    $list70 = ConsesLow.list( makeSymbol( "REF-TYPE" ), makeSymbol( "&REST" ), makeSymbol( "REF-ARGS" ) );
    $str71$references____before_it_is_comple = makeString( "references #$ before it is completely defined~%" );
    $kw72$API_MACRO_VISIBILITY = makeKeyword( "API-MACRO-VISIBILITY" );
    $list73 = ConsesLow.cons( makeSymbol( "MACRO" ), makeSymbol( "NON-API-EXPANSIONS" ) );
    $str74$__API_macro__A_expands_to_non_API = makeString( "~&API macro ~A expands to non-API methods~% ~S~%" );
    $sym75$API_PREDEFINED_MACRO_P = makeSymbol( "API-PREDEFINED-MACRO-P" );
    $str76$Determining_build_problems = makeString( "Determining build problems" );
    $kw77$MULTIPLE_DEFINITIONS = makeKeyword( "MULTIPLE-DEFINITIONS" );
    $kw78$UNDEFINED_REFERENCES = makeKeyword( "UNDEFINED-REFERENCES" );
    $kw79$PRIVACY_VIOLATIONS = makeKeyword( "PRIVACY-VIOLATIONS" );
    $kw80$EARLY_CONSTANT_REFERENCES = makeKeyword( "EARLY-CONSTANT-REFERENCES" );
    $kw81$EARLY_REFERENCE_VIOLATIONS = makeKeyword( "EARLY-REFERENCE-VIOLATIONS" );
    $kw82$EARLY_REBINDING_VIOLATIONS = makeKeyword( "EARLY-REBINDING-VIOLATIONS" );
    $str83$__No_build_problems_ = makeString( "~%No build problems!" );
    $list84 = ConsesLow.list( makeSymbol( "PROBLEM-CLASS" ), makeSymbol( "PROBLEMS" ) );
    $str85$Unknown_build_problem_class__S = makeString( "Unknown build problem class ~S" );
    $sym86$STRINGP = makeSymbol( "STRINGP" );
    $sym87$_ = makeSymbol( ">" );
    $sym88$CDR = makeSymbol( "CDR" );
    $kw89$UNINITIALIZED = makeKeyword( "UNINITIALIZED" );
    $str90$CLEAR = makeString( "CLEAR" );
    $str91$REMOVE = makeString( "REMOVE" );
  }
}
/*
 * 
