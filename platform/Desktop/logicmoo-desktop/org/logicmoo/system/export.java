...
public final class export extends SubLTranslatedFile {
	...
	public static SubLObject prolog_need_quote(SubLObject str) {
		if ((!(Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen, str, UNPROVIDED, UNPROVIDED,
				UNPROVIDED, UNPROVIDED, UNPROVIDED).equal(str)))) {
			return T;
		}
		{
			...
			return makeBoolean(((NIL == Characters.lower_case_p(ch)) || (NIL != Sequences.find(Characters.CHAR_colon,
					str, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED))));
		}
	}
		...
		} else {
			{
				...
				if ((NIL != Functions.funcall(makeFunctionSymbol("FUNCTION?"), pred))) {
					if ((NIL != Functions.funcall(makeFunctionSymbol("ISA?"), pred, $const32$UnreifiableFunction))) {
						...
						plw_argz(va, cdrtrm);
						plw_str($str30$_);
					} else if ((NIL != Functions.funcall(makeFunctionSymbol("ISA?"), pred,
							$const34$ReifiableFunction))) {
						plw_str(str);
						plw_str($str35$R_);
						plw_term(pred, UNPROVIDED);
						plw_argz(va, cdrtrm);
	...
	public static SubLObject assertion_varnames(SubLObject assrt) {
		{
			...
			SubLObject v = NIL;
			for (v = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
					.rest(), v = cdolist_list_var.first()) {
				varnames = ConsesLow.append(varnames,
						list(Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen,
								Functions.funcall(makeFunctionSymbol("EL-VAR-NAME-WITHOUT-PREFIX"), v), UNPROVIDED,
	...
	public static SubLObject plw_string(SubLObject trm) {
		if ((!(((NIL != Sequences.search($str45$_, trm, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
				UNPROVIDED)) && THREE_INTEGER.numL(Sequences.length(trm)))))) {
			...
			for (index = ZERO_INTEGER; index.numL(slen); index = Numbers.add(index, ONE_INTEGER)) {
				{
					...
					} else if ((curr.eql(Characters.CHAR_period)
							&& (index.eql(Numbers.subtract(slen, ONE_INTEGER)) || Strings
									.sublisp_char(str, Numbers.add(index, ONE_INTEGER)).eql(Characters.CHAR_space)))) {
						if ((NIL != word)) {
							tokens = cons(rsw(word), tokens);
							word = NIL;
							tokens = cons($str51$_, tokens);
						}
					} else if (((NIL != Characters.alphanumericp(curr))
							|| (NIL != conses_high.member(curr, $list52, UNPROVIDED, UNPROVIDED)))) {
						if ((NIL != word)) {
							word = PrintLow.format(NIL, $str50$_A_C, word, curr);
						} else {
	...
	public static SubLObject unescape(SubLObject str) {
		str = Sequences.substitute(Characters.CHAR_vertical, Characters.CHAR_return,
				Sequences.substitute(Characters.CHAR_vertical, Characters.CHAR_newline, str, UNPROVIDED, UNPROVIDED,
	...
	public static SubLObject plw_term(SubLObject trm, SubLObject is_pred) {
		...
		}
		{
			...
			if ((NIL != Functions.funcall(makeFunctionSymbol("NART-P"), trm))) {
				...
				plw_term(
						ConsesLow.nth(Functions.funcall(makeFunctionSymbol("VARIABLE-ID"), trm), Functions
								.funcall(makeFunctionSymbol("ASSERTION-EL-VARIABLES"), $a$.getDynamicValue(thread))),
						UNPROVIDED);
			} else if ((NIL != Functions.funcall(makeFunctionSymbol("EL-VARIABLE-P"), trm))) {
				plw_str(Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen,
						Functions.funcall(makeFunctionSymbol("EL-VAR-NAME-WITHOUT-PREFIX"), trm), UNPROVIDED,
						UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED));
	...
	public static SubLObject plw_term_cons(SubLObject trm, SubLObject is_pred) {
		...
		}
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				...
				} else if (((NIL != conses_high.member(pred, $no_tify$.getDynamicValue(thread), UNPROVIDED, UNPROVIDED))
						|| ((NIL != Functions.funcall(makeFunctionSymbol("CONSTANT-P"), pred))
								&& (NIL != Functions.funcall(makeFunctionSymbol("PREDICATE?"), pred))))) {
	...
	public static SubLObject pre_export() {
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				SubLObject cdolist_list_var = Functions.funcall(makeFunctionSymbol("ALL-INSTANCES"),
						$const63$Quantifier);
				SubLObject s = NIL;
				for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
						.rest(), s = cdolist_list_var.first()) {
					{
						SubLObject item_var = s;
						if ((NIL == conses_high.member(item_var, $no_tify$.getDynamicValue(thread),
			...
			{
				SubLObject cdolist_list_var = Functions.funcall(makeFunctionSymbol("ALL-INSTANCES"),
						$const65$MicrotheoryDesignatingPredicate);
				SubLObject s = NIL;
				for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
						.rest(), s = cdolist_list_var.first()) {
					{
		...
		}
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				...
				SubLObject found = NIL;
				{
					...
					try {
						StreamsLow.$standard_output$.bind(((NIL != out) ? ((SubLObject) out)
								: StreamsLow.$standard_output$.getDynamicValue(thread)), thread);
						if (assrt.isInteger()) {
						...
						$assrtform$.setDynamicValue(Functions.funcall(makeFunctionSymbol("ASSERTION-FORMULA"), assrt),
								thread);
						if ((NIL != $assrtform$.getDynamicValue(thread))) {
							if ((!($assrtform$.getDynamicValue(thread).first().equal($const70$termOfUnit)))) {
								...
								if ((NIL != elvars)) {
									...
									plw_str($assrtid$.getDynamicValue(thread));
									plw_str($str77$__);
									plw_list(assertion_varnames($a$.getDynamicValue(thread)), UNPROVIDED, UNPROVIDED,
											UNPROVIDED);
									plw_str($str78$___);
									plw_nl();
								}
								plw_ruleinfo(((NIL != Functions.funcall(makeFunctionSymbol("RULE-ASSERTION?"), assrt))
										? ((SubLObject) $str79$rule) : $str80$fact), UNPROVIDED);
								plw_ruleinfo(Strings.string_downcase(
										Symbols.symbol_name(
												Functions.funcall(makeFunctionSymbol("ASSERTION-STRENGTH"), assrt)),
										UNPROVIDED, UNPROVIDED), UNPROVIDED);
								plw_ruleinfo(Strings.string_downcase(
										Symbols.symbol_name(
												Functions.funcall(makeFunctionSymbol("ASSERTION-DIRECTION"), assrt)),
										UNPROVIDED, UNPROVIDED), UNPROVIDED);
	...
	public static SubLObject assertion_variable_guard(SubLObject assrt) {
		...
		return Functions.funcall(makeFunctionSymbol("FORMULA-VARIABLES-ARG-CONSTRAINTS"),
				Functions.funcall(makeFunctionSymbol("ASSERTION-FORMULA"), assrt),
				Functions.funcall(makeFunctionSymbol("ASSERTION-MT"), assrt));
	}
	...
	public static SubLObject ri() {
		{
			SubLObject anum = NIL;
			for (anum = $int84$990000; (!(anum.numE(Numbers.add($int84$990000, $int85$500)))); anum = Functions
		...
		if (type.isCons()) {
			{
				...
				SubLObject tipe = NIL;
				for (tipe = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
						.rest(), tipe = cdolist_list_var.first()) {
	...
	public static SubLObject no_errors(SubLObject macroform, SubLObject environment) {
		{
			...
			current = current.rest();
			{
				...
				return list($sym87$CLET, $list88,
						list($sym89$WITH_ERROR_HANDLER, $list90,
								list($sym91$CSETQ, $sym92$LASTVAL,
										listS($sym93$PROGN, $list94, $list95, ConsesLow.append(code, NIL)))),
						listS($sym96$FIF, $sym97$HADERROR, list($sym93$PROGN, $list98, list($sym99$QUOTE, err)),
								$list100));
			}
		}
	}
	...
	// {
	// SubLObject anum = NIL;
	// for (anum = ONE_INTEGER; (!(anum.numE($int85$500))); anum =
	...
	// {
	// SubLObject anum = NIL;
	// for (anum = Numbers.add(start, $int116$250); (!(anum.numL(start))); anum
	// = Functions
	// .funcall(makeFunctionSymbol("1-"), anum)) {
	...
	public static SubLObject plw_support(SubLObject d) {
		plw_term(list(Functions.funcall(makeFunctionSymbol("SUPPORT-MT"), d),
				Functions.funcall(makeFunctionSymbol("SUPPORT-SENTENCE"), d),
				Symbols.symbol_name(Functions.funcall(makeFunctionSymbol("SUPPORT-TRUTH"), d)),
				Symbols.symbol_name(Functions.funcall(makeFunctionSymbol("SUPPORT-STRENGTH"), d)),
				Symbols.symbol_name(Functions.funcall(makeFunctionSymbol("SUPPORT-MODULE"), d))), UNPROVIDED);
		return NIL;
	}
	...
	public static SubLObject plw_deduction(SubLObject ded) {
		{
			...
			$a$.setDynamicValue(Functions.funcall(makeFunctionSymbol("DEDUCTION-ASSERTION"), ded), thread);
			{
				...
				SubLObject d = NIL;
				for (d = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
						.rest(), d = cdolist_list_var.first()) {
					...
					if ((NIL != Functions.funcall(makeFunctionSymbol("ASSERTION-P"), $a$.getDynamicValue(thread)))) {
						plw_id($str71$a,
								Functions.funcall(makeFunctionSymbol("ASSERTION-ID"), $a$.getDynamicValue(thread)));
					} else {
	...
	public static SubLObject dt() {
		Eval.load($str130$e2c_prologmud_lisp);
		{
			...
			SubLObject anum = NIL;
			for (anum = ZERO_INTEGER; (!(anum
					.numE(Functions.funcall(makeFunctionSymbol("DEDUCTION-COUNT"))))); anum = Functions
							.funcall(makeFunctionSymbol("1+"), anum)) {
	...
	public static SubLObject dumpt(SubLObject anum, SubLObject total, SubLObject filename) {
		{
			if(UNPROVIDED==total||NIL==total) total = Functions.funcall(makeFunctionSymbol("ASSERTION-COUNT"));
			if(UNPROVIDED==anum||NIL==anum) anum  =  ZERO_INTEGER;
			if(filename==UNPROVIDED) filename = $str133$dump4_txt;
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				...
				print_high.princ($str134$_____style_check__discontiguous__, file_output);
				{
					...
					try {
						...
						$percent_progress_start_time$.bind(Time.get_universal_time(), thread);
						try {
							Functions.funcall(makeFunctionSymbol("NOTING-PERCENT-PROGRESS-PREAMBLE"),
									$str135$exporting_KB___);
							{
								//$int136$415542
								...
								for ( 
									...
									{
										SubLObject assrt = Functions.funcall(makeFunctionSymbol("FIND-ASSERTION-BY-ID"),
												anum);
										try {
	...
	public static SubLObject et() {		
		$no_tify$ = defvar("*NO-TIFY*",
				Functions.funcall(makeFunctionSymbol("ALL-INSTANCES"), $const62$WFFConstraintPredicate));
		pre_export();
		...
		//dumpt(ZERO_INTEGER, $int136$415542,makeString("part0.txt"));
	...
	public static SubLObject save_constsl() {
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				...
				SubLObject x = NIL;
				for (x = ZERO_INTEGER; (!(x.numE(Functions.funcall(makeFunctionSymbol("CONSTANT-COUNT"))))); x = Numbers
						.add(x, ONE_INTEGER)) {
					{
						...
						if ((NIL == found)) {
							{
								...
								if ((NIL != prolog_need_quote(name))) {
									PrintLow.format(T, $str140$_safely_rename_or_merge__S__S____, name,
											Sequences.cconcatenate($str143$cyc, name));
								}
							}
						}
					}
	...
	public static SubLObject simplrename(SubLObject found) {
		{
			SubLObject nfound = Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen,
					Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_colon, found, UNPROVIDED, UNPROVIDED,
							UNPROVIDED, UNPROVIDED, UNPROVIDED),
					UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
			...
			if ((ch.eql(Characters.CHAR_t) && (NIL != Characters.lower_case_p(ch1)))) {
				return Sequences.cconcatenate($str145$t,
						new SubLObject[] { Strings.string_upcase(Strings.string(ch1), UNPROVIDED, UNPROVIDED),
								Functions.funcall(makeFunctionSymbol("SUBSTRING"), nfound, TWO_INTEGER) });
			}
			return nfound;
		}
	}
	...
	public static SubLObject declare_export_file() {
		...
		// declareFunction(myName, "plw_ruleinfo", "PLW-RULEINFO", 1, 1, false);
		 declareFunction(myName, "assertion_variable_guard",
		"ASSERTION-VARIABLE-GUARD", 1, 0, false);
	...
	public static SubLObject init_export_file() {
		...
		$renames$ = defvar("*RENAMES*", Hashtables.make_hash_table($int18$336790, UNPROVIDED, UNPROVIDED));
	...
	private static final SubLList $list6 = list(makeKeyword("CYC-DATE"), makeKeyword("CYC-TIME"),
			makeKeyword("CYC-QUANT"), makeKeyword("CYC-HTML"), makeKeyword("CYC-BROWSER"), makeKeyword("CYC-EXTERNAL"),
			makeKeyword("CYC-EVENT"), makeKeyword("COMMON-LISP"));
	...
	private static final SubLString $str12$_opt_cyc_e2c_system_manifest_lisp = makeString(
	...
	private static final SubLInteger $int18$336790 = makeInteger(336790);
	...
	private static final SubLList $list22 = list(Characters.CHAR_colon, Characters.CHAR_quote,
			Characters.CHAR_backquote, Characters.CHAR_hash);
	...
	private static final SubLObject $const31$VariableArityRelation = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("VariableArityRelation"));
	private static final SubLObject $const32$UnreifiableFunction = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("UnreifiableFunction"));
	private static final SubLString $str33$U_ = makeString("U(");
	private static final SubLObject $const34$ReifiableFunction = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("ReifiableFunction"));
	...
	private static final SubLList $list47 = list(new SubLObject[] { Characters.CHAR_quote, Characters.CHAR_quotation,
			Characters.CHAR_semicolon, Characters.CHAR_question, Characters.CHAR_percent, Characters.CHAR_comma,
			Characters.CHAR_ampersand, Characters.CHAR_lparen, Characters.CHAR_rparen });
	...
	private static final SubLList $list52 = list(Characters.CHAR_underbar, Characters.CHAR_period, Characters.CHAR_hash,
			Characters.CHAR_hyphen, Characters.CHAR_slash, Characters.CHAR_less);
	...
	private static final SubLObject $const62$WFFConstraintPredicate = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("WFFConstraintPredicate"));
	private static final SubLObject $const63$Quantifier = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("Quantifier"));
	private static final SubLObject $const64$HLPredicate = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("HLPredicate"));
	private static final SubLObject $const65$MicrotheoryDesignatingPredicate = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("MicrotheoryDesignatingPredicate"));
	private static final SubLObject $const66$LogicalConnective = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("LogicalConnective"));
	private static final SubLObject $const67$KBDependentRelation = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("KBDependentRelation"));
	private static final SubLObject $const68$ModalPredicate = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("ModalPredicate"));
	private static final SubLObject $const69$trueRule = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("trueRule"));
	private static final SubLObject $const70$termOfUnit = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("termOfUnit"));
	...
	private static final SubLInteger $int84$990000 = makeInteger(990000);
	private static final SubLInteger $int85$500 = makeInteger(500);
	...
	private static final SubLList $list90 = list(makeSymbol("FUNCTION"),
			list(makeSymbol("LAMBDA"), list(makeSymbol("&REST"), makeSymbol("WHATEVAH")),
					list(makeSymbol("CSETQ"), makeSymbol("HADERROR"), makeSymbol("*ERROR-MESSAGE*")),
					list(makeSymbol("FORMAT"), makeSymbol("*TERMINAL-IO*"),
							makeString("~&~&~&~&;; made-handler ~S durring: ~S ~S ~&"), makeSymbol("*ERROR-MESSAGE*"),
							makeSymbol("WHATEVAH"), makeSymbol("CODE")),
					list(makeSymbol("RET"), makeSymbol("HADERROR"))));
	...
	private static final SubLList $list101 = list(makeSymbol("LAMBDA"),
			list(makeSymbol("&REST"), makeSymbol("WHATEVAH")),
			list(makeSymbol("CSETQ"), makeSymbol("HADERROR"), makeSymbol("*ERROR-MESSAGE*")),
			list(makeSymbol("FORMAT"), makeSymbol("*TERMINAL-IO*"),
					makeString("~&~&~&~&;; made-handler ~S durring: ~S ~S ~&"), makeSymbol("*ERROR-MESSAGE*"),
					makeSymbol("WHATEVAH"), makeSymbol("CODE")),
	...
	private static final SubLList $list104 = list(new SubLObject[] { makeSymbol("SELF-LOOPING-RULE?"),
			makeSymbol("PROOF-VIEW-PRUNING-RULE?"), makeSymbol("SELF-EXPANDING-RULE?"), makeSymbol("INERT-RULE?"),
			makeSymbol("LOOKS-LIKE-KAPPA-REPHRASE-RULE?"), makeSymbol("EXPANSION-RULE?"),
			makeSymbol("APPEARS-IN-FORWARD-REIFICATION-RULE?"), makeSymbol("PSP-TRACED-RULE?"),
			makeSymbol("UNIVERSAL-LIFTING-RULE?"), makeSymbol("DECONTEXTUALIZED-RULE?"),
			makeSymbol("SOME-SUGGESTED-PRAGMA-SENTENCES-FOR-RULE?"), makeSymbol("CURRENT-NC-RULE?"),
			makeSymbol("SINGLE-LITERAL-ANTECEDENT-RULE?"), makeSymbol("CONSIDERED-BUT-NOT-SUCCESSFUL-RULE?"),
			makeSymbol("CONSTRAINT-RULE?"), makeSymbol("LIFTING-CONSEQUENT-RULE?"), makeSymbol("ASSERTIVE-WFF-RULE?"),
			makeSymbol("TWO-LITERAL-ANTECEDENT-RULE?"), makeSymbol("REWRITE-RULE?"), makeSymbol("SINGLE-LITERAL-RULE?"),
			makeSymbol("OBSOLETE-RULE?"), makeSymbol("SKOLEM-RULE?"), makeSymbol("FORWARD-TMS-RULE?"),
			makeSymbol("CYCL-REFORMULATION-RULE?"), makeSymbol("IS-KNOWN-ILP-EXPORT-RULE?"),
			makeSymbol("VERBOSIFIED-EL-RULE?"), makeSymbol("PPH-REQUIRED-RULE?"), makeSymbol("NON-ABDUCIBLE-RULE?"),
	...
	private static final SubLList $list107 = list(makeSymbol("VALID-PPH-BLACKLIST-ASSERTION?"),
			makeSymbol("WHY-KBS-ASSERTION?"), // makeSymbol("EXPORTABLE-ASSERTION?"), 
			makeSymbol("TRUE-ASSERTION?"),
			makeSymbol("HL-ASSERTION-P"), makeSymbol("NON-TVA-GAF-ASSERTION-P"), makeSymbol("STR-META-ASSERTION-P"),
			makeSymbol("FIRST-ORDER-ASSERTION-P"));
	private static final SubLList $list108 = list(new SubLObject[] { makeSymbol("ASSERTED-ASSERTION?"),
			makeSymbol("DEDUCED-ASSERTION?"), makeSymbol("CODE-ASSERTION?"), makeSymbol("ASSERTION-HAS-DEPENDENTS-P"),
			makeSymbol("ASSERTION-VARIABLE-GUARD"), makeSymbol("ASSERTED-BY"), makeSymbol("ASSERTED-WHEN"),
			makeSymbol("ASSERTION-HAS-META-ASSERTIONS?"), makeSymbol("SYNTACTICALLY-ILL-FORMED-ASSERTION?"),
			makeSymbol("SOME-CONSERVATIVE-VIA-ARG-ASSERTION?"), makeSymbol("RELEVANT-ASSERTION?"),
			makeSymbol("REFORMULATOR-RELEVANT-ASSERTION?"), makeSymbol("PPH-HL-SUPPORT-WITH-ISOMORPHIC-ASSERTION?"),
			makeSymbol("OWL-EXPORTABLE-RULE-ASSERTION?"), makeSymbol("NL-TRIE-SYNTACTIC-ASSERTION?"),
			makeSymbol("IST-SENTENCE-DESIGNATING-ASSERTION?"), makeSymbol("INVALID-ASSERTION?"),
			makeSymbol("INFERENCE-RELEVANT-ASSERTION?"), makeSymbol("INFERENCE-IRRELEVANT-ASSERTION?"),
			makeSymbol("INDIRECT-LEXICAL-ASSERTION?"), makeSymbol("IMPLEMENTATION-ASSERTION?"),
			makeSymbol("HAS-GENSTRING-ASSERTION?"), makeSymbol("FALSE-ASSERTION?"),
			makeSymbol("CURRENTLY-FORWARD-PROPAGATING-ASSERTION?"),
			makeSymbol("CONTEXTUALLY-DEPENDENT-LEXICAL-ASSERTION?"), makeSymbol("COMPUTED-SKOLEM-ASSERTION?"),
			makeSymbol("CHECK-FOR-ISOMORPHIC-ASSERTION?"), makeSymbol("BACKWARD-RULE-REQUIRED-FOR-ASSERTION?"),
			makeSymbol("NOT-ASSERTIBLE-ASSERTION-P"), makeSymbol("GEN-TEMPLATE-QUERY-SENTENCE-ASSERTION-P"),
			makeSymbol("RULE-META-ASSERTION-P"), makeSymbol("FTEMPLATE-POLYCANONICALIZED-ASSERTION-P"),
			makeSymbol("UNBOUND-RULE-ASSERTION-P"), makeSymbol("LIFTING-ASSERTION-P"),
			makeSymbol("ASSERTION-PRODUCED-BY-TEMPORAL-PROJECTION?"), makeSymbol("ABNORMAL-ASSERTION-P"),
			makeSymbol("ABBREVIATION-ASSERTION-P"), makeSymbol("TVA-ASSERTION-P"),
			makeSymbol("TERM-OF-UNIT-ASSERTION-P"), makeSymbol("LIST-OF-RULE-ASSERTION-P"),
			makeSymbol("DOCUMENTATION-ASSERTION-P"), makeSymbol("BOOKKEEPING-ASSERTION-P"),
			makeSymbol("META-ASSERTION-P"), makeSymbol("HIGHER-ORDER-ASSERTION-P"), makeSymbol("GTQS-ASSERTION-P"),
			makeSymbol("CVA-ASSERTION-P"), makeSymbol("ASSERTION-NEEDS-REVIEW-P"),
			makeSymbol("ASSERTION-USES-NON-EXTENSIONAL-SET?"), makeSymbol("ASSERTION-SYNTACTICALLY-ILLFORMED?") });
	// private static final SubLString $str115$e2c_export_lisp =
	// makeString("e2c/export.lisp");
	private static final SubLInteger $int116$250 = makeInteger(250);
	...
	private static final SubLObject $const126$InferencePSC = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("InferencePSC"));
	private static final SubLSymbol $sym127$RELEVANT_MT_IS_EVERYTHING = makeSymbol("RELEVANT-MT-IS-EVERYTHING");
	private static final SubLObject $const128$EverythingPSC = Functions
			.funcall(makeFunctionSymbol("READER-MAKE-CONSTANT-SHELL"), makeString("EverythingPSC"));
	...
	private static final SubLInteger $int136$415542 = makeInteger(415542);
	private static final SubLInteger $int137$10000 = makeInteger(10000);
	...
	private static final SubLString $str140$_safely_rename_or_merge__S__S____ = makeString(
			"(safely-rename-or-merge ~S ~S) ~%");
