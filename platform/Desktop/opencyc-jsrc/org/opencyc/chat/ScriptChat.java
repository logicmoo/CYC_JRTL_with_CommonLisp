...
//    public String ircDestination = "#opencycchat";
     ...
     */
    public ScriptChat(CycAccess access,
                   String nick,
                   String comment,
                   String server,
                   int port,
...
//                }
        ...
        try {
            ...
            while( roles.hasNext() )  {
                ...
                try {
                     ...
                     *-------------------------------------------------------------------------*/
                    query = cyc.makeCycList("(#$and (#$equals (#$ScriptRoleFn " + script.stringApiValue() + " " + role.stringApiValue() + ")(#$ScriptRoleFn ?S ?R))"
                        + "(#$not (#$equals ?S " + script.stringApiValue() + "))(#$not (#$equals ?R " + role.stringApiValue() +  ")))");
                    ...
                    while( eqs.hasNext() )  {
                        ...
                        if( !eerste2 ) { result = result + ", "; eerste2 = false; }
    ...
    public void serviceQuery(String cyclist,String returnpath, String query) {
        try {
            sendMessage(returnpath, cyc.converseObject( "(cyc-query '" +
...
//                + ") " + mtForUser(cyclist).stringApiValue() + " '(:backchain 1 :time 60))"));
            sendAnswers(returnpath, cyc.converseObject( "(cyc-query '" +toCycListString(query)
                + cyc.getKnownConstantByName("ScriptBotBeliefsMt").stringApiValue() + " '(:backchain 1 :time 60))"));
        } catch ( Exception e ) {
            sendMessage(returnpath, ""+ e);
        }
    }
    ...
    public CycFort mtForUser(String cyclist) {
        ...
        if ( mt==null ) {
            try {
                ...
                cyc.assertGaf(cyc.universalVocabularyMt, cyc.genlMt, mt, cyc.getKnownConstantByName("#$SPGUTestDataMt"));
                cyc.assertGaf(cyc.universalVocabularyMt, cyc.genlMt, mt, cyc.getKnownConstantByName("#$ScriptBotMt"));
                cyc.assertGaf(cyc.universalVocabularyMt, cyc.genlMt, mt, cyc.getKnownConstantByName("#$RegelingPGBScriptMt"));
                cyc.assertGaf(cyc.universalVocabularyMt, cyc.genlMt, mt, cyc.getKnownConstantByName("#$RegelingPGBDataMt"));
                // genlMt ScriptBotBeliefsMt <from>chatMt
                cyc.assertGaf(cyc.universalVocabularyMt, cyc.genlMt, cyc.getKnownConstantByName("#$ScriptBotBeliefsMt"),mt);
                // make <from>Chatter
                ...
                mt = (CycConstant) cyc.baseKB;
            }
            if( verbosity > 1 ) sendMessage(ircDestination,"(Using microtheory: " + mt.cyclify() 
...
//                    CycList roles = cyc.askWithVariable(query3, roleVar, mtForUser(cyclist));
                    ...
                    while( roleIt.hasNext() )  {
                        ...
                        beliefs.add(belief);
                        // some verbosity 
                        if( verbosity > 1 ) sendMessage(returnpath, "(script and role match for subevent " + subEvents.get(i).toString()
                            + ", script " + scripts.get(j).toString()
                            + ", role " + role.toString() + ")" );
            ...
            while( ssIt.hasNext() )  {
                ...
                serviceSubL(cyclist, returnpath, "(cyc-assert '" + beliefs.stringApiValue() + " " +
                            cyc.getKnownConstantByName("ScriptBotBeliefsMt").stringApiValue() + ")");
                // print it
...
//            sendMessage( returnpath, "jow, ben nu hier hoa" );
            ...
            CycList query = cyc.makeCycList("(#$and (#$subEvents #$" + story.toString() + " ?EVENT) " + 
                    "(#$storyReferences ?EVENT ?SCRIPT) " +
                    "(#$requiredRolesForScript ?SCRIPT ?ROLE) " +
                    "(#$scriptRoleIsa (#$ScriptRoleFn ?SCRIPT ?ROLE) ?COL))");
...
//            while( it.hasNext() && !anewthing )  {
            while( it.hasNext()  )  {
                ...
                if( verbosity > 4 ) sendMessage( returnpath, "Checking existence for event " + ((CycConstant)result.first()).toString()
                           + ", role " + ((CycConstant)result.third()).toString()
                           + ", col " + ((CycConstant)result.fourth()).toString() );
                /*--------------------------------------------------------------------------
                        ...
                        "))" ), cyc.getKnownConstantByName("ScriptBotBeliefsMt"), 2, 100 ) ) // note the backchain steps and time.
                {
                    ...
                    CycList newrolefiller = (CycList) cyc.converseList("(fi-hypothesize '"+hypThis.stringApiValue()
                        + " " + cyc.getKnownConstantByName("ScriptBotBeliefsMt").stringApiValue() + ")" );
...
//            while( newstuff ) {
            if( newstuff ) {
                newstuff = false;
                if ( possibleNextActs( cyclist,returnpath,story )>0 )
                    newstuff = true;
                if ( possibleNextStates( cyclist,returnpath,story )>0 )
                    newstuff = true;
            }
        } catch ( Exception e ) {
            sendMessage(returnpath, ""+ e);
...
//            sendMessage( returnpath, "possibleNextActs called" );
            ...
            variables.add(sVar);
            variables.add(aVar);
            CycList query = cyc.makeCycList("(#$and (#$subSituations #$" + story.toString() + " ?STATE) " + 
                    "(#$isa ?STATESCRIPT #$StateScript) (#$storyReferences ?STATE ?STATESCRIPT) (#$cd-E-ScriptScript ?STATESCRIPT ?ACTSCRIPT))");
            // ask the query (important!! must be done in scriptbotbeliefs, because #$storyReferences are also beliefs.
//            sendMessage(returnpath, query.toString() );
            ...
            while( it.hasNext() )  {
                ...
                if( !cyc.isQueryTrue(
                    cyc.makeCycList("(#$thereExists ?SUB (#$and " +
                        "(#$isa ?SUB #$CD-ACT) " +
                        "(#$subEvents " + story.stringApiValue() + " ?SUB) " +
                        "(#$storyReferences ?SUB " + ((CycConstant)result.second()).stringApiValue() + ")))" ), cyc.getKnownConstantByName("ScriptBotBeliefsMt") ) )
                {
                    /*--------------------------------------------------------------------------
                    ...
                    CycList acts = (CycList) cyc.converseList("(fi-hypothesize '"+hypThis.stringApiValue()
                        + " " + cyc.getKnownConstantByName("ScriptBotBeliefsMt").stringApiValue() + ")" );
...
//            sendMessage( returnpath, "possibleNextStates called" );
            ...
            variables.add(aVar);
            variables.add(sVar);
            CycList query = cyc.makeCycList("(#$and (#$subEvents #$" + story.toString() + " ?ACT) " + 
                    "(#$isa ?ACTSCRIPT #$AtomicActionScript) (#$storyReferences ?ACT ?ACTSCRIPT) (#$cd-r-ScriptScript ?ACTSCRIPT ?STATESCRIPT))");
            // ask the query (important!! must be done in scriptbotbeliefs, because #$storyReferences are also beliefs.
            CycList results = cyc.askWithVariables(query, variables, cyc.getKnownConstantByName("ScriptBotBeliefsMt"));
            Iterator it = results.iterator();
            /*--------------------------------------------------------------------------
            ...
            while( it.hasNext() )  {
                ...
                if( !cyc.isQueryTrue(
                    cyc.makeCycList("(#$thereExists ?SUB (#$and " +
                        "(#$isa ?SUB #$CD-STATE) " +
                        "(#$subSituations " + story.stringApiValue() + " ?SUB) " +
                        "(#$storyReferences ?SUB " + ((CycConstant)result.second()).stringApiValue() + ")))" ), cyc.getKnownConstantByName("ScriptBotBeliefsMt") ) )
                {
                    /*--------------------------------------------------------------------------
                     * Hypothesize the state
                     *-------------------------------------------------------------------------*/
                    CycList states = (CycList) cyc.converseList("(fi-hypothesize '"+
                        cyc.makeCycList( "(#$and (#$isa ?STATE #$CD-STATE)" +
                        "(#$subSituations " + story.stringApiValue() + " ?STATE)" + 
                        "(#$storyReferences ?STATE #$" + result.second().toString()+ ")" + 
                        "(#$cd-r #$" + result.first().toString() + " ?STATE)" +
                        ")" ).stringApiValue() + " " + cyc.getKnownConstantByName("ScriptBotBeliefsMt").stringApiValue() + ")" );
                    // was the hypothesize succesful?
    ...
    //                    sendMessage(returnpath, "(" + token + " is a primitive act in " + mtForUser(cyclist).toString() + ")" );
            ...
            if( expected.equals("continueornot") )  {
                if( token.equals( "finish" ) ) {
                    ...
                    // hypothesize the story.
                    CycList results = (CycList) cyc.converseList("(fi-hypothesize '"+
                        toCycListString(storyFromUser(cyclist).toString()) + " " +
                        mtForUser(cyclist).stringApiValue() + ")" );
                    // was the hypothesize succesful?
