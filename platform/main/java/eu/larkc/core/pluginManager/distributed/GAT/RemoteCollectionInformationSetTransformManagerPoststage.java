 ...
 */
public class RemoteCollectionInformationSetTransformManagerPoststage extends RemoteMPIPluginManager <String, Collection<InformationSet>> {
	 ...
	 */
	public RemoteCollectionInformationSetTransformManagerPoststage(InformationSetTransformer transformer, Queue<String> inputQueue, Queue<Collection<InformationSet>> outputQueue, GATResource resource, String outputID) {
		...
		public void run() {
			for (;;) {
				...
				if (controlMessage.equals(PluginManager.Message.NEXT)) {
					...
					result = doPoststage();	
					Collection<InformationSet> transformedResources = (Collection<InformationSet>) result.get(0);
