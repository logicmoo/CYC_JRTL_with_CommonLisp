...
public abstract class ReferenceCache<K, V> {
	...
	public ReferenceCache(Type keyType, Type valueType, int initialSize) {
		...
		t.setName(getClass().getName() + " " + keyType + " " + valueType);
		t.setDaemon(true);
		t.start();
	}
	...
	* @param <T> the type of the reference value */
	private class ReferenceQueueMonitor<T> extends ReferenceQueue<T> implements Runnable {
		...
		public void run() {
			for (;;)
				try {
					...
					/* ignore try again */ System.out.println(e + " ooops");
				}
		}
	}
}
