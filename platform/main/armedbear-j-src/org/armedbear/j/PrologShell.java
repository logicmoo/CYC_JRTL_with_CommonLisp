...
//		setResetCommand(":reset");
	...
	private static Shell createPrologShell(String shellCommand, String title, boolean startSlime)
	{
	...
	public void enter()
	{
		...
		Position end = getEnd();
		Position pos = null;// JavaScriptMode.findContainingSexp(end);
	...
	public void electricCloseParen()
	{
		...
		if (needsRenumbering) renumber();
		final Position endOfOutput = getEndOfOutput();
		if (endOfOutput == null) return;
		if (dot.isBefore(endOfOutput)) return;
		final Line promptLine = endOfOutput.getLine();
		final boolean atPrompt = (promptRE.getMatch(promptLine.getText()) != null);
		Position end = getEnd();
		if (dot.isBefore(end)) return;
		Position pos = null; //JavaScriptMode.findContainingSexp(end);
		boolean isComplete = (pos == null || pos.isBefore(endOfOutput));
//		if (isComplete)
//		{
...
//		}
	...
	protected void stdOutUpdate(final String s)
	{
		...
		Runnable r = new Runnable()
		{
			...
			{
				Position pos = getEnd();
				if (pos != null) pos.getLine().setFlags(0); // This value will propagate.
				if (output.length() > 0)
				{
					appendString(output);
					if (match != null)
		...
		try
		{
			if (slime)
			{
				...
				if (pos != null)
				{
					...
					if (needsRenumbering()) renumber();
					enforceOutputLimit(Property.SHELL_OUTPUT_LIMIT);
					if (pos != posBeforeLastPrompt) setEndOfOutput(pos.copy());
				}
				else
				{
					// empty buffer
	...
	private void indentLineAtDot(Editor editor)
	{
		...
		try
		{
			...
			else
			{
				...
				if (getBooleanProperty(Property.RESTRICT_CARET)) editor.fillToCaret();
			}
			resetUndo(); // Why?
		} finally
		{
	...
	private void sendInputToProlog(String input)
	{
		...
		for (EditorIterator it = new EditorIterator(); it.hasNext();)
		{
			...
			if (ed.getBuffer() == this) ed.setWaitCursor();
		}
	}
	...
	private static void prolog(String shellCommand, String title, boolean requireJpty, boolean startSlime)
	{
		...
		if (Platform.isPlatformWindows()) if (!Platform.isPlatformWindows5()) return;
		final Editor editor = Editor.currentEditor();
	...
	public static CommandInterpreter findProlog(String title)
	{
		for (BufferIterator it = new BufferIterator(); it.hasNext();)
		{
			...
			if (b instanceof CommandInterpreter)
			{
				...
				if (comint.isProlog())
				{
					if (title == null || title.equals(comint.getTitle())) return comint;
				}
			}
		}
		return null;
