    ...
    if (startSlime)
      {
        ...
        if (shellCommand.indexOf("abcl") >= 0 ||
          ...
          {
            File lispHome = File.getInstance(Site.getLispHome().STRING().getString());
            if (lispHome == null)
    ...
    if (shellCommand.indexOf("alisp") >= 0)
    ...
    else if (shellCommand.indexOf("xcl") >= 0)
      {
        ...
        lisp.setResetCommand("(ext:reset)");
             ...
             shellCommand.indexOf("lx86cl64") >= 0)
      {
...
public String toString()
  ...
  {
    final String defaultLispShellCommand = getDefaultLispShellCommand();
	_slime(defaultLispShellCommand, "slime abcl", false);
  }
  ...
  static int lispCount = 0;
  public static void lisp()
  {
    String defaultLispShellCommand = getDefaultLispShellCommand();
    lispCount++;
	lisp(defaultLispShellCommand, "abcl.0"+lispCount, false, false);
  }
            ...
            if (vendor.indexOf("Sun") >= 0 || vendor.indexOf("Blackdown") >= 0)
              {
                ...
                if (Platform.isPlatformUnix())
                  {
                    String lispHome = org.armedbear.lisp.Site.getLispHome().getStringValue();
                    if (lispHome != null)
                      {
