...
public final class Editor extends JPanel implements Constants, ComponentListener, MouseWheelListener {
	...
	public static void main(String[] argsIn) {
		...
		if (argsIn == null || argsIn.length == 0) {
			argsIn = new String[] { "--gui" };
		}
		String[] args = Startup.extractOptions(Editor.class, argsIn);
			...
			if (arg.startsWith("-")) {
				...
				if (arg.startsWith("--home")) {
					...
					if (userHomeDir == null || !userHomeDir.isDirectory()) {
						fatal(mayExit, "Specified home directory \"" + userHomeDir.canonicalPath() + "\" does not exist.");
					}

					if (!userHomeDir.canWrite()) {
						fatal(mayExit, "Specified home directory \"" + userHomeDir.canonicalPath() + "\" is not writable.");
					}
	...
	public void closeParen() {
		insertNormalChar(')');
		if (prefs.getBooleanProperty(Property.HIGHLIGHT_MATCHING_BRACKET) || prefs.getBooleanProperty(Property.HIGHLIGHT_BRACKETS))
			return;
	...
	public void insertNormalChar(char c) {
		...
		try {
			...
			if (mark != null) {
				...
				// No selection.
				if (buffer.getBooleanProperty(Property.WRAP) && getDotCol() >= buffer.getIntegerProperty(Property.WRAP_COL)) {
					CompoundEdit compoundEdit = beginCompoundEdit();
					insertChar(c);
	...
	public Object[] getKeyMapping(String command) {
		...
		if (mapping == null) {
			...
			if (mapping != null) {
				javax.swing.KeyStroke keyStroke = javax.swing.KeyStroke.getKeyStroke(mapping.getKeyCode(), mapping.getModifiers());
	...
	public void eol() {
		if (dot == null)
			return;
		if (mark != null || dot.getOffset() != dot.getLineLength() || buffer.getCol(dot) != display.getCaretCol() + display.getShift())
	...
	private void pasteInternal(String toBeInserted, boolean leavePasteSelected) {
		final Mode mode = buffer.getMode();
		CompoundEdit compoundEdit = beginCompoundEdit();
		if (mark == null && Utilities.isLinePaste(toBeInserted) && mode.acceptsLinePaste(this) && buffer.getBooleanProperty(Property.AUTO_PASTE_LINES)) {
	...
	public void dirDeleteFiles() {
		if (mark != null && getMarkLine() != getDotLine()) {
			MessageDialog.showMessageDialog(this, "This operation is not supported with multi-line text selections.", "Delete Files");
			return;
		}
		if (buffer instanceof Directory) {
	...
	public void updateHorizontalScrollBar() {
		if (horizontalScrollBar != null)
			horizontalScrollBar.setValues(display.getShift() * Display.getCharWidth(), display.getWidth(), 0, buffer.getDisplayWidth());
	}
