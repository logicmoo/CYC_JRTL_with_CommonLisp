...
public class TestJUnit extends TestCase {
	...
	public static final String home = (System.getenv("SWI_HOME_DIR") == null ? "../.." : System.getenv("SWI_HOME_DIR"));
	 ...
	 */
	private static void initForJunit() {
		if (Main.noPrologJNI)
			return;
		if (syntax.equals("traditional")) {
			JPL.setTraditional();
			Prolog.set_default_init_args(new String[] { "libswipl.dll", "-x", startup, "-f", "none", "-g", "true", "--traditional", "-q", "--home=" + home, "--nosignals" });
		} else {
			Prolog.set_default_init_args(new String[] { "libswipl.dll", "-x", startup, "-f", "none", "-g", "true", "-q", "--home=" + home, "--nosignals" });
		}
	}
	...
	//	public void testInfo() { 		if (Main.noPrologJNI) return;
	...
	public void testEmptyParentheses() {
		...
		Term t = Query.oneSolution("T = a()").get("T"); // valid in both
														...
														// Prolog 7.x
		assertTrue("T is not bound to an atom", t.isAtomSymbol());
	...
	// public void testCompoundZeroArity3() { 		if (Main.noPrologJNI) return;
	// Term t = Query.oneSolution("T = foo()").get("T");
	// assertTrue("term is a compound", t.isCompound());
	// assertFalse("term is an atom", t.isAtomSymbol());
	// }
	...
	public void testListNil1() {
		...
		if (syntax.equals("traditional")) {
			assertTrue("empty list is text atom []", x.isAtomSymbol() && x.atomType().equals("text") && x.name().equals("[]"));
		} else {
			assertTrue("empty list is reserved atom []", x.isAtomSymbol() && x.atomType().equals("reserved_symbol") && x.name().equals("[]"));
		}
	}
	...
	public void testGenerate1() {
		if (Main.noPrologJNI)
			return; // we chickened out of verifying each solution
		// :-)
	...
	// public void testFetchLongList2a() { 		if (Main.noPrologJNI) return; /* leads to stack overflow */
	// assertTrue((new
	// Query("findall(foo(N),between(0,2000,N),L)")).hasSolution());
	// }
	// public void testFetchLongList2b() { 		if (Main.noPrologJNI) return;
	// assertTrue((new
	// Query("findall(foo(N),between(0,3000,N),L)")).hasSolution());
	// }
	// public void testFetchLongList3() { 		if (Main.noPrologJNI) return;
	// assertTrue((new
	...
	// public void testMaxInteger1() { 		if (Main.noPrologJNI) return;
	...
	// public void testSingleton1() { 		if (Main.noPrologJNI) return;
	...
	public void testUtilListToTermArray1() {
		...
		assertTrue(array[2].isAtomSymbol() && array[2].name().equals("c"));
	}
	...
	public void testWouter1() {
		if (Main.noPrologJNI)
			return; // Wouter says this fails under OS X Mavericks
		// 10.9 x86-64
