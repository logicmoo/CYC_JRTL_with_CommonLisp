...
public final class export extends SubLTranslatedFile {
    ...
    public static SubLObject prolog_need_quote(SubLObject str) {
        if ((!(Sequences
                .substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen, str, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED)
                .equal(str)))) {
            return T;
        }
        {
            ...
            return makeBoolean(((NIL == Characters.lower_case_p(ch)) || (NIL != Sequences
                    .find(Characters.CHAR_colon, str, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED))));
        }
    }
        ...
        } else {
            {
                ...
                if ((NIL != Functions.funcall(makeFunctionSymbol("FUNCTION?"), pred))) {
                    if ((NIL != Functions.funcall(makeFunctionSymbol("ISA?"), pred, $const32$UnreifiableFunction))) {
                        ...
                        plw_argz(va, cdrtrm);
                        plw_str($str30$_);
                    } else if ((NIL != Functions
                            .funcall(makeFunctionSymbol("ISA?"), pred, $const34$ReifiableFunction))) {
                        plw_str(str);
                        plw_str($str35$R_);
                        plw_term(pred, UNPROVIDED);
                        plw_argz(va, cdrtrm);
    ...
    public static SubLObject assertion_varnames(SubLObject assrt) {
        {
            ...
            for (v = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                ...
                varnames = ConsesLow.append(varnames, list(Sequences
                        .substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen, Functions
                                .funcall(makeFunctionSymbol("EL-VAR-NAME-WITHOUT-PREFIX"), v), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED)));
            }
            return varnames;
        }
    }
    ...
    public static SubLObject plw_string(SubLObject trm) {
        if ((!(((NIL != Sequences
                .search($str45$_, trm, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED))
                && THREE_INTEGER.numL(Sequences.length(trm)))))) {
    ...
    public static SubLObject unescape(SubLObject str) {
        str = Sequences.substitute(Characters.CHAR_vertical, Characters.CHAR_return, Sequences
                .substitute(Characters.CHAR_vertical, Characters.CHAR_newline, str, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
    ...
    public static SubLObject plw_term(SubLObject trm, SubLObject is_pred) {
        ...
        }
        {
            ...
            if ((NIL != Functions.funcall(makeFunctionSymbol("NART-P"), trm))) {
                ...
                plw_term(ConsesLow.nth(Functions.funcall(makeFunctionSymbol("VARIABLE-ID"), trm), Functions
                        .funcall(makeFunctionSymbol("ASSERTION-EL-VARIABLES"), $a$
                                .getDynamicValue(thread))), UNPROVIDED);
            } else if ((NIL != Functions.funcall(makeFunctionSymbol("EL-VARIABLE-P"), trm))) {
                plw_str(Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen, Functions
                        .funcall(makeFunctionSymbol("EL-VAR-NAME-WITHOUT-PREFIX"), trm), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED));
    ...
    public static SubLObject pre_export() {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject cdolist_list_var = Functions
                        .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const63$Quantifier);
                SubLObject s = NIL;
                for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                        .rest(), s = cdolist_list_var.first()) {
                    {
                        ...
                        if ((NIL == conses_high.member(item_var, $no_tify$.getDynamicValue(thread), Symbols
            ...
            {
                SubLObject cdolist_list_var = Functions
                        .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const64$HLPredicate);
                SubLObject s = NIL;
                for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                        .rest(), s = cdolist_list_var.first()) {
                    {
            ...
            {
                SubLObject cdolist_list_var = Functions
                        .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const65$MicrotheoryDesignatingPredicate);
                SubLObject s = NIL;
                for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                        .rest(), s = cdolist_list_var.first()) {
                    {
            ...
            {
                SubLObject cdolist_list_var = Functions
                        .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const66$LogicalConnective);
                SubLObject s = NIL;
                for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                        .rest(), s = cdolist_list_var.first()) {
                    {
            ...
            {
                SubLObject cdolist_list_var = Functions
                        .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const67$KBDependentRelation);
                SubLObject s = NIL;
                for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                        .rest(), s = cdolist_list_var.first()) {
                    {
            ...
            {
                SubLObject cdolist_list_var = Functions
                        .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const68$ModalPredicate);
                SubLObject s = NIL;
                for (s = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                        .rest(), s = cdolist_list_var.first()) {
                    {
            ...
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                ...
                SubLObject found = NIL;
                {
                    ...
                    try {
                        ...
                        $assrtform$.setDynamicValue(Functions
                                .funcall(makeFunctionSymbol("ASSERTION-FORMULA"), assrt), thread);
                        if ((NIL != $assrtform$.getDynamicValue(thread))) {
                            if ((!($assrtform$.getDynamicValue(thread).first().equal($const70$termOfUnit)))) {
                                ...
                                if ((NIL != elvars)) {
                                    ...
                                    plw_str($assrtid$.getDynamicValue(thread));
                                    plw_str($str77$__);
                                    plw_list(assertion_varnames($a$
                                            .getDynamicValue(thread)), UNPROVIDED, UNPROVIDED, UNPROVIDED);
                                    plw_str($str78$___);
                                    plw_nl();
                                }
                                plw_ruleinfo(((NIL != Functions.funcall(makeFunctionSymbol("RULE-ASSERTION?"), assrt))
                                        ? ((SubLObject) $str79$rule)
                                        : $str80$fact), UNPROVIDED);
                                plw_ruleinfo(Strings.string_downcase(Symbols.symbol_name(Functions
                                        .funcall(makeFunctionSymbol("ASSERTION-STRENGTH"), assrt)), UNPROVIDED, UNPROVIDED), UNPROVIDED);
                                plw_ruleinfo(Strings.string_downcase(Symbols.symbol_name(Functions
                                        .funcall(makeFunctionSymbol("ASSERTION-DIRECTION"), assrt)), UNPROVIDED, UNPROVIDED), UNPROVIDED);
    ...
    public static SubLObject assertion_variable_guard(SubLObject assrt) {
        ...
        return Functions.funcall(makeFunctionSymbol("FORMULA-VARIABLES-ARG-CONSTRAINTS"), Functions
                .funcall(makeFunctionSymbol("ASSERTION-FORMULA"), assrt), Functions
                        .funcall(makeFunctionSymbol("ASSERTION-MT"), assrt));
    }
    ...
    public static SubLObject no_errors(SubLObject macroform, SubLObject environment) {
        {
            ...
            current = current.rest();
            {
                ...
                return list($sym87$CLET, $list88, list($sym89$WITH_ERROR_HANDLER, $list90, list($sym91$CSETQ, $sym92$LASTVAL, listS($sym93$PROGN, $list94, $list95, ConsesLow
                        .append(code, NIL)))), listS($sym96$FIF, $sym97$HADERROR, list($sym93$PROGN, $list98, list($sym99$QUOTE, err)), $list100));
            }
        }
    }
    ...
    public static SubLObject plw_support(SubLObject d) {
        plw_term(list(Functions.funcall(makeFunctionSymbol("SUPPORT-MT"), d), Functions
                .funcall(makeFunctionSymbol("SUPPORT-SENTENCE"), d), Symbols
                        .symbol_name(Functions.funcall(makeFunctionSymbol("SUPPORT-TRUTH"), d)), Symbols
                                .symbol_name(Functions.funcall(makeFunctionSymbol("SUPPORT-STRENGTH"), d)), Symbols
                                        .symbol_name(Functions
            ...
            $a$.setDynamicValue(Functions.funcall(makeFunctionSymbol("DEDUCTION-ASSERTION"), ded), thread);
            {
                ...
                for (d = cdolist_list_var.first(); (NIL != cdolist_list_var); cdolist_list_var = cdolist_list_var
                    ...
                    if ((NIL != Functions.funcall(makeFunctionSymbol("ASSERTION-P"), $a$.getDynamicValue(thread)))) {
                        plw_id($str71$a, Functions
                                .funcall(makeFunctionSymbol("ASSERTION-ID"), $a$.getDynamicValue(thread)));
                    } else {
            ...
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                ...
                print_high.princ($str134$_____style_check__discontiguous__, file_output);
                {
                    ...
                    try {
                        ...
                        try {
                            Functions
                                    .funcall(makeFunctionSymbol("NOTING-PERCENT-PROGRESS-PREAMBLE"), $str135$exporting_KB___);
                            {
                                //$int136$415542
                                ...
                                ; (!(anum.numE(total))); anum = anum.inc()) {
                                    {
                                        SubLObject assrt = Functions
                                                .funcall(makeFunctionSymbol("FIND-ASSERTION-BY-ID"), anum);
                                        try {
    ...
    public static SubLObject et() {
        $no_tify$ = defvar("*NO-TIFY*", Functions
                .funcall(makeFunctionSymbol("ALL-INSTANCES"), $const62$WFFConstraintPredicate));
        pre_export();
                        ...
                        if ((NIL == found)) {
                            {
                                ...
                                if ((NIL != prolog_need_quote(name))) {
                                    PrintLow.format(T, $str140$_safely_rename_or_merge__S__S____, name, Sequences
                                            .cconcatenate($str143$cyc, name));
                                }
                            }
                        }
                    }
    ...
    public static SubLObject simplrename(SubLObject found) {
        {
            SubLObject nfound = Sequences.substitute(Characters.CHAR_underbar, Characters.CHAR_hyphen, Sequences
                    .substitute(Characters.CHAR_underbar, Characters.CHAR_colon, found, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
            ...
            if ((ch.eql(Characters.CHAR_t) && (NIL != Characters.lower_case_p(ch1)))) {
                return Sequences.cconcatenate($str145$t, new SubLObject[] {
                        Strings.string_upcase(Strings.string(ch1), UNPROVIDED, UNPROVIDED),
