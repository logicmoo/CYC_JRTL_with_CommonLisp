...
public class CycServer extends Server implements Serializable
{
	...
	{
		super(hostName, (basePort < 100) ? CycConnection.DEFAULT_BASE_PORT + basePort : basePort);
	}
	...
	public static boolean isValidString(String string)
	{
		if (Server.isValidString(string))
		{
			...
			if (server.getPort() == null) { return false; }
	...
	{
		final int defaultPort = CycConnection.DEFAULT_BASE_PORT;
		
		if ((port >= defaultPort) && (port <= (defaultPort + 99)))
		{
			...
			if ((offset == 0) || isAPortOffset(offset)) { return defaultPort + image; }
		}
		return null;
	}
	...
	{
		return (offset == CYC_ASCII_PORT_OFFSET) || (offset == CYC_HTTP_PORT_OFFSET) || (offset == CYC_SERVLET_PORT_OFFSET) || (offset == CYC_CFASL_PORT_OFFSET);
	}
