...
public final class rkf_concept_summarizer
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject rkf_summarize(final SubLObject fort_or_naut, SubLObject mt, SubLObject include_nartsP)
  {
    ...
    if( NIL != forts.fort_p( fort_or_naut ) )
    {
      ...
      view_formulas = Sequences.delete_duplicates( view_formulas, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
    ...
    view_formulas2 = Sequences.delete_duplicates( view_formulas2, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject rkf_summarize_fort_view(final SubLObject fort, SubLObject mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        fort_view_formulas = ConsesLow.cons( asserted_term_formula, fort_view_formulas );
      }
  ...
  public static SubLObject rkf_summarize_function_view(final SubLObject fort, SubLObject mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$1)
      {
        ...
        {
          function_view_formulas = ConsesLow.cons( nart_fort_view_formula, function_view_formulas );
        }
  ...
  public static SubLObject rkf_asserted_term_formulas(final SubLObject fort, SubLObject mt)
  {
    ...
    try
    {
      mt_relevance_macros.$relevant_mt_function$.bind( $sym1$RELEVANT_MT_IS_GENL_MT, thread );
          ...
          if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( fort, arg_key, pred_var ) )
          {
            ...
            while ( NIL == done_var)
            {
              ...
              if( NIL != valid )
              {
                ...
                try
                {
                  final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw2$GAF, $kw3$TRUE, NIL );
                  ...
                  while ( NIL == done_var_$3)
                  {
                    ...
                    if( NIL != valid_$5 )
                    {
                      ...
                      try
                      {
                        ...
                        final SubLObject formula = conses_high.copy_tree( fi.assertion_fi_formula( assertion, UNPROVIDED ) );
                        asserted_term_formulas = ConsesLow.cons( formula, asserted_term_formulas );
                      }
                      finally
                      {
  ...
  public static SubLObject rkf_taxonomic_term_formula(final SubLObject fort, final SubLObject formula, SubLObject mt)
  {
    ...
    SubLObject predicate = NIL;
    SubLObject args = NIL;
    cdestructuring_bind.destructuring_bind_must_consp( formula, formula, $list4 );
    predicate = formula.first();
    final SubLObject current = args = formula.rest();
    final SubLObject pcase_var = predicate;
    if( pcase_var.eql( $const5$isa ) )
    {
      return T;
    }
    if( pcase_var.eql( $const6$genls ) )
    {
      return T;
    }
    return NIL;
  ...
  {
    return el_utilities.formula_arity( cycl_utilities.formula_arg2( formula, UNPROVIDED ), UNPROVIDED );
  }
  ...
  public static SubLObject rkf_sort_view_formulas(final SubLObject v_term, final SubLObject formulas, SubLObject istP)
  {
    ...
    SubLObject formulas_not_directly_about_term = Sequences.remove( v_term, formulas, Symbols.symbol_function( $sym7$FIND_EQUAL ), ( NIL != istP ) ? $sym8$SENTENCE_ARG2 : IDENTITY, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    SubLObject formulas_directly_about_term = conses_high.set_difference( formulas, formulas_not_directly_about_term, UNPROVIDED, UNPROVIDED );
    formulas_directly_about_term = list_utilities.sort_via_position( formulas_directly_about_term, $list9, Symbols.symbol_function( EQL ), ( NIL != istP ) ? $sym10$ARITY_OF_ARG2 : $sym11$FORMULA_ARITY );
    formulas_directly_about_term = list_utilities.stable_sort_via_position( formulas_directly_about_term, $list12, Symbols.symbol_function( EQ ), ( NIL != istP ) ? $sym13$OPERATOR_OF_ARG2 : $sym14$FORMULA_OPERATOR );
    formulas_not_directly_about_term = list_utilities.sort_via_position( formulas_not_directly_about_term, $list9, Symbols.symbol_function( EQL ), ( NIL != istP ) ? $sym10$ARITY_OF_ARG2 : $sym11$FORMULA_ARITY );
  ...
  public static SubLObject rkf_summarize_naut_view(final SubLObject naut, SubLObject mt)
  {
    ...
    SubLObject cdolist_list_var;
    SubLObject queries = cdolist_list_var = ConsesLow.list( ConsesLow.listS( $const5$isa, naut, $list15 ), ConsesLow.list( $const5$isa, $sym16$_ARG1, naut ) );
    ...
    if( NIL != rkf_query_utilities.rkf_known( ConsesLow.listS( $const5$isa, naut, $list17 ), mt, UNPROVIDED ) )
    {
      queries = ( cdolist_list_var = ConsesLow.list( ConsesLow.listS( $const18$nearestGenls, naut, $list15 ), ConsesLow.list( $const18$nearestGenls, $sym16$_ARG1, naut ) ) );
      query = NIL;
      query = cdolist_list_var.first();
      while ( NIL != cdolist_list_var)
      {
        ...
        if( NIL != binding_lists )
        {
          ...
          {
            formulae = ConsesLow.cons( cycl_utilities.expression_nsubst( $const6$genls, $const18$nearestGenls, formula, UNPROVIDED, UNPROVIDED ), formulae );
  ...
  public static SubLObject rkf_instantiate_formula(final SubLObject formula, final SubLObject binding_lists)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      formulae = ConsesLow.cons( instance, formulae );
      cdolist_list_var = cdolist_list_var.rest();
      binding_list = cdolist_list_var.first();
    }
    return Sequences.nreverse( formulae );
  ...
  {
    return Sequences.find( v_term, list, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject rkf_summarize_all(final SubLObject fort, SubLObject mt)
  {
    ...
    SubLObject all_view_formulas = ConsesLow.list( ConsesLow.list( fort, local_view_formulas ) );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        all_view_formulas = ConsesLow.cons( ConsesLow.list( inherited_view, inherited_view_formulas ), all_view_formulas );
      }
      cdolist_list_var = cdolist_list_var.rest();
      inherited_view = cdolist_list_var.first();
    }
  ...
  public static SubLObject rkf_inherited_views(final SubLObject fort, SubLObject mt)
  {
    ...
    {
      views = Sequences.delete( fort, genls.all_genls( fort, mt, UNPROVIDED ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    else
    {
    ...
    views = Sequences.delete( $rkf_view_generality_cutoff$.getDynamicValue( thread ), views, Symbols.symbol_function( $sym21$_ ), Symbols.symbol_function( $sym22$GENERALITY_ESTIMATE ), UNPROVIDED, UNPROVIDED,
        UNPROVIDED );
    views = Sort.sort( views, Symbols.symbol_function( $sym21$_ ), Symbols.symbol_function( $sym22$GENERALITY_ESTIMATE ) );
    return views;
  }
  ...
  public static SubLObject rkf_summarize_inherited_view(final SubLObject fort, final SubLObject view, SubLObject mt)
  {
    ...
    try
    {
      ...
      view_formulas = ask_utilities.ask_template( $sym23$_FORMULA, ConsesLow.listS( $const24$inheritedTermSentences, fort, view, $list25 ), mt, ONE_INTEGER, NIL, NIL, NIL );
    }
    finally
    {
  ...
  {
    final SubLObject first_try = constants_high.find_constant( $str26$exampleSentences );
    if( NIL != first_try )
    {
      return first_try;
    }
    final SubLObject second_try = constants_high.find_constant( $str27$exampleFormulas );
    if( NIL != second_try )
    {
      return second_try;
    }
  ...
  public static SubLObject rkf_term_examples(final SubLObject fort, SubLObject number, SubLObject mt)
  {
    ...
    try
    {
      ...
      control_vars.$unique_inference_result_bindings$.bind( T, thread );
      api_control_vars.$generate_readable_fi_results$.bind( NIL, thread );
      examples = ask_utilities.ask_template( $sym23$_FORMULA, ConsesLow.listS( rkf_example_predicate(), fort, $list25 ), mt, NIL, number, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
  ...
  public static SubLObject rkf_unknown_term_formulas(final SubLObject fort, final SubLObject indexical, SubLObject mt)
  {
    ...
    formulas = conses_high.subst( indexical, fort, formulas, Symbols.symbol_function( EQUAL ), UNPROVIDED );
    return formulas;
  }
  ...
  public static SubLObject rkf_asserted_sentences(final SubLObject fort_or_naut, final SubLObject domain_mt)
  {
    ...
    sentences = list_utilities.fast_delete_duplicates( sentences, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject rkf_ist_sentences_for_fort(final SubLObject fort, final SubLObject domain_mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == rkf_relevance_utilities.rkf_irrelevant_term_formula( fort, hl_sentence, domain_mt ) || NIL == rkf_relevance_utilities.rkf_irrelevant_term_formula( fort, el_sentence, domain_mt ) )
      {
        ...
        sentences = ConsesLow.cons( ConsesLow.list( $const28$ist, mt, el_sentence ), sentences );
      }
      cdolist_list_var = cdolist_list_var.rest();
      assertion = cdolist_list_var.first();
    }
  ...
  public static SubLObject rkf_ist_sentences_for_naut(final SubLObject naut, final SubLObject domain_mt)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      result = ConsesLow.cons( ConsesLow.list( $const28$ist, domain_mt, sentence ), result );
      cdolist_list_var = cdolist_list_var.rest();
      sentence = cdolist_list_var.first();
    }
  ...
  {
    SubLFiles.declareFunction( me, "rkf_summarize", "RKF-SUMMARIZE", 1, 2, false );
    SubLFiles.declareFunction( me, "rkf_summarize_fort_view", "RKF-SUMMARIZE-FORT-VIEW", 1, 1, false );
    SubLFiles.declareFunction( me, "rkf_summarize_function_view", "RKF-SUMMARIZE-FUNCTION-VIEW", 1, 1, false );
    SubLFiles.declareFunction( me, "rkf_asserted_term_formulas", "RKF-ASSERTED-TERM-FORMULAS", 1, 1, false );
    SubLFiles.declareFunction( me, "rkf_taxonomic_term_formula", "RKF-TAXONOMIC-TERM-FORMULA", 2, 1, false );
    SubLFiles.declareFunction( me, "arity_of_arg2", "ARITY-OF-ARG2", 1, 0, false );
    SubLFiles.declareFunction( me, "operator_of_arg2", "OPERATOR-OF-ARG2", 1, 0, false );
    SubLFiles.declareFunction( me, "rkf_sort_view_formulas", "RKF-SORT-VIEW-FORMULAS", 2, 1, false );
    SubLFiles.declareFunction( me, "rkf_summarize_naut_view", "RKF-SUMMARIZE-NAUT-VIEW", 1, 1, false );
    SubLFiles.declareFunction( me, "rkf_instantiate_formula", "RKF-INSTANTIATE-FORMULA", 2, 0, false );
    SubLFiles.declareFunction( me, "find_equal", "FIND-EQUAL", 2, 0, false );
    SubLFiles.declareFunction( me, "rkf_summarize_all", "RKF-SUMMARIZE-ALL", 1, 1, false );
    SubLFiles.declareFunction( me, "rkf_inherited_views", "RKF-INHERITED-VIEWS", 1, 1, false );
    SubLFiles.declareFunction( me, "rkf_summarize_inherited_view", "RKF-SUMMARIZE-INHERITED-VIEW", 2, 1, false );
    SubLFiles.declareFunction( me, "rkf_example_predicate", "RKF-EXAMPLE-PREDICATE", 0, 0, false );
    SubLFiles.declareFunction( me, "rkf_term_examples", "RKF-TERM-EXAMPLES", 1, 2, false );
    SubLFiles.declareFunction( me, "rkf_unknown_term_formulas", "RKF-UNKNOWN-TERM-FORMULAS", 2, 1, false );
    SubLFiles.declareFunction( me, "rkf_asserted_sentences", "RKF-ASSERTED-SENTENCES", 2, 0, false );
    SubLFiles.declareFunction( me, "rkf_asserted_sentences_for_fort", "RKF-ASSERTED-SENTENCES-FOR-FORT", 2, 0, false );
    SubLFiles.declareFunction( me, "rkf_asserted_sentences_for_naut", "RKF-ASSERTED-SENTENCES-FOR-NAUT", 2, 0, false );
    SubLFiles.declareFunction( me, "rkf_ist_sentences", "RKF-IST-SENTENCES", 2, 0, false );
    SubLFiles.declareFunction( me, "rkf_ist_sentences_for_fort", "RKF-IST-SENTENCES-FOR-FORT", 2, 0, false );
    SubLFiles.declareFunction( me, "rkf_ist_sentences_for_naut", "RKF-IST-SENTENCES-FOR-NAUT", 2, 0, false );
    return NIL;
  }

  public static SubLObject init_rkf_concept_summarizer_file()
  {
    $rkf_view_generality_cutoff$ = SubLFiles.defparameter( "*RKF-VIEW-GENERALITY-CUTOFF*", $int20$100000 );
    return NIL;
  }
  ...
  static
  {
    me = new rkf_concept_summarizer();
    $rkf_view_generality_cutoff$ = null;
    $sym0$FORT_P = makeSymbol( "FORT-P" );
    $sym1$RELEVANT_MT_IS_GENL_MT = makeSymbol( "RELEVANT-MT-IS-GENL-MT" );
    $kw2$GAF = makeKeyword( "GAF" );
    $kw3$TRUE = makeKeyword( "TRUE" );
    $list4 = ConsesLow.cons( makeSymbol( "PREDICATE" ), makeSymbol( "ARGS" ) );
    ...
    $sym7$FIND_EQUAL = makeSymbol( "FIND-EQUAL" );
    $sym8$SENTENCE_ARG2 = makeSymbol( "SENTENCE-ARG2" );
    $list9 = ConsesLow.list( TWO_INTEGER, THREE_INTEGER, FOUR_INTEGER, ONE_INTEGER );
    $sym10$ARITY_OF_ARG2 = makeSymbol( "ARITY-OF-ARG2" );
    $sym11$FORMULA_ARITY = makeSymbol( "FORMULA-ARITY" );
    ...
    $sym13$OPERATOR_OF_ARG2 = makeSymbol( "OPERATOR-OF-ARG2" );
    $sym14$FORMULA_OPERATOR = makeSymbol( "FORMULA-OPERATOR" );
    $list15 = ConsesLow.list( makeSymbol( "?ARG2" ) );
    $sym16$_ARG1 = makeSymbol( "?ARG1" );
    $list17 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "Collection" ) ) );
    $const18$nearestGenls = constant_handles.reader_make_constant_shell( makeString( "nearestGenls" ) );
    $list19 = ConsesLow.cons( makeSymbol( "VAR" ), makeSymbol( "VALUE" ) );
    $int20$100000 = makeInteger( 100000 );
    $sym21$_ = makeSymbol( "<" );
    $sym22$GENERALITY_ESTIMATE = makeSymbol( "GENERALITY-ESTIMATE" );
    $sym23$_FORMULA = makeSymbol( "?FORMULA" );
    $const24$inheritedTermSentences = constant_handles.reader_make_constant_shell( makeString( "inheritedTermSentences" ) );
    $list25 = ConsesLow.list( makeSymbol( "?FORMULA" ) );
    $str26$exampleSentences = makeString( "exampleSentences" );
    $str27$exampleFormulas = makeString( "exampleFormulas" );
    $const28$ist = constant_handles.reader_make_constant_shell( makeString( "ist" ) );
  }
}
/*
 * 
