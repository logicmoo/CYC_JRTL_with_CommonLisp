      ...
      SubLTranslatedFile
{
  ...
  {
    Functions.funcall( visitor_fn, obj, $kw19$BEGIN, $sym20$MAKE_REMOVAL_LINK_DATA, THREE_INTEGER );
    Functions.funcall( visitor_fn, obj, $kw21$SLOT, $kw15$HL_MODULE, remov_link_data_hl_module( obj ) );
    Functions.funcall( visitor_fn, obj, $kw21$SLOT, $kw16$BINDINGS, remov_link_data_bindings( obj ) );
    Functions.funcall( visitor_fn, obj, $kw21$SLOT, $kw17$SUPPORTS, remov_link_data_supports( obj ) );
    Functions.funcall( visitor_fn, obj, $kw22$END, $sym20$MAKE_REMOVAL_LINK_DATA, THREE_INTEGER );
    return obj;
  }
  ...
  {
    final SubLObject removal_link = inference_datastructures_problem_link.new_problem_link( $kw25$REMOVAL, problem );
  ...
  public static SubLObject destroy_removal_link(final SubLObject removal_link)
  {
    ...
    _csetf_remov_link_data_hl_module( data, $kw26$FREE );
    _csetf_remov_link_data_bindings( data, $kw26$FREE );
    _csetf_remov_link_data_supports( data, $kw26$FREE );
    return removal_link;
  }
  ...
  public static SubLObject removal_link_tactic(final SubLObject removal_link)
  {
    ...
    if( NIL != inference_modules.removal_module_p( hl_module ) )
    {
      ...
      {
        if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( candidate_tactic, $kw25$REMOVAL ) && inference_datastructures_tactic.tactic_hl_module( candidate_tactic ).eql( hl_module ) )
        {
      ...
      if( NIL != inference_datastructures_problem_store.problem_store_add_restriction_layer_of_indirectionP( store ) && NIL != inference_datastructures_problem.closed_problem_p( problem ) )
      {
            ...
            .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
        {
          ...
          if( NIL != set_contents.do_set_contents_element_validP( state, restriction_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( restriction_link, $kw31$RESTRICTION ) )
          {
            ...
            {
              if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( candidate_tactic2, $kw25$REMOVAL ) && inference_datastructures_tactic.tactic_hl_module( candidate_tactic2 ).eql( hl_module ) )
              {
    ...
    else
    {
          ...
          .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
      {
        ...
        if( NIL != set_contents.do_set_contents_element_validP( state, split_link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( split_link, $kw32$SPLIT ) )
        {
          ...
          while ( NIL != cdolist_list_var2)
          {
            if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( candidate_tactic2, $kw33$REMOVAL_CONJUNCTIVE ) && inference_datastructures_tactic.tactic_hl_module( candidate_tactic2 ).eql(
                hl_module ) )
            {
              return candidate_tactic2;
            }
          ...
          for( basis_object_$2 = set_contents.do_set_contents_basis_object( set_contents_var_$1 ), state_$3 = NIL, state_$3 = set_contents.do_set_contents_initial_state( basis_object_$2,
              set_contents_var_$1 ); NIL == set_contents.do_set_contents_doneP( basis_object_$2, state_$3 ); state_$3 = set_contents.do_set_contents_update_state( state_$3 ) )
          {
            restriction_link2 = set_contents.do_set_contents_next( basis_object_$2, state_$3 );
            if( NIL != set_contents.do_set_contents_element_validP( state_$3, restriction_link2 ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( restriction_link2, $kw31$RESTRICTION ) )
            {
              ...
              {
                if( NIL != inference_datastructures_problem.do_problem_tactics_type_match( candidate_tactic3, $kw33$REMOVAL_CONJUNCTIVE ) && inference_datastructures_tactic.tactic_hl_module( candidate_tactic3 ).eql(
                    hl_module ) )
                {
  ...
  {
    return makeBoolean( NIL != inference_datastructures_tactic.tactic_p( tactic ) && $kw33$REMOVAL_CONJUNCTIVE == inference_datastructures_tactic.tactic_type( tactic ) );
  }
          ...
          if( NIL == supplanted_hl_modules || NIL == conses_high.member( hl_module, supplanted_hl_modules, UNPROVIDED, UNPROVIDED ) )
          {
            ...
            if( NIL == exclusive_func || ( exclusive_func.isFunctionSpec() && NIL != Functions.funcall( exclusive_func, dnf_clause ) ) )
            {
              ...
              if( NIL != cost )
              {
                ...
                final SubLObject tactic_spec = ConsesLow.list( hl_module, productivity, completeness );
  ...
  public static SubLObject determine_applicable_conjunctive_removal_modules(final SubLObject contextualized_dnf_clause)
  {
    ...
    final SubLObject allowed_modules_spec = ( NIL != inference_macros.current_controlling_inference() ) ? inference_datastructures_inference.inference_allowed_modules( inference_macros.current_controlling_inference() )
        : $kw36$ALL;
        ...
        .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state, hl_module ) && ( NIL == some_backchain_requiredP || NIL != removal_modules_conjunctive_pruning.conjunctive_pruning_module_p( hl_module )
          || ( NIL != inference_worker_restriction.$simplification_tactics_execute_early_and_pass_down_transformation_motivationP$.getDynamicValue( thread ) && NIL != simplification_modules.simplification_module_p(
              hl_module ) ) ) && NIL != inference_modules.hl_module_allowed_by_allowed_modules_specP( hl_module, allowed_modules_spec ) )
      {
        ...
        try
        {
          ...
          if( NIL != inference_modules.hl_module_direction_relevantP( hl_module ) )
          {
            ...
            while ( NIL != cdolist_list_var)
            {
              ...
              {
                applicable_modules = ConsesLow.cons( hl_module, applicable_modules );
              }
              cdolist_list_var = cdolist_list_var.rest();
              subclause_spec = cdolist_list_var.first();
            }
  ...
  public static SubLObject motivated_multi_literal_subclause_specs(final SubLObject contextualized_dnf_clause)
  {
    ...
    if( NIL == inference_trampolines.inference_some_backchain_required_asent_in_clauseP( contextualized_dnf_clause ) )
    {
      ...
      try
      {
        ...
        {
          hl_module = set_contents.do_set_contents_next( basis_object, state );
          if( NIL != set_contents.do_set_contents_element_validP( state, hl_module ) && NIL == removal_modules_conjunctive_pruning.conjunctive_pruning_module_p( hl_module ) && NIL == simplification_modules
              .simplification_module_p( hl_module ) && NIL != inference_modules.hl_module_direction_relevantP( hl_module ) )
          {
            new_subclause_specs = ( cdolist_list_var = hl_module_applicable_subclause_specs( hl_module, contextualized_dnf_clause ) );
            subclause_spec = NIL;
            subclause_spec = cdolist_list_var.first();
  ...
  public static SubLObject hl_module_applicable_subclause_specs(final SubLObject hl_module, final SubLObject contextualized_dnf_clause)
  {
    ...
    else
    {
      ...
      if( applicability_method.isFunctionSpec() )
      {
        ...
        try
        {
          hl_macros.$forward_inference_metric_last_metric_type$.bind( $kw37$INFERENCE, thread );
          ...
          finally
          {
            ...
            try
            {
              ...
              inference_metrics.increment_forward_inference_metrics( $kw37$INFERENCE, applicability_method, NIL, NIL, NIL );
  ...
  public static SubLObject new_conjunctive_removal_tactic(final SubLObject problem, final SubLObject hl_module, final SubLObject productivity, final SubLObject completeness)
  {
    ...
    if( NIL == id_index.id_index_objects_empty_p( idx, $kw39$SKIP ) )
    {
      ...
      if( NIL == id_index.id_index_dense_objects_empty_p( idx_$8, $kw39$SKIP ) )
      {
        ...
        for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
        {
          ...
          if( NIL == id_index.id_index_tombstone_p( inference ) || NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) )
          {
  ...
  public static SubLObject execute_conjunctive_removal_tactic(final SubLObject tactic)
  {
    ...
    else
    {
      ...
      if( NIL != inference_datastructures_tactic.valid_tactic_p( tactic ) && NIL != inference_tactician_strategic_uninterestingness.tactic_completeP( tactic, $kw40$TACTICAL ) )
      {
  ...
  {
    return ( NIL != inference_modules.hl_module_expand_iterative_pattern( inference_datastructures_tactic.tactic_hl_module( tactic ) ) ) ? maybe_make_conjunctive_removal_tactic_progress_expand_iterative_iterator(
        tactic ) : maybe_make_conjunctive_removal_tactic_progress_expand_iterator( tactic );
  }
  ...
  {
    return inference_datastructures_tactic.new_tactic_progress_iterator( $kw42$CONJUNCTIVE_REMOVAL_EXPAND_ITERATIVE, tactic, output_iterator );
  }
  ...
  public static SubLObject handle_one_conjunctive_removal_tactic_expand_iterative_result(final SubLObject tactic, final SubLObject output_iterator)
  {
    ...
    if( NIL != validP )
    {
      ...
      if( NIL == current )
      {
        SubLObject cdolist_list_var = cycl_utilities.expression_gather( inference_datastructures_problem.problem_query( inference_datastructures_tactic.tactic_problem( tactic ) ), $sym43$HL_VAR_, UNPROVIDED, UNPROVIDED,
  ...
  {
    return inference_datastructures_tactic.new_tactic_progress_iterator( $kw44$CONJUNCTIVE_REMOVAL_EXPAND, tactic, results );
  }
  ...
  public static SubLObject conjunctive_removal_callback(SubLObject removal_bindings, final SubLObject justifications)
  {
    ...
    $conjunctive_removal_tactic_expand_results_queue$.setDynamicValue( ConsesLow.cons( ConsesLow.list( removal_bindings, justifications ), $conjunctive_removal_tactic_expand_results_queue$.getDynamicValue( thread ) ),
        thread );
    return NIL;
  }
  ...
  public static SubLObject conjunctive_removal_suppress_split_justificationP()
  {
    ...
    if( NIL != $conjunctive_removal_suppress_split_justificationP$.getDynamicValue( thread ) )
    {
      ...
      return makeBoolean( NIL == Sequences.find( $kw45$INFERENCE_PROOF_SPEC, metrics, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
    }
    return NIL;
  }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      for( foundP = NIL, rest = NIL, rest = clauses.neg_lits( expected_restricted_clause ); NIL == foundP && NIL != rest; rest = rest.rest() )
      {
        ...
        if( expected_asent.equal( actual_asent ) )
        {
          ...
          reordered_justifications = ConsesLow.cons( justification, reordered_justifications );
          foundP = T;
        }
  ...
  public static SubLObject maybe_new_split_and_removal_links(final SubLObject problem, final SubLObject tactic, SubLObject justifications)
  {
    ...
    else
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        maybe_new_removal_link_for_split_link( split_link, tactic, contextualized_asent, $kw48$NEG, literal_supports );
        cdolist_list_var = cdolist_list_var.rest();
        contextualized_asent = cdolist_list_var.first();
      }
      ...
      while ( NIL != cdolist_list_var)
      {
        final SubLObject literal_supports = justifications.first();
        justifications = justifications.rest();
        maybe_new_removal_link_for_split_link( split_link, tactic, contextualized_asent, $kw49$POS, literal_supports );
        cdolist_list_var = cdolist_list_var.rest();
        contextualized_asent = cdolist_list_var.first();
      }
    }
  ...
  public static SubLObject executed_conjunctive_removal_problems(final SubLObject store, SubLObject module_subtype)
  {
    ...
    if( NIL == id_index.id_index_objects_empty_p( idx, $kw39$SKIP ) )
    {
      ...
      if( NIL == id_index.id_index_dense_objects_empty_p( idx_$11, $kw39$SKIP ) )
      {
        ...
        for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
        {
          ...
          if( NIL == id_index.id_index_tombstone_p( problem ) || NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) )
          {
            ...
            {
              tactic = rest.first();
              if( NIL != executed_conjunctive_removal_tactic_p( tactic ) && ( NIL == module_subtype || NIL != list_utilities.member_eqP( module_subtype, inference_modules.hl_module_subtypes(
      ...
      if( NIL == id_index.id_index_sparse_objects_empty_p( idx_$12 ) || NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) )
      {
        ...
        while ( id2.numL( end_id ))
        {
          ...
          if( NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) || NIL == id_index.id_index_tombstone_p( problem2 ) )
          {
            SubLObject witness2;
            SubLObject rest2;
            SubLObject tactic2;
            ...
            {
              tactic2 = rest2.first();
              if( NIL != executed_conjunctive_removal_tactic_p( tactic2 ) && ( NIL == module_subtype || NIL != list_utilities.member_eqP( module_subtype, inference_modules.hl_module_subtypes(
                  inference_datastructures_tactic.tactic_hl_module( tactic2 ) ) ) ) )
              {
                witness2 = tactic2;
              }
            }
  ...
  public static SubLObject problem_store_has_some_executed_sksi_conjunctive_removal_problemP(final SubLObject store)
  {
    ...
    if( NIL == id_index.id_index_objects_empty_p( idx, $kw39$SKIP ) )
    {
      ...
      if( NIL == id_index.id_index_dense_objects_empty_p( idx_$13, $kw39$SKIP ) )
      {
          ...
          if( NIL == witness )
          {
            ...
            for( end_var = end, id = NIL, id = start; NIL == witness && NIL == subl_macros.do_numbers_endtest( id, delta, end_var ); id = Numbers.add( id, delta ) )
            {
              ...
              if( NIL == id_index.id_index_tombstone_p( problem ) || NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) )
              {
                ...
                {
                  tactic = rest.first();
                  if( NIL != executed_conjunctive_removal_tactic_p( tactic ) && NIL != list_utilities.member_eqP( $kw51$SKSI, inference_modules.hl_module_subtypes( inference_datastructures_tactic.tactic_hl_module(
                      tactic ) ) ) )
                  {
                    witness = tactic;
                  }
      ...
      if( NIL == id_index.id_index_sparse_objects_empty_p( idx_$14 ) || NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) )
      {
        ...
        while ( id2.numL( end_id ) && NIL == witness)
        {
          ...
          if( NIL == id_index.id_index_skip_tombstones_p( $kw39$SKIP ) || NIL == id_index.id_index_tombstone_p( problem2 ) )
          {
            ...
            {
              tactic2 = rest2.first();
              if( NIL != executed_conjunctive_removal_tactic_p( tactic2 ) && NIL != list_utilities.member_eqP( $kw51$SKSI, inference_modules.hl_module_subtypes( inference_datastructures_tactic.tactic_hl_module(
                  tactic2 ) ) ) )
              {
                witness = tactic2;
              }
  ...
  {
    return makeBoolean( NIL != conjunctive_removal_tactic_p( tactic ) && ( NIL != inference_datastructures_tactic.tactic_in_progressP( tactic ) || NIL != inference_datastructures_tactic.tactic_executedP( tactic ) )
        && NIL == inference_worker_restriction.simplification_tactic_p( tactic ) && NIL == removal_modules_conjunctive_pruning.conjunctive_pruning_tactic_p( tactic ) );
  }
  ...
  public static SubLObject with_problem_store_removal_assumptions(final SubLObject macroform, final SubLObject environment)
  {
    ...
    return ConsesLow.listS( $sym54$CLET, ConsesLow.list( ConsesLow.list( store_var, store ), ConsesLow.list( $sym55$_NEGATION_BY_FAILURE_, ConsesLow.list( $sym56$PROBLEM_STORE_NEGATION_BY_FAILURE_, store_var ) ) ),
  ...
  {
    return makeBoolean( NIL != inference_datastructures_tactic.tactic_p( v_object ) && $kw57$META_REMOVAL == inference_datastructures_tactic.tactic_type( v_object ) );
  }
  ...
  {
    return makeBoolean( NIL != inference_datastructures_problem_link.problem_link_p( v_object ) && $kw25$REMOVAL == inference_datastructures_problem_link.problem_link_type( v_object ) );
  }
  ...
  {
    return makeBoolean( NIL != inference_datastructures_tactic.tactic_p( v_object ) && $kw25$REMOVAL == inference_datastructures_tactic.tactic_type( v_object ) );
  }
    ...
    try
    {
      ...
      finally
      {
        ...
        try
        {
          Threads.$is_thread_performing_cleanupP$.bind( T, thread );
          final SubLObject _values = Values.getValuesAsVector();
          inference_metrics.increment_forward_inference_metrics( $kw37$INFERENCE, func, NIL, NIL, NIL );
          Values.restoreValuesFromVector( _values );
        }
        finally
        {
  ...
  public static SubLObject filter_modules_wrt_allowed_modules_spec(final SubLObject candidate_hl_modules, final SubLObject allowed_modules_spec)
  {
    ...
    if( allowed_modules_spec == $kw36$ALL )
    {
    ...
    else
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          hl_modules = ConsesLow.cons( module, hl_modules );
        }
        cdolist_list_var = cdolist_list_var.rest();
        module = cdolist_list_var.first();
      }
  ...
  public static SubLObject hl_module_applicable_to_asentP(final SubLObject hl_module, final SubLObject asent)
  {
    return makeBoolean( NIL != inference_modules.hl_module_predicate_relevant_p( hl_module, cycl_utilities.atomic_sentence_predicate( asent ) ) && NIL != inference_modules.hl_module_arity_relevant_p( hl_module, asent )
        && NIL != inference_modules.hl_module_required_pattern_matched_p( hl_module, asent ) && NIL != inference_modules.hl_module_required_mt_relevantP( hl_module ) && NIL != inference_modules
            .hl_module_direction_relevantP( hl_module ) );
  }
  ...
  {
    return hl_module_guts( $kw78$DETERMINE_NEW_REMOVAL_TACTIC_SPECS_FROM_HL_MODULES, hl_modules, asent, sense, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject update_supplanted_hl_modules(final SubLObject hl_module, SubLObject applicable_hl_modules, SubLObject supplanted_hl_modules)
  {
    ...
    else
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          SubLObject patterns = ConsesLow.list( supplanted_hl_module_pattern );
          ...
          while ( NIL != patterns)
          {
            ...
            if( pcase_var_$25.eql( $kw79$NOT ) )
            {
              negateP = makeBoolean( NIL == negateP );
              patterns = ConsesLow.cons( rest.first(), patterns );
            }
            else
            {
              if( !pcase_var_$25.eql( $kw80$MODULE_SUBTYPE ) )
              {
                continue;
              }
              ...
              {
                if( ( NIL == negateP && NIL != list_utilities.member_eqP( subtype, inference_modules.hl_module_subtypes( supplanted_hl_module ) ) ) || ( NIL != negateP && NIL == list_utilities.member_eqP( subtype,
                    inference_modules.hl_module_subtypes( supplanted_hl_module ) ) ) )
                {
        ...
        else
        {
          ...
          {
            supplanted_hl_modules = ConsesLow.cons( item_var, supplanted_hl_modules );
          }
  ...
  public static SubLObject literal_removal_candidate_hl_modules_for_predicate_with_sense(final SubLObject predicate, final SubLObject sense)
  {
    ...
    return literal_removal_candidate_hl_modules_for_predicate_with_sense_int( predicate, sense, $kw36$ALL );
  }
  ...
  public static SubLObject literal_removal_candidate_hl_modules_for_predicate_with_sense_int(final SubLObject predicate, final SubLObject sense, final SubLObject allowed_modules_spec)
  {
    ...
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym83$LITERAL_REMOVAL_CANDIDATE_HL_MODULES_FOR_PREDICATE_WITH_SENSE_INT, THREE_INTEGER, NIL,
          EQL, UNPROVIDED );
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( literal_removal_candidate_hl_modules_for_predicate_with_sense_int_internal( predicate, sense,
        allowed_modules_spec ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( predicate, sense, allowed_modules_spec ) );
  ...
  public static SubLObject literal_meta_removal_candidate_hl_modules_for_predicate_internal(final SubLObject predicate)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        v_meta_removal_modules = ConsesLow.cons( meta_removal_module, v_meta_removal_modules );
      }
      cdolist_list_var = cdolist_list_var.rest();
      meta_removal_module = cdolist_list_var.first();
    }
  ...
  public static SubLObject literal_meta_removal_candidate_hl_modules_for_predicate(final SubLObject predicate)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym84$LITERAL_META_REMOVAL_CANDIDATE_HL_MODULES_FOR_PREDICATE, ONE_INTEGER, NIL, EQUAL,
          UNPROVIDED );
  ...
  public static SubLObject new_removal_tactic(final SubLObject problem, final SubLObject hl_module, final SubLObject productivity, final SubLObject completeness)
  {
    ...
    if( NIL == id_index.id_index_objects_empty_p( idx, $kw39$SKIP ) )
    {
      ...
      if( NIL == id_index.id_index_dense_objects_empty_p( idx_$27, $kw39$SKIP ) )
      {
  ...
  public static SubLObject with_removal_tactic_execution_assumptions(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym54$CLET, ConsesLow.list( ConsesLow.list( tactic_var, tactic ) ), ConsesLow.list( $sym87$WITH_INFERENCE_MT_RELEVANCE, mt, ConsesLow.list( $sym54$CLET, ConsesLow.list( ConsesLow.list(
          $sym88$_INFERENCE_EXPAND_HL_MODULE_, ConsesLow.list( $sym89$TACTIC_HL_MODULE, tactic_var ) ), ConsesLow.list( $sym90$_INFERENCE_EXPAND_SENSE_, sense ) ), ConsesLow.listS(
              $sym91$WITH_PROBLEM_STORE_REMOVAL_ASSUMPTIONS, ConsesLow.list( $sym92$TACTIC_STORE, tactic_var ), ConsesLow.append( body, NIL ) ) ) ) );
    }
  ...
  {
    return inference_datastructures_tactic.new_tactic_progress_iterator( $kw93$REMOVAL_OUTPUT_GENERATE, tactic, ConsesLow.list( output_iterator, encoded_bindings ) );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject expand_results = hl_module_guts( $kw95$MAYBE_MAKE_REMOVAL_TACTIC_EXPAND_RESULTS_PROGRESS_ITERATOR, tactic, asent, sense, UNPROVIDED, UNPROVIDED );
    final SubLObject new_productivity = inference_datastructures_enumerated_types.productivity_for_number_of_children( Sequences.length( expand_results ) );
    inference_datastructures_tactic.update_tactic_productivity( tactic, new_productivity );
    if( NIL != list_utilities.lengthGE( expand_results, $removal_tactic_iteration_threshold$.getDynamicValue( thread ), UNPROVIDED ) )
    {
  ...
  public static SubLObject handle_removal_add_node_for_expand_results(SubLObject removal_bindings, final SubLObject supports)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    removal_bindings = bindings.inference_simplify_unification_bindings( removal_bindings );
    $removal_tactic_expand_results_queue$.setDynamicValue( ConsesLow.cons( ConsesLow.list( removal_bindings, supports ), $removal_tactic_expand_results_queue$.getDynamicValue( thread ) ), thread );
    return NIL;
  }
  ...
  {
    return inference_datastructures_tactic.new_tactic_progress_iterator( $kw97$REMOVAL_EXPAND, tactic, expand_results );
  }
  ...
  public static SubLObject do_all_legacy_inference_outputs(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      return ConsesLow.list( $sym54$CLET, ConsesLow.list( ConsesLow.list( iterator, raw_output_iterator ) ), ConsesLow.list( $sym104$PIF, ConsesLow.list( $sym105$ITERATOR_P, iterator ), ConsesLow.list(
          $sym106$CUNWIND_PROTECT, ConsesLow.listS( $sym107$DO_ITERATOR, ConsesLow.list( raw_output, iterator ), ConsesLow.append( body, NIL ) ), ConsesLow.list( $sym108$ITERATION_FINALIZE, iterator ) ), ConsesLow.listS(
              $sym109$DO_LIST, ConsesLow.list( raw_output, iterator ), ConsesLow.append( body, NIL ) ) ) );
    }
  ...
  {
    return hl_module_guts( $kw110$MAYBE_MAKE_INFERENCE_OUTPUT_ITERATOR, hl_module, cycl_input_asent, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject handle_one_output_generate_result(final SubLObject cycl_input_asent, final SubLObject hl_module, final SubLObject raw_output, final SubLObject encoded_bindings)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    thread.resetMultipleValues();
    final SubLObject successP = hl_module_guts( $kw111$HANDLE_ONE_OUTPUT_GENERATE_RESULT, cycl_input_asent, hl_module, raw_output, encoded_bindings, UNPROVIDED );
  ...
  public static SubLObject handle_one_output_generate_result_guts(final SubLObject cycl_input_asent, final SubLObject hl_module, final SubLObject raw_output, final SubLObject encoded_bindings)
  {
    ...
    if( NIL != inference_output_verifier( hl_module, cycl_output ) )
    {
      ...
      if( NIL != unify_bindings )
      {
        ...
        final SubLObject more_supports = thread.secondMultipleValue();
        thread.resetMultipleValues();
        return subl_promotions.values4( T, support, unify_bindings, ConsesLow.append( more_supports, unify_justification ) );
      }
    }
  ...
  public static SubLObject inference_support_constructor(final SubLObject hl_module, final SubLObject cycl_output_asent, SubLObject v_bindings)
  {
    ...
    final SubLObject support_tv = enumeration_types.tv_from_truth_strength( $kw113$TRUE, support_strength );
  ...
  public static SubLObject hl_module_guts(final SubLObject type, SubLObject arg1, SubLObject arg2, SubLObject arg3, SubLObject arg4, SubLObject arg5)
  {
    ...
    if( type.eql( $kw78$DETERMINE_NEW_REMOVAL_TACTIC_SPECS_FROM_HL_MODULES ) )
    {
      return determine_new_removal_tactic_specs_from_hl_modules_guts( arg1, arg2, arg3 );
    }
    if( type.eql( $kw95$MAYBE_MAKE_REMOVAL_TACTIC_EXPAND_RESULTS_PROGRESS_ITERATOR ) )
    {
      return maybe_make_removal_tactic_expand_results_progress_iterator_guts( arg1, arg2, arg3 );
    }
    if( type.eql( $kw111$HANDLE_ONE_OUTPUT_GENERATE_RESULT ) )
    {
      return handle_one_output_generate_result_guts( arg1, arg2, arg3, arg4 );
    }
    if( type.eql( $kw110$MAYBE_MAKE_INFERENCE_OUTPUT_ITERATOR ) )
    {
  ...
  {
    SubLFiles.declareFunction( me, "removal_link_data_print_function_trampoline", "REMOVAL-LINK-DATA-PRINT-FUNCTION-TRAMPOLINE", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_link_data_p", "REMOVAL-LINK-DATA-P", 1, 0, false );
    new $removal_link_data_p$UnaryFunction();
    SubLFiles.declareFunction( me, "remov_link_data_hl_module", "REMOV-LINK-DATA-HL-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "remov_link_data_bindings", "REMOV-LINK-DATA-BINDINGS", 1, 0, false );
    SubLFiles.declareFunction( me, "remov_link_data_supports", "REMOV-LINK-DATA-SUPPORTS", 1, 0, false );
    SubLFiles.declareFunction( me, "_csetf_remov_link_data_hl_module", "_CSETF-REMOV-LINK-DATA-HL-MODULE", 2, 0, false );
    SubLFiles.declareFunction( me, "_csetf_remov_link_data_bindings", "_CSETF-REMOV-LINK-DATA-BINDINGS", 2, 0, false );
    SubLFiles.declareFunction( me, "_csetf_remov_link_data_supports", "_CSETF-REMOV-LINK-DATA-SUPPORTS", 2, 0, false );
    SubLFiles.declareFunction( me, "make_removal_link_data", "MAKE-REMOVAL-LINK-DATA", 0, 1, false );
    SubLFiles.declareFunction( me, "visit_defstruct_removal_link_data", "VISIT-DEFSTRUCT-REMOVAL-LINK-DATA", 2, 0, false );
    SubLFiles.declareFunction( me, "visit_defstruct_object_removal_link_data_method", "VISIT-DEFSTRUCT-OBJECT-REMOVAL-LINK-DATA-METHOD", 2, 0, false );
    SubLFiles.declareFunction( me, "new_removal_link", "NEW-REMOVAL-LINK", 4, 0, false );
    SubLFiles.declareFunction( me, "new_removal_link_int", "NEW-REMOVAL-LINK-INT", 4, 0, false );
    SubLFiles.declareFunction( me, "new_removal_link_data", "NEW-REMOVAL-LINK-DATA", 1, 0, false );
    SubLFiles.declareFunction( me, "destroy_removal_link", "DESTROY-REMOVAL-LINK", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_link_hl_module", "REMOVAL-LINK-HL-MODULE", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_link_bindings", "REMOVAL-LINK-BINDINGS", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_link_supports", "REMOVAL-LINK-SUPPORTS", 1, 0, false );
    SubLFiles.declareFunction( me, "set_removal_link_hl_module", "SET-REMOVAL-LINK-HL-MODULE", 2, 0, false );
    SubLFiles.declareFunction( me, "set_removal_link_bindings", "SET-REMOVAL-LINK-BINDINGS", 2, 0, false );
    SubLFiles.declareFunction( me, "set_removal_link_supports", "SET-REMOVAL-LINK-SUPPORTS", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_link_tactic", "REMOVAL-LINK-TACTIC", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_link_data_equals_specP", "REMOVAL-LINK-DATA-EQUALS-SPEC?", 3, 0, false );
    SubLFiles.declareFunction( me, "generalized_removal_tactic_p", "GENERALIZED-REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_removal_tactic_p", "CONJUNCTIVE-REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_removal_link_p", "CONJUNCTIVE-REMOVAL-LINK-P", 1, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_removal_proof_p", "CONJUNCTIVE-REMOVAL-PROOF-P", 1, 0, false );
    SubLFiles.declareFunction( me, "determine_new_conjunctive_removal_tactics", "DETERMINE-NEW-CONJUNCTIVE-REMOVAL-TACTICS", 2, 0, false );
    SubLFiles.declareFunction( me, "sort_applicable_conjunctive_removal_modules_by_priority", "SORT-APPLICABLE-CONJUNCTIVE-REMOVAL-MODULES-BY-PRIORITY", 1, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_removal_module_priorityL", "CONJUNCTIVE-REMOVAL-MODULE-PRIORITY<", 2, 0, false );
    SubLFiles.declareFunction( me, "determine_applicable_conjunctive_removal_modules", "DETERMINE-APPLICABLE-CONJUNCTIVE-REMOVAL-MODULES", 1, 0, false );
    SubLFiles.declareFunction( me, "motivated_multi_literal_subclause_specs", "MOTIVATED-MULTI-LITERAL-SUBCLAUSE-SPECS", 1, 0, false );
    SubLFiles.declareFunction( me, "hl_module_applicable_subclause_specs", "HL-MODULE-APPLICABLE-SUBCLAUSE-SPECS", 2, 0, false );
    SubLFiles.declareFunction( me, "new_conjunctive_removal_tactic", "NEW-CONJUNCTIVE-REMOVAL-TACTIC", 4, 0, false );
    SubLFiles.declareFunction( me, "compute_strategic_properties_of_conjunctive_removal_tactic", "COMPUTE-STRATEGIC-PROPERTIES-OF-CONJUNCTIVE-REMOVAL-TACTIC", 2, 0, false );
    SubLFiles.declareFunction( me, "execute_conjunctive_removal_tactic", "EXECUTE-CONJUNCTIVE-REMOVAL-TACTIC", 1, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_conjunctive_removal_tactic_progress_iterator", "MAYBE-MAKE-CONJUNCTIVE-REMOVAL-TACTIC-PROGRESS-ITERATOR", 1, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_conjunctive_removal_tactic_progress_expand_iterative_iterator", "MAYBE-MAKE-CONJUNCTIVE-REMOVAL-TACTIC-PROGRESS-EXPAND-ITERATIVE-ITERATOR", 1, 0, false );
    SubLFiles.declareFunction( me, "new_conjunctive_removal_tactic_progress_expand_iterative_iterator", "NEW-CONJUNCTIVE-REMOVAL-TACTIC-PROGRESS-EXPAND-ITERATIVE-ITERATOR", 2, 0, false );
    SubLFiles.declareFunction( me, "handle_one_conjunctive_removal_tactic_expand_iterative_result", "HANDLE-ONE-CONJUNCTIVE-REMOVAL-TACTIC-EXPAND-ITERATIVE-RESULT", 2, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_conjunctive_removal_tactic_progress_expand_iterator", "MAYBE-MAKE-CONJUNCTIVE-REMOVAL-TACTIC-PROGRESS-EXPAND-ITERATOR", 1, 0, false );
    SubLFiles.declareFunction( me, "new_conjunctive_removal_tactic_progress_expand_iterator", "NEW-CONJUNCTIVE-REMOVAL-TACTIC-PROGRESS-EXPAND-ITERATOR", 2, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_removal_callback", "CONJUNCTIVE-REMOVAL-CALLBACK", 2, 0, false );
    SubLFiles.declareFunction( me, "handle_one_conjunctive_removal_tactic_expand_result", "HANDLE-ONE-CONJUNCTIVE-REMOVAL-TACTIC-EXPAND-RESULT", 2, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_removal_suppress_split_justificationP", "CONJUNCTIVE-REMOVAL-SUPPRESS-SPLIT-JUSTIFICATION?", 0, 0, false );
    SubLFiles.declareFunction( me, "handle_one_conjunctive_removal_tactic_result", "HANDLE-ONE-CONJUNCTIVE-REMOVAL-TACTIC-RESULT", 3, 0, false );
    SubLFiles.declareFunction( me, "maybe_new_simplification_link", "MAYBE-NEW-SIMPLIFICATION-LINK", 3, 0, false );
    SubLFiles.declareFunction( me, "maybe_new_restriction_split_and_removal_links", "MAYBE-NEW-RESTRICTION-SPLIT-AND-REMOVAL-LINKS", 4, 0, false );
    SubLFiles.declareFunction( me, "reorder_conjunctive_removal_justifications", "REORDER-CONJUNCTIVE-REMOVAL-JUSTIFICATIONS", 4, 0, false );
    SubLFiles.declareFunction( me, "maybe_new_split_and_removal_links", "MAYBE-NEW-SPLIT-AND-REMOVAL-LINKS", 3, 0, false );
    SubLFiles.declareFunction( me, "maybe_new_removal_link_for_split_link", "MAYBE-NEW-REMOVAL-LINK-FOR-SPLIT-LINK", 5, 0, false );
    SubLFiles.declareFunction( me, "executed_conjunctive_removal_problems", "EXECUTED-CONJUNCTIVE-REMOVAL-PROBLEMS", 1, 1, false );
    SubLFiles.declareFunction( me, "problem_store_has_some_executed_sksi_conjunctive_removal_problemP", "PROBLEM-STORE-HAS-SOME-EXECUTED-SKSI-CONJUNCTIVE-REMOVAL-PROBLEM?", 1, 0, false );
    SubLFiles.declareFunction( me, "executed_conjunctive_removal_tactic_p", "EXECUTED-CONJUNCTIVE-REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareMacro( me, "with_problem_store_removal_assumptions", "WITH-PROBLEM-STORE-REMOVAL-ASSUMPTIONS" );
    SubLFiles.declareFunction( me, "meta_removal_tactic_p", "META-REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "compute_strategic_properties_of_meta_removal_tactic", "COMPUTE-STRATEGIC-PROPERTIES-OF-META-REMOVAL-TACTIC", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_link_p", "REMOVAL-LINK-P", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_tactic_p", "REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_proof_p", "REMOVAL-PROOF-P", 1, 0, false );
    SubLFiles.declareFunction( me, "removal_module_exclusive_func_funcall", "REMOVAL-MODULE-EXCLUSIVE-FUNC-FUNCALL", 3, 0, false );
    SubLFiles.declareFunction( me, "removal_module_required_func_funcall", "REMOVAL-MODULE-REQUIRED-FUNC-FUNCALL", 3, 0, false );
    SubLFiles.declareFunction( me, "removal_module_expand_func_funcall", "REMOVAL-MODULE-EXPAND-FUNC-FUNCALL", 3, 0, false );
    SubLFiles.declareFunction( me, "determine_new_literal_removal_tactics", "DETERMINE-NEW-LITERAL-REMOVAL-TACTICS", 3, 0, false );
    SubLFiles.declareFunction( me, "determine_new_literal_meta_removal_tactics", "DETERMINE-NEW-LITERAL-META-REMOVAL-TACTICS", 3, 0, false );
    SubLFiles.declareFunction( me, "determine_new_literal_simple_removal_tactics", "DETERMINE-NEW-LITERAL-SIMPLE-REMOVAL-TACTICS", 3, 0, false );
    SubLFiles.declareFunction( me, "literal_removal_options", "LITERAL-REMOVAL-OPTIONS", 2, 1, false );
    SubLFiles.declareFunction( me, "literal_removal_options_hl_modules", "LITERAL-REMOVAL-OPTIONS-HL-MODULES", 3, 0, false );
    SubLFiles.declareFunction( me, "filter_modules_wrt_allowed_modules_spec", "FILTER-MODULES-WRT-ALLOWED-MODULES-SPEC", 2, 0, false );
    SubLFiles.declareFunction( me, "literal_removal_options_candidate_hl_modules", "LITERAL-REMOVAL-OPTIONS-CANDIDATE-HL-MODULES", 3, 0, false );
    SubLFiles.declareFunction( me, "hl_module_applicable_to_asentP", "HL-MODULE-APPLICABLE-TO-ASENT?", 2, 0, false );
    SubLFiles.declareFunction( me, "determine_new_removal_tactics_from_hl_modules", "DETERMINE-NEW-REMOVAL-TACTICS-FROM-HL-MODULES", 4, 0, false );
    SubLFiles.declareFunction( me, "determine_new_removal_tactic_specs_from_hl_modules", "DETERMINE-NEW-REMOVAL-TACTIC-SPECS-FROM-HL-MODULES", 3, 0, false );
    SubLFiles.declareFunction( me, "determine_new_removal_tactic_specs_from_hl_modules_guts", "DETERMINE-NEW-REMOVAL-TACTIC-SPECS-FROM-HL-MODULES-GUTS", 3, 0, false );
    SubLFiles.declareFunction( me, "determine_applicable_hl_modules_for_asent", "DETERMINE-APPLICABLE-HL-MODULES-FOR-ASENT", 3, 0, false );
    SubLFiles.declareFunction( me, "update_applicable_hl_modules", "UPDATE-APPLICABLE-HL-MODULES", 5, 0, false );
    SubLFiles.declareFunction( me, "update_supplanted_hl_modules", "UPDATE-SUPPLANTED-HL-MODULES", 3, 0, false );
    SubLFiles.declareFunction( me, "update_supplanted_modules_wrt_tactic_specs", "UPDATE-SUPPLANTED-MODULES-WRT-TACTIC-SPECS", 3, 0, false );
    SubLFiles.declareFunction( me, "compute_tactic_specs_for_asent", "COMPUTE-TACTIC-SPECS-FOR-ASENT", 3, 0, false );
    SubLFiles.declareFunction( me, "literal_simple_removal_candidate_hl_modules", "LITERAL-SIMPLE-REMOVAL-CANDIDATE-HL-MODULES", 2, 0, false );
    SubLFiles.declareFunction( me, "literal_removal_candidate_hl_modules_for_predicate_with_sense", "LITERAL-REMOVAL-CANDIDATE-HL-MODULES-FOR-PREDICATE-WITH-SENSE", 2, 0, false );
    SubLFiles.declareFunction( me, "literal_removal_candidate_hl_modules_for_predicate_with_sense_int_internal", "LITERAL-REMOVAL-CANDIDATE-HL-MODULES-FOR-PREDICATE-WITH-SENSE-INT-INTERNAL", 3, 0, false );
    SubLFiles.declareFunction( me, "literal_removal_candidate_hl_modules_for_predicate_with_sense_int", "LITERAL-REMOVAL-CANDIDATE-HL-MODULES-FOR-PREDICATE-WITH-SENSE-INT", 3, 0, false );
    SubLFiles.declareFunction( me, "literal_meta_removal_candidate_hl_modules", "LITERAL-META-REMOVAL-CANDIDATE-HL-MODULES", 2, 0, false );
    SubLFiles.declareFunction( me, "literal_meta_removal_candidate_hl_modules_for_predicate_internal", "LITERAL-META-REMOVAL-CANDIDATE-HL-MODULES-FOR-PREDICATE-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "literal_meta_removal_candidate_hl_modules_for_predicate", "LITERAL-META-REMOVAL-CANDIDATE-HL-MODULES-FOR-PREDICATE", 1, 0, false );
    SubLFiles.declareFunction( me, "literal_level_removal_tactic_p", "LITERAL-LEVEL-REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "literal_level_meta_removal_tactic_p", "LITERAL-LEVEL-META-REMOVAL-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "new_removal_tactic", "NEW-REMOVAL-TACTIC", 4, 0, false );
    SubLFiles.declareFunction( me, "compute_strategic_properties_of_removal_tactic", "COMPUTE-STRATEGIC-PROPERTIES-OF-REMOVAL-TACTIC", 2, 0, false );
    SubLFiles.declareMacro( me, "with_removal_tactic_execution_assumptions", "WITH-REMOVAL-TACTIC-EXECUTION-ASSUMPTIONS" );
    SubLFiles.declareFunction( me, "execute_literal_level_removal_tactic", "EXECUTE-LITERAL-LEVEL-REMOVAL-TACTIC", 4, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_removal_tactic_progress_iterator", "MAYBE-MAKE-REMOVAL-TACTIC-PROGRESS-ITERATOR", 3, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_removal_tactic_output_generate_progress_iterator", "MAYBE-MAKE-REMOVAL-TACTIC-OUTPUT-GENERATE-PROGRESS-ITERATOR", 2, 0, false );
    SubLFiles.declareFunction( me, "new_removal_tactic_output_generate_progress_iterator", "NEW-REMOVAL-TACTIC-OUTPUT-GENERATE-PROGRESS-ITERATOR", 3, 0, false );
    SubLFiles.declareFunction( me, "handle_one_removal_tactic_output_generate_result", "HANDLE-ONE-REMOVAL-TACTIC-OUTPUT-GENERATE-RESULT", 3, 0, false );
    SubLFiles.declareFunction( me, "handle_removal_add_node_for_output_generate", "HANDLE-REMOVAL-ADD-NODE-FOR-OUTPUT-GENERATE", 2, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_removal_tactic_expand_results_progress_iterator", "MAYBE-MAKE-REMOVAL-TACTIC-EXPAND-RESULTS-PROGRESS-ITERATOR", 3, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_removal_tactic_expand_results_progress_iterator_guts", "MAYBE-MAKE-REMOVAL-TACTIC-EXPAND-RESULTS-PROGRESS-ITERATOR-GUTS", 3, 0, false );
    SubLFiles.declareFunction( me, "handle_removal_add_node_for_expand_results", "HANDLE-REMOVAL-ADD-NODE-FOR-EXPAND-RESULTS", 2, 0, false );
    SubLFiles.declareFunction( me, "new_removal_tactic_expand_results_progress_iterator", "NEW-REMOVAL-TACTIC-EXPAND-RESULTS-PROGRESS-ITERATOR", 2, 0, false );
    SubLFiles.declareFunction( me, "handle_one_removal_tactic_expand_result", "HANDLE-ONE-REMOVAL-TACTIC-EXPAND-RESULT", 2, 0, false );
    SubLFiles.declareFunction( me, "handle_one_removal_tactic_result", "HANDLE-ONE-REMOVAL-TACTIC-RESULT", 3, 0, false );
    SubLFiles.declareFunction( me, "maybe_new_restriction_and_removal_link", "MAYBE-NEW-RESTRICTION-AND-REMOVAL-LINK", 4, 0, false );
    SubLFiles.declareFunction( me, "maybe_new_removal_link", "MAYBE-NEW-REMOVAL-LINK", 4, 0, false );
    SubLFiles.declareFunction( me, "find_removal_link", "FIND-REMOVAL-LINK", 4, 0, false );
    SubLFiles.declareFunction( me, "new_removal_proof", "NEW-REMOVAL-PROOF", 1, 0, false );
    SubLFiles.declareFunction( me, "execute_literal_level_meta_removal_tactic", "EXECUTE-LITERAL-LEVEL-META-REMOVAL-TACTIC", 4, 0, false );
    SubLFiles.declareFunction( me, "inference_remove_check_default", "INFERENCE-REMOVE-CHECK-DEFAULT", 1, 1, false );
    SubLFiles.declareMacro( me, "do_all_legacy_inference_outputs", "DO-ALL-LEGACY-INFERENCE-OUTPUTS" );
    SubLFiles.declareFunction( me, "inference_remove_unify_default", "INFERENCE-REMOVE-UNIFY-DEFAULT", 1, 1, false );
    new $inference_remove_unify_default$UnaryFunction();
    new $inference_remove_unify_default$BinaryFunction();
    SubLFiles.declareFunction( me, "maybe_make_inference_output_iterator", "MAYBE-MAKE-INFERENCE-OUTPUT-ITERATOR", 2, 0, false );
    SubLFiles.declareFunction( me, "maybe_make_inference_output_iterator_guts", "MAYBE-MAKE-INFERENCE-OUTPUT-ITERATOR-GUTS", 2, 0, false );
    SubLFiles.declareFunction( me, "handle_one_output_generate_result", "HANDLE-ONE-OUTPUT-GENERATE-RESULT", 4, 0, false );
    SubLFiles.declareFunction( me, "handle_one_output_generate_result_guts", "HANDLE-ONE-OUTPUT-GENERATE-RESULT-GUTS", 4, 0, false );
    SubLFiles.declareFunction( me, "inference_current_hl_module", "INFERENCE-CURRENT-HL-MODULE", 0, 0, false );
    SubLFiles.declareFunction( me, "inference_current_sense", "INFERENCE-CURRENT-SENSE", 0, 0, false );
    SubLFiles.declareFunction( me, "inference_input_extractor", "INFERENCE-INPUT-EXTRACTOR", 2, 1, false );
    SubLFiles.declareFunction( me, "inference_input_verifier", "INFERENCE-INPUT-VERIFIER", 2, 0, false );
    SubLFiles.declareFunction( me, "inference_input_encoder", "INFERENCE-INPUT-ENCODER", 2, 1, false );
    SubLFiles.declareFunction( me, "inference_output_checker", "INFERENCE-OUTPUT-CHECKER", 2, 1, false );
    SubLFiles.declareFunction( me, "inference_output_generator", "INFERENCE-OUTPUT-GENERATOR", 2, 1, false );
    SubLFiles.declareFunction( me, "inference_output_decoder", "INFERENCE-OUTPUT-DECODER", 2, 1, false );
    SubLFiles.declareFunction( me, "inference_output_verifier", "INFERENCE-OUTPUT-VERIFIER", 2, 0, false );
    SubLFiles.declareFunction( me, "inference_output_constructor", "INFERENCE-OUTPUT-CONSTRUCTOR", 2, 1, false );
    SubLFiles.declareFunction( me, "inference_support_constructor", "INFERENCE-SUPPORT-CONSTRUCTOR", 2, 1, false );
    SubLFiles.declareFunction( me, "hl_module_guts", "HL-MODULE-GUTS", 1, 5, false );
    return NIL;
  }

  public static SubLObject init_inference_worker_removal_file()
  {
    $dtp_removal_link_data$ = SubLFiles.defconstant( "*DTP-REMOVAL-LINK-DATA*", $sym0$REMOVAL_LINK_DATA );
    ...
    $removal_tactic_iteration_threshold$ = SubLFiles.defparameter( "*REMOVAL-TACTIC-ITERATION-THRESHOLD*", TWO_INTEGER );
    $removal_tactic_expand_results_queue$ = SubLFiles.defparameter( "*REMOVAL-TACTIC-EXPAND-RESULTS-QUEUE*", NIL );
    return NIL;
  }
  ...
  public static SubLObject setup_inference_worker_removal_file()
  {
    ...
    Structures.register_method( visitation.$visit_defstruct_object_method_table$.getGlobalValue(), $dtp_removal_link_data$.getGlobalValue(), Symbols.symbol_function(
        $sym23$VISIT_DEFSTRUCT_OBJECT_REMOVAL_LINK_DATA_METHOD ) );
  ...
  static
  {
    ...
    $list4 = ConsesLow.list( makeSymbol( "REMOV-LINK-DATA-HL-MODULE" ), makeSymbol( "REMOV-LINK-DATA-BINDINGS" ), makeSymbol( "REMOV-LINK-DATA-SUPPORTS" ) );
    $list5 = ConsesLow.list( makeSymbol( "_CSETF-REMOV-LINK-DATA-HL-MODULE" ), makeSymbol( "_CSETF-REMOV-LINK-DATA-BINDINGS" ), makeSymbol( "_CSETF-REMOV-LINK-DATA-SUPPORTS" ) );
    ...
    $list8 = ConsesLow.list( makeSymbol( "OPTIMIZE-FUNCALL" ), makeSymbol( "REMOVAL-LINK-DATA-P" ) );
    ...
    $kw25$REMOVAL = makeKeyword( "REMOVAL" );
    ...
    $kw31$RESTRICTION = makeKeyword( "RESTRICTION" );
    $kw32$SPLIT = makeKeyword( "SPLIT" );
    $kw33$REMOVAL_CONJUNCTIVE = makeKeyword( "REMOVAL-CONJUNCTIVE" );
    $list34 = ConsesLow.list( makeSymbol( "HL-MODULE" ), makeSymbol( "PRODUCTIVITY" ), makeSymbol( "COMPLETENESS" ) );
    $sym35$CONJUNCTIVE_REMOVAL_MODULE_PRIORITY_ = makeSymbol( "CONJUNCTIVE-REMOVAL-MODULE-PRIORITY<" );
    $kw36$ALL = makeKeyword( "ALL" );
    $kw37$INFERENCE = makeKeyword( "INFERENCE" );
    $str38$_s_stated_its_applicability_to_th = makeString(
        "~s stated its applicability to the subclause spec ~s, which does not specify more than one literal.\nConjunctive removal modules must apply to more than one literal in the clause." );
    $kw39$SKIP = makeKeyword( "SKIP" );
    $kw40$TACTICAL = makeKeyword( "TACTICAL" );
    $list41 = ConsesLow.list( makeSymbol( "REMOVAL-BINDINGS" ), makeSymbol( "JUSTIFICATIONS" ) );
    $kw42$CONJUNCTIVE_REMOVAL_EXPAND_ITERATIVE = makeKeyword( "CONJUNCTIVE-REMOVAL-EXPAND-ITERATIVE" );
    $sym43$HL_VAR_ = makeSymbol( "HL-VAR?" );
    $kw44$CONJUNCTIVE_REMOVAL_EXPAND = makeKeyword( "CONJUNCTIVE-REMOVAL-EXPAND" );
    $kw45$INFERENCE_PROOF_SPEC = makeKeyword( "INFERENCE-PROOF-SPEC" );
    ...
    $kw51$SKSI = makeKeyword( "SKSI" );
    $list52 = ConsesLow.list( makeSymbol( "STORE" ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    ...
    $kw57$META_REMOVAL = makeKeyword( "META-REMOVAL" );
    ...
    $kw78$DETERMINE_NEW_REMOVAL_TACTIC_SPECS_FROM_HL_MODULES = makeKeyword( "DETERMINE-NEW-REMOVAL-TACTIC-SPECS-FROM-HL-MODULES" );
    $kw79$NOT = makeKeyword( "NOT" );
    $kw80$MODULE_SUBTYPE = makeKeyword( "MODULE-SUBTYPE" );
    ...
    $list85 = ConsesLow.list( ConsesLow.list( makeSymbol( "TACTIC" ), makeSymbol( "MT" ), makeSymbol( "SENSE" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    ...
    $kw93$REMOVAL_OUTPUT_GENERATE = makeKeyword( "REMOVAL-OUTPUT-GENERATE" );
    $sym94$HANDLE_REMOVAL_ADD_NODE_FOR_OUTPUT_GENERATE = makeSymbol( "HANDLE-REMOVAL-ADD-NODE-FOR-OUTPUT-GENERATE" );
    $kw95$MAYBE_MAKE_REMOVAL_TACTIC_EXPAND_RESULTS_PROGRESS_ITERATOR = makeKeyword( "MAYBE-MAKE-REMOVAL-TACTIC-EXPAND-RESULTS-PROGRESS-ITERATOR" );
    $sym96$HANDLE_REMOVAL_ADD_NODE_FOR_EXPAND_RESULTS = makeSymbol( "HANDLE-REMOVAL-ADD-NODE-FOR-EXPAND-RESULTS" );
    $kw97$REMOVAL_EXPAND = makeKeyword( "REMOVAL-EXPAND" );
    $list98 = ConsesLow.list( makeSymbol( "REMOVAL-BINDINGS" ), makeSymbol( "SUPPORTS" ) );
    ...
    $list102 = ConsesLow.list( ConsesLow.list( makeSymbol( "RAW-OUTPUT" ), makeSymbol( "RAW-OUTPUT-ITERATOR" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    ...
    $kw110$MAYBE_MAKE_INFERENCE_OUTPUT_ITERATOR = makeKeyword( "MAYBE-MAKE-INFERENCE-OUTPUT-ITERATOR" );
    $kw111$HANDLE_ONE_OUTPUT_GENERATE_RESULT = makeKeyword( "HANDLE-ONE-OUTPUT-GENERATE-RESULT" );
    $list112 = ConsesLow.list( makeSymbol( "&OPTIONAL" ), makeSymbol( "SUPPORT" ), makeSymbol( "&REST" ), makeSymbol( "MORE-SUPPORTS" ) );
    $kw113$TRUE = makeKeyword( "TRUE" );
    $str114$unknown_thing_to_do_in_the_HL_mod = makeString( "unknown thing to do in the HL module guts: ~s" );
  }

  public static final class $removal_link_data_native
      extends
        SubLStructNative
  {
    ...
    static
    {
      structDecl = Structures.makeStructDeclNative( $removal_link_data_native.class, $sym0$REMOVAL_LINK_DATA, $sym1$REMOVAL_LINK_DATA_P, $list2, $list3, new String[] { "$hl_module", "$bindings", "$supports"
      }, $list4, $list5, $sym6$DEFAULT_STRUCT_PRINT_FUNCTION );
    }
  }

  public static final class $removal_link_data_p$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $inference_remove_unify_default$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $inference_remove_unify_default$BinaryFunction
      extends
        BinaryFunction
  {
