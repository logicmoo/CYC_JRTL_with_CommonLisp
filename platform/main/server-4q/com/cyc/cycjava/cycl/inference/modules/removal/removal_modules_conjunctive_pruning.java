      ...
      SubLTranslatedFile
{
  ...
  {
    return makeBoolean( NIL != inference_modules.hl_module_p( hl_module ) && NIL != subl_promotions.memberP( inference_modules.hl_module_name( hl_module ), $conjunctive_pruning_module_names$.getGlobalValue(), UNPROVIDED,
        UNPROVIDED ) );
  }
  ...
  public static SubLObject supporting_residual_conjunction_problem_minimal_problem_query_int_internal(final SubLObject problem, SubLObject add_isomophic_literalsP)
  {
        ...
        && NIL == set_contents.do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state, link ) && NIL != inference_datastructures_problem_link.problem_link_has_typeP( link, $kw4$RESIDUAL_TRANSFORMATION ) )
      {
        doneP = T;
        residual_problem_query = bindings.apply_bindings_backwards( inference_worker_residual_transformation.residual_transformation_link_supporting_variable_map( link ), inference_datastructures_problem.problem_query(
            inference_worker_residual_transformation.residual_transformation_link_residual_problem( link ) ) );
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        while ( NIL != cdolist_list_var_$2)
        {
          ...
          if( NIL == current )
          {
            ...
            {
              new_problem_query_sweep_1 = ConsesLow.cons( inference_datastructures_problem_query.make_contextualized_asent( mt, asent ), new_problem_query_sweep_1 );
            }
          }
          else
          {
                  ...
                  while ( NIL != cdolist_list_var_$5)
                  {
                    ...
                    if( NIL == current_$10 )
                    {
                      if( mt2.equal( mt_sweep_1 ) && ( asent2.equal( asent_sweep_1 ) || NIL != unification.unify( asent2, asent_sweep_1, UNPROVIDED, UNPROVIDED ) ) )
                      {
                        ...
                        {
                          new_problem_query_sweep_2 = ConsesLow.cons( item_var, new_problem_query_sweep_2 );
                        }
                        doneP2 = T;
                      }
                    }
  ...
  public static SubLObject supporting_residual_conjunction_problem_minimal_problem_query_int(final SubLObject problem, SubLObject add_isomophic_literalsP)
  {
    ...
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym3$SUPPORTING_RESIDUAL_CONJUNCTION_PROBLEM_MINIMAL_PROBLEM_QUERY_INT, TWO_INTEGER, NIL,
          EQL, UNPROVIDED );
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( supporting_residual_conjunction_problem_minimal_problem_query_int_internal( problem, add_isomophic_literalsP ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( problem, add_isomophic_literalsP ) );
  ...
  public static SubLObject prune_non_wff_conjunction_applicability(final SubLObject contextualized_dnf_clause)
  {
    ...
    if( ( NIL != $prune_root_problem_of_some_inference_non_wff_conjunctionP$.getDynamicValue( thread ) && NIL != inference_worker.problem_is_the_root_problem_of_some_inferenceP( problem ) && NIL == kb_control_vars
        .within_forward_inferenceP() ) || NIL != problem_is_a_topological_mergingP( problem ) )
    {
  ...
  public static SubLObject prune_non_wff_conjunction_expand(final SubLObject contextualized_dnf_clause)
  {
    ...
    {
      inference_worker.make_problem_no_good( inference_worker.currently_active_problem(), NIL, $kw11$TACTICAL );
    }
    return NIL;
  }
  ...
  public static SubLObject problem_query_semantically_invalidP(final SubLObject problem_query)
  {
    ...
    try
    {
        ...
        while ( NIL != cdolist_list_var_$32)
        {
          ...
          if( NIL == current )
          {
            if( sense == $kw8$POS )
            {
              ...
              if( NIL == cycl_grammar.hl_variable_p( pred ) && NIL == kb_utilities.kbeq( pred, $const15$evaluate ) )
              {
                SubLObject argnum = ZERO_INTEGER;
                SubLObject cdolist_list_var_$33;
                final SubLObject args = cdolist_list_var_$33 = cycl_utilities.formula_args( asent, $kw16$IGNORE );
                SubLObject arg = NIL;
                arg = cdolist_list_var_$33.first();
                while ( NIL != cdolist_list_var_$33)
                {
                  argnum = Numbers.add( argnum, ONE_INTEGER );
                  if( !argnum.isZero() && NIL == variables.not_fully_bound_p( arg ) && NIL == kb_accessors.quoted_argumentP( pred, argnum ) && NIL == el_utilities.kappa_predicate_p( pred ) && NIL == at_admitted
                      .admitted_argumentP( arg, argnum, pred, mt_relevance_macros.conservative_constraint_mt( mt ) ) )
                  {
                    if( NIL != $gathering_problem_query_semantically_invalid_reasonsP$.getDynamicValue( thread ) )
                    {
                      $problem_query_semantically_invalid_reason$.setDynamicValue( Sequences.cconcatenate( format_nil.format_nil_a_no_copy( arg ), new SubLObject[] { $str17$_is_not_a_valid_arg, format_nil
                          .format_nil_a_no_copy( argnum ), $str18$_for_, format_nil.format_nil_a_no_copy( pred ), $str19$_in_, format_nil.format_nil_a_no_copy( mt )
                      } ), thread );
                    }
          ...
          if( NIL == current )
          {
            if( sense == $kw8$POS )
            {
              ...
              if( NIL == cycl_grammar.hl_variable_p( pred ) && NIL == kb_utilities.kbeq( pred, $const15$evaluate ) )
              {
                ...
                final SubLObject args = cdolist_list_var_$35 = cycl_utilities.formula_args( asent, $kw16$IGNORE );
                SubLObject arg = NIL;
      ...
      final SubLObject v_variables = list_utilities.tree_gather( variable_constraint_tuples, $sym5$VARIABLE_P, UNPROVIDED, UNPROVIDED, UNPROVIDED );
      ...
      if( NIL == some_variable_invalidP )
      {
        ...
        while ( NIL == some_variable_invalidP && NIL != csome_list_var)
        {
          ...
          if( NIL != variable_semantically_invalidP( tuples_for_this_var ) )
          {
            if( NIL != $gathering_problem_query_semantically_invalid_reasonsP$.getDynamicValue( thread ) )
            {
              $problem_query_semantically_invalid_reason$.setDynamicValue( Sequences.cconcatenate( format_nil.format_nil_a_no_copy( var ), new SubLObject[] { $str22$_is_constrained_to_disjoint_colle, format_nil
                  .format_nil_a_no_copy( tuples_for_this_var )
              } ), thread );
            }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != list_utilities.simple_tree_findP( $const24$rdf_type, dnf ) )
    {
      dnf = conses_high.subst( $const25$isa, $const24$rdf_type, dnf, UNPROVIDED, UNPROVIDED );
    }
  ...
  public static SubLObject dnf_variable_constraint_tuples(final SubLObject dnf)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym23$DNF_VARIABLE_CONSTRAINT_TUPLES, ONE_INTEGER, $int27$128, EQUAL, UNPROVIDED );
  ...
  {
    final SubLObject collections_set = set.new_set( Symbols.symbol_function( EQL ), UNPROVIDED );
    ...
    if( NIL != inference_worker.currently_active_problem() )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        while ( NIL != cdolist_list_var_$36)
        {
          ...
          if( NIL == current )
          {
            if( $const32$unknownSentence.eql( cycl_utilities.atomic_sentence_predicate( asent ) ) )
            {
    ...
    if( NIL != somewhere_cache.some_pred_assertion_somewhereP( $const39$applicableWhenTypedOnlyWhenSpecia, rule_assertion, THREE_INTEGER, UNPROVIDED ) )
    {
      ...
      if( NIL != awtows_el_var_col_pairs )
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          {
            SubLObject literal_sense = $kw6$NEG;
            SubLObject index_var = ZERO_INTEGER;
            ...
            while ( NIL != cdolist_list_var_$63)
            {
              ...
              if( NIL == current )
              {
                ...
                if( ( pred.eql( $const25$isa ) || pred.eql( $const24$rdf_type ) ) && arg1.eql( jo_link_non_focal_problem_hl_var ) && NIL != term.el_fort_p( arg2 ) )
                {
  ...
  {
    SubLFiles.declareFunction( me, "conjunctive_pruning_module_p", "CONJUNCTIVE-PRUNING-MODULE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "conjunctive_pruning_tactic_p", "CONJUNCTIVE-PRUNING-TACTIC-P", 1, 0, false );
    SubLFiles.declareFunction( me, "supporting_residual_conjunction_problem_minimal_problem_query", "SUPPORTING-RESIDUAL-CONJUNCTION-PROBLEM-MINIMAL-PROBLEM-QUERY", 1, 1, false );
    SubLFiles.declareFunction( me, "supporting_residual_conjunction_problem_minimal_problem_query_int_internal", "SUPPORTING-RESIDUAL-CONJUNCTION-PROBLEM-MINIMAL-PROBLEM-QUERY-INT-INTERNAL", 1, 1, false );
    SubLFiles.declareFunction( me, "supporting_residual_conjunction_problem_minimal_problem_query_int", "SUPPORTING-RESIDUAL-CONJUNCTION-PROBLEM-MINIMAL-PROBLEM-QUERY-INT", 1, 1, false );
    SubLFiles.declareFunction( me, "prune_non_wff_conjunction_applicability", "PRUNE-NON-WFF-CONJUNCTION-APPLICABILITY", 1, 0, false );
    new $prune_non_wff_conjunction_applicability$UnaryFunction();
    SubLFiles.declareFunction( me, "prune_non_wff_conjunction_expand", "PRUNE-NON-WFF-CONJUNCTION-EXPAND", 1, 0, false );
    SubLFiles.declareFunction( me, "problem_is_a_topological_mergingP", "PROBLEM-IS-A-TOPOLOGICAL-MERGING?", 1, 0, false );
    SubLFiles.declareFunction( me, "why_problem_query_semantically_invalid", "WHY-PROBLEM-QUERY-SEMANTICALLY-INVALID", 1, 0, false );
    SubLFiles.declareFunction( me, "problem_query_semantically_invalidP", "PROBLEM-QUERY-SEMANTICALLY-INVALID?", 1, 0, false );
    SubLFiles.declareFunction( me, "problem_query_variable_constraint_tuples", "PROBLEM-QUERY-VARIABLE-CONSTRAINT-TUPLES", 1, 0, false );
    SubLFiles.declareFunction( me, "dnf_variable_constraint_tuples_internal", "DNF-VARIABLE-CONSTRAINT-TUPLES-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "dnf_variable_constraint_tuples", "DNF-VARIABLE-CONSTRAINT-TUPLES", 1, 0, false );
    SubLFiles.declareFunction( me, "variable_constraint_tuples_for_var", "VARIABLE-CONSTRAINT-TUPLES-FOR-VAR", 2, 0, false );
    SubLFiles.declareFunction( me, "variable_semantically_invalidP", "VARIABLE-SEMANTICALLY-INVALID?", 1, 0, false );
    SubLFiles.declareFunction( me, "thing_tupleP", "THING-TUPLE?", 1, 0, false );
    new $thing_tupleP$UnaryFunction();
    SubLFiles.declareFunction( me, "prune_unknown_sentence_literal_inconsistency_applicability", "PRUNE-UNKNOWN-SENTENCE-LITERAL-INCONSISTENCY-APPLICABILITY", 1, 0, false );
    new $prune_unknown_sentence_literal_inconsistency_applicability$UnaryFunction();
    SubLFiles.declareFunction( me, "prune_unknown_sentence_literal_inconsistency_expand", "PRUNE-UNKNOWN-SENTENCE-LITERAL-INCONSISTENCY-EXPAND", 1, 0, false );
    SubLFiles.declareFunction( me, "prune_rt_problems_applicable_when_typed_only_when_specialization_applicability", "PRUNE-RT-PROBLEMS-APPLICABLE-WHEN-TYPED-ONLY-WHEN-SPECIALIZATION-APPLICABILITY", 1, 0, false );
    SubLFiles.declareFunction( me, "transformation_non_applicable_due_to_rule_type_contraint_meta_assertionP", "TRANSFORMATION-NON-APPLICABLE-DUE-TO-RULE-TYPE-CONTRAINT-META-ASSERTION?", 2, 0, false );
    SubLFiles.declareFunction( me, "prune_rt_problems_applicable_when_typed_only_when_specialization_expand", "PRUNE-RT-PROBLEMS-APPLICABLE-WHEN-TYPED-ONLY-WHEN-SPECIALIZATION-EXPAND", 1, 0, false );
    SubLFiles.declareFunction( me, "prune_circular_term_of_unit_applicability", "PRUNE-CIRCULAR-TERM-OF-UNIT-APPLICABILITY", 1, 0, false );
    SubLFiles.declareFunction( me, "prune_circular_term_of_unit_expand", "PRUNE-CIRCULAR-TERM-OF-UNIT-EXPAND", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_removal_modules_conjunctive_pruning_file()
  {
    $conjunctive_pruning_module_names$ = SubLFiles.deflexical( "*CONJUNCTIVE-PRUNING-MODULE-NAMES*", $list0 );
    ...
    $problem_query_semantically_invalid_reason$ = SubLFiles.defparameter( "*PROBLEM-QUERY-SEMANTICALLY-INVALID-REASON*", $kw13$UNINITIALIZED );
  ...
  {
    inference_modules.inference_conjunctive_removal_module( $kw1$PRUNE_NON_WFF_CONJUNCTION, $list2 );
    ...
    inference_modules.inference_conjunctive_removal_module( $kw30$PRUNE_UNKNOWN_SENTENCE_LITERAL_INCONSISTENCY, $list31 );
    ...
    inference_modules.inference_conjunctive_removal_module( $kw36$PRUNE_RT_PROBLEMS_APPLICABLE_WHEN_TYPED_ONLY_WHEN_SPECIALIZATION, $list37 );
    ...
    inference_modules.inference_conjunctive_removal_module( $kw42$PRUNE_CIRCULAR_TERM_OF_UNIT, $list43 );
  ...
  static
  {
    ...
    $list0 = ConsesLow.list( makeKeyword( "PRUNE-NON-WFF-CONJUNCTION" ), makeKeyword( "PRUNE-UNKNOWN-SENTENCE-LITERAL-INCONSISTENCY" ), makeKeyword( "PRUNE-RT-PROBLEMS-APPLICABLE-WHEN-TYPED-ONLY-WHEN-SPECIALIZATION" ),
        makeKeyword( "PRUNE-CIRCULAR-TERM-OF-UNIT" ) );
    $kw1$PRUNE_NON_WFF_CONJUNCTION = makeKeyword( "PRUNE-NON-WFF-CONJUNCTION" );
    $list2 = ConsesLow.list( new SubLObject[] { makeKeyword( "EVERY-PREDICATES" ), NIL, makeKeyword( "APPLICABILITY" ), makeSymbol( "PRUNE-NON-WFF-CONJUNCTION-APPLICABILITY" ), makeKeyword( "COMPLETENESS" ), makeKeyword(
        "COMPLETE" ), makeKeyword( "COST-EXPRESSION" ), ZERO_INTEGER, makeKeyword( "EXPAND" ), makeSymbol( "PRUNE-NON-WFF-CONJUNCTION-EXPAND" ), makeKeyword( "DOCUMENTATION" ), makeString(
            "(#$and <lit0> ... <litN>) \n    which has a dependent residual transformation link\n    and is non-wff" ), makeKeyword( "EXAMPLE" ), makeString(
                "(#$and\n      (#$isa ?AGENT #$City)\n      (#$spouse ?AGENT ?SPOUSE))" )
    } );
    ...
    $kw4$RESIDUAL_TRANSFORMATION = makeKeyword( "RESIDUAL-TRANSFORMATION" );
    $sym5$VARIABLE_P = makeSymbol( "VARIABLE-P" );
    $kw6$NEG = makeKeyword( "NEG" );
    $list7 = ConsesLow.list( makeSymbol( "MT" ), makeSymbol( "ASENT" ) );
    $kw8$POS = makeKeyword( "POS" );
    $list9 = ConsesLow.list( makeSymbol( "MT-SWEEP-1" ), makeSymbol( "ASENT-SWEEP-1" ) );
    ...
    $const15$evaluate = constant_handles.reader_make_constant_shell( makeString( "evaluate" ) );
    ...
    $const24$rdf_type = constant_handles.reader_make_constant_shell( makeString( "rdf-type" ) );
    $const25$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $sym26$THING_TUPLE_ = makeSymbol( "THING-TUPLE?" );
    $int27$128 = makeInteger( 128 );
    $list28 = ConsesLow.list( makeSymbol( "VAR" ), makeSymbol( "MT" ), makeSymbol( "COLLECTIONS" ) );
    $list29 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "Thing" ) ) );
    $kw30$PRUNE_UNKNOWN_SENTENCE_LITERAL_INCONSISTENCY = makeKeyword( "PRUNE-UNKNOWN-SENTENCE-LITERAL-INCONSISTENCY" );
    $list31 = ConsesLow.list( new SubLObject[] { makeKeyword( "EVERY-PREDICATES" ), NIL, makeKeyword( "APPLICABILITY" ), makeSymbol( "PRUNE-UNKNOWN-SENTENCE-LITERAL-INCONSISTENCY-APPLICABILITY" ), makeKeyword(
        "COMPLETENESS" ), makeKeyword( "COMPLETE" ), makeKeyword( "EXCLUSIVE" ), makeSymbol( "TRUE" ), makeKeyword( "COST-EXPRESSION" ), ZERO_INTEGER, makeKeyword( "EXPAND" ), makeSymbol(
            "PRUNE-UNKNOWN-SENTENCE-LITERAL-INCONSISTENCY-EXPAND" ), makeKeyword( "DOCUMENTATION" ), makeString( "(#$and <lit0> ... <litN> ... (#$unknownSentence <litN>) ...)" ), makeKeyword( "EXAMPLE" ), makeString(
                "(#$and\n      (#$children ?ANIMAL ?CHILD)\n      (#$unknownSentence (#$children ?ANIMAL ?CHILD)))" )
    } );
    $const32$unknownSentence = constant_handles.reader_make_constant_shell( makeString( "unknownSentence" ) );
    $list33 = ConsesLow.list( makeSymbol( "MT2" ), makeSymbol( "ASENT2" ) );
    ...
    $kw36$PRUNE_RT_PROBLEMS_APPLICABLE_WHEN_TYPED_ONLY_WHEN_SPECIALIZATION = makeKeyword( "PRUNE-RT-PROBLEMS-APPLICABLE-WHEN-TYPED-ONLY-WHEN-SPECIALIZATION" );
    $list37 = ConsesLow.list( new SubLObject[] { makeKeyword( "EVERY-PREDICATES" ), NIL, makeKeyword( "APPLICABILITY" ), makeSymbol( "PRUNE-RT-PROBLEMS-APPLICABLE-WHEN-TYPED-ONLY-WHEN-SPECIALIZATION-APPLICABILITY" ),
      makeKeyword( "COMPLETENESS" ), makeKeyword( "COMPLETE" ), makeKeyword( "EXCLUSIVE" ), makeSymbol( "TRUE" ), makeKeyword( "COST-EXPRESSION" ), ZERO_INTEGER, makeKeyword( "EXPAND" ), makeSymbol(
          "PRUNE-RT-PROBLEMS-APPLICABLE-WHEN-TYPED-ONLY-WHEN-SPECIALIZATION-EXPAND" ), makeKeyword( "DOCUMENTATION" ), makeString(
              "Apply to problems created via residual transformation where the rule has a #$applicableWhenTypedOnlyWhenSpecialization assertion on it." )
    } );
    ...
    $const39$applicableWhenTypedOnlyWhenSpecia = constant_handles.reader_make_constant_shell( makeString( "applicableWhenTypedOnlyWhenSpecialization" ) );
    $list40 = ConsesLow.list( ONE_INTEGER, TWO_INTEGER );
    ...
    $kw42$PRUNE_CIRCULAR_TERM_OF_UNIT = makeKeyword( "PRUNE-CIRCULAR-TERM-OF-UNIT" );
    $list43 = ConsesLow.list( new SubLObject[] { makeKeyword( "EVERY-PREDICATES" ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "termOfUnit" ) ) ), makeKeyword( "APPLICABILITY" ), makeSymbol(
        "PRUNE-CIRCULAR-TERM-OF-UNIT-APPLICABILITY" ), makeKeyword( "COMPLETENESS" ), makeKeyword( "COMPLETE" ), makeKeyword( "EXCLUSIVE" ), makeSymbol( "TRUE" ), makeKeyword( "COST-EXPRESSION" ), ZERO_INTEGER,
      makeKeyword( "EXPAND" ), makeSymbol( "PRUNE-CIRCULAR-TERM-OF-UNIT-EXPAND" ), makeKeyword( "DOCUMENTATION" ), makeString(
          "Applies to syntactically circular termOfUnit literals, for instance (#$termOfUnit ?var0 (#$MotherFn ?var0))" )
    } );
  ...
  public static final class $prune_non_wff_conjunction_applicability$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $thing_tupleP$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $prune_unknown_sentence_literal_inconsistency_applicability$UnaryFunction
      extends
        UnaryFunction
  {
