...
public final class cb_show_consequences
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject cb_link_show_consequences(final SubLObject gaf, SubLObject rules, SubLObject linktext)
  {
    ...
    final SubLObject frame_name_var = cb_utilities.cb_frame_name( $kw4$SELF );
    html_utilities.html_markup( html_macros.$html_anchor_head$.getGlobalValue() );
    html_utilities.html_markup( html_macros.$html_anchor_href$.getGlobalValue() );
    html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
    cb_parameters.cyc_cgi_url_int();
    PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $str5$cb_show_consequences__a__a, assertion_handles.assertion_id( gaf ), cb_parameters.cb_glob_id( rules ) );
    html_utilities.html_char( Characters.CHAR_quotation, UNPROVIDED );
  ...
  public static SubLObject cb_show_consequences(final SubLObject args)
  {
    ...
    if( NIL == current )
    {
      ...
      final SubLObject title_var = $str9$Show_Consequences;
      ...
      try
      {
        ...
        try
        {
          cyc_file_dependencies.$html_emitted_file_dependencies$.bind( ( cyc_file_dependencies.$html_emitted_file_dependencies$.getDynamicValue( thread ) == $kw12$UNINITIALIZED ) ? ConsesLow.list(
          ...
          try
          {
            ...
            try
            {
              ...
              PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $str22$__considering__a_rules___, ( $kw2$ALL == rules ) ? $str23$all : Sequences.length( rules ) );
              html_utilities.html_newline( UNPROVIDED );
              streams_high.force_output( html_macros.$html_stream$.getDynamicValue( thread ) );
              html_utilities.html_hr( UNPROVIDED, UNPROVIDED );
              thread.resetMultipleValues();
              final SubLObject queryables = forward_propagate_assertions.generate_consequences( gaf, rules, $const24$InferencePSC, $cb_show_consequences_max_time$.getDynamicValue( thread ) );
              final SubLObject halt_reason = thread.secondMultipleValue();
              thread.resetMultipleValues();
              if( $kw25$MAX_TIME == halt_reason )
              {
                PrintLow.format( html_macros.$html_stream$.getDynamicValue( thread ), $str26$Timed_out_after__a_second_P_, $cb_show_consequences_max_time$.getDynamicValue( thread ), $cb_show_consequences_max_time$
  ...
  public static SubLObject cb_display_consequence(final SubLObject queryable)
  {
    ...
    if( NIL == current )
    {
      ...
      cb_utilities.cb_link( $kw31$SENTENCE_QUERY_SIMILAR, ist_sentence, $str32$_Query_, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $show_consequences_rules_from_mt_caching_state$.getGlobalValue(), ConsesLow.list( mt ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject show_consequences_rules_from_mt_internal(final SubLObject mt)
  {
    ...
    final SubLObject rule_set = set.new_set( Symbols.symbol_function( EQL ), UNPROVIDED );
    final SubLObject pcase_var = kb_mapping_macros.do_mt_contents_method( mt );
    if( pcase_var.eql( $kw36$MT ) )
    {
    ...
    else if( pcase_var.eql( $kw37$BROAD_MT ) && NIL != kb_mapping_macros.do_broad_mt_index_key_validator( mt, NIL ) )
    {
  ...
  public static SubLObject show_consequences_rules_from_mt(final SubLObject mt)
  {
    ...
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym34$SHOW_CONSEQUENCES_RULES_FROM_MT, $sym41$_SHOW_CONSEQUENCES_RULES_FROM_MT_CACHING_STATE_, NIL, EQL, ONE_INTEGER, ZERO_INTEGER );
    }
    SubLObject results = memoization_state.caching_state_lookup( caching_state, mt, memoization_state.$memoized_item_not_found$.getGlobalValue() );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( show_consequences_rules_from_mt_internal( mt ) ) );
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $biology_practice_rules_caching_state$.getGlobalValue(), ConsesLow.list( EMPTY_SUBL_OBJECT_ARRAY ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject biology_practice_rules_internal()
  {
    ...
    try
    {
      ...
      final SubLObject pred_var = $const45$queryPracticeRules;
  ...
  public static SubLObject biology_practice_rules()
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym42$BIOLOGY_PRACTICE_RULES, $sym48$_BIOLOGY_PRACTICE_RULES_CACHING_STATE_, NIL, EQL, ZERO_INTEGER, ZERO_INTEGER );
    }
    SubLObject results = memoization_state.caching_state_get_zero_arg_results( caching_state, UNPROVIDED );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( biology_practice_rules_internal() ) );
  ...
  {
    SubLFiles.declareFunction( me, "cb_link_show_consequences", "CB-LINK-SHOW-CONSEQUENCES", 1, 2, false );
    SubLFiles.declareFunction( me, "cb_show_consequences", "CB-SHOW-CONSEQUENCES", 1, 0, false );
    SubLFiles.declareFunction( me, "cb_display_consequence", "CB-DISPLAY-CONSEQUENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_show_consequences_rules_from_mt", "CLEAR-SHOW-CONSEQUENCES-RULES-FROM-MT", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_show_consequences_rules_from_mt", "REMOVE-SHOW-CONSEQUENCES-RULES-FROM-MT", 1, 0, false );
    SubLFiles.declareFunction( me, "show_consequences_rules_from_mt_internal", "SHOW-CONSEQUENCES-RULES-FROM-MT-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "show_consequences_rules_from_mt", "SHOW-CONSEQUENCES-RULES-FROM-MT", 1, 0, false );
    SubLFiles.declareFunction( me, "clear_biology_practice_rules", "CLEAR-BIOLOGY-PRACTICE-RULES", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_biology_practice_rules", "REMOVE-BIOLOGY-PRACTICE-RULES", 0, 0, false );
    SubLFiles.declareFunction( me, "biology_practice_rules_internal", "BIOLOGY-PRACTICE-RULES-INTERNAL", 0, 0, false );
    SubLFiles.declareFunction( me, "biology_practice_rules", "BIOLOGY-PRACTICE-RULES", 0, 0, false );
    return NIL;
  }

  public static SubLObject init_cb_show_consequences_file()
  {
    $cb_show_consequences_link_enabledP$ = SubLFiles.defparameter( "*CB-SHOW-CONSEQUENCES-LINK-ENABLED?*", NIL );
    $cb_show_consequences_mt$ = SubLFiles.defparameter( "*CB-SHOW-CONSEQUENCES-MT*", $const0$BiologyMt );
    $cb_show_consequences_max_time$ = SubLFiles.defparameter( "*CB-SHOW-CONSEQUENCES-MAX-TIME*", $int1$30 );
    $show_consequences_rules_from_mt_caching_state$ = SubLFiles.deflexical( "*SHOW-CONSEQUENCES-RULES-FROM-MT-CACHING-STATE*", NIL );
    $biology_practice_rules_caching_state$ = SubLFiles.deflexical( "*BIOLOGY-PRACTICE-RULES-CACHING-STATE*", NIL );
    return NIL;
  }

  public static SubLObject setup_cb_show_consequences_file()
  {
    cb_utilities.setup_cb_link_method( $kw6$SHOW_CONSEQUENCES, $sym7$CB_LINK_SHOW_CONSEQUENCES, THREE_INTEGER );
    html_macros.note_cgi_handler_function( $sym27$CB_SHOW_CONSEQUENCES, $kw28$HTML_HANDLER );
  ...
  static
  {
    ...
    $kw4$SELF = makeKeyword( "SELF" );
    $str5$cb_show_consequences__a__a = makeString( "cb-show-consequences&~a&~a" );
    $kw6$SHOW_CONSEQUENCES = makeKeyword( "SHOW-CONSEQUENCES" );
    $sym7$CB_LINK_SHOW_CONSEQUENCES = makeSymbol( "CB-LINK-SHOW-CONSEQUENCES" );
    $list8 = ConsesLow.list( makeSymbol( "ASSERTION-ID" ), makeSymbol( "RULES-ID" ) );
    ...
    $kw28$HTML_HANDLER = makeKeyword( "HTML-HANDLER" );
    $list29 = ConsesLow.list( makeSymbol( "QUERY-EL-FORMULA" ), makeSymbol( "MT" ), makeSymbol( "JUSTIFICATION" ) );
    $str30$In_Mt__ = makeString( "In Mt: " );
    $kw31$SENTENCE_QUERY_SIMILAR = makeKeyword( "SENTENCE-QUERY-SIMILAR" );
    ...
    $kw37$BROAD_MT = makeKeyword( "BROAD-MT" );
    $str38$do_broad_mt_index = makeString( "do-broad-mt-index" );
    $sym39$STRINGP = makeSymbol( "STRINGP" );
    $kw40$SKIP = makeKeyword( "SKIP" );
    $sym41$_SHOW_CONSEQUENCES_RULES_FROM_MT_CACHING_STATE_ = makeSymbol( "*SHOW-CONSEQUENCES-RULES-FROM-MT-CACHING-STATE*" );
    ...
    $const47$CytologicalProcessProjectKBConten = constant_handles.reader_make_constant_shell( makeString( "CytologicalProcessProjectKBContentTestQuery" ) );
