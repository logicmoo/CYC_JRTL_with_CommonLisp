...
public final class removal_modules_rule_cascade
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject removal_first_implied_dependent_of_sentence_check_expand(final SubLObject asent, SubLObject sense)
  {
    ...
    {
      format_nil.force_format( T, $str4$first_implied_dependent_of_senten, Sequences.length( rules ), rule_cascade, asent, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
      ...
      try
      {
        ...
        try
        {
          if( NIL == result_bindings )
          {
            ...
            {
              final SubLObject assertibles_queue = forward.forward_propagate_one_support_wrt_rule_and_generate_assertibles_cached( arguments.make_hl_support( $kw6$OPAQUE, sentence, asent_mt, UNPROVIDED ), rule );
              SubLObject rest;
    ...
    if( NIL != result_bindings )
    {
      ...
      backward.removal_add_node( arguments.make_hl_support( $kw6$OPAQUE, el_utilities.make_ternary_formula( $const0$firstImpliedDependentOfSentence, sentence, result_asent, rule_cascade ), UNPROVIDED, UNPROVIDED ),
            ...
            try
            {
              ...
              while ( NIL == done_var_$5)
              {
                ...
                if( NIL != valid_$7 )
                {
                  ...
                  if( NIL == conses_high.member( item_var, $cascade_rule_parent_rules_for_sort$.getDynamicValue( thread ), Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
                  {
                    $cascade_rule_parent_rules_for_sort$.setDynamicValue( ConsesLow.cons( item_var, $cascade_rule_parent_rules_for_sort$.getDynamicValue( thread ) ), thread );
                  }
                }
            ...
            try
            {
              ...
              while ( NIL == done_var_$6)
              {
                ...
                if( NIL != valid_$8 )
                {
                  ...
                  item_var = assertions_high.gaf_arg1( gaf );
                  if( NIL == conses_high.member( item_var, $cascade_rule_terminal_rules_for_sort$.getDynamicValue( thread ), Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
                  {
                    $cascade_rule_terminal_rules_for_sort$.setDynamicValue( ConsesLow.cons( item_var, $cascade_rule_terminal_rules_for_sort$.getDynamicValue( thread ) ), thread );
                  }
                }
            ...
            try
            {
              ...
              while ( NIL == done_var_$7)
              {
                ...
                if( NIL != valid_$9 )
                {
                  ...
                  item_var = ConsesLow.list( assertions_high.gaf_arg2( gaf ), assertions_high.gaf_arg3( gaf ) );
                  if( NIL == conses_high.member( item_var, $cascade_rule_ordering_rule_pairs_for_sort$.getDynamicValue( thread ), Symbols.symbol_function( EQL ), Symbols.symbol_function( IDENTITY ) ) )
                  {
                    $cascade_rule_ordering_rule_pairs_for_sort$.setDynamicValue( ConsesLow.cons( item_var, $cascade_rule_ordering_rule_pairs_for_sort$.getDynamicValue( thread ) ), thread );
                  }
                }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != subl_promotions.memberP( ConsesLow.list( rule1, rule2 ), $cascade_rule_ordering_rule_pairs_for_sort$.getDynamicValue( thread ), Symbols.symbol_function( EQUAL ), UNPROVIDED ) )
    {
      return T;
    }
    if( NIL != subl_promotions.memberP( ConsesLow.list( rule2, rule1 ), $cascade_rule_ordering_rule_pairs_for_sort$.getDynamicValue( thread ), Symbols.symbol_function( EQUAL ), UNPROVIDED ) )
    {
      return NIL;
    }
    if( NIL != subl_promotions.memberP( rule1, $cascade_rule_parent_rules_for_sort$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) && NIL == subl_promotions.memberP( rule2, $cascade_rule_parent_rules_for_sort$
        .getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) )
    {
      return T;
    }
    if( NIL != subl_promotions.memberP( rule2, $cascade_rule_parent_rules_for_sort$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) && NIL == subl_promotions.memberP( rule1, $cascade_rule_parent_rules_for_sort$
        .getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) )
    {
      return NIL;
    }
    if( NIL != subl_promotions.memberP( rule1, $cascade_rule_terminal_rules_for_sort$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) && NIL == subl_promotions.memberP( rule2, $cascade_rule_terminal_rules_for_sort$
        .getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) )
    {
      return NIL;
    }
    if( NIL != subl_promotions.memberP( rule2, $cascade_rule_terminal_rules_for_sort$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) && NIL == subl_promotions.memberP( rule1, $cascade_rule_terminal_rules_for_sort$
        .getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) )
    {
      return T;
    }
  ...
  {
    SubLFiles.declareFunction( me, "removal_first_implied_dependent_of_sentence_check_expand", "REMOVAL-FIRST-IMPLIED-DEPENDENT-OF-SENTENCE-CHECK-EXPAND", 1, 1, false );
    SubLFiles.declareFunction( me, "sorted_rules_in_cascade", "SORTED-RULES-IN-CASCADE", 1, 0, false );
    SubLFiles.declareFunction( me, "cascade_rule_L", "CASCADE-RULE-<", 2, 0, false );
    return NIL;
  }

  public static SubLObject init_removal_modules_rule_cascade_file()
  {
    $default_first_implied_dependent_of_sentence_check_cost$ = SubLFiles.defparameter( "*DEFAULT-FIRST-IMPLIED-DEPENDENT-OF-SENTENCE-CHECK-COST*", control_vars.$expensive_hl_module_check_cost$.getGlobalValue() );
    $debug_first_implied_dependent_of_sentenceP$ = SubLFiles.defparameter( "*DEBUG-FIRST-IMPLIED-DEPENDENT-OF-SENTENCE?*", NIL );
    $cascade_rule_ordering_rule_pairs_for_sort$ = SubLFiles.defparameter( "*CASCADE-RULE-ORDERING-RULE-PAIRS-FOR-SORT*", NIL );
    $cascade_rule_parent_rules_for_sort$ = SubLFiles.defparameter( "*CASCADE-RULE-PARENT-RULES-FOR-SORT*", NIL );
    $cascade_rule_terminal_rules_for_sort$ = SubLFiles.defparameter( "*CASCADE-RULE-TERMINAL-RULES-FOR-SORT*", NIL );
    return NIL;
  }
  ...
  public static SubLObject setup_removal_modules_rule_cascade_file()
  {
    ...
    preference_modules.doomed_unless_arg_bindable( $kw1$POS, $const0$firstImpliedDependentOfSentence, ONE_INTEGER );
    preference_modules.doomed_unless_arg_bindable( $kw1$POS, $const0$firstImpliedDependentOfSentence, THREE_INTEGER );
    inference_modules.inference_removal_module( $kw2$REMOVAL_FIRST_IMPLIED_DEPENDENT_OF_SENTENCE_CHECK, $list3 );
    return NIL;
  }
  ...
  static
  {
    ...
    $const0$firstImpliedDependentOfSentence = constant_handles.reader_make_constant_shell( makeString( "firstImpliedDependentOfSentence" ) );
    $kw1$POS = makeKeyword( "POS" );
    $kw2$REMOVAL_FIRST_IMPLIED_DEPENDENT_OF_SENTENCE_CHECK = makeKeyword( "REMOVAL-FIRST-IMPLIED-DEPENDENT-OF-SENTENCE-CHECK" );
    $list3 = ConsesLow.list( new SubLObject[] { makeKeyword( "SENSE" ), makeKeyword( "POS" ), makeKeyword( "PREDICATE" ), constant_handles.reader_make_constant_shell( makeString( "firstImpliedDependentOfSentence" ) ),
      makeKeyword( "REQUIRED-PATTERN" ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "firstImpliedDependentOfSentence" ) ), makeKeyword( "FULLY-BOUND" ), makeKeyword( "ANYTHING" ),
          makeKeyword( "FULLY-BOUND" ) ), makeKeyword( "COST-EXPRESSION" ), makeSymbol( "*DEFAULT-FIRST-IMPLIED-DEPENDENT-OF-SENTENCE-CHECK-COST*" ), makeKeyword( "COMPLETENESS" ), makeKeyword( "COMPLETE" ), makeKeyword(
              "EXPAND" ), makeSymbol( "REMOVAL-FIRST-IMPLIED-DEPENDENT-OF-SENTENCE-CHECK-EXPAND" ), makeKeyword( "DOCUMENTATION" ), makeString(
      ...
      makeKeyword( "EXAMPLE" ), makeString( "(#$firstImpliedDependentOfSentence (#$stepObject (#$TheList 5 4 3 2 1)) ?RESULT #$ListSortingDemoRuleCascade)" )
    } );
    $str4$first_implied_dependent_of_senten = makeString( "first-implied-dependent-of-sentence has ~A rules in cascade ~S for ~S~%" );
    ...
    $const10$terminalNodeOfCascade = constant_handles.reader_make_constant_shell( makeString( "terminalNodeOfCascade" ) );
    $const11$successorRulesInCascade = constant_handles.reader_make_constant_shell( makeString( "successorRulesInCascade" ) );
