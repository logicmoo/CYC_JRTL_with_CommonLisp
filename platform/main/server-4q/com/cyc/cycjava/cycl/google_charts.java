...
public final class google_charts extends SubLTranslatedFile {
	...
	public static SubLObject google_property_simplify_wrt_prefix_internal(final SubLObject property, final SubLObject prefix_symbol) {
		...
		if (ZERO_INTEGER.eql(prefix_start)) {
			...
			return Packages.intern(simpler_name, reader.$keyword_package$.getGlobalValue());
		}
		return property;
	}
	...
	public static SubLObject inference_answer_timeline_google_chart(final SubLObject inference, SubLObject chart_properties) {
		...
		final SubLObject title = Sequences.cconcatenate($str70$Inference_, new SubLObject[] { format_nil.format_nil_a_no_copy(store_suid), $str71$_, format_nil.format_nil_a_no_copy(inference_suid), $str72$_Answer_Timeline });
		...
		if (NIL != use_millisecondsP) {
			tuples = Mapping.mapcar(Symbols.symbol_function($sym78$LIST), Mapping.mapcar(Symbols.symbol_function($sym79$_), Mapping.mapcar(Symbols.symbol_function($sym80$FIRST), tuples), new SubLObject[] { ConsesLow.make_list(Sequences.length(tuples), $int69$1000) }), new SubLObject[] { Mapping.mapcar(Symbols.symbol_function($sym81$SECOND), tuples) });
		}
		return ConsesLow.list(new SubLObject[] { $kw1$CHART_TYPE, $kw2$SCATTER_CHART, $kw82$TITLE, title, $kw23$WIDTH, width, $kw25$HEIGHT, height, $kw83$H_AXIS_TITLE, h_axis_title, $kw84$H_AXIS_MIN_VALUE, ZERO_INTEGER, $kw85$H_AXIS_MAX_VALUE, last_time, $kw86$V_AXIS_TITLE, v_axis_title, $kw87$V_AXIS_MIN_VALUE, ZERO_INTEGER, $kw88$V_AXIS_MAX_VALUE, last_answer, $kw89$LEGEND_POSITION, $kw90$NONE, $kw44$DATA_COLUMNS,
				ConsesLow.list(h_axis_title, $str91$Time__Answer), $kw45$DATA_ROWS, tuples });
	}
