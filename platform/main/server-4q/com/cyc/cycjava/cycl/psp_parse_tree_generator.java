...
public final class psp_parse_tree_generator
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject psp_generate_parse_tree_for_chart(final SubLObject chart, SubLObject category)
  {
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          final SubLObject pos_pred = $kw0$ANY;
    ...
    parsing_utilities.assert_parse_tree_info( ConsesLow.list( $const1$parseTreeOfTokenization, v_parse_tree, tokenization ), mt );
    return v_parse_tree;
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    assert NIL != psp_chart.phrase_structure_parser_chart_p( parsing_vars.$psp_chart$.getDynamicValue( thread ) ) : parsing_vars.$psp_chart$.getDynamicValue( thread );
  ...
  {
    assert NIL != psp_edge_equivalence_set_p( v_object ) : v_object;
  ...
  public static SubLObject make_psp_edge_equivalence_set(SubLObject arglist)
  {
    ...
    for( next = NIL, next = arglist; NIL != next; next = conses_high.cddr( next ) )
    {
      ...
      if( pcase_var.eql( $kw18$CHOICE_DTR_NUM ) )
      {
        _csetf_psp_equivalence_set_choice_dtr_num( v_new, current_value );
      }
      else if( pcase_var.eql( $kw19$EDGES ) )
      {
  ...
  {
    Functions.funcall( visitor_fn, obj, $kw21$BEGIN, $sym22$MAKE_PSP_EDGE_EQUIVALENCE_SET, TWO_INTEGER );
    Functions.funcall( visitor_fn, obj, $kw23$SLOT, $kw18$CHOICE_DTR_NUM, psp_equivalence_set_choice_dtr_num( obj ) );
    Functions.funcall( visitor_fn, obj, $kw23$SLOT, $kw19$EDGES, psp_equivalence_set_edges( obj ) );
    Functions.funcall( visitor_fn, obj, $kw24$END, $sym22$MAKE_PSP_EDGE_EQUIVALENCE_SET, TWO_INTEGER );
    return obj;
  }
  ...
  public static SubLObject pprint_psp_equivalence_set(final SubLObject equivalence_set, SubLObject stream, SubLObject depth)
  {
    ...
    return PrintLow.format( stream, $str26$_PSP_EQUIV_SET_of__S_edges_, set.set_size( psp_equivalence_set_edges( equivalence_set ) ) );
  }
  ...
  {
    return make_psp_edge_equivalence_set( ConsesLow.list( $kw18$CHOICE_DTR_NUM, NIL, $kw19$EDGES, set.new_set( UNPROVIDED, UNPROVIDED ) ) );
  }
  ...
  public static SubLObject psp_equivalence_sets_for_edges(final SubLObject edges)
  {
        ...
        .do_set_contents_doneP( basis_object, state ); state = set_contents.do_set_contents_update_state( state ) )
    {
      ...
      if( NIL != set_contents.do_set_contents_element_validP( state, edge ) )
      {
        ...
        for( basis_object_$5 = set_contents.do_set_contents_basis_object( set_contents_var_$4 ), state_$6 = NIL, state_$6 = set_contents.do_set_contents_initial_state( basis_object_$5,
            set_contents_var_$4 ); NIL == foundP && NIL == set_contents.do_set_contents_doneP( basis_object_$5, state_$6 ); state_$6 = set_contents.do_set_contents_update_state( state_$6 ) )
        {
  ...
  public static SubLObject psp_edges_choice_node_equivalentP(final SubLObject edge1, final SubLObject edge2, SubLObject choice_dtr_num)
  {
    ...
    if( psp_chart.get_psp_edge_category( edge1 ).equal( psp_chart.get_psp_edge_category( edge2 ) ) && psp_chart.get_psp_edge_rule( edge1 ).eql( psp_chart.get_psp_edge_rule( edge2 ) ) && NIL != list_utilities
        .same_length_p( psp_chart.get_psp_edge_non_head_dtrs( edge1 ), psp_chart.get_psp_edge_non_head_dtrs( edge2 ) ) )
    {
      ...
      dtr = list_var.first();
      for( i = ZERO_INTEGER; NIL == mismatchP && NIL != list_var; list_var = list_var.rest(), dtr = list_var.first(), i = Numbers.add( ONE_INTEGER, i ) )
      {
  ...
  public static SubLObject psp_create_node_for_equivalence_set(final SubLObject equivalence_set)
  {
    ...
    dtr = list_var.first();
    for( i = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), dtr = list_var.first(), i = Numbers.add( ONE_INTEGER, i ) )
    {
      final SubLObject dtr_num = number_utilities.f_1X( i );
  ...
  public static SubLObject with_psp_parse_tree_assert_parameters(final SubLObject macroform, final SubLObject environment)
  {
    ...
    return ConsesLow.list( $sym29$CLET, $list30, ConsesLow.list( $sym31$WITH_PARSE_TREE_MT, $list32, ConsesLow.listS( $sym33$WITH_PARSE_TREE_PREFIX, $str34$PSP, ConsesLow.append( body, NIL ) ) ) );
  }
  ...
  public static SubLObject psp_create_parse_tree_with_root_node(final SubLObject root_node)
  {
    ...
    try
    {
      ...
      parsing_vars.$parse_tree_prefix$.bind( $str34$PSP, thread );
  ...
  {
    final SubLObject constant = constants_high.find_constant( $str35$TheCycPhraseStructureParser );
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject existing = ask_utilities.ask_variable( $sym37$_DTR_NODE, ConsesLow.listS( $const38$syntacticNodeNthDaughter, node, dtr_num, $list39 ), psp_chart_parse_tree_mt( UNPROVIDED ), UNPROVIDED, UNPROVIDED,
  ...
  public static SubLObject psp_chart_tokenization(SubLObject chart)
  {
    ...
    {
      tokenization = psp_create_instance( $const42$CycTokenization );
  ...
  public static SubLObject psp_chart_parse_tree_mt(SubLObject chart)
  {
    ...
    final SubLObject name = $str43$ParseTreeSandboxMt;
    ...
    if( NIL == constant_handles.valid_constantP( mt, UNPROVIDED ) )
    {
      ...
      try
      {
        ...
        ke.ke_assert_now( ConsesLow.listS( $const44$isa, mt, $list45 ), $const46$BaseKB, UNPROVIDED, UNPROVIDED );
        ke.ke_assert_now( ConsesLow.listS( $const47$genlMt, mt, $list48 ), $const46$BaseKB, UNPROVIDED, UNPROVIDED );
        ke.ke_assert_now( ConsesLow.listS( $const47$genlMt, mt, $list49 ), $const46$BaseKB, UNPROVIDED, UNPROVIDED );
      }
      finally
      {
        api_control_vars.$the_cyclist$.rebind( _prev_bind_0, thread );
  ...
  {
    SubLFiles.declareFunction( me, "psp_generate_parse_tree_for_string", "PSP-GENERATE-PARSE-TREE-FOR-STRING", 1, 1, false );
    SubLFiles.declareFunction( me, "psp_generate_parse_tree_for_chart", "PSP-GENERATE-PARSE-TREE-FOR-CHART", 1, 1, false );
    SubLFiles.declareFunction( me, "psp_find_or_create_node_for_edges", "PSP-FIND-OR-CREATE-NODE-FOR-EDGES", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_create_node_for_edges_int", "PSP-CREATE-NODE-FOR-EDGES-INT", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_edge_equivalence_set_print_function_trampoline", "PSP-EDGE-EQUIVALENCE-SET-PRINT-FUNCTION-TRAMPOLINE", 2, 0, false );
    SubLFiles.declareFunction( me, "psp_edge_equivalence_set_p", "PSP-EDGE-EQUIVALENCE-SET-P", 1, 0, false );
    new $psp_edge_equivalence_set_p$UnaryFunction();
    SubLFiles.declareFunction( me, "psp_equivalence_set_choice_dtr_num", "PSP-EQUIVALENCE-SET-CHOICE-DTR-NUM", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_equivalence_set_edges", "PSP-EQUIVALENCE-SET-EDGES", 1, 0, false );
    SubLFiles.declareFunction( me, "_csetf_psp_equivalence_set_choice_dtr_num", "_CSETF-PSP-EQUIVALENCE-SET-CHOICE-DTR-NUM", 2, 0, false );
    SubLFiles.declareFunction( me, "_csetf_psp_equivalence_set_edges", "_CSETF-PSP-EQUIVALENCE-SET-EDGES", 2, 0, false );
    SubLFiles.declareFunction( me, "make_psp_edge_equivalence_set", "MAKE-PSP-EDGE-EQUIVALENCE-SET", 0, 1, false );
    SubLFiles.declareFunction( me, "visit_defstruct_psp_edge_equivalence_set", "VISIT-DEFSTRUCT-PSP-EDGE-EQUIVALENCE-SET", 2, 0, false );
    SubLFiles.declareFunction( me, "visit_defstruct_object_psp_edge_equivalence_set_method", "VISIT-DEFSTRUCT-OBJECT-PSP-EDGE-EQUIVALENCE-SET-METHOD", 2, 0, false );
    SubLFiles.declareFunction( me, "pprint_psp_equivalence_set", "PPRINT-PSP-EQUIVALENCE-SET", 1, 2, false );
    SubLFiles.declareFunction( me, "new_psp_equivalence_set", "NEW-PSP-EQUIVALENCE-SET", 0, 0, false );
    SubLFiles.declareFunction( me, "psp_equivalence_set_add_edge", "PSP-EQUIVALENCE-SET-ADD-EDGE", 2, 1, false );
    SubLFiles.declareFunction( me, "psp_equivalence_sets_for_edges", "PSP-EQUIVALENCE-SETS-FOR-EDGES", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_edge_fits_in_equivalence_setP", "PSP-EDGE-FITS-IN-EQUIVALENCE-SET?", 2, 0, false );
    SubLFiles.declareFunction( me, "psp_edges_choice_node_equivalentP", "PSP-EDGES-CHOICE-NODE-EQUIVALENT?", 3, 0, false );
    SubLFiles.declareFunction( me, "psp_create_node_for_equivalence_sets", "PSP-CREATE-NODE-FOR-EQUIVALENCE-SETS", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_find_or_create_node_for_equivalence_set", "PSP-FIND-OR-CREATE-NODE-FOR-EQUIVALENCE-SET", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_create_node_for_equivalence_set", "PSP-CREATE-NODE-FOR-EQUIVALENCE-SET", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_find_or_create_node_for_edge", "PSP-FIND-OR-CREATE-NODE-FOR-EDGE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_create_node_for_edge", "PSP-CREATE-NODE-FOR-EDGE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_create_node_shell_for_edge", "PSP-CREATE-NODE-SHELL-FOR-EDGE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_edge_note_meaning_on_node_if_known", "PSP-EDGE-NOTE-MEANING-ON-NODE-IF-KNOWN", 2, 1, false );
    SubLFiles.declareFunction( me, "psp_note_semantic_dependency", "PSP-NOTE-SEMANTIC-DEPENDENCY", 4, 0, false );
    SubLFiles.declareMacro( me, "with_psp_parse_tree_assert_parameters", "WITH-PSP-PARSE-TREE-ASSERT-PARAMETERS" );
    SubLFiles.declareFunction( me, "psp_create_parse_tree_with_root_node", "PSP-CREATE-PARSE-TREE-WITH-ROOT-NODE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_agent", "PSP-AGENT", 0, 0, false );
    SubLFiles.declareFunction( me, "psp_create_instance", "PSP-CREATE-INSTANCE", 1, 0, false );
    SubLFiles.declareFunction( me, "psp_create_node", "PSP-CREATE-NODE", 5, 0, false );
    SubLFiles.declareFunction( me, "psp_span_wrt_tokenization", "PSP-SPAN-WRT-TOKENIZATION", 2, 0, false );
    SubLFiles.declareFunction( me, "psp_node_add_dtr", "PSP-NODE-ADD-DTR", 3, 0, false );
    SubLFiles.declareFunction( me, "psp_node_propose_meaning", "PSP-NODE-PROPOSE-MEANING", 2, 1, false );
    SubLFiles.declareFunction( me, "psp_node_note_semantic_dependency", "PSP-NODE-NOTE-SEMANTIC-DEPENDENCY", 4, 0, false );
    SubLFiles.declareFunction( me, "psp_generate_choice_node", "PSP-GENERATE-CHOICE-NODE", 0, 0, false );
    SubLFiles.declareFunction( me, "psp_choice_node_add_option_node", "PSP-CHOICE-NODE-ADD-OPTION-NODE", 2, 0, false );
    SubLFiles.declareFunction( me, "psp_chart_tokenization", "PSP-CHART-TOKENIZATION", 0, 1, false );
    SubLFiles.declareFunction( me, "psp_chart_parse_tree_mt", "PSP-CHART-PARSE-TREE-MT", 0, 1, false );
    return NIL;
  }

  public static SubLObject init_psp_parse_tree_generator_file()
  {
    $dtp_psp_edge_equivalence_set$ = SubLFiles.defconstant( "*DTP-PSP-EDGE-EQUIVALENCE-SET*", $sym5$PSP_EDGE_EQUIVALENCE_SET );
    $psp_chart_tokenizations$ = SubLFiles.deflexical( "*PSP-CHART-TOKENIZATIONS*", maybeDefault( $sym41$_PSP_CHART_TOKENIZATIONS_, $psp_chart_tokenizations$, () -> ( dictionary.new_dictionary( UNPROVIDED,
        UNPROVIDED ) ) ) );
    return NIL;
  }
  ...
  {
    Structures.register_method( print_high.$print_object_method_table$.getGlobalValue(), $dtp_psp_edge_equivalence_set$.getGlobalValue(), Symbols.symbol_function(
        $sym12$PSP_EDGE_EQUIVALENCE_SET_PRINT_FUNCTION_TRAMPOLINE ) );
    SubLSpecialOperatorDeclarations.proclaim( $list13 );
    Structures.def_csetf( $sym14$PSP_EQUIVALENCE_SET_CHOICE_DTR_NUM, $sym15$_CSETF_PSP_EQUIVALENCE_SET_CHOICE_DTR_NUM );
    Structures.def_csetf( $sym16$PSP_EQUIVALENCE_SET_EDGES, $sym17$_CSETF_PSP_EQUIVALENCE_SET_EDGES );
    Equality.identity( $sym5$PSP_EDGE_EQUIVALENCE_SET );
    Structures.register_method( visitation.$visit_defstruct_object_method_table$.getGlobalValue(), $dtp_psp_edge_equivalence_set$.getGlobalValue(), Symbols.symbol_function(
        $sym25$VISIT_DEFSTRUCT_OBJECT_PSP_EDGE_EQUIVALENCE_SET_METHOD ) );
    subl_macro_promotions.declare_defglobal( $sym41$_PSP_CHART_TOKENIZATIONS_ );
    return NIL;
  }
  ...
  static
  {
    me = new psp_parse_tree_generator();
    $dtp_psp_edge_equivalence_set$ = null;
    $psp_chart_tokenizations$ = null;
    $kw0$ANY = makeKeyword( "ANY" );
    $const1$parseTreeOfTokenization = constant_handles.reader_make_constant_shell( makeString( "parseTreeOfTokenization" ) );
    $sym2$PHRASE_STRUCTURE_PARSER_CHART_P = makeSymbol( "PHRASE-STRUCTURE-PARSER-CHART-P" );
    $str3$Can_t_generate_a_node_from_no_edg = makeString( "Can't generate a node from no edges!" );
    $str4$Found_no_equivalence_sets_for__S_ = makeString( "Found no equivalence sets for ~S." );
    $sym5$PSP_EDGE_EQUIVALENCE_SET = makeSymbol( "PSP-EDGE-EQUIVALENCE-SET" );
    $sym6$PSP_EDGE_EQUIVALENCE_SET_P = makeSymbol( "PSP-EDGE-EQUIVALENCE-SET-P" );
    $list7 = ConsesLow.list( makeSymbol( "CHOICE-DTR-NUM" ), makeSymbol( "EDGES" ) );
    $list8 = ConsesLow.list( makeKeyword( "CHOICE-DTR-NUM" ), makeKeyword( "EDGES" ) );
    $list9 = ConsesLow.list( makeSymbol( "PSP-EQUIVALENCE-SET-CHOICE-DTR-NUM" ), makeSymbol( "PSP-EQUIVALENCE-SET-EDGES" ) );
    $list10 = ConsesLow.list( makeSymbol( "_CSETF-PSP-EQUIVALENCE-SET-CHOICE-DTR-NUM" ), makeSymbol( "_CSETF-PSP-EQUIVALENCE-SET-EDGES" ) );
    $sym11$PPRINT_PSP_EQUIVALENCE_SET = makeSymbol( "PPRINT-PSP-EQUIVALENCE-SET" );
    $sym12$PSP_EDGE_EQUIVALENCE_SET_PRINT_FUNCTION_TRAMPOLINE = makeSymbol( "PSP-EDGE-EQUIVALENCE-SET-PRINT-FUNCTION-TRAMPOLINE" );
    $list13 = ConsesLow.list( makeSymbol( "OPTIMIZE-FUNCALL" ), makeSymbol( "PSP-EDGE-EQUIVALENCE-SET-P" ) );
    $sym14$PSP_EQUIVALENCE_SET_CHOICE_DTR_NUM = makeSymbol( "PSP-EQUIVALENCE-SET-CHOICE-DTR-NUM" );
    $sym15$_CSETF_PSP_EQUIVALENCE_SET_CHOICE_DTR_NUM = makeSymbol( "_CSETF-PSP-EQUIVALENCE-SET-CHOICE-DTR-NUM" );
    $sym16$PSP_EQUIVALENCE_SET_EDGES = makeSymbol( "PSP-EQUIVALENCE-SET-EDGES" );
    $sym17$_CSETF_PSP_EQUIVALENCE_SET_EDGES = makeSymbol( "_CSETF-PSP-EQUIVALENCE-SET-EDGES" );
    $kw18$CHOICE_DTR_NUM = makeKeyword( "CHOICE-DTR-NUM" );
    $kw19$EDGES = makeKeyword( "EDGES" );
    $str20$Invalid_slot__S_for_construction_ = makeString( "Invalid slot ~S for construction function" );
    $kw21$BEGIN = makeKeyword( "BEGIN" );
    $sym22$MAKE_PSP_EDGE_EQUIVALENCE_SET = makeSymbol( "MAKE-PSP-EDGE-EQUIVALENCE-SET" );
    $kw23$SLOT = makeKeyword( "SLOT" );
    $kw24$END = makeKeyword( "END" );
    $sym25$VISIT_DEFSTRUCT_OBJECT_PSP_EDGE_EQUIVALENCE_SET_METHOD = makeSymbol( "VISIT-DEFSTRUCT-OBJECT-PSP-EDGE-EQUIVALENCE-SET-METHOD" );
    $str26$_PSP_EQUIV_SET_of__S_edges_ = makeString( "<PSP-EQUIV-SET of ~S edges>" );
    $str27$New_choice_dtr_num_of__S_doesn_t_ = makeString( "New choice-dtr-num of ~S doesn't match ~S" );
    $const28$proposedMeaning = constant_handles.reader_make_constant_shell( makeString( "proposedMeaning" ) );
    $sym29$CLET = makeSymbol( "CLET" );
    $list30 = ConsesLow.list( ConsesLow.list( makeSymbol( "*THE-CYCLIST*" ), ConsesLow.list( makeSymbol( "PSP-AGENT" ) ) ) );
    $sym31$WITH_PARSE_TREE_MT = makeSymbol( "WITH-PARSE-TREE-MT" );
    $list32 = ConsesLow.list( makeSymbol( "PSP-CHART-PARSE-TREE-MT" ) );
    $sym33$WITH_PARSE_TREE_PREFIX = makeSymbol( "WITH-PARSE-TREE-PREFIX" );
    $str34$PSP = makeString( "PSP" );
    $str35$TheCycPhraseStructureParser = makeString( "TheCycPhraseStructureParser" );
    $list36 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "InstanceWithRelationFromFn" ) ), ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "ProgramCopyFn" ) ),
        constant_handles.reader_make_constant_shell( makeString( "CycPhraseStructureParser" ) ) ), constant_handles.reader_make_constant_shell( makeString( "partOfIBT" ) ), constant_handles.reader_make_constant_shell(
            makeString( "Cyc" ) ) );
    $sym37$_DTR_NODE = makeSymbol( "?DTR-NODE" );
    $const38$syntacticNodeNthDaughter = constant_handles.reader_make_constant_shell( makeString( "syntacticNodeNthDaughter" ) );
    $list39 = ConsesLow.list( makeSymbol( "?DTR-NODE" ) );
    $str40$__Trying_to_add__S_to____S___as_d = makeString( "~&Trying to add ~S to~% ~S~% as dtr ~S of ~S~%" );
    $sym41$_PSP_CHART_TOKENIZATIONS_ = makeSymbol( "*PSP-CHART-TOKENIZATIONS*" );
    $const42$CycTokenization = constant_handles.reader_make_constant_shell( makeString( "CycTokenization" ) );
    $str43$ParseTreeSandboxMt = makeString( "ParseTreeSandboxMt" );
    $const44$isa = constant_handles.reader_make_constant_shell( makeString( "isa" ) );
    $list45 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "Microtheory" ) ) );
    $const46$BaseKB = constant_handles.reader_make_constant_shell( makeString( "BaseKB" ) );
    $const47$genlMt = constant_handles.reader_make_constant_shell( makeString( "genlMt" ) );
    $list48 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "DiscourseModellingVocabularyMt" ) ) );
    $list49 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "EnglishMt" ) ) );
  }

  public static final class $psp_edge_equivalence_set_native
      extends
        SubLStructNative
  {
    ...
    {
      this.$choice_dtr_num = CommonSymbols.NIL;
    ...
    {
      return $psp_edge_equivalence_set_native.structDecl;
    }
    ...
    static
    {
      structDecl = Structures.makeStructDeclNative( $psp_edge_equivalence_set_native.class, $sym5$PSP_EDGE_EQUIVALENCE_SET, $sym6$PSP_EDGE_EQUIVALENCE_SET_P, $list7, $list8, new String[] { "$choice_dtr_num", "$edges"
      }, $list9, $list10, $sym11$PPRINT_PSP_EQUIVALENCE_SET );
    }
  }

  public static final class $psp_edge_equivalence_set_p$UnaryFunction
      extends
        UnaryFunction
  {
