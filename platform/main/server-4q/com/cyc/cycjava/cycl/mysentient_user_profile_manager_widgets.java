...
public final class mysentient_user_profile_manager_widgets extends SubLTranslatedFile implements V02 {
    ...
    public static final SubLObject expansion_cycls_for_strategy(SubLObject v_term, SubLObject strategy, SubLObject mt, SubLObject max_number, SubLObject instantiate_collectionsP, SubLObject max_time) {
        ...
        SubLTrampolineFile.checkType(mt, HLMT_P);
        {
            ...
            results = ((NIL != instantiate_collectionsP) && (NIL != fort_types_interface.isa_collectionP(v_term, UNPROVIDED))) ? ((SubLObject) (ask_utilities.query_variable($SET, list($$implies, list($$isa, $sym29$_X, v_term), listS($$irExpansionCandidate, $sym29$_X, strategy, $list_alt31)), listS($$MtSpace, mt, $list_alt33), list(new SubLObject[]{ $MAX_TRANSFORMATION_DEPTH, ONE_INTEGER, $MAX_NUMBER, max_number, $ANSWER_LANGUAGE, $HL, $kw36$CONDITIONAL_SENTENCE_, T, $CONTINUABLE_, NIL, $BROWSABLE_, NIL, $MAX_TIME, max_time })))) : ask_utilities.query_variable($SET, listS($$irExpansionCandidate, v_term, strategy, $list_alt31), listS($$MtSpace, mt, $list_alt33), list(new SubLObject[]{ $MAX_TRANSFORMATION_DEPTH, ONE_INTEGER, $MAX_NUMBER, max_number, $ANSWER_LANGUAGE, $HL, $CONTINUABLE_, NIL, $BROWSABLE_, NIL, $MAX_TIME, max_time }));
            ...
            SubLTrampolineFile.checkType(mt, MICROTHEORY_P);
            {
                ...
                SubLObject weights = NIL;
                {
                    ...
                    try {
                        ...
                        result = remove(known_phrase, pph_methods_lexicon.all_phrases_for_term(narts_high.nart_substitute(v_term), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED), EQUALP, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
    ...
    public static final SubLObject assign_np_referent(SubLObject expr_peg, SubLObject useful_semantics, SubLObject np_string, SubLObject profile_mt, SubLObject session_mt, SubLObject parsing_mt) {
        {
            ...
            if (NIL != cyc_kernel.closed_query_success_result_p(inference_kernel.new_cyc_query(list($$and, list($$unknownSentence, listS($$isa, parsing_utilities.strip_nl_tags(useful_semantics, UNPROVIDED), $list_alt97)), list($$unknownSentence, listS($$isa, parsing_utilities.strip_nl_tags(useful_semantics, UNPROVIDED), $list_alt98))), session_mt, UNPROVIDED))) {
                ...
                if (NIL != plural_semanticsP(useful_semantics)) {
                } else {
    ...
    public static final SubLObject note_np_reference(SubLObject np_string, SubLObject raw_semantics, SubLObject profile_mt, SubLObject session_mt, SubLObject parsing_mt) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                ...
                if (NIL != $mys_treat_all_nps_as_nbars$.getDynamicValue(thread)) {
                    ...
                    if ((NIL != $mys_treat_indefinite_nps_as_nbars$.getDynamicValue(thread)) && (NIL != indefinite_referenceP(resolved_semantics.first()))) {
                        return note_nbar_reference(np_string, resolved_semantics, profile_mt, session_mt, parsing_mt);
                    } else
                        if ((NIL != $mys_treat_definite_nps_as_nbars$.getDynamicValue(thread)) && (NIL != definite_referenceP(resolved_semantics.first()))) {
                            return note_nbar_reference(np_string, resolved_semantics, profile_mt, session_mt, parsing_mt);
                        } else {
    ...
    public static final SubLObject most_recent_expression_pegs_for_referent_pegs(SubLObject referent_pegs, SubLObject mt) {
        {
            ...
            for (referent_peg = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , referent_peg = cdolist_list_var.first()) {
                expression_pegs = cconcatenate(expression_pegs, ask_utilities.query_variable($sym226$_EXPR_RECENT, list($$thereExists, $sym228$_NUM, list($$and, list($$assertedSentence, list($$intendedReferent, $sym226$_EXPR_RECENT, referent_peg)), $list_alt230, listS($$nthLargestElement, $sym228$_NUM, ONE_INTEGER, list($$TheSetOf, $sym233$_M, list($$thereExists, $sym234$_EXPR, listS($$and, list($$assertedSentence, list($$intendedReferent, $sym234$_EXPR, referent_peg)), $list_alt235))), $list_alt236))), mt, UNPROVIDED));
            }
                ...
                }
                {
                    ...
                    SubLObject message = NIL;
                    {
                        ...
                        try {
                            ...
                            thread.resetMultipleValues();
                            {
                                SubLObject successP_6 = mysentient_assert_wff_assertion_specifications(list(bq_cons(listS($$isa, session_context, $list_alt265), $list_alt266), list(listS($$isa, session_context, $list_alt267), assert_mt), list(list($$mySentientProfileSessionRecords, user_profile_mt, session_context), assert_mt), bq_cons(listS($$isa, session_term, $list_alt269), $list_alt266), list(listS($$isa, session_term, $list_alt270), session_context), list(list($$sessionStartDate, session_term, date_utilities.indexical_now()), session_context), list(list($$recordOfSession, session_context, session_term), session_context)), $str_alt272$All_KB_Operations_completed_succe);
                ...
                }
                {
                    ...
                    SubLObject message = NIL;
                    {
                        ...
                        try {
                            ...
                            thread.resetMultipleValues();
                            {
                                SubLObject successP_10 = mysentient_assert_wff_assertion_specifications(list(new SubLObject[]{ bq_cons(listS($$isa, user_term, $list_alt269), $list_alt266), list(listS($$isa, user_term, $list_alt350), assert_mt), list(listS($$isa, user_term, $list_alt351), assert_mt), bq_cons(listS($$isa, user_profile_mt, $list_alt265), $list_alt266), list(listS($$isa, user_profile_mt, $list_alt352), assert_mt), list(list($$mySentientUserProfile, user_term, user_profile_mt), assert_mt), bq_cons(listS($$isa, user_generation_mt, $list_alt265), $list_alt266), list(listS($$isa, user_generation_mt, $list_alt353), assert_mt), list(list($$mySentientGenerationMtForProfile, user_generation_mt, user_profile_mt), assert_mt), bq_cons(listS($$isa, user_lexical_mt, $list_alt265), $list_alt266), list(listS($$isa, user_lexical_mt, $list_alt354), assert_mt), list(list($$nameString, user_term, cconcatenate($str_alt356$User_, id_string)), user_lexical_mt), list(list($$mySentientLexicalMtForProfile, user_lexical_mt, user_profile_mt), assert_mt), bq_cons(listS($$isa, user_parsing_mt, $list_alt265), $list_alt266), list(listS($$isa, user_parsing_mt, $list_alt357), assert_mt), list(list($$mySentientParsingMtForProfile, user_parsing_mt, user_profile_mt), assert_mt), list(list($$synonymousExternalConcept, user_term, user_database, id_string), assert_mt) }), $str_alt272$All_KB_Operations_completed_succe);
    ...
    static private final SubLList $list_alt93 = list(new SubLObject[]{ cons(makeString("terse"), constant_handles.reader_make_constant_shell(makeString("ProvideTerseExpansions-IRExpansionStrategy"))), cons(makeString("asserted"), constant_handles.reader_make_constant_shell(makeString("ProvideDefinitionalAssertions-IRExpansionStrategy"))), cons(makeString("coarse-grained"), constant_handles.reader_make_constant_shell(makeString("ProvideCoarseYetReasonableDefinitionalInformation-IRExpansionStrategy"))), cons(makeString("parts_sub"), constant_handles.reader_make_constant_shell(makeString("ProvideSubPartsExpansions-IRExpansionStrategy"))), cons(makeString("parts_super"), constant_handles.reader_make_constant_shell(makeString("ProvideSuperPartsExpansions-IRExpansionStrategy"))), cons(makeString("specializations"), constant_handles.reader_make_constant_shell(makeString("ProvideExpansionsBasedOnInstancesOfFacetingCollections-IRExpansionStrategy"))), cons(makeString("genls"), constant_handles.reader_make_constant_shell(makeString("ProvideBestGeneralizations-IRExpansionStrategy"))), cons(makeString("isa"), constant_handles.reader_make_constant_shell(makeString("ProvideBestIsas-IRExpansionStrategy"))), cons(makeString("related"), constant_handles.reader_make_constant_shell(makeString("ProvideConceptuallyRelatedConcepts-IRExpansionStrategy"))), cons(makeString("alt_phrase"), makeSymbol("ALT-PHRASE-EXPANSIONS-FOR-PEG")) });
    ...
    static private final SubLString $str_alt223$Perform_the_KB_edit_specified__an = makeString("Perform the KB edit specified, and return any pegs that should be re-expanded as a result. \n   This will currently be the case only if one of the terms in either the new or old sentence\n   was in fact a peg mentioned in the most recent user utterance.\n   @todo This function should probably throw an error of some sort if the kb-edit can\'t be made");
    ...
    static private final SubLList $list_alt238 = list(list(constant_handles.reader_make_constant_shell(makeString("and")), list(constant_handles.reader_make_constant_shell(makeString("isa")), makeSymbol("?PEG"), constant_handles.reader_make_constant_shell(makeString("LinguisticExpressionPeg-CompleteUtterance"))), list(constant_handles.reader_make_constant_shell(makeString("nthExpressionOfSession")), makeSymbol("??SESS"), makeSymbol("?PEG"), makeSymbol("?UTTR-NUM"))));
    ...
    static private final SubLList $list_alt334 = list(list(constant_handles.reader_make_constant_shell(makeString("ist-Asserted")), makeSymbol("?SESSION-MT"), list(constant_handles.reader_make_constant_shell(makeString("recordOfSession")), makeSymbol("?SESSION-MT"), makeSymbol("?SESSION-TERM"))), list(constant_handles.reader_make_constant_shell(makeString("isa")), makeSymbol("?SESSION-TERM"), constant_handles.reader_make_constant_shell(makeString("ActiveSession"))));
