...
public final class pph_phrase_resolution
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject pph_phrase_note_unfinished_dtrs(final SubLObject phrase)
  {
    ...
    if( document.sign_constituents( phrase ).isVector() )
    {
      ...
      for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
      {
        ...
        {
          pph_note_unfinished_phrase( dtr, $str2$Unfinished_dtr );
        }
  ...
  public static SubLObject pph_phrase_get_string(final SubLObject phrase, SubLObject cycl, SubLObject matrix_phrase)
  {
    ...
    try
    {
      ...
      {
        final SubLObject new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
            $str7$__Exceeded_PPH_PHRASE_GET_STRING_ )
        } );
        pph_error.pph_handle_error( new_format_string, ConsesLow.list( $pph_phrase_get_string_max_depth$.getDynamicValue( thread ), phrase ) );
      }
      if( $kw4$PHRASE_CYCL == cycl )
      {
      ...
      if( NIL == pph_phrase.pph_phrase_doneP( phrase ) )
      {
        ...
        else if( NIL != pph_phrase.pph_variable_binding_phrase_p( phrase ) )
        {
          pph_note_unfinished_phrase( phrase, UNPROVIDED );
        }
        else if( NIL != pph_phrase.pph_cycl_phrase_p( phrase ) )
        {
          ...
          else if( NIL != cycl_variables.el_varP( cycl_$1 ) )
          {
        ...
        else if( NIL != anaphor_needed_for_phraseP( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else if( NIL != pph_phrase.pph_coordination_phrase_p( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
          ...
          final SubLObject bullets_okP = makeBoolean( NIL != pph_phrase.pph_phrase_p( mother, UNPROVIDED ) || pph_vars.current_generation_level().numG( pph_vars.$pph_use_bulleted_lists_min_level$.getDynamicValue(
              thread ) ) );
        ...
        else if( NIL != pph_phrase.pph_chemical_formula_from_lists_phrase_p( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else if( NIL != pph_phrase.pph_percent_phrase_p( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else if( NIL != pph_phrase.pph_symbol_phrase_p( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else if( NIL != pph_phrase.pph_string_mention_phrase_p( phrase ) && NIL != pph_string.pph_string_p( pph_phrase.pph_phrase_cycl( phrase, UNPROVIDED ) ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else if( NIL != pph_phrase.pph_quoted_paraphrase_phrase_p( phrase ) )
        {
          pph_quoted_phrase_get_string( phrase, matrix_phrase );
        }
        else if( NIL != pph_phrase.pph_nospace_phrase_p( phrase ) )
        {
          pph_nospace_phrase_get_string( phrase, cycl, matrix_phrase );
        }
        else if( NIL != pph_phrase.pph_terse_phrase_p( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else if( NIL != pph_phrase.pph_precise_phrase_p( phrase ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        else
        {
          ...
          final SubLObject argnum = ( NIL != el_utilities.el_formula_p( cycl ) && NIL == pph_utilities.pph_formula_with_dotsP( cycl ) ) ? Sequences.position( function_terms.nart_to_naut( pph_phrase.pph_phrase_cycl(
              phrase, UNPROVIDED ) ), cycl, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED ) : NIL;
          assert NIL != pph_utilities.pph_precision_p( pph_utilities.paraphrase_precision_for_arg( argnum, cycl ) ) : pph_utilities.paraphrase_precision_for_arg( argnum, cycl );
    ...
    return Values.values( pph_phrase.pph_phrase_string( phrase, UNPROVIDED ), phrase );
  }
  ...
  public static SubLObject maybe_throw_pph_hl_supports(final SubLObject v_term, final SubLObject string, final SubLObject agr_preds, final SubLObject supports)
  {
    ...
    if( NIL != list_utilities.non_empty_list_p( supports ) && NIL != misc_utilities.initialized_p( pph_vars.$pph_paraphrase_to_justify$.getDynamicValue( thread ) ) )
    {
      ...
      if( NIL == current )
      {
        ...
        {
          Dynamic.sublisp_throw( $kw29$PPH_HL_SUPPORTS_FOUND, supports );
        }
      }
      else
      {
  ...
  public static SubLObject pph_nospace_phrase_output_list_from_dtr(final SubLObject dtr)
  {
    ...
    if( document.sign_constituents( dtr ).isVector() )
    {
      ...
      for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
      {
        ...
        gdtr_olists = ConsesLow.cons( pph_phrase.pph_phrase_output_list( gdtr ), gdtr_olists );
        gdtr_num = Numbers.add( gdtr_num, ONE_INTEGER );
      }
    }
    return ConsesLow.list( pph_data_structures.new_pph_phrase_output_item_nospace_group_from_olists( Sequences.nreverse( gdtr_olists ) ) );
  }
  ...
  public static SubLObject pph_phrase_get_string_from_dtrs(final SubLObject phrase, final SubLObject formula)
  {
    ...
    if( resolvedP == $kw30$RESOLVED )
    {
      pph_phrase_set_output_list_from_finished_dtrs( phrase );
      if( pph_phrase.pph_phrase_category( phrase, UNPROVIDED ).eql( $const31$PossessivePhrase ) )
      {
  ...
  public static SubLObject pph_phrase_try_to_resolve_dtr_strings(final SubLObject phrase, final SubLObject formula)
  {
    ...
    if( NIL == abortP )
    {
      ...
      SubLObject element_num;
      SubLObject dtr;
      for( new_unfinished_count = NIL, new_unfinished_count = pph_phrase.pph_phrase_unfinished_dtr_count( phrase ); NIL == abortP && !new_unfinished_count.isZero() && !unfinished_dtr_count.eql(
          new_unfinished_count ); new_unfinished_count = pph_phrase.pph_phrase_unfinished_dtr_count( phrase ) )
      {
        ...
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
          format_nil.force_format( T, $str36$__Finished__S_iteration_s____D_of, iteration_count, pph_phrase.pph_phrase_unfinished_dtr_count( phrase ), pph_phrase.pph_phrase_dtr_count( phrase ), UNPROVIDED, UNPROVIDED,
      ...
      if( NIL == successP )
      {
        ...
        if( document.sign_constituents( phrase ).isVector() )
        {
          ...
          for( length2 = Sequences.length( vector_var2 ), v_iteration2 = NIL, v_iteration2 = ZERO_INTEGER; v_iteration2.numL( length2 ); v_iteration2 = Numbers.add( v_iteration2, ONE_INTEGER ) )
          {
            ...
            {
              unfinished_dtrs = ConsesLow.cons( dtr2, unfinished_dtrs );
            }
    ...
    if( NIL != successP )
    {
      return $kw30$RESOLVED;
    }
    return $kw38$NOT_RESOLVED;
  }
  ...
  public static SubLObject pph_phrase_resolve_repeated_phrase_dtrs(final SubLObject phrase, final SubLObject formula)
  {
      ...
      for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
      {
        ...
        if( NIL == problemP )
        {
          ...
          else
          {
            ...
            if( NIL != list_utilities.non_empty_list_of_type_p( $sym39$PPH_PHRASE_P, expansion ) )
            {
    ...
    {
      pph_phrase.set_pph_phrase_dtrs_from_list( phrase, Sequences.nreverse( new_dtrs ), UNPROVIDED );
    }
  ...
  public static SubLObject pph_repeated_phrase_expansion(final SubLObject repeated_phrase, final SubLObject formula)
  {
    ...
    if( NIL == el_utilities.el_formula_p( formula ) )
    {
      ...
      return ConsesLow.list( repeated_phrase );
    }
  ...
  public static SubLObject pph_repeated_phrase_expansion_internal(final SubLObject repeated_phrase, final SubLObject first_phrase, final SubLObject formula)
  {
    ...
    try
    {
      ...
      final SubLObject sub_formula = cycl_utilities.hl_to_el( ( NIL != list_utilities.lengthG( first_arg_position, ONE_INTEGER, UNPROVIDED ) ) ? cycl_utilities.formula_arg_position( formula, conses_high.butlast(
          first_arg_position, UNPROVIDED ), UNPROVIDED ) : formula );
      final SubLObject formula_arity = el_utilities.formula_arity( sub_formula, UNPROVIDED );
      SubLObject all_phrases = NIL;
      if( !formula_arity.isInteger() )
      {
        all_phrases = ConsesLow.cons( first_phrase, all_phrases );
      }
      else if( first_arg_num.numLE( formula_arity ) )
      {
        ...
        {
          all_phrases = ConsesLow.cons( pph_one_repeated_phrase( first_phrase, first_arg_position, first_cycl_template, curr_arg_num ), all_phrases );
        }
  ...
  public static SubLObject pph_one_repeated_phrase(final SubLObject first_phrase, final SubLObject first_arg_position, final SubLObject first_cycl_template, final SubLObject curr_arg_num)
  {
    ...
    pph_phrase.pph_phrase_cycl_subst( curr_cycl_template, first_cycl_template, curr_arg_phrase, Symbols.symbol_function( EQUAL ), UNPROVIDED );
    if( pph_phrase.pph_phrase_cycl( curr_arg_phrase, UNPROVIDED ).equal( pph_phrase.pph_phrase_cycl( first_phrase, UNPROVIDED ) ) && NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) && NIL == pph_error
        .suppress_pph_warningsP() )
    {
  ...
  {
    SubLObject all_phrases = NIL;
    pph_phrase.pph_phrase_set_string( first_phrase, Sequences.cconcatenate( pph_phrase.pph_phrase_string( first_phrase, NIL ), $str44$__etc_ ) );
    all_phrases = ConsesLow.cons( first_phrase, all_phrases );
    return all_phrases;
  }
  ...
  public static SubLObject pph_current_list_item(final SubLObject list_expression, final SubLObject offset)
  {
    ...
    {
      final SubLObject new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
          $str45$Can_t_get_current_list_item_from_ )
      } );
      pph_error.pph_handle_error( new_format_string, ConsesLow.list( list_expression ) );
      return NIL;
    }
    final SubLObject first_index = ZERO_INTEGER;
    SubLObject index = pph_lookup_current_list_item( list_expression );
    if( $kw46$UNSPECIFIED == index )
    {
      index = first_index;
    ...
    return ConsesLow.nth( Numbers.add( index, offset ), cycl_utilities.formula_args( list_expression, UNPROVIDED ) );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return list_utilities.alist_lookup( $pph_repeated_phrase_lists$.getDynamicValue( thread ), list_expression, Symbols.symbol_function( EQUAL ), $kw46$UNSPECIFIED );
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_formula_p( v_object ) && NIL != formula_pattern_match.formula_matches_pattern( v_object, $list47 ) );
  }
  ...
  {
    return cycl_utilities.expression_transform( expression, $sym49$PPH_CURRENT_LIST_ITEM_FORMULA_P, $sym50$PPH_CURRENT_LIST_ITEM_FORMULA_ITEM, UNPROVIDED, UNPROVIDED );
  }
  ...
  {
    return makeBoolean( NIL != el_utilities.el_formula_p( v_object ) && NIL != formula_pattern_match.formula_matches_pattern( v_object, $list51 ) );
  }
  ...
  public static SubLObject pph_phrase_try_to_resolve_dtr_string(final SubLObject dtr, final SubLObject dtr_num, final SubLObject formula, final SubLObject phrase)
  {
    ...
    if( NIL == pph_phrase.pph_conditional_phrase_p( dtr ) )
    {
      ...
      else if( NIL != pph_phrase.pph_phrase_doneP( dtr ) )
      {
      ...
      else if( NIL != pph_phrase_agr_target_resolvedP( dtr, phrase, UNPROVIDED ) )
      {
  ...
  public static SubLObject pph_phrase_set_dtr_relative_agr(final SubLObject dtr, final SubLObject dtr_num, final SubLObject phrase)
  {
    ...
    if( NIL != pph_phrase_agr_target_resolvedP( dtr, phrase, UNPROVIDED ) )
    {
      ...
      {
        format_nil.force_format( T, $str59$__Set_agr_of_dtr___D_to__S___, dtr_num, pph_phrase.pph_phrase_agr_constraint_of_type( dtr, relative_constraint_type ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
      ...
      if( NIL != pph_phrase.pph_phrase_head_dtr_numP( phrase, dtr_num ) )
      {
        ...
        {
          format_nil.force_format( T, $str60$___Set_head_agr_to__S___, pph_phrase.pph_phrase_agr_target_of_type( dtr, relative_constraint_type ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
              UNPROVIDED );
        }
      }
    }
  ...
  public static SubLObject pph_phrase_agr_target_resolvedP(final SubLObject phrase, final SubLObject mother, SubLObject mother_must_be_doneP)
  {
    ...
    if( agr_target_phrase == $kw64$MOTHER )
    {
  ...
  public static SubLObject pph_phrase_set_output_list_from_finished_dtrs(final SubLObject phrase)
  {
    ...
    else if( NIL != pph_phrase.pph_quoted_paraphrase_phrase_p( phrase ) )
    {
      ...
      final SubLObject new_items = ConsesLow.cons( ConsesLow.list( pph_data_structures.new_pph_phrase_output_item( $str68$_, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ), list_utilities.add_to_end(
          ConsesLow.list( pph_data_structures.new_pph_phrase_output_item( $str68$_, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ), old_items ) );
      pph_data_structures.pph_phrase_output_item_nospace_group_set_items( nospace_item, new_items );
      pph_phrase.pph_phrase_set_output_list( phrase, ConsesLow.list( nospace_item ), UNPROVIDED );
    }
    else
    {
      ...
      if( document.sign_constituents( phrase ).isVector() )
      {
        ...
        for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
        {
          ...
          if( NIL == pph_phrase.pph_phrase_doneP( dtr ) )
          {
            new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
                $str69$Setting_mother_s_output_list_from )
            } );
            pph_error.pph_handle_error( new_format_string, ConsesLow.list( pph_phrase.pph_phrase_cycl( dtr, UNPROVIDED ), dtr ) );
          }
          ...
          while ( NIL != cdolist_list_var)
          {
            if( NIL != output_items && NIL != pph_utilities.pph_final_punctuation_string_p( pph_data_structures.pph_phrase_output_item_string( dtr_item ) ) && NIL != pph_utilities.pph_bulleted_list_end_itemP(
                output_items.first() ) )
            {
              insert_position = ( NIL != pph_utilities.pph_bulleted_list_item_end_itemP( conses_high.second( output_items ) ) ) ? TWO_INTEGER : ONE_INTEGER;
              output_items = list_utilities.ninsert_by_position( dtr_item, output_items, insert_position );
            }
            else
            {
              output_items = ConsesLow.cons( dtr_item, output_items );
            }
            cdolist_list_var = cdolist_list_var.rest();
            dtr_item = cdolist_list_var.first();
          }
      ...
      pph_phrase.pph_phrase_set_output_list( phrase, Sequences.nreverse( Sequences.delete_if( $sym71$PPH_PHRASE_OUTPUT_ITEM_EMPTY_P, output_items, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ), UNPROVIDED );
      pph_phrase.pph_phrase_note_done( phrase );
    }
  ...
  {
    return makeBoolean( NIL != list_utilities.lengthE( pph_data_structures.pph_phrase_output_item_string( item ), ZERO_INTEGER, UNPROVIDED ) && NIL == string_utilities.non_empty_stringP( pph_data_structures
        .pph_phrase_output_item_html_open_tag( item ) ) && NIL == string_utilities.non_empty_stringP( pph_data_structures.pph_phrase_output_item_html_close_tag( item ) ) && NIL == pph_phrase.pph_known_cycl_p(
            pph_data_structures.pph_phrase_output_item_cycl( item ) ) );
  }
  ...
  public static SubLObject pph_phrase_arg_dtrs(final SubLObject phrase)
  {
    ...
    if( document.sign_constituents( phrase ).isVector() )
    {
      ...
      for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
      {
        ...
        {
          arg_dtrs = ConsesLow.cons( dtr_num, arg_dtrs );
        }
        dtr_num = Numbers.add( dtr_num, ONE_INTEGER );
      }
    }
  ...
  public static SubLObject pph_phrase_arg0_phraseP(final SubLObject phrase)
  {
    ...
    if( NIL == arg_phraseP )
    {
      ...
      SubLObject item;
      for( csome_list_var = pph_phrase.pph_phrase_output_list( phrase ), item = NIL, item = csome_list_var.first(); NIL == arg_phraseP && NIL != csome_list_var; arg_phraseP = Equality.equal( $list72, pph_data_structures
          .pph_phrase_output_item_arg_position( item ) ), csome_list_var = csome_list_var.rest(), item = csome_list_var.first() )
      {
      }
    }
    if( NIL != arg_phraseP )
  ...
  public static SubLObject pph_phrase_resolve_condition(final SubLObject phrase, final SubLObject formula, final SubLObject matrix_phrase)
  {
    ...
    if( NIL != cycl_utilities.expression_find_if( $sym73$INCOMPLETE_PPH_PHRASE_P, test, UNPROVIDED, UNPROVIDED ) )
    {
      if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) && NIL == pph_error.suppress_pph_warningsP() )
      {
    ...
    if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
    {
      format_nil.force_format( T, $str75$__Ran_conditional_test_____S___Re, test, ( NIL != successP ) ? $str76$Pass : $str77$Fail, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    final SubLObject resolved_phrase = ( NIL != successP ) ? pph_phrase.pph_conditional_phrase_values( phrase ).first() : conses_high.second( pph_phrase.pph_conditional_phrase_values( phrase ) );
    if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
    {
      format_nil.force_format( T, $str78$___Initial_winning_phrase_____S__, resolved_phrase, pph_phrase.pph_phrase_agr_preds( resolved_phrase, UNPROVIDED ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
          UNPROVIDED );
    }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != el_utilities.el_formula_p( formula ) && NIL != pph_utilities.pph_cycl_template_p( dnf_template ) )
    {
    ...
    if( NIL != cycl_utilities.expression_find_if( $sym81$PPH_NTH_PHRASE_NAUT_P, dnf_template, UNPROVIDED, UNPROVIDED ) )
    {
  ...
  {
    if( NIL != el_utilities.possibly_fo_naut_p( obj ) )
    {
  ...
  public static SubLObject pph_phrase_resolve_nth_phrases(final SubLObject dnf_template, final SubLObject phrase)
  {
    ...
    try
    {
      ...
      ans = cycl_utilities.expression_transform( dnf_template, $sym81$PPH_NTH_PHRASE_NAUT_P, $sym83$PPH_PHRASE_TRANSFORM_NTH, UNPROVIDED, UNPROVIDED );
    }
    finally
    {
  ...
  public static SubLObject pph_phrase_transform_nth(final SubLObject nth_phrase_naut)
  {
    ...
    return pph_phrase.pph_phrase_dtr_count( $pph_phrase_transform_nth_reference_phrase$.getDynamicValue( thread ) ).numG( dtr_num ) ? pph_phrase.pph_phrase_nth_dtr( $pph_phrase_transform_nth_reference_phrase$
        .getDynamicValue( thread ), dtr_num ) : NIL;
  }
    ...
    if( NIL != pph_phrase.pph_phrase_p( target, UNPROVIDED ) && NIL != pph_phrase.pph_phrase_doneP( target ) && NIL != list_utilities.empty_list_p( pph_phrase.pph_phrase_local_alternatives( phrase ) ) )
    {
      ...
      while ( NIL == queues.queue_empty_p( queue ))
      {
        ...
        while ( NIL != cdolist_list_var)
        {
          ...
          number_added = Numbers.add( number_added, ONE_INTEGER );
          ...
          {
            final SubLObject new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
                $str85$Done__S_alternatives_of__S )
            } );
            pph_error.pph_handle_error( new_format_string, ConsesLow.list( done_count, target ) );
          }
  ...
  {
    if( NIL != pph_phrase.pph_phrase_has_agr_constraint_type_p( phrase, $kw87$SUBJECT_VERB ) )
    {
      pph_phrase_set_subject_verb_agreement( phrase, matrix_phrase );
    }
    else if( NIL != pph_phrase.pph_phrase_has_agr_constraint_type_p( phrase, $kw88$DET_NBAR ) )
    {
      pph_phrase_set_det_nbar_agreement( phrase, matrix_phrase );
    }
    else if( NIL != pph_phrase.pph_phrase_has_agr_constraint_type_p( phrase, $kw89$PEER ) )
    {
      pph_phrase_set_peer_agreement( phrase, matrix_phrase );
    }
    else if( NIL != pph_phrase.pph_phrase_has_agr_constraint_type_p( phrase, $kw90$TENSE ) )
    {
  ...
  public static SubLObject anaphor_needed_for_phraseP(final SubLObject phrase)
  {
    ...
    if( NIL != pph_vars.$pph_introduce_anaphorsP$.getDynamicValue( thread ) && NIL == pph_vars.pph_generate_alternative_phrasesP() && NIL != pph_phrase.pph_phrase_p( phrase, UNPROVIDED ) && NIL != pph_phrase
        .pph_phrase_has_known_cyclP( phrase ) )
    {
      final SubLObject phrase_cycl = pph_phrase.pph_phrase_cycl( phrase, UNPROVIDED );
      v_answer = makeBoolean( NIL != pph_drs.pph_discourse_referentP( phrase_cycl ) && NIL == pph_macros.pph_mapped_string_for_term( phrase_cycl ) && ( NIL == cycl_variables.el_varP( phrase_cycl )
          || NIL == pph_variable_handling.pph_var_paraphrased_explicitlyP( phrase_cycl ) ) && NIL != pph_utilities.pph_possible_pronoun_denotP( phrase_cycl, T ) && NIL != pph_drs
              .pph_anaphoric_to_backward_looking_centerP( phrase ) && ( NIL == pph_phrase.pph_phrase_has_ancestor_of_categoryP( phrase, pph_utilities.pph_np_category() ) || $const31$PossessivePhrase.eql( pph_phrase
                  .pph_phrase_category( phrase, UNPROVIDED ) ) ) && NIL == pph_methods_lexicon.pph_maybe_ambiguous_pronounP( phrase_cycl ) && NIL != pph_category_ok_for_anaphorP( pph_phrase.pph_phrase_category( phrase,
                      UNPROVIDED ) ) );
    }
    return v_answer;
  }
  ...
  {
    return makeBoolean( $kw97$CYCL_PHRASE == category || NIL != pph_utilities.pph_npP( category ) || $const31$PossessivePhrase.eql( category ) );
  }
  ...
  public static SubLObject reflexive_needed_for_phraseP(final SubLObject phrase)
  {
    ...
    if( NIL != pph_phrase.pph_phrase_p( phrase, UNPROVIDED ) )
    {
      ...
      if( $const31$PossessivePhrase.eql( pph_phrase.pph_phrase_category( phrase, UNPROVIDED ) ) )
      {
        return NIL;
      }
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          if( ( NIL == pph_phrase.pph_phrase_p( rm_phrase, UNPROVIDED ) || !pph_phrase.pph_phrase_cycl( rm_phrase, UNPROVIDED ).equal( phrase_cycl ) ) && NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER )
              && NIL == pph_error.suppress_pph_warningsP() )
          {
  ...
  public static SubLObject pph_phrase_source_pos(final SubLObject phrase, SubLObject v_default)
  {
    ...
    return pph_data_structures.pph_phrase_info_lookup( phrase, $kw100$SOURCE_POS, v_default );
  }
  ...
  {
    return pph_data_structures.pph_phrase_info_set( phrase, $kw100$SOURCE_POS, pos );
  }
  ...
  public static SubLObject pph_phrase_replace_generic_args_top_level(final SubLObject phrase, final SubLObject formula, final SubLObject arg_position_map)
  {
    ...
    else if( NIL != pph_utilities.pph_cycl_template_p( cycl_template ) && ( NIL == pph_vars.$pph_keep_generic_argsP$.getDynamicValue( thread ) || NIL == pph_utilities.pph_cycl_template_p( formula ) ) )
    {
      pph_phrase.pph_phrase_register_local_arg_positions( phrase, arg_position_map );
    }
    else if( NIL != pph_utilities.pph_known_arg_position_p( pph_utilities.pph_arg_position_lookup( $list72, arg_position_map ) ) && NIL != pph_phrase.pph_unknown_cycl_p( cycl_template ) && NIL != pph_phrase_ok_for_arg0P(
        phrase ) )
    {
      if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
      {
        format_nil.force_format( T, $str101$__Using_arg0_for__S_with_formula_, phrase, pph_vars.$pph_formula_for_generic_arg_replacement$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
            UNPROVIDED, UNPROVIDED );
      }
      ...
      final SubLObject new_map = pph_utilities.pph_figure_new_arg_position_map( arg_position_map, $kw102$ARG0 );
    ...
    else if( NIL != pph_utilities.pph_identity_map_p( pph_phrase.pph_phrase_arg_position_map( phrase ) ) )
    {
    ...
    return Values.values( pph_phrase.pph_phrase_cycl( phrase, UNPROVIDED ), pph_phrase.pph_phrase_output_list( phrase ), pph_phrase.pph_phrase_arg_position_map( phrase ) );
  }
  ...
  public static SubLObject pph_phrase_ok_for_arg0P(final SubLObject phrase)
  {
    ...
    final SubLObject result = makeBoolean( NIL != pph_phrase_reserved_for_arg0_p( phrase ) && NIL != pph_utilities.pph_unknown_arg_position_p( pph_phrase.pph_phrase_arg_position( phrase ) ) && NIL != list_utilities
        .lengthL( pph_phrase.pph_phrase_output_list( phrase ), TWO_INTEGER, UNPROVIDED ) && ( NIL != pph_phrase.pph_phrase_wu( phrase, UNPROVIDED ) || ( NIL != pph_string.non_empty_pph_stringP( string )
            && NIL == pph_utilities.pph_final_punctuation_string_p( string ) ) ) );
    return result;
  }
  ...
  public static SubLObject pph_phrase_replace_generic_args_recursive(final SubLObject phrase, final SubLObject formula, final SubLObject arg_position_map)
  {
    ...
    if( NIL != pph_types.pph_phrase_nautP( new_cycl ) )
    {
      ...
      if( NIL != agr_pred )
      {
        new_cycl = ConsesLow.list( pph_functions.phraseformfn(), agr_pred, new_cycl );
        local_arg_position_map = pph_utilities.pph_contextualize_map( $list109, local_arg_position_map );
      }
      if( NIL != category )
      {
        new_cycl = ConsesLow.list( pph_functions.phraseformfn(), category, new_cycl );
        local_arg_position_map = pph_utilities.pph_contextualize_map( $list109, local_arg_position_map );
      }
      ...
      if( category.eql( $const31$PossessivePhrase ) && NIL != pph_utilities.pph_npP( pph_phrase.pph_phrase_category( phrase, UNPROVIDED ) ) )
      {
        pph_phrase.pph_phrase_set_category( phrase, category, UNPROVIDED );
      }
      else if( NIL != category && NIL == pph_phrase.pph_phrase_reconcile_categories( category, pph_phrase.pph_phrase_category( phrase, UNPROVIDED ), UNPROVIDED ) )
      {
        final SubLObject new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
            $str110$Can_t_reconcile__S_and__S_ )
        } );
        pph_error.pph_handle_error( new_format_string, ConsesLow.list( category, pph_phrase.pph_phrase_category( phrase, UNPROVIDED ) ) );
      }
    }
    else
    {
    ...
    return Values.values( ( NIL != pph_phrase.pph_phrase_has_known_cyclP( phrase ) ) ? pph_phrase.pph_phrase_cycl( phrase, UNPROVIDED ) : pph_phrase.pph_phrase_category( phrase, UNPROVIDED ), pph_phrase
        .pph_phrase_arg_position_map( phrase ) );
  }
  ...
  public static SubLObject pph_phrase_reserve_matrix_arg0_cycl(final SubLObject phrase)
  {
    ...
    SubLObject result = $kw111$NO_ACTION;
  ...
  {
    final SubLObject result = ConsesLow.cons( arg0, arg0_position );
    pph_data_structures.pph_phrase_info_set_by_keyword_or_integer( phrase, $kw114$RESERVED_FOR_MATRIX_ARG0, ONE_INTEGER, result );
    return result;
  }
  ...
  {
    return pph_data_structures.pph_phrase_info_lookup_by_keyword_or_integer( phrase, $kw114$RESERVED_FOR_MATRIX_ARG0, ONE_INTEGER, NIL );
  }
  ...
  public static SubLObject pph_chemical_formula_from_lists_phrase_get_string(final SubLObject phrase)
  {
    ...
    pph_phrase.pph_phrase_set_output_list( phrase, ConsesLow.list( item ), UNPROVIDED );
    pph_phrase.pph_phrase_note_done( phrase );
    return pph_phrase.pph_phrase_output_list( phrase );
  }
  ...
  public static SubLObject pph_output_item_from_atomic_composition_lists(final SubLObject element_list, final SubLObject quantity_list)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject output_items = NIL;
    SubLObject abortP = makeBoolean( NIL == el_utilities.el_list_p( element_list ) || NIL == el_utilities.el_list_p( quantity_list ) || NIL == list_utilities.same_length_p( el_utilities.el_list_items( element_list ),
        el_utilities.el_list_items( quantity_list ) ) );
    if( NIL == abortP )
    {
      ...
      while ( NIL == abortP && ( NIL != quantity || NIL != element ))
      {
        ...
        {
          output_items = ConsesLow.cons( pph_data_structures.new_pph_phrase_output_item( atomic_symbol, NIL, element_$9, UNPROVIDED, UNPROVIDED, UNPROVIDED ), output_items );
        }
        else
        {
          abortP = T;
        ...
        {
          final SubLObject quantity_item = pph_data_structures.new_pph_phrase_output_item( print_high.princ_to_string( quantity_$10 ), NIL, quantity_$10, UNPROVIDED, UNPROVIDED, UNPROVIDED );
          if( pph_vars.$paraphrase_mode$.getDynamicValue( thread ) == $kw115$HTML )
          {
            pph_html.pph_phrase_output_item_subscript( quantity_item );
          }
          output_items = ConsesLow.cons( quantity_item, output_items );
        }
    ...
    return ( NIL != abortP ) ? NIL : pph_data_structures.new_pph_phrase_output_item_nospace_group( Sequences.nreverse( output_items ) );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return pph_utilities.pph_ask_variable( $sym116$_SYMBOL, ConsesLow.listS( $const117$atomicSymbol, element, $list118 ), pph_vars.$pph_language_mt$.getDynamicValue( thread ), NIL, ONE_INTEGER ).first();
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject symbol = pph_data_structures.pph_phrase_info_lookup( phrase, $kw119$SYMBOL, UNPROVIDED );
    ...
    if( NIL != pph_string.pph_string_p( string ) )
    {
      ...
      pph_phrase.pph_phrase_set_agr_pred( phrase, $const120$nameString, UNPROVIDED );
    }
    else if( NIL != subl_promotions.memberP( phrase, $pph_symbol_phrase_stack$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ) )
    {
    ...
    else
    {
      ...
      try
      {
        $pph_symbol_phrase_stack$.bind( ConsesLow.cons( phrase, $pph_symbol_phrase_stack$.getDynamicValue( thread ) ), thread );
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL == kb_indexing_datastructures.indexed_term_p( symbol ) )
    {
    ...
    SubLObject string = pph_utilities.pph_ask_variable( $sym122$_CODE, ConsesLow.listS( $const123$htmlGlyphCode, symbol, $list124 ), pph_vars.$pph_language_mt$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED ).first();
    if( !string.isString() )
    {
      string = pph_utilities.pph_ask_variable( $sym122$_CODE, ConsesLow.listS( $const125$htmlStringVersionOfCharacter, symbol, $list124 ), pph_vars.$pph_language_mt$.getDynamicValue( thread ), UNPROVIDED, UNPROVIDED )
          .first();
    }
    if( !string.isString() )
    {
      string = pph_methods_lexicon.some_name_for_term( symbol, $const126$termStrings, pph_vars.$pph_language_mt$.getDynamicValue( thread ) );
    }
  ...
  public static SubLObject pph_phrase_output_list_add_sentential_force(final SubLObject olist, final SubLObject force)
  {
    ...
    else if( force.eql( $kw128$INTERROGATIVE ) || force.eql( $kw129$QUESTION ) )
    {
      pph_phrase_output_list_upcase_leading_smart( olist );
  ...
  {
    if( force.eql( $kw127$DECLARATIVE ) || force.eql( $kw128$INTERROGATIVE ) || force.eql( $kw129$QUESTION ) )
    {
  ...
  public static SubLObject pph_string_add_sentence_final_force(final SubLObject string, final SubLObject force)
  {
    ...
    if( force.eql( $kw128$INTERROGATIVE ) || force.eql( $kw129$QUESTION ) )
    {
  ...
  public static SubLObject pph_phrase_output_list_upcase_leading_smart(final SubLObject olist)
  {
    if( NIL == olist )
    {
      return Values.values( olist, NIL );
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != pph_string.non_empty_pph_stringP( string ) && NIL != pph_string.pph_string_find_if( $sym134$ALPHANUMERIC_CHAR_P, string ) )
      {
        final SubLObject new_car = pph_phrase_output_item_upcase_leading_smart( item );
        final SubLObject position = Sequences.position( item, olist, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
        list_utilities.nreplace_nth( position, new_car, olist );
        return Values.values( olist, T );
      }
      cdolist_list_var = cdolist_list_var.rest();
      item = cdolist_list_var.first();
    }
  ...
  {
    if( NIL != Characters.upper_case_p( pph_string.pph_string_char( string, ZERO_INTEGER ) ) )
    {
      return T;
    }
    return NIL;
  ...
  public static SubLObject pph_phrase_output_list_add_punctuation_smart(final SubLObject olist, final SubLObject punc_string)
  {
    ...
    if( NIL != tail )
    {
      ...
      if( NIL != pph_utilities.pph_punctuation_ok_after_stringP( last_string, punc_string ) )
      {
        ...
        ConsesLow.rplacd( tail, ConsesLow.cons( new_item, trailing_items ) );
      }
    }
    return olist;
  }
  ...
  public static SubLObject pph_output_list_nonempty_tail(final SubLObject olist)
  {
    ...
    if( NIL == ans )
    {
      ...
      SubLObject n;
      SubLObject tail;
      for( end_var = number_utilities.f_1X( Sequences.length( olist ) ), n = NIL, n = ONE_INTEGER; NIL == ans && !n.numGE( end_var ); n = number_utilities.f_1X( n ) )
      {
  ...
  public static SubLObject pph_percent_phrase_get_string(final SubLObject phrase, final SubLObject top_level_cycl)
  {
    ...
    {
      pph_phrase.pph_phrase_set_cycl( phrase, Numbers.multiply( $int136$100, percent_as_decimal ) );
      final SubLObject cutoff = pph_float_precision( percent_as_decimal );
      final SubLObject candidates = list_utilities.remove_if_not( $sym137$POSITIVE_INTEGER_P, ConsesLow.list( cutoff, pph_vars.$pph_significant_digits_cutoff$.getDynamicValue( thread ) ), UNPROVIDED, UNPROVIDED,
          UNPROVIDED, UNPROVIDED );
      ...
      try
      {
        ...
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
          format_nil.force_format( T, $str138$Using_significant_digits_cutoff_o, pph_vars.$pph_significant_digits_cutoff$.getDynamicValue( thread ), pph_phrase.pph_phrase_cycl( phrase, UNPROVIDED ), UNPROVIDED,
  ...
  public static SubLObject pph_float_precision(final SubLObject v_float)
  {
    ...
    final SubLObject no_dot = Sequences.remove( Characters.CHAR_period, float_string, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    final SubLObject trimmed = Strings.string_left_trim( $list140, no_dot );
    final SubLObject first_alpha_position = Sequences.position_if( $sym141$ALPHA_CHAR_P, trimmed, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    final SubLObject cutoff = ( NIL != first_alpha_position ) ? first_alpha_position : Sequences.length( trimmed );
  ...
  public static SubLObject pph_coordination_phrase_get_string(final SubLObject phrase, final SubLObject top_level_cycl)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject abortP = NIL;
    if( NIL != pph_nbar_coordination_phrase_p( phrase ) && NIL != pph_phrase.pph_phrase_p( pph_phrase.pph_phrase_mother( phrase ), UNPROVIDED ) && NIL != pph_phrase.pph_phrase_p( pph_phrase.pph_phrase_mother( pph_phrase
        .pph_phrase_mother( phrase ) ), UNPROVIDED ) )
    {
    ...
    if( NIL == abortP )
    {
      ...
      try
      {
        ...
        {
          final SubLObject new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
              $str142$__Recursion_limit___S__exceeded_i )
          } );
          pph_error.pph_handle_error( new_format_string, ConsesLow.list( $pph_coordination_phrase_nesting_max$.getDynamicValue( thread ) ) );
        }
        else
        {
  ...
  public static SubLObject pph_coordination_phrase_get_string_internal(final SubLObject phrase, final SubLObject top_level_cycl)
  {
    ...
    if( dtrs_resolvedP == $kw30$RESOLVED )
    {
      SubLObject current;
      ...
      if( NIL == current )
      {
        if( coord_keyword.eql( $kw144$AND ) && NIL != pph_phrase_agrees_with_no_quantified_phrase_p( pph_phrase.pph_phrase_maximal_projection( phrase ) ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
        ...
        if( document.sign_constituents( phrase ).isVector() )
        {
          ...
          for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
          {
            ...
            if( NIL != pos && pos.numG( idx ) )
            {
              ...
              new_dtrs = ConsesLow.cons( dummy_phrase, new_dtrs );
            }
        ...
        if( $kw144$AND == coord_keyword && NIL != pph_utilities.pph_genl_posP( pph_utilities.pph_pos_of_category( pph_phrase.pph_phrase_category( pph_phrase.pph_phrase_nth_dtr( phrase, ZERO_INTEGER ), UNPROVIDED ),
            UNPROVIDED ), $const148$Noun, UNPROVIDED ) )
        {
          final SubLObject agr_pred = pph_phrase.pph_phrase_dtr_count( phrase ).numG( ONE_INTEGER ) ? $const149$plural_Generic : $const150$singular_Generic;
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
  ...
  {
    return Sequences.search( dtr_olist, olist, Symbols.symbol_function( EQUAL ), Symbols.symbol_function( IDENTITY ), ZERO_INTEGER, NIL, idx, UNPROVIDED );
  }
  ...
  public static SubLObject pph_no_quantified_det_nbar_phrase_p(final SubLObject v_object)
  {
    return makeBoolean( NIL != pph_phrase.pph_phrase_p( v_object, NIL ) && NIL != pph_phrase.pph_phrase_npP( v_object ) && TWO_INTEGER.eql( pph_phrase.pph_phrase_dtr_count( v_object ) ) && ( NIL != pph_phrase_detpP(
        pph_phrase.pph_phrase_nth_dtr( v_object, ZERO_INTEGER ) ) || NIL != pph_phrase_determinerP( pph_phrase.pph_phrase_nth_dtr( v_object, ZERO_INTEGER ) ) ) && $const152$No_NLAttr.eql( pph_phrase_lexical_head_cycl(
            pph_phrase.pph_phrase_nth_dtr( v_object, ZERO_INTEGER ), UNPROVIDED ) ) );
  }
  ...
  public static SubLObject pph_nbar_coordination_phrase_distribute_determiner(final SubLObject phrase, final SubLObject top_level_cycl)
  {
    ...
    if( NIL == problemP && TWO_INTEGER.eql( pph_phrase.pph_phrase_dtr_count( mother_np ) ) && NIL != pph_utilities.pph_genl_posP( pph_utilities.pph_pos_of_category( pph_phrase.pph_phrase_category( det_sister,
        UNPROVIDED ), UNPROVIDED ), $const153$Determiner, UNPROVIDED ) )
    {
      ...
      if( NIL != repeated_phrase_dtrs_resolvedP )
      {
        ...
        if( document.sign_constituents( phrase ).isVector() )
        {
          ...
          for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
          {
            ...
            pph_phrase.set_pph_phrase_dtrs_from_list( new_np_conjunct, ConsesLow.list( det_sister_copy, nbar_conjunct_copy ), UNPROVIDED );
    ...
    return Values.values( makeBoolean( NIL == problemP ), ( NIL != problemP ) ? NIL : mother_np );
  }
  ...
  {
    final SubLObject subject_pointer = pph_phrase.pph_phrase_agr_target_of_type( verbal_phrase, $kw87$SUBJECT_VERB );
    ...
    {
      final SubLObject new_format_string = Sequences.cconcatenate( $str5$_PPH_error_level_, new SubLObject[] { format_nil.format_nil_s_no_copy( ONE_INTEGER ), $str6$__, format_nil.format_nil_a_no_copy(
          $str155$subject_pointer_is_nth_phrase___S )
      } );
      pph_error.pph_handle_error( new_format_string, ConsesLow.list( subject_pointer ) );
    ...
    final SubLObject matrix_constraints = ( NIL != pph_phrase.pph_phrase_head_dtrP( verbal_phrase, matrix_phrase ) ) ? pph_phrase.pph_phrase_absolute_agr_preds( matrix_phrase ) : $kw157$ANY;
    ...
    final SubLObject filtered_constraints = pph_utilities.pph_filter_preds( verbs_constraints, lexicon_accessors.max_preds_of_pos( $const158$Verb ), UNPROVIDED );
  ...
  public static SubLObject pph_phrase_set_peer_agreement(final SubLObject phrase, final SubLObject matrix_phrase)
  {
    ...
    if( NIL != pph_utilities.pph_isaP( pph_phrase.pph_phrase_cycl( target, UNPROVIDED ), $const163$MeasurableQuantity, UNPROVIDED ) )
    {
      target_agr_pred = $const150$singular_Generic;
    }
    if( NIL != pph_utilities.pph_english_contextP( UNPROVIDED ) && NIL != pph_utilities.pph_speech_part_predP( pph_phrase.pph_phrase_agr_pred( phrase ) ) && NIL != pph_utilities.pph_genl_pos_predP( pph_phrase
        .pph_phrase_agr_pred( phrase ), $const164$adjStrings, UNPROVIDED ) )
    {
      if( NIL == pph_utilities.pph_genl_posP( pph_phrase.pph_phrase_category( phrase, UNPROVIDED ), $const165$Adjective, UNPROVIDED ) && NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER )
          && NIL == pph_error.suppress_pph_warningsP() )
      {
    ...
    else if( NIL != pph_utilities.pph_genl_pos_predP( pph_phrase.pph_phrase_agr_pred( phrase ), $const168$untensed, UNPROVIDED ) )
    {
      pph_phrase.pph_phrase_set_agr_pred( phrase, pph_phrase.pph_phrase_agr_pred( phrase ), UNPROVIDED );
      if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
      {
    ...
    else
    {
      ...
      if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
      {
        format_nil.force_format( T, $str171$__Set_peer_agreement_to_default__, pph_utilities.$pph_default_np_agr_pred$.getGlobalValue(), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
            UNPROVIDED );
      }
    }
    return phrase;
  ...
  {
    SubLObject agr_preds = $kw157$ANY;
    final SubLObject matrix_preds = pph_phrase.pph_phrase_agr_preds( matrix_phrase.eql( $kw64$MOTHER ) ? pph_phrase.pph_phrase_mother( phrase ) : matrix_phrase, UNPROVIDED );
    if( NIL != pph_utilities.pph_nl_pred_listP( matrix_preds ) )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        if( NIL != pph_utilities.pph_genl_pos_predP( matrix_pred, $const172$tensed, UNPROVIDED ) )
        {
          agr_preds = pph_utilities.pph_filter_preds( agr_preds, ConsesLow.list( matrix_pred ), UNPROVIDED );
        }
        cdolist_list_var = cdolist_list_var.rest();
        matrix_pred = cdolist_list_var.first();
      }
  ...
  {
    return makeBoolean( string.equal( PrintLow.format( NIL, $str174$_S, cycl ) ) || string.equal( pph_vars.$pph_blank_string$.getGlobalValue() ) );
  }
  ...
  public static SubLObject pph_phrase_set_det_nbar_agreement(final SubLObject phrase, final SubLObject matrix_phrase)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject target = pph_phrase_agr_target_phrase( phrase, matrix_phrase );
    final SubLObject phrase_is_detP = Equality.eq( pph_utilities.pph_pos_of_category( pph_phrase.pph_phrase_category( phrase, UNPROVIDED ), UNPROVIDED ), $const153$Determiner );
    ...
    try
    {
      ...
      if( NIL != phrase_is_detP && pph_phrase.pph_phrase_cycl( det, UNPROVIDED ).eql( $const176$BareForm_NLAttr ) )
      {
      ...
      else if( NIL != phrase_is_detP && NIL != pph_phrase.pph_phrase_doneP( nbar ) && NIL != pph_phrase.pph_phrase_indefinite_articleP( det ) && NIL != pph_utilities.pph_english_contextP( UNPROVIDED ) )
      {
        pph_phrase_english_indefinite_article( det, nbar );
      }
      else if( NIL != phrase_is_detP && NIL != pph_phrase.pph_phrase_doneP( nbar ) && NIL != pph_variable_handling.pph_string_is_posP( string_utilities.string_tokenize( pph_string.pph_string_to_display( pph_phrase
          .pph_phrase_string( nbar, NIL ) ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ).first(), $const178$PossessivePronoun_Pre, $kw179$NEVER, UNPROVIDED ) )
      {
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
      ...
      else if( NIL != phrase_is_detP && NIL != pph_phrase.pph_ranged_det_phrase_p( phrase, UNPROVIDED ) )
      {
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
        ...
        pph_phrase.pph_phrase_set_agr_pred( nbar, $const149$plural_Generic, UNPROVIDED );
      }
      else if( NIL != phrase_is_detP && NIL != nbar_agr_pred )
      {
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
      ...
      else if( NIL == phrase_is_detP && NIL != lexicon_accessors.agreement_lookup( det_wu ) )
      {
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
      ...
      else if( NIL != cycl_utilities.expression_find_if( $sym185$EL_VAR_, det_head_cycl, UNPROVIDED, UNPROVIDED ) )
      {
        if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
        {
        ...
        pph_phrase.pph_phrase_set_agr_pred( phrase, $const187$nonSingular_Generic, UNPROVIDED );
      ...
      else if( NIL != pph_utilities.pph_numeric_det_cyclP( det_head_cycl ) )
      {
        final SubLObject numeric_agr_pred = pph_utilities.numeric_det_agr_pred( det_head_cycl );
        if( NIL != numeric_agr_pred )
        {
          final SubLObject filtered = pph_utilities.pph_filter_preds( ConsesLow.list( numeric_agr_pred ), pph_phrase.pph_phrase_agr_preds( nbar, UNPROVIDED ), UNPROVIDED );
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
  ...
  public static SubLObject pph_phrase_lexical_head_cycl(final SubLObject phrase, SubLObject matrix_formula)
  {
    ...
    if( NIL != el_utilities.el_formula_p( matrix_formula ) )
    {
  ...
  public static SubLObject pph_phrase_find_potential_binders_int(final SubLObject phrase, final SubLObject look_in, final SubLObject calling_phrase, final SubLObject subjects, final SubLObject others,
      final SubLObject embedded_only)
  {
      ...
      for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
      {
        ...
        if( !dtr.eql( calling_phrase ) )
        {
          if( dtr.eql( the_subject ) )
          {
            if( NIL != subjects )
            {
              potentials = ConsesLow.cons( dtr, potentials );
            }
          }
          else if( NIL != pph_phrase.pph_phrase_npP( dtr ) && NIL != others )
          {
            potentials = ConsesLow.cons( dtr, potentials );
          }
        }
        do_num = Numbers.add( do_num, ONE_INTEGER );
      }
    }
    ...
    {
      return list_utilities.flatten( ConsesLow.list( potentials, recursed ) );
    }
  ...
  public static SubLObject pph_phrase_english_indefinite_article(final SubLObject det, final SubLObject nbar)
  {
    ...
    final SubLObject already_indefiniteP = pph_string.pph_string_starts_with( nbar_string, $str191$some, UNPROVIDED );
    ...
    else if( NIL != already_indefiniteP )
    {
      pph_phrase.pph_phrase_set_string( det, string_utilities.$empty_string$.getGlobalValue() );
    }
    else if( NIL != sing_nbarP )
    {
      pph_phrase.pph_phrase_set_string( det, pph_singular_determiner_string( nbar_string ) );
    }
    else if( NIL != list_utilities.lengthG( nbar_string, TWO_INTEGER, UNPROVIDED ) && NIL != pph_string.pph_string_starts_with( nbar_string, constant_reader.constant_reader_prefix(), UNPROVIDED )
        && ( NIL == fort_types_interface.collection_p( pph_phrase.pph_phrase_cycl( nbar, UNPROVIDED ) ) || NIL == pph_utilities.pph_isaP( pph_phrase.pph_phrase_cycl( nbar, UNPROVIDED ), $const193$ExistingStuffType,
            UNPROVIDED ) ) )
    {
    ...
    if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
    {
      format_nil.force_format( T, $str194$__Using_English_specific_indefini, pph_phrase.pph_phrase_string( det, NIL ), pph_phrase.pph_phrase_string( nbar, NIL ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
          UNPROVIDED, UNPROVIDED );
    }
  ...
  public static SubLObject pph_phrase_english_indefinite_np(final SubLObject det, final SubLObject nbar)
  {
    ...
    if( NIL != pph_phrase.pph_phrase_p( nbar_head, UNPROVIDED ) && NIL != pph_phrase.pph_phrase_doneP( nbar ) && NIL != pph_phrase.pph_phrase_compatible_with_predsP( nbar, $list196 ) && NIL == pph_phrase
        .pph_cycl_phrase_p( nbar ) && ( NIL == pph_phrase.pph_phrase_p( pph_phrase.pph_phrase_mother( nbar ), UNPROVIDED ) || NIL == pph_copular_complementP( pph_phrase.pph_phrase_mother( nbar ) ) ) )
    {
      ...
      for( rest = NIL, rest = $pph_english_lexical_indefinites$.getGlobalValue(); NIL == doneP && NIL != rest; rest = rest.rest() )
      {
        ...
        if( nbar_head_cycl.eql( type ) )
        {
          ...
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) )
          {
            format_nil.force_format( T, $str198$Nbar_string_is__S__Output_list___, pph_phrase.pph_phrase_string( nbar, NIL ), pph_phrase.pph_phrase_output_list( nbar ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
                UNPROVIDED, UNPROVIDED );
          }
  ...
  public static SubLObject pph_noun_predP(final SubLObject pred, SubLObject mt)
  {
    ...
    return pph_utilities.pph_genl_pos_predP( pred, $const199$nounStrings, mt );
  }
  ...
  {
    return pph_utilities.pph_genl_pos_predP( pred, $const150$singular_Generic, UNPROVIDED );
  }
  ...
  {
    return pph_utilities.pph_genl_pos_predP( pred, $const187$nonSingular_Generic, UNPROVIDED );
  }
  ...
  {
    return pph_utilities.select_determiner_string( $list200, target_string, $const150$singular_Generic );
  }
  ...
  public static SubLObject tense_pred_for_constraints(final SubLObject preds)
  {
    ...
    SubLObject result = pph_utilities.pph_unify_speech_part_preds( ConsesLow.list( unified, pph_utilities.pph_present_tense_pred() ) );
    if( NIL == result )
    {
      result = pph_utilities.pph_unify_speech_part_preds( ConsesLow.list( unified, pph_utilities.pph_past_tense_pred() ) );
    }
    if( NIL == result )
    {
      result = pph_utilities.pph_unify_speech_part_preds( ConsesLow.list( unified, pph_utilities.pph_future_tense_pred() ) );
    }
  ...
  public static SubLObject verb_preds_for_subject(final SubLObject subject_phrase, SubLObject internal_constraints)
  {
    ...
    final SubLObject index = pph_data_structures.pph_phrase_info_lookup( subject_phrase, $kw201$INDEX, UNPROVIDED );
  ...
  public static SubLObject verb_pred_for_subject(final SubLObject subject_phrase, SubLObject internal_constraints)
  {
    ...
    internal_constraints = pph_strengthen_verb_constraints( internal_constraints );
    if( NIL != pph_utilities.pph_genl_posP( pph_phrase.pph_phrase_category( subject_phrase, UNPROVIDED ), $const202$Pronoun, UNPROVIDED ) )
    {
  ...
  public static SubLObject pph_strengthen_verb_constraints(final SubLObject internal_constraints)
  {
    ...
    if( NIL == pph_vars.pph_generate_morphological_alternativesP() && ( !constraints.isList() || NIL != pph_verb_constraints_could_be_present_tenseP( ConsesLow.list( constraints.first() ) ) ) )
    {
  ...
  {
    return pph_utilities.pph_filter_preds( constraints, ConsesLow.list( pph_utilities.pph_present_tense_pred() ), UNPROVIDED );
  }
  ...
  {
    final SubLObject remaining = Sequences.remove( $const203$verbStrings, internal_constraints, $sym204$GENL_POS_PRED_, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    return ( NIL != remaining ) ? remaining : $kw157$ANY;
  }
  ...
  {
    if( NIL != pph_utilities.pph_isaP( pph_phrase.pph_phrase_cycl( subject_phrase, UNPROVIDED ), $const163$MeasurableQuantity, UNPROVIDED ) )
    {
      return ConsesLow.list( $const205$singularVerb_Generic );
    }
    SubLObject verb_preds = NIL;
    ...
    return ( NIL != verb_preds ) ? verb_preds : ConsesLow.list( pph_phrase_default_verb_pred() );
  }
  ...
  public static SubLObject verb_preds_for_subject_pronoun(final SubLObject subject_phrase, final SubLObject internal_constraints)
  {
    ...
    if( NIL != pph_drs.pph_discourse_referentP( referent ) && pph_drs.pph_discourse_person_for_cycl( referent ).eql( $const207$SecondPerson_NLAttr ) )
    {
      final SubLObject good_preds = pph_utilities.pph_filter_preds( internal_constraints, ConsesLow.list( $const208$secondPersonSg_Generic ), UNPROVIDED );
      return ( NIL != good_preds ) ? good_preds : ConsesLow.list( $const209$secondPersonSg_Present );
    }
    if( NIL != pph_drs.pph_discourse_referentP( referent ) && pph_drs.pph_discourse_person_for_cycl( referent ).eql( $const210$FirstPerson_NLAttr ) )
    {
      final SubLObject good_preds = pph_utilities.pph_filter_preds( internal_constraints, ConsesLow.list( $const211$firstPersonSg_Generic ), UNPROVIDED );
      return ( NIL != good_preds ) ? good_preds : ConsesLow.list( $const212$firstPersonSg_Present );
    }
    return verb_preds_for_subject_int( subject_phrase, internal_constraints );
  }
  ...
  public static SubLObject verb_pred_for_subject_pronoun(final SubLObject subject_phrase, final SubLObject internal_constraints)
  {
    ...
    {
      final SubLObject good_preds = pph_utilities.pph_filter_preds( internal_constraints, ConsesLow.list( $const208$secondPersonSg_Generic ), UNPROVIDED );
      return ( NIL != good_preds ) ? good_preds.first() : $const209$secondPersonSg_Present;
    }
    if( NIL != pph_drs.pph_discourse_referentP( referent ) && pph_drs.pph_discourse_person_for_cycl( referent ).eql( $const210$FirstPerson_NLAttr ) )
    {
      final SubLObject good_preds = pph_utilities.pph_filter_preds( internal_constraints, ConsesLow.list( $const211$firstPersonSg_Generic ), UNPROVIDED );
      return ( NIL != good_preds ) ? good_preds.first() : $const212$firstPersonSg_Present;
    }
    return verb_pred_for_subject_int( subject_phrase, internal_constraints );
  }
  ...
  public static SubLObject verb_preds_for_subject_preds(final SubLObject subj_preds, SubLObject internal_constraints)
  {
    ...
    if( NIL != list_utilities.empty_list_p( Sequences.find_if( Symbols.symbol_function( $sym214$PPH_FINITE_VERB_PRED_ ), tenses, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ) )
    {
      return tenses;
    }
    if( NIL != list_utilities.empty_list_p( list_utilities.find_if_not( Symbols.symbol_function( $sym215$PPH_NON_PLURAL_NOUN_PRED_ ), subj_agr_preds, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ) )
    {
      return pph_singular_verb_preds_for_tenses( tenses );
    }
    if( NIL != list_utilities.empty_list_p( list_utilities.find_if_not( Symbols.symbol_function( $sym216$PPH_PLURAL_NOUN_PRED_ ), subj_agr_preds, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ) )
    {
      return pph_plural_verb_preds_for_tenses( tenses );
    }
    if( NIL != list_utilities.empty_list_p( list_utilities.find_if_not( Symbols.symbol_function( $sym217$PPH_PLURAL_VERB_PRED_ ), tenses, UNPROVIDED, UNPROVIDED, UNPROVIDED ) ) )
    {
      return tenses;
    }
    if( subj_preds.equal( ConsesLow.list( pph_utilities.$pph_default_np_agr_pred$.getGlobalValue() ) ) )
    {
      return pph_singular_verb_preds_for_tenses( tenses );
    }
    return verb_preds_for_subject_preds( ConsesLow.list( pph_utilities.$pph_default_np_agr_pred$.getGlobalValue() ), tenses );
  }
  ...
  public static SubLObject verb_pred_for_subject_pred(final SubLObject subj_pred, SubLObject internal_constraints)
  {
    ...
    return verb_pred_for_subject_pred( pph_utilities.$pph_default_np_agr_pred$.getGlobalValue(), ConsesLow.list( tense ) );
  }
  ...
  {
    return pph_utilities.pph_genl_pos_predP( pos_pred, $const172$tensed, UNPROVIDED );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return pph_utilities.pph_genl_pos_predP( pred, $const219$nonPlural_Generic, pph_vars.$pph_language_mt$.getDynamicValue( thread ) );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return pph_utilities.pph_genl_pos_predP( pred, $const149$plural_Generic, pph_vars.$pph_language_mt$.getDynamicValue( thread ) );
  }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return pph_utilities.pph_genl_pos_predP( pred, $const220$pluralVerb_Generic, pph_vars.$pph_language_mt$.getDynamicValue( thread ) );
  }
  ...
  {
    return pph_utilities.pph_filter_preds( ConsesLow.list( $const221$thirdPersonSg_Generic ), tenses, UNPROVIDED );
  }
  ...
  {
    return pph_utilities.pph_unify_speech_part_preds_or_choose( $const221$thirdPersonSg_Generic, tense );
  }
  ...
  {
    return pph_utilities.pph_filter_preds( ConsesLow.list( $const220$pluralVerb_Generic ), tenses, UNPROVIDED );
  }
  ...
  {
    return pph_utilities.pph_unify_speech_part_preds_or_choose( $const220$pluralVerb_Generic, tense );
  }
  ...
  public static SubLObject pph_phrase_unify_pos_preds(final SubLObject phrase1, final SubLObject phrase2)
  {
    ...
    if( !p1_preds.equal( p2_preds ) )
    {
      ...
      try
      {
        ...
        else if( NIL != pph_phrase.pph_phrase_doneP( phrase1 ) && NIL != pph_phrase.pph_phrase_head_dtrP( phrase1, phrase2 ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) && NIL == pph_error.suppress_pph_warningsP() )
          {
        ...
        else if( NIL != pph_phrase.pph_phrase_doneP( phrase2 ) && NIL != pph_phrase.pph_phrase_head_dtrP( phrase2, phrase1 ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) && NIL == pph_error.suppress_pph_warningsP() )
          {
      ...
      if( NIL != pph_error.pph_trace_level_exceeds_minimumP( TWO_INTEGER ) )
      {
        format_nil.force_format( T, $str225$__AGRs_are_now____S_and____S___, pph_phrase.pph_phrase_agr( phrase1 ), pph_phrase.pph_phrase_agr( phrase2 ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED,
            UNPROVIDED );
      }
    }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return pph_utilities.pph_genl_posP( pph_phrase.pph_phrase_category( phrase, T ), $const153$Determiner, pph_vars.$pph_language_mt$.getDynamicValue( thread ) );
  }
  ...
  {
    return memoization_state.caching_state_remove_function_results_with_args( $pph_detp_nart_cached_caching_state$.getGlobalValue(), ConsesLow.list( EMPTY_SUBL_OBJECT_ARRAY ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject pph_detp_nart_cached()
  {
    ...
    {
      caching_state = memoization_state.create_global_caching_state_for_name( $sym226$PPH_DETP_NART_CACHED, $sym228$_PPH_DETP_NART_CACHED_CACHING_STATE_, NIL, EQL, ZERO_INTEGER, ZERO_INTEGER );
    }
    SubLObject results = memoization_state.caching_state_get_zero_arg_results( caching_state, UNPROVIDED );
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( Values.resetMultipleValues(), Values.multiple_value_list( pph_detp_nart_cached_internal() ) );
  ...
  public static SubLObject pph_wordify_noun(final SubLObject phrase)
  {
    ...
    {
      final SubLObject wu = lexicon_accessors.best_wu_for_string( string, $const148$Noun );
      if( NIL != wu )
      {
        final SubLObject pos = lexicon_accessors.best_pos_for_stringXwuXpos( string, wu, $const148$Noun );
  ...
  public static SubLObject pph_phrase_switch_arg1_and_arg2_in_dtrs(final SubLObject phrase)
  {
    ...
    if( document.sign_constituents( phrase ).isVector() )
    {
      ...
      for( length = Sequences.length( vector_var ), v_iteration = NIL, v_iteration = ZERO_INTEGER; v_iteration.numL( length ); v_iteration = Numbers.add( v_iteration, ONE_INTEGER ) )
      {
        element_num = ( ( NIL != backwardP_var ) ? Numbers.subtract( length, v_iteration, ONE_INTEGER ) : v_iteration );
        dtr = Vectors.aref( vector_var, element_num );
        if( NIL != pph_phrase.pph_repeated_phrase_p( dtr ) && NIL != pph_phrase.pph_repeated_phrase_references_argP( dtr, $kw230$ARG2 ) )
        {
          if( NIL != pph_error.pph_trace_level_exceeds_minimumP( ONE_INTEGER ) && NIL == pph_error.suppress_pph_warningsP() )
          {
  ...
  {
    SubLFiles.declareFunction( me, "pph_note_unfinished_phrase", "PPH-NOTE-UNFINISHED-PHRASE", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_note_unfinished_dtrs", "PPH-PHRASE-NOTE-UNFINISHED-DTRS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_get_string", "PPH-PHRASE-GET-STRING", 1, 2, false );
    SubLFiles.declareFunction( me, "pph_quoted_phrase_get_string", "PPH-QUOTED-PHRASE-GET-STRING", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_nospace_phrase_get_string", "PPH-NOSPACE-PHRASE-GET-STRING", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_maybe_throw_hl_supports", "PPH-PHRASE-MAYBE-THROW-HL-SUPPORTS", 1, 0, false );
    SubLFiles.declareFunction( me, "maybe_throw_pph_hl_supports", "MAYBE-THROW-PPH-HL-SUPPORTS", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_nospace_phrase_output_list_from_dtr", "PPH-NOSPACE-PHRASE-OUTPUT-LIST-FROM-DTR", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_get_string_from_dtrs", "PPH-PHRASE-GET-STRING-FROM-DTRS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_try_to_resolve_dtr_strings", "PPH-PHRASE-TRY-TO-RESOLVE-DTR-STRINGS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_repeated_phrase_dtrs", "PPH-PHRASE-RESOLVE-REPEATED-PHRASE-DTRS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_repeated_phrase_expansion", "PPH-REPEATED-PHRASE-EXPANSION", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_repeated_phrase_expansion_internal", "PPH-REPEATED-PHRASE-EXPANSION-INTERNAL", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_one_repeated_phrase", "PPH-ONE-REPEATED-PHRASE", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_generic_arg_sequence", "PPH-GENERIC-ARG-SEQUENCE", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_current_list_item", "PPH-CURRENT-LIST-ITEM", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_lookup_current_list_item", "PPH-LOOKUP-CURRENT-LIST-ITEM", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_extensional_list_p", "PPH-EXTENSIONAL-LIST-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_current_list_items", "PPH-PHRASE-RESOLVE-CURRENT-LIST-ITEMS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_resolve_current_list_items", "PPH-RESOLVE-CURRENT-LIST-ITEMS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_current_list_item_formula_p", "PPH-CURRENT-LIST-ITEM-FORMULA-P", 1, 0, false );
    new $pph_current_list_item_formula_p$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_current_list_item_formula_item", "PPH-CURRENT-LIST-ITEM-FORMULA-ITEM", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_repeated_phrase_increment", "PPH-REPEATED-PHRASE-INCREMENT", 0, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_try_to_resolve_dtr_string", "PPH-PHRASE-TRY-TO-RESOLVE-DTR-STRING", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_clarifying_dtr", "PPH-PHRASE-RESOLVE-CLARIFYING-DTR", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_note_phraseXdtr_agr", "PPH-NOTE-PHRASE&DTR-AGR", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_dtr_relative_agr", "PPH-PHRASE-SET-DTR-RELATIVE-AGR", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_dtr", "PPH-PHRASE-RESOLVE-DTR", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_arg0_items", "PPH-PHRASE-SET-ARG0-ITEMS", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_maybe_make_arg0_item", "PPH-PHRASE-MAYBE-MAKE-ARG0-ITEM", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_make_arg0_item", "PPH-PHRASE-MAKE-ARG0-ITEM", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_independent_phraseP", "PPH-PHRASE-INDEPENDENT-PHRASE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_agr_target_resolvedP", "PPH-PHRASE-AGR-TARGET-RESOLVED?", 2, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_output_list_from_finished_dtrs", "PPH-PHRASE-SET-OUTPUT-LIST-FROM-FINISHED-DTRS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_item_empty_p", "PPH-PHRASE-OUTPUT-ITEM-EMPTY-P", 1, 0, false );
    new $pph_phrase_output_item_empty_p$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_phrase_arg0_item", "PPH-PHRASE-ARG0-ITEM", 2, 1, false );
    SubLFiles.declareFunction( me, "reverse_string_list_term_anchor", "REVERSE-STRING-LIST-TERM-ANCHOR", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_arg_dtrs", "PPH-PHRASE-ARG-DTRS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_arg_phraseP", "PPH-PHRASE-ARG-PHRASE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_arg0_phraseP", "PPH-PHRASE-ARG0-PHRASE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_condition", "PPH-PHRASE-RESOLVE-CONDITION", 3, 0, false );
    SubLFiles.declareFunction( me, "incomplete_pph_phrase_p", "INCOMPLETE-PPH-PHRASE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_indexicals_in_test", "PPH-PHRASE-RESOLVE-INDEXICALS-IN-TEST", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_nth_phrase_naut_p", "PPH-NTH-PHRASE-NAUT-P", 1, 0, false );
    new $pph_nth_phrase_naut_p$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_phrase_resolve_nth_phrases", "PPH-PHRASE-RESOLVE-NTH-PHRASES", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_transform_nth", "PPH-PHRASE-TRANSFORM-NTH", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_relative_agr", "PPH-PHRASE-SET-RELATIVE-AGR", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_relative_agr_internal", "PPH-PHRASE-SET-RELATIVE-AGR-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_resolve_string", "PPH-PHRASE-RESOLVE-STRING", 2, 0, false );
    SubLFiles.declareFunction( me, "anaphor_needed_for_phraseP", "ANAPHOR-NEEDED-FOR-PHRASE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_category_ok_for_anaphorP", "PPH-CATEGORY-OK-FOR-ANAPHOR?", 1, 0, false );
    SubLFiles.declareFunction( me, "reflexive_needed_for_phraseP", "REFLEXIVE-NEEDED-FOR-PHRASE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_source_pos", "PPH-PHRASE-SOURCE-POS", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_source_pos", "PPH-PHRASE-SET-SOURCE-POS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_replace_generic_args", "PPH-PHRASE-REPLACE-GENERIC-ARGS", 2, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_replace_generic_args_top_level", "PPH-PHRASE-REPLACE-GENERIC-ARGS-TOP-LEVEL", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_update_generic_args_in_conditional_test", "PPH-PHRASE-UPDATE-GENERIC-ARGS-IN-CONDITIONAL-TEST", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_update_generic_args_in_variable_binding_query", "PPH-PHRASE-UPDATE-GENERIC-ARGS-IN-VARIABLE-BINDING-QUERY", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_replace_generic_arg_simple", "PPH-PHRASE-REPLACE-GENERIC-ARG-SIMPLE", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_kept_generic_arg_phrase_p", "PPH-KEPT-GENERIC-ARG-PHRASE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_ok_for_arg0P", "PPH-PHRASE-OK-FOR-ARG0?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_replace_generic_args_recursive", "PPH-PHRASE-REPLACE-GENERIC-ARGS-RECURSIVE", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reserve_matrix_arg0_cycl", "PPH-PHRASE-RESERVE-MATRIX-ARG0-CYCL", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reset_arg0_reservations", "PPH-PHRASE-RESET-ARG0-RESERVATIONS", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reset_arg0_reservations_int", "PPH-PHRASE-RESET-ARG0-RESERVATIONS-INT", 4, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reserve_arg0", "PPH-PHRASE-RESERVE-ARG0", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reserved_arg0", "PPH-PHRASE-RESERVED-ARG0", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reserved_arg0_low", "PPH-PHRASE-RESERVED-ARG0-LOW", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reserved_arg0_position", "PPH-PHRASE-RESERVED-ARG0-POSITION", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_reserved_for_arg0_p", "PPH-PHRASE-RESERVED-FOR-ARG0-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_chemical_formula_from_lists_phrase_get_string", "PPH-CHEMICAL-FORMULA-FROM-LISTS-PHRASE-GET-STRING", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_output_item_from_atomic_composition_lists", "PPH-OUTPUT-ITEM-FROM-ATOMIC-COMPOSITION-LISTS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_atomic_symbol", "PPH-ATOMIC-SYMBOL", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_symbol_phrase_get_string", "PPH-SYMBOL-PHRASE-GET-STRING", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_symbol_string", "PPH-SYMBOL-STRING", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_list_add_sentential_force", "PPH-PHRASE-OUTPUT-LIST-ADD-SENTENTIAL-FORCE", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_add_sentential_force", "PPH-PHRASE-ADD-SENTENTIAL-FORCE", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_string_add_sentence_initial_force", "PPH-STRING-ADD-SENTENCE-INITIAL-FORCE", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_string_add_sentence_final_force", "PPH-STRING-ADD-SENTENCE-FINAL-FORCE", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_list_has_sentential_forceP", "PPH-PHRASE-OUTPUT-LIST-HAS-SENTENTIAL-FORCE?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_string_has_sentential_forceP", "PPH-STRING-HAS-SENTENTIAL-FORCE?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_string_has_sentence_initial_forceP", "PPH-STRING-HAS-SENTENCE-INITIAL-FORCE?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_string_has_sentence_final_forceP", "PPH-STRING-HAS-SENTENCE-FINAL-FORCE?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_list_upcase_leading_smart", "PPH-PHRASE-OUTPUT-LIST-UPCASE-LEADING-SMART", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_item_upcase_leading_smart", "PPH-PHRASE-OUTPUT-ITEM-UPCASE-LEADING-SMART", 1, 0, false );
    SubLFiles.declareFunction( me, "upcased_leading_smartP", "UPCASED-LEADING-SMART?", 1, 0, false );
    SubLFiles.declareFunction( me, "ends_with_period_smartP", "ENDS-WITH-PERIOD-SMART?", 1, 0, false );
    SubLFiles.declareFunction( me, "ends_with_question_mark_smartP", "ENDS-WITH-QUESTION-MARK-SMART?", 1, 0, false );
    SubLFiles.declareFunction( me, "ends_with_punctuation_smartP", "ENDS-WITH-PUNCTUATION-SMART?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_list_add_period_smart", "PPH-PHRASE-OUTPUT-LIST-ADD-PERIOD-SMART", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_list_add_question_mark_smart", "PPH-PHRASE-OUTPUT-LIST-ADD-QUESTION-MARK-SMART", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_output_list_add_punctuation_smart", "PPH-PHRASE-OUTPUT-LIST-ADD-PUNCTUATION-SMART", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_string_add_punctuation_smart", "PPH-STRING-ADD-PUNCTUATION-SMART", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_output_list_nonempty_tail", "PPH-OUTPUT-LIST-NONEMPTY-TAIL", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_percent_phrase_get_string", "PPH-PERCENT-PHRASE-GET-STRING", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_float_precision", "PPH-FLOAT-PRECISION", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_coordination_phrase_get_string", "PPH-COORDINATION-PHRASE-GET-STRING", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_coordination_phrase_get_string_internal", "PPH-COORDINATION-PHRASE-GET-STRING-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "find_dtr_olist_in_olist", "FIND-DTR-OLIST-IN-OLIST", 3, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_agrees_with_no_quantified_phrase_p", "PPH-PHRASE-AGREES-WITH-NO-QUANTIFIED-PHRASE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_no_quantified_det_nbar_phrase_p", "PPH-NO-QUANTIFIED-DET-NBAR-PHRASE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_coordination_phrase_operator_item", "PPH-COORDINATION-PHRASE-OPERATOR-ITEM", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_nbar_coordination_phrase_p", "PPH-NBAR-COORDINATION-PHRASE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_nbar_coordination_phrase_distribute_determiner", "PPH-NBAR-COORDINATION-PHRASE-DISTRIBUTE-DETERMINER", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_subject_verb_agreement", "PPH-PHRASE-SET-SUBJECT-VERB-AGREEMENT", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_default_verb_pred", "PPH-PHRASE-DEFAULT-VERB-PRED", 0, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_peer_agreement", "PPH-PHRASE-SET-PEER-AGREEMENT", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_tense_agreement", "PPH-PHRASE-SET-TENSE-AGREEMENT", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_clarifying_agreement", "PPH-PHRASE-SET-CLARIFYING-AGREEMENT", 2, 0, false );
    SubLFiles.declareFunction( me, "uninformative_paraphrase_p", "UNINFORMATIVE-PARAPHRASE-P", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_agr_target_phrase", "PPH-PHRASE-AGR-TARGET-PHRASE", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_relative_agr_target", "PPH-PHRASE-RELATIVE-AGR-TARGET", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_set_det_nbar_agreement", "PPH-PHRASE-SET-DET-NBAR-AGREEMENT", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_lexical_head_cycl", "PPH-PHRASE-LEXICAL-HEAD-CYCL", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_phrase_dtr_nested_withinP", "PPH-PHRASE-DTR-NESTED-WITHIN?", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_subject_of_clause", "PPH-PHRASE-SUBJECT-OF-CLAUSE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_subject_domain", "PPH-PHRASE-SUBJECT-DOMAIN", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_find_potential_binders", "PPH-PHRASE-FIND-POTENTIAL-BINDERS", 1, 3, false );
    SubLFiles.declareFunction( me, "pph_phrase_find_potential_binders_int", "PPH-PHRASE-FIND-POTENTIAL-BINDERS-INT", 6, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_could_bind_possessive_here", "PPH-PHRASE-COULD-BIND-POSSESSIVE-HERE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_could_bind_reflexive_here", "PPH-PHRASE-COULD-BIND-REFLEXIVE-HERE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_english_indefinite_article", "PPH-PHRASE-ENGLISH-INDEFINITE-ARTICLE", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_english_indefinite_np", "PPH-PHRASE-ENGLISH-INDEFINITE-NP", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_copular_complementP", "PPH-COPULAR-COMPLEMENT?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_noun_predP", "PPH-NOUN-PRED?", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_singular_noun_predP", "PPH-SINGULAR-NOUN-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_non_singular_noun_predP", "PPH-NON-SINGULAR-NOUN-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_singular_determiner_string", "PPH-SINGULAR-DETERMINER-STRING", 1, 0, false );
    SubLFiles.declareFunction( me, "tense_preds_for_constraints", "TENSE-PREDS-FOR-CONSTRAINTS", 1, 0, false );
    SubLFiles.declareFunction( me, "tense_pred_for_constraints", "TENSE-PRED-FOR-CONSTRAINTS", 1, 0, false );
    SubLFiles.declareFunction( me, "verb_pred_for_subject_preds", "VERB-PRED-FOR-SUBJECT-PREDS", 1, 0, false );
    SubLFiles.declareFunction( me, "verb_preds_for_subject", "VERB-PREDS-FOR-SUBJECT", 1, 1, false );
    SubLFiles.declareFunction( me, "verb_pred_for_subject", "VERB-PRED-FOR-SUBJECT", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_strengthen_verb_constraints", "PPH-STRENGTHEN-VERB-CONSTRAINTS", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_verb_constraints_could_be_present_tenseP", "PPH-VERB-CONSTRAINTS-COULD-BE-PRESENT-TENSE?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_verb_constraints_ensure_present_tense", "PPH-VERB-CONSTRAINTS-ENSURE-PRESENT-TENSE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_remove_weak_verb_constraints", "PPH-REMOVE-WEAK-VERB-CONSTRAINTS", 1, 0, false );
    SubLFiles.declareFunction( me, "verb_preds_for_subject_int", "VERB-PREDS-FOR-SUBJECT-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "verb_pred_for_subject_int", "VERB-PRED-FOR-SUBJECT-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "verb_preds_for_subject_pronoun", "VERB-PREDS-FOR-SUBJECT-PRONOUN", 2, 0, false );
    SubLFiles.declareFunction( me, "verb_pred_for_subject_pronoun", "VERB-PRED-FOR-SUBJECT-PRONOUN", 2, 0, false );
    SubLFiles.declareFunction( me, "verb_preds_for_subject_preds", "VERB-PREDS-FOR-SUBJECT-PREDS", 1, 1, false );
    SubLFiles.declareFunction( me, "verb_pred_for_subject_pred", "VERB-PRED-FOR-SUBJECT-PRED", 1, 1, false );
    SubLFiles.declareFunction( me, "pph_finite_verb_predP", "PPH-FINITE-VERB-PRED?", 1, 0, false );
    new $pph_finite_verb_predP$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_non_plural_noun_predP", "PPH-NON-PLURAL-NOUN-PRED?", 1, 0, false );
    new $pph_non_plural_noun_predP$UnaryFunction();
    SubLFiles.declareFunction( me, "pph_plural_noun_predP", "PPH-PLURAL-NOUN-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_plural_verb_predP", "PPH-PLURAL-VERB-PRED?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_singular_verb_preds_for_tenses", "PPH-SINGULAR-VERB-PREDS-FOR-TENSES", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_singular_verb_pred_for_tense", "PPH-SINGULAR-VERB-PRED-FOR-TENSE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_plural_verb_preds_for_tenses", "PPH-PLURAL-VERB-PREDS-FOR-TENSES", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_plural_verb_pred_for_tense", "PPH-PLURAL-VERB-PRED-FOR-TENSE", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_unify_pos_preds", "PPH-PHRASE-UNIFY-POS-PREDS", 2, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_starts_with_detpP", "PPH-PHRASE-STARTS-WITH-DETP?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_starts_with_detP", "PPH-PHRASE-STARTS-WITH-DET?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_determinerP", "PPH-PHRASE-DETERMINER?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_detpP", "PPH-PHRASE-DETP?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_detp_nart", "PPH-DETP-NART", 0, 0, false );
    SubLFiles.declareFunction( me, "clear_pph_detp_nart_cached", "CLEAR-PPH-DETP-NART-CACHED", 0, 0, false );
    SubLFiles.declareFunction( me, "remove_pph_detp_nart_cached", "REMOVE-PPH-DETP-NART-CACHED", 0, 0, false );
    SubLFiles.declareFunction( me, "pph_detp_nart_cached_internal", "PPH-DETP-NART-CACHED-INTERNAL", 0, 0, false );
    SubLFiles.declareFunction( me, "pph_detp_nart_cached", "PPH-DETP-NART-CACHED", 0, 0, false );
    SubLFiles.declareFunction( me, "pph_wordify_sem_free_noun", "PPH-WORDIFY-SEM-FREE-NOUN", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_wordify_noun", "PPH-WORDIFY-NOUN", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_noun_stringP", "PPH-NOUN-STRING?", 1, 0, false );
    SubLFiles.declareFunction( me, "pph_phrase_switch_arg1_and_arg2_in_dtrs", "PPH-PHRASE-SWITCH-ARG1-AND-ARG2-IN-DTRS", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_pph_phrase_resolution_file()
  {
    $pph_phrase_get_string_depth$ = SubLFiles.defparameter( "*PPH-PHRASE-GET-STRING-DEPTH*", ZERO_INTEGER );
    $pph_phrase_get_string_max_depth$ = SubLFiles.defparameter( "*PPH-PHRASE-GET-STRING-MAX-DEPTH*", $int3$99 );
    $pph_repeated_phrase_lists$ = SubLFiles.defparameter( "*PPH-REPEATED-PHRASE-LISTS*", NIL );
    $pph_phrase_transform_nth_reference_phrase$ = SubLFiles.defparameter( "*PPH-PHRASE-TRANSFORM-NTH-REFERENCE-PHRASE*", NIL );
    $pph_symbol_phrase_stack$ = SubLFiles.defparameter( "*PPH-SYMBOL-PHRASE-STACK*", NIL );
    $pph_coordination_phrase_nesting$ = SubLFiles.defvar( "*PPH-COORDINATION-PHRASE-NESTING*", ZERO_INTEGER );
    $pph_coordination_phrase_nesting_max$ = SubLFiles.defparameter( "*PPH-COORDINATION-PHRASE-NESTING-MAX*", TEN_INTEGER );
    $pph_english_lexical_indefinites$ = SubLFiles.deflexical( "*PPH-ENGLISH-LEXICAL-INDEFINITES*", $list195 );
    $pph_detp_nart_cached_caching_state$ = SubLFiles.deflexical( "*PPH-DETP-NART-CACHED-CACHING-STATE*", NIL );
    return NIL;
  }

  public static SubLObject setup_pph_phrase_resolution_file()
  {
    memoization_state.note_globally_cached_function( $sym226$PPH_DETP_NART_CACHED );
    return NIL;
  }
  ...
  static
  {
    me = new pph_phrase_resolution();
    $pph_phrase_get_string_depth$ = null;
    $pph_phrase_get_string_max_depth$ = null;
    $pph_repeated_phrase_lists$ = null;
    $pph_phrase_transform_nth_reference_phrase$ = null;
    $pph_symbol_phrase_stack$ = null;
    $pph_coordination_phrase_nesting$ = null;
    $pph_coordination_phrase_nesting_max$ = null;
    $pph_english_lexical_indefinites$ = null;
    $pph_detp_nart_cached_caching_state$ = null;
    $str0$Unable_to_finish = makeString( "Unable to finish" );
    $str1$___A__S__ = makeString( "~&~A ~S~%" );
    $str2$Unfinished_dtr = makeString( "Unfinished dtr" );
    $int3$99 = makeInteger( 99 );
    $kw4$PHRASE_CYCL = makeKeyword( "PHRASE-CYCL" );
    $str5$_PPH_error_level_ = makeString( "(PPH error level " );
    $str6$__ = makeString( ") " );
    $str7$__Exceeded_PPH_PHRASE_GET_STRING_ = makeString( "~&Exceeded PPH-PHRASE-GET-STRING max depth of ~S.~% Phrase: ~S~%" );
    $str8$__Resolved_conditional_phrase_to_ = makeString( "~&Resolved conditional phrase to ~S ~S." );
    $str9$Tried_to_generating_CycL_phrase_w = makeString( "Tried to generating CycL phrase with unknown CycL. Trying again as regular phrase:~% ~S" );
    $str10$Resolved_formerly_CycL_phrase_wit = makeString( "Resolved formerly CycL phrase with unknown CycL to ~S" );
    $str11$__Using_CycL_for_object___S___ = makeString( "~&Using CycL for object: ~S.~%" );
    $str12$__Doing_anaphor_paraphrase_with_f = makeString( "~&Doing anaphor paraphrase with formula ~S.~%" );
    $str13$__Doing_coordination_paraphrase_w = makeString( "~&Doing coordination paraphrase with formula ~S.~%" );
    $str14$__Suspending_bullet_use_for____S_ = makeString( "~&Suspending bullet use for~% ~S~%" );
    $str15$__Turning_bullet_use_back_on___ = makeString( "~&Turning bullet use back on.~%" );
    $str16$__Doing_chemical_formula_from_lis = makeString( "~&Doing chemical-formula-from-lists paraphrase.~%" );
    $str17$__Doing_percent_paraphrase___ = makeString( "~&Doing percent paraphrase.~%" );
    $str18$__Generating_symbol_paraphrase_of = makeString( "~&Generating symbol paraphrase of ~S.~%" );
    $str19$__Using_quoted_string_for_string_ = makeString( "~&Using quoted string for string: ~S.~%" );
    $str20$__Using_terse_paraphrase_for__S__ = makeString( "~&Using terse paraphrase for ~S~%" );
    $str21$__Using_precise_paraphrase_for__S = makeString( "~&Using precise paraphrase for ~S~%" );
    $sym22$PPH_PRECISION_P = makeSymbol( "PPH-PRECISION-P" );
    $str23$__Unknown_cycl_for__S___ = makeString( "~&Unknown cycl for ~S.~%" );
    $str24$__Using_precision__S__ = makeString( "~&Using precision ~S~%" );
    $str25$__Using_quoted_paraphrase_for_ter = makeString( "~&Using quoted paraphrase for term.~%" );
    $str26$Using__S_for_quoted_paraphrase_of = makeString( "Using ~S for quoted paraphrase of ~S" );
    $str27$__Using_nospace_paraphrase_for__S = makeString( "~&Using nospace paraphrase for ~S~%" );
    $list28 = ConsesLow.list( makeSymbol( "TARGET-TERM" ), makeSymbol( "TARGET-STRING" ), makeSymbol( "TARGET-PREDS" ) );
    $kw29$PPH_HL_SUPPORTS_FOUND = makeKeyword( "PPH-HL-SUPPORTS-FOUND" );
    $kw30$RESOLVED = makeKeyword( "RESOLVED" );
    $const31$PossessivePhrase = constant_handles.reader_make_constant_shell( makeString( "PossessivePhrase" ) );
    $str32$Couldn_t_resolve_repeated_dtrs_fo = makeString( "Couldn't resolve repeated dtrs for ~S" );
    $str33$__Starting_dtr_resolution_for_phr = makeString( "~&Starting dtr resolution for phrase ~D" );
    $str34$___D_of__D_daughters_unfinished_ = makeString( "~&~D of ~D daughters unfinished." );
    $str35$Can_t_resolve_mother_due_to_impos = makeString( "Can't resolve mother due to impossible dtr:~% ~S" );
    $str36$__Finished__S_iteration_s____D_of = makeString( "~&Finished ~S iteration(s): ~D of ~D daughters unfinished." );
    $str37$Couldn_t_finish__S___because_of_u = makeString( "Couldn't finish ~S~% because of unfinished dtrs:~% ~S~%" );
    $kw38$NOT_RESOLVED = makeKeyword( "NOT-RESOLVED" );
    $sym39$PPH_PHRASE_P = makeSymbol( "PPH-PHRASE-P" );
    $str40$Problem_expanding_repeated_phrase = makeString( "Problem expanding repeated phrase~% ~S" );
    $str41$PPH_REPEATED_PHRASE_EXPANSION_got = makeString( "PPH-REPEATED-PHRASE-EXPANSION got non-formula ~S" );
    $sym42$_EXIT = makeSymbol( "%EXIT" );
    $str43$Repeating_phrase__S = makeString( "Repeating phrase ~S" );
    $str44$__etc_ = makeString( ", etc." );
    $str45$Can_t_get_current_list_item_from_ = makeString( "Can't get current list item from ~S" );
    $kw46$UNSPECIFIED = makeKeyword( "UNSPECIFIED" );
    $list47 = ConsesLow.cons( constant_handles.reader_make_constant_shell( makeString( "TheList" ) ), makeKeyword( "ANYTHING" ) );
    $str48$Updating__S_to____S____Result___S = makeString( "Updating ~S to~% ~S.~% Result: ~S~%" );
    $sym49$PPH_CURRENT_LIST_ITEM_FORMULA_P = makeSymbol( "PPH-CURRENT-LIST-ITEM-FORMULA-P" );
    $sym50$PPH_CURRENT_LIST_ITEM_FORMULA_ITEM = makeSymbol( "PPH-CURRENT-LIST-ITEM-FORMULA-ITEM" );
    $list51 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "CurrentListItemFn" ) ), ConsesLow.list( makeKeyword( "TEST" ), makeSymbol( "PPH-EXTENSIONAL-LIST-P" ) ), ConsesLow.list(
        makeKeyword( "TEST" ), makeSymbol( "NON-NEGATIVE-INTEGER-P" ) ) );
    $list52 = ConsesLow.list( makeSymbol( "LIST-EXPRESSION" ), makeSymbol( "OFFSET" ) );
    $str53$__Resolving__S_to__S__ = makeString( "~&Resolving ~S to ~S~%" );
    $list54 = ConsesLow.cons( makeSymbol( "LIST-EXPRESSION" ), makeSymbol( "INDEX" ) );
    $str55$ = makeString( "" );
    $str56$__Phrase_agr___S__ = makeString( "~&Phrase agr: ~S~%" );
    $str57$___Dtr__D_category_is__S___ = makeString( "~& Dtr ~D category is ~S.~%" );
    $str58$___Dtr__D_agr_is__S___ = makeString( "~& Dtr ~D agr is ~S.~%" );
    $str59$__Set_agr_of_dtr___D_to__S___ = makeString( "~&Set agr of dtr #~D to ~S.~%" );
    $str60$___Set_head_agr_to__S___ = makeString( "~& Set head agr to ~S.~%" );
    $str61$__Finished__S____with__D_dtrs____ = makeString( "~&Finished ~S~% (with ~D dtrs).~%" );
    $str62$___Set_phrase_agr_to__S_based_on_ = makeString( "~& Set phrase agr to ~S based on head dtr.~%" );
    $str63$___Dtrs_left_unfinished___D___ = makeString( "~& Dtrs left unfinished: ~D.~%" );
    $kw64$MOTHER = makeKeyword( "MOTHER" );
    $str65$__Agr_target__mother__not_resolve = makeString( "~&Agr target (mother) not resolved: ~S.~%" );
    $str66$__Phrase_agrees_with_itself_____ = makeString( "~&Phrase agrees with itself(!)~%" );
    $str67$__Agr_target_not_resolved___S___ = makeString( "~&Agr target not resolved: ~S.~%" );
    $str68$_ = makeString( "\"" );
    $str69$Setting_mother_s_output_list_from = makeString( "Setting mother's output list from unfinished dtr with CycL ~S:~% ~S~%" );
    $str70$_Dth_dtr_output_list___S = makeString( "~Dth dtr output list: ~S" );
    $sym71$PPH_PHRASE_OUTPUT_ITEM_EMPTY_P = makeSymbol( "PPH-PHRASE-OUTPUT-ITEM-EMPTY-P" );
    $list72 = ConsesLow.list( ZERO_INTEGER );
    $sym73$INCOMPLETE_PPH_PHRASE_P = makeSymbol( "INCOMPLETE-PPH-PHRASE-P" );
    $str74$__Test_has_unresolved_indexical_p = makeString( "~&Test has unresolved indexical phrases. Postponing: ~S.~%" );
    $str75$__Ran_conditional_test_____S___Re = makeString( "~&Ran conditional test:~% ~S~% Result: ~S.~%" );
    $str76$Pass = makeString( "Pass" );
    $str77$Fail = makeString( "Fail" );
    $str78$___Initial_winning_phrase_____S__ = makeString( "~& Initial winning phrase:~% ~S~% with agr preds ~S" );
    $str79$___New_phrase___S___with_agr_pred = makeString( "~& New phrase: ~S~% with agr preds ~S" );
    $str80$___Phrase_has__D_daughters___ = makeString( "~& Phrase has ~D daughters.~%" );
    $sym81$PPH_NTH_PHRASE_NAUT_P = makeSymbol( "PPH-NTH-PHRASE-NAUT-P" );
    $list82 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "TheNthFn" ) ), constant_handles.reader_make_constant_shell( makeString( "SententialConstituent" ) ), makeKeyword( "ANYTHING" ) );
    $sym83$PPH_PHRASE_TRANSFORM_NTH = makeSymbol( "PPH-PHRASE-TRANSFORM-NTH" );
    $str84$Adding_alternatives_for_agr_targe = makeString( "Adding alternatives for agr target~% ~S~% to ~S" );
    $str85$Done__S_alternatives_of__S = makeString( "Done ~S alternatives of ~S" );
    $str86$Added__D_alternatives_ = makeString( "Added ~D alternatives." );
    $kw87$SUBJECT_VERB = makeKeyword( "SUBJECT-VERB" );
    $kw88$DET_NBAR = makeKeyword( "DET-NBAR" );
    $kw89$PEER = makeKeyword( "PEER" );
    $kw90$TENSE = makeKeyword( "TENSE" );
    $str91$__Paraphrasing__S_with_agreement_ = makeString( "~&Paraphrasing ~S with agreement ~S.~%" );
    $str92$___Output_list_is____S___ = makeString( "~& Output list is~% ~S.~%" );
    $str93$___Arg_position_map_is__S___ = makeString( "~& Arg-position map is ~S.~%" );
    $str94$___SUID_is__S___ = makeString( "~& SUID is ~S.~%" );
    $str95$_____of_dtrs___S___ = makeString( "~& # of dtrs: ~S.~%" );
    $str96$Can_t_resolve_string_of__S = makeString( "Can't resolve string of ~S" );
    $kw97$CYCL_PHRASE = makeKeyword( "CYCL-PHRASE" );
    $str98$Sanity_check_failed__rm_cycl__S_i = makeString( "Sanity check failed: rm cycl ~S is not equal to phrase cycl ~S" );
    $str99$Must_use_reflexive_for____S___due = makeString( "Must use reflexive for~% ~S~% due to~% ~S" );
    $kw100$SOURCE_POS = makeKeyword( "SOURCE-POS" );
    $str101$__Using_arg0_for__S_with_formula_ = makeString( "~&Using arg0 for ~S with formula ~S~%" );
    $kw102$ARG0 = makeKeyword( "ARG0" );
    $str103$Supposedly___S_isn_t_part_of_old_ = makeString( "Supposedly, ~S isn't part of old CycL." );
    $str104$Updated_conditional_test_to__S__ = makeString( "Updated conditional test to ~S~%" );
    $str105$Updated_test_to____S_using_non_te = makeString( "Updated test to~% ~S using non-template~% ~S~%" );
    $str106$Updated_variable_bindings_query_t = makeString( "Updated variable bindings query-template to ~S~%" );
    $str107$Updated_variable_bindings_query_t = makeString( "Updated variable bindings query-template to~% ~S using non-template~% ~S~%" );
    $str108$__Replacing__S_with__S___ = makeString( "~&Replacing ~S with ~S.~%" );
    $list109 = ConsesLow.list( TWO_INTEGER );
    $str110$Can_t_reconcile__S_and__S_ = makeString( "Can't reconcile ~S and ~S." );
    $kw111$NO_ACTION = makeKeyword( "NO-ACTION" );
    $str112$No_current_arg0_to_reserve_for__S = makeString( "No current arg0 to reserve for ~S" );
    $kw113$NO_CURRENT_ARG0 = makeKeyword( "NO-CURRENT-ARG0" );
    $kw114$RESERVED_FOR_MATRIX_ARG0 = makeKeyword( "RESERVED-FOR-MATRIX-ARG0" );
    $kw115$HTML = makeKeyword( "HTML" );
    $sym116$_SYMBOL = makeSymbol( "?SYMBOL" );
    $const117$atomicSymbol = constant_handles.reader_make_constant_shell( makeString( "atomicSymbol" ) );
    $list118 = ConsesLow.list( makeSymbol( "?SYMBOL" ) );
    $kw119$SYMBOL = makeKeyword( "SYMBOL" );
    $const120$nameString = constant_handles.reader_make_constant_shell( makeString( "nameString" ) );
    $list121 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "PhraseFn-Bar1" ) ), constant_handles.reader_make_constant_shell( makeString( "Noun" ) ) );
    $sym122$_CODE = makeSymbol( "?CODE" );
    $const123$htmlGlyphCode = constant_handles.reader_make_constant_shell( makeString( "htmlGlyphCode" ) );
    $list124 = ConsesLow.list( makeSymbol( "?CODE" ) );
    $const125$htmlStringVersionOfCharacter = constant_handles.reader_make_constant_shell( makeString( "htmlStringVersionOfCharacter" ) );
    $const126$termStrings = constant_handles.reader_make_constant_shell( makeString( "termStrings" ) );
    $kw127$DECLARATIVE = makeKeyword( "DECLARATIVE" );
    $kw128$INTERROGATIVE = makeKeyword( "INTERROGATIVE" );
    $kw129$QUESTION = makeKeyword( "QUESTION" );
    $kw130$DEFAULT = makeKeyword( "DEFAULT" );
    $str131$_ = makeString( "." );
    $str132$_ = makeString( "?" );
    $kw133$NONE = makeKeyword( "NONE" );
    $sym134$ALPHANUMERIC_CHAR_P = makeSymbol( "ALPHANUMERIC-CHAR-P" );
    $list135 = ConsesLow.cons( makeSymbol( "LAST-NON-EMPTY-ITEM" ), makeSymbol( "TRAILING-ITEMS" ) );
    $int136$100 = makeInteger( 100 );
    $sym137$POSITIVE_INTEGER_P = makeSymbol( "POSITIVE-INTEGER-P" );
    $str138$Using_significant_digits_cutoff_o = makeString( "Using significant digits cutoff of ~S for ~S" );
    $str139$_ = makeString( "%" );
    $list140 = ConsesLow.list( Characters.CHAR_0 );
    $sym141$ALPHA_CHAR_P = makeSymbol( "ALPHA-CHAR-P" );
    $str142$__Recursion_limit___S__exceeded_i = makeString( "~&Recursion limit (~S) exceeded in PPH-COORDINATION-PHRASE-GET-STRING." );
    $list143 = ConsesLow.list( makeSymbol( "COORD-KEYWORD" ), makeSymbol( "LIST-TYPE-ID-STRING" ) );
    $kw144$AND = makeKeyword( "AND" );
    $str145$Changing_coordination_to__OR__bas = makeString( "Changing coordination to :OR, based on :NO determiner" );
    $kw146$OR = makeKeyword( "OR" );
    $str147$Inserting_dummy_phrase__S = makeString( "Inserting dummy phrase ~S" );
    $const148$Noun = constant_handles.reader_make_constant_shell( makeString( "Noun" ) );
    $const149$plural_Generic = constant_handles.reader_make_constant_shell( makeString( "plural-Generic" ) );
    $const150$singular_Generic = constant_handles.reader_make_constant_shell( makeString( "singular-Generic" ) );
    $str151$__Setting_conjunctive_agr_of__S_t = makeString( "~&Setting conjunctive agr of ~S to ~S" );
    $const152$No_NLAttr = constant_handles.reader_make_constant_shell( makeString( "No-NLAttr" ) );
    $const153$Determiner = constant_handles.reader_make_constant_shell( makeString( "Determiner" ) );
    $str154$Removed_det_sister_from_mother_ = makeString( "Removed det-sister from mother." );
    $str155$subject_pointer_is_nth_phrase___S = makeString( "subject pointer is nth phrase: ~S" );
    $str156$__Not_setting_subject_verb_agreem = makeString( "~&Not setting subject verb agreement because subject dtr is not finished.~%" );
    $kw157$ANY = makeKeyword( "ANY" );
    $const158$Verb = constant_handles.reader_make_constant_shell( makeString( "Verb" ) );
    $str159$__Using_internal_constraints___S_ = makeString( "~&Using internal constraints: ~S~%" );
    $str160$___filtered_from__S___ = makeString( "...filtered from ~S.~%" );
    $str161$__Setting_verb_agreement_to__S___ = makeString( "~&Setting verb agreement to ~S.~%" );
    $str162$Using_default_verb_form___S_ = makeString( "Using default verb form: ~S." );
    $const163$MeasurableQuantity = constant_handles.reader_make_constant_shell( makeString( "MeasurableQuantity" ) );
    $const164$adjStrings = constant_handles.reader_make_constant_shell( makeString( "adjStrings" ) );
    $const165$Adjective = constant_handles.reader_make_constant_shell( makeString( "Adjective" ) );
    $str166$__This_phrase_s_agr_pred_is__S_bu = makeString( "~&This phrase's agr-pred is ~S but its category is ~S:~%~S" );
    $str167$__Set_adjective_peer_agreement_to = makeString( "~&Set adjective peer agreement to ~S.~%" );
    $const168$untensed = constant_handles.reader_make_constant_shell( makeString( "untensed" ) );
    $str169$__Set_untensed_peer_agreement_to_ = makeString( "~&Set untensed peer agreement to ~S.~%" );
    $str170$__Set_peer_agreement_to__S___ = makeString( "~&Set peer agreement to ~S.~%" );
    $str171$__Set_peer_agreement_to_default__ = makeString( "~&Set peer agreement to default: ~S.~%" );
    $const172$tensed = constant_handles.reader_make_constant_shell( makeString( "tensed" ) );
    $str173$__Incorporating_tense_agreement__ = makeString( "~&Incorporating tense agreement ~S.~%" );
    $str174$_S = makeString( "~S" );
    $str175$__Using_daughter__D_as_agreement_ = makeString( "~&Using daughter ~D as agreement target:~% ~S" );
    $const176$BareForm_NLAttr = constant_handles.reader_make_constant_shell( makeString( "BareForm-NLAttr" ) );
    $str177$__Using_hard_coded_null_determine = makeString( "~&Using hard-coded null determiner hack for #$BareForm-NLAttr.~%" );
    $const178$PossessivePronoun_Pre = constant_handles.reader_make_constant_shell( makeString( "PossessivePronoun-Pre" ) );
    $kw179$NEVER = makeKeyword( "NEVER" );
    $str180$__Setting_det_nbar_agreement_for_ = makeString( "~&Setting det-nbar agreement for possessive.~%" );
    $str181$__Using_null_determiner_for_posse = makeString( "~&Using null determiner for possessive nbar.~%" );
    $str182$__Setting_det_nbar_agreement_for_ = makeString( "~&Setting det-nbar agreement for ranged phrase.~%" );
    $str183$__Setting_determiner_s_agr_pred_f = makeString( "~&Setting determiner's agr-pred from nbar to ~S." );
    $str184$__Setting_agr_pred_from_determine = makeString( "~&Setting agr-pred from determiner to ~S." );
    $sym185$EL_VAR_ = makeSymbol( "EL-VAR?" );
    $str186$__Setting_determiner_nbar_agreeme = makeString( "~&Setting determiner-nbar agreement based on variable determiner." );
    $const187$nonSingular_Generic = constant_handles.reader_make_constant_shell( makeString( "nonSingular-Generic" ) );
    $str188$__Setting_determiner_nbar_agreeme = makeString( "~&Setting determiner-nbar agreement to ~S based on number-denoting determiner." );
    $str189$Couldn_t_determine_agreement_info = makeString( "Couldn't determine agreement information: ~S~% DET-HEAD-CYCL: ~S~%" );
    $str190$PPH_PHRASE_DTR_NESTED_WITHIN__loo = makeString( "PPH-PHRASE-DTR-NESTED-WITHIN? looking for dtr ~S in non-PPH-PHRASE ~S~%" );
    $str191$some = makeString( "some" );
    $str192$__Nbar_is_now__S__ = makeString( "~&Nbar is now ~S~%" );
    $const193$ExistingStuffType = constant_handles.reader_make_constant_shell( makeString( "ExistingStuffType" ) );
    $str194$__Using_English_specific_indefini = makeString( "~&Using English-specific indefinite determiner hack: ~S with Nbar ~S.~%" );
    $list195 = ConsesLow.list( ConsesLow.cons( constant_handles.reader_make_constant_shell( makeString( "Thing" ) ), makeString( "something" ) ), ConsesLow.cons( constant_handles.reader_make_constant_shell( makeString(
        "Person" ) ), makeString( "someone" ) ), ConsesLow.cons( constant_handles.reader_make_constant_shell( makeString( "Place" ) ), makeString( "someplace" ) ) );
    $list196 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "singular" ) ) );
    $list197 = ConsesLow.cons( makeSymbol( "TYPE" ), makeSymbol( "STRING" ) );
    $str198$Nbar_string_is__S__Output_list___ = makeString( "Nbar string is ~S. Output list: ~S" );
    $const199$nounStrings = constant_handles.reader_make_constant_shell( makeString( "nounStrings" ) );
    $list200 = ConsesLow.list( makeKeyword( "A" ), NIL );
    $kw201$INDEX = makeKeyword( "INDEX" );
    $const202$Pronoun = constant_handles.reader_make_constant_shell( makeString( "Pronoun" ) );
    $const203$verbStrings = constant_handles.reader_make_constant_shell( makeString( "verbStrings" ) );
    $sym204$GENL_POS_PRED_ = makeSymbol( "GENL-POS-PRED?" );
    $const205$singularVerb_Generic = constant_handles.reader_make_constant_shell( makeString( "singularVerb-Generic" ) );
    $str206$__Subject_agr___S__ = makeString( "~&Subject agr: ~S~%" );
    $const207$SecondPerson_NLAttr = constant_handles.reader_make_constant_shell( makeString( "SecondPerson-NLAttr" ) );
    $const208$secondPersonSg_Generic = constant_handles.reader_make_constant_shell( makeString( "secondPersonSg-Generic" ) );
    $const209$secondPersonSg_Present = constant_handles.reader_make_constant_shell( makeString( "secondPersonSg-Present" ) );
    $const210$FirstPerson_NLAttr = constant_handles.reader_make_constant_shell( makeString( "FirstPerson-NLAttr" ) );
    $const211$firstPersonSg_Generic = constant_handles.reader_make_constant_shell( makeString( "firstPersonSg-Generic" ) );
    $const212$firstPersonSg_Present = constant_handles.reader_make_constant_shell( makeString( "firstPersonSg-Present" ) );
    $str213$__Using_tenses__S__ = makeString( "~&Using tenses ~S~%" );
    $sym214$PPH_FINITE_VERB_PRED_ = makeSymbol( "PPH-FINITE-VERB-PRED?" );
    $sym215$PPH_NON_PLURAL_NOUN_PRED_ = makeSymbol( "PPH-NON-PLURAL-NOUN-PRED?" );
    $sym216$PPH_PLURAL_NOUN_PRED_ = makeSymbol( "PPH-PLURAL-NOUN-PRED?" );
    $sym217$PPH_PLURAL_VERB_PRED_ = makeSymbol( "PPH-PLURAL-VERB-PRED?" );
    $str218$__Using_tense__S__ = makeString( "~&Using tense ~S~%" );
    $const219$nonPlural_Generic = constant_handles.reader_make_constant_shell( makeString( "nonPlural-Generic" ) );
    $const220$pluralVerb_Generic = constant_handles.reader_make_constant_shell( makeString( "pluralVerb-Generic" ) );
    $const221$thirdPersonSg_Generic = constant_handles.reader_make_constant_shell( makeString( "thirdPersonSg-Generic" ) );
    $str222$__Unifying__S_and__S_to__S___ = makeString( "~&Unifying ~S and ~S to ~S.~%" );
    $str223$Using_agr_preds__S_from_done_head = makeString( "Using agr preds ~S from done head dtr~% ~S~% for ~S." );
    $str224$Couldn_t_unify__S_and__S___ = makeString( "Couldn't unify ~S and ~S.~%" );
    $str225$__AGRs_are_now____S_and____S___ = makeString( "~&AGRs are now~% ~S and~% ~S.~%" );
    $sym226$PPH_DETP_NART_CACHED = makeSymbol( "PPH-DETP-NART-CACHED" );
    $list227 = ConsesLow.list( constant_handles.reader_make_constant_shell( makeString( "PhraseFn" ) ), constant_handles.reader_make_constant_shell( makeString( "Determiner" ) ) );
    $sym228$_PPH_DETP_NART_CACHED_CACHING_STATE_ = makeSymbol( "*PPH-DETP-NART-CACHED-CACHING-STATE*" );
    $str229$__Switching_arg1_and_arg2_in__S__ = makeString( "~&Switching arg1 and arg2 in ~S~%" );
    $kw230$ARG2 = makeKeyword( "ARG2" );
    $str231$Can_t_switch_arg1_and_arg2_in_rep = makeString( "Can't switch arg1 and arg2 in repeated phrase~S" );
  }

  public static final class $pph_current_list_item_formula_p$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_phrase_output_item_empty_p$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_nth_phrase_naut_p$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_finite_verb_predP$UnaryFunction
      extends
        UnaryFunction
  {
  ...
  public static final class $pph_non_plural_noun_predP$UnaryFunction
      extends
        UnaryFunction
  {
