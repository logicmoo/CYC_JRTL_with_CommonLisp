...
import static com.cyc.tool.subl.jrtl.nativeCode.subLisp.Types.sublisp_false;
...
public final class id_index extends SubLTranslatedFile implements V12 {
    ...
    public static final SubLObject do_id_index_old_objects(SubLObject macroform, SubLObject environment) {
	{
		...
		current = current.first();
		{
			...
			}
			{
				...
				return list(PUNLESS, list(DO_ID_INDEX_OLD_OBJECTS_EMPTY_P, v_id_index, tombstone), list(DO_VECTOR_INDEX, list(id, v_object, list(ID_INDEX_OLD_OBJECTS, v_id_index), $DONE, done), list(CSETQ, v_object, list(DO_ID_INDEX_OLD_OBJECTS_EXTRACT_OBJECT, v_object, tombstone)),
					listS(PWHEN, list($sym91$DO_ID_INDEX_OBJECT_VALID_, v_object, tombstone), append(body, NIL))));
			    }
			}
		    }
		}
			...
			current = current.rest();
			{
				...
				current = temp;
				{
				    ...
				    if (NIL != progress_message) {
					{
					    ...
					    return list(CLET, list(list(total, list(ID_INDEX_NEW_OBJECT_COUNT, v_id_index)), bq_cons(sofar, $list_alt71)), listS(CHECK_TYPE, progress_message, $list_alt73), list(NOTING_PERCENT_PROGRESS, progress_message,
						    listS(DO_ID_INDEX_NEW_OBJECTS, list(new SubLObject[] { id, v_object, v_id_index, $TOMBSTONE, tombstone, $ORDERED, ordered, $DONE, done }), list(NOTE_PERCENT_PROGRESS, sofar, total), list(CINC, sofar), append(body, NIL))));
					}
				    }
				    if (!tombstone.isAtom()) {
					{
					...
					if (T == ordered) {
						...
						return list(PUNLESS, list(CAND, list(ID_INDEX_NEW_OBJECTS_EMPTY_P, v_id_index), list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone)), list(CLET,
							list(list(v_new, list(ID_INDEX_NEW_OBJECTS, v_id_index)), list(id, list(ID_INDEX_NEW_ID_THRESHOLD, v_id_index)), list(end_id, list(ID_INDEX_NEXT_ID, v_id_index)),
								list(v_default, list(FUNLESS, list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone), tombstone))),
							list(WHILE, list(CAND, list($sym93$_, id, end_id), list(CNOT, done)),
								list(CLET, list(list(v_object, list(GETHASH_WITHOUT_VALUES, id, v_new, v_default))), listS(PUNLESS, list(CAND, list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone), list(ID_INDEX_TOMBSTONE_P, v_object)), append(body, NIL))), list(CINC, id))));
					    }
					} else {
 ...
 public static final String myName = "com.cyc.cycjava.cycl.id_index";
    ...
    private static final SubLList $list63 = list(list(makeSymbol("ID"), makeSymbol("OBJECT"), makeSymbol("ID-INDEX"), makeSymbol("&KEY"), list(makeSymbol("TOMBSTONE"), $SKIP), makeSymbol("ORDERED"), makeSymbol("PROGRESS-MESSAGE"), makeSymbol("DONE")), makeSymbol("&BODY"),
	    makeSymbol("BODY"));
    ...
    private static final SubLList $list124 = list(
	    list(new SubLObject[] { makeSymbol("ID"), makeSymbol("OBJECT"), makeSymbol("ID-INDEX"), makeSymbol("&KEY"), makeSymbol("START-ID"), makeSymbol("END-ID"), list(makeSymbol("TOMBSTONE"), $SKIP), makeSymbol("ORDERED"), makeSymbol("PROGRESS-MESSAGE"), makeSymbol("DONE") }),
    ...
    private static final SubLList $list178 = list(list(list(makeInteger(100), TWENTY_INTEGER, TWENTY_INTEGER), makeKeyword("SUCCESS")), list(list(makeInteger(1000), makeInteger(200), makeInteger(200)), makeKeyword("SUCCESS")),
	    list(list(makeInteger(255), ZERO_INTEGER, makeInteger(40)), makeKeyword("SUCCESS")));
	...
	return v_object.getClass() == $id_index_native.class ? ((SubLObject) (T)) : NIL;
    }
	...
	assert NIL != subl_promotions.non_negative_integer_p(initial_next_id) : "! subl_promotions.non_negative_integer_p(initial_next_id) " + ("subl_promotions.non_negative_integer_p(initial_next_id) " + "CommonSymbols.NIL != subl_promotions.non_negative_integer_p(initial_next_id) ")
		+ initial_next_id;
	assert NIL != subl_promotions.non_negative_integer_p(dense_objects_size) : "! subl_promotions.non_negative_integer_p(dense_objects_size) " + ("subl_promotions.non_negative_integer_p(dense_objects_size) " + "CommonSymbols.NIL != subl_promotions.non_negative_integer_p(dense_objects_size) ")
		+ dense_objects_size;
		...
		current = current.first();
		{
			...
			}
			{
				...
				if (NIL == progress_message) {
					...
					return list(CLET, list(list(idx, v_id_index)), list(PUNLESS, list(ID_INDEX_OBJECTS_EMPTY_P, idx, tombstone), listS(DO_ID_INDEX_OLD_OBJECTS, list(id, v_object, idx, $DONE, done, $TOMBSTONE, tombstone), append(body, NIL)),
						listS(DO_ID_INDEX_NEW_OBJECTS, list(new SubLObject[] { id, v_object, idx, $ORDERED, ordered, $TOMBSTONE, tombstone, $DONE, done }), append(body, NIL))));
				    }
				} else {
				    {
					...
					return list(CLET, list(list(idx, v_id_index), list(mess, progress_message), list(total, list(ID_INDEX_COUNT, idx)), bq_cons(sofar, $list_alt71)), listS(CHECK_TYPE, mess, $list_alt73),
						list(NOTING_PERCENT_PROGRESS, mess, listS(DO_ID_INDEX, list(id, v_object, idx, $DONE, done, $ORDERED, ordered), list(NOTE_PERCENT_PROGRESS, sofar, total), list(CINC, sofar), append(body, NIL))));
				    }
				}
			    }
			}
	...
	if (NIL == progress_message) {
	    ...
	    return list(CLET, list(list(idx, v_id_index)), list(PUNLESS, list(ID_INDEX_OBJECTS_EMPTY_P, idx, tombstone), listS(DO_ID_INDEX_DENSE_OBJECTS, list(id, v_object, idx, $DONE, done, $TOMBSTONE, tombstone), append(body, NIL)),
		    listS(DO_ID_INDEX_SPARSE_OBJECTS, list(new SubLObject[] { id, v_object, idx, $ORDERED, ordered, $TOMBSTONE, tombstone, $DONE, done }), append(body, NIL))));
	}
	...
	return list(CLET, list(list(idx, v_id_index), list(mess, progress_message), list(total, list(ID_INDEX_COUNT, idx)), bq_cons(sofar, $list80)), listS(CHECK_TYPE, mess, $list82),
		list(NOTING_PERCENT_PROGRESS, mess, listS(DO_ID_INDEX, list(id, v_object, idx, $DONE, done, $ORDERED, ordered), append(body, list(list(CINC, sofar), list(NOTE_PERCENT_PROGRESS, sofar, total))))));
    }
	...
	if (NIL == progress_message) {
	    ...
	    return list(CLET, list(list(idx, v_id_index)), list(PUNLESS, list(ID_INDEX_DENSE_OBJECTS_EMPTY_P, idx, tombstone), list(DO_VECTOR_INDEX, list(id, v_object, list(ID_INDEX_DENSE_OBJECTS, idx), $DONE, done),
		    listS(PUNLESS, list(CAND, list(ID_INDEX_TOMBSTONE_P, v_object), list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone)), list(PWHEN, list(ID_INDEX_TOMBSTONE_P, v_object), list(CSETQ, v_object, tombstone)), append(body, NIL)))));
	}
	...
	return list(CLET, list(list(idx, v_id_index), list(mess, progress_message), list(total, list(ID_INDEX_DENSE_OBJECT_COUNT, idx)), bq_cons(sofar, $list80)), listS(CHECK_TYPE, mess, $list82),
		list(NOTING_PERCENT_PROGRESS, mess, listS(DO_ID_INDEX_DENSE_OBJECTS, list(id, v_object, idx, $DONE, done), append(body, list(list(CINC, sofar), list(NOTE_PERCENT_PROGRESS, sofar, total))))));
    }
	...
	if (NIL != progress_message) {
	    ...
	    return list(CLET, list(list(idx, v_id_index), list(mess, progress_message), list(total, list(ID_INDEX_SPARSE_OBJECT_COUNT, idx)), bq_cons(sofar, $list80)), listS(CHECK_TYPE, mess, $list82),
		    list(NOTING_PERCENT_PROGRESS, mess, listS(DO_ID_INDEX_SPARSE_OBJECTS, list(id, v_object, idx, $ORDERED, ordered, $DONE, done), append(body, list(list(CINC, sofar), list(NOTE_PERCENT_PROGRESS, sofar, total))))));
	}
	...
	if (T == ordered) {
	    ...
	    return list(CLET, list(list(idx, v_id_index)),
		    list(PUNLESS, list(CAND, list(ID_INDEX_SPARSE_OBJECTS_EMPTY_P, idx), list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone)),
			    list(CLET, list(list(sparse, list(ID_INDEX_SPARSE_OBJECTS, idx)), list(id, list(ID_INDEX_SPARSE_ID_THRESHOLD, idx)), list(end_id, list(ID_INDEX_NEXT_ID, idx)), list(v_default, list(FUNLESS, list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone), tombstone))),
				    list(WHILE, list(CAND, list($sym117$_, id, end_id), list(CNOT, done)),
					    list(CLET, list(list(v_object, list(GETHASH_WITHOUT_VALUES, id, sparse, v_default))), listS(PUNLESS, list(CAND, list(ID_INDEX_SKIP_TOMBSTONES_P, tombstone), list(ID_INDEX_TOMBSTONE_P, v_object)), append(body, NIL))), list(CINC, id)))));
	}
		...
		current = current.first();
		{
			...
			}
			{
				...
				if (NIL != progress_message) {
					...
					return list(CLET, list(list(total, list(ID_INDEX_COUNT, v_id_index)), bq_cons(sofar, $list_alt71)), listS(CHECK_TYPE, progress_message, $list_alt73), list(NOTING_PERCENT_PROGRESS, progress_message,
						listS(NEW_DO_ID_INDEX, list(new SubLObject[] { id, v_object, v_id_index, $TOMBSTONE, tombstone, $ORDERED, ordered, $DONE, done }), list(NOTE_PERCENT_PROGRESS, sofar, total), list(CINC, sofar), append(body, NIL))));
				    }
				}
				if (!tombstone.isAtom()) {
				    {
				...
				}
				{
				    ...
				    return list(PUNLESS, list(DO_ID_INDEX_EMPTY_P, v_id_index, tombstone),
					    list(CLET, list(list(id, listS(DO_ID_INDEX_NEXT_ID, v_id_index, ordered, $list_alt125)), list(state_var, listS(DO_ID_INDEX_NEXT_STATE, v_id_index, ordered, id, $list_alt127)), v_object),
						    list(WHILE, loop_form, list(CSETQ, v_object, list(DO_ID_INDEX_STATE_OBJECT, v_id_index, tombstone, id, state_var)), listS(PWHEN, list($sym129$DO_ID_INDEX_ID_AND_OBJECT_VALID_, id, v_object, tombstone), append(body, NIL)),
							    list(CSETQ, id, list(DO_ID_INDEX_NEXT_ID, v_id_index, ordered, id, state_var)), list(CSETQ, state_var, list(DO_ID_INDEX_NEXT_STATE, v_id_index, ordered, id, state_var)))));
				}
			    }
			}
		    }
	...
	if (NIL != progress_message) {
	    ...
	    return list(CLET, list(list(total, list(ID_INDEX_COUNT, v_id_index)), bq_cons(sofar, $list80)), listS(CHECK_TYPE, progress_message, $list82), list(NOTING_PERCENT_PROGRESS, progress_message,
		    listS(NEW_DO_ID_INDEX, list(new SubLObject[] { id, v_object, v_id_index, $START_ID, start_id, $END_ID, end_id, $TOMBSTONE, tombstone, $ORDERED, ordered, $DONE, done }), append(body, list(list(CINC, sofar), list(NOTE_PERCENT_PROGRESS, sofar, total))))));
	}
	if (!tombstone.isAtom()) {
	...
	return list(PUNLESS, list(ID_INDEX_OBJECTS_EMPTY_P, v_id_index, tombstone),
		list(CLET,
			list(list(last_id, listS(FIF, list(NON_NEGATIVE_INTEGER_P, start_id), listS($sym139$_, start_id, $list140), $list141)), list(id, listS(DO_ID_INDEX_NEXT_ID, v_id_index, ordered, last_id, $list141)),
				list(state_var, listS(DO_ID_INDEX_NEXT_STATE, v_id_index, ordered, id, $list141)), v_object),
			list(WHILE, loop_form, list(CSETQ, v_object, list(DO_ID_INDEX_STATE_OBJECT, v_id_index, tombstone, id, state_var)), listS(PWHEN, list($sym145$DO_ID_INDEX_OBJECT_VALID_, v_object, tombstone), append(body, NIL)),
				list(CSETQ, id, list(DO_ID_INDEX_NEXT_ID, v_id_index, ordered, id, state_var)), list(CSETQ, state_var, list(DO_ID_INDEX_NEXT_STATE, v_id_index, ordered, id, state_var)))));
    }
    ...
    static private final SubLList $list_alt52 = list(list(makeSymbol("ID"), makeSymbol("OBJECT"), makeSymbol("ID-INDEX"), makeSymbol("&KEY"), list(makeSymbol("TOMBSTONE"), $SKIP), makeSymbol("ORDERED"), makeSymbol("PROGRESS-MESSAGE"), makeSymbol("DONE")), makeSymbol("&BODY"),
	    makeSymbol("BODY"));
	...
	if (((($$$a.equal(id_index_lookup(v_id_index, id_a, UNPROVIDED)) && $$$b.equal(id_index_lookup(v_id_index, id_b, UNPROVIDED))) && $$$c.equal(id_index_lookup(v_id_index, id_c, UNPROVIDED))) && $$$d.equal(id_index_lookup(v_id_index, id_d, UNPROVIDED)))
		&& $$$e.equal(id_index_lookup(v_id_index, id_e, UNPROVIDED))) {
	    return $SUCCESS;
	}
    ...
    static private final SubLList $list_alt166 = list(list(list(makeInteger(100), TWENTY_INTEGER, TWENTY_INTEGER), makeKeyword("SUCCESS")), list(list(makeInteger(1000), makeInteger(200), makeInteger(200)), makeKeyword("SUCCESS")),
	    list(list(makeInteger(255), ZERO_INTEGER, makeInteger(40)), makeKeyword("SUCCESS")));
