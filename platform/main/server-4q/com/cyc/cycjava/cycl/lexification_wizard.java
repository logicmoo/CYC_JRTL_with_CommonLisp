...
import static com.cyc.cycjava.cycl.lexification_wizard.*;
...
public final class lexification_wizard extends SubLTranslatedFile {
    ...
    public static SubLObject lexwiz_id(final SubLObject v_object) {
        assert NIL != lexification_parameters_p(v_object) : "lexification_wizard.lexification_parameters_p(v_object) " + "CommonSymbols.NIL != lexification_wizard.lexification_parameters_p(v_object) " + v_object;
                ...
                if (pcase_var.eql($TERM)) {
                    ...
                    if (pcase_var.eql($PHRASE)) {
                        ...
                        if (pcase_var.eql($MT)) {
                            ...
                            if (pcase_var.eql($kw78$IS_PREFERRED_)) {
                                _csetf_lexwiz_is_preferredP(v_new, current_value);
                            } else
                                if (pcase_var.eql($kw79$IS_PROPER_)) {
                                        ...
                                        if (pcase_var.eql($PROPER_NAME_PRED)) {
                                            ...
                                            if (pcase_var.eql($TEMPLATE)) {
                                                ...
                                                if (pcase_var.eql($TEMPLATE_ARGS)) {
                                                    ...
                                                    if (pcase_var.eql($TEMPLATE_SPEECH_PARTS)) {
                                                        _csetf_lexwiz_template_speech_parts(v_new, current_value);
                                                    } else
                                                        if (pcase_var.eql($kw85$IGNORE_ARITY_)) {
                                                                        ...
                                                                        if (pcase_var.eql($STATE)) {
                                                                            ...
                                                                            if (pcase_var.eql($READY_)) {
                                                                                ...
                                                                                if (pcase_var.eql($FORMULA)) {
                                                                                    ...
                                                                                    if (pcase_var.eql($kw92$USER_EDITED_)) {
                ...
                if (predicate.eql($$compoundString)) {
                    ...
                    if (predicate.eql($$multiWordString)) {
                        ...
                        if (predicate.eql($$headMedialString)) {
                            ...
                            if (predicate.eql($default_denotation_predicate$.getGlobalValue()) && (NIL == other_words)) {
    ...
    public static final class $lexification_parameters_native extends SubLStructNative {
        ...
        static {
            structDecl = makeStructDeclNative(lexification_wizard.$lexification_parameters_native.class, LEXIFICATION_PARAMETERS, LEXIFICATION_PARAMETERS_P, $list2, $list3, new String[]{ "$id", "$term", "$phrase", "$mt", "$is_preferredP", "$is_properP", "$nonrelationalP", "$proper_name_pred", "$template", "$template_args", "$template_speech_parts", "$ignore_arityP", "$headword_position", "$headword_part_of_speech", "$error_message", "$state", "$readyP", "$formula", "$user_editedP", "$user_formula", "$other_formulas", "$user_action", "$parse_template_mt", "$parse_template", "$parse_template_formula", "$lexical_mt", "$syntactic_mt", "$generation_mt" }, $list4, $list5, DEFAULT_STRUCT_PRINT_FUNCTION);
        }
    }
