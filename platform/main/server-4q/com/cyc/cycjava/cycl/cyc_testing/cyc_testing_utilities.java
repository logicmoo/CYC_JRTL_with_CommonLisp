...
public final class cyc_testing_utilities
    extends
      SubLTranslatedFile
{
      ...
      try
      {
        try
        {
          ...
          try
          {
            ...
            try
            {
              final SubLObject pathname = Sequences.cconcatenate( path, new SubLObject[] { filename, $str3$_lisp
              } );
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    return ( NIL != $cyc_test_write_image_directory$.getDynamicValue( thread ) ) ? ( ( NIL != string_utilities.ends_with( $cyc_test_write_image_directory$.getDynamicValue( thread ), $str6$_, UNPROVIDED ) )
        ? $cyc_test_write_image_directory$.getDynamicValue( thread )
        : Sequences.cconcatenate( $cyc_test_write_image_directory$.getDynamicValue( thread ), $str6$_ ) ) : NIL;
  }
  ...
  public static SubLObject run_cyc_test(final SubLObject macroform, final SubLObject environment)
  {
    ...
    while ( NIL != rest)
    {
      ...
      rest = rest.rest();
      cdestructuring_bind.destructuring_bind_must_consp( rest, datum, $list7 );
      if( NIL == conses_high.member( current_$3, $list8, UNPROVIDED, UNPROVIDED ) )
      {
        bad = T;
      }
      if( current_$3 == $kw9$ALLOW_OTHER_KEYS )
      {
    ...
    final SubLObject verbosity_tail = cdestructuring_bind.property_list_member( $kw10$VERBOSITY, current );
    final SubLObject verbosity = ( NIL != verbosity_tail ) ? conses_high.cadr( verbosity_tail ) : $kw11$TERSE;
    final SubLObject browsableP_tail = cdestructuring_bind.property_list_member( $kw12$BROWSABLE_, current );
    final SubLObject browsableP = ( NIL != browsableP_tail ) ? conses_high.cadr( browsableP_tail ) : NIL;
    final SubLObject blockP_tail = cdestructuring_bind.property_list_member( $kw13$BLOCK_, current );
    final SubLObject blockP = ( NIL != blockP_tail ) ? conses_high.cadr( blockP_tail ) : NIL;
    final SubLObject output_format_tail = cdestructuring_bind.property_list_member( $kw14$OUTPUT_FORMAT, current );
    final SubLObject output_format = ( NIL != output_format_tail ) ? conses_high.cadr( output_format_tail ) : $kw15$STANDARD;
    final SubLObject stream_tail = cdestructuring_bind.property_list_member( $kw16$STREAM, current );
    final SubLObject stream = ( NIL != stream_tail ) ? conses_high.cadr( stream_tail ) : $sym17$_STANDARD_OUTPUT_;
    final SubLObject run_tiny_kb_tests_in_full_kbP_tail = cdestructuring_bind.property_list_member( $kw18$RUN_TINY_KB_TESTS_IN_FULL_KB_, current );
    final SubLObject run_tiny_kb_tests_in_full_kbP = ( NIL != run_tiny_kb_tests_in_full_kbP_tail ) ? conses_high.cadr( run_tiny_kb_tests_in_full_kbP_tail ) : $sym19$_RUN_TINY_KB_TESTS_IN_FULL_KB__;
    return ConsesLow.list( $sym20$RUN_CYC_TEST_INT, name, verbosity, browsableP, blockP, output_format, stream, run_tiny_kb_tests_in_full_kbP );
  }
  ...
  public static SubLObject run_cyc_test_in_range(final SubLObject macroform, final SubLObject environment)
  {
    ...
    while ( NIL != rest)
    {
      ...
      rest = rest.rest();
      cdestructuring_bind.destructuring_bind_must_consp( rest, datum, $list21 );
      if( NIL == conses_high.member( current_$4, $list8, UNPROVIDED, UNPROVIDED ) )
      {
        bad = T;
      }
    ...
    final SubLObject run_tiny_kb_tests_in_full_kbP_tail = cdestructuring_bind.property_list_member( $kw18$RUN_TINY_KB_TESTS_IN_FULL_KB_, current );
    final SubLObject run_tiny_kb_tests_in_full_kbP = ( NIL != run_tiny_kb_tests_in_full_kbP_tail ) ? conses_high.cadr( run_tiny_kb_tests_in_full_kbP_tail ) : $sym19$_RUN_TINY_KB_TESTS_IN_FULL_KB__;
    return ConsesLow.list( new SubLObject[] { $sym22$RUN_CYC_TEST_IN_RANGE_INT, name, min_number, max_number, verbosity, browsableP, blockP, output_format, stream, run_tiny_kb_tests_in_full_kbP
    } );
  }
  ...
  public static SubLObject run_cyc_test_verbose(final SubLObject macroform, final SubLObject environment)
  {
    ...
    while ( NIL != rest)
    {
      ...
      rest = rest.rest();
      cdestructuring_bind.destructuring_bind_must_consp( rest, datum, $list23 );
      if( NIL == conses_high.member( current_$5, $list24, UNPROVIDED, UNPROVIDED ) )
      {
        bad = T;
      }
    ...
    final SubLObject run_tiny_kb_tests_in_full_kbP_tail = cdestructuring_bind.property_list_member( $kw18$RUN_TINY_KB_TESTS_IN_FULL_KB_, current );
    final SubLObject run_tiny_kb_tests_in_full_kbP = ( NIL != run_tiny_kb_tests_in_full_kbP_tail ) ? conses_high.cadr( run_tiny_kb_tests_in_full_kbP_tail ) : $sym19$_RUN_TINY_KB_TESTS_IN_FULL_KB__;
    return ConsesLow.list( $sym20$RUN_CYC_TEST_INT, name, $kw25$VERBOSE, browsableP, blockP, output_format, stream, run_tiny_kb_tests_in_full_kbP );
  }

  @SubLTranslatedFile.SubL(source = "cycl/cyc-testing/cyc-testing-utilities.lisp", position = 3402L)
  public static SubLObject run_cyc_test_int(final SubLObject name, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, final SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    assert NIL != cyc_testing.cyc_test_verbosity_level_p( verbosity ) : verbosity;
    assert NIL != Types.booleanp( browsableP ) : browsableP;
    assert NIL != Types.booleanp( blockP ) : blockP;
    assert NIL != cyc_testing.cyc_test_output_format_p( output_format ) : output_format;
    assert NIL != Types.streamp( stream ) : stream;
    assert NIL != Types.booleanp( run_tiny_kb_tests_in_full_kbP ) : run_tiny_kb_tests_in_full_kbP;
  ...
  public static SubLObject run_cyc_test_in_range_int(final SubLObject name, final SubLObject min_number, final SubLObject max_number, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP,
      final SubLObject output_format, final SubLObject stream, final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    assert NIL != cyc_testing.cyc_test_verbosity_level_p( verbosity ) : verbosity;
    assert NIL != Types.booleanp( browsableP ) : browsableP;
    assert NIL != Types.booleanp( blockP ) : blockP;
  ...
  public static SubLObject run_cyc_test_parallel(final SubLObject macroform, final SubLObject environment)
  {
    ...
    while ( NIL != rest)
    {
      cdestructuring_bind.destructuring_bind_must_consp( rest, datum, $list31 );
      current_$6 = rest.first();
      rest = rest.rest();
      cdestructuring_bind.destructuring_bind_must_consp( rest, datum, $list31 );
      if( NIL == conses_high.member( current_$6, $list32, UNPROVIDED, UNPROVIDED ) )
      {
        bad = T;
      }
    ...
    final SubLObject nthreads_tail = cdestructuring_bind.property_list_member( $kw33$NTHREADS, current );
    final SubLObject nthreads = ( NIL != nthreads_tail ) ? conses_high.cadr( nthreads_tail ) : FOUR_INTEGER;
    final SubLObject ntimes_tail = cdestructuring_bind.property_list_member( $kw34$NTIMES, current );
    final SubLObject ntimes = ( NIL != ntimes_tail ) ? conses_high.cadr( ntimes_tail ) : TEN_INTEGER;
    final SubLObject verbosity_tail = cdestructuring_bind.property_list_member( $kw10$VERBOSITY, current );
    final SubLObject verbosity = ( NIL != verbosity_tail ) ? conses_high.cadr( verbosity_tail ) : $kw11$TERSE;
    final SubLObject browsableP_tail = cdestructuring_bind.property_list_member( $kw12$BROWSABLE_, current );
    final SubLObject browsableP = ( NIL != browsableP_tail ) ? conses_high.cadr( browsableP_tail ) : NIL;
    ...
    final SubLObject run_tiny_kb_tests_in_full_kbP_tail = cdestructuring_bind.property_list_member( $kw18$RUN_TINY_KB_TESTS_IN_FULL_KB_, current );
    final SubLObject run_tiny_kb_tests_in_full_kbP = ( NIL != run_tiny_kb_tests_in_full_kbP_tail ) ? conses_high.cadr( run_tiny_kb_tests_in_full_kbP_tail ) : $sym19$_RUN_TINY_KB_TESTS_IN_FULL_KB__;
    return ConsesLow.list( new SubLObject[] { $sym35$RUN_CYC_TEST_PARALLEL_INT, name, nthreads, ntimes, verbosity, browsableP, blockP, output_format, stream, run_tiny_kb_tests_in_full_kbP
    } );
  }

  @SubLTranslatedFile.SubL(source = "cycl/cyc-testing/cyc-testing-utilities.lisp", position = 5238L)
  public static SubLObject run_cyc_test_parallel_int(final SubLObject name, final SubLObject nthreads, final SubLObject ntimes, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP,
      final SubLObject output_format, final SubLObject stream, final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    assert NIL != subl_promotions.non_negative_integer_p( nthreads ) : nthreads;
    assert NIL != subl_promotions.non_negative_integer_p( ntimes ) : ntimes;
    assert NIL != cyc_testing.cyc_test_verbosity_level_p( verbosity ) : verbosity;
    assert NIL != cyc_testing.cyc_test_verbosity_level_p( verbosity ) : verbosity;
    assert NIL != Types.booleanp( browsableP ) : browsableP;
    assert NIL != Types.booleanp( blockP ) : blockP;
    ...
    assert NIL != Types.booleanp( run_tiny_kb_tests_in_full_kbP ) : run_tiny_kb_tests_in_full_kbP;
    SubLObject i;
    for( i = NIL, i = ZERO_INTEGER; i.numL( nthreads ); i = Numbers.add( i, ONE_INTEGER ) )
    {
      subl_promotions.make_process_with_args( Sequences.cconcatenate( $str38$Parallel_Test_Thread_for_, new SubLObject[] { format_nil.format_nil_a_no_copy( name ), $str39$_No__, format_nil.format_nil_a_no_copy( i )
      } ), Symbols.symbol_function( $sym40$RUN_CYC_TEST_OBJECT_PARALLEL ), ConsesLow.list( new SubLObject[] { name, i, ntimes, verbosity, browsableP, blockP, output_format, stream, run_tiny_kb_tests_in_full_kbP
      } ) );
    }
    return NIL;
  }
  ...
  public static SubLObject run_cyc_test_object_parallel(final SubLObject name, final SubLObject thread_num, final SubLObject ntimes, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP,
      final SubLObject output_format, final SubLObject stream, final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    SubLObject test_runs = NIL;
    SubLObject cdolist_list_var = cyc_testing.find_cyc_tests( name );
    ...
    {
      SubLObject i;
      for( i = NIL, i = ZERO_INTEGER; i.numL( ntimes ); i = Numbers.add( i, ONE_INTEGER ) )
      {
        test_runs = ConsesLow.nconc( test_runs, run_cyc_test_object( ct, verbosity, browsableP, blockP, output_format, stream, run_tiny_kb_tests_in_full_kbP ) );
      }
      cdolist_list_var = cdolist_list_var.rest();
  ...
  public static SubLObject run_cyc_test_object(final SubLObject ct, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, final SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
  ...
  public static SubLObject run_cyc_test_iut(final SubLObject iut, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, final SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject verboseP = makeBoolean( $kw50$SILENT != verbosity );
    ...
    thread.resetMultipleValues();
    return ConsesLow.list( test_run );
  }

  @SubLTranslatedFile.SubL(source = "cycl/cyc-testing/cyc-testing-utilities.lisp", position = 8640L)
  public static SubLObject run_cyc_test_it(final SubLObject it, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, final SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( $kw50$SILENT != verbosity )
    {
      if( NIL != browsableP )
      {
    ...
    final SubLObject progress_blipP = makeBoolean( $kw50$SILENT != verbosity );
    final SubLObject verboseP = Equality.eq( $kw25$VERBOSE, verbosity );
    ...
    try
    {
      ...
      backward_test_run = run_cyc_test_it_int( it, progress_blipP, verboseP, $kw53$BACKWARD );
      forward_test_run = run_cyc_test_it_int( it, progress_blipP, verboseP, $kw54$FORWARD );
    }
    finally
    {
    ...
    return ConsesLow.list( backward_test_run, forward_test_run );
  }
  ...
  public static SubLObject run_cyc_test_it_int(final SubLObject it, final SubLObject progress_blipP, final SubLObject verboseP, final SubLObject testing_direction)
  {
    ...
    if( NIL == cyc_testing.$cyc_test_debugP$.getDynamicValue( thread ) )
    {
      try
      {
        ...
        try
        {
          ...
          try
          {
            if( testing_direction.eql( $kw53$BACKWARD ) )
            {
              test_run = inference_testing.run_test( it, progress_blipP, verboseP, $kw53$BACKWARD, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
            }
            else if( testing_direction.eql( $kw54$FORWARD ) )
            {
              test_run = inference_testing.run_test( it, progress_blipP, verboseP, $kw54$FORWARD, NIL, UNPROVIDED, UNPROVIDED, UNPROVIDED );
            }
            else
            {
    ...
    else if( testing_direction.eql( $kw53$BACKWARD ) )
    {
      test_run = inference_testing.run_test( it, progress_blipP, verboseP, $kw53$BACKWARD, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    else if( testing_direction.eql( $kw54$FORWARD ) )
    ...
    {
      test_run = cyc_testing.new_cyc_test_run( $kw42$IT, ConsesLow.list( it, testing_direction ), $kw56$ERROR, total_time );
    }
    return test_run;
  }

  @SubLTranslatedFile.SubL(source = "cycl/cyc-testing/cyc-testing-utilities.lisp", position = 10180L)
  public static SubLObject run_cyc_test_rmt(final SubLObject rmt, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject test_runs = NIL;
    ...
    try
    {
      ...
      final SubLObject test_run = thread.secondMultipleValue();
      thread.resetMultipleValues();
      test_runs = ConsesLow.list( test_run );
    }
    finally
    {
  ...
  public static SubLObject run_cyc_test_tmt(final SubLObject tmt, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    SubLObject test_runs = NIL;
  ...
  public static SubLObject run_cyc_test_rmct(final SubLObject rmct, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( $kw50$SILENT != verbosity )
  ...
  public static SubLObject run_cyc_test_ert(final SubLObject ert, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( $kw50$SILENT != verbosity && NIL != blockP )
    {
  ...
  public static SubLObject run_cyc_test_tct(final SubLObject tct, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, final SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( $kw50$SILENT != verbosity )
    ...
    SubLObject test_runs = NIL;
    final SubLObject generic_test_verbosity_level = ( $kw62$POST_BUILD == output_format ) ? $kw62$POST_BUILD : verbosity;
    final SubLObject _prev_bind_0 = cyc_testing.$run_tiny_kb_tests_in_full_kbP$.currentBinding( thread );
    try
    {
      cyc_testing.$run_tiny_kb_tests_in_full_kbP$.bind( run_tiny_kb_tests_in_full_kbP, thread );
      thread.resetMultipleValues();
      final SubLObject result = generic_testing.run_generic_test_case_int( stream, tct, generic_test_verbosity_level, NIL );
  ...
  public static SubLObject run_cyc_test_kct(final SubLObject kct, final SubLObject verbosity, final SubLObject browsableP, final SubLObject blockP, final SubLObject output_format, final SubLObject stream,
      final SubLObject run_tiny_kb_tests_in_full_kbP)
  {
    return ConsesLow.list( kct_cyc_testing.kct_run_test_as_cyc_test( kct, verbosity, browsableP, blockP, output_format, stream ) );
  }

  @SubLTranslatedFile.SubL(source = "cycl/cyc-testing/cyc-testing-utilities.lisp", position = 13474L)
  public static SubLObject run_all_loaded_cyc_tests_int(final SubLObject stream, final SubLObject verbosity, final SubLObject stop_at_first_failureP, final SubLObject output_format,
      final SubLObject run_tiny_kb_tests_in_full_kbP, final SubLObject run_test_case_tablesP, final SubLObject type, final SubLObject run_non_working_tests, final SubLObject progress_file,
      SubLObject rerun_crashing_tests, SubLObject tests, SubLObject phase, SubLObject phases)
  {
    if( tests == UNPROVIDED )
    {
    ...
    assert NIL != Types.booleanp( run_tiny_kb_tests_in_full_kbP ) : run_tiny_kb_tests_in_full_kbP;
    assert NIL != cyc_testing.cyc_test_type_spec_p( type ) : type;
    if( NIL != progress_file && !assertionsDisabledInClass && NIL == file_utilities.file_valid_for_writing_p( progress_file ) )
    {
    ...
    assert NIL != subl_promotions.non_negative_integer_p( rerun_crashing_tests ) : rerun_crashing_tests;
    if( NIL != cyc_test_write_image_directory() && !assertionsDisabledInClass && NIL == file_utilities.directory_writeable_p( cyc_test_write_image_directory(), UNPROVIDED ) )
    {
    ...
    final SubLObject silentP = Equality.eq( $kw50$SILENT, verbosity );
    ...
    if( NIL != progress_file )
    {
      ...
      try
      {
        ...
        try
        {
          ...
          stream_$7 = compatibility.open_binary( progress_file, $kw67$APPEND );
        }
        finally
        {
        ...
        try
        {
          ...
          try
          {
            ...
            try
            {
              ...
              list_var = tests;
              ct = list_var.first();
              for( test_num = ZERO_INTEGER; NIL == doneP && NIL != list_var; list_var = list_var.rest(), ct = list_var.first(), test_num = Numbers.add( ONE_INTEGER, test_num ) )
              {
                ...
                {
                  partial_test_runs = ConsesLow.list( new_cyc_test_null_run( ct ) );
                }
                else if( $kw47$TCT != cyc_testing.cyc_test_type( ct ) && NIL != cyc_test_mentions_invalid_constantP( ct ) )
                {
                  partial_test_runs = ConsesLow.list( new_cyc_test_invalid_run( ct ) );
                }
                else
                {
      ...
      finally
      {
        ...
        try
        {
          ...
          if( stream_$7.isStream() )
          {
            streams_high.close( stream_$7, UNPROVIDED );
          }
          Values.restoreValuesFromVector( _values );
        }
        finally
      ...
      try
      {
        ...
        try
        {
          ...
          try
          {
            ...
            list_var2 = tests;
            ct2 = list_var2.first();
            for( test_num2 = ZERO_INTEGER; NIL == doneP && NIL != list_var2; list_var2 = list_var2.rest(), ct2 = list_var2.first(), test_num2 = Numbers.add( ONE_INTEGER, test_num2 ) )
            {
              ...
              {
                partial_test_runs2 = ConsesLow.list( new_cyc_test_null_run( ct2 ) );
              }
              else if( $kw47$TCT != cyc_testing.cyc_test_type( ct2 ) && NIL != cyc_test_mentions_invalid_constantP( ct2 ) )
              {
                partial_test_runs2 = ConsesLow.list( new_cyc_test_invalid_run( ct2 ) );
              }
              else
              {
    ...
    {
      overall_result = $kw70$HARNESS_ERROR;
    }
  ...
  public static SubLObject load_cyc_tests_progress_file(final SubLObject progress_file, final SubLObject rerun_crashing_tests)
  {
    ...
    {
      return Values.values( NIL, set.new_set( Symbols.symbol_function( EQ ), UNPROVIDED ) );
    }
    final SubLObject dict = dictionary.new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED );
    final SubLObject history_dict = dictionary.new_dictionary( Symbols.symbol_function( EQ ), UNPROVIDED );
    ...
    try
    {
      ...
      {
        stream_macros.$stream_requires_locking$.bind( NIL, thread );
        stream = compatibility.open_binary( progress_file, $kw71$INPUT );
      }
      finally
      {
        stream_macros.$stream_requires_locking$.rebind( _prev_bind_0, thread );
      ...
      for( stream_$10 = stream, current_test_run = cfasl.cfasl_input( stream_$10, NIL, $kw72$EOF ); $kw72$EOF != current_test_run; current_test_run = cfasl.cfasl_input( stream_$10, NIL, $kw72$EOF ) )
      {
        assert NIL != cyc_testing.cyc_test_run_p( current_test_run ) : current_test_run;
    ...
    final SubLObject tests_to_avoid = set_utilities.construct_set_from_list( dictionary.dictionary_keys( dict ), Symbols.symbol_function( EQ ), UNPROVIDED );
  ...
  public static SubLObject new_cyc_test_null_run(final SubLObject ct)
  {
    ...
    return cyc_testing.new_cyc_test_run( type, name, $kw74$NOT_RUN, NIL );
  }
  ...
  public static SubLObject new_cyc_test_invalid_run(final SubLObject ct)
  {
    final SubLObject type = cyc_testing.cyc_test_type( ct );
    final SubLObject name = cyc_testing.cyc_test_name( ct );
    return cyc_testing.new_cyc_test_run( type, name, $kw75$INVALID, NIL );
  }
  ...
  public static SubLObject new_cyc_test_error_run(final SubLObject ct)
  {
    final SubLObject type = cyc_testing.cyc_test_type( ct );
    final SubLObject name = cyc_testing.cyc_test_name( ct );
    return cyc_testing.new_cyc_test_run( type, name, $kw56$ERROR, NIL );
  }
  ...
  public static SubLObject run_cyc_testP(final SubLObject ct, final SubLObject type_spec, final SubLObject run_test_case_tablesP, final SubLObject run_non_working_tests, SubLObject tests_to_avoid)
  {
    ...
    if( NIL == run_test_case_tablesP && $kw47$TCT == cyc_testing.cyc_test_type( ct ) )
    {
      return NIL;
    }
    ...
    if( T != run_non_working_tests )
    {
      ...
      {
        if( $kw77$ONLY != run_non_working_tests )
        {
  ...
  public static SubLObject print_failing_cyc_tests_message(final SubLObject stream, final SubLObject test_runs, SubLObject show_ownersP, SubLObject show_projectsP)
  {
    ...
    PrintLow.format( stream, $str79$____Failing_regression_tests___ );
    final SubLObject failing_test_runs = list_utilities.remove_if_not( $sym80$FAILING_CYC_TEST_RUN_P, test_runs, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    ...
    {
      PrintLow.format( stream, $str81$None____ );
    }
  ...
  public static SubLObject print_succeeding_cyc_tests_message(final SubLObject stream, final SubLObject test_runs, SubLObject show_ownersP, SubLObject show_projectsP)
  {
    ...
    PrintLow.format( stream, $str82$____Succeeding_regression_tests__ );
    final SubLObject succeeding_test_runs = list_utilities.remove_if_not( $sym83$SUCCEEDING_CYC_TEST_RUN_P, test_runs, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    ...
    {
      PrintLow.format( stream, $str84$None____ );
    }
    streams_high.force_output( stream );
    return NIL;
  }
  ...
  public static SubLObject print_ignored_cyc_tests_message(final SubLObject stream, final SubLObject test_runs, SubLObject show_ownersP, SubLObject show_projectsP)
  {
    ...
    final SubLObject ignored_test_runs = list_utilities.remove_if_not( $sym85$IGNORED_CYC_TEST_RUN_P, test_runs, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    if( NIL == ignored_test_runs )
    {
      PrintLow.format( stream, $str86$____Non_regression_tests__None_ );
    }
    else
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        if( pcase_var.eql( $kw87$NON_REGRESSION_SUCCESS ) )
        {
          ignored_success = ConsesLow.cons( test_run, ignored_success );
        }
        else if( pcase_var.eql( $kw88$NON_REGRESSION_FAILURE ) )
        {
          ignored_fail = ConsesLow.cons( test_run, ignored_fail );
        }
        cdolist_list_var = cdolist_list_var.rest();
        test_run = cdolist_list_var.first();
      }
      ...
      PrintLow.format( stream, $str89$__Failing_non_regression_tests___ );
      ...
      PrintLow.format( stream, $str90$__Succeeding_non_regression_tests );
  ...
  public static SubLObject run_all_loaded_cyc_tests_print_header(final SubLObject stream, final SubLObject start_time, final SubLObject tests_specifiedP, SubLObject phase, SubLObject phases)
  {
    ...
    final SubLObject tiny_kbP = makeBoolean( NIL == control_vars.non_tiny_kb_loadedP() );
    if( NIL != tests_specifiedP )
    {
      PrintLow.format( stream, $str91$_Running_a_specified_list_of_test );
    }
    else
    {
      PrintLow.format( stream, $str92$_Running__a_file_P_of_tests_, cyc_testing.cyc_test_file_count(), cyc_testing.cyc_test_file_count() );
    }
    PrintLow.format( stream, $str93$in_the__a_KB__KB__a___System__a_S, new SubLObject[] { ( NIL != tiny_kbP ) ? $str94$Tiny : $str95$Full, control_vars.kb_loaded(), system_info.cyc_revision_string(),
      numeric_date_utilities.timestring( start_time ), ( ONE_INTEGER.numE( phases ) && ZERO_INTEGER.numE( phase ) ) ? $str96$ : Sequences.cconcatenate( $str97$_, new SubLObject[]
      { format_nil.format_nil_a_no_copy( Numbers.add( ONE_INTEGER, phase ) ), $str98$_of_, format_nil.format_nil_a_no_copy( phases ), $str99$_phases_
      } )
    } );
    streams_high.force_output( stream );
    return NIL;
  ...
  public static SubLObject run_all_loaded_cyc_tests_print_footer(final SubLObject stream, final SubLObject start_time, final SubLObject test_runs, final SubLObject tests_specifiedP, final SubLObject type)
  {
    if( NIL == test_runs )
    {
      PrintLow.format( stream, $str100$__No_tests_were_run___ );
    }
    else
    {
      if( NIL == tests_specifiedP )
      {
        ...
        {
          final SubLObject non_working_test_names = Mapping.mapcar( Symbols.symbol_function( $sym101$CYC_TEST_NAME ), non_working_tests );
          PrintLow.format( stream, $str102$__NOTE__The_following_cyc_tests_w );
      ...
      PrintLow.format( stream, $str103$___Tests_finished_at__a_Total_tim, new SubLObject[] { numeric_date_utilities.timestring( end_time ), numeric_date_utilities.elapsed_time_abbreviation_string( total_time ),
        numeric_date_utilities.elapsed_time_abbreviation_string( total_test_guts_time )
      } );
      list_utilities.pretty_print_plist( list_utilities.histogram_as_plist( Mapping.mapcar( $sym104$CYC_TEST_RUN_RESULT, test_runs ), UNPROVIDED, UNPROVIDED, UNPROVIDED ), UNPROVIDED );
      PrintLow.format( stream, $str105$Overall_result___s__, overall_result );
    }
    streams_high.force_output( stream );
    return NIL;
  }
  ...
  {
    PrintLow.format( stream, $str106$_S__A__S__A, new SubLObject[] { cyc_testing.cyc_test_run_type( test_run ), cyc_testing.cyc_test_run_name( test_run ), cyc_testing.cyc_test_run_result( test_run ), ( NIL != cyc_testing
        .cyc_test_run_time( test_run ) ) ? number_utilities.significant_digits( cyc_testing.cyc_test_run_time( test_run ), FOUR_INTEGER ) : NIL
    } );
    if( NIL != show_ownerP )
    {
      PrintLow.format( stream, $str107$__A, cyc_testing.cyc_test_run_owner( test_run ) );
    }
    if( NIL != show_projectP )
    {
      ...
      if( NIL != project )
      {
        PrintLow.format( stream, $str107$__A, project );
      }
    }
    streams_high.terpri( stream );
    streams_high.force_output( stream );
  ...
  public static SubLObject cyc_test_possibly_cleanup_write_image(final SubLObject ct, final SubLObject test_runs)
  {
    ...
    final SubLObject filename = cyc_test_image_filename( ct, dir );
    if( $kw108$SUCCESS == cyc_testing.cyc_test_runs_overall_result( test_runs ) )
    {
      SubLObject message_var = NIL;
      final SubLObject was_appendingP = Eval.eval( $sym0$_APPEND_STACK_TRACES_TO_ERROR_MESSAGES__ );
      Eval.eval( $list1 );
  ...
  public static SubLObject cyc_test_image_filename(final SubLObject ct, final SubLObject dir)
  {
    ...
    final SubLObject filename = Sequences.cconcatenate( $str109$cyc_test_image_, test_identifier );
    return Strings.string_downcase( file_utilities.relative_filename( dir, filename, $str110$load ), UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject directory_file_size_dict(final SubLObject directory)
  {
    ...
    try
    {
      ...
      try
      {
        utilities_macros.$progress_note$.bind( ( NIL != progress_message_var ) ? progress_message_var : $str115$cdolist, thread );
  ...
  public static SubLObject compare_file_size_dictionaries(final SubLObject old_dict, final SubLObject new_dict)
  {
        ...
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      ...
      if( NIL == new_size )
      {
        alerts = ConsesLow.cons( new_file_existence_difference_alert( filename, T ), alerts );
      }
      else if( NIL == file_size_difference_within_toleranceP( old_size, new_size, filename ) )
      {
        alerts = ConsesLow.cons( new_file_size_difference_alert( old_size, new_size, filename ), alerts );
      }
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( new_dict ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
      final SubLObject filename = dictionary_contents.do_dictionary_contents_key_value( iteration_state );
      ...
      {
        alerts = ConsesLow.cons( new_file_existence_difference_alert( filename, NIL ), alerts );
      }
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
  ...
  {
    final SubLObject grewP = file_size_grewP( old_size, new_size );
    return Sequences.cconcatenate( format_nil.format_nil_a_no_copy( filename ), new SubLObject[] { $str116$_, format_nil.format_nil_a_no_copy( ( NIL != grewP ) ? $str117$grew : $str118$shrunk ), $str119$_from_,
      format_nil.format_nil_a_no_copy( old_size ), $str120$_bytes_to_, format_nil.format_nil_a_no_copy( new_size ), $str121$_bytes
    } );
  }
  ...
  public static SubLObject new_file_existence_difference_alert(final SubLObject filename, final SubLObject exists_in_oldP)
  {
    ...
    {
      return Sequences.cconcatenate( format_nil.format_nil_a_no_copy( filename ), $str122$_disappeared );
    }
    return Sequences.cconcatenate( format_nil.format_nil_a_no_copy( filename ), $str123$_is_a_new_file );
  }
  ...
  public static SubLObject write_directory_file_size_log(final SubLObject directory, final SubLObject logfile)
  {
    ...
    final SubLObject header_1 = $str124$filename;
    final SubLObject header_2 = Sequences.cconcatenate( $str125$size_in_bytes_as_of_, numeric_date_utilities.timestring( UNPROVIDED ) );
  ...
  {
    return dictionary_utilities.new_dictionary_from_csv( logfile, Symbols.symbol_function( EQUAL ), T, T );
  }
  ...
  {
    SubLFiles.declareFunction( me, "load_lisp_test_file_from_path", "LOAD-LISP-TEST-FILE-FROM-PATH", 2, 0, false );
    SubLFiles.declareFunction( me, "cyc_test_write_image_directory", "CYC-TEST-WRITE-IMAGE-DIRECTORY", 0, 0, false );
    SubLFiles.declareMacro( me, "run_cyc_test", "RUN-CYC-TEST" );
    SubLFiles.declareMacro( me, "run_cyc_test_in_range", "RUN-CYC-TEST-IN-RANGE" );
    SubLFiles.declareMacro( me, "run_cyc_test_verbose", "RUN-CYC-TEST-VERBOSE" );
    SubLFiles.declareFunction( me, "run_cyc_test_int", "RUN-CYC-TEST-INT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_in_range_int", "RUN-CYC-TEST-IN-RANGE-INT", 9, 0, false );
    SubLFiles.declareMacro( me, "run_cyc_test_parallel", "RUN-CYC-TEST-PARALLEL" );
    SubLFiles.declareFunction( me, "run_cyc_test_parallel_int", "RUN-CYC-TEST-PARALLEL-INT", 9, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_object_parallel", "RUN-CYC-TEST-OBJECT-PARALLEL", 9, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_object", "RUN-CYC-TEST-OBJECT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_iut", "RUN-CYC-TEST-IUT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_it", "RUN-CYC-TEST-IT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_it_int", "RUN-CYC-TEST-IT-INT", 4, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_rmt", "RUN-CYC-TEST-RMT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_tmt", "RUN-CYC-TEST-TMT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_rmct", "RUN-CYC-TEST-RMCT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_ert", "RUN-CYC-TEST-ERT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_tct", "RUN-CYC-TEST-TCT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_test_kct", "RUN-CYC-TEST-KCT", 7, 0, false );
    SubLFiles.declareFunction( me, "run_all_loaded_cyc_tests_int", "RUN-ALL-LOADED-CYC-TESTS-INT", 10, 3, false );
    SubLFiles.declareFunction( me, "cyc_test_part_of_phaseP", "CYC-TEST-PART-OF-PHASE?", 4, 0, false );
    SubLFiles.declareFunction( me, "load_cyc_tests_progress_file", "LOAD-CYC-TESTS-PROGRESS-FILE", 2, 0, false );
    SubLFiles.declareFunction( me, "new_cyc_test_null_run", "NEW-CYC-TEST-NULL-RUN", 1, 0, false );
    SubLFiles.declareFunction( me, "new_cyc_test_invalid_run", "NEW-CYC-TEST-INVALID-RUN", 1, 0, false );
    SubLFiles.declareFunction( me, "new_cyc_test_error_run", "NEW-CYC-TEST-ERROR-RUN", 1, 0, false );
    SubLFiles.declareFunction( me, "run_cyc_testP", "RUN-CYC-TEST?", 4, 1, false );
    SubLFiles.declareFunction( me, "print_failing_cyc_tests_message", "PRINT-FAILING-CYC-TESTS-MESSAGE", 2, 2, false );
    SubLFiles.declareFunction( me, "print_succeeding_cyc_tests_message", "PRINT-SUCCEEDING-CYC-TESTS-MESSAGE", 2, 2, false );
    SubLFiles.declareFunction( me, "print_ignored_cyc_tests_message", "PRINT-IGNORED-CYC-TESTS-MESSAGE", 2, 2, false );
    SubLFiles.declareFunction( me, "run_all_loaded_cyc_tests_print_header", "RUN-ALL-LOADED-CYC-TESTS-PRINT-HEADER", 3, 2, false );
    SubLFiles.declareFunction( me, "run_all_loaded_cyc_tests_print_footer", "RUN-ALL-LOADED-CYC-TESTS-PRINT-FOOTER", 5, 0, false );
    SubLFiles.declareFunction( me, "show_cyc_test_run", "SHOW-CYC-TEST-RUN", 4, 0, false );
    SubLFiles.declareFunction( me, "cyc_test_possibly_write_image", "CYC-TEST-POSSIBLY-WRITE-IMAGE", 1, 0, false );
    SubLFiles.declareFunction( me, "cyc_test_possibly_cleanup_write_image", "CYC-TEST-POSSIBLY-CLEANUP-WRITE-IMAGE", 2, 0, false );
    SubLFiles.declareFunction( me, "cyc_test_image_filename", "CYC-TEST-IMAGE-FILENAME", 2, 0, false );
    SubLFiles.declareFunction( me, "cyc_test_mentions_invalid_constantP", "CYC-TEST-MENTIONS-INVALID-CONSTANT?", 1, 0, false );
    SubLFiles.declareFunction( me, "compare_directories", "COMPARE-DIRECTORIES", 2, 0, false );
    SubLFiles.declareFunction( me, "directory_file_size_dict", "DIRECTORY-FILE-SIZE-DICT", 1, 0, false );
    SubLFiles.declareFunction( me, "compare_file_size_dictionaries", "COMPARE-FILE-SIZE-DICTIONARIES", 2, 0, false );
    SubLFiles.declareFunction( me, "file_size_difference_within_toleranceP", "FILE-SIZE-DIFFERENCE-WITHIN-TOLERANCE?", 3, 0, false );
    SubLFiles.declareFunction( me, "size_change_tolerance", "SIZE-CHANGE-TOLERANCE", 1, 0, false );
    SubLFiles.declareFunction( me, "file_size_grewP", "FILE-SIZE-GREW?", 2, 0, false );
    SubLFiles.declareFunction( me, "new_file_size_difference_alert", "NEW-FILE-SIZE-DIFFERENCE-ALERT", 3, 0, false );
    SubLFiles.declareFunction( me, "new_file_existence_difference_alert", "NEW-FILE-EXISTENCE-DIFFERENCE-ALERT", 2, 0, false );
    SubLFiles.declareFunction( me, "ignore_filename_due_to_regexP", "IGNORE-FILENAME-DUE-TO-REGEX?", 1, 0, false );
    SubLFiles.declareFunction( me, "string_matches_any_regexP", "STRING-MATCHES-ANY-REGEX?", 2, 0, false );
    SubLFiles.declareFunction( me, "write_directory_file_size_log", "WRITE-DIRECTORY-FILE-SIZE-LOG", 2, 0, false );
    SubLFiles.declareFunction( me, "read_directory_file_size_log", "READ-DIRECTORY-FILE-SIZE-LOG", 1, 0, false );
    SubLFiles.declareFunction( me, "compare_file_size_logs", "COMPARE-FILE-SIZE-LOGS", 2, 0, false );
    return NIL;
  }

  public static SubLObject init_cyc_testing_utilities_file()
  {
    $default_rerun_crashing_tests$ = SubLFiles.defparameter( "*DEFAULT-RERUN-CRASHING-TESTS*", ONE_INTEGER );
    $cyc_test_write_image_directory$ = SubLFiles.defparameter( "*CYC-TEST-WRITE-IMAGE-DIRECTORY*", NIL );
    $tests_that_dont_work_with_real_time_pruning$ = SubLFiles.deflexical( "*TESTS-THAT-DONT-WORK-WITH-REAL-TIME-PRUNING*", $list111 );
    $file_size_change_tolerance$ = SubLFiles.defparameter( "*FILE-SIZE-CHANGE-TOLERANCE*", $float112$1_2 );
    $file_size_ignore_regexes$ = SubLFiles.defparameter( "*FILE-SIZE-IGNORE-REGEXES*", $list113 );
    $size_logs_directory$ = SubLFiles.deflexical( "*SIZE-LOGS-DIRECTORY*", $str96$ );
    return NIL;
  }

  public static SubLObject setup_cyc_testing_utilities_file()
  {
    access_macros.register_macro_helper( $sym20$RUN_CYC_TEST_INT, $sym26$RUN_CYC_TEST );
    access_macros.register_macro_helper( $sym22$RUN_CYC_TEST_IN_RANGE_INT, $sym26$RUN_CYC_TEST );
    access_macros.register_macro_helper( $sym35$RUN_CYC_TEST_PARALLEL_INT, $sym36$RUN_CYC_TEST_PARALLEL );
    return NIL;
  }
  ...
  static
  {
    me = new cyc_testing_utilities();
    $default_rerun_crashing_tests$ = null;
    $cyc_test_write_image_directory$ = null;
    $tests_that_dont_work_with_real_time_pruning$ = null;
    $file_size_change_tolerance$ = null;
    $file_size_ignore_regexes$ = null;
    $size_logs_directory$ = null;
    ...
    $str3$_lisp = makeString( ".lisp" );
    $sym4$CSETQ = makeSymbol( "CSETQ" );
    $str5$_A = makeString( "~A" );
    $str6$_ = makeString( "/" );
    $list7 = ConsesLow.list( makeSymbol( "NAME" ), makeSymbol( "&KEY" ), ConsesLow.list( makeSymbol( "VERBOSITY" ), makeKeyword( "TERSE" ) ), makeSymbol( "BROWSABLE?" ), makeSymbol( "BLOCK?" ), ConsesLow.list(
        makeSymbol( "OUTPUT-FORMAT" ), makeKeyword( "STANDARD" ) ), ConsesLow.list( makeSymbol( "STREAM" ), ConsesLow.list( makeSymbol( "QUOTE" ), makeSymbol( "*STANDARD-OUTPUT*" ) ) ), ConsesLow.list( makeSymbol(
            "RUN-TINY-KB-TESTS-IN-FULL-KB?" ), ConsesLow.list( makeSymbol( "QUOTE" ), makeSymbol( "*RUN-TINY-KB-TESTS-IN-FULL-KB?*" ) ) ) );
    $list8 = ConsesLow.list( makeKeyword( "VERBOSITY" ), makeKeyword( "BROWSABLE?" ), makeKeyword( "BLOCK?" ), makeKeyword( "OUTPUT-FORMAT" ), makeKeyword( "STREAM" ), makeKeyword( "RUN-TINY-KB-TESTS-IN-FULL-KB?" ) );
    $kw9$ALLOW_OTHER_KEYS = makeKeyword( "ALLOW-OTHER-KEYS" );
    $kw10$VERBOSITY = makeKeyword( "VERBOSITY" );
    $kw11$TERSE = makeKeyword( "TERSE" );
    $kw12$BROWSABLE_ = makeKeyword( "BROWSABLE?" );
    $kw13$BLOCK_ = makeKeyword( "BLOCK?" );
    $kw14$OUTPUT_FORMAT = makeKeyword( "OUTPUT-FORMAT" );
    $kw15$STANDARD = makeKeyword( "STANDARD" );
    $kw16$STREAM = makeKeyword( "STREAM" );
    $sym17$_STANDARD_OUTPUT_ = makeSymbol( "*STANDARD-OUTPUT*" );
    $kw18$RUN_TINY_KB_TESTS_IN_FULL_KB_ = makeKeyword( "RUN-TINY-KB-TESTS-IN-FULL-KB?" );
    $sym19$_RUN_TINY_KB_TESTS_IN_FULL_KB__ = makeSymbol( "*RUN-TINY-KB-TESTS-IN-FULL-KB?*" );
    $sym20$RUN_CYC_TEST_INT = makeSymbol( "RUN-CYC-TEST-INT" );
    $list21 = ConsesLow.list( new SubLObject[] { makeSymbol( "NAME" ), makeSymbol( "MIN-NUMBER" ), makeSymbol( "MAX-NUMBER" ), makeSymbol( "&KEY" ), ConsesLow.list( makeSymbol( "VERBOSITY" ), makeKeyword( "TERSE" ) ),
      makeSymbol( "BROWSABLE?" ), makeSymbol( "BLOCK?" ), ConsesLow.list( makeSymbol( "OUTPUT-FORMAT" ), makeKeyword( "STANDARD" ) ), ConsesLow.list( makeSymbol( "STREAM" ), ConsesLow.list( makeSymbol( "QUOTE" ),
          makeSymbol( "*STANDARD-OUTPUT*" ) ) ), ConsesLow.list( makeSymbol( "RUN-TINY-KB-TESTS-IN-FULL-KB?" ), ConsesLow.list( makeSymbol( "QUOTE" ), makeSymbol( "*RUN-TINY-KB-TESTS-IN-FULL-KB?*" ) ) )
    } );
    $sym22$RUN_CYC_TEST_IN_RANGE_INT = makeSymbol( "RUN-CYC-TEST-IN-RANGE-INT" );
    $list23 = ConsesLow.list( makeSymbol( "NAME" ), makeSymbol( "&KEY" ), makeSymbol( "BROWSABLE?" ), makeSymbol( "BLOCK?" ), ConsesLow.list( makeSymbol( "OUTPUT-FORMAT" ), makeKeyword( "STANDARD" ) ), ConsesLow.list(
        makeSymbol( "STREAM" ), ConsesLow.list( makeSymbol( "QUOTE" ), makeSymbol( "*STANDARD-OUTPUT*" ) ) ), ConsesLow.list( makeSymbol( "RUN-TINY-KB-TESTS-IN-FULL-KB?" ), ConsesLow.list( makeSymbol( "QUOTE" ),
            makeSymbol( "*RUN-TINY-KB-TESTS-IN-FULL-KB?*" ) ) ) );
    $list24 = ConsesLow.list( makeKeyword( "BROWSABLE?" ), makeKeyword( "BLOCK?" ), makeKeyword( "OUTPUT-FORMAT" ), makeKeyword( "STREAM" ), makeKeyword( "RUN-TINY-KB-TESTS-IN-FULL-KB?" ) );
    $kw25$VERBOSE = makeKeyword( "VERBOSE" );
    $sym26$RUN_CYC_TEST = makeSymbol( "RUN-CYC-TEST" );
    $sym27$CYC_TEST_VERBOSITY_LEVEL_P = makeSymbol( "CYC-TEST-VERBOSITY-LEVEL-P" );
    $sym28$BOOLEANP = makeSymbol( "BOOLEANP" );
    $sym29$CYC_TEST_OUTPUT_FORMAT_P = makeSymbol( "CYC-TEST-OUTPUT-FORMAT-P" );
    $sym30$STREAMP = makeSymbol( "STREAMP" );
    $list31 = ConsesLow.list( new SubLObject[] { makeSymbol( "NAME" ), makeSymbol( "&KEY" ), ConsesLow.list( makeSymbol( "NTHREADS" ), FOUR_INTEGER ), ConsesLow.list( makeSymbol( "NTIMES" ), TEN_INTEGER ), ConsesLow
        .list( makeSymbol( "VERBOSITY" ), makeKeyword( "TERSE" ) ), makeSymbol( "BROWSABLE?" ), makeSymbol( "BLOCK?" ), ConsesLow.list( makeSymbol( "OUTPUT-FORMAT" ), makeKeyword( "STANDARD" ) ), ConsesLow.list(
            makeSymbol( "STREAM" ), ConsesLow.list( makeSymbol( "QUOTE" ), makeSymbol( "*STANDARD-OUTPUT*" ) ) ), ConsesLow.list( makeSymbol( "RUN-TINY-KB-TESTS-IN-FULL-KB?" ), ConsesLow.list( makeSymbol( "QUOTE" ),
                makeSymbol( "*RUN-TINY-KB-TESTS-IN-FULL-KB?*" ) ) )
    } );
    $list32 = ConsesLow.list( makeKeyword( "NTHREADS" ), makeKeyword( "NTIMES" ), makeKeyword( "VERBOSITY" ), makeKeyword( "BROWSABLE?" ), makeKeyword( "BLOCK?" ), makeKeyword( "OUTPUT-FORMAT" ), makeKeyword( "STREAM" ),
        makeKeyword( "RUN-TINY-KB-TESTS-IN-FULL-KB?" ) );
    $kw33$NTHREADS = makeKeyword( "NTHREADS" );
    $kw34$NTIMES = makeKeyword( "NTIMES" );
    $sym35$RUN_CYC_TEST_PARALLEL_INT = makeSymbol( "RUN-CYC-TEST-PARALLEL-INT" );
    $sym36$RUN_CYC_TEST_PARALLEL = makeSymbol( "RUN-CYC-TEST-PARALLEL" );
    $sym37$NON_NEGATIVE_INTEGER_P = makeSymbol( "NON-NEGATIVE-INTEGER-P" );
    $str38$Parallel_Test_Thread_for_ = makeString( "Parallel Test Thread for " );
    $str39$_No__ = makeString( " No. " );
    $sym40$RUN_CYC_TEST_OBJECT_PARALLEL = makeSymbol( "RUN-CYC-TEST-OBJECT-PARALLEL" );
    $kw41$IUT = makeKeyword( "IUT" );
    $kw42$IT = makeKeyword( "IT" );
    $kw43$RMT = makeKeyword( "RMT" );
    $kw44$TMT = makeKeyword( "TMT" );
    $kw45$RMCT = makeKeyword( "RMCT" );
    $kw46$ERT = makeKeyword( "ERT" );
    $kw47$TCT = makeKeyword( "TCT" );
    $kw48$KCT = makeKeyword( "KCT" );
    $str49$Cyc_test_of_unexpected_type___s = makeString( "Cyc test of unexpected type: ~s" );
    $kw50$SILENT = makeKeyword( "SILENT" );
    $str51$Can_t_run_inference_tests_in_brow = makeString( "Can't run inference tests in browsable mode -- ignoring :browsable? setting~%" );
    $str52$Can_t_run_inference_tests_in_bloc = makeString( "Can't run inference tests in blocking mode -- ignoring :block? setting~%" );
    $kw53$BACKWARD = makeKeyword( "BACKWARD" );
    $kw54$FORWARD = makeKeyword( "FORWARD" );
    $str55$Unexpected_testing_direction____S = makeString( "Unexpected testing direction : ~S" );
    $kw56$ERROR = makeKeyword( "ERROR" );
    $str57$Can_t_run_removal_module_cost_tes = makeString( "Can't run removal module cost tests in browsable mode -- ignoring :browsable? setting~%" );
    $str58$Can_t_run_removal_module_cost_tes = makeString( "Can't run removal module cost tests in blocking mode -- ignoring :block? setting~%" );
    $str59$Can_t_run_evaluatable_relation_te = makeString( "Can't run evaluatable relation tests in blocking mode -- ignoring :block? setting~%" );
    $str60$Can_t_run_test_case_tables_in_bro = makeString( "Can't run test case tables in browsable mode -- ignoring :browsable? setting~%" );
    $str61$Can_t_run_test_case_tables_in_blo = makeString( "Can't run test case tables in blocking mode -- ignoring :block? setting~%" );
    $kw62$POST_BUILD = makeKeyword( "POST-BUILD" );
    $sym63$CYC_TEST_TYPE_SPEC_P = makeSymbol( "CYC-TEST-TYPE-SPEC-P" );
    $sym64$FILE_VALID_FOR_WRITING_P = makeSymbol( "FILE-VALID-FOR-WRITING-P" );
    $sym65$DIRECTORY_WRITEABLE_P = makeSymbol( "DIRECTORY-WRITEABLE-P" );
    $str66$Invalid_phase__A_of__A_specified_ = makeString( "Invalid phase ~A of ~A specified." );
    $kw67$APPEND = makeKeyword( "APPEND" );
    $str68$Unable_to_open__S = makeString( "Unable to open ~S" );
    $str69$___HARNESS_ERROR___A__ = makeString( "~&:HARNESS-ERROR: ~A~%" );
    $kw70$HARNESS_ERROR = makeKeyword( "HARNESS-ERROR" );
    $kw71$INPUT = makeKeyword( "INPUT" );
    $kw72$EOF = makeKeyword( "EOF" );
    $sym73$CYC_TEST_RUN_P = makeSymbol( "CYC-TEST-RUN-P" );
    $kw74$NOT_RUN = makeKeyword( "NOT-RUN" );
    $kw75$INVALID = makeKeyword( "INVALID" );
    $list76 = ConsesLow.list( makeKeyword( "RMT" ), makeKeyword( "TMT" ), makeKeyword( "TCT" ) );
    $kw77$ONLY = makeKeyword( "ONLY" );
    $str78$Unexpected_value__s_for__run_non_ = makeString( "Unexpected value ~s for :run-non-working-tests" );
    $str79$____Failing_regression_tests___ = makeString( "~&~%Failing regression tests:~%" );
    $sym80$FAILING_CYC_TEST_RUN_P = makeSymbol( "FAILING-CYC-TEST-RUN-P" );
    $str81$None____ = makeString( "None! (:" );
    $str82$____Succeeding_regression_tests__ = makeString( "~&~%Succeeding regression tests:~%" );
    $sym83$SUCCEEDING_CYC_TEST_RUN_P = makeSymbol( "SUCCEEDING-CYC-TEST-RUN-P" );
    $str84$None____ = makeString( "None! ):" );
    $sym85$IGNORED_CYC_TEST_RUN_P = makeSymbol( "IGNORED-CYC-TEST-RUN-P" );
    $str86$____Non_regression_tests__None_ = makeString( "~&~%Non-regression tests: None." );
    $kw87$NON_REGRESSION_SUCCESS = makeKeyword( "NON-REGRESSION-SUCCESS" );
    $kw88$NON_REGRESSION_FAILURE = makeKeyword( "NON-REGRESSION-FAILURE" );
    $str89$__Failing_non_regression_tests___ = makeString( "~&Failing non-regression tests:~%" );
    $str90$__Succeeding_non_regression_tests = makeString( "~&Succeeding non-regression tests:~%" );
    $str91$_Running_a_specified_list_of_test = makeString( "\nRunning a specified list of tests " );
    $str92$_Running__a_file_P_of_tests_ = makeString( "\nRunning ~a file~P of tests " );
    $str93$in_the__a_KB__KB__a___System__a_S = makeString( "in the ~a KB (KB ~a):\nSystem ~a\nStarted at ~a ~a\n\n" );
    $str94$Tiny = makeString( "Tiny" );
    $str95$Full = makeString( "Full" );
    $str96$ = makeString( "" );
    $str97$_ = makeString( "(" );
    $str98$_of_ = makeString( " of " );
    $str99$_phases_ = makeString( " phases)" );
    $str100$__No_tests_were_run___ = makeString( "~&No tests were run.~%" );
    $sym101$CYC_TEST_NAME = makeSymbol( "CYC-TEST-NAME" );
    $str102$__NOTE__The_following_cyc_tests_w = makeString( "~&NOTE: The following cyc tests were noted as not working:~%" );
    $str103$___Tests_finished_at__a_Total_tim = makeString( "~&\nTests finished at ~a\nTotal time: ~a\nTotal test guts time: ~a\nHistogram of results:~%" );
    $sym104$CYC_TEST_RUN_RESULT = makeSymbol( "CYC-TEST-RUN-RESULT" );
    $str105$Overall_result___s__ = makeString( "Overall result: ~s~%" );
    $str106$_S__A__S__A = makeString( "~S ~A ~S ~A" );
    $str107$__A = makeString( " ~A" );
    $kw108$SUCCESS = makeKeyword( "SUCCESS" );
    $str109$cyc_test_image_ = makeString( "cyc-test-image-" );
    $str110$load = makeString( "load" );
    $list111 = ConsesLow.list( new SubLObject[] { makeKeyword( "CANONICALIZE-INFERENCE-ANSWER-JUSTIFICATIONS" ), makeKeyword( "NON-EXPLANATORY-SENTENCE-SUPPORTS" ), makeKeyword( "NON-EXPLANATORY-VARIABLE-MAP-SUPPORTS" ),
      makeKeyword( "TRUE-SENTENCE-OF-ATOMIC-SENTENCE-REDUCTION" ), makeKeyword( "IST-OF-ATOMIC-SENTENCE-REDUCTION" ), makeKeyword( "RELATION-ALL-INSTANCE-ITERATE-2" ), makeKeyword( "RELATION-INSTANCE-ALL-ITERATE-2" ),
      makeKeyword( "REJECT-PREVIOUSLY-PROVEN-PROOFS" ), makeKeyword( "INFERENCE-HARNESS-OVERHEAD" ), makeKeyword( "TACTICALLY-UNEXAMINED-NO-GOOD-IMPLIES-STRATEGICALLY-UNEXAMINED-NO-GOOD" ), makeKeyword(
          "KAPPA-REMOVAL-WORKS" ), makeKeyword( "DONT-REOPEN-ANSWER-LINK" ), makeKeyword( "REMOVAL-TRUE-SENTENCE-UNIVERSAL-DISJUNCTION-14A" ), makeKeyword( "CLOSED-ASENT-WITH-3-CHILDREN" ), makeKeyword(
              "SIMPLE-EXCEPT-WHEN" ), makeKeyword( "SIMPLE-EXCEPT-WHEN-RESIDUAL-TRANSFORMATION" ), makeKeyword( "PARTIAL-EXCEPT-WHEN" ), makeKeyword( "VARIABLE-MAP-EXCEPT-WHEN" ), makeKeyword(
                  "EXCEPTION-TMS-BACKWARD-NO-OP" ), makeKeyword( "MULTIPLE-TRANSFORMATION-PROOFS-FOR-CLOSED-PROBLEM" ), makeKeyword( "BACKCHAIN-TO-REMOVAL-TRUE-SENTENCE-UNIVERSAL-DISJUNCTION-1" ), makeKeyword(
                      "BACKCHAIN-TO-REMOVAL-TRUE-SENTENCE-UNIVERSAL-DISJUNCTION-2" ), makeKeyword( "BACKCHAIN-TO-REMOVAL-TRUE-SENTENCE-UNIVERSAL-DISJUNCTION-3" ), makeKeyword(
                          "EXCEPT-MT-IN-MID-MT-BLOCKS-HIGH-MT-FROM-LOW-MT" ), makeKeyword( "EXCEPT-MT-IN-HIGH-MT-HOSES-BACKWARD-INFERENCE" ), makeKeyword( "TWO-STEP" ), makeKeyword( "TWO-STEP-ARG-1" ), makeKeyword(
                              "RECONSIDER-DEDUCTION" ), makeKeyword( "THERE-EXISTS" ), makeKeyword( "EXCEPT-WHEN" ), makeKeyword( "STRENGTH-PROPAGATION" ), makeKeyword( "SEQUENCE-VARIABLES-INFERENCE" ), makeKeyword(
                                  "INFERENCE-ANSWER-TEMPLATE" ), makeKeyword( "EXCEPT-BLOCKS-BACKWARD" ), makeKeyword( "EXCEPT-BLOCKS-FORWARD" ), makeKeyword( "TRUE-SENTENCE-UNIVERSAL-DISJUNCTION-SCOPING" ), makeKeyword(
                                      "TMS-RECONSIDERATION-WITH-BACKCHAIN-FORBIDDEN" ), makeKeyword( "LAZILY-MANIFEST-NON-FOCALS" ), makeKeyword( "CONSIDER-NO-GOOD-AFTER-DETERMINING-TACTICS" ), makeKeyword(
                                          "IRRELEVANT-DOES-NOT-IMPLY-PENDING" ), makeKeyword( "PROBLEM-STORE-PRUNING-MAX-INSUFFICIENT" ), makeKeyword( "RESTRICTED-CLOSED-GOOD-PROBLEMS-STAY-UNEXAMINED" ), makeKeyword(
                                              "GENLS-BETWEEN" ), makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-1" ), makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-2" ), makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-3" ),
      makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-4" ), makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-5" ), makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-6" ), makeKeyword( "CONJUNCTIVE-INTEGER-BETWEEN-7" ), makeKeyword(
          "CONJUNCTIVE-INTEGER-BETWEEN-8" ), makeKeyword( "CONJUNCTIVE-FOLLOWUP-ADDITIONAL-JOIN-ORDERED" ), makeKeyword( "CONJUNCTIVE-FOLLOWUP-ADDITIONAL-JOIN-ORDERED-WITHOUT-INFERENCE" ), makeKeyword(
              "CIRCULAR-PROOFS" ), makeKeyword( "POSSIBLE-REUSED-PROBLEMS-GET-STRATEGICALLY-EVALUATED" ), makeKeyword( "RESIDUAL-TRANSFORMATION-PROBLEM-REUSE" )
    } );
    $float112$1_2 = makeDouble( 1.2 );
    $list113 = ConsesLow.list( makeString( ".*~[0-9]+~" ) );
    $sym114$DIRECTORY_P = makeSymbol( "DIRECTORY-P" );
    $str115$cdolist = makeString( "cdolist" );
    $str116$_ = makeString( " " );
    $str117$grew = makeString( "grew" );
    $str118$shrunk = makeString( "shrunk" );
    $str119$_from_ = makeString( " from " );
    $str120$_bytes_to_ = makeString( " bytes to " );
    $str121$_bytes = makeString( " bytes" );
    $str122$_disappeared = makeString( " disappeared" );
    $str123$_is_a_new_file = makeString( " is a new file" );
    $str124$filename = makeString( "filename" );
    $str125$size_in_bytes_as_of_ = makeString( "size in bytes as of " );
  }
}
/*
 * 
