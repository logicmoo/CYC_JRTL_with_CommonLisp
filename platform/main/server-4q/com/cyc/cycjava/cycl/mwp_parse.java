      ...
      SubLTranslatedFile
{
  ...
  {
    assert NIL != mwp_parse_p( parse ) : parse;
  ...
  {
    return conses_high.getf( v_mwp_parse, key, NIL );
  }
  ...
  public static SubLObject mwp_parse_update(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      return ConsesLow.list( $sym3$CSETQ, v_mwp_parse, ConsesLow.list( $sym4$PUTF, v_mwp_parse, key, value ) );
    }
  ...
  {
    assert NIL != mwp_parse_p( v_mwp_parse ) : v_mwp_parse;
    return conses_high.getf( v_mwp_parse, $kw5$TYPE, UNPROVIDED );
  }
  ...
  {
    assert NIL != mwp_parse_p( v_mwp_parse ) : v_mwp_parse;
    return conses_high.getf( v_mwp_parse, $kw6$DENOT, UNPROVIDED );
  }
  ...
  {
    assert NIL != mwp_parse_p( v_mwp_parse ) : v_mwp_parse;
    return conses_high.getf( v_mwp_parse, $kw7$POS_PRED, UNPROVIDED );
  }
  ...
  public static SubLObject mwp_parse_destructuring_bind(final SubLObject macroform, final SubLObject environment)
  {
    ...
    return ConsesLow.listS( $sym9$CDESTRUCTURING_BIND, pattern, parse, ConsesLow.append( body, NIL ) );
  }
  ...
  {
    final SubLObject ok_sets = list_utilities.remove_if_not( $sym10$MWP_PARSE_SET_P, sets, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    return ( NIL != ok_sets ) ? set_utilities.set_union( ok_sets, Symbols.symbol_function( EQUAL ) ) : new_mwp_parse_set();
  }
  ...
  {
    return set_utilities.construct_set_from_list( set.set_element_list( v_set ), Symbols.symbol_function( EQUAL ), UNPROVIDED );
  }

  public static SubLObject declare_mwp_parse_file()
  {
    SubLFiles.declareFunction( me, "mwp_parse_p", "MWP-PARSE-P", 1, 0, false );
    SubLFiles.declareFunction( me, "new_mwp_parse", "NEW-MWP-PARSE", 0, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_copy", "MWP-PARSE-COPY", 1, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_lookup", "MWP-PARSE-LOOKUP", 2, 0, false );
    SubLFiles.declareMacro( me, "mwp_parse_update", "MWP-PARSE-UPDATE" );
    SubLFiles.declareFunction( me, "mwp_parse_type", "MWP-PARSE-TYPE", 1, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_denot", "MWP-PARSE-DENOT", 1, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_pos_pred", "MWP-PARSE-POS-PRED", 1, 0, false );
    SubLFiles.declareMacro( me, "mwp_parse_destructuring_bind", "MWP-PARSE-DESTRUCTURING-BIND" );
    SubLFiles.declareFunction( me, "mwp_parse_set_p", "MWP-PARSE-SET-P", 1, 0, false );
    SubLFiles.declareFunction( me, "new_mwp_parse_set", "NEW-MWP-PARSE-SET", 0, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_set_add", "MWP-PARSE-SET-ADD", 2, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_set_union", "MWP-PARSE-SET-UNION", 1, 0, false );
    SubLFiles.declareFunction( me, "mwp_parse_set_copy", "MWP-PARSE-SET-COPY", 1, 0, false );
    return NIL;
  }
  ...
  {
    me = new mwp_parse();
    $list0 = ConsesLow.list( makeKeyword( "ALLOW-OTHER-KEYS" ), T );
    $sym1$MWP_PARSE_P = makeSymbol( "MWP-PARSE-P" );
    $list2 = ConsesLow.list( makeSymbol( "MWP-PARSE" ), makeSymbol( "KEY" ), makeSymbol( "VALUE" ) );
    $sym3$CSETQ = makeSymbol( "CSETQ" );
    $sym4$PUTF = makeSymbol( "PUTF" );
    $kw5$TYPE = makeKeyword( "TYPE" );
    $kw6$DENOT = makeKeyword( "DENOT" );
    $kw7$POS_PRED = makeKeyword( "POS-PRED" );
    $list8 = ConsesLow.list( makeSymbol( "PATTERN" ), makeSymbol( "PARSE" ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym9$CDESTRUCTURING_BIND = makeSymbol( "CDESTRUCTURING-BIND" );
    $sym10$MWP_PARSE_SET_P = makeSymbol( "MWP-PARSE-SET-P" );
  }
}
/*
 * 
