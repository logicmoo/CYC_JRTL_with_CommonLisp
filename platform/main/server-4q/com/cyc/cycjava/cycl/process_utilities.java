...
import com.cyc.cycjava.cycl.process_utilities.$process_wrapper_p$UnaryFunction.$thinking_task_native;
...
public class process_utilities extends SubLTranslatedFile {
	...
	public static SubLObject task_process(final SubLObject v_object) {
		assert NIL != task_p(v_object) : "process_utilities.task_p error :" + v_object;
	...
	public static SubLObject make_task(SubLObject arglist) {
		...
		for (next = NIL, next = arglist; NIL != next; next = cddr(next)) {
			...
			} else if (pcase_var.eql($COMPLETED)) {
	...
	public static SubLObject t_task_lock(final SubLObject v_object) {
		assert NIL != thinking_task_p(v_object) : "process_utilities.thinking_task_p error :" + v_object;
		return v_object.getField2();
	}
	...
	public static SubLObject ipc_queue_lock(final SubLObject v_object) {
		assert NIL != ipc_queue_p(v_object) : "process_utilities.ipc_queue_p error :" + v_object;
		return v_object.getField2();
	}
	...
	public static SubLObject make_ipc_queue(SubLObject arglist) {
		...
		for (next = NIL, next = arglist; NIL != next; next = cddr(next)) {
			...
			} else if (pcase_var.eql($SEMAPHORE)) {
				_csetf_ipc_queue_semaphore(v_new, current_value);
			} else if (pcase_var.eql($DATA_QUEUE)) {
	...
	public static SubLObject ordrd_ipcq_lock(final SubLObject v_object) {
		assert NIL != ordered_ipc_queue_p(v_object) : "process_utilities.ordered_ipc_queue_p error :" + v_object;
		return v_object.getField2();
	}
	...
	public static SubLObject process_wrapper_id(final SubLObject v_object) {
		assert NIL != process_wrapper_p(v_object) : "process_utilities.process_wrapper_p error :" + v_object;
		return v_object.getField2();
	}
	...
	public static final class $task_native extends SubLStructNative {
		...
		static {
			structDecl = makeStructDeclNative($task_native.class, TASK, TASK_P, $list15, $list16, new String[] { "$process", "$completed" }, $list17, $list18, DEFAULT_STRUCT_PRINT_FUNCTION);
		}
	}
	...
	public static final class $ipc_queue_native extends SubLStructNative {
		...
		static {
			structDecl = makeStructDeclNative($ipc_queue_native.class, IPC_QUEUE, IPC_QUEUE_P, $list108, $list109, new String[] { "$lock", "$semaphore", "$data_queue" }, $list110, $list111, DEFAULT_STRUCT_PRINT_FUNCTION);
		}
	}
	...
	public static final class $ordered_ipc_queue_native extends SubLStructNative {
		...
		private $ordered_ipc_queue_native() {
			this.$lock = Lisp.NIL;
		...
		static {
			structDecl = makeStructDeclNative($ordered_ipc_queue_native.class, ORDERED_IPC_QUEUE, ORDERED_IPC_QUEUE_P, $list159, $list160, new String[] { "$lock", "$producer_isg", "$consumer_isg", "$payload", "$early_consumer_semaphore" }, $list161, $list162, DEFAULT_STRUCT_PRINT_FUNCTION);
		}
	}
	...
	public static final class $process_wrapper_native extends SubLStructNative {
		...
		private $process_wrapper_native() {
			this.$id = Lisp.NIL;
			this.$process = Lisp.NIL;
			this.$state = Lisp.NIL;
			this.$lock = Lisp.NIL;
		...
		static {
			structDecl = makeStructDeclNative($process_wrapper_native.class, PROCESS_WRAPPER, PROCESS_WRAPPER_P, $list198, $list199, new String[] { "$id", "$process", "$state", "$lock", "$plist" }, $list200, $list201, DEFAULT_STRUCT_PRINT_FUNCTION);
		}
	}
	...
	public static final class $process_wrapper_p$UnaryFunction extends UnaryFunction {
		...
		// }
		public static final class $thinking_task_native extends SubLStructNative {
			private static final SubLStructDeclNative structDecl = makeStructDeclNative($thinking_task_native.class, THINKING_TASK, THINKING_TASK_P, new String[] { "$lock", "$thread", "$name", "$status", "$progress_message", "$progress_sofar", "$progress_total", "$start_time", "$finish_time", "$result", "$error_message", "$properties" }, DEFAULT_STRUCT_PRINT_FUNCTION);
