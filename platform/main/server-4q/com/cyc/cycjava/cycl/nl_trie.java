...
import com.cyc.tool.subl.util.SubLTranslatedFile; 
 public final class nl_trie extends SubLTranslatedFile {
    ...
    public static SubLObject init_nl_trie_file() {
        ...
        deflexical("*NL-TRIE-CACHE-STRATEGY*", SubLSystemTrampolineFile.maybeDefault($nl_trie_cache_strategy$, $nl_trie_cache_strategy$, () -> cache_utilities.new_metered_preallocated_cache($default_nl_trie_cache_strategy_size$.getGlobalValue(), $nl_trie_default_test$.getGlobalValue())));
        deflexical("*NL-TRIE-LOCK*", SubLSystemTrampolineFile.maybeDefault($nl_trie_lock$, $nl_trie_lock$, () -> make_lock($$$NL_Trie_Lock)));
        ...
        deflexical("*NL-TRIE-LEXICAL-MT*", SubLSystemTrampolineFile.maybeDefault($nl_trie_lexical_mt$, $nl_trie_lexical_mt$, $$AllLexicalMicrotheoryPSC));
        deflexical("*NL-TRIE-SYNTACTIC-PREDICATES*", SubLSystemTrampolineFile.maybeDefault($nl_trie_syntactic_predicates$, $nl_trie_syntactic_predicates$, $UNINITIALIZED));
        deflexical("*NL-TRIE-DENOTATION-PREDICATES*", SubLSystemTrampolineFile.maybeDefault($nl_trie_denotation_predicates$, $nl_trie_denotation_predicates$, $UNINITIALIZED));
        deflexical("*NL-TRIE-SEMTRANS-PREDICATES*", SubLSystemTrampolineFile.maybeDefault($nl_trie_semtrans_predicates$, $nl_trie_semtrans_predicates$, $UNINITIALIZED));
        deflexical("*NL-TRIE-ABBREVIATION-PREDICATES*", SubLSystemTrampolineFile.maybeDefault($nl_trie_abbreviation_predicates$, $nl_trie_abbreviation_predicates$, $UNINITIALIZED));
        deflexical("*NL-TRIE-ARG-IN-RELN-PREDICATES*", SubLSystemTrampolineFile.maybeDefault($nl_trie_arg_in_reln_predicates$, $nl_trie_arg_in_reln_predicates$, $UNINITIALIZED));
        deflexical("*NL-TRIE-PRAGMATIC-PREDICATES*", SubLSystemTrampolineFile.maybeDefault($nl_trie_pragmatic_predicates$, $nl_trie_pragmatic_predicates$, $UNINITIALIZED));
        defparameter("*NL-TRIES-BEING-INITIALIZED*", NIL);
        deflexical("*NL-TRIE*", SubLSystemTrampolineFile.maybeDefault($nl_trie$, $nl_trie$, () -> new_nl_trie(UNPROVIDED)));
        ...
        deflexical("*NL-TRIE-NAME-STRING-PREDS-SET*", SubLSystemTrampolineFile.maybeDefault($nl_trie_name_string_preds_set$, $nl_trie_name_string_preds_set$, () -> lexicon_vars.new_lex_pred_set()));
        return NIL;
    }
