...
public final class cb_uia_tools_review_and_testing extends SubLTranslatedFile implements V02 {
	...
	public static final SubLObject uiat_query_constructor_child_completed(SubLObject parent, SubLObject child) {
		{
			...
			} else if (pcase_var.eql($SENTENCE_READER)) {
				{
	...
	public static final SubLObject cb_uiat_query_constructor_display(SubLObject interaction) {
		{
			...
			if (NIL == current) {
				if (NIL != rkf_scenario_manipulator.sm_scenario_p(v_scenario)) {
				...
				} else if ((NIL != rkf_query_constructor.qc_query_p(query)) && advanced.eql($TRUE)) {
					return cb_query_constructor.cb_qc_show_state_guts(qc_state);
				} else if (NIL != rkf_query_constructor.qc_query_p(query)) {
				...
				} else if ((question_text.isString() && (scenario_parse == $UNDETERMINED)) && (direct_parse == $UNDETERMINED)) {
					{
				...
				} else if (((scenario_parse == $NONE) && (NIL == el_utilities.possibly_sentence_p(direct_parse))) && (v_scenario == $UNDETERMINED)) {
					{
				...
				} else if (NIL != rkf_scenario_manipulator.sm_scenario_expression_p(scenario_parse)) {
					{
					...
					return cb_query_constructor.cb_qc_show_state_guts(qc_state);
				} else if (NIL != el_utilities.possibly_sentence_p(direct_parse)) {
					{
	...
	public static final SubLObject cb_uiat_common_queries_render_request(SubLObject request) {
		{
			...
			plist = current;
			{
				...
				} else if (pcase_var.eql($FOCUS_REQUESTED)) {
					{
				...
				} else if (pcase_var.eql($FOCUS_RESULTS)) {
					{
	...
	public static final SubLObject uiat_sf_build_new_solution_finder(SubLObject v_agenda, SubLObject text) {
		{
			final SubLThread thread = SubLProcess.currentSubLThread();
			{
				SubLObject sf_state = NIL;
				{
					...
					try {
						rkf_solution_finder.$sf_typical_backchain_default$
								.bind(NIL != user_interaction_agenda.uia_state_lookup(v_agenda, $DEFAULT_SF_BACKCHAIN, $uiat_sf_default_backchain$.getDynamicValue(thread)) ? ((SubLObject) (user_interaction_agenda.uia_state_lookup(v_agenda, $DEFAULT_SF_BACKCHAIN, $uiat_sf_default_backchain$.getDynamicValue(thread)))) : rkf_solution_finder.$sf_typical_backchain_default$.getDynamicValue(thread), thread);
						rkf_solution_finder.$sf_typical_number_default$.bind(NIL != user_interaction_agenda.uia_state_lookup(v_agenda, $DEFAULT_SF_NUMBER, $uiat_sf_default_number$.getDynamicValue(thread)) ? ((SubLObject) (user_interaction_agenda.uia_state_lookup(v_agenda, $DEFAULT_SF_NUMBER, $uiat_sf_default_number$.getDynamicValue(thread)))) : rkf_solution_finder.$sf_typical_number_default$.getDynamicValue(thread),
								thread);
															...
															html_utilities.html_char(CHAR_greater, UNPROVIDED);
															{
																...
																try {
																	html_macros.$html_safe_print$.bind(T, thread);
																	{
																		...
																		} else if (pcase_var.eql($INVALIDATED)) {
					...
					try {
						...
						html_utilities.html_char(CHAR_greater, UNPROVIDED);
						{
							...
							try {
								...
								} else if (NIL != term_allow_salient_descriptorP) {
	...
	public static final SubLObject cb_uia_test_suite_tool_render_request(SubLObject request) {
		{
			...
			plist = current;
			{
				...
				} else if (pcase_var.eql($SHOW_ALL_QUERY_RESULTS)) {
					{
				...
				} else if (pcase_var.eql($SHOW_TEST_RESULT)) {
					{
				...
				} else if (pcase_var.eql($SHOW_TEST_SUITE)) {
					{
	...
	static private final SubLList $list_alt93 = list(
			new SubLObject[] { $NAME, makeString("Solution Finder"), makeKeyword("HINT-METHOD"), makeSymbol("UIAT-SOLUTION-FINDER-HINT"), makeKeyword("CHILD-COMPLETED-METHOD"), makeSymbol("UIAT-SOLUTION-FINDER-CHILD-COMPLETED"), makeKeyword("CHILD-REJECTED-METHOD"), makeSymbol("UIAT-SOLUTION-FINDER-CHILD-REJECTED"), $CYCL, constant_handles.reader_make_constant_shell(makeString("CycSolutionFinder")) });
