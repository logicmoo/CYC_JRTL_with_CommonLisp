...
public final class corba_removal_module_utilities
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject corba_removal_module_generic_expand_fn(final SubLObject literal, final SubLObject sense)
  {
    ...
    final SubLObject arg1_var = el_utilities.literal_arg1( literal, UNPROVIDED );
    ...
    if( NIL != result )
    {
      if( NIL != variables.variable_p( arg1_var ) )
      {
        ...
        if( NIL != v_bindings )
        {
          backward.removal_add_node( arguments.make_hl_support( $kw0$OPAQUE, ConsesLow.cons( corba_predicate, ConsesLow.cons( result, args ) ), UNPROVIDED, UNPROVIDED ), v_bindings, unify_justification );
        }
      }
      else
      {
        backward.removal_add_node( arguments.make_hl_support( $kw0$OPAQUE, ConsesLow.cons( corba_predicate, ConsesLow.cons( result, args ) ), UNPROVIDED, UNPROVIDED ), NIL, UNPROVIDED );
      }
    }
    return NIL;
  }
  ...
  {
    assert NIL != corba_utilities.corba_removal_predicate_p( pred ) : pred;
    SubLObject result = ConsesLow.list( pred );
    if( NIL != corba_utilities.corba_storage_predicate_p( pred ) )
    {
      result = ConsesLow.cons( $kw2$FULLY_BOUND, result );
    }
    else
    {
      result = ConsesLow.cons( $kw3$NOT_FULLY_BOUND, result );
    }
    SubLObject cdotimes_end_var;
    SubLObject i;
    for( cdotimes_end_var = Numbers.subtract( arity.arity( pred ), ONE_INTEGER ), i = NIL, i = ZERO_INTEGER; i.numL( cdotimes_end_var ); i = Numbers.add( i, ONE_INTEGER ) )
    {
      result = ConsesLow.cons( $kw2$FULLY_BOUND, result );
    }
  ...
  {
    assert NIL != corba_utilities.corba_removal_predicate_p( pred ) : pred;
    return $sym4$CORBA_REMOVAL_MODULE_GENERIC_EXPAND_FN;
  }

  public static SubLObject declare_corba_removal_module_utilities_file()
  {
    SubLFiles.declareFunction( me, "corba_removal_module_generic_expand_fn", "CORBA-REMOVAL-MODULE-GENERIC-EXPAND-FN", 2, 0, false );
    SubLFiles.declareFunction( me, "determine_corba_removal_module_required_pattern", "DETERMINE-CORBA-REMOVAL-MODULE-REQUIRED-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "corba_pred_get_removal_module_expand_fn_symbol", "CORBA-PRED-GET-REMOVAL-MODULE-EXPAND-FN-SYMBOL", 1, 0, false );
    return NIL;
  }
  ...
  static
  {
    me = new corba_removal_module_utilities();
    $kw0$OPAQUE = makeKeyword( "OPAQUE" );
    $sym1$CORBA_REMOVAL_PREDICATE_P = makeSymbol( "CORBA-REMOVAL-PREDICATE-P" );
    $kw2$FULLY_BOUND = makeKeyword( "FULLY-BOUND" );
    $kw3$NOT_FULLY_BOUND = makeKeyword( "NOT-FULLY-BOUND" );
    $sym4$CORBA_REMOVAL_MODULE_GENERIC_EXPAND_FN = makeSymbol( "CORBA-REMOVAL-MODULE-GENERIC-EXPAND-FN" );
  }
}
/*
 * 
