...
import static com.cyc.cycjava.cycl.html_utilities.html_table_heading_list;
...
/**
 * Copyright (c) 1995 - 2019 Cycorp, Inc.  All rights reserved.
 * module:      SKSI-SKS-MANAGER
 * source file: /cyc/top/cycl/sksi/modeling-tools/interfaces/sksi-sks-manager.lisp
 * created:     2019/07/03 17:38:35
 */
public final class sksi_sks_manager extends SubLTranslatedFile implements V12 {
    /**
     * Take an SKS and all its subs and supers out of the registered list
     */
    @LispMethod(comment = "Take an SKS and all its subs and supers out of the registered list")
    public static final SubLObject sksi_registered_knowledge_sources_remove(SubLObject sks) {
        {
            SubLObject subs = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_get_sub_skses(sks);
            SubLObject v_super = sksi_sks_accessors.sks_get_super_sks(sks);
            $sksi_registered_knowledge_sources$.setGlobalValue(remove(sks, $sksi_registered_knowledge_sources$.getGlobalValue(), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED));
            if (NIL != subs) {
                {
                    ...
                    for (sub = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , sub = cdolist_list_var.first()) {
                        com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources_remove(sub);
                    }
                }
            }
            if (NIL != v_super) {
                com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources_remove_partial(v_super);
            }
        }
        return $sksi_registered_knowledge_sources$.getGlobalValue();
    }

    /**
     * Returns a list of all of sks's immediate *registered* sub-KSes
     */
    @LispMethod(comment = "Returns a list of all of sks\'s immediate *registered* sub-KSes")
    public static final SubLObject registered_sub_skses(SubLObject sks) {
        {
            SubLObject registered_subs = NIL;
            SubLObject sub_skses = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_get_sub_skses(sks);
            SubLObject cdolist_list_var = sub_skses;
            SubLObject sub = NIL;
            for (sub = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , sub = cdolist_list_var.first()) {
                if (NIL != com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registered_p(sub)) {
                    {
                        SubLObject item_var = sub;
                        if (NIL == member(item_var, registered_subs, symbol_function(EQL), symbol_function(IDENTITY))) {
                            registered_subs = cons(item_var, registered_subs);
                        }
                    }
                }
            }
            return registered_subs;
        }
    }

    public static final SubLObject register_parent_ks_conjunctive_rms_if_warranted(SubLObject sks_list) {
        {
                ...
                for (sks = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , sks = cdolist_list_var.first()) {
                    {
                        ...
                        SubLObject mt_var = mapping_mt;
                        {
                            ...
                            try {
                                ...
                                {
                                    SubLObject parent = sksi_kb_accessors.immediate_genl_ks(sks);
                                    if ((NIL != parent) && (NIL == subl_promotions.memberP(parent, registered_parents, symbol_function(EQ), UNPROVIDED))) {
                                        {
                                            SubLObject siblings = sksi_kb_accessors.sk_source_proper_sub_ks_closure(parent);
                                            if (NIL != list_utilities.every_in_list($sym62$SK_SOURCE_REGISTERED_, siblings, UNPROVIDED)) {
    ...
    public static final SubLObject deregister_parent_ks_conjunctive_rms_if_warranted(SubLObject sks_list) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject count = ZERO_INTEGER;
    ...
    /**
     * Returns T iff one of the immediate subKSes of SKS is registered.
     */
    @LispMethod(comment = "Returns T iff one of the immediate subKSes of SKS is registered.")
    public static final SubLObject any_sub_ks_registeredP(SubLObject sks) {
        {
            SubLObject sub_skses = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_get_sub_skses(sks);
            SubLObject registeredP = NIL;
            if (NIL == registeredP) {
                {
                    SubLObject csome_list_var = sub_skses;
                    SubLObject sub = NIL;
                    for (sub = csome_list_var.first(); !((NIL != registeredP) || (NIL == csome_list_var)); csome_list_var = csome_list_var.rest() , sub = csome_list_var.first()) {
                        if (NIL != com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registered_p(sub)) {
                            registeredP = T;
                        }
                    }
    ...
    /**
     * Returns T iff one of the closure subKSes of SKS is registered.
     */
    @LispMethod(comment = "Returns T iff one of the closure subKSes of SKS is registered.")
    public static final SubLObject any_closure_sub_ks_registeredP(SubLObject sks) {
        {
            SubLObject sub_skses = sksi_kb_accessors.sk_source_proper_sub_ks_closure_in_mapping_mt(sks);
            SubLObject registeredP = NIL;
            if (NIL == registeredP) {
                {
                    SubLObject csome_list_var = sub_skses;
    ...
    static private final SubLString $str_alt38$ = makeString("");

    public static final SubLSymbol $sksi_registered_knowledge_sources$ = makeSymbol("*SKSI-REGISTERED-KNOWLEDGE-SOURCES*");

    // defparameter
    @LispMethod(comment = "defparameter")
    private static final SubLSymbol $partial_bg_color$ = makeSymbol("*PARTIAL-BG-COLOR*");

    // defparameter
    @LispMethod(comment = "defparameter")
    private static final SubLSymbol $reg_bg_color$ = makeSymbol("*REG-BG-COLOR*");

    // defparameter
    @LispMethod(comment = "defparameter")
    private static final SubLSymbol $no_reg_string$ = makeSymbol("*NO-REG-STRING*");

    // defparameter
    @LispMethod(comment = "defparameter")
    private static final SubLSymbol $partial_reg_string$ = makeSymbol("*PARTIAL-REG-STRING*");

    // defparameter
    @LispMethod(comment = "defparameter")
    private static final SubLSymbol $reg_string$ = makeSymbol("*REG-STRING*");
 ...
 public static final String myName = "com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager";
    ...
    static private final SubLString $str0$SKSI_Structured_Knowledge_Source_ = makeString("SKSI Structured Knowledge Source Manager");

    private static final SubLSymbol SKSI_SKS_MANAGER = makeSymbol("SKSI-SKS-MANAGER");
    ...
    public static final SubLObject sksi_sks_manager_alt(SubLObject args) {
        if (args == UNPROVIDED) {
            args = NIL;
        }
        if (NIL != ke.cyclist_is_guest()) {
            cyc_navigator_internals.guest_warn($str_alt0$SKSI_Structured_Knowledge_Source_);
        } else {
            com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_sks_manager_internal(args);
        }
        return NIL;
    }
     ...
     * @return nil
    Defines a CYC Browser link method called :sksi-sks-manager
     */
    @LispMethod(comment = "@param linktext;\r\n\t\ta different link name if wanted.\r\n@return nil\r\nDefines a CYC Browser link method called :sksi-sks-manager")
    public static final SubLObject cb_link_sksi_sks_manager_alt(SubLObject linktext) {
        ...
        }
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            if (NIL == linktext) {
                linktext = $str_alt2$_SKSI_SKS_Manager_;
            }
            {
                ...
                cyc_cgi_url_int();
                html_princ($str_alt4$sksi_sks_manager);
                html_char(CHAR_quotation, UNPROVIDED);
    ...
    /**
     * Returns a list of all sks's immediate sub-KSes
     *
     * @unknown move to sksi-sks-accessors
     */
    @LispMethod(comment = "Returns a list of all sks\'s immediate sub-KSes\r\n\r\n@unknown move to sksi-sks-accessors")
    public static final SubLObject sks_get_sub_skses_alt(SubLObject sks) {
    ...
    /**
     * Return depth of a knowledge source
     *
     * @unknown rename to something better
     * @unknown move to sksi-sks-accessors
     */
    @LispMethod(comment = "Return depth of a knowledge source\r\n\r\n@unknown rename to something better\r\n@unknown move to sksi-sks-accessors")
    public static final SubLObject sks_depth_alt(SubLObject sks, SubLObject depth) {
        if (depth == UNPROVIDED) {
            depth = ZERO_INTEGER;
        }
        {
            SubLObject superks = sksi_sks_accessors.sks_get_super_sks(sks);
            if (NIL != superks) {
                return com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_depth(superks, add(ONE_INTEGER, depth));
            } else {
                return depth;
            }
        }
    ...
    public static final SubLObject clear_sksi_registered_knowledge_sources_alt() {
        $sksi_registered_knowledge_sources$.setGlobalValue(NIL);
        return $sksi_registered_knowledge_sources$.getGlobalValue();
    }
    ...
    public static final SubLObject sksi_registered_knowledge_sources_alt() {
        return $sksi_registered_knowledge_sources$.getGlobalValue();
    }
    ...
    public static final SubLObject sks_registered_p_alt(SubLObject sks) {
        return subl_promotions.memberP(sks, com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources(), symbol_function(EQ), UNPROVIDED);
    }
    ...
    public static final SubLObject sksi_registered_knowledge_sources_add_alt(SubLObject sks) {
        if (NIL == com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registered_p(sks)) {
            {
                SubLObject subs = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_get_sub_skses(sks);
                SubLObject v_super = sksi_sks_accessors.sks_get_super_sks(sks);
                SubLObject siblings = (NIL != v_super) ? ((SubLObject) (com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_get_sub_skses(v_super))) : NIL;
                SubLObject item_var = sks;
                if (NIL == member(item_var, $sksi_registered_knowledge_sources$.getGlobalValue(), symbol_function(EQL), symbol_function(IDENTITY))) {
                    $sksi_registered_knowledge_sources$.setGlobalValue(cons(item_var, $sksi_registered_knowledge_sources$.getGlobalValue()));
                }
                $sksi_registered_knowledge_sources$.setGlobalValue(sksi_infrastructure_utilities.sort_knowledge_sources($sksi_registered_knowledge_sources$.getGlobalValue()));
                if (NIL != subs) {
                    {
                        SubLObject cdolist_list_var = subs;
                        SubLObject sub = NIL;
                        for (sub = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , sub = cdolist_list_var.first()) {
                            com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources_add(sub);
                        }
                    }
                }
                if ((NIL != v_super) && (NIL != subsetp(siblings, $sksi_registered_knowledge_sources$.getGlobalValue(), symbol_function(EQUAL), UNPROVIDED))) {
                    if (NIL == com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registered_p(v_super)) {
                        com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources_add(v_super);
                    }
                }
            }
        }
    ...
    /**
     * Take an SKS out of the registered list and deregister its parent(s), but
     * don't mess with its siblings
     */
    @LispMethod(comment = "Take an SKS out of the registered list and deregister its parent(s), but\r\ndon\'t mess with its siblings\nTake an SKS out of the registered list and deregister its parent(s), but\ndon\'t mess with its siblings")
    public static final SubLObject sksi_registered_knowledge_sources_remove_partial_alt(SubLObject sks) {
        if (NIL != com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registered_p(sks)) {
            {
                SubLObject v_super = sksi_sks_accessors.sks_get_super_sks(sks);
                $sksi_registered_knowledge_sources$.setGlobalValue(remove(sks, $sksi_registered_knowledge_sources$.getGlobalValue(), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED));
    ...
    public static final SubLObject sksi_expanded_knowledge_sources_alt() {
    ...
    public static final SubLObject sks_expanded_p_alt(SubLObject sks) {
        return subl_promotions.memberP(sks, com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_expanded_knowledge_sources(), symbol_function(EQ), UNPROVIDED);
    }
    ...
    public static final SubLObject sksi_expanded_knowledge_sources_add_alt(SubLObject sks) {
        if (NIL == com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_expanded_p(sks)) {
            $sksi_expanded_knowledge_sources$.setGlobalValue(list_utilities.sort_adjoin(sks, $sksi_expanded_knowledge_sources$.getGlobalValue(), symbol_function(EQ), $sym12$TERM___WITH_CONSTANT_BY_NAME, IDENTITY));
        }
        return $sksi_expanded_knowledge_sources$.getGlobalValue();
    }
    ...
    public static final SubLObject sksi_expanded_knowledge_sources_remove_alt(SubLObject sks) {
    ...
    public static final SubLObject sksi_toggle_expansion_alt(SubLObject sks) {
        if (NIL != com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_expanded_p(sks)) {
            com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_expanded_knowledge_sources_remove(sks);
        } else {
            com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_expanded_knowledge_sources_add(sks);
        }
        return $sksi_expanded_knowledge_sources$.getGlobalValue();
    }
    ...
    public static final SubLObject sksi_sks_manager_internal_alt(SubLObject args) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject title_var = $str_alt27$Structured_Knowledge_Source_Manag;
                {
                                        ...
                                        }
                                        {
                                            ...
                                            html_char(CHAR_greater, UNPROVIDED);
                                            {
                                                ...
                                                try {
                                                    ...
                                                    html_hidden_input($str_alt31$sksi_sks_manager_input, T, UNPROVIDED);
                                                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_input_table_framework(sksi_infrastructure_utilities.gather_all_top_level_complete_knowledge_sources());
    ...
    /**
     * Create the table that displays our input elements, write out special
     * heading lines and trailing lines (for 'All'), display the options area.
     */
    @LispMethod(comment = "Create the table that displays our input elements, write out special\r\nheading lines and trailing lines (for \'All\'), display the options area.\nCreate the table that displays our input elements, write out special\nheading lines and trailing lines (for \'All\'), display the options area.")
    public static final SubLObject display_input_table_framework_alt(SubLObject sks_list) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            ...
            html_char(CHAR_greater, UNPROVIDED);
            {
                ...
                try {
                    html_macros.$html_safe_print$.bind(T, thread);
                    html_table_heading_list($list_alt32);
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_user_input_table(sks_list);
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_sks_all_row();
                } finally {
                    html_macros.$html_safe_print$.rebind(_prev_bind_0, thread);
                }
            }
    ...
    /**
     * Call set-sks-table-row-parameters on each SKS recursively
     */
    @LispMethod(comment = "Call set-sks-table-row-parameters on each SKS recursively")
    public static final SubLObject display_user_input_table_alt(SubLObject sks_list) {
        if (NIL == sks_list) {
            return NIL;
        } else {
            {
                SubLObject sks = sks_list.first();
                SubLObject expandedP = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_expanded_p(sks);
                com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.set_sks_table_row_parameters(sks, expandedP);
                if (NIL != expandedP) {
                    {
                        SubLObject sub_skses = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_get_sub_skses(sks);
                        com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_user_input_table(sksi_infrastructure_utilities.sort_knowledge_sources(sub_skses));
                    }
                }
                com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_user_input_table(sks_list.rest());
            }
        }
        return NIL;
    }
    ...
    public static final SubLObject choose_image_for_row_alt(SubLObject expP, SubLObject sks) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            if (NIL != expP) {
    ...
    /**
     * selects parameters based on registration state and expansion for
     * things like background color, what image to use, etc., and
     * calls display-sks-table-row.
     */
    @LispMethod(comment = "selects parameters based on registration state and expansion for\r\nthings like background color, what image to use, etc., and\r\ncalls display-sks-table-row.\nselects parameters based on registration state and expansion for\nthings like background color, what image to use, etc., and\ncalls display-sks-table-row.")
    public static final SubLObject set_sks_table_row_parameters(SubLObject sks, SubLObject expP) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject reg_state = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registration_state(sks);
                SubLObject img_url = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.choose_image_for_row(expP, sks);
                SubLObject pcase_var = reg_state;
                if (pcase_var.eql($PARTIAL)) {
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_sks_table_row(sks, img_url, $partial_reg_string$.getDynamicValue(thread), T, $partial_bg_color$.getDynamicValue(thread));
                } else {
                    if (pcase_var.eql($REG)) {
                        com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_sks_table_row(sks, img_url, $reg_string$.getDynamicValue(thread), T, $reg_bg_color$.getDynamicValue(thread));
                    } else {
                        if (pcase_var.eql($NOREG)) {
                            com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.display_sks_table_row(sks, img_url, $no_reg_string$.getDynamicValue(thread), NIL, UNPROVIDED);
                        }
                    }
                }
            }
    ...
    // Internal Constants
    @LispMethod(comment = "Internal Constants")
    static private final SubLString $str_alt0$SKSI_Structured_Knowledge_Source_ = makeString("SKSI Structured Knowledge Source Manager");

    /**
     * Display one row of the input table (one SKS)
     */
    @LispMethod(comment = "Display one row of the input table (one SKS)")
    public static final SubLObject display_sks_table_row(SubLObject sks, SubLObject img_url, SubLObject reg_string, SubLObject regP, SubLObject row_color) {
                ...
                html_char(CHAR_greater, UNPROVIDED);
                {
                    ...
                    try {
                        ...
                        html_char(CHAR_greater, UNPROVIDED);
                        {
                            ...
                            try {
                                html_macros.$html_safe_print$.bind(T, thread);
                                html_indent(multiply(FOUR_INTEGER, com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_depth(sks, UNPROVIDED)));
                                html_image_input(cconcatenate($str_alt41$expansion_, name), img_url, UNPROVIDED, UNPROVIDED);
                        ...
                        html_char(CHAR_greater, UNPROVIDED);
                        {
                            ...
                            try {
                                html_macros.$html_safe_print$.bind(T, thread);
                                {
                                    SubLObject checkbox_name = cconcatenate($str_alt43$register_, name);
                        ...
                        html_char(CHAR_greater, UNPROVIDED);
                        {
                            ...
                            try {
                                ...
                                if (NIL != regP) {
                                    {
                                        SubLObject checkbox_name = cconcatenate($str_alt44$deregister_, name);
                                        html_checkbox_input(checkbox_name, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
                                    }
                                } else {
                                    {
                                        SubLObject color_val = $$$gray;
                                        html_markup(html_macros.$html_font_head$.getGlobalValue());
                                        if (NIL != color_val) {
                                            html_markup(html_macros.$html_font_color$.getGlobalValue());
                                            html_char(CHAR_quotation, UNPROVIDED);
                                        ...
                                        html_char(CHAR_greater, UNPROVIDED);
                                        {
                                            ...
                                            try {
                                                html_macros.$html_safe_print$.bind(T, thread);
                                                html_princ($str_alt46$_not_registered_);
                                            } finally {
    ...
    static private final SubLString $str_alt2$_SKSI_SKS_Manager_ = makeString("[SKSI SKS Manager]");

    static private final SubLString $str_alt4$sksi_sks_manager = makeString("sksi-sks-manager");

    static private final SubLSymbol $sym12$TERM___WITH_CONSTANT_BY_NAME = makeSymbol("TERM-<-WITH-CONSTANT-BY-NAME");

    static private final SubLString $str_alt14$right_triangle_blue_gif = makeString("right-triangle-blue.gif");

    static private final SubLString $str_alt16$down_triangle_blue_gif = makeString("down-triangle-blue.gif");

    static private final SubLString $str_alt18$blank_gif = makeString("blank.gif");

    static private final SubLString $str_alt19$_cycdoc_img_cb_right_triangle_blu = makeString("/cycdoc/img/cb/right-triangle-blue.gif");

    static private final SubLString $str_alt20$_cycdoc_img_cb_down_triangle_blue = makeString("/cycdoc/img/cb/down-triangle-blue.gif");

    static private final SubLString $str_alt21$_cycdoc_img_cb_blank_gif = makeString("/cycdoc/img/cb/blank.gif");

    static private final SubLString $$$Registered = makeString("Registered");

    static private final SubLString $$$Partially_Registered = makeString("Partially Registered");

    static private final SubLString $$$Not_Registered = makeString("Not Registered");

    static private final SubLString $str_alt25$_AAAAFF = makeString("#AAAAFF");

    static private final SubLString $str_alt26$_DDDDFF = makeString("#DDDDFF");

    static private final SubLString $str_alt27$Structured_Knowledge_Source_Manag = makeString("Structured Knowledge Source Manager");
    ...
    static private final SubLString $str_alt31$sksi_sks_manager_input = makeString("sksi-sks-manager-input");

    static private final SubLList $list_alt32 = list(makeString("Source"), makeString("Status"), makeString("Register or Reregister"), makeString("Deregister"));

    static private final SubLString $str_alt41$expansion_ = makeString("expansion-");

    static private final SubLString $str_alt43$register_ = makeString("register-");

    static private final SubLString $str_alt44$deregister_ = makeString("deregister-");

    static private final SubLString $str_alt46$_not_registered_ = makeString("(not registered)");

    static private final SubLString $str_alt47$_DDDDDD = makeString("#DDDDDD");

    static private final SubLString $str_alt50$register_all = makeString("register-all");

    static private final SubLString $str_alt51$deregister_all = makeString("deregister-all");

    static private final SubLString $str_alt52$_x = makeString(".x");

    static private final SubLString $str_alt53$_y = makeString(".y");

    static private final SubLString $str_alt56$_A_new_SKSI_removal_modules_have_ = makeString("~A new SKSI removal modules have been (re)registered with your Cyc image.");

    static private final SubLString $str_alt57$_A_new_SKSI_preference_modules_ha = makeString("~A new SKSI preference modules have been (re)registered with your Cyc image.");

    static private final SubLString $str_alt58$_A_SKSI_removal_modules_have_been = makeString("~A SKSI removal modules have been deregistered with your Cyc image.");

    static private final SubLString $str_alt59$_A_SKSI_preference_modules_have_b = makeString("~A SKSI preference modules have been deregistered with your Cyc image.");

    static private final SubLString $str_alt60$Currently__you_have__A_SKSI_remov = makeString("Currently, you have ~A SKSI removal modules and ~A SKSI preference modules registered with your image.");

    static private final SubLSymbol $sym62$SK_SOURCE_REGISTERED_ = makeSymbol("SK-SOURCE-REGISTERED?");

    static private final SubLString $str_alt63$registering_knowledge_sources_ = makeString("registering knowledge sources.");

    static private final SubLString $str_alt64$cb_sks_register_applied_to_non_kn = makeString("cb-sks-register applied to non-knowledge-source");

    static private final SubLString $str_alt66$cb_sks_register__A = makeString("cb-sks-register&~A");

    static private final SubLString $str_alt67$_Register_KS_ = makeString("[Register KS]");

    static private final SubLString $str_alt70$deregistering_knowledge_sources_ = makeString("deregistering knowledge sources.");

    static private final SubLString $str_alt71$cb_register_applied_to_non_knowle = makeString("cb-register applied to non-knowledge-source");

    static private final SubLString $str_alt73$cb_sks_deregister__A = makeString("cb-sks-deregister&~A");

    static private final SubLString $str_alt74$_Deregister_KS_ = makeString("[Deregister KS]");

    /**
     * Special: display the register-all and deregister-all row
     */
    @LispMethod(comment = "Special: display the register-all and deregister-all row")
    public static final SubLObject display_sks_all_row_alt() {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            html_markup(html_macros.$html_table_row_head$.getGlobalValue());
            if (true) {
                ...
                try {
                    ...
                    html_char(CHAR_greater, UNPROVIDED);
                    {
                        ...
                        try {
                            html_macros.$html_safe_print$.bind(T, thread);
                            {
                                SubLObject image_src = cb_icon_file_path($SKS_BLANK);
                                SubLObject align = $TOP;
                                SubLObject alt = cb_get_icon_alt_string($SKS_BLANK);
                    ...
                    html_char(CHAR_greater, UNPROVIDED);
                    {
                        ...
                        try {
                            html_macros.$html_safe_print$.bind(T, thread);
                            html_checkbox_input($str_alt50$register_all, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
                        } finally {
                    ...
                    html_char(CHAR_greater, UNPROVIDED);
                    {
                        ...
                        try {
                            html_macros.$html_safe_print$.bind(T, thread);
                            html_checkbox_input($str_alt51$deregister_all, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
                        } finally {
    ...
    public static final SubLObject sks_source_by_input_alt(SubLObject input, SubLObject prefix_string) {
        {
            SubLObject name = string_utilities.post_remove(string_utilities.post_remove(string_utilities.pre_remove(input.first(), prefix_string, UNPROVIDED), $str_alt52$_x, UNPROVIDED), $str_alt53$_y, UNPROVIDED);
    ...
    public static final SubLObject sksi_sks_manager_input_alt(SubLObject args) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject register_inputs = NIL;
                SubLObject deregister_inputs = NIL;
                SubLObject expansion_inputs = NIL;
                SubLObject register_allP = NIL;
                SubLObject deregister_allP = NIL;
                ...
                for (input = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , input = cdolist_list_var.first()) {
                    if (input.isList() && (NIL != string_utilities.starts_with(input.first(), $str_alt41$expansion_))) {
                        {
                            SubLObject item_var = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_source_by_input(input, $str_alt41$expansion_);
                    ...
                    if (input.isList() && second(input).equal($$$on)) {
                        if (NIL != string_utilities.starts_with(input.first(), $str_alt43$register_)) {
                            {
                                SubLObject name = string_utilities.pre_remove(input.first(), $str_alt43$register_, UNPROVIDED);
                                if (name.equal($$$all)) {
                                    register_allP = T;
                                } else {
                                    register_inputs = cons(cb_guess_fort(name, UNPROVIDED), register_inputs);
                                }
                            }
                        } else {
                            {
                                SubLObject name = string_utilities.pre_remove(input.first(), $str_alt44$deregister_, UNPROVIDED);
                                if (name.equal($$$all)) {
                                    deregister_allP = T;
                                } else {
                                    deregister_inputs = cons(cb_guess_fort(name, UNPROVIDED), deregister_inputs);
                                }
                            }
                        }
                    }
                ...
                if (NIL == expansion_inputs) {
                    if (NIL != deregister_allP) {
                        thread.resetMultipleValues();
                        {
                            SubLObject deregistered_rm_count_14 = sksi_query_utilities.deregister_all_combined_sksi_removal_modules(UNPROVIDED);
                            SubLObject deregistered_pm_count_15 = thread.secondMultipleValue();
                            thread.resetMultipleValues();
                            deregistered_rm_count = deregistered_rm_count_14;
                            deregistered_pm_count = deregistered_pm_count_15;
                        }
                        com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.clear_sksi_registered_knowledge_sources();
                    } else {
                        thread.resetMultipleValues();
                        {
                            SubLObject deregistered_rm_count_16 = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.deregister_sksi_removal_modules_for_skses(deregister_inputs);
                            SubLObject deregistered_pm_count_17 = thread.secondMultipleValue();
                            thread.resetMultipleValues();
                            deregistered_rm_count = deregistered_rm_count_16;
                            deregistered_pm_count = deregistered_pm_count_17;
                        }
                    }
                    if (NIL != register_allP) {
                        {
                            SubLObject all_complete_skses = sksi_infrastructure_utilities.gather_all_top_level_complete_knowledge_sources();
                            SubLObject unregistered_complete_skses = list_utilities.fast_set_difference(all_complete_skses, com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources(), UNPROVIDED);
                            thread.resetMultipleValues();
                            {
                                SubLObject registered_rm_count_18 = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.register_sksi_removal_modules_for_skses(unregistered_complete_skses);
                                SubLObject registered_pm_count_19 = thread.secondMultipleValue();
                                thread.resetMultipleValues();
                                registered_rm_count = registered_rm_count_18;
                                registered_pm_count = registered_pm_count_19;
                            }
                        }
                    } else {
                        thread.resetMultipleValues();
                        {
                            SubLObject registered_rm_count_20 = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.register_sksi_removal_modules_for_skses(register_inputs);
                            SubLObject registered_pm_count_21 = thread.secondMultipleValue();
                            thread.resetMultipleValues();
                            registered_rm_count = registered_rm_count_20;
                            registered_pm_count = registered_pm_count_21;
                        }
                    }
                    if (NIL != registered_rm_count) {
                        format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt56$_A_new_SKSI_removal_modules_have_, registered_rm_count);
                    }
                    html_newline(UNPROVIDED);
                    if (NIL != registered_pm_count) {
                        format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt57$_A_new_SKSI_preference_modules_ha, registered_pm_count);
                    }
                    html_newline(TWO_INTEGER);
                    if (NIL != deregistered_rm_count) {
                        format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt58$_A_SKSI_removal_modules_have_been, deregistered_rm_count);
                    }
                    html_newline(UNPROVIDED);
                    if (NIL != deregistered_pm_count) {
                        format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt59$_A_SKSI_preference_modules_have_b, deregistered_pm_count);
                    }
                    html_newline(TWO_INTEGER);
                    format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt60$Currently__you_have__A_SKSI_remov, sksi_query_utilities.combined_sksi_removal_modules_count(), sksi_preference_module_generation.sksi_preference_modules_count());
                    html_newline(THREE_INTEGER);
                    cb_link($SKSI_SKS_MANAGER, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED);
                    html_newline(TWO_INTEGER);
                } else {
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_toggle_expansion(expansion_inputs.first());
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_sks_manager(UNPROVIDED);
                }
            }
            return NIL;
        }
    ...
    public static final SubLObject register_sksi_removal_modules_for_skses_alt(SubLObject sks_list) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                ...
                SubLObject sks = NIL;
                for (sks = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , sks = cdolist_list_var.first()) {
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources_add(sks);
                    thread.resetMultipleValues();
                    {
                        SubLObject sks_rm_count = sksi_query_utilities.register_combined_sksi_removal_modules_for_sks(sks, UNPROVIDED, UNPROVIDED);
                        SubLObject sks_pm_count = thread.secondMultipleValue();
                        thread.resetMultipleValues();
                        rm_count = add(rm_count, sks_rm_count);
                        pm_count = add(pm_count, sks_pm_count);
                    }
                }
                rm_count = add(rm_count, com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.register_parent_ks_conjunctive_rms_if_warranted(sks_list));
    ...
    public static final SubLObject deregister_sksi_removal_modules_for_skses_alt(SubLObject sks_list) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject rm_count = ZERO_INTEGER;
                ...
                SubLObject sks = NIL;
                for (sks = cdolist_list_var.first(); NIL != cdolist_list_var; cdolist_list_var = cdolist_list_var.rest() , sks = cdolist_list_var.first()) {
                    com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sksi_registered_knowledge_sources_remove(sks);
                    thread.resetMultipleValues();
                    {
                        SubLObject sks_rm_count = sksi_query_utilities.deregister_combined_sksi_removal_modules_for_sks(sks, UNPROVIDED, UNPROVIDED);
                        SubLObject sks_pm_count = thread.secondMultipleValue();
                        thread.resetMultipleValues();
                        rm_count = add(rm_count, sks_rm_count);
                        pm_count = add(pm_count, sks_pm_count);
                ...
                rm_count = add(rm_count, com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.deregister_parent_ks_conjunctive_rms_if_warranted(sks_list));
                return values(rm_count, pm_count);
            }
        }
    }
    ...
    /**
     * returns one of :REG, :NOREG, :PARTIAL
     *
     * @unknown incorrectly returns :NOREG if something has partially
    registered immediate descendant
     */
    @LispMethod(comment = "returns one of :REG, :NOREG, :PARTIAL\r\n\r\n@unknown incorrectly returns :NOREG if something has partially\r\nregistered immediate descendant")
    public static final SubLObject sks_registration_state_alt(SubLObject sks) {
        if (NIL != com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.sks_registered_p(sks)) {
            return $REG;
        } else {
            if (NIL != sksi_sks_accessors.sks_atomic_p(sks)) {
                ...
                if (NIL != com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.any_closure_sub_ks_registeredP(sks)) {
                    return $PARTIAL;
                } else {
                    return $NOREG;
                }
    ...
    public static final SubLObject cb_sks_register_alt(SubLObject args) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            if (NIL != ke.cyclist_is_guest()) {
                cyc_navigator_internals.guest_warn($str_alt63$registering_knowledge_sources_);
            } else {
                {
                    SubLObject fort = cb_guess_fort(args.first(), UNPROVIDED);
                    SubLObject registered_count = NIL;
                    if (NIL == Errors.$ignore_mustsP$.getDynamicValue(thread)) {
                        if (NIL == sksi_kb_accessors.sk_source_p(fort)) {
                            Errors.error($str_alt64$cb_sks_register_applied_to_non_kn);
                        }
                    }
                    registered_count = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.register_sksi_removal_modules_for_skses(list(fort));
                    format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt56$_A_new_SKSI_removal_modules_have_, registered_count);
                }
            }
            return NIL;
        }
    ...
    public static final SubLObject cb_link_sksi_register_alt(SubLObject fort) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject frame_name_var = cb_frame_name($MAIN);
                ...
                html_char(CHAR_quotation, UNPROVIDED);
                cyc_cgi_url_int();
                format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt66$cb_sks_register__A, cb_fort_identifier(fort));
                html_char(CHAR_quotation, UNPROVIDED);
                if (NIL != frame_name_var) {
                    html_markup(html_macros.$html_anchor_target$.getGlobalValue());
                    html_char(CHAR_quotation, UNPROVIDED);
                ...
                html_char(CHAR_greater, UNPROVIDED);
                {
                    ...
                    try {
                        html_macros.$html_safe_print$.bind(T, thread);
                        html_princ($str_alt67$_Register_KS_);
                    } finally {
                        html_macros.$html_safe_print$.rebind(_prev_bind_0, thread);
                    }
                }
    ...
    public static final SubLObject cb_sks_deregister_alt(SubLObject args) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            if (NIL != ke.cyclist_is_guest()) {
                cyc_navigator_internals.guest_warn($str_alt70$deregistering_knowledge_sources_);
            } else {
                {
                    SubLObject fort = cb_guess_fort(args.first(), UNPROVIDED);
                    ...
                    if (NIL == Errors.$ignore_mustsP$.getDynamicValue(thread)) {
                        if (NIL == sksi_kb_accessors.sk_source_p(fort)) {
                            Errors.error($str_alt71$cb_register_applied_to_non_knowle);
                        }
                    }
                    deregistered_count = com.cyc.cycjava.cycl.sksi.modeling_tools.interfaces.sksi_sks_manager.deregister_sksi_removal_modules_for_skses(list(fort));
                    format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt58$_A_SKSI_removal_modules_have_been, deregistered_count);
                }
            }
            return NIL;
        }
    ...
    public static final SubLObject cb_link_sksi_deregister_alt(SubLObject fort) {
        {
            final SubLThread thread = SubLProcess.currentSubLThread();
            {
                SubLObject frame_name_var = cb_frame_name($MAIN);
                ...
                html_char(CHAR_quotation, UNPROVIDED);
                cyc_cgi_url_int();
                format(html_macros.$html_stream$.getDynamicValue(thread), $str_alt73$cb_sks_deregister__A, cb_fort_identifier(fort));
                html_char(CHAR_quotation, UNPROVIDED);
                if (NIL != frame_name_var) {
                    html_markup(html_macros.$html_anchor_target$.getGlobalValue());
                    html_char(CHAR_quotation, UNPROVIDED);
                ...
                html_char(CHAR_greater, UNPROVIDED);
                {
                    ...
                    try {
                        html_macros.$html_safe_print$.bind(T, thread);
                        html_princ($str_alt74$_Deregister_KS_);
                    } finally {
                        html_macros.$html_safe_print$.rebind(_prev_bind_0, thread);
                    }
                }
    ...
    public static SubLObject execute_sks_registration_state(final SubLObject state) {
        final SubLThread thread = SubLProcess.currentSubLThread();
        assert NIL != sks_registration_state_p(state) : "! sksi_sks_manager.sks_registration_state_p(state) " + ("sksi_sks_manager.sks_registration_state_p(state) " + "CommonSymbols.NIL != sksi_sks_manager.sks_registration_state_p(state) ") + state;
    ...
    public static final SubLObject declare_sksi_sks_manager_file_alt() {
        declareFunction("sksi_sks_manager", "SKSI-SKS-MANAGER", 0, 1, false);
        declareFunction("cb_link_sksi_sks_manager", "CB-LINK-SKSI-SKS-MANAGER", 0, 1, false);
        declareFunction("sks_get_sub_skses", "SKS-GET-SUB-SKSES", 1, 0, false);
        declareFunction("sks_depth", "SKS-DEPTH", 1, 1, false);
        declareFunction("sksi_registered_knowledge_sources", "SKSI-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
        declareFunction("clear_sksi_registered_knowledge_sources", "CLEAR-SKSI-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
        declareFunction("sks_registered_p", "SKS-REGISTERED-P", 1, 0, false);
        declareFunction("sksi_registered_knowledge_sources_add", "SKSI-REGISTERED-KNOWLEDGE-SOURCES-ADD", 1, 0, false);
        declareFunction("sksi_registered_knowledge_sources_remove_partial", "SKSI-REGISTERED-KNOWLEDGE-SOURCES-REMOVE-PARTIAL", 1, 0, false);
        declareFunction("sksi_registered_knowledge_sources_remove", "SKSI-REGISTERED-KNOWLEDGE-SOURCES-REMOVE", 1, 0, false);
        declareFunction("registered_sub_skses", "REGISTERED-SUB-SKSES", 1, 0, false);
        declareFunction("any_sub_ks_registeredP", "ANY-SUB-KS-REGISTERED?", 1, 0, false);
        declareFunction("any_closure_sub_ks_registeredP", "ANY-CLOSURE-SUB-KS-REGISTERED?", 1, 0, false);
        declareFunction("sksi_expanded_knowledge_sources", "SKSI-EXPANDED-KNOWLEDGE-SOURCES", 0, 0, false);
        declareFunction("sks_expanded_p", "SKS-EXPANDED-P", 1, 0, false);
        declareFunction("sksi_expanded_knowledge_sources_add", "SKSI-EXPANDED-KNOWLEDGE-SOURCES-ADD", 1, 0, false);
        declareFunction("sksi_expanded_knowledge_sources_remove", "SKSI-EXPANDED-KNOWLEDGE-SOURCES-REMOVE", 1, 0, false);
        declareFunction("sksi_toggle_expansion", "SKSI-TOGGLE-EXPANSION", 1, 0, false);
        declareFunction("sksi_sks_manager_internal", "SKSI-SKS-MANAGER-INTERNAL", 1, 0, false);
        declareFunction("display_input_table_framework", "DISPLAY-INPUT-TABLE-FRAMEWORK", 1, 0, false);
        declareFunction("display_user_input_table", "DISPLAY-USER-INPUT-TABLE", 1, 0, false);
        declareFunction("choose_image_for_row", "CHOOSE-IMAGE-FOR-ROW", 2, 0, false);
        declareFunction("set_sks_table_row_parameters", "SET-SKS-TABLE-ROW-PARAMETERS", 2, 0, false);
        declareFunction("display_sks_table_row", "DISPLAY-SKS-TABLE-ROW", 4, 1, false);
        declareFunction("display_sks_all_row", "DISPLAY-SKS-ALL-ROW", 0, 0, false);
        declareFunction("sks_source_by_input", "SKS-SOURCE-BY-INPUT", 2, 0, false);
        declareFunction("sksi_sks_manager_input", "SKSI-SKS-MANAGER-INPUT", 1, 0, false);
        declareFunction("register_sksi_removal_modules_for_skses", "REGISTER-SKSI-REMOVAL-MODULES-FOR-SKSES", 1, 0, false);
        declareFunction("register_parent_ks_conjunctive_rms_if_warranted", "REGISTER-PARENT-KS-CONJUNCTIVE-RMS-IF-WARRANTED", 1, 0, false);
        declareFunction("deregister_sksi_removal_modules_for_skses", "DEREGISTER-SKSI-REMOVAL-MODULES-FOR-SKSES", 1, 0, false);
        declareFunction("deregister_parent_ks_conjunctive_rms_if_warranted", "DEREGISTER-PARENT-KS-CONJUNCTIVE-RMS-IF-WARRANTED", 1, 0, false);
        declareFunction("sks_registration_state", "SKS-REGISTRATION-STATE", 1, 0, false);
        declareFunction("cb_sks_register", "CB-SKS-REGISTER", 1, 0, false);
        declareFunction("cb_link_sksi_register", "CB-LINK-SKSI-REGISTER", 1, 0, false);
        declareFunction("cb_sks_deregister", "CB-SKS-DEREGISTER", 1, 0, false);
        declareFunction("cb_link_sksi_deregister", "CB-LINK-SKSI-DEREGISTER", 1, 0, false);
        return NIL;
    }
    ...
    public static SubLObject declare_sksi_sks_manager_file() {
        if (SubLFiles.USE_V1) {
            ...
            declareFunction("clear_sksi_registered_knowledge_sources", "CLEAR-SKSI-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("sksi_registered_knowledge_sources", "SKSI-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("sksi_removal_modules_registered_knowledge_sources", "SKSI-REMOVAL-MODULES-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("clear_sksi_removal_modules_registered_knowledge_sources", "CLEAR-SKSI-REMOVAL-MODULES-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("sks_registered_p", "SKS-REGISTERED-P", 1, 0, false);
            declareFunction("sks_removal_modules_registered_p", "SKS-REMOVAL-MODULES-REGISTERED-P", 1, 0, false);
            declareFunction("sksi_content_mts_with_registered_module", "SKSI-CONTENT-MTS-WITH-REGISTERED-MODULE", 0, 0, false);
            declareFunction("sksi_content_mts_with_registered_removal_module", "SKSI-CONTENT-MTS-WITH-REGISTERED-REMOVAL-MODULE", 0, 0, false);
            declareFunction("clear_sksi_content_mts_with_registered_module", "CLEAR-SKSI-CONTENT-MTS-WITH-REGISTERED-MODULE", 0, 0, false);
            declareFunction("clear_sksi_content_mts_with_registered_removal_module", "CLEAR-SKSI-CONTENT-MTS-WITH-REGISTERED-REMOVAL-MODULE", 0, 0, false);
            declareFunction("add_sksi_content_mts_with_registered_module_for_sks", "ADD-SKSI-CONTENT-MTS-WITH-REGISTERED-MODULE-FOR-SKS", 1, 0, false);
            declareFunction("add_sksi_content_mts_with_registered_removal_module_for_sks", "ADD-SKSI-CONTENT-MTS-WITH-REGISTERED-REMOVAL-MODULE-FOR-SKS", 1, 0, false);
            declareFunction("remove_sksi_content_mts_with_registered_module_for_sks", "REMOVE-SKSI-CONTENT-MTS-WITH-REGISTERED-MODULE-FOR-SKS", 1, 0, false);
            declareFunction("remove_sksi_content_mts_with_registered_removal_module_for_sks", "REMOVE-SKSI-CONTENT-MTS-WITH-REGISTERED-REMOVAL-MODULE-FOR-SKS", 1, 0, false);
            declareFunction("sksi_storage_modules_registered_knowledge_sources", "SKSI-STORAGE-MODULES-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("clear_sksi_storage_modules_registered_knowledge_sources", "CLEAR-SKSI-STORAGE-MODULES-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("sks_storage_modules_registered_p", "SKS-STORAGE-MODULES-REGISTERED-P", 1, 0, false);
            declareFunction("sksi_content_mts_with_registered_storage_module", "SKSI-CONTENT-MTS-WITH-REGISTERED-STORAGE-MODULE", 0, 0, false);
            declareFunction("clear_sksi_content_mts_with_registered_storage_module", "CLEAR-SKSI-CONTENT-MTS-WITH-REGISTERED-STORAGE-MODULE", 0, 0, false);
            declareFunction("add_sksi_content_mts_with_registered_storage_module_for_sks", "ADD-SKSI-CONTENT-MTS-WITH-REGISTERED-STORAGE-MODULE-FOR-SKS", 1, 0, false);
            declareFunction("remove_sksi_content_mts_with_registered_storage_module_for_sks", "REMOVE-SKSI-CONTENT-MTS-WITH-REGISTERED-STORAGE-MODULE-FOR-SKS", 1, 0, false);
            declareFunction("sksi_registered_knowledge_sources_add", "SKSI-REGISTERED-KNOWLEDGE-SOURCES-ADD", 1, 0, false);
            declareFunction("sksi_removal_modules_registered_knowledge_sources_add", "SKSI-REMOVAL-MODULES-REGISTERED-KNOWLEDGE-SOURCES-ADD", 1, 0, false);
            declareFunction("sksi_registered_knowledge_sources_remove_partial", "SKSI-REGISTERED-KNOWLEDGE-SOURCES-REMOVE-PARTIAL", 1, 0, false);
            declareFunction("sksi_removal_modules_registered_knowledge_sources_remove_partial", "SKSI-REMOVAL-MODULES-REGISTERED-KNOWLEDGE-SOURCES-REMOVE-PARTIAL", 1, 0, false);
            declareFunction("sksi_removal_modules_registered_knowledge_sources_remove", "SKSI-REMOVAL-MODULES-REGISTERED-KNOWLEDGE-SOURCES-REMOVE", 1, 0, false);
            declareFunction("removal_modules_registered_sub_skses", "REMOVAL-MODULES-REGISTERED-SUB-SKSES", 1, 0, false);
            declareFunction("any_sub_ks_removal_modules_registeredP", "ANY-SUB-KS-REMOVAL-MODULES-REGISTERED?", 1, 0, false);
            declareFunction("any_closure_sub_ks_removal_modules_registeredP", "ANY-CLOSURE-SUB-KS-REMOVAL-MODULES-REGISTERED?", 1, 0, false);
            declareFunction("sksi_storage_modules_registered_knowledge_sources_add", "SKSI-STORAGE-MODULES-REGISTERED-KNOWLEDGE-SOURCES-ADD", 1, 0, false);
            declareFunction("sksi_storage_modules_registered_knowledge_sources_remove_partial", "SKSI-STORAGE-MODULES-REGISTERED-KNOWLEDGE-SOURCES-REMOVE-PARTIAL", 1, 0, false);
            declareFunction("sksi_storage_modules_registered_knowledge_sources_remove", "SKSI-STORAGE-MODULES-REGISTERED-KNOWLEDGE-SOURCES-REMOVE", 1, 0, false);
            declareFunction("storage_modules_registered_sub_skses", "STORAGE-MODULES-REGISTERED-SUB-SKSES", 1, 0, false);
            declareFunction("any_sub_ks_storage_modules_registeredP", "ANY-SUB-KS-STORAGE-MODULES-REGISTERED?", 1, 0, false);
            declareFunction("any_closure_sub_ks_storage_modules_registeredP", "ANY-CLOSURE-SUB-KS-STORAGE-MODULES-REGISTERED?", 1, 0, false);
            declareFunction("sksi_expanded_knowledge_sources", "SKSI-EXPANDED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareFunction("sks_expanded_p", "SKS-EXPANDED-P", 1, 0, false);
            declareFunction("sksi_expanded_knowledge_sources_add", "SKSI-EXPANDED-KNOWLEDGE-SOURCES-ADD", 1, 0, false);
            declareFunction("sksi_expanded_knowledge_sources_remove", "SKSI-EXPANDED-KNOWLEDGE-SOURCES-REMOVE", 1, 0, false);
            ...
            declareFunction("display_user_input_table", "DISPLAY-USER-INPUT-TABLE", 1, 0, false);
            declareFunction("choose_image_for_row", "CHOOSE-IMAGE-FOR-ROW", 2, 0, false);
            declareFunction("set_sks_table_row_parameters", "SET-SKS-TABLE-ROW-PARAMETERS", 3, 0, false);
            declareFunction("display_sks_table_row", "DISPLAY-SKS-TABLE-ROW", 6, 2, false);
            declareFunction("display_sks_all_row", "DISPLAY-SKS-ALL-ROW", 0, 0, false);
            declareFunction("sks_source_by_input", "SKS-SOURCE-BY-INPUT", 2, 0, false);
            declareFunction("sksi_sks_manager_input", "SKSI-SKS-MANAGER-INPUT", 1, 0, false);
            declareFunction("register_sksi_removal_modules_for_skses", "REGISTER-SKSI-REMOVAL-MODULES-FOR-SKSES", 1, 0, false);
            declareFunction("register_removal_parent_ks_conjunctive_rms_if_warranted", "REGISTER-REMOVAL-PARENT-KS-CONJUNCTIVE-RMS-IF-WARRANTED", 1, 0, false);
            declareFunction("deregister_sksi_removal_modules_for_skses", "DEREGISTER-SKSI-REMOVAL-MODULES-FOR-SKSES", 1, 0, false);
            declareFunction("deregister_removal_parent_ks_conjunctive_rms_if_warranted", "DEREGISTER-REMOVAL-PARENT-KS-CONJUNCTIVE-RMS-IF-WARRANTED", 1, 0, false);
            declareFunction("sks_registration_state", "SKS-REGISTRATION-STATE", 1, 0, false);
            declareFunction("sks_removal_modules_registration_state", "SKS-REMOVAL-MODULES-REGISTRATION-STATE", 1, 0, false);
            declareFunction("register_sksi_storage_modules_for_skses", "REGISTER-SKSI-STORAGE-MODULES-FOR-SKSES", 1, 0, false);
            declareFunction("deregister_sksi_storage_modules_for_skses", "DEREGISTER-SKSI-STORAGE-MODULES-FOR-SKSES", 1, 0, false);
            declareFunction("sks_storage_modules_registration_state", "SKS-STORAGE-MODULES-REGISTRATION-STATE", 1, 0, false);
            declareFunction("cb_sks_register", "CB-SKS-REGISTER", 1, 0, false);
            declareFunction("cb_link_sksi_register", "CB-LINK-SKSI-REGISTER", 1, 0, false);
            declareFunction("cb_sks_deregister", "CB-SKS-DEREGISTER", 1, 0, false);
            declareFunction("cb_link_sksi_deregister", "CB-LINK-SKSI-DEREGISTER", 1, 0, false);
            declareFunction("sks_registration_state_p", "SKS-REGISTRATION-STATE-P", 1, 0, false);
            declareFunction("current_sks_registration_state", "CURRENT-SKS-REGISTRATION-STATE", 0, 0, false);
            declareFunction("current_top_level_and_straggler_registered_knowledge_sources", "CURRENT-TOP-LEVEL-AND-STRAGGLER-REGISTERED-KNOWLEDGE-SOURCES", 0, 0, false);
            declareMacro("destructure_sks_registration_state", "DESTRUCTURE-SKS-REGISTRATION-STATE");
            declareFunction("execute_sks_registration_state", "EXECUTE-SKS-REGISTRATION-STATE", 1, 0, false);
            declareFunction("clear_sks_registration_state", "CLEAR-SKS-REGISTRATION-STATE", 0, 0, false);
        }
        if (SubLFiles.USE_V2) {
            declareFunction("sksi_registered_knowledge_sources_remove", "SKSI-REGISTERED-KNOWLEDGE-SOURCES-REMOVE", 1, 0, false);
            declareFunction("registered_sub_skses", "REGISTERED-SUB-SKSES", 1, 0, false);
    ...
    public static SubLObject declare_sksi_sks_manager_file_Previous() {
        declareFunction("sksi_sks_manager", "SKSI-SKS-MANAGER", 0, 1, false);
        declareFunction("cb_link_sksi_sks_manager", "CB-LINK-SKSI-SKS-MANAGER", 0, 1, false);
        declareFunction("sks_get_sub_skses", "SKS-GET-SUB-SKSES", 1, 0, false);
        declareFunction("sks_depth", "SKS-DEPTH", 1, 1, false);
    ...
    public static final SubLObject init_sksi_sks_manager_file_alt() {
        deflexical("*SKSI-REGISTERED-KNOWLEDGE-SOURCES*", NIL != boundp($sksi_registered_knowledge_sources$) ? ((SubLObject) ($sksi_registered_knowledge_sources$.getGlobalValue())) : NIL);
        deflexical("*SKSI-EXPANDED-KNOWLEDGE-SOURCES*", NIL != boundp($sksi_expanded_knowledge_sources$) ? ((SubLObject) ($sksi_expanded_knowledge_sources$.getGlobalValue())) : NIL);
        defparameter("*URL-EXPAND-IMAGE*", $str_alt19$_cycdoc_img_cb_right_triangle_blu);
        defparameter("*URL-CONTRACT-IMAGE*", $str_alt20$_cycdoc_img_cb_down_triangle_blue);
        defparameter("*URL-BLANK-IMAGE*", $str_alt21$_cycdoc_img_cb_blank_gif);
        defparameter("*REG-STRING*", $$$Registered);
        defparameter("*PARTIAL-REG-STRING*", $$$Partially_Registered);
        defparameter("*NO-REG-STRING*", $$$Not_Registered);
        defparameter("*REG-BG-COLOR*", $str_alt25$_AAAAFF);
        defparameter("*PARTIAL-BG-COLOR*", $str_alt26$_DDDDFF);
        return NIL;
    }
    ...
    public static SubLObject init_sksi_sks_manager_file_Previous() {
        defparameter("*SKSI-SKS-MANAGER-STORAGE-MODULE-SUPPORT-ENABLED?*", NIL);
        defparameter("*SKSI-SKS-MANAGER-CHECK-SKS-STATUS-ENABLED?*", NIL);
        deflexical("*SKSI-REMOVAL-MODULES-REGISTERED-KNOWLEDGE-SOURCES*", SubLTrampolineFile.maybeDefault($sksi_removal_modules_registered_knowledge_sources$, $sksi_removal_modules_registered_knowledge_sources$, NIL));
        deflexical("*SKSI-CONTENT-MTS-WITH-REGISTERED-REMOVAL-MODULE*", SubLTrampolineFile.maybeDefault($sksi_content_mts_with_registered_removal_module$, $sksi_content_mts_with_registered_removal_module$, NIL));
    ...
    public static final SubLObject setup_sksi_sks_manager_file_alt() {
        html_macros.note_html_handler_function(SKSI_SKS_MANAGER);
        ...
        declare_defglobal($sksi_registered_knowledge_sources$);
        declare_defglobal($sksi_expanded_knowledge_sources$);
        sethash($SKS_EXPAND, $cb_icon_definitions$.getGlobalValue(), list($str_alt14$right_triangle_blue_gif, NIL));
        sethash($SKS_CONTRACT, $cb_icon_definitions$.getGlobalValue(), list($str_alt16$down_triangle_blue_gif, NIL));
        sethash($SKS_BLANK, $cb_icon_definitions$.getGlobalValue(), list($str_alt18$blank_gif, NIL));
        html_macros.note_html_handler_function(SKSI_SKS_MANAGER_INPUT);
        html_macros.note_html_handler_function(CB_SKS_REGISTER);
        setup_cb_link_method($SKSI_REGISTER, CB_LINK_SKSI_REGISTER, ONE_INTEGER);
        html_macros.note_html_handler_function(CB_SKS_DEREGISTER);
    ...
    public static SubLObject setup_sksi_sks_manager_file_Previous() {
        html_macros.note_cgi_handler_function(SKSI_SKS_MANAGER, $HTML_HANDLER);
        setup_cb_link_method($SKSI_SKS_MANAGER, CB_LINK_SKSI_SKS_MANAGER, ONE_INTEGER);
        declare_cb_tool($SKSI_SKS_MANAGER, $$$SKS_Manager, $$$SKSMan, $$$SKSI_SKS_Manager);
        declare_defglobal($sksi_removal_modules_registered_knowledge_sources$);
