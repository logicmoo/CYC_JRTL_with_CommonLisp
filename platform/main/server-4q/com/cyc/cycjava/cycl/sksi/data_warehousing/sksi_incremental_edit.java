...
public final class sksi_incremental_edit
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject sksi_store_ist_logical_sentence(final SubLObject ist_logical_sentence, final SubLObject sk_source, final SubLObject logical_schema, final SubLObject physical_schema, SubLObject meta_mt,
      SubLObject access_path)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == current )
      {
        final SubLObject partial_successP = sksi_store_or_delete_raw_value( sk_source, access_path, raw_tuple, physical_fields, physical_schema, meta_mt, $kw1$INSERT, csql_conditions );
  ...
  public static SubLObject sksi_delete_ist_logical_sentence(final SubLObject ist_logical_sentence, final SubLObject sk_source, final SubLObject logical_schema, final SubLObject physical_schema, SubLObject meta_mt,
      SubLObject access_path)
  {
    if( meta_mt == UNPROVIDED )
    {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == current )
      {
        final SubLObject partial_successP = sksi_store_or_delete_raw_value( sk_source, access_path, raw_tuple, physical_fields, physical_schema, meta_mt, $kw2$DELETE, csql_conditions );
        if( NIL != partial_successP )
        {
          successP = partial_successP;
        }
  ...
  public static SubLObject sksi_store_logical_sentence(final SubLObject logical_sentence, final SubLObject sk_source, final SubLObject logical_schema, final SubLObject physical_schema, SubLObject meta_mt,
      SubLObject access_path)
  {
    if( meta_mt == UNPROVIDED )
    {
  ...
  public static SubLObject sksi_delete_logical_sentence(final SubLObject logical_sentence, final SubLObject sk_source, final SubLObject logical_schema, final SubLObject physical_schema, SubLObject meta_mt,
      SubLObject access_path)
  {
    if( meta_mt == UNPROVIDED )
    {
  ...
  public static SubLObject sksi_translate_ist_logical_sentence_to_raw_tuples(final SubLObject ist_logical_sentence, final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject meta_mt,
  ...
  public static SubLObject sksi_translate_logical_sentence_to_raw_tuples(final SubLObject logical_sentence, final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject meta_mt,
      final SubLObject sk_source, SubLObject rewrite_cmses_with_set_arg3sP)
  {
    ...
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str4$_sksi_translate_logical_sentence_ ), new SubLObject[] { NINE_INTEGER, logical_sentence,
        logical_schema, physical_schema, meta_mt, sk_source, rewrite_cmses_with_set_arg3sP
      } );
    ...
    try
    {
      ...
      try
      {
        ...
        try
        {
          ...
          if( NIL == list_utilities.every_in_list( $sym5$PHYSICAL_FIELD_INDEXICAL_P, relevant_pf_indexicals, UNPROVIDED ) )
          {
            ...
            while ( NIL != relevant_pf_indexical || NIL != physical_field)
            {
              ...
              {
                sksi_debugging.sksi_error( Sequences.cconcatenate( $str6$Could_not_find_indexical_for_phys, format_nil.format_nil_s_no_copy( physical_field_$2 ) ) );
              }
          ...
          thread.resetMultipleValues();
          final SubLObject relevant_logical_field_indexicals_lists = logical_schema_field_indexicals_relevant_to_logical_sentence( logical_sentence, logical_schema, physical_schema, sk_source,
          ...
          while ( NIL != cdolist_list_var)
          {
            ...
            while ( NIL != cdolist_list_var_$4)
            {
              ...
              final SubLObject decoded_sentences = cdolist_list_var_$5 = sksi_translate_logical_sentence_to_decoded_sentences( each_logical_sentence, logical_schema, physical_schema,
                  relevant_logical_field_indexicals_list, content_mt, meta_mt, new_cms_list );
              ...
              while ( NIL != cdolist_list_var_$5)
              {
                ...
                if( NIL == current )
                {
                  ...
                  {
                    final SubLObject raw_sentence = sksi_translate_decoded_sentence_to_raw_sentence( ref_decoded_sentence, physical_schema, relevant_pf_indexicals, logical_schema, relevant_logical_field_indexicals_list,
                        sk_source );
                    if( NIL != raw_sentence )
                    {
                      ...
                      {
                        raw_tuples = ConsesLow.cons( ConsesLow.list( raw_tuple, csql_conditions ), raw_tuples );
                      }
                      else
                      {
                        raw_tuples = ConsesLow.cons( ConsesLow.list( raw_tuple ), raw_tuples );
                      }
                    }
    ...
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str9$sksi_translate_logical_sentence_t ), new SubLObject[] { NINE_INTEGER, raw_tuples,
        physical_schema
      } );
      streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
    }
  ...
  public static SubLObject logical_schema_field_indexicals_relevant_to_logical_sentence(final SubLObject logical_sentence, final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject sk_source,
      final SubLObject rewrite_cmses_with_set_arg3sP)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject subsentences = ( NIL != el_utilities.el_conjunction_p( logical_sentence ) ) ? cycl_utilities.formula_args( logical_sentence, UNPROVIDED ) : ConsesLow.list( logical_sentence );
    ...
    thread.resetMultipleValues();
    final SubLObject pred_to_meaning_sentences_table = meaning_sentence_predicate_table_for_logical_sentence_logical_schema_physical_schema( logical_sentence, logical_schema, physical_schema, sk_source,
        rewrite_cmses_with_set_arg3sP );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      SubLObject logical_field_indexicals_appearing_in_all = list_utilities.intersect_all( meaning_sentence_args_lists, Symbols.symbol_function( EQUAL ), UNPROVIDED );
      SubLObject logical_field_indexicals_appearing_in_some = NIL;
      if( NIL != list_utilities.singletonP( meaning_sentence_args_lists ) )
      {
        unambiguous_subsentences = ConsesLow.cons( sentence, unambiguous_subsentences );
      ...
      {
        final SubLObject lfis_appearing_in_some = list_utilities.fast_set_difference( arg_list, logical_field_indexicals_appearing_in_all, Symbols.symbol_function( EQUAL ) );
        if( NIL != lfis_appearing_in_some )
        {
          logical_field_indexicals_appearing_in_some = ConsesLow.cons( lfis_appearing_in_some, logical_field_indexicals_appearing_in_some );
          dictionary_utilities.dictionary_push( ambiguous_subsentences_table, pred, sentence );
        }
        else if( NIL != list_utilities.fast_sets_equalP( arg_list, logical_field_indexicals_appearing_in_all, UNPROVIDED ) )
        {
      ...
      {
        ambiguous_logical_field_indexicals = ConsesLow.cons( logical_field_indexicals_appearing_in_some, ambiguous_logical_field_indexicals );
      }
      cdolist_list_var = cdolist_list_var.rest();
      sentence = cdolist_list_var.first();
    }
    unambiguous_subsentences = list_utilities.fast_delete_duplicates( unambiguous_subsentences, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    ...
    ambiguous_logical_field_indexicals = list_utilities.fast_delete_duplicates( Sequences.remove_if( $sym10$EQUAL_DUPLICATES_, list_utilities.cross_products( ambiguous_logical_field_indexicals ), UNPROVIDED, UNPROVIDED,
        UNPROVIDED, UNPROVIDED ), $sym11$SETS_EQUAL_EQUAL_, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    logical_field_indexicals_lists = compute_possible_logical_field_indexicals_lists( ambiguous_logical_field_indexicals, unambiguous_logical_field_indexicals, ( NIL != list_utilities.lengthE( subsentences, ONE_INTEGER,
        UNPROVIDED ) ) ? exclusive_lfi_list : NIL );
    return Values.values( list_utilities.fast_delete_duplicates( logical_field_indexicals_lists, $sym11$SETS_EQUAL_EQUAL_, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ), list_utilities.fast_delete_duplicates(
        ambiguous_variants_of_logical_sentence, $sym12$EQUALS_, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ), new_cms_list );
  }
  ...
  public static SubLObject map_pred_to_logical_field_indexical_tuples_from_meaning_sentences(final SubLObject pred_to_meaning_sentences_table, final SubLObject pred)
  {
    ...
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( pred_to_meaning_sentences_table ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
  ...
  public static SubLObject filter_meaning_sentences_by_pred_arity(final SubLObject meaning_sentences, final SubLObject pred)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject meaning_sentence_arity = el_utilities.formula_arity( sentence, UNPROVIDED );
  ...
  public static SubLObject extract_logical_field_indexicals_from_meaning_sentences(final SubLObject meaning_sentences)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      result = ConsesLow.cons( sksi_meaning_sentence_utilities.meaning_sentence_logical_field_indexicals( sentence ), result );
      cdolist_list_var = cdolist_list_var.rest();
      sentence = cdolist_list_var.first();
    }
    return Sequences.nreverse( result );
  ...
  public static SubLObject compute_possible_logical_field_indexicals_lists(final SubLObject ambiguous_logical_field_indexicals, final SubLObject unambiguous_logical_field_indexicals, final SubLObject exclusive_lfi_list)
  {
    ...
    {
      result = ConsesLow.list( Mapping.mapcar( Symbols.symbol_function( $sym13$LIST ), unambiguous_logical_field_indexicals ) );
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      result = ConsesLow.cons( ConsesLow.append( each_ambiguous_lfi_set, unambiguous_logical_field_indexicals ), result );
      cdolist_list_var = cdolist_list_var.rest();
      each_ambiguous_lfi_set = cdolist_list_var.first();
    }
    if( NIL != exclusive_lfi_list )
    {
      result = ConsesLow.cons( exclusive_lfi_list, result );
    }
    return result;
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-incremental-edit.lisp", position = 17392L)
  public static SubLObject meaning_sentence_predicate_table_for_logical_sentence_logical_schema_physical_schema(final SubLObject logical_sentence, final SubLObject logical_schema, final SubLObject physical_schema,
      final SubLObject sk_source, final SubLObject rewrite_cmses_with_set_arg3sP)
  {
    ...
    SubLObject pred_to_meaning_sentences_table = dictionary.new_dictionary( Symbols.symbol_function( EQ ), Sequences.length( meaning_sentences ) );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        pred_to_meaning_sentences_table = maybe_add_meaning_sentence_to_meaning_sentence_predicate_table( sksi_meaning_sentence_utilities.get_consequent_meaning_sentence_from_gaf( cms_gaf, UNPROVIDED ), logical_sentence,
            pred_to_meaning_sentences_table, logical_schema, physical_schema, sk_source );
      }
      else if( NIL != rewrite_cmses_with_set_arg3sP )
      {
        final SubLObject new_sentence = rewrite_conditional_meaning_sentence( cms_gaf );
        if( NIL != new_sentence )
        {
          pred_to_meaning_sentences_table = maybe_add_meaning_sentence_to_meaning_sentence_predicate_table( cycl_utilities.formula_arg4( new_sentence, UNPROVIDED ), logical_sentence, pred_to_meaning_sentences_table,
              logical_schema, physical_schema, sk_source );
          new_cms_list = ConsesLow.cons( new_sentence, new_cms_list );
        }
      }
      cdolist_list_var = cdolist_list_var.rest();
      cms_gaf = cdolist_list_var.first();
  ...
  public static SubLObject maybe_add_meaning_sentence_to_meaning_sentence_predicate_table(final SubLObject meaning_sentence, final SubLObject logical_sentence, SubLObject pred_to_meaning_sentences_table,
      final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject sk_source)
  {
    if( NIL != meaning_sentence_appropriate_for_logical_sentenceP( meaning_sentence, logical_sentence, sk_source ) )
    {
      pred_to_meaning_sentences_table = add_meaning_sentence_to_meaning_sentence_predicate_table( meaning_sentence, pred_to_meaning_sentences_table, logical_schema, physical_schema );
  ...
  public static SubLObject add_meaning_sentence_to_meaning_sentence_predicate_table(final SubLObject meaning_sentence, final SubLObject pred_to_meaning_sentences_table, final SubLObject logical_schema,
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL == el_utilities.el_conjunction_p( logical_sentence ) && NIL == meaning_sentence_pred_appropriate_for_logical_sentence_predP( cycl_utilities.formula_operator( meaning_sentence ), cycl_utilities
        .formula_operator( logical_sentence ) ) )
    {
      return NIL;
    }
    final SubLObject logical_sentence_conjuncts = ( NIL != el_utilities.el_conjunction_p( logical_sentence ) ) ? cycl_utilities.formula_args( logical_sentence, UNPROVIDED ) : ConsesLow.list( logical_sentence );
    SubLObject appropriateP = NIL;
    if( NIL == appropriateP )
    {
      SubLObject csome_list_var;
      SubLObject conjunct;
      for( csome_list_var = logical_sentence_conjuncts, conjunct = NIL, conjunct = csome_list_var.first(); NIL == appropriateP && NIL != csome_list_var; appropriateP = sksi_meaning_sentence_utilities
          .meaning_sentence_type_check( conjunct, meaning_sentence, NIL, sksi_kb_accessors.sk_source_content_mt( sk_source ), mt_relevance_macros.$mt$.getDynamicValue( thread ), $kw14$RAW, T,
              T ), csome_list_var = csome_list_var.rest(), conjunct = csome_list_var.first() )
      {
      }
    }
    return appropriateP;
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject complex_operator = ( NIL != el_utilities.el_conjunction_p( logical_sentence ) ) ? cycl_utilities.formula_operator( logical_sentence ) : NIL;
    SubLObject ambiguous_subsentences = NIL;
    SubLObject ambiguous_variants_of_logical_sentence = NIL;
    SubLObject iteration_state;
    for( iteration_state = dictionary_contents.do_dictionary_contents_state( dictionary.dictionary_contents( ambiguous_subsentences_table ) ); NIL == dictionary_contents.do_dictionary_contents_doneP(
        iteration_state ); iteration_state = dictionary_contents.do_dictionary_contents_next( iteration_state ) )
    {
      thread.resetMultipleValues();
      ...
      thread.resetMultipleValues();
      ambiguous_subsentences = ConsesLow.nconc( ambiguous_subsentences, list_utilities.permute_list( list_utilities.fast_delete_duplicates( ambiguous_subsentences_with_pred, UNPROVIDED, UNPROVIDED, UNPROVIDED,
          UNPROVIDED, UNPROVIDED ), UNPROVIDED ) );
    }
    dictionary_contents.do_dictionary_contents_finalize( iteration_state );
    SubLObject cdolist_list_var;
    ambiguous_subsentences = ( cdolist_list_var = Sequences.remove_if( $sym10$EQUAL_DUPLICATES_, list_utilities.cross_products( ambiguous_subsentences ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        each_logical_variant = el_utilities.make_formula( complex_operator, each_logical_variant, UNPROVIDED );
      }
      if( NIL != el_utilities.el_conjunction_p( each_logical_variant ) )
      {
        ambiguous_variants_of_logical_sentence = ConsesLow.cons( each_logical_variant, ambiguous_variants_of_logical_sentence );
      }
      else
      {
    ...
    {
      ambiguous_variants_of_logical_sentence = ConsesLow.list( logical_sentence );
    }
  ...
  public static SubLObject field_encodings_relevant_to_logical_field_indexical_internal(final SubLObject logical_field_indexical, final SubLObject logical_schema, final SubLObject physical_schema)
  {
    ...
    final SubLObject l_index = kb_indexing.best_gaf_lookup_index( ConsesLow.listS( $const16$fieldEncoding, physical_schema, $kw17$WHAT, logical_schema, $list18 ), $kw19$TRUE, NIL );
    final SubLObject pcase_var;
    final SubLObject method = pcase_var = kb_mapping_macros.do_gli_extract_method( l_index );
    if( pcase_var.eql( $kw20$GAF_ARG ) )
    {
      thread.resetMultipleValues();
          ...
          if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( v_term, argnum, pred_var ) )
          {
            ...
            while ( NIL == done_var)
            {
              ...
              if( NIL != valid )
              {
                ...
                try
                {
                  final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw21$GAF, $kw19$TRUE, NIL );
                  ...
                  while ( NIL == done_var_$9)
                  {
                    ...
                    if( NIL != valid_$11 && physical_schema.eql( assertions_high.gaf_arg1( ass ) ) && logical_schema.eql( assertions_high.gaf_arg3( ass ) ) && list_utilities.tree_count( logical_field_indexical_fort,
                        assertions_high.gaf_arg4( ass ), UNPROVIDED, UNPROVIDED ).isPositive() )
                    {
                      final SubLObject logical_recipe = assertions_high.gaf_arg4( ass );
                      final SubLObject pf_indexical = assertions_high.gaf_arg2( ass );
                      v_answer = ConsesLow.cons( ConsesLow.list( logical_recipe, pf_indexical ), v_answer );
                    }
              ...
              if( NIL != valid )
              {
                ...
                try
                {
                  ...
                  while ( NIL == done_var_$10)
                  {
                    ...
                    if( NIL != valid_$12 && physical_schema.eql( assertions_high.gaf_arg1( ass ) ) && logical_schema.eql( assertions_high.gaf_arg3( ass ) ) && list_utilities.tree_count( logical_field_indexical_fort,
                        assertions_high.gaf_arg4( ass ), UNPROVIDED, UNPROVIDED ).isPositive() )
                    {
                      final SubLObject logical_recipe = assertions_high.gaf_arg4( ass );
                      final SubLObject pf_indexical = assertions_high.gaf_arg2( ass );
            ...
            if( NIL != valid )
            {
              ...
              try
              {
                ...
                while ( NIL == done_var_$11)
                {
                  ...
                  if( NIL != valid_$13 && physical_schema.eql( assertions_high.gaf_arg1( ass ) ) && logical_schema.eql( assertions_high.gaf_arg3( ass ) ) && list_utilities.tree_count( logical_field_indexical_fort,
                      assertions_high.gaf_arg4( ass ), UNPROVIDED, UNPROVIDED ).isPositive() )
                  {
                    final SubLObject logical_recipe = assertions_high.gaf_arg4( ass );
                    final SubLObject pf_indexical = assertions_high.gaf_arg2( ass );
            ...
            if( NIL != valid )
            {
              ...
              try
              {
                ...
                while ( NIL == done_var_$12)
                {
                  ...
                  if( NIL != valid_$14 && physical_schema.eql( assertions_high.gaf_arg1( ass ) ) && logical_schema.eql( assertions_high.gaf_arg3( ass ) ) && list_utilities.tree_count( logical_field_indexical_fort,
                      assertions_high.gaf_arg4( ass ), UNPROVIDED, UNPROVIDED ).isPositive() )
                  {
                    final SubLObject logical_recipe = assertions_high.gaf_arg4( ass );
                    final SubLObject pf_indexical = assertions_high.gaf_arg2( ass );
    ...
    else if( pcase_var.eql( $kw22$PREDICATE_EXTENT ) )
    {
        ...
        try
        {
          ...
          while ( NIL == done_var2)
          {
            ...
            if( NIL != valid2 )
            {
              ...
              try
              {
                final_index_iterator2 = kb_mapping_macros.new_final_index_iterator( final_index_spec2, $kw21$GAF, $kw19$TRUE, NIL );
                ...
                while ( NIL == done_var_$13)
                {
                  ...
                  if( NIL != valid_$15 && physical_schema.eql( assertions_high.gaf_arg1( ass2 ) ) && logical_schema.eql( assertions_high.gaf_arg3( ass2 ) ) && list_utilities.tree_count( logical_field_indexical_fort,
                      assertions_high.gaf_arg4( ass2 ), UNPROVIDED, UNPROVIDED ).isPositive() )
                  {
                    ...
                    v_answer = ConsesLow.cons( ConsesLow.list( logical_recipe2, pf_indexical2 ), v_answer );
                  }
    ...
    else if( pcase_var.eql( $kw23$OVERLAP ) )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        if( NIL != assertions_high.assertion_has_truth( ass3, $kw19$TRUE ) && physical_schema.eql( assertions_high.gaf_arg1( ass3 ) ) && logical_schema.eql( assertions_high.gaf_arg3( ass3 ) ) && list_utilities
            .tree_count( logical_field_indexical_fort, assertions_high.gaf_arg4( ass3 ), UNPROVIDED, UNPROVIDED ).isPositive() )
        {
          ...
          v_answer = ConsesLow.cons( ConsesLow.list( logical_recipe3, pf_indexical3 ), v_answer );
        }
        cdolist_list_var = cdolist_list_var.rest();
        ass3 = cdolist_list_var.first();
      }
  ...
  public static SubLObject field_encodings_relevant_to_logical_field_indexical(final SubLObject logical_field_indexical, final SubLObject logical_schema, final SubLObject physical_schema)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym15$FIELD_ENCODINGS_RELEVANT_TO_LOGICAL_FIELD_INDEXICAL, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym15$FIELD_ENCODINGS_RELEVANT_TO_LOGICAL_FIELD_INDEXICAL, THREE_INTEGER, NIL, EQ,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym15$FIELD_ENCODINGS_RELEVANT_TO_LOGICAL_FIELD_INDEXICAL, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( field_encodings_relevant_to_logical_field_indexical_internal( logical_field_indexical, logical_schema,
        physical_schema ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( logical_field_indexical, logical_schema, physical_schema ) );
      ...
      if( NIL != asent_arg_ls )
      {
        ...
        if( NIL != asent_arg_ps )
        {
          ...
          if( NIL != primary_key )
          {
            ...
            new_cms = el_utilities.make_el_formula( $const24$conditionalMeaningSentenceOfSchem, ConsesLow.list( el_utilities.make_el_set( antecedent_logical_schemata, UNPROVIDED ), new_antecedent, new_lfi_ls,
                cycl_utilities.expression_subst( new_lfi, asent_arg, consequent_meaning_sentence, Symbols.symbol_function( EQUAL ), UNPROVIDED ) ), UNPROVIDED );
          }
        }
      }
    }
  ...
  {
    if( NIL != cycl_utilities.atomic_sentence_with_pred_p( sentence, $const25$equals ) )
    {
      return Values.values( atomic_sentence_constraining_lfi_as_lfi( sentence, lfi, logical_schemata ), NIL );
    }
    if( NIL != el_utilities.el_conjunction_p( sentence ) )
    {
      SubLObject constrained_lfi = NIL;
      SubLObject new_sentence = NIL;
      final SubLObject args = cycl_utilities.formula_args( sentence, $kw26$IGNORE );
      SubLObject rest;
      SubLObject subsentence;
      for( rest = NIL, rest = args; NIL == constrained_lfi && NIL != rest; constrained_lfi = antecedent_meaning_sentence_constraining_lfi_as_lfi( subsentence, lfi, logical_schemata ), new_sentence = el_utilities
          .make_el_formula( cycl_utilities.formula_operator( sentence ), Sequences.remove( subsentence, cycl_utilities.formula_args( sentence, UNPROVIDED ), Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED,
              UNPROVIDED, UNPROVIDED ), UNPROVIDED ), rest = rest.rest() )
      {
  ...
  {
    if( cycl_utilities.atomic_sentence_arg1( sentence, UNPROVIDED ).eql( lfi ) && NIL != sksi_kb_accessors.logical_field_indexical_p( cycl_utilities.atomic_sentence_arg2( sentence, UNPROVIDED ) ) && NIL != list_utilities
        .member_eqP( sksi_kb_accessors.logical_field_indexical_schema( cycl_utilities.atomic_sentence_arg2( sentence, UNPROVIDED ) ), logical_schemata ) )
    {
      return cycl_utilities.atomic_sentence_arg2( sentence, UNPROVIDED );
    }
    if( cycl_utilities.atomic_sentence_arg2( sentence, UNPROVIDED ).eql( lfi ) && NIL != sksi_kb_accessors.logical_field_indexical_p( cycl_utilities.atomic_sentence_arg1( sentence, UNPROVIDED ) ) && NIL != list_utilities
        .member_eqP( sksi_kb_accessors.logical_field_indexical_schema( cycl_utilities.atomic_sentence_arg1( sentence, UNPROVIDED ) ), logical_schemata ) )
    {
  ...
  {
    final SubLObject field_encodings = sksi_field_translation_utilities.get_field_encodings_for_physical_field_indexical( pfi, ConsesLow.list( logical_schema ), ConsesLow.list( physical_schema ) );
    ...
    while ( NIL != cdolist_list_var)
    {
      lfi_list = ConsesLow.append( sksi_infrastructure_utilities.expression_logical_field_indexicals( encoding, UNPROVIDED, UNPROVIDED ), lfi_list );
      cdolist_list_var = cdolist_list_var.rest();
      encoding = cdolist_list_var.first();
    }
    return lfi_list;
  ...
  public static SubLObject guess_ls_for_meaning_sentence(final SubLObject sentence)
  {
    ...
    final SubLObject ls_list = list_utilities.fast_delete_duplicates( Mapping.mapcar( $sym27$LOGICAL_FIELD_INDEXICAL_SCHEMA, lfi_list ), Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject sksi_incremental_edit_reformulate(final SubLObject logical_value, final SubLObject logical_field_indexical, final SubLObject field_encoding, final SubLObject sk_source)
  {
    ...
    if( NIL == pf_indexical_subst || pf_indexical_subst == $kw28$UNREFORMULATABLE )
    {
      ...
      pf_indexical_subst = sksi_reformulate.sksi_reformulate( temp_pf_indexical_subst, $kw29$ENCODE, sk_source );
    }
  ...
  public static SubLObject sksi_incremental_edit_reformulate_decoded_sentence(final SubLObject decoded_sentence, final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject sk_source)
  {
    ...
    if( NIL == failP )
    {
      ...
      while ( NIL == failP && NIL != csome_list_var)
      {
        ...
        final SubLObject field_decodings = sksi_field_translation_utilities.relevant_field_decodings_for_indexical( logical_field_indexical, ConsesLow.list( logical_schema ), ConsesLow.list( physical_schema ) );
        ...
        if( NIL == result_sub_sentence_list )
        {
          ...
          while ( NIL == result_sub_sentence_list && NIL != csome_list_var_$25)
          {
            ...
            SubLObject logical_value_substs = ConsesLow.list( el_utilities.copy_expression( simplified_decoding ) );
                ...
                if( NIL != pf_indexical_subst && pf_indexical_subst != $kw28$UNREFORMULATABLE )
                {
                  ...
                  if( NIL != sksi_reformulate.sksi_true_list_p( pf_indexical_subst ) )
                  {
                    ...
                    while ( NIL != cdolist_list_var_$26)
                    {
                      ...
                      {
                        logical_value_substs = ConsesLow.cons( cycl_utilities.expression_subst( each_ref, pf_indexical, each_logical_value_subst, UNPROVIDED, UNPROVIDED ), logical_value_substs );
                  ...
                  else
                  {
                    ...
                    {
                      logical_value_substs = ConsesLow.cons( cycl_utilities.expression_subst( genl_ks, $const31$TheCurrentKnowledgeSource, cycl_utilities.expression_subst( pf_indexical_subst, pf_indexical,
                          each_logical_value_subst, UNPROVIDED, UNPROVIDED ), UNPROVIDED, UNPROVIDED ), logical_value_substs );
                      cdolist_list_var_$28 = cdolist_list_var_$28.rest();
                      each_logical_value_subst = cdolist_list_var_$28.first();
                    }
                  }
                }
                else if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( TEN_INTEGER ) )
                {
                  PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str32$sksi_incremental_edit_reformulate ), new SubLObject[] { TEN_INTEGER,
                    logical_value, logical_field_indexical, field_encoding
                  } );
                  streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
                }
              }
            ...
            if( NIL != foundP )
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                result_sub_sentence_list = ConsesLow.cons( el_utilities.make_binary_formula( $const33$indexicalReferent, logical_field_indexical, each_logical_value_subst2 ), result_sub_sentence_list );
                cdolist_list_var = cdolist_list_var.rest();
                each_logical_value_subst2 = cdolist_list_var.first();
              }
            }
        ...
        if( NIL != result_sub_sentence_list )
        {
          result_sub_sentences = ( NIL != result_sub_sentences ) ? multiply_reformulation_lists( result_sub_sentence_list, result_sub_sentences )
              : ( ( NIL != list_utilities.singletonP( result_sub_sentence_list ) ) ? ConsesLow.cons( result_sub_sentence_list, result_sub_sentences ) : result_sub_sentence_list );
        }
        else
        {
          failP = T;
    ...
    {
      result = ConsesLow.cons( el_utilities.make_conjunction( Sequences.reverse( each_result_sub_sentences ) ), result );
  ...
  public static SubLObject multiply_reformulation_lists(final SubLObject a_list, final SubLObject b_list)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        result = ConsesLow.cons( ( NIL != el_utilities.el_formula_with_operator_p( b_el, $const33$indexicalReferent ) ) ? ConsesLow.list( a_el, b_el ) : ConsesLow.cons( a_el, b_el ), result );
  ...
  public static SubLObject sksi_translate_logical_sentence_to_decoded_sentences(final SubLObject logical_sentence, final SubLObject logical_schema, final SubLObject physical_schema,
      final SubLObject relevant_logical_field_indexicals, final SubLObject content_mt, final SubLObject meta_mt, final SubLObject new_cms_list)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
      ...
      if( NIL == current )
      {
        ...
        if( NIL != decoded_sentence )
        {
          ...
          if( NIL != successP )
          {
            if( NIL != csql_conditions )
            {
              decoded_sentences = ConsesLow.cons( ConsesLow.list( decoded_sentence, csql_conditions ), decoded_sentences );
            }
            else
            {
              decoded_sentences = ConsesLow.cons( ConsesLow.list( decoded_sentence ), decoded_sentences );
            }
          }
        }
      }
  ...
  public static SubLObject decoded_sentence_satisfies_conditionsP(final SubLObject decoded_sentence, final SubLObject relevant_cmses, final SubLObject content_mt, final SubLObject meta_mt)
  {
    ...
    if( NIL == failP )
    {
      ...
      while ( NIL == failP && NIL != csome_list_var)
      {
        ...
        if( possibly_from_expression != T )
        {
          ...
          else if( NIL != list_utilities.list_of_type_p( $sym35$CSQL_TABLE_EXPRESSION_P, possibly_from_expression ) && NIL != possibly_where_expression )
          {
            conditions = ConsesLow.cons( ConsesLow.list( possibly_from_expression, possibly_where_expression ), conditions );
          }
        }
        csome_list_var = csome_list_var.rest();
        cms = csome_list_var.first();
    ...
    return Values.values( makeBoolean( NIL == failP ), conditions );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-incremental-edit.lisp", position = 34564L)
  public static SubLObject sksi_logical_sentence_to_decoded_sentence_translation_patterns(final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject relevant_logical_field_indexicals,
      final SubLObject logical_sentence, final SubLObject new_cms_list)
  {
    final SubLObject flat_logical_field_indexical_list = list_utilities.tree_find_all_if( $sym36$LOGICAL_FIELD_INDEXICAL_P, relevant_logical_field_indexicals, UNPROVIDED );
    ...
    for( field_number = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), logical_field_indexical = list_var.first(), field_number = Numbers.add( ONE_INTEGER, field_number ) )
    {
      final SubLObject arg_var = sksi_batch_translate.sksi_get_arg_var( field_number );
      final SubLObject indexical_referent_literal = ConsesLow.list( $const33$indexicalReferent, logical_field_indexical, ConsesLow.list( $kw37$VALUE, arg_var ) );
      arg_var_map = ConsesLow.cons( ConsesLow.cons( logical_field_indexical, ConsesLow.list( $kw38$BIND, arg_var ) ), arg_var_map );
      indexical_referent_literals = ConsesLow.cons( indexical_referent_literal, indexical_referent_literals );
    }
    arg_var_map = Sequences.nreverse( arg_var_map );
    indexical_referent_literals = Sequences.nreverse( indexical_referent_literals );
    final SubLObject logical_sentence_conjuncts = ( NIL != el_utilities.el_conjunction_p( logical_sentence ) ) ? cycl_utilities.formula_args( logical_sentence, UNPROVIDED ) : ConsesLow.list( logical_sentence );
    final SubLObject logical_sentence_predicates = Mapping.mapcar( $sym39$FORMULA_OPERATOR, logical_sentence_conjuncts );
    final SubLObject relevant_predicates = Sequences.remove_duplicates( logical_sentence_predicates, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    SubLObject cdolist_list_var;
    final SubLObject relevant_meaning_sentences_lists = cdolist_list_var = relevant_logical_schema_meaning_sentences_with_preds( logical_schema, physical_schema, relevant_logical_field_indexicals, relevant_predicates,
        logical_sentence_predicates, new_cms_list, UNPROVIDED, UNPROVIDED );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$30)
      {
        ...
        cdestructuring_bind.destructuring_bind_must_listp( current, datum, $list40 );
        current = current.rest();
        if( NIL == current )
        {
          meaning_sentence_list = ConsesLow.cons( meaning_sentence, meaning_sentence_list );
          if( NIL != cms_list )
          {
            relevant_cmses = ConsesLow.cons( cms_list, relevant_cmses );
          }
        }
        else
        {
      ...
      sorted_meaning_sentences = list_utilities.sort_via_position( meaning_sentence_list, logical_sentence_predicates, Symbols.symbol_function( EQ ), $sym39$FORMULA_OPERATOR );
      logical_sentence_recipe = simplifier.simplify_cycl_sentence_syntax( el_utilities.make_conjunction( sorted_meaning_sentences ), UNPROVIDED );
      logical_sentence_template = conses_high.sublis( arg_var_map, logical_sentence_recipe, Symbols.symbol_function( EQUAL ), UNPROVIDED );
      pattern = ConsesLow.list( $kw41$TEMPLATE, logical_sentence_template, ( NIL != indexical_referent_literals ) ? el_utilities.make_conjunction( indexical_referent_literals ) : NIL );
      result = ConsesLow.cons( ( NIL != relevant_cmses ) ? ConsesLow.list( pattern, relevant_cmses ) : ConsesLow.list( pattern ), result );
      cdolist_list_var = cdolist_list_var.rest();
      relevant_meaning_sentences = cdolist_list_var.first();
    }
    return result;
  ...
  public static SubLObject relevant_logical_schema_meaning_sentences_with_preds_internal(final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject relevant_logical_field_indexicals,
      final SubLObject relevant_preds, final SubLObject logical_sentence_predicates, final SubLObject new_cms_list, SubLObject asserted_meaning_sentences_onlyP, SubLObject unwrap_istsP)
  {
    ...
    thread.resetMultipleValues();
    final SubLObject relevant_logical_field_indexicals_lists = list_utilities.partition_list( relevant_logical_field_indexicals, Symbols.symbol_function( $sym43$LISTP ) );
    final SubLObject relevant_logical_field_indexical_list = thread.secondMultipleValue();
    thread.resetMultipleValues();
    final SubLObject irrelevant_logical_field_indexicals = conses_high.set_difference( all_logical_field_indexicals, relevant_logical_field_indexical_list, UNPROVIDED, UNPROVIDED );
    ...
    final SubLObject conditional_meaning_sentences = ConsesLow.append( Mapping.mapcar( $sym44$GAF_FORMULA, sksi_meaning_sentence_utilities.logical_schema_consequent_conditional_meaning_sentence_gafs( logical_schema, T,
        NIL, NIL ) ), new_cms_list );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      while ( NIL != cdolist_list_var_$31)
      {
        ...
        {
          relevant_pred_sentences = ConsesLow.cons( ConsesLow.list( sentence ), relevant_pred_sentences );
        }
        cdolist_list_var_$31 = cdolist_list_var_$31.rest();
        sentence = cdolist_list_var_$31.first();
      }
      if( NIL != list_utilities.lengthG( relevant_pred_sentences, ONE_INTEGER, UNPROVIDED ) && NIL != list_utilities.occurs_exactly_n_timesP( relevant_pred, logical_sentence_predicates, Sequences.length(
          relevant_pred_sentences ), UNPROVIDED ) )
      {
        relevant_pred_sentences = ConsesLow.list( relevant_pred_sentences );
      }
      ...
      {
        if( NIL == el_utilities.el_set_p( cycl_utilities.formula_arg3( cms, UNPROVIDED ) ) )
        {
          final SubLObject consequent_meaning_sentence = cycl_utilities.formula_arg4( cms, UNPROVIDED );
          if( NIL != relevant_logical_schema_meaning_sentenceP( consequent_meaning_sentence, relevant_pred, relevant_logical_field_indexicals_lists, irrelevant_logical_field_indexicals, logical_schema,
              physical_schema ) )
          {
            ...
            final SubLObject same_lfis_sentences = list_utilities.find_all( sentence_lfis, relevant_pred_sentences, $sym45$FAST_INTERSECT_, $sym46$MEANING_SENTENCE_LOGICAL_FIELD_INDEXICALS );
            if( NIL != same_lfis_sentences )
            {
              ...
              {
                relevant_pred_sentences = Sequences.delete( same_lfis_sentence, relevant_pred_sentences, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
                if( NIL != sksi_reformulate.sksi_true_list_p( same_lfis_sentence.first() ) )
                {
                  new_same_lfis_sentences = ConsesLow.cons( same_lfis_sentence, new_same_lfis_sentences );
                }
                else
                {
                  new_same_lfis_sentences = ConsesLow.cons( ConsesLow.list( same_lfis_sentence ), new_same_lfis_sentences );
                }
                cdolist_list_var_$33 = cdolist_list_var_$33.rest();
                same_lfis_sentence = cdolist_list_var_$33.first();
              }
              new_same_lfis_sentences = Sequences.nreverse( new_same_lfis_sentences );
              relevant_pred_sentences = multiply_meaning_sentence_lists( list_utilities.nadd_to_end( ConsesLow.list( ConsesLow.list( consequent_meaning_sentence, cms ) ), new_same_lfis_sentences ),
                  relevant_pred_sentences );
            }
            else
            {
              relevant_pred_sentences = ConsesLow.cons( ConsesLow.list( consequent_meaning_sentence, cms ), relevant_pred_sentences );
            }
          }
        }
  ...
  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-incremental-edit.lisp", position = 37152L)
  public static SubLObject relevant_logical_schema_meaning_sentences_with_preds(final SubLObject logical_schema, final SubLObject physical_schema, final SubLObject relevant_logical_field_indexicals,
      final SubLObject relevant_preds, final SubLObject logical_sentence_predicates, final SubLObject new_cms_list, SubLObject asserted_meaning_sentences_onlyP, SubLObject unwrap_istsP)
  {
    if( asserted_meaning_sentences_onlyP == UNPROVIDED )
    {
    ...
    if( NIL == v_memoization_state )
    {
      return relevant_logical_schema_meaning_sentences_with_preds_internal( logical_schema, physical_schema, relevant_logical_field_indexicals, relevant_preds, logical_sentence_predicates, new_cms_list,
          asserted_meaning_sentences_onlyP, unwrap_istsP );
    }
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym42$RELEVANT_LOGICAL_SCHEMA_MEANING_SENTENCES_WITH_PREDS, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym42$RELEVANT_LOGICAL_SCHEMA_MEANING_SENTENCES_WITH_PREDS, EIGHT_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym42$RELEVANT_LOGICAL_SCHEMA_MEANING_SENTENCES_WITH_PREDS, caching_state );
    }
    final SubLObject sxhash = memoization_state.sxhash_calc_8( logical_schema, physical_schema, relevant_logical_field_indexicals, relevant_preds, logical_sentence_predicates, new_cms_list,
        asserted_meaning_sentences_onlyP, unwrap_istsP );
    final SubLObject collisions = memoization_state.caching_state_lookup( caching_state, sxhash, UNPROVIDED );
    if( !collisions.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( relevant_logical_schema_meaning_sentences_with_preds_internal( logical_schema, physical_schema,
        relevant_logical_field_indexicals, relevant_preds, logical_sentence_predicates, new_cms_list, asserted_meaning_sentences_onlyP, unwrap_istsP ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( logical_schema, physical_schema, relevant_logical_field_indexicals, relevant_preds,
        logical_sentence_predicates, new_cms_list, asserted_meaning_sentences_onlyP, unwrap_istsP ) );
    return memoization_state.caching_results( results3 );
  }
  ...
  public static SubLObject multiply_meaning_sentence_lists(final SubLObject a_list, final SubLObject b_list)
  {
    if( NIL != b_list )
    {
      ...
      while ( NIL != cdolist_list_var)
      {
        ...
        {
          result = ConsesLow.cons( ( NIL != sksi_reformulate.sksi_true_list_p( a_el.first() ) ) ? ( ( NIL != sksi_reformulate.sksi_true_list_p( b_el.first() ) ) ? ConsesLow.append( a_el, b_el )
              : list_utilities.nadd_to_end( b_el, a_el ) ) : ( ( NIL != sksi_reformulate.sksi_true_list_p( b_el.first() ) ) ? ConsesLow.cons( a_el, b_el ) : ConsesLow.list( a_el, b_el ) ), result );
    ...
    if( NIL != sksi_reformulate.sksi_true_list_p( conses_high.caar( a_list ) ) )
    {
      return a_list;
    }
    return ConsesLow.list( a_list );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-incremental-edit.lisp", position = 41550L)
  public static SubLObject relevant_logical_schema_meaning_sentenceP(final SubLObject meaning_sentence, final SubLObject relevant_pred, final SubLObject relevant_logical_field_indexicals_lists,
      final SubLObject irrelevant_logical_field_indexicals, final SubLObject logical_schema, final SubLObject physical_schema)
  {
    if( NIL != sksi_meaning_sentence_has_relevant_predP( meaning_sentence, relevant_pred, logical_schema, physical_schema ) )
    {
      ...
      if( NIL == relevantP )
      {
        ...
        while ( NIL == relevantP && NIL != csome_list_var)
        {
          ...
          if( NIL == failP )
          {
            ...
            SubLObject lfi;
            for( csome_list_var_$35 = relevant_logical_field_indexicals_list, lfi = NIL, lfi = csome_list_var_$35.first(); NIL == failP && NIL != csome_list_var_$35; failP = makeBoolean(
                NIL == sksi_meaning_sentence_utilities.meaning_sentence_logical_field_indexical_p( lfi, meaning_sentence ) ), csome_list_var_$35 = csome_list_var_$35.rest(), lfi = csome_list_var_$35.first() )
            {
            }
          }
      ...
      if( NIL == relevantP )
      {
        ...
        if( NIL == irrelevantP )
        {
          ...
          for( csome_list_var2 = irrelevant_logical_field_indexicals, irrelevant_field_indexical = NIL, irrelevant_field_indexical = csome_list_var2.first(); NIL == irrelevantP
              && NIL != csome_list_var2; irrelevantP = sksi_meaning_sentence_utilities.meaning_sentence_logical_field_indexical_p( irrelevant_field_indexical, meaning_sentence ), csome_list_var2 = csome_list_var2
                  .rest(), irrelevant_field_indexical = csome_list_var2.first() )
          {
          }
        }
  ...
  public static SubLObject sksi_translate_decoded_sentence_to_raw_sentence(final SubLObject decoded_sentence, final SubLObject physical_schema, final SubLObject pf_indexicals, final SubLObject logical_schema,
      final SubLObject relevant_logical_field_indexicals, final SubLObject sk_source)
  {
    final SubLObject translation_pattern = sksi_decoded_sentence_to_raw_sentence_translation_pattern( physical_schema, pf_indexicals, logical_schema, Mapping.mapcar( $sym47$FORMULA_ARG1, cycl_utilities.formula_args(
        decoded_sentence, UNPROVIDED ) ), sk_source );
  ...
  public static SubLObject physical_field_indexical_logical_field_indexical_mappings_given_logical_physical_schemata_internal(final SubLObject logical_schema, final SubLObject physical_schema)
  {
    ...
    final SubLObject mappings_cache = dictionary.new_dictionary( Symbols.symbol_function( EQ ), Sequences.length( physical_fields ) );
  ...
  public static SubLObject physical_field_indexical_logical_field_indexical_mappings_given_logical_physical_schemata(final SubLObject logical_schema, final SubLObject physical_schema)
  {
    ...
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym48$PHYSICAL_FIELD_INDEXICAL_LOGICAL_FIELD_INDEXICAL_MAPPINGS_GIVEN_L, TWO_INTEGER, NIL,
          EQ, UNPROVIDED );
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( physical_field_indexical_logical_field_indexical_mappings_given_logical_physical_schemata_internal( logical_schema,
        physical_schema ) ) );
  ...
  public static SubLObject sksi_decoded_sentence_to_raw_sentence_translation_pattern_internal(final SubLObject physical_schema, final SubLObject pf_indexicals, final SubLObject logical_schema,
      final SubLObject relevant_logical_field_indexicals, final SubLObject sk_source)
  {
    ...
    final SubLObject relevant_logical_field_indexical_forts = Mapping.mapcar( $sym50$LFI_FORT_FOR_LFI, relevant_logical_field_indexicals );
    final SubLObject genl_ks = sksi_kb_accessors.max_genl_ks( sk_source );
    ...
    for( field_number = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), pf_indexical = list_var.first(), field_number = Numbers.add( ONE_INTEGER, field_number ) )
    {
      final SubLObject pf_indexical_usedP = list_utilities.intersectP( dictionary.dictionary_lookup_without_values( pf_indexical_lf_mappings_cache, pf_indexical, UNPROVIDED ), relevant_logical_field_indexical_forts,
      ...
      {
        field_value_literal = ConsesLow.list( $const51$fieldValue, pf_indexical, ConsesLow.list( $kw37$VALUE, raw_var ) );
        raw_var_map = ConsesLow.cons( ConsesLow.cons( pf_indexical, ConsesLow.list( $kw38$BIND, raw_var ) ), raw_var_map );
      }
      else
      {
        field_value_literal = ConsesLow.listS( $const51$fieldValue, pf_indexical, $list52 );
      }
      field_value_literals = ConsesLow.cons( field_value_literal, field_value_literals );
    }
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject recipes = cdolist_list_var_$36 = sksi_field_translation_utilities.relevant_field_decodings_for_indexical( logical_field_indexical, ConsesLow.list( logical_schema ), ConsesLow.list(
          physical_schema ) );
      SubLObject recipe = NIL;
      recipe = cdolist_list_var_$36.first();
      while ( NIL != cdolist_list_var_$36)
      {
        final SubLObject decoding = cycl_utilities.expression_subst( genl_ks, $const31$TheCurrentKnowledgeSource, conses_high.sublis( raw_var_map, maybe_evaluate_field_translation( recipe ), UNPROVIDED, UNPROVIDED ),
            UNPROVIDED, UNPROVIDED );
        final SubLObject indexical_referent_literal = ConsesLow.list( $const33$indexicalReferent, logical_field_indexical, decoding );
        indexical_referent_literals = ConsesLow.cons( indexical_referent_literal, indexical_referent_literals );
    ...
    indexical_referent_literals = Sequences.nreverse( Sequences.delete_duplicates( indexical_referent_literals, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
    final SubLObject translation_pattern = ConsesLow.list( $kw41$TEMPLATE, el_utilities.make_conjunction( indexical_referent_literals ), el_utilities.make_conjunction( field_value_literals ) );
    return Values.values( translation_pattern, relevant_logical_field_indexicals );
  }

  @SubLTranslatedFile.SubL(source = "cycl/sksi/data-warehousing/sksi-incremental-edit.lisp", position = 44729L)
  public static SubLObject sksi_decoded_sentence_to_raw_sentence_translation_pattern(final SubLObject physical_schema, final SubLObject pf_indexicals, final SubLObject logical_schema,
      final SubLObject relevant_logical_field_indexicals, final SubLObject sk_source)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    final SubLObject v_memoization_state = memoization_state.$memoization_state$.getDynamicValue( thread );
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym49$SKSI_DECODED_SENTENCE_TO_RAW_SENTENCE_TRANSLATION_PATTERN, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym49$SKSI_DECODED_SENTENCE_TO_RAW_SENTENCE_TRANSLATION_PATTERN, FIVE_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym49$SKSI_DECODED_SENTENCE_TO_RAW_SENTENCE_TRANSLATION_PATTERN, caching_state );
    }
    ...
    final SubLObject results3 = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_decoded_sentence_to_raw_sentence_translation_pattern_internal( physical_schema, pf_indexicals, logical_schema,
        relevant_logical_field_indexicals, sk_source ) ) );
    memoization_state.caching_state_enter_multi_key_n( caching_state, sxhash, collisions, results3, ConsesLow.list( physical_schema, pf_indexicals, logical_schema, relevant_logical_field_indexicals, sk_source ) );
    return memoization_state.caching_results( results3 );
  }
  ...
  {
    return cycl_utilities.expression_transform( expression, $sym54$NON_SKSI_EVALUATABLE_EXPRESSION_, $sym55$MAYBE_EVALUATE_EXPRESSION, UNPROVIDED, UNPROVIDED );
  }
  ...
  public static SubLObject maybe_evaluate_field_translation(final SubLObject expression)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym53$MAYBE_EVALUATE_FIELD_TRANSLATION, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym53$MAYBE_EVALUATE_FIELD_TRANSLATION, ONE_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym53$MAYBE_EVALUATE_FIELD_TRANSLATION, caching_state );
    }
    ...
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( maybe_evaluate_field_translation_internal( expression ) ) );
  ...
  public static SubLObject sksi_raw_sentence_to_raw_tuple_translation_pattern_internal(SubLObject pf_indexicals)
  {
    ...
    for( field_number = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), pf_indexical = list_var.first(), field_number = Numbers.add( ONE_INTEGER, field_number ) )
    {
      ...
      datum_vars = ConsesLow.cons( ConsesLow.list( $kw37$VALUE, datum_var ), datum_vars );
      if( NIL != pf_indexical )
      {
        final SubLObject field_value_literal = ConsesLow.list( $const51$fieldValue, pf_indexical, ConsesLow.list( $kw38$BIND, datum_var ) );
        field_value_literals = ConsesLow.cons( field_value_literal, field_value_literals );
      }
    }
    datum_vars = Sequences.nreverse( datum_vars );
    field_value_literals = Sequences.nreverse( field_value_literals );
    pf_indexicals = Sequences.delete( NIL, pf_indexicals, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    final SubLObject translation_pattern = ConsesLow.list( $kw41$TEMPLATE, el_utilities.make_conjunction( field_value_literals ), datum_vars );
  ...
  public static SubLObject sksi_raw_sentence_to_raw_tuple_translation_pattern(final SubLObject pf_indexicals)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym57$SKSI_RAW_SENTENCE_TO_RAW_TUPLE_TRANSLATION_PATTERN, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym57$SKSI_RAW_SENTENCE_TO_RAW_TUPLE_TRANSLATION_PATTERN, ONE_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym57$SKSI_RAW_SENTENCE_TO_RAW_TUPLE_TRANSLATION_PATTERN, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_raw_sentence_to_raw_tuple_translation_pattern_internal( pf_indexicals ) ) );
  ...
  public static SubLObject sksi_store_or_delete_raw_value(final SubLObject sk_source, final SubLObject access_path, final SubLObject raw_tuple, final SubLObject physical_fields, final SubLObject physical_schema,
      final SubLObject meta_mt, final SubLObject operator, final SubLObject csql_conditions)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str58$_sksi_store_or_delete_raw_value__ ), new SubLObject[] { NINE_INTEGER, sk_source,
    ...
    try
    {
      ...
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str59$sksi_store_or_delete_raw_value_CS ), NINE_INTEGER, csql );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
      if( NIL != csql )
      {
          ...
          try
          {
            ...
            try
            {
              ...
              try
              {
                ...
                assert NIL != sksi_debugging.valid_sksi_error_handling_mode_p( $kw60$BREAK ) : $kw60$BREAK;
                final SubLObject _prev_bind_0_$39 = sksi_debugging.$sksi_sks_error_handling_mode$.currentBinding( thread );
                try
                {
                  sksi_debugging.$sksi_sks_error_handling_mode$.bind( $kw60$BREAK, thread );
    ...
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str62$sksi_store_or_delete_raw_value___ ), NINE_INTEGER, successP );
      streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
    }
    return successP;
  }
  ...
  public static SubLObject sksi_store_or_delete_raw_value_int(final SubLObject access_path, final SubLObject csql, final SubLObject physical_schema)
  {
    ...
    if( NIL != sksi_csql_utilities.invalid_csql_expressionP( csql ) )
    {
      if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
      {
        PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str63$Invalid_CSQL_expression_at__sksi_ ), new SubLObject[] { NINE_INTEGER, access_path,
          csql, physical_schema
        } );
        streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
      }
      return NIL;
  ...
  public static SubLObject sksi_store_or_delete_raw_value_db(final SubLObject csql, final SubLObject access_path)
  {
    ...
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str67$_sksi_store_or_delete_raw_value_d ), new SubLObject[] { NINE_INTEGER, csql,
        access_path
      } );
      streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
    }
    ...
    if( NIL != sksi_debugging.sksi_trace_level_exceeds_minimumP( NINE_INTEGER ) )
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str3$__SKSI__A_, $str68$sksi_store_or_delete_raw_value_db ), NINE_INTEGER, result );
      streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
    }
    return result;
  }
  ...
  public static SubLObject sksi_store_or_delete_raw_value_cache(final SubLObject csql, final SubLObject access_path)
  {
    ...
    cdestructuring_bind.destructuring_bind_must_consp( csql, csql, $list71 );
    operator_keyword = csql.first();
    SubLObject current = csql.rest();
    cdestructuring_bind.destructuring_bind_must_consp( current, csql, $list71 );
    into = current.first();
    current = current.rest();
    cdestructuring_bind.destructuring_bind_must_consp( current, csql, $list71 );
    ...
    if( NIL != current )
    {
      cdestructuring_bind.cdestructuring_bind_error( csql, $list71 );
      return NIL;
    }
    if( NIL == Errors.$ignore_mustsP$.getDynamicValue( thread ) && NIL == conses_high.member( operator_keyword, $list69, UNPROVIDED, UNPROVIDED ) )
    {
  ...
  {
    SubLFiles.declareFunction( me, "sksi_store_ist_logical_sentence", "SKSI-STORE-IST-LOGICAL-SENTENCE", 4, 2, false );
    SubLFiles.declareFunction( me, "sksi_delete_ist_logical_sentence", "SKSI-DELETE-IST-LOGICAL-SENTENCE", 4, 2, false );
    SubLFiles.declareFunction( me, "sksi_store_logical_sentence", "SKSI-STORE-LOGICAL-SENTENCE", 4, 2, false );
    SubLFiles.declareFunction( me, "sksi_delete_logical_sentence", "SKSI-DELETE-LOGICAL-SENTENCE", 4, 2, false );
    SubLFiles.declareFunction( me, "sksi_translate_ist_logical_sentence_to_raw_tuples", "SKSI-TRANSLATE-IST-LOGICAL-SENTENCE-TO-RAW-TUPLES", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_logical_sentence_to_raw_tuples", "SKSI-TRANSLATE-LOGICAL-SENTENCE-TO-RAW-TUPLES", 5, 1, false );
    SubLFiles.declareFunction( me, "logical_schema_field_indexicals_relevant_to_logical_sentence", "LOGICAL-SCHEMA-FIELD-INDEXICALS-RELEVANT-TO-LOGICAL-SENTENCE", 5, 0, false );
    SubLFiles.declareFunction( me, "map_pred_to_logical_field_indexical_tuples_from_meaning_sentences", "MAP-PRED-TO-LOGICAL-FIELD-INDEXICAL-TUPLES-FROM-MEANING-SENTENCES", 2, 0, false );
    SubLFiles.declareFunction( me, "filter_meaning_sentences_by_pred_arity", "FILTER-MEANING-SENTENCES-BY-PRED-ARITY", 2, 0, false );
    SubLFiles.declareFunction( me, "extract_logical_field_indexicals_from_meaning_sentences", "EXTRACT-LOGICAL-FIELD-INDEXICALS-FROM-MEANING-SENTENCES", 1, 0, false );
    SubLFiles.declareFunction( me, "compute_possible_logical_field_indexicals_lists", "COMPUTE-POSSIBLE-LOGICAL-FIELD-INDEXICALS-LISTS", 3, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_predicate_table_for_logical_sentence_logical_schema_physical_schema", "MEANING-SENTENCE-PREDICATE-TABLE-FOR-LOGICAL-SENTENCE-LOGICAL-SCHEMA-PHYSICAL-SCHEMA", 5, 0,
        false );
    SubLFiles.declareFunction( me, "maybe_add_meaning_sentence_to_meaning_sentence_predicate_table", "MAYBE-ADD-MEANING-SENTENCE-TO-MEANING-SENTENCE-PREDICATE-TABLE", 6, 0, false );
    SubLFiles.declareFunction( me, "add_meaning_sentence_to_meaning_sentence_predicate_table", "ADD-MEANING-SENTENCE-TO-MEANING-SENTENCE-PREDICATE-TABLE", 4, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_pred_appropriate_for_logical_sentence_predP", "MEANING-SENTENCE-PRED-APPROPRIATE-FOR-LOGICAL-SENTENCE-PRED?", 2, 0, false );
    SubLFiles.declareFunction( me, "meaning_sentence_appropriate_for_logical_sentenceP", "MEANING-SENTENCE-APPROPRIATE-FOR-LOGICAL-SENTENCE?", 3, 0, false );
    SubLFiles.declareFunction( me, "compute_ambiguous_variants_of_logical_sentence", "COMPUTE-AMBIGUOUS-VARIANTS-OF-LOGICAL-SENTENCE", 3, 0, false );
    SubLFiles.declareFunction( me, "field_encodings_relevant_to_logical_field_indexical_internal", "FIELD-ENCODINGS-RELEVANT-TO-LOGICAL-FIELD-INDEXICAL-INTERNAL", 3, 0, false );
    SubLFiles.declareFunction( me, "field_encodings_relevant_to_logical_field_indexical", "FIELD-ENCODINGS-RELEVANT-TO-LOGICAL-FIELD-INDEXICAL", 3, 0, false );
    SubLFiles.declareFunction( me, "rewrite_conditional_meaning_sentence", "REWRITE-CONDITIONAL-MEANING-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "antecedent_meaning_sentence_constraining_lfi_as_lfi", "ANTECEDENT-MEANING-SENTENCE-CONSTRAINING-LFI-AS-LFI", 3, 0, false );
    SubLFiles.declareFunction( me, "atomic_sentence_constraining_lfi_as_lfi", "ATOMIC-SENTENCE-CONSTRAINING-LFI-AS-LFI", 3, 0, false );
    SubLFiles.declareFunction( me, "physical_field_indexical_logical_field_indexicals", "PHYSICAL-FIELD-INDEXICAL-LOGICAL-FIELD-INDEXICALS", 3, 0, false );
    SubLFiles.declareFunction( me, "guess_ls_for_meaning_sentence", "GUESS-LS-FOR-MEANING-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_incremental_edit_reformulate", "SKSI-INCREMENTAL-EDIT-REFORMULATE", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_incremental_edit_reformulate_decoded_sentence", "SKSI-INCREMENTAL-EDIT-REFORMULATE-DECODED-SENTENCE", 4, 0, false );
    SubLFiles.declareFunction( me, "multiply_reformulation_lists", "MULTIPLY-REFORMULATION-LISTS", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_logical_sentence_to_decoded_sentences", "SKSI-TRANSLATE-LOGICAL-SENTENCE-TO-DECODED-SENTENCES", 7, 0, false );
    SubLFiles.declareFunction( me, "decoded_sentence_satisfies_conditionsP", "DECODED-SENTENCE-SATISFIES-CONDITIONS?", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_logical_sentence_to_decoded_sentence_translation_patterns", "SKSI-LOGICAL-SENTENCE-TO-DECODED-SENTENCE-TRANSLATION-PATTERNS", 5, 0, false );
    SubLFiles.declareFunction( me, "relevant_logical_schema_meaning_sentences_with_preds_internal", "RELEVANT-LOGICAL-SCHEMA-MEANING-SENTENCES-WITH-PREDS-INTERNAL", 6, 2, false );
    SubLFiles.declareFunction( me, "relevant_logical_schema_meaning_sentences_with_preds", "RELEVANT-LOGICAL-SCHEMA-MEANING-SENTENCES-WITH-PREDS", 6, 2, false );
    SubLFiles.declareFunction( me, "multiply_meaning_sentence_lists", "MULTIPLY-MEANING-SENTENCE-LISTS", 2, 0, false );
    SubLFiles.declareFunction( me, "relevant_logical_schema_meaning_sentenceP", "RELEVANT-LOGICAL-SCHEMA-MEANING-SENTENCE?", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_meaning_sentence_has_relevant_predP", "SKSI-MEANING-SENTENCE-HAS-RELEVANT-PRED?", 4, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_decoded_sentence_to_raw_sentence", "SKSI-TRANSLATE-DECODED-SENTENCE-TO-RAW-SENTENCE", 6, 0, false );
    SubLFiles.declareFunction( me, "physical_field_indexical_logical_field_indexical_mappings_given_logical_physical_schemata_internal",
        "PHYSICAL-FIELD-INDEXICAL-LOGICAL-FIELD-INDEXICAL-MAPPINGS-GIVEN-LOGICAL-PHYSICAL-SCHEMATA-INTERNAL", 2, 0, false );
    SubLFiles.declareFunction( me, "physical_field_indexical_logical_field_indexical_mappings_given_logical_physical_schemata", "PHYSICAL-FIELD-INDEXICAL-LOGICAL-FIELD-INDEXICAL-MAPPINGS-GIVEN-LOGICAL-PHYSICAL-SCHEMATA",
        2, 0, false );
    SubLFiles.declareFunction( me, "sksi_decoded_sentence_to_raw_sentence_translation_pattern_internal", "SKSI-DECODED-SENTENCE-TO-RAW-SENTENCE-TRANSLATION-PATTERN-INTERNAL", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_decoded_sentence_to_raw_sentence_translation_pattern", "SKSI-DECODED-SENTENCE-TO-RAW-SENTENCE-TRANSLATION-PATTERN", 5, 0, false );
    SubLFiles.declareFunction( me, "maybe_evaluate_field_translation_internal", "MAYBE-EVALUATE-FIELD-TRANSLATION-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "maybe_evaluate_field_translation", "MAYBE-EVALUATE-FIELD-TRANSLATION", 1, 0, false );
    SubLFiles.declareFunction( me, "maybe_evaluate_expression", "MAYBE-EVALUATE-EXPRESSION", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_translate_raw_sentence_to_raw_tuple", "SKSI-TRANSLATE-RAW-SENTENCE-TO-RAW-TUPLE", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_raw_sentence_to_raw_tuple_translation_pattern_internal", "SKSI-RAW-SENTENCE-TO-RAW-TUPLE-TRANSLATION-PATTERN-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_raw_sentence_to_raw_tuple_translation_pattern", "SKSI-RAW-SENTENCE-TO-RAW-TUPLE-TRANSLATION-PATTERN", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_store_or_delete_raw_value", "SKSI-STORE-OR-DELETE-RAW-VALUE", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_store_or_delete_raw_value_int", "SKSI-STORE-OR-DELETE-RAW-VALUE-INT", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_store_or_delete_raw_value_db", "SKSI-STORE-OR-DELETE-RAW-VALUE-DB", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_store_or_delete_raw_value_db_int", "SKSI-STORE-OR-DELETE-RAW-VALUE-DB-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_store_or_delete_raw_value_fht", "SKSI-STORE-OR-DELETE-RAW-VALUE-FHT", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_store_or_delete_raw_value_cache", "SKSI-STORE-OR-DELETE-RAW-VALUE-CACHE", 2, 0, false );
    return NIL;
  }
  ...
  {
    memoization_state.note_memoized_function( $sym15$FIELD_ENCODINGS_RELEVANT_TO_LOGICAL_FIELD_INDEXICAL );
    memoization_state.note_memoized_function( $sym42$RELEVANT_LOGICAL_SCHEMA_MEANING_SENTENCES_WITH_PREDS );
    memoization_state.note_memoized_function( $sym48$PHYSICAL_FIELD_INDEXICAL_LOGICAL_FIELD_INDEXICAL_MAPPINGS_GIVEN_L );
    memoization_state.note_memoized_function( $sym49$SKSI_DECODED_SENTENCE_TO_RAW_SENTENCE_TRANSLATION_PATTERN );
    memoization_state.note_memoized_function( $sym53$MAYBE_EVALUATE_FIELD_TRANSLATION );
    memoization_state.note_memoized_function( $sym57$SKSI_RAW_SENTENCE_TO_RAW_TUPLE_TRANSLATION_PATTERN );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_incremental_edit();
    $list0 = ConsesLow.list( makeSymbol( "RAW-TUPLE" ), makeSymbol( "&OPTIONAL" ), makeSymbol( "CSQL-CONDITIONS" ) );
    $kw1$INSERT = makeKeyword( "INSERT" );
    $kw2$DELETE = makeKeyword( "DELETE" );
    $str3$__SKSI__A_ = makeString( "~&SKSI-~A " );
    $str4$_sksi_translate_logical_sentence_ = makeString( "(sksi-translate-logical-sentence-to-raw-tuples ~S ~S ~S ~S ~S ~S)~%" );
    $sym5$PHYSICAL_FIELD_INDEXICAL_P = makeSymbol( "PHYSICAL-FIELD-INDEXICAL-P" );
    $str6$Could_not_find_indexical_for_phys = makeString( "Could not find indexical for physical field " );
    $sym7$_EXIT = makeSymbol( "%EXIT" );
    $list8 = ConsesLow.list( makeSymbol( "DECODED-SENTENCE" ), makeSymbol( "&OPTIONAL" ), makeSymbol( "CSQL-CONDITIONS" ) );
    $str9$sksi_translate_logical_sentence_t = makeString( "sksi-translate-logical-sentence-to-raw-tuples -> ~S ~S~%" );
    $sym10$EQUAL_DUPLICATES_ = makeSymbol( "EQUAL-DUPLICATES?" );
    $sym11$SETS_EQUAL_EQUAL_ = makeSymbol( "SETS-EQUAL-EQUAL?" );
    $sym12$EQUALS_ = makeSymbol( "EQUALS?" );
    $sym13$LIST = makeSymbol( "LIST" );
    $kw14$RAW = makeKeyword( "RAW" );
    $sym15$FIELD_ENCODINGS_RELEVANT_TO_LOGICAL_FIELD_INDEXICAL = makeSymbol( "FIELD-ENCODINGS-RELEVANT-TO-LOGICAL-FIELD-INDEXICAL" );
    $const16$fieldEncoding = constant_handles.reader_make_constant_shell( makeString( "fieldEncoding" ) );
    $kw17$WHAT = makeKeyword( "WHAT" );
    $list18 = ConsesLow.list( makeKeyword( "WHAT" ) );
    $kw19$TRUE = makeKeyword( "TRUE" );
    $kw20$GAF_ARG = makeKeyword( "GAF-ARG" );
    $kw21$GAF = makeKeyword( "GAF" );
    $kw22$PREDICATE_EXTENT = makeKeyword( "PREDICATE-EXTENT" );
    $kw23$OVERLAP = makeKeyword( "OVERLAP" );
    $const24$conditionalMeaningSentenceOfSchem = constant_handles.reader_make_constant_shell( makeString( "conditionalMeaningSentenceOfSchema" ) );
    $const25$equals = constant_handles.reader_make_constant_shell( makeString( "equals" ) );
    $kw26$IGNORE = makeKeyword( "IGNORE" );
    $sym27$LOGICAL_FIELD_INDEXICAL_SCHEMA = makeSymbol( "LOGICAL-FIELD-INDEXICAL-SCHEMA" );
    $kw28$UNREFORMULATABLE = makeKeyword( "UNREFORMULATABLE" );
    $kw29$ENCODE = makeKeyword( "ENCODE" );
    $list30 = ConsesLow.list( makeSymbol( "FIELD-ENCODING" ), makeSymbol( "PF-INDEXICAL" ) );
    $const31$TheCurrentKnowledgeSource = constant_handles.reader_make_constant_shell( makeString( "TheCurrentKnowledgeSource" ) );
    $str32$sksi_incremental_edit_reformulate = makeString( "sksi-incremental-edit-reformulate-decoded-sentence could not encode ~S as ~S using encoding ~S" );
    $const33$indexicalReferent = constant_handles.reader_make_constant_shell( makeString( "indexicalReferent" ) );
    $list34 = ConsesLow.list( makeSymbol( "TRANSLATION-PATTERN" ), makeSymbol( "&OPTIONAL" ), makeSymbol( "RELEVANT-CMSES" ) );
    $sym35$CSQL_TABLE_EXPRESSION_P = makeSymbol( "CSQL-TABLE-EXPRESSION-P" );
    $sym36$LOGICAL_FIELD_INDEXICAL_P = makeSymbol( "LOGICAL-FIELD-INDEXICAL-P" );
    $kw37$VALUE = makeKeyword( "VALUE" );
    $kw38$BIND = makeKeyword( "BIND" );
    $sym39$FORMULA_OPERATOR = makeSymbol( "FORMULA-OPERATOR" );
    $list40 = ConsesLow.list( makeSymbol( "MEANING-SENTENCE" ), makeSymbol( "&OPTIONAL" ), makeSymbol( "CMS-LIST" ) );
    $kw41$TEMPLATE = makeKeyword( "TEMPLATE" );
    $sym42$RELEVANT_LOGICAL_SCHEMA_MEANING_SENTENCES_WITH_PREDS = makeSymbol( "RELEVANT-LOGICAL-SCHEMA-MEANING-SENTENCES-WITH-PREDS" );
    $sym43$LISTP = makeSymbol( "LISTP" );
    $sym44$GAF_FORMULA = makeSymbol( "GAF-FORMULA" );
    $sym45$FAST_INTERSECT_ = makeSymbol( "FAST-INTERSECT?" );
    $sym46$MEANING_SENTENCE_LOGICAL_FIELD_INDEXICALS = makeSymbol( "MEANING-SENTENCE-LOGICAL-FIELD-INDEXICALS" );
    $sym47$FORMULA_ARG1 = makeSymbol( "FORMULA-ARG1" );
    $sym48$PHYSICAL_FIELD_INDEXICAL_LOGICAL_FIELD_INDEXICAL_MAPPINGS_GIVEN_L = makeSymbol( "PHYSICAL-FIELD-INDEXICAL-LOGICAL-FIELD-INDEXICAL-MAPPINGS-GIVEN-LOGICAL-PHYSICAL-SCHEMATA" );
    $sym49$SKSI_DECODED_SENTENCE_TO_RAW_SENTENCE_TRANSLATION_PATTERN = makeSymbol( "SKSI-DECODED-SENTENCE-TO-RAW-SENTENCE-TRANSLATION-PATTERN" );
    $sym50$LFI_FORT_FOR_LFI = makeSymbol( "LFI-FORT-FOR-LFI" );
    $const51$fieldValue = constant_handles.reader_make_constant_shell( makeString( "fieldValue" ) );
    $list52 = ConsesLow.list( makeKeyword( "NO-VALUE" ) );
    $sym53$MAYBE_EVALUATE_FIELD_TRANSLATION = makeSymbol( "MAYBE-EVALUATE-FIELD-TRANSLATION" );
    $sym54$NON_SKSI_EVALUATABLE_EXPRESSION_ = makeSymbol( "NON-SKSI-EVALUATABLE-EXPRESSION?" );
    $sym55$MAYBE_EVALUATE_EXPRESSION = makeSymbol( "MAYBE-EVALUATE-EXPRESSION" );
    $sym56$CATCH_ERROR_MESSAGE_HANDLER = makeSymbol( "CATCH-ERROR-MESSAGE-HANDLER" );
    $sym57$SKSI_RAW_SENTENCE_TO_RAW_TUPLE_TRANSLATION_PATTERN = makeSymbol( "SKSI-RAW-SENTENCE-TO-RAW-TUPLE-TRANSLATION-PATTERN" );
    $str58$_sksi_store_or_delete_raw_value__ = makeString( "(sksi-store-or-delete-raw-value  ~S ~S ~S ~S ~S ~S ~S ~S)~%" );
    $str59$sksi_store_or_delete_raw_value_CS = makeString( "sksi-store-or-delete-raw-value CSQL = ~S~%" );
    $kw60$BREAK = makeKeyword( "BREAK" );
    $sym61$VALID_SKSI_ERROR_HANDLING_MODE_P = makeSymbol( "VALID-SKSI-ERROR-HANDLING-MODE-P" );
    $str62$sksi_store_or_delete_raw_value___ = makeString( "sksi-store-or-delete-raw-value -> ~A~%" );
    $str63$Invalid_CSQL_expression_at__sksi_ = makeString( "Invalid CSQL expression at (sksi-store-or-delete-raw-value-int ~S ~S ~S) -> NIL~%" );
    $const64$Database_Physical = constant_handles.reader_make_constant_shell( makeString( "Database-Physical" ) );
    $const65$FileHashTable_File = constant_handles.reader_make_constant_shell( makeString( "FileHashTable-File" ) );
    $kw66$CACHE = makeKeyword( "CACHE" );
    $str67$_sksi_store_or_delete_raw_value_d = makeString( "(sksi-store-or-delete-raw-value-db ~S ~S)~%" );
    $str68$sksi_store_or_delete_raw_value_db = makeString( "sksi-store-or-delete-raw-value-db -> ~A~%" );
    $list69 = ConsesLow.list( makeKeyword( "INSERT" ), makeKeyword( "DELETE" ) );
    $str70$malformed_csql_subexpression__a__ = makeString( "malformed csql subexpression ~a, expected :insert or :delete" );
    $list71 = ConsesLow.list( makeSymbol( "OPERATOR-KEYWORD" ), makeSymbol( "INTO" ), makeSymbol( "FIELDS" ), makeSymbol( "FIELD-VALUES" ) );
    $str72$malformed_csql_expression__a__exp = makeString( "malformed csql expression ~a, expected :insert or :delete" );
    $list73 = ConsesLow.list( makeKeyword( "TABLE" ), makeString( "Language_Spoken" ) );
    $str74$unknown_cache_for_csql__a_and_acc = makeString( "unknown cache for csql ~a and access path ~a" );
  }
}
/*
 * 
