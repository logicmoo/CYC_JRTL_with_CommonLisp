...
public final class sksi_conjunctive_removal_modules_applicability
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject removal_sksi_conjunction_pos_lits_applicability(final SubLObject contextualized_dnf_clause, final SubLObject sks_profile)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      {
        good_subclause_specs = ConsesLow.cons( subclause_spec, good_subclause_specs );
      }
  ...
  public static SubLObject removal_sksi_conjunction_pos_lits_applicability_int(final SubLObject contextualized_dnf_clause, final SubLObject sks_profile)
  {
    ...
    try
    {
      ...
      $sksi_crm_applicability_start_time$.bind( Time.get_universal_time(), thread );
      try
      {
        thread.throwStack.push( $kw0$SKSI_CRM_APPLICABILITY_MAX_TIME );
        ...
        try
        {
          ...
          thread.resetMultipleValues();
          if( NIL != candidate_lit_nums && ( NIL != list_utilities.lengthGE( candidate_lit_nums, sksi_macros.$sksi_conjunction_pos_lits_applicability_minimum_candidate_literal_count$.getDynamicValue( thread ),
              UNPROVIDED ) || NIL != comparison_lit_nums || NIL != evaluate_lit_nums || NIL != sksi_unknown_sentence_lit_nums ) )
          {
            final SubLObject pos_subclause_specs = sksi_crm_gather_literals( candidate_lit_nums, comparison_lit_nums, evaluate_lit_nums, sksi_unknown_sentence_lit_nums, contextualized_dnf_clause,
                meaning_sentence_gaf_iterators, sks_profile );
            if( NIL != pos_subclause_specs && NIL == sksi_debugging.sksi_ignore_errorsP() )
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                if( NIL == list_utilities.member_eqP( lit_num, pos_subclause_specs.first() ) )
                {
                  ...
                  {
                    sksi_debugging.sksi_error( Sequences.cconcatenate( $str1$SKSI_conjunctive_removal_was_appl, new SubLObject[] { format_nil.format_nil_a_no_copy( ( NIL != inference_worker.currently_active_problem() )
                        ? inference_datastructures_problem.problem_suid( inference_worker.currently_active_problem() )
                        : $kw2$UNKNOWN ), $str3$_but_was_lacking_this___sksiUnkno, format_nil.format_nil_a_no_copy( asent )
                    } ) );
                  }
                }
                cdolist_list_var = cdolist_list_var.rest();
            ...
            subclause_specs = Mapping.mapcar( $sym4$NEW_POS_SUBCLAUSE_SPEC, pos_subclause_specs );
          }
        }
        finally
        {
      ...
      {
        max_time_cutoff_reached = Errors.handleThrowable( ccatch_env_var, $kw0$SKSI_CRM_APPLICABILITY_MAX_TIME );
      }
      finally
      {
  ...
  public static SubLObject sksi_crm_get_subclause_candidates(final SubLObject contextualized_dnf_clause, final SubLObject sks_profile)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL == current )
      {
        if( NIL != hlmt.hlmt_equalP( mt, content_mt ) || NIL != sksi_infrastructure_utilities.mt_relevant_to_mt_memoizedP( mt, content_mt, UNPROVIDED ) )
        {
          ...
          {
            if( NIL == list_utilities.member_eqP( el_utilities.literal_predicate( asent, UNPROVIDED ), forbidden_comp_ops ) )
            {
              comparison_lit_nums = ConsesLow.cons( num, comparison_lit_nums );
            }
            meaning_sentence_gaf_iterators = ConsesLow.cons( NIL, meaning_sentence_gaf_iterators );
          }
          else if( NIL != sksi_query_utilities.evaluate_literal_p( asent ) )
          {
            if( NIL != sks_supports_evaluation_asentsP( sks ) && NIL != sksi_supported_evaluate_asentP( asent ) )
            {
              evaluate_lit_nums = ConsesLow.cons( num, evaluate_lit_nums );
            }
            meaning_sentence_gaf_iterators = ConsesLow.cons( NIL, meaning_sentence_gaf_iterators );
          }
          else if( NIL != sksi_query_utilities.sksi_unknown_sentence_literal_p( asent ) )
          {
            sksi_unknown_sentence_lit_nums = ConsesLow.cons( num, sksi_unknown_sentence_lit_nums );
            meaning_sentence_gaf_iterators = ConsesLow.cons( NIL, meaning_sentence_gaf_iterators );
          }
          else
          {
            final SubLObject lit_meaning_sentence_gaf_iterator = sksi_conjunctive_removal_module_utilities.get_meaning_sentence_gaf_iterator_for_asent( asent, logical_schemata, sksi_query_datastructures.sks_profile_sks(
                sks_profile ), T, mt, UNPROVIDED, UNPROVIDED );
            meaning_sentence_gaf_iterators = ConsesLow.cons( lit_meaning_sentence_gaf_iterator, meaning_sentence_gaf_iterators );
            if( NIL != lit_meaning_sentence_gaf_iterator )
            {
              candidate_lit_nums = ConsesLow.cons( num, candidate_lit_nums );
            }
          }
        }
        else
    ...
    return Values.values( Sequences.nreverse( candidate_lit_nums ), Sequences.nreverse( meaning_sentence_gaf_iterators ), Sequences.nreverse( comparison_lit_nums ), Sequences.nreverse( evaluate_lit_nums ), Sequences
        .nreverse( sksi_unknown_sentence_lit_nums ) );
  }
  ...
  {
    return makeBoolean( NIL == kb_utilities.kbeq( $const6$RDFTripleStore_Physical, sksi_kb_accessors.sk_source_ks_type( sks ) ) );
  }
  ...
  {
    return cycl_utilities.expression_find_if( $sym7$SKSI_SUPPORTED_FUNCTION_P, asent, T, UNPROVIDED );
  }
  ...
  public static SubLObject sksi_crm_fully_forbidden_comparison_operators_internal(final SubLObject physical_schemata)
  {
    ...
    if( NIL == doneP )
    {
      ...
      while ( NIL == doneP && NIL != csome_list_var)
      {
        ...
        comparison_operators = keyhash_utilities.fast_intersection( comparison_operators, ps_comparison_operators, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject sksi_crm_fully_forbidden_comparison_operators(final SubLObject physical_schemata)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym8$SKSI_CRM_FULLY_FORBIDDEN_COMPARISON_OPERATORS, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym8$SKSI_CRM_FULLY_FORBIDDEN_COMPARISON_OPERATORS, ONE_INTEGER, NIL, EQUAL, UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym8$SKSI_CRM_FULLY_FORBIDDEN_COMPARISON_OPERATORS, caching_state );
    }
    ...
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( sksi_crm_fully_forbidden_comparison_operators_internal( physical_schemata ) ) );
  ...
  public static SubLObject with_sksi_crm_pos_subclause_spec_gathering(final SubLObject macroform, final SubLObject environment)
  {
    ...
    if( NIL == current )
    {
      ...
      current = ( body = temp );
      return ConsesLow.listS( $sym10$CLET, $list11, ConsesLow.append( body, ConsesLow.list( ConsesLow.listS( $sym12$CSETQ, result_var, $list13 ) ) ) );
    }
  ...
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL == conses_high.member( pos_subclause_spec, $sksi_crm_gathered_pos_subclause_specs$.getDynamicValue( thread ), Symbols.symbol_function( EQUAL ), Symbols.symbol_function( IDENTITY ) ) )
    {
      $sksi_crm_gathered_pos_subclause_specs$.setDynamicValue( ConsesLow.cons( pos_subclause_spec, $sksi_crm_gathered_pos_subclause_specs$.getDynamicValue( thread ) ), thread );
    }
    return NIL;
  }
  ...
  public static SubLObject sksi_crm_pos_subclause_spec_irrelevantP(final SubLObject pos_subclause_spec)
  {
    ...
    {
      if( NIL != subl_promotions.memberP( pos_subclause_spec, $sksi_crm_gathered_pos_subclause_specs$.getDynamicValue( thread ), Symbols.symbol_function( $sym14$SUBSETP ), UNPROVIDED ) )
      {
        return T;
      }
    }
    else if( NIL != subl_promotions.memberP( pos_subclause_spec, $sksi_crm_gathered_pos_subclause_specs$.getDynamicValue( thread ), Symbols.symbol_function( EQUAL ), UNPROVIDED ) )
    {
    }
    return NIL;
  }
  ...
  public static SubLObject sksi_crm_gather_literals(final SubLObject candidate_lit_nums, final SubLObject comparison_lit_nums, final SubLObject evaluate_lit_nums, final SubLObject sksi_unknown_sentence_lit_nums,
      final SubLObject contextualized_dnf_clause, final SubLObject meaning_sentence_gaf_iterators, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    if( NIL != sksi_macros.$sksi_crm_only_total_subclause_specs_relevantP$.getDynamicValue( thread ) && !Sequences.length( pos_subclause_spec ).eql( Sequences.length( clauses.pos_lits( contextualized_dnf_clause ) ) ) )
    {
      return NIL;
    }
    final SubLObject islands = list_utilities.nmapcar( $sym15$NCANONICALIZE_LITERAL_INDICES, sksi_crm_form_islands( pos_subclause_spec, contextualized_dnf_clause, UNPROVIDED ) );
    ...
    try
    {
      ...
      $sksi_crm_rejected_pos_subclause_specs$.bind( dictionary.new_dictionary( Symbols.symbol_function( EQUAL ), UNPROVIDED ), thread );
      ...
      result = ( ( NIL != $sksi_crm_ignore_subsumed_subclause_specsP$.getDynamicValue( thread ) ) ? list_utilities.delete_subsumed_items( $sksi_crm_gathered_pos_subclause_specs$.getDynamicValue( thread ),
          $sym16$SUPERSETP, UNPROVIDED ) : $sksi_crm_gathered_pos_subclause_specs$.getDynamicValue( thread ) );
    }
    finally
    {
  ...
  public static SubLObject sksi_crm_gather_literals_iterative(final SubLObject initial_pos_subclause_spec, final SubLObject candidate_lit_nums, final SubLObject comparison_lit_nums, final SubLObject evaluate_lit_nums,
      final SubLObject sksi_unknown_sentence_lit_nums, final SubLObject contextualized_dnf_clause, final SubLObject meaning_sentence_gaf_iterators, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    while ( NIL == queues.p_queue_empty_p( pos_subclause_specs_queue ) && ( NIL == $sksi_crm_gather_literals_iterative_method_return_first_successP$.getDynamicValue( thread ) || NIL == number_utilities.positive_number_p(
        max_added_subclause_spec_length ) ))
    {
      ...
      try
      {
        ...
        else if( NIL != sksi_crm_valid_queryP( pos_subclause_spec, candidate_lit_nums, comparison_lit_nums, evaluate_lit_nums, sksi_unknown_sentence_lit_nums, contextualized_dnf_clause, meaning_sentence_gaf_iterators,
            sks_profile ) )
        {
        ...
        if( NIL != sksi_macros.$sksi_crm_only_total_subclause_specs_relevantP$.getDynamicValue( thread ) || NIL != add_smaller_specs_to_queueP || ( NIL != $sksi_crm_ignore_subsumed_subclause_specsP$.getDynamicValue(
            thread ) && max_added_subclause_spec_length.numG( Sequences.length( pos_subclause_spec ) ) ) || ( NIL != $sksi_crm_gather_literals_stop_if_total_subclause_specs_fails_enumerability_optimizationP$
                .getDynamicValue( thread ) && NIL != list_utilities.same_length_p( pos_subclause_spec, initial_pos_subclause_spec ) && NIL != $sksi_crm_gather_literals_subclause_specs_failed_due_to_enumerabilityP$
        ...
        while ( NIL != cdolist_list_var)
        {
          final SubLObject test_spec = Sequences.remove( num, pos_subclause_spec, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
          SubLObject cdolist_list_var_$3;
          final SubLObject islands = cdolist_list_var_$3 = list_utilities.nmapcar( $sym15$NCANONICALIZE_LITERAL_INDICES, sksi_crm_form_islands( test_spec, contextualized_dnf_clause, UNPROVIDED ) );
          SubLObject island = NIL;
          island = cdolist_list_var_$3.first();
          while ( NIL != cdolist_list_var_$3)
          {
            if( NIL == queues.p_queue_full_p( pos_subclause_specs_queue ) && NIL == queues.p_queue_find( island, pos_subclause_specs_queue, Symbols.symbol_function( EQUAL ), UNPROVIDED ) )
            {
  ...
  public static SubLObject sksi_crm_gather_literals_recursive(final SubLObject pos_subclause_spec, final SubLObject candidate_lit_nums, final SubLObject comparison_lit_nums, final SubLObject evaluate_lit_nums,
      final SubLObject sksi_unknown_sentence_lit_nums, final SubLObject contextualized_dnf_clause, final SubLObject meaning_sentence_gaf_iterators, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( NIL != sksi_crm_pos_subclause_spec_irrelevantP( pos_subclause_spec ) )
    ...
    if( NIL != sksi_crm_valid_queryP( pos_subclause_spec, candidate_lit_nums, comparison_lit_nums, evaluate_lit_nums, sksi_unknown_sentence_lit_nums, contextualized_dnf_clause, meaning_sentence_gaf_iterators,
        sks_profile ) )
    {
      sksi_crm_add_pos_subclause_spec( pos_subclause_spec );
      if( NIL == $sksi_crm_ignore_subsumed_subclause_specsP$.getDynamicValue( thread ) )
      {
        sksi_crm_gather_literals_recursive_continue_recursion( pos_subclause_spec, candidate_lit_nums, comparison_lit_nums, evaluate_lit_nums, sksi_unknown_sentence_lit_nums, contextualized_dnf_clause,
            meaning_sentence_gaf_iterators, sks_profile );
      }
      return NIL;
    }
  ...
  public static SubLObject sksi_crm_gather_literals_recursive_continue_recursion(final SubLObject pos_subclause_spec, final SubLObject candidate_lit_nums, final SubLObject comparison_lit_nums,
      final SubLObject evaluate_lit_nums, final SubLObject sksi_unknown_sentence_lit_nums, final SubLObject contextualized_dnf_clause, final SubLObject meaning_sentence_gaf_iterators, final SubLObject sks_profile)
  {
    SubLObject cdolist_list_var = pos_subclause_spec;
    SubLObject num = NIL;
    num = cdolist_list_var.first();
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject islands = cdolist_list_var_$4 = list_utilities.nmapcar( $sym15$NCANONICALIZE_LITERAL_INDICES, sksi_crm_form_islands( test_spec, contextualized_dnf_clause, UNPROVIDED ) );
      SubLObject island = NIL;
  ...
  public static SubLObject sksi_crm_valid_queryP(final SubLObject pos_subclause_spec, final SubLObject candidate_lit_nums, final SubLObject comparison_lit_nums, final SubLObject evaluate_lit_nums,
      final SubLObject sksi_unknown_sentence_lit_nums, final SubLObject contextualized_dnf_clause, final SubLObject meaning_sentence_gaf_iterators, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    if( ( NIL != inference_worker.currently_active_problem() && NIL != inference_strategist.current_controlling_inference_max_time_reachedP() ) || ( NIL != number_utilities.positive_number_p(
        $sksi_crm_applicability_max_time_override$.getDynamicValue( thread ) ) && NIL != number_utilities.positive_number_p( $sksi_crm_applicability_start_time$.getDynamicValue( thread ) ) && Numbers.add(
            $sksi_crm_applicability_start_time$.getDynamicValue( thread ), $sksi_crm_applicability_max_time_override$.getDynamicValue( thread ) ).numLE( Time.get_universal_time() ) ) )
    {
      Dynamic.sublisp_throw( $kw0$SKSI_CRM_APPLICABILITY_MAX_TIME, Numbers.subtract( Time.get_universal_time(), $sksi_crm_applicability_start_time$.getDynamicValue( thread ) ) );
    }
    final SubLObject candidate_spec = list_utilities.ordered_intersection( candidate_lit_nums, pos_subclause_spec, Symbols.symbol_function( $sym18$_ ), UNPROVIDED );
    final SubLObject comparison_spec = list_utilities.ordered_intersection( comparison_lit_nums, pos_subclause_spec, Symbols.symbol_function( $sym18$_ ), UNPROVIDED );
    final SubLObject evaluate_spec = list_utilities.ordered_intersection( evaluate_lit_nums, pos_subclause_spec, Symbols.symbol_function( $sym18$_ ), UNPROVIDED );
    final SubLObject sksi_unknown_sentence_spec = list_utilities.ordered_intersection( sksi_unknown_sentence_lit_nums, pos_subclause_spec, Symbols.symbol_function( $sym18$_ ), UNPROVIDED );
  ...
  public static SubLObject sksi_crm_valid_query_int(final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents,
      final SubLObject meaning_sentence_gaf_iterators, final SubLObject sks_profile)
  {
  ...
  public static SubLObject sksi_crm_applicability_iterate_over_meaning_sentence_gafs(final SubLObject meaning_sentence_gafs_so_far, final SubLObject remaining_meaning_sentence_gaf_iterators, final SubLObject lits_so_far,
      final SubLObject remaining_lits, final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    else
    {
      ...
      if( NIL != iterator )
      {
        ...
        if( NIL == current )
        {
          ...
          for( SubLObject done_var = successP; NIL == done_var; done_var = makeBoolean( NIL == valid || NIL != successP ) )
          {
            ...
            thread.resetMultipleValues();
            if( NIL != valid && NIL != sksi_crm_literal_coherentP( lit, mt, ConsesLow.list( gaf ), physical_schemata, sksi_query_datastructures.sks_profile_sks( sks_profile ) )
                && NIL != sksi_conjunctive_removal_module_utilities.sksi_crm_literal_coherent_with_other_literalsP( asent, gaf, lits_so_far, meaning_sentence_gafs_so_far, physical_schemata ) )
            {
              successP = sksi_crm_applicability_iterate_over_meaning_sentence_gafs( list_utilities.snoc( gaf, meaning_sentence_gafs_so_far ), remaining_meaning_sentence_gaf_iterators.rest(), list_utilities.snoc( asent,
                  lits_so_far ), remaining_lits.rest(), asents, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents, sks_profile );
            }
          }
      ...
      {
        successP = sksi_crm_applicability_iterate_over_meaning_sentence_gafs( list_utilities.snoc( NIL, meaning_sentence_gafs_so_far ), remaining_meaning_sentence_gaf_iterators.rest(), lits_so_far, remaining_lits.rest(),
            asents, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents, sks_profile );
      }
    }
    return successP;
  }
  ...
  public static SubLObject sksi_crm_applicability_process_meaning_sentence_gafs(final SubLObject meaning_sentence_gafs, final SubLObject lits, final SubLObject comparison_asents, final SubLObject evaluate_asents,
      final SubLObject sksi_unknown_sentence_asents, final SubLObject sks_profile)
  {
    ...
    else
    {
      ...
      list_var = lits;
      lit = list_var.first();
      for( num = ZERO_INTEGER; NIL != list_var; list_var = list_var.rest(), lit = list_var.first(), num = Numbers.add( ONE_INTEGER, num ) )
      {
        SubLObject current;
        final SubLObject datum = current = lit;
        SubLObject mt = NIL;
        ...
        if( NIL == current )
        {
          ...
          if( NIL != meaning_sentence_gaf )
          {
            ...
            if( NIL != sksi_conjunctive_removal_module_utilities.variable_predicate_litP( asent ) )
            {
              ...
              ConsesLow.set_nth( ZERO_INTEGER, keywords, ConsesLow.cons( $kw20$VARPRED, ConsesLow.nth( ZERO_INTEGER, keywords ) ) );
            }
            else
            {
              ...
              while ( NIL != cdolist_list_var)
              {
                ...
                ConsesLow.set_nth( ZERO_INTEGER, keywords, ConsesLow.cons( $kw21$GENLPREDS, ConsesLow.nth( ZERO_INTEGER, keywords ) ) );
                cdolist_list_var = cdolist_list_var.rest();
                spec_pred = cdolist_list_var.first();
              }
              ...
              while ( NIL != cdolist_list_var)
              {
                ...
                final SubLObject new_lit = inference_datastructures_problem_query.make_contextualized_asent( mt, new_asent );
                ConsesLow.set_nth( ZERO_INTEGER, asents, ConsesLow.cons( new_lit, ConsesLow.nth( ZERO_INTEGER, asents ) ) );
                ConsesLow.set_nth( ZERO_INTEGER, keywords, ConsesLow.cons( $kw22$GENLINVERSE, ConsesLow.nth( ZERO_INTEGER, keywords ) ) );
                cdolist_list_var = cdolist_list_var.rest();
                spec_inverse = cdolist_list_var.first();
              }
            }
  ...
  public static SubLObject sksi_crm_applicability_iterate_over_asents(final SubLObject asents_so_far, final SubLObject remaining_asents, final SubLObject keywords_so_far, final SubLObject remaining_keywords,
      final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents, final SubLObject meaning_sentence_gafs, final SubLObject sks_profile)
  {
    SubLObject successP = NIL;
    if( NIL == remaining_asents )
    {
      successP = makeBoolean( NIL != sksi_conjunctive_removal_module_utilities.sksi_crm_joinable_asentsP( asents_so_far, meaning_sentence_gafs, sksi_query_datastructures.sks_profile_physical_schemata( sks_profile ) )
          && NIL != sksi_crm_asents_generate_sqlP( asents_so_far, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents, meaning_sentence_gafs, sks_profile ) );
    }
    else if( NIL == successP )
    {
      ...
      for( asent = NIL, asent_$5 = NIL, keyword = NIL, keyword_$6 = NIL, asent = remaining_asents.first(), asent_$5 = asent.first(), keyword = remaining_keywords.first(), keyword_$6 = keyword.first(); NIL == successP
          && ( NIL != keyword || NIL != asent ); successP = sksi_crm_applicability_iterate_over_asents( list_utilities.snoc( asent_$5, asents_so_far ), remaining_asents.rest(), list_utilities.snoc( keyword_$6,
              keywords_so_far ), remaining_keywords.rest(), comparison_asents, evaluate_asents, sksi_unknown_sentence_asents, meaning_sentence_gafs, sks_profile ), asent = asent.rest(), asent_$5 = asent
                  .first(), keyword = keyword.rest(), keyword_$6 = keyword.first() )
      {
      }
    }
    return successP;
  ...
  public static SubLObject sksi_crm_asents_generate_csqlP(final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents,
      final SubLObject meaning_sentence_gafs, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    try
    {
      ...
      error_message = Values.nth_value_step_2( Values.nth_value_step_1( THREE_INTEGER ), sksi_conjunctive_removal_module_utilities.sksi_crm_generate_csql_for_query( asents, comparison_asents, evaluate_asents,
          sksi_unknown_sentence_asents, meaning_sentence_gafs, sks_profile ) );
    }
    finally
    {
    ...
    {
      PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str23$__SKSI__A_, $str24$Could_not_form_a_CSQL_query_with_ ), new SubLObject[] { FIVE_INTEGER,
        meaning_sentence_gafs, ConsesLow.append( asents, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents ), error_message
      } );
  ...
  public static SubLObject sksi_crm_asents_generate_sqlP(final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents,
      final SubLObject meaning_sentence_gafs, final SubLObject sks_profile)
  {
    final SubLThread thread = SubLProcess.currentSubLThread();
    ...
    try
    {
      ...
      thread.resetMultipleValues();
      if( NIL == error_message && NIL != kb_utilities.kbeq( $const25$Database_Physical, sksi_query_datastructures.sks_profile_type( sks_profile ) ) )
      {
        try
        {
          ...
          try
          {
            ...
            try
            {
              ...
              if( NIL == sql )
              {
                ...
                {
                  PrintLow.format( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ), Sequences.cconcatenate( $str23$__SKSI__A_, $str27$Could_not_form_a_SQL_query_with__ ), new SubLObject[] { FIVE_INTEGER,
                    meaning_sentence_gafs, ConsesLow.append( asents, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents ), error_message
                  } );
                  streams_high.force_output( sksi_debugging.$sksi_trace_stream$.getDynamicValue( thread ) );
                }
  ...
  public static SubLObject sksi_crm_form_islands(final SubLObject pos_lits_subclause_spec, final SubLObject contextualized_dnf_clause, SubLObject fully_bound_islandP)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      final SubLObject lit_vars = clause_literal_variables( contextualized_dnf_clause, $kw29$POS, lit_num );
      vars = ConsesLow.append( vars, lit_vars, NIL );
      prepared_spec = ConsesLow.cons( ConsesLow.list( lit_num, lit_vars ), prepared_spec );
      cdolist_list_var = cdolist_list_var.rest();
      lit_num = cdolist_list_var.first();
    }
    vars = ConsesLow.cons( NIL, list_utilities.fast_delete_duplicates( vars, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED ) );
    thread.resetMultipleValues();
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      final SubLObject island = Mapping.mapcar( Symbols.symbol_function( $sym30$FIRST ), pre_island );
    ...
    while ( NIL != cdolist_list_var2)
    {
      ...
      final SubLObject island2 = Mapping.mapcar( Symbols.symbol_function( $sym30$FIRST ), pre_island2 );
    ...
    {
      islands = ConsesLow.cons( nil_islands, islands );
    }
  ...
  public static SubLObject sksi_crm_literal_coherent(final SubLObject lit, final SubLObject mt, final SubLObject lit_meaning_sentence_gafs, final SubLObject physical_schemata, final SubLObject sks)
  {
    ...
    final SubLObject lit_vars = cycl_utilities.expression_gather( lit, $sym32$VARIABLE_P, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED, UNPROVIDED );
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      if( NIL != sksi_conjunctive_removal_module_utilities.genl_pred_meaning_sentenceP( meaning_sentence, lit, mt, UNPROVIDED ) && NIL == genl_pred_incoherentP )
      {
        ...
        {
          final SubLObject var_positions = list_utilities.tree_positions( lit_var, lit, Symbols.symbol_function( EQ ), UNPROVIDED );
      ...
      if( NIL != sksi_conjunctive_removal_module_utilities.genl_inverse_meaning_sentenceP( meaning_sentence, lit, mt, UNPROVIDED ) && NIL == genl_inverse_incoherentP )
      {
        ...
        {
          final SubLObject var_positions = list_utilities.tree_positions( lit_var, inverse_lit, Symbols.symbol_function( EQ ), UNPROVIDED );
          if( NIL == list_utilities.singletonP( var_positions ) && NIL == sksi_crm_literal_coherent_int( meaning_sentence, var_positions, physical_schemata ) )
          {
      ...
      {
        new_lit_meaning_sentence_gafs = ConsesLow.cons( meaning_sentence_gaf, new_lit_meaning_sentence_gafs );
      }
      cdolist_list_var = cdolist_list_var.rest();
      meaning_sentence_gaf = cdolist_list_var.first();
    }
  ...
  public static SubLObject sksi_crm_literal_coherent_int(final SubLObject meaning_sentence, final SubLObject positions, final SubLObject physical_schemata)
  {
    ...
    {
      final SubLObject term0 = Sequences.find_if( $sym33$LOGICAL_FIELD_INDEXICAL_P, meaning_sentence_terms, UNPROVIDED, UNPROVIDED, UNPROVIDED );
      if( NIL != term0 )
      {
        final SubLObject other_terms = Sequences.remove( term0, meaning_sentence_terms, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  ...
  public static SubLObject sksi_crm_literals_coherent_together(final SubLObject lit0, final SubLObject lit0_meaning_sentence_gafs, final SubLObject lit1, final SubLObject lit1_meaning_sentence_gafs,
  ...
  public static SubLObject sksi_crm_literals_coherent_together_int(final SubLObject lit0, final SubLObject inverse_lit0, final SubLObject lit1, final SubLObject inverse_lit1, final SubLObject lit0_meaning_sentence_gaf,
      final SubLObject lit1_meaning_sentence_gaf, final SubLObject physical_schemata)
  {
    ...
    SubLObject coherent = NIL;
    if( NIL != sksi_conjunctive_removal_module_utilities.variable_predicate_litP( lit0 ) || NIL != sksi_meaning_sentence_utilities.meaning_sentence_with_logical_field_indexical_predicateP( lit0_meaning_sentence )
        || NIL != sksi_conjunctive_removal_module_utilities.genl_pred_meaning_sentenceP( lit0_meaning_sentence, lit0, UNPROVIDED, UNPROVIDED ) )
    {
      if( ( NIL != sksi_conjunctive_removal_module_utilities.variable_predicate_litP( lit1 ) || NIL != sksi_meaning_sentence_utilities.meaning_sentence_with_logical_field_indexical_predicateP( lit1_meaning_sentence )
          || NIL != sksi_conjunctive_removal_module_utilities.genl_pred_meaning_sentenceP( lit1_meaning_sentence, lit1, UNPROVIDED, UNPROVIDED ) ) && NIL == sksi_conjunctive_removal_module_utilities
              .sksi_crm_literals_incoherent_together( lit0, lit0_meaning_sentence, lit1, lit1_meaning_sentence, physical_schemata ) )
      {
        coherent = ConsesLow.cons( ConsesLow.list( lit0_meaning_sentence_gaf, lit1_meaning_sentence_gaf ), coherent );
      }
      if( NIL != sksi_conjunctive_removal_module_utilities.genl_inverse_meaning_sentenceP( lit1_meaning_sentence, lit1, UNPROVIDED, UNPROVIDED ) && NIL == sksi_conjunctive_removal_module_utilities
          .sksi_crm_literals_incoherent_together( lit0, lit0_meaning_sentence, inverse_lit1, lit1_meaning_sentence, physical_schemata ) )
      {
        coherent = ConsesLow.cons( ConsesLow.list( lit0_meaning_sentence_gaf, lit1_meaning_sentence_gaf ), coherent );
      }
    }
    if( NIL != sksi_conjunctive_removal_module_utilities.genl_inverse_meaning_sentenceP( lit0_meaning_sentence, lit0, UNPROVIDED, UNPROVIDED ) )
    {
      if( ( NIL != sksi_conjunctive_removal_module_utilities.variable_predicate_litP( lit1 ) || NIL != sksi_meaning_sentence_utilities.meaning_sentence_with_logical_field_indexical_predicateP( lit1_meaning_sentence )
          || NIL != sksi_conjunctive_removal_module_utilities.genl_pred_meaning_sentenceP( lit1_meaning_sentence, lit1, UNPROVIDED, UNPROVIDED ) ) && NIL == sksi_conjunctive_removal_module_utilities
              .sksi_crm_literals_incoherent_together( inverse_lit0, lit0_meaning_sentence, lit1, lit1_meaning_sentence, physical_schemata ) )
      {
        coherent = ConsesLow.cons( ConsesLow.list( lit0_meaning_sentence_gaf, lit1_meaning_sentence_gaf ), coherent );
      }
      if( NIL != sksi_conjunctive_removal_module_utilities.genl_inverse_meaning_sentenceP( lit1_meaning_sentence, lit1, UNPROVIDED, UNPROVIDED ) && NIL == sksi_conjunctive_removal_module_utilities
          .sksi_crm_literals_incoherent_together( inverse_lit0, lit0_meaning_sentence, inverse_lit1, lit1_meaning_sentence, physical_schemata ) )
      {
        coherent = ConsesLow.cons( ConsesLow.list( lit0_meaning_sentence_gaf, lit1_meaning_sentence_gaf ), coherent );
      }
    }
  ...
  public static SubLObject sksi_crm_enumerability_check_asents(final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents,
      final SubLObject meaning_sentence_gafs)
  {
    return makeBoolean( ( NIL == sksi_crm_fully_unbound_asentsP( asents, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents, meaning_sentence_gafs ) || NIL != sksi_crm_enumerability_check(
        meaning_sentence_gafs ) ) && ( NIL == sksi_unknown_sentence_asents || NIL == sksi_crm_unbound_unknown_sentence_asentsP( asents, comparison_asents, evaluate_asents, sksi_unknown_sentence_asents,
  ...
  public static SubLObject sksi_crm_unbound_unknown_sentence_asentsP(final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents,
      final SubLObject meaning_sentence_gafs)
  {
    SubLObject unboundP = NIL;
    final SubLObject all_asent_vars = ConsesLow.append( Mapping.mapcan( $sym34$CONTEXTUALIZED_LITERAL_HL_VARIABLES, asents, EMPTY_SUBL_OBJECT_ARRAY ), Mapping.mapcan( $sym34$CONTEXTUALIZED_LITERAL_HL_VARIABLES,
        comparison_asents, EMPTY_SUBL_OBJECT_ARRAY ), Mapping.mapcan( $sym34$CONTEXTUALIZED_LITERAL_HL_VARIABLES, evaluate_asents, EMPTY_SUBL_OBJECT_ARRAY ) );
    if( NIL == unboundP )
    {
  ...
  public static SubLObject sksi_crm_enumerability_check(final SubLObject meaning_sentence_gafs)
  {
    ...
    if( NIL == non_enumerableP )
    {
      ...
      {
        final SubLObject pred_var = $const35$enumerableSchema;
        if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( physical_schema, ONE_INTEGER, pred_var ) )
        {
          ...
          while ( NIL == done_var)
          {
            ...
            if( NIL != valid )
            {
              ...
              try
              {
                final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw36$GAF, $kw37$FALSE, NIL );
  ...
  public static SubLObject sksi_crm_fully_unbound_asentsP(final SubLObject asents, final SubLObject comparison_asents, final SubLObject evaluate_asents, final SubLObject sksi_unknown_sentence_asents,
      final SubLObject meaning_sentence_gafs)
  {
    SubLObject boundP = list_utilities.any_in_list( $sym38$SKSI_CRM_ANY_FULLY_BOUND_ARG_, comparison_asents, $sym39$CONTEXTUALIZED_ASENT_ASENT );
  ...
  {
    return list_utilities.any_in_list( $sym40$FULLY_BOUND_P, cycl_utilities.formula_args( formula, UNPROVIDED ), UNPROVIDED );
  }
  ...
  public static SubLObject sksi_crm_fully_unbound_for_meaning_sentence_int(final SubLObject formula_tree, final SubLObject ms_tree)
  {
    ...
    else if( ms_tree.isCons() && NIL == boundP )
    {
  ...
  public static SubLObject collect_physical_schemata_from_meaning_sentence_gafs(final SubLObject meaning_sentence_gafs)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      if( NIL != sksi_meaning_sentence_utilities.gaf_assertion_is_conditional_meaning_sentence_gafP( meaning_sentence_gaf ) )
      {
        ...
        physical_schemata = ConsesLow.nconc( new SubLObject[] { physical_schemata, Mapping.mapcan( $sym41$LOGICAL_SCHEMA_PHYSICAL_SCHEMATA, ant_lses, EMPTY_SUBL_OBJECT_ARRAY ), Mapping.mapcan(
            $sym41$LOGICAL_SCHEMA_PHYSICAL_SCHEMATA, con_lses, EMPTY_SUBL_OBJECT_ARRAY ), collect_physical_schemata_from_meaning_sentence( ant_ms ), collect_physical_schemata_from_meaning_sentence( con_ms )
        } );
      }
      else
      {
    ...
    return list_utilities.fast_delete_duplicates( physical_schemata, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
  }
  ...
  {
    final SubLObject lfis = cycl_utilities.expression_gather( meaning_sentence, $sym33$LOGICAL_FIELD_INDEXICAL_P, T, Symbols.symbol_function( EQUAL ), UNPROVIDED, UNPROVIDED );
    SubLObject physical_schemata = NIL;
  ...
  public static SubLObject collect_physical_schemata_from_logical_field_indexical_internal(final SubLObject logical_field_indexical)
  {
    ...
    SubLObject physical_schemata = NIL;
    final SubLObject pred_var = $const43$logicalPhysicalFieldMap;
    if( NIL != kb_mapping_macros.do_gaf_arg_index_key_validator( logical_field, ONE_INTEGER, pred_var ) )
    {
      ...
      while ( NIL == done_var)
      {
        ...
        if( NIL != valid )
        {
          ...
          try
          {
            final_index_iterator = kb_mapping_macros.new_final_index_iterator( final_index_spec, $kw36$GAF, $kw44$TRUE, NIL );
            ...
            while ( NIL == done_var_$16)
            {
              ...
              if( NIL != valid_$18 )
              {
                ...
                physical_schemata = ConsesLow.cons( physical_schema, physical_schemata );
              }
  ...
  public static SubLObject collect_physical_schemata_from_logical_field_indexical(final SubLObject logical_field_indexical)
  {
    ...
    caching_state = memoization_state.memoization_state_lookup( v_memoization_state, $sym42$COLLECT_PHYSICAL_SCHEMATA_FROM_LOGICAL_FIELD_INDEXICAL, UNPROVIDED );
    if( NIL == caching_state )
    {
      caching_state = memoization_state.create_caching_state( memoization_state.memoization_state_lock( v_memoization_state ), $sym42$COLLECT_PHYSICAL_SCHEMATA_FROM_LOGICAL_FIELD_INDEXICAL, ONE_INTEGER, NIL, EQUAL,
          UNPROVIDED );
      memoization_state.memoization_state_put( v_memoization_state, $sym42$COLLECT_PHYSICAL_SCHEMATA_FROM_LOGICAL_FIELD_INDEXICAL, caching_state );
    }
    ...
    if( results.eql( memoization_state.$memoized_item_not_found$.getGlobalValue() ) )
    {
      results = Values.arg2( thread.resetMultipleValues(), Values.multiple_value_list( collect_physical_schemata_from_logical_field_indexical_internal( logical_field_indexical ) ) );
  ...
  public static SubLObject contextualized_literal_variables(final SubLObject lit)
  {
    ...
    return cycl_utilities.expression_gather( asent, $sym45$CYC_VAR_, NIL, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED );
  }
  ...
  {
    final SubLObject asent = inference_datastructures_problem_query.contextualized_asent_asent( lit );
    return cycl_utilities.expression_gather( asent, $sym46$HL_VAR_, NIL, Symbols.symbol_function( EQ ), UNPROVIDED, UNPROVIDED );
  }

  public static SubLObject declare_sksi_conjunctive_removal_modules_applicability_file()
  {
    SubLFiles.declareFunction( me, "removal_sksi_conjunction_pos_lits_applicability", "REMOVAL-SKSI-CONJUNCTION-POS-LITS-APPLICABILITY", 2, 0, false );
    SubLFiles.declareFunction( me, "removal_sksi_conjunction_pos_lits_applicability_int", "REMOVAL-SKSI-CONJUNCTION-POS-LITS-APPLICABILITY-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_profile_relevantP", "SKSI-CRM-PROFILE-RELEVANT?", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_sks_relevantP", "SKSI-CRM-SKS-RELEVANT?", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_genl_ks", "SKSI-CRM-GENL-KS", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_get_subclause_candidates", "SKSI-CRM-GET-SUBCLAUSE-CANDIDATES", 2, 0, false );
    SubLFiles.declareFunction( me, "sks_supports_evaluation_asentsP", "SKS-SUPPORTS-EVALUATION-ASENTS?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_supported_evaluate_asentP", "SKSI-SUPPORTED-EVALUATE-ASENT?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_fully_forbidden_comparison_operators_internal", "SKSI-CRM-FULLY-FORBIDDEN-COMPARISON-OPERATORS-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_fully_forbidden_comparison_operators", "SKSI-CRM-FULLY-FORBIDDEN-COMPARISON-OPERATORS", 1, 0, false );
    SubLFiles.declareMacro( me, "with_sksi_crm_pos_subclause_spec_gathering", "WITH-SKSI-CRM-POS-SUBCLAUSE-SPEC-GATHERING" );
    SubLFiles.declareFunction( me, "sksi_crm_add_pos_subclause_spec", "SKSI-CRM-ADD-POS-SUBCLAUSE-SPEC", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_reject_pos_subclause_spec", "SKSI-CRM-REJECT-POS-SUBCLAUSE-SPEC", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_pos_subclause_spec_irrelevantP", "SKSI-CRM-POS-SUBCLAUSE-SPEC-IRRELEVANT?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_gather_literals", "SKSI-CRM-GATHER-LITERALS", 7, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_gather_literals_iterative", "SKSI-CRM-GATHER-LITERALS-ITERATIVE", 8, 0, false );
    SubLFiles.declareFunction( me, "pos_subclause_spec_G", "POS-SUBCLAUSE-SPEC->", 2, 0, false );
    new $pos_subclause_spec_G$BinaryFunction();
    SubLFiles.declareFunction( me, "sksi_crm_gather_literals_recursive", "SKSI-CRM-GATHER-LITERALS-RECURSIVE", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_gather_literals_recursive_continue_recursion", "SKSI-CRM-GATHER-LITERALS-RECURSIVE-CONTINUE-RECURSION", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_valid_queryP", "SKSI-CRM-VALID-QUERY?", 8, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_valid_query_int", "SKSI-CRM-VALID-QUERY-INT", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_applicability_iterate_over_meaning_sentence_gafs", "SKSI-CRM-APPLICABILITY-ITERATE-OVER-MEANING-SENTENCE-GAFS", 9, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_applicability_process_meaning_sentence_gafs", "SKSI-CRM-APPLICABILITY-PROCESS-MEANING-SENTENCE-GAFS", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_applicability_iterate_over_asents", "SKSI-CRM-APPLICABILITY-ITERATE-OVER-ASENTS", 9, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_asents_generate_csqlP", "SKSI-CRM-ASENTS-GENERATE-CSQL?", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_asents_generate_sqlP", "SKSI-CRM-ASENTS-GENERATE-SQL?", 6, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_form_islands", "SKSI-CRM-FORM-ISLANDS", 2, 1, false );
    SubLFiles.declareFunction( me, "sksi_crm_literal_coherentP", "SKSI-CRM-LITERAL-COHERENT?", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_literal_coherent", "SKSI-CRM-LITERAL-COHERENT", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_literal_coherent_int", "SKSI-CRM-LITERAL-COHERENT-INT", 3, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_literals_coherent_together", "SKSI-CRM-LITERALS-COHERENT-TOGETHER", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_literals_coherent_together_int", "SKSI-CRM-LITERALS-COHERENT-TOGETHER-INT", 7, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_enumerability_check_asents", "SKSI-CRM-ENUMERABILITY-CHECK-ASENTS", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_unbound_unknown_sentence_asentsP", "SKSI-CRM-UNBOUND-UNKNOWN-SENTENCE-ASENTS?", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_enumerability_check", "SKSI-CRM-ENUMERABILITY-CHECK", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_multi_table_check", "SKSI-CRM-MULTI-TABLE-CHECK", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_fully_unbound_asentsP", "SKSI-CRM-FULLY-UNBOUND-ASENTS?", 5, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_any_fully_bound_argP", "SKSI-CRM-ANY-FULLY-BOUND-ARG?", 1, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_fully_unbound_for_meaning_sentenceP", "SKSI-CRM-FULLY-UNBOUND-FOR-MEANING-SENTENCE?", 2, 0, false );
    SubLFiles.declareFunction( me, "sksi_crm_fully_unbound_for_meaning_sentence_int", "SKSI-CRM-FULLY-UNBOUND-FOR-MEANING-SENTENCE-INT", 2, 0, false );
    SubLFiles.declareFunction( me, "collect_physical_schemata_from_meaning_sentence_gafs", "COLLECT-PHYSICAL-SCHEMATA-FROM-MEANING-SENTENCE-GAFS", 1, 0, false );
    SubLFiles.declareFunction( me, "collect_physical_schemata_from_meaning_sentence", "COLLECT-PHYSICAL-SCHEMATA-FROM-MEANING-SENTENCE", 1, 0, false );
    SubLFiles.declareFunction( me, "collect_physical_schemata_from_logical_field_indexical_internal", "COLLECT-PHYSICAL-SCHEMATA-FROM-LOGICAL-FIELD-INDEXICAL-INTERNAL", 1, 0, false );
    SubLFiles.declareFunction( me, "collect_physical_schemata_from_logical_field_indexical", "COLLECT-PHYSICAL-SCHEMATA-FROM-LOGICAL-FIELD-INDEXICAL", 1, 0, false );
    SubLFiles.declareFunction( me, "clause_literal_variables", "CLAUSE-LITERAL-VARIABLES", 3, 0, false );
    SubLFiles.declareFunction( me, "contextualized_literal_variables", "CONTEXTUALIZED-LITERAL-VARIABLES", 1, 0, false );
    SubLFiles.declareFunction( me, "contextualized_literal_hl_variables", "CONTEXTUALIZED-LITERAL-HL-VARIABLES", 1, 0, false );
    return NIL;
  }

  public static SubLObject init_sksi_conjunctive_removal_modules_applicability_file()
  {
    $sksi_crm_ignore_subsumed_subclause_specsP$ = SubLFiles.defparameter( "*SKSI-CRM-IGNORE-SUBSUMED-SUBCLAUSE-SPECS?*", T );
    $sksi_crm_applicability_max_time_enabledP$ = SubLFiles.deflexical( "*SKSI-CRM-APPLICABILITY-MAX-TIME-ENABLED?*", T );
    $sksi_crm_applicability_max_time_override$ = SubLFiles.defparameter( "*SKSI-CRM-APPLICABILITY-MAX-TIME-OVERRIDE*", NIL );
    $sksi_crm_applicability_start_time$ = SubLFiles.defparameter( "*SKSI-CRM-APPLICABILITY-START-TIME*", NIL );
    $sksi_crm_gathered_pos_subclause_specs$ = SubLFiles.defparameter( "*SKSI-CRM-GATHERED-POS-SUBCLAUSE-SPECS*", NIL );
    $sksi_crm_rejected_pos_subclause_specs$ = SubLFiles.defparameter( "*SKSI-CRM-REJECTED-POS-SUBCLAUSE-SPECS*", NIL );
    $sksi_crm_gather_literals_using_iterative_methodP$ = SubLFiles.defparameter( "*SKSI-CRM-GATHER-LITERALS-USING-ITERATIVE-METHOD?*", T );
    $sksi_crm_gather_literals_iterative_method_return_first_successP$ = SubLFiles.defparameter( "*SKSI-CRM-GATHER-LITERALS-ITERATIVE-METHOD-RETURN-FIRST-SUCCESS?*", T );
    $sksi_crm_gather_literals_stop_if_total_subclause_specs_fails_enumerability_optimizationP$ = SubLFiles.defparameter( "*SKSI-CRM-GATHER-LITERALS-STOP-IF-TOTAL-SUBCLAUSE-SPECS-FAILS-ENUMERABILITY-OPTIMIZATION?*", T );
    $sksi_crm_gather_literals_subclause_specs_failed_due_to_enumerabilityP$ = SubLFiles.defparameter( "*SKSI-CRM-GATHER-LITERALS-SUBCLAUSE-SPECS-FAILED-DUE-TO-ENUMERABILITY?*", NIL );
    return NIL;
  }

  public static SubLObject setup_sksi_conjunctive_removal_modules_applicability_file()
  {
    memoization_state.note_memoized_function( $sym8$SKSI_CRM_FULLY_FORBIDDEN_COMPARISON_OPERATORS );
    memoization_state.note_memoized_function( $sym42$COLLECT_PHYSICAL_SCHEMATA_FROM_LOGICAL_FIELD_INDEXICAL );
    return NIL;
  }
  ...
  static
  {
    me = new sksi_conjunctive_removal_modules_applicability();
    $sksi_crm_ignore_subsumed_subclause_specsP$ = null;
    $sksi_crm_applicability_max_time_enabledP$ = null;
    $sksi_crm_applicability_max_time_override$ = null;
    $sksi_crm_applicability_start_time$ = null;
    $sksi_crm_gathered_pos_subclause_specs$ = null;
    $sksi_crm_rejected_pos_subclause_specs$ = null;
    $sksi_crm_gather_literals_using_iterative_methodP$ = null;
    $sksi_crm_gather_literals_iterative_method_return_first_successP$ = null;
    $sksi_crm_gather_literals_stop_if_total_subclause_specs_fails_enumerability_optimizationP$ = null;
    $sksi_crm_gather_literals_subclause_specs_failed_due_to_enumerabilityP$ = null;
    $kw0$SKSI_CRM_APPLICABILITY_MAX_TIME = makeKeyword( "SKSI-CRM-APPLICABILITY-MAX-TIME" );
    $str1$SKSI_conjunctive_removal_was_appl = makeString( "SKSI conjunctive removal was applicable to problem " );
    $kw2$UNKNOWN = makeKeyword( "UNKNOWN" );
    $str3$_but_was_lacking_this___sksiUnkno = makeString( " but was lacking this #$sksiUnknownSentence literal: " );
    $sym4$NEW_POS_SUBCLAUSE_SPEC = makeSymbol( "NEW-POS-SUBCLAUSE-SPEC" );
    $list5 = ConsesLow.list( makeSymbol( "MT" ), makeSymbol( "ASENT" ) );
    $const6$RDFTripleStore_Physical = constant_handles.reader_make_constant_shell( makeString( "RDFTripleStore-Physical" ) );
    $sym7$SKSI_SUPPORTED_FUNCTION_P = makeSymbol( "SKSI-SUPPORTED-FUNCTION-P" );
    $sym8$SKSI_CRM_FULLY_FORBIDDEN_COMPARISON_OPERATORS = makeSymbol( "SKSI-CRM-FULLY-FORBIDDEN-COMPARISON-OPERATORS" );
    $list9 = ConsesLow.list( ConsesLow.list( makeSymbol( "RESULT-VAR" ) ), makeSymbol( "&BODY" ), makeSymbol( "BODY" ) );
    $sym10$CLET = makeSymbol( "CLET" );
    $list11 = ConsesLow.list( ConsesLow.list( makeSymbol( "*SKSI-CRM-GATHERED-POS-SUBCLAUSE-SPECS*" ), NIL ), ConsesLow.list( makeSymbol( "*SKSI-CRM-REJECTED-POS-SUBCLAUSE-SPECS*" ), ConsesLow.list( makeSymbol(
        "NEW-DICTIONARY" ), ConsesLow.list( makeSymbol( "FUNCTION" ), EQUAL ) ) ) );
    $sym12$CSETQ = makeSymbol( "CSETQ" );
    $list13 = ConsesLow.list( ConsesLow.list( makeSymbol( "FIF" ), makeSymbol( "*SKSI-CRM-IGNORE-SUBSUMED-SUBCLAUSE-SPECS?*" ), ConsesLow.list( makeSymbol( "DELETE-SUBSUMED-ITEMS" ), makeSymbol(
        "*SKSI-CRM-GATHERED-POS-SUBCLAUSE-SPECS*" ), ConsesLow.list( makeSymbol( "QUOTE" ), makeSymbol( "SUPERSETP" ) ) ), makeSymbol( "*SKSI-CRM-GATHERED-POS-SUBCLAUSE-SPECS*" ) ) );
    $sym14$SUBSETP = makeSymbol( "SUBSETP" );
    $sym15$NCANONICALIZE_LITERAL_INDICES = makeSymbol( "NCANONICALIZE-LITERAL-INDICES" );
    $sym16$SUPERSETP = makeSymbol( "SUPERSETP" );
    $sym17$POS_SUBCLAUSE_SPEC__ = makeSymbol( "POS-SUBCLAUSE-SPEC->" );
    $sym18$_ = makeSymbol( "=" );
    $list19 = ConsesLow.list( makeSymbol( "MT" ), makeSymbol( "LIT" ) );
    $kw20$VARPRED = makeKeyword( "VARPRED" );
    $kw21$GENLPREDS = makeKeyword( "GENLPREDS" );
    $kw22$GENLINVERSE = makeKeyword( "GENLINVERSE" );
    $str23$__SKSI__A_ = makeString( "~&SKSI-~A " );
    $str24$Could_not_form_a_CSQL_query_with_ = makeString( "Could not form a CSQL query with:~%  Meaning sentence GAFs: ~a~%  Literals: ~a~% for the reason: ~a" );
    ...
    $str27$Could_not_form_a_SQL_query_with__ = makeString( "Could not form a SQL query with:~%  Meaning sentence GAFs: ~a~%  Literals: ~a~% for the reason: ~a" );
    $sym28$_EXIT = makeSymbol( "%EXIT" );
    $kw29$POS = makeKeyword( "POS" );
    $sym30$FIRST = makeSymbol( "FIRST" );
    $list31 = ConsesLow.list( NIL );
    $sym32$VARIABLE_P = makeSymbol( "VARIABLE-P" );
    $sym33$LOGICAL_FIELD_INDEXICAL_P = makeSymbol( "LOGICAL-FIELD-INDEXICAL-P" );
    $sym34$CONTEXTUALIZED_LITERAL_HL_VARIABLES = makeSymbol( "CONTEXTUALIZED-LITERAL-HL-VARIABLES" );
    $const35$enumerableSchema = constant_handles.reader_make_constant_shell( makeString( "enumerableSchema" ) );
    $kw36$GAF = makeKeyword( "GAF" );
    $kw37$FALSE = makeKeyword( "FALSE" );
    $sym38$SKSI_CRM_ANY_FULLY_BOUND_ARG_ = makeSymbol( "SKSI-CRM-ANY-FULLY-BOUND-ARG?" );
    $sym39$CONTEXTUALIZED_ASENT_ASENT = makeSymbol( "CONTEXTUALIZED-ASENT-ASENT" );
    $sym40$FULLY_BOUND_P = makeSymbol( "FULLY-BOUND-P" );
    $sym41$LOGICAL_SCHEMA_PHYSICAL_SCHEMATA = makeSymbol( "LOGICAL-SCHEMA-PHYSICAL-SCHEMATA" );
    $sym42$COLLECT_PHYSICAL_SCHEMATA_FROM_LOGICAL_FIELD_INDEXICAL = makeSymbol( "COLLECT-PHYSICAL-SCHEMATA-FROM-LOGICAL-FIELD-INDEXICAL" );
    $const43$logicalPhysicalFieldMap = constant_handles.reader_make_constant_shell( makeString( "logicalPhysicalFieldMap" ) );
    $kw44$TRUE = makeKeyword( "TRUE" );
    $sym45$CYC_VAR_ = makeSymbol( "CYC-VAR?" );
    $sym46$HL_VAR_ = makeSymbol( "HL-VAR?" );
  }

  public static final class $pos_subclause_spec_G$BinaryFunction
      extends
        BinaryFunction
  {
