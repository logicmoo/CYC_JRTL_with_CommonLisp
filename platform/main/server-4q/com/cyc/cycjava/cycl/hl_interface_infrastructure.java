...
import static com.cyc.cycjava.cycl.hl_interface_infrastructure.*;
...
public final class hl_interface_infrastructure extends SubLTranslatedFile {
    ...
    public static SubLObject define_hl_modifier(final SubLObject macroform, final SubLObject environment) {
        ...
        return list(DEFINE_API, name, arglist, documentation_string, type_declarations, list(CLET, list(result), $list17, listS(NOTE_HL_MODIFIER_INVOCATION, list(QUOTE, name), append(formal_args, NIL)), list(PWHEN, $list28, list(CSETQ, result, list(HL_STORE_REMOTE_EVAL, listS(LIST, list(QUOTE, name), append(eval_args, NIL))))), list(PIF, $list31, list(OVERRIDE_REMOTE_HL_STORE_ACCESS, bq_cons(WITHIN_HL_MODIFICATION, append(body, NIL))), list(RET, result))));
    }
