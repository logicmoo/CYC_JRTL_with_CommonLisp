...
public final class user_interaction_agenda extends SubLTranslatedFile implements V12 {
	...
	public static final class $user_interaction_native extends SubLStructNative {
		...
		private static final SubLStructDeclNative structDecl = makeStructDeclNative(com.cyc.cycjava.cycl.user_interaction_agenda.$user_interaction_native.class, USER_INTERACTION, USER_INTERACTION_P, $list_alt8, $list_alt9, new String[] { "$id", "$agenda", "$mode", "$status", "$start_time", "$finish_time", "$parent_interaction", "$child_interactions", "$operator", "$args", "$result", "$state" }, $list_alt10,
				$list_alt11, PRINT_USER_INTERACTION);
	}
	...
	public static final class $user_interaction_agenda_native extends SubLStructNative {
		...
		private static final SubLStructDeclNative structDecl = makeStructDeclNative(com.cyc.cycjava.cycl.user_interaction_agenda.$user_interaction_agenda_native.class, USER_INTERACTION_AGENDA, USER_INTERACTION_AGENDA_P, $list_alt60, $list_alt61,
				new String[] { "$id", "$meta_agenda", "$isg", "$index", "$history", "$undos", "$messages", "$requirements", "$relevant_suggestions", "$possible_suggestions", "$comments", "$state" }, $list_alt62, $list_alt63, PRINT_USER_INTERACTION_AGENDA);
	}
	...
	static private final SubLList $list11 = list(new SubLObject[] { makeSymbol("_CSETF-UI-ID"), makeSymbol("_CSETF-UI-AGENDA"), makeSymbol("_CSETF-UI-MODE"), makeSymbol("_CSETF-UI-STATUS"), makeSymbol("_CSETF-UI-START-TIME"), makeSymbol("_CSETF-UI-FINISH-TIME"), makeSymbol("_CSETF-UI-PARENT-INTERACTION"), makeSymbol("_CSETF-UI-CHILD-INTERACTIONS"), makeSymbol("_CSETF-UI-OPERATOR"), makeSymbol("_CSETF-UI-ARGS"),
			makeSymbol("_CSETF-UI-RESULT"), makeSymbol("_CSETF-UI-STATE") });
	...
	private static final SubLList $list69 = list(new SubLObject[] { makeSymbol("_CSETF-UIA-ID"), makeSymbol("_CSETF-UIA-META-AGENDA"), makeSymbol("_CSETF-UIA-ISG"), makeSymbol("_CSETF-UIA-INDEX"), makeSymbol("_CSETF-UIA-HISTORY"), makeSymbol("_CSETF-UIA-UNDOS"), makeSymbol("_CSETF-UIA-MESSAGES"), makeSymbol("_CSETF-UIA-REQUIREMENTS"), makeSymbol("_CSETF-UIA-RELEVANT-SUGGESTIONS"),
			makeSymbol("_CSETF-UIA-POSSIBLE-SUGGESTIONS"), makeSymbol("_CSETF-UIA-COMMENTS"), makeSymbol("_CSETF-UIA-STATE") });
	...
	public static SubLObject make_user_interaction(SubLObject arglist) {
		...
		for (next = NIL, next = arglist; NIL != next; next = cddr(next)) {
			...
			if (pcase_var.eql($ID)) {
				...
				_csetf_ui_finish_time(v_new, current_value);
			} else if (pcase_var.eql($PARENT_INTERACTION)) {
				_csetf_ui_parent_interaction(v_new, current_value);
			} else if (pcase_var.eql($CHILD_INTERACTIONS)) {
				_csetf_ui_child_interactions(v_new, current_value);
			} else if (pcase_var.eql($OPERATOR)) {
				_csetf_ui_operator(v_new, current_value);
			} else if (pcase_var.eql($ARGS)) {
				_csetf_ui_args(v_new, current_value);
	...
	public static SubLObject make_user_interaction_agenda(SubLObject arglist) {
		...
		for (next = NIL, next = arglist; NIL != next; next = cddr(next)) {
			...
			if (pcase_var.eql($ID)) {
				_csetf_uia_id(v_new, current_value);
			} else if (pcase_var.eql($META_AGENDA)) {
				_csetf_uia_meta_agenda(v_new, current_value);
			} else if (pcase_var.eql($ISG)) {
				_csetf_uia_isg(v_new, current_value);
			} else if (pcase_var.eql($INDEX)) {
				_csetf_uia_index(v_new, current_value);
			} else if (pcase_var.eql($HISTORY)) {
				_csetf_uia_history(v_new, current_value);
			} else if (pcase_var.eql($UNDOS)) {
				_csetf_uia_undos(v_new, current_value);
			} else if (pcase_var.eql($MESSAGES)) {
				_csetf_uia_messages(v_new, current_value);
			} else if (pcase_var.eql($REQUIREMENTS)) {
				_csetf_uia_requirements(v_new, current_value);
			} else if (pcase_var.eql($RELEVANT_SUGGESTIONS)) {
				_csetf_uia_relevant_suggestions(v_new, current_value);
			} else if (pcase_var.eql($POSSIBLE_SUGGESTIONS)) {
				_csetf_uia_possible_suggestions(v_new, current_value);
	...
	public static SubLObject uia_classify(final SubLObject v_agenda, final SubLObject interaction) {
		...
		if (pcase_var.eql($UNDO)) {
			...
			uia_add_message(v_agenda, interaction);
		} else if (pcase_var.eql($REQUIRED)) {
			uia_add_requirement(v_agenda, interaction);
		} else if (pcase_var.eql($RELEVANT)) {
			uia_add_relevant_suggestion(v_agenda, interaction);
	...
	public static SubLObject make_user_interaction_meta_agenda(SubLObject arglist) {
		...
		for (next = NIL, next = arglist; NIL != next; next = cddr(next)) {
			...
			if (pcase_var.eql($ID)) {
				...
				_csetf_uima_index(v_new, current_value);
			} else if (pcase_var.eql($OPEN_AGENDAS)) {
				_csetf_uima_open_agendas(v_new, current_value);
			} else if (pcase_var.eql($CURRENT_AGENDA)) {
				_csetf_uima_current_agenda(v_new, current_value);
			} else if (pcase_var.eql($STATE)) {
				_csetf_uima_state(v_new, current_value);
			} else {
	...
	static private final SubLList $list_alt11 = list(new SubLObject[] { makeSymbol("_CSETF-UI-ID"), makeSymbol("_CSETF-UI-AGENDA"), makeSymbol("_CSETF-UI-MODE"), makeSymbol("_CSETF-UI-STATUS"), makeSymbol("_CSETF-UI-START-TIME"), makeSymbol("_CSETF-UI-FINISH-TIME"), makeSymbol("_CSETF-UI-PARENT-INTERACTION"), makeSymbol("_CSETF-UI-CHILD-INTERACTIONS"), makeSymbol("_CSETF-UI-OPERATOR"), makeSymbol("_CSETF-UI-ARGS"),
			makeSymbol("_CSETF-UI-RESULT"), makeSymbol("_CSETF-UI-STATE") });
	...
	static private final SubLList $list_alt63 = list(new SubLObject[] { makeSymbol("_CSETF-UIA-ID"), makeSymbol("_CSETF-UIA-META-AGENDA"), makeSymbol("_CSETF-UIA-ISG"), makeSymbol("_CSETF-UIA-INDEX"), makeSymbol("_CSETF-UIA-HISTORY"), makeSymbol("_CSETF-UIA-UNDOS"), makeSymbol("_CSETF-UIA-MESSAGES"), makeSymbol("_CSETF-UIA-REQUIREMENTS"), makeSymbol("_CSETF-UIA-RELEVANT-SUGGESTIONS"),
			makeSymbol("_CSETF-UIA-POSSIBLE-SUGGESTIONS"), makeSymbol("_CSETF-UIA-COMMENTS"), makeSymbol("_CSETF-UIA-STATE") });
