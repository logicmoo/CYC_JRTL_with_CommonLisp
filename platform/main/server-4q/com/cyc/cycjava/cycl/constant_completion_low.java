...
public final class constant_completion_low
    extends
      SubLTranslatedFile
{
  ...
  public static SubLObject kb_constant_complete_exact_internal(final SubLObject string, final SubLObject start, final SubLObject end)
  {
    ...
    if( NIL != constant_handles.constant_p( v_answer ) && NIL != $require_valid_constants$.getDynamicValue( thread ) && NIL == constant_handles.valid_constant_handleP( v_answer ) )
    {
      v_answer = NIL;
    }
    return v_answer;
  ...
  public static SubLObject kb_constant_complete_internal(final SubLObject prefix, final SubLObject case_sensitiveP, final SubLObject exact_lengthP, final SubLObject start, final SubLObject end)
  {
    ...
    {
      v_answer = Sequences.delete_if( $sym3$INVALID_CONSTANT_HANDLE_, v_answer, UNPROVIDED, UNPROVIDED, UNPROVIDED, UNPROVIDED );
    }
    return v_answer;
  }
  ...
  public static SubLObject kb_constant_postfix_internal(final SubLObject postfix, final SubLObject case_sensitiveP, final SubLObject start, final SubLObject end)
  {
    ...
    while ( NIL != cdolist_list_var)
    {
      ...
      final SubLObject postfix_end = ( NIL != end ) ? end : Sequences.length( postfix );
      ...
      if( NIL != Sequences.search( postfix, constant_name, Symbols.symbol_function( $sym4$CHAR_EQUAL ), Symbols.symbol_function( IDENTITY ), start, end, name_start, name_length ) )
      {
        postfix_results = ConsesLow.cons( result, postfix_results );
      }
  ...
  public static SubLObject kb_n_constants_internal(final SubLObject n, final SubLObject string, final SubLObject case_sensitiveP, final SubLObject start, final SubLObject end, final SubLObject forwardP,
      final SubLObject strictP)
  {
    ...
    if( !n.isZero() )
    {
      ...
      SubLObject done_var;
      SubLObject valid;
      for( SubLObject new_done = done_var = Numbers.numGE( count, n ); NIL == done_var; done_var = makeBoolean( NIL == valid || NIL != new_done ) )
      {
        ...
        if( NIL != valid )
        {
          constants = ConsesLow.cons( constant, constants );
  ...
  {
    SubLFiles.declareFunction( me, "constant_shell_from_name", "CONSTANT-SHELL-FROM-NAME", 1, 0, false );
    SubLFiles.declareFunction( me, "kb_constant_complete_exact_internal", "KB-CONSTANT-COMPLETE-EXACT-INTERNAL", 3, 0, false );
    SubLFiles.declareFunction( me, "kb_constant_complete_internal", "KB-CONSTANT-COMPLETE-INTERNAL", 5, 0, false );
    SubLFiles.declareFunction( me, "kb_constant_apropos_internal", "KB-CONSTANT-APROPOS-INTERNAL", 4, 0, false );
    SubLFiles.declareFunction( me, "kb_constant_postfix_internal", "KB-CONSTANT-POSTFIX-INTERNAL", 4, 0, false );
    SubLFiles.declareFunction( me, "kb_n_constants_before_internal", "KB-N-CONSTANTS-BEFORE-INTERNAL", 6, 0, false );
    SubLFiles.declareFunction( me, "kb_n_constants_after_internal", "KB-N-CONSTANTS-AFTER-INTERNAL", 6, 0, false );
    SubLFiles.declareFunction( me, "kb_n_constants_internal", "KB-N-CONSTANTS-INTERNAL", 7, 0, false );
    SubLFiles.declareFunction( me, "clear_constant_completion_table", "CLEAR-CONSTANT-COMPLETION-TABLE", 0, 0, false );
    SubLFiles.declareFunction( me, "add_constant_to_completions", "ADD-CONSTANT-TO-COMPLETIONS", 2, 0, false );
    SubLFiles.declareFunction( me, "remove_constant_from_completions", "REMOVE-CONSTANT-FROM-COMPLETIONS", 2, 0, false );
    SubLFiles.declareFunction( me, "kb_new_constant_completion_iterator_internal", "KB-NEW-CONSTANT-COMPLETION-ITERATOR-INTERNAL", 0, 1, false );
    SubLFiles.declareFunction( me, "kb_new_directed_constant_completion_iterator_internal", "KB-NEW-DIRECTED-CONSTANT-COMPLETION-ITERATOR-INTERNAL", 1, 5, false );
    return NIL;
  }

  public static SubLObject init_constant_completion_low_file()
  {
    $constant_completion_table$ = SubLFiles.deflexical( "*CONSTANT-COMPLETION-TABLE*", maybeDefault( $sym0$_CONSTANT_COMPLETION_TABLE_, $constant_completion_table$, () -> ( tries.create_trie( T,
        $str1$Constant_Completion_Table, UNPROVIDED, UNPROVIDED ) ) ) );
    $require_valid_constants$ = SubLFiles.defparameter( "*REQUIRE-VALID-CONSTANTS*", T );
    return NIL;
  }
  ...
  static
  {
    me = new constant_completion_low();
    $constant_completion_table$ = null;
    $require_valid_constants$ = null;
    $sym0$_CONSTANT_COMPLETION_TABLE_ = makeSymbol( "*CONSTANT-COMPLETION-TABLE*" );
    $str1$Constant_Completion_Table = makeString( "Constant Completion Table" );
    $sym2$STRINGP = makeSymbol( "STRINGP" );
    $sym3$INVALID_CONSTANT_HANDLE_ = makeSymbol( "INVALID-CONSTANT-HANDLE?" );
    $sym4$CHAR_EQUAL = makeSymbol( "CHAR-EQUAL" );
    $sym5$CONSTANT_P = makeSymbol( "CONSTANT-P" );
  }
}
/*
 * 
