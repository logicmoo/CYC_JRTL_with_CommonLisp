    ...
    public void commit() {
        try {
            ...
            try {
                ...
                mt = CycClient.getCurrent().getObjectTool().makeELMt("#$BaseKB");
                ...
                CycClient.getCurrent().getAssertTool().edit(unassertConjunction.toString(), assertConjunction.toString(), mt, bookkeeping, transcripting, wffCheckingDisabled, templates);
            } catch (Exception ex) {
//                Logger.getLogger(SimpleKBTransaction.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void rollback() {
        for (Fort createdFort : createdForts) {
            try {
                CycClient.getCurrent().getUnassertTool().kill(createdFort, isBookkeeping(), isTranscripting());
            } catch (Exception ex) {
