 ...
 * 
 * @version $Id: CfaslInputStream.java 150785 2014-04-26 01:34:40Z nwinant $
...
public class CfaslInputStream extends BufferedInputStream {
  ...
  /**
   * Reads the body of a CycGuid from the CfaslInputStream.  The CFASL opcode has already been read in
   * at this point, so we only read in what follows.
   * 
   * @return the <tt>CycGuid</tt> read
   * 
   * @throws IOException if a communications error occurs
   */
  public CycGuid readLegacyGuid()
          throws IOException {
    CycGuid guid = CycObjectFactory.makeGuid((String) readObject());
  ...
  public CycGuid readGuid()
    ...
    final CycGuid guid = CycObjectFactory.makeGuid(data);
    if (trace == API_TRACE_DETAILED) {
   ...
   */
  public CycConstantI readCompleteConstant()
          throws IOException {
    CycConstantI cycConstant = null;
    Object idObject = readObject();

    if (idObject instanceof CycGuid) {
      final CycGuid guid = (CycGuid) idObject;
   ...
   * 
   * @return an incomplete <tt>DefaultCycAssertion</tt> having the input id
   * 
   * @throws IOException if a communications error occurs
   */
  public CycAssertion readAssertion()
    ...
    if (formulaObject.toString().equals("NIL")) {
      ...
      try {
        ...
        cycAssertion = new DefaultCycAssertion(formula, mt);
