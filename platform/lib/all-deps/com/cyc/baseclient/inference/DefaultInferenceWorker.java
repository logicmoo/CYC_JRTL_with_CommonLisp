 ...
 * @version $Id: DefaultInferenceWorker.java 150787 2014-04-26 01:55:03Z nwinant $
 */
public class DefaultInferenceWorker extends DefaultSubLWorker implements InferenceWorker {
   ...
   */
  public DefaultInferenceWorker(String query, ELMt mt, InferenceParameters queryProperties,
          Map nlGenerationProperties, CycSymbolI answerProcessingFunction,
   ...
   * @see com.cyc.baseclient.inference.DefaultInferenceWorker#continueInference(com.cyc.baseclient.inference.params.InferenceParameters) 
   **/
...
//      System.out.println("Inference Interrupted "+event.getStatus()+" "+event.getWork());
    ...
    try {
      ...
      throw new RuntimeException("Failed to continue inference.", cce);
    }
  }
  ...
  //// Private Area
  private void fireInferenceStatusChanged(final DefaultInferenceStatus oldStatus) throws RuntimeException {
  ...
  private void handleInferenceStatusChangedResult(CycList data) {
    ...
    if ((statusObj == null) || (!(statusObj instanceof CycSymbolI))) {
      throw new RuntimeException("Got bad inference status: " + statusObj);
    }
    DefaultInferenceStatus newStatus = DefaultInferenceStatus.findInferenceStatus((CycSymbolI) statusObj);
    DefaultInferenceStatus oldStatus = status;
    status = newStatus;
    if (status == null) {
  ...
  protected static String createInferenceCommand(CycList query, ELMt mt,
          ...
          CycSymbolI answerProcessingFunction, boolean optimizeVariables, CycAccess cycAccess) {
  ...
  protected CycSymbolI answerProcessingFunction;
  ...
  public static void main(String[] args) {
    try {
      CycAccess access = CycClientManager.get().getAccess("localhost", 3640);
      InferenceWorker worker = new DefaultInferenceWorker("(#$isa ?X #$Dog)",
              CommonConstants.inferencePSC, null, access, 10000);
