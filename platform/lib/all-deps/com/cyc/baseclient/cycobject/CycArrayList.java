...
*/
 ...
 dotted pair, returns the dotted element.
  ...
  /**
   * Returns a new <tt>CycArrayList</tt> with every occurrance of <tt>Object</tt> oldObject
   * replaced by <tt>Object</tt> newObject.  Substitute recursively into embedded
   * <tt>CycArrayList</tt> objects.
   *
   * @return a new <tt>CycArrayList</tt> with every occurrance of <tt>Object</tt> oldObject
  ...
  /**
   * Returns a cyclified string representation of the OpenCyc <tt>CycArrayList</tt>.
  ...
  public Map<ArgPosition, Span> getPrettyStringDetails() {
    ...
    getPrettyStringDetails(this, "", 0, new CycArgPosition(), map);
    Span loc = new Span(0, toPrettyString("").length());
    map.put(CycArgPosition.TOP, loc);
    return map;
  }

  private static int getPrettyStringDetails(final CycArrayList list, final String indent,
          int currentPos, final CycArgPosition argPos, final Map<ArgPosition, Span> map) {
    String str;
    CycArgPosition newArgPos;
    String tab = "  ";
    str = indent + "(";
    ...
    for (int i = 0, size = list.size(); i < size; i++) {
      ...
      if (element instanceof String) {
        ...
        CycArgPosition deepCopy = argPos.deepCopy();
  ...
  public CycArrayList treeConstants() {
    ...
    while (!stack.empty()) {
      ...
      if (obj instanceof CycConstant) {
        ...
        stack.push(((DefaultCycAssertion) obj).getMt());
        pushTreeConstantElements(((DefaultCycAssertion) obj).getFormula(), stack);
   ...
   */
  @Override
  public Object getf(CycSymbolI indicator) {
   ...
   */
  @Override
  public Object getf(CycSymbolI indicator, Object defaultResult, boolean treatNilAsAbsent) {
   ...
   */
  @Override
  public Object getf(CycSymbolI indicator, Object defaultResult) {
 ...
 consisting of alternating keywords and values, with no repeating keywords.
  ...
  public static void toXML(final Object object, final XMLWriter xmlWriter, final int indent, final boolean relative) throws IOException {
    ...
    if (object instanceof Integer) {
      ...
      CycGuid.fromGuid((Guid) object).toXML(xmlWriter, indentLength, true);
    } else if (object instanceof CycArrayList) {
    ...
    } else if (object instanceof DefaultCycAssertion) {
      ((DefaultCycAssertion) object).toXML(xmlWriter, indentLength, true);
    } else {
   ...
   */
  @Override
  public Object getValueForKeyword(final CycSymbolI keyword) {
 ...
 the given term can be found in this CycArrayList. An arg position is a flat
  ...
  public List<ArgPosition> getArgPositionsForTerm(final Object term) {
    ...
    CycArgPosition curArgPosition = CycArgPosition.TOP;
  ...
  private static void internalGetArgPositionsForTerm(Object term, Object subTree,
          final CycArgPosition curPosPath, final List<ArgPosition> result) {
    if (term instanceof CycNart) {
      term = ((CycNart) term).toCycList();
    }
    if (term == subTree) {
      final CycArgPosition newArgPos = new CycArgPosition(curPosPath.getPath());
    ...
    if ((subTree instanceof CycArrayList) && ((CycArrayList) subTree).treeContains(term)) {
      ...
      for (Iterator iter = ((List) subTree).iterator(); iter.hasNext(); newPos++) {
        final CycArgPosition newPosPath = new CycArgPosition(curPosPath.getPath());
