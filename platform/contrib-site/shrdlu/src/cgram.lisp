...
(DEFUN CLAUSE  NIL
      (PROG (FE  H ME NB  C SM CUT NN T1 T2 T3 RESULT POSITION-OF-PRT MVB  LOCATIONMARKER SUBJ-VB-BACKUP-TYPE1 POSITION-OF-PTW)
       (DECLARE (SPECIAL ME FE C CUT SM NN  NB H POSITION-OF-PRT MVB LOCATIONMARKER  SUBJ-VB-BACKUP-TYPE1 POSITION-OF-PTW))
       (SETQ NN T)
       (SETQ CUT END)
       (SETQ C (BUILDNODE (SETQ FE (REVERSE R3ST)) (SETQ NB (OR (NB RE) N)) N (SETQ H RE) NIL))
       (SETR 'PARENT PARENT C)
       ENTERING-CLAUSE
       (AND LABELTRACE (PASSING 'ENTERING-CLAUSE))
       (SETR 'TIME (BUILD-SHRDLU TSSNODE= (MAKESYM 'TSS)) C)
       (SETQ RESULT (CQ SIMP))
       (COND (RESULT (GO SUBJ)))
       ...
       (AND (NQ ADV) (PARSE ADJG ADV VBAD) (OR (CALLSM (SMRELATE H)) (SHRDLU-POP)))
       (PARSE NG TIME)
       (SETQ RESULT (EQ LOCATIONMARKER N))
       (COND (RESULT
              (COND ((NULL NN) (GO INPOP))
                (T (GO CLAUSETYPE))))
        (T (GO INIT)))
       INPOP
       ...
       (COND (RESULT
              (COND ((NULL NN) (M 'VB-ADJG) (GO FAIL))
                (T (GO VB)))))
       (RQ VBLOK)
       VBL
       ...
       (COND ((CQ SUBJFORK) (FQ VBFORK) (GO FINDOBJ1))
        ...
        ((ISQ H CLAUSE) (SETQ SUBJ-VB-BACKUP-TYPE1 T) (SHRDLU-POP) (GO SUBJ4))
        ((ISQ H SUBJ) (SHRDLU-POP) (FQ SUBJFORK) (GO VBL)))
       ...
       (COND (RESULT
              (COND ((NULL NN) (M 'BE) (GO FAIL))
                (T (GO BE)))))
       (SETQ RESULT (ISQ MVB VPRT))
       (COND ((NULL RESULT)
              (COND ((NULL NN) (GO CHECKPASV))
                (T (GO CHECKPASV)))))
       ...
       (COND (RESULT
              (COND ((NULL NN) (M 'ONT-NOT-FOUND) (GO FAIL))
                (T (GO PREPSHORT))))
        (T (GO TONT)))
       PONT
       ...
       (COND ((NULL RESULT)
              (COND ((NULL NN) (M 'PREPQ-INCOMPLETE) (GO FAIL))
                (T (GO NGQUES)))))
       ...
       (SETQ RESULT
           (OR (AND (NEXTWORD? 'HOW) (PARSE ADJG QUEST) (SETR 'RELHEAD H C))
            (AND (NQ QADJ) (PARSE QADJ) (FQ QADJ) (SETR 'QADJ H C))))

       (COND (RESULT (GO POLAR))
        (T (GO POLAR)))
       (FQ SHORTQUES)
       (CALLSM (SMADJQSHORT))
       ADJQS
       ...
       (COND ((PARSE VG EN PASV) (OR (ISQ MVB TRANS) (GO FAIL)) (SETR 'SUBJECT (GETR 'RELHEAD C) C) (GO VG1))
        ...
        ((CQ COMPONENT) (SETR 'RELHEAD (GETR 'RELHEAD (MOVE-PT C PC)) C) (GO REL))
...
(DEFUN NG  NIL
   (PROG (FE H ME NB C SM CUT NN T1 T2 T3 RESULT)
    (DECLARE (SPECIAL ME FE C CUT SM NN NB H))
    (SETQ NN T)
    (SETQ CUT END)
    (SETQ C (BUILDNODE (SETQ FE (REVERSE R3ST)) (SETQ NB (OR (NB RE) N)) N (SETQ H RE) NIL))
    (SETR 'PARENT PARENT C)
    ...
    (SETQ RESULT (COND ((EQ (SM H) 1) (AND (CQ NS) (RQ NPL)))
                  ((CQ NPL) (RQ NS))))
    (COND ((NULL RESULT)
           (COND ((NULL NN) (GO INCOM))
             (T (M 'NUMD) (GO FAIL)))))
    ...
    (FQ OF)
    (SETQ RESULT (OR (CALLSM (SMNGOF)) (NOT (SHRDLU-POP))))
    (COND (RESULT (GO RETSM))
     (T (GO INCOM)))
    NONE
    (AND LABELTRACE (PASSING 'NONE))
    (SETQ RESULT (EQ (WORD (NB H)) 'NONE))
    (COND (RESULT (GO INCOM))
     (T (GO ADJ)))
    ADJ
    ...
    (COND ((NULL RESULT)
           (COND ((NULL NN) (GO INCOM))
             (T (GO CLASF)))))
    (FQ ADJ)
    (AND (NEXTWORD? 'OF) (PARSE PREPG OF) (OR (CALLSM (SMNGOF)) (GO FAIL)) (FQ OF) (GO RETSM))
    CLASF
    (AND LABELTRACE (PASSING 'CLASF))
    (SETQ RESULT (OR (PARSE VB ING (CLASF)) (PARSE VB EN (CLASF)) (PARSE CLASF)))
    (COND (RESULT
           (COND ((NULL NN) (GO REDUC))
             (T (GO CLASF)))))
    (SETQ T2 (+ T2 1))
    (AND (> T2 100) (GO FAIL))
    NOUN
    ...
    (SETQ RESULT
          (AND (NQ PREP)
           (NOT  (OR (AND (NQ PLACE) (CQ NOLOC)) (AND (CQ OBJ1) (ISQ MVB TRANSL) (NOT (ISQ (MOVE-PT C U) QUEST)))))
           (PARSE PREPG Q)))
...
(DEFUN VG  NIL
      (PROG (FE H ME NB C SM CUT NN T1 T2 T3 RESULT TENSE)
       (DECLARE (SPECIAL ME FE C CUT SM NN NB H TENSE))
       (SETQ NN T)
       (SETQ CUT END)
       (SETQ C (BUILDNODE (SETQ FE (REVERSE R3ST)) (SETQ NB (OR (NB RE) N)) N (SETQ H RE) NIL))
       (SETR 'PARENT PARENT C)
       ...
       (COND ((OR (EQUAL TENSE '(PAST)) (CQ NAGR) (ISQ (MOVE-PT C U) IMPER) (ISQ PT THERE) (ISQ PT RSNG))
              (GO NAUX))
       ...
       (COND ((ISQ PT NFS) (OR (SETQ T3 (MEET FE '(VFS INF))) (GO NAGR)))
        ...
        ((OR (ISQ PT NS) (ISQ PT MASS))
         (OR (AND (CQ V3PS) (SETQ T3 T)) (FESET PT (SETDIF (FE PT) '(NS MASS))))))

       (COND ((OR (ISQ PT PART) (ISQ PT NPL))
              (OR (AND (MEET FE '(INF VPL)) (SETQ T3 T)) (FESET PT (SETDIF (FE PT) '(PART NPL))))))
       NAGR
...
(DEFUN PREPG  NIL
      (PROG (FE H ME NB C SM CUT NN T1 T2 T3 RESULT)
       ...
       (COND (RESULT
              (COND ((NULL NN) (M 'PREPADV) (GO FAIL))
                (T (GO ADV)))))
...
(DEFUN ADJG  NIL
      (PROG (FE H ME NB C SM CUT NN T1 T2 T3 RESULT)
       ...
       ADJ
       (AND LABELTRACE (PASSING 'ADJ))
       (SETQ RESULT (COND ((CQ ADV) (PARSE ADV VBAD))
                     (T (PARSE ADJ))))
...
(DEFUN CONJOIN  NIL
      (PROG (FE H ME NB C SM CUT NN T1 T2 T3 RESULT PREV)
       (DECLARE (SPECIAL ME FE C CUT SM NN NB H PREV))
       (SETQ NN T)
       (SETQ CUT END)
       (SETQ C (BUILDNODE (SETQ FE (REVERSE R3ST)) (SETQ NB (OR (NB RE) N)) N (SETQ H RE) NIL))
       (SETR 'PARENT PARENT C)
       ...
       (COND
        ((AND  (EQ PREV '|,|)
          (OR (CDR H) (> (- (LENGTH (NB H)) (LENGTH (N H))) 4))
           ...
           (COND ((MEMQ (CAR R3ST) '(ADJ NUM NOUN PREP VB ADV)) (PARSE3 (APPEND R3ST '(COMPONENT)) NIL))
             ...
             ((EQ (CAR R3ST) 'CLAUSE)
              ((LAMBDA (LASTSENT AUXFE)
                  (AND (PARSE2 (APPEND R3ST AUXFE '(COMPONENT)) NIL)
       ...
       (COND ((NOT (ATOM PREV)) (GO RETSM))
        ...
        ((MEMQ PREV '(AND OR NOR BUT))
         ...
         (COND
           ((OR (NEXTWORD? 'BUT) (AND (NEXTWORD? PREV) (NOT (AND (EQ BOTH (NB H)) (EQ PREV 'AND)))))
            (FQ LISTA)
            (F PREV)
            (GO UP))
           (T (GO LISTA)))))
       ...
       (RETURN
        (COND
          ((MEMQ 'RSNG TYPE)
           (MEMQ
                    (READLIST
                       (APPEND (COND ((MEMQ 'TO TYPE) '(T O))
                                ((MEMQ 'ING TYPE) '(I N G))
                                ((MEMQ 'REPORT TYPE) '(R E P)))
                             '(O B)
          ...
          ((MEMQ 'NG TYPE)
           (COND ((EQUAL NUM 1) (NOT (NULL (MEET '(TRANS TRANS2 TRANSL TRANSINT) VBFEAT))))
              ...
              (APPLY-PARSE
                 (APPEND TYPE
                       (COND ((MEMQ 'COMP TYPE) (SETQ REG 'COMP) NIL)
                             (T
                              (LIST 'OBJ
                                          (SETQ REG  (COND ((OR (MEMQ 'LOC TYPE) (MEMQ 'PLACE TYPE)) 'LOBJ)
                                                      ((EQUAL NUM 1) 'OBJ1)
                                                      (T 'OBJ2))))))))
