       ...
       (PROG (THNF THWH THCK THLAS THTTL THT1 THFST THFSTP THFOO)      ;RETURNS NIL IF ALREADY THERE ELSE RETURNS THTT
	     ...
	     (SETQ THCK
		   (COND ((ATOM THTT)
			  ...
			  (OR (SETQ THT1 (THGET THTT 'THEOREM))
...
(DEFS THCONSE COMMENT (DEFINES AND OPTIONALLY ASSERTS CONSEQUENT THEOREMS))
...
(DEFS THDEF COMMENT (DEFINES AND OPTIONALLY ASSERTS THEOREMS))
...
(DEFUN THIP
       ...
       (PROG (THT1 THT3 THSV THT2 THI1)
	     ...
	     (COND ((NOT (SETQ THT1 (THGET THI1 THWH)))
...
(DEFUN THMATCHLIST
	...
	THP3 (COND ((OR (NULL (ATOM THB2))
	     ...
	     (COND ((NOT (SETQ THA2 (THGET 'THVRB THWH)))
		    (SETQ THA2 '(0. 0.)))
...
(DEFUN THNOFAIL (THX) (COND (THX (THSETF (G3T 'THPROG 'THFAIL) 'THPROGT))
			    (T (SET (THGET 'THPROG 'THFAIL) 'THPROGF))))

(DEFUN-FEXPR THNOHASH
       (THA)
       (MAPC #'(LAMBDA (X) (THSETF (G3T (CAR THA) X)
				    'THNOHASH))
	     (OR (CDR THA)
...
(DEFUN THPUTPROP
       ...
       (THPUSH THTREE
	       (LIST 'THMUNG
		     (LIST (LIST 'THSETF (LIST 'GET
				    (LIST 'QUOTE ATO)
				    (LIST 'QUOTE IND))
				 (LIST 'QUOTE (THGET ATO IND))))))
       (THSETF (G3T ATO IND) VAL))
...
(DEFUN THREM1
       ...
       (PROG (THA THSV THA1 THA2 THA3 THA4 THA5 THONE THPC)
	     ...
	     ;;ALL THE R3ST SERVES THE SAME PURPOSE AS THE
...
(DEFUN THVAL
	...
	SUCCEED
	     ...
	     (COND ((NULL THTREE) (SETQ THLEVEL (CDR THLEVEL))
		   ...
		   ((SETQ THEXP (THGET (CAAR THTREE) 'THSUCCEED))
		    (GO GO2))
	...
	FAIL (COND (THSTEPF (EVAL THSTEPF)))
	     ...
	     (COND ((NULL THTREE) (SETQ THLEVEL (CDR THLEVEL))
		   ...
		   ((SETQ THEXP (THGET (CAAR THTREE) 'THFAIL))
		    (GO GO2))
...
(DEFS THTAG THFAIL THTAGF)

(DEFS THTAG THSUCCEED THTAGT)

(DEFS THGOAL THSUCCEED THGOALT)

(DEFS THGOAL THFAIL THGOALF)

(DEFS THFAIL? THFAIL THFAIL?F)

(DEFS THFAIL? THSUCCEED THFAIL?T)

(DEFS THAMONG THFAIL THAMONGF)

(DEFS THFIND THFAIL THFINDF)

(DEFS THFIND THSUCCEED THFINDT)

(DEFS THPROG THSUCCEED THPROGT)

(DEFS THAND THSUCCEED THANDT)

(DEFS THMUNG THSUCCEED THMUNGT)

(DEFS THERASE THSUCCEED THERASET)

(DEFS THASSERT THSUCCEED THASSERTT)

(DEFS THOR THSUCCEED THORT)

(DEFS THCOND THSUCCEED THCONDT)

(DEFS THAND THFAIL THANDF)

(DEFS THPROG THFAIL THPROGF)

(DEFS THMUNG THFAIL THMUNGF)

(DEFS THASSERT THFAIL THASSERTF)

(DEFS THERASE THFAIL THERASEF)

(DEFS THCOND THFAIL THCONDF)

(DEFS THOR THFAIL THORF)

(DEFS THDO THSUCCEED THDOB)

(DEFS THDO THFAIL THDOB)

(DEFS THUNDO THFAIL THUNDOF)

(DEFS THUNDO THSUCCEED THUNDOT)

(DEFS THMESSAGE THFAIL THMESSAGEF)

(DEFS THMESSAGE THSUCCEED THMESSAGET)

(DEFS THREMBIND THSUCCEED THREMBINDT)

(DEFS THREMBIND THFAIL THREMBINDF)
