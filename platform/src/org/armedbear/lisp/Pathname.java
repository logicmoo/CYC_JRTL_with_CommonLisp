...
public class Pathname extends NLispObject {
	...
	static final String uriSeparator = "/";
	...
	private final void init(String s) {
		...
		if (Utilities.isPlatformWindows) {
			if (s.startsWith("\\\\") || s.startsWith("//")) {
				...
				if (s.startsWith("\\\\")) {
					...
					shareIndex = s.indexOf(uriSeparator, 2);
					dirIndex = s.indexOf(uriSeparator, shareIndex + 1);
				}
			...
			if (jar.startsWith("jar:file:")) {
				...
				if (file.length() > 0) {
					...
					try {
						...
						error(new SimpleError("Failed to create URI from " + "'" + file + "'" + ": " + e1.getMessage()));
					} catch (URISyntaxException e2) {
						error(new SimpleError("Failed to create URI from " + "'" + file + "'" + ": " + e2.getMessage()));
					}
		...
		if (separatorIndex > 0 && s.startsWith("jar:")) {
			...
			try {
				...
				error(new LispError("Failed to parse URL " + "'" + jarURL + "'" + ex.getMessage()));
			}
		...
		if (isValidURL(s)) {
			...
			if (scheme.equals("file")) {
				...
				try {
					...
					error(new SimpleError("Improper URI syntax for " + "'" + url.toString() + "'" + ": " + ex.toString()));
				}
			...
			try {
				...
				error(new LispError("Couldn't form URI from " + "'" + url + "'" + " because: " + e));
			}
	...
	public String getNamestring() {
		...
		if (host != NIL) {
			Debug.assertTrue(host instanceof AbstractString || isURL());
			if (isURL()) {
		...
		if (device == NIL) {
			...
			if (this instanceof LogicalPathname || host == NIL) {
	...
	protected String getDirectoryNamestring() {
		...
		if (directory != NIL && directory != Keyword.UNSPECIFIC) {
			...
			if (part == Keyword.ABSOLUTE) {
				...
				error(new FileError("Unsupported directory component " + part.printObject() + ".", this));
			}
			while (temp != NIL) {
				part = temp.car();
				if (part instanceof AbstractString) {
					...
					if (!ansi)
						error(new FileError("Unsupported directory component " + part.princToString() + ".", this));
				}
				sb.append(uriSeparator);
				temp = temp.cdr();
			}
	...
	@Override
	public int sxhash() {
		return ((host.sxhash() ^ device.sxhash() ^ directory.sxhash() ^ name.sxhash() ^ type.sxhash()) & 0x7fffffff);
	}
	...
	public static HashTable LOGICAL_PATHNAME_TRANSLATIONS = HashTable.newEqualHashTable(64, NIL, NIL);
	...
	URLConnection getURLConnection() {
		...
		try {
			...
			error(new FileError("Failed to open URL connection.", this));
		}
		return result;
	}
	...
	public static LogicalPathname parseNamestring(AbstractString namestring, AbstractString host) {
		...
		if (h != null) {
			if (!h.equals(host.getStringValue())) {
				error(new LispError("Host in " + s + " does not match requested host " + host.getStringValue()));
				// Not reached.
				return null;
			}
	...
	static final void checkCaseArgument(LispObject arg) {
		if (arg != Keyword.COMMON && arg != Keyword.LOCAL) {
			type_error(arg, list(Symbol.MEMBER, Keyword.COMMON, Keyword.LOCAL));
		}
	}
	...
	private static class pf_namestring extends Primitive {
		...
		public LispObject execute(LispObject arg) {
			...
			if (namestring == null) {
				error(new SimpleError("Pathname has no namestring: " + pathname.princToString()));
			}
	...
	private static class pf_parse_namestring extends Primitive {
		pf_parse_namestring() {
			super("%parse-namestring", PACKAGE_SYS, false, "namestring host default-pathname");
		}
		...
		public LispObject execute(LispObject first, LispObject second, LispObject third) {
			...
			if (second == NIL) {
				...
				if (third instanceof LogicalPathname) {
					...
					return thread.setValues(parseNamestring(namestring), namestring.LENGTH());
				}
			}
			Debug.assertTrue(second != NIL);
			final AbstractString host = checkString(second);
			return thread.setValues(parseNamestring(namestring, host), namestring.LENGTH());
		}
	}
	...
	static final Pathname _makePathname(LispObject[] args) {
		...
		for (int i = 0; i < args.length; i += 2) {
			...
			if (key == Keyword.HOST) {
				...
				device = value;
				deviceSupplied = true;
				if (!(value instanceof AbstractString || Keyword.UNSPECIFIC.symbolSame(value) || NULL(value) || value instanceof Cons))
				...
				if (value instanceof AbstractString) {
					...
					if ((value instanceof Cons
							...
							|| value == Keyword.UNSPECIFIC || NULL(value))) {
						directory = value;
					} else {
	...
	private static final AbstractString validateStringComponent(AbstractString s) {
		...
		for (int i = 0; i < limit; i++) {
			...
			if (c == separator || c == '\\' && Utilities.isPlatformWindows) {
				error(new LispError("Invalid character #\\" + c + " in pathname component \"" + s + '"'));
				// Not reached.
				return null;
			}
		}
	...
	private final boolean validateDirectory(boolean signalError) {
		...
		while (temp != NIL) {
			...
			} else if (first != Keyword.RELATIVE && first != Keyword.WILD && first != Keyword.UP && first != Keyword.BACK && !(first instanceof AbstractString)) {
				if (signalError) {
					error(new FileError("Unsupported directory component " + first.princToString() + ".", this));
				}
				return false;
			}
		}
	...
	@DocString(name = "logical-pathname-p", args = "object", returns = "generalized-boolean",
			...
			if (pathname.isJar()) {
				...
				for (Enumeration<? extends ZipEntry> entries = jar.entries(); entries.hasMoreElements();) {
					...
					if (entryName.endsWith("/")) {
						matches = Symbol.PATHNAME_MATCH_P.execute(new SimpleString(entryName), wildcardDirectory);
					} else {
						matches = Symbol.PATHNAME_MATCH_P.execute(new SimpleString(entryName), wildcard);
					}
					if (!NULL(matches)) {
			...
			if (pathname.device.cdr() instanceof Cons) {
				...
				try {
					while ((entry = zipInputStream.getNextEntry()) != null) {
						String entryName = "/" + entry.getName();
						LispObject matches = Symbol.PATHNAME_MATCH_P.execute(new SimpleString(entryName), wildcard);
	...
	private static class pf_pathname_url_p extends Primitive {
		pf_pathname_url_p() {
			super("pathname-url-p", PACKAGE_EXT, true, "pathname", "Predicate for whether PATHNAME references a URL.");
		}
	...
	static final class pf_wild_pathname_p extends Primitive {
		...
		public LispObject execute(LispObject first, LispObject second) {
			...
			if (second == Keyword.HOST) {
				...
				return program_error("Unrecognized keyword " + second.princToString() + ".");
			}
	...
	static final class pf_merge_pathnames extends Primitive {
		...
		public LispObject execute(LispObject arg) {
			Pathname pathname = coerceToPathname(arg);
			Pathname defaultPathname = coerceToPathname(Symbol.DEFAULT_PATHNAME_DEFAULTS.symbolValue());
	...
	public static final Pathname mergePathnames(final Pathname pathname, final Pathname defaultPathname, final LispObject defaultVersion) {
		Pathname result;
		...
		if (pathname.device != NIL) { // XXX if device represent JARs we want to merge
			...
			if (!p.isURL()) {
				...
				if (pathname.host == NIL && pathname.device == NIL && d.isJar() && !Utilities.isPlatformWindows) {
					if (pathname.directory != NIL && pathname.directory.car() == Keyword.ABSOLUTE) {
		...
		if (pathname.isJar()) {
			...
			if (jar instanceof Pathname) {
				...
				if (o.directory instanceof Cons && ((Cons) o.directory).length() == 1) { // i.e. (:ABSOLUTE) or (:RELATIVE)
					o.directory = NIL;
				}
		...
		if (isJar()) {
			...
			if (NULL(d.cdr())) {
				if (entryPath.length() == 0) {
					...
					// 3. Entry in JAR
					final ZipEntry entry = ZipCache.get((Pathname) device.car()).getEntry(entryPath);
					if (entry == null) {
						return 0;
					}
	...
	private static class pf_mkdir extends Primitive {
		...
		public LispObject execute(LispObject arg) {
			...
			Pathname defaultedPathname = mergePathnames(pathname, coerceToPathname(Symbol.DEFAULT_PATHNAME_DEFAULTS.symbolValue()), NIL);
	...
	private static class pf_rename_file extends Primitive {
		...
		public LispObject execute(LispObject first, LispObject second) {
			...
			if (defaultedNewName instanceof LogicalPathname) {
				destination = LogicalPathname.translateLogicalPathname((LogicalPathname) defaultedNewName).getFile();
			} else {
			...
			return error(new FileError("Unable to rename " + oldTruename.princToString() + " to " + newName.princToString() + ".", oldTruename));
		}
	}
	...
	File getFile() {
		...
		error(new FileError("Pathname has no namestring: " + princToString(), this));
		// Not reached.
		return null;
	}
