...
abstract public class SlotClass extends LispClass {
	...
	public LispObject getParts() {
		...
		result = result.push(new Cons("DIRECT-SLOTS", getDirectSlotDefinitions()));
		result = result.push(new Cons("SLOTS", getSlotDefinitions()));
		result = result.push(new Cons("DIRECT-DEFAULT-INITARGS", getDirectDefaultInitargs()));
		result = result.push(new Cons("DEFAULT-INITARGS", getDefaultInitargs()));
	...
	LispObject computeDefaultInitargs() {
		...
		while (cpl != NIL) {
			LispObject c = cpl.car();
			assert c instanceof LispClass;
	...
	public void reFinalizeClass() {
		...
		if (defs == NIL) {
			...
			defs = getSlotDefinitions();
		}
		setClassLayout(computeLayout());
		final LispObject computeDefaultInitargs = computeDefaultInitargs();
	...
	public Layout computeLayout() {
		...
		LispObject[] instanceSlotNames = new LispObject[tail.length()];
		int i = 0;
		while (tail != NIL) {
			LispObject car = tail.car();
			final Fixnum loc = Fixnum.getInstance(i);

			if (car instanceof SlotDefinition) {
				SlotDefinition slotDefinition = (SlotDefinition) car;
				slotDefinition.setInstanceSlotValue(Symbol.LOCATION, loc);
				instanceSlotNames[i] = slotDefinition.getInstanceSlotValue(Symbol.NAME);
			} else if (car instanceof SimpleVector) {
				SimpleVector slotDefinition = (SimpleVector) car;
		...
		final SubLStructDecl layout2 = new SubLStructDecl(this, instanceSlotNames, NIL);
		return layout2;
	}
