...
public class Symbol extends AbstractSubLSymbol implements java.io.Serializable, SubLSymbol
{
  ...
  SubLObject callerValue;
	...
	protected LispObject unboundError() {
		...
		if(Lisp.insideToString>0) return SubLObjectFactory.makeString("UNBOUND VALUE: " + this.getQualifiedName());
		if(Main.isSubLisp()) Errors.error("UNBOUND VALUE: "  + this.getQualifiedName());
	...
	public SubLFunction getFunc() {
		...
		if (function!=null)return function.getFunc();
	...
	public boolean isGlobal() {
		return accessModeVar == VariableAccessMode.LEXICAL
				|| accessModeVar == VariableAccessMode.CONSTANT;
	}
	@Override
	...
	public CharSequence getPackageNamePrecise() {
		...
		if(pkg instanceof SubLPackage)  pkg.toPackage().getNameAsSubLString().getStringValue();
		return "#";
	}
	...
	public int hashCode(int currentDepth) {
		...
		if(this.identityHashCode <0) identityHashCode = (thePackage==null ? 0 : thePackage.hashCode()) ^ name.hashCode();
...
public Symbol(AbstractString string, Package pkg)
  ...
  private void checkSymbol() {
		...
		if(false)if(pkg==Lisp.PACKAGE_CYC) {
    ...
    public void setValueCL(LispObject argVal) {
		...
		if (symbol.isSpecialVariable() || env.isDeclaredSpecial(symbol)) {
			...
			if (!saved) {
				...
				thread.bindSpecial(symbol, argVal);
				 if(accessModeVar == VariableAccessMode.UNDECLARED) return;
				 symbol.setValueSL(argVal);
  ...
  public final LispObject symbolValueNoThrowInternal(LispThread thread)
  {
	  ...
	  SubLThread sublThread = thread.currentSubLThread();
		...
		if (result instanceof SymbolMacro) {
			if(env==null) env = Environment.newEnvironment();
