...
public final class Load {
	...
	public static final LispObject load(InputStream in, LispObject format) {
		Stream stream = Stream.createStream(Symbol.SYSTEM_STREAM, in, Symbol.CHARACTER, format);
		final LispThread thread = LispThread.currentThread();
		return loadFileFromStream(null, null, stream, Symbol.LOAD_VERBOSE.symbolValue(thread) != NIL,
				Symbol.LOAD_PRINT.symbolValue(thread) != NIL, false);
	}


	public static final LispObject load(final Pathname pathname, boolean verbose, boolean print, boolean ifDoesNotExist,
			boolean returnLastResult, LispObject externalFormat) {
		Pathname mergedPathname = null;
		if (!pathname.isAbsolute() && !pathname.isJar()) {
			Pathname pathnameDefaults = coerceToPathname(Symbol.DEFAULT_PATHNAME_DEFAULTS.symbolValue());
		...
		if (truename == null || NULL(truename)) {
			if (ifDoesNotExist) {
		...
		if (Utilities.checkZipFile(truename)) {
			...
			if (n.startsWith("jar:")) {
				n = "jar:" + n + "!/" + name + "." + getCompileFileTypeString();
			} else if (n.startsWith("zip:")) {
				n = "zip:" + n + "!/" + name + "." + getCompileFileTypeString();
			} else {
				n = "jar:file:" + Pathname.uriEncode(n) + "!/" + name + "." + getCompileFileTypeString();
			}
			if (!((mergedPathname = new Pathname(n)) instanceof Pathname)) {
				return error(
						new FileError((MessageFormat.format("Failed to address JAR-PATHNAME truename {0} for name {1}",
								truename.princToString(), name)), truename));
			}

			LispObject initTruename = coercePathnameOrNull(Pathname.truename(mergedPathname));
			if (initTruename == null || NULL(initTruename)) {
				...
				if (result instanceof Cons && ((Cons) result).length() == 1
						&& ((Cons) result).car() instanceof Pathname) {
					initTruename = result.car();
				} else {
					String errorMessage = "Loadable FASL not found for " + "'" + pathname.printObject() + "'" + " in "
		...
		try {
			return loadFileFromStream(pathname, truename,
					Stream.createStream(Symbol.SYSTEM_STREAM, in, Symbol.CHARACTER, externalFormat), verbose, print,
	...
	public static LispObject loadSystemFile(String filename, boolean auto) {
		...
		if (auto) {
			...
			thread.bindSpecial(Symbol.CURRENT_READTABLE, STANDARD_READTABLE.symbolValue(thread));
			...
			try {
				return loadSystemFile(filename, _AUTOLOAD_VERBOSE_.symbolValue(thread) != NIL,
						Symbol.LOAD_PRINT.symbolValue(thread) != NIL, auto);
			} finally {
				thread.resetSpecialBindings(mark);
			}
		} else {
			return loadSystemFile(filename, Symbol.LOAD_VERBOSE.symbolValue(thread) != NIL,
					Symbol.LOAD_PRINT.symbolValue(thread) != NIL, auto);
		}
	}
	...
	public static final LispObject loadSystemFile(final String filename, boolean verbose, boolean print, boolean auto) {
		...
		if (truename == null || Lisp.NULL(truename) || Lisp.NULL(bootPath)) {
			...
			if (url == null || url.toString().endsWith("/")) {
				...
				if (url == null) {
					url = Lisp.class.getResource(path + "." + _LISP_FILE_TYPE_.symbolValue().getStringValue());
				}
			}
			if (url == null) {
		...
		if (truename != null && truename.type.princToString().equals(COMPILE_FILE_TYPE)
			...
			if (t instanceof Pathname) {
				...
				return error(new LispError("Failed to find loadable init FASL in " + "'" + init.getNamestring() + "'"));
			}
		}
		...
		if (in != null) {
			...
			try {
				Stream stream = Stream.createStream(Symbol.SYSTEM_STREAM, in, Symbol.CHARACTER);
				return loadFileFromStream(pathname, truename, stream, verbose, print, auto);
			} finally {
				thread.resetSpecialBindings(mark);
				try {
					in.close();
	...
	// ### *fasl-version*
	// internal symbol
	static final Symbol _FASL_VERSION_ = exportConstant("*FASL-VERSION*", PACKAGE_SYS, Fixnum.getInstance(43));

	// ### *fasl-external-format*
	// internal symbol
	private static final Symbol _FASL_EXTERNAL_FORMAT_ = internConstant("*FASL-EXTERNAL-FORMAT*", PACKAGE_SYS,
			new SimpleString("UTF-8"));

	// ### *fasl-uninterned-symbols*
	// internal symbol
	/**
	 * This variable gets bound to NIL upon loading a FASL, but gets set to a vector
	 * of symbols as one of the first actions by the FASL itself.
	 *
	 */
	public static final Symbol _FASL_UNINTERNED_SYMBOLS_ = internSpecial("*FASL-UNINTERNED-SYMBOLS*", PACKAGE_SYS, NIL);

	// Function to access the uninterned symbols "array"
	public final static LispObject getUninternedSymbol(int n) {
		LispThread thread = LispThread.currentThread();
		LispObject uninternedSymbols = Load._FASL_UNINTERNED_SYMBOLS_.symbolValue(thread);
	...
	private static class init_fasl extends Primitive {
...

		{
			...
			return error(new SimpleError("FASL version mismatch; found '" + second.princToString() + "' but expected '"
					+ _FASL_VERSION_.getSymbolValue().princToString() + "' in "
					+ Symbol.LOAD_PATHNAME.symbolValue(thread).princToString() + " (try recompiling the file)"));
		}
	}

	private static final LispObject loadFileFromStream(Pathname pathname, Pathname truename, Stream in, boolean verbose,
			boolean print, boolean auto) {
		return loadFileFromStream(pathname == null ? NIL : pathname, truename == null ? NIL : truename, in, verbose,
				print, auto, false);
	}

	private static Symbol[] savedSpecials = new Symbol[] { // CLHS Specified
		...
		try {
			...
			if (!Lisp.NULL(truename)) {
				...
				if (type.equals(Lisp._COMPILE_FILE_TYPE_.symbolValue(thread).getStringValue())
						|| type.equals(COMPILE_FILE_INIT_FASL_TYPE.getStringValue())) {
				...
				if (truePathname.type.getStringValue().equals(COMPILE_FILE_INIT_FASL_TYPE.getStringValue())
					...
					if (truePathname.device.cdr() != NIL) {
						...
						if (!(truePathname.device.car() instanceof AbstractString)) {
							...
							truePathname = (Pathname) probe_file.PROBE_FILE.execute(p);
							truePathname.invalidateNamestring();
						}
					}
			...
			if (verbose) {
				...
				out._writeString(!NULL(truename) ? truePathname.princToString() : "stream");
				out._writeLine(" ...");
				out._finishOutput();
	 ...
	 * @return
	 */
	private static String getCompileFileTypeString() {
		return COMPILE_FILE_INIT_FASL_TYPE.getStringValue();
	}
...

	{
		...
		SpecialBinding sourcePositionBinding = thread.bindSpecial(_SOURCE_POSITION_, Fixnum.ZERO);
		try {
	...
	private static class _load extends Primitive {
		_load() {
			super("%load", PACKAGE_SYS, false, "filespec verbose print if-does-not-exist external-format");
		}
	...
	static final LispObject load(LispObject filespec, LispObject verbose, LispObject print, LispObject ifDoesNotExist,
			LispObject returnLastResult, LispObject externalFormat) {
		if (filespec instanceof Stream) {
			if (((Stream) filespec).isOpen()) {
				...
				if (pathname instanceof Pathname)
					...
					truename = NIL;
				return loadFileFromStream(pathname, truename, (Stream) filespec, verbose != NIL, print != NIL, false,
		...
		return load(pathname, verbose != NIL, print != NIL, ifDoesNotExist != NIL, returnLastResult != NIL,
				externalFormat);
	}
