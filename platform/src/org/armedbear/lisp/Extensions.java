...
public final class Extensions {
	// ### *ed-functions*
	public static final Symbol _ED_FUNCTIONS_ = exportSpecial("*ED-FUNCTIONS*", PACKAGE_EXT, list(intern("DEFAULT-ED-FUNCTION", PACKAGE_SYS)));
	...
	private static class quit extends Primitive {
		...
		public LispObject execute() {
			...
			if (Main.isNoDebug() || Startup.noExit) {
				return T;
			}
			throw new ProcessingTerminated();
		}
	...
	private static class getenv extends Primitive {
		getenv() {
			super("getenv", PACKAGE_EXT, true, "variable", "Return the value of the environment VARIABLE if it exists, otherwise return NIL.");
		}
	...
	private static class getenv_all extends Primitive {
		getenv_all() {
			super("getenv-all", PACKAGE_EXT, true, "variable", "Returns all environment variables as an alist containing (name . value)");
		}
		...
		public LispObject execute() {
			...
			for (Map.Entry<String, String> entry : env.entrySet()) {
				Cons entryPair = new Cons(new SimpleString(entry.getKey()), new SimpleString(entry.getValue()));
