...
public final class Java
{
	...
	@DocString(name = "register-java-exception", // => T
		args = "exception-name condition-symbol", doc = "Registers the Java Throwable named by the symbol EXCEPTION-NAME as the condition " + "designated by CONDITION-SYMBOL.  Returns T if successful, NIL if not.")
	...
	static Symbol getCondition(Class cl)
	{
		...
		for (Class c = cl; c != o; c = c.getSuperclass())
		{
			...
			if (object instanceof Symbol)
			{
				...
				if (isJavaException(lispClass)) { return (Symbol) object; }
			}
		}
		return null;
	}
	...
	{
		if (args.length < 2 || args.length > 4) error(new WrongNumberOfArgumentsException(fun, 2, 4));
	...
	private static final class pf_jconstructor extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 1) error(new WrongNumberOfArgumentsException(this, 1, -1));
			try
			{
				...
				for (int i = 0; i < constructors.length; i++)
				{
					...
					if (constructor.getParameterTypes().length == argCount) return JavaObject.getInstance(constructor);
				}
			...
			{ // ControlTransfer addressed above
				error(new LispError(getMessage(t)));
			}
			// Not reached.
			return NIL;
	...
	private static final class pf_jmethod extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 2) error(new WrongNumberOfArgumentsException(this, 2, -1));
			final Class<?> c = javaClass(args[0]);
			...
			try
			{
				...
				for (int i = 0; i < methods.length; i++)
				{
					...
					if (method.getName().equals(methodName) && method.getParameterTypes().length == argCount) return JavaObject.getInstance(method);
				}
				throw new NoSuchMethodException();
			} catch (final NoSuchMethodException e)
			{
...

	{
		...
		{ // ControlTransfer handled above
			...
			if (condition == null)
				...
				Symbol.SIGNAL.execute(condition, Keyword.CAUSE, JavaObject.getInstance(t), Keyword.FORMAT_CONTROL, new SimpleString(getMessage(t)));
		}
		// Not reached.
		return NIL;
	}
	...
	{
		if (args.length < 2) error(new WrongNumberOfArgumentsException(fun, 2, -1));
		try
		{
			Object a = args[0].javaInstance();
			for (int i = 1; i < args.length - 1; i++)
				a = Array.get(a, ((Integer) args[i].javaInstance()).intValue());
			return JavaObject.getInstance(Array.get(a, ((Integer) args[args.length - 1].javaInstance()).intValue()), translate);
		} catch (final Throwable t)
		{ // no code -> no ControlTransfer
			final Symbol condition = getCondition(t.getClass());
			if (condition == null)
	...
	@DocString(name = "jarray-ref", args = "java-array &rest indices", doc = "Dereferences the Java array JAVA-ARRAY using the given INDICIES, " + "coercing the result into a Lisp object, if possible.")
	...
	@DocString(name = "jarray-ref-raw", args = "java-array &rest indices", doc = "Dereference the Java array JAVA-ARRAY using the given INDICIES. " + "Does not attempt to coerce the result into a Lisp object.")
	...
	private static final class pf_jarray_set extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 3) error(new WrongNumberOfArgumentsException(this, 3, -1));
			try
			{
				Object a = args[0].javaInstance();
	...
	private static final class pf_jresolve_method extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			...
			if (instance == null) { return program_error("JRESOLVE-METHOD: instance must not be null."); }
	...
	private static Method findMethod(Object instance, Class<?> intendedClass, String methodName, Object[] methodArgs)
	{
		...
		if (method == null)
		{
			...
			if (intendedClass != actualClass)
			{
				...
				if (method != null)
				{
					if (isMethodCallableOnInstance(actualClass, method)) { return method; }
				}
			}
		}
		return method;
	...
	private static boolean isMethodCallableOnInstance(Class instance, Method method)
	{
		...
		if (instance.isMemberClass()) { return isMethodCallableOnInstance(instance.getEnclosingClass(), method); }
		return false;
	}
	...
	private static boolean isAssignable(Class<?> from, Class<?> to)
	{
		...
		else if (Float.class.equals(from)) { return Double.class.equals(to); }
		return false;
	}
	...
	private static Class<?> getBoxedClass(Class<?> clazz)
	{
		...
		{ // if (methodType.equals(void.class))
			return Void.class;
		}
	}
	...
	static Class javaClass(LispObject obj, ClassLoader classLoader)
	{
		if (obj instanceof AbstractString || obj instanceof Symbol)
		{
			...
			if (c == null) error(new LispError(s + " does not designate a Java class."));
		...
		if (javaObjectgetObject instanceof Class) { return (Class) javaObjectgetObject; }
	...
	static final String getMessage(Throwable t)
	{
		...
		if (message == null || message.length() == 0) message = t.getClass().getName();
		return message;
	}
}
