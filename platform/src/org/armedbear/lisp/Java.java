...
public final class Java {
    ...
    @DocString(name = "register-java-exception", // => T
            args = "exception-name condition-symbol", doc = "Registers the Java Throwable named by the symbol EXCEPTION-NAME as the condition "
    ...
    private static final class pf_register_java_exception extends Primitive {
        ...
        {
            final LispClass lispClass = (LispClass) LispClass.findClass(symbol, true);
            // FIXME Signal a continuable error if the exception is already registered.
            if (isJavaException(lispClass)) {
                registeredExceptions.put(classForName(className.getStringValue()), (Symbol) symbol);
                return T;
            }
            return NIL;
        }
    ...
    @DocString(name = "unregister-java-exception", args = "exception-name", doc = "Unregisters the Java Throwable EXCEPTION-NAME previously registered"
    ...
    static Symbol getCondition(Class cl) {
        final Class o = classForName("java.lang.Object");
        for (Class c = cl; c != o; c = c.getSuperclass()) {
            final Object object = registeredExceptions.get(c);
            if (object instanceof Symbol) {
                final LispClass lispClass = (LispClass) LispClass.findClass((Symbol) object, true);
                if (isJavaException(lispClass)) {
    ...
    @DocString(name = "jclass", args = "name-or-class-ref &optional class-loader", doc = "Returns a reference to the Java class designated by"
    ...
    private static final class pf_jclass extends Primitive {
        ...
        public LispObject execute(LispObject className, LispObject classLoader) {
            final ClassLoader loader = (ClassLoader) classLoader.javaInstance(ClassLoader.class);
    ...
    @DocString(name = "jconstructor", args = "class-ref &rest parameter-class-refs", doc = "Returns a reference to the Java constructor of CLASS-REF with the"
        ...
        public LispObject execute(LispObject[] args) {
            ...
            try {
                ...
                if (args.length == 2 && args[1] instanceof Fixnum) {
                    ...
                    final Class<?>[] parameterTypes = new Class[args.length - 1];
                ...
                throw e;
            } catch (final Throwable t) { // ControlTransfer addressed above
                error(new LispError(getMessage(t)));
            }
            // Not reached.
            return NIL;
    ...
    @DocString(name = "jmethod", args = "class-ref method-name &rest parameter-class-refs", doc = "Returns a reference to the Java method METHOD-NAME of CLASS-REF with the"
            + " given PARAMETER-CLASS-REFS.")
    private static final class pf_jmethod extends Primitive {
        ...
        public LispObject execute(LispObject[] args) {
            ...
            final Class<?> c = javaClass(args[0]);
            final String methodName = args[1].getStringValue();
            try {
                int argCount = 0;
...

    {
        ...
        try {
            ...
            if (t instanceof InvocationTargetException)
                t = t.getCause();
            final Symbol condition = getCondition(t.getClass());
            if (condition == null)
                error(new JavaException(t));
            else
                Symbol.SIGNAL.execute(condition, Keyword.CAUSE, JavaObject
                        .getInstance(t), Keyword.FORMAT_CONTROL, new SimpleString(getMessage(t)));
        }
        // Not reached.
        return NIL;
    }
    ...
    @DocString(name = "jstatic-raw", args = "method class &rest args", doc = "Invokes the static method METHOD on class CLASS with ARGS. Does not "
    ...
    @DocString(name = "jnew-array", args = "element-type &rest dimensions", doc = "Creates a new Java array of type ELEMENT-TYPE, with the given"
            + " DIMENSIONS.")
    private static final class pf_jnew_array extends Primitive {
        ...
        public LispObject execute(LispObject[] args) {
            ...
            try {
                final Class c = javaClass(args[0]);
                final int[] dimensions = new int[args.length - 1];
                for (int i = 1; i < args.length; i++)
...

    {
        ...
        try {
            ...
            return JavaObject
                    .getInstance(Array.get(a, ((Integer) args[args.length - 1].javaInstance()).intValue()), translate);
        } catch (final Throwable t) { // no code -> no ControlTransfer
            final Symbol condition = getCondition(t.getClass());
            if (condition == null)
                error(new JavaException(t));
    ...
    @DocString(name = "jarray-ref", args = "java-array &rest indices", doc = "Dereferences the Java array JAVA-ARRAY using the given INDICIES, "
    ...
    @DocString(name = "jarray-ref-raw", args = "java-array &rest indices", doc = "Dereference the Java array JAVA-ARRAY using the given INDICIES. "
    ...
    @DocString(name = "jarray-set", args = "java-array new-value &rest indices", doc = "Stores NEW-VALUE at the given index in JAVA-ARRAY.")
    private static final class pf_jarray_set extends Primitive {
        ...
        public LispObject execute(LispObject[] args) {
            ...
            try {
                Object a = args[0].javaInstance();
                final LispObject v = args[1];
                for (int i = 2; i < args.length - 1; i++)
                    a = Array.get(a, ((Integer) args[i].javaInstance()).intValue());
                final Object value = v.javaInstance();
                final int index = ((Integer) args[args.length - 1].javaInstance()).intValue();
                if (value instanceof java.lang.Number && a.getClass().getComponentType().equals(Byte.TYPE)) {
    ...
    @DocString(name = "jcall", args = "method-ref instance &rest args", doc = "Invokes the Java method METHOD-REF on INSTANCE with arguments ARGS,"
            + " coercing the result into a Lisp object, if possible.")
    ...
    @DocString(name = "jcall-raw", args = "method-ref instance &rest args", doc = "Invokes the Java method METHOD-REF on INSTANCE with arguments ARGS."
            + " Does not attempt to coerce the result into a Lisp object.")
    ...
    private static final class pf_jresolve_method extends Primitive {
        ...
        public LispObject execute(LispObject[] args) {
            ...
            if (instanceArg instanceof AbstractString) {
                ...
                final JavaObject jobj = ((JavaObject) instanceArg);
            ...
            final String methodName = methodArg.getStringValue();
            final Object[] methodArgs = translateMethodArguments(args, 2);
...

    {
        ...
        try {
                ...
                if (method == null) {
                    if (intendedClass == null) {
                        final String msg = MessageFormat
                                .format("No instance method named {0} found for type {1}", methodName, instance
                                        .getClass().getName());
            ...
            if (argTypes.length != args.length - 2) {
                return error(new WrongNumberOfArgumentsException("Wrong number of arguments for " + method
    ...
    private static Object[] translateMethodArguments(LispObject[] args, int offs) {
        final int argCount = args.length - offs;
        final Object[] javaArgs = new Object[argCount];
        for (int i = 0; i < argCount; ++i) {
            final Object x = args[i + offs];
            if (x.equals(NIL)) {
                javaArgs[i] = false;
            } else if (x.equals(T)) {
                javaArgs[i] = true;
    ...
    private static Method findMethod(Method[] methods, String methodName, Object[] javaArgs) {
        final int argCount = javaArgs.length;
        Method result = null;
    ...
    private static Method findMethod(Class<?> c, String methodName, LispObject[] args, int offset) {
        final Object[] javaArgs = translateMethodArguments(args, offset);
    ...
    static Constructor findConstructor(Class<?> c, LispObject[] args) throws NoSuchMethodException {
        ...
        final Object[] javaArgs = translateMethodArguments(args, 1);
    ...
    private static boolean isAssignable(Class<?> from, Class<?> to) {
        ...
        if (Byte.class.equals(from)) {
            return Short.class.equals(to) || Integer.class.equals(to) || Long.class.equals(to) || Float.class.equals(to)
                    || Double.class.equals(to);
        } else if (Short.class.equals(from) || Character.class.equals(from)) {
            return Integer.class.equals(to) || Long.class.equals(to) || Float.class.equals(to)
                    || Double.class.equals(to);
    ...
    public static boolean isApplicableMethod(Class<?>[] methodTypes, Object[] args) {
    ...
    public static boolean isMoreSpecialized(Class<?>[] xtypes, Class<?>[] ytypes) {
        for (int i = 0; i < xtypes.length; ++i) {
            final Class<?> xtype = maybeBoxClass(xtypes[i]);
            final Class<?> ytype = maybeBoxClass(ytypes[i]);
    ...
    private static final class pf_make_immediate_object extends Primitive {
        ...
        public LispObject execute(LispObject[] args) {
            ...
            if (args.length < 1)
                error(new WrongNumberOfArgumentsException(this, 1, -1));
            final LispObject object = args[0];
            if (args.length > 1) {
                final LispObject type = args[1];
    ...
    private static final class pf_jnull_ref_p extends Primitive {
        ...
        public LispObject execute(LispObject ref) {
            if (ref instanceof JavaObject) {
                final JavaObject jref = (JavaObject) ref;
    ...
    @DocString(name = "jcoerce", args = "object intended-class", doc = "Attempts to coerce OBJECT into a JavaObject of class INTENDED-CLASS."
    ...
    @DocString(name = "jrun-exception-protected", args = "closure", doc = "Invokes the function CLOSURE and returns the result.  "
    ...
    static Class javaClass(LispObject obj, ClassLoader classLoader) {
        ...
        if (obj instanceof JavaObject) {
            ...
            type_error(obj, list(Symbol.OR, Symbol.STRING, Symbol.JAVA_OBJECT));
            // Not reached.
            return null;
        }
