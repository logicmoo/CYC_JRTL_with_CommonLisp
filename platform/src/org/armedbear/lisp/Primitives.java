...
public final class Primitives
{
	...
	private static final class pf_min extends Primitive
	{
		...
		{
			LispObject result = args[0];
			if (!result.realp()) type_error(result, Symbol.REAL);
			for (int i = 1; i < args.length; i++)
			{
				if (args[i].isLessThan(result)) result = args[i];
			}
			return result;
		}
	};
	...
	private static final class pf_max extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			...
			for (int i = 1; i < args.length; i++)
			{
				if (args[i].isGreaterThan(result)) result = args[i];
			}
			return result;
		}
	};
	...
	private static final class pf_compiled_lisp_function_p extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			return (arg instanceof CompiledClosure || arg instanceof CompiledPrimitive) ? T : NIL;
		}
	}
	...
	private static final class pf__eval extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			return eval(arg, Environment.newEnvironment(), LispThread.currentThread());
		}
	};
	...
	private static final class pf_values_list extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg == NIL) return LispThread.currentThread().setValues();
			if (arg.cdr() == NIL) return arg.car();
	...
	// ### elt-safe
	private static final Primitive ELT_SAFE = new pf_elt_safe();

	private static final class pf_elt_safe extends Primitive
	{
		pf_elt_safe()
		{
			super("ELT-SAFE", PACKAGE_SYS, false, "sequence index");
		}

		LispObject expected = null;
		int len = 0;
		int offset = 0;
...

		{
			...
			int index = Fixnum.getValue(second) - offset;
			if (index < 0 || index >= len) { return NIL; }
			return first.elt(index);
		}
	};
	...
	// ### special-method-p
	private static final Primitive SPECIAL_METHOD_P = new pf_SPECIAL_METHOD_P();

	private static final class pf_SPECIAL_METHOD_P extends Primitive
	{
		pf_SPECIAL_METHOD_P()
		{
			super("special-method-p", PACKAGE_EXT, true, "symbol");
		}
		...
		public LispObject execute(LispObject arg)
		{
			...
			{
				arg = ((Symbol) arg).getSymbolFunction();
				if (arg == null) return NIL;
			}

			if (arg instanceof SpecialMethod) { return T; }
			return NIL;
		}
	};
	...
	private static final class sf_when extends SpecialOperator
	{
		...
		{
			if (args == NIL) return error(new WrongNumberOfArgumentsException(this, 1, -1));
	...
	private static final class pf__terpri extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			else if (arg == NIL) arg = Symbol.STANDARD_OUTPUT.symbolValue();
			final Stream stream;
	...
	private static final class pf_boundp extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			if (binding != null) return binding.value != null ? T : NIL;
			// No binding.
	...
	private static final class pf_fboundp extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Symbol) return arg.getSymbolFunction() != null ? T : NIL;
	...
	private static final class pf_append extends Primitive
	{
		...
		{
			if (first == NIL) return execute(second, third);
			Cons result = new Cons(first.car());
			Cons splice = result;
			first = first.cdr();
			while (first != NIL)
	...
	private static final class pf_equals extends Primitive
	{
		...
		public LispObject execute(LispObject[] array)
		{
			...
			{
				if (array[i].isNotEqualTo(obj)) return NIL;
			}
			return T;
		}
	};
	...
	private static final class pf_not_equals extends Primitive
	{
		...
		{
			if (first.isEqualTo(second)) return NIL;
			if (first.isEqualTo(third)) return NIL;
			if (second.isEqualTo(third)) return NIL;
			return T;
		}
		...
		public LispObject execute(LispObject[] array)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				{
					if (array[j].isEqualTo(obj)) return NIL;
				}
			}
			return T;
		}
	...
	private static final class pf_lt extends Primitive
	{
		...
		public LispObject execute(LispObject[] array)
		{
			...
			for (int i = 1; i < length; i++)
			{
				if (array[i].isLessThanOrEqualTo(array[i - 1])) return NIL;
			}
			return T;
		}
	};
	...
	private static final class pf_le extends Primitive
	{
		...
		public LispObject execute(LispObject[] array)
		{
			...
			for (int i = 1; i < length; i++)
			{
				if (array[i].isLessThan(array[i - 1])) return NIL;
			}
			return T;
		}
	};
	...
	private static final class pf_gt extends Primitive
	{
		...
		public LispObject execute(LispObject[] array)
		{
			...
			for (int i = 1; i < length; i++)
			{
				if (array[i].isGreaterThanOrEqualTo(array[i - 1])) return NIL;
			}
			return T;
		}
	};
	...
	private static final class pf_ge extends Primitive
	{
		...
		public LispObject execute(LispObject[] array)
		{
			...
			for (int i = 1; i < length; i++)
			{
				if (array[i].isGreaterThan(array[i - 1])) return NIL;
			}
			return T;
		}
	};
	...
	private static final class pf__set_nth extends Primitive
	{
...

		{
			...
			if (index < 0) error(new TypeError("(SETF NTH): invalid index " + index + "."));
			int i = 0;
			while (true)
			{
				if (i == index)
				...
				second = second.cdr();
				if (second == NIL) { return error(new LispError("(SETF NTH): the index " + index + "is too large.")); }
				++i;
			}
		}
	};
	...
	private static final class pf_nthcdr extends Primitive
	{
...

		{
			...
			if (index < 0) return type_error(first, list(Symbol.INTEGER, Fixnum.ZERO));
	...
	private static final class pf_signal extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 1) return error(new WrongNumberOfArgumentsException(this, 1, -1));
			if (args[0] instanceof Condition) return error((Condition) args[0]);
	...
	private static final class pf__format extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 2) return error(new WrongNumberOfArgumentsException(this, 2, -1));
...

		{
			...
			if (destination == NIL) return new SimpleString(s);
			if (destination instanceof TwoWayStream)
			{
				...
				error(new TypeError("The value " + destination.princToString() + " is not a character output stream."));
			}
		...
		if (_WARN_ON_REDEFINITION_.symbolValue(thread) != NIL)
		{
			if (arg instanceof Symbol)
			{
				...
				if (oldDefinition != null && !(oldDefinition instanceof Autoload))
				{
					...
					if (currentSource == NIL) currentSource = Keyword.TOP_LEVEL;
					if (oldSource != NIL)
					{
						if (currentSource.equal(oldSource)) return; // OK
					}
	...
	private static final class pf__defun extends Primitive
	{
...

		{
			if (name instanceof Symbol)
			{
				...
				if (symbol.getSymbolFunction() instanceof SpecialOperator) { return program_error(symbol.getName() + " is a special operator and may not be redefined."); }
			}
			else if (!isValidSetfFunctionName(name)) return type_error(name, FUNCTION_NAME);
	...
	private static final class pf_fdefinition_block_name extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Symbol) return arg;
			if (isValidSetfFunctionName(arg)) return arg.cadr();
			return type_error(arg, FUNCTION_NAME);
		}
	};
	...
	private static final class pf_macro_function extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			if (obj instanceof MacroObject) return ((MacroObject) obj).getExpander();
			if (obj instanceof SpecialOperator)
			{
				...
				{
					((AutoloadMacro) obj).load();
					obj1 = get(arg, Symbol.MACROEXPAND_MACRO, NIL);
					if (obj1 != NIL)
					{
						obj = obj1;
					}
				...
				if (obj instanceof MacroObject) return ((MacroObject) obj).getExpander();
				if (obj instanceof SpecialOperator) obj1 = (LispObject) ((SpecialOperator) obj).getEvaluationFunction();
				if (obj1 != null && obj1 != NIL) { return obj1; }
			}
			return NIL;
		}
	...
	private static final class pf__defparameter extends Primitive
	{
...

		{
			...
			else if (third != NIL) type_error(third, Symbol.STRING);
	...
	private static final class pf__defvar extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			final Symbol symbol;
			symbol = checkSymbol(arg);
			//symbol.setSpecialDynamic(true);
			symbol.setAccessMode(VariableAccessMode.DYNAMIC);
			return symbol;
		}
	...
	private static final class sf_cond extends SpecialOperator
	{
		...
		public LispObject execute(LispObject args, Environment env)
		{
			...
			while (args != NIL)
			{
				...
				if (!(clause instanceof Cons)) return error(new ProgramError("COND clause is not a non-empty list: " + clause.princToString()));
			...
			while (args != NIL)
			{
				...
				else
				{
					...
					if (candidate == T || candidate == Symbol.OTHERWISE)
						match = true;
					else if (key.eql(candidate)) match = true;
				}
				if (match) { return progn(clause.cdr(), env, thread); }
				args = args.cdr();
			}
			return NIL;
		}
	...
	private static final class sf_ecase extends SpecialOperator
	{
...

		{
			...
			while (clauses != NIL)
			{
				...
				{
					LispObject candidate = keys;
					if (key.eql(candidate)) match = true;
				}
				if (match) { return progn(clause.cdr(), env, thread); }
				clauses = clauses.cdr();
			}
		...
		public LispObject execute(LispObject[] args)
		{
			...
			for (int i = 0; i < rank; i++)
			{
				...
				if (arg instanceof Fixnum)
				{
					...
					if (subscript < 0 || subscript >= array.getDimension(i)) return NIL;
				}
	...
	private static final class pf_aref extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			if (array.getRank() == 0) return array.AREF(0);
	...
	private static final class pf_aset extends Primitive
	{
...

		{
			...
			{
				return error(new TypeError("The value " + first.princToString() + " is not an array of rank 0."));
			}
			array.aset(0, second);
			return second;
		}
	...
	private static final class pf_fill_pointer extends Primitive
	{
...

		{
			if (arg instanceof AbstractArray)
			{
				...
				if (aa.hasFillPointer()) return Fixnum.getInstance(aa.getFillPointer());
			}
	...
	private static final class pf_vector_push extends Primitive
	{
...

		{
			...
			if (fillPointer < 0) v.noFillPointer();
			if (fillPointer >= v.capacity()) return NIL;
	...
	private static final class pf_vector_pop extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			int fillPointer = v.getFillPointer();
			if (fillPointer < 0) v.noFillPointer();
			if (fillPointer == 0) error(new LispError("nothing left to pop"));
	...
	private static final class pf_mapcar extends Primitive
	{
		...
		{
			final int numArgs = args.length;
			if (numArgs < 2) return error(new WrongNumberOfArgumentsException(this, 2, -1));
			int commonLength = -1;
			for (int i = 1; i < numArgs; i++)
			{
				if (!args[i].listp()) type_error(args[i], Symbol.LIST);
				int len = args[i].length();
				if (commonLength < 0)
					commonLength = len;
				else if (commonLength > len) commonLength = len;
			}
			final LispThread thread = LispThread.currentThread();
	...
	private static final class pf_macroexpand extends Primitive
	{
		...
		{
			return macroexpand(form, Environment.newEnvironment(), LispThread.currentThread());
		}
		...
		{
			return macroexpand(form, env != NIL ? checkEnvironment(env) : Environment.newEnvironment(), LispThread.currentThread());
		}
	};
	...
	private static final class pf_macroexpand_1 extends Primitive
	{
		...
		public LispObject execute(LispObject form)
		{
			return macroexpand_1(form, Environment.newEnvironment(), LispThread.currentThread());
		}
		...
		{
			return macroexpand_1(form, env != NIL ? checkEnvironment(env) : Environment.newEnvironment(), LispThread.currentThread());
		}
	};
	...
	private static final class pf_gensym extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			else if (arg instanceof AbstractString) return gensym(arg.getStringValue(), LispThread.currentThread());
	...
	private static final class pf_unintern extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length == 0 || args.length > 2) return error(new WrongNumberOfArgumentsException(this, 1, 2));
	...
	private static final class pf_find_package extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Package) return arg;
			if (arg instanceof AbstractString)
			{
				Package pkg = getCurrentPackage().findPackage(arg.getStringValue());
	...
	private static final class pf__make_package extends Primitive
	{
...

		{
			...
			if (pkg != null) error(new LispError("Package " + packageName + " already exists."));
			...
			if (use != NIL)
			{
				...
				while (list != NIL)
				{
					...
					else
					{
						...
						if (p == null)
						{
							error(new LispError(obj.princToString() + " is not the name of a package."));
							return NIL;
						}
					}
					list = list.cdr();
	...
	private static final class pf__in_package extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			if (pkg == null) return error(new PackageError("The name " + packageName + " does not designate any package."));
	...
	private static final class pf_use_package extends Primitive
	{
		...
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 1 || args.length > 2) return error(new WrongNumberOfArgumentsException(this, 1, 2));
			Package pkg;
			if (args.length == 2)
				pkg = coerceToPackage(args[1]);
			else
	...
	// ### import-package packages-to-use &optional package => t
	private static final Primitive IMPORT_PACKAGE = new pf_import_package();

	private static final class pf_import_package extends Primitive
	{
		pf_import_package()
		{
			super("IMPORT-PACKAGE", PACKAGE_SYS, true, "packages-to-import &optional package");
		}

		@Override
		public LispObject execute(LispObject[] args)
		{
			if (args.length < 1 || args.length > 3) return error(new WrongNumberOfArgumentsException(this, 1, 3));
			Package pkg;
			boolean overrideImports = false;
			if (args.length == 3)
				pkg = coerceToPackage(args[2]);
			else
				pkg = getCurrentPackage();

			pkg.ALLOW_INHERIT_CONFLICTS = true;

			if (args.length > 1)
			{
				overrideImports = args[1].getBooleanValue();
			}
			...
			if (args[0].listp())
			{
				...
				while (list != NIL)
				{
					pkg.importPackage(coerceToPackage(list.car()), overrideImports);
					list = list.cdr();
				}
			}
			else
				pkg.importPackage(coerceToPackage(args[0]), overrideImports);
			return T;
		}
	};
	...
	private static final class pf_find_symbol extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			return getCurrentPackage().findSymbol(checkString(arg).getStringValue());
		}
		...
		{
			return coerceToPackage(second).findSymbol(checkString(first).getStringValue());
		}
	};
	...
	private static final class pf_fset extends Primitive
	{
...

		{
			if (first instanceof Symbol)
			{
				...
				if (sourcePathname != NIL) sourcePosition = _SOURCE_POSITION_.symbolValue(thread);
				if (sourcePathname == NIL) sourcePathname = Keyword.TOP_LEVEL;
			...
			if (second instanceof Operator)
			{
				...
				if (fourth != NIL) op.setLambdaList(fourth);
				if (fifth != NIL) op.setDocumentation(Symbol.FUNCTION, fifth);
			}
			return second;
		}
	};
...

		{
			...
			try
			{
				...
				while (defs != NIL)
				{
					...
					Symbol make_expander_for_macrolet = PACKAGE_SYS.intern("MAKE-MACRO-EXPANDER");
	...
	private static final class pf_make_macro_expander extends Primitive
	{
...

		{
			...
			LispObject toBeApplied = list(Symbol.FUNCTION, list(Symbol.MACRO_FUNCTION, lambdaList, block));
			final LispThread thread = LispThread.currentThread();
			LispObject formArg = gensym("WHOLE-", thread);
			LispObject envArg = gensym("ENVIRONMENT-", thread); // Ignored.
			LispObject expander = list(Symbol.LAMBDA, list(formArg, envArg), list(Symbol.APPLY, toBeApplied, list(Symbol.CDR, formArg)));
			return expander;
		}
	};
	...
	private static final class sf_go extends SpecialOperator
	{
		...
		{
			if (args.length() != 1) return error(new WrongNumberOfArgumentsException(this, 1));
			Binding binding = env.getTagBinding(args.car());
			if (binding == null) return error(new ControlError("No tag named " + args.car().princToString() + " is currently visible."));
	...
	private static final class sf_block extends SpecialOperator
	{
...

		{
			...
			{
				if (ret.getBlock() == block) { return ret.getResult(); }
				throw ret;
			} finally
			{
				ext.inactive = true;
	...
	private static final class sf_return_from extends SpecialOperator
	{
		...
		{
			final int length = args.length();
			if (length < 1 || length > 2) return error(new WrongNumberOfArgumentsException(this, 1, 2));
			Symbol symbol;
			symbol = checkSymbol(args.car());

			return Lisp.nonLocalReturn(env.getBlockBinding(symbol), symbol, (length == 2) ? eval(args.cadr(), env, LispThread.currentThread()) : NIL);
		}
	};
	...
	private static final class sf_catch extends SpecialOperator
	{
...

		{
			...
			{
				if (t.tag == tag) { return t.getResult(thread); }
				throw t;
			} catch (Return ret)
			{
				throw ret;
	...
	private static final class sf_throw extends SpecialOperator
	{
		...
		{
			if (args.length() != 2) return error(new WrongNumberOfArgumentsException(this, 2));
			final LispThread thread = LispThread.currentThread();
	...
	private static final class sf_eval_when extends SpecialOperator
	{
...

		{
			...
			if (situations != NIL)
			{
				if (memq(Keyword.EXECUTE, situations) || memq(Symbol.EVAL, situations)) { return progn(args.cdr(), env, LispThread.currentThread()); }
			}
			return NIL;
		}
	};
	...
	private static final class sf_multiple_value_prog1 extends SpecialOperator
	{
		...
		{
			if (args.length() == 0) return error(new WrongNumberOfArgumentsException(this, 1, -1));
			final LispThread thread = LispThread.currentThread();
	...
	private static final class sf_multiple_value_call extends SpecialOperator
	{
...

		{
			...
			if (obj instanceof Symbol)
			{
				...
				if (function == null) error(new UndefinedFunction(obj));
			}
			...
			{
				error(new LispError(obj.princToString() + " is not a function name."));
				return NIL;
			}
	...
	private static final class sf_multiple_value_list extends SpecialOperator
	{
...

		{
			...
			LispObject[] values = thread._values;
			if (values == null) return new Cons(result);
			thread._values = null;
			LispObject list = NIL;
	...
	private static final class pf_lambda_name extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Operator) { return ((Operator) arg).getLambdaName(); }
			if (arg.typep(Symbol.GENERIC_FUNCTION) != NIL) { return Symbol.GENERIC_FUNCTION_NAME.execute(arg); }
			if (arg instanceof FuncallableStandardObject) { return this.execute(((FuncallableStandardObject) arg).function); }
			return type_error(arg, Symbol.FUNCTION);
		}
	};
	...
	private static final class pf__set_lambda_name extends Primitive
	{
...

		{
			...
			if (first instanceof FuncallableStandardObject) { return this.execute(((FuncallableStandardObject) first).function, second); }
	...
	private static final class pf_subseq extends Primitive
	{
...

		{
			...
			if (first.listp()) return list_subseq(first, start, -1);
			if (first instanceof AbstractVector)
			{
...

		{
			...
			if (first.listp()) return list_subseq(first, start, end);
			if (first instanceof AbstractVector)
			{
				final AbstractVector v = (AbstractVector) first;
...

	{
		...
		while (list != NIL)
		{
			if (end >= 0 && index == end) return result.nreverse();
			if (index++ >= start) result = new Cons(list.car(), result);
			list = list.cdr();
		}
	...
	private static final class pf__set_elt extends Primitive
	{
...

		{
			...
			if (first instanceof Cons)
			{
				...
				if (index < 0) error(new TypeError());
				...
				while (true)
				{
					...
					list = list.cdr();
					if (list == NIL) error(new TypeError());
					++i;
				}
			}
			return type_error(first, Symbol.SEQUENCE);
	...
	private static final class pf__make_list extends Primitive
	{
...

		{
			...
			if (size < 0) return type_error(first, list(Symbol.INTEGER, Fixnum.ZERO, Symbol.MOST_POSITIVE_FIXNUM.getSymbolValue()));
			LispObject result = NIL;
			for (int i = size; i-- > 0;)
	...
	private static final class pf__member extends Primitive
	{
...

		{
			...
			if (test != NIL && testNot != NIL) error(new LispError("MEMBER: test and test-not both supplied"));
			if (testNot == NIL)
			{
				if (test == NIL || test == Symbol.EQL) test = EQL;
			}
			if (key == NIL)
			{
				if (test == EQL)
				{
					while (tail instanceof Cons)
					{
						if (item.eql(((Cons) tail).car)) return tail;
						tail = ((Cons) tail).cdr;
					}
				}
				else if (test != NIL)
				{
					while (tail instanceof Cons)
					{
						...
						if (test.execute(item, candidate) != NIL) return tail;
						tail = ((Cons) tail).cdr;
					}
				}
				else
				{
					...
					{
						LispObject candidate = ((Cons) tail).car;
						if (testNot.execute(item, candidate) == NIL) return tail;
						tail = ((Cons) tail).cdr;
					}
				}
			}
			...
			if (tail != NIL) type_error(tail, Symbol.LIST);
			return NIL;
		}
	};
	...
	private static final class pf_funcall_key extends Primitive
	{
		...
		{
			if (first != NIL) return LispThread.currentThread().execute(first, second);
			return second;
		}
	};
	...
	private static final class pf_make_closure extends Primitive
	{
...

		{
			if (first instanceof Cons && ((Cons) first).car == Symbol.LAMBDA)
			{
				...
				return new LambdaClosure(first, env);
			}
	...
	private static final class pf_complex extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof SingleFloat) return Complex.getInstance(arg, SingleFloat.ZERO);
			if (arg instanceof DoubleFloat) return Complex.getInstance(arg, DoubleFloat.ZERO);
			if (arg.realp()) return arg;
			return type_error(arg, Symbol.REAL);
		}
	...
	private static final class pf_realpart extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Complex) return ((Complex) arg).getRealPart();
			if (arg.numberp()) return arg;
			return type_error(arg, Symbol.NUMBER);
		}
	};
	...
	private static final class pf_imagpart extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Complex) return ((Complex) arg).getImaginaryPart();
	...
	private static final class pf_integer_length extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			...
			if (arg instanceof Bignum) return Fixnum.getInstance(((Bignum) arg).value.bitLength());
	...
	private static final class pf_simple_vector_search extends Primitive
	{
...

		{
			...
			if (first.length() == 0) return Fixnum.ZERO;
			...
			if (first instanceof AbstractVector)
			{
				...
				for (int i = 0; i <= limit; i++)
				{
					if (v.AREF(i).eql(element))
					{
						...
						if (match) return Fixnum.getInstance(i);
					}
				}
			}
			else
	...
	private static final class pf_keywordp extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof Symbol)
			{
				if (checkSymbol(arg).getPackageOrNil() == PACKAGE_KEYWORD) return T;
			}
			return NIL;
		}
	};
	...
	private static final class pf_make_symbol extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof AbstractString) return new Symbol((AbstractString) arg);
			// Not a simple string.
			if (arg instanceof AbstractString) return new Symbol(arg.getStringValue());
	...
	private static final class pf__class_name extends Primitive
	{
		...
		public LispObject execute(LispObject arg)
		{
			if (arg instanceof LispClass) return ((LispClass) arg).getLispClassName();
	...
	private static final class pf__set_class_name extends Primitive
	{
...

		{
			if (second instanceof LispClass)
				...
				((StandardObject) second).setInstanceSlotValue(StandardClass.symName, checkSymbol(first));
			return first;
		}
	};
...

		{
			...
			if (doc == NIL)
			{
				if (docType == Symbol.FUNCTION && object instanceof Symbol)
				{
					...
					if (function != null) doc = function.getDocumentation(docType);
				}
			}
			return doc;
		}
