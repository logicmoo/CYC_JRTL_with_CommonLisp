...
public class SimpleArray_UnsignedByte16 extends AbstractSimpleArray {
...

	{
		...
		int axis = 0;
		SimpleArray_UnsignedByte16.copySubArray(a1, a2, subscripts, axis);
	}
...

	{
		if (axis < subscripts.length) {
			...
			for (int i = 0; i < limit; i++) {
				subscripts[axis] = i;
				SimpleArray_UnsignedByte16.copySubArray(a1, a2, subscripts, axis + 1);
			}
		} else {
...

	{
		...
		this.data = new int[this.totalSize];
...

	{
		...
		for (int i = 0; i < dimv.length; i++)
			if (dimv[i] != this.dimv[i]) {
				SimpleArray_UnsignedByte16 newArray = new SimpleArray_UnsignedByte16(dimv);
				if (initialElement != null)
					newArray.fillVoid(initialElement);
				SimpleArray_UnsignedByte16.copyArray(this, newArray);
				return newArray;
			}
	...
	public SubLObject AREF(int index) {
		try {
			return LispObjectFactory.makeInteger(this.data[index]);
		} catch (ArrayIndexOutOfBoundsException e) {
	...
	public SubLObject get(int[] subscripts) {
		try {
			return LispObjectFactory.makeInteger(this.data[this.getRowMajorIndex(subscripts)]);
		} catch (ArrayIndexOutOfBoundsException e) {
	...
	{
		try {
			this.data[this.getRowMajorIndex(subscripts)] = obj.intValue();
		} catch (ArrayIndexOutOfBoundsException e) {
...

	{
		if (dims.length == 0) {
			try {
				this.data[index] = Lisp.coerceLispObjectToJavaByte(contents);
			} catch (ArrayIndexOutOfBoundsException e) {
	...
	public SubLObject typeOf() {
		return Lisp.list(LispSymbols.SIMPLE_ARRAY, Lisp.UNSIGNED_BYTE_16, this.getDimensions());
	}
