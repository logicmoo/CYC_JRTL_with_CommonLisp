...
public class Fixnum extends AbstractSubLIntegerBignum
{
  ...
  public static final Fixnum THIRTY_TWO= constants[32];

  public static final Fixnum MINUS_ONE = Fixnum.getInstanceInternal(-1);
...
public LispObject LDB(int size, int position)
  ...
  public LispObject pow(LispObject obj)
  {
    ...
    if (y.compareTo (BigInteger.ZERO) < 0)
      return (Fixnum.getInstance(1)).divideBy((LispObject)this.pow((LispObject)Bignum.getInstance(y.negate())));
...
final public String printObject()
  {
    final LispThread thread = LispThread.currentThread();
    int base;
    if(Symbol.PRINT_BASE!=null && Symbol.PRINT_BASE.boundp()) {
    	base = Fixnum.getValue(Symbol.PRINT_BASE.symbolValue(thread));
    } else {
    	base =10;
    }
	...
	public boolean greaterThanInternal(SubLObject p0) {
		return getInteger().compareTo(p0.intValue()) > 0;
	}
