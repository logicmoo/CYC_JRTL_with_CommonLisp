...
abstract public class Lisp extends ABCLStatic {
	...
	static final LispObject nonLocalGo(Binding binding, LispObject tag) {
		if (binding.env.inactive)
			return error(new ControlError("Unmatched tag " + binding.symbol.princToString() + " for GO outside of lexical extent."));
	...
	{
		if (blockId == null)
			return error(new ControlError("Unmatched block " + blockName.princToString() + " for " + "RETURN-FROM outside lexical extent."));
	...
	public static final LispObject nonLocalReturn(Binding binding, Symbol block, LispObject result) {
		...
		if (binding.env.inactive)
			return error(new ControlError("Unmatched block " + binding.symbol.princToString() + " for RETURN-FROM outside of" + " lexical extent."));
			...
			if (current instanceof Cons) {
				try {
					...
					if (((Cons) current).car == Symbol.GO) {
						...
						if (binding == null)
							return error(new ControlError("No tag named " + tag.princToString() + " is currently visible."));
	...
	public static final Cons list7(LispObject obj1, LispObject obj2, LispObject obj3, LispObject obj4, LispObject obj5, LispObject obj6, LispObject obj7) {
		return new Cons(obj1, new Cons(obj2, new Cons(obj3, new Cons(obj4, new Cons(obj5, new Cons(obj6, new Cons(obj7)))))));
	}
	...
	public static final Cons list8(LispObject obj1, LispObject obj2, LispObject obj3, LispObject obj4, LispObject obj5, LispObject obj6, LispObject obj7, LispObject obj8) {
		return new Cons(obj1, new Cons(obj2, new Cons(obj3, new Cons(obj4, new Cons(obj5, new Cons(obj6, new Cons(obj7, new Cons(obj8))))))));
	}
	...
	public static final Cons list9(LispObject obj1, LispObject obj2, LispObject obj3, LispObject obj4, LispObject obj5, LispObject obj6, LispObject obj7, LispObject obj8, LispObject obj9) {
		return new Cons(obj1, new Cons(obj2, new Cons(obj3, new Cons(obj4, new Cons(obj5, new Cons(obj6, new Cons(obj7, new Cons(obj8, new Cons(obj9)))))))));
	}
...

	{
		...
		if (binding != null) {
			...
			if ((oldValue instanceof Fixnum || oldValue instanceof Bignum) && Fixnum.ZERO.isLessThanOrEqualTo(oldValue)) {
				...
				error(new TypeError("The value of *GENSYM-COUNTER* was not a nonnegative integer. Old value: " + oldValue.princToString() + " New value: 0"));
			}
		} else {
...

	{
		...
		while (list != NIL) {
			if (!(list.cdr() instanceof Cons))
				error(new ProgramError("The symbol " + symbol.princToString() + " has an odd number of items in its property list."));
			if (list.car() == indicator) {
				// Found it!
				if (prev != null)
	...
	// ### *compile-file-type*
	public static final Symbol _COMPILE_FILE_TYPE_ = exportSpecial("*COMPILE-FILE-TYPE*", PACKAGE_SYS, new SimpleString("abcl"));

	// ### *lisp-file-type*
	public static final Symbol _LISP_FILE_TYPE_ = exportSpecial("*LISP-FILE-TYPE*", PACKAGE_EXT, new SimpleString("lisp"));

	// ### *lisp-file-type-list*
	public static final Symbol _LISP_FILE_TYPE_LIST_ = exportSpecial("*LISP-FILE-TYPE-LIST*", PACKAGE_EXT, list(new SimpleString("lisp"), new SimpleString("cl"), new SimpleString("lsp")));

	// ### *compile-file-class-extension*
	public static final Symbol _COMPILE_FILE_CLASS_EXTENSION_ = exportSpecial("*COMPILE-FILE-CLASS-EXTENSION*", PACKAGE_SYS, new SimpleString("cls"));
	...
	static {
		...
		Symbol.PRINT_READABLY.isTraced = 1;
	...
	// ### *disassembler*
	public static final Symbol _DISASSEMBLER_ = exportSpecial("*DISASSEMBLER*", PACKAGE_EXT, new SimpleString("jad -a -p")); // or "jad -dis -p"
	...
	// ### *bq-vector-flag*
	public static final Symbol _BQ_VECTOR_FLAG_ = internSpecial("*BQ-VECTOR-FLAG*", PACKAGE_SYS, list(new Symbol("bqv")));

	// ### *traced-names*
	public static final Symbol _TRACED_NAMES_ = exportSpecial("*TRACED-NAMES*", PACKAGE_SYS, NIL);

	// ### *default-special-bindings*
	public static final Symbol _DEFAULT_SPECIAL_BINDINGS_ = exportSpecial("*DEFAULT-SPECIAL-BINDINGS*", PACKAGE_THREADS, list(Symbol._RANDOM_STATE_, Symbol.CURRENT_READTABLE));
	 ...
	 * @return
	 */
	public static String stringValueOf(Object arg) {
		if (arg == null)
		...
		try {
			...
			return ((LispObject) arg).printObject();
		} finally {
	...
	static public String valueOfString(Object obj, boolean readably, int newInside, Object po) {
		LispObject lo = null;
		...
		if (obj instanceof LispObject) {
			...
			lo = ((LispObject) obj).toLispObject();
		} else {
			return String.valueOf(obj);
		}
		...
		boolean tooSoon0 = isTooSoon();
		int wasPRTraced = Symbol.PRINT_READABLY.isTraced;
		try {
	...
	public static String getDotName(Class class1) {
		...
		final String str = "javacyc_";
		final int contains = cname.lastIndexOf(str);
		if (contains > 0) {
			cname = cname.substring(contains + str.length(), 3);
			firstName = cname + firstName;
		}
	...
	public static LispObject makeStringOrJNull(String result) {
		if (result == null)
			return JavaObject.getInstance(null, String.class);
	...
	static public void flushOutputStreams() {
		...
		if (o instanceof Symbol)
			o = ((Symbol) o).getSymbolValue();
		if (o instanceof Stream)
			((Stream) o)._finishOutput();
					...
					if (sym.isConstantSymbol()) {
						if (sym.boundp()) {
							...
							if (name.startsWith("+") || name.startsWith("*")) {
								final SubLObject constValue = sym.getValue();
								if (!ObjectMerger.tooSimpleValue(constValue)) {
									if (name.length() > 4) {
										lst.add(sym);
									}
								}
