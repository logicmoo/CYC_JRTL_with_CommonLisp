...
public final class Interpreter implements Runnable
{
	...
	public static Interpreter createInstance()
	{
		try
		{
			synchronized (Interpreter.class)
			{
				if (globalInterpreter != null) return null;
	...
	public static synchronized Interpreter initInstance(Interpreter interp, String[] args, boolean jLisp)
	{
		...
		if (args != null) preprocessCommandLineArguments(args);
		if (!noinform)
		{
		...
		if (Main.subLisp != null) SubLPackage.setCurrentPackage("CL-USER");
	...
	public static void postProcessCommandLine(String[] passedArgs)
	{
		...
		synchronized (Interpreter.class)
		{
			if (Main.passedArgs == passedArgs) Main.passedArgs = null;
		}
		if (passedArgs != null)
		{
			if (postProcess)
	...
	public static Interpreter createJLispInstance(InputStream in, OutputStream out, String initialDirectory, String version)
	{
		try
		{
			synchronized (Interpreter.class)
			{
				if (Interpreter.globalInterpreter != null) { return null; }
	...
	private Interpreter(InputStream inputStream, OutputStream outputStream, String initialDirectory)
	{
		...
		if (!initialDirectory.endsWith(File.separator)) initialDirectory = initialDirectory.concat(File.separator);
			...
			try
			{
				...
				if (file.isFile())
				{
					...
					if (!noinform)
					{
						...
						getStandardOutput()._writeString("Loading " + file + " completed in " + loadtime + " seconds.\n");
					}
					return;
				}
			} catch (IOException e)
	...
	private static void reportError(ControlTransfer c, LispThread thread)
	{
		...
		if (thread != null) thread.printBacktrace();
	}
	...
	private static final Primitive _DEBUGGER_HOOK_FUNCTION = new Primitive("%debugger-hook-function", PACKAGE_SYS, false)
	{
		...
		public LispObject execute(LispObject first, LispObject second)
		{
			...
			if (condition.typep(Symbol.JAVA_EXCEPTION) != NIL) uc.initCause((Throwable) JavaException.JAVA_EXCEPTION_CAUSE.execute(condition).javaInstance());
			throw uc;
		}
	};
	...
	{
		if (!initialized) initializeJLisp();
		...
		try
		{ // catch possible errors from use of SHARPSIGN_DOT macros in --eval stanzas
			obj = stream.read(false, EOF, false, thread, Stream.currentReadtable);
		} finally
		{
			thread.resetSpecialBindings(mark0);
		}
		if (obj == EOF) return error(new EndOfFile(stream));
	...
	public void kill()
	{
		...
		if (true) Errors.warn("Auto-generated method stub:  Interpreter.kill");
