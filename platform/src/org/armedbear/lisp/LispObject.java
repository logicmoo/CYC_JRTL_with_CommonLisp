...
public class LispObject extends AbstractSubLObject
{
	...
	public LispObject typep(LispObject typeSpecifier)
	{
		...
		if (typeSpecifier == Symbol.ATOM) return atom() ? T : NIL;

		if (typeSpecifier == Symbol.NUMBER || typeSpecifier == BuiltInClass.NUMBER) return numberp() ? T : NIL;

		if (classOf() == typeSpecifier) return T;
		if (typeOf() == typeSpecifier) return T;
		return NIL;
	}
	...
	{
		if (cIn == null || cIn == Object.class) return javaInstance();
		final Class<? extends LispObject> thisClass = getClass();
		if (cIn.isAssignableFrom(thisClass)) { return this; }
		...
		String cn = c.getName();
		if (cn.equals("java.lang.Boolean")) { return !isNil(); }
	...
	public LispObject car()
	{
		...
		else if (this instanceof Nil) { return NIL; }
	...
	{
		if (n < 0) return type_error(Fixnum.getInstance(n), list(Symbol.INTEGER, Fixnum.ZERO));
		if (this instanceof Cons)
		{
	...
	public final LispObject push(LispObject obj)
	{
		...
		else if (this instanceof Nil) { return new Cons(obj); }
		return type_error(this, Symbol.LIST);
	}
	...
	public final boolean endp()
	{
		if (this instanceof Cons)
			return false;
		else if (this instanceof Nil) return true;
		type_error(this, Symbol.LIST);
		// Not reached.
		return false;
	}
...

	{
		...
		if (alist != null)
		{
			...
			if (entry instanceof Cons) return ((Cons) entry).cdr;
		}
		if (docType == Symbol.FUNCTION && this instanceof Symbol)
		{
			...
			if (fn instanceof Function)
			{
				...
				if (ds != null)
				{
					...
					if (arglist.length() != 0) ((Function) fn).setLambdaList(new SimpleString(arglist));
					...
					else if (fn.typep(Symbol.STANDARD_GENERIC_FUNCTION) != NIL) { return Symbol.SLOT_VALUE.execute(fn, Symbol._DOCUMENTATION); }
				}
			}
		}
		return NIL;
	...
	static class LinkedIdentityHashSet extends AbstractList
	{
		...
		static class IdentityWrapper
		{
			...
			{
				if (obj instanceof IdentityWrapper) { return ((IdentityWrapper) obj).obj == this.obj; }
	...
	* @throws IOException 
	...
	public LispObject MOD(LispObject divisor)
	{
		...
		if (!remainder.zerop())
		{
			if (divisor.minusp())
			{
				if (plusp()) return remainder.add(divisor);
			}
			else
			{
				if (minusp()) return remainder.add(divisor);
			}
		}
		return remainder;
	}
