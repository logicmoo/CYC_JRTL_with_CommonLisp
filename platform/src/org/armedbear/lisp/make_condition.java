...
Expected value: T
Actual value: #<SIMPLE-ERROR {75C7A1B8}>.
 ...
 */
...

    {
        ...
        else if (type instanceof Cons)
        {
        	...
        	if(type.car()==Symbol.AND)
        	{
        		type = type.cdr();
        		LispObject a1 = type.car();
        		LispObject a2 = type.cadr();
				if (a1 == Symbol.SIMPLE_ERROR && a2 == Symbol.TYPE_ERROR)
					return execute(Symbol.SIMPLE_TYPE_ERROR, initArgs);
				if (a2 == Symbol.SIMPLE_ERROR && a1 == Symbol.TYPE_ERROR)
					return execute(Symbol.SIMPLE_TYPE_ERROR, initArgs);
        	...
        	if(true)
        	{
        		//return Symbol.MAKE_INSTANCE.execute(type,initArgs);
        	}
        	// PROGRAM-ERROR + TYPE-ERROR = METHOD-CALL-TYPE-ERROR
        	return NIL;
        }
        else {
