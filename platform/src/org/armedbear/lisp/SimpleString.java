...
//	}
    ...
    public  SimpleString(char[] chars, int length) {
		capacity = length;
    ...
    public  SimpleString(char[] chars, int start, int end) {
    	int size = end - start;
	...
	public SimpleString(int length, char defaultChar) {
		capacity = length;
		if (length < 0)
			Errors.error("Attempt to create string with negative size: " + length);
		Arrays.fill(this.chars = new char[length], defaultChar);
	}
...
//	}
    ...
    public final AbstractString substring(int start)
    {
        return substring(start, capacity);
    }

    public final AbstractString substring(int start, int end)
