...
public class StructureObject extends SubLStructInterpreted implements SubLStruct {
	...
	static LispObject intoSlotName(LispObject[] effectiveSlotsArray, int i) {
		if (effectiveSlotsArray == null)
			return new SimpleString("SLOT_" + i);
		if (effectiveSlotsArray.length < i)
			return new SimpleString("SLOT_" + i + "_OF_" + effectiveSlotsArray.length);
