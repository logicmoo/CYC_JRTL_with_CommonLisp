...
public class DoubleFloat extends LispFloat {
	...
	public static DoubleFloat DOUBLE_FLOAT_POSITIVE_INFINITY = DoubleFloat.makeDouble(Double.POSITIVE_INFINITY);

	public static DoubleFloat DOUBLE_FLOAT_NEGATIVE_INFINITY = DoubleFloat.makeDouble(Double.NEGATIVE_INFINITY);

	static {
		LispSymbols.DOUBLE_FLOAT_POSITIVE_INFINITY.initializeConstant(DoubleFloat.DOUBLE_FLOAT_POSITIVE_INFINITY);
		LispSymbols.DOUBLE_FLOAT_NEGATIVE_INFINITY.initializeConstant(DoubleFloat.DOUBLE_FLOAT_NEGATIVE_INFINITY);
	}
	...
	public static DoubleFloat coerceToFloat(SubLObject obj) {
		...
		if (obj instanceof Fixnum)
			return DoubleFloat.makeDouble(((Fixnum) obj).value);
		if (obj instanceof Bignum)
			return DoubleFloat.makeDouble(((Bignum) obj).doubleValue());
		if (obj instanceof SingleFloat)
			return DoubleFloat.makeDouble(((SingleFloat) obj).value);
		if (obj instanceof Ratio)
			return DoubleFloat.makeDouble(((Ratio) obj).doubleValue());
	...
	public static DoubleFloat makeDouble(double d) {
		if (d == 0) {
			...
			if (bits < 0)
				return DoubleFloat.MINUS_ZERO;
			else
				return DoubleFloat.ZERO;
		} else if (d == 1)
			return DoubleFloat.ONE;
		else if (d == -1)
			return DoubleFloat.MINUS_ONE;
		else
	...
	public SubLObject ABS() {
		...
		if (this.value == 0) // 0.0 or -0.0
			return DoubleFloat.ZERO;
		return DoubleFloat.makeDouble(-this.value);
	}

	public SubLObject add(SubLObject obj) {
		if (obj instanceof Fixnum)
			return DoubleFloat.makeDouble(this.value + ((Fixnum) obj).value);
		if (obj instanceof SingleFloat)
			return DoubleFloat.makeDouble(this.value + ((SingleFloat) obj).value);
		if (obj instanceof DoubleFloat)
			return DoubleFloat.makeDouble(this.value + ((DoubleFloat) obj).value);
		if (obj instanceof Bignum)
			return DoubleFloat.makeDouble(this.value + ((Bignum) obj).doubleValue());
		if (obj instanceof Ratio)
			return DoubleFloat.makeDouble(this.value + ((Ratio) obj).doubleValue());
	...
	public SubLObject dec() {
		return DoubleFloat.makeDouble(this.value - 1);
	}
	...
	public SubLObject negate() {
		if (this.value == 0) {
			...
			return bits < 0 ? DoubleFloat.ZERO : DoubleFloat.MINUS_ZERO;
		}
		return DoubleFloat.makeDouble(-this.value);
	}
	...
	public SubLObject sub(SubLObject obj) {
		...
		if (obj instanceof Complex) {
			Complex c = (Complex) obj;
			return LispObjectFactory.makeComplex(this.sub(c.getRealPart()), DoubleFloat.ZERO.sub(c.getImaginaryPart()));
		}
		return Lisp.type_error(obj, LispSymbols.NUMBER);
	}
	...
	public SubLObject truncate(SubLObject obj) {
		...
		// 12.1.4.1
		if (obj instanceof Fixnum)
			return this.truncate(DoubleFloat.makeDouble(((Fixnum) obj).value));
		if (obj instanceof Bignum)
			return this.truncate(DoubleFloat.makeDouble(((Bignum) obj).doubleValue()));
		if (obj instanceof Ratio)
			return this.truncate(DoubleFloat.makeDouble(((Ratio) obj).doubleValue()));
		if (obj instanceof SingleFloat) {
			...
			if (quotient >= Integer.MIN_VALUE && quotient <= Integer.MAX_VALUE) {
				...
				return thread.setValues(LispObjectFactory.makeInteger(q),
						DoubleFloat.makeDouble(this.value - q * divisor));
			}
		...
		if (obj instanceof DoubleFloat) {
			...
			SubLObject remainder = DoubleFloat.coerceToFloat(thread._values[1]);
