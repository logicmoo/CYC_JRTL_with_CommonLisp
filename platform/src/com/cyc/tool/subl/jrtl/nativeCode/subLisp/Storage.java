...
public class Storage implements SubLFile {
	...
	public static SubLObject dump_heap(SubLObject fileName) {
		HeapDumper.dumpHeap(fileName.getString(), true);
		return CommonSymbols.NIL;
	}

	public static SubLObject gc(SubLObject level) {
		return Storage.gc_dynamic();
	}

	public static SubLObject gc_dynamic() {
		return Storage.gc_ephemeral();
	}
	...
	public static SubLObject room(SubLObject x) {
		...
		Storage.room_print_stats_helper("VM memory:", maxMemory, totalMemory, usedMemory, freeMemory);
		if (x == CommonSymbols.T) {
			List<MemoryPoolMXBean> pools = ManagementFactory.getMemoryPoolMXBeans();
			for (MemoryPoolMXBean pool : pools) {
				MemoryType curPoolType = pool.getType();
				MemoryUsage usage = pool.getUsage();
				Storage.room_print_stats_helper(" " + pool.getName() + " (" + pool.getType() + "):", usage.getMax(),
						usage.getCommitted(), usage.getUsed(), usage.getMax() - usage.getUsed());
			}
		}
		return CommonSymbols.NIL;
	}

	private static void room_print_stats_helper(String info, long maxMemory, long totalMemory, long usedMemory,
			long freeMemory) {
		...
		PrintLow.format(StreamsLow.$terminal_io$.getDynamicValue().getStream(true),
				SubLObjectFactory.makeString(buf.toString()));
	...
	public void declareFunctions() {
		...
		SubLFiles.declareFunction(Storage.me, "register_low_memory_callback", "REGISTER-LOW-MEMORY-CALLBACK", 1, 0,
				false);
		SubLFiles.declareFunction(Storage.me, "deregister_low_memory_callback", "DEREGISTER-LOW-MEMORY-CALLBACK", 1, 0,
				false);
	...
	public void initializeVariables() {
		Storage.$current_area$ = SubLFiles.defvar(Storage.me, "*CURRENT-AREA*", CommonSymbols.NIL);
	}
