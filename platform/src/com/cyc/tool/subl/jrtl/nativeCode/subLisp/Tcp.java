...
public class Tcp extends SubLTrampolineFile {
				...
				while (true) {
					...
					try {
						...
						if (acceptAll || isLocalHost)
							SubLObjectFactory.makeProcess(SubLObjectFactory.makeString("Socket Connection Handler: " + socket), new ServerSocketHandlerProcess(funcSpec.getFunc(), socket));
						else {
							socket.close();
							socket = null;
						}
	...
	public static SubLObject start_tcp_server(SubLObject port, SubLObject handler) {
		...
		SubLProcess result = SubLObjectFactory.makeProcess(SubLObjectFactory.makeString("Socket Server (port: " + portTyped + " handler: " + handler + ")"), new ServerSocketProcess(portTyped, handler));
