...
public class JavaLink extends SubLSystemTrampolineFile {
	...
	private static Class getJavaClass(Object subLObject) {
		if (!(subLObject instanceof SubLObject)) {
			if (subLObject instanceof Class) {
				return (Class) subLObject;
			}
	...
	private static Object getJavaObject(Class targetClass, Object subLObject, boolean throwRuntimeException) {
		...
		try {
			if (targetClass.isPrimitive()) {
				Class wideTargetClass = Java.maybeBoxClass(targetClass);
				if (wideTargetClass != null && wideTargetClass != targetClass) {
					Object o2 = getJavaObject(wideTargetClass, subLObject, false);
					if (wideTargetClass.isInstance(o2))
						return o2;
				}
			}
	...
	private static Object coerceToClass(Class intoClass, Object coercible, boolean throwRuntimeException) {

		final Class<? extends Object> iclass = coercible.getClass();
		if (JavaLink.DEBUG)
			System.out.println("Coercing (" + iclass + ") " + coercible + " into class " + intoClass);
		try {
			if (intoClass.isAssignableFrom(iclass))
				return coercible;
			if (coercible instanceof SubLObject) {
				final Class classForNameZ = classForName("Z");
				if (intoClass.equals(classForNameZ))
					return !((SubLObject) coercible).isNil();
				if (intoClass.equals(classForName("C")))
					return ((SubLObject) coercible).charValue();
				if (intoClass.equals(classForName("D")))
					return ((SubLObject) coercible).doubleValue();
				if (intoClass.equals(classForName("I")))
					return ((SubLObject) coercible).intValue();
				if (intoClass.equals(classForName("J")))
					return ((SubLObject) coercible).longValue();
				/// if (coercible instanceof SubLSymbol) return subLObjectToString(coercible);
			}
			if (isArrayClass(intoClass)) {
				if (coercible instanceof SubLList)
					return getJavaObject(intoClass, ((SubLList) coercible).toSubLObjectArray(), true);
				if (coercible instanceof SubLVector)
					return getJavaObject(intoClass, ((SubLVector) coercible).toArray(new Array[((SubLVector) coercible).size()]), true);
				if (iclass.isArray()) {
					Class elemClass = classForName(intoClass.getName().substring(1));
					return eachArrayElement(coercible, elemClass);
				}
			}
			if (!throwRuntimeException)
				return null;
			throw new RuntimeException("JavaLink.coerceToClass doen't know how to coerce (" + iclass + ") " + coercible + " to " + intoClass);
		} catch (ClassNotFoundException e) {
			throw new RuntimeException(e.getCause() != null ? e.getCause() : e);
		}
	}
	 ...
	 * @return
	 */
	public static boolean isArrayClass(Class classClass) {

		final String name = classClass.getName();
		if (name.endsWith("[]"))
			return true;
	...
	private static Object getJavaObject0(Class targetClass, Object subLObject, boolean throwRuntimeException) {
		if (subLObject instanceof SubLT && targetClass.isAssignableFrom(Boolean.TYPE))
			return ((SubLT) subLObject).toBoolean();
		if (subLObject instanceof SubLNil && canAssign(Boolean.TYPE, targetClass))
			return ((SubLNil) subLObject).toBoolean();

		if (canAssign(String.class, targetClass))
			return subLObjectToString(subLObject);
		if (subLObject instanceof SubLString && canAssign(String.class, targetClass))
			return ((SubLString) subLObject).getStringValue();

		if (subLObject instanceof SubLCharacter && canAssign(Character.TYPE, targetClass)) {
			return ((SubLCharacter) subLObject).charValue();
		}

		if (subLObject instanceof AbstractSubLFloat && canAssign(Double.TYPE, targetClass))
			return ((AbstractSubLFloat) subLObject).doubleValue();
		if (subLObject instanceof AbstractSubLIntegerBignum && canAssign(Integer.TYPE, targetClass))
			return ((AbstractSubLIntegerBignum) subLObject).intValue();
		if (subLObject instanceof SubLLongBignum && canAssign(Long.TYPE, targetClass))
			return ((SubLLongBignum) subLObject).longValue();
		if (subLObject instanceof SubLBigIntBignum && canAssign(BigInteger.class, targetClass))
			return ((SubLBigIntBignum) subLObject).bigIntegerValue();
		...
		if (subLObject instanceof SubLNil && canAssign(List.class, targetClass)) {
		...
		if (subLObject instanceof SubLT)
			...
			if (!(subLObject instanceof SubLHashtable)) {
				...
				if ((subLObject instanceof LispObject)) {
					...
					if (result != null && result != subLObject) {
						return result;
					}
				}
	...
	private static String subLObjectToString(Object object) {
		if (object instanceof SubLSymbol)
			return ((SubLSymbol) object).getName();
	...
	public static Object[] tryRecast(final Class<?>[] parameterTypes, Object[] argObjects) {
		...
		if (argObjects != null) {
			...
			for (int j = 0; j < argsCount; ++j) {
				final Class<?> targetClass = parameterTypes[j];
				final Object subLObject = argObjects[j];
				Object nonNull = getJavaObject(targetClass, subLObject, false);
				if (nonNull != null) {
					args[j] = nonNull;
				} else {
					Startup.bug();
