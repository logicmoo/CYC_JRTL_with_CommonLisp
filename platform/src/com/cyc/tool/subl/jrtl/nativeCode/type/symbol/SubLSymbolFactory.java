...
public class SubLSymbolFactory {
	...
	SubLSystem system = SubLSystem.getSystem(thePackage);
	Symbol findAccessibleSymbol = system.findSymbol(symbolName);
	if (findAccessibleSymbol != null) {
	    if (thePackage != findAccessibleSymbol.getPackage()) {
    ...
    public static SubLSymbol makeSymbolFromSystem(SubLString symbolName, SubLPackage thePackage) {
	if (thePackage == null)
	    return new Symbol(symbolName);
	SubLSystem system = SubLSystem.getSystem(thePackage);
	final String stringValue = symbolName.getStringValue();
	Symbol findAccessibleSymbol = system.findSymbol(stringValue);
	if (findAccessibleSymbol != null) {
	    if (thePackage != findAccessibleSymbol.getPackage()) {
		thePackage.importSymbol(findAccessibleSymbol);
		thePackage.export(findAccessibleSymbol);
	...
	return thePackage.internAndExport(stringValue);
    }
