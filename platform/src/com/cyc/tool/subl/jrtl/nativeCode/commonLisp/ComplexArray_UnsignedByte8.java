...
public class ComplexArray_UnsignedByte8 extends AbstractComplexArray {
	private int[] dimv;
	...
	public ComplexArray_UnsignedByte8(int[] dimv, SubLObject initialContents)
	...
	public SubLObject AREF(int index) {
		if (this.data != null)
			try {
				return Lisp.coerceJavaByteToLispObject(this.data[index]);
	...
	public void aset(int index, SubLObject newValue) {
		if (this.data != null)
			try {
				this.data[index] = Lisp.coerceLispObjectToJavaByte(newValue);
			} catch (ArrayIndexOutOfBoundsException e) {
	...
	public SubLObject typeOf() {
		return Lisp.list(LispSymbols.ARRAY, Lisp.UNSIGNED_BYTE_8, this.getDimensions());
	}
