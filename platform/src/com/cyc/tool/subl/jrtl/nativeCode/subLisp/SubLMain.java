...
public class SubLMain
{
	...
	public static final class InitialEmbeddedMain extends SubLProcess
	{
		...
		public void safeRun()
		{
			try
			{
				...
				if (!shouldRunInBackground()) SystemCurrent.out.println("Starting Cyc.");
				...
				if (!noInitCyc) initializeTranslatedSystems();

				long endTime = System.currentTimeMillis();
				double theTime = (endTime - startTime) / 1000.0;
				if (!shouldRunInBackground()) SystemCurrent.out.println("Internal initialization time = " + theTime + " secs.");

				startTime = System.currentTimeMillis();

				if (!delayEvalParams) handleInits();
	...
	private static class MemoryListener implements NotificationListener
	{
		...
		public void handleNotification(Notification notification, Object handback)
		{
			...
			if (notifType.equals("java.management.memory.threshold.exceeded") || notifType.equals("java.management.memory.collection.threshold.exceeded")) lowMemorySemaphore.release();
		}
	}
	...
	{
		if (func == null) Errors.error("Unable to deregister low memory callback for null function.");
	...
	{
		if (isInAllegroCompatibilityMode()) SubLProcess.setAllegoSingleThreadedThread(SubLProcess.currentSubLThread());
	}
	...
	public static synchronized void handleInits()
	{
		...
		try
		{
			...
			if (shouldQuitAfterExecutingInitializationForm()) me.doSystemCleanupAndExit(0);
		} finally
		{
	...
	private static void initializeLowMemoryDetection()
	{
		...
		for (MemoryPoolMXBean pool : pools)
		{
			...
			if (pool.getName().contains("Tenured") || pool.getName().contains("Old"))
			{
				...
				if (threshold < percent90) threshold = percent90;
		...
		SubLObjectFactory.makeProcess(SubLObjectFactory.makeString("Low Memory Scavenger"), new Runnable()
		{
			...
			public void run()
			{
				while (true)
				{
					...
					if (usedMemory >= eightyPercentMemory) lowMemorySituation();
	...
	public static void main(String[] args)
	{
		...
		else if (args[0].equalsIgnoreCase("--nosubl"))
		{
			JPL.init();
	...
	{
		if (func == null) Errors.error("Unable to register low memory callback for null function.");
	...
	private static void writeSystemInfo()
	{
		...
		{
			// use reader to avoid direct code dependency on cycl
			SystemCurrent.out.println("Patch Level: " + Eval.eval(reader.read_from_string(SubLObjectFactory.makeString("(clet ((result \"<unknown>\")) (ignore-errors (pwhen (fboundp 'CYC-REVISION-STRING) (csetq result (format nil \"~A\" (cyc-revision-string))))) result)"), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED)).getStringValue());
		} catch (Exception e)
		{
		...
		try
		{
			SystemCurrent.out.println("Working Directory: " + Eval.eval(reader.read_from_string(SubLObjectFactory.makeString("(clet ((result \"" + System.getProperty("user.dir") + "\")) " + "(ignore-errors (pwhen (fboundp 'CANONICAL-CYC-WORKING-DIRECTORY) (csetq result (canonical-cyc-working-directory))) result))"), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED)).getStringValue());
		} catch (Exception e)
		{
	...
	public void processCommandLineArgs(String[] argsIn)
	{
		...
		for (int i = 0, size = args.length; i < size; ++i)
		{
			...
			{
				if (i == size) Errors.error("Not enough command line arguments given for: " + arg);
