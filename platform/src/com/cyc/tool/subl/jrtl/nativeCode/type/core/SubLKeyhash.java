...
public class SubLKeyhash extends FromSubLisp implements SubLObject {
  ...
  SubLKeyhash(int size, BinaryFunction test) {
    hash = new HashSet<SubLHashtableKeyEntry>((int) (size * 1.5) + 1);
    this.test = test;
  ...
  // the entrySet basically
  private HashSet<SubLHashtableKeyEntry>  hash;
  ...
  public void addKey(SubLObject key) {
    hash.add(SubLHashtable.makeKeyEntry(keyFactory, test, key));
  }
  ...
  public Iterator<SubLHashtableKeyEntry> getEntrySetIterator() {
  ...
  public boolean remKey(SubLObject obj) {
    ...
    SubLHashtable.SubLHashtableKeyEntry key2 = null;
    Resourcer resourcer = Resourcer.getInstance();
    try {
      key2 = resourcer.acquireHashtableKeyEntry();
      key2.init(obj, test);
  ...
  public String printObjectImpl() {
    checkUnreadableOk();
    return "#<" + test.getFunction().getFunctionSymbol() + " " + toTypeName() + " with " + this.size() + " entries @ " + super.easyToString() + ">";
  }
  ...
  public SubLList toSubLList() {
    ...
    final SubLObject[] a = new SubLObject[hash.size()];
    SubLObject[] values = hash.toArray(a);
