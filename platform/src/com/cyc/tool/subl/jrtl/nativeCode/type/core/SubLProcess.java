...
public abstract class SubLProcess extends SafeRunnable implements Runnable, SubLObject {
	...
	public static class InterruptReadLoopThunk implements Thunk {
		@Override
		public SubLObject invoke() {
			throw new ResumeException("InterruptReadLoopThunk");
		}
	...
	public static void currentThrowStackPop(SubLObject object) {
		...
		if (object != null && object != top)
			Errors.error("Throw-Stack for " + currentSubLThread() + " out of sync; expected " + object + ", but got " + top);
	}
	...
	final private Throwable whyCreated = new Throwable("Created").fillInStackTrace();
	private Throwable whyDone;
	...
	public void processInterrupts() {
		...
		boolean ctWasInterrupted = Thread.currentThread().isInterrupted();
		if (runThread != null) {
			Thread jtread = runThread.asJavaTread();
			if (jtread.isInterrupted()) {
				wasInterrupted = true;
			}
		}
		if (ctWasInterrupted || wasInterrupted) {
			Thread.interrupted();
		}
	...
	@Override
	public abstract void safeRun() throws TerminationRequest;
