...
public abstract class SubLProcess extends SafeRunnable implements Runnable, SubLObject
{
	...
	public static class SubLProcessState
	{
		...
		{
			if (stateSymbol == null) stateSymbol = SubLObjectFactory.makeSublispSymbol(toString());
			return stateSymbol;
		}
	}
			...
			if (thread == null)
			{
				thread = new SubLThread(new Runnable()
				{
					...
					public void run()
					{
						...
						if (true) Errors.unimplementedMethod("Auto-generated method stub:  Type1477805356591.run");
	...
	public static SubLThread currentSubLThread()
	{
		...
		if (t instanceof SubLThread) return (SubLThread) t;
		if (SubLProcess.ALLEGRO_SINGLE_THREADED_THREAD != null) return SubLProcess.ALLEGRO_SINGLE_THREADED_THREAD;
		synchronized (map)
		{
			if (t == oneOnlyJT) return oneOnlyLT;
		}
		return threads.get();
	...
	public static void currentThrowStackPop(SubLObject object)
	{
		...
		if (object != null && object != top) Errors.error("Throw-Stack for " + currentSubLThread() + " out of sync; expected " + object + ", but got " + top);
	}
	...
	public static SubLObject nthMultipleValue(SubLObject nth)
	{
		...
		switch (n)
		{
			...
			default:
				if (n >= thread.valuesCount) return SubLNil.NIL;
	...
	public void enforceType(SubLSymbol predicate) throws SubLException
	{
		...
		if (SubLNil.NIL == Function.processItem(this)) Errors.error("Got invalid type for object: " + this + "." + "\nWanted: " + predicate + "\nGot: " + toTypeName());
	}
	...
	{
		return Threads.$is_thread_performing_cleanupP$ == null || Threads.$is_thread_performing_cleanupP$.getDynamicValue() != SubLNil.NIL;
	}
	...
	public Thunk nextInterrupt()
	{
		...
		if (!interrupts.isEmpty()) interruptRequest = interrupts.remove(0);
	...
	{
		if (runThread != null) Thread.interrupted();
		...
		while (hasWatingInterrupts())
		{
			...
			if (current != null)
			{
				if (current.isTerminationRequest()) throw new TerminationRequest();
				current.invoke();
			}
		}
	}
	...
	{
		if (state == null) Errors.error("Invalid null state attempted to be assigned to process: " + this);
	...
	{
		if (SubLThread.currentThread() == runThread) return;
