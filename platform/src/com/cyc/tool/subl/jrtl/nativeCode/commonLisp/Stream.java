...
public class Stream extends ABCLStructureObject implements LispStream {
    ...
    static final protected SubLSymbol keywordDefault = internKeyword("DEFAULT");

    static final private SubLSymbol keywordCodePage = internKeyword("CODE-PAGE");
    static final private SubLSymbol keywordID = internKeyword("ID");

    static final private SubLSymbol keywordEolStyle = internKeyword("EOL-STYLE");
    static final private SubLSymbol keywordCR = internKeyword("CR");
    static final private SubLSymbol keywordLF = internKeyword("LF");
    static final private SubLSymbol keywordCRLF = internKeyword("CRLF");
    static final private SubLSymbol keywordRAW = internKeyword("RAW");
    ...
    public Stream(SubLSymbol structureClass, InputStream inputStream,
    ...
    public Stream(SubLSymbol structureClass, OutputStream outputStream,
    ...
    public void setExternalFormat(SubLObject format) {
        ...
        if (format instanceof SubLCons) {
            ...
            if (enc == keywordCodePage) {
                ...
                enc = Lisp.getf(format.rest(), keywordID, null);
            }

            SubLObject eol = Lisp.getf(format.rest(), keywordEolStyle, keywordRAW);
            if (eol == keywordCR)
    ...
    // ### *sharp-equal-alist*
    // internal symbol
    private static final SubLSymbol _SHARP_EQUAL_ALIST_ =
    ...
    private final SubLObject faslReadPreservingWhitespace(boolean eofError,
            SubLObject eofValue,
            boolean recursive,
            LispThread thread)
    throws IOException {
    ...
    private final SubLObject processChar(char c, Readtable rt)

    {
        final SubLObject handler = rt.getReaderMacroFunction(c);
    ...
    public SubLObject readSymbol() {
        final Readtable rt =
            (Readtable) LispSymbols.CURRENT_READTABLE.symbolValue(LispThread.currentThread());
    ...
    private final SubLObject readToken(char c, Readtable rt)

    {
        ...
        if (LispSymbols.READ_SUPPRESS.symbolValue(thread) != NIL)
            return NIL;
        final SubLObject readtableCase = rt.getReadtableCase();
        final String token;
        ...
        return ((SubLPackage)LispSymbols._PACKAGE_.symbolValue(thread)).intern(makeString(token));
    }
    ...
    {
        final int readBase;
        final SubLObject readBaseObject = LispSymbols.READ_BASE.symbolValue(thread);
    ...
    private final SubLObject makeNumber(String token, int length, int radix)
    ...
    private final SubLObject makeRatio(String token, int radix)
    ...
    private static final SubLObject makeFloat(final String token,
            final int length)
