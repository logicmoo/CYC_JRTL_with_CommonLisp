...
public class foreign extends SubLTranslatedFile {
	...
	public static SubLObject analyze_define_foreign_function_arguments(SubLObject symbol, SubLObject arglist,
		...
		while (SubLNil.NIL != rest) {
			...
			rest = rest.rest();
			cdestructuring_bind.destructuring_bind_must_consp(rest, rules, foreign.$list17);
			if (SubLNil.NIL == conses_high.member(current_$1, foreign.$list18, UNPROVIDED,
					UNPROVIDED))
				bad = T;
		...
		SubLObject calling_convention_tail = cdestructuring_bind.property_list_member(foreign.$kw20$CALLING_CONVENTION,
				rules);
		SubLObject calling_convention = SubLNil.NIL != calling_convention_tail
		...
		SubLObject foreign_shared_library = SubLNil.NIL != foreign_shared_library_tail
				? conses_high.cadr(foreign_shared_library_tail) : SubLNil.NIL;
		SubLObject foreign_name_tail = cdestructuring_bind.property_list_member(foreign.$kw23$FOREIGN_NAME, rules);
		SubLObject foreign_name = SubLNil.NIL != foreign_name_tail ? conses_high.cadr(foreign_name_tail)
				: SubLNil.NIL;
		SubLObject arg_conversion_tail = cdestructuring_bind.property_list_member(foreign.$kw24$ARG_CONVERSION, rules);
		SubLObject arg_conversion = SubLNil.NIL != arg_conversion_tail ? conses_high.cadr(arg_conversion_tail)
				: SubLNil.NIL;
		SubLObject return_type_tail = cdestructuring_bind.property_list_member(foreign.$kw25$RETURN_TYPE, rules);
		SubLObject return_type = SubLNil.NIL != return_type_tail ? conses_high.cadr(return_type_tail)
				: foreign.$kw26$VOID;
		...
		if (SubLNil.NIL == Errors.$ignore_mustsP$.getDynamicValue(thread)
				&& SubLNil.NIL == valid_arg_conversion_pattern(arg_conversion))
			Errors.error(foreign.$str30$_S_is_not_a_valid__ARG_CONVERSION, arg_conversion);
		if (SubLNil.NIL == Errors.$ignore_mustsP$.getDynamicValue(thread) && !return_type.isKeyword()
				&& !return_type.isList())
			...
			SubLObject retit = ConsesLow.list(foreign.$sym37$PROGN, main_function, ConsesLow.list(foreign.$sym38$DEFINE,
					...
					ConsesLow.list(foreign.$sym39$CLET, ConsesLow.list(ConsesLow.list(foreign.$sym40$_VAL,
							ConsesLow.listS(foreign.$sym41$FFUNCALL, ConsesLow.list(foreign.$sym36$QUOTE, symbol_ff),
									ConsesLow.append(dummy_arglist, SubLNil.NIL)))),
	...
	public static SubLObject find_foreign_shared_library_by_property(SubLObject property, SubLObject value,
		...
		if (SubLNil.NIL == found) {
			...
			while (SubLNil.NIL == found && SubLNil.NIL != csome_list_var) {
				...
				if (SubLNil.NIL != Functions.funcall(test, value,
						conses_high.getf(properties, property, UNPROVIDED)))
					found = moniker;
	...
	public static SubLObject foreign_shared_library_installedP(SubLObject moniker) {
		return SubLObjectFactory.makeBoolean(SubLNil.NIL != foreign_shared_library_loadedP(moniker)
				|| SubLNil.NIL != foreign_shared_library_loadableP(moniker));
	}
	...
	public static SubLObject foreign_shared_library_loaded_intP(SubLObject moniker) {
		SubLThread thread = SubLProcess.currentSubLThread();
		return SubLObjectFactory.makeBoolean(SubLNil.NIL != conses_high.member(moniker,
				foreign.$active_foreign_shared_libraries$.getDynamicValue(thread), UNPROVIDED,
				UNPROVIDED));
	}
	...
	public static SubLObject generate_foreign_name_from_sublisp_symbol(SubLObject calling_convention,
		...
		if (calling_convention.eql(foreign.$kw21$C))
			return Strings.string_downcase(Symbols.symbol_name(symbol), UNPROVIDED,
					UNPROVIDED);
