...
public class print_high extends SubLTranslatedFile {
	...
	public static SubLObject circular_reference_p(SubLObject object, SubLObject stream) {
		...
		if (SubLNil.NIL != print_high.$print_circle$.getDynamicValue(thread)
				&& SubLNil.NIL == read_print_eq(object)) {
		...
		try {
			...
			if(object instanceof LispObject)
			{
				try {
					...
					streams_high.write_string(SubLObjectFactory.makeString(string), stream, CommonSymbols.UNPROVIDED,
	...
	public static SubLObject really_write(SubLObject object, SubLObject keys) {
		...
		while (SubLNil.NIL != rest) {
			...
			rest = rest.rest();
			cdestructuring_bind.destructuring_bind_must_consp(rest, keys, print_high.$list36);
			if (SubLNil.NIL == conses_high.member(current_$2, print_high.$list37, CommonSymbols.UNPROVIDED,
					CommonSymbols.UNPROVIDED))
				bad = CommonSymbols.T;
		...
		SubLObject stream = SubLNil.NIL != stream_tail ? conses_high.cadr(stream_tail)
		...
		try {
			print_high.$print_array$.bind(SubLNil.NIL != print_arrayX_tail ? conses_high.cadr(print_arrayX_tail)
			...
			try {
				print_high.$print_base$.bind(SubLNil.NIL != print_baseX_tail ? conses_high.cadr(print_baseX_tail)
				...
				try {
					print_high.$print_case$.bind(SubLNil.NIL != print_caseX_tail
					...
					try {
						print_high.$print_circle$
								.bind(SubLNil.NIL != print_circleX_tail ? conses_high.cadr(print_circleX_tail)
						...
						try {
							print_high.$print_escape$
									.bind(SubLNil.NIL != print_escapeX_tail ? conses_high.cadr(print_escapeX_tail)
							...
							try {
								print_high.$print_gensym$.bind(
										SubLNil.NIL != print_gensymX_tail ? conses_high.cadr(print_gensymX_tail)
								...
								try {
									print_high.$print_length$.bind(SubLNil.NIL != print_lengthX_tail
									...
									try {
										print_high.$print_level$.bind(SubLNil.NIL != print_levelX_tail
										...
										try {
											print_high.$print_lines$.bind(SubLNil.NIL != print_linesX_tail
											...
											try {
												print_high.$print_miser_width$
														.bind(SubLNil.NIL != print_miser_widthX_tail
												...
												try {
													print_high.$print_pprint_dispatch$
															.bind(SubLNil.NIL != print_pprint_dispatchX_tail
													...
													try {
														print_high.$print_pretty$
																.bind(SubLNil.NIL != print_prettyX_tail
														...
														try {
															print_high.$print_radix$
																	.bind(SubLNil.NIL != print_radixX_tail
															...
															try {
																print_high.$print_readably$.bind(
																		SubLNil.NIL != print_readablyX_tail
																...
																try {
																	print_high.$print_right_margin$.bind(
																			SubLNil.NIL != print_right_marginX_tail
																					? conses_high
	...
	public static SubLObject with_printer_state(SubLObject macroform, SubLObject environment) {
		...
		while (SubLNil.NIL != rest) {
			...
			rest = rest.rest();
			cdestructuring_bind.destructuring_bind_must_consp(rest, datum, print_high.$list3);
			if (SubLNil.NIL == conses_high.member(current_$1, print_high.$list4, CommonSymbols.UNPROVIDED,
					CommonSymbols.UNPROVIDED))
				bad = CommonSymbols.T;
	...
	static {
		...
		$list22 = ConsesLow.list(ConsesLow.list(SubLObjectFactory.makeSymbol("MAKE-HASH-TABLE"),
		...
		$list23 = ConsesLow
				.list(ConsesLow.list(SubLObjectFactory.makeSymbol("MAKE-BROADCAST-STREAM"), SubLNil.NIL));
		...
		$list36 = ConsesLow.list(new SubLObject[] { SubLObjectFactory.makeSymbol("&KEY"),
