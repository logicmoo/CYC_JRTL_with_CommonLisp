...
public class reader extends SubLTranslatedFile {
	...
	static {
	    structDecl = Structures.makeStructDeclNative($read_token_native.class, reader.$sym119$READ_TOKEN, reader.$sym120$READ_TOKEN_P, reader.$list121, reader.$list122, new String[] { "$pointer", "$buffer", "$escapes" }, reader.$list123, reader.$list124,
	...
	private static SubLStructDeclNative structDecl;
	static {
	    structDecl = Structures.makeStructDeclNative($readtable_native.class, reader.$sym6$READTABLE, reader.$sym7$READTABLE_P, reader.$list8, reader.$list9,
		    new String[] { "$case", "$character_syntax", "$single_byte_char_syntax", "$macro_functions", "$single_byte_macro_functions", "$constituent_traits", "$single_byte_constit_traits", "$dispatch_tables" }, reader.$list10, reader.$list11, reader.$sym12$DEFAULT_STRUCT_PRINT_FUNCTION);
	}
	...
	if (read_token_pointer(token).numGE(length)) {
	    SubLObject new_buffer = Strings.make_string(Numbers.multiply(length, CommonSymbols.TWO_INTEGER), CommonSymbols.UNPROVIDED);
	    SubLObject i;
	    for (i = SubLNil.NIL, i = CommonSymbols.ZERO_INTEGER; i.numL(length); i = Numbers.add(i, CommonSymbols.ONE_INTEGER))
	...
	return SubLObjectFactory.makeBoolean(Numbers.logand(reader.$alphabetic$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))).isPositive()
		&& (!Numbers.logand(reader.$digit$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))).isPositive() || SubLNil.NIL == Characters.digit_char_p(ch, reader.$read_base$.getDynamicValue(thread))));
    }
	...
	return SubLObjectFactory.makeBoolean(
		Numbers.logand(reader.$alphabetic$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))).isPositive() && Numbers.logand(reader.$digit$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))).isPositive());
    }
	...
	try {
	    reader.$backquote_depth$.bind(Numbers.add(reader.$backquote_depth$.getDynamicValue(thread), CommonSymbols.ONE_INTEGER), thread);
	    ...
	    thread.resetMultipleValues();
	    if (flag.eql(reader.$bq_atsign$.getDynamicValue(thread)) || flag.eql(reader.$bq_dot$.getDynamicValue(thread)))
	...
	if (code.isAtom()) {
	    ...
	    if (code.first().eql(reader.$bq_atsign$.getDynamicValue(thread)) || code.first().eql(reader.$bq_dot$.getDynamicValue(thread)))
	    ...
	    thread.resetMultipleValues();
	    if (dflag2.eql(reader.$bq_atsign$.getDynamicValue(thread)) || dflag2.eql(reader.$bq_dot$.getDynamicValue(thread)))
		...
		return Values.values(reader.$sym189$APPEND, dflag2 == reader.$sym189$APPEND ? ConsesLow.cons(a, d2) : ConsesLow.list(a, backquotify_1(dflag2, d2)));
	    if (aflag.eql(reader.$bq_dot$.getDynamicValue(thread)))
		return Values.values(reader.$sym190$NCONC, dflag2 == reader.$sym190$NCONC ? ConsesLow.cons(a, d2) : ConsesLow.list(a, backquotify_1(dflag2, d2)));
	    if (SubLNil.NIL == dflag2) {
		if (SubLNil.NIL != conses_high.member(aflag, reader.$list199, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED))
		...
		return Values.values(reader.$sym188$LIST_, ConsesLow.list(backquotify_1(aflag, a), backquotify_1(dflag2, d2)));
	    } else {
	...
	if (flag == reader.$sym188$LIST_) {
	    ...
	    return ConsesLow.cons(conses_high.assoc(flag, reader.$list204, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED).rest(), thing);
	}
    }
	...
	SubLObject lists;
	SubLObject current = lists = datum;
	return ConsesLow.list(reader.$sym192$BQ_VECTOR, bq_cons(reader.$sym193$BQ_APPEND, ConsesLow.append(lists, SubLNil.NIL)));
    }
	...
	if (SubLNil.NIL == current)
	    return ConsesLow.list(reader.$sym53$FIF, ConsesLow.list(reader.$sym54$SINGLE_BYTE_P, ch), ConsesLow.list(reader.$sym55$AREF, ConsesLow.list(reader.$sym19$READTABLE_SINGLE_BYTE_CHAR_SYNTAX, readtable), ConsesLow.list(reader.$sym56$CHAR_CODE, ch)),
		    ConsesLow.listS(reader.$sym57$GETHASH, ch, ConsesLow.list(reader.$sym17$READTABLE_CHARACTER_SYNTAX, readtable), reader.$list58));
	...
	if (readtable == CommonSymbols.UNPROVIDED)
	    readtable = reader.$readtable$.getDynamicValue();
	return SubLNil.NIL != single_byte_p(ch) ? Vectors.aref(readtable_single_byte_constit_traits(readtable), Characters.char_code(ch)) : Hashtables.gethash(ch, readtable_constituent_traits(readtable), reader.$alphabetic$.getGlobalValue());
    }
	...
	if (code.isAtom()) {
	    ...
	    if (SubLNil.NIL != conses_high.member(code.first(), reader.$list200, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED))
		return Values.values(code.first(), code.rest());
	...
	if (!reader.$backquote_depth$.getDynamicValue(thread).isZero()) {
	    SubLObject peek_char = streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	    SubLObject result = SubLNil.NIL;
	    SubLObject _prev_bind_0 = reader.$backquote_depth$.currentBinding(thread);
	    try {
		reader.$backquote_depth$.bind(Numbers.subtract(reader.$backquote_depth$.getDynamicValue(thread), CommonSymbols.ONE_INTEGER), thread);
		SubLObject pcase_var = peek_char;
		if (pcase_var.eql(Characters.CHAR_at))
		    result = ConsesLow.cons(reader.$bq_atsign$.getDynamicValue(thread), read(stream, CommonSymbols.T, SubLNil.NIL, CommonSymbols.T));
		else if (pcase_var.eql(Characters.CHAR_period))
		    result = ConsesLow.cons(reader.$bq_dot$.getDynamicValue(thread), read(stream, CommonSymbols.T, SubLNil.NIL, CommonSymbols.T));
		else {
		    streams_high.unread_char(peek_char, stream);
		    result = ConsesLow.cons(reader.$bq_comma$.getDynamicValue(thread), read(stream, CommonSymbols.T, SubLNil.NIL, CommonSymbols.T));
		}
	    } finally {
		reader.$backquote_depth$.rebind(_prev_bind_0, thread);
	    }
	...
	_csetf_read_token_buffer(token, Strings.make_string(reader.$initial_token_size$.getGlobalValue(), CommonSymbols.UNPROVIDED));
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Equality.eq(
		SubLNil.NIL != single_byte_p(ch) ? Vectors.aref(readtable_single_byte_char_syntax(reader.$readtable$.getDynamicValue(thread)), Characters.char_code(ch)) : Hashtables.gethash(ch, readtable_character_syntax(reader.$readtable$.getDynamicValue(thread)), reader.$kw49$CONSTITUENT),
	...
	SubLObject i;
	for (i = SubLNil.NIL, i = CommonSymbols.ZERO_INTEGER; i.numL(reader.$single_byte_max_code$.getGlobalValue()); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
	...
	while (SubLNil.NIL != cdolist_list_var) {
	    ...
	    dispatch_table = ConsesLow.cons(ConsesLow.cons(ch, Xcopy_hash_table(table, Hashtables.make_hash_table(Hashtables.hash_table_count(table), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED))), dispatch_table);
	...
	_csetf_readtable_character_syntax(readtable, Hashtables.make_hash_table(reader.$initial_table_size$.getDynamicValue(thread), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED));
	_csetf_readtable_single_byte_char_syntax(readtable, Vectors.make_vector(reader.$single_byte_max_code$.getGlobalValue(), reader.$kw49$CONSTITUENT));
	_csetf_readtable_macro_functions(readtable, Hashtables.make_hash_table(reader.$initial_table_size$.getDynamicValue(thread), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED));
	_csetf_readtable_single_byte_macro_functions(readtable, Vectors.make_vector(reader.$single_byte_max_code$.getGlobalValue(), reader.$sym50$CONSTITUENT_RMF));
	_csetf_readtable_constituent_traits(readtable, Hashtables.make_hash_table(reader.$initial_table_size$.getDynamicValue(thread), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED));
	_csetf_readtable_single_byte_constit_traits(readtable, Vectors.make_vector(reader.$single_byte_max_code$.getGlobalValue(), reader.$alphabetic$.getGlobalValue()));
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$decimal_point$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }

    public static SubLObject declare_reader_file() {
	SubLFiles.declareMacro(myName, "loop", "LOOP");
	SubLFiles.declareFunction(myName, "whitespace_1_char_p", "WHITESPACE-1-CHAR-P", 1, 0, false);
	SubLFiles.declareFunction(myName, "whitespace_2_char_p", "WHITESPACE-2-CHAR-P", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_print_function_trampoline", "READTABLE-PRINT-FUNCTION-TRAMPOLINE", 2, 0, false);
	SubLFiles.declareFunction(myName, "readtable_p", "READTABLE-P", 1, 0, false);
	new $readtable_p$UnaryFunction();
	SubLFiles.declareFunction(myName, "readtable_case", "READTABLE-CASE", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_character_syntax", "READTABLE-CHARACTER-SYNTAX", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_single_byte_char_syntax", "READTABLE-SINGLE-BYTE-CHAR-SYNTAX", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_macro_functions", "READTABLE-MACRO-FUNCTIONS", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_single_byte_macro_functions", "READTABLE-SINGLE-BYTE-MACRO-FUNCTIONS", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_constituent_traits", "READTABLE-CONSTITUENT-TRAITS", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_single_byte_constit_traits", "READTABLE-SINGLE-BYTE-CONSTIT-TRAITS", 1, 0, false);
	SubLFiles.declareFunction(myName, "readtable_dispatch_tables", "READTABLE-DISPATCH-TABLES", 1, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_case", "_CSETF-READTABLE-CASE", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_character_syntax", "_CSETF-READTABLE-CHARACTER-SYNTAX", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_single_byte_char_syntax", "_CSETF-READTABLE-SINGLE-BYTE-CHAR-SYNTAX", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_macro_functions", "_CSETF-READTABLE-MACRO-FUNCTIONS", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_single_byte_macro_functions", "_CSETF-READTABLE-SINGLE-BYTE-MACRO-FUNCTIONS", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_constituent_traits", "_CSETF-READTABLE-CONSTITUENT-TRAITS", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_single_byte_constit_traits", "_CSETF-READTABLE-SINGLE-BYTE-CONSTIT-TRAITS", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_readtable_dispatch_tables", "_CSETF-READTABLE-DISPATCH-TABLES", 2, 0, false);
	SubLFiles.declareFunction(myName, "make_readtable", "MAKE-READTABLE", 0, 1, false);
	SubLFiles.declareFunction(myName, "visit_defstruct_readtable", "VISIT-DEFSTRUCT-READTABLE", 2, 0, false);
	SubLFiles.declareFunction(myName, "visit_defstruct_object_readtable_method", "VISIT-DEFSTRUCT-OBJECT-READTABLE-METHOD", 2, 0, false);
	SubLFiles.declareFunction(myName, "create_readtable", "CREATE-READTABLE", 0, 0, false);
	SubLFiles.declareFunction(myName, "single_byte_p", "SINGLE-BYTE-P", 1, 0, false);
	SubLFiles.declareMacro(myName, "character_syntax", "CHARACTER-SYNTAX");
	SubLFiles.declareFunction(myName, "set_character_syntax", "SET-CHARACTER-SYNTAX", 2, 1, false);
	SubLFiles.declareFunction(myName, "invalid_syntaxp", "INVALID-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "terminating_macro_syntaxp", "TERMINATING-MACRO-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "non_terminating_macro_syntaxp", "NON-TERMINATING-MACRO-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "multiple_escape_syntaxp", "MULTIPLE-ESCAPE-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "single_escape_syntaxp", "SINGLE-ESCAPE-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "whitespace_syntaxp", "WHITESPACE-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "constituent_syntaxp", "CONSTITUENT-SYNTAXP", 1, 0, false);
	SubLFiles.declareFunction(myName, "character_function", "CHARACTER-FUNCTION", 1, 1, false);
	SubLFiles.declareFunction(myName, "set_character_function", "SET-CHARACTER-FUNCTION", 2, 1, false);
	SubLFiles.declareFunction(myName, "character_traits", "CHARACTER-TRAITS", 1, 1, false);
	SubLFiles.declareMacro(myName, "has_traitp", "HAS-TRAITP");
	SubLFiles.declareFunction(myName, "set_trait", "SET-TRAIT", 3, 0, false);
	SubLFiles.declareFunction(myName, "reset_trait", "RESET-TRAIT", 1, 1, false);
	SubLFiles.declareFunction(myName, "alphabetic_traitp", "ALPHABETIC-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "digit_traitp", "DIGIT-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "package_marker_traitp", "PACKAGE-MARKER-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "plus_sign_traitp", "PLUS-SIGN-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "minus_sign_traitp", "MINUS-SIGN-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "dot_traitp", "DOT-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "decimal_point_traitp", "DECIMAL-POINT-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "ratio_marker_traitp", "RATIO-MARKER-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "exponent_marker_traitp", "EXPONENT-MARKER-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "invalid_traitp", "INVALID-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "alphadigit_traitp", "ALPHADIGIT-TRAITP", 1, 0, false);
	SubLFiles.declareFunction(myName, "Xcopy_hash_table", "%COPY-HASH-TABLE", 2, 0, false);
	SubLFiles.declareFunction(myName, "copy_readtable", "COPY-READTABLE", 0, 2, false);
	SubLFiles.declareFunction(myName, "make_dispatch_macro_character", "MAKE-DISPATCH-MACRO-CHARACTER", 1, 2, false);
	SubLFiles.declareFunction(myName, "readtablep", "READTABLEP", 1, 0, false);
	SubLFiles.declareFunction(myName, "set_dispatch_macro_character", "SET-DISPATCH-MACRO-CHARACTER", 3, 1, false);
	SubLFiles.declareFunction(myName, "get_dispatch_macro_character", "GET-DISPATCH-MACRO-CHARACTER", 2, 1, false);
	SubLFiles.declareFunction(myName, "get_macro_character", "GET-MACRO-CHARACTER", 1, 1, false);
	SubLFiles.declareFunction(myName, "set_macro_character", "SET-MACRO-CHARACTER", 2, 2, false);
	SubLFiles.declareFunction(myName, "set_syntax_from_char", "SET-SYNTAX-FROM-CHAR", 2, 2, false);
	SubLFiles.declareFunction(myName, "set_syntax", "SET-SYNTAX", 3, 0, false);
	SubLFiles.declareFunction(myName, "simple_reader_error", "SIMPLE-READER-ERROR", 2, 0, false);
	SubLFiles.declareFunction(myName, "end_of_file_error", "END-OF-FILE-ERROR", 1, 0, false);
	SubLFiles.declareFunction(myName, "invalid_token_syntax_error", "INVALID-TOKEN-SYNTAX-ERROR", 2, 0, false);
	SubLFiles.declareFunction(myName, "unimplemented_reader_macro_error", "UNIMPLEMENTED-READER-MACRO-ERROR", 2, 0, false);
	SubLFiles.declareFunction(myName, "unnecessary_argument_error", "UNNECESSARY-ARGUMENT-ERROR", 2, 0, false);
	SubLFiles.declareFunction(myName, "illegal_argument_error", "ILLEGAL-ARGUMENT-ERROR", 3, 0, false);
	SubLFiles.declareFunction(myName, "read_internal", "READ-INTERNAL", 1, 2, false);
	SubLFiles.declareFunction(myName, "read_preserving_whitespace", "READ-PRESERVING-WHITESPACE", 0, 4, false);
	SubLFiles.declareFunction(myName, "read", "READ", 0, 4, false);
	SubLFiles.declareFunction(myName, "read_delimited_list", "READ-DELIMITED-LIST", 1, 2, false);
	SubLFiles.declareFunction(myName, "read_from_string", "READ-FROM-STRING", 1, 5, false);
	SubLFiles.declareMacro(myName, "with_standard_io_syntax", "WITH-STANDARD-IO-SYNTAX");
	SubLFiles.declareMacro(myName, "with_special_reader_error_handling", "WITH-SPECIAL-READER-ERROR-HANDLING");
	SubLFiles.declareFunction(myName, "read_ignoring_errors", "READ-IGNORING-ERRORS", 0, 3, false);
	SubLFiles.declareFunction(myName, "read_from_string_ignoring_errors", "READ-FROM-STRING-IGNORING-ERRORS", 1, 4, false);
	SubLFiles.declareFunction(myName, "read_token_print_function_trampoline", "READ-TOKEN-PRINT-FUNCTION-TRAMPOLINE", 2, 0, false);
	SubLFiles.declareFunction(myName, "read_token_p", "READ-TOKEN-P", 1, 0, false);
	new $read_token_p$UnaryFunction();
	SubLFiles.declareFunction(myName, "read_token_pointer", "READ-TOKEN-POINTER", 1, 0, false);
	SubLFiles.declareFunction(myName, "read_token_buffer", "READ-TOKEN-BUFFER", 1, 0, false);
	SubLFiles.declareFunction(myName, "read_token_escapes", "READ-TOKEN-ESCAPES", 1, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_read_token_pointer", "_CSETF-READ-TOKEN-POINTER", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_read_token_buffer", "_CSETF-READ-TOKEN-BUFFER", 2, 0, false);
	SubLFiles.declareFunction(myName, "_csetf_read_token_escapes", "_CSETF-READ-TOKEN-ESCAPES", 2, 0, false);
	SubLFiles.declareFunction(myName, "make_read_token", "MAKE-READ-TOKEN", 0, 1, false);
	SubLFiles.declareFunction(myName, "visit_defstruct_read_token", "VISIT-DEFSTRUCT-READ-TOKEN", 2, 0, false);
	SubLFiles.declareFunction(myName, "visit_defstruct_object_read_token_method", "VISIT-DEFSTRUCT-OBJECT-READ-TOKEN-METHOD", 2, 0, false);
	SubLFiles.declareMacro(myName, "with_read_token", "WITH-READ-TOKEN");
	SubLFiles.declareFunction(myName, "add_to_token", "ADD-TO-TOKEN", 3, 0, false);
	SubLFiles.declareFunction(myName, "read_token_string", "READ-TOKEN-STRING", 1, 0, false);
	SubLFiles.declareFunction(myName, "read_token", "READ-TOKEN", 3, 0, false);
	SubLFiles.declareFunction(myName, "potential_number_p", "POTENTIAL-NUMBER-P", 2, 0, false);
	SubLFiles.declareFunction(myName, "parse_token", "PARSE-TOKEN", 2, 0, false);
	SubLFiles.declareFunction(myName, "symbol_escapep", "SYMBOL-ESCAPEP", 1, 0, false);
	SubLFiles.declareFunction(myName, "reader_find_package", "READER-FIND-PACKAGE", 3, 0, false);
	SubLFiles.declareFunction(myName, "frob_case", "FROB-CASE", 1, 0, false);
	SubLFiles.declareFunction(myName, "parse_symbol_or_consing_dot", "PARSE-SYMBOL-OR-CONSING-DOT", 2, 0, false);
	SubLFiles.declareFunction(myName, "float_parse_integer", "FLOAT-PARSE-INTEGER", 4, 0, false);
	SubLFiles.declareFunction(myName, "parse_number", "PARSE-NUMBER", 2, 0, false);
	SubLFiles.declareFunction(myName, "parse_integer", "PARSE-INTEGER", 1, 4, false);
	SubLFiles.declareFunction(myName, "whitespace_rmf", "WHITESPACE-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "constituent_rmf", "CONSTITUENT-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "invalid_rmf", "INVALID-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "single_escape_rmf", "SINGLE-ESCAPE-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "multiple_escape_rmf", "MULTIPLE-ESCAPE-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_capital_g_rmf", "SHARPSIGN-CAPITAL-G-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "left_paren_rmf", "LEFT-PAREN-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "right_paren_rmf", "RIGHT-PAREN-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "single_quote_rmf", "SINGLE-QUOTE-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "semicolon_rmf", "SEMICOLON-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "double_quote_rmf", "DOUBLE-QUOTE-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "bq_cons", "BQ-CONS", 2, 0, false);
	SubLFiles.declareMacro(myName, "bq_list", "BQ-LIST");
	SubLFiles.declareMacro(myName, "bq_listX", "BQ-LIST*");
	SubLFiles.declareMacro(myName, "bq_append", "BQ-APPEND");
	SubLFiles.declareMacro(myName, "bq_nconc", "BQ-NCONC");
	SubLFiles.declareFunction(myName, "bq_vector", "BQ-VECTOR", 1, 0, false);
	SubLFiles.declareMacro(myName, "bq_vector_append", "BQ-VECTOR-APPEND");
	SubLFiles.declareFunction(myName, "backquotify", "BACKQUOTIFY", 2, 0, false);
	SubLFiles.declareFunction(myName, "comma", "COMMA", 1, 0, false);
	SubLFiles.declareFunction(myName, "self_evaluating_atom_p", "SELF-EVALUATING-ATOM-P", 1, 0, false);
	SubLFiles.declareFunction(myName, "backquotify_1", "BACKQUOTIFY-1", 2, 0, false);
	SubLFiles.declareFunction(myName, "backquote_rmf", "BACKQUOTE-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "comma_rmf", "COMMA-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "dispatching_rmf", "DISPATCHING-RMF", 2, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_backslash_rmf", "SHARPSIGN-BACKSLASH-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_single_quote_rmf", "SHARPSIGN-SINGLE-QUOTE-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_left_paren_rmf", "SHARPSIGN-LEFT-PAREN-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_asterisk_rmf", "SHARPSIGN-ASTERISK-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_colon_rmf", "SHARPSIGN-COLON-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_dot_rmf", "SHARPSIGN-DOT-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "read_number_in_radix", "READ-NUMBER-IN-RADIX", 2, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_b_rmf", "SHARPSIGN-B-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_o_rmf", "SHARPSIGN-O-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_x_rmf", "SHARPSIGN-X-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_r_rmf", "SHARPSIGN-R-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_c_rmf", "SHARPSIGN-C-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_a_rmf", "SHARPSIGN-A-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_s_rmf", "SHARPSIGN-S-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_p_rmf", "SHARPSIGN-P-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "read_lookup", "READ-LOOKUP", 1, 0, false);
	SubLFiles.declareFunction(myName, "set_read_lookup", "SET-READ-LOOKUP", 2, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_equals_rmf", "SHARPSIGN-EQUALS-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_sharpsign_rmf", "SHARPSIGN-SHARPSIGN-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "features_match", "FEATURES-MATCH", 1, 0, false);
	SubLFiles.declareFunction(myName, "fix_features", "FIX-FEATURES", 1, 0, false);
	SubLFiles.declareFunction(myName, "read_features", "READ-FEATURES", 1, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_plus_rmf", "SHARPSIGN-PLUS-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_minus_rmf", "SHARPSIGN-MINUS-RMF", 3, 0, false);
	SubLFiles.declareFunction(myName, "sharpsign_vertical_bar_rmf", "SHARPSIGN-VERTICAL-BAR-RMF", 3, 0, false);
	return SubLNil.NIL;
    }
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return SubLObjectFactory.makeBoolean(Numbers.logand(reader.$digit$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))).isPositive() && SubLNil.NIL != Characters.digit_char_p(ch, reader.$read_base$.getDynamicValue(thread)));
    }
	...
	SubLObject next_ch;
	for (next_ch = streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED); SubLNil.NIL != Characters.digit_char_p(next_ch, CommonSymbols.TEN_INTEGER); next_ch = streams_high.read_char(stream, CommonSymbols.UNPROVIDED,
		CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED)) {
	    if (SubLNil.NIL == arg)
		arg = CommonSymbols.ZERO_INTEGER;
	    arg = Numbers.add(Numbers.multiply(arg, CommonSymbols.TEN_INTEGER), Characters.digit_char_p(next_ch, CommonSymbols.TEN_INTEGER));
	}
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$dot$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	if (SubLNil.NIL != reader.$read_suppress$.getDynamicValue(thread)) {
	    while (true) {
		SubLObject next_ch = streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	...
	_csetf_read_token_escapes(token, SubLNil.NIL);
	while (true) {
	    SubLObject next_ch2 = streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$exponent_marker$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	if (!feature.isCons())
	    return conses_high.member(feature, reader.$features$.getDynamicValue(thread), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	...
	for (i = SubLNil.NIL, i = start; !i.numGE(end); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
	    ...
	    value = Numbers.add(Numbers.multiply(SubLNil.NIL != value ? value : reader.$float$0_0, radix), Numbers.sublisp_float(digit, CommonSymbols.UNPROVIDED));
	}
	return value;
    }
	...
	if (!pcase_var.eql(reader.$kw147$PRESERVE))
		...
		SubLObject ch;
		for (cdotimes_end_var = read_token_pointer(token), i = SubLNil.NIL, i = CommonSymbols.ZERO_INTEGER; i.numL(cdotimes_end_var); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
	...
	if (SubLNil.NIL != frobber) {
	    ...
	    SubLObject curr;
	    for (cdotimes_end_var2 = read_token_pointer(token), j = SubLNil.NIL, j = CommonSymbols.ZERO_INTEGER; j.numL(cdotimes_end_var2); j = Numbers.add(j, CommonSymbols.ONE_INTEGER)) {
	...
	if (readtable == CommonSymbols.UNPROVIDED)
	    readtable = reader.$readtable$.getDynamicValue();
	SubLObject bucket = conses_high.assoc(disp_char, readtable_dispatch_tables(readtable), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	...
	if (readtable == CommonSymbols.UNPROVIDED)
	    readtable = reader.$readtable$.getDynamicValue();
	SubLObject syntax = SubLNil.NIL != single_byte_p(v_char) ? Vectors.aref(readtable_single_byte_char_syntax(readtable), Characters.char_code(v_char)) : Hashtables.gethash(v_char, readtable_character_syntax(readtable), reader.$kw49$CONSTITUENT);
	if (syntax == reader.$kw60$TERMINATING_MACRO || syntax == reader.$kw61$NON_TERMINATING_MACRO)
	    return Values.values(character_function(v_char, readtable), Equality.eq(syntax, reader.$kw61$NON_TERMINATING_MACRO));
	return Values.values(SubLNil.NIL, SubLNil.NIL);
    }
	...
	current = current.rest();
	if (SubLNil.NIL == current)
	    return ConsesLow.list(reader.$sym66$PLUSP, ConsesLow.list(reader.$sym67$LOGAND, trait, ConsesLow.list(reader.$sym68$CHARACTER_TRAITS, ch, readtable)));
	...
	reader.$whitespace_1_chars$ = SubLFiles.defvar("*WHITESPACE-1-CHARS*", ConsesLow.list(Characters.CHAR_space, Characters.code_char(CommonSymbols.TEN_INTEGER), Characters.code_char(CommonSymbols.THIRTEEN_INTEGER), Characters.CHAR_tab, Characters.CHAR_page, Characters.CHAR_rubout));
	...
	reader.$initial_table_size$ = SubLFiles.defparameter("*INITIAL-TABLE-SIZE*", reader.$int$32);
	reader.$single_byte_max_code$ = SubLFiles.defconstant("*SINGLE-BYTE-MAX-CODE*", reader.$int$128);
	...
	reader.$dot$ = SubLFiles.defconstant("*DOT*", reader.$int$32);
	reader.$decimal_point$ = SubLFiles.defconstant("*DECIMAL-POINT*", reader.$int$64);
	reader.$ratio_marker$ = SubLFiles.defconstant("*RATIO-MARKER*", reader.$int$128);
	reader.$exponent_marker$ = SubLFiles.defconstant("*EXPONENT-MARKER*", reader.$int$256);
	reader.$invalid$ = SubLFiles.defconstant("*INVALID*", reader.$int$512);
	...
	reader.$initial_token_size$ = SubLFiles.defconstant("*INITIAL-TOKEN-SIZE*", reader.$int$32);
	...
	reader.$keyword_package$ = SubLFiles.deflexical("*KEYWORD-PACKAGE*", Packages.find_package(reader.$str150$KEYWORD));
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$invalid$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	return ConsesLow.listS(reader.$sym0$CDO, SubLNil.NIL, reader.$list1, ConsesLow.append(body, SubLNil.NIL));
    }
	...
	if (readtable == CommonSymbols.UNPROVIDED)
	    readtable = reader.$readtable$.getDynamicValue();
	SubLThread thread = SubLProcess.currentSubLThread();
	_csetf_readtable_dispatch_tables(readtable, Sequences.delete(v_char, readtable_dispatch_tables(readtable), Symbols.symbol_function(CommonSymbols.EQL), Symbols.symbol_function(reader.$sym70$CAR), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED));
	_csetf_readtable_dispatch_tables(readtable, ConsesLow.cons(ConsesLow.cons(v_char, Hashtables.make_hash_table(reader.$initial_table_size$.getDynamicValue(thread), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED)), readtable_dispatch_tables(readtable)));
	set_character_function(v_char, reader.$sym71$DISPATCHING_RMF, readtable);
	set_character_syntax(v_char, SubLNil.NIL != non_terminating_p ? reader.$kw61$NON_TERMINATING_MACRO : reader.$kw60$TERMINATING_MACRO, readtable);
	return CommonSymbols.T;
    }
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$minus_sign$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$package_marker$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	while (!i.numGE(end)) {
		...
		if (SubLNil.NIL == whitespace_1_char_p(ch))
			...
			simple_reader_error(reader.$str169$_S_does_not_contain_an_integer_, ConsesLow.list(Sequences.subseq(string, start, end)));
		    }
	...
	if (pcase_var2.eql(reader.$kw158$DIGITS))
	    return Numbers.multiply(sign, parse_integer(buffer, start1, length, reader.$read_base$.getDynamicValue(thread), CommonSymbols.UNPROVIDED));
	if (pcase_var2.eql(reader.$kw157$DECIMALS))
	    return Numbers.multiply(sign, parse_integer(buffer, start1, length, reader.$read_base$.getDynamicValue(thread), CommonSymbols.UNPROVIDED));
	if (pcase_var2.eql(reader.$kw160$DOT2))
	    return Numbers.multiply(sign, parse_integer(buffer, start1, end1, CommonSymbols.TEN_INTEGER, CommonSymbols.UNPROVIDED));
	if (pcase_var2.eql(reader.$kw162$DENOMINATOR))
	    return Numbers.multiply(sign, Numbers.divide(parse_integer(buffer, start1, end1, reader.$read_base$.getDynamicValue(thread), CommonSymbols.UNPROVIDED), parse_integer(buffer, start2, length, reader.$read_base$.getDynamicValue(thread), CommonSymbols.UNPROVIDED)));
	if (pcase_var2.eql(reader.$kw163$FRACTION))
	    return Numbers.multiply(sign, math_utilities.bellerophon(parse_integer(buffer, start2, length, CommonSymbols.TEN_INTEGER, CommonSymbols.UNPROVIDED), Numbers.minus(Numbers.subtract(length, start2))));
	if (pcase_var2.eql(reader.$kw164$DECIMALS2)) {
	    ...
	    SubLObject right = parse_integer(buffer, start2, length, CommonSymbols.TEN_INTEGER, CommonSymbols.UNPROVIDED);
	    return Numbers.multiply(sign, math_utilities.bellerophon(Numbers.add(Numbers.multiply(left, Numbers.expt(CommonSymbols.TEN_INTEGER, Numbers.subtract(length, start2))), right), Numbers.minus(Numbers.subtract(length, start2))));
	}
	if (!pcase_var2.eql(reader.$kw166$EXPONENT_VALUE))
	    return invalid_token_syntax_error(stream, token);
	SubLObject left = SubLNil.NIL != start1 ? parse_integer(buffer, start1, end1, CommonSymbols.TEN_INTEGER, CommonSymbols.UNPROVIDED) : SubLNil.NIL;
	SubLObject right = SubLNil.NIL != start2 ? parse_integer(buffer, start2, end2, CommonSymbols.TEN_INTEGER, CommonSymbols.UNPROVIDED) : SubLNil.NIL;
	SubLObject exponent = parse_integer(buffer, start3, length, CommonSymbols.TEN_INTEGER, CommonSymbols.UNPROVIDED);
	if (SubLNil.NIL != left && SubLNil.NIL != right)
	    return Numbers.multiply(sign, math_utilities.bellerophon(Numbers.add(Numbers.multiply(left, Numbers.expt(CommonSymbols.TEN_INTEGER, Numbers.subtract(end2, start2))), right), Numbers.subtract(Numbers.multiply(exponent_sign, exponent), Numbers.subtract(end2, start2))));
	if (SubLNil.NIL != left)
	...
	if (dot_count.numE(length)) {
	    if (length.numE(CommonSymbols.ONE_INTEGER) && SubLNil.NIL != reader.$reading_list$.getDynamicValue(thread) && SubLNil.NIL == reader.$reading_fake_list$.getDynamicValue(thread))
		return reader.$consing_dot$.getDynamicValue(thread);
	    return invalid_token_syntax_error(stream, token);
	} else {
	    if (SubLNil.NIL != pkg_idx1 && SubLNil.NIL == pkg_idx2 && pkg_idx1.numE(CommonSymbols.ZERO_INTEGER)) {
	    ...
	    if (SubLNil.NIL != pkg_idx1 && SubLNil.NIL == pkg_idx2 && pkg_idx1.numL(Numbers.subtract(length, CommonSymbols.ONE_INTEGER))) {
		...
		if (SubLNil.NIL != pkg_idx1 && SubLNil.NIL != pkg_idx2 && pkg_idx2.numE(Numbers.add(pkg_idx1, CommonSymbols.ONE_INTEGER)) && pkg_idx1.numG(CommonSymbols.ZERO_INTEGER) && pkg_idx2.numL(Numbers.subtract(length, CommonSymbols.ONE_INTEGER)))
		    return Packages.intern(Sequences.subseq(buffer, Numbers.add(pkg_idx2, CommonSymbols.ONE_INTEGER), length), reader_find_package(buffer, CommonSymbols.ZERO_INTEGER, pkg_idx1));
		if (SubLNil.NIL == pkg_idx1 && SubLNil.NIL == pkg_idx2 && length.isPositive())
		    return Packages.intern(Sequences.subseq(buffer, CommonSymbols.ZERO_INTEGER, length), Packages.$package$.getDynamicValue(thread));
		return invalid_token_syntax_error(stream, token);
	    }
	}
    }
	...
	if (SubLNil.NIL == escapes && SubLNil.NIL != potential_number_p(read_token_buffer(token), read_token_pointer(token)))
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$plus_sign$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	for (i = SubLNil.NIL, i = CommonSymbols.ZERO_INTEGER; i.numL(length); i = Numbers.add(i, CommonSymbols.ONE_INTEGER)) {
	    ...
	    if (SubLNil.NIL != digit_traitp(ch) || SubLNil.NIL != plus_sign_traitp(ch) || SubLNil.NIL != minus_sign_traitp(ch) || SubLNil.NIL != ratio_marker_traitp(ch) || SubLNil.NIL != decimal_point_traitp(ch) || SubLNil.NIL != Characters.charE(ch, Characters.CHAR_caret)
	...
	SubLObject j;
	SubLObject ch2;
	for (j = SubLNil.NIL, j = CommonSymbols.ZERO_INTEGER; j.numL(length); j = Numbers.add(j, CommonSymbols.ONE_INTEGER)) {
	...
	if (SubLNil.NIL != decimal_point_p && SubLNil.NIL == decimal_digit_p || SubLNil.NIL == digit_p)
	    return SubLNil.NIL;
	SubLObject first = Strings.sublisp_char(string, CommonSymbols.ZERO_INTEGER);
	if (SubLNil.NIL == digit_traitp(first) && SubLNil.NIL == plus_sign_traitp(first) && SubLNil.NIL == minus_sign_traitp(first) && SubLNil.NIL == decimal_point_traitp(first) && SubLNil.NIL == Characters.charE(first, Characters.CHAR_caret)
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return Numbers.plusp(Numbers.logand(reader.$ratio_marker$.getGlobalValue(), character_traits(ch, reader.$readtable$.getDynamicValue(thread))));
    }
	...
	SubLObject tail = SubLNil.NIL;
	while (true) {
	    SubLObject ch = streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	    if (SubLNil.NIL != Characters.charE(ch, v_char))
		break;
	    if (SubLNil.NIL != whitespace_syntaxp(ch))
		continue;
	    streams_high.unread_char(ch, stream);
	    SubLObject next = ConsesLow.cons(read(stream, CommonSymbols.T, SubLNil.NIL, CommonSymbols.T), SubLNil.NIL);
	    if (SubLNil.NIL != list) {
	...
	try {
		...
		try {
		    reader.$throw_end_of_file_p$.bind(SubLNil.NIL != reader.$throw_end_of_file_p$.getDynamicValue(thread) ? CommonSymbols.T : SubLObjectFactory.makeBoolean(SubLNil.NIL == eof_error_p), thread);
	...
	if (SubLNil.NIL != reader.$read_lookup_table$.getDynamicValue(thread)) {
	    thread.resetMultipleValues();
	    SubLObject value = Hashtables.gethash(number, reader.$read_lookup_table$.getDynamicValue(thread), CommonSymbols.UNPROVIDED);
	...
	while (true) {
		...
		else if (SubLNil.NIL != constituent_syntaxp(ch) || SubLNil.NIL != non_terminating_macro_syntaxp(ch) || SubLNil.NIL != terminating_macro_syntaxp(ch) || SubLNil.NIL != whitespace_syntaxp(ch))
		    add_to_token(ch, token, CommonSymbols.T);
		else if (SubLNil.NIL != single_escape_syntaxp(ch))
		    add_to_token(streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED), token, CommonSymbols.T);
		else if (SubLNil.NIL != multiple_escape_syntaxp(ch))
		    escapep = SubLNil.NIL;
		else if (SubLNil.NIL != invalid_syntaxp(ch) && SubLNil.NIL == reader.$read_suppress$.getDynamicValue(thread))
		    simple_reader_error(reader.$str143$_A_is_an_invalid_character_, ConsesLow.list(ch));
		else
		...
		if (SubLNil.NIL == ch)
		    return token;
		if (SubLNil.NIL != constituent_syntaxp(ch) || SubLNil.NIL != non_terminating_macro_syntaxp(ch))
		    add_to_token(ch, token, SubLNil.NIL);
		else if (SubLNil.NIL != single_escape_syntaxp(ch))
		    add_to_token(streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED), token, CommonSymbols.T);
		else if (SubLNil.NIL != multiple_escape_syntaxp(ch))
		    ...
		    if (SubLNil.NIL != terminating_macro_syntaxp(ch) || SubLNil.NIL != whitespace_syntaxp(ch)) {
			streams_high.unread_char(ch, stream);
			return token;
		    }
		    simple_reader_error(reader.$str144$Fell_through_character_syntax_on_, ConsesLow.list(ch));
	...
	if (SubLNil.NIL == v_package)
	    simple_reader_error(reader.$str146$_S_is_not_a_known_package_, ConsesLow.list(Sequences.subseq(string, start, end)));
	return v_package;
    }
	...
	if (readtable == CommonSymbols.UNPROVIDED)
	    readtable = reader.$readtable$.getDynamicValue();
	if (SubLNil.NIL != single_byte_p(ch))
	    Vectors.set_aref(readtable_single_byte_constit_traits(readtable), Characters.char_code(ch), reader.$alphabetic$.getGlobalValue());
	...
	return SubLObjectFactory.makeBoolean(object.isAtom() && (SubLNil.NIL == object || CommonSymbols.T == object || object.isKeyword() || !object.isSymbol()));
    }
	...
	while (SubLNil.NIL != next_ch && SubLNil.NIL == Characters.charE(next_ch, Characters.CHAR_return) && SubLNil.NIL == Characters.charE(next_ch, Characters.CHAR_newline));
	return Values.values(SubLNil.NIL, SubLNil.NIL);
    }
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	if (SubLNil.NIL == reader.$read_lookup_table$.getDynamicValue(thread))
	    reader.$read_lookup_table$.setDynamicValue(Hashtables.make_hash_table(reader.$int$32, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED), thread);
	...
	if (SubLNil.NIL == from_readtable)
	    from_readtable = reader.$standard_readtable$.getDynamicValue(thread);
	SubLObject pcase_var;
	SubLObject syntax = pcase_var = SubLNil.NIL != single_byte_p(from_char) ? Vectors.aref(readtable_single_byte_char_syntax(from_readtable), Characters.char_code(from_char)) : Hashtables.gethash(from_char, readtable_character_syntax(from_readtable), reader.$kw49$CONSTITUENT);
	if (pcase_var.eql(reader.$kw60$TERMINATING_MACRO) || pcase_var.eql(reader.$kw61$NON_TERMINATING_MACRO)) {
		...
		SubLObject from_table = conses_high.assoc(from_char, readtable_dispatch_tables(from_readtable), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED).rest();
		SubLObject to_table = Hashtables.make_hash_table(Hashtables.hash_table_count(from_table), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
		_csetf_readtable_dispatch_tables(to_readtable, ConsesLow.cons(ConsesLow.cons(to_char, Xcopy_hash_table(from_table, to_table)),
			Sequences.delete(to_char, readtable_dispatch_tables(to_readtable), Symbols.symbol_function(CommonSymbols.EQL), Symbols.symbol_function(reader.$sym70$CAR), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED)));
	    }
	...
	Structures.register_method(print_high.$print_object_method_table$.getGlobalValue(), reader.$dtp_readtable$.getGlobalValue(), Symbols.symbol_function(reader.$sym13$READTABLE_PRINT_FUNCTION_TRAMPOLINE));
	...
	Structures.def_csetf(reader.$sym19$READTABLE_SINGLE_BYTE_CHAR_SYNTAX, reader.$sym20$_CSETF_READTABLE_SINGLE_BYTE_CHAR_SYNTAX);
	Structures.def_csetf(reader.$sym21$READTABLE_MACRO_FUNCTIONS, reader.$sym22$_CSETF_READTABLE_MACRO_FUNCTIONS);
	Structures.def_csetf(reader.$sym23$READTABLE_SINGLE_BYTE_MACRO_FUNCTIONS, reader.$sym24$_CSETF_READTABLE_SINGLE_BYTE_MACRO_FUNCTIONS);
	Structures.def_csetf(reader.$sym25$READTABLE_CONSTITUENT_TRAITS, reader.$sym26$_CSETF_READTABLE_CONSTITUENT_TRAITS);
	Structures.def_csetf(reader.$sym27$READTABLE_SINGLE_BYTE_CONSTIT_TRAITS, reader.$sym28$_CSETF_READTABLE_SINGLE_BYTE_CONSTIT_TRAITS);
	...
	Structures.register_method(visitation.$visit_defstruct_object_method_table$.getGlobalValue(), reader.$dtp_readtable$.getGlobalValue(), Symbols.symbol_function(reader.$sym44$VISIT_DEFSTRUCT_OBJECT_READTABLE_METHOD));
	...
	set_macro_character(Characters.CHAR_lparen, reader.$sym77$LEFT_PAREN_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	set_macro_character(Characters.CHAR_rparen, reader.$sym78$RIGHT_PAREN_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	set_macro_character(Characters.CHAR_quote, reader.$sym79$SINGLE_QUOTE_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	set_macro_character(Characters.CHAR_semicolon, reader.$sym80$SEMICOLON_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	set_macro_character(Characters.CHAR_quotation, reader.$sym81$DOUBLE_QUOTE_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	set_macro_character(Characters.CHAR_backquote, reader.$sym82$BACKQUOTE_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	set_macro_character(Characters.CHAR_comma, reader.$sym83$COMMA_RMF, SubLNil.NIL, reader.$standard_readtable$.getDynamicValue());
	make_dispatch_macro_character(Characters.CHAR_hash, CommonSymbols.T, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_backslash, reader.$sym84$SHARPSIGN_BACKSLASH_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_quote, reader.$sym85$SHARPSIGN_SINGLE_QUOTE_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_lparen, reader.$sym86$SHARPSIGN_LEFT_PAREN_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_asterisk, reader.$sym87$SHARPSIGN_ASTERISK_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_colon, reader.$sym88$SHARPSIGN_COLON_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_period, reader.$sym89$SHARPSIGN_DOT_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_B, reader.$sym90$SHARPSIGN_B_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_O, reader.$sym91$SHARPSIGN_O_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_X, reader.$sym92$SHARPSIGN_X_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_R, reader.$sym93$SHARPSIGN_R_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_C, reader.$sym94$SHARPSIGN_C_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_A, reader.$sym95$SHARPSIGN_A_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_S, reader.$sym96$SHARPSIGN_S_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_P, reader.$sym97$SHARPSIGN_P_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_G, reader.$sym98$SHARPSIGN_CAPITAL_G_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_equal, reader.$sym99$SHARPSIGN_EQUALS_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_hash, reader.$sym100$SHARPSIGN_SHARPSIGN_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_plus, reader.$sym101$SHARPSIGN_PLUS_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_hyphen, reader.$sym102$SHARPSIGN_MINUS_RMF, reader.$standard_readtable$.getDynamicValue());
	set_dispatch_macro_character(Characters.CHAR_hash, Characters.CHAR_vertical, reader.$sym103$SHARPSIGN_VERTICAL_BAR_RMF, reader.$standard_readtable$.getDynamicValue());
	...
	set_trait(Characters.CHAR_colon, reader.$standard_readtable$.getDynamicValue(), reader.$package_marker$.getGlobalValue());
	set_trait(Characters.CHAR_plus, reader.$standard_readtable$.getDynamicValue(), reader.$alphabetic$.getGlobalValue());
	set_trait(Characters.CHAR_plus, reader.$standard_readtable$.getDynamicValue(), reader.$plus_sign$.getGlobalValue());
	set_trait(Characters.CHAR_hyphen, reader.$standard_readtable$.getDynamicValue(), reader.$alphabetic$.getGlobalValue());
	set_trait(Characters.CHAR_hyphen, reader.$standard_readtable$.getDynamicValue(), reader.$minus_sign$.getGlobalValue());
	set_trait(Characters.CHAR_period, reader.$standard_readtable$.getDynamicValue(), reader.$alphabetic$.getGlobalValue());
	set_trait(Characters.CHAR_period, reader.$standard_readtable$.getDynamicValue(), reader.$dot$.getGlobalValue());
	set_trait(Characters.CHAR_period, reader.$standard_readtable$.getDynamicValue(), reader.$decimal_point$.getGlobalValue());
	set_trait(Characters.CHAR_slash, reader.$standard_readtable$.getDynamicValue(), reader.$alphabetic$.getGlobalValue());
	set_trait(Characters.CHAR_slash, reader.$standard_readtable$.getDynamicValue(), reader.$ratio_marker$.getGlobalValue());
	set_trait(Characters.CHAR_D, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_d, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_E, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_e, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_F, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_f, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_L, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_l, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_S, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	set_trait(Characters.CHAR_s, reader.$standard_readtable$.getDynamicValue(), reader.$exponent_marker$.getGlobalValue());
	if (SubLNil.NIL == Symbols.boundp(reader.$sym52$_READTABLE_) || SubLNil.NIL == reader.$readtable$.getDynamicValue())
	    reader.$readtable$.setDynamicValue(copy_readtable(reader.$standard_readtable$.getDynamicValue(), CommonSymbols.UNPROVIDED));
	Structures.register_method(print_high.$print_object_method_table$.getGlobalValue(), reader.$dtp_read_token$.getGlobalValue(), Symbols.symbol_function(reader.$sym125$READ_TOKEN_PRINT_FUNCTION_TRAMPOLINE));
	...
	Structures.register_method(visitation.$visit_defstruct_object_method_table$.getGlobalValue(), reader.$dtp_read_token$.getGlobalValue(), Symbols.symbol_function(reader.$sym137$VISIT_DEFSTRUCT_OBJECT_READ_TOKEN_METHOD));
	return SubLNil.NIL;
    }
	...
	return Values.values(Functions.apply(Symbols.symbol_function(reader.$sym191$VECTOR), contents), CommonSymbols.T);
    }
	...
	if (SubLNil.NIL == reader.$read_eval$.getDynamicValue(thread)) {
	    simple_reader_error(reader.$str210$Attempt_the_use_the__A_reader_mac, ConsesLow.list(ch, reader.$sym211$_READ_EVAL_, reader.$read_eval$.getDynamicValue(thread)));
	    return SubLNil.NIL;
	}
	...
	if (SubLNil.NIL != arg)
	    if (!arg.numE(length))
		if (length.numL(arg))
		    elts = ConsesLow.append(elts, ConsesLow.make_list(Numbers.subtract(arg, length), conses_high.last(elts, CommonSymbols.UNPROVIDED).first()));
		else
		    illegal_argument_error(stream, ch, arg);
	return Values.values(reader.$backquote_depth$.getDynamicValue(thread).numG(CommonSymbols.ZERO_INTEGER) ? ConsesLow.cons(reader.$bq_vector$.getDynamicValue(thread), elts) : Functions.apply(Symbols.symbol_function(reader.$sym191$VECTOR), elts), CommonSymbols.T);
    }
	...
	if (SubLNil.NIL == arg || !arg.numGE(CommonSymbols.TWO_INTEGER) || !arg.numLE(reader.$int$36))
	    illegal_argument_error(stream, ch, arg);
	...
	ch_$4 = streams_high.read_char(stream, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
	while (true) {
	    SubLObject pcase_var = state;
	...
	return Values.values(SubLNil.NIL != reader.$read_suppress$.getDynamicValue(thread) ? SubLNil.NIL : ConsesLow.list(reader.$sym184$QUOTE, object), CommonSymbols.T);
    }
	...
	Functions.funcall(visitor_fn, obj, reader.$kw40$BEGIN, reader.$sym136$MAKE_READ_TOKEN, CommonSymbols.THREE_INTEGER);
	...
	Functions.funcall(visitor_fn, obj, reader.$kw43$END, reader.$sym136$MAKE_READ_TOKEN, CommonSymbols.THREE_INTEGER);
	return obj;
    }

    @SubL(source = "sublisp/reader.lisp", position = 6265L)
    public static SubLObject visit_defstruct_readtable(SubLObject obj, SubLObject visitor_fn) {
	Functions.funcall(visitor_fn, obj, reader.$kw40$BEGIN, reader.$sym41$MAKE_READTABLE, CommonSymbols.EIGHT_INTEGER);
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw31$CASE, readtable_case(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw32$CHARACTER_SYNTAX, readtable_character_syntax(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw33$SINGLE_BYTE_CHAR_SYNTAX, readtable_single_byte_char_syntax(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw34$MACRO_FUNCTIONS, readtable_macro_functions(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw35$SINGLE_BYTE_MACRO_FUNCTIONS, readtable_single_byte_macro_functions(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw36$CONSTITUENT_TRAITS, readtable_constituent_traits(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw37$SINGLE_BYTE_CONSTIT_TRAITS, readtable_single_byte_constit_traits(obj));
	Functions.funcall(visitor_fn, obj, reader.$kw42$SLOT, reader.$kw38$DISPATCH_TABLES, readtable_dispatch_tables(obj));
	...
	SubLThread thread = SubLProcess.currentSubLThread();
	return conses_high.member(v_char, reader.$whitespace_1_chars$.getDynamicValue(thread), CommonSymbols.UNPROVIDED, CommonSymbols.UNPROVIDED);
    }
	...
	return ConsesLow.listS(reader.$sym114$CLET, ConsesLow.list(bq_cons(token, reader.$list139)), reader.$list140, reader.$list141, reader.$list142, ConsesLow.append(body, SubLNil.NIL));
    }
    ...
    public final static String myName = "com.cyc.tool.subl.jrtl.translatedCode.sublisp.reader";
    ...
    private static SubLInteger $int$512;
	...
	$list8 = ConsesLow.list(SubLObjectFactory.makeSymbol("CASE"), SubLObjectFactory.makeSymbol("CHARACTER-SYNTAX"), SubLObjectFactory.makeSymbol("SINGLE-BYTE-CHAR-SYNTAX"), SubLObjectFactory.makeSymbol("MACRO-FUNCTIONS"), SubLObjectFactory.makeSymbol("SINGLE-BYTE-MACRO-FUNCTIONS"),
		SubLObjectFactory.makeSymbol("CONSTITUENT-TRAITS"), SubLObjectFactory.makeSymbol("SINGLE-BYTE-CONSTIT-TRAITS"), SubLObjectFactory.makeSymbol("DISPATCH-TABLES"));
	$list9 = ConsesLow.list(SubLObjectFactory.makeKeyword("CASE"), SubLObjectFactory.makeKeyword("CHARACTER-SYNTAX"), SubLObjectFactory.makeKeyword("SINGLE-BYTE-CHAR-SYNTAX"), SubLObjectFactory.makeKeyword("MACRO-FUNCTIONS"), SubLObjectFactory.makeKeyword("SINGLE-BYTE-MACRO-FUNCTIONS"),
		SubLObjectFactory.makeKeyword("CONSTITUENT-TRAITS"), SubLObjectFactory.makeKeyword("SINGLE-BYTE-CONSTIT-TRAITS"), SubLObjectFactory.makeKeyword("DISPATCH-TABLES"));
	$list10 = ConsesLow.list(SubLObjectFactory.makeSymbol("READTABLE-CASE"), SubLObjectFactory.makeSymbol("READTABLE-CHARACTER-SYNTAX"), SubLObjectFactory.makeSymbol("READTABLE-SINGLE-BYTE-CHAR-SYNTAX"), SubLObjectFactory.makeSymbol("READTABLE-MACRO-FUNCTIONS"),
		SubLObjectFactory.makeSymbol("READTABLE-SINGLE-BYTE-MACRO-FUNCTIONS"), SubLObjectFactory.makeSymbol("READTABLE-CONSTITUENT-TRAITS"), SubLObjectFactory.makeSymbol("READTABLE-SINGLE-BYTE-CONSTIT-TRAITS"), SubLObjectFactory.makeSymbol("READTABLE-DISPATCH-TABLES"));
	$list11 = ConsesLow.list(SubLObjectFactory.makeSymbol("_CSETF-READTABLE-CASE"), SubLObjectFactory.makeSymbol("_CSETF-READTABLE-CHARACTER-SYNTAX"), SubLObjectFactory.makeSymbol("_CSETF-READTABLE-SINGLE-BYTE-CHAR-SYNTAX"), SubLObjectFactory.makeSymbol("_CSETF-READTABLE-MACRO-FUNCTIONS"),
		SubLObjectFactory.makeSymbol("_CSETF-READTABLE-SINGLE-BYTE-MACRO-FUNCTIONS"), SubLObjectFactory.makeSymbol("_CSETF-READTABLE-CONSTITUENT-TRAITS"), SubLObjectFactory.makeSymbol("_CSETF-READTABLE-SINGLE-BYTE-CONSTIT-TRAITS"),
	...
	$sym13$READTABLE_PRINT_FUNCTION_TRAMPOLINE = SubLObjectFactory.makeSymbol("READTABLE-PRINT-FUNCTION-TRAMPOLINE");
	$list14 = ConsesLow.list(SubLObjectFactory.makeSymbol("OPTIMIZE-FUNCALL"), SubLObjectFactory.makeSymbol("READTABLE-P"));
	...
	$sym20$_CSETF_READTABLE_SINGLE_BYTE_CHAR_SYNTAX = SubLObjectFactory.makeSymbol("_CSETF-READTABLE-SINGLE-BYTE-CHAR-SYNTAX");
	...
	$sym23$READTABLE_SINGLE_BYTE_MACRO_FUNCTIONS = SubLObjectFactory.makeSymbol("READTABLE-SINGLE-BYTE-MACRO-FUNCTIONS");
	$sym24$_CSETF_READTABLE_SINGLE_BYTE_MACRO_FUNCTIONS = SubLObjectFactory.makeSymbol("_CSETF-READTABLE-SINGLE-BYTE-MACRO-FUNCTIONS");
	$sym25$READTABLE_CONSTITUENT_TRAITS = SubLObjectFactory.makeSymbol("READTABLE-CONSTITUENT-TRAITS");
	$sym26$_CSETF_READTABLE_CONSTITUENT_TRAITS = SubLObjectFactory.makeSymbol("_CSETF-READTABLE-CONSTITUENT-TRAITS");
	$sym27$READTABLE_SINGLE_BYTE_CONSTIT_TRAITS = SubLObjectFactory.makeSymbol("READTABLE-SINGLE-BYTE-CONSTIT-TRAITS");
	$sym28$_CSETF_READTABLE_SINGLE_BYTE_CONSTIT_TRAITS = SubLObjectFactory.makeSymbol("_CSETF-READTABLE-SINGLE-BYTE-CONSTIT-TRAITS");
	...
	$sym44$VISIT_DEFSTRUCT_OBJECT_READTABLE_METHOD = SubLObjectFactory.makeSymbol("VISIT-DEFSTRUCT-OBJECT-READTABLE-METHOD");
	$int$64 = SubLObjectFactory.makeInteger(64);
	$int$256 = SubLObjectFactory.makeInteger(256);
	$int$512 = SubLObjectFactory.makeInteger(512);
	...
	$list51 = ConsesLow.list(SubLObjectFactory.makeSymbol("CH"), SubLObjectFactory.makeSymbol("&OPTIONAL"), ConsesLow.list(SubLObjectFactory.makeSymbol("READTABLE"), ConsesLow.list(SubLObjectFactory.makeSymbol("QUOTE"), SubLObjectFactory.makeSymbol("*READTABLE*"))));
	...
	$list65 = ConsesLow.list(SubLObjectFactory.makeSymbol("CH"), SubLObjectFactory.makeSymbol("TRAIT"), SubLObjectFactory.makeSymbol("&OPTIONAL"),
		ConsesLow.list(SubLObjectFactory.makeSymbol("READTABLE"), ConsesLow.list(SubLObjectFactory.makeSymbol("QUOTE"), SubLObjectFactory.makeSymbol("*READTABLE*"))));
	...
	$str72$Attempt_to_make_decimal_digit__A_ = SubLObjectFactory.makeString("Attempt to make decimal digit ~A be a dispatching macro character.");
	$str73$_A_is_not_a_dispatching_macro_cha = SubLObjectFactory.makeString("~A is not a dispatching macro character in ~S.");
	...
	$list104 = ConsesLow.list(Characters.CHAR_backspace, Characters.CHAR_tab, Characters.CHAR_newline, Characters.CHAR_newline, Characters.CHAR_page, Characters.CHAR_return, Characters.CHAR_space, Characters.CHAR_rubout);
	$list105 = ConsesLow.list(new SubLObject[] { Characters.CHAR_0, Characters.CHAR_1, Characters.CHAR_2, Characters.CHAR_3, Characters.CHAR_4, Characters.CHAR_5, Characters.CHAR_6, Characters.CHAR_7, Characters.CHAR_8, Characters.CHAR_9, Characters.CHAR_a, Characters.CHAR_b, Characters.CHAR_c,
		Characters.CHAR_d, Characters.CHAR_e, Characters.CHAR_f, Characters.CHAR_g, Characters.CHAR_h, Characters.CHAR_i, Characters.CHAR_j, Characters.CHAR_k, Characters.CHAR_l, Characters.CHAR_m, Characters.CHAR_n, Characters.CHAR_o, Characters.CHAR_p, Characters.CHAR_q, Characters.CHAR_r,
		Characters.CHAR_s, Characters.CHAR_t, Characters.CHAR_u, Characters.CHAR_v, Characters.CHAR_w, Characters.CHAR_x, Characters.CHAR_y, Characters.CHAR_z, Characters.CHAR_A, Characters.CHAR_B, Characters.CHAR_C, Characters.CHAR_D, Characters.CHAR_E, Characters.CHAR_F, Characters.CHAR_G,
		Characters.CHAR_H, Characters.CHAR_I, Characters.CHAR_J, Characters.CHAR_K, Characters.CHAR_L, Characters.CHAR_M, Characters.CHAR_N, Characters.CHAR_O, Characters.CHAR_P, Characters.CHAR_Q, Characters.CHAR_R, Characters.CHAR_S, Characters.CHAR_T, Characters.CHAR_U, Characters.CHAR_V,
	...
	$str111$The__A_reader_macro_is_not_implem = SubLObjectFactory.makeString("The ~A reader macro is not implemented.");
	$str112$The__A_reader_macro_does_not_take = SubLObjectFactory.makeString("The ~A reader macro does not take an argument.");
	$str113$Illegal_argument__D_for_the__A_re = SubLObjectFactory.makeString("Illegal argument ~D for the ~A reader macro.");
	$sym114$CLET = SubLObjectFactory.makeSymbol("CLET");
	$list115 = ConsesLow.list(ConsesLow.list(SubLObjectFactory.makeSymbol("*READ-BASE*"), CommonSymbols.TEN_INTEGER), ConsesLow.list(SubLObjectFactory.makeSymbol("*READ-DEFAULT-FLOAT-FORMAT*"), ConsesLow.list(SubLObjectFactory.makeSymbol("QUOTE"), SubLObjectFactory.makeSymbol("SINGLE-FLOAT"))),
		ConsesLow.list(SubLObjectFactory.makeSymbol("*READ-EVAL*"), CommonSymbols.T), ConsesLow.list(SubLObjectFactory.makeSymbol("*READ-SUPPRESS*"), SubLNil.NIL), ConsesLow.list(SubLObjectFactory.makeSymbol("*READTABLE*"), SubLObjectFactory.makeSymbol("*STANDARD-READTABLE*")));
	...
	$list121 = ConsesLow.list(SubLObjectFactory.makeSymbol("POINTER"), SubLObjectFactory.makeSymbol("BUFFER"), SubLObjectFactory.makeSymbol("ESCAPES"));
	$list122 = ConsesLow.list(SubLObjectFactory.makeKeyword("POINTER"), SubLObjectFactory.makeKeyword("BUFFER"), SubLObjectFactory.makeKeyword("ESCAPES"));
	$list123 = ConsesLow.list(SubLObjectFactory.makeSymbol("READ-TOKEN-POINTER"), SubLObjectFactory.makeSymbol("READ-TOKEN-BUFFER"), SubLObjectFactory.makeSymbol("READ-TOKEN-ESCAPES"));
	$list124 = ConsesLow.list(SubLObjectFactory.makeSymbol("_CSETF-READ-TOKEN-POINTER"), SubLObjectFactory.makeSymbol("_CSETF-READ-TOKEN-BUFFER"), SubLObjectFactory.makeSymbol("_CSETF-READ-TOKEN-ESCAPES"));
	$sym125$READ_TOKEN_PRINT_FUNCTION_TRAMPOLINE = SubLObjectFactory.makeSymbol("READ-TOKEN-PRINT-FUNCTION-TRAMPOLINE");
	$list126 = ConsesLow.list(SubLObjectFactory.makeSymbol("OPTIMIZE-FUNCALL"), SubLObjectFactory.makeSymbol("READ-TOKEN-P"));
	...
	$sym137$VISIT_DEFSTRUCT_OBJECT_READ_TOKEN_METHOD = SubLObjectFactory.makeSymbol("VISIT-DEFSTRUCT-OBJECT-READ-TOKEN-METHOD");
	$list138 = ConsesLow.list(SubLObjectFactory.makeSymbol("TOKEN"), SubLObjectFactory.makeSymbol("&BODY"), SubLObjectFactory.makeSymbol("BODY"));
	$list139 = ConsesLow.list(ConsesLow.list(SubLObjectFactory.makeSymbol("MAKE-READ-TOKEN")));
	$list140 = ConsesLow.list(SubLObjectFactory.makeSymbol("CSETF"), ConsesLow.list(SubLObjectFactory.makeSymbol("READ-TOKEN-POINTER"), SubLObjectFactory.makeSymbol("TOKEN")), CommonSymbols.ZERO_INTEGER);
	$list141 = ConsesLow.list(SubLObjectFactory.makeSymbol("CSETF"), ConsesLow.list(SubLObjectFactory.makeSymbol("READ-TOKEN-BUFFER"), SubLObjectFactory.makeSymbol("TOKEN")), ConsesLow.list(SubLObjectFactory.makeSymbol("MAKE-STRING"), SubLObjectFactory.makeSymbol("*INITIAL-TOKEN-SIZE*")));
	$list142 = ConsesLow.list(SubLObjectFactory.makeSymbol("CSETF"), ConsesLow.list(SubLObjectFactory.makeSymbol("READ-TOKEN-ESCAPES"), SubLObjectFactory.makeSymbol("TOKEN")), ConsesLow.list(SubLObjectFactory.makeSymbol("QUOTE"), SubLNil.NIL));
	$str143$_A_is_an_invalid_character_ = SubLObjectFactory.makeString("~A is an invalid character.");
	$str144$Fell_through_character_syntax_on_ = SubLObjectFactory.makeString("Fell through character syntax on ~A.");
	...
	$float$0_0 = SubLObjectFactory.makeDouble(0.0);
	...
	$str171$The__S_reader_macro_does_not_take = SubLObjectFactory.makeString("The ~S reader macro does not take an argument.");
	$str172$The__S_reader_macro_expected_a_st = SubLObjectFactory.makeString("The ~S reader macro expected a string.");
	$str173$The__S_reader_macro_could_not_int = SubLObjectFactory.makeString("The ~S reader macro could not interpret ~S as a GUID string.");
	...
	$str182$Ignoring_unmatched_close_parenthe = SubLObjectFactory.makeString("Ignoring unmatched close parenthesis.");
	...
	$list200 = ConsesLow.list(SubLObjectFactory.makeSymbol("APPEND"), SubLObjectFactory.makeSymbol("LIST"), SubLObjectFactory.makeSymbol("LIST*"), SubLObjectFactory.makeSymbol("NCONC"));
	...
	$list204 = ConsesLow.list(ConsesLow.cons(SubLObjectFactory.makeSymbol("CONS"), SubLObjectFactory.makeSymbol("BQ-CONS")), ConsesLow.cons(SubLObjectFactory.makeSymbol("LIST"), SubLObjectFactory.makeSymbol("BQ-LIST")),
		ConsesLow.cons(SubLObjectFactory.makeSymbol("APPEND"), SubLObjectFactory.makeSymbol("BQ-APPEND")), ConsesLow.cons(SubLObjectFactory.makeSymbol("NCONC"), SubLObjectFactory.makeSymbol("BQ-NCONC")));
	...
	$str210$Attempt_the_use_the__A_reader_mac = SubLObjectFactory.makeString("Attempt the use the ~A reader macro when ~S is ~S.");
	$sym211$_READ_EVAL_ = SubLObjectFactory.makeSymbol("*READ-EVAL*");
	$int$36 = SubLObjectFactory.makeInteger(36);
	$str213$The_form_after__A_reader_macro_wa = SubLObjectFactory.makeString("The form after ~A reader macro was not a list.");
