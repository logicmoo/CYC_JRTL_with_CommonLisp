...
public class SubLFiles {
	...
	public static void declareFunction(SubLFile file, String methodName, String functionName, int requiredArgCount,
			int optionalArgCount, boolean allowsRest) {
		declareFunction(file.getClass(), null, methodName, functionName, requiredArgCount, optionalArgCount,
				allowsRest);
	}

	public static void declareFunction(Class file, String className, String methodName, String functionName,
		...
		try {
			...
			if (functionSymbol.fboundp()) {
				...
				if (currentPackage == Lisp.PACKAGE_CYC || currentPackage == Lisp.PACKAGE_SUBLISP) {
					if (package1 != Lisp.PACKAGE_SUBLISP && package1 != Lisp.PACKAGE_CYC) {
						...
						if (meth == null) {
							SubLObjectFactory.makeCompiledFunction(className, methodName, parameterArray,
									SubLObject.class, functionSymbol, requiredArgCount, optionalArgCount, allowsRest);
						} else {
							SubLObjectFactory.makeCompiledFunction(meth, functionSymbol, requiredArgCount,
			...
			// SubLFunction func =
			if (meth == null) {
				SubLObjectFactory.makeCompiledFunction(className, methodName, parameterArray, SubLObject.class,
						functionSymbol, requiredArgCount, optionalArgCount, allowsRest);
			} else {
				SubLObjectFactory.makeCompiledFunction(meth, functionSymbol, requiredArgCount, optionalArgCount,
						allowsRest);
			}
		} catch (Exception e) {
	...
	public static SubLSymbol defProcessVar(SubLFile file, Object variableName, SubLObject initialValue) {
		 ...
		 * SubL defvar : mutable dynamic global, not re-initialized on re-evaluation
		 */
		return declareSymbol(variableName, initialValue, VariableAccessMode.DYNAMIC, false).toLispObject()
				.setProcessScope(true);
	}
