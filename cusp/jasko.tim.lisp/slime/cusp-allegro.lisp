...
;;;
;;; cusp-allegro.lisp --- Allegro CL specific code for SLIME. 
;;;
;;; Created 2003
;;;
...
(import-from :excl *gray-stream-symbols* :cusp-backend)
...
(defun find-topframe ()
  (let ((magic-symbol (intern (symbol-name :cusp-debugger-hook)
                              (find-package :cusp)))
...
(defimplementation frame-restartable-p (frame)
  (handler-case (debugger:frame-retryable-p frame)
    (serious-condition (c)
      (funcall (read-from-string "cusp::background-message")
