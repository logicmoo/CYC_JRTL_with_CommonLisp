...
;;;
;;; cusp-ecl.lisp --- SLIME backend for ECL.
;;;
...
(if (find-package :gray)
  (import-from :gray *gray-stream-symbols* :cusp-backend)
  (import-from :ext *gray-stream-symbols* :cusp-backend))

(cusp-backend::import-cusp-mop-symbols :clos
 '(:eql-specializer
...
(defun in-cusp-package-p (x)
  (and
   (symbolp x)
   (member (symbol-package x)
           (list #.(find-package :cusp)
                 #.(find-package :cusp-backend)
                 #.(ignore-errors (find-package :cusp-mop))
                 #.(ignore-errors (find-package :cusp-loader))))
   t))

(defun is-cusp-source-p (name)
  (setf name (pathname name))
  (pathname-match-p
   name
   (make-pathname :defaults cusp-loader::*source-directory*
...
(defun is-ignorable-fun-p (x)
  (or
   (in-cusp-package-p (frame-name x))
   (multiple-value-bind (file position)
       (ignore-errors (si::bc-file (car x)))
     (declare (ignore position))
     (if file (is-cusp-source-p file)))))

#+#.(cusp-backend::with-symbol '+ECL-VERSION-NUMBER+ 'EXT)
