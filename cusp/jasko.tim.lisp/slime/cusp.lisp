...
;;;; cusp.lisp
;;;
;;; This file defines the "Cusp" TCP server for Emacs to talk to. The
...
;;; so we have defined them in `cusp-backend.lisp' and implemented
;;; them separately for each Lisp implementation. These extensions are
;;; available to us here via the `CUSP-BACKEND' package.
...
(defvar *cusp-io-package*
  (let ((package (make-package :cusp-io-package :use '())))
...
(defslimefun state-stack ()
  "Return the value of *CUSP-STATE-STACK*."
  *cusp-state-stack*)
...
(define-condition cusp-error (error) 
  ((condition :initarg :condition :reader cusp-error.condition)
   (backtrace :initarg :backtrace :reader cusp-error.backtrace))
  (:report (lambda (condition stream)
             (princ (cusp-error.condition condition) stream))))

(defun make-cusp-error (condition)
  (make-condition 'cusp-error :condition condition 
...
(defvar *debug-on-cusp-error* nil
  "When non-nil invoke the system debugger on cusp internal errors.
Do not set this to T unless you want to debug cusp internals.")

(defmacro with-cusp-error-handler ((connection) &body body)
  (let ((var (gensym)))
  `(let ((,var ,connection))
     (handler-case 
         (handler-bind ((cusp-error 
                         (lambda (condition)
                           (when *debug-on-cusp-error*
                             (invoke-default-debugger condition)))))
           (progn ,@body))
       (cusp-error (condition)
         (close-connection ,var
                           (cusp-error.condition condition)
                           (cusp-error.backtrace condition)))))))
...
(defun call-with-connection (connection function)
  (if (eq *emacs-connection* connection)
      ...
      (let ((*emacs-connection* connection)
        ...
        (without-slime-interrupts
          (with-cusp-error-handler (*emacs-connection*)
            (with-io-redirection (*emacs-connection*)
              (call-with-debugger-hook #'cusp-debugger-hook function)))))))
...
(defvar *use-dedicated-output-stream* nil
  "When T cusp will attempt to create a second connection to
...
(defvar *listener-sockets* nil
  "A property list of lists containing style, socket pairs used 
   by cusp server listeners, keyed on socket port number. They 
...
(defun create-server (&key (port default-server-port)
                      ...
                      (dont-close *dont-close*) 
                      (coding-system *coding-system*))
  "Start a CUSP server on PORT running in STYLE.
         ...
         (initialize-multiprocessing
          (lambda ()
            (spawn (lambda () 
                   ...
                   :name (cat "Cusp " (princ-to-string port))))))
        ((:fd-handler :sigio)
...
(defun restart-server (&key (port default-server-port)
                       ...
                       (dont-close *dont-close*) 
                       (coding-system *coding-system*))
  "Stop the server listening on PORT, then start a new CUSP server 
...
(defun simple-announce-function (port)
  (when *cusp-debug-p*
    (format *log-output* "~&;; Cusp started at port: ~D.~%" port)
    (force-output *log-output*)))
...
(defun close-connection (c condition backtrace)
  ...
  (format *log-output* "~&;; cusp:close-connection: ~A~%" condition)
...
(defun read-loop (connection)
  (let ((input-stream (connection.socket-io connection))
    ...
    (with-cusp-error-handler (connection)
...
(defun prefixed-var (prefix variable-symbol)
  "(PREFIXED-VAR \"FOO\" '*BAR*) => CUSP::*FOO-BAR*"
  (let ((basename (subseq (symbol-name variable-symbol) 1)))
    (intern (format nil "*~A-~A" (string prefix) basename) :cusp)))
...
(defun call-with-thread-description (description thunk)
  ...
  ;; created by cusp are currently doing.
...
(defun decode-message (stream)
  ...
  (let ((*cusp-state-stack* (cons :read-next-form *cusp-state-stack*)))
    (handler-bind ((error (lambda (c) (error (make-cusp-error c)))))
...
(defvar *cusp-wire-protocol-version* nil
  "The version of the cusp/slime communication protocol.")
...
VERSION: the protocol version"
  ...
  `(:pid ,(getpid) :style ,(connection.communication-style *emacs-connection*)
    ...
    :package (:name ,(package-name *package*)
              :prompt ,(package-string-for-prompt *package*))
    :version ,*cusp-wire-protocol-version*))
...
Return the package or nil."
  ...
  (ignore-errors
    (find-package (let ((*package* *cusp-io-package*))
                    (read-from-string string)))))
...
(defun cusp-debugger-hook (condition hook)
  "Debugger function for binding *DEBUGGER-HOOK*."
  (declare (ignore hook))
  (handler-case
      (call-with-debugger-hook #'cusp-debugger-hook
...
(defun debug-in-emacs (condition)
  (let ((*cusp-debugger-condition* condition)
        ...
        (*sldb-stepping-p* nil)
        (*cusp-state-stack* (cons :cusp-debugger-hook *cusp-state-stack*)))
    (force-user-output)
    (call-with-debugging-environment
     (lambda ()
...
(defun debugger-condition-for-emacs ()
  (list (safe-condition-message *cusp-debugger-condition*)
        (format nil "   [Condition of type ~S]"
                (type-of *cusp-debugger-condition*))
        (condition-extras *cusp-debugger-condition*)))

(defun format-restarts-for-emacs ()
  "Return a list of restarts for *cusp-debugger-condition* in a
...
(defslimefun sldb-break-with-default-debugger (dont-unwind)
  ...
  (cond (dont-unwind 
         (invoke-default-debugger *cusp-debugger-condition*))
        (t
...
(defmacro define-stepper-function (name backend-function-name)
  `(defslimefun ,name (frame)
     (cond ((sldb-stepper-condition-p *cusp-debugger-condition*)
...
(defslimefun cusp-require (modules &optional filename)
...
(defslimefun cusp-macroexpand-1 (string)
  (apply-macro-expander #'macroexpand-1 string))

(defslimefun cusp-macroexpand (string)
  (apply-macro-expander #'macroexpand string))

(defslimefun cusp-macroexpand-all (string)
  (apply-macro-expander #'macroexpand-all string))

(defslimefun cusp-compiler-macroexpand-1 (string)
  (apply-macro-expander #'compiler-macroexpand-1 string))

(defslimefun cusp-compiler-macroexpand (string)
  (apply-macro-expander #'compiler-macroexpand string))

(defslimefun cusp-format-string-expand (string)
...
(defslimefun cusp-toggle-trace (spec-string)
...
(defslimefun pprint-inspector-part (index)
  ...
  (with-buffer-syntax ()
    (cusp-pprint (list (inspector-nth-part index)))))
...
(defslimefun inspect-current-condition ()
  (with-buffer-syntax ()
    (reset-inspector)
    (inspect-object *cusp-debugger-condition*)))
...
(defslimefun start-cusp-server-in-thread (index port-file-name)
  "Interrupt the INDEXth thread and make it start a cusp server.
...
(defslimefun mop (type symbol-name)
  ...
  (let ((symbol (parse-symbol symbol-name *buffer-package*)))
    (ecase type
      (:subclasses
       (mop-helper symbol #'cusp-mop:class-direct-subclasses))
      (:superclasses 
       (mop-helper symbol #'cusp-mop:class-direct-superclasses)))))
...
(defun before-init (version load-path)
  (setq *cusp-wire-protocol-version* version)
  (setq *load-path* load-path)
  (cusp-backend::warn-unimplemented-interfaces))
