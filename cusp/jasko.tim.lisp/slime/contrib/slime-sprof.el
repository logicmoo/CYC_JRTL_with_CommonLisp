...
(slime-require :cusp-sprof)

(defvar slime-sprof-exclude-cusp nil
  "*Display cusp functions in the report.")
...
(slime-define-keys slime-sprof-browser-mode-map
  ...
  ("s" 'slime-sprof-toggle-cusp-exclusion)
...
(defun slime-sprof-start ()
  (interactive)
  (slime-eval `(cusp:cusp-sprof-start)))

(defun slime-sprof-stop ()
  (interactive)
  (slime-eval `(cusp:cusp-sprof-stop)))
...
(defun* slime-sprof-update (&optional (exclude-cusp slime-sprof-exclude-cusp))
  (slime-eval-async `(cusp:cusp-sprof-get-call-graph
                      :exclude-cusp ,exclude-cusp)
                    'slime-sprof-format))
...
(defun slime-sprof-toggle-cusp-exclusion ()
  (interactive)
  (setq slime-sprof-exclude-cusp
        (not slime-sprof-exclude-cusp))
  (slime-sprof-update))
...
(defun slime-sprof-browser-expand ()
  (lexical-let* ((buffer (current-buffer))
    ...
    (slime-eval-async `(cusp:cusp-sprof-expand-node ,index)
                      (lambda (data)
...
(defun slime-sprof-browser-disassemble-function ()
  ...
  (let ((index (or (get-text-property (point) 'profile-index)
    ...
    (when index
      (slime-eval-describe `(cusp:cusp-sprof-disassemble
                             ,index)))))
...
(defun slime-sprof-browser-view-source ()
  ...
  (let ((index (or (get-text-property (point) 'profile-index)
    ...
    (when index
      (slime-eval-async
       `(cusp:cusp-sprof-source-location ,index)
