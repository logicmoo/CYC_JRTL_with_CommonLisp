;;; cusp-fancy-inspector.lisp --- Fancy inspector for CLOS objects
;;
...
(defun method-specializers-for-inspect (method)
  ...
  (mapcar (lambda (spec)
            (typecase spec
              (cusp-mop:eql-specializer
               `(eql ,(cusp-mop:eql-specializer-object spec)))
              (t (cusp-mop:class-name spec))))
          (cusp-mop:method-specializers method)))
...
(defun method-for-inspect-value (method)
  ...
  (append (list (cusp-mop:generic-function-name
		 (cusp-mop:method-generic-function method)))
	  (cusp-mop:method-qualifiers method)
...
(defun specializer< (specializer1 specializer2)
  ...
  (let ((s1 specializer1) (s2 specializer2) )
    (cond ((typep s1 'cusp-mop:eql-specializer)
	   (not (typep s2 'cusp-mop:eql-specializer)))
	  (t
	   (flet ((cpl (class)
		    (and (cusp-mop:class-finalized-p class)
			 (cusp-mop:class-precedence-list class))))
	     (member s2 (cpl s1)))))))
...
`method-specializer<' is used for sorting."
  ...
  (labels ((method< (meth1 meth2)
             (loop for s1 in (cusp-mop:method-specializers meth1)
                   for s2 in (cusp-mop:method-specializers meth2)
                   do (cond ((specializer< s2 s1) (return nil))
                            ((specializer< s1 s2) (return t))))))
    (stable-sort (copy-seq (cusp-mop:generic-function-methods gf)) #'method<)))
...
(defgeneric all-slots-for-inspector (object)
  (:method ((object standard-object))
    (let* ((class           (class-of object))
           (direct-slots    (cusp-mop:class-direct-slots class))
           (effective-slots (sort (copy-seq (cusp-mop:class-slots class))
                                  #'string< :key #'cusp-mop:slot-definition-name))
      ...
      `("--------------------"
        ...
        (:action "[make unbound]"
                 ,(lambda ()
                    (do-checklist (idx checklist)
                      (cusp-mop:slot-makunbound-using-class
...
(defun make-slot-listing (checklist object class effective-slots direct-slots
  ...
  (flet ((padding-for (slot-name)
    ...
    (loop
      ...
      for direct-slot = (find (cusp-mop:slot-definition-name effective-slot)
                              direct-slots :key #'cusp-mop:slot-definition-name)
      for slot-name   = (inspector-princ
                         (cusp-mop:slot-definition-name effective-slot))
...
(defgeneric slot-value-for-inspector (class object slot)
  (:method (class object slot)
    (let ((boundp (cusp-mop:slot-boundp-using-class class object slot)))
      (if boundp
          `(:value ,(cusp-mop:slot-value-using-class class object slot))
          "#<unbound>"))))

(defun slot-home-class-using-class (slot class)
  (let ((slot-name (cusp-mop:slot-definition-name slot)))
    (loop for class in (reverse (cusp-mop:class-precedence-list class))
          thereis (and (member slot-name (cusp-mop:class-direct-slots class)
                               :key #'cusp-mop:slot-definition-name :test #'eq)
                       class))))
  ...
  (let* ((slot-name (cusp-mop:slot-definition-name slot))
    ...
    (when (and value-string (not (string= value-string "")))
      (with-simple-restart (abort "Abort setting slot ~S" slot-name)
        (setf (cusp-mop:slot-value-using-class class object slot)
...
(defmethod emacs-inspect ((gf standard-generic-function)) 
  (flet ((lv (label value) (label-value-line label value)))
    (append 
      (lv "Name" (cusp-mop:generic-function-name gf))
      (lv "Arguments" (cusp-mop:generic-function-lambda-list gf))
      (docstring-ispec "Documentation" gf t)
      (lv "Method class" (cusp-mop:generic-function-method-class gf))
      (lv "Method combination" 
	  (cusp-mop:generic-function-method-combination gf))
      `("Methods: " (:newline))
...
(defmethod emacs-inspect ((method standard-method))
	    ...
	    (:value ,(cusp-mop:method-generic-function method)
		    ,(inspector-princ
		      (cusp-mop:generic-function-name
		       (cusp-mop:method-generic-function method))))
            (:newline)
	    ,@(docstring-ispec "Documentation" method t)
            "Lambda List: " (:value ,(cusp-mop:method-lambda-list method))
            (:newline)
            "Specializers: " (:value ,(cusp-mop:method-specializers method)
                                     ,(inspector-princ (method-specializers-for-inspect method)))
            (:newline)
            "Qualifiers: " (:value ,(cusp-mop:method-qualifiers method))
            (:newline)
            "Method function: " (:value ,(cusp-mop:method-function method))
            (:newline)
...
(defmethod emacs-inspect ((class standard-class))
          `("Name: " (:value ,(class-name class))
            (:newline)
            "Super classes: "
            ,@(common-seperated-spec (cusp-mop:class-direct-superclasses class))
            (:newline)
            "Direct Slots: "
            ,@(common-seperated-spec
               (cusp-mop:class-direct-slots class)
               (lambda (slot)
                 `(:value ,slot ,(inspector-princ (cusp-mop:slot-definition-name slot)))))
            (:newline)
            "Effective Slots: "
            ,@(if (cusp-mop:class-finalized-p class)
                  (common-seperated-spec
                   (cusp-mop:class-slots class)
                   (lambda (slot)
            ...
            "Sub classes: "
            ,@(common-seperated-spec (cusp-mop:class-direct-subclasses class)
                                     (lambda (sub)
                                       `(:value ,sub ,(inspector-princ (class-name sub)))))
            (:newline)
            "Precedence List: "
            ,@(if (cusp-mop:class-finalized-p class)
                  (common-seperated-spec (cusp-mop:class-precedence-list class)
                                         (lambda (class)
                                           `(:value ,class ,(inspector-princ (class-name class)))))
                  '("#<N/A (class not finalized)>"))
            (:newline)
            ,@(when (cusp-mop:specializer-direct-methods class)
               `("It is used as a direct specializer in the following methods:" (:newline)
                 ,@(loop
                      for method in (sort (copy-seq (cusp-mop:specializer-direct-methods class))
                                          #'string< :key (lambda (x)
                                                           (symbol-name
                                                            (let ((name (cusp-mop::generic-function-name
                                                                         (cusp-mop::method-generic-function x))))
                      ...
                      collect '(:newline))))
            "Prototype: " ,(if (cusp-mop:class-finalized-p class)
                               `(:value ,(cusp-mop:class-prototype class))
                               '"#<N/A (class not finalized)>")
            (:newline)
            ,@(all-slots-for-inspector class)))

(defmethod emacs-inspect ((slot cusp-mop:standard-slot-definition))
          `("Name: " (:value ,(cusp-mop:slot-definition-name slot))
            (:newline)
            ,@(when (cusp-mop:slot-definition-documentation slot)
                `("Documentation:"  (:newline)
                  (:value ,(cusp-mop:slot-definition-documentation slot))
                  (:newline)))
            "Init args: " (:value ,(cusp-mop:slot-definition-initargs slot)) (:newline)
            "Init form: "  ,(if (cusp-mop:slot-definition-initfunction slot)
                             `(:value ,(cusp-mop:slot-definition-initform slot))
                             "#<unspecified>") (:newline)
            "Init function: " (:value ,(cusp-mop:slot-definition-initfunction slot))            
            (:newline)
