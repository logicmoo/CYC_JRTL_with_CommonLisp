;;; cusp-mit-scheme.scm --- SLIME server for MIT Scheme
;;
	  ...
	  `(begin
	    ...
	    (eval 
	     '(construct-normal-package-from-description
	       (make-package-description '(cusp) '(()) 
					 (vector) (vector) (vector) false))
	     (->environment '(package)))
	    (load ,(expand-file-name 
		    ".../contrib/cusp-mit-scheme.scm" ; <-- insert your path
		    slime-path)
		  (->environment '(cusp)))
	    (eval '(start-cusp ,file) (->environment '(cusp))))))
...
(setq slime-find-buffer-package-function 'find-mit-scheme-package)
   ...
   libraries, then creates a package "(cusp)", and loads this file
...
(define cusp-env (->environment (cusp-package)))
...
(define (cusp:quit-lisp _)
  (%exit))


;;;; Evaluation

(define (cusp:listener-eval socket string)
...
(define (cusp:interactive-eval socket string)
...
(define (cusp:pprint-eval _ string)
  (pprint-to-string (eval (read-from-string string) 
			  (user-env *buffer-package*))))

(define (cusp:interactive-eval-region socket string)
  (format-values (eval-region string socket)))

(define (cusp:set-package _ package)
...
(define (cusp:create-repl socket . _)
...
(define (cusp:compile-string-for-emacs _ string . x)
  (call-compiler
   (lambda ()
...
(define (cusp:compiler-notes-for-emacs _) nil)

(define (cusp:compile-file-for-emacs socket file load?)
  (call-compiler
   (lambda ()
     (with-output-to-repl socket
...
(define (cusp:load-file socket file)
  (with-output-to-repl socket
    (lambda () (load file (user-env *buffer-package*)))))

(define (cusp:disassemble-symbol _ string)
...
(define (cusp:cusp-macroexpand-all _ string) 
  (with-output-to-string
      (lambda ()
	(pp (syntax (read-from-string string)
		    (user-env *buffer-package*))))))
(define cusp:cusp-macroexpand-1 cusp:cusp-macroexpand-all)
(define cusp:cusp-macroexpand cusp:cusp-macroexpand-all)


;;; Arglist

(define (cusp:operator-arglist socket name pack)
  (let ((v (ignore-errors
	    (lambda ()
	      (with-output-to-string 
		(lambda ()
...
(define (cusp:buffer-first-change . _) nil)
(define (cusp:filename-to-modulename . _) nil)

;; M-. is beyond my capabilities.
(define (cusp:find-definitions-for-emacs . _) nil)
...
(define (cusp:throw-to-toplevel . _)
  (invoke-restart *top-level-restart*))

(define (cusp:sldb-abort . _)
  (abort (sldb-state.restarts *sldb-state*)))

(define (cusp:sldb-continue . _)
  (continue (sldb-state.restarts *sldb-state*)))

(define (cusp:invoke-nth-restart-for-emacs _ _sldb-level n)
  (invoke-restart (list-ref (sldb-state.restarts *sldb-state*) n)))

(define (cusp:debugger-info-for-emacs _ from to)
  (sldb-info *sldb-state* from to))

(define (cusp:backtrace _ from to)
...
(define (cusp:frame-locals-and-catch-tags _ frame)
...
(define (cusp:inspect-frame-var _ frame var)
  (reset-inspector)
...
(define (cusp:simple-completions _ string package)
...
(define (cusp:apropos-list-for-emacs _ name #!optional 
...
(define (cusp:list-all-package-names . _)
...
(define (cusp:init-inspector _ string)
  (reset-inspector)
