...
profiling before running the benchmark."
  ...
  (sit-for 0)
  (slime-repl-send-string "(cusp:io-speed-test 4000 1)")
...
(defun slime-repl-eval-string (string)
  (slime-rex ()
      ((list 'cusp:listener-eval string) (slime-lisp-package))
    ((:ok result)
  ...
  (with-current-buffer (slime-output-buffer)
    (let ((previouse-point (- (point) slime-repl-input-start-mark)))
      (destructuring-bind (name prompt-string)
          (slime-repl-shortcut-eval `(cusp:set-package ,package))
...
(defslime-repl-shortcut nil ("pwd")
  (:handler (lambda () 
              (interactive)
              (let ((dir (slime-eval `(cusp:default-directory))))
  ...
  (:handler (lambda (directory)
              (interactive
               (list (read-directory-name
                      "Push directory: "
                      (slime-eval '(cusp:default-directory))
                      nil nil "")))
              (push (slime-eval '(cusp:default-directory))
                    slime-repl-directory-stack)
  ...
  (let ((buffer (get-buffer-create "*SLIME Trace Output*")))
    (with-current-buffer buffer
      (let ((marker (copy-marker (buffer-size)))
        ...
        (slime-eval `(cusp:redirect-trace-output ,target))))
...
(defun slime-inspector-copy-down-to-repl (number)
   ...
   (slime-repl-send-string (format "%s" `(cusp:inspector-nth-part ,number)))
   (slime-repl))
  ...
  (let ((dir (expand-file-name directory)))
    (message "default-directory: %s"
             (slime-from-lisp-filename
              (slime-repl-shortcut-eval `(cusp:set-default-directory
...
(defun slime-sync-package-and-default-directory ()
  ...
  (let* ((package (slime-current-package))
         (exists-p (or (null package)
                       (slime-eval `(cl:packagep (cusp::guess-package ,package)))))
...
(defun slime-repl-connected-hook-function ()
  (destructuring-bind (package prompt) 
	...
	(slime-eval '(cusp:create-repl nil)))
...
canonicalized to \"CUSP\"---we do actually switch to that
package, though."
...
{}3
CUSP> *[]")
      ("(princ 10)" "CUSP> (princ 10)
{10
}10
CUSP> *[]")
      ("(princ 10)(princ 20)" "CUSP> (princ 10)(princ 20)
{1020
}20
CUSP> *[]")
      ("(dotimes (i 10 77) (princ i) (terpri))" 
       "CUSP> (dotimes (i 10 77) (princ i) (terpri))
{0
1
2
3
...
CUSP> *[]")
      ("(progn (princ 10) (force-output) (abort))" 
       "CUSP> (progn (princ 10) (force-output) (abort))
{10}; Evaluation aborted.
CUSP> *[]")
      ("(progn (princ 10) (abort))" 
       ;; output can be flushed after aborting
       "CUSP> (progn (princ 10) (abort))
{10}; Evaluation aborted.
CUSP> *[]")
      ("(if (fresh-line) 1 0)"
       "CUSP> (if (fresh-line) 1 0)
{
}1
CUSP> *[]")
      ("(values 1 2 3)" "CUSP> (values 1 2 3)
{}1
2
3
CUSP> *[]")
...
(def-slime-test repl-read
    ...
    "Test simple commands in the minibuffer."
    '(("(read-line)" "foo" "CUSP> (values (read-line))
foo
\"foo\"
CUSP> ")
      ("(read-char)" "1" "CUSP> (values (read-char))
1
#\\1
CUSP> ")
      ("(read)" "(+ 2 3
...
(def-slime-test repl-read-lines
    ...
    '(("(list (read-line) (read-line) (read-line))" 
       ("a" "b" "c")
       "CUSP> (list (read-line) (read-line) (read-line))
a
b
c
\(\"a\" \"b\" \"c\")
...
In particular, input inserted while waiting for a result."
    '(("(sleep 0.1)" "foo*" "CUSP> (sleep 0.1)
{}NIL
CUSP> [foo*]")
      ("(sleep 0.1)" "*foo" "CUSP> (sleep 0.1)
{}NIL
CUSP> [*foo]")
      ("(progn (sleep 0.1) (abort))" "*foo" "CUSP> (progn (sleep 0.1) (abort))
{}; Evaluation aborted.
CUSP> [*foo]"))
  (with-canonicalized-slime-repl-buffer
    (insert command)
