...
(defun filter-cusp-nodes (nodes)
  (let ((cusp-package (find-package :cusp)))
...
(defun serialize-call-graph (&key exclude-cusp)
  (let ((nodes (sb-sprof::call-graph-flat-nodes *call-graph*)))
    (when exclude-cusp
      (setf nodes (filter-cusp-nodes nodes)))
...
(defslimefun cusp-sprof-get-call-graph (&key exclude-cusp)
  (setf *call-graph* (sb-sprof:report :type nil))
  (serialize-call-graph :exclude-cusp exclude-cusp))

(defslimefun cusp-sprof-expand-node (index)
...
(defslimefun cusp-sprof-disassemble (index)
  (let* ((node (gethash index *number-nodes*))
...
(defslimefun cusp-sprof-source-location (index)
  (let* ((node (gethash index *number-nodes*))
         (debug-info (sb-sprof::node-debug-info node)))
...
(defslimefun cusp-sprof-start ()
  (sb-sprof:start-profiling))

(defslimefun cusp-sprof-stop ()
