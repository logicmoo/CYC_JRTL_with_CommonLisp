...
(defun slime-arglist (name)
  ...
  (let ((arglist (slime-eval `(cusp:arglist-for-echo-area '((,name))))))
    (if arglist
...
(def-slime-test autodoc.1
    ...
    ""
    '(("(cusp::emacs-connected*HERE*"    "(emacs-connected)")
      ("(cusp::create-socket*HERE*"      "(create-socket host port)")
      ("(cusp::create-socket *HERE*"     "(create-socket ===> host <=== port)")
      ("(cusp::create-socket foo *HERE*" "(create-socket host ===> port <===)")

      ("(cusp::symbol-status foo *HERE*" 
       "(symbol-status symbol &optional ===> (package (symbol-package symbol)) <===)")

      ("(apply 'cusp::eval-for-emacs*HERE*"
       "(apply ===> 'eval-for-emacs <=== &optional form buffer-package id &rest args)")

      ("(apply #'cusp::eval-for-emacs*HERE*"
       "(apply ===> #'eval-for-emacs <=== &optional form buffer-package id &rest args)")

      ("(apply 'cusp::eval-for-emacs foo *HERE*"
       "(apply 'eval-for-emacs &optional form ===> buffer-package <=== id &rest args)")
