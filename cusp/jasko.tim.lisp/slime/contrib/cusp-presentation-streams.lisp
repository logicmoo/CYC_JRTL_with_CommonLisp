;;; cusp-presentation-streams.lisp --- Streams that allow attaching object identities
...
(cusp-require :cusp-presentations)
...
(eval-when (:load-toplevel)
  (handler-bind ((simple-error 
    ...
    (sb-ext:without-package-locks
      (cusp-backend::with-debootstrapping
	(load (make-pathname 
	       :name "sbcl-pprint-patch"
	       :type "lisp"
	       :directory (pathname-directory cusp-loader:*source-directory*)))))))
...
says that I am starting to print an object with this id. The second says I am finished"
  ...
  (let ((slime-stream-p 
    ...
    (if slime-stream-p
	(let* ((pid (cusp::save-presented-object object))
...
(let ((*warn-if-redefine-kernel* nil)
  ...
  (defun %print-unreadable-object (object stream type id thunk)
    ...
    (cusp::presenting-object object stream
  ...
  (defmethod print-object :around ((pathname pathname) stream)
    (cusp::presenting-object-if
	(cusp::can-present-readable-objects stream)
	pathname stream (call-next-method))))

#+openmcl
(ccl::def-load-pointers clear-presentations ()
  (cusp::clear-presentation-tables))
...
(progn
  (excl:def-fwrapper presenting-unreadable-wrapper (object stream type identity continuation) 
    (cusp::presenting-object object stream (excl:call-next-fwrapper)))
...
(provide :cusp-presentation-streams)
