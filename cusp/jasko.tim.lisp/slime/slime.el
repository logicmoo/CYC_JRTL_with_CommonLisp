...
;; Cusp. Cusp is distributed with slime.el and will automatically be
...
This is used to load the supporting Common Lisp library, Cusp.
...
(defcustom slime-backend "cusp-loader.lisp"
  "The name of the Lisp file that loads the Cusp server.
...
INIT is a function that should return a string to load and start
  Cusp. The function will be called with the PORT-FILENAME and ENCODING as
...
(defun slime (&optional command coding-system)
  "Start an inferior^_superior Lisp and connect to its Cusp server."
  (interactive)
...
(defun slime-connect (host port &optional coding-system)
  "Connect to a running Cusp server. Returns the connection."
  ...
  (message "Connecting to Cusp on port %S.." port)
...
;;    (slime-eval-async `(cusp:load-file ,filename)
...
;;;   2. Emacs tells Lisp (via stdio) to load and start Cusp.
;;;   3. Lisp recompiles the Cusp if needed.
;;;   4. Lisp starts the Cusp server and writes its TCP port to a temp file.
...
;;; fair while if Cusp needs recompilation.
...
;;; Starting the inferior Lisp and loading Cusp:
...
(defun slime-inferior-connect (process args)
  "Start a Cusp server in the inferior Lisp and connect."
  (slime-delete-cusp-port-file 'quiet)
  (slime-start-cusp-server process args)
...
(defun slime-start-cusp-server (process args)
  "Start a Cusp server on the inferior lisp."
  (destructuring-bind (&key coding-system init &allow-other-keys) args
    (with-current-buffer (process-buffer process)
      ...
      (let ((str (funcall init (slime-cusp-port-file) coding-system)))
  ...
  (let ((loader (if (file-name-absolute-p slime-backend)
    ...
    (format "%S\n\n"
            `(progn
               ...
               (funcall (read-from-string "cusp:start-server")
                        ,port-filename
                        :coding-system ,encoding)))))

(defun slime-cusp-port-file ()
  "Filename where the CUSP server writes its TCP port number."
...
(defun slime-delete-cusp-port-file (&optional quiet)
  (condition-case data
      (delete-file (slime-cusp-port-file))
    (error
     (ecase quiet
       ((nil) (signal (car data) (cdr data)))
       (quiet)
       (message (message "Unable to delete cusp port file %S"
                         (slime-cusp-port-file)))))))
...
(defun slime-read-cusp-port ()
  "Read the Cusp server port number from the `slime-cusp-port-file'."
  (save-excursion
    (with-temp-buffer
      (insert-file-contents (slime-cusp-port-file))
      (goto-char (point-min))
...
;;; functions in this file and in cusp.lisp.
...
(defun slime-init-connection-state (proc)
  ...
  (let ((slime-current-thread t))
    (slime-eval-async '(cusp:connection-info)
...
(defun slime-check-version (version conn)
  (or (equal version slime-protocol-version)
      ...
      (y-or-n-p 
       (format "Versions differ: %s (slime) vs. %s (cusp). Continue? "
...
(defun slime-connected-p ()
  "Return true if the Cusp connection is open."
    ...
    (or (run-hook-with-args-until-success 'slime-event-hooks event)
        (destructure-case event
          ...
          ((:read-from-minibuffer thread tag prompt initial-value)
           (slime-read-from-minibuffer-for-cusp thread tag prompt initial-value))
...
See `slime-compile-and-load-file' for further details."
  ...
  (let ((file (slime-to-lisp-filename (buffer-file-name))))
    (slime-eval-async
     `(cusp:compile-file-for-emacs ,file ,(if load t nil) 
...
(defun slime-recompile-locations (locations cont)
  (slime-eval-async 
   `(cusp:compile-multiple-strings-for-emacs
...
(defun slime-show-arglist ()
  (let ((op (slime-operator-before-point)))
    (when op 
      (slime-eval-async `(cusp:operator-arglist ,op ,(slime-current-package))
			(lambda (arglist)
			  (when arglist
...
(defun slime-simple-completions (prefix)
  (let ((slime-current-thread t))
    (slime-eval
     `(cusp:simple-completions ,prefix ',(slime-current-package)))))
...
(defun slime-find-definitions-rpc (name)
  (slime-eval `(cusp:find-definitions-for-emacs ,name)))
    ...
    (save-match-data
      (when (and (buffer-file-name)
        ...
        (let ((filename (slime-to-lisp-filename (buffer-file-name))))          
           (slime-eval-async `(cusp:buffer-first-change ,filename)))))))
...
(defun slime-read-from-minibuffer-for-cusp (thread tag prompt initial-value)
...
inserted in the current buffer."
  ...
  (cond ((not current-prefix-arg)
         (slime-eval-with-transcript `(cusp:interactive-eval ,string)))
        (t
...
(defun slime-eval-print (string)
  ...
  (slime-eval-async `(cusp:eval-and-grab-output ,string)
                    (lambda (result)
...
First make the variable unbound, then evaluate the entire form."
  ...
  (slime-eval-with-transcript `(cusp:re-evaluate-defvar ,form)))

(defun slime-pprint-eval-last-expression ()
  "Evaluate the form before point; pprint the value in a buffer."
  (interactive)
  (slime-eval-describe `(cusp:pprint-eval ,(slime-last-expression))))
...
in Lisp when committed with \\[slime-edit-value-commit]."
  (interactive 
   (list (slime-read-from-minibuffer "Edit value (evaluated): "
				     (slime-sexp-at-point))))
  (slime-eval-async `(cusp:value-for-editing ,form-string)
  ...
  (if (null slime-edit-form-string)
    ...
    (let ((value (buffer-substring-no-properties (point-min) (point-max))))
      (lexical-let ((buffer (current-buffer)))
        (slime-eval-async `(cusp:commit-edited-value ,slime-edit-form-string
                                                      ,value)
                          (lambda (_)
                            (with-current-buffer buffer
...
(defun slime-toggle-trace-fdefinition (&optional using-context-p)
  ...
  (let* ((spec (if using-context-p
    ...
    (message "%s" (slime-eval `(cusp:cusp-toggle-trace ,spec)))))
...
(defun slime-disassemble-symbol (symbol-name)
  ...
  (slime-eval-describe `(cusp:disassemble-symbol ,symbol-name)))
...
(defun slime-undefine-function (symbol-name)
  ...
  (slime-eval-async `(cusp:undefine-function ,symbol-name)
...
(defun slime-load-file (filename)
  ...
  (let ((lisp-filename (slime-to-lisp-filename (expand-file-name filename))))
    (slime-eval-with-transcript `(cusp:load-file ,lisp-filename))))
...
Return whatever cusp:set-default-directory returns."
  (let ((dir (expand-file-name directory)))
    (prog1 (slime-eval `(cusp:set-default-directory
...
(defun slime-pwd ()
  ...
  (interactive)
  (message "Directory %s" (slime-eval `(cusp:default-directory))))
...
(defun slime-toggle-profile-fdefinition (fname-string)
  ...
  (interactive (list (slime-read-from-minibuffer 
                      "(Un)Profile: "
                      (slime-symbol-at-point))))
  (slime-eval-async `(cusp:toggle-profile-fdefinition ,fname-string)
                    (lambda (r) (message "%s" r))))

(defun slime-unprofile-all ()
  "Unprofile all functions."
  (interactive)
  (slime-eval-async '(cusp:unprofile-all)
                    (lambda (r) (message "%s" r))))

(defun slime-profile-report ()
  "Print profile report."
  (interactive)
  (slime-eval-with-transcript '(cusp:profile-report)))

(defun slime-profile-reset ()
  "Reset profile counters."
  (interactive)
  (slime-eval-async (slime-eval `(cusp:profile-reset))
                    (lambda (r) (message "%s" r))))

(defun slime-profiled-functions ()
  "Return list of names of currently profiled functions."
  (interactive)
  (slime-eval-async `(cusp:profiled-functions)
                    (lambda (r) (message "%s" r))))
...
having names in the given package."
  ...
  (slime-eval-async `(cusp:profile-package ,package ,callers ,methods)
                    (lambda (r) (message "%s" r))))
...
If PACKAGE is NIL, then search in all packages."
  ...
  (let ((package (unless (equal package "") package)))
    (slime-eval-async `(cusp:profile-by-substring ,substring ,package)
                      (lambda (r) (message "%s" r)) )))

;;;; Documentation
...
(defun slime-describe-symbol (symbol-name)
  ...
  (slime-eval-describe `(cusp:describe-symbol ,symbol-name)))
...
(defun slime-documentation (symbol-name)
  ...
  (when (not symbol-name)
    (error "No symbol given"))
  (slime-eval-describe 
   `(cusp:documentation-symbol ,symbol-name)))

(defun slime-describe-function (symbol-name)
  (interactive (list (slime-read-symbol-name "Describe symbol: ")))
  (when (not symbol-name)
    (error "No symbol given"))
  (slime-eval-describe `(cusp:describe-function ,symbol-name)))
...
arg, you're interactively asked for parameters of the search."
  ...
  (let ((buffer-package (or package (slime-current-package))))
    (slime-eval-async
     `(cusp:apropos-list-for-emacs ,string ,only-external-p
...
(defun slime-call-describer (arg)
  (let* ((pos (if (markerp arg) arg (point)))
    ...
    (slime-eval-describe `(cusp:describe-definition-for-emacs ,item ,type))))
...
CL:MACROEXPAND."
  ...
  (slime-eval-macroexpand
   (if repeatedly 'cusp:cusp-macroexpand 'cusp:cusp-macroexpand-1)))
...
(defun slime-macroexpand-all ()
  ...
  (interactive)
  (slime-eval-macroexpand 'cusp:cusp-macroexpand-all))

(defun slime-macroexpand-all-inplace ()
  "Display the recursively macro expanded sexp at point."
  (interactive)
  (slime-eval-macroexpand-inplace 'cusp:cusp-macroexpand-all))

(defun slime-compiler-macroexpand ()
  "Display the compiler-macro expansion of sexp at point."
  (interactive)
  (slime-eval-macroexpand 'cusp:cusp-compiler-macroexpand))

(defun slime-compiler-macroexpand-inplace ()
  "Display the compiler-macro expansion of sexp at point."
  (interactive)
  (slime-eval-macroexpand-inplace 'cusp:cusp-compiler-macroexpand))

(defun slime-compiler-macroexpand-1 ()
  "Display the compiler-macro expansion of sexp at point."
  (interactive)
  (slime-eval-macroexpand 'cusp:cusp-compiler-macroexpand-1))

(defun slime-compiler-macroexpand-1-inplace ()
  "Display the compiler-macro expansion of sexp at point."
  (interactive)
  (slime-eval-macroexpand-inplace 'cusp:cusp-compiler-macroexpand-1))

(defun slime-format-string-expand ()
  "Format the format-string at point, and display its expansion."
  (interactive)
  (slime-eval-macroexpand 'cusp:cusp-format-string-expand
...
(defun slime-quit-lisp-internal (connection sentinel kill)
  (let ((slime-dispatching-connection connection))
    (slime-eval-async '(cusp:quit-lisp))
...
(defun sldb-reinitialize (thread level)
  (slime-rex (thread level)
      ('(cusp:debugger-info-for-emacs 0 10)
       nil thread)
    ((:ok result)
...
Regexp heuristics are used to avoid showing CUSP-internal frames."
  (let* ((case-fold-search t)
         (rx "^\\([() ]\\|lambda\\)*cusp\\>"))
  ...
  (let ((inhibit-point-motion-hooks t)
    ...
    (when prev
      (let* ((count 40)
             (from (1+ prev))
             (to (+ from count))
             (frames (slime-eval `(cusp:backtrace ,from ,to)))
...
(defun sldb-fetch-all-frames ()
  (let ((inhibit-read-only t)
    ...
    (let ((last (sldb-frame-number-at-point)))
      ...
      (save-excursion
        (sldb-insert-frames (slime-eval `(cusp:backtrace ,(1+ last) nil))
                            nil)))))
...
(defun sldb-show-frame-source (frame-number)
  (slime-eval-async
   `(cusp:frame-source-location ,frame-number)
  ...
  (let* ((frame (get-text-property (point) 'frame))
    ...
    (destructuring-bind (start end) (sldb-frame-region)
      (list* start end frame 
             (slime-eval `(cusp:frame-locals-and-catch-tags ,num))))))
...
(defun sldb-disassemble ()
  ...
  (let ((frame (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:sldb-disassemble ,frame)
                      (lambda (result)
...
(defun sldb-eval-in-frame (string)
  ...
  (let* ((number (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:eval-string-in-frame ,string ,number)
                      (if current-prefix-arg
                          'slime-write-string
...
(defun sldb-pprint-eval-in-frame (string)
  ...
  (interactive (list (slime-read-from-minibuffer "Eval in frame: ")))
  (let* ((number (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:pprint-eval-string-in-frame ,string ,number)
		      (lambda (result)
			(slime-show-description result nil)))))
...
(defun sldb-inspect-in-frame (string)
  ...
  (interactive (list (slime-read-from-minibuffer 
                      ...
                      (slime-sexp-at-point))))
  (let ((number (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:inspect-in-frame ,string ,number)
                      'slime-open-inspector)))

(defun sldb-inspect-var ()
  (let ((frame (sldb-frame-number-at-point))
        (var (sldb-var-number-at-point)))
    (slime-eval-async `(cusp:inspect-frame-var ,frame ,var) 
                      'slime-open-inspector)))

(defun sldb-inspect-condition ()
  "Inspect the current debugger condition."
  (interactive)
  (slime-eval-async '(cusp:inspect-current-condition)
                    'slime-open-inspector))
...
(defun sldb-quit ()
  ...
  (slime-rex () ('(cusp:throw-to-toplevel))
...
(defun sldb-abort ()
  ...
  (interactive)
  (slime-eval-async '(cusp:sldb-abort)
...
use the restart at point."
  ...
  (let ((restart (or number (sldb-restart-at-point))))
    (slime-rex ()
        ((list 'cusp:invoke-nth-restart-for-emacs sldb-level restart))
...
(defun sldb-break-with-default-debugger (&optional dont-unwind)
  ...
  (interactive "P")
  (slime-rex ()
      ((list 'cusp:sldb-break-with-default-debugger 
             (not (not dont-unwind)))
...
(defun sldb-step ()
  ...
  (interactive)
  (let ((frame (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:sldb-step ,frame))))

(defun sldb-next ()
  "Select the \"continue\" restart and set a new break point."
  (interactive)
  (let ((frame (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:sldb-next ,frame))))

(defun sldb-out ()
  "Select the \"continue\" restart and set a new break point."
  (interactive)
  (let ((frame (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:sldb-out ,frame))))
...
The debugger is entered when the frame exits."
  (interactive)
  (let ((frame (sldb-frame-number-at-point)))
    (slime-eval-async `(cusp:sldb-break-on-return ,frame)
...
(defun sldb-break (name)
  ...
  (slime-eval-async `(cusp:sldb-break ,name)
                    (lambda (msg) (message "%s" msg))))
...
return that value, evaluated in the context of the frame."
  ...
  (let* ((number (sldb-frame-number-at-point)))
    (slime-rex ()
        ((list 'cusp:sldb-return-from-frame number string))
...
was called originally."
  ...
  (let* ((number (sldb-frame-number-at-point)))
    (slime-rex ()
        ((list 'cusp:restart-frame number))
      ((:ok value) (message "%s" value))
      ((:abort)))))
...
(defun sldb-recompile-frame-source (&optional raw-prefix-arg)
  (interactive "P")
  (slime-eval-async
   `(cusp:frame-source-location ,(sldb-frame-number-at-point))
...
(defun slime-quit-threads-buffer (&optional _)
  (slime-eval-async `(cusp:quit-thread-browser))
  (slime-popup-buffer-quit t))

(defun slime-update-threads-buffer ()
  (interactive)
  (let ((threads (cdr (slime-eval '(cusp:list-threads)))))
...
(defun slime-thread-kill ()
  ...
  (let ((id (get-text-property (point) 'thread-id)))
    (slime-eval `(cusp:kill-nth-thread ,id)))
  (call-interactively 'slime-list-threads))

(defun slime-thread-attach ()
  (interactive)
  (let ((id (get-text-property (point) 'thread-id))
        (file (slime-cusp-port-file)))
    (slime-eval-async `(cusp:start-cusp-server-in-thread ,id ,file)))
  (slime-read-port-and-connect nil nil))

(defun slime-thread-debug ()
  (interactive)
  (let ((id (get-text-property (point) 'thread-id)))
    (slime-eval-async `(cusp:debug-nth-thread ,id))))
...
(defun slime-inspect (string)
  ...
  (interactive 
   (list (slime-read-from-minibuffer "Inspect value (evaluated): "
				     (slime-sexp-at-point))))
  (slime-eval-async `(cusp:init-inspector ,string) 'slime-open-inspector))
...
3. If point is on a range-button fetch and insert the range."
  ...
  (let ((part-number (get-text-property (point) 'slime-part-number))
    ...
    (cond (part-number
           (slime-eval-async `(cusp:inspect-nth-part ,part-number)
                             opener)
           (push (slime-inspector-position) slime-inspector-mark-stack))
          (range-button
           (slime-inspector-fetch-more range-button))
          (action-number 
           (slime-eval-async `(cusp::inspector-call-nth-action ,action-number)
                             opener))
...
(defun slime-inspector-next ()
  (interactive)
  (let ((result (slime-eval `(cusp:inspector-next))))
    (cond (result 
	   (push (slime-inspector-position) slime-inspector-mark-stack)
...
(defun slime-inspector-quit ()
  (interactive)
  (slime-eval-async `(cusp:quit-inspector))
...
(defun slime-inspector-toggle-verbose ()
  (interactive)
  (slime-eval-async `(cusp:inspector-toggle-verbose)
                    (lexical-let ((point (slime-inspector-position)))
                      (lambda (parts)
                        (slime-open-inspector parts point)))))
...
(defun slime-update-indentation ()
  ...
  (interactive)
  (slime-eval-async '(cusp:update-indentation-information)))
...
(defun slime-load-contribs ()
  (let ((needed (remove-if (lambda (s) 
    ...
    (when needed
      (slime-eval-async `(cusp:cusp-require ',needed)
                        (lambda (new-modules)
...
(def-slime-test narrowing ()
  ...
  (let ((random-buffer-name (symbol-name (gensym)))
    ...
    (with-temp-buffer
      ...
      (let ((slime-buffer-package "CUSP")
            (symbol '*buffer-package*))
        (slime-edit-definition (symbol-name symbol))
        (slime-check ("Checking that we've got M-. into cusp.lisp. %S" symbol)
...
(def-slime-test find-definition
    ...
    "Find the definition of a function or macro in cusp.lisp."
    '(("start-server" "CUSP" "(defun start-server ")
      ("cusp::start-server" "CL-USER" "(defun start-server ")
      ("cusp:start-server" "CL-USER" "(defun start-server "))
  ...
  (let ((orig-buffer (current-buffer))
    ...
    (slime-edit-definition name)
    ;; Postconditions
    (slime-check ("Definition of `%S' is in cusp.lisp." name)
      (string= (file-name-nondirectory (buffer-file-name)) "cusp.lisp"))
...
(def-slime-test complete-symbol
    ...
    '(("cl:compile" (("cl:compile" "cl:compile-file" "cl:compile-file-pathname"
                     ...
                     "cl:compile"))
      ("cl:foobar" (nil ""))
      ("cusp::compile-file" (("cusp::compile-file" 
                               "cusp::compile-file-for-emacs"
                               "cusp::compile-file-if-needed"
                               "cusp::compile-file-pathname")
                              "cusp::compile-file"))
      ("cl:m-v-l" (nil "")))
...
Confirm that EXPECTED-ARGLIST is displayed."
    '(("cusp::operator-arglist" "(cusp::operator-arglist name package)")
      ("cusp::create-socket" "(cusp::create-socket host port)")
      ("cusp::emacs-connected" "(cusp::emacs-connected )")
      ("cusp::compile-string-for-emacs"
       "(cusp::compile-string-for-emacs string buffer position filename policy)")
      ("cusp::connection.socket-io"
       "(cusp::connection.socket-io \\(struct\\(ure\\)?\\|object\\|instance\\|x\\))")
      ("cl:lisp-implementation-type" "(cl:lisp-implementation-type )")
      ("cl:class-name" 
       "(cl:class-name \\(class\\|object\\|instance\\|structure\\))"))
  (let ((arglist (slime-eval `(cusp:operator-arglist ,function-name 
                                                      "cusp"))))
...
Confirm that SUBFORM is correctly located."
  ...
  (with-temp-buffer 
    (lisp-mode)
    (insert program)
    (setq slime-buffer-package ":cusp")
...
(def-slime-test (compile-file (:fails-for "allegro" "lispworks" "clisp"))
  ...
  (let ((filename "/tmp/slime-tmp-file.lisp"))
    ...
    (let ((cell (cons nil nil)))
      (slime-eval-async
       `(cusp:compile-file-for-emacs ,filename nil)
...
;;;;;; (abort) ...
;;{}CUSP> *[]" nil)
;;      ("(progn (princ 10) (finish-output) (abort))" "CUSP> 
;;;;;; (progn (princ 10) (finish-output) (abort)) ...
;;{10}
;;CUSP> *[]" t)
;;      ("(progn (princ 11) (finish-output) (abort))" "CUSP> 
...
;;    (setf (slime-lisp-package-prompt-string) "CUSP"))
;;  (kill-buffer (slime-output-buffer))
;;  (with-current-buffer (slime-output-buffer)
...
CONTINUES  ... how often the continue restart should be invoked"
  ...
  (when (and (eq (slime-communication-style) :spawn)
             (not (featurep 'slime-repl)))
    (slime-eval-async '(cusp::without-slime-interrupts
                        (cusp::receive)))
    (sit-for 0.2))
