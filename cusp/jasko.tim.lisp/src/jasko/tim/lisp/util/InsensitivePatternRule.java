...
/*
 * Java being very silly, it won't allow me to modify private variables (except perhaps 
 * through reflections, but that is slower?). Therefore, i copied the code from PatternRule class and then change what i want
 * because i'm that crazy! That's one reason i like lisp better...
 */
/*******************************************************************************
 ...
 */
public class InsensitivePatternRule implements IPredicateRule {
	...
	public boolean charEqualIgnoreCase (char char1,char char2) {
		return	Character.toLowerCase(char1)==Character.toLowerCase(char2);
	}
	public InsensitivePatternRule(String startSequence, String endSequence, IToken token, char escapeCharacter, boolean breaksOnEOL) {
	 ...
	 * @since 2.1
	 */
	public InsensitivePatternRule(String startSequence, String endSequence, IToken token, char escapeCharacter, boolean breaksOnEOL, boolean breaksOnEOF) {
	 ...
	 * @since 3.0
	 */
	public InsensitivePatternRule(String startSequence, String endSequence, IToken token, char escapeCharacter, boolean breaksOnEOL, boolean breaksOnEOF, boolean escapeContinuesLine) {
	...
	protected boolean endSequenceDetected(ICharacterScanner scanner) {
		...
		if (fLineDelimiters == null || originalDelimiters.length != count) {
			...
			while (count > 0 && fLineDelimiters[count-1].toString().equalsIgnoreCase(originalDelimiters[count-1].toString()))
				count--;
		}
		if (count != 0) {
		...
		while ((c= scanner.read()) != ICharacterScanner.EOF) {
			if (charEqualIgnoreCase((char)c,fEscapeCharacter)) {
				// Skip escaped character(s)
				if (fEscapeContinuesLine) {
					c= scanner.read();
					for (int i= 0; i < fSortedLineDelimiters.length; i++) {
						if (charEqualIgnoreCase((char)c,fSortedLineDelimiters[i][0]) && sequenceDetected(scanner, fSortedLineDelimiters[i], true))
							break;
					}
				} else
					scanner.read();

			} else if (fEndSequence.length > 0 && charEqualIgnoreCase((char)c,fEndSequence[0])) {
	...
	protected boolean sequenceDetected(ICharacterScanner scanner, char[] sequence, boolean eofAllowed) {
		for (int i= 1; i < sequence.length; i++) {
			...
			if (c == ICharacterScanner.EOF && eofAllowed) {
				return true;
			} else if (!charEqualIgnoreCase((char)c,sequence[i])) {
