...
public final class LispThread extends ALispObject {
	...
	private static class StackSegment implements org.armedbear.lisp.protocol.Inspectable {
		...
		public LispObject getParts() {
			...
			return result.push(new Symbol("INITIAL-SEGMENT-SIZE"))
					.push(LispInteger.makeInteger(LispThread.INITIAL_SEGMENT_SIZE)).push(new Symbol("SEGMENT-SIZE"))
	...
	public LispObject execute(LispObject function) {
		if (NO_STACK_FRAMES)
			return prevRet = function.execute();
	...
	public LispObject execute(LispObject function, LispObject arg) {
		if (NO_STACK_FRAMES)
			return prevRet = function.execute(arg);
	...
	public LispObject execute(LispObject function, LispObject first, LispObject second) {
		if (NO_STACK_FRAMES)
			return prevRet = function.execute(first, second);
	...
	public LispObject execute(LispObject function, LispObject first, LispObject second, LispObject third) {
		if (NO_STACK_FRAMES)
			return prevRet = function.execute(first, second, third);
	...
	public LispObject execute(LispObject function, LispObject first, LispObject second, LispObject third,
			LispObject fourth) {
		if (NO_STACK_FRAMES)
			return prevRet = function.execute(first, second, third, fourth);
	...
	public LispObject execute(LispObject function, LispObject first, LispObject second, LispObject third,
		...
		if (NO_STACK_FRAMES)
			return prevRet = function.execute(first, second, third, fourth, fifth);
	...
	public LispObject execute(LispObject function, LispObject first, LispObject second, LispObject third,
		...
		if (NO_STACK_FRAMES)
			return prevRet = function.execute(first, second, third, fourth, fifth, sixth);
	...
	private void printCurrentFrame(String why, boolean iSOut) {
	...
	private static final Primitive MAKE_THREAD = new Primitive("make-thread", PACKAGE_THREADS, true,
		...
		public LispObject execute(LispObject[] args) {
			...
			if (length > 1) {
				...
				if (length > 3)
					error(new WrongNumberOfArgumentsException(this, -1, 2)); // don't count the keyword itself as an
																				// argument
				if (args[1] == Keyword.NAME)
					name = args[2].STRING();
				else
					program_error("Unrecognized keyword argument " + args[1].princToString() + ".");
			}
